...
TL;DR: 提出了一种名为PuzzlePole的新型圆柱形基准标记，基于PuzzleBoard校准模式，能够从360度视角进行可靠识别和姿态估计。
...
Method: 利用PuzzleBoard模式的独特组合结构，开发圆柱形标记，提供高精度的定位和方向估计，同时对遮挡具有鲁棒性。

Result: PuzzlePole能够从全方位视角进行可靠识别和姿态估计，在定位和定向方面提供高精度。

Conclusion: PuzzlePole设计灵活，适用于各种自主系统场景，包括机器人导航、SLAM和可触摸界面。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的评估方法无法捕捉个人视觉偏好的多样性和复杂性，需要开发能够理解个体用户偏好的个性化评估系统。

Method: 采用自引导策略，在有限参考数据上进行推理构建丰富的用户上下文，通过动态生成用户条件评估维度和链式思维推理来评估图像质量。

Result: PIGReward在准确性和可解释性方面优于现有方法，建立了可扩展的个性化T2I评估和优化基础，并引入了PIGBench用户偏好基准。

Conclusion: PIGReward是实现个体对齐T2I生成的稳健步骤，为个性化评估和优化提供了推理基础。
...
TL;DR: Pistachio是一个通过生成式流水线构建的视频异常检测与理解基准，解决了现有基准在场景多样性、异常覆盖度和时间复杂性方面的不足。
...
Method: 利用视频生成模型构建可控流水线，包括场景条件化异常分配、多步骤故事情节生成和时间一致的长视频合成策略，生成41秒的连贯视频。

Result: Pistachio具有规模、多样性和复杂性，为现有方法带来新挑战，推动动态和多事件异常理解的研究。

Conclusion: Pistachio基准通过生成式方法有效消除了互联网收集数据集的偏差和限制，为视频异常检测与理解提供了更可靠的评估平台。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习视觉模型中估计训练点对测试预测的影响对于定位噪声数据至关重要，但传统影响函数面临计算成本高、训练非平稳性等挑战，现有方法存在离线计算滞后和置信度校准缺失的问题。

Method: SG-OIF框架将算法稳定性作为实时控制器，维护轻量级锚点IHVP，提出模块化曲率后端，使用稳定性引导的残差阈值、异常门控和置信度来调制每个样本的影响分数。

Result: 在CIFAR-10（20%不对称噪声）上，前1%预测样本准确率达到91.1%；在MNIST上AUPR得分达到99.8%，在多个数据集的各种损坏情况下均达到SOTA性能。

Conclusion: SG-OIF框架是用于在线影响估计的实用控制器，有效解决了深度学习视觉模型中影响估计的挑战。
...
TL;DR: SATA是一个通用的跟踪与分割框架，通过解耦混合专家机制和任务感知多目标跟踪管道，统一处理多种模态输入的跟踪与分割子任务，在18个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理跟踪和分割任务时使用专门架构或模态特定参数，限制了泛化能力和可扩展性。跨模态分布差异和跨任务特征表示差异阻碍了有效的知识共享，制约了通用模型的发展。
...
TL;DR: 本文提出了一种基于行列式比值矩阵(DRaM)的方法来解决3D-3D(EnP)和3D-2D正交投影(OnP)姿态估计问题，将现有方法统一到DRaM框架下，并展示了该方法可推广到N维欧几里得姿态估计问题。


<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的重要问题，但现有的3D-2D正交投影(OnP)问题缺乏类似SVD和最优四元数特征系统那样的闭式解。需要开发新的数学框架来统一解决EnP和OnP问题。

Method: 使用行列式比值矩阵(DRaM)方法求解无误差EnP和OnP问题的最小二乘系统，对于含噪声数据采用直接的旋转校正方案。将QR分解和Moore-Penrose伪逆变换等方法统一到DRaM框架下。

Result: 提出了EnP和OnP旋转估计问题的DRaM家族解决方案，比较了不同解决方案家族的行为。该方法可推广到所有类似的N维欧几里得姿态估计问题。

Conclusion: 这项工作不仅为3D和2D正交投影姿态估计问题提供了新的解决方案，还对这些问题类别提供了有价值的见解。DRaM方法具有可追溯到高斯时代的数学渊源，具有广泛的适用性。
...
TL;DR: LatentDreamer是一个从单张图像生成3D对象的新框架，使用预训练的变分自编码器将3D几何映射到潜在特征，通过粗几何、精几何和纹理的序列生成，在70秒内实现高保真3D生成。


<details>
  <summary>Details</summary>
Motivation: 当前自动3D生成方法难以同时实现快速、详细和高保真度的生成，需要一种能够高效生成高质量3D对象的解决方案。

Method: 使用预训练变分自编码器将3D几何映射到潜在特征，通过序列化管道依次生成粗几何、精几何和真实感纹理。

Result: 生成的3D对象与输入图像高度匹配，整个生成过程可在70秒内完成，在少量训练下与当代方法相比具有竞争力。

Conclusion: LatentDreamer通过潜在特征映射和序列化生成流程，实现了快速、高质量的3D对象生成，为图像到3D转换提供了有效解决方案。
...
TL;DR: 本文研究了模型容量与保留图像语义所需最少视觉token数量之间的关系，提出了正交过滤方法来自适应聚类冗余token，并发现了token与模型规模的缩放规律。


<details>
  <summary>Details</summary>
Motivation: 受最小描述长度原则启发，研究模型容量与图像语义保存所需最少视觉token之间的基本关系，重新解释视觉token作为视觉语义空间中的向量。

Method: 提出正交过滤方法，这是一个轻量级模块，能够自适应地将冗余token聚类成紧凑的正交基向量集。

Result: 通过广泛的ViT模型实验，揭示了token与模型规模的一致缩放规律：更大的模型需要显著更少的token来跨越视觉语义空间。

Conclusion: 贡献了一个视觉长上下文数据集，并验证了模型容量与语义token需求之间的内在关系，为高效视觉表示提供了理论基础。
...
TL;DR: 提出了一种无需任务特定微调的智能视觉定位框架GroundingAgent，通过集成预训练模型实现零样本视觉定位，在多个基准测试中达到65.1%的平均准确率。
...
Method: 采用结构化迭代推理机制，结合预训练开放词汇目标检测器、多模态大语言模型和大语言模型，通过联合语义和空间分析逐步优化候选区域。

Result: 在RefCOCO、RefCOCO+、RefCOCOg基准测试中实现65.1%的零样本定位准确率；仅使用原始查询文本时选择阶段准确率可达约90%，接近监督方法性能。

Conclusion: GroundingAgent展示了无需微调的视觉定位可行性，LLM推理能力在定位中起关键作用，同时提供良好的可解释性。
...
TL;DR: InfoPrune是一个基于信息论的视觉语言模型压缩框架，通过信息瓶颈原则实现自适应结构压缩，在保持任务相关语义的同时去除冗余依赖，达到3.2倍FLOP减少和1.8倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型规模不断增长，部署和效率面临严重挑战。现有压缩方法依赖启发式重要性度量或经验性剪枝规则，缺乏信息保留的理论保证。

Method: 基于信息瓶颈原则，引入基于熵的有效秩(eRank)量化注意力头贡献，使用KS距离测量原始与压缩结构差异。包含两种方案：基于信息损失目标的训练式注意力头剪枝，以及通过自适应低秩近似的免训练FFN压缩。
...
TL;DR: 提出一种基于3D面部标志的新颖眼睑角度(ELA)度量，用于可靠检测驾驶员疲劳，并利用ELA信号生成合成数据集以解决自然疲劳数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 可靠检测驾驶员疲劳对提高道路安全和支持高级驾驶辅助系统至关重要，但现有方法如EAR对相机角度变化敏感，且自然疲劳数据收集困难。

Method: 从3D面部标志推导出眼睑角度(ELA)作为眼睑运动的稳定几何描述，设计眨眼检测框架提取时间特征，并利用ELA信号在Blender 3D中生成可控的合成数据集。

Result: 在公共驾驶员监控数据集上的实验表明，ELA在视角变化下比EAR具有更低的方差，实现准确的眨眼检测，同时合成数据增强扩展了疲劳识别训练数据的多样性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解多智能体系统大多采用静态、不可学习的工具调用机制，限制了在时空复杂视频中发现多样化线索的能力，影响感知和推理的鲁棒性。

Method: 采用协作策略规划(CPP)范式，包含三个关键过程：策略生成（各智能体生成独特工具调用策略）、策略执行（顺序调用工具探索视频内容）、策略通信（执行过程中智能体间交互更新策略），并配备多智能体强化学习进行联合优化。

Result: 在8个基准测试的4个任务中达到SOTA性能，在LongVideoBench上比Gemini 2.5 pro高出3.6%，比GPT-4o高出15.6%。

Conclusion: VideoChat-M1通过动态协作策略规划和多智能体强化学习，显著提升了视频理解的性能，证明了协作策略规划在复杂视频理解任务中的有效性。
...
TL;DR: 提出了感知分类法（Perceptual Taxonomy）作为结构化场景理解框架，并创建了相应的基准测试来评估视觉语言模型在物理基础视觉推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准测试主要关注表面识别或图像-文本对齐，缺乏对结构化场景理解和物理推理能力的全面评估。

Method: 构建了包含3173个对象、84个细粒度属性的感知分类法基准，包含28033个模板问题和50个专家设计问题，涵盖对象描述、空间推理、属性匹配和分类推理四种类型。
...
<details>
  <summary>Details</summary>
Motivation: 现有在线HD地图构建方法依赖昂贵的3D地图标注进行训练，这限制了它们在不同驾驶环境中的泛化能力和可扩展性。

Method: 提出MapRF弱监督框架，包含基于地图预测的NeRF模块生成视图一致的3D几何和语义伪标签，通过自训练迭代优化地图网络，并使用Map-to-Ray匹配策略减轻错误累积。
...
<details>
  <summary>Details</summary>
Motivation: 视频已成为互联网上主要的沟通和创意媒介，需要可扩展、高质量的视频生产解决方案。现有模型在复杂视频理解任务上仍有局限，特别是细粒度的时空定位能力。

Method: Vidi2模型扩展了视频理解能力，实现端到端的时空定位，给定文本查询可识别对应时间戳和边界框。同时引入了新的基准VUE-STG和升级的VUE-TR-V2进行综合评估。

Result: Vidi2在VUE-TR-V2和VUE-STG基准测试中显著优于Gemini 3 Pro和GPT-5等专有系统，在视频问答基准上与类似规模的开源模型竞争性结果。

Conclusion: Vidi2展示了强大的视频理解能力，特别是在时空定位方面，为复杂编辑场景如情节理解、多视角切换等应用提供了技术基础。
...
Method: 利用结构化提示和微调，将检测、定位和量化整合到统一框架中，使用多模态大语言模型进行光伏评估。
...
TL;DR: 该论文提出了大规模分析制图遗产的方法和数据集，通过77万条地图记录和近10万张数字化图像，揭示了制图的空间焦点与政治动态的关联，开发了语义分割和目标检测技术分析地图内容，并发现了制图符号系统的演变规律。


<details>
  <summary>Details</summary>
Motivation: 尽管全球已有超过100万张地图被数字化，但现有自动化方法很少从文化视角和历史制图学角度分析地图作为语义符号系统和文化对象的价值。

Method: 收集来自38个数字目录的771,561条地图记录和99,715张数字化图像，进行数据标准化；开发语义分割技术和目标检测模型，使用标注数据和合成图像训练；分析制图符号和土地分类。

Result: 揭示了制图空间焦点与政治动态的关联（如大西洋海图绘制、三角贸易和殖民扩张）；发现地图是经过设计的图像，具有中心化和语义对称特征；编码6300万个符号和2500万个片段，揭示了制图符号的演变规律（如等高线取代晕渲法）。

Conclusion: 制图遗产的大规模分析揭示了地图作为文化对象的重要性，制图符号系统形成局部一致的系统，合作与传播在制图规范传播中起关键作用。
...
<details>
  <summary>Details</summary>
Motivation: 现有高斯溅射变形方法依赖代理（如笼子或网格），存在代理质量依赖和额外计算开销问题；而直接应用拉普拉斯变形技术又因缺乏明确结构而无法正确捕捉表面信息。

Method: 构建表面感知的溅射图，基于溅射间的相交关系而非中心距离定义邻域关系；引入高斯核自适应技术以在变形过程中保持表面结构。

Result: 在ShapeNet、Objaverse、Sketchfab和NeRF-Synthetic数据集的50个挑战性对象上，相比基于代理和无代理基线方法表现出优越性能。

Conclusion: SpLap方法能够实现更合理的变形，保持细节和拓扑结构，同时提高变形后的渲染质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI方法在灾害损害评估中面临数据标注成本高、数据集有限、传统分类框架灵活性不足以及生成模型易产生幻觉或通用回答的问题。

Method: 两阶段推理框架：首先生成结构化推理轨迹（使用思维链提示和上下文学习），然后通过答案选择模块评估和选择最相关准确的答案，结合定制信息检索系统和领域特定提示。

Result: 在FloodNet和RescueNet-VQA数据集上的实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面表现优异，适用于真实世界的灾后损害评估任务。

Conclusion: ThiFAN-VQA成功结合了零样本和监督方法的优势，在保持灵活性的同时提高了回答的一致性，为灾害损害评估提供了有效的解决方案。
...
TL;DR: HunyuanOCR是一个商业级、开源、轻量级（10亿参数）的视觉语言模型，专为OCR任务设计。它在感知任务（文本定位、解析）和语义任务（信息提取、文本图像翻译）上表现出色，在ICDAR 2025 DIMT挑战赛（小模型赛道）中获得第一名，并在OCRBench上实现了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 解决传统'OCR专家模型'功能单一和'通用VLM'效率低下的问题，实现多功能与高效性的统一；通过端到端架构消除预处理模块依赖，解决传统流程中的错误传播问题；探索高质量数据和强化学习策略在OCR任务中的关键作用。

Method: 采用原生视觉变换器（ViT）和轻量级LLM架构，通过MLP适配器连接；采用纯端到端范式，无需布局分析等预处理模块；利用高质量数据和强化学习策略进行训练优化。

Result: 在ICDAR 2025 DIMT挑战赛（小模型赛道）中获得第一名；在OCRBench上实现参数量少于30亿的VLM中最先进的结果；性能超越商业API、传统流程和更大模型（如Qwen3-VL-4B）。

Conclusion: HunyuanOCR成功统一了多功能性和高效性，通过端到端架构简化了系统部署，证明了高质量数据和强化学习策略在OCR任务中的重要性。该模型已开源，提供了基于vLLM的高性能部署方案，为前沿研究和工业应用提供了坚实基础。
...
TL;DR: 提出一种基于GAN的半监督分割方法，用于在非对比增强CT中准确描绘早期缺血性卒中区域，即使病灶微弱或体积小也能有效识别。


<details>
  <summary>Details</summary>
Motivation: 缺血性卒中是时间敏感的医疗紧急情况，但非对比增强CT在早期阶段往往无法显示细微的缺血变化，这会延误关键干预。

Method: 使用生成对抗网络的半监督分割方法，通过对抗框架从有限标注数据和大量未标注数据中学习，结合Dice损失、交叉熵损失、特征匹配损失和自训练损失。

Result: 在公开的急性缺血性卒中数据集上的实验表明，该方法能提升诊断能力，减少人工标注负担，支持更高效的卒中临床决策。

Conclusion: 该半监督分割方法有望改善早期缺血性卒中的诊断，为卒中护理提供更有效的临床决策支持。
...
TL;DR: 提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)，用于生成包含异常病变的无线胶囊内窥镜图像，以解决医疗数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 无线胶囊内窥镜(WCE)产生大量需要人工筛查的图像，但由于隐私限制和标注成本，缺乏大规模、多样化的医疗训练数据，阻碍了临床决策支持系统的开发。

Method: 开发了多尺度向量量化变分自编码器(MSVQ-VAE)，能够将异常无缝引入正常WCE图像中，并支持条件生成不同类型的异常病变。
...
TL;DR: SkillSight是一个用于从第一人称视角数据中进行能效技能评估的系统，通过结合注视数据和视频数据来预测技能水平，最终仅使用注视数据即可实现高效评估。


<details>
  <summary>Details</summary>
Motivation: 解决智能眼镜上自动技能评估的技术挑战，利用注视数据来降低能耗，实现实时的技能学习支持。

Method: 采用两阶段框架：首先联合建模注视和第一人称视频来预测技能水平，然后蒸馏出仅使用注视数据的学生模型。

Result: 在烹饪、音乐和体育三个数据集上验证了注视在技能理解中的价值，学生模型在保持高精度的同时能耗降低73倍。

Conclusion: SkillSight为野外AI支持的技能学习铺平了道路，证明了注视数据在技能评估中的重要作用和能效优势。
...
TL;DR: 提出了一种基于视觉语言基础模型的语义分布引导重建框架，通过对比学习目标将重建图像与目标语义分布对齐，提升欠采样MRI重建质量。
...
Result: 在膝关节和脑部数据集上的实验表明，基于图像的语义先验能保留精细解剖结构，获得更好的感知质量（更低的LPIPS值、更高的Tenengrad分数和读者研究评分）。图像-语言信息进一步扩展语义分布，实现对重建属性的高层控制。
...
TL;DR: GIANT框架让通用大模型能够像病理学家一样迭代导航全切片图像，在病理图像解读任务中显著超越传统方法，接近或超过专门训练的病理模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有通用大模型在医学图像解读特别是病理学领域表现不佳，主要因为使用了低分辨率缩略图或随机图像块，未能充分利用千兆像素图像信息。

Method: 提出GIANT框架，让大模型能够迭代导航全切片图像；同时发布MultiPathQA基准，包含934个WSI级别问题和128个病理学家编写的问题。

Result: 在病理学家编写的问题上，GPT-5结合GIANT达到62.5%准确率，显著优于TITAN(43.8%)和SlideChat(37.5%)等专门病理模型。
...
TL;DR: 本文提出CodeV视觉代理模型，通过工具感知策略优化(TAPO)训练，显著提高了视觉工具使用的忠实度，在保持高准确率的同时解决了视觉推理中的不忠实问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在调用图像工具时存在不忠实问题——可能在无关区域调用工具或忽略工具输出但仍能猜对答案，这影响了视觉推理的可信度。

Method: 提出CodeV代码视觉代理，采用工具感知策略优化(TAPO)框架，这是一个过程级强化学习框架，直接在视觉工具输入输出上定义密集奖励，而非思维链token，从而更容易验证监督且不易被奖励攻击。

Result: CodeV在相关视觉搜索基准测试中实现了竞争性或更优的准确率，同时大幅提高了忠实工具使用率。在多模态推理和数学基准测试中也表现出色。

Conclusion: 明确监督中间工具行为对于构建可信赖的、代理式视觉推理系统至关重要，CodeV和TAPO为此提供了有效解决方案。
...
TL;DR: OncoVision是一个多模态AI系统，结合乳腺X光图像和临床数据，通过注意力机制编码器-解码器架构实现四个关键区域的分割和十个临床特征的预测，采用晚期融合策略提高诊断精度。
...
Method: 使用基于注意力的编码器-解码器架构进行四个ROI的分割，预测十个结构化临床特征，并开发了两种晚期融合策略来整合影像和临床信息。

Result: 实现了最先进的分割精度，能够稳健预测临床特征，通过多模态融合提高了诊断精度，并开发了用户友好的Web应用程序。
...
Method: 分析连续三层中的局部冗余，移除前两层中最冗余的层，微调剩余层以补偿损失的能力，并冻结第三层作为稳定锚点。采用交错微调-冻结设计，在剪枝后使用少量数据进行快速收敛。

Result: 仅使用FineVision数据集的1%进行一轮微调，在剪除25%网络层后实现了88.9%的平均性能保留，达到了最先进的性能水平。

Conclusion: INTERLACE框架通过交错微调-冻结策略，能够高效地剪枝视觉语言模型中的冗余层，同时保持优异的性能表现。
...
TL;DR: IndEgo是一个多模态工业任务数据集，包含3,460个自我中心视角记录和1,092个外部视角记录，专注于协作工业任务，提供丰富的注释和基准测试。


<details>
  <summary>Details</summary>
Motivation: 解决工业任务理解和协作工作分析中缺乏多模态数据集的问题，特别是在认知和体力密集型任务方面。

Method: 收集了约197小时的自我中心视角记录和97小时的外部视角记录，包含眼动追踪、叙述、声音、动作等多模态数据，并提供详细注释和元数据。

Result: 数据集在程序性和非程序性任务理解、错误检测和推理问答等基准测试中，对当前最先进的多模态模型提出了挑战。

Conclusion: IndEgo数据集为工业任务分析和协作工作理解提供了有价值的资源，展示了现有模型在该领域的局限性。
...
TL;DR: 提出了一种基于原型的方法，用于通过密度图估计进行可解释的细胞计数，该方法在不影响计数效果的情况下提供模型透明度。
...
Method: 在密度估计网络中集成原型层，学习细胞和背景伪影的代表性视觉模式，并通过突出显示与每个原型最相似的输入图像区域来生成解释。

Result: 在两个公共数据集上的广泛实验表明，该方法在保持计数效果的同时实现了可解释性。生物学家调查确认了学习到的视觉模式的相关性。

Conclusion: 为研究人员和临床医生提供了一种透明可靠的细胞计数工具，可能增加信任并加速深度学习在关键生物医学应用中的采用。
...
TL;DR: RADSeg利用RADIO视觉基础模型改进零样本开放词汇语义分割，在mIoU、延迟和参数效率三个关键维度同时提升，以更少的计算和内存成本达到最先进精度。
...
Result: RADSeg在基础ViT类别上实现6-30% mIoU提升，速度快3.95倍，参数减少2.5倍。RADSeg-base仅105M参数却超越之前850-1350M参数的大型视觉模型组合。
...
TL;DR: 提出一种基于分支的结构重参数化技术，通过训练阶段的并行分支合并来减少Vision Transformer的层数，在保持精度的同时实现推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有ViT加速策略主要关注算法级优化，但深度架构带来的计算开销尚未充分探索。研究能否在减少层数的同时保持表征能力。

Method: 在训练阶段使用并行分支，通过逐步合并非线性组件入口处的分支，对FFN和MHSA模块进行精确数学重参数化，无需测试时近似。

Result: 在ViT-Tiny上成功将12层架构压缩到6、4甚至3层，在ImageNet-1K上保持分类精度，移动CPU推理速度提升达37%。

Conclusion: 传统偏好极深Transformer堆栈的认知可能过于保守，为构建高效视觉Transformer提供了新思路。
...
TL;DR: 提出了一种基于高度感知的动态分块方法，通过根据无人机高度动态调整图像分块大小，显著提升了海上搜救任务中小物体的检测性能。


<details>
  <summary>Details</summary>
Motivation: 无人机在海上搜救任务中面临小物体检测困难的问题，特别是在高海拔下物体与背景像素比低，导致检测精度下降。

Method: 结合高度依赖的缩放和自适应分块因子，动态调整图像分块大小，减少不必要的计算同时保持检测性能。

Result: 在SeaDronesSee数据集上测试，小物体的平均精度(mAP)比基线提高了38%，推理速度比静态分块快两倍以上。
...
<details>
  <summary>Details</summary>
Motivation: 切片传输方法通过利用1D OT问题的闭式解降低计算成本，但现有方法未解决学习到的最优切片器在分布偏移下是否能迁移到新分布对的问题，这在数据演化或重复OT计算场景中至关重要。

Method: 研究min-STP框架，分析优化切片器的可迁移性；引入小批量min-STP公式化方法，并提供统计精度保证；通过点云对齐和基于流的生成建模验证方法。

Result: 理论证明优化切片器在数据分布轻微扰动下保持接近；实验表明可迁移min-STP在一次性匹配任务中表现优异，并支持点云对齐和生成建模的摊销训练。

Conclusion: 优化的切片器具有可迁移性，能够在相关任务间高效转移，小批量min-STP进一步提高了方法的可扩展性，为处理演化数据或重复OT计算提供了实用解决方案。
...
Method: 开发了PathFMTools工具包，用于与CONCH和MUSK两个先进的视觉-语言基础模型交互，并在440个cSCC H&E全切片图像上测试多种适应策略。
...
TL;DR: 本文提出了一种多模态原型网络方法，用于物种检测，能够智能分配昂贵的基因数据，同时保持模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决多模态神经网络在物种检测中的两个主要问题：黑盒性质导致决策过程不可解释，以及基因数据采集成本高且具有侵入性。

Method: 扩展原型网络(ProtoPNets)到多模态、成本感知设置，通过集成每个模态的原型并分配权重来确定预测对各模态的依赖程度，引入方法识别无需昂贵基因信息即可做出自信预测的情况。
...
TL;DR: 提出了VESSA框架，将视觉语言模型集成到半监督医学图像分割中，通过两阶段方法显著提升了在有限标注条件下的分割精度
...
TL;DR: 提出了一种新的相对角度特征，用于点云损伤重建，该特征在保持与法向量相似性能的同时，显著减少了数据存储和计算需求。


<details>
  <summary>Details</summary>
Motivation: 点云重建方法对背景噪声具有鲁棒性，但受限于3D数据的高体积。需要一种能够压缩数据同时保持损伤特征有效性的方法。

Method: 计算点云中点的法向量与父点云平均法向量之间的相对角度作为单维特征，通过基于熵的特征评估来筛选冗余信息。

Result: 基于相对角度的模型在PointNet++上达到与法向量模型相似性能，同时实现27.6%存储减少和83%输入通道压缩。

Conclusion: 相对角度特征能够在资源受限硬件上实现更大批量执行，且无需修改模型架构，具有实际应用潜力。
...
TL;DR: CrispFormer通过三个小改动增强SegFormer解码器：边界分支监督物体轮廓、不确定性引导的细化器以及动态多尺度融合层，显著提升弱监督语义分割效果。


<details>
  <summary>Details</summary>
Motivation: 弱监督语义分割需要从噪声和欠指定的线索中学习密集掩码，现有方法在边界保持、尺度选择和噪声抵抗方面存在不足。

Method: 在SegFormer解码器中添加：(1)边界分支使用轻量级边缘头和边界感知损失；(2)不确定性引导细化器预测像素级不确定性并用于加权损失和门控残差校正；(3)动态多尺度融合层用空间softmax门控替代静态拼接。

Result: 在标准WSSS流程中，CrispFormer相比SegFormer基线持续提升边界F分数、小物体召回率和mIoU，同时计算开销最小。

Conclusion: CrispFormer的解码器中心化设计简单易实现，与现有SegFormer变体广泛兼容，为从图像级监督获得高保真掩码提供了可复现路径。
...
TL;DR: 提出TPSO方法解决文本到图像扩散模型生成多样性不足的问题，通过优化token嵌入空间来探索被忽视区域，避免模型重复生成强模式样本


<details>
  <summary>Details</summary>
Motivation: 图像多样性是文本到图像扩散模型的基本挑战，低多样性导致重复输出，增加采样冗余，阻碍创意探索和下游应用。主要原因是生成往往坍缩到学习分布中的强模式

Method: 提出TPSO（Token-Prompt嵌入空间优化），这是一种无需训练且模型无关的模块。通过引入可学习参数探索token嵌入空间中被忽视区域，减少模型重复生成强模式样本的趋势

Result: 在MS-COCO和三个扩散骨干网络上的广泛实验表明，TPSO显著提升了生成多样性，将基线性能从1.10提高到4.18分，且不牺牲图像质量

Conclusion: TPSO通过token级空间优化有效解决了扩散模型多样性不足的问题，在保持高质量的同时显著提升生成多样性
...
Method: 采用先剪枝后规划的方法：使用Holm-Bonferroni启发式剪枝过程去除不可信的边界选择，然后将最终决策委托给基于覆盖率的规划器，将过度自信的预测转化为保守可解释的动作。

Result: 在3D-Mem EQA框架中集成该方法，在视觉基础SPL和LLM-Match指标上分别比基线相对提升49%和33%，在相同探索预算下在OpenEQA和EXPRESS-Bench数据集上实现了更好的场景覆盖。

Conclusion: Prune-Then-Plan框架通过步骤级校准有效稳定了大型视觉语言模型的探索行为，显著提升了具身问答任务的性能。
...
TL;DR: CropVLM是一种低成本的外部方法，通过强化学习训练模型动态放大图像相关区域，提升视觉语言模型在细粒度图像理解任务上的性能，无需修改或微调原模型。
...
Method: 使用强化学习训练CropVLM模型，无需人工标注边界框监督信号或昂贵的合成评估，模型训练一次后可与开源和专有VLMs配对使用，动态放大图像相关区域。

Result: 在需要高分辨率图像理解的任务上取得显著改进，特别是在目标VLM域外基准测试中表现优异，且不会导致灾难性遗忘。

Conclusion: CropVLM是一种有效的低成本解决方案，能够显著提升VLMs在细粒度图像理解任务上的性能，同时保持模型原有的能力。
...
TL;DR: 本文提出Cross-Resolution Phase-Aligned Attention (CRPA)方法，解决了扩散变换器在混合分辨率去噪中使用RoPE线性插值时出现的注意力机制崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 当不同空间网格的token混合时，线性坐标重映射导致RoPE相位采样率不兼容，产生相位混叠，破坏分数景观。预训练的DiT对此特别敏感，微小的跨分辨率不一致就会导致模糊、伪影或完全崩溃。

Method: 提出CRPA方法，通过修改每个注意力调用的RoPE索引映射，将所有Q/K位置表达在查询的步长上，确保相同物理距离总是产生相同的相位增量。

Result: CRPA能够稳定所有头和层，实现高保真和高效的混合分辨率生成，在图像和视频生成任务上优于之前的最先进方法。

Conclusion: CRPA是一个无需训练的即插即用解决方案，从源头上消除了混合分辨率去噪中的RoPE相位对齐问题，与预训练DiT完全兼容。
...
TL;DR: MAPS是一个用于Vision-Language-Action模型微调的模块化邻近调度框架，通过线性放松约束来平衡稳定性和灵活性，提升泛化性能。
...
Method: MAPS通过系统分析发现模块放松约束的实证顺序，线性调度这种放松，使视觉编码器保持接近预训练先验，而面向动作的语言层更自由地适应。

Result: 在多个基准测试和真实世界评估中，MAPS一致提升内分布和外分布性能（最高+30%）。

Conclusion: 经验指导的与预训练VLM的邻近性是一个简单而强大的原则，可在VLM到VLA迁移中保持广泛的泛化能力。
...
TL;DR: 提出Latent Representation Probing (LRP)方法，通过分析VLM内部表示来检测不确定性，在STVQA任务中显著提升弃权准确率。


<details>
  <summary>Details</summary>
Motivation: VLM在安全关键应用中需要知道何时弃权回答，特别是在OCR错误可能导致严重后果的STVQA任务中。现有弃权方法效果不佳，因为不确定性信号可能隐藏在VLM内部表示中。

Method: 提出LRP方法：在隐藏状态或注意力模式上训练轻量级探针。探索三种探针设计：跨层连接表示、聚合视觉token注意力、多数投票集成单层探针。

Result: 在四个图像和视频基准测试中，LRP比最佳基线方法提升弃权准确率7.6%。探针能泛化到各种不确定性来源和数据集，最佳信号来自中间层而非最终层。

Conclusion: 建立了一个从内部状态而非不可靠输出中检测置信信号的框架，为构建可部署AI系统提供了原则性方法。
...
TL;DR: 提出了CounterVQA基准来评估VLMs的反事实推理能力，发现现有模型在复杂因果链推理上表现较差，并开发了CFGPT方法来提升视觉反事实推理能力


<details>
  <summary>Details</summary>
Motivation: 虽然VLMs在视频理解方面取得进展，但其反事实推理能力（推断假设条件下的替代结果）尚未被充分探索，这对稳健的视频理解至关重要

Method: 引入CounterVQA基准评估三个难度等级的反事实推理，并开发CFGPT后训练方法，从语言模态中提取反事实推理能力来增强视觉反事实推理

Result: 评估发现现有模型在简单反事实问题上表现尚可，但在复杂多跳因果链上性能显著下降；CFGPT方法在所有难度级别上都带来了持续改进

Conclusion: 反事实推理是VLMs的关键能力缺口，CounterVQA基准和CFGPT方法为解决这一挑战提供了系统评估和改进途径
...
TL;DR: ReDirector是一种新颖的相机控制视频重拍生成方法，通过改进RoPE在时空位置对齐上的应用，并引入Rotary Camera Encoding (RoCE)来捕捉多视角关系，实现了对分布外相机轨迹和视频长度的泛化能力。
...
Method: 提出Rotary Camera Encoding (RoCE)，一种相机条件的RoPE相位偏移，用于捕捉和整合输入和目标视频内部及之间的多视角关系；将相机条件集成到RoPE中。

Result: 实验显示在相机可控性、几何一致性和视频质量方面有显著提升，能够泛化到分布外的相机轨迹和视频长度。

Conclusion: ReDirector通过改进RoPE对齐和引入RoCE，有效提升了视频重拍生成的性能，特别是在相机控制和几何一致性方面表现优异。
...
TL;DR: 提出了BHD-RAG框架，通过整合DCLD专业知识与多模态大语言模型，提升Birt-Hogg-Dube综合征的CT诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习在罕见病诊断中面临的两个挑战：临床样本有限和DCLD疾病间差异小，同时减少多模态大语言模型的幻觉风险。

Method: 使用专门代理生成CT图像描述构建多模态语料库，基于余弦相似度的检索器匹配相关图像-描述对，MLLM结合检索证据进行诊断。

Result: 在包含四种DCLD的数据集上验证，取得了优越的准确率，生成的描述与专家见解高度一致。

Conclusion: BHD-RAG框架有效整合领域知识和临床先例，显著提高了BHD的诊断准确性并减少了幻觉现象。
...
Conclusion: Rectified SpaAttn有效解决了稀疏注意力中的系统偏差问题，在显著加速的同时保持了生成质量，为高效视频生成提供了可行方案。
...
TL;DR: 提出了4DWorldBench基准，用于系统评估世界生成模型在感知质量、条件-4D对齐、物理真实性和4D一致性四个关键维度的表现。


<details>
  <summary>Details</summary>
Motivation: 传统2D视觉生成向3D/4D世界生成演进，但现有基准缺乏对世界真实感能力的统一评估，需要系统评估模型在空间、时间、物理和指令控制方面的连贯性。

Method: 引入自适应多模态条件评估，将所有模态条件映射到统一文本空间，整合LLM-as-judge、MLLM-as-judge和传统网络方法，涵盖图像到3D/4D、视频到4D、文本到3D/4D等任务。

Result: 初步人类研究表明，自适应工具选择与主观人类判断更接近，基准为客观比较和改进提供了基础。

Conclusion: 4DWorldBench基准将加速从"视觉生成"向"世界生成"的转变，为下一代多模态智能系统的发展提供重要支撑。
...
TL;DR: 提出了DesignPref数据集，包含12k个UI设计生成的成对比较，由20位专业设计师标注多级偏好评分，研究个性化视觉设计评估问题。


<details>
  <summary>Details</summary>
Motivation: 由于视觉设计具有主观性和高度个性化特性，不同个体的设计偏好差异很大，传统基于多数投票的偏好聚合方法无法准确反映个人偏好。

Method: 引入DesignPref数据集，分析设计师之间的分歧程度，研究多种个性化策略，特别是微调或将设计师特定标注整合到RAG管道中。

Result: 设计师之间存在显著分歧（Krippendorff's alpha = 0.25），个性化模型在预测个体设计师偏好方面始终优于聚合基线模型，即使使用少20倍的示例。

Conclusion: 这是首个研究个性化视觉设计评估的数据集，支持未来对个体设计品味的建模研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决基础模型在微调时出现的灾难性遗忘问题，创建能同时处理多个任务的统一模型。

Method: 引入IMIC训练策略，使用梯度耦合的交错训练计划，在DINOv3、CLIP和EVA-02三个基础模型上同时微调四个任务。

Result: EVA-02和CLIP模型在所有四个任务上表现与领域专家相当，在多任务处理上比人类更准确，且不损害分布外泛化能力。

Conclusion: IMIC方法成功创建了统一嵌入空间，四个任务表示线性可分但特征共享，少量主成分即可完成所有任务。
...
### [53] [Does Understanding Inform Generation in Unified Multimodal Models? From Analysis to Path Forward](https://arxiv.org/abs/2511.20561)
...
Result: 发现理解-生成差距主要体现在推理生成和知识迁移两个维度；显式思维链能有效弥合差距；自训练可内化推理能力；查询架构具有潜在的思维链特性。
...
TL;DR: 提出了DOGE框架，使用Bézier曲线表示道路网络，直接从分割掩码学习参数化Bézier图，无需曲线标注数据，在SpaceNet和CityScale基准测试中达到最先进水平。
...
Method: 提出Bézier图表示法，通过DOGE框架进行全局优化，包含DiffAlign模块（通过可微分渲染优化几何）和TopoAdapt模块（通过离散算子优化拓扑）。

Result: 在SpaceNet和CityScale大规模基准测试中实现了新的最先进性能。

Conclusion: 该方法为生成高保真矢量道路地图提供了一种新范式，无需曲线标注数据。
...
TL;DR: STAvatar提出了一种从单目视频重建高保真可动画3D头部化身的方法，通过UV自适应软绑定框架和时序自适应密度控制策略，解决了现有方法在运动刚性和遮挡区域处理方面的限制。
...
Method: 1. UV自适应软绑定框架：利用图像和几何先验学习UV空间中每个高斯特征偏移，支持动态重采样，与自适应密度控制完全兼容；2. 时序ADC策略：首先聚类结构相似的帧，引入融合感知误差作为克隆标准，联合捕捉几何和纹理差异。

Result: 在四个基准数据集上的广泛实验表明，STAvatar实现了最先进的重建性能，特别是在捕捉细粒度细节和重建频繁遮挡区域方面表现优异。

Conclusion: STAvatar通过创新的UV自适应绑定和时序密度控制策略，显著提升了3D头部化身的重建质量和动画能力，代码将公开可用。
...
TL;DR: TimeArtist是一个时序-视觉转换框架，通过语义级对齐实现从时间序列到高质量图像的直接生成，同时捕捉时序波动模式用于风格迁移。


<details>
  <summary>Details</summary>
Motivation: 现有方法将时间序列转换为"伪图像"进行时序预测，但未能建立语义级对齐。探索非视觉连续序列作为条件信号用于高保真图像生成的潜力。
...
Conclusion: 建立了跨模态生成的新范式，弥合了时序动态与视觉语义之间的差距。
...
TL;DR: GigaWorld-0是一个统一的世界模型框架，通过视频生成和3D建模相结合的方式，为视觉-语言-动作学习提供高质量合成数据，使VLA模型在物理机器人上实现零真实世界交互训练下的强泛化性能。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI领域数据稀缺、成本高昂的问题，通过世界模型作为数据引擎，为VLA学习提供可扩展、数据高效的训练数据。

Method: 整合GigaWorld-0-Video（大规模视频生成）和GigaWorld-0-3D（3D生成建模+物理系统识别+运动规划），通过GigaTrain框架实现高效训练，利用FP8精度和稀疏注意力降低计算需求。

Result: 生成高质量、多样化、可控的数据，VLA模型（如GigaBrain-0）在物理机器人上表现出强大的真实世界性能，显著提高了泛化能力和任务成功率，无需真实世界交互训练。

Conclusion: GigaWorld-0证明了世界模型作为数据引擎的有效性，能够生成视觉吸引、空间一致、物理合理、指令对齐的具身交互数据，为具身AI提供了可扩展的解决方案。
...
TL;DR: ChessMamba是一个用于多时相遥感图像变化检测的结构感知框架，通过棋盘交错扫描和结构感知融合解决时空错位问题，在多个变化检测任务中显著优于现有方法。
...
Method: ChessMamba集成了SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错扫描顺序将多时相特征序列化，并通过多扩张卷积进行结构感知融合。

Result: 在三个变化检测任务（二元变化检测、语义变化检测和多模态建筑损伤评估）上的综合评估表明，ChessMamba能有效融合异构特征，相比最先进方法实现了显著精度提升。

Conclusion: ChessMamba通过结构感知的状态空间建模，为多时相输入提供了稳健的变化检测解决方案，在保持局部结构一致性的同时实现了准确的变化定位。
...
Method: 利用频域特征，对低频特征实施强对齐损失，对高频特征采用宽松对齐，并提出尺度一致性损失和共享分类器来统一特征空间

Result: 在多个基准数据集上的实验表明，该方法显著优于传统知识蒸馏和现有最先进的跨模态知识蒸馏方法

Conclusion: 频率解耦的跨模态知识蒸馏能有效解决跨模态知识迁移中的表示不一致问题，提升知识蒸馏效果
...
TL;DR: 构建了一个多任务肝脏数据集LiMT，用于肝脏和肿瘤分割、多标签病灶分类和病灶检测，基于动脉期增强CT，包含150个病例，涵盖四种肝脏疾病和正常情况。


<details>
  <summary>Details</summary>
Motivation: 现有CAD数据集通常只支持单一任务，限制了CAD技术的发展。需要构建一个多任务数据集来探索任务间的相关性，避免训练时任务特定数据集间的异质性。

Method: 基于动脉期增强CT构建多任务肝脏数据集，包含150个不同病例，涵盖四种肝脏疾病和正常情况。每个体积都由经验丰富的临床医生仔细注释和校准。

Result: 提供了一个公开的多任务数据集LiMT，包含相关基线实验结果，并回顾了现有肝脏相关任务的数据集和方法。

Conclusion: LiMT数据集可能成为医学影像研究社区的宝贵资源，为探索任务间相关性提供了解决方案，避免了任务特定数据集间的异质性。
...
TL;DR: 提出了一个生成-验证框架来创建高质量的科学视觉问答数据集VeriSciQA，包含20,351个QA对，覆盖20个科学领域和12种图表类型，显著提升了开源模型的SVQA性能。


<details>
  <summary>Details</summary>
Motivation: 开源的大视觉语言模型在科学视觉问答任务上表现不佳，主要瓶颈在于缺乏高质量、大规模的公开数据集。现有使用LVLM生成数据的方法存在系统性错误。
...
Conclusion: 验证中心的生成-验证框架能够持续扩展高质量数据，进一步推动开源社区在科学视觉问答能力方面的进步。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言智能体的学习受限于人工标注监督，纯文本的自我评估难以验证复杂的视觉推理步骤且容易产生评估幻觉。

Method: 提出Agent0-VL，将工具使用集成到推理、自我评估和自我修复中，包含求解器和验证器两个角色，通过自我演化推理循环实现稳定自我改进。

Result: 在几何问题解决和视觉科学分析任务上，Agent0-VL比基础模型提升了12.5%的性能。

Conclusion: Agent0-VL能够在无需人工标注或外部奖励模型的情况下，通过工具基础的验证和强化学习实现推理和评估行为的对齐，实现持续自我改进。
...
<details>
  <summary>Details</summary>
Motivation: 提高连续手语识别的鲁棒性，利用3D骨骼特征捕捉手语特性及其动态变化，并整合手形信息来改进边界检测。

Method: 使用3D骨骼特征和手形分类器，预训练87个语言学定义的手形类别分类器，通过多模态融合模块结合视频分割和手形分类模型。

Result: 在ASLLRP语料库上评估，相比之前工作有显著改进。

Conclusion: 该方法通过多模态融合和边界检测技术，有效提升了连续手语识别的性能。
...
TL;DR: Motion Marionette是一个零样本的刚性运动迁移框架，能够从单目源视频迁移运动到单张目标图像，无需外部先验，通过内部时空变换先验实现通用且时序一致的运动迁移。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖几何、生成或仿真等外部先验来指导运动迁移，但这些外部先验引入额外约束，导致通用性和时序一致性之间的权衡问题。

Method: 首先将源视频和目标图像提升到统一的3D表示空间，从源视频提取运动轨迹构建独立于物体几何和语义的时空先验，然后与目标物体结合生成可控速度场，最后使用基于位置的动力学进行细化。

Result: 实验结果表明，该方法能够泛化到多种物体，生成与源运动对齐的时序一致视频，并支持可控视频生成。

Conclusion: Motion Marionette通过内部时空先验实现了无需外部约束的高质量运动迁移，在通用性和时序一致性方面表现出色。
...
TL;DR: Reasoning-VLA是一个快速动作生成的视觉-语言-动作框架，通过可学习的动作查询和推理增强的视觉语言特征，在自动驾驶中实现并行连续动作轨迹生成，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在推理效率和泛化到新的自动驾驶配置和场景方面存在困难，需要开发更通用和快速的框架。

Method: 使用高斯采样从训练数据中初始化可学习动作查询，与推理增强的视觉语言特征交互生成连续动作轨迹；整合8个自动驾驶数据集为标准化推理格式；结合监督学习和强化学习微调。

Result: 在多个基准测试中达到最先进性能，具有优越的泛化能力和迄今为止最快的推理速度。

Conclusion: Reasoning-VLA框架在自动驾驶决策中实现了高效推理、强大泛化和快速动作生成，为VLA模型提供了新的发展方向。
...
TL;DR: 提出了一个预测3D打印物体光化学转化的框架，通过C-PGA架构从3D视觉数据预测非视觉体积物理性质，实现了虚拟化学表征。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型缺乏对光学物理和材料物理耦合非线性相互作用的归纳偏置，无法从3D视觉数据预测密集的非视觉体积物理性质。

Method: 提出C-PGA多模态融合架构，使用稀疏几何和工艺参数作为Query，通过FiLM动态门控和适应密集视觉特征，空间调制双3D视觉流。

Result: 该方法在虚拟化学表征方面取得突破，无需传统后打印测量即可精确控制化学转化状态。

Conclusion: C-PGA框架成功解决了从3D视觉数据预测物理性质的挑战，为复杂3D打印物体的光化学转化预测提供了有效解决方案。
...
TL;DR: LoTTS提出了一种局部测试时缩放方法，通过对比注意力信号定位图像缺陷区域并仅对这些区域进行重采样，在保持高质量区域的同时显著降低计算成本。
...
Method: LoTTS框架包含两个核心组件：1）通过对比交叉注意力和自注意力信号来定位缺陷区域；2）仅对缺陷区域进行局部扰动和去噪，确保全局一致性。
...
Result: 在DocLayNet和M$^6$Doc基准测试中达到83.5%的平均精度(mAP)，创造了最先进的性能，超越了先前的方法。
...
TL;DR: DetVLM是一个新颖的智能图像检索框架，通过融合目标检测和视觉大模型，实现了细粒度图像检索中的状态搜索和零样本搜索能力。
...
Method: 提出两阶段流水线：首先使用YOLO检测器进行高效、高召回率的组件级筛选，确定组件存在；然后使用VLM作为召回增强单元，对检测器遗漏的组件进行二次验证。

Result: 在车辆组件数据集上，DetVLM实现了94.82%的整体检索准确率，显著优于仅使用检测的基线方法。在零样本搜索中达到94.95%的准确率，在状态搜索任务中平均准确率超过90%。

Conclusion: DetVLM成功融合了目标检测和视觉大模型的优势，为细粒度图像检索提供了有效的解决方案，特别是在状态搜索和零样本搜索方面表现出色。
...
TL;DR: CPDATrack是一个基于Transformer的跟踪框架，通过目标概率估计和选择性注意力机制来抑制背景和干扰物令牌的干扰，同时保持目标上下文信息，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的一流Transformer跟踪器在连接模板和搜索区域令牌时，过多的背景搜索令牌会关注目标模板令牌，削弱跟踪器的判别能力。现有的令牌剪枝方法往往会移除目标附近的令牌，导致重要上下文信息丢失。此外，搜索令牌中的干扰物进一步降低了跟踪器准确识别目标的能力。
...
Conclusion: 所提出的CPDATrack框架通过有效抑制背景和干扰物令牌的干扰，同时保持计算效率，显著提升了跟踪性能，在多个基准测试中表现出色。
...
TL;DR: DRIFT框架利用预训练图像扩散模型的自注意力机制实现零样本视频目标跟踪，通过重新解释注意力图为语义标签传播核，结合SAM引导的掩码优化，在标准基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 图像扩散模型虽然主要用于图像生成，但其隐含的丰富语义结构可以用于识别和定位任务。本研究探索如何利用扩散模型的自注意力机制实现像素级语义对应关系，并扩展到视频中的目标跟踪。

Method: 将自注意力图重新解释为语义标签传播核，构建时间传播核实现视频中的零样本目标跟踪。采用测试时优化策略（DDIM反转、文本反转、自适应头权重）来增强扩散特征的鲁棒性，结合SAM引导的掩码优化。

Result: 提出的DRIFT框架在标准视频目标分割基准测试中实现了最先进的零样本性能，证明了扩散模型在跟踪任务中的有效性。

Conclusion: 扩散模型的自注意力机制可以作为强大的语义标签传播工具，通过适当的优化策略能够实现鲁棒的视频目标跟踪，为扩散模型在生成任务之外的应用开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率内容创建是视觉和图形领域的核心挑战，现有方法仅限于低分辨率设置（通常只支持1K分辨率），需要有效的可控高分辨率图像编辑机制

Method: 采用测试时优化框架，对高分辨率源图像进行分块优化，然后通过精细细节传输模块和新型同步策略来保持分块间的一致性

Result: 大量实验表明该方法能产生高质量的编辑结果，为高分辨率内容创建铺平了道路
...
TL;DR: SPROUT是一个无需训练和标注的核实例分割框架，利用组织学先验构建特定切片的参考原型，通过部分最优传输方案进行特征对齐，并使用SAM模型实现精确的核分割。


<details>
  <summary>Details</summary>
Motivation: 当前大多数核分割方法依赖密集监督和计算昂贵的微调，而无需训练的方法研究不足。SPROUT旨在开发完全无需训练和标注的核实例分割解决方案。

Method: 利用组织学先验构建切片特定参考原型，通过部分最优传输方案进行渐进式特征对齐，将特征转换为正负点提示，使用SAM模型进行分割而无需参数更新。

Result: 在多个组织病理学基准测试中，SPROUT在无监督和重新训练的情况下实现了具有竞争力的性能。
...
TL;DR: GFT-GCN是一个结合谱图学习和扩散模板保护的隐私保护3D人脸识别框架，在保持高识别精度的同时提供模板安全保护。


<details>
  <summary>Details</summary>
Motivation: 3D人脸识别虽然具有抗欺骗性，但存储的生物特征模板需要保护。需要开发既能保持高识别精度又能保护模板隐私的解决方案。

Method: 结合图傅里叶变换(GFT)和图卷积网络(GCN)从3D人脸网格提取紧凑的谱特征，并通过谱扩散机制生成不可逆、可更新且不可链接的模板。采用轻量级客户端-服务器架构，原始生物数据不离开客户端设备。

Result: 在BU-3DFE和FaceScape数据集上实验显示，该方法具有高识别精度和强抗重建攻击能力。
...
TL;DR: MambaEye是一种新颖的因果序列视觉编码器，基于Mamba2架构，采用单向处理方式，通过相对移动嵌入和扩散启发式损失函数，实现输入尺寸无关的视觉处理，在ImageNet-1K分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉编码器无法真正实现输入尺寸无关的问题，这是人类视觉的一个基本特征。

Method: 使用纯Mamba2骨干网络，采用严格单向处理方式，引入相对移动嵌入编码空间位移，并使用扩散启发式损失函数进行密集步进监督。

Result: 在ImageNet-1K分类任务中，特别是在高分辨率（如1536²）下表现出鲁棒性能，同时保持相对于补丁数量的线性时间和内存复杂度。

Conclusion: MambaEye成功实现了输入尺寸无关的视觉编码，为视觉处理提供了新的因果序列方法。
...
TL;DR: 提出了HiCoGen框架，通过链式合成和强化学习解决复杂提示下多对象生成的构图问题，显著提升了概念覆盖率和组合准确性。
...
Method: 基于链式合成范式，首先用LLM分解复杂提示为最小语义单元，然后迭代合成这些单元；引入强化学习框架，提出衰减随机性调度来增强探索，并采用分层奖励机制评估图像。

Result: 实验表明该方法在概念覆盖率和组合准确性方面显著优于现有方法。

Conclusion: HiCoGen框架通过层次化组合生成和增强的探索策略，有效解决了复杂提示下的图像生成问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有4D方法依赖外部先验、繁重后优化或需要在4D数据集上微调，而VGGT在移动物体主导时性能显著下降，需要解决动态场景重建的挑战。

Method: 通过Gram相似性挖掘和放大全局动态线索，跨时间窗口聚合，使用投影梯度驱动的细化策略锐化掩码边界，并将精确掩码集成到VGGT早期推理中。

Result: 在六个数据集上，该方法在动态对象分割、相机姿态估计和密集重建方面实现了优越性能，支持超过500帧序列的单次推理。

Conclusion: VGGT4D成功扩展了3D基础模型到4D场景重建，无需额外训练即可有效缓解运动干扰，实现鲁棒的动态场景重建。
...
TL;DR: 本文提出了一种名为Activation Replay的训练免费方法，通过重放基础LMM的低熵激活来提升经过RLVR后训练的大型多模态模型的推理能力，无需昂贵的策略优化。


<details>
  <summary>Details</summary>
Motivation: 虽然RLVR已被证明能有效提升LMM的推理能力，但其背后的工作机制尚不明确。本文旨在探索RLVR如何影响输入激活，并基于此开发提升推理性能的方法。

Method: 通过logit lens视角系统研究RLVR对输入激活的影响，发现RLVR主要改变低熵激活。提出Activation Replay方法，在测试时重放基础LMM的低熵视觉token激活来调节RLVR模型的激活状态。

Result: Activation Replay在数学推理、视觉代理和视频推理等多种场景中显著提升推理性能，提高Pass@K指标并缓解RLVR推理覆盖范围较窄的问题。

Conclusion: 调制低熵激活对LMM推理具有有益作用，Activation Replay是一种简单有效的训练免费方法，能够显著提升后训练LMM的多模态推理能力。
...
TL;DR: 提出EmoFeedback2框架，通过生成-理解-反馈的强化范式，利用大视觉语言模型为连续情感图像生成提供奖励和文本反馈，解决现有方法缺乏情感反馈和情感保真度不足的问题。
...
Method: 采用生成-理解-反馈强化范式，引入情感感知奖励反馈策略（LVLM评估图像情感值并计算奖励）和自我提升文本反馈框架（LVLM迭代分析图像情感内容并生成提示优化建议）。

Result: 实验结果表明该方法能有效生成具有期望情感的高质量图像，在自定义数据集上优于现有最先进方法。

Conclusion: EmoFeedback2框架通过LVLM的推理能力为连续情感图像生成提供奖励和文本反馈，显著提升了情感连续性和情感保真度。
...
Method: 优化初始种子噪声以匹配未掩码区域（仅需数十步优化），采用线性近似避免计算展开，并在谱域进行优化以提升稳定性。
...
TL;DR: 提出了一种结合时间注视模式、头部运动数据和视觉场景信息的多模态VR注视预测方法，通过门控融合机制和跨模态注意力自适应加权不同模态，在22个VR场景的530万注视样本数据集上验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: VR环境中的注视行为预测对于渲染优化和界面设计具有重要意义，但现有方法面临挑战。本文旨在通过多模态融合来改进预测准确性。

Method: 采用多模态方法，结合时间注视模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力来学习基于上下文相关性的自适应权重分配。

Result: 在22个VR场景的530万注视样本数据集上，多模态组合相比单一数据流显著提高了预测准确性，在1-3个未来帧的预测中表现优异，跨场景泛化测试达到93.1%的验证准确率。

Conclusion: 该方法有助于理解虚拟环境中的注意力机制，在渲染优化、交互设计和用户体验评估方面具有应用潜力，为实现无需昂贵眼动追踪硬件的高效VR系统迈出了重要一步。
...
TL;DR: 提出了一个两阶段的细节感知细化框架，通过参考图像驱动的修正来增强像素级一致性，解决了现有扩散模型在保持细粒度视觉细节方面的局限性。
...
Result: 在具有挑战性的参考引导恢复基准测试中，该方法显著提高了参考对齐和细粒度细节保持能力，生成的结果超越了开源和商业模型。
...
TL;DR: 本文提出了首个类型特定的创意奖励模型CREward，涵盖几何、材质和纹理三个创意维度，用于评估和生成创意图像。


<details>
  <summary>Details</summary>
Motivation: 传统方法将创意视为单一未分化的数量过于简单，需要更细粒度的创意表示和评估方法。

Method: 首先进行人类基准评估获取人类对各类创意的感知，分析LVLM与人类判断的相关性，然后收集LVLM生成的标签训练CREward模型。

Result: LVLM与人类感知高度一致，CREward模型可用于创意评估、可解释创意分析和创意样本获取。

Conclusion: CREward模型通过类型特定的创意维度，为创意图像的评估和生成提供了有效工具，在创意评估、解释和生成方面具有广泛应用前景。
...
TL;DR: 本文揭示了一种新型威胁：单个扰动可以劫持整个决策链，通过语义感知通用扰动(SAUPs)同时操控模型对多个预定义目标的输出。


<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常在一系列决策中运行，孤立错误容易被纠正，但级联错误可能导致严重风险。传统对抗攻击只关注单次决策操纵，无法应对这种级联威胁。

Method: 提出语义感知通用扰动(SAUPs)，通过在归一化空间中使用语义分离策略搜索扰动，开发有效算法克服优化挑战。使用RIST数据集进行实际威胁评估。

Result: 在三个多模态大语言模型上的广泛实验显示，仅使用一个对抗帧就能在控制五个不同目标时达到70%的攻击成功率。

Conclusion: 多模态大语言模型对语义感知通用扰动存在脆弱性，单个扰动能够劫持整个决策链，这揭示了现实应用中严重的安全风险。
...
Method: 使用基于Transformer的特征提取模块从原始输入中提取运动和视觉特征；通过深度引导注意力模块利用深度信息引导注意力；设计模态注意力和时间注意力机制来选择性强调信息丰富的模态并捕捉时间依赖关系。
...
TL;DR: 提出多上下文融合Transformer(MFT)模型，通过融合行人行为、环境、行人定位和车辆运动四个维度的上下文信息，实现准确的行人过街意图预测。


<details>
  <summary>Details</summary>
Motivation: 城市环境中行人过街意图预测对自动驾驶车辆提升行人安全和减少交通事故至关重要，但由于影响行人行为的因素众多，准确预测仍具挑战性。

Method: 采用渐进式融合策略：首先通过相互内部上下文注意力实现各上下文内部特征融合，生成上下文特定表示；然后通过相互跨上下文注意力整合不同上下文特征；最后通过引导式内部和跨上下文注意力进一步优化表示。

Result: 在JAADbeh、JAADall和PIE数据集上分别达到73%、93%和90%的准确率，优于现有最先进方法。

Conclusion: MFT模型通过有效的多上下文融合机制显著提升了行人过街意图预测性能，消融研究验证了网络架构和不同输入上下文的有效性。
...
TL;DR: 提出了Safety-Critical Reasoning任务，通过多视角输入解决自动驾驶中的安全关键推理问题，并构建了WaymoQA数据集来提升多模态大语言模型在复杂驾驶场景中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在自动驾驶场景理解方面表现良好，但在安全关键场景中的高级推理能力仍然不足，特别是当避免一个交通风险可能引发另一个风险时。

Method: 将安全关键推理分为两个阶段：首先解决即时风险，然后缓解决策引发的下游风险。构建了包含35,000个人工标注问答对的WaymoQA数据集，涵盖图像和视频模态的复杂高风险驾驶场景。

Result: 实验表明现有MLLMs在安全关键场景中表现不佳，但使用WaymoQA进行微调后其推理能力显著提升。

Conclusion: WaymoQA数据集能有效提升多模态大语言模型在安全关键驾驶场景中的推理能力，为开发更安全的自动驾驶代理提供了重要支持。
...
TL;DR: SAM-MI是一个新颖的掩码注入框架，通过文本引导稀疏点提示器、浅层掩码聚合和解耦掩码注入，有效解决SAM的过分割问题，并在多个基准测试中取得显著性能提升。
...
TL;DR: 提出Vision-Guided Attention (VGA)方法，通过利用视觉token的语义内容构建精确的视觉定位，引导MLLMs关注相关视觉区域，从而减少幻觉现象。
...
Method: VGA是一种无需训练的方法，首先构建精确的视觉定位，然后使用该定位引导模型关注相关视觉区域。在图像描述中，VGA还会在生成过程中动态抑制已描述区域。

Result: VGA在多个MLLMs和幻觉基准测试中实现了最先进的去幻觉性能，仅引入4.36%的延迟开销，且与FlashAttention等高效注意力实现完全兼容。
...
TL;DR: COVec是一个基于明暗对比原理的照明感知矢量框架，通过内在图像分解将图像分离为反照率、阴影和光照层，实现统一的矢量表示。
...
Conclusion: COVec是首个在矢量域中引入内在图像分解的框架，通过明暗对比原理实现了更好的图像矢量表示和编辑能力。
...
Result: MFM-Point在基于点的方法中达到最佳性能，挑战了最佳基于表示的方法，在多类别和高分辨率生成任务中表现出色。

Conclusion: MFM-Point框架成功提升了基于点方法的性能，证明了多尺度生成策略在点云生成中的有效性，同时保持了方法的简单性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 行星遥感图像受成像环境和硬件限制影响，存在多样且未知的退化，限制了图像质量，且缺乏地面真值图像阻碍了监督盲超分辨率方法的应用。

Method: HACBSR包含两个组件：(1) 带核相似性控制的对比核采样机制，减轻高斯采样的分布偏差；(2) 历史增强对比学习，使用历史模型生成负样本，实现非贪婪优化和强凸性诱导。

Result: 在多个放大因子下，HACBSR与最先进的无监督方法相比具有竞争力，并引入了Ceres-50数据集支持行星应用评估。

Conclusion: HACBSR是一种有效的无监督盲超分辨率框架，在行星遥感图像处理中表现出色，无需地面真值图像即可处理未知退化问题。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜导航系统中，自监督单目深度估计性能因图像中不均匀光照（特别是低强度区域）而下降。现有低光增强技术无法有效指导深度网络，而其他领域的解决方案需要良好光照图像，不适用且增加数据收集负担。

Method: 设计光照-反射-深度模型表示内窥镜图像，并通过辅助网络进行分解。提出自监督联合优化框架，利用解耦组件的新损失函数来减轻不均匀光照对深度估计的影响。

Result: 在两个公共数据集上通过广泛比较和消融研究验证了所提方法的有效性。

Conclusion: DeLight-Mono框架通过光照解耦有效解决了内窥镜图像中不均匀光照对深度估计的负面影响。
...
<details>
  <summary>Details</summary>
Motivation: 传统点云压缩方法难以平衡压缩比和重建质量，因为低频和高频分量在同一分辨率下贡献不同。需要一种能够解耦不同频率成分的压缩方法。

Method: 将体素化嵌入转换为三平面表示以减少稀疏性和计算成本；设计频率解耦技术提取紧凑低频内容并收集多尺度高频细节；使用二进制格式存储解耦分量；通过调制块逐步恢复全频谱信号；引入基于频率的高效注意力机制补偿3D相关性损失。

Result: 在SemanticKITTI和Ford数据集上，BD-rate分别比标准编解码器提升78%和94%，实现了最先进的率失真性能。

Conclusion: FLaTEC通过频率感知机制成功解决了点云压缩中平衡压缩比和重建质量的难题，在保持高压缩比的同时实现了优异的重建质量。
...
TL;DR: PRADA是一种基于概率比率的简单可解释方法，能够可靠检测自回归生成的图像并溯源到具体生成模型


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成器能生成逼真图像，但目前缺乏专门检测此类生成图像的方法

Method: 通过检查模型对图像自回归token序列的条件概率和无条件概率之比，利用其独特特征进行阈值检测和溯源

Result: 实验评估显示PRADA对8个类别到图像和4个文本到图像模型高度有效

Conclusion: 概率比率特征可用于可靠检测和溯源自回归生成的图像
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过任务和步骤级别的文本描述与视觉内容对齐来注入程序语义，但由于高度抽象性，这些描述无法与视觉数据中的具体可观察细节形成稳健对齐。

Method: 提出TSS框架，任务通过步骤实现，步骤驱动可观察状态之间的转换；采用渐进式预训练策略，逐步展开TSS层次结构，强制模型将表示基于状态，同时与步骤和高级任务关联。
...
TL;DR: 提出了一种新的盲自适应局部去噪方法BALD，用于解决CEST MRI中空间变化噪声和异方差性问题，通过自适应方差稳定变换和两阶段去噪提升定量对比度映射的准确性。
...
Method: BALD利用CEST数据的自相似性推导自适应方差稳定变换，通过局部SVD分解作为线性变换进行两阶段去噪，分离分子信号与噪声。
...
TL;DR: 将概念瓶颈模型(CBMs)扩展到视觉异常检测(VAD)领域，提出CONVAD方法，在保持传统VAD性能的同时提供人类可理解的概念解释。


<details>
  <summary>Details</summary>
Motivation: 传统VAD模型虽然能提供视觉异常区域定位，但缺乏语义层面的直接解释，难以让用户理解异常的具体含义。

Method: 开发概念数据集，改进CBM架构生成概念和视觉双重解释，引入人工异常合成管道以减少对罕见异常样本的依赖。

Result: CONVAD在性能上与传统VAD方法相当，但提供了更丰富、基于概念的解释，增强了系统的可解释性和可信度。

Conclusion: 概念瓶颈模型成功应用于VAD，实现了语义解释与定位解释的结合，为VAD系统提供了更深入的洞察力。
...
TL;DR: 提出了一个用于早期森林火灾检测的数据集，通过游戏模拟器合成烟雾和初始火灾图像，并比较了YOLOv7和检测变换器模型在火灾检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有森林火灾检测方法由于缺乏大规模数据集和专门优化的模型，存在漏检问题。本文旨在开发早期预警系统，通过视觉分析检测初始火灾。

Method: 1) 使用Red Dead Redemption 2等游戏模拟器合成包含烟雾羽流和初始火灾的图像数据集；2) 结合已发布的图像构建更全面的数据集；3) 在数据集上比较YOLOv7和不同检测变换器模型的分类和定位性能。

Result: 创建了一个专门用于早期森林火灾识别的数据集，该数据集包含多个烟雾羽流和初始火灾实例，与现有广泛火灾图像数据集形成对比。

Conclusion: 通过游戏模拟器合成数据的方法为早期森林火灾检测提供了有效的数据支持，YOLOv7和检测变换器模型在该任务上表现出良好的检测性能。
...
TL;DR: 本文提出了一种结合深度学习和注意力机制的集成方法，用于从X射线图像中自动检测心脏肥大。该方法使用CNN架构（基于Inception V3模型）和多层注意力机制，能够自动学习特征并选择性关注输入图像的关键区域，实现高精度的心脏肥大诊断。


<details>
  <summary>Details</summary>
Motivation: 医疗影像技术革命性地推动了心血管疾病的诊断，特别是心脏肥大等结构异常的检测。传统方法依赖人工分析，效率较低且易受主观因素影响。因此需要开发自动化的深度学习方法来提高诊断准确性和效率。

Method: 1. 数据收集：收集带标注的各种类型X射线图像
2. 预处理：优化图像质量
3. 核心模型：基于Inception V3的CNN架构
4. 注意力机制：采用多层注意力机制，特别是多头注意力机制，能够自动学习特征并选择性关注输入图像的关键区域
5. 评估：使用准确率、精确度等指标严格评估模型性能
...
Conclusion: 提出的集成深度学习和注意力机制的方法能够有效自动检测心脏肥大，各项性能指标优异，具有显著的临床应用价值。该方法通过注意力机制实现了对关键区域的精准关注，提高了诊断的敏感性和特异性。
...
TL;DR: LungEvaty是一个基于transformer的全肺CT扫描肺癌风险预测框架，无需像素级标注，在9万多个CT扫描上训练，匹配最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着低剂量CT筛查项目的普及，需要可扩展的方法来处理整个肺容积，现有方法要么依赖像素级标注限制可扩展性，要么分析肺部分段削弱性能。

Method: 基于transformer的全肺输入框架，直接从大规模筛查数据学习，可选解剖学注意力引导损失来增强解剖学关注。

Result: 在超过9万个CT扫描（包括2.8万个微调和6000个评估）上训练，仅使用影像数据且无区域监督，匹配最先进性能。

Conclusion: 提供了一个简单、数据高效、完全开源的解决方案，为纵向和多模态肺癌风险预测研究提供了可扩展基础。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器在超出训练长度时存在两个失败模式：模型特定的周期性内容重复和普遍的质量下降。现有方法只关注解决重复问题，忽略了质量下降。

Method: 从注意力图角度分析问题，发现两种失败模式都源于注意力分散。提出UltraViCo方法，通过恒定衰减因子抑制超出训练窗口的token注意力。
...
TL;DR: 提出了PETRG-3D框架，用于自动生成PET/CT报告，解决了功能成像中代谢模式变化和全身体积上下文需求的挑战，在淋巴瘤数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: PET/CT在肿瘤学中至关重要，但扫描仪快速扩张超过了专业人员的可用性，需要自动化报告生成来减轻临床工作负担。功能PET成像相比结构成像面临代谢模式随示踪剂生理变化和需要全身体积上下文而非局部区域解释的独特挑战。
...
TL;DR: HCFSSNet是一种用于学习图像压缩的混合卷积和频率状态空间网络，结合CNN的局部高频细节捕捉能力和状态空间模型的长程建模能力，通过频率感知模块实现更高效的比特分配。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer和状态空间模型在图像压缩中虽然具有强大的长程建模能力，但可能导致结构信息丢失或忽略对压缩至关重要的频率特性，而CNN能有效捕捉局部高频细节但缺乏长程建模。

Method: 提出HCFSSNet混合架构：使用CNN提取局部高频结构；引入视觉频率状态空间块，包含全向邻域状态空间模块和自适应频率调制模块；集成频率Swin Transformer注意力模块进行频率感知的边信息建模。

Result: 在Kodak、Tecnick和CLIC数据集上，HCFSSNet相比VTM锚点分别降低BD率18.06%、24.56%和22.44%，与最近的MambaIC等SSM编解码器相比具有竞争性的率失真性能，同时使用显著更少的参数。

Conclusion: HCFSSNet为未来的学习图像压缩系统提供了一种高效且可解释的混合架构，成功结合了CNN的局部建模能力和状态空间模型的长程建模优势。
...
Method: 使用退化掩码引导流匹配采样，并加入轨迹校正机制来确保与退化输入的一致性，整个过程无需训练。

Result: 在自然和医学数据集上的多种图像修复任务（修复、超分辨率和去噪）中，相比基于扩散和流匹配的参考方法，展现出更优的感知质量和更快的处理时间。

Conclusion: Restora-Flow通过创新的引导采样和校正机制，成功解决了现有流模型在图像修复中的效率和质量平衡问题。
...
TL;DR: 提出了R-TNAG框架，通过黎曼流形映射、时间感知神经常微分方程和注意力机制门控循环单元，处理不规则采样的纵向脑部MRI数据，有效预测阿尔茨海默病进展。


<details>
  <summary>Details</summary>
Motivation: 临床检查的不确定性导致纵向影像数据观测间隔不规则，现有欧几里得空间模型难以充分捕捉不规则采样图像的内在连续性和非线性几何结构。

Method: 首先将高维sMRI特征投影到流形空间以保持疾病进展的内在几何结构，然后使用时间感知神经常微分方程建模观测间的连续演化，同时采用注意力机制黎曼门控循环单元自适应整合历史信息处理不规则间隔。

Result: 实验表明该方法在疾病状态预测和认知评分回归方面均优于现有最优模型，消融研究验证了各模块的互补作用，模型在不同序列长度和缺失数据率下表现稳定，跨数据集验证确认了其鲁棒性。

Conclusion: R-TNAG框架通过联合设计提高了时间一致性，在不规则采样条件下实现了稳健的阿尔茨海默病轨迹预测，具有强大的时间泛化能力和临床适用性。
...
TL;DR: 提出MAP-World框架，通过掩码动作规划和路径加权世界模型实现无先验的多模态规划，避免使用手工锚点或强化学习，在保持实时推理的同时达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶运动规划系统依赖手工锚点或强化学习来选择单一最佳模式，这丢弃了替代未来信息并使优化复杂化，需要一种能处理多模态未来并保持计算效率的方法

Method: 使用掩码动作规划模块将未来自车运动视为掩码序列补全，结合驾驶意图路径作为粗略支架；通过注入噪声生成多样化轨迹查询；轻量级世界模型基于候选轨迹展开未来BEV语义；训练时使用轨迹概率作为离散路径权重计算语义损失的期望

Result: 在NAVSIM数据集上，该方法与基于锚点的方法相当，并在基于世界模型的方法中达到最先进性能，同时避免强化学习并保持实时推理延迟

Conclusion: MAP-World框架成功实现了无先验的多模态规划，能够从完整的合理未来分布中学习，而不依赖单一选择路径，为自动驾驶运动规划提供了更高效和全面的解决方案
...
TL;DR: SKEL-CF是一个从粗到精的框架，用于估计SKEL参数，通过转换器编码器-解码器架构预测并逐步优化相机和SKEL参数，在MOYO数据集上显著优于之前的最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有的参数化3D人体模型如SMPL在运动学方面简化，限制了生物力学真实性。SKEL模型通过解剖学准确的骨骼重新绑定解决了这个问题，但直接估计SKEL参数仍面临训练数据有限、透视模糊和人体关节复杂性等挑战。

Method: 采用基于转换器的编码器-解码器架构，编码器预测粗略的相机和SKEL参数，解码器在连续层中逐步优化这些参数。将现有SMPL数据集4DHuman转换为SKEL对齐版本4DHuman-SKEL，并在管道中显式整合相机建模以缓解深度和尺度模糊问题。

Result: 在MOYO数据集上，SKEL-CF达到85.0 MPJPE / 51.4 PA-MPJPE，显著优于之前的SKEL-based SOTA方法HSMR（104.5 / 79.6）。

Conclusion: SKEL-CF建立了一个可扩展且解剖学忠实的人体运动分析框架，弥合了计算机视觉与生物力学之间的差距。
...
TL;DR: LMMs在描述视频中手与物体交互时，虽然能识别物体和动作，但无法准确定位交互开始/结束的时刻和位置，缺乏对动态场景的深层理解。


<details>
  <summary>Details</summary>
Motivation: 探索大型多模态模型是否真正将语义理解建立在视觉输入上，特别是在手与物体交互场景中定位交互事件的能力。

Method: 创建大规模标注数据集（20K+交互），让250名人工标注者标记接触和分离事件，然后测试两个LMM模型（Qwen-2.5VL和GPT-4o）在短视频中定位这些事件的能力。

Result: 模型能可靠地命名目标物体、识别动作并提供连贯推理，但持续失败于识别交互开始/结束的帧数，也无法在场景中定位事件位置。

Conclusion: 模型在精确定义交互的物理接触时刻和位置方面存在困难，缺乏对动态场景深层理解所需的感知基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注无安全对齐的模型，忽略了现实世界MLLMs需要安全机制来降低风险。安全对齐MLLMs在持续适应中不仅会遭受任务遗忘，还会出现安全性能退化，平衡安全性和任务性能成为关键挑战。

Method: 提出和谐参数适应（HPA）框架，包括：基于关注度的参数分区（将参数分为关注安全性和任务性能两类）、和谐平衡的参数选择（从平衡角度选择需要保留的参数）、正交参数调整（对参数更新施加正交约束以减轻灾难性遗忘）。

Result: 在CVIT基准测试和安全评估数据集上的大量实验表明，HPA比现有基线方法能更好地保持高安全性和减轻遗忘问题。

Conclusion: HPA框架有效解决了安全对齐MLLMs在持续适应中的安全退化问题，实现了安全性和任务性能的更好平衡，为实际应用提供了重要解决方案。
...
### [113] [OmniAlpha: A Sequence-to-Sequence Framework for Unified Multi-Task RGBA Generation](https://arxiv.org/abs/2511.20211)
*Hao Yu,Jiabo Zhan,Zile Wang,Jinglin Wang,Huaisong Zhang,Hongyu Li,Xinrui Chen,Yongxian Wei,Chun Yuan*

Main category: cs.CV

TL;DR: OmniAlpha是首个统一的序列到序列RGBA图像生成和编辑框架，通过创新的MSRoPE-BiL架构和AlphaLayers数据集，在21个任务上超越专业模型。


<details>
  <summary>Details</summary>
Motivation: 解决现有生成模型在RGBA处理上的局限性：专业模型缺乏通用性，统一框架局限于RGB域，需要桥接这一关键差距。

Method: 提出MSRoPE-BiL（新型RoPE方法，具有双向可扩展层轴）的DiT骨干架构，以及AlphaLayers数据集（1000个高质量多层三元组）。

Result: 在AIM-500上实现无掩码抠图的SAD相对减少84.8%，在层条件完成任务中赢得90%以上人类偏好，全面超越专业基线模型。
...
TL;DR: 提出了ADNet，一个大规模多领域异常检测基准，包含380个类别和196,294张图像，揭示了现有方法在扩展到多类别时的性能下降问题，并提出了Dinomaly-m解决方案。
...
Method: 构建ADNet基准，汇集49个公开数据集的380个类别；提出Dinomaly-m，一种上下文引导的专家混合扩展方法，在不增加推理成本的情况下扩展解码器容量。

Result: 现有SOTA方法在单类别设置下达到90.6% I-AUROC，但在扩展到380个类别时降至78.5%；Dinomaly-m达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet为异常检测提供了标准化和可扩展的基准，支持社区扩展跨领域数据集，并为未来异常检测基础模型提供可扩展基础。
...
TL;DR: 首个电池供电的可穿戴瞳孔中心追踪系统，结合事件传感和神经形态处理，实现100Hz双目光学追踪，单眼功耗低于5mW。


<details>
  <summary>Details</summary>
Motivation: 可穿戴平台需要实现鲁棒、高频的眼动追踪，同时保持超低功耗。事件视觉传感器虽具微秒级分辨率和稀疏数据流，但缺乏完全集成的低功耗实时处理方案。

Method: 在商用Speck2f系统芯片上结合事件传感和神经形态处理，采用轻量级坐标解码的低功耗微控制器。开发了具有门控时序解码的不确定性量化脉冲神经网络，针对严格的内存和带宽限制进行优化。

Result: 在新型多用户数据集上验证，可穿戴原型实现100Hz双目光学瞳孔追踪，单眼平均功耗低于5mW。
...
### [116] [Uplifting Table Tennis: A Robust, Real-World Application for 3D Trajectory and Spin Estimation](https://arxiv.org/abs/2511.20250)
*Daniel Kienzle,Katja Ludwig,Julian Lorenz,Shin'ichi Satoh,Rainer Lienhart*

Main category: cs.CV

TL;DR: 提出了一种新颖的两阶段流水线方法，用于从单目视频中精确重建乒乓球3D轨迹和旋转，通过分离前端感知和后端2D到3D提升任务来解决合成数据训练模型在真实世界中的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于合成数据训练的方法难以泛化到真实世界的噪声和不完美的球与球台检测，主要原因是缺乏真实世界视频的3D地面真实轨迹和旋转标注。

Method: 采用两阶段流水线：前端感知任务使用新创建的TTHQ数据集进行2D监督训练，后端2D到3D提升网络仅使用物理正确的合成数据训练，并重新设计提升模型以应对真实世界常见伪影。

Result: 通过集成球检测器和球台关键点检测器，将概念验证的提升方法转化为实用、鲁棒且高性能的端到端应用，用于3D乒乓球轨迹和旋转分析。

Conclusion: 该方法成功解决了从单目视频中精确重建乒乓球3D运动的挑战，通过分离训练策略和鲁棒性设计实现了在真实世界中的有效应用。
...
Conclusion: 该框架为基础模型的可扩展跨视角视频生成铺平了道路，代码和模型将公开发布。
...
### [118] [XiCAD: Camera Activation Detection in the Da Vinci Xi User Interface](https://arxiv.org/abs/2511.20254)
*Alexander C. Jenke,Gregor Just,Claas de Boer,Martin Wagner,Sebastian Bodenstedt,Stefanie Speidel*

Main category: cs.CV

TL;DR: 开发基于ResNet18的轻量级管道，自动检测达芬奇Xi手术系统中摄像头激活状态和位置，为下游手术数据分析任务提供支持。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助微创手术依赖内窥镜视频作为唯一术中视觉反馈。检测摄像头激活状态可提供有价值的元数据，支持工具追踪、技能评估和摄像头控制自动化等下游任务。

Method: 基于ResNet18卷积神经网络开发轻量级管道，在SurgToolLoc数据集上进行微调，并在三个公共数据集上评估，包含超过70,000帧。

Result: 模型在二分类检测主动摄像头方面F1分数达到0.993-1.000，在所有情况下正确定位摄像头区域，无错误多摄像头检测。

Conclusion: 该管道能够可靠、实时地从手术视频中提取摄像头激活元数据，为各种下游应用提供自动化预处理和分析支持。所有代码、模型和标注均已公开。
...
TL;DR: 提出SFA训练免费框架，首个基于Video-LLM的视频文本视觉问答方法，通过自适应扫描视频帧、选择性关注关键区域并直接放大，引导模型注意力到关键线索，在多个数据集上实现新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 视频文本视觉问答任务面临重大挑战，需要模型准确感知和理解不同尺度、方向、清晰度的场景文本，同时有效整合时间和语义上下文来生成精确答案，并识别问题相关文本线索、过滤冗余信息。

Method: SFA训练免费框架，基于Video-LLM，通过自适应扫描视频帧、选择性关注关键区域并直接放大这些区域，引导模型注意力到关键线索。

Result: 在多个公开Video TextVQA数据集上实现新的最先进结果，显著超越先前方法，证明了其有效性和泛化能力。

Conclusion: SFA框架通过模拟人类问答过程，有效解决了视频文本视觉问答中的关键挑战，在多个数据集上表现出色，为Video-LLM在该任务中的应用提供了有效解决方案。
...
TL;DR: GHR-VQA是一个基于场景图的人为中心的视频问答框架，通过构建视频级图结构捕捉人-物交互，使用图神经网络处理并分层整合问题特征，在AGQA数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素的方法难以捕捉视频中复杂的人-物交互关系，需要更结构化、可解释的方法来理解视频内容。

Method: 将视频帧表示为场景图，跨帧连接人节点到全局根节点形成视频级图，使用GNN处理图结构，然后与问题特征在分层网络中整合。

Result: 在AGQA数据集上取得显著性能提升，特别是在对象关系推理方面比现有最佳方法提升7.3%。

Conclusion: 基于图的人为中心方法能有效捕捉视频中的时空动态，提高推理性能和可解释性。
...
TL;DR: 本文介绍了一个在ASNR-MICCAI BraTS-Inpainting 2025挑战赛中获胜的深度学习框架，用于在3D脑肿瘤MRI扫描中合成健康组织，以减轻数据集偏差。


<details>
  <summary>Details</summary>
Motivation: 解决限制深度学习模型在脑肿瘤MRI定量分析中的数据集偏差问题。

Method: 使用U-Net架构训练合成受损区域的修复，并采用随机掩码增强策略来提高泛化能力。
...
Conclusion: 该方法在BraTS-Inpainting 2025挑战赛中排名第一，性能超越了2023和2024年的获胜方案。
...
### [122] [Prompting Lipschitz-constrained network for multiple-in-one sparse-view CT reconstruction](https://arxiv.org/abs/2511.20296)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有稀疏视图CT重建算法的两个主要限制：1）深度展开算法的先验网络难以明确证明满足Lipschitz约束；2）多视图设置下需要训练多个模型，存储成本高。
...
Conclusion: 提出的PromptCT框架成功解决了稀疏视图CT重建中的理论约束和存储效率问题，理论证明了LipNet的Lipschitz连续性及相应迭代算法的收敛性。
...
TL;DR: 提出CT-CIG方法，通过可控文本引导生成逻辑合理的伪装图像，利用VLM标注高质量文本提示，结合轻量控制器和频率交互细化模块提升伪装效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成伪装图像时忽视伪装对象与背景环境的逻辑关系，导致结果不自然，需要解决这一关键问题。

Method: 设计伪装揭示对话机制标注数据集，微调Stable Diffusion并加入轻量控制器指导对象位置形状，使用频率交互细化模块捕捉高频纹理特征。

Result: 通过CLIPScore评估和伪装效果测试，证明生成的文本提示语义对齐，并能产生逼真的伪装图像。

Conclusion: CT-CIG方法能够生成逻辑合理且视觉一致的伪装图像，在语义对齐和伪装效果方面表现优异。
...
TL;DR: 本文针对胶质母细胞瘤的异质性问题，开发了一种基于Vision Transformer的深度学习模型，在BraTS-Path 2025挑战赛中获得了第二名，验证集MCC为0.7064，测试集MCC为0.6509。
...
Conclusion: 我们的结果为基于ViT的组织病理学分析建立了坚实的基线，未来工作将专注于缩小在未见验证数据上观察到的性能差距。
...
### [125] [3D Motion Perception of Binocular Vision Target with PID-CNN](https://arxiv.org/abs/2511.20332)
...
Method: 设计了一个相对较小的PID卷积神经网络，共17层413千参数，通过连接和池化实现了简单实用的特征重用方法，使用模拟随机移动球数据集进行训练和测试。

Result: 实验结果显示预测精度接近输入图像分辨率所能代表的极限，分析了实验结果、误差以及现有不足和改进方向。
...
TL;DR: V-Attack是一种针对大型视觉语言模型的对抗攻击方法，通过操纵transformer注意力块中的值特征（V）来实现精确的局部语义操控，相比现有方法平均提高36%的攻击成功率。
...
Method: 提出V-Attack方法，包含两个核心组件：自值增强模块来优化V的语义丰富度，以及文本引导的值操纵模块，利用文本提示定位源概念并优化到目标概念。

Result: 在LLaVA、InternVL、DeepseekVL和GPT-4o等多种LVLM上的广泛实验表明，V-Attack相比最先进方法平均提高36%的攻击成功率。

Conclusion: V-Attack通过绕过纠缠的补丁特征，实现了高效的语义控制，暴露了现代视觉语言理解中的关键漏洞。
...
TL;DR: 提出了HistoSpeckle-Net深度学习架构，用于从多模光纤散斑中重建结构丰富的医学图像，通过分布感知学习策略和基于直方图的互信息损失，在有限数据和光纤弯曲条件下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在多模光纤成像中主要关注简单数据集，难以应用于复杂的真实世界成像任务，且通常需要大量数据，这在处理多样化复杂图像时更具挑战性。

Method: 开发了包含直方图计算单元和三级特征细化模块的深度学习架构，采用基于直方图的互信息损失和多尺度结构相似性损失，通过空间光调制器和多模光纤构建临床相关数据集。

Result: 在复杂的OrganAMNIST数据集上，HistoSpeckle-Net相比U-Net和Pix2Pix等基线模型实现了更高的保真度，在有限训练样本和不同光纤弯曲条件下均表现出优越性能。

Conclusion: 通过有效重建复杂解剖特征并减少数据需求和光纤扰动影响，HistoSpeckle-Net使多模光纤成像更接近实际临床环境中的部署应用。
...
### [128] [From Passive Perception to Active Memory: A Weakly Supervised Image Manipulation Localization Framework Driven by Coarse-Grained Annotations](https://arxiv.org/abs/2511.20359)
*Zhiqing Guo,Dongdong Xi,Songlin Li,Gaobo Yang*

Main category: cs.CV

TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略平衡标注成本与定位精度，使用知识蒸馏和双引导特征融合模块实现高效部署和准确定位。
...
Method: 提出粗粒度区域标注策略降低标注成本；设计轻量级学生模型通过SAM教师模型进行知识蒸馏；基于人类潜意识记忆机制的双引导特征融合模块，动态结合原型模式与实时观测线索。
...
### [129] [PromptMoG: Enhancing Diversity in Long-Prompt Image Generation via Prompt Embedding Mixture-of-Gaussian Sampling](https://arxiv.org/abs/2511.20251)
*Bo-Kai Ruan,Teng-Fang Hsiao,Ling Lo,Yi-Lun Wu,Hong-Han Shuai*

Main category: cs.CV

TL;DR: 本文研究了文本到图像生成中长提示语导致多样性下降的问题，提出了PromptMoG方法通过在嵌入空间使用高斯混合模型采样来增强多样性，同时保持语义准确性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在处理长提示语时存在保真度-多样性困境，长提示语虽然能增强内容保真度，但往往会抑制生成图像的多样性，导致重复和缺乏创意的输出。

Method: 提出了PromptMoG方法，这是一种无需训练的方法，通过在嵌入空间中对提示语嵌入进行高斯混合模型采样，增加采样熵来增强多样性，同时保持语义不变。

Result: 在四个最先进模型（SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image）上的广泛实验表明，PromptMoG能持续提升长提示语生成的多样性，且不会导致语义漂移。

Conclusion: PromptMoG方法有效解决了长提示语生成中的多样性问题，为文本到图像生成提供了保持语义准确性的多样性增强方案。
...
### [130] [Zoo3D: Zero-Shot 3D Object Detection at Scene Level](https://arxiv.org/abs/2511.20253)
...
<details>
  <summary>Details</summary>
Motivation: 现实环境需要能够识别多样化、未见物体的模型，而现有的开放词汇3D检测器仍依赖训练场景，这限制了其应用范围。

Method: 通过2D实例掩码的图聚类构建3D边界框，使用最佳视图选择和视图一致性掩码生成的开放词汇模块分配语义标签，提供零样本和自监督两种模式。

Result: 在ScanNet200和ARKitScenes基准测试中，Zoo3D的零样本和自监督版本均达到开放词汇3D物体检测的最先进结果，零样本版本甚至优于所有现有自监督方法。
...
### [131] [StableTrack: Stabilizing Multi-Object Tracking on Low-Frequency Detections](https://arxiv.org/abs/2511.20418)
*Matvei Shelukhan,Timur Mamedov,Karina Kvanchiani*

Main category: cs.CV

TL;DR: StableTrack是一种针对低频率检测的多目标跟踪方法，通过两阶段匹配策略和Bbox-Based距离改进跨帧关联，在计算资源受限条件下保持跟踪质量。


<details>
  <summary>Details</summary>
Motivation: 解决当前多目标跟踪方法在计算资源有限条件下无法有效运行的问题，特别是在低频率检测场景下跟踪质量下降的问题。

Method: 提出两阶段匹配策略，使用Bbox-Based距离替代传统马氏距离，并将视觉跟踪集成到卡尔曼滤波器和整体跟踪流程中。

Result: 在MOT17-val数据集上，1Hz低频率检测时HOTA指标提升11.6%，同时在标准MOT17、MOT20和DanceTrack基准测试中与最佳方法保持相当性能。

Conclusion: StableTrack方法有效解决了低频率检测下的多目标跟踪问题，在保持标准性能的同时显著提升了资源受限条件下的跟踪质量。
...
### [132] [The Image as Its Own Reward: Reinforcement Learning with Adversarial Reward for Image Generation](https://arxiv.org/abs/2511.20256)
*Weijia Mao,Hao Chen,Zhenheng Yang,Mike Zheng Shou*

Main category: cs.CV

TL;DR: Adv-GRPO：一种对抗性奖励的RL框架，通过迭代更新奖励模型和生成器来避免奖励破解，使用参考图像和视觉基础模型提供密集视觉奖励，在图像质量和美学方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前RL方法依赖预训练偏好模型输出标量奖励来近似人类偏好，但这些奖励往往无法捕捉人类感知且容易受到奖励破解的影响，即更高分数并不对应更好的图像质量。

Method: 引入Adv-GRPO框架，使用对抗性奖励迭代更新奖励模型和生成器；奖励模型以参考图像作为正样本进行监督；采用图像本身作为奖励，利用参考图像和视觉基础模型（如DINO）提供密集视觉信号而非单一标量。

Result: 在人类评估中，Adv-GRPO在图像质量和美学方面分别以70.0%和72.4%的胜率优于Flow-GRPO和SD3；能够实现分布迁移和灵活的风格定制。

Conclusion: 通过结合参考样本和基础模型奖励，Adv-GRPO在图像质量、美学和任务特定指标上取得一致提升，有效解决了奖励破解问题并生成更高质量的图像。
...
### [133] [Block Cascading: Training Free Acceleration of Block-Causal Video Models](https://arxiv.org/abs/2511.20426)
...
Method: 关键洞见：未来视频块不需要完全去噪的当前块即可开始生成。通过使用部分去噪的上下文启动块生成，将顺序流水线转换为并行级联，多个块同时去噪。

Result: 使用5个GPU利用时间并行性，在所有模型规模上实现约2倍加速：1.3B模型从16加速到30 FPS，14B模型从4.5加速到12.5 FPS。同时消除了KV重缓存的开销(~200ms)。

Conclusion: 广泛的评估验证了从块因果切换到Block Cascading流水线进行推理时，生成质量没有显著损失，为交互式生成提供了高效的解决方案。
...
### [134] [Modality-Balanced Collaborative Distillation for Multi-Modal Domain Generalization](https://arxiv.org/abs/2511.20258)
*Xiaohan Wang,Zhangtao Cheng,Ting Zhong,Leiting Chen,Fan Zhou*

Main category: cs.CV

TL;DR: MBCD是一个多模态领域泛化的协作蒸馏框架，通过自适应模态丢弃、梯度一致性约束和基于权重平均的跨模态蒸馏，解决传统权重平均在多模态场景中因优化速度差异导致的模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统权重平均方法在多模态领域泛化中面临挑战：不同模态优化速度差异导致早期阶段偏向快速收敛的模态，抑制了较慢但互补模态的贡献，阻碍有效的模态融合并导致损失曲面偏向尖锐、泛化性差的极小值。

Method: MBCD框架包含三个关键组件：1）学生模型中的自适应模态丢弃，抑制早期对主导模态的偏向；2）梯度一致性约束，对齐单模态分支与融合表示的学习信号；3）基于权重平均的教师模型进行跨模态蒸馏，将融合知识传递给每个单模态分支。

Result: 在MMDG基准测试上的广泛实验表明，MBCD始终优于现有方法，在不同未见域上实现了更优的准确性和鲁棒性。
...
### [135] [Object-Centric Vision Token Pruning for Vision Language Models](https://arxiv.org/abs/2511.20439)
*Guangyuan Li,Rongzhen Zhao,Jinhong Deng,Yanbo Wang,Joni Pajarinen*

Main category: cs.CV

TL;DR: OC-VTP是一种直接且保证性的视觉token剪枝方法，通过轻量级预训练的对象中心视觉token剪枝器，在不微调任何模型的情况下提升VLM推理效率并保持精度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的视觉token数量多但信息分散，消耗过多不必要的计算资源，现有剪枝方法都是间接且无保证的。

Method: 训练轻量级对象中心视觉token剪枝器，通过最小化从选定token重构原始未剪枝token的误差来保证保留最具代表性的视觉token。

Result: 在各种视觉剪枝比例下，OC-VTP都能帮助主流VLM保持最高的推理精度，同时展现出有趣的解释性。

Conclusion: OC-VTP提供了一种直接且保证性的视觉token剪枝方案，显著提升VLM推理效率而不损失精度。
...
<details>
  <summary>Details</summary>
Motivation: 传统分类方法在图像损坏或训练数据有限等高不确定性条件下性能不佳，需要更稳健的分类方法
...
### [137] [DRL-Guided Neural Batch Sampling for Semi-Supervised Pixel-Level Anomaly Detection](https://arxiv.org/abs/2511.20270)
...
Result: 在MVTec AD数据集上，相比现有方法在F1_max和AUC指标上分别平均提升0.15和0.06，最佳情况下F1_max提升达0.37。
...
### [138] [VKnowU: Evaluating Visual Knowledge Understanding in Multimodal LLMs](https://arxiv.org/abs/2511.20272)
...
### [139] [ScenarioCLIP: Pretrained Transferable Visual Language Models and Action-Genome Dataset for Natural Scene Analysis](https://arxiv.org/abs/2511.20274)
*Advik Sinha,Saurabh Atreya,Aashutosh A,Sk Aziz Ali,Abhijit Das*

Main category: cs.CV

TL;DR: ScenarioCLIP是一个改进的CLIP模型，专门用于处理包含多个对象和关系的复杂场景图像，通过显式建模对象间关系来提升场景分析能力。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型主要关注单对象分类或短描述检索，无法有效处理现实世界中包含多个对象和关系的复杂场景。需要显式建模对象间的关系结构来提升场景理解能力。

Method: 提出ScenarioCLIP模型，接受输入文本、基础关系、输入图像和聚焦区域来突出关系。模型在精心策划的场景数据上预训练，并在特定下游任务上微调。创建了新的数据集，通过扩展现有室内外场景数据并利用语言模型来基础动作、对象和关系。

Result: ScenarioCLIP在各种领域特定任务上展示了强大的零样本和微调性能，在多个场景基准任务上优于基线方法。

Conclusion: ScenarioCLIP通过显式建模对象间关系，有效提升了复杂场景的理解能力，为跨模态检索和细粒度视觉理解任务提供了更好的解决方案。
...
### [140] [DAPointMamba: Domain Adaptive Point Mamba for Point Cloud Completion](https://arxiv.org/abs/2511.20278)
...
<details>
  <summary>Details</summary>
Motivation: 现有领域自适应点云补全方法存在感受野有限或二次复杂度问题，直接应用SSMs会破坏3D点云的空间拓扑和局部几何特征，且缺乏领域无关表示学习设计。

Method: 提出DAPointMamba框架，包含三个新模块：跨域补丁级扫描建立局部几何对应关系；跨域空间SSM对齐通过相似性调制增强空间一致性；跨域通道SSM对齐处理全局语义差距。
...
### [141] [SelfMOTR: Revisiting MOTR with Self-Generating Detection Priors](https://arxiv.org/abs/2511.20279)
...
Method: 基于MOTR类模型实际上是强大检测模型的洞察，提出SelfMOTR跟踪transformer，依赖自生成的检测先验，并通过分析研究开发了有效利用这些隐藏检测能力的实用工具集。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最近的端到端跟踪方法相竞争。

Conclusion: 通过揭示和利用MOTR类模型的隐藏检测能力，SelfMOTR提供了一种有效的端到端跟踪解决方案，无需依赖外部检测器。
...
### [142] [Bootstrapping Physics-Grounded Video Generation through VLM-Guided Iterative Self-Refinement](https://arxiv.org/abs/2511.20280)
...
Method: 采用多模态思维链过程，基于物理不一致性反馈迭代优化提示词，逐步提升生成质量。该方法无需训练且即插即用，可广泛应用于各类视频生成模型。

Result: 在PhyIQ基准测试中，物理智商得分从56.31提升至62.38，显著改善了生成视频的物理一致性。
...
### [143] [Automated Monitoring of Cultural Heritage Artifacts Using Semantic Segmentation](https://arxiv.org/abs/2511.20541)
*Andrea Ranieri,Giorgio Palmieri,Silvia Biasotti*

Main category: cs.CV

TL;DR: 比较U-Net架构与不同CNN编码器在文化遗产裂缝检测中的性能，评估其在OmniCrack30k数据集上的分割效果和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 文化遗产保护需要自动化的裂缝检测方法，通过语义分割技术实现像素级的裂缝识别。

Method: 使用U-Net架构配合不同CNN编码器进行裂缝语义分割，在OmniCrack30k测试集上进行定量评估，并在真实世界雕像和纪念碑上进行定性评估。
...
### [144] [Back to the Feature: Explaining Video Classifiers with Video Counterfactual Explanations](https://arxiv.org/abs/2511.20295)
*Chao Wang,Chengan Che,Xinyue Chen,Sophia Tsoka,Luis C. Garcia-Peraza-Herrera*

Main category: cs.CV

TL;DR: 提出了BTTF框架，用于为视频分类器生成反事实解释，通过优化方法生成物理合理、时间连贯的反事实视频。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实解释方法主要针对图像分类器设计，缺乏生成时间连贯、平滑且物理合理的视频反事实解释的能力。

Method: BTTF框架包含：1）基于输入视频第一帧的条件化初始潜在噪声优化；2）两阶段优化策略在输入视频附近搜索反事实视频；3）渐进式优化策略加速收敛。

Result: 在Shape-Moving、MEAD和NTU RGB+D等视频数据集上的实验表明，BTTF能有效生成有效、视觉相似且逼真的反事实视频。

Conclusion: BTTF框架成功地为视频分类器生成了有意义的反事实解释，揭示了分类器的决策机制。
...
### [145] [New York Smells: A Large Multimodal Dataset for Olfaction](https://arxiv.org/abs/2511.20544)
...
TL;DR: 本文介绍了纽约气味数据集，这是一个包含7000个气味-图像对的大规模多模态数据集，用于机器嗅觉研究。
...
Method: 构建了包含7000个气味-图像对的数据集，涵盖室内外3500个不同物体，比现有数据集多70倍。

Result: 视觉数据能够实现跨模态嗅觉表征学习，学习到的嗅觉表征优于广泛使用的手工特征。

Conclusion: 该数据集为机器嗅觉研究提供了重要资源，展示了视觉数据在嗅觉表征学习中的价值。
...
TL;DR: Flash-DMD是一个新颖的框架，通过时间步蒸馏和联合RL微调，实现快速收敛的高质量扩散模型生成，显著降低训练成本并防止策略崩溃。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的迭代采样过程计算成本高，时间步蒸馏技术虽然能加速生成但训练成本大且会导致图像质量下降，而使用RL进行特定目标微调时存在不稳定和奖励破解问题。

Method: 提出高效的时间步感知蒸馏策略，大幅降低训练成本；引入联合训练方案，在继续时间步蒸馏训练的同时进行RL目标微调，利用蒸馏的稳定损失作为正则化器。

Result: Flash-DMD仅需DMD2 2.1%的训练成本，在少步采样机制下实现最先进的生成质量，在视觉质量、人类偏好和文本图像对齐指标上优于现有方法。

Conclusion: Flash-DMD为训练高效、高保真和稳定的生成模型提供了一个有效范例，显著加速收敛并提升生成质量。
...
### [147] [CrossEarth-Gate: Fisher-Guided Adaptive Tuning Engine for Efficient Adaptation of Cross-Domain Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2511.20302)
...
<details>
  <summary>Details</summary>
Motivation: 现有的参数高效微调方法在处理大规模地球观测任务时往往失效，因为它们无法完全处理遥感数据中固有的多面性和不可预测的域差距（如空间、语义和频率偏移）。

Method: 1. 建立全面的遥感模块工具箱，包含空间、语义和频率模块；2. 开发基于Fisher信息的自适应选择机制，量化每个模块的重要性，动态激活关键模块以最大化适应效果和效率。

Result: 在16个遥感语义分割跨域基准测试中，CrossEarth-Gate达到了最先进的性能，验证了方法的有效性和泛化能力。

Conclusion: CrossEarth-Gate通过模块化工具箱和智能选择机制，成功解决了遥感数据中的多面域差距问题，为遥感领域的参数高效微调提供了有效解决方案。
...
TL;DR: 提出TaCo网络，通过时空语义联合约束增强遥感变化检测，将变化概念化为双时态状态间的语义转换，引入文本引导的转换生成器和时空语义联合约束，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖掩码监督，能有效指导空间定位但对时间语义转换约束有限，导致预测存在语义不一致问题。

Method: 提出TaCo网络，包含文本引导的转换生成器来构建跨时态转换特征，以及包含双时态重构约束和转换约束的时空语义联合约束。

Result: 在六个公共数据集上的广泛实验表明，TaCo在二值和语义变化检测任务中均达到SOTA性能，且推理时不引入额外计算开销。

Conclusion: TaCo通过时空语义联合约束有效解决了遥感变化检测中的语义不一致问题，在多个任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Rectified Flow模型在图像翻译中仍依赖昂贵的多步去噪，阻碍实时应用。虽然CycleGAN-Turbo在预训练扩散模型中实现了单步翻译，但直接应用于RF模型会导致严重收敛问题。

Method: 提出TReFT方法，直接使用预训练DiT或UNet预测的速度作为输出，配合内存高效的潜在循环一致性和身份损失，以及轻量级架构简化。
...
### [150] [IrisNet: Infrared Image Status Awareness Meta Decoder for Infrared Small Targets Detection](https://arxiv.org/abs/2511.20319)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于深度学习的编码器-解码器框架在红外小目标检测中存在静态模式学习的局限性，无法适应不同场景（如昼夜变化、天空/海洋/地面领域）的模式漂移，限制了检测的鲁棒性。

Method: 提出IrisNet框架，通过图像到解码器变换器建立红外图像特征与完整解码器参数之间的动态映射；将参数化解码器表示为保留层次层相关性的结构化2D张量；利用自注意力建模层间依赖关系，通过交叉注意力生成自适应解码模式；集成高频分量增强目标位置和场景边缘信息的感知能力。

Result: 在NUDT-SIRST、NUAA-SIRST和IRSTD-1K数据集上的实验表明，IrisNet实现了最先进的性能。

Conclusion: IrisNet通过动态适应不同红外图像状态的检测策略，有效解决了红外小目标检测中的模式漂移问题，显著提升了检测的鲁棒性和性能。
...
### [151] [AD-R1: Closed-Loop Reinforcement Learning for End-to-End Autonomous Driving with Impartial World Models](https://arxiv.org/abs/2511.20325)
...
TL;DR: 提出了一个基于公正世界模型的端到端自动驾驶后训练策略优化框架，通过反事实合成方法生成碰撞和偏离道路事件的丰富课程，使模型能够准确预测危险，显著减少安全违规。
...
Method: 引入反事实合成数据管道，系统生成可信的碰撞和偏离道路事件课程，构建公正世界模型作为内部批评器，在闭环强化学习框架中查询候选动作的结果。

Result: 在包括新的风险预见基准在内的广泛实验中，模型在预测失败方面显著优于基线方法，在挑战性模拟中大幅减少安全违规。

Conclusion: 教会模型预见危险是构建真正安全和智能自主代理的关键步骤，公正世界模型能够有效提升自动驾驶系统的安全性。
...
### [152] [Evaluating the Performance of Deep Learning Models in Whole-body Dynamic 3D Posture Prediction During Load-reaching Activities](https://arxiv.org/abs/2511.20615)
*Seyede Niloofar Hosseini,Ali Mojibi,Mahdi Mohseni,Navid Arjmand,Alireza Taheri*

Main category: cs.CV

TL;DR: 本研究开发了基于BLSTM和Transformer的深度学习模型，用于预测动态负重活动中的全身人体姿态，并提出了一种通过优化代价函数来保持身体段长度恒定的新方法，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 探索深度学习在动态负重活动中全身人体姿态预测的应用，旨在更好地理解和预测人工物料搬运活动中的运动动态。

Method: 使用双向长短期记忆网络(BLSTM)和Transformer架构训练时间序列模型，输入包括手部负重位置3D坐标、举重技术、搬运技术、体重身高以及任务前25%时长的身体姿态数据，预测剩余75%时长的身体坐标。提出通过优化新代价函数来强制保持身体段长度恒定的方法。

Result: 新代价函数使手臂和腿部模型的预测误差分别降低约8%和21%。Transformer架构在长期性能上比BLSTM模型准确约58%，均方根误差为47.0毫米。

Conclusion: 研究表明，捕捉时间序列依赖性的神经网络在3D运动帧分析中具有重要价值，为理解和预测人工物料搬运活动中的运动动态提供了独特方法。
...
### [153] [ShelfRectNet: Single View Shelf Image Rectification with Homography Estimation](https://arxiv.org/abs/2511.20335)
*Onur Berk Tore,Ibrahim Samil Yalciner,Server Calap*

Main category: cs.CV

TL;DR: 提出基于深度学习的单图像单应性估计框架，使用4点参数化单应性矩阵校正货架图像，在测试集上达到1.298像素的平均角点误差。


<details>
  <summary>Details</summary>
Motivation: 解决零售领域货架监控中单视角图像校正的挑战，传统方法在只有单一视角时难以准确估计单应性变换。

Method: 使用ConvNeXt骨干网络进行特征提取，采用归一化坐标回归提高稳定性，并引入合成单应性变换的数据增强策略解决数据稀缺问题。

Result: 在测试集上平均角点误差为1.298像素，在准确性和推理速度方面均优于传统计算机视觉和深度学习方法。

Conclusion: 该方法为现实世界单视角图像校正提供了鲁棒高效的解决方案，并公开了ShelfRectSet数据集和代码以促进进一步研究。
...
### [154] [AMB3R: Accurate Feed-forward Metric-scale 3D Reconstruction with Backend](https://arxiv.org/abs/2511.20343)
*Hengyi Wang,Lourdes Agapito*

Main category: cs.CV

TL;DR: AMB3R是一个多视图前馈模型，用于度量尺度的密集3D重建，可扩展到视觉里程计和大规模运动恢复结构任务，无需任务特定微调或测试时优化。


<details>
  <summary>Details</summary>
Motivation: 解决密集3D重建任务，并探索如何将单一训练模型扩展到其他3D视觉任务，如视觉里程计和运动恢复结构。

Method: 使用稀疏但紧凑的体素场景表示作为后端，支持几何推理和空间紧凑性，仅针对多视图重建进行训练。

Result: 在相机姿态、深度、度量尺度估计和3D重建方面达到最先进性能，甚至超过基于优化的SLAM和SfM方法。

Conclusion: AMB3R展示了单一模型可无缝扩展到多种3D视觉任务的能力，无需额外调整，在多个基准测试中表现优异。
...
### [155] [MapReduce LoRA: Advancing the Pareto Front in Multi-Preference Optimization for Generative Models](https://arxiv.org/abs/2511.20629)
...
TL;DR: 提出了MapReduce LoRA和Reward-aware Token Embedding (RaTE)两种方法来解决多奖励联合优化中的对齐税问题，在文本到图像、文本到视频和语言任务中均取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习人类反馈中多奖励联合优化时出现的对齐税问题，即优化一个维度时其他维度会退化。
...
TL;DR: STARFlow-V是一个基于标准化流的视频生成模型，通过全局-局部架构和流-分数匹配技术，在视频生成中实现了高质量、时序一致的结果，为标准化流在视频生成领域提供了新的可能性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成领域主要依赖基于扩散的模型，而标准化流在图像生成中取得了进展但在视频生成中应用较少。作者希望探索标准化流在视频生成中的潜力，解决时空复杂性和计算成本高的挑战。

Method: STARFlow-V采用时空潜在空间的全局-局部架构，将因果依赖限制在全局潜在空间，同时保留帧内局部交互。提出流-分数匹配技术增强时序一致性，并使用视频感知的Jacobi迭代方案提高采样效率。

Result: 实验表明STARFlow-V在视觉保真度和时序一致性方面表现强劲，采样吞吐量相对于基于扩散的基线模型具有实用性，支持文本到视频、图像到视频和视频到视频等多种生成任务。

Conclusion: 这是首个证明标准化流能够实现高质量自回归视频生成的证据，为构建世界模型提供了一个有前景的研究方向。
...
### [157] [Material-informed Gaussian Splatting for 3D World Reconstruction in a Digital Twin](https://arxiv.org/abs/2511.20348)
*João Malheiro Silva,Andy Huynh,Tong Duy Son,Holger Caesar*

Main category: cs.CV

TL;DR: 提出仅使用相机的3D重建流程，通过多视角图像进行3D高斯溅射重建，结合视觉模型提取语义材质掩码，转换为带材质标签的网格表面，为现代图形引擎和模拟器提供基于物理的材质属性，实现与LiDAR-相机融合相当的传感器模拟保真度。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR-相机融合方法需要复杂校准，且对玻璃等材质处理不佳。相机能自然捕捉语义和纹理信息，但缺乏LiDAR的精确几何信息。

Method: 使用多视角图像进行3D高斯溅射重建，通过视觉模型提取语义材质掩码，将高斯表示转换为带投影材质标签的网格表面，并分配基于物理的材质属性。

Result: 在仪器化测试车辆内部数据集上验证，利用LiDAR作为反射率地面真值，结合图像相似性指标，显示与LiDAR-相机融合相当的传感器模拟保真度。

Conclusion: 该方法结合了照片级真实重建与基于物理的材质分配，在消除硬件复杂性和校准要求的同时，提供了可比的传感器模拟性能。
...
Result: 该方法能够以低计算成本实现对不同舞蹈风格的稳健分类，无需复杂模型架构。

Conclusion: 可解释的运动表示能够有效捕捉舞蹈风格的细微差别。
...
TL;DR: 提出了人形视觉搜索任务，让智能体在360°全景图像中主动旋转头部搜索物体或路径，并构建了H* Bench基准测试来评估在复杂真实场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法局限于静态图像，忽略了物理体现与3D世界的交互。需要开发能像人类一样高效搜索的具身智能体，同时避免真实世界硬件的限制。

Method: 使用人形智能体在360°全景图像中主动旋转头部进行搜索，通过后训练技术增强开源模型Qwen2.5-VL的能力。

Result: 顶级专有模型在物体和路径搜索中成功率仅约30%。通过后训练，Qwen2.5-VL的物体搜索成功率从14.83%提升到47.38%，路径搜索从6.44%提升到24.94%。

Conclusion: 路径搜索的较低成功率揭示了其内在难度，需要复杂的空间常识。研究展示了前进方向，但也量化了构建能无缝融入人类日常生活的MLLM智能体面临的巨大挑战。
...
### [160] [MotionV2V: Editing Motion in a Video](https://arxiv.org/abs/2511.20640)
*Ryan Burgert,Charles Herrmann,Forrester Cole,Michael S Ryoo,Neal Wadhwa,Andrey Voynov,Nataniel Ruiz*

Main category: cs.CV

TL;DR: 本文提出了一种通过直接编辑稀疏轨迹来修改视频运动的方法，将输入与输出轨迹之间的偏差称为"运动编辑"，结合生成式主干网络实现强大的视频编辑功能。
...
Method: 提出了一个生成"运动反事实"视频对的流程，这些视频对具有相同内容但不同运动，并在该数据集上微调了运动条件视频扩散架构。通过编辑从输入视频提取的稀疏轨迹来实现运动修改。

Result: 在四向头对头用户研究中，该模型相对于先前工作获得了超过65%的偏好率。该方法允许从任意时间戳开始编辑，并能自然传播编辑效果。

Conclusion: 通过直接编辑稀疏轨迹的运动编辑表示，结合生成式主干网络，能够实现强大的视频编辑能力，为视频运动控制提供了新的有效范式。
...
TL;DR: 提出并评估了三个独立的感知模块：用于眼状态检测的CNN、用于面部表情识别的深度CNN和用于语音识别的LSTM网络，在各自数据集上分别达到93.0%、97.8%和96.89%的准确率。


<details>
  <summary>Details</summary>
Motivation: 开发全面的辅助技术需要无缝集成视觉和听觉感知，研究受感知系统核心功能启发，探索模块化架构的可行性。

Method: 采用模块化架构，包含三个独立模块：CNN用于眼状态检测（困倦/注意力）、深度CNN用于面部表情识别、LSTM用于语音说话人识别。
...
TL;DR: 提出GS-Checker方法用于定位3D高斯泼溅模型中的篡改区域，通过引入3D篡改属性和对比机制，无需昂贵的3D标注即可实现精确的篡改定位。
...
Method: 在3D高斯参数中集成3D篡改属性来标记高斯是否被篡改；设计3D对比机制比较高斯间关键属性的相似性；引入循环优化策略来细化3D篡改属性。

Result: 广泛的实验结果证明了该方法在定位篡改3DGS区域方面的有效性。

Conclusion: GS-Checker是一种无需3D监督标签就能准确定位3D高斯泼溅模型中篡改区域的有效方法。
...
### [163] [MedROV: Towards Real-Time Open-Vocabulary Detection Across Diverse Medical Imaging Modalities](https://arxiv.org/abs/2511.20650)
*Tooba Tehreem Sheikh,Jean Lahoud,Rao Muhammad Anwer,Fahad Shahbaz Khan,Salman Khan,Hisham Cholakkal*

Main category: cs.CV

TL;DR: MedROV是首个医学影像实时开放词汇检测模型，通过大规模数据集Omnis和对比学习，能够检测已知和新型结构，性能超越现有最佳基础模型40 mAP50，运行速度达70 FPS。


<details>
  <summary>Details</summary>
Motivation: 传统医学影像目标检测模型采用封闭集范式，无法检测新标签对象。开放词汇目标检测在医学影像中研究不足，主要受限于数据集稀缺和文本-图像对齐弱的问题。

Method: 1) 构建大规模数据集Omnis，包含60万个检测样本，涵盖9种成像模态；2) 采用伪标签策略处理多源数据集的缺失标注；3) 集成预训练基础模型知识；4) 利用对比学习和跨模态表示。

Result: MedROV在医学图像检测中平均绝对提升40 mAP50，超越封闭集检测器3 mAP50以上，运行速度达70 FPS，创下医学检测新基准。

Conclusion: MedROV成功解决了医学影像开放词汇检测的挑战，通过大规模数据集和先进学习策略，实现了对已知和未知结构的高效检测，为医学影像分析开辟了新方向。
...
TL;DR: 提出了一种基于物理学的损失函数(PIL)，通过模拟血管边界间的弹性相互作用来改进脑血管分割的几何一致性和稳定性。
...
Result: 在DIAS和DSCA两个公开基准测试中，PIL损失函数在U-Net、U-Net++、SegFormer和MedFormer等架构上均优于传统损失函数，在敏感性、F1分数和边界一致性方面表现更优。

Conclusion: 将基于物理的边界相互作用融入深度神经网络能够显著提升动态血管成像中血管分割的精度和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要繁琐的手动操作、对野外数据泛化能力不足，或受限于3D形变模型的表达能力有限。

Method: 利用VGGT的点图表示，结合Pixel3DMM通过像素对齐的UV值注入拓扑信息，将点图转换为带拓扑的点云，并提出拓扑感知束调整策略进行融合。

Result: 在单张NVIDIA RTX 4090上，16个视角的图像仅需10秒即可实现高质量重建，在基准测试中达到最先进水平，并对野外数据展现出令人印象深刻的泛化能力。

Conclusion: VGGTFace通过结合3D基础模型和拓扑注入技术，实现了高效、自动且高质量的面部几何重建，解决了现有方法的局限性。
...
TL;DR: FREE是一个用于加速扩散变换器(DiTs)推理的新框架，通过特征级自回归和并行验证实现无损加速，在ImageNet-512²上达到2.25倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器虽然生成质量优秀，但需要长序列去噪轨迹，导致推理延迟高。现有的推测推理方法在DiTs上加速有限，因为验证阶段草稿准确性不足。
...
TL;DR: MultiID是一个无需训练的多ID定制方法，通过ID解耦交叉注意力机制将多个个体身份无缝集成到生成图像中，解决了现有方法的复制粘贴问题和文本控制性差的问题。


<details>
  <summary>Details</summary>
Motivation: 多ID定制比单ID定制更困难，面临两个主要挑战：1）训练时基于裁剪人物区域重建图像导致推理时出现复制粘贴问题；2）文本控制性差，生成结果只是简单组合人物而不考虑文本对齐。

Method: 提出ID解耦交叉注意力机制，将不同ID嵌入注入对应图像区域；引入局部提示、深度引导空间控制和扩展自注意力三种策略增强生成控制性；构建IDBench基准进行评估。

Result: 广泛的定性和定量结果表明MultiID有效解决了上述两个挑战，其性能与基于训练的多ID定制方法相当甚至更好。

Conclusion: MultiID在无需训练的情况下实现了高质量的多ID定制，通过创新的注意力机制和控制策略显著提升了生成质量和文本对齐能力。
...
TL;DR: 提出轨迹反向一致性模型(TBCM)，通过直接从教师模型的生成轨迹中提取潜在表示，消除对外部训练数据的依赖，显著提高蒸馏效率和简化训练过程。
...
Method: 采用自包含的蒸馏范式，从教师模型的生成轨迹中提取潜在表示，无需VAE编码和大规模数据集，通过轨迹提取的样本自然弥合训练与推理之间的分布差距。

Result: 在MJHQ-30k数据集上，一步生成达到6.52 FID和28.08 CLIP分数，相比Sana-Sprint减少约40%的训练时间并节省大量GPU内存，在保持质量的同时展现卓越效率。

Conclusion: TBCM提供了一种高效且简化的蒸馏方法，同时揭示了连续时间一致性蒸馏中的扩散-生成空间差异，为未来蒸馏研究提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以平衡基于文本生成的创意灵活性与显式结构表示提供的对象级可编辑性，需要开发能够同时满足风格多样性、细粒度控制和结构一致性的3D城市生成方法。

Method: 将城市表示为可控布局、资产和材料的组合，采用四阶段流水线，并集成MajutsuAgent支持五种对象级编辑操作，同时构建了包含2D语义布局、高度图、3D建筑资产和PBR材料的多模态数据集。

Result: 实验表明MajutsuCity相比CityDreamer和CityCraft分别将布局FID降低了83.7%和20.1%，在所有AQS和RDR评分中排名第一，在几何保真度、风格适应性和语义可控性方面达到最先进水平。

Conclusion: MajutsuCity在3D城市生成的几何保真度、风格适应性和语义可控性方面确立了新的最先进水平，预期将推动3D城市生成研究的新方向。
...
### [170] [BRIC: Bridging Kinematic Plans and Physical Control at Test Time](https://arxiv.org/abs/2511.20431)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成多样化的运动，但经常产生物理上不可行的输出，导致模拟过程中的执行漂移问题。

Method: BRIC在测试时动态调整物理控制器以适应噪声运动规划，同时通过损失函数保留预训练技能以防止灾难性遗忘，并引入轻量级测试时引导机制在信号空间中引导扩散模型。

Result: 在多种长期任务（包括运动组合、避障和人-场景交互）上验证了BRIC的有效性，在所有任务上都达到了最先进的性能。

Conclusion: BRIC通过结合两种自适应策略，确保在不同环境中实现一致且物理上合理的长期执行，具有高效性和有效性。
...
### [171] [Unleashing the Power of Vision-Language Models for Long-Tailed Multi-Label Visual Recognition](https://arxiv.org/abs/2511.20641)
*Wei Tang,Zuo-Zheng Wang,Kun Zhang,Tong Wei,Min-Ling Zhang*

Main category: cs.CV

TL;DR: 提出CAPNET框架解决长尾多标签视觉识别问题，通过CLIP文本编码器建模标签相关性，结合图卷积网络和可学习软提示，在多个基准数据集上取得显著改进


<details>
  <summary>Details</summary>
Motivation: 现有方法直接从不平衡数据集中推导语义关系，导致尾部类别的相关性不可靠；CLIP的零样本范式针对单标签任务优化，在多标签任务中表现不佳

Method: CAPNET框架：从CLIP文本编码器显式建模标签相关性；图卷积网络进行标签感知传播；可学习软提示优化嵌入；分布平衡的Focal损失和类感知重加权；测试时集成和参数高效微调

Result: 在VOC-LT、COCO-LT和NUS-WIDE等基准数据集上相比最先进方法取得显著改进

Conclusion: CAPNET有效解决了真实世界长尾多标签视觉识别问题，验证了其有效性
...
### [172] [Concept-Aware Batch Sampling Improves Language-Image Pretraining](https://arxiv.org/abs/2511.20643)
...
TL;DR: 本文提出了一种在线概念感知的数据筛选方法DataConcept和CABS框架，通过动态构建包含多样化概念或高频概念的批次来优化视觉语言模型的训练数据选择。


<details>
  <summary>Details</summary>
Motivation: 现有数据筛选方法多为离线和概念无关的，存在数据偏见问题。本文旨在开发更灵活、任务自适应的在线概念感知数据筛选方法。

Method: 提出了DataConcept数据集（1.28亿图像-文本对）和概念感知批次采样框架CABS，包括多样性最大化（CABS-DM）和频率最大化（CABS-FM）两种变体。
...
TL;DR: 该论文提出了人类-人类-物体交互(HHOIs)的新研究问题，开发了HHOIs数据集和生成方法，通过整合人类-物体交互和人类-人类交互模型，实现了基于文本描述的多人类交互生成。


<details>
  <summary>Details</summary>
Motivation: 为了理解人类在特定情境下的复杂交互行为，需要建模多人与周围场景的关系。现有研究主要关注单人与物体的交互，缺乏对多人共享物体交互的建模。

Method: 提出HHOIs概念，构建专用数据集；使用图像生成模型合成HHOI数据；分别训练文本到HOI和文本到HHI的基于分数的扩散模型；开发统一生成框架整合两个模型，通过高级采样过程合成完整HHOIs。

Result: 实验结果表明，该方法能够根据文本描述生成逼真的HHOIs，在多人交互生成方面优于仅关注单人类HOIs的先前方法，并扩展到了多人类运动生成应用。

Conclusion: 提出的HHOIs框架成功解决了多人共享物体交互的建模问题，扩展了多人类交互生成能力，为理解复杂人类行为提供了有效工具。
...
TL;DR: ZoomSearch是一个无需训练、即插即用的超高分遥感视觉问答管道，通过自适应多分支缩放搜索和布局感知补丁重组来解决超高分辨率遥感图像处理中的计算和细节保留问题。
...
Method: ZoomSearch将'看哪里'与'如何回答'解耦，结合自适应多分支缩放搜索（在图像块上进行分层搜索以定位查询相关区域）和布局感知补丁重组（将选定块重新组织为紧凑、布局忠实的画布）。

Result: 在LLaVA-ov上集成ZoomSearch后，在LRS-VQA和MME-RealWorld-RS基准测试中分别比基线提高了26.3%和114.8%，同时推理效率比之前基于搜索的方法快20%~44%。

Conclusion: ZoomSearch在超高分辨率遥感视觉问答任务中实现了最先进的准确性和更高的推理效率，证明了其解耦方法的有效性。
...
TL;DR: AlignBench是一个评估图像-文本对齐的新基准，通过评估多样化模型生成的详细图像-描述对来衡量细粒度对齐能力，揭示了CLIP模型在组合推理方面的局限性和检测器的系统性偏差。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要依赖基于规则的扰动或简短描述，无法有效衡量细粒度的图像-文本对齐能力，需要更全面的评估方法。

Method: 构建AlignBench基准，使用多样化图像到文本和文本到图像模型生成详细的图像-描述对，并对每个句子进行正确性标注，直接评估视觉语言模型作为对齐评估器的能力。

Result: 评估发现：(i) CLIP模型在组合推理方面几乎盲目；(ii) 检测器系统性地对早期句子评分过高；(iii) 检测器表现出强烈的自偏好，偏向自身输出并损害检测性能。

Conclusion: AlignBench为图像-文本对齐提供了新的评估指标，揭示了现有模型的局限性，并为改进视觉语言模型的对齐能力提供了重要见解。
...
TL;DR: HBridge提出一种非对称H形架构，通过选择性桥接中间层来优化多模态生成，减少40%注意力共享，提高效率并增强生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型采用对称设计，但由于模态差异导致性能欠佳，需要更优化的架构来充分利用预训练先验。

Method: 采用非对称H形架构，选择性桥接中间层，解耦浅层和深层模态特定表示，引入语义重建令牌来增强跨模态一致性。

Result: 在多个基准测试中表现出有效性和优越性能，建立了统一多模态生成的新范式。

Conclusion: HBridge通过非对称设计和选择性桥接策略，显著提升了多模态生成的效率和质量。
...
TL;DR: 提出了Mistake Attribution (MATT)任务，用于细粒度理解第一人称视频中的人类错误，通过MisEngine数据引擎自动构建错误样本，并开发了MisFormer统一模型进行错误归因。
...
Method: 开发MisEngine数据引擎自动构建错误样本，提出MisFormer统一注意力模型，在语义、时间和空间维度进行错误归因。

Result: 构建了EPIC-KITCHENS-M和Ego4D-M数据集，比现有错误数据集大两个数量级；MisFormer模型在多个基准测试中优于现有方法。

Conclusion: MATT任务和MisEngine/MisFormer框架为细粒度错误理解提供了有效解决方案，显著提升了错误归因性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型缺乏明确的物理可控性和合理性，物理渲染方法在建模复杂物理属性和控制长时间序列行为方面存在挑战。

Method: 两阶段方法：1) 通过部件感知的物理属性重建估计图像中所有物体的静态初始物理属性；2) 通过时间指令和物理可编辑模拟合成高质量视频。
...
TL;DR: ReaDe是一个通用的、模型无关的解释器，能将模糊的用户指令转换为精确的视频生成规范，通过"先推理后描述"范式提升视频生成的指令忠实度和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前扩散变换器虽然提升了视频质量和时序一致性，但实际可控性有限。用户输入的简洁性、模糊性与训练时使用的详细提示之间存在不匹配，导致意图输出偏差。

Method: 采用"先推理后描述"范式：先分析用户请求识别核心需求并解决模糊性，然后生成详细指导。通过两阶段优化训练：推理增强监督提供逐步解析轨迹和密集描述，多维奖励分配器实现稳定反馈驱动的自然风格描述精炼。

Result: 在单条件和多条件场景下的实验显示，在指令忠实度、描述准确性和下游视频质量方面均获得一致提升，对推理密集型和新输入具有强泛化能力。
...
TL;DR: DINO-Tok是一个基于DINO的视觉分词器，通过整合浅层细节特征和深层语义特征，在统一的信息完整潜在空间中实现高保真重建，显著优于现有分词器。
...
Method: 整合DINO的浅层细节特征和深层语义特征，提出全局PCA重加权机制来稳定向量量化并保护关键信息。

Result: 在ImageNet 256×256上达到28.54 PSNR（自编码）和23.98 PSNR（VQ建模），创下最先进的重建性能。

Conclusion: 将预训练视觉模型（如DINO）适配用于分词能够实现语义对齐和高保真的潜在表示，推动下一代视觉生成模型发展。
...
TL;DR: 该论文提出了VQ-VA World框架，用于训练开源模型实现视觉问答-视觉回答（VQ-VA）能力，即根据视觉问题生成图像而非文本回答。


<details>
  <summary>Details</summary>
Motivation: 让开源模型具备像NanoBanana和GPT-Image等专有系统那样的视觉问答-视觉回答能力，填补开源领域在此方面的空白。

Method: 构建数据中心的框架，通过代理管道进行大规模、有针对性的数据构建，爬取了约180万高质量的交错图像-文本样本用于模型训练。

Result: 使用VQ-VA World数据训练的LightFusion模型在IntelligentBench基准上达到53.06分，显著超越之前最好的开源基线（7.78分），并大幅缩小与领先专有系统的差距（NanoBanana 81.67分，GPT-Image 82.64分）。
...
TL;DR: 本文提出ImageCritic，一种参考图像引导的后编辑方法，通过注意力对齐损失和细节编码器来解决生成图像中的细节不一致问题。
...
Method: 构建参考-退化-目标三元组数据集，通过VLM选择和显式退化模拟常见不准确性；设计注意力对齐损失和细节编码器来精确纠正不一致性；集成到智能体框架中进行多轮局部编辑。

Result: 大量实验表明，ImageCritic在各种定制生成场景中能有效解决细节相关问题，相比现有方法有显著改进。

Conclusion: ImageCritic通过参考引导的后编辑方法成功解决了生成图像中的细节不一致问题，为定制生成任务提供了有效解决方案。
...
TL;DR: Wanderland是一个真实到仿真的框架，用于解决具身AI中可复现闭环评估的瓶颈，通过多传感器捕获、可靠重建、精确几何和鲁棒视图合成，为开放世界具身导航提供可信测试平台。


<details>
  <summary>Details</summary>
Motivation: 解决具身AI中可复现闭环评估的主要瓶颈，现有视频-3DGS方法存在较大的视觉和几何仿真到真实差距，不适合基准测试。

Method: 引入Wanderland真实到仿真框架，包含多传感器捕获、可靠重建、精确几何和鲁棒视图合成，并策划了室内外城市场景的多样化数据集。

Result: 系统展示了仅图像流程扩展性差、几何质量影响新视图合成，以及这些因素对导航策略学习和评估可靠性的不利影响。

Conclusion: Wanderland为开放世界具身AI的可复现研究建立了新基础，其丰富的原始传感器数据还可用于3D重建和新视图合成模型的基准测试。
...
TL;DR: ShapeGen通过改进3D表示和监督、分辨率提升以及线性变换器优势，实现了高质量的图像到3D形状生成，解决了现有方法缺乏细节、表面过度平滑和薄壳结构碎片化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D形状生成方法存在缺乏精细细节、表面过度平滑和薄壳结构碎片化的问题，导致生成的3D资产无法达到艺术家期望的标准。

Method: 通过3D表示和监督改进、分辨率扩展以及利用线性变换器的优势来实现高质量的图像到3D生成。

Result: ShapeGen在图像到3D生成方面实现了显著飞跃，建立了新的最先进性能，生成的资产可以无缝集成到3D管道中。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视频模型具有强大的时间连贯性先验，但其动态范围受限于连续训练数据的约束。通过将图像数据的丰富多样性注入到这一连贯的时间框架中，可以生成既保持自然过渡又具有更广泛动态范围的图像集。

Method: 提出iMontage框架，采用优雅且最小侵入性的适应策略，配合定制化的数据整理过程和训练范式，使模型获得广泛的图像处理能力而不破坏其原有的运动先验。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视频空间推理方面仍落后于人类水平，主要存在语义-几何不对齐和缺乏持久3D记忆两个挑战。

Method: 提出VLM²模型，采用双记忆模块：工作记忆作为滑动窗口关注即时上下文，情景记忆整合存储关键长期信息，实现固定计算成本下的高效长时程空间推理。

Result: 在多个基准测试中，VLM²在仅使用视频输入的模型中达到了最先进的性能。

Conclusion: VLM²显著推进了视觉空间智能的前沿，通过持久记忆和视图一致的3D感知表示有效解决了视频空间推理的关键挑战。
...
TL;DR: PixelDiT是一个单阶段端到端的像素空间扩散变换器模型，消除了传统两阶段流程中对自编码器的依赖，直接在像素空间学习扩散过程，在图像生成质量上超越了现有像素生成模型。


<details>
  <summary>Details</summary>
Motivation: 传统的潜在空间扩散变换器依赖两阶段流程，预训练的自编码器会引入有损重建，导致误差累积并阻碍联合优化。为了解决这些问题，需要开发直接在像素空间工作的单阶段模型。

Method: PixelDiT采用完全基于变换器的架构，包含双层级设计：补丁级DiT捕捉全局语义，像素级DiT细化纹理细节，实现像素空间扩散模型的高效训练并保留精细细节。

Result: 在ImageNet 256x256上达到1.61 FID，大幅超越现有像素生成模型。扩展到文本到图像生成并在1024x1024分辨率预训练后，在GenEval上达到0.74，在DPG-bench上达到83.5，接近最佳潜在扩散模型性能。

Conclusion: 有效的像素级令牌建模对于像素扩散的成功至关重要，PixelDiT证明了直接在像素空间进行端到端扩散建模的可行性，在保持高质量生成的同时消除了对自编码器的依赖。
...
Method: 引入轻量级跨视图模块（CvM），通过跨视图信息交换和相关性捕捉来注入几何一致性，该模块与MTL编码器特征集成进行多任务预测。

Result: 在NYUv2和PASCAL-Context数据集上的广泛实验表明，该方法能有效提升现有MTL方法的性能。

Conclusion: 通过集成跨视图相关性作为几何一致性，可以显著改善多任务密集预测任务的性能，该方法架构无关且适用于单视图和多视图数据。
...
TL;DR: DPP-GRPO是一个用于多样化视频生成的框架，通过结合行列式点过程和组相对策略优化理论，在保持提示对齐和感知质量的同时显著提升文本到视频模型的生成多样性。
...
Method: 将多样化视频生成建模为集合级策略优化问题，使用DPPs对冗余样本施加递减回报，并通过GRPO在候选集上提供组级反馈，形成一个即插即用且模型无关的框架。

Result: 在WAN和CogVideoX上实现的方法在VBench、VideoScore和人类偏好研究中一致提升了视频多样性，同时保持了提示保真度和感知质量。

Conclusion: DPP-GRPO框架有效解决了文本到视频模型的多样性问题，为未来研究提供了新的基准数据集和代码支持。
...
TL;DR: LocateAnything3D是一个将3D检测转化为下一个token预测问题的视觉语言模型方法，通过链式视觉推理实现开放词汇的3D目标检测，在Omni3D基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型擅长2D描述和定位，但缺乏多目标3D检测能力。模型需要在3D世界中感知和定位物体才能有效行动。

Method: 提出链式视觉序列方法：先在2D图像中检测物体，然后推断其距离、尺寸和姿态。采用由易到难的课程学习策略，按近到远顺序处理物体，并分解3D框预测为中心点、尺寸和旋转角度。

Result: 在Omni3D基准上达到49.89 AP_3D，比之前最佳方法提升15.51个绝对点，即使基线使用真实2D框。在零样本泛化到未见类别时表现出强鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前自回归视频扩散模型受限于3D-RoPE的有限时间视野、长序列生成时提示响应缓慢，以及无法在单次生成中实现电影式场景切换。

Method: 提出三个组件：Block-Relativistic RoPE（将时间编码重构为移动局部参考系）、KV Flush（通过保留全局sink和最后生成帧来更新KV缓存）、RoPE Cut（在时间RoPE坐标中引入受控不连续性）。

Result: 实验表明∞-RoPE在VBench评分上持续超越之前的自回归模型，实现了无限视野、可控和电影化的视频生成。

Conclusion: ∞-RoPE为无限视野、可控和电影化视频扩散提供了一个无需训练的推理基础框架。
...
TL;DR: RubricRL是一个基于评分标准的强化学习框架，通过分解式检查表评估细粒度视觉标准，提供可解释和可组合的奖励设计，用于文本到图像生成模型的对齐。
...
Conclusion: RubricRL为文本到图像架构的可解释RL对齐提供了灵活且可扩展的基础。
...
Conclusion: 可验证的多跳推理不需要大规模模型，而是需要结合符号结构与学习表示的适当架构归纳偏差。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在低资源语言中生成自解释的普及，需要评估这些解释的忠实性，特别是在波斯语等低资源语言的情感分类任务中。

Method: 通过比较LLM识别的影响词与人类标注者的判断，使用基于token级对数概率的置信度评分来评估忠实性，并测试两种提示策略（预测-解释和解释-预测）。
...
Method: 回顾现有证据，提出认知神经科学的最新进展为直接检验该假说提供了概念基础和方法。

Result: 论证了语言系统处理能力有限，深度理解需要其他脑区的参与，包括感知、运动、心理模型构建和记忆存储区域。

Conclusion: 这一新策略为揭示语言理解的认知和神经基础开辟了道路，强调了多脑区协作在深度语言理解中的关键作用。
...
TL;DR: 评估大语言模型在情绪心智理论中的性别偏见，并比较不同去偏策略的效果。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的广泛应用，评估和确保其公平性变得至关重要。本研究关注情绪心智理论领域的性别偏见问题。

Method: 通过向模型描述人物及其环境并询问"这个人感觉如何？"来评估偏见，并测试了多种去偏策略，包括基于训练和基于提示的方法。

Result: 研究发现，实现有意义的偏见减少需要基于训练的方法，而仅依赖推理时提示工程等方法是无效的。

Conclusion: 在大语言模型的情绪心智理论应用中，需要采用基于训练的去偏干预措施来有效减少性别偏见。
...
TL;DR: 本文介绍了NRC VAD Lexicon v2，这是一个包含10,000个英语多词表达及其组成词的情感评分的词典，扩展了2018年发布的v1版本，新增了25,000个单词条目。


<details>
  <summary>Details</summary>
Motivation: 现有的情感词典如NRC VAD Lexicon主要关注单个单词的情感维度，缺乏对多词表达的情感分析。随着语言使用变化，需要更新词典覆盖范围并增加多词表达的情感评分。

Method: 通过人工标注的方式，对10,000个英语多词表达及其组成词进行情感维度（效价、唤醒度、支配度）评分，并扩展了自2018年以来更常用的单词覆盖。

Result: 新词典包含10,000个多词表达和25,000个单词的情感评分，数据具有高可靠性。研究发现多词表达具有显著的情感特性，并探讨了多词表达的情感组合性。

Conclusion: NRC VAD Lexicon v2为NLP、心理学、公共卫生、数字人文和社会科学等领域的研究提供了重要的情感分析资源，支持对多词表达情感特性的深入研究。
...
TL;DR: R2R是一个领域感知的检索增强生成重排序框架，通过动态专家路由和两阶段训练策略解决领域特定重排序中的表面形式过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 在金融、法律等高风险领域，通用重排序模型缺乏领域特定知识，而简单微调会导致表面形式过拟合和灾难性遗忘。

Method: 采用动态专家路由和两阶段训练策略（EAG），通过掩码最具预测性的表面线索，强制重排序器学习领域不变的相关性模式。使用轻量级潜在语义路由器从冻结的主干解码器中选择最优LoRA专家。
...
Method: 利用情感表情符号和情感词汇进行语言无关的情感标注

Result: 英语推文标注准确率66%，Sepedi推文69%，Setswana推文63%，平均只需修正34%的自动标注

Conclusion: 该方法能有效减少手动标注工作量，为低资源语言情感分析提供可行方案
...
Method: 提出PersonaPulse框架，利用LLMs对人格特质的固有知识迭代增强角色扮演提示，同时整合情境响应基准作为评分工具，确保更真实和情境化的评估来指导优化过程。

Result: 定量评估显示PersonaPulse生成的提示优于基于心理学研究人格描述的先前工作；发现模型大小与人格建模的关系；对于某些人格特质，可以通过暂停优化过程来部分控制人格激发程度。

Conclusion: 提示优化在塑造LLMs中人格表达方面至关重要，为未来自适应AI交互研究提供了宝贵见解。
...
TL;DR: 提出了一种结合图像数据增强技术的网格标记方法，用于改进不连续命名实体识别，解决了传统方法在跨句子不连续实体上的分割和遗漏问题。
...
Conclusion: 该方法有效解决了不连续实体识别中的分割和遗漏问题，显著提升了识别性能。
...
Result: 零样本提示在检测任务中召回率低，静态随机示例提示提高了召回率但增加了假阳性率，检索增强动态提示在所有模型中降低了约15%的假阳性率，在错误句子检测中提高了5-10%的召回率，并生成了更准确的修正。

Conclusion: 检索增强动态提示在多样化的大型语言模型中表现优于零样本和静态随机示例提示，使用检索示例提高了检测准确性、减少了假阳性，并增强了医疗错误修正的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估细粒度API选择，缺乏对模型在不同应用间进行推理和选择能力的评估，而应用选择是CUAs有效运行的基础能力。

Method: 开发了新颖的用户任务生成流程，创建真实、多样且语义基础的用户意图，并建立了统一的评估协议，涵盖随机、启发式、零样本、少样本和检索增强设置。

Result: 对闭源和开源大语言模型的广泛实验显示，即使在最强大的模型中，应用选择仍存在系统性弱点，难以做出一致的应用选择决策。
...
TL;DR: 论文通过合成基准测试发现，即使在没有语言统计偏见的条件下，因果Transformer模型仍存在显著的方向性优化差距，表明方向性学习困难是架构本身的内在特性。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer模型在自然语言处理中表现出的"反转诅咒"问题，明确方向性学习失败是源于语言统计特性还是模型架构本身。

Method: 使用完全合成的、熵可控的基准测试，构建零条件熵的前向任务和具有分析确定熵底线的逆任务，在不同分支因子K下训练GPT-2模型。

Result: 即使从头训练的GPT-2模型也表现出强烈的方向性优化差距（如K=5时1.16 nats），远大于在相同数据上训练的MLP。预训练初始化改变了优化行为但未消除差距，LoRA在高熵逆映射中遇到容量墙。

Conclusion: 因果Transformer训练中存在固有的方向性摩擦，这种摩擦在移除语言先验、词频和语料级时间不对称性后仍然存在，表明反转任务对Transformer来说本质上更难。
...
TL;DR: 提出一个统一的多类别分类框架，用于从社交媒体数据中检测10种不同的心理健康和网络欺凌类别。通过端到端微调的MentalBERT模型表现最佳，并开发了结合SHAPLLM解释框架的原型仪表板。
...
Method: 从Twitter和Reddit收集数据集，采用"分割后平衡"流程，在平衡数据上训练，在真实的不平衡测试集上评估。比较了传统词汇模型、混合方法和端到端微调transformer模型。

Result: 端到端微调对性能至关重要，领域适应的MentalBERT成为最佳模型，准确率达到0.92，宏观F1分数为0.76，超越了通用模型和零样本LLM基线。

Conclusion: 该系统被定位为人工参与循环的筛查辅助工具而非诊断工具。工作为在线安全和计算心理健康的交叉领域提供了稳健基线，强调了未来对多标签、临床验证数据集的需求。
...
Method: 利用双曲表示开发了Online-PVLM框架，实现免训练的概念嵌入生成，支持测试时的在线概念学习。

Result: 在包含1,292个概念和30K+实例的OP-Eval基准测试中表现出最先进的性能。

Conclusion: 该框架使个性化视觉语言模型更具可扩展性和效率，解决了在线概念学习的关键挑战。
...
TL;DR: 提出了MTA框架，通过合并-适应方法解决个性化大语言模型的存储扩展和稀疏数据问题，包含元LoRA银行构建、自适应LoRA融合和少样本个性化LoRA堆叠三个阶段。


<details>
  <summary>Details</summary>
Motivation: 现有为每个用户微调单独模块的方法存在存储成本随用户数量线性增长的问题，且对于数据稀疏的用户性能不佳，需要更可扩展和高效的个性化方法。

Method: MTA框架：1) 构建共享元LoRA银行，预训练元个性化特征；2) 自适应LoRA融合，动态合并相关锚点元LoRA合成用户特定LoRA；3) 少样本个性化LoRA堆叠，在合并LoRA上应用超低秩轻量LoRA模块进行微调。

Result: 在LaMP基准测试上的广泛实验表明，该方法在多个任务上优于现有最先进方法。

Conclusion: MTA框架通过合并-适应方法有效解决了PLLMs的存储扩展和稀疏数据问题，实现了更好的个性化性能。
...
TL;DR: 提出BiasPrompting框架，通过让LLMs为所有选项生成推理并综合评估，提升多项选择题的推理能力


<details>
  <summary>Details</summary>
Motivation: 现有方法在多项选择题任务中缺乏对选项的上下文解释和全面探索，限制了模型的推理能力

Method: 包含两个阶段：1)为每个选项生成支持性推理；2)综合所有推理选择最合理答案

Result: 在五个广泛使用的多项选择题基准测试中显著提升性能，特别是在现有方法表现不佳的复杂问题上
...
TL;DR: SSA是一种统一的稀疏注意力训练框架，通过双向对齐机制解决现有稀疏注意力方法中的梯度更新缺陷问题，在保持高性能的同时实现更强的稀疏性。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法存在一个关键悖论：虽然旨在近似全注意力，但产生的注意力稀疏性反而低于全注意力模型，这限制了其有效性。作者将此归因于梯度更新缺陷——在稀疏训练中被排除的低秩键值对既没有前向贡献也没有反向梯度，因此无法学习适当的抑制。

Method: 提出SSA（稀疏稀疏注意力）训练框架，同时考虑稀疏和全注意力，在每一层强制执行双向对齐。这种设计保留了所有token的梯度流，同时明确鼓励稀疏注意力输出与其全注意力对应物对齐，从而促进更强的稀疏性。

Result: SSA在多个常识基准测试中，在稀疏和全注意力推理下都实现了最先进的性能。此外，SSA使模型能够平滑适应不同的稀疏性预算；随着允许更多token参与注意力，性能持续提升，支持在推理时灵活权衡计算与性能。

Conclusion: 原生稀疏注意力训练通过减轻sink区域注意力值的过度分配，意外地改善了长上下文外推能力，其中SSA表现出最强的外推能力。
...
TL;DR: EM2LDL是一个多语言语音语料库，专门用于通过标签分布学习进行混合情感识别，包含英语、普通话和粤语的表达性话语，捕捉多语言环境中的语码转换现象。


<details>
  <summary>Details</summary>
Motivation: 解决现有情感语料库主要为单语言和单标签的局限性，这些语料库缺乏语言多样性、无法建模混合情感且生态效度不足。

Method: 构建包含英语、普通话和粤语的多语言语料库，整合来自在线平台的自发情感表达，使用32个类别的细粒度情感分布进行标注，并采用自监督学习模型进行实验基线评估。

Result: 实验基线显示，在说话人独立的性别、年龄和个性评估中表现稳健，其中HuBERT-large-EN模型取得了最佳结果。

Conclusion: EM2LDL通过整合语言多样性和生态效度，为探索多语言环境中的复杂情感动态提供了平台，为开发适应性强的共情系统提供了多功能测试平台。
...
<details>
  <summary>Details</summary>
Motivation: 传统发音错误检测方法需要训练评分模型或音素级模型，过程复杂且需要大量标注数据，希望开发一种无需训练的简化方法。

Method: 基于预训练自动语音识别模型，结合检索技术构建训练免费框架，无需音素特定建模或额外任务特定训练。

Result: 在L2-ARCTIC数据集上实验，F1分数达到69.60%，优于传统方法，同时避免了模型训练的复杂性。

Conclusion: 该方法提供了一种高效、简化的发音错误检测解决方案，无需复杂训练过程即可实现准确的错误检测与诊断。
...
TL;DR: 本文探索使用大型语言模型（GPT-4.1、Gemini-2.5、LLaMA-4）结合少样本策略来解决低资源印度语言的语法错误校正问题，在多个印度语言上取得了领先结果。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的模型和大规模标注数据显著提升了英语等高资源语言的语法错误校正性能，但大多数印度语言由于资源有限、语言多样性和复杂形态学，GEC仍然是一个挑战性任务。

Method: 采用基于提示的方法，结合零样本和少样本策略，使用最先进的大型语言模型，并通过精心设计的提示和轻量级适应来增强跨多个印度语言的校正质量。
...
Conclusion: 研究结果表明提示驱动的NLP技术非常有效，并强调了大规模LLMs在弥合多语言GEC资源差距方面的潜力。
...
TL;DR: 介绍了首个公开可用的吉尔吉斯语单语BERT模型KyrgyzBERT，该模型有3590万参数，使用针对吉尔吉斯语形态结构设计的自定义分词器。通过创建kyrgyz-sst2情感分析基准进行评估，KyrgyzBERT在该数据集上微调后达到0.8280的F1分数，与比其大五倍的mBERT模型表现相当。


<details>
  <summary>Details</summary>
Motivation: 吉尔吉斯语作为低资源语言，缺乏基础的NLP工具。为了填补这一空白，需要开发专门针对吉尔吉斯语的语言模型。

Method: 开发了KyrgyzBERT模型，包含3590万参数，使用专门为吉尔吉斯语形态结构设计的自定义分词器。通过翻译斯坦福情感树库并手动标注完整测试集，创建了kyrgyz-sst2情感分析基准数据集。

Result: KyrgyzBERT在kyrgyz-sst2数据集上微调后获得0.8280的F1分数，与比其大五倍的多语言BERT模型表现相当。

Conclusion: KyrgyzBERT作为首个公开的吉尔吉斯语单语BERT模型，为吉尔吉斯语NLP研究提供了重要基础工具。所有模型、数据和代码都已发布以支持未来研究。
...
TL;DR: REFLEX是一个基于大语言模型的自优化事实核查范式，通过角色扮演对话和对比激活向量，在无需外部知识的情况下提升判决准确性和解释质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有LLM事实核查方法过度依赖外部知识源导致的延迟、幻觉问题，以及可靠性和实时性不足的挑战。

Method: 将事实核查重构为角色扮演对话，联合训练判决预测和解释生成，通过提取骨干模型与微调变体的对比激活对来构建引导向量，分离真相的风格与实质。

Result: 在真实数据集上超越先前方法，仅用465个自优化训练样本就达到最先进性能，解释信号能有效指导无解释能力的模型，提升达7.57%。
...
TL;DR: SpecFormer是一种新颖的架构，结合了单向和双向注意力机制，在低验证资源和低调度成本下实现LLM推理加速，无需依赖大型前缀树。


<details>
  <summary>Details</summary>
Motivation: 现有的推测解码方法假设有大量可用计算能力，但在批处理等主流推理系统中，可用空闲计算能力被压缩。因此，在低验证资源和低调度成本下进行推测解码成为重要研究问题。
...
TL;DR: LLMs能够编码类比关系但应用能力有限，研究发现模型能表示实体间关系但难以应用到新情境，通过隐藏表示修补可改善推理，成功案例显示结构对齐而失败案例显示对齐缺失。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否能编码高层次关系概念并通过结构化比较应用到新情境，了解模型与人类类比推理的异同。

Method: 使用比例类比和故事类比测试LLMs，分析隐藏层表示，通过策略性修补关键token位置的隐藏表示来促进信息传递。

Result: LLMs能有效编码类比实体间的关系，但难以将关系应用到新实体；成功推理时存在强结构对齐，失败时对齐缺失或错位；隐藏表示修补可在一定程度上改善推理。
...
TL;DR: BengaliFig是一个针对孟加拉语的挑战数据集，包含435个来自孟加拉口头和文学传统的谜语，用于评估LLM在低资源文化背景下的隐喻和文化推理能力。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在低资源语言中处理比喻和文化推理的能力，因为现有基准测试在这方面存在不足。

Method: 创建包含435个孟加拉谜语的数据集，每个项目标注五个维度（推理类型、陷阱类型、文化深度、答案类别、难度），并通过约束感知的AI辅助流程自动转换为多项选择题格式。
...
TL;DR: 提出了一种新颖的面向任务的词干提取方法评估框架，包含三个评估维度：词干提取效用(SES)、下游任务影响(MPD)和语义相似性(ANLD)，通过比较孟加拉语和英语词干提取器的性能，揭示了高效用可能源于有害的过度词干提取。


<details>
  <summary>Details</summary>
Motivation: 当前词干提取方法评估存在局限性，无法捕捉过度词干提取造成的潜在危害，需要开发新的评估方法来全面评估词干提取方法。

Method: 提出包含三个方面的综合评估框架：1)使用词干提取效用分数(SES)评估效用；2)使用模型性能差异(MPD)评估下游任务影响；3)使用平均归一化莱文斯坦距离(ANLD)评估语义相似性。

Result: 孟加拉语词干提取器获得最高SES(1.67)，但ANLD显示这是有害的过度词干提取(ANLD=0.26)，导致下游性能下降；英语词干提取器获得中等SES(1.31)但语义距离安全(ANLD=0.14)，对下游性能有积极贡献。

Conclusion: 该研究提供了区分潜在效率增益(高SES)和语义保持(低ANLD)的有价值工具，英语词干提取器是更可靠的选择。
...
TL;DR: 提出了一个使用大型语言模型生成和评估19世纪小说家风格文本的框架，通过微调模型和基于transformer的检测器来分析写作风格。


<details>
  <summary>Details</summary>
Motivation: 解决生成模型在缺乏配对数据时的训练问题，以及不依赖人工判断来评估风格化文本的挑战。

Method: 使用最小化单token提示微调大型语言模型生成特定作者风格的文本，并训练transformer检测器进行风格分类和解释，结合句法比较和可解释AI方法分析语言线索。

Result: 生成的文本反映了作者的独特模式，AI评估为人类评估提供了可靠的替代方案。
...
TL;DR: 提出了一种针对多模态大语言模型的新型对抗攻击方法——对抗混淆攻击，旨在通过对抗图像使模型产生不连贯或自信的错误输出，从而破坏MLLM代理的可靠运行。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击主要关注越狱或目标误分类，而本文旨在开发能系统性破坏MLLM可靠性的攻击方法，防止MLLM代理在嵌入对抗图像的网站上正常运行。

Method: 使用开源MLLM的小型集成，通过最大化下一标记熵的基本对抗技术（PGD）生成对抗图像，在完整图像和对抗验证码两种设置下进行攻击。

Result: 在white-box设置下，单个对抗图像能破坏集成中的所有模型，且生成的扰动能迁移到未见过的开源模型（如Qwen3-VL）和专有模型（如GPT-5.1）。

Conclusion: 对抗混淆攻击是一种有效的威胁，能够系统性地破坏MLLM的可靠性，且具有很好的迁移性，突显了MLLM在对抗攻击下的脆弱性。
...
TL;DR: 开发了文本失语症电池(TAB)，这是一个基于临床失语症评估的文本基准，用于评估大语言模型中的失语症样语言缺陷。


<details>
  <summary>Details</summary>
Motivation: 传统临床评估不适合评估LLMs，因为它们假设了类似人类的语用压力，并探测人工架构中不存在的认知过程。需要专门针对LLMs的失语症评估工具。

Method: 从快速失语症电池(QAB)改编开发了TAB，包含四个子测试：连接文本、单词理解、句子理解和重复。使用Gemini 2.5 Flash验证了自动化评估协议。

Result: 自动化评估协议与专家人类评分者具有可比性（模型-共识一致性Cohen's kappa = 0.255 vs 人类-人类一致性0.286）。

Conclusion: TAB提供了一个基于临床的、可扩展的框架，用于分析人工系统中的语言缺陷。
...
TL;DR: 提出了一种新的语音数据增强技术，旨在缩小低资源语言与高资源语言在语音识别性能上的差距。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型在英语等资源丰富语言上表现优异，但在低资源语言上由于数据收集困难且成本高昂，存在不公平的性能差距。
...
TL;DR: 本文提出了一个教育对话数据集，用于自动识别学生对话中的知识建构和任务完成话语特征，并使用GPT-3.5和Llama-3.1建立了基线模型，但现有模型表现不佳，表明需要进一步研究。


<details>
  <summary>Details</summary>
Motivation: 教育研究者需要识别学生对话中的话语特征来理解课程和教学变量如何影响学生从完成任务转向知识建构，但手动分析耗时耗力，限制了研究规模。

Method: 构建了一个带注释的教育对话数据集，包含知识建构和任务完成话语特征，并使用预训练大语言模型GPT-3.5和Llama-3.1建立自动预测这些话语属性的基线模型。

Result: 实验结果表明，这些最先进的模型在此任务上表现欠佳，显示出未来研究的潜力。

Conclusion: 虽然自动检测教育对话中的话语特征具有重要价值，但现有大语言模型在此任务上表现不佳，需要进一步研究来改进模型性能。
...
TL;DR: 本文研究了LLMs生成能力与评估能力之间的一致性，提出了AlignEval基准，通过评估LLMs作为评判者的能力来间接衡量其与人类偏好的对齐程度，无需直接评估模型输出。


<details>
  <summary>Details</summary>
Motivation: 传统评估LLMs与人类偏好对齐的方法需要直接评估其开放生成结果，这需要大量人工标注或强LLM评判者。本文旨在探索LLMs生成与评估能力之间的关系，并开发更高效的评估方法。

Method: 首先分析各种LLMs的生成-评估一致性，发现两者在强LLM偏好预言机评估下存在强相关性。基于此提出AlignEval基准，通过评估LLMs作为评判者的能力来间接衡量其对齐程度。

Result: AlignEval基准在捕捉人类偏好对LLMs进行排序方面，匹配或超越了AlpacaEval和Arena-Hard等广泛使用的自动LLM评估基准。

Conclusion: 研究揭示了LLMs生成与评估能力之间的紧密联系，提出的AlignEval基准为评估LLMs对齐提供了一种无需直接评估模型输出的有效方法。
...
TL;DR: LatentMAS是一个无需训练的端到端框架，通过潜在空间协作实现多智能体系统，相比基于文本的MAS具有更高表达能力和无损信息传递，显著提升推理质量并大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖基于文本的中介进行推理和通信，这限制了系统级智能的协调效率。作者希望让模型直接在连续潜在空间中协作，实现更高效的多智能体系统。

Method: 提出LatentMAS框架：每个智能体通过最后一层隐藏嵌入进行自回归潜在思维生成；共享潜在工作内存保存和传输智能体的内部表示，确保无损信息交换；无需额外训练。

Result: 在9个基准测试中，LatentMAS在数学和科学推理、常识理解和代码生成方面表现优于单模型和基于文本的MAS基线，准确率提升高达14.6%，输出token使用减少70.8%-83.7%，端到端推理速度提升4-4.3倍。

Conclusion: 潜在协作框架显著提升了系统级推理质量，同时提供了实质性的效率增益，无需任何额外训练，证明了在潜在空间中直接协作的有效性。
...
### [227] [SCoTER: Structured Chain-of-Thought Transfer for Enhanced Recommendation](https://arxiv.org/abs/2511.19514)
*Yang Wu,Qian Li,Yuling Xiong,Hongbo Tang,Xun Liu,Jun Zhang,Huan Yu,Jie Jiang,Hailong Shi*

Main category: cs.IR

TL;DR: SCoTER是一个统一的框架，通过自动模式发现和结构保持集成，将结构化LLM推理转移到推荐系统中，消除了在线LLM推理成本并提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在推荐系统中的两个基本挑战：缺乏自动发现有效推理模式的机制，以及结构崩溃的集成方法导致推理链逻辑丢失。

Method: 提出SCoTER框架，包含两个协同组件：GVM管道用于自动模式发现，结构保持集成架构将逐步逻辑转移到高效模型中。

Result: 在四个基准测试上比强基线TIGER提升3.75%-11.59%；在腾讯广告平台部署中GMV提升2.14%，同时消除在线LLM推理成本。

Conclusion: SCoTER为将结构化LLM推理转移到大规模推荐系统提供了一个原则性和生产验证的蓝图。
...
### [228] [LLM-EDT: Large Language Model Enhanced Cross-domain Sequential Recommendation with Dual-phase Training](https://arxiv.org/abs/2511.19931)
*Ziwei Liu,Qidong Liu,Wanyu Wang,Yejing Wang,Tong Xu,Wei Huang,Chong Chen,Peng Chuan,Xiangyu Zhao*

Main category: cs.IR

TL;DR: 提出了LLM-EDT方法，通过双阶段训练策略解决跨域序列推荐中的不平衡问题和转移问题，利用大语言模型增强推荐性能。


<details>
  <summary>Details</summary>
Motivation: 当前跨域序列推荐面临不平衡问题和转移问题，现有LLM增强方法未能有效处理无关噪声和粗略画像问题。

Method: 使用可转移项目增强器生成跨域行为，采用双阶段训练策略，设计领域感知画像模块来聚合用户偏好。

Result: 在三个公共数据集上的实验验证了LLM-EDT的有效性。

Conclusion: LLM-EDT成功解决了跨域序列推荐中的关键挑战，提高了推荐性能。
...
### [229] [The 2nd Workshop on Human-Centered Recommender Systems](https://arxiv.org/abs/2511.19979)
*Kaike Zhang,Jiakai Tang,Du Su,Shuchang Liu,Julian McAuley,Lina Yao,Qi Cao,Yue Feng,Fei Sun*

Main category: cs.IR

TL;DR: 该工作坊呼吁推荐系统从优化参与度转向真正理解、参与和造福人类的设计，聚焦人类理解、人类参与和人类影响三大主题轴。


<details>
  <summary>Details</summary>
Motivation: 随着推荐系统影响力增长，传统指标（如准确性、点击率和参与度）已无法捕捉对人类真正重要的价值，需要将人类价值观融入推荐过程。

Method: 通过汇集推荐系统、人机交互、AI安全和社交计算领域的研究人员，举办主题演讲、小组讨论和论文展示，探索如何将信任、安全、公平、透明和福祉等人类价值观整合到推荐过程中。

Result: 工作坊围绕三大主题轴展开：人类理解、人类参与和人类影响，涵盖从基于LLM的交互式推荐器到社会福祉优化等主题。

Conclusion: 通过促进跨学科合作，HCRS旨在塑造未来十年负责任且与人类价值观一致的推荐研究发展方向。
...
### [230] [Popularity Bias Alignment Estimates](https://arxiv.org/abs/2511.19999)
*Anton Lyubinin*
...
Method: 将定理推广到任意度分布，并推导对齐估计的上下界

Result: 成功扩展了定理适用范围，获得了对齐估计的理论界限

Conclusion: 该研究显著扩展了流行度偏差记忆化理论，为更广泛的应用场景提供了理论支撑
...
### [231] [Adaptive Knowledge Transfer for Cross-Disciplinary Cold-Start Knowledge Tracing](https://arxiv.org/abs/2511.20009)
*Yulong Deng,Zheng Guan,Min He,Xue Wang,Jie Liu,Zheng Li*

Main category: cs.IR

TL;DR: 提出基于专家混合和对抗生成网络的跨学科冷启动知识追踪框架，解决目标学科学生交互数据不足的问题，通过聚类、专家网络和对抗判别器实现有效的跨领域知识迁移。


<details>
  <summary>Details</summary>
Motivation: 跨学科冷启动知识追踪面临目标学科学生交互数据不足的挑战，现有方法依赖学科间重叠实体进行简单映射，但在实际场景中重叠实体稀少且简单映射无法捕捉跨学科知识复杂性。

Method: 1) 预训练源学科模型并聚类学生知识状态为K类；2) 聚类属性通过门控机制指导专家混合网络作为跨领域映射桥梁；3) 对抗判别器通过拉近同属性学生特征、推远不同属性特征来增强特征分离，缓解小样本限制。

Result: 在20个极端跨学科冷启动场景中验证了方法的有效性。

Conclusion: 所提出的框架能够有效解决跨学科冷启动知识追踪中的关键挑战，实现更好的知识状态建模和性能预测。
...
TL;DR: 提出了一种基于热力学视角的扩散推荐框架，通过最大化亥姆霍兹自由能结合能量最大化和熵减少，并设计了保持各向异性的去噪器和AR-GSP采样策略来提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的推荐系统缺乏严谨的理论分析，作者从热力学角度发现扩散模型通过最大化能量工作，而经典推荐模型通过减少熵工作，需要结合两者优势。

Method: 提出最小化扩散框架，通过最大化亥姆霍兹自由能结合能量和熵因素；设计保持二部图用户-物品交叉相关性的去噪器；采用AR-GSP采样策略，结合接受-拒绝采样和时间步相关的Gumbel Softmax。

Result: 理论分析和大量实验表明，所提框架在准确性和效率方面均优于基线方法。

Conclusion: 该工作为扩散模型在推荐系统中的应用提供了严谨的理论基础，提出的框架在性能和效率上都有显著优势。
...
### [233] [Enhancing Sequential Recommendation with World Knowledge from Large Language Models](https://arxiv.org/abs/2511.20177)
*Tianjie Dai,Xu Chen,Yunmeng Shu,Jinsong Lan,Xiaoyong Zhu,Jiangchao Yao,Bo Zheng*

Main category: cs.IR

TL;DR: GRASP是一个灵活的序列推荐框架，结合生成增强检索和整体注意力增强，有效利用LLM的世界知识并缓解幻觉噪声，在多个数据集上实现最先进性能。
...
Method: 提出GRASP框架，包含生成增强检索（描述性合成和相似性检索）和整体注意力增强（多级注意力机制），利用检索到的相似用户/物品作为上下文信息。

Result: 在两个公共基准和一个工业数据集上的综合评估显示，GRASP与不同骨干网络结合时始终实现最先进性能。

Conclusion: GRASP通过生成增强检索和整体注意力增强，有效利用LLM知识并缓解幻觉问题，在序列推荐任务中表现优异。
...
### [234] [HKRAG: Holistic Knowledge Retrieval-Augmented Generation Over Visually-Rich Documents](https://arxiv.org/abs/2511.20227)
*Anyang Tong,Xiang Niu,ZhiPing Liu,Chang Tian,Yanyan Wei,Zenglin Shi,Meng Wang*

Main category: cs.IR

TL;DR: HKRAG是一个针对视觉丰富文档的多模态检索增强生成框架，通过混合掩码检索器分别建模显著知识和细则知识，并使用不确定性引导的生成器动态整合两种知识流，显著提升视觉问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法偏向于检索显著知识（如突出文本和视觉元素），而忽略了关键的细则知识（如小文本、上下文细节），导致检索不完整和生成答案不准确。

Method: 提出混合掩码检索器分别建模显著和细则知识，以及不确定性引导的生成器动态评估初始答案的不确定性并主动决定如何整合两种知识流。

Result: 在开放域视觉问答基准测试中，HKRAG在零样本和监督设置下均优于现有方法。

Conclusion: 整体知识检索对于视觉丰富文档理解至关重要，HKRAG框架有效解决了现有方法的局限性。
...
### [235] [HHFT: Hierarchical Heterogeneous Feature Transformer for Recommendation Systems](https://arxiv.org/abs/2511.20235)
*Liren Yu,Wenming Zhang,Silu Zhou,Zhixuan Zhang,Dan Ou*

Main category: cs.IR

TL;DR: HHFT是一种基于Transformer的CTR预测架构，通过语义特征分区、异构Transformer编码器和Hiformer层设计，在工业场景中显著优于DNN基线，已成功部署在淘宝生产平台。


<details>
  <summary>Details</summary>
Motivation: 解决DNN在CTR预测中的局限性，特别是处理异构特征（用户画像、商品信息、行为序列）时的语义混淆问题。

Method: 1. 语义特征分区：将异构特征按语义分组；2. 异构Transformer编码器：采用块特定的QKV投影和FFN避免语义混淆；3. Hiformer层：捕捉特征间的高阶交互。

Result: Transformer显著优于DNN基线，CTR AUC提升+0.4%，在淘宝生产平台部署后GMV提升+0.6%。

Conclusion: HHFT架构有效解决了异构特征处理的挑战，在工业级CTR预测中表现出色，已成功应用于实际业务场景。
...
### [236] [Games in Product Form for Demand Response Modelling](https://arxiv.org/abs/2511.19532)
*Thomas Buchholtzer,Michel de Lara*

Main category: cs.GT

TL;DR: 本文提出了一个通用的模块化框架来处理领导者-跟随者问题，特别关注常被忽视的信息问题。该框架包含W模型定义游戏规则，W-博弈在乘积形式上建模玩家偏好，并重新表述了需求响应研究中的信息问题。


<details>
  <summary>Details</summary>
Motivation: 能源系统正快速变化，能源生产日益分散化、可变且间歇性，而需求多样化，导致供需平衡更加复杂。需求响应作为典型的领导者-跟随者问题，需要处理常被忽视的信息问题。

Method: 提出W模型定义游戏规则，将代理视为决策者，Nature封装不可控因素。采用Witsenhausen内在模型在乘积集上有效表示代理信息。引入W-博弈为玩家配备偏好，并明确纳什和斯塔克尔伯格均衡定义。

Result: 开发了一个通用框架，能够处理任何领导者-跟随者问题，特别关注信息处理。重新表述了多篇需求响应论文，突出了被忽视的信息问题，并基于泰国需求响应项目提供了应用示例。

Conclusion: 该框架为领导者-跟随者问题提供了通用的模块化解决方案，特别强调了信息问题的重要性，在需求响应等应用中具有实际价值。
...
### [237] [Strategy-robust Online Learning in Contextual Pricing](https://arxiv.org/abs/2511.19842)
*Joon Suk Huh,Kirthevasan Kandasamy*

Main category: cs.GT

TL;DR: 本文研究在线上下文定价问题，考虑买家可能策略性误报估值的情况，提出了策略鲁棒的遗憾概念，并设计了稀疏更新机制（SUM）来确保对买家纳什均衡的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在数字市场中，买家可能策略性地误报估值以影响未来价格（战略过拟合问题），传统在线学习框架无法处理这种策略性行为。

Method: 提出了策略鲁棒的遗憾概念，开发了多项式时间近似方案（PTAS）用于学习线性定价策略，并设计了稀疏更新机制（SUM）作为顺序机制。

Result: SUM机制能够确保对买家所有纳什均衡的鲁棒性，并提供了从在线专家算法到策略鲁棒学习器的黑盒归约。

Conclusion: 该研究为策略性环境下的在线定价问题提供了有效的解决方案，通过稀疏更新机制实现了对策略性行为的鲁棒性。
...
### [238] [Designing Reputation Systems for Manufacturing Data Trading Markets: A Multi-Agent Evaluation with Q-Learning and IRL-Estimated Utilities](https://arxiv.org/abs/2511.19930)
*Kenta Yamamoto,Teruaki Hayashi*

Main category: cs.GT

TL;DR: 本文开发了一个多智能体数据市场模拟器，研究信任形成机制，发现PeerTrust声誉系统在数据价格与质量对齐方面表现最佳，并提出了一种混合声誉机制来改善市场稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着数据成为经济资产，数据市场作为数据驱动创新的关键基础设施出现。但由于信息不对称，买家无法在购买前验证数据内容或质量，信任和质量保证成为核心挑战。

Method: 开发多智能体数据市场模拟器，集成强化学习(RL)用于自适应智能体行为，逆强化学习(IRL)从经验行为数据估计效用函数。评估五种代表性声誉系统：Time-decay、Bayesian-beta、PageRank、PowerTrust和PeerTrust。

Result: PeerTrust实现了数据价格与质量的最强对齐，同时防止垄断主导。基于这些结果开发了混合声誉机制，整合现有系统的优势。

Conclusion: 本研究通过将信任和声誉作为内生机制，扩展了基于模拟的数据市场分析，为设计可靠高效的数据生态系统提供了方法和制度见解。
...
### [239] [One Action Too Many: Inapproximability of Budgeted Combinatorial Contracts](https://arxiv.org/abs/2511.20110)
*Michal Feldman,Yoav Gal-Tzur,Tomasz Ponitka,Maya Schlesinger*

Main category: cs.GT

TL;DR: 该论文研究了预算约束下的多智能体组合契约设计，证明了次模奖励函数无法在多项式时间内获得任何有限因子的近似，但发现了替代品函数可以实现常数因子近似，并提出了加性奖励的FPTAS。


<details>
  <summary>Details</summary>
Motivation: 研究预算约束下多智能体组合契约设计的计算复杂性，填补了无预算组合行动和预算二元行动之间的研究空白，探索在预算限制下哪些目标函数可以实现高效近似。

Method: 使用计算复杂性理论分析，证明了次模奖励函数的强不可能性结果，同时展示了替代品函数的多项式时间确定性近似算法，并为加性奖励函数开发了FPTAS。

Result: 证明了次模奖励函数无法在多项式时间内获得任何有限因子近似，但替代品函数可以实现O(1)常数因子近似，加性奖励函数可以获得任意精度的FPTAS。

Conclusion: 该研究首次在组合契约中划清了预算和无预算设置之间的界限，识别出替代品函数作为预算组合契约的可处理边界，并为加性奖励提供了首个多智能体组合行动设置的FPTAS。
...
### [240] [Lower Bias, Higher Welfare: How Creator Competition Reshapes Bias-Variance Tradeoff in Recommendation Platforms?](https://arxiv.org/abs/2511.20289)
*Kang Wang,Renzhe Xu,Bo Li*

Main category: cs.GT

TL;DR: 该论文分析了内容创作者竞争如何改变推荐系统中偏差-方差权衡的最优策略，发现在战略环境中平台应选择更弱的正则化来降低偏差，从而提高用户福利。


<details>
  <summary>Details</summary>
Motivation: 理解用户表示学习中的偏差-方差权衡对于提高内容平台推荐质量至关重要，但在内容创作者战略性适应平台激励的情况下，这种权衡变得更加复杂。

Method: 提出内容创作者竞争与偏差-方差权衡框架，通过博弈论模型分析平台在用户特征估计中正则化强度的决策，并在非战略基准和战略环境中比较最优策略。

Result: 理论分析和实验验证表明，与静态环境相比，内容创作者竞争使平台的最优策略倾向于更弱的正则化，从而在偏差-方差权衡中偏好更低的偏差。

Conclusion: 在战略环境中减少偏差能带来更高的用户福利，这为现实世界推荐算法设计提供了实际启示。
...
### [241] [Building Browser Agents: Architecture, Security, and Practical Solutions](https://arxiv.org/abs/2511.19477)
*Aram Vardanyan*

Main category: cs.SE

TL;DR: 本文分析了生产环境中浏览器代理的可靠性和安全性挑战，指出模型能力不是限制因素，架构决策才是关键。研究发现提示注入攻击使通用自主操作不安全，建议开发具有编程约束的专用工具而非通用浏览智能。


<details>
  <summary>Details</summary>
Motivation: 解决浏览器代理在生产环境中的可靠性和安全问题，分析当前方法失败的原因，探索安全自主操作的限制因素。

Method: 采用混合上下文管理（结合可访问性树快照和选择性视觉）、全面的浏览器工具集（匹配人类交互能力）以及智能提示工程。

Result: 在WebGames基准测试的53个多样化挑战中，实现了约85%的成功率（相比之前浏览器代理约50%的成功率和95.7%的人类基准）。
...
### [242] [Z-Space: A Multi-Agent Tool Orchestration Framework for Enterprise-Grade LLM Automation](https://arxiv.org/abs/2511.19483)
...
Method: 建立多智能体协作架构和工具过滤算法：(1)通过意图解析模型实现用户查询的结构化语义理解；(2)基于融合子空间加权算法(FSWW)的工具过滤模块实现意图与工具的细粒度语义对齐；(3)构建推理执行智能体支持多步任务的动态规划和容错执行。

Result: 系统在工具推理中平均token消耗减少96.26%，工具调用准确率达到92%，显著提升了智能测试数据生成系统的效率和可靠性。

Conclusion: Z-Space框架成功解决了大规模MCP服务中的工具匹配问题，在饿了么平台技术部部署并服务于淘天、高德、盒马等多个业务单元的大规模测试数据生成场景，验证了其有效性和实用性。
...
### [243] [stable-pretraining-v1: Foundation Model Research Made Simple](https://arxiv.org/abs/2511.19484)
*Randall Balestriero,Hugues Van Assel,Sami BuGhanem,Lucas Maes*

Main category: cs.SE

TL;DR: stable-pretraining是一个模块化、可扩展的自监督学习库，旨在解决基础模型研究中代码复杂、重复实现和工程负担重的问题。


<details>
  <summary>Details</summary>
Motivation: 当前自监督学习和基础模型研究面临代码库复杂、重复实现和实验扩展的沉重工程负担，阻碍了研究进展。

Method: 基于PyTorch、Lightning、Hugging Face和TorchMetrics构建模块化库，统一了SSL核心工具（探针、崩溃检测、数据增强、评估流程），采用全面日志记录设计原则。

Result: 验证了库的有效性，能够以最小开销生成新的研究见解，包括深度表示探针分析和CLIP在合成数据微调下的退化分析。
...
### [244] [Evolution without an Oracle: Driving Effective Evolution with LLM Judges](https://arxiv.org/abs/2511.19489)
*Zhe Zhao,Yuheng Yang,Haibin Wen,Xiaojie Qiu,Zaixi Zhang,Qingfu Zhang*

Main category: cs.SE

TL;DR: MADE框架通过问题分解将主观LLM评估转化为稳定选择压力，在无客观评估函数的纯主观环境中实现有效进化优化


<details>
  <summary>Details</summary>
Motivation: 打破传统进化计算依赖客观评估函数的限制，探索在纯主观LLM评判环境中进行进化的可能性

Method: 提出MADE框架，通过问题规范将模糊指令分解为可验证的子要求，降低主观评估的噪声

Result: 在DevAI和InfoBench基准上，软件需求满足度提升50%以上（39.9%到61.9%），复杂指令遵循达到95%完美通过率

Conclusion: 实现了从优化可计算指标到优化可描述质量的范式转变，为无真实标签的开放领域解锁了进化优化潜力
...
### [245] [CodeR3: A GenAI-Powered Workflow Repair and Revival Ecosystem](https://arxiv.org/abs/2511.19510)
*Asif Zaman,Kallol Naha,Khalid Belhajjame,Hasan M. Jamil*

Main category: cs.SE

TL;DR: CodeR³是一个利用生成式AI将过时的Taverna工作流迁移到Snakemake和VisFlow等现代技术的系统，通过自动化减少手动工作量，但关键任务仍需人工监督。


<details>
  <summary>Details</summary>
Motivation: 科学工作流包含宝贵的领域专业知识，但大量已发布的工作流会随时间衰减，特别是像Taverna这样的遗留系统，由于服务停止、依赖过时和系统退役，导致原本可用的工作流变得无法使用。

Method: 开发CodeR³系统，利用生成式AI分析衰减工作流的特征，将其重制为现代工作流技术，集成逐步工作流分析可视化、自动化服务替换和人在回路验证。

Result: 通过多个Taverna工作流复兴案例研究，证明该方法的可行性，自动化显著减少了工作流解析和服务识别的手动工作，但服务替换和数据验证等关键任务仍需领域专业知识。

Conclusion: 提出了一个平衡自动化效率与必要人工判断的工作流复兴框架，将开发众包平台让社区协作复兴衰减工作流并验证功能正确性。
...
### [246] [Agint: Agentic Graph Compilation for Software Engineering Agents](https://arxiv.org/abs/2511.19635)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM编码代理在上下文管理、延迟、可靠性、可重现性和可扩展性方面的挑战，提供更可靠的代码生成和执行环境。

Method: 引入显式类型层（文本→数据→规范→代码）和语义图转换，采用混合LLM和基于函数的JIT运行时，支持动态图优化、推测执行和与现有开发工具互操作。

Result: 提高了可靠性，支持并发组合，使用更小更快的模型，降低延迟，提高吞吐量，支持可重现和高效的并行生成。

Conclusion: Agint通过连接自然语言、编译器方法和开发工具，实现了可组合、团队导向的编码代理的大规模部署，支持快速原型到生产代码的无缝过渡。
...
### [247] [CodeFuse-CommitEval: Towards Benchmarking LLM's Power on Commit Message and Code Change Inconsistency Detection](https://arxiv.org/abs/2511.19875)
*Qingyu Zhang,Puzhuo Liu,Peng Di,Chenxiong Qian*

Main category: cs.SE

TL;DR: 提出了第一个专门用于评估大语言模型检测提交消息与代码不一致性(MCI)的基准CODEFUSE-COMMITEVAL，通过规则引导的突变生成七种不一致消息类型，评估了六种开源LLM在多种增强策略下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的提交消息往往质量低且与代码变更不一致，这会误导代码审查、阻碍维护、污染研究数据集，甚至可能掩盖安全补丁，但目前缺乏专门的基准来评估MCI检测模型。

Method: 基于ApacheCM数据集，通过规则引导的突变生成七种不一致消息类型，并应用双重验证确保正负样本质量。在普通设置和三种增强策略（少样本提示、思维链、扩展上下文）下评估六种开源LLM。

Result: 模型检测不一致提交比一致提交更可靠（平均召回率85.95%，精确率80.28%，特异性63.8%）；gpt-oss-20B表现最佳但token使用量是其他模型的两倍多；增强策略效果各异，类型分析显示组件、文件路径和操作不一致更容易检测。

Conclusion: CODEFUSE-COMMITEVAL为MCI检测提供了严格的评估基础，强调了需要更丰富的上下文和平衡的数据来捕捉高级语义差异。
...
### [248] [LLMs for Automated Unit Test Generation and Assessment in Java: The AgoneTest Framework](https://arxiv.org/abs/2511.20403)
*Andrea Lops,Fedelucio Narducci,Azzurra Ragone,Michelantonio Trizio,Claudio Barto*

Main category: cs.SE

TL;DR: AgoneTest是一个用于评估LLM生成的Java单元测试的自动化框架，提供标准化评估流程来比较不同LLM和提示策略，实验显示LLM生成的测试在覆盖率和缺陷检测方面可与人工测试媲美。


<details>
  <summary>Details</summary>
Motivation: 单元测试对软件开发至关重要但资源密集，需要支持研究人员和开发者比较不同LLM和提示策略在真实条件下的表现。

Method: 引入Classes2Test数据集映射Java类到对应测试类，集成变异分数和测试异味等高级评估指标，提供端到端标准化评估流程。

Result: 对于能够编译的测试子集，LLM生成的测试在覆盖率和缺陷检测方面可匹配甚至超过人工编写的测试，增强的提示策略有助于提高测试质量。
...
### [249] [Translating Large-Scale C Repositories to Idiomatic Rust](https://arxiv.org/abs/2511.20617)
*Saman Dehghan,Tianran Sun,Tianxiang Wu,Zihan Li,Reyhaneh Jabbarvand*

Main category: cs.SE

TL;DR: Rustine是一个自动化的C到Rust翻译工具，在保持可扩展性的同时生成更安全、更符合惯用法、更可读的Rust代码。


<details>
  <summary>Details</summary>
Motivation: 现有的C到Rust翻译技术无法平衡质量和可扩展性：基于转译的方法可扩展但代码质量差，基于LLM的方法质量高但成本昂贵且不可靠。

Method: Rustine是一个完全自动化的流水线，用于实现有效且高效的仓库级C到惯用安全Rust的翻译。

Result: 在23个C程序上评估，Rustine能为所有程序生成完全可编译的Rust代码，达到87%功能等价性，相比现有技术更安全、更符合惯用法、更可读。

Conclusion: Rustine在C到Rust翻译中实现了质量与可扩展性的平衡，当翻译无法完全通过测试时，开发者平均只需4.5小时即可完成调试。
...
### [250] [Hidden markov model to predict tourists visited place](https://arxiv.org/abs/2511.19465)
*Theo Demessance,Chongke Bi,Sonia Djebali,Guillaume Guerard*

Main category: cs.LG

TL;DR: 基于社交网络数据分析游客移动行为，使用机器学习语法推断算法预测游客未来移动路径，特别针对大数据环境进行了算法适配。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上游客留下的数字痕迹为分析旅游行为提供了丰富数据，预测游客移动对旅游营销和决策支持至关重要。

Method: 采用机器学习语法推断算法，构建表示游客移动模式的隐马尔可夫模型，该模型具有灵活性和可编辑性。

Result: 以法国巴黎为例验证了方法的有效性，能够成功预测游客的移动路径。

Conclusion: 提出的方法能够有效利用社交网络数据理解和预测游客移动行为，为旅游决策提供支持。
...
### [251] [Quantifying Modality Contributions via Disentangling Multimodal Representations](https://arxiv.org/abs/2511.19470)
*Padegal Amit,Omkar Mahesh Kashyap,Namitha Rayasam,Nidhi Shekhar,Surabhi Narayan*

Main category: cs.LG

TL;DR: 提出了基于部分信息分解(PID)的框架，通过分解内部嵌入中的预测信息为独特、冗余和协同成分来量化多模态模型中各模态的贡献，并开发了基于迭代比例拟合程序(IPFP)的算法进行可扩展的推理分析。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖基于准确性的方法，将移除模态后的性能下降解释为其影响力，但这类结果驱动指标无法区分模态是本身具有信息价值还是仅通过与其他模态交互产生价值，特别是在跨注意力架构中。

Method: 使用部分信息分解(PID)框架，将内部嵌入中的预测信息分解为独特、冗余和协同成分；开发基于迭代比例拟合程序(IPFP)的算法，无需重新训练即可计算层和数据集级别的贡献。

Result: 提供了一个原则性的、表示级别的多模态行为视图，比基于结果的指标提供更清晰和可解释的见解。

Conclusion: 该框架能够更准确地量化多模态模型中各模态的贡献，区分模态的固有信息价值与交互产生的价值，为多模态模型分析提供了更深入的理解。
...
### [252] [Ranking-Enhanced Anomaly Detection Using Active Learning-Assisted Attention Adversarial Dual AutoEncoders](https://arxiv.org/abs/2511.20480)
...
<details>
  <summary>Details</summary>
Motivation: APT攻击具有隐蔽性和长期性，传统监督学习方法需要大量标注数据，但在真实网络安全环境中标注数据稀缺且成本高昂。

Method: 采用注意力对抗双自编码器框架进行无监督异常检测，结合主动学习策略选择性查询不确定样本的标签，迭代提升模型性能。

Result: 在DARPA透明计算项目的真实溯源跟踪数据库上评估，涵盖Android、Linux、BSD和Windows多个操作系统，结果显示主动学习过程中检测率显著提升，优于现有方法。

Conclusion: 该方法通过主动学习最小化标注成本，在数据极度不平衡的情况下有效提高了APT异常检测性能，为网络安全中的稀缺标注数据问题提供了可行解决方案。
...
### [253] [PrefixGPT: Prefix Adder Optimization by a Generative Pre-trained Transformer](https://arxiv.org/abs/2511.19472)
*Ruogu Ding,Xin Ning,Ulf Schlichtmann,Weikang Qian*

Main category: cs.LG

TL;DR: PrefixGPT是一种基于GPT的生成式预训练Transformer，能够从零开始直接生成优化的前缀加法器，通过二维坐标序列表示拓扑结构并在生成过程中应用合法性掩码确保设计有效。


<details>
  <summary>Details</summary>
Motivation: 设计优化的前缀加法器具有挑战性，因为存在严格的设计规则和指数级大的设计空间，需要一种能够自动生成优化设计的方法。

Method: 使用自定义的解码器Transformer架构，首先在随机合成的有效前缀加法器语料库上进行预训练以学习设计规则，然后通过微调在设计空间中导航以优化设计质量。
...
### [254] [From One Attack Domain to Another: Contrastive Transfer Learning with Siamese Networks for APT Detection](https://arxiv.org/abs/2511.20500)
*Sidahmed Benabderrahmane,Talal Rahwan*

Main category: cs.LG

TL;DR: 提出了一种混合迁移学习框架，结合迁移学习、可解释AI、对比学习和孪生网络，用于提高APT检测的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习检测器在APT检测中存在类别不平衡、高维特征和真实数据稀缺的问题，且在跨域场景中泛化能力差。

Method: 使用基于注意力的自编码器进行跨域知识迁移，SHAP选择稳定特征降维，孪生编码器通过对比学习对齐源域和目标域表示。

Result: 在DARPA透明计算项目的真实数据和合成攻击场景上评估，相比经典和深度基线方法，在源域到目标域迁移中获得了更好的检测分数。
...
### [255] [WavefrontDiffusion: Dynamic Decoding Schedule or Improved Reasoning](https://arxiv.org/abs/2511.19473)
...
<details>
  <summary>Details</summary>
Motivation: 主流去噪策略（标准扩散和块扩散）存在局限性：标准扩散容易过早结束序列，块扩散的固定结构会破坏语义连贯性。需要一种能自适应语义结构的方法。

Method: 提出WavefrontDiffusion方法，动态扩展活动令牌波前，从已确定位置向外扩散，遵循语义结构的自然流动。
...
### [256] [Exploiting the Experts: Unauthorized Compression in MoE-LLMs](https://arxiv.org/abs/2511.19480)
...
<details>
  <summary>Details</summary>
Motivation: MoE架构的模块化特性引入了独特漏洞：攻击者可通过修剪专家和廉价微调来绕过许可和安全约束，压缩或重新利用模型。

Method: 开发专家归因框架识别任务关键专家，评估修剪和重新对齐的性能权衡，使用主动学习驱动的微调方法。

Result: 发现关键的知识损失-恢复权衡：某些专家可被隔离以保持任务准确性，但缺乏目标重新对齐会导致显著性能下降。
...
### [257] [Quality analysis and evaluation prediction of RAG retrieval based on machine learning algorithms](https://arxiv.org/abs/2511.19481)
*Ruoxin Zhang,Zhizhao Wen,Chao Wang,Chenchen Tang,Puyang Xu,Yifan Jiang*

Main category: cs.LG

TL;DR: 本文提出基于特征工程和粒子群优化的XGBoost回归模型，用于优化RAG系统的检索质量，实验表明该模型在预测精度和稳定性方面优于其他对比模型。


<details>
  <summary>Details</summary>
Motivation: RAG系统的性能高度依赖检索模块质量，现有模型在处理表格特征方面存在性能瓶颈，需要提升文档相关性以提高答案质量。

Method: 采用特征工程和粒子群优化构建XGBoost机器学习回归模型，通过相关性分析识别关键特征关系。
...
### [258] [OmniTFT: Omni Target Forecasting for Vital Signs and Laboratory Result Trajectories in Multi Center ICU Data](https://arxiv.org/abs/2511.19485)
*Wanzhe Xu,Yutong Dai,Yitao Yang,Martin Loza,Weihang Zhang,Yang Cui,Xin Zeng,Sung Joon Park,Kenta Nakai*

Main category: cs.LG

TL;DR: OmniTFT是一个基于时序融合变换器的深度学习框架，用于联合预测ICU中的高频生命体征和稀疏采样的实验室结果，通过四种创新策略提升性能，并在多个数据集上取得了显著改进。
...
Method: 基于时序融合变换器，采用滑动窗口均衡采样、频率感知嵌入收缩、分层变量选择和影响对齐注意力校准四种策略。

Result: 在MIMIC-III、MIMIC-IV和eICU数据集上，OmniTFT在生命体征和实验室结果的预测任务中都取得了显著的性能提升。

Conclusion: OmniTFT能够统一建模多个异质临床目标，保持跨机构泛化能力，其注意力模式可解释且与已知病理生理学一致，具有临床决策支持的潜在价值。
...
### [259] [REWA: Witness-Overlap Theory -- Foundations for Composable Binary Similarity Systems](https://arxiv.org/abs/2511.19998)
*Nikit Phadke*

Main category: cs.LG

TL;DR: REWA提出了一种基于见证重叠结构的通用相似性理论，将各种相似性度量统一为单调见证重叠问题，并证明可使用对数级比特数进行压缩编码且保持排名不变。


<details>
  <summary>Details</summary>
Motivation: 现有相似性系统（如Bloom过滤器、minhash等）缺乏统一的理论基础，且依赖于特定的哈希函数设计。REWA旨在提供一个基于见证重叠的通用框架，统一各种相似性度量方法。

Method: REWA系统包含三个组件：(1)有限见证集W(v)，(2)从每个见证生成的半随机比特分配，(3)相似性在重叠Δ(u,v)=|W(u)∩W(v)|上的单调性。通过重叠间隙条件，使用m=O(log(|V|/δ))比特即可保持top-k排名。

Result: 证明了在见证集满足重叠间隙条件时，相似性排名可使用对数级比特数精确保持。该框架统一了Bloom过滤器、minhash、LSH位图等多种相似性方法作为特例。

Conclusion: REWA为相似性系统提供了基于见证重叠的通用理论基础，支持模块化组合设计，大幅扩展了相似性定义的设计空间，同时保证编码复杂度为对数级。
...
### [260] [Efficient Inference Using Large Language Models with Limited Human Data: Fine-Tuning then Rectification](https://arxiv.org/abs/2511.19486)
*Lei Wang,Zikun Ye,Jinglong Zhao*

Main category: cs.LG

TL;DR: 本文提出了一个结合微调和校正的框架，通过优化分配有限标注样本来提升LLMs在市场研究中的表现，使用预测误差方差最小化作为微调目标，并基于经验缩放定律优化样本分配。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要单独使用微调或校正来提升LLMs性能，但缺乏系统性的框架来结合这两种方法并优化有限的标注样本分配。

Method: 开发了一个结合微调和校正的两阶段框架，提出以预测误差方差最小化作为微调目标，并利用经验缩放定律优化样本在微调和校正阶段之间的分配。

Result: 实证分析验证了该框架的有效性，相比单独使用微调或校正，在估计和推断性能方面都有显著提升。

Conclusion: 提出的框架通过系统性结合微调和校正方法，并优化样本分配策略，能够更有效地提升LLMs在市场研究和社会科学应用中的表现。
...
### [261] [The Generalized Proximity Forest](https://arxiv.org/abs/2511.19487)
...
<details>
  <summary>Details</summary>
Motivation: 随机森林邻近性在多种监督学习任务中很有用，但其效用依赖于RF模型本身，而RF并非在所有场景下都是理想模型。需要将RF邻近性扩展到更广泛的基于距离的机器学习场景。

Method: 提出了广义邻近森林模型，将RF邻近性扩展到所有基于距离的监督机器学习；引入了PF模型的回归变体；将广义PF模型作为元学习框架，扩展监督插补能力到任何预训练分类器。

Result: 实验证明广义PF模型相比随机森林模型和k近邻模型具有独特优势。

Conclusion: 广义邻近森林模型成功扩展了RF邻近性的应用范围，为基于距离的监督机器学习提供了更通用的框架。
...
### [262] [Generative Model-Aided Continual Learning for CSI Feedback in FDD mMIMO-OFDM Systems](https://arxiv.org/abs/2511.19490)
*Guijun Liu,Yuwen Cao,Tomoaki Ohtsuki,Jiguang He,Shahid Mumtaz*

Main category: cs.LG

TL;DR: 提出基于GAN的持续学习方法解决CSI反馈中的灾难性遗忘问题，通过GAN生成器作为记忆单元来保持过去环境的知识，提高DAE框架在动态环境中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的CSI反馈模型难以适应由用户移动性引起的动态环境，遇到新的CSI分布时需要重新训练，且返回先前环境时由于灾难性遗忘会导致性能下降。

Method: 使用生成对抗网络（GAN）作为学习框架，将GAN生成器作为记忆单元来保存过去环境的知识，确保在不同场景下保持高性能且不遗忘。

Result: 仿真结果表明，该方法增强了DAE框架的泛化能力，同时保持低内存开销，并能与其他先进的CSI反馈模型无缝集成。

Conclusion: 所提出的GAN基学习方法有效解决了CSI反馈中的持续学习问题，具有鲁棒性和适应性，能够在不遗忘旧知识的情况下适应新环境。
...
### [263] [OpenCML: End-to-End Framework of Open-world Machine Learning to Learn Unknown Classes Incrementally](https://arxiv.org/abs/2511.19491)
...
### [264] [RFX: High-Performance Random Forests with GPU Acceleration and QLORA Compression](https://arxiv.org/abs/2511.19493)
*Chris Kuchar*

Main category: cs.LG

TL;DR: RFX v1.0 是一个生产就绪的随机森林分类实现，通过QLORA压缩和TriBlock存储等技术解决了邻近矩阵内存瓶颈，使分析样本量从6万扩展到20万+，支持CPU/GPU加速。


<details>
  <summary>Details</summary>
Motivation: 解决随机森林分析中邻近矩阵的内存瓶颈问题，该问题限制了分析样本量至约6万个样本，阻碍了在大规模数据集上的应用。

Method: 采用四种解决方案：(1) QLORA压缩用于GPU邻近矩阵，实现12,500倍压缩；(2) CPU TriBlock邻近矩阵，结合上三角存储和块稀疏阈值；(3) SM感知GPU批处理大小；(4) GPU加速3D MDS可视化。

Result: 验证了四种实现模式，GPU比CPU在500+棵树时提速1.4倍，邻近矩阵计算从1,000扩展到200,000+样本，QLORA压缩将内存从80GB降至6.4MB，保持99%几何结构。

Conclusion: RFX v1.0消除了邻近矩阵内存瓶颈，使基于邻近的随机森林分析能够在比以往大几个数量级的数据集上进行，提供了开源的生产就绪分类实现。
...
### [265] [A Systematic Study of Compression Ordering for Large Language Models](https://arxiv.org/abs/2511.19495)
*Shivansh Chhawri,Rahul Mahadik,Suparna Rooj*

Main category: cs.LG

TL;DR: 本文系统研究了知识蒸馏、结构化剪枝和低比特量化三种LLM压缩技术的独立效果及组合顺序对Qwen2.5 3B模型的影响，发现技术顺序对最终质量有显著影响，其中P-KD-Q序列能实现最佳平衡。


<details>
  <summary>Details</summary>
Motivation: LLM需要大量计算资源，在受限环境中需要高效部署。虽然主要压缩技术已有研究，但它们之间的相互作用和最优顺序仍不清楚。

Method: 系统评估多种压缩流水线，包括单技术和三技术组合序列，使用困惑度、G-Eval、清晰度、提示对齐和压缩比作为指标。

Result: 量化提供最大独立压缩，剪枝引入中等质量下降。技术顺序显著影响最终质量：P-KD-Q序列实现最佳平衡，达到3.68x压缩比同时保持强指令跟随和语言理解能力。

Conclusion: 本研究为在资源受限环境中部署LLM提供了实用的、顺序感知的压缩流水线设计见解。
...
### [266] [Xmodel-2.5: 1.3B Data-Efficient Reasoning SLM](https://arxiv.org/abs/2511.19496)
*Yang Liu,Xiaolong Zhong,Ling Jiang*

Main category: cs.LG

TL;DR: Xmodel-2.5是一个13亿参数的小型语言模型，专为边缘计算和成本敏感部署设计，采用μP训练方法实现超参数从2000万参数代理模型到完整模型的直接迁移，并通过AdamW到Muon的优化器切换策略提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备强大的推理和工具使用能力，但计算需求过高，不适合边缘或成本敏感的应用场景。

Method: 采用最大更新参数化(μP)训练方法，使用1.4T token的Warmup-Stable-Decay课程学习，在衰减阶段从AdamW切换到Muon优化器，并采用FP8混合精度训练。

Result: 优化器切换策略使13个任务的推理平均性能提升4.58%，同时保持其他超参数不变，验证了早期AdamW稳定性和后期Muon锐化的组合优势。

Conclusion: Xmodel-2.5作为drop-in agent core，在保持高性能的同时显著降低了计算需求，所有检查点、配方和评估代码均已开源。
...
### [267] [PeriodNet: Boosting the Potential of Attention Mechanism for Time Series Forecasting](https://arxiv.org/abs/2511.19497)
*Bowen Zhao,Huanlai Xing,Zhiwen Xiao,Jincheng Peng,Li Feng,Xinhan Wang,Rong Qu,Hui Li*

Main category: cs.LG

TL;DR: PeriodNet提出了一种新的时间序列预测架构，结合周期注意力机制和迭代分组机制，在单变量和多变量时间序列预测中优于现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 注意力机制在NLP领域表现出色（如BERT、GPT），但在时间序列预测中尚未达到预期效果，需要探索更好的网络结构来提升预测性能。

Method: 使用周期注意力和稀疏周期注意力机制分析相邻周期，引入迭代分组机制减少跨变量冗余，重新设计Transformer架构并添加周期扩散器进行多周期预测。

Result: 在8个数据集上的实验表明，PeriodNet在单变量和多变量时间序列预测的MSE和MAE指标上均优于6个最先进模型，在预测720长度时间序列时相对改进达22%。

Conclusion: PeriodNet通过创新的周期注意力机制和架构设计，显著提升了时间序列预测的性能，特别是在长序列预测方面表现突出。
...
### [268] [Hierarchical Dual-Strategy Unlearning for Biomedical and Healthcare Intelligence Using Imperfect and Privacy-Sensitive Medical Data](https://arxiv.org/abs/2511.19498)
*Yi Zhang,Tianxiang Xu,Zijian Li,Chao Zhang,Kunyu Zhang,Zhan Gao,Meinuo Li,Xiaohan Zhang,Qichao Qi,Bing Chen*

Main category: cs.LG

TL;DR: 提出分层双策略框架进行选择性知识遗忘，在医疗领域精确移除专业知识同时保留基础医学能力，仅需修改0.1%参数即可实现82.7%遗忘率和88.5%知识保留。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在医疗环境中因训练数据记忆带来的隐私风险，特别是涉及不完整或敏感患者信息时，需要满足监管合规和伦理标准。

Method: 分层双策略框架：几何约束梯度更新选择性调节目标参数，概念感知令牌级干预通过统一四级医学概念层次区分保留关键令牌和遗忘目标令牌。

Result: 在MedMCQA（外科）和MHQA（焦虑、抑郁、创伤）数据集上评估，达到82.7%遗忘率和88.5%知识保留率，仅修改0.1%参数。

Conclusion: 该框架在保持强大隐私保证的同时，解决了临床研究中监管合规、可审计性和伦理标准的关键需求。
...
### [269] [Beyond Binary Classification: A Semi-supervised Approach to Generalized AI-generated Image Detection](https://arxiv.org/abs/2511.19499)
*Hong-Hanh Nguyen-Le,Van-Tuan Tran,Dinh-Thuc Nguyen,Nhien-An Le-Khac*

Main category: cs.LG

TL;DR: 该论文提出TriDetect检测器来解决生成图像检测中的跨架构泛化问题，特别是GAN和扩散模型之间的检测迁移问题。


<details>
  <summary>Details</summary>
Motivation: 当前伪造检测器在跨不同架构生成器（如GAN到扩散模型）时泛化能力不足，这是由于不同架构产生的伪影存在根本性差异。

Method: 提出TriDetect半监督方法，通过Sinkhorn-Knopp算法进行平衡聚类分配和跨视图一致性机制，学习不同架构的基本特征差异。

Result: 在两个标准基准和三个真实数据集上评估，与13个基线方法相比，证明了其对未见生成器的泛化能力。

Conclusion: TriDetect通过发现"伪造"类别中的潜在架构模式，有效提升了跨架构生成图像的检测性能。
...
### [270] [Position: The Complexity of Perfect AI Alignment -- Formalizing the RLHF Trilemma](https://arxiv.org/abs/2511.19504)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.LG

TL;DR: 本文提出了对齐三难困境：RLHF系统无法同时实现代表性、计算可扩展性和鲁棒性。通过复杂性理论分析证明，要实现全球规模人群的代表性和鲁棒性需要超多项式计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF系统在实践中面临安全性与公平性、可扩展性与多样性、鲁棒性与偏见放大之间的持续矛盾，需要从理论层面理解这些根本性权衡。

Method: 采用复杂性理论分析，结合统计学习理论和鲁棒优化，形式化定义了对齐三难困境，并证明了代表性(ε≤0.01)和鲁棒性(δ≤0.001)同时实现的计算复杂度下界。

Result: 证明实现全球规模人群的代表性和鲁棒性需要Ω(2^{d_context})操作，这是超多项式复杂度。当前RLHF系统通过牺牲代表性来解决这一困境，仅从同质标注者池收集10^3-10^4样本，而真实全球代表性需要10^7-10^8样本。

Conclusion: 对齐三难困境为观察到的RLHF病理现象提供了统一解释，并提出了通过战略性地放宽对齐要求来导航这些根本性权衡的具体方向。
...
### [271] [Profile Generators: A Link between the Narrative and the Binary Matrix Representation](https://arxiv.org/abs/2511.19506)
*Raoul H. Kutil,Georg Zimmermann,Barbara Strasser-Kirchweger,Christian Borgelt*

Main category: cs.LG

TL;DR: 本文开发了一种症状配置文件生成器来替代二元矩阵表示法，用于处理DSM-5中复杂认知障碍的诊断标准，解决了大规模症状组合矩阵不可行的问题。


<details>
  <summary>Details</summary>
Motivation: DSM-5中认知障碍的简化机器可操作表示法不适合处理最复杂的病例，而生成或应用完整的二元矩阵进行相似性计算由于症状组合数量庞大而不可行。

Method: 开发症状配置文件生成器，使用列表、集合和数字的严格预定义格式，将DSM-5的叙述形式与二元矩阵表示法连接起来，实现有效症状组合的自动生成。

Result: 通过将几种精神障碍表示为生成器形式并生成所有症状组合，发现复杂障碍的矩阵表示变得过于庞大而无法管理。开发了使用针对性生成器操作的配置文件缩减方法。

Conclusion: 生成器为大型矩阵提供了更易创建的二元表示，使得通过条件生成器计算复杂障碍之间的特定MPCS值成为可能，为处理复杂诊断路径提供了可读、适应性强且全面的替代方案。
...
### [272] [TouchFormer: A Robust Transformer-based Framework for Multimodal Material Perception](https://arxiv.org/abs/2511.19509)
*Kailin Lyu,Long Xiao,Jianing Zeng,Junhao Dong,Xuexin Liu,Zhuojun Zou,Haoyue Yang,Lin Shu,Jie Hao*

Main category: cs.LG

TL;DR: TouchFormer是一个鲁棒的多模态融合框架，通过模态自适应门控机制和跨模态注意力机制解决传统视觉方法在视觉受损条件下的性能下降问题，在材料感知任务中显著提升了分类精度。


<details>
  <summary>Details</summary>
Motivation: 传统视觉材料感知方法在视觉受损条件下性能大幅下降，现有方法对多模态输入进行简单融合，忽略了模态特定噪声、缺失模态等关键挑战，导致在多个基准任务中表现不佳。

Method: 提出TouchFormer框架，采用模态自适应门控机制和内外模态注意力机制自适应整合跨模态特征，并引入跨实例嵌入正则化策略提升细粒度子类别材料识别任务的分类精度。

Result: 在SSMC和USMC任务上分别实现了2.48%和6.83%的分类精度提升，真实机器人实验验证了框架在环境感知和解释方面的有效性。

Conclusion: TouchFormer为机器人在安全关键应用中的部署铺平了道路，代码和数据集将开源，视频可在补充材料中获取。
...
### [273] [Row-stochastic matrices can provably outperform doubly stochastic matrices in decentralized learning](https://arxiv.org/abs/2511.19513)
*Bing Liu,Boao Kong,Limin Lu,Kun Yuan,Chengcheng Zhao*

Main category: cs.LG

TL;DR: 本文比较了去中心化学习中两种权重处理策略的收敛性差异，通过加权希尔伯特空间框架获得了比欧几里得分析更严格的收敛率，揭示了行随机矩阵在特定条件下即使谱隙更小也能收敛更快的现象。


<details>
  <summary>Details</summary>
Motivation: 去中心化学习中节点权重λ的处理策略对收敛性能有重要影响，现有研究虽然表明两种策略在期望下降方向上相同，但对其收敛行为差异的本质理解不足，需要更深入的理论分析。

Method: 建立了加权希尔伯特空间框架L²(λ;ℝᵈ)，在该几何结构中分析行随机矩阵和双随机矩阵的性质差异，使用Rayleigh商和Loewner序特征值比较方法。

Result: 在加权希尔伯特空间中，行随机矩阵是自伴的而双随机矩阵不是，这产生了额外的惩罚项放大共识误差从而减慢收敛。行随机设计在特定条件下即使谱隙更小也能收敛更快。

Conclusion: 收敛差异不仅源于谱隙，还源于几何结构产生的惩罚项。通过拓扑条件可以保证行随机策略的优势，为实际拓扑设计提供了指导原则。
...
### [274] [Automating Deception: Scalable Multi-Turn LLM Jailbreaks](https://arxiv.org/abs/2511.19517)
*Adarsh Kumarappan,Ananya Mujoo*

Main category: cs.LG

TL;DR: 本文提出了一种自动化生成大规模心理驱动多轮越狱数据集的方法，评估了不同LLM在多轮对话攻击中的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 多轮对话攻击利用心理学原理（如登门槛效应）绕过LLM的安全对齐，而现有防御方法依赖难以扩展的手动数据集创建。

Method: 开发自动化管道，将登门槛技术系统化操作化为可复现模板，创建包含1,500个场景的基准数据集，评估7个模型在多轮和单轮条件下的表现。

Result: GPT系列模型对对话历史显著脆弱，攻击成功率增加高达32个百分点；Gemini 2.5 Flash表现出卓越韧性，几乎免疫攻击；Claude 3 Haiku有强但不完美的抵抗力。
...
### [275] [Shortcut Invariance: Targeted Jacobian Regularization in Disentangled Latent Space](https://arxiv.org/abs/2511.19525)
*Shivam Pal,Sakshi Varshney,Piyush Rai*

Main category: cs.LG

TL;DR: 提出一种通过功能不变性而非表示鲁棒性的方法来解决深度神经网络中的捷径学习问题，通过解耦潜在空间和注入各向异性噪声使分类器对捷径特征不敏感。
...
Method: 在解耦的潜在空间中，通过标签强相关性识别候选捷径特征，然后在训练期间注入有针对性的各向异性潜在噪声，使分类器对这些特征不敏感。

Result: 在已建立的捷径学习基准测试中实现了最先进的分布外性能。

Conclusion: 通过功能不变性而非表示鲁棒性的方法，可以简单有效地解决捷径学习问题，使分类器依赖更复杂的核心语义信号。
...
### [276] [Learning to Solve Weighted Maximum Satisfiability with a Co-Training Architecture](https://arxiv.org/abs/2511.19544)
*Kaidi Wan,Minghao Liu,Yong Lai*

Main category: cs.LG

TL;DR: SplitGNN是一种基于图神经网络的加权最大可满足性问题求解方法，通过协同训练架构和新的边分裂因子图表示，在大型加权MaxSAT基准测试中优于现代启发式求解器。


<details>
  <summary>Details</summary>
Motivation: 解决加权最大可满足性问题在现实应用中很重要，但传统方法在处理大规模和复杂实例时存在局限性，需要更有效的学习型方法。

Method: 提出SplitGNN，包含监督消息传递机制和无监督解增强层的协同训练架构，使用基于生成树和边分类的新图表示——边分裂因子图，并实现GPU加速的分数计算和基于松弛的优化。

Result: 实验显示SplitGNN比其他基于GNN的架构收敛速度快3倍，预测效果更好，在更大更难的加权MaxSAT基准测试中成功找到优于现代启发式求解器的解，并在不同结构实例上表现出优异的泛化能力。

Conclusion: SplitGNN通过创新的图表示和协同训练架构，为加权MaxSAT问题提供了高效且泛化能力强的解决方案，在性能和效率方面均优于现有方法。
...
### [277] [When Should Neural Data Inform Welfare? A Critical Framework for Policy Uses of Neuroeconomics](https://arxiv.org/abs/2511.19548)
*Yiven,Zhu*

Main category: cs.LG

TL;DR: 本文提出了一个神经经济学福利分析框架，将神经信号、计算决策模型和规范福利标准联系起来，强调神经数据只有在特定条件下才能为政策福利判断提供依据。


<details>
  <summary>Details</summary>
Motivation: 随着神经数据在政策制定和商业干预中被越来越多地使用，需要明确神经数据何时能合法地为福利判断提供依据，而不仅仅是描述行为。

Method: 开发了一个基于模型的非实证框架，使用actor-critic强化学习模型，将神经活动与潜在价值、预测误差以及福利主张联系起来。

Result: 提出了神经经济学福利推断清单，应用于成瘾、神经营销和环境政策等领域，表明内部奖励信号不能直接作为福利衡量标准。

Conclusion: 神经证据只有在神经-计算映射得到充分验证、决策模型能区分真实利益与情境依赖错误、福利标准明确指定并得到辩护的情况下，才能约束福利判断。
...
### [278] [Online Sparse Feature Selection in Data Streams via Differential Evolution](https://arxiv.org/abs/2511.19555)
*Ruiyang Xu*
...
<details>
  <summary>Details</summary>
Motivation: 现有在线稀疏流特征选择方法在特征评估方面存在显著局限性，导致性能下降，需要改进特征评估机制。

Method: 结合潜在因子分析模型进行缺失值填补，并使用差分进化算法评估特征重要性。

Result: 在六个真实世界数据集上的实验表明，ODESFS始终优于最先进的OSFS和OS2FS方法，能够选择最优特征子集并获得更高的准确率。

Conclusion: ODESFS方法通过改进的特征评估机制，有效解决了在线流特征选择中的数据不完整性和特征选择性能问题。
...
### [279] [Merging without Forgetting: Continual Fusion of Task-Specific Models via Optimal Transport](https://arxiv.org/abs/2511.19561)
...
TL;DR: OTMF是一种基于最优传输理论的模型融合框架，通过发现任务向量上的共同掩码来对齐任务特定模型的语义几何，解决参数插值引起的分布偏移问题。
...
Method: OTMF使用最优传输计划发现任务向量上的共同掩码，选择性提取可转移和任务无关的组件，同时保持每个任务的独特结构特性。支持持续融合范式，可增量集成新任务向量。

Result: 在多个视觉和语言基准测试中，OTMF在准确性和效率方面均达到最先进性能。

Conclusion: OTMF为模型融合提供了实用且理论上有价值的方法，能够有效解决分布偏移问题并实现高效的多任务融合。
...
### [280] [ModHiFi: Identifying High Fidelity predictive components for Model Modification](https://arxiv.org/abs/2511.19566)
*Dhruva Kashyap,Chaitanya Murti,Pranav K Nayak,Tanay Narshana,Chiranjib Bhattacharyya*

Main category: cs.LG

TL;DR: 提出了ModHiFi算法，无需训练数据或损失函数即可进行模型修改，包括剪枝和遗忘任务，在图像和语言模型上表现优异。


<details>
  <summary>Details</summary>
Motivation: 开源权重模型通常不提供训练数据或损失函数，使得模型修改任务（如剪枝、遗忘）受到限制。现有方法需要梯度或真实标签，在计算资源有限的情况下不可行。

Method: 理论证明全局重构误差受局部重构误差线性约束，提出子集保真度指标来量化组件重要性。在特征不相关情况下，通过子集保真度选择组件是最优的，据此提出ModHiFi算法。

Result: ModHiFi-P在ImageNet模型上比现有技术快11%，在语言模型上表现有竞争力；ModHiFi-U在CIFAR-10上无需微调即可完全遗忘，在Swin Transformers上表现良好。

Conclusion: 证明了无需训练数据或损失函数即可有效识别模型关键组件，提出的ModHiFi算法在模型剪枝和遗忘任务中表现出色。
...
### [281] [An Invariant Latent Space Perspective on Language Model Inversion](https://arxiv.org/abs/2511.19569)
*Wentao Ye,Jiaqi Hu,Haobo Wang,Xinpeng Ti,Zhiqing Xiao,Hao Chen,Liyao Li,Lei Feng,Sai Wu,Junbo Zhao*

Main category: cs.LG

TL;DR: 本文提出语言模型逆向攻击(LMI)威胁，基于不变潜在空间假设(ILSH)开发Inv^2A方法，通过轻量级逆编码器从输出恢复隐藏提示，在9个数据集上优于基线4.77% BLEU分数。
...
Method: 提出不变潜在空间假设(ILSH)，将LLM视为不变解码器，学习轻量级逆编码器将输出映射到去噪伪表示。采用两阶段训练：对比对齐(源不变性)和监督强化(循环不变性)。

Result: 在9个数据集上，Inv^2A平均比基线方法提高4.77% BLEU分数，同时减少对大逆向语料库的依赖。

Conclusion: 现有防御措施提供的保护有限，需要更强的防御策略来应对语言模型逆向攻击威胁。
...
### [282] [Neural Tractability via Structure: Learning-Augmented Algorithms for Graph Combinatorial Optimization](https://arxiv.org/abs/2511.19573)
*Jialiang Li,Weitong Chen,Mingyu Guo*

Main category: cs.LG

TL;DR: 提出一个结合神经模型推理效率和参数化算法最优性保证的混合框架，通过神经模型处理结构困难部分，参数化算法搜索剩余简单部分，实现更好的解质量和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 神经模型在图组合优化问题中推理快但解质量不如传统搜索算法，传统算法慢但能提供最优性保证。需要结合两者的优势。

Method: 使用参数化算法识别实例的结构简单部分，神经模型处理困难部分并提供指导信号，参数化算法整合指导信号系统搜索剩余部分。

Result: 在多个组合优化任务上，该框架获得优于纯神经求解器的解质量，与商业求解器相当，且具有更好的分布外泛化能力。

Conclusion: 该混合框架成功结合了神经模型的推理效率和参数化算法的最优性保证，解决了现有神经求解器解质量不足和泛化能力差的问题。
...
### [283] [Learning Massively Multitask World Models for Continuous Control](https://arxiv.org/abs/2511.19584)
*Nicklas Hansen,Hao Su,Xiaolong Wang*

Main category: cs.LG

TL;DR: 提出了一个包含200个多样化任务的基准测试，并开发了Newt模型——一个语言条件化的多任务世界模型，通过演示预训练和在线交互联合优化，在多任务性能和数据效率方面优于强基线方法。


<details>
  <summary>Details</summary>
Motivation: 通用控制需要能够在多种任务和实体上操作的智能体，但当前的强化学习研究主要局限于单任务或离线设置，缺乏对在线RL可扩展性的探索。受基础模型（大规模预训练+轻量RL）的启发，研究是否可以通过在线交互训练单一智能体处理数百个任务。

Method: 1) 创建包含200个多样化任务的基准测试，每个任务都有语言指令、演示和可选图像观察；2) 开发Newt模型——语言条件化多任务世界模型，先通过演示预训练获取任务感知表示和动作先验，然后与所有任务的在线交互联合优化。

Result: 实验表明Newt在多任务性能和数据效率方面优于强基线方法，表现出强大的开环控制能力，并能快速适应未见过的任务。

Conclusion: 该工作证明了在线强化学习在多任务连续控制中的可扩展性，通过大规模多任务训练和预训练策略可以实现通用控制智能体，为未来研究提供了基准环境和工具。
...
### [284] [Many Ways to be Right: Rashomon Sets for Concept-Based Neural Networks](https://arxiv.org/abs/2511.19636)
...
<details>
  <summary>Details</summary>
Motivation: 现代神经网络存在Rashomon效应，即多个模型可以达到相同性能但依赖不同特征或推理模式。然而在深度架构中发现这种多样性具有挑战性，因为连续参数空间包含无数数值不同但行为相似的近最优解。

Method: 结合轻量级适配器模块与多样性正则化训练目标，无需从头训练即可高效构建多样化的基于概念的深度模型集合。
...
### [285] [Lower Complexity Bounds for Nonconvex-Strongly-Convex Bilevel Optimization with First-Order Oracles](https://arxiv.org/abs/2511.19656)
*Kaiyi Ji*

Main category: cs.LG

TL;DR: 本文针对双层优化问题，在光滑非凸-强凸设置下，建立了新的困难实例，为确定性和随机一阶oracle模型提供了非平凡的下界。


<details>
  <summary>Details</summary>
Motivation: 尽管双层优化的上界保证已被广泛研究，但由于双层结构的复杂性，下界方面的进展有限。本文旨在填补这一空白，为双层优化的复杂性提供更准确的理解。

Method: 开发新的困难实例，在光滑非凸-强凸设置下，分析确定性和随机一阶oracle模型的下界。通过构造特定的优化问题实例来证明所需的最小oracle调用次数。

Result: 在确定性情况下，任何一阶零尊重算法至少需要Ω(κ^{3/2}ε^{-2})次oracle调用来找到ε-精确的驻点；在随机情况下，至少需要Ω(κ^{5/2}ε^{-4})次随机oracle调用。这些下界优于单层非凸优化和非凸-强凸min-max问题的最优下界。

Conclusion: 当前双层优化的上下界之间存在显著差距，即使是简化情况（如二次下层目标）也需要进一步研究，以理解标准一阶oracle下双层优化的最优复杂性。
...
### [286] [Structured Noise Modeling for Enhanced Time-Series Forecasting](https://arxiv.org/abs/2511.19657)
*Sepideh Koohfar*

Main category: cs.LG

TL;DR: 提出了一种预测-模糊-去噪框架，通过结构化噪声建模提高时间序列预测的保真度，结合可学习的高斯过程模块生成平滑相关扰动，提升多尺度时间模式的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的时间序列预测面临多尺度时间模式（从宏观趋势到微观波动）的挑战，现有神经网络模型难以有效表示这些交互动态，导致预测不稳定和下游应用可靠性降低。

Method: 采用预测-模糊-去噪框架，包含可学习高斯过程模块生成平滑相关扰动，鼓励预测主干捕捉长程结构，同时专用细化模型恢复高分辨率时间细节，各组件联合训练实现自然能力分工。

Result: 在电力、交通和太阳能数据集上的实验显示，该方法在多时间跨度预测精度和稳定性方面取得一致提升，模块化设计还支持作为轻量级增强层用于预训练模型。

Conclusion: 该框架通过增强细粒度时间预测的可靠性和可解释性，为能源、基础设施等时间关键领域的预测驱动决策支持系统提供了更可信的AI解决方案。
...
### [287] [BlockCert: Certified Blockwise Extraction of Transformer Mechanisms](https://arxiv.org/abs/2511.17645)
*Sandro Andric*

Main category: cs.LG

TL;DR: BlockCert是一个用于变压器机制认证块提取的框架，支持认证的局部编辑，提供近似误差边界、覆盖度指标和底层工件的哈希值，并通过Lipschitz组合定理将局部保证提升为全局偏差界限。


<details>
  <summary>Details</summary>
Motivation: 机械可解释性和模型编辑通常缺乏正式保证，无法明确提取或编辑后的模型在相关输入上与原始模型的偏差程度。

Method: 给定预训练变压器和提示分布，BlockCert提取残差块的结构化替代实现，并提供机器可检查的证书。使用Lipschitz组合定理在Lean 4中形式化局部保证到全局偏差边界的提升。

Result: 在GPT-2 small、TinyLlama-1.1B-Chat和Llama-3.2-3B上应用该框架，获得高每块覆盖度和小残差误差，在TinyLlama设置中完全拼接模型在压力提示上的困惑度与基线相差约6e-5。

Conclusion: 具有明确证书的块提取对于真实变压器语言模型是可行的，为机械可解释性和模型行为的形式推理提供了实用桥梁。
...
### [288] [Demystifying Diffusion Objectives: Reweighted Losses are Better Variational Bounds](https://arxiv.org/abs/2511.19664)
*Jiaxin Shi,Michalis K. Titsias*

Main category: cs.LG

TL;DR: 本文提出了扩散模型重新加权损失函数的新理论解释，通过构建时间相关的变分下界来改进标准证据下界，从而降低数据模型KL散度。


<details>
  <summary>Details</summary>
Motivation: 为广泛使用的扩散模型重新加权损失函数提供理论依据，改进标准证据下界，提升模型性能。

Method: 构建时间相关的变分下界级联，结合这些下界产生重新加权目标函数，适用于连续高斯扩散和掩码扩散模型。

Result: 在掩码扩散模型中展示了显著改进，像素空间图像建模质量接近连续扩散模型水平，并为掩码图像模型的简单加权方案提供了理论支持。

Conclusion: 提出的理论框架为扩散模型重新加权损失提供了严格的理论基础，并在实践中表现出优越性能。
...
### [289] [TREASURE: A Transformer-Based Foundation Model for High-Volume Transaction Understanding](https://arxiv.org/abs/2511.19693)
...
TL;DR: TREASURE是一个基于Transformer的多用途交易数据基础模型，能同时捕捉消费者行为和支付网络信号，在异常行为检测和推荐系统等应用中表现优异。
...
Method: TREASURE采用基于Transformer的架构，包含专门的静态和动态属性输入模块，以及针对高基数分类属性的高效训练范式。

Result: 验证结果显示，TREASURE作为独立模型将异常行为检测性能提升111%，作为嵌入提供者将推荐模型性能提升104%。

Conclusion: TREASURE是一个有效的交易数据基础模型，在行业级数据集上验证了其优越性能，为交易数据分析提供了有价值的解决方案。
...
### [290] [TiCT: A Synthetically Pre-Trained Foundation Model for Time Series Classification](https://arxiv.org/abs/2511.19694)
*Chin-Chia Michael Yeh,Uday Singh Saini,Junpeng Wang,Xin Dai,Xiran Fan,Jiarui Sun,Yujie Fan,Yan Zheng*

Main category: cs.LG

TL;DR: TiCT是一个基于Transformer的时间序列基础模型，专门用于上下文学习分类任务，仅使用合成数据进行预训练，无需微调即可在推理时实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据普遍存在，但标注成本高昂。现有的大规模时间序列模型主要关注预测任务，缺乏无需微调的通用分类基础模型。
...
Conclusion: TiCT证明了仅使用合成数据预训练的模型能够实现有效的上下文学习分类，为时间序列基础模型开发提供了新方向。
...
### [291] [CafeQ: Calibration-free Quantization via Learned Transformations and Adaptive Rounding](https://arxiv.org/abs/2511.19705)
*Ziteng Sun,Adrian Benton,Samuel Kushnir,Asher Trockman,Vikas Singh,Suhas Diggavi,Ananda Theertha Suresh*

Main category: cs.LG

TL;DR: 提出无需校准数据的后训练量化方法，通过结构化矩阵变换和自适应舍入优化，在Gemma 2模型上实现性能提升，接近需要校准数据的GPTQ方法。


<details>
  <summary>Details</summary>
Motivation: 传统后训练量化方法依赖校准数据，但在现实场景中校准数据可能不可用或受隐私法规限制，需要开发不依赖校准数据的量化方法。

Method: 设计无校准数据的量化损失代理函数，使用结构化矩阵变换处理单个矩阵，对计算图中直接交互的权重对使用双矩阵变换和自适应舍入方法。

Result: 在Gemma 2 9B模型上，4位量化平均基准分数从61.9提升到62.4，3位量化从52.0提升到60.6，计算开销增加不到3%。

Conclusion: 该方法在不依赖校准数据的情况下实现了与需要校准数据的GPTQ方法相当的性能，为实际部署提供了更灵活的解决方案。
...
### [292] [Training-Free Active Learning Framework in Materials Science with Large Language Models](https://arxiv.org/abs/2511.19730)
*Hongchen Wang,Rafael Espinosa Castañeda,Jay R. Werber,Yao Fehlis,Edward Kim,Jason Hattrick-Simpers*

Main category: cs.LG

TL;DR: LLM-AL框架使用大语言模型进行主动学习，在材料科学四个数据集上相比传统ML模型减少70%以上实验次数，性能更优且探索性更强。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习主动学习方法存在冷启动问题和领域特定特征工程限制，大语言模型凭借预训练知识和通用表示能力，可直接从文本描述中提出实验建议。

Method: 提出LLM-AL框架，在迭代少样本设置下运行，探索两种提示策略：简洁数值输入和扩展描述文本，适应不同特征类型的数据集。

Result: 在所有数据集上，LLM-AL将找到最优候选所需的实验次数减少超过70%，始终优于传统ML模型，进行更广泛探索性搜索且更快达到最优。
...
### [293] [DISCO: A Browser-Based Privacy-Preserving Framework for Distributed Collaborative Learning](https://arxiv.org/abs/2511.19750)
...
TL;DR: DISCO是一个开源的分布式协作学习平台，允许非技术用户在无需共享原始数据或编程知识的情况下协作构建机器学习模型。


<details>
  <summary>Details</summary>
Motivation: 数据共享存在隐私、知识产权和法律限制等问题，这不仅分散了预测模型的统计能力，还造成了可访问性偏见，使得只有具备资源的人才能获得准确的模型。

Method: DISCO的Web应用在浏览器中本地训练模型，支持跨平台使用（包括智能手机）。采用模块化设计，提供联邦学习和去中心化学习两种范式选择，支持不同级别的隐私保证和多种权重聚合策略。

Result: 开发了一个开源平台（https://github.com/epfml/disco），并提供了展示Web界面（https://discolab.ai）。

Conclusion: DISCO为协作机器学习提供了一个实用的解决方案，能够在保护数据隐私的同时实现模型协作训练，并支持模型个性化和偏见弹性。
...
### [294] [When +1% Is Not Enough: A Paired Bootstrap Protocol for Evaluating Small Improvements](https://arxiv.org/abs/2511.19794)
*Wenzhang Du*

Main category: cs.LG

TL;DR: 提出了一种保守的评估协议，用于在有限计算预算下判断机器学习中1-2%的性能提升是否真实，避免因随机因素导致的过度宣称。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习论文中报告的1-2%性能提升往往缺乏不确定性估计和显著性检验，难以区分是真实算法改进还是随机噪声。

Method: 基于配对多轮次运行、偏差校正加速(BCa)自助法置信区间和符号翻转置换检验的评估协议。

Result: 在CIFAR-10、CIFAR-10N和AG News数据集上的实验表明，单次运行和非配对t检验容易错误地宣称显著提升，而提出的协议在仅有3次运行的情况下不会错误宣称显著性。

Conclusion: 在有限计算预算下，保守的评估协议是判断小幅度性能提升的更安全默认方法。
...
### [295] [Terminal Velocity Matching](https://arxiv.org/abs/2511.19797)
...
<details>
  <summary>Details</summary>
Motivation: 现有的流匹配方法在少步生成建模中存在局限性，需要改进以实现更高效的生成过程。

Method: 提出TVM方法，引入最小架构改动实现稳定单阶段训练，开发融合注意力核支持Jacobian向量积的反向传播。

Result: 在ImageNet-256x256上，TVM单步评估达到3.29 FID，4步评估达到1.99 FID；在ImageNet-512x512上，单步4.32 FID，4步2.94 FID。

Conclusion: TVM在一步/少步生成模型中实现了最先进的性能，无需从零开始训练。
...
### [296] [Scalable Data Attribution via Forward-Only Test-Time Inference](https://arxiv.org/abs/2511.19803)
...
### [297] [EfficientXpert: Efficient Domain Adaptation for Large Language Models via Propagation-Aware Pruning](https://arxiv.org/abs/2511.19935)
*Songlin Zhao,Michael Pitts,Zhuwei Qin*

Main category: cs.LG

TL;DR: EfficientXpert是一个轻量级领域剪枝框架，通过传播感知剪枝准则和高效适配器更新算法，将通用预训练模型一步转换为稀疏的领域适应专家模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限环境中部署困难，现有压缩方法在跨领域时泛化能力差或开销高。
...
Conclusion: 领域依赖的结构变化会降低通用剪枝掩码的有效性，需要针对每个领域采用自适应、领域感知的剪枝策略。
...
### [298] [Learning to Clean: Reinforcement Learning for Noisy Label Correction](https://arxiv.org/abs/2511.19808)
*Marzi Heidari,Hanping Zhang,Yuhong Guo*

Main category: cs.LG

TL;DR: 提出RLNLC框架，将噪声标签校正问题建模为强化学习任务，通过策略网络迭代修正噪声标签并训练预测模型，在多个基准数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 噪声标签会严重降低机器学习模型的预测性能，需要有效的方法来处理这一问题。

Method: 将噪声标签校正定义为强化学习问题，构建包含数据和标签的状态空间、标签校正动作空间以及评估校正效果的奖励机制，使用actor-critic方法训练深度特征表示策略网络。

Result: 在多个基准数据集上的实验表明，RLNLC框架在噪声标签学习任务中持续优于当前最先进的技术。

Conclusion: RLNLC通过强化学习框架有效解决了噪声标签校正问题，为处理标签噪声提供了新的解决方案。
...
### [299] [The Devil in the Details: Emergent Misalignment, Format and Coherence in Open-Weights LLMs](https://arxiv.org/abs/2511.20104)
*Craig Dickson*

Main category: cs.LG

TL;DR: 现代开源权重模型在微调后会出现新兴错位现象，但错位率显著低于专有模型GPT-4o。JSON格式输出会使错位率翻倍。


<details>
  <summary>Details</summary>
Motivation: 评估当前一代开源权重模型是否像Qwen-2.5系列一样具有对新兴错位的抵抗力，并测量不同架构和规模模型的错位鲁棒性。

Method: 在九个现代开源权重模型（Gemma 3和Qwen 3系列，1B-32B参数）上复制新兴错位效应，比较微调前后模型在不安全代码生成任务上的表现。

Result: 微调模型错位率为0.68%（基础模型为0.07%），远低于GPT-4o的20%。JSON格式输出使错位率翻倍（0.96% vs 0.42%）。

Conclusion: 新兴错位是现代开源权重模型中可复现的现象，但错位率显著低于专有系统，且结构约束可能通过减少模型拒绝的'自由度'来绕过安全训练。
...
### [300] [Provably Outlier-resistant Semi-parametric Regression for Transferable Calibration of Low-cost Air-quality Sensors](https://arxiv.org/abs/2511.19810)
...
TL;DR: 提出RESPIRE技术用于校准低成本空气质量传感器的CO检测，相比基线方法在跨站点、跨季节、跨传感器设置中具有更好的预测性能，并提供可解释的模型和异常值抵抗能力。


<details>
  <summary>Details</summary>
Motivation: 低成本空气质量传感器在校准过程中面临昂贵、耗时的问题，特别是在大规模地理分散部署时，需要更高效的校准方法。

Method: RESPIRE技术提供了一种训练算法，能够抵抗异常值，并建立可解释的模型来检测模型过拟合情况。

Result: 基于在4个站点、2个季节和6个传感器包上的广泛部署数据，RESPIRE在跨站点、跨季节和跨传感器设置中表现出改进的预测性能。
...
### [301] [Beyond Components: Singular Vector-Based Interpretability of Transformer Circuits](https://arxiv.org/abs/2511.20273)
*Areeb Ahmad,Abhinav Joshi,Ashutosh Modi*

Main category: cs.LG

TL;DR: 本文提出了一种细粒度的视角，将Transformer中的注意力头和MLP层分解为正交奇异方向，揭示了单个组件内部叠加的独立计算。
...
Method: 通过将Transformer组件分解为正交奇异方向，识别出单个头或MLP内部叠加的独立计算，并在IOI、GP、GT等标准任务上进行验证。

Result: 发现先前识别的典型功能头（如名称移动器）编码了多个重叠的子功能，这些子功能与不同的奇异方向对齐；计算图中的节点在特定的低秩方向上表现出强激活。

Conclusion: Transformer计算比先前假设的更加分布式、结构化和组合化，这为细粒度的机制可解释性和对模型内部更深层次理解开辟了新途径。
...
### [302] [Mosaic Pruning: A Hierarchical Framework for Generalizable Pruning of Mixture-of-Experts Models](https://arxiv.org/abs/2511.19822)
*Wentao Hu,Mingkuan Zhao,Shuangyong Song,Xiaoyan Zhu,Xin Lai,Jiayin Wang*

Main category: cs.LG

TL;DR: 提出了Mosaic Pruning (MoP)方法，通过"聚类-选择"过程构建功能全面的专家集合，解决稀疏专家混合模型在跨域应用中的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏专家混合模型存在显著的静态内存开销，现有后训练剪枝方法基于单一语料库，导致模型在其他领域应用时性能严重下降，需要为每个新领域重新剪枝。

Method: 使用跨任务域专家性能相似性指标对专家进行功能聚类，然后基于提出的激活变异性评分从每个簇中选择最具代表性的专家，构建功能互补的专家集合。

Result: 在多个MoE模型上的实验表明，MoP显著优于现有方法，在通用任务上获得7.24%的性能提升，在数学推理和代码生成等专业任务上获得8.92%的提升。

Conclusion: Mosaic Pruning能够保留原始模型能力的完整功能表示，使剪枝后的模型能够处理多样化的下游任务，解决了跨域泛化问题。
...
### [303] [Geometry of Decision Making in Language Models](https://arxiv.org/abs/2511.20315)
*Abhinav Joshi,Divyanshu Bhatt,Ashutosh Modi*

Main category: cs.LG

TL;DR: 通过内在维度分析LLMs在多项选择题任务中的决策过程，发现早期层在低维流形上操作，中间层扩展维度，后期层压缩维度并收敛到决策相关表示。


<details>
  <summary>Details</summary>
Motivation: LLMs在多样化任务中表现出强大的泛化能力，但其内部决策过程仍然不透明，需要研究其隐藏表示的几何特性。
...
### [304] [GED-Consistent Disentanglement of Aligned and Unaligned Substructures for Graph Similarity Learning](https://arxiv.org/abs/2511.19837)
*Zhentao Zhan,Xiaoliang Xu,Jingjing Wang,Junmei Wang*

Main category: cs.LG

TL;DR: 提出GCGSim框架解决现有GNN方法在图相似度计算中的局限性，通过图级匹配和子结构级编辑成本实现更准确的图编辑距离估计


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的图编辑距离计算方法存在节点中心匹配范式与GED核心原则不匹配的问题，导致无法捕捉全局结构对应关系和错误归因编辑成本

Method: 提出GCGSim框架，采用图级匹配和子结构级编辑成本，包含三个核心技术贡献，学习解耦且语义有意义的子结构表示

Result: 在四个基准数据集上的实验表明GCGSim达到最先进性能，有效学习解耦且语义有意义的子结构表示

Conclusion: GCGSim通过图级匹配和子结构级编辑成本解决了现有方法的局限性，实现了更准确和一致的图编辑距离计算
...
### [305] [Soft Adaptive Policy Optimization](https://arxiv.org/abs/2511.20347)
*Chang Gao,Chujie Zheng,Xiong-Hui Chen,Kai Dang,Shixuan Liu,Bowen Yu,An Yang,Shuai Bai,Jingren Zhou,Junyang Lin*

Main category: cs.LG

TL;DR: 提出Soft Adaptive Policy Optimization (SAPO)方法，用平滑的温度控制门替代硬裁剪，自适应地衰减离策略更新，同时保留有用的学习信号，解决了强化学习中策略优化的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于分组的策略优化方法（如GSPO和GRPO）通过硬裁剪缓解重要性比率高方差问题，但难以同时保持稳定性和有效学习。特别是在混合专家模型中，令牌级重要性比率的高方差会导致更新不稳定。

Method: SAPO使用平滑的温度控制门替代硬裁剪，形成连续信任区域。当序列包含少量高度离策略令牌时，SAPO选择性地仅对违规令牌进行降权，保留近策略令牌的学习信号，提高样本效率。
...
### [306] [Cisco Time Series Model Technical Report](https://arxiv.org/abs/2511.19841)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多分辨率输入的时间序列基础模型，特别针对可观测性领域的数据特点进行优化，同时保持通用预测能力。

Method: 对流行的仅解码器时间序列模型(TimesFM)进行架构创新，使其能够接受多分辨率输入，构建多分辨率仅解码器模型，并在300B+数据点上训练，其中超过一半来自可观测性领域。

Result: 模型在可观测性数据集上实现卓越性能，在通用预测基准(GIFT-Eval)上保持相似性能，多分辨率结构使模型在长上下文输入上做出更准确的预测。

Conclusion: 多分辨率架构创新成功提升了时间序列基础模型在特定领域的性能，同时不损害其通用预测能力，为时间序列预测提供了有效的解决方案。
...
### [307] [SX-GeoTree: Self-eXplaining Geospatial Regression Tree Incorporating the Spatial Similarity of Feature Attributions](https://arxiv.org/abs/2511.19845)
*Chaogui Kang,Lijian Luo,Qingfeng Guan,Yu Liu*

Main category: cs.LG

TL;DR: SX-GeoTree是一种自解释地理空间回归树，通过结合三个耦合目标（MSE、全局Moran's I和模块度最大化）来解决传统决策树在空间依赖性和解释稳定性方面的不足。
...
Method: 在递归分裂过程中集成三个耦合目标：MSE（杂质减少）、全局Moran's I（空间残差控制）和基于共识相似网络的模块度最大化（解释鲁棒性），该网络由GWR系数距离和SHAP归因距离构成。

Result: 在两个任务（福建县级GDP和西雅图房价）上的实验显示，SX-GeoTree在保持竞争性预测精度的同时，改善了残差空间均匀性并将归因共识度翻倍。

Conclusion: 该框架展示了如何将空间相似性（通过GWR扩展几何邻近性）嵌入可解释模型，推进可信地理空间机器学习，并为领域感知可解释性提供了可转移模板。
...
### [308] [Accelerating Wireless Distributed Learning via Hybrid Split and Federated Learning Optimization](https://arxiv.org/abs/2511.19851)
*Kun Guo,Xuefei Li,Xijun Wang,Howard H. Yang,Wei Feng,Tony Q. S. Quek*

Main category: cs.LG

TL;DR: HSFL结合了联邦学习和分割学习的优势，通过联合优化学习模式选择、批次大小以及通信和计算资源来加速分布式学习。


<details>
  <summary>Details</summary>
Motivation: 联邦学习支持低延迟并行训练但可能收敛到精度较低的模型，而分割学习通过顺序训练实现更高精度但延迟增加。为了结合两者的优势，需要研究混合模式下的加速方法。
...
Conclusion: HSFL通过联合优化学习模式、批次大小和资源分配，能够有效加速分布式学习过程，在保持精度的同时降低延迟。
...
### [309] [Frailty-Aware Transformer for Recurrent Survival Modeling of Driver Retention in Ride-Hailing Platforms](https://arxiv.org/abs/2511.19893)
*Shuoyan Xu,Yu Zhang,Eric J. Miller*

Main category: cs.LG

TL;DR: 提出FACT框架，使用Transformer建模网约车司机的空闲行为作为循环生存过程，在预测性能上优于传统和深度学习生存模型。


<details>
  <summary>Details</summary>
Motivation: 网约车平台具有高频、行为驱动的特点，但生存分析在司机行为建模中的应用仍较少探索，需要更准确的风险估计来支持平台留存策略。

Method: 将空闲行为建模为循环生存过程，使用Transformer框架捕捉长期时间依赖关系，结合因果掩码和司机特定嵌入来建模潜在异质性。

Result: 在多伦多网约车数据上，FACT模型在时间相关C指数和Brier得分方面表现最佳，优于经典和深度学习生存模型。
...
### [310] [Adaptivity and Universality: Problem-dependent Universal Regret for Online Convex Optimization](https://arxiv.org/abs/2511.19937)
...
<details>
  <summary>Details</summary>
Motivation: 现有通用在线学习方法缺乏问题依赖的自适应性，无法提供与梯度变化V_T相关的遗憾界限，而V_T在随机优化和博弈快速收敛中起关键作用。

Method: 提出了UniGrad方法，包括UniGrad.Correct和UniGrad.Bregman两个变体，采用元算法结构，通过O(log T)个基础学习器实现自适应梯度变化的遗憾界限。

Result: UniGrad方法同时实现了通用性和自适应性，UniGrad.Correct保持RVU属性，UniGrad.Bregman达到最优遗憾界限。UniGrad++进一步将每轮梯度查询减少到1次。

Conclusion: UniGrad是首个同时实现通用性和自适应梯度变化的在线学习方法，在保持理论最优性的同时提供了实际计算效率的改进。
...
### [311] [Optimize Flip Angle Schedules In MR Fingerprinting Using Reinforcement Learning](https://arxiv.org/abs/2511.19941)
...
<details>
  <summary>Details</summary>
Motivation: MRF中的参数优化是一个复杂的高维序列决策问题，传统方法难以找到最优的翻转角调度方案

Method: 引入强化学习框架，自动化选择翻转角参数，优化脉冲序列以最大化参数空间中指纹的区分度

Result: 学习到的调度方案呈现非周期性模式，增强了指纹可分性，并可能减少重复时间数量

Conclusion: 强化学习为MRF序列优化提供了有效方法，能够生成非周期性翻转角调度，提高性能并可能加速采集
...
### [312] [Differential Smoothing Mitigates Sharpening and Improves LLM Reasoning](https://arxiv.org/abs/2511.19942)
*Jingchu Gai,Guanning Zeng,Huaqing Zhang,Aditi Raghunathan*

Main category: cs.LG

TL;DR: 本文提出了一种名为"差分平滑"的原理性方法，通过理论证明和实验验证，解决了RL微调大语言模型时出现的多样性崩溃问题，在保持正确性的同时提升输出多样性。


<details>
  <summary>Details</summary>
Motivation: RL微调大语言模型会导致多样性崩溃，现有启发式方法存在临时性、效果不稳定、甚至相互矛盾的问题，需要建立理论基础并提出更优的解决方案。

Method: 首先形式化证明了RL微调导致多样性崩溃的机制（选择和强化偏差），然后提出差分平滑方法，仅在正确轨迹上应用奖励修改来同时提升正确性和多样性。

Result: 在1B到7B参数的模型上，在CountDown和数学推理等领域的实验中，差分平滑方法在Pass@1和Pass@k指标上均优于原始RL和基于熵的启发式方法，在AIME24数据集上提升达6.7%。

Conclusion: 差分平滑方法在理论上优于现有启发式方法，实验证明其能一致性地提升正确性和多样性，为解决RL微调中的多样性崩溃问题提供了原理性解决方案。
...
### [313] [Hierarchical Spatio-Temporal Attention Network with Adaptive Risk-Aware Decision for Forward Collision Warning in Complex Scenarios](https://arxiv.org/abs/2511.19952)
*Haoran Hu,Junren Shi,Shuo Jiang,Kun Cheng,Xia Yang,Changhao Piao*

Main category: cs.LG

TL;DR: 提出了一种集成前向碰撞预警框架，结合分层时空注意力网络和动态风险阈值调整算法，解决了计算复杂性和建模不足的双重挑战，在保持实时性的同时显著提升了预警精度和可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前前向碰撞预警系统存在计算成本高、交互模型简化导致的不可靠性，以及传统静态阈值预警的高误报率问题，需要平衡多智能体交互建模精度与实时决策适应性。

Method: 采用分层时空注意力网络（HSTAN）进行解耦架构设计：图注意力网络处理空间交互，级联GRU与自注意力机制处理时序依赖；结合保形分位数回归生成预测区间，通过动态风险阈值调整算法将预测转换为及时预警。

Result: 在NGSIM数据集上，推理时间仅需12.3毫秒（比Transformer方法快73%），平均位移误差降至0.73米（比Social_LSTM提升42.2%）；系统整体F1分数达0.912，误报率8.2%，预警提前时间2.8秒。

Conclusion: 该框架在复杂环境中展现出卓越性能和实际部署可行性，通过高效的时空建模和自适应风险阈值机制，成功解决了前向碰撞预警系统的关键挑战。
...
### [314] [Prompt Fairness: Sub-group Disparities in LLMs](https://arxiv.org/abs/2511.19956)
*Meiyu Zhong,Noel Teku,Ravi Tandon*

Main category: cs.LG

TL;DR: 该论文研究了LLM中的提示公平性问题，发现相同问题但不同措辞的提示会引发LLM不同的响应质量。作者提出信息论指标量化这种偏差，并通过多数投票和提示中性化等干预措施改善公平性。


<details>
  <summary>Details</summary>
Motivation: LLM对不同用户/风格的提示措辞会产生不同的响应质量，这种提示公平性问题需要被量化和解决。

Method: 使用信息论指标（子组敏感性和跨组一致性）量化偏差，提出多数投票和提示中性化等干预措施。

Result: 实验显示，在缓解措施前，跨组分歧值达到0.28，范围在0.14-0.22之间；应用缓解措施后，分歧值持续下降，最大差距降至0.22，许多距离降至0.17以下。

Conclusion: 提出的干预措施能有效改善LLM在不同用户群体间的响应稳定性和公平性。
...
### [315] [ParaBlock: Communication-Computation Parallel Block Coordinate Federated Learning for Large Language Models](https://arxiv.org/abs/2511.19959)
...
<details>
  <summary>Details</summary>
Motivation: 在联邦学习大语言模型时，即使单个模型块也包含大量参数，导致通信延迟显著，特别是对于资源受限的客户端。

Method: 建立两个并行线程分别处理通信和计算，通过并行化来提高通信效率。

Result: 在通用指令跟随和数学推理任务上的实验表明，ParaBlock不仅保持强大性能，还显著提高了通信效率。

Conclusion: ParaBlock成功解决了联邦学习大语言模型中的通信效率问题，同时保持了模型的收敛性能。
...
### [316] [Stragglers Can Contribute More: Uncertainty-Aware Distillation for Asynchronous Federated Learning](https://arxiv.org/abs/2511.19966)
*Yujia Wang,Fenglong Ma,Jinghui Chen*

Main category: cs.LG

TL;DR: FedEcho是一个新颖的异步联邦学习框架，通过不确定性感知蒸馏来解决异步延迟和数据异构性带来的挑战，在保持隐私的同时提升模型性能。
...
Result: 通过广泛实验证明，FedEcho在异步联邦学习基准上持续优于现有方法，实现了稳健的性能，且无需访问私有客户端数据。

Conclusion: FedEcho通过不确定性感知蒸馏有效缓解了异步联邦学习中过时更新和数据异构性的负面影响，提供了一种平衡的解决方案。
...
### [317] [Rethinking Semi-Supervised Node Classification with Self-Supervised Graph Clustering](https://arxiv.org/abs/2511.19976)
*Songbo Wang,Renchi Yang,Yurui Lai,Xiaoyang Lin,Tsz Nam Chan*

Main category: cs.LG

TL;DR: NCGC是一个将自监督图聚类和半监督分类结合的统一框架，通过软正交GNN生成节点表示，利用聚类模块为未标记节点提供自监督训练，在多任务目标下实现分类和聚类的协同增强。


<details>
  <summary>Details</summary>
Motivation: 现实图中的节点倾向于形成紧密的社区/簇，这些簇包含丰富信号可以补偿半监督节点分类中的标签稀缺性，但现有方法未充分探索这一特性。

Method: 提出NCGC框架：1）理论统一GNN和图谱聚类的优化目标，开发软正交GNN；2）包含自监督图聚类模块，使用两个聚类目标和Sinkhorn-Knopp归一化生成平衡软伪标签；3）通过多任务目标结合分类和聚类损失。

Result: 在七个真实图数据集上的广泛实验表明，NCGC框架在使用各种经典GNN骨干网络时，始终显著优于流行的GNN模型和近期基线方法。

Conclusion: NCGC通过整合自监督图聚类和半监督分类，有效利用了图中的社区结构信息，在半监督节点分类任务上取得了显著性能提升。
...
### [318] [Operator Learning at Machine Precision](https://arxiv.org/abs/2511.19980)
...
TL;DR: CHONKNORIS是一种能够达到机器精度的算子学习方法，通过回归Tikhonov正则化牛顿-康托罗维奇更新的椭圆算子的Cholesky因子，而非直接回归解算子本身，实现了高精度求解非线性偏微分方程。


<details>
  <summary>Details</summary>
Motivation: 现有的神经算子学习方法在增加复杂度时往往无法显著提高精度，与简单的核方法和传统降阶模型相当。本文旨在解决这一缺陷，开发能够达到机器精度的算子学习方法。

Method: CHONKNORIS方法基于数值分析思想，回归Tikhonov正则化牛顿-康托罗维奇更新中椭圆算子的Cholesky因子，通过展开迭代构建神经架构，实现压缩映射。还提出了FONKNORIS基础模型变体，聚合多个预训练专家来求解新的非线性PDE。

Result: 在多个非线性正向和反问题上的基准测试表明，CHONKNORIS能够达到机器精度，包括非线性椭圆方程、Burgers方程、非线性Darcy流问题、Calderón问题、逆波散射问题和地震成像问题。FONKNORIS能够准确求解未见过的非线性PDE如Klein-Gordon和Sine-Gordon方程。

Conclusion: CHONKNORIS通过结合数值分析和算子学习，实现了机器精度的PDE求解，为科学计算中的算子学习提供了新的范式，并展示了基础模型在求解新PDE问题上的潜力。
...
### [319] [Rethinking Message Passing Neural Networks with Diffusion Distance-guided Stress Majorization](https://arxiv.org/abs/2511.19984)
...
<details>
  <summary>Details</summary>
Motivation: 传统消息传递神经网络(MPNNs)存在过平滑和过相关问题，这源于其最小化Dirichlet能量的目标导向和邻域聚合操作。需要新的优化框架来克服这些限制。

Method: 基于应力优化和正交正则化的优化框架，引入节点间的扩散距离来指导新的消息传递操作，并开发了高效的距离近似算法。

Result: DDSM在15个强基线模型上表现一致且显著优于，在异质性和同质性图数据上都取得了优越性能。

Conclusion: DDSM通过新的优化框架有效解决了MPNN的过平滑和过相关问题，为图结构数据学习提供了更强大的模型。
...
### [320] [On-Demand Multi-Task Sparsity for Efficient Large-Model Deployment on Edge Devices](https://arxiv.org/abs/2511.19986)
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏优化方法针对单个任务独立优化，忽视了任务频繁切换时产生的显著I/O开销问题。

Method: 将权重分解为可重用的块粒度单元，跨任务对齐稀疏结构以最大化重叠，动态加载下一个任务所需的少量差异块集合。

Result: 在真实自动驾驶平台上，相比现有稀疏方法，平均加速任务切换超过6.6倍。

Conclusion: 该框架通过参数重用和动态加载机制，有效缓解了传统整体方法固有的冷启动延迟问题。
...
### [321] [RankOOD -- Class Ranking-based Out-of-Distribution Detection](https://arxiv.org/abs/2511.19996)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于交叉熵损失的深度学习模型在ID类预测中会产生排序模式，但OOD样本可能不遵循这种模式，这为OOD检测提供了新的思路。

Method: 首先用初始分类器提取每个类的排序列表，然后用Plackett-Luce损失进行第二轮训练，将类排序作为预测变量。

Result: 在TinyImageNet近OOD评估基准上达到SOTA性能，将FPR95降低了4.3%。

Conclusion: RankOOD通过利用ID类预测中的排序模式，为OOD检测提供了一种有效的新方法。
...
### [322] [Zero-Shot Transfer Capabilities of the Sundial Foundation Model for Leaf Area Index Forecasting](https://arxiv.org/abs/2511.20004)
*Peining Zhang,Hongchen Qin,Haochen Zhang,Ziqi Guo,Guiling Wang,Jinbo Bi*

Main category: cs.LG

TL;DR: 研究发现，在零样本设置下，Sundial基础模型能够超越完全训练的LSTM模型，前提是输入上下文窗口足够长（覆盖超过1-2个完整季节周期）。


<details>
  <summary>Details</summary>
Motivation: 研究时间序列基础模型在农业监测中LAI预测的零样本预测能力，探索通用基础模型是否能在无需任务特定调优的情况下超越专门的监督模型。
...
Conclusion: 预训练的时间序列基础模型在农业和环境应用中具有作为即插即用预测器的强大潜力，首次证明通用基础模型可在无需任务特定调优的情况下超越专门的监督模型。
...
### [323] [iRadioDiff: Physics-Informed Diffusion Model for Indoor Radio Map Construction and Localization](https://arxiv.org/abs/2511.20015)
...
Method: 提出采样自由的扩散框架iRadioDiff，通过接入点位置、材料反射和透射系数的物理信息提示进行条件生成，并引入衍射点、强透射边界和视距轮廓等多路径关键先验来指导生成过程。

Result: 实验表明iRadioDiff在室内无线电地图构建和基于接收信号强度的室内定位任务中达到最先进性能，并能在不同布局和材料配置下有效泛化。

Conclusion: iRadioDiff能够准确建模非平稳场不连续性，高效构建物理一致的无线电地图，为室内定位提供可靠的环境感知电磁表示。
...
### [324] [Cross-Contrastive Clustering for Multimodal Attributed Graphs with Dual Graph Filtering](https://arxiv.org/abs/2511.20030)
*Haoran Zheng,Renchi Yang,Hongtao Wang,Jianliang Xu*

Main category: cs.LG

TL;DR: 提出DGF方案用于多模态属性图聚类，通过双重图滤波和三元交叉对比学习解决预训练模型输出中的模态相关性低和特征噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有多视图聚类方法依赖视图间高相关性，但忽略了多模态属性图中预训练模型输出的低模态相关性和强特征噪声问题。

Method: 提出双重图滤波方案，包含特征去噪组件和三元交叉对比训练策略，在模态、邻居和社区层面进行对比学习。

Result: 在8个基准MMAG数据集上的实验表明，DGF在聚类质量上显著优于现有最优方法。

Conclusion: DGF通过有效处理多模态属性中的噪声和低相关性，实现了更优的聚类性能。
...
### [325] [RED-F: Reconstruction-Elimination based Dual-stream Contrastive Forecasting for Multivariate Time Series Anomaly Prediction](https://arxiv.org/abs/2511.20044)
*PengYu Chen,Xiaohou Shi,Yuan Chang,Yan Sun,Sajal K. Das*

Main category: cs.LG

TL;DR: 提出了RED-F框架，通过重构消除和双流对比预测来解决多元时间序列中微弱异常前兆的检测问题，将绝对信号检测转化为相对轨迹比较任务。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法在预测微弱异常前兆时，倾向于重构正常模式而淹没前兆信号，难以检测隐藏在正常信号中的细微异常前兆。

Method: RED-F框架包含重构消除模型(REM)和双流对比预测模型(DFM)。REM使用时频混合机制消除前兆生成纯净基线，DFM接收纯净基线和保留前兆的原始序列进行对比预测，采用多序列预测目标增强预测敏感性。

Result: 在六个真实世界数据集上的广泛实验表明，RED-F在异常预测任务中具有优越性能。

Conclusion: RED-F通过将困难的绝对信号检测转化为更简单的相对轨迹比较，有效放大了微弱的前兆信号，在异常预测方面表现出色。
...
### [326] [SOMBRL: Scalable and Optimistic Model-Based RL](https://arxiv.org/abs/2511.20066)
*Bhavya Sukhija,Lenart Treven,Carmelo Sferrazza,Florian Dörfler,Pieter Abbeel,Andreas Krause*

Main category: cs.LG

TL;DR: SOMBRL是一种基于模型强化学习方法，通过乐观不确定性原则实现高效探索，在多种设置下具有次线性遗憾性能，并在仿真和硬件实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决基于模型强化学习中系统动态未知时的有效探索挑战，需要直接从在线交互中学习。

Method: 学习不确定性感知的动态模型，并贪婪地最大化外在奖励与智能体认知不确定性的加权和，兼容任何策略优化器或规划器。

Result: 在状态基和视觉控制环境中表现优异，在动态RC车硬件上超越现有最优方法，展示了基于原则探索的优势。

Conclusion: SOMBRL为基于模型强化学习提供了灵活、可扩展且基于原则的探索解决方案，在理论和实验验证中均表现出色。
...
### [327] [QiMeng-CRUX: Narrowing the Gap between Natural Language and Verilog via Core Refined Understanding eXpression](https://arxiv.org/abs/2511.20099)
...
TL;DR: 提出了CRUX结构化中间表示空间，通过两阶段训练框架CRUX-V，显著提升了从自然语言到Verilog代码生成的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于自由形式自然语言描述的硬件代码生成方法存在模糊、冗余和非结构化问题，难以生成精确的Verilog代码。

Method: 引入CRUX结构化中间空间，设计包含联合表达建模和双空间优化的两阶段训练框架CRUX-V。

Result: 在多个Verilog生成基准测试中达到最先进性能，特别是在复杂设计任务中表现优异，CRUX空间对其他代码模型也具可迁移性。

Conclusion: CRUX有效缩小了自由形式自然语言描述与精确Verilog生成之间的差距，为硬件代码生成提供了结构化解决方案。
...
### [328] [Multivariate Forecasting of Bitcoin Volatility with Gradient Boosting: Deterministic, Probabilistic, and Feature Importance Perspectives](https://arxiv.org/abs/2511.20105)
...
<details>
  <summary>Details</summary>
Motivation: 加密货币市场具有高度非线性和高方差特性，传统模型难以有效捕捉其波动规律。研究旨在探索LightGBM模型在比特币波动率预测中的表现，并提供对波动驱动因素的可解释性分析。

Method: 使用69个预测因子构建LightGBM模型进行确定性和概率性预测。概率预测采用两种分位数方法：基于pinball损失函数的直接分位数回归，以及将点预测转换为预测分布的残差模拟方法。使用增益和置换特征重要性技术识别主要波动驱动因素。

Result: LightGBM模型在比特币波动率预测中表现优异，能够有效捕捉加密货币市场的非线性特征。特征重要性分析一致显示交易量、滞后波动率指标、投资者关注度和市值是主要波动驱动因素。

Conclusion: LightGBM模型不仅能够准确预测比特币波动率，还能提供对波动动态的可解释性见解，在加密货币市场分析中具有重要应用价值。
...
### [329] [CLIMATEAGENT: Multi-Agent Orchestration for Complex Climate Data Science Workflows](https://arxiv.org/abs/2511.20109)
...
Method: 采用多代理架构：Orchestrate-Agent和Plan-Agent协调任务分解，Data-Agent动态内省API合成下载脚本，Coding-Agent生成Python代码、可视化并带自校正循环。

Result: 在Climate-Agent-Bench-85基准测试中，ClimateAgent实现100%任务完成率和8.32的报告质量分数，优于GitHub-Copilot(6.27)和GPT-5基线(3.26)。
...
### [330] [IDAP++: Advancing Divergence-Based Pruning via Filter-Level and Layer-Level Optimization](https://arxiv.org/abs/2511.20141)
...
<details>
  <summary>Details</summary>
Motivation: 现有神经网络压缩方法通常单独处理滤波器冗余或架构冗余，缺乏统一的指导原则来同时优化不同层级的结构冗余。

Method: 两阶段优化：第一阶段使用迭代散度感知剪枝去除冗余滤波器；第二阶段分析层间信息传播贡献，选择性移除对性能影响最小的整个层。

Result: 在多种现代架构和数据集上验证，实现了显著的模型压缩同时保持竞争力精度，参数减少效果与最先进方法相当且在卷积网络到Transformer等架构上表现更优。
...
### [331] [On the Limits of Momentum in Decentralized and Federated Optimization](https://arxiv.org/abs/2511.20168)
*Riccardo Zaccone,Sai Praneeth Karimireddy,Carlo Masone*

Main category: cs.LG

TL;DR: 本文分析了联邦学习中动量方法在循环客户端参与下的表现，理论上证明了动量仍然受到统计异质性的影响，且与SGD类似，任何比$Θ(1/t)$更快的步长衰减都会导致收敛到依赖于初始化和异质性边界的常数值。
...
Result: 动量仍然不可避免地受到统计异质性的影响，与SGD类似，任何比$Θ(1/t)$更快的步长衰减都会导致收敛到依赖于初始化和异质性边界的常数值。

Conclusion: 动量方法在联邦学习中无法完全克服统计异质性的影响，数值结果和深度学习实验都证实了这一理论发现的实际相关性。
...
### [332] [AdaCap: An Adaptive Contrastive Approach for Small-Data Neural Networks](https://arxiv.org/abs/2511.20170)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络在小样本表格数据集上表现不佳，而树模型仍然占主导地位。需要一种方法在小样本场景下增强神经网络的性能。

Method: 提出AdaCap训练方案，结合置换对比损失和Tikhonov闭式输出映射，通过元预测器根据数据集特征预测AdaCap的适用性。

Result: 在85个真实世界回归数据集上，AdaCap在小样本场景下带来一致且统计显著的改进，特别是对残差模型效果更佳。

Conclusion: AdaCap作为一种定向正则化机制，在神经网络最脆弱的地方进行强化，有效提升了小样本表格数据上的性能。
...
### [333] [Learning Subgroups with Maximum Treatment Effects without Causal Heuristics](https://arxiv.org/abs/2511.20189)
...
<details>
  <summary>Details</summary>
Motivation: 现有亚组发现方法存在两个主要问题：一是依赖点状条件处理效应估计，将亚组发现问题转化为更困难的点状估计问题；二是使用缺乏严格理论依据的因果启发式规则。

Method: 在基于划分的模型假设下，将最优亚组发现问题转化为数据生成模型的恢复问题，从而转化为标准回归或分类问题。使用CART方法实例化该框架来学习具有最大处理效应的亚组。

Result: 在大量合成和半合成数据集上的实验表明，该方法比广泛基线方法更准确地识别具有最大处理效应的亚组。

Conclusion: 在结构因果模型框架下直接解决亚组发现问题，避免了因果启发式规则，能够更准确地识别最大处理效应亚组，为精准决策提供可靠支持。
...
### [334] [In-Context Compositional Learning via Sparse Coding Transformer](https://arxiv.org/abs/2511.20194)
*Wei Chen,Jingxi Yu,Zichen Miao,Qiang Qiu*

Main category: cs.LG

TL;DR: 提出了一种基于稀疏编码的注意力机制改进方法，用于增强Transformer在组合学习任务中的性能，特别是在需要组合泛化的任务上。


<details>
  <summary>Details</summary>
Motivation: Transformer在处理组合学习任务时存在局限，因为它们并非专门为组合任务设计且缺乏结构性归纳偏置。受稀疏编码原理启发，希望通过改进注意力机制来增强模型对组合规则的学习能力。

Method: 将注意力块重新解释为通过两个学习字典（编码字典和解码字典）的投影映射。编码字典将输入分解为稀疏系数，这些系数表示输入的组合结构；稀疏系数用于线性组合解码字典原子来生成输出。

Result: 在S-RAVEN和RAVEN数据集上验证了方法的有效性，对于某些组合泛化任务，该方法在标准Transformer失败时仍能保持性能。

Conclusion: 提出的基于稀疏编码的注意力改进方法能够有效学习和应用组合规则，增强了Transformer在组合学习任务中的能力。
...
### [335] [Communication-Efficient Learning for Satellite Constellations](https://arxiv.org/abs/2511.20220)
...
Method: 设计了一种通信高效的联邦学习算法，采用本地训练减少通信次数，压缩减少通信大小，并提出了误差反馈机制来提高准确性，该机制还可作为算法无关的误差反馈方案广泛应用。

Result: 通过理论分析算法的收敛性，并在真实空间场景中进行仿真，与现有技术相比展现出优越性能。

Conclusion: 所提出的算法在卫星星座联邦学习中实现了通信效率与准确性的良好平衡，误差反馈机制具有更广泛的应用潜力。
...
### [336] [Decoupling and Damping: Structurally-Regularized Gradient Matching for Multimodal Graph Condensation](https://arxiv.org/abs/2511.20222)
*Lian Shen,Zhendan Chen,Yinhui jiang,Meijia Song,Ziming Su,Juan Liu,Xiangrong Liu*

Main category: cs.LG

TL;DR: 提出SR-GM方法解决多模态图压缩中的梯度冲突和结构噪声放大问题，通过梯度解耦和结构阻尼正则化提升压缩效果。


<details>
  <summary>Details</summary>
Motivation: 多模态图在关键应用中日益重要，但大规模训练带来计算负担。现有图压缩方法在多模态场景下因梯度冲突和结构噪声放大而失效。

Method: SR-GM框架包含两个组件：梯度解耦机制通过正交投影解决模态间冲突，结构阻尼正则化利用图的Dirichlet能量将拓扑结构转化为稳定力。

Result: 实验表明SR-GM显著提高精度并加速收敛，消融研究证实同时解决梯度冲突和结构放大对性能提升至关重要。

Conclusion: 该方法为资源受限环境中的多模态图学习提供了可扩展方案，压缩后的图具有良好的跨架构泛化能力。
...
### [337] [DiCaP: Distribution-Calibrated Pseudo-labeling for Semi-Supervised Multi-Label Learning](https://arxiv.org/abs/2511.20225)
...
### [338] [Leveraging weights signals -- Predicting and improving generalizability in reinforcement learning](https://arxiv.org/abs/2511.20234)
...
Result: 实验结果表明改进的PPO算法能产生具有更强泛化能力的智能体
...
### [339] [Interpretable Air Pollution Forecasting by Physics-Guided Spatiotemporal Decoupling](https://arxiv.org/abs/2511.20257)
...
Method: 将污染物浓度的时空行为分解为两个透明加性模块：物理引导的传输核（考虑风和地理条件）和可解释的注意力机制（学习局部响应并归因于历史滞后和外部驱动因素）。

Result: 在斯德哥尔摩地区数据集上评估，模型在多个预测时间范围内持续优于最先进的基线方法。

Conclusion: 该模型结合了高预测性能和时空可解释性，为实际空气质量管理提供了更可靠的基础。
...
### [340] [HVAdam: A Full-Dimension Adaptive Optimizer](https://arxiv.org/abs/2511.20277)
*Yiheng Zhang,Shaowu Wu,Yuanzhuo Xu,Jiajun Wu,Shang Xu,Steve Drew,Xiaoguang Niu*

Main category: cs.LG

TL;DR: 提出Anon优化器，通过可调节的自适应机制在SGD和Adam之间切换甚至超越两者，解决了自适应优化器泛化性能差的问题。


<details>
  <summary>Details</summary>
Motivation: 自适应优化器（如Adam）在大规模模型训练中成功，但在传统架构上泛化性能不如非自适应方法（如SGD），主要原因是预调节器的自适应限制了优化器适应不同优化场景的能力。

Method: 提出Anon优化器，具有连续可调的自适应特性，引入增量延迟更新（IDU）机制确保收敛，比AMSGrad的最大值跟踪策略更灵活且增强对梯度噪声的鲁棒性。

Result: 在图像分类、扩散模型和语言建模任务上，Anon持续优于最先进的优化器，理论分析在凸和非凸设置下都建立了收敛保证。
...
### [341] [MXtalTools: A Toolkit for Machine Learning on Molecular Crystals](https://arxiv.org/abs/2511.20327)
*Michael Kilgour,Mark E. Tuckerman,Jutta Rogal*

Main category: cs.LG

TL;DR: MXtalTools是一个用于分子晶体数据驱动建模的灵活Python包，支持机器学习研究分子固态体系。


<details>
  <summary>Details</summary>
Motivation: 为分子晶体的数据驱动建模提供灵活的工具包，促进分子固态体系的机器学习研究。

Method: 包含数据集合成整理、模型训练推理工作流、晶体参数化表示、结构采样优化、端到端可微分晶体构建分析等模块化功能。

Result: 开发了支持CUDA加速的高通量晶体建模工具包，代码开源并提供详细文档。

Conclusion: MXtalTools提供了模块化的分子晶体建模功能，可集成到现有工作流或构建新的建模流程。
...
### [342] [Complexity Reduction Study Based on RD Costs Approximation for VVC Intra Partitioning](https://arxiv.org/abs/2511.20349)
*M. E. A. Kherchouche,F. Galpin,T. Dumas,F. Schnitzler,D. Menard,L. Zhang*

Main category: cs.LG

TL;DR: 该论文提出了两种机器学习方法来加速VVC帧内分区的复杂度优化，包括基于回归的RD成本预测方法和基于强化学习的决策方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决VVC帧内分区在率失真优化过程中涉及的穷举搜索带来的高计算复杂度问题。

Method: 1. 基于回归的方法：预测编码单元的归一化RD成本；2. 基于强化学习的方法：使用DQN算法从CU决策轨迹中学习，将分区决策建模为马尔可夫决策过程。

Result: 两种方法都通过预定义阈值来选择合适的分区方式，且方法具有尺寸独立性并考虑了相邻块的RD成本作为输入特征。

Conclusion: 提出的两种机器学习技术能够有效加速VVC帧内分区的RDO过程，相比现有方法具有更好的适应性。
...
### [343] [PRISM: Periodic Representation with multIscale and Similarity graph Modelling for enhanced crystal structure property prediction](https://arxiv.org/abs/2511.20362)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法往往忽略了晶体结构固有的周期性边界条件和多尺度相互作用，这限制了图表示学习在晶体结构表征中的应用效果。

Method: 采用专家模块集合，每个模块专门编码周期性系统的不同结构和化学方面，明确整合多尺度表示和周期性特征编码。

Result: 在多个基于晶体结构的基准测试中，PRISM提高了最先进的预测精度。

Conclusion: PRISM框架通过有效处理周期性边界条件和多尺度相互作用，显著增强了晶体性质预测能力。
...
### [344] [MoRE: Batch-Robust Multi-Omics Representations from Frozen Pre-trained Transformers](https://arxiv.org/abs/2511.20382)
*Audrey Pei-Hsuan Chen*

Main category: cs.LG

TL;DR: MoRE是一个多组学表示学习框架，利用冻结的预训练transformer将异质数据对齐到共享潜在空间，通过参数高效微调策略实现跨样本和跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 解决多组学数据中极端维度、模态异质性和批次效应的挑战，探索预训练transformer在多组学整合中的应用潜力。

Method: 使用冻结的预训练transformer作为骨干，附加轻量级模态特定适配器和任务自适应融合层，联合优化掩码建模目标与监督对比和批次不变对齐损失。

Result: MoRE在批次鲁棒性和生物保守性方面表现竞争性，同时显著减少了可训练参数数量，在未见细胞类型和平台上具有良好的泛化能力。

Conclusion: MoRE为通用组学基础模型提供了实用步骤，展示了参数高效微调在多组学表示学习中的有效性。
...
### [345] [Identifying environmental factors associated with tetrodotoxin contamination in bivalve mollusks using eXplainable AI](https://arxiv.org/abs/2511.20395)
...
<details>
  <summary>Details</summary>
Motivation: 自2012年以来，欧洲温带水域的双壳类软体动物中发现河豚毒素，这种污染带来食品安全风险和经济损失，需要早期预测方法。现有研究指出浅水栖息地和水温是主要驱动因素，但环境因素与毒素污染的时间关系尚未探索。

Method: 开发了一个基于深度学习的可解释模型，输入气象和水文特征，输出河豚毒素污染的存在与否。

Result: 结果显示日出时间、日落时间、全球辐射、水温和氯化物浓度对河豚毒素污染贡献最大，有效日照时数是重要驱动因素。

Conclusion: 该可解释深度学习模型识别出日照时数、全球辐射、水温和水氯化物浓度与双壳类软体动物河豚毒素污染相关，为食品行业和主管部门提供了有价值的海洋毒素风险缓解工具。
...
### [346] [Model-Based Learning of Whittle indices](https://arxiv.org/abs/2511.20397)
*Joël Charles-Rebuffé,Nicolas Gast,Bruno Gaujal*

Main category: cs.LG

TL;DR: BLINQ是一种新的基于模型的算法，用于学习可索引、通信和单链马尔可夫决策过程的Whittle指数，在样本效率和计算成本方面优于现有的Q学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有的Q学习方法在样本效率和计算成本方面存在不足，需要开发更高效的算法来学习Whittle指数。

Method: 通过构建MDP的经验估计，然后使用现有算法的扩展版本计算其Whittle指数。

Result: BLINQ在样本数量上显著优于Q学习方法，且总计算成本更低，即使在Q学习使用预训练神经网络加速的情况下也是如此。

Conclusion: BLINQ为学习Whittle指数提供了一种高效且收敛的解决方案，在样本效率和计算复杂度方面具有优势。
...
### [347] [Short-Range Oversquashing](https://arxiv.org/abs/2511.20406)
*Yaaqov Mishayev,Yonatan Sverdlov,Tal Amir,Nadav Dym*

Main category: cs.LG

TL;DR: 本文发现过压缩现象不仅出现在长距离任务中，短距离任务也会出现，并区分了瓶颈现象和梯度消失两种机制。研究表明虚拟节点无法解决短距离瓶颈，而Transformer是更好的解决方案。


<details>
  <summary>Details</summary>
Motivation: MPNN在图学习中广泛应用，但其处理长距离信息的能力受限于过压缩现象。现有研究对此有分歧：一些推荐Graph Transformer，另一些认为可以通过虚拟节点等技术在MPNN框架内缓解。

Method: 通过分析短距离任务中的过压缩现象，区分瓶颈现象和梯度消失两种机制，并测试虚拟节点和Transformer在不同场景下的表现。

Result: 发现短距离任务也会出现过压缩，且现有解释无法捕捉短距离瓶颈效应。虚拟节点无法解决此问题，而Transformer在类似任务中表现成功。
...
### [348] [Tight Margin-Based Generalization Bounds for Voting Classifiers over Finite Hypothesis Sets](https://arxiv.org/abs/2511.20407)
...
<details>
  <summary>Details</summary>
Motivation: 为投票分类器建立更精确的泛化理论保证，填补现有理论在权衡参数方面不够紧致的空白

Method: 通过理论分析证明基于间隔的泛化界，考虑多个关键参数之间的权衡关系

Result: 得到了第一个在假设集大小、间隔、训练点比例、样本数量和失败概率之间渐近紧致的泛化界

Conclusion: 该工作为投票分类器提供了更精确的理论基础，在多个关键参数的权衡上达到了最优的渐近紧致性
...
### [349] [Diffusion for Fusion: Designing Stellarators with Generative AI](https://arxiv.org/abs/2511.20445)
*Misha Padidar,Teresa Huang,Andrew Giuliani,Marina Spivak*

Main category: cs.LG

TL;DR: 使用条件扩散模型快速生成具有理想特性的准对称仿星器设计，在训练数据之外的特征上也能生成性能良好的设计，准对称性偏差小于5%。


<details>
  <summary>Details</summary>
Motivation: 仿星器设计传统上是耗时长的PDE约束优化问题，机器学习方法特别是生成模型有望加速这一过程。

Method: 在QUASR数据库上训练条件扩散模型，生成具有特定纵横比和平均旋转变换的准对称仿星器设计。

Result: 生成的仿星器在未见过的特征上表现良好，准对称性偏差小于5%，接近1%的目标。

Conclusion: 生成建模在仿星器设计中具有广阔前景，能够快速产生高质量设计，为聚变研究提供新工具。
...
### [350] [Towards Trustworthy Wi-Fi Sensing: Systematic Evaluation of Deep Learning Model Robustness to Adversarial Attacks](https://arxiv.org/abs/2511.20456)
*Shreevanth Krishnaa Gopalakrishnan,Stephen Hailes*

Main category: cs.LG

TL;DR: 本文系统评估了CSI深度学习模型在不同威胁模型下的鲁棒性，发现小型模型虽然高效但在对抗攻击下鲁棒性较差，物理可实现的信号空间扰动相比无约束特征空间攻击成功率更低，对抗训练能有效缓解这些漏洞。
...
Result: 实验表明小型模型在干净数据上表现同等但鲁棒性显著较差；物理可实现的信号空间扰动相比无约束特征空间攻击成功率显著降低；对抗训练能提高平均鲁棒准确率，仅对干净性能造成适度下降。

Conclusion: 这些发现为鲁棒性评估提供了量化基准，并为设计安全可信的人类中心感知系统提供了指导原则。
...
### [351] [NVIDIA Nemotron Parse 1.1](https://arxiv.org/abs/2511.20478)
...
TL;DR: Nemotron-Parse-1.1是一个轻量级文档解析和OCR模型，相比前代在OCR、Markdown格式、表格解析和图像文本提取方面有显著改进，支持更长的输出序列，采用885M参数的编码器-解码器架构。


<details>
  <summary>Details</summary>
Motivation: 提升文档解析和OCR能力，在保持轻量化的同时提供更强的功能覆盖，包括处理视觉密集文档和多种文档元素类型。
...
Conclusion: Nemotron-Parse-1.1是一个功能强大的轻量级文档解析模型，在多个方面超越前代，并通过公开发布模型权重、NIM容器和训练数据子集来促进社区使用。
...
### [352] [MTBBench: A Multimodal Sequential Clinical Decision-Making Benchmark in Oncology](https://arxiv.org/abs/2511.20490)
...
TL;DR: MTBBench是一个模拟分子肿瘤委员会决策过程的多模态基准测试，用于评估LLMs在临床肿瘤学中的多模态和纵向推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态LLM基准测试无法捕捉真实临床工作流程的复杂性，特别是分子肿瘤委员会这种需要多专家协作、整合异质数据和随时间演变的决策环境。

Method: 开发MTBBench基准测试，包含临床挑战性的多模态和纵向肿瘤学问题，通过临床验证的真实标注，并提供一个基于基础模型的工具框架来增强多模态和纵向推理。

Result: 现有LLMs在MTBBench上表现不可靠，经常产生幻觉，难以处理时间分辨数据，无法调和冲突证据或不同模态。使用工具框架后，任务级性能分别提升9.0%和11.2%。

Conclusion: MTBBench为推进多模态LLM在精准肿瘤学中的推理、可靠性和工具使用提供了一个具有挑战性和真实性的测试平台。
...
### [353] [DP-MicroAdam: Private and Frugal Algorithm for Training and Fine-tuning](https://arxiv.org/abs/2511.20509)
...
Method: 开发DP-MicroAdam优化器，具有内存高效和稀疏感知特性，在随机非凸优化中证明以最优O(1/√T)速率收敛（受隐私相关常数影响）。

Result: 在CIFAR-10、大规模ImageNet训练和预训练transformer的私有微调等基准测试中，DP-MicroAdam优于现有自适应DP优化器，与DP-SGD相比达到竞争或更优的准确率。

Conclusion: 自适应优化可以在差分隐私下同时提高性能和稳定性，为DP训练提供了更有效的优化方法。
...
### [354] [Adam Simplified: Bias Correction Simplified](https://arxiv.org/abs/2511.20516)
*Sam Laing,Antonio Orvieto*

Main category: cs.LG

TL;DR: 本文通过系统实验证明Adam优化器中的偏差校正组件在最优超参数配置下对最终测试性能没有改进，有时甚至有害，挑战了该组件普遍适用的传统观点。


<details>
  <summary>Details</summary>
Motivation: Adam优化器是现代深度学习的基石，但其各个组件的经验必要性往往被视为理所当然。本文聚焦研究偏差校正组件的作用，该组件的贡献一直未被充分理解。

Method: 通过在视觉和语言建模任务上进行一系列系统消融实验，分析偏差校正的影响，并将其重新解释为一种隐式学习率调度形式。

Result: 在最优超参数配置下，包含偏差校正不会改善最终测试性能；除非实施适当的学习率调度，否则偏差校正有时会损害性能。偏差校正的行为强烈依赖于平滑超参数β₁, β₂的选择。

Conclusion: 研究结果挑战了偏差校正组件普遍适用的传统观点，表明其必要性可能被高估。
...
### [355] [Feature-Modulated UFNO for Improved Prediction of Multiphase Flow in Porous Media](https://arxiv.org/abs/2511.20543)
*Alhasan Abdellatif,Hannah P. Menke,Ahmed H. Elsheikh,Florian Doster,Kamaljit Singh*

Main category: cs.LG

TL;DR: UFNO-FiLM通过引入FiLM层解耦标量输入与空间特征，并使用空间加权损失函数，在保持UFNO优势的同时解决了其处理标量输入效率低和误差敏感性不均的问题。


<details>
  <summary>Details</summary>
Motivation: UFNO虽然通过并行UNet路径改进了FNO，但存在两个主要问题：1) 将标量输入作为空间分布场处理导致频域冗余计算；2) 标准损失函数未考虑误差敏感性的空间变化。

Method: 1) 使用FiLM层解耦标量输入与空间特征，避免在傅里叶变换中引入恒定信号；2) 采用空间加权损失函数，优先学习关键区域。

Result: 在地下多相流实验中，相比UFNO实现了21%的气体饱和度平均绝对误差降低。

Conclusion: UFNO-FiLM通过FiLM层和空间加权损失的创新设计，显著提升了预测精度，特别是在关键物理区域。
...
### [356] [E2E-GRec: An End-to-End Joint Training Framework for Graph Neural Networks and Recommender Systems](https://arxiv.org/abs/2511.20564)
*Rui Xue,Shichao Zhu,Liang Qin,Guangmou Pan,Yang Song,Tianfu Wu*

Main category: cs.LG

TL;DR: E2E-GRec是一个端到端的图神经网络推荐训练框架，解决了传统两阶段方法计算开销大和缺乏联合优化的问题。


<details>
  <summary>Details</summary>
Motivation: 传统两阶段GNN推荐系统存在计算开销大和缺乏联合优化的问题，导致GNN对推荐任务的指导性不足。

Method: 提出E2E-GRec框架，包含高效子图采样、图特征自编码器作为自监督任务、以及基于Gradnorm的动态损失平衡的两级特征融合机制。

Result: 在离线评估和在线A/B测试中均优于传统方法，在用户停留时长和跳过视频数量等指标上取得显著提升。

Conclusion: E2E-GRec通过端到端训练统一GNN与推荐系统，在多个推荐指标上实现显著增益，证明了其有效性。
...
### [357] [MSTN: Fast and Efficient Multivariate Time Series Model](https://arxiv.org/abs/2511.20577)
*Sumit S Shevtekar,Chandresh K Maurya,Gourab Sil*

Main category: cs.LG

TL;DR: MSTN是一种多尺度时间网络，通过分层多尺度架构和序列建模，自适应地建模从毫秒级到长期依赖的时间模式，在时间序列预测、插补和分类任务中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据具有高度非平稳性和复杂动态，现有模型依赖固定尺度结构先验，导致对时间动态的过度正则化，限制了它们建模全谱时间变化的能力。

Method: 提出MSTN框架，包含：(i)多尺度卷积编码器构建分层特征金字塔；(ii)序列建模组件处理长期时间依赖；(iii)门控融合机制结合SE和多头时间注意力进行动态特征集成。
...
### [358] [A Tale of Two Geometries: Adaptive Optimizers and Non-Euclidean Descent](https://arxiv.org/abs/2511.20584)
*Shuo Xie,Tianhao Wang,Beining Wu,Zhiyuan Li*

Main category: cs.LG

TL;DR: 本文揭示了自适应优化器与归一化最速下降(NSD)之间的紧密联系，建立了非凸设置下的自适应平滑理论，并证明了自适应平滑性能够实现带Nesterov动量的自适应优化器加速，这在某些非欧几里得几何下是标准平滑性无法实现的。


<details>
  <summary>Details</summary>
Motivation: 自适应优化器在仅适应当前梯度时会退化为归一化最速下降，但两者的分析依赖于不同的几何概念。本文旨在统一这两种算法家族的理论框架，特别是在非凸设置下扩展自适应平滑理论。

Method: 将自适应平滑理论扩展到非凸设置，引入自适应梯度方差概念，并分析带Nesterov动量的自适应优化器的加速性能。

Result: 证明了自适应平滑性精确刻画了自适应优化器的收敛性，在凸设置下实现了标准平滑性无法达到的加速效果，并在随机优化中获得了维度无关的收敛保证。

Conclusion: 自适应平滑性和自适应梯度方差为理解自适应优化器提供了统一的理论框架，揭示了在某些非欧几里得几何下自适应优化器相对于传统方法的优势。
...
### [359] [Anatomica: Localized Control over Geometric and Topological Properties for Anatomical Diffusion Models](https://arxiv.org/abs/2511.20587)
*Karim Kadry,Abdallah Abdelwahed,Shoaib Goraya,Ajay Manicka,Naravich Chutisilp,Farhad Nezami,Elazer Edelman*

Main category: cs.LG

TL;DR: Anatomica是一个推理时框架，用于生成具有局部几何拓扑控制的多类解剖体素图，通过可微分惩罚函数和持续同调技术控制几何和拓扑特征。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够灵活控制解剖结构几何和拓扑特性的框架，用于合成数据集的设计，支持虚拟试验和机器学习工作流。

Method: 使用不同维度的立方体控制域切片相关子结构，通过体素矩控制几何特征，通过持续同调控制拓扑特征，并在潜在扩散模型中实现神经场解码器部分提取子结构。

Result: Anatomica能够灵活应用于多种解剖系统，在任意维度和坐标系下组合约束来控制复杂结构。

Conclusion: 该框架实现了解剖特性的高效控制，为合成数据集的合理设计提供了有效工具。
...
### [360] [Attention Trajectories as a Diagnostic Axis for Deep Reinforcement Learning](https://arxiv.org/abs/2511.20591)
*Charlotte Beylier,Hannah Selder,Arthur Fleig,Simon M. Hofmann,Nico Scherf*

Main category: cs.LG

TL;DR: 提出了注意力导向指标（ATOMs）来研究强化学习智能体在训练过程中的注意力发展，通过三个Pong游戏变体实验验证了该方法能区分不同注意力模式并揭示注意力发展的阶段性特征。


<details>
  <summary>Details</summary>
Motivation: 强化学习智能体的学习过程除了数学公式外缺乏深入理解，需要新的方法来研究其注意力发展过程。

Method: 引入注意力导向指标（ATOMs），在三个设计不同的Pong游戏变体上进行实验，结合行为评估分析智能体的注意力模式。

Result: ATOMs成功区分了在不同游戏变体上训练的智能体的注意力模式，这些差异转化为行为差异；训练过程中观察到注意力发展呈现阶段性特征，且在不同游戏变体中一致。
...
### [361] [Latent Diffusion Inversion Requires Understanding the Latent Space](https://arxiv.org/abs/2511.20592)
*Mingxing Rao,Bowen Qu,Daniel Moyer*

Main category: cs.LG

TL;DR: 本文研究发现潜在扩散模型在潜在空间中存在非均匀记忆现象，提出基于解码器回拉度量的维度排序方法，显著提升了成员推理攻击性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型反转技术主要关注数据域扩散模型，忽略了潜在空间生成模型中编码器/解码器对和潜在代码的作用，特别是潜在扩散模型的记忆特性。

Method: 提出基于解码器回拉度量的维度排序方法，识别对记忆贡献最大的潜在维度，在计算攻击统计量时移除较少记忆的维度。

Result: 在CIFAR-10、CelebA、ImageNet-1K等数据集上，平均AUROC提升2.7%，TPR@1%FPR显著提高6.42%，在极低误报率下成员识别置信度更强。

Conclusion: 研究揭示了自编码器几何结构对LDM记忆的潜在影响，为分析基于扩散的生成模型的隐私风险提供了新视角。
...
### [362] [BrowseSafe: Understanding and Preventing Prompt Injection Within AI Browser Agents](https://arxiv.org/abs/2511.20597)
*Kaiyuan Zhang,Mark Tenenholtz,Kyle Polley,Jerry Ma,Denis Yarats,Ninghui Li*

Main category: cs.LG

TL;DR: 本文研究了AI网络代理中的提示注入攻击，构建了包含真实HTML载荷的攻击基准，评估现有防御措施，并提出多层次防御策略。


<details>
  <summary>Details</summary>
Motivation: AI代理集成到浏览器中带来了超越传统网络应用威胁模型的安全挑战，特别是提示注入攻击在真实环境中的影响尚未充分理解。

Method: 构建包含真实HTML载荷的提示注入攻击基准，强调影响实际操作的攻击而非仅文本输出，并进行现有防御措施的全面实证评估。

Result: 开发了复杂度和干扰频率与真实环境相似的攻击载荷基准，评估了前沿AI模型的防御效果，提出了有效的多层次防御策略。

Conclusion: 通过深度防御方法为设计实用、安全的网络代理提供了蓝图，能够抵御不断演变的提示注入攻击。
...
### [363] [The Driver-Blindness Phenomenon: Why Deep Sequence Models Default to Autocorrelation in Blood Glucose Forecasting](https://arxiv.org/abs/2511.20601)
*Heman Shakeri*

Main category: cs.LG

TL;DR: 论文提出了'驱动因素盲区'概念，指出深度学习血糖预测模型未能有效利用胰岛素、饮食和活动等临床驱动因素，并定义了Δ_drivers指标来衡量多变量模型相比单变量基线的性能增益。


<details>
  <summary>Details</summary>
Motivation: 当前血糖预测的深度序列模型虽然理论上可以利用胰岛素、饮食和活动等临床驱动因素，但实际上这些模型未能有效利用这些信息，导致性能提升有限。

Method: 通过分析文献，识别了导致驱动因素盲区的三个因素：架构偏好自相关、数据保真度不足导致驱动因素噪声和混淆、生理异质性影响群体模型效果。提出了生理特征编码器、因果正则化和个性化等缓解策略。

Result: 研究发现文献中Δ_drivers通常接近零，表明多变量模型相比单变量基线几乎没有性能提升。

Conclusion: 建议未来工作应常规报告Δ_drivers指标，防止驱动因素盲区模型被误认为是最先进技术，并推荐采用生理特征编码、因果正则化和个性化等策略来缓解该问题。
...
### [364] [How to Purchase Labels? A Cost-Effective Approach Using Active Learning Markets](https://arxiv.org/abs/2511.20605)
*Xiwen Huang,Pierre Pinson*

Main category: cs.LG

TL;DR: 提出主动学习市场机制来购买标签，通过优化问题整合预算约束和改进阈值，在房地产定价和能源预测应用中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 在模型拟合和预测分析应用中，现有方法主要关注购买特征和样本，但缺乏专门针对标签购买的机制设计，特别是在资源受限环境下需要优化数据获取。

Method: 采用单买家-多卖家设置，结合两种主动学习策略（基于方差和基于查询委员会）与不同定价机制，并与随机采样基准进行比较。

Result: 在真实数据集上的实验表明，该方法比传统方法用更少的标签获得了更好的性能，证明了方法的鲁棒性。

Conclusion: 提出的主动学习市场为资源受限环境下的数据获取优化提供了一个易于实现的实用解决方案。
...
### [365] [Adaptive Hopfield Network: Rethinking Similarities in Associative Memory](https://arxiv.org/abs/2511.20609)
*Shurong Wang,Yuqi Pan,Zhuoyang Shen,Meng Zhang,Hongwei Wang,Guoqi Li*

Main category: cs.LG

TL;DR: 本文提出了一种新的关联记忆模型，通过定义变体分布和自适应相似性机制，解决了现有模型无法保证检索正确性的问题。
...
Method: 提出变体分布建模查询生成过程，开发自适应相似性机制来近似未知的似然函数，并集成到自适应Hopfield网络(A-Hop)中。

Result: 理论证明在噪声、掩码和偏差三种变体类型下实现最优正确检索，实验表明在多种任务上达到最先进性能。

Conclusion: 自适应相似性机制能够有效解决关联记忆的正确检索问题，在多个应用场景中表现出优越性能。
...
### [366] [Sparse-to-Field Reconstruction via Stochastic Neural Dynamic Mode Decomposition](https://arxiv.org/abs/2511.20612)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界系统（如风场、洋流）动态复杂难以建模，传统DMD方法受限于稀疏观测、线性近似和缺乏不确定性量化。

Method: 基于神经ODE的概率扩展DMD方法，支持连续时空重建，在任意坐标点进行预测，并提供预测不确定性。

Result: 在四个基准测试中，仅使用10%观测密度时重建精度优于基线，能恢复动态结构并与真实模式对齐，在多实现数据集上学习校准的潜在动态分布。

Conclusion: 该方法成功解决了传统DMD的局限性，提供了连续、非线性、可解释且具有不确定性量化的动态建模框架。
...
### [367] [Can Vibe Coding Beat Graduate CS Students? An LLM vs. Human Coding Tournament on Market-driven Strategic Planning](https://arxiv.org/abs/2511.20613)
*Panayiotis Danassis,Naman Goel*

Main category: cs.LG

TL;DR: LLM生成的代码在真实世界物流优化问题中表现不佳，人类编写的代理在竞争中全面胜出，显示LLM在需要推理的现实场景代码生成方面存在明显差距。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试过于强调单元测试通过率和语法正确性，低估了需要规划、优化和战略交互的真实世界问题的难度，需要开发更能反映现实复杂性的评估方法。

Method: 引入基于真实世界物流优化问题（拍卖、取货和配送问题）的多智能体推理驱动基准，评估40个LLM编码的代理与17个人类编码代理在12场双循环锦标赛中的表现。

Result: 人类编码代理始终占据前5名；40个LLM代理中有33个被非常简单的基线击败；即使提供最佳人类解决方案并要求改进，表现最好的LLM反而使解决方案显著恶化。

Conclusion: LLM在生成能在真实世界中具有竞争力的代码方面存在能力差距，需要开发强调推理驱动代码合成的新评估方法。
...
### [368] [DiFR: Inference Verification Despite Nondeterminism](https://arxiv.org/abs/2511.20621)
*Adam Karvonen,Daniel Reuter,Roy Rinberg,Luke Marks,Adrià Garriga-Alonso,Keri Warr*

Main category: cs.LG

TL;DR: Token-DiFR是一种通过比较生成token与可信参考实现预测结果来验证LLM推理输出的方法，使用采样种子同步来约束有效输出，能以零额外成本检测错误和篡改。


<details>
  <summary>Details</summary>
Motivation: 随着LLM推理需求增长，需要验证推理过程是否正确执行且未被篡改，但由于数值噪声导致重复运行结果不同，难以区分正常变化与实际问题。

Method: Token-DiFR：使用相同随机种子条件下，将生成token与可信参考实现预测进行比较；Activation-DiFR：使用随机正交投影压缩激活值为紧凑指纹进行验证。

Result: Token-DiFR能在300个输出token内检测4位量化（AUC>0.999）；Activation-DiFR仅需2个输出token即可检测4位量化（AUC>0.999），通信开销比现有方法减少25-75%。

Conclusion: 提出的DiFR方法能有效验证LLM推理正确性，Token-DiFR适合一般验证，Activation-DiFR适合样本高效的验证场景，已开源集成vLLM以加速实际部署。
...
### [369] [ROOT: Robust Orthogonalized Optimizer for Neural Network Training](https://arxiv.org/abs/2511.20626)
*Wei He,Kai Han,Hang Zhou,Hanting Chen,Zhicheng Liu,Xinghao Chen,Yunhe Wang*

Main category: cs.LG

TL;DR: ROOT是一种鲁棒正交化优化器，通过维度鲁棒正交化方案和优化鲁棒框架解决大语言模型训练中的稳定性和噪声问题，在噪声和非凸场景下表现出更快的收敛速度和更好的最终性能。
...
Method: 提出ROOT优化器：1）使用自适应牛顿迭代和细粒度系数的维度鲁棒正交化方案；2）通过近端优化的优化鲁棒框架抑制异常值噪声同时保留有意义的梯度方向。

Result: 大量实验表明ROOT在鲁棒性、收敛速度和最终性能方面显著优于Muon和基于Adam的优化器，特别是在噪声和非凸场景下。
...
### [370] [Image2Gcode: Image-to-G-code Generation for Additive Manufacturing Using Diffusion-Transformer Model](https://arxiv.org/abs/2511.20636)
...
Method: 使用手绘或捕获的2D图像作为唯一输入，首先从图像中提取切片结构线索，然后采用基于G代码序列的去噪扩散概率模型(DDPM)。通过迭代去噪，将高斯噪声转换为可执行的打印移动轨迹和相应的挤出参数。

Result: 建立了从视觉输入到原生工具路径的直接映射，无需CAD或STL中间文件，降低了增材制造的门槛，加速了从设计到制造的周期。

Conclusion: Image2Gcode提供了一个灵活、计算高效的框架，支持从简单草图或视觉参考进行按需原型制作，并集成了上游2D到3D重建模块，实现了从概念到物理工件的自动化流程。
...
### [371] [It Hears, It Sees too: Multi-Modal LLM for Depression Detection By Integrating Visual Understanding into Audio Language Models](https://arxiv.org/abs/2511.19877)
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症是全球最普遍的心理健康障碍之一。虽然多模态数据和LLMs推动了AI辅助抑郁症评估系统的发展，但传统LLMs主要处理文本，无法有效利用音频和视觉模态中的非语言线索，而这些对心理健康评估至关重要。现有的多模态LLMs很少针对心理学应用进行定制。

Method: 提出新颖的多模态LLM框架，通过时间戳级别的视听特征对齐来增强音频语言模型的视觉理解能力。这种细粒度对齐改进了跨模态的时间动态建模，同时减少了对大量训练数据和计算资源的需求。

Result: 在DAIC-WoZ数据集上的实验表明，该模型在抑郁症检测方面优于单模态方法和先前的多模态方法。

Conclusion: 所提出的框架可以扩展到整合更多生理信号，为超越心理健康领域的更广泛临床应用铺平了道路。
...
### [372] [FINE: Factorized multimodal sentiment analysis via mutual INformation Estimation](https://arxiv.org/abs/2511.20167)
*Yadong Liu,Shangfei Wang*

Main category: cs.MM

TL;DR: 提出了一种因子化多模态融合框架，通过解耦共享和独特表示、抑制任务无关噪声来提升多模态情感分析的性能，并引入辅助模块增强特征提取和长期时序建模。


<details>
  <summary>Details</summary>
Motivation: 多模态情感分析面临模态间异构性挑战，表现为异步信号、信息不平衡和任务无关噪声干扰，阻碍了鲁棒准确的情感表示学习。

Method: 采用因子化多模态融合框架，将每个模态解耦为共享和独特表示，基于互信息优化策略抑制任务无关噪声；引入混合Q-Former提取细粒度情感特征，动态对比队列进行对比学习。

Result: 在多个公开数据集上的实验表明，该方法持续优于现有方法，验证了框架的有效性和鲁棒性。

Conclusion: 所提出的因子化融合框架通过精细化解耦和噪声抑制，有效提升了多模态情感分析的性能，证明了该方法的优越性。
...
### [373] [Using Wearable Devices to Improve Chronic PainTreatment among Patients with Opioid Use Disorder](https://arxiv.org/abs/2511.19577)
...
<details>
  <summary>Details</summary>
Motivation: 慢性疼痛和鸦片使用障碍是相互关联的常见慢性疾病，目前缺乏针对接受MOUD治疗患者的循证整合治疗方案。可穿戴设备有潜力监测复杂患者信息，但LLMs在疼痛峰值分析中的应用尚未探索。

Method: 采用试点研究方法，使用可穿戴设备收集数据，应用多种AI方法（包括机器学习模型和LLMs）分析疼痛峰值的临床相关性。

Result: 机器学习模型在预测疼痛峰值方面达到相对较高的准确率（>0.7），但LLMs在提供疼痛峰值见解方面表现有限。

Conclusion: 可穿戴设备的实时监测结合先进AI模型可促进疼痛峰值的早期检测，支持个性化干预。鉴于LLMs整体表现有限，需要开发能在OUD/CP背景下提供可行见解的LLMs。
...
### [374] [NNGPT: Rethinking AutoML with Large Language Models](https://arxiv.org/abs/2511.20333)
...
TL;DR: NNGPT是一个开源框架，将大语言模型转化为自改进的AutoML引擎，通过生成-评估-自改进的闭环系统持续优化神经网络开发，在计算机视觉领域实现了端到端的自动化模型生成和验证。


<details>
  <summary>Details</summary>
Motivation: 构建自改进的AI系统是AI领域的根本挑战，现有框架无法实现神经网络的持续扩展和LLM的闭环自改进。

Method: 集成五个协同的LLM管道：零样本架构合成、超参数优化、代码感知精度预测、检索增强的PyTorch块合成、强化学习，基于LEMUR数据集构建闭环系统。

Result: NN-RAG在1289个目标上达到73%可执行性，3-shot提示提升常见数据集精度，HPO在LEMUR上RMSE 0.60优于Optuna，已生成超过5000个验证模型。

Conclusion: NNGPT证明可作为自主的AutoML引擎，代码和检查点将公开发布以促进可复现性和社区使用。
...
### [375] [Fara-7B: An Efficient Agentic Model for Computer Use](https://arxiv.org/abs/2511.19663)
...
TL;DR: FaraGen是一个用于多步骤网页任务的合成数据生成系统，能够生成多样化的任务和解决方案，并过滤成功轨迹。基于这些数据训练的Fara-7B模型在多个基准测试中表现优异，甚至能与更大的前沿模型竞争。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理（CUAs）的发展受到缺乏大规模高质量人类与计算机交互数据集的限制，而现有数据集无法与LLMs使用的文本数据规模相比。

Method: 开发FaraGen系统，从常用网站生成多样化任务，产生多个解决方案尝试，并使用多个验证器过滤成功轨迹。利用这些数据训练Fara-7B模型，该模型仅通过截图感知计算机，通过预测坐标执行动作，且足够小可在设备上运行。

Result: FaraGen以约1美元的成本生成每个验证轨迹，实现高吞吐量、产量和多样性。Fara-7B在WebVoyager、Online-Mind2Web和WebTailBench等基准测试中优于同类规模的CUA模型，并与更大的前沿模型竞争。
...
### [376] [HeaRT: A Hierarchical Circuit Reasoning Tree-Based Agentic Framework for AMS Design Optimization](https://arxiv.org/abs/2511.19669)
*Souradip Poddar,Chia-Tung Ho,Ziming Wei,Weidong Cao,Haoxing Ren,David Z. Pan*

Main category: cs.AI

TL;DR: HeaRT是一个基础推理引擎，用于AMS设计自动化循环，实现了智能、自适应、类人风格的设计优化，在40电路基准测试中达到>97%的推理准确率和>98%的Pass@1性能，同时运行成本仅为SOTA基线的<0.5倍。
...
Result: 在40电路基准测试中，推理准确率>97%，Pass@1性能>98%，即使电路复杂度增加也能保持性能；在尺寸和拓扑设计适应任务中实现>3倍的收敛速度提升，同时保持先前的设计意图。

Conclusion: HeaRT是迈向智能自适应AMS设计优化的第一步，显著提升了设计效率和性能，同时降低了计算成本。
...
### [377] [FISCAL: Financial Synthetic Claim-document Augmented Learning for Efficient Fact-Checking](https://arxiv.org/abs/2511.19671)
...
Method: 提出FISCAL框架生成金融合成数据，并训练MiniCheck-FISCAL轻量级验证器进行金融数值声明的事实核查。
...
### [378] [Scaling Item-to-Standard Alignment with Large Language Models: Accuracy, Limits, and Solutions](https://arxiv.org/abs/2511.19749)
*Farzan Karimi-Malekabadi,Pooya Razavi,Sonya Powers*

Main category: cs.AI

TL;DR: 研究表明大型语言模型（特别是GPT-4o-mini）能够在保持准确性的同时，显著加速教育评估项目与内容标准的对齐过程，特别是在结合候选技能过滤策略时效果更佳。


<details>
  <summary>Details</summary>
Motivation: 传统的人工对齐审查虽然准确但耗时费力，特别是在大型题库中。需要找到既能保持准确性又能提高效率的自动化解决方案。

Method: 使用超过12,000个K-5年级的项目-技能对，测试了三种LLM（GPT-3.5 Turbo、GPT-4o-mini和GPT-4o）在三个任务上的表现：识别错位项目、从完整标准集中选择正确技能、以及在分类前缩小候选列表。

Result: GPT-4o-mini在识别对齐状态方面达到83-94%的准确率；数学表现良好但阅读较低；预过滤候选技能可将正确技能出现在前五建议的概率提高到95%以上。

Conclusion: LLM结合候选过滤策略能显著减少人工审查负担，建议开发混合流程，将基于LLM的筛选与模糊情况下的人工审查相结合，为持续的项目验证和教学对齐提供可扩展解决方案。
...
### [379] [Scaling Agentic Reinforcement Learning for Tool-Integrated Reasoning in VLMs](https://arxiv.org/abs/2511.19773)
...
TL;DR: VISTA-Gym是一个可扩展的训练环境，用于增强视觉语言模型的多步骤视觉推理能力，通过工具集成和强化学习提升模型在复杂视觉任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型虽然在图像理解方面表现出色，但在多步骤视觉推理和工具集成方面仍有局限，需要专门的环境来训练这些能力。

Method: 开发了VISTA-Gym统一训练环境，集成7个任务13个数据集，提供标准化视觉工具接口、可执行交互循环和反馈信号，通过多轮轨迹采样和端到端强化学习训练VISTA-R1模型。
...
### [380] [NOEM$^{3}$A: A Neuro-Symbolic Ontology-Enhanced Method for Multi-Intent Understanding in Mobile Agents](https://arxiv.org/abs/2511.19780)
...
Result: 在MultiWOZ 2.3的模糊/复杂对话子集上，3B Llama模型通过本体增强接近GPT-4的准确率（85% vs 90%），同时显著降低能耗和内存占用。
...
### [381] [KOM: A Multi-Agent Artificial Intelligence System for Precision Management of Knee Osteoarthritis (KOA)](https://arxiv.org/abs/2511.19798)
...
Conclusion: KOM有助于促进KOA自动化管理，整合到临床工作流程中可提高护理效率，其模块化架构为开发其他慢性病的AI辅助管理系统提供了宝贵见解。
...
### [382] [A Unified Evaluation-Instructed Framework for Query-Dependent Prompt Optimization](https://arxiv.org/abs/2511.19829)
*Ke Chen,Yifeng Wang,Hassan Almosapeeh,Haohan Wang*

Main category: cs.AI

TL;DR: 提出了一种基于评估指导的提示优化方法，通过建立系统化的提示评估框架和训练执行无关的评估器，实现可解释的、查询相关的提示优化，在多个数据集和模型上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法大多优化静态模板，在复杂动态场景中效果有限；查询相关方法依赖不稳定的文本反馈或黑盒奖励模型，提供弱且不可解释的优化信号；提示质量本身缺乏统一系统定义。

Method: 建立性能导向的系统化提示评估框架；开发并微调执行无关的评估器，直接从文本预测多维度质量分数；评估器指导度量感知优化器，以可解释方式诊断失败模式并重写提示。

Result: 评估器在预测提示性能方面达到最强准确度；评估指导的优化在8个数据集和3个骨干模型上持续超越静态模板和查询相关基线方法。

Conclusion: 提出了统一的、基于度量的提示质量视角，证明评估指导的优化管道能够提供稳定、可解释且模型无关的改进，适用于多样化任务。
...
### [383] [Reinforcement Learning with $ω$-Regular Objectives and Constraints](https://arxiv.org/abs/2511.19849)
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖标量奖励，表达能力有限，容易导致奖励黑客行为，且无法明确表达时间性、条件性或安全关键目标。同时，单一标量指标掩盖了安全与性能之间的权衡。

Method: 开发了基于线性规划的模型强化学习算法，将ω-正则目标与约束分离处理，确保在满足约束阈值的同时最大化目标满足概率。建立了到约束极限平均问题的转换方法。

Result: 该方法在极限情况下能够生成既最大化ω-正则目标满足概率，又遵守ω-正则约束在指定阈值内的策略。

Conclusion: 提出的框架成功解决了强化学习中安全约束与优化目标分离的问题，为复杂行为规范和安全关键应用提供了有效的解决方案。
...
### [384] [MicroSims: A Framework for AI-Generated, Scalable Educational Simulations with Universal Embedding and Adaptive Learning Support](https://arxiv.org/abs/2511.19864)
*Valerie Lockhart,Dan McCreary,Troy A. Peterson*

Main category: cs.AI

TL;DR: MicroSims是一个用于创建轻量级交互式教育模拟的AI驱动框架，通过标准化设计模式、iframe架构和可修改代码，实现快速生成、跨平台嵌入和无编程定制。
...
Method: 采用标准化设计模式支持AI辅助生成，iframe架构实现通用嵌入和安全沙盒，透明可修改代码支持定制和教学透明度。

Result: 基于物理教育研究和STEM元分析，交互模拟比传统教学可提高概念理解30-40%，MicroSims在保持这些优势的同时降低了使用门槛。
...
### [385] [Agentic AI-Empowered Conversational Embodied Intelligence Networks in 6G](https://arxiv.org/abs/2511.19865)
*Mingkai Chen,Zijie Feng,Lei Wang,Yaser Khamayseh*

Main category: cs.AI

TL;DR: 提出CC-EIN网络解决6G时代多智能体协作中的多模态信息融合、自适应通信和决策可解释性问题，在灾后救援场景中实现95.4%任务完成率和95%传输效率。


<details>
  <summary>Details</summary>
Motivation: 现有系统在多模态信息融合、自适应通信和决策可解释性方面存在局限，难以满足6G时代多智能体设备复杂任务执行的需求。

Method: 集成多模态特征融合、自适应语义通信、任务协调和可解释性模块。PerceptiNet融合图像和雷达数据生成统一语义表示，自适应语义通信策略根据任务紧急程度和信道质量动态调整编码方案和传输功率，语义驱动协作机制支持任务分解和无冲突协调，InDec模块通过Grad-CAM可视化增强决策透明度。

Result: 在灾后救援场景仿真中，CC-EIN实现了95.4%的任务完成率和95%的传输效率，同时保持了强大的语义一致性和能源效率。
...
### [386] [Simulated Self-Assessment in Large Language Models: A Psychometric Approach to AI Self-Efficacy](https://arxiv.org/abs/2511.19872)
*Daniel I Jackson,Emma L Jensen,Syed-Amad Hussain,Emre Sezgin*

Main category: cs.AI

TL;DR: 该研究将通用自我效能感量表(GSES)应用于10个大型语言模型，发现模型在不同任务条件下的自我评估稳定但低于人类标准，且自我评估与实际能力不匹配。
...
Method: 将10项通用自我效能感量表(GSES)应用于10个LLM，在四种条件下进行测试：无任务、计算推理、社会推理和摘要任务，并分析响应的稳定性、准确性和后续置信度提示的影响。

Result: 模型自我评估在不同条件下稳定但低于人类标准；所有模型在计算和社会问题上准确率100%，但摘要性能差异大；自我评估与实际能力不相关；后续置信度提示导致适度向下修正。

Conclusion: 心理测量提示提供了对LLM沟通行为的结构化洞察，但不能提供校准的性能估计，自我评估与能力不匹配。
...
### [387] [RPM-MCTS: Knowledge-Retrieval as Process Reward Model with Monte Carlo Tree Search for Code Generation](https://arxiv.org/abs/2511.19895)
*Yuanyuan Lin,Xiangyu Ouyang,Teng Zhang,Kaixin Sui*

Main category: cs.AI

TL;DR: RPM-MCTS是一种基于蒙特卡洛树搜索的代码生成方法，通过知识检索作为过程奖励模型来评估中间算法步骤，无需复杂训练，同时使用沙箱执行反馈定位和纠正错误步骤，显著提升代码生成质量并减少15%的token消耗。
...
### [388] [Semantic-KG: Using Knowledge Graphs to Construct Benchmarks for Measuring Semantic Similarity](https://arxiv.org/abs/2511.19925)
...
<details>
  <summary>Details</summary>
Motivation: 现有评估LLM文本输出的语义相似度方法存在缺陷：可能更关注语法而非语义内容，基准数据集生成成本高，依赖主观人工判断，领域适用性有限，且语义等价定义不清晰。
...
Conclusion: 该方法为评估语义相似度方法提供了有效的基准生成方案，揭示了当前方法在不同语义变化类型和领域中的性能差异，对使用LLM作为评判者具有重要启示。
...
### [389] [A System-Level Taxonomy of Failure Modes in Large Language Model Applications](https://arxiv.org/abs/2511.19933)
*Vaishali Vinay*

Main category: cs.AI

TL;DR: 本文提出了15种LLM在真实应用中的隐藏故障模式，分析了现有评估方法的局限性，并提供了构建可靠LLM系统的设计原则。


<details>
  <summary>Details</summary>
Motivation: LLM被快速集成到决策支持工具和自动化工作流中，但其在生产环境中的行为仍未被充分理解，且故障模式与传统机器学习模型有根本不同。

Method: 提出了系统级的故障模式分类法，包括多步推理漂移、潜在不一致性、上下文边界退化、错误工具调用等15种故障模式，并分析了部署挑战。

Result: 识别了现有评估方法与实际生产需求之间的差距，现有基准主要测量知识和推理能力，但对稳定性、可重现性、漂移和工作流集成提供的信息有限。
...
### [390] [M$^3$Prune: Hierarchical Communication Graph Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](https://arxiv.org/abs/2511.19969)
*Weizi Shao,Taolin Zhang,Zijie Zhou,Chen Chen,Chengyu Wang,Xiaofeng He*

Main category: cs.AI

TL;DR: 提出了M³Prune框架，通过剪枝多模态多代理层次通信图中的冗余边，在保持任务性能的同时显著降低token开销


<details>
  <summary>Details</summary>
Motivation: 现有多代理系统存在大量token开销和计算成本问题，阻碍大规模部署，需要找到任务性能和token开销之间的最佳平衡

Method: 采用分层剪枝策略：先进行模态内图稀疏化识别关键边，然后构建动态通信拓扑进行模态间图稀疏化，最后逐步剪枝冗余边获得高效层次拓扑

Result: 在通用和领域特定的mRAG基准测试中，该方法持续优于单代理和鲁棒多代理系统，同时显著减少token消耗

Conclusion: M³Prune框架有效解决了多代理系统的效率问题，实现了性能与开销的最佳平衡
...
### [391] [Reducing Latency of LLM Search Agent via Speculation-based Algorithm-System Co-Design](https://arxiv.org/abs/2511.20048)
...
TL;DR: SPAgent通过推测执行和系统协同设计，显著降低LLM搜索代理的延迟，实现1.65倍端到端加速，同时保持或提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM搜索代理存在严重延迟问题，因为每个步骤都需要串行化LLM推理和工具执行。传统推测执行范式虽然能打破串行执行，但收益有限，因为它保留了完整原始工作负载并增加了额外推理开销。

Method: SPAgent采用算法-系统协同设计框架：算法层面引入两阶段自适应推测机制，在安全时选择性省略验证；系统层面通过两级调度器基于引擎负载调节推测请求，确保推测保持有益。

Result: 在广泛实验设置中，SPAgent实现了高达1.65倍的端到端加速，同时保持相同甚至实现更高的准确性。

Conclusion: SPAgent通过扩展推测在搜索代理中的作用，显著降低了延迟，使多步搜索代理的实际部署成为可能。
...
### [392] ["Are We Done Yet?": A Vision-Based Judge for Autonomous Task Completion of Computer Use Agents](https://arxiv.org/abs/2511.20067)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: 提出基于视觉语言模型的自主评估反馈框架，通过屏幕截图直接评估任务完成情况，提高计算机使用代理的可靠性和自校正能力。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在自主操作数字界面时，往往难以可靠判断任务是否完成，需要有效的评估反馈机制。

Method: 使用视觉语言模型直接从屏幕截图和任务描述评估任务完成情况，构建包含42个macOS应用和1260个人工标注任务的数据集。

Result: 任务成功检测准确率达到73%，应用评估反馈后整体任务成功率平均相对提升27%。

Conclusion: 基于视觉的评估可作为有效的反馈机制，显著提升自主计算机使用代理的可靠性和自校正能力。
...
### [393] [VICoT-Agent: A Vision-Interleaved Chain-of-Thought Framework for Interpretable Multimodal Reasoning and Scalable Remote Sensing Analysis](https://arxiv.org/abs/2511.20085)
*Chujie Wang,Zhiyuan Luo,Ruiqi Liu,Can Ran,Shenghua Fan,Xi Chen,Chu He*

Main category: cs.AI

TL;DR: 提出了VICoT多模态智能体框架，通过将视觉工具动态融入思维链实现显式多轮推理，在遥感图像分析任务中显著优于现有SOTA方法。
...
Method: 采用基于堆栈的推理结构和模块化MCP兼容工具套件，实现多轮交错视觉语言推理；提出推理堆栈蒸馏方法将复杂智能体行为迁移到轻量模型。

Result: 在多个遥感基准测试中，VICoT在推理透明度、执行效率和生成质量方面显著优于现有SOTA框架。

Conclusion: VICoT框架通过显式多轮推理和工具动态集成，有效提升了遥感图像分析的推理能力和灵活性，并通过蒸馏方法实现了轻量化部署。
...
### [394] [From data to concepts via wiring diagrams](https://arxiv.org/abs/2511.20138)
*Jason Lo,Mohammadnima Jafari*

Main category: cs.AI

TL;DR: 本文介绍了准骨架接线图的概念，证明其与Hasse图对应，并设计了从序列数据中提取接线图的算法，成功应用于分析自主代理在电脑游戏中的行为策略。


<details>
  <summary>Details</summary>
Motivation: 接线图是表示抽象概念（如时间过程）的标记有向图，需要开发能够从序列数据中自动提取接线图的算法。

Method: 引入准骨架接线图概念，证明其与Hasse图的对应关系，设计基于此的算法，并与DBSCAN和凝聚层次聚类算法进行对比。

Result: 算法成功识别了自主代理在电脑游戏中的获胜策略，在数据扰动情况下表现良好。
...
### [395] [Towards Benign Memory Forgetting for Selective Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2511.20196)
*Zhen Zeng,Leijiang Gu,Zhangling Duan,Feng Li,Zenglin Shi,Cees G. M. Snoek,Meng Wang*

Main category: cs.AI

TL;DR: SMFA是一种针对多模态大语言模型的精准遗忘方法，通过遗忘适配器和保留锚点掩码机制，在移除敏感知识的同时保持模型的通用图像理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型遗忘方法在移除隐私敏感信息时，往往会损害模型的通用图像理解性能，需要一种更精准的遗忘机制。

Method: 提出Sculpted Memory Forgetting Adapter (SMFA)，首先微调模型将敏感响应替换为拒绝回答，然后应用保留锚点引导的掩码机制来防止对无关知识和理解能力的干扰。

Result: 实验表明SMFA能够实现精确可控的遗忘，同时保持模型的基础图像理解能力，优于现有方法。

Conclusion: SMFA通过局部化遗忘机制成功解决了多模态大语言模型在隐私保护与性能保持之间的平衡问题。
...
### [396] [Interactive AI NPCs Powered by LLMs: Technical Report for the CPDC Challenge 2025](https://arxiv.org/abs/2511.20200)
...
Method: 上下文工程（动态工具剪枝、角色信息裁剪、参数归一化、函数合并）和GPU赛道中的GRPO强化学习训练。
...
### [397] [CostNav: A Navigation Benchmark for Cost-Aware Evaluation of Embodied Agents](https://arxiv.org/abs/2511.20216)
...
TL;DR: CostNav是首个将导航性能与商业经济可行性结合的微导航经济测试平台，通过成本收益分析揭示导航研究指标与商业部署之间的差距。
...
Method: CostNav建立完整的成本收益模型，包括硬件、训练、能源、维护成本和配送收入，使用行业参数从缩小规模模拟投影到实际配送场景。

Result: 基线方法达到43.0%的服务水平协议合规率，但商业上不可行：每次运行亏损30.009美元，无盈亏平衡点，其中碰撞导致的维护成本占每次运行成本的99.7%。
...
### [398] [Actionable and diverse counterfactual explanations incorporating domain knowledge and causal constraints](https://arxiv.org/abs/2511.20236)
*Szymon Bobek,Łukasz Bałec,Grzegorz J. Nalepa*

Main category: cs.AI

TL;DR: 提出DANCE方法，生成多样、可操作且知识约束的反事实解释，通过整合特征依赖和因果约束确保反事实的合理性和现实可行性。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法常忽略真实数据集中的复杂依赖关系，导致不现实或不实用的修改。受网络安全在电子邮件营销领域应用的启发，需要生成更合理的反事实解释。
...
### [399] [SMoG: Schema Matching on Graph](https://arxiv.org/abs/2511.20285)
*Mingyu Jeon,Jaeyoung Suh,Suwan Cho*

Main category: cs.AI

TL;DR: SMoG是一个基于知识图谱的架构匹配框架，通过迭代执行简单的1跳SPARQL查询来提升医学领域架构匹配的可靠性和可解释性，同时显著降低存储需求。


<details>
  <summary>Details</summary>
Motivation: 在医学数据集成中，大型语言模型存在幻觉问题和缺乏最新领域知识的问题，而现有知识图谱增强方法通常依赖低效的多跳查询或存储密集的向量检索方法。

Method: SMoG框架采用迭代执行简单1跳SPARQL查询的策略，受知识图谱问答成功方法的启发，直接查询SPARQL端点，生成可人工验证的查询路径。

Result: 在真实世界医学数据集上的实验结果表明，SMoG达到了与最先进基线方法相当的性能，验证了其在知识图谱增强架构匹配中的有效性和效率。

Conclusion: SMoG通过简单高效的1跳SPARQL查询方法，在保持性能的同时显著提升了架构匹配的可解释性和可靠性，为医学数据集成提供了实用的解决方案。
...
### [400] [Improving Language Agents through BREW](https://arxiv.org/abs/2511.20297)
*Shashank Kirtania,Param Biyani,Priyanshu Gupta,Yasharth Bajpai,Roshni Iyer,Sumit Gulwani,Gustavo Soares*

Main category: cs.AI

TL;DR: BREW框架通过构建和精炼经验学习知识库来优化智能体，替代传统权重优化方法，在保持计算效率的同时提升任务精度和减少API调用。
...
Conclusion: BREW将知识库确立为模块化、可控的智能体优化基础，提供透明、可解释和可扩展的行为塑造机制，与将记忆视为静态上下文的先前工作形成对比。
...
### [401] [Data Augmentation Techniques to Reverse-Engineer Neural Network Weights from Input-Output Queries](https://arxiv.org/abs/2511.20312)
*Alexander Beiser,Flavio Martinelli,Wulfram Gerstner,Johanni Brea*

Main category: cs.AI

TL;DR: 提出新的数据增强技术来改进神经网络参数逆向工程，能够恢复参数数量比训练数据多100倍的网络


<details>
  <summary>Details</summary>
Motivation: 现有方法在教师网络参数数量超过训练数据时失败，因为学生会过拟合查询数据而不是对齐教师参数

Method: 设计专门针对网络隐藏层表示空间采样的新数据增强技术，而非使用标准的旋转、翻转、加噪等增强方法

Result: 扩展了可恢复网络大小的最先进范围，能够恢复参数数量比训练数据点多100倍的网络

Conclusion: 专门针对网络表示空间设计的数据增强技术能显著改进参数逆向工程效果，比标准增强方法更有效
...
### [402] [Active Inference in Discrete State Spaces from First Principles](https://arxiv.org/abs/2511.20321)
*Patrick Kenny*

Main category: cs.AI

TL;DR: 本文澄清了主动推理与自由能原理的关系，提出了基于约束散度最小化的优化框架，无需依赖期望自由能概念。


<details>
  <summary>Details</summary>
Motivation: 澄清主动推理的概念，将其与自由能原理分离，展示如何通过标准平均场方法实现离散状态空间中的主动推理。

Method: 将主动推理的优化问题表述为约束散度最小化问题，使用标准平均场方法求解，不依赖期望自由能概念。
...
### [403] [VibraVerse: A Large-Scale Geometry-Acoustics Alignment Dataset for Physically-Consistent Multimodal Learning](https://arxiv.org/abs/2511.20422)
*Bo Pang,Chenxi Xu,Jierui Ren,Guoping Wang,Sheng Li*

Main category: cs.AI

TL;DR: VibraVerse是一个大规模几何-声学对齐数据集，通过CLASP对比学习框架建立物体物理结构与声学响应的因果关系，为物理一致的多模态学习提供基准。
...
Method: 构建VibraVerse数据集，包含3D几何模型、物理属性、模态参数和声学信号；提出CLASP对比学习框架进行跨模态对齐。

Result: 在几何到声音预测、声音引导形状重建和跨模态表示学习任务上，基于VibraVerse训练的模型表现出更高的准确性、可解释性和泛化能力。
...
### [404] [DRAFT-RL: Multi-Agent Chain-of-Draft Reasoning for Reinforcement Learning-Enhanced LLMs](https://arxiv.org/abs/2511.20468)
*Yuanhao Li,Mingshan Liu,Hongbo Wang,Yiding Zhang,Yifei Ma,Wei Tan*

Main category: cs.AI

TL;DR: DRAFT-RL是一个新颖的多智能体强化学习框架，将链式草稿推理集成到训练中，通过生成多个草稿、同行评估和奖励模型选择来提升LLM智能体的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的多智能体反思框架依赖单次响应生成，缺乏推理探索的结构多样性，限制了智能体的鲁棒性和可解释性。
...
### [405] [Universe of Thoughts: Enabling Creative Reasoning with Large Language Models](https://arxiv.org/abs/2511.20471)
*Yuto Suzuki,Farnoush Banaei-Kashani*

Main category: cs.AI

TL;DR: 本文提出了一个基于大语言模型的创造性推理框架，包含组合式、探索式和转换式三种核心推理范式，并开发了思想宇宙(UoT)方法来实现这些创造性过程。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型主要关注常规问题解决，缺乏创造性推理能力。在药物发现、商业策略等需要创新解决方案的领域，创造性推理至关重要。

Method: 提出了思想宇宙(UoT)方法，实现三种创造性推理过程：组合式、探索式和转换式推理，系统性地探索思想空间以生成创造性解决方案。
...
### [406] [Quantifying the Privacy Implications of High-Fidelity Synthetic Network Traffic](https://arxiv.org/abs/2511.20497)
*Van Tran,Shinan Liu,Tian Li,Nick Feamster*

Main category: cs.AI

TL;DR: 本文提出了针对合成网络流量的隐私评估框架，发现不同生成模型存在显著的隐私泄露风险，成员推理攻击成功率最高达88%，网络标识符恢复率可达100%。


<details>
  <summary>Details</summary>
Motivation: 解决合成网络流量数据稀缺和隐私问题，但现有生成模型缺乏对隐私泄露程度的系统评估方法。

Method: 引入全面的隐私评估指标，结合成员推理攻击、数据提取攻击以及网络特定标识符和属性，系统评估不同代表性生成模型的脆弱性。

Result: 发现模型间隐私风险差异显著：成员推理攻击成功率0%-88%，网络标识符恢复率最高100%；训练数据多样性和模型拟合度是影响攻击成功的关键因素。

Conclusion: 为设计和部署最小化隐私泄露的生成模型提供可操作指导，为更安全的合成网络流量生成奠定基础。
...
### [407] [FRAGMENTA: End-to-end Fragmentation-based Generative Model with Agentic Tuning for Drug Lead Optimization](https://arxiv.org/abs/2511.20510)
*Yuto Suzuki,Paul Awolade,Daniel V. LaBarbera,Farnoush Banaei-Kashani*

Main category: cs.AI

TL;DR: FRAGMENTA是一个用于药物先导化合物优化的端到端框架，包含生成模型和智能体系统，通过动态Q学习优化分子碎片化和生成，并通过对话反馈学习领域知识，在癌症药物发现实验中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 分子生成在药物发现中至关重要，但类别特定数据集通常少于100个训练样本。现有启发式碎片化方法限制了多样性并遗漏关键片段，且模型调优需要药物化学家和AI工程师之间的缓慢间接协作。

Method: 1) 将碎片化重构为"词汇选择"问题，使用动态Q学习联合优化碎片化和生成；2) 智能体AI系统通过领域专家的对话反馈精炼目标，逐步学习领域知识以实现自动化调优。

Result: 在真实世界癌症药物发现实验中，FRAGMENTA的人-智能体配置识别的高分分子数量是基线的近两倍。完全自主的智能体-智能体系统优于传统的人-人调优。

Conclusion: FRAGMENTA框架通过智能体调优有效捕捉专家意图，证明了在药物发现中自动化调优的效能，能够减少对AI工程师的依赖并提高分子生成效率。
...
### [408] [Assessing LLMs' Performance: Insights from the Chinese Pharmacist Exam](https://arxiv.org/abs/2511.20526)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在数字健康教育和评估中的广泛应用，需要评估其在专业认证任务中的能力。中国药师执业资格考试作为标准化基准，适合评估模型在药学领域的专业能力。

Method: 收集2017-2021年药师执业资格考试的2,306道纯文本选择题，排除含表格或图片的题目。以原始中文格式输入问题，评估模型回答的准确率，使用卡方检验和Fisher精确检验进行统计分析。

Result: DeepSeek-R1总体准确率显著高于ChatGPT-4o（90.0% vs 76.1%，p < 0.001）。在基础和临床综合模块中，DeepSeek-R1表现一致更优。虽然逐年分析也显示DeepSeek-R1优势，但单元年度差异未达到统计显著性。

Conclusion: DeepSeek-R1在药师执业资格考试中表现出色，表明领域特定模型在该情境下具有优势。研究强调了在法律和伦理敏感情境中仍需人工监督的重要性。
...
### [409] [Beyond Generation: Multi-Hop Reasoning for Factual Accuracy in Vision-Language Models](https://arxiv.org/abs/2511.20531)
*Shamima Hossain*
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)虽然功能强大，但经常产生事实不准确的输出，缺乏稳健的推理能力。在多模态环境中，需要无缝桥接多种模态来增强推理能力。

Method: 开发了一个知识引导的VLM推理框架，利用结构化知识图谱进行多跳验证，包括视觉实体识别、知识图谱遍历和基于事实的标题精炼等系统化推理步骤。

Result: 在由Google Landmarks v2、Conceptual captions和Coco captions组成的精选数据集上，该方法将事实准确性提高了约31%，揭示了推理模式和失败案例的关键见解。

Conclusion: 这项工作展示了将外部知识集成到VLM中以推进推理能力的潜力，为构建更可靠和知识丰富的多模态系统铺平了道路。
...
### [410] [PaTAS: A Parallel System for Trust Propagation in Neural Networks Using Subjective Logic](https://arxiv.org/abs/2511.20586)
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标如准确率无法捕捉模型预测的不确定性或可靠性，特别是在对抗性或退化条件下。

Method: 使用主观逻辑，通过信任节点和信任函数在网络中传播输入、参数和激活信任度，定义参数信任更新机制和推理路径信任评估方法。
...
### [411] [Building a Foundation Model for Trajectory from Scratch](https://arxiv.org/abs/2511.20610)
*Gaspard Merten,Mahmoud Sakr,Gilles Dejaegere*

Main category: cs.AI

TL;DR: 本教程展示了如何从GPT-2开始构建轨迹基础模型的最小实现，比较了TrajFM和TrajGPT等代表性模型，并介绍了TimesFM的补丁方法，旨在帮助SIGSPATIAL社区构建和评估移动性基础模型。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏关于如何从零开始构建移动性轨迹基础模型的明确文档和指南，本教程旨在填补这一空白，为研究人员和实践者提供概念解释和实现层面的指导。

Method: 通过逐步代码驱动的方法，演示如何将GPT-2适配到时空数据，并比较分析TrajFM、TrajGPT等轨迹基础模型的架构创新和差异，同时引入TimesFM的补丁方法等补充技术。

Result: 提供了一个完整的教育材料，支持SIGSPATIAL社区构建和评估移动性基础模型，提升了移动性AI领域的研究清晰度和同行评审效率。

Conclusion: 创建这样的教育材料对于支持社区构建和评估移动性基础模型是及时且不可或缺的，有助于增强研究清晰度和同行评审效果。
...
### [412] [Copyright Detection in Large Language Models: An Ethical Approach to Generative AI Development](https://arxiv.org/abs/2511.20623)
*David Szczecina,Senan Gaffori,Edmond Li*

Main category: cs.AI

TL;DR: 开发了一个开源版权检测平台，帮助内容创作者验证其作品是否被用于LLM训练数据，相比现有方法计算开销减少10-30%，并提供用户友好的界面。


<details>
  <summary>Details</summary>
Motivation: 解决LLM训练数据中未经授权包含受版权保护内容的问题，现有检测框架计算密集且对独立创作者不友好，需要可扩展、透明且用户友好的解决方案。

Method: 通过优化API调用提高效率，改进相似性检测，优化数据集验证，并提供直观的用户界面和可扩展的后端。

Result: 计算开销减少10-30%，提供易于使用的平台，增强了AI开发透明度和伦理合规性。

Conclusion: 该框架为负责任AI开发和版权执法奠定了基础，促进了AI开发透明度。
...
### [413] [Fighting AI with AI: Leveraging Foundation Models for Assuring AI-Enabled Safety-Critical Systems](https://arxiv.org/abs/2511.20627)
...
<details>
  <summary>Details</summary>
Motivation: AI组件（特别是DNN）集成到安全关键系统面临验证挑战，包括AI系统不透明性、高层需求与底层网络表示之间的语义鸿沟，以及需求工程中长期存在的自然语言规范模糊性和形式化可扩展性问题。
...
Conclusion: 通过AI技术本身解决AI系统验证挑战是可行的，REACT和SemaLens为安全关键系统中AI组件的可靠集成提供了有效方法。
...
### [414] [Energy Costs and Neural Complexity Evolution in Changing Environments](https://arxiv.org/abs/2511.20018)
*Sian Heesom-Green,Jonathan Shock,Geoff Nitschke*

Main category: cs.NE

TL;DR: 在能量约束下，季节性增加导致人工神经网络变小，挑战了认知缓冲假说，支持昂贵大脑假说，并显示能量成本促进了更高效网络结构的演化。


<details>
  <summary>Details</summary>
Motivation: 研究环境变异性和能量成本如何影响神经复杂性的演化，探讨大脑大小和组织结构在认知能力中的作用，以及如何通过合适的架构缓解能量挑战。

Method: 使用强化学习代理演化人工神经网络，研究环境变异性和能量成本对神经网络大小和结构复杂性的影响。

Result: 在能量约束条件下，季节性增加导致人工神经网络变小；网络结构复杂性主要是大小的副产品，能量成本促进了更高效网络的发展。
