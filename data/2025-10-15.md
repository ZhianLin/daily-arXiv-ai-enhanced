...
Motivation: 随着全球月球探索活动加剧，对高质量3D月球地图的需求日益迫切，特别是针对长距离任务如NASA的Endurance任务概念。Kaguya TC图像虽然全球可用，但存在立体匹配误差和JPEG压缩伪影导致的海拔不准确问题。

Method: 分析Kaguya TC图像的压缩行为，识别系统性视差噪声模式（特别是在较暗区域），提出通过减少压缩图像生成的视差图中的残余噪声来增强3D地图质量的方法。

Result: 实验结果表明，所提出的方法有效减少了高程噪声。

Conclusion: 该方法能够提升未来月球任务地形数据的安全性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 澄清CLIP相对于自监督模型DINO在视觉语言模型中的优势是否源于语言监督还是更大的训练数据，为视觉编码器设计提供科学依据。

Method: 在相同架构、数据集和训练配置下预训练CLIP和DINO，分析嵌入特征，并集成到视觉语言模型中评估20个VQA基准。

Result: CLIP捕捉高级语义（对象类别、文本），DINO对低级特征（颜色、样式）更敏感。CLIP在文本密集型VQA任务表现优异，DINO在视觉中心任务略优。语言监督变体带来的增益有限。

Conclusion: 研究为视觉编码器设计及其对视觉语言模型性能影响提供了科学见解，表明不同监督方式导致特征表示差异，从而影响下游任务表现。
...
Method: 引入乳腺组织感知数据增强采样器进行图像级和块级监督，以及利用3D数字乳腺断层合成结构的跨切片对比学习目标。

Result: 在多个乳腺癌筛查任务中达到最先进性能，在五个基准数据集上表现出良好的泛化能力。

Conclusion: 为乳腺X线摄影提供可扩展、无需标注的多用途计算机辅助诊断工具基础，有助于减轻放射科医生工作量并提高乳腺癌筛查诊断效率。
...
Method: 双模型框架：VideoLLaMA负责视频描述生成，Qwen2.5-VL负责视觉问答，分别训练以发挥各自优势。

Result: 在WTS数据集上，VideoLLaMA获得CIDEr 1.1001，Qwen2.5-VL获得VQA准确率60.80%，AI City Challenge Track 2中S2得分45.7572排名第10。

Conclusion: 分离训练策略优于联合训练，VQA准确率提升8.6%且保持描述质量，证明了任务专业化的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 准确估计房间3D布局在机器人、增强现实和室内设计中有重要应用价值，需要能够处理各种复杂布局的方法。
...
Conclusion: 该方法在房间布局估计中具有高精度，证明了TPS变换与全景图像的兼容性，能鲁棒处理各种布局类型。
...
Method: 将掩码维度（边界框坐标）嵌入输入提示，使模型能够推理对象几何和布局；针对四个问题类别进行任务特定监督的微调；在训练集中向GPT响应添加规范化答案。
...
<details>
  <summary>Details</summary>
Motivation: 在医学影像中，模型决策的可解释性直接影响临床信任和采用。Vision Transformers虽表现优异，但其复杂注意力机制给可解释性带来挑战。
...
Conclusion: 通过提高模型透明度，这项研究支持ViTs在关键医学诊断工作流程中的可靠和可解释集成。
...
<details>
  <summary>Details</summary>
Motivation: 水下伪装目标检测对海洋生态研究和资源勘探至关重要，但现有方法面临水下图像退化（低对比度、颜色失真）和海洋生物自然伪装两大挑战。传统图像增强技术难以恢复退化图像的关键特征，而陆地场景的伪装目标检测方法由于缺乏对水下光学特性的考虑，往往无法适应水下环境。

Method: 提出APGNet网络：1）使用MSRCR算法进行数据增强，生成光照不变图像以减轻退化效应；2）设计扩展感受野模块和多尺度渐进解码器来捕获多尺度上下文信息并优化特征表示；3）提出自适应先验引导机制，通过在高层特征中嵌入空间注意力进行粗定位，在低层特征中使用可变形卷积细化轮廓。

Result: 在两个公共MAS数据集上的大量实验结果表明，APGNet在广泛使用的评估指标下优于15种最先进的方法。

Conclusion: APGNet通过自适应先验引导机制有效解决了水下伪装目标检测的挑战，在检测精度和鲁棒性方面表现出色。
...
Method: 利用姿态和位置先验从源视频中学习通用运动表示，通过这种表示生成保持原始运动的新视频，同时允许结构和语义的灵活性。

Result: 定性和定量评估都表明该方法优于现有方法，能够有效解决时间不一致和身份漂移问题。

Conclusion: VidMP3通过运动表示学习实现了运动保持的视频编辑，在保持原始运动的同时提供了结构和语义的编辑灵活性，代码将开源发布。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉领域面临高质量标注数据稀缺的挑战，获取标注数据成本高昂且有时不可能。需要找到可靠方法来克服数据不足问题，达到与基准结果相当的性能。

Method: 采用领域自适应方法，使用在特定数据集上训练的模型来预测来自不同领域但相同类型的数据，例如用飞机绘画训练的模型预测真实飞机图像。

Result: 领域自适应方法能够有效应对标注数据稀缺问题，使模型能够在不同领域间进行知识迁移。

Conclusion: 领域自适应是解决计算机视觉中数据稀缺问题的有效方法，能够实现跨领域知识迁移和预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有音频驱动视频生成方法在唇形同步准确性、长视频时间一致性以及多角色动画方面仍存在挑战，需要更高效、可控的解决方案。

Method: 采用扩散变换器框架，结合LoRA训练策略和位置偏移推理实现长视频生成；使用部分参数更新和奖励反馈增强唇形同步；提出无需训练的Mask-CFG方法支持多角色动画。

Result: 实验结果表明该方法在质量、时间一致性和多角色动画方面优于现有最先进方法，实现了高质量、高效且成本效益高的音频驱动视频生成。

Conclusion: 该方法通过简单高效的方式解决了音频驱动视频生成中的关键挑战，在唇形同步、长视频一致性和多角色动画方面取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内布局设计中多样化、高质量训练数据的迫切需求，支持基于LLM的3D场景生成研究。

Method: 构建包含18种常见房间类型的大规模数据集，提供实例级自然语言标注，建立严格基准评估LLM驱动的场景生成，采用监督微调方法。

Result: 在IL3D上进行监督微调的LLM显著提高了泛化能力，性能优于在其他数据集上的微调结果。

Conclusion: IL3D作为一个多功能、强大的资源，通过提供高保真场景数据，显著推进了3D场景生成和具身智能的研究，支持具身代理的环境感知任务。
...
Method: 提出边缘引导注意力块(EGAB)将显式边缘先验嵌入Transformer架构，基于此开发EG-Restormer处理严重模糊；设计轻量高效的LENet处理轻度模糊；集成两者形成自适应双网络(ADNet)，根据输入模糊程度动态选择合适网络。

Result: 大量实验表明，EG-Restormer和ADNet在保持竞争性速度的同时实现了最先进的性能，显著提高了严重模糊QR码的解码率。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法存在流程僵化、搜索调用过多、搜索查询构建不佳等问题，导致效率低下和结果不理想。需要开发能够动态适应现实世界变化信息的多模态模型。

Method: 采用两阶段训练：冷启动监督微调阶段和在线强化学习优化。创建了DeepMMSearchVQA多模态问答数据集，包含多样化的多跳查询，教导模型何时搜索、搜索什么、使用哪种搜索工具以及如何对检索信息进行推理。

Result: 在多个知识密集型基准测试中进行了广泛实验，证明了该方法的优越性。

Conclusion: 该研究为推进多模态网络搜索提供了有价值的见解，展示了动态搜索查询构建和迭代信息检索的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏可靠的几何监督，难以在观测区域产生高质量重建，更不用说未观测区域；同时缺乏有效机制来缓解生成图像中的多视角不一致性，导致严重的形状-外观模糊和场景几何退化。

Method: 利用平面结构生成精确度量尺度深度图，为观测和未观测区域提供可靠监督；将几何引导融入生成管道，改进可见性掩码估计、指导新视角选择，并使用视频扩散模型增强多视角一致性修复。

Result: 在Replica、ScanNet++和DeepBlending上的实验表明，该方法在几何和外观重建方面始终优于现有基线，特别是在未观测区域；支持单视图输入和无姿态视频，在室内外场景中具有强泛化性和实际应用价值。

Conclusion: 准确几何是利用生成模型增强3D场景重建的基本前提，所提出的几何引导方法能够实现准确一致的场景补全，在多种场景下表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决预训练模型在非排练类增量学习中的三个核心问题：模型复杂度不断增加、增量学习过程中的表示漂移问题，以及阶段优化与全局推理之间的不一致性。

Method: 使用增量并行适配器（IPA）网络构建在预训练模型之上，通过轻量级适配器继承和传播表示能力；设计分离锚监督（DAS）方法，通过虚拟锚点分别约束正负样本，促进判别表示学习。

Result: 在六个基准测试上的广泛实验表明，DRL在整个类增量学习过程中始终优于其他最先进方法，同时在训练和推理阶段保持高效率。

Conclusion: DRL框架通过IPA网络和DAS方法有效解决了类增量学习的关键挑战，实现了高性能和高效率的增量学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的方法依赖显式退化先验或全局统计指导，难以处理局部伪影和面部颜色问题，需要更有效的区域控制修复方法。

Method: 使用预训练扩散模型在弱指导下生成伪参考脸，通过人脸解析图和划痕掩码进行区域选择性修复，采用分阶段监督：结构引导贯穿去噪过程，颜色细化在后期步骤进行。

Result: 构建了包含300张真实老照片的VintageFace基准数据集，SSDiff在感知质量、保真度和区域可控性方面优于现有GAN和扩散方法。

Conclusion: SSDiff通过自监督选择性引导扩散模型，有效解决了老照片人脸修复中的局部退化和颜色问题，实现了更好的区域控制修复效果。
...
<details>
  <summary>Details</summary>
Motivation: RAIG系统的广泛采用引发了未经授权使用私有图像数据集的担忧，传统数字水印方法在RAIG系统中由于复杂的特征提取和重组过程而失效。
...
Conclusion: 该方法为压缩单光子3D成像提供了实用的编码函数设计解决方案，能够有效应对实际硬件约束并提升系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前开源视觉描述模型与商业模型存在较大性能差距，限制了数据合成等应用。需要开发能够桥接这一差距的解决方案。

Method: 提出CapFlow多智能体协作工作流，利用开源模型实现高质量视觉描述；使用CapFlow作为数据合成器，大规模生成图像和视频描述数据；通过微调获得通用视觉描述器MetaCaptioner。

Result: CapFlow在多个视觉领域实现与GPT-4相当的描述质量，成本降低89.5%；MetaCaptioner不仅达到与商业模型相当的描述能力，还在开源社区达到顶级多模态性能。

Conclusion: CapFlow和MetaCaptioner为未来多模态研究提供了强大且经济高效的视觉描述解决方案，能够推动多模态研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 远程生理测量需要收集用户隐私信息，现有方法依赖带标签的客户端数据，难以在缺乏标签的情况下更新已部署模型。

Method: 使用联邦无监督域泛化协议，包含最小偏差聚合模块和全局分布感知学习控制器，动态调整聚合权重和训练策略。

Result: 在RGB视频和毫米波雷达的生理参数估计中，性能优于现有技术。

Conclusion: FedHUG框架有效解决了隐私保护、数据异构和标签分布偏差等问题，在无监督场景下实现了优越的生理测量性能。
...
Method: 设计域知识分裂(KFI)模块自适应从配对类感知域提示池中分离新域知识，减轻旧域负知识影响；设计域知识融合(KFU)模块以最小成本将分裂的新知识合并到现有知识池中，采用贪婪知识动态合并策略。

Result: 在ImageNet-C数据集上的大量实验验证了所提方法相对于其他方法的有效性。

Conclusion: KFF方法通过知识融合与分裂机制，能够有效解决CTTA中的知识遗忘和新知识学习问题，提升模型在未知下游域分布上的适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于原型的单样本分割方法使用确定性平均特征，无法捕捉类内多样性，导致泛化能力不足

Method: 使用扩散过程生成原型变体，结合空间感知条件机制和保守融合策略，保持训练-推理一致性

Result: 在腹部MRI和CT数据集上取得显著改进，达到最先进的单样本医学图像分割性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的预训练状态空间模型虽然能高效处理视频数据，但其顺序压缩的视觉提示令牌无法充分捕捉视频的空间和时间上下文信息，限制了空间信息在视频帧内和时间信息在帧间的有效传播，从而影响判别性信息的提取。

Method: 设计了帧内聚集（IFG）模块来聚合每帧内的空间关键信息，以及帧间传播（IFS）模块来在不同帧间传播判别性时空信息。通过自适应平衡和压缩帧内和帧间的关键时空信息，SSP以互补方式有效传播视频中的判别性信息。

Result: 在四个视频基准数据集上的大量实验验证了SSP方法的有效性，相比现有最先进方法平均提升2.76%，同时减少了微调参数的开销。

Conclusion: SSP方法通过有效聚合和传播视频中的关键时空信息，显著提升了视频理解的性能，同时保持了参数效率，为视频分类任务提供了一种有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在多模态重建方面存在局限性，需要统一的框架来同时实现高保真的几何和语义重建。

Method: 采用CUDA加速的光栅化管线，通过可微射线-椭球体相交渲染深度，推导表面法线渲染的解析梯度，并引入可学习属性进行高斯修剪。

Result: 定量和定性实验表明，该方法在所有模态上都达到了最先进的重建精度。

Conclusion: UniGS框架通过几何感知的范式，在多模态3D重建中实现了高效且准确的结果。
...
<details>
  <summary>Details</summary>
Motivation: 完全监督的3D实例分割需要密集的点级标注，成本高昂。边界框标注虽然更易获取，但在重叠区域存在模糊性，传统两阶段方法训练复杂且效率低。

Method: 采用师生框架，教师模型作为伪标签生成器，通过指数移动平均更新。引入基于实例中心的查询精化来提升定位精度，并设计了查询一致性损失和掩码特征一致性损失来对齐预测与伪掩码。

Result: 在ScanNetV2和S3DIS数据集上的实验表明，BEEP3D在保持计算效率的同时，达到了与最先进弱监督方法相当或更优的性能。

Conclusion: BEEP3D提供了一种高效且有效的端到端解决方案，显著降低了3D实例分割的标注成本，同时保持了高性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法难以有效传递教师MLLM的丰富视觉感知能力到学生模型，主要原因是视觉注意力错位问题。

Method: 提出CompoDistill框架，显式对齐学生和教师模型的视觉注意力，增强学生的视觉感知能力。

Result: 在需要视觉感知能力的组合推理任务上显著提升性能，同时在视觉问答任务上保持强性能，并在更先进骨干网络上验证了泛化性。

Conclusion: CompoDistill通过视觉注意力对齐有效解决了MLLM知识蒸馏中的视觉感知能力传递问题，具有良好泛化性。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在计算机视觉领域的发展为优化物流和制造工作流程提供了新机会，但AI应用常受限于专业知识和资源，依赖通用模型需要领域特定数据进行微调，而收集真实数据成本高昂。
...
Method: DIANet双流框架：一个流编码起始-顶点相位，另一个流捕捉顶点-结束相位；每个流使用专用CNN处理，通过交叉注意力融合模块自适应整合特征。

Result: 在CASME-II、SAMM和MMEW三个基准数据集上的广泛实验表明，该方法持续优于传统单相位动态图像方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在推理任务上表现优异，但构建高性能推理训练数据集的原则仍不明确，需要系统研究数据筛选策略对模型推理能力的影响。

Method: 通过控制训练和评估设置，分析上下文来源的影响，实施针对性数据干预（如图像描述辅助信号、纯文本推理），并探索图像、问题和思维链解决方案的规模化扩展。

Result: 发现上下文来源策略显著影响性能，数据干预带来实质性增益，扩展所有数据维度持续提升推理能力。基于此构建的HoneyBee数据集使3B参数模型在MathVerse上超越SOTA模型7.8%。

Conclusion: 本研究为视觉语言推理数据集的筛选提供了改进策略，提出的测试时缩放策略能在不牺牲准确性的情况下降低73%解码成本。
...
<details>
  <summary>Details</summary>
Motivation: 提高生成模型的推理效率是商业化应用的关键挑战，但并行预测多个标记会因标记不兼容导致结构不一致问题，且传统方法无法回溯修正错误预测

Method: 采用新颖的训练方案，在上下文中注入随机标记以提高鲁棒性，并在采样过程中实现标记修复，结合自回归序列标记建模与每步多标记预测

Result: 在ImageNet-256、CIFAR-10图像生成以及UCF-101、NuScenes视频生成任务上均取得显著改进

Conclusion: 该方法在保持并行标记预测效率优势的同时，通过迭代细化机制有效解决了标记不兼容问题，显著提升了生成质量
...
Method: 采用两阶段方法：第一阶段使用小波引导的跨域合成生成与目标域对齐的训练样本；第二阶段通过真实域噪声不变性学习构建动态噪声库，并利用自监督损失学习噪声不变性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频表示方法过度依赖不稳定且过于细粒度的运动和外观建模先验（如像素级匹配和跟踪），几个像素的跟踪错误就会导致视觉对象表示崩溃，无法处理遮挡和大范围运动。

Method: 使用分层代理节点稳定表达视觉对象的多尺度结构，通过动态表示更新机制利用视频时空先验来缓解不准确跟踪器的影响，并对形状和纹理表示进行解耦编码。

Result: 实验表明该方法以更少的参数实现高视频重建精度，支持视频修复和基于关键帧的时间一致性视频编辑等复杂任务。

Conclusion: 代理节点表示方法能够有效处理跟踪误差、长期运动、遮挡和视角变化，提供可控和细粒度的外观编辑能力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像由于隐私、伦理和标注成本等原因存在数据稀缺问题，而广泛使用的交叉熵和Dice损失的加法组合对超参数敏感且性能欠佳，特别是在数据有限的情况下。

Method: 乘法损失将交叉熵和Dice损失相乘，基于预测置信度动态调节梯度；置信度自适应乘法损失在此基础上应用置信度驱动的指数缩放，强调困难样本。

Result: 在细胞和医学分割基准测试中，该框架始终优于调优的加法损失和现有损失函数。

Conclusion: 该框架为在具有挑战性的数据限制下进行鲁棒分割提供了一种简单、有效且无需超参数调节的机制。
...
Result: 在多个标准OOD检测基准测试中验证了方法的有效性，与现有后处理方法具有广泛的组合兼容性，达到了新的最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督融合方法依赖手工设计的损失函数，存在参考图像细节不足、亮度不均，以及梯度损失仅关注梯度幅度的局限性。

Method: 设计跨模态互补掩码模块、结合拉普拉斯边缘增强和自适应直方图均衡化的参考图像合成策略，以及同时约束梯度幅值和方向的角度感知损失。

Result: 在MSRS、RoadScene和M3FD数据集上的实验表明，AngularFuse明显优于现有主流方法，在挑战性场景中产生更清晰、更详细的结果。

Conclusion: AngularFuse通过同时约束梯度幅值和方向，能够保留纹理强度和正确的边缘方向，展示了优越的融合能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的医学基准主要评估一般医学任务，无法充分捕捉在脊柱等依赖视觉输入的细微领域的性能表现，需要专门的评估工具。

Method: 通过整合和标准化开源脊柱疾病数据集的图像-标签对，构建SpineBench基准，并为每个VQA对基于视觉相似性采样具有挑战性的硬负样本选项。
...
<details>
  <summary>Details</summary>
Motivation: 当前动态3D城市场景重建方法面临保真度与计算成本的明显权衡，这是因为它们采用语义无关的设计，对静态背景和安全关键对象分配相同资源。

Method: PAGS引入两个核心贡献：1) 语义引导的剪枝和正则化策略，使用混合重要性度量来简化非关键场景元素，同时保留对导航至关重要的对象的精细细节；2) 优先级驱动的渲染流水线，使用基于优先级的深度预传递来剔除被遮挡的基本体并加速最终着色计算。

Result: 在Waymo和KITTI数据集上的广泛实验表明，PAGS实现了卓越的重建质量，特别是在安全关键对象上，同时显著减少了训练时间并将渲染速度提升到超过350 FPS。

Conclusion: PAGS通过将任务感知语义优先级直接注入3D重建和渲染流水线，有效解决了动态3D城市场景重建中的效率与质量权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本-视频检索方法假设视频经过预剪辑且只包含文本相关内容，但实际视频通常未经剪辑、时长较长且包含复杂背景内容，因此需要解决更实用的部分相关视频检索任务。

Method: 采用双学习框架与动态知识蒸馏，包含大型教师模型和紧凑的双分支学生网络。学生网络包括继承分支（吸收教师模型的可迁移知识）和探索分支（学习PRVR数据集的任务特定信息）。引入动态软目标构建机制，用自适应软目标替代刚性硬目标监督。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态推理方面取得显著进展，但仍容易产生幻觉输出，特别是在logo识别中，模型会基于纯符号logo错误生成品牌名称，这一问题尚未被系统研究。

Method: 使用纯符号、混合和含文本logo的精选数据集，以及Hard-60挑战子集，系统测量领先VLMs的幻觉程度。通过九种结构化扰动测试鲁棒性，并在嵌入层分析LLaVA模型，识别与幻觉相关的投影器维度。

Result: 研究发现幻觉在强失真下仍然存在，遮挡暴露了最严重的弱点。幻觉与投影器的小部分维度相关，针对性消融可大幅减少错误，同时保持OCR准确性。模型往往依赖符号先验而非真正的字形感知。

Conclusion: 投影器子空间在幻觉失败模式中起决定性作用。研究为构建更可信的多模态系统提供了新的诊断视角和可行的缓解策略，包括投影器解耦和OCR引导解码等方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决街景场景中的新视角合成挑战，要求从训练轨迹的车载帧生成不同轨迹（如不同车道或行驶方向）的同一城市环境渲染。

Method: 两阶段方法：首先使用高斯溅射进行3D场景重建并渲染目标相机视角，然后使用专用单步扩散模型增强生成的帧。

Result: 在公开测试排行榜上获得综合得分0.432，总体排名第二。在PSNR、SSIM和LPIPS指标上评估了新视角质量。

Conclusion: 提出的混合方法有效解决了街景新视角合成问题，通过高斯溅射重建和扩散模型增强的组合取得了竞争性结果。
...
Method: 使用多级融合策略通过预训练光流对齐分割、视觉和深度特征；采用课程学习机制从稀疏LiDAR深度渐进过渡到密集立体深度；利用SAM提供类别无关语义先验。

Result: 在SemanticKITTI基准测试中达到16.9的平均IoU，实现最先进性能。

Conclusion: 运动引导和课程感知设计有效提升了基于相机的3D语义场景补全能力。
...
TL;DR: 提出了STORM-PSR双流框架，结合空间和时间特征来改进程序步骤识别，特别针对部分遮挡情况下的鲁棒性提升。
...
Result: 在MECCANO和IndustReal数据集上，相比现有方法，将实际与预测装配步骤完成之间的平均延迟分别降低了11.2%和26.1%。

Conclusion: 时空流不依赖物体无遮挡视图即可推断完成步骤，显著提升了程序步骤识别的性能。
...
TL;DR: 该论文提出了一种概率重解释的场景坐标回归模型训练方法，通过注入高层次重建先验来改善模型在训练图像多视角约束不足时的退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的场景坐标回归模型在训练图像缺乏足够多视角约束时会退化，需要引入先验知识来提升模型性能。

Method: 提出了概率重解释的训练框架，注入从简单深度分布先验到基于大规规模室内扫描数据训练的3D点云扩散模型等多种先验，在训练过程中将预测的3D场景点推向更合理的几何结构。

Result: 在三个室内数据集上的实验表明，该方法能够学习到更好的场景表示，产生更连贯的场景点云、更高的配准率和更好的相机位姿，对下游任务如新视角合成和相机重定位有积极影响。

Conclusion: 通过注入高层次重建先验可以显著提升场景坐标回归模型的性能，特别是在训练数据多视角约束不足的情况下。
...
TL;DR: 这篇系统综述研究了视觉语言模型在城市监测中的应用，特别关注零样本应用，分析了32篇相关研究来回答四个核心研究问题。


<details>
  <summary>Details</summary>
Motivation: 传统城市基础设施监测方法依赖物联网传感器和人工检查，成本高、难以扩展，且与市民的视觉感知不一致。视觉语言模型整合了视觉理解和自然语言推理，有望解决这一挑战。

Method: 采用PRISMA方法论，分析了2021-2025年间发表的32篇同行评审研究，重点关注零样本应用。

Result: 研究回答了四个核心问题：VLM有效解决的城市监测任务、常用且性能优越的VLM架构、支持该领域的数据集资源，以及VLM应用的评估方法和性能水平。

Conclusion: 视觉语言模型在城市监测领域展现出巨大潜力，特别是在零样本应用方面，为城市基础设施监测提供了新的技术路径。
...
TL;DR: 提出基于条件流匹配(CFM)的图像质量转换框架，用于从低场MRI重建高场质量图像，在减少参数的同时实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 低场MRI(LF-MRI)具有经济便携优势，但信噪比低、诊断质量差，需要在不增加昂贵基础设施的情况下提升图像质量。

Method: 使用条件流匹配(CFM)框架，通过直接回归最优速度场，学习噪声分布到目标数据分布的连续流，而非依赖迭代采样或对抗目标。

Result: CFM在低场MRI重建中实现最先进性能，对分布内外数据都具有鲁棒性，且参数数量显著少于竞争深度学习方法。
...
TL;DR: VideoLucy是一个基于深度记忆回溯的长视频理解框架，通过分层记忆结构和迭代回溯机制解决现有方法在时序理解和关键信息保留方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的代理系统在长视频理解中存在两个主要问题：无法有效捕捉连续帧的时序上下文，以及稀疏帧采样可能导致关键信息丢失。

Method: 提出分层记忆结构，采用从粗到细的渐进粒度，通过基于代理的迭代回溯机制系统挖掘视频范围内的深度记忆，直到收集足够信息给出答案。

Result: 在多个长视频理解基准测试中显著优于现有最先进方法，基于开源模型的性能甚至超过了GPT-4o等专有模型。

Conclusion: VideoLucy框架通过深度记忆回溯机制有效解决了长视频理解中的时序建模和细节保留问题，并引入了EgoMem基准来全面评估模型能力。
...
TL;DR: 这篇论文是关于纵向放射学报告生成（LRRG）的首个全面综述，特别关注胸部X光片报告生成，旨在解决传统单图像方法的局限性，通过整合纵向数据来提升临床准确性。


<details>
  <summary>Details</summary>
Motivation: 传统胸部X光片报告生成方法依赖单张图像，无法捕捉纵向临床背景，导致比较性描述不准确。现有综述主要关注单图像方法，缺乏对纵向设置的系统指导。

Method: 本综述系统性地审查了纵向放射学报告生成的数据集构建策略、报告生成架构设计、纵向定制化设计以及评估协议，包括纵向特定指标和广泛使用的基准。
...
<details>
  <summary>Details</summary>
Motivation: 针对深度伪造检测器在黑盒环境下的防御需求，开发能够生成高转移性和视觉不可察觉对抗样本的方法。

Method: 采用两阶段框架：第一阶段使用双流攻击模块（MNTD-PGD和SG-PGD）生成对抗候选；第二阶段通过度量感知选择模块基于成功率和结构相似性评估候选样本。
...
TL;DR: 提出了一种早期融合框架，在数据增强前结合文本和视觉特征以保持空间一致性，并设计轻量级生成器将文本嵌入投影到视觉空间，在医学图像分割任务中取得先进结果。


<details>
  <summary>Details</summary>
Motivation: 解决传统数据增强方法（如旋转、翻转）破坏图像与文本空间对齐的问题，提升多模态医学图像分割的性能。

Method: 采用早期融合框架，在数据增强前融合文本和视觉特征；设计轻量级生成器将文本嵌入投影到视觉空间，弥合语义鸿沟。

Result: 在三个医学成像任务和四个分割框架上评估，取得了最先进的结果；生成伪图像的视觉化显示准确的区域定位。

Conclusion: 提出的早期融合方法有效保持了多模态数据的空间一致性，在医学图像分割任务中表现优异，代码已开源。
...
TL;DR: 提出了一种双阶段3D高斯泼溅框架(BSGS)，用于从运动模糊图像中准确重建3D场景，通过相机姿态优化和全局刚性变换解决现有方法的局限性。
...
Method: 采用双阶段框架：1)相机姿态精化阶段粗略优化相机姿态；2)全局刚性变换阶段修正运动引起的模糊失真。引入子帧梯度聚合策略和时空双阶段优化策略。

Result: 综合实验验证了该去模糊方法的有效性，并显示其在性能上优于现有最先进方法。

Conclusion: BSGS框架成功解决了从运动模糊图像重建高质量3D场景的挑战，通过创新的双阶段优化策略显著提升了重建质量。
...
TL;DR: 提出了一种轻量级、无需网络的Voronoi辅助扩散方法，用于从无定向点云直接计算无符号距离场，解决了现有神经方法在数值稳定性、计算成本和可控性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 无符号距离场能够灵活表示任意拓扑结构的3D形状，但现有神经方法存在数值不稳定、计算成本高和可控性有限的问题。

Method: 使用Voronoi辅助扩散方法：首先基于两个Voronoi几何准则为输入点分配双向法向量，然后扩散对齐的法向量形成近似UDF梯度场，最后积分恢复最终UDF。

Result: 实验表明VAD方法能够稳健处理水密和开放表面，以及复杂的非流形和非可定向几何体，同时保持计算高效和稳定。

Conclusion: VAD方法提供了一种计算高效且稳定的UDF计算方案，适用于各种复杂几何结构。
...
TL;DR: 提出了一个基于分数扩散的简单有效方法，仅通过特征空间归一化和分析推导的损失权重，就能在无条件人体运动生成任务上达到最先进水平，同时直接生成运动和形状。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖过度参数化的输入特征和辅助损失，但这些对于扩散模型匹配人体运动分布应该不是必需的。

Method: 使用分数扩散模型，仅采用特征空间归一化和分析推导的L2分数匹配损失权重，直接生成运动和形状。

Result: 在无条件人体运动生成任务上达到了与最先进方法相当的结果，避免了缓慢的关节后处理形状恢复。

Conclusion: 通过理论驱动的组件设计和针对性消融实验，证明了所提方法的有效性，为人体运动生成提供了更简洁的解决方案。
...
TL;DR: 提出CoIRL-AD框架，通过竞争机制结合模仿学习和强化学习，提升自动驾驶模型的泛化能力，减少18%碰撞率。
...
Result: 在nuScenes数据集上，碰撞率比基线降低18%，在长尾场景中表现更好，泛化能力更强。

Conclusion: CoIRL-AD通过竞争机制有效结合IL和RL，显著提升自动驾驶性能，优于传统的两阶段方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的RGB跟踪算法在无人机视角下因空间外观线索退化而可靠性不足，多光谱图像能提供光谱反射率线索增强目标区分能力，但缺乏专用多光谱无人机数据集阻碍了该领域发展。

Method: 提出多光谱方向感知MOT方案：轻量级Spectral 3D-Stem集成光谱特征并保持RGB预训练兼容性；方向感知卡尔曼滤波器进行精确状态估计；端到端方向自适应transformer。

Result: 广泛实验表明，多光谱输入相比RGB基线显著提升了跟踪性能，特别是对小目标和密集目标效果更明显。

Conclusion: MMOT基准和提出的方法将推动无人机多光谱多目标跟踪研究的发展，相关数据和代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于交叉注意力机制的方法主要捕捉全局交互，难以保持逐步时间对齐，这限制了基于时间相关输入信号学习人类运动的效果。
...
Conclusion: Temporally Conditional Mamba模型通过改进条件信息整合机制，有效解决了人类运动生成中的时间对齐问题，为基于条件输入的运动生成提供了更好的解决方案。
...
TL;DR: 提出了一种零样本农业图像分割方法，结合Plantnet植物分类模型、DinoV2主干网络和Segment Anything Model (SAM)，无需收集新数据集即可实现植物区域的精细分割。


<details>
  <summary>Details</summary>
Motivation: 解决农业图像分割中数据标注瓶颈问题，利用现有植物专用模型避免重新收集和标注数据集的成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明扩散模型的生成质量与其学习的表示相关，外部指导可以加速训练。本文重新思考这一范式，利用模型自身的中间表示进行正则化。

Method: 基于扩散模型不同层表示质量不同的观察，让语义最丰富的表示作为内在指导来引导较弱的表示，提出LayerSync作为即插即用的正则化项。

Result: 在图像生成上广泛评估，并扩展到音频、视频和运动生成等领域。例如，在ImageNet数据集上加速基于流的transformer训练8.75倍，生成质量提升23.6%。

Conclusion: LayerSync是一种自给自足的正则化方法，能一致地提升生成质量和训练效率，且无需额外成本，适用于多种模态。
...
TL;DR: 提出一种两阶段训练框架，显著提升了像素空间扩散模型和一致性模型的性能，在ImageNet上达到SOTA结果，首次成功在无需预训练VAE或扩散模型的情况下直接训练高分辨率一致性模型。


<details>
  <summary>Details</summary>
Motivation: 解决像素空间生成模型训练困难且性能通常不如潜在空间模型的问题，缩小两者之间的性能和效率差距。

Method: 两阶段训练框架：第一阶段预训练编码器捕获干净图像的语义信息并与确定性采样轨迹对齐；第二阶段将编码器与随机初始化解码器集成，对完整模型进行端到端微调。

Result: 在ImageNet-256上扩散模型FID达到2.04（75 NFE），ImageNet-512上达到2.35，大幅超越先前像素空间方法；一致性模型在单步采样下FID达到8.82，显著优于潜在空间对应模型。

Conclusion: 该训练框架成功缩小了像素空间与潜在空间生成模型之间的性能差距，为直接训练高分辨率像素空间模型提供了有效解决方案。
...
TL;DR: 提出IVT-LR方法，通过隐式视觉-文本潜在推理实现多模态推理，无需显式推理步骤标注，在提升准确率的同时显著提高推理速度。
...
Method: IVT-LR方法在潜在空间注入视觉和文本信息，每个推理步骤由潜在文本（前一步隐藏状态）和潜在视觉（选定图像嵌入）组成，采用渐进式多阶段训练策略。
...
TL;DR: 提出了WaterFlow框架，利用整流流技术将水下物理成像信息作为显式先验融入网络训练，提升水下显著目标检测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视水下成像物理原理，将图像退化现象简单视为干扰因素，未能充分利用其中包含的宝贵信息

Method: 基于整流流的框架，将水下物理成像信息作为显式先验直接融入网络训练过程，并引入时间维度建模

Result: 在USOD10K数据集上，S_m指标提升0.072，证明了方法的有效性和优越性

Conclusion: WaterFlow框架通过整合水下物理成像先验和时间维度建模，显著增强了显著目标识别能力
...
Method: 利用图像纹理在不同频带间具有位置相似性和内容一致性的特性，而噪声不具备这一特性，开发了跨频一致性损失和超轻量网络来实现图像去噪。

Result: 在多个真实世界图像数据集上的实验表明，ZSCFC在计算效率和去噪性能方面优于其他最先进的零样本方法。

Conclusion: ZSCFC方法提供了一种高效有效的真实世界去噪解决方案，突破了传统零样本方法的限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进的人体网格恢复方法依赖大型非分层视觉变换器，计算成本高。研究旨在开发简单高效的模型，通过利用分层视觉基础模型的早期阶段来优化性能与效率的平衡。

Method: 构建了三种轻量级HMR2.0变体，并提出使用Swin Transformer、GroupMixFormer和VMamba等分层视觉基础模型的前两个或三个阶段作为编码器，因为观察到这些中间阶段产生的特征图分辨率与非分层模型相当或更高。

Result: 评估了27个基于分层视觉基础模型的人体网格恢复和姿态估计模型，发现仅使用前两或三个阶段就能达到与完整阶段模型相当的性能，且在准确性和计算效率之间具有更好的权衡。

Conclusion: 分层视觉基础模型的早期阶段能够有效替代完整模型，在人体网格恢复和姿态估计任务中实现更好的性能效率平衡，为轻量级模型设计提供了新思路。
...
TL;DR: TerraCodec (TEC) 是一个专为地球观测设计的深度学习压缩算法家族，包括图像压缩变体和时序Transformer模型，在Sentinel-2数据上实现3-10倍压缩提升，并支持零样本云修复。


<details>
  <summary>Details</summary>
Motivation: 地球观测卫星产生大量多光谱图像时间序列，现有压缩方法存在局限性：图像编解码器忽略时间冗余，视频编解码器的运动先验无法捕捉静态场景的辐射演化。

Method: 提出TerraCodec系列，包括适应多光谱输入的高效图像压缩变体和利用时间依赖性的时序Transformer模型(TEC-TT)，并引入Latent Repacking方法训练灵活速率的Transformer模型。

Result: 在Sentinel-2数据上，TerraCodec优于传统编解码器，在同等图像质量下实现3-10倍更强压缩。TEC-TT在AllClear基准测试中零样本云修复性能超越最先进方法。

Conclusion: 定制化的学习压缩算法是地球观测领域有前景的方向，代码和模型权重将以宽松许可证发布。
...
TL;DR: 提出了一种多无人机协作占用预测框架，通过空间感知特征编码和跨智能体特征集成来保持3D空间结构和语义信息，显著提升了协作感知性能并大幅降低了通信开销。


<details>
  <summary>Details</summary>
Motivation: 当前基于鸟瞰图的无人机群协作感知方法存在两个主要局限：边界框表示无法捕捉完整的场景语义和几何信息，且在遇到未定义或被遮挡物体时性能显著下降。

Method: 提出多无人机协作占用预测框架，包含空间感知特征编码器、跨智能体特征集成、高度感知特征压缩和双掩码感知引导机制，以紧凑表示场景信息并减少通信开销。

Result: 实验结果表明，该方法在三个扩展数据集上实现了最先进的精度，显著优于现有协作方法，同时将通信开销降低到先前方法的一小部分。

Conclusion: 该框架有效解决了无人机群协作感知中的语义信息缺失和性能下降问题，为多无人机系统提供了高效可靠的协作感知解决方案。
...
TL;DR: 提出EReLiFM方法解决带噪声标签的开集域泛化问题，通过证据可靠性感知的残差流元学习提升模型在有限干净数据下识别已知类别和拒绝未知类别的能力


<details>
  <summary>Details</summary>
Motivation: 现有方法在带噪声标签的开集域泛化中难以弥合域间差距，特别是在有限干净标注数据的情况下，噪声标签会破坏源域知识，影响模型性能

Method: 采用两阶段证据损失聚类方法提升标签可靠性感知，提出残差流匹配机制建模结构化的域和类别条件残差，通过元学习优化使干净集上的更新方向最大化减少噪声集上的损失

Result: 实验结果显示EReLiFM在带噪声标签的开集域泛化任务上优于现有方法，达到最先进的性能

Conclusion: EReLiFM通过证据可靠性感知和残差流匹配机制有效解决了带噪声标签的开集域泛化问题，为实际应用提供了更可靠的解决方案
...
TL;DR: 提出H-EGL框架，结合自监督和人工指导约束来增强注意力对齐，提高医学影像分类的泛化能力，在胸部X光分类任务中表现优于现有方法。
...
Method: 提出混合解释引导学习框架H-EGL，结合自监督和人工指导约束。自监督组件利用类别区分性注意力，不依赖限制性先验，增强鲁棒性和灵活性。

Result: 在胸部X光分类任务中使用Vision Transformer验证，H-EGL优于两种最先进的解释引导学习方法，展示了更好的分类准确率和泛化能力，产生的注意力图与人类专家知识更一致。
...
TL;DR: IRIS是一个评估多模态大语言模型在"与图像思考"范式下能力的基准，包含1,204个复杂视觉任务，显示当前模型在整合视觉和通用工具方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要遵循"关于图像思考"范式，将图像视为静态输入，而实际应用中需要模型主动操作图像并整合工具来解决复杂任务。

Method: 构建IRIS基准，包含1,204个挑战性开放视觉任务(603单轮，601多轮)，涵盖五个不同领域，配有详细评分标准。

Result: 当前MLLMs在需要有效整合视觉和通用工具的任务上表现不佳，最强模型(GPT-5-think)仅达到18.68%通过率，不同模型展现出不同的工具使用行为。
...
TL;DR: 提出一种新的个性化联邦微调方法，通过正交LoRA适配器分离通用和客户端特定知识，使客户端能够充分利用自身和其他客户端的数据。


<details>
  <summary>Details</summary>
Motivation: 基础模型在医疗AI中应用广泛，但需要针对特定任务进行微调。由于患者隐私限制，难以共享和聚合不同来源的数据进行训练。联邦学习可解决此问题，但需要更好的个性化方法。

Method: 使用正交LoRA适配器来解耦通用知识和客户端特定知识，在多个客户端（医院、诊所等）上进行联邦微调。

Result: 在真实世界联邦医学影像任务上的初步结果显示，该方法与当前联邦微调方法相比具有竞争力。

Conclusion: 提出的个性化联邦微调方法能够有效利用多方数据，同时保护患者隐私，在医疗AI应用中具有良好前景。
...
TL;DR: FlashVSR是首个基于扩散模型的实时视频超分辨率框架，通过三阶段蒸馏、稀疏注意力和小型解码器实现17 FPS的高效处理，在A100 GPU上支持768x1408分辨率视频。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频超分辨率应用中面临高延迟、计算量大和超高分辨率泛化能力差的问题，需要实现高效、可扩展的实时处理方案。

Method: 提出三阶段蒸馏管道实现流式超分辨率，使用局部约束稀疏注意力减少冗余计算，采用小型条件解码器加速重建，并构建了包含12万视频和18万图像的大规模训练数据集VSR-120K。

Result: FlashVSR在超高分辨率下可靠扩展，达到最先进性能，相比之前的一步扩散VSR模型速度提升达12倍，在A100 GPU上实现约17 FPS的处理速度。

Conclusion: FlashVSR使基于扩散模型的视频超分辨率变得实用，实现了效率、可扩展性和实时性能的平衡，将发布代码、预训练模型和数据集以促进高效扩散VSR研究。
...
TL;DR: SPORTS框架通过整合视频全景分割、视觉里程计和场景渲染任务，解决场景感知中的分割缺陷、动态物体干扰、数据稀疏性和视角限制问题，在多个任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有场景感知解决方案存在分割缺陷、动态物体干扰、传感器数据稀疏和视角限制等问题，需要一种更全面的场景理解方法。
...
Conclusion: SPORTS框架通过紧密集成多个任务实现了全面的场景理解，在多个基准测试中表现出色，证明了其有效性。
...
TL;DR: 提出了VQArt-Bench，一个用于文化遗产领域的大规模视觉问答基准，通过多智能体管道生成细致、验证过的多样化问题，评估多模态大语言模型在视觉艺术分析中的深层语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有VQA基准在评估深层语义理解方面存在不足，特别是在视觉艺术分析等复杂领域，问题局限于简单句法结构和表面属性，促使模型利用统计捷径而非进行视觉推理。

Method: 使用新颖的多智能体管道，专门智能体协作生成细致、验证过的语言多样化问题，基准结构围绕相关视觉理解维度设计，探索模型解释符号意义、叙事和复杂视觉关系的能力。

Result: 对14个最先进MLLMs的评估显示当前模型存在显著局限性，包括在简单计数任务中的意外弱点，以及专有模型和开源模型之间的明显性能差距。

Conclusion: VQArt-Bench填补了现有VQA基准的空白，揭示了当前MLLMs在深层视觉语义理解方面的不足，特别是在文化遗产领域的视觉艺术分析中。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将光流和6-DoF自运动估计分开处理，在神经形态视觉（如事件相机）中由于缺乏鲁棒的数据关联而成为不适定问题。现有方法要么引入偏差和计算开销，要么容易陷入局部最优。

Method: 将相机自运动建模为连续样条，光流建模为隐式神经表示，通过归纳偏置嵌入时空连贯性；通过微分几何约束融入结构和运动先验，避免显式深度估计但保持几何一致性。
...
TL;DR: 提出DL-HMC++深度学习方法来预测PET成像中的头部运动，无需硬件追踪设备，在多种扫描仪和示踪剂上表现优异，结果与金标准硬件追踪相当。


<details>
  <summary>Details</summary>
Motivation: 头部运动是脑PET成像的主要挑战，会导致伪影和定量误差。硬件运动追踪在临床实践中应用受限，需要开发数据驱动的运动校正方法。

Method: 使用深度学习结合交叉注意力机制，从1秒3D PET原始数据预测刚性头部运动，利用已有动态PET扫描和外部硬件追踪的金标准数据进行监督训练。

Result: 在两个PET扫描仪和四种示踪剂上验证，DL-HMC++优于现有数据驱动方法，产生的无运动图像与金标准硬件追踪难以区分，标准摄取值差异率仅为1.2±0.5%和0.5±0.2%。

Conclusion: DL-HMC++展示了数据驱动PET头部运动校正的潜力，可消除硬件追踪负担，使运动校正技术更易在临床环境中推广应用。
...
TL;DR: AnyUp是一种无需特定编码器训练的特征上采样方法，适用于任何视觉特征和分辨率，能泛化到不同特征类型。
...
Method: 提出了一种推理时特征无关的上采样架构，无需特定编码器训练即可应用于任何视觉特征。

Result: AnyUp在特征上采样方面达到了新的最先进水平，能泛化到不同特征类型，保持特征语义，且高效易用于各种下游任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型存在视觉理解与视觉生成能力不匹配的问题，模型能正确理解图像但无法忠实生成图像。

Method: 设计了全局-局部双奖励系统：全局奖励确保整体视觉语义和布局正确性，局部奖励优化细粒度对象级保真度。

Result: 在T2I-CompBench上从82.18提升至88.37，在T2I-ReasonBench上从43.82提升至46.75。

Conclusion: 建立了一个新范式，使统一多模态模型的理解模块能够通过自奖励机制指导和增强其生成能力。
...
TL;DR: 本文提出了高效感知超分辨率（EPSR）的全面研究和基准测试，旨在在严格的效率约束下（最多500万参数和2000 GFLOPs）复制或改进Real-ESRGAN的感知结果。


<details>
  <summary>Details</summary>
Motivation: 虽然高效的PSNR导向超分辨率取得了显著进展，但专注于感知质量指标的方法仍然相对低效。

Method: 在严格的效率约束下（最多500万参数和2000 GFLOPs）开发高效感知超分辨率方法，并在包含500张4K测试图像的新数据集上进行评估。
...
TL;DR: USplat4D是一个不确定性感知的动态高斯泼溅框架，通过估计每个高斯的时间变化不确定性并构建时空图进行优化，解决了动态3D场景重建中的运动漂移和遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 传统动态高斯泼溅模型对所有高斯基元进行统一优化，忽略了它们是否被充分观测，导致遮挡下的运动漂移和极端视角下的合成质量下降。

Method: 引入时间变化的每个高斯不确定性估计，构建时空图进行不确定性感知优化，将可靠的运动线索传播到整个场景。

Result: 在多样化的真实和合成数据集上的实验表明，显式建模不确定性持续改善了动态高斯泼溅模型，在遮挡下产生更稳定的几何结构，在极端视角下实现高质量的合成。

Conclusion: 不确定性建模对于动态高斯泼溅至关重要，USplat4D通过不确定性感知优化显著提升了4D重建的质量和稳定性。
...
TL;DR: 提出Flow Poke Transformer (FPT)框架，通过稀疏交互（"pokes"）直接预测局部运动分布，提供可解释的多模态场景运动表示。
...
Method: 使用Flow Poke Transformer框架，基于稀疏交互条件直接预测局部运动分布，提供可解释的运动表示。

Result: 在密集面部运动生成任务中，通用预训练模型超越专用基线；在强分布外任务中微调后，在关节物体运动估计上显著优于域内方法；在基于pokes的运动部件分割任务中表现竞争力。

Conclusion: FPT框架能够直接预测显式运动分布，在多种下游任务中展现出优异的性能和灵活性。
...
TL;DR: MVP4D是一个基于预训练视频扩散模型的系统，能够从单张参考图像生成可动画化的多视角数字人视频，并将输出提炼为可实时渲染的4D虚拟形象。


<details>
  <summary>Details</summary>
Motivation: 传统创建和动画化照片级真实感人类虚拟形象的过程昂贵且耗时，需要大型相机捕捉设备和专业3D艺术家的大量手动工作。现有单图像方法虽然降低了创建门槛，但缺乏多视角信息约束，导致视角偏离参考图像时质量下降。

Method: 基于最先进的预训练视频扩散模型，从单张参考图像和目标表情生成可动画化的多视角视频，能够同时生成数百帧，视角变化可达360度，并将输出提炼为可实时渲染的4D虚拟形象。

Result: 与先前方法相比，该方法显著提高了生成虚拟形象的真实感、时间一致性和3D一致性。

Conclusion: MVP4D通过生成多视角视频并将其提炼为4D虚拟形象，解决了单图像方法在视角偏离时的质量下降问题，为数字人虚拟形象的创建提供了更高效和逼真的解决方案。
...
TL;DR: 本文回顾了AIM 2025高效真实世界去模糊挑战赛，该挑战基于RSBlur数据集，要求参赛者在严格效率约束下（少于500万参数，计算量低于200 GMACs）开发单图像去模糊方法。71个团队注册，4个提交有效方案，最佳方法PSNR达31.1298 dB。
...
Method: 基于RSBlur数据集的双相机系统采集模糊-清晰图像对，参赛者需开发满足参数和计算量限制的去模糊模型。

Result: 71个团队注册，4个提交有效解决方案，最佳方法PSNR达到31.1298 dB，展示了高效方法在该领域的潜力。
...
TL;DR: UniFusion是一个基于扩散的生成模型，使用冻结的大型视觉语言模型作为统一多模态编码器，通过层注意力池化机制提取文本和视觉token的语义和细节信息，实现跨模态推理和知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成架构通常使用分离的图像和文本编码器，限制了扩散模型的跨模态推理和知识迁移能力。之前的方法需要大量计算资源和数据，限制了可访问性。

Method: 提出Layerwise Attention Pooling机制从冻结VLM中提取多层级信息；开发VERIFI方法，在模型内提示重写时仅使用VLM生成的文本token来条件化扩散变换器。

Result: LAP在文本-图像对齐方面优于其他浅层融合架构；在编辑任务上微调不仅提高了生成的对齐度，还展现出强大的泛化能力，能够零样本推广到多图像参考。

Conclusion: UniFusion的统一编码器设计实现了有效的跨模态知识迁移，展示了在有限计算资源下实现高质量多模态生成和编辑的潜力。
...
TL;DR: 提出ViCO方法，通过多MLP连接器根据图像语义复杂度动态调整视觉token数量，减少MLLMs推理成本
...
Method: 使用多个不同压缩比的MLP连接器下采样视觉token，训练时最小化不同连接器间响应的KL散度，推理时通过ViR路由器自动选择合适压缩率

Result: 实验证明该方法可减少最多50%视觉token，同时保持模型的感知、推理和OCR能力

Conclusion: ViCO方法为开发更高效MLLMs做出贡献，代码和模型将开源以促进未来研究
...
TL;DR: CuMPerLay是一种可微分的向量化层，将立方多参数持久性(CMP)集成到深度学习管道中，通过分解CMP为可学习的单参数持久性组合，为图像分析提供稳健的拓扑特征。


<details>
  <summary>Details</summary>
Motivation: 立方多参数持久性(CMP)是处理图像拓扑结构的强大工具，但其应用受到多滤过结构复杂性和向量化困难的阻碍。

Method: 提出新算法将CMP分解为可学习的单参数持久性组合，其中双滤过函数被联合学习，创建可微分的拓扑特征向量层。

Result: 在医学影像和计算机视觉基准数据集上的实验显示，CuMPerLay在分类和分割任务中表现优异，特别是在数据有限的情况下。
...
TL;DR: DriveVLA-W0通过世界建模预测未来图像来解决VLA模型的监督不足问题，使用自回归和扩散两种世界模型架构，在NAVSIM基准测试中显著优于BEV和VLA基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决视觉-语言-动作(VLA)模型在大规模数据训练中面临的监督不足问题，即模型容量巨大但仅由稀疏的低维动作监督，导致表示能力未充分利用。

Method: 提出DriveVLA-W0训练范式，使用世界建模预测未来图像，生成密集的自监督信号。为两种主流VLA架构分别实现：使用离散视觉token的自回归世界模型和使用连续视觉特征的扩散世界模型。

Result: 在NAVSIM v1/v2基准测试和680倍更大的内部数据集上，DriveVLA-W0显著优于BEV和VLA基线方法，并放大了数据缩放定律，显示随着训练数据集增大性能增益加速。

Conclusion: 世界建模是解决VLA模型监督不足的有效方法，能够学习驾驶环境的底层动态，DriveVLA-W0展示了该范式的通用性和优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统坐标回归模型主导物体检测，但MLLM方法面临召回率低、重复预测、坐标不对齐等问题，需要弥合这一差距。

Method: 1) 使用特殊token表示量化坐标；2) 构建多数据引擎生成高质量数据；3) 采用两阶段训练：监督微调+基于GRPO的强化学习后训练。

Result: 在COCO和LVIS基准测试中，Rex-Omni在零样本设置下达到或超过回归模型性能，并展示了物体指代、指向、视觉提示等多种能力。
...
TL;DR: LLMs在逻辑谜题中表现出记忆模板而非真正推理，当谜题被微调时性能显著下降，揭示了模型在语言流畅性和逻辑理解之间的差距。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在解决逻辑谜题时是否真正进行推理，还是仅仅依赖记忆的模板，以及如何系统评估和改进它们的推理能力。

Method: 引入PHANTOM RECALL基准，包含25个经典逻辑谜题和149个精心设计的扰动，评估11个领先LLMs，并开发自动化逻辑等价判断、错误分类和提示缓解框架。

Result: LLMs在未修改谜题上准确率接近完美，但在扰动谜题上表现显著低于人类，出现幻影回忆和过度阐述等错误模式。

Conclusion: LLMs在上下文线索变化时往往无法重新推理，突显了语言流畅性和逻辑理解之间的关键局限性。
...
TL;DR: LLMs作为世界模型在数字环境中增强决策制定，但存在幻觉和长期模拟误差累积问题。R-WoM通过检索外部教程知识来增强LLM模拟，显著提升了长期模拟性能。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否适合作为世界模型，解决其在长期模拟中的幻觉和误差累积问题，以提升在数字环境中的决策能力。

Method: 提出检索增强世界模型(R-WoM)，通过从外部教程检索事实性、最新知识来增强LLM模拟，减少幻觉和误差。评估了三个核心能力：下一状态预测、完整程序规划对齐和里程碑转换识别。

Result: LLMs能有效捕捉即时下一状态和有意义的转换，但在完整程序规划中性能迅速下降。R-WoM相比基线在OSWorld和WebArena上分别提升25.3%和18.1%，在长期模拟中优势更明显。

Conclusion: LLMs在短期状态预测方面有效，但长期模拟可靠性有限。R-WoM通过检索增强显著改善了长期模拟性能，为LLM作为世界模型提供了可行解决方案。
...
TL;DR: 研究发现LLM对陈述真实性的内部表示在输入发生轻微变化时会崩溃，表明模型学习的是浅层、非鲁棒的知识表示，限制了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLM性能脆弱性是否源于内部知识表示的不稳定性，特别是当输入经历语义保持的扰动时，真实陈述与虚假陈述的可分离性如何变化。

Method: 通过应用语义保持的扰动（如拼写错误、重新表述）使样本分布外，在四个LLM家族、五个评估数据集和三种知识探测方法上评估表示可分离性的退化情况。

Result: 当样本呈现与预训练数据相似度降低时，陈述真实性的内部表示会崩溃，模型区分真假陈述的能力高度依赖于陈述的确切表面形式。
...
### [92] [Omni-Captioner: Data Pipeline, Models, and Benchmark for Omni Detailed Perception](https://arxiv.org/abs/2510.12720)
...
TL;DR: 本文提出了Omni-Detective系统，通过工具调用的代理数据生成管道来生成高质量的多模态详细描述数据，并训练了Audio-Captioner和Omni-Captioner模型，在音频和音视频详细感知任务上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前全语言模型在捕捉和描述细粒度细节方面的能力仍有局限，且存在细节与幻觉共增长的问题，需要系统性地解决多模态详细感知的挑战。

Method: 提出了Omni-Detective代理数据生成管道，集成工具调用来自主生成高质量多模态数据；训练了两个描述模型：Audio-Captioner用于音频详细感知，Omni-Captioner用于音视频详细感知；设计了Omni-Cloze填空式评估基准。

Result: Audio-Captioner在MMAU和MMAR上超越所有开源模型，性能媲美Gemini 2.5 Pro；Omni-Captioner在VDC上达到新SOTA，在video-SALMONN 2上实现细节与幻觉的最佳平衡。

Conclusion: Omni-Detective能有效生成高质量详细描述，Omni-Cloze在评估详细描述方面具有优越性，为多模态详细感知提供了系统解决方案。
...
### [93] [LLM Reasoning for Machine Translation: Synthetic Data Generation over Thinking Tokens](https://arxiv.org/abs/2510.11919)
*Armel Zebaze,Rachel Bawden,Benoît Sagot*

Main category: cs.CL

TL;DR: 大型推理模型在机器翻译任务中生成中间思考标记并不能提升翻译性能，这与数学和编程任务中的表现不同。
...
Method: 使用蒸馏思维链方法，通过人工翻译实践启发，生成逐步翻译的合成解释来微调模型。同时测试模块化翻译特定提示策略构建中间标记的方法。

Result: 思维标记不能帮助大型推理模型更好地执行机器翻译。使用合成思维链解释的微调方法不优于标准的输入输出微调。但通过模块化翻译特定提示策略构建中间标记能带来改进。

Conclusion: 中间标记在微调过程中的贡献高度依赖于其中是否包含翻译尝试。使用教师模型优化目标翻译或扩展平行语料库比将思维链解释蒸馏到思考型机器翻译模型中更有效。
...
TL;DR: 提出了MIND用户参与的事实检查流程，用于检测多语言问答知识库中的事实和文化差异，特别关注母婴健康领域的双语问答系统。
...
Method: 开发了用户参与的事实检查流程MIND，通过标注双语问题的事实和文化不一致性来检测差异，并在母婴健康领域进行测试。

Result: MIND在所有测试案例中都能可靠地识别不一致性，支持开发更具文化意识和事实一致性的问答系统。

Conclusion: MIND方法有效检测多语言问答系统中的事实和文化差异，为构建更准确和文化的敏感问答系统提供了支持。
...
### [95] [TopoAlign: A Framework for Aligning Code to Math via Topological Decomposition](https://arxiv.org/abs/2510.11944)
*Yupei Li,Philipp Borchert,Gerasimos Lampouras*

Main category: cs.CL

TL;DR: TopoAlign是一个利用代码库训练数学大语言模型的框架，通过将代码分解并重组为结构上类似形式化数学语句的格式，显著提升了模型在自动形式化任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前数学大语言模型在自动形式化（将非形式化数学语句转换为形式化语句）方面表现不佳，主要受限于大规模训练数据的稀缺。虽然现有模型能够从自然语言生成代码，但代码与形式化数学在结构和语法上的差异限制了有效的迁移学习。

Method: TopoAlign框架将代码分解为文档字符串、主函数和依赖函数，然后将这些组件重新组装成结构上与形式化数学语句对齐的代码数据。这种方法无需额外的人工标注，就能利用广泛可用的代码库作为训练资源。

Result: 在DeepSeek-Math和Herald两个先进模型上的实验表明，TopoAlign带来了显著性能提升：DeepSeek-Math在BEq@10上提升17.77%，在typecheck@10上提升68.82%；即使对于专门的Herald模型，也分别获得了0.12%和1.09%的提升。

Conclusion: TopoAlign框架证明，通过在结构对齐的代码数据上进行训练，即使不引入新的数学知识，也能显著提升数学大语言模型在自动形式化任务上的性能，为利用代码库资源训练数学模型提供了有效途径。
...
### [96] [GRAVITY: A Framework for Personalized Text Generation via Profile-Grounded Synthetic Preferences](https://arxiv.org/abs/2510.11952)
...
Result: 在400名亚马逊用户的图书描述生成任务中，相比提示调节、标准微调和朴素合成对生成方法，GRAVITY实现了超过4%的偏好增益，用户研究显示86%的情况下更偏好GRAVITY输出
...
### [97] [Evaluating Retrieval-Augmented Generation Systems on Unanswerable, Uncheatable, Realistic, Multi-hop Queries](https://arxiv.org/abs/2510.11956)
*Gabrielle Kaili-May Liu,Bryan Li,Arman Cohan,William Gantt Walden,Eugene Yang*

Main category: cs.CL

TL;DR: 提出了CRUMQs（不可作弊、真实、不可回答和多跳查询）自动生成管道，用于创建更具挑战性的RAG基准测试，显著降低作弊可能性并提高任务复杂性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG基准测试难以反映真实任务复杂性，多跳或超出范围的问题常可通过非多跳推理解决，限制了发现现有RAG系统局限性的能力。

Method: 开发了首个自动、难度可控的CRUMQs生成管道，可适应任何语料库和领域，创建不可作弊、真实、不可回答和多跳查询。

Result: 在两个流行RAG数据集上创建CRUMQs，实验显示相比先前基准，CRUMQs对RAG系统极具挑战性，作弊分数降低达81.0%。

Conclusion: 该管道提供了增强基准难度和真实性的简单方法，有助于推动更强大RAG系统的开发。
...
### [98] [Direct Multi-Token Decoding](https://arxiv.org/abs/2510.11958)
*Xuan Luo,Weizhi Wang,Xifeng Yan*

Main category: cs.CL

TL;DR: 提出Direct Multi-Token Decoding (DMTD)方法，利用预训练LLM中不同层级的特性，通过仅使用后层一次性生成多个token，实现推理加速。


<details>
  <summary>Details</summary>
Motivation: 基于预训练LLM中早期层处理输入上下文、中间层处理任务特定信息、后期层转换为输出token的分工特性，假设隐藏状态在通过早期和中间层处理后已包含足够信息支持一次性生成多个token。

Method: DMTD方法：仅使用后期层直接解码多个token，无需重复遍历早期和中间层。不引入额外参数、辅助程序或后生成验证。

Result: 在有限数据集上微调的DMTD Qwen3-4B模型已实现最高2倍加速，仅伴随轻微性能损失。扩展分析表明性能可随训练数据集增大而进一步提升。

Conclusion: DMTD是一种有效的推理加速方法，无需额外开销即可显著提升生成速度，且具有良好的扩展潜力。
...
### [99] [Scaling Long-Horizon LLM Agent via Context-Folding](https://arxiv.org/abs/2510.11967)
*Weiwei Sun,Miao Lu,Zhan Ling,Kang Liu,Xuesong Yao,Yiming Yang,Jiecao Chen*

Main category: cs.CL

TL;DR: 提出了Context-Folding框架，让LLM智能体能够主动管理工作上下文，通过分支处理子任务并折叠中间步骤，大幅减少上下文使用。
...
Method: 开发了端到端强化学习框架FoldGRPO，使用特定过程奖励来鼓励有效的任务分解和上下文管理，智能体可以分支处理子任务并在完成后折叠中间步骤。

Result: 在复杂长视野任务（Deep Research和SWE）上，折叠智能体匹配或优于ReAct基线，同时使用10倍更小的活跃上下文，显著优于基于摘要的上下文管理模型。
...
### [100] [Conjecturing: An Overlooked Step in Formal Mathematical Reasoning](https://arxiv.org/abs/2510.11986)
*Jasivan Alex Sivakumar,Philipp Borchert,Ronald Cardenas,Gerasimos Lampouras*

Main category: cs.CL

TL;DR: 论文提出数学自动形式化需要先进行猜想步骤，创建了ConjectureBench数据集和评估框架来衡量LLMs的猜想能力，并设计了Lean-FIRe方法来改进猜想和自动形式化性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自动形式化方法忽视了关键的猜想步骤，许多数学问题需要先猜想结论才能进行形式化。由于LLMs在自动形式化方面已有困难，且其猜想能力的评估有限且常与自动形式化或证明任务混淆，因此需要专门评估和改进这一能力。

Method: 通过扩展现有数据集创建ConjectureBench，重新设计评估框架和指标来专门衡量LLMs的猜想能力。设计了推理时方法Lean-FIRe来改进猜想和自动形式化。

Result: 评估显示当考虑猜想步骤时，GPT-4.1和DeepSeek-V3.1等基础模型的自动形式化性能被显著高估。使用Lean-FIRe方法，GPT-4.1成功端到端自动形式化了13个PutnamBench问题，DeepSeek-V3.1成功形式化了7个。

Conclusion: LLMs具备生成准确猜想所需的知识，但改进自动形式化性能需要将猜想视为独立任务，并研究如何正确将其整合到自动形式化流程中。论文为未来研究提供了改进数学形式化推理中猜想步骤的指导。
...
### [101] [SAGE: A Top-Down Bottom-Up Knowledge-Grounded User Simulator for Multi-turn AGent Evaluation](https://arxiv.org/abs/2510.11997)
*Ryan Shea,Yunan Lu,Liang Qiu,Zhou Yu*

Main category: cs.CL

TL;DR: SAGE是一个用于多轮交互式智能体评估的用户模拟框架，通过整合商业逻辑知识和企业基础设施，生成更真实多样的交互，能发现更多智能体错误。


<details>
  <summary>Details</summary>
Motivation: 现有模拟用户方法通常建模通用用户，忽视了领域特定原则，无法捕捉真实用户行为，需要结合商业背景知识来改进评估效果。

Method: 提出SAGE框架，整合自上而下的商业逻辑知识（如理想客户画像）和自下而上的企业基础设施（如产品目录、FAQ、知识库），基于真实客户角色生成交互。

Result: 该方法生成的交互更加真实和多样化，能够识别出高达33%的智能体错误，证明了其作为评估工具的有效性。

Conclusion: SAGE通过整合商业背景知识，显著提升了多轮交互智能体评估的真实性和有效性，支持错误发现和迭代改进。
...
### [102] [Generate Logical Equivalence Questions](https://arxiv.org/abs/2510.12001)
*Xinyu Wang,Haoming Yu,Yicheng Yang,Zhiyuan Li*

Main category: cs.CL

TL;DR: 提出了一种新的自动问题生成方法，用于生成离散数学中的逻辑等价问题，以解决学术不端行为并为学生提供大量练习题。


<details>
  <summary>Details</summary>
Motivation: 在线教学时代抄袭现象日益严重，需要为每个学生生成独特问题来防止作弊，同时提供大量练习题。现有方法存在效率低下和问题难度不一致的问题。

Method: 使用形式化语言定义逻辑等价问题，将其转换为两组生成规则，并开发线性时间算法来生成问题。

Result: 学生实验显示生成问题的准确性与教科书问题相当；难度评估表明生成问题与教科书问题难度相似，优于大型语言模型生成的问题。

Conclusion: 提出的自动问题生成系统能够高效生成高质量的逻辑等价问题，其质量和难度与教科书问题相当，为解决学术不端和提供练习资源提供了有效方案。
...
### [103] [Information Extraction from Conversation Transcripts: Neuro-Symbolic vs. LLM](https://arxiv.org/abs/2510.12023)
*Alice Saebom Kwak,Maria Alexeeva,Gus Hahn-Powell,Keith Alcock,Kevin McLaughlin,Doug McCorkle,Gabe McNunn,Mihai Surdeanu*

Main category: cs.CL

TL;DR: 比较神经符号系统和LLM系统在农业领域信息抽取的性能，LLM系统总体表现更好但各有优缺点


<details>
  <summary>Details</summary>
Motivation: 当前信息抽取过度依赖大语言模型，忽略了传统符号或统计系统的经验积累，需要对比不同方法的实际表现

Method: 在农业领域的猪肉、乳制品和作物三个子领域，对9个访谈进行信息抽取，比较神经符号系统和LLM系统的性能

Result: LLM系统在总体F1分数(69.4 vs 52.7)和核心信息F1分数(63.0 vs 47.2)上都优于神经符号系统

Conclusion: 两种系统各有优缺点，部署NLP系统需要平衡性能、效率和可控性，存在"隐藏成本"
...
### [104] [CPR: Mitigating Large Language Model Hallucinations with Curative Prompt Refinement](https://arxiv.org/abs/2510.12029)
*Jung-Woo Shim,Yeong-Joon Ju,Ji-Hoon Park,Seong-Whan Lee*

Main category: cs.CL

TL;DR: 提出CPR框架，通过清理不良提示和生成额外任务描述来减少LLM幻觉，显著提高生成质量


<details>
  <summary>Details</summary>
Motivation: LLM常因用户提示结构不良而产生看似合理但错误的幻觉事实，损害可信度

Method: 使用微调的小型语言模型清理不良提示并生成额外任务描述，实现用户意图与提示的对齐

Result: 应用CPR后，提示在无外部知识的情况下胜率超过90%，显著减少幻觉

Conclusion: CPR框架能有效缓解由不良提示引起的幻觉问题，提高LLM生成质量
...
### [105] [Multi-stage Prompt Refinement for Mitigating Hallucinations in Large Language Models](https://arxiv.org/abs/2510.12032)
*Jung-Woo Shim,Yeong-Joon Ju,Ji-Hoon Park,Seong-Whan Lee*

Main category: cs.CL

TL;DR: 提出了多阶段提示词优化框架MPR，通过多阶段迭代优化存在语法错误、歧义或不完整信息的提示词，显著减少大语言模型的幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在幻觉问题上仍面临挑战，而提示词质量不佳（如歧义、语法错误、信息不完整）对幻觉的影响尚未充分研究。

Method: MPR框架采用多阶段方法，使用专门微调的小语言模型逐步修正提示词中的标点、拼写错误和关键词误用，并通过自反思机制对优化结果进行排序。

Result: 在幻觉基准测试中，经MPR优化的提示词相比原始提示词获得了超过85%的胜率，且能与现有后处理幻觉缓解框架结合使用。

Conclusion: MPR提供了一个轻量级、适应性强的解决方案，可有效提升大语言模型在不同领域的可靠性。
...
### [106] [On the Interplay between Human Label Variation and Model Fairness](https://arxiv.org/abs/2510.12036)
...
<details>
  <summary>Details</summary>
Motivation: 研究人类标签变异对模型公平性的影响，这是一个尚未探索的话题

Method: 通过比较在多数投票标签上训练与一系列HLV方法的训练效果

Result: 实验表明，在没有显式去偏的情况下，HLV训练方法对公平性有积极影响

Conclusion: HLV训练方法可以在不进行显式去偏的情况下提升模型公平性
...
### [107] [Uncertainty Quantification for Hallucination Detection in Large Language Models: Foundations, Methodology, and Future Directions](https://arxiv.org/abs/2510.12040)
*Sungmin Kang,Yavuz Faruk Bakman,Duygu Nur Yaldiz,Baturalp Buyukates,Salman Avestimehr*

Main category: cs.CL

TL;DR: 本文综述了大语言模型中的不确定性量化方法，重点关注其在幻觉检测中的应用，系统分类现有方法并讨论未来研究方向。
...
Method: 从不确定性量化的理论基础出发，分析其在LLM中的适配，系统分类现有幻觉检测方法，并展示代表性方法的实证结果。

Result: 不确定性量化为识别不可靠生成提供了机制，能够有效改善LLM的可靠性，但现有方法仍存在局限性。

Conclusion: 不确定性量化是解决LLM幻觉问题的关键研究方向，需要进一步探索更有效的方法来提升模型的可信度评估。
...
### [108] [Improving Text-to-Image Generation with Input-Side Inference-Time Scaling](https://arxiv.org/abs/2510.12041)
*Ruibo Chen,Jiacheng Pan,Heng Huang,Zhenheng Yang*

Main category: cs.CL

TL;DR: 提出一种基于大语言模型的提示词重写框架，通过奖励系统和迭代DPO训练来优化文本到图像生成中的提示词，无需监督微调数据即可提升图像质量和对齐度。
...
Method: 使用大语言模型作为提示词重写器，引入精心设计的奖励系统和迭代直接偏好优化训练流程，在无需监督数据的情况下优化用户输入的提示词。

Result: 该方法在多个文本到图像模型和基准测试中一致提升了图像-文本对齐度、视觉质量和美学效果，优于强基线方法，且具有良好的跨模型迁移能力。

Conclusion: 提示词重写是一种有效、可扩展且实用的模型无关策略，能够显著改进文本到图像生成系统，具有很好的泛化性和可扩展性。
...
### [109] [Hierarchical Alignment: Surgical Fine-Tuning via Functional Layer Specialization in Large Language Models](https://arxiv.org/abs/2510.12044)
*Yukun Zhang,Qi Dong*

Main category: cs.CL

TL;DR: 提出分层对齐方法，针对Transformer架构中不同功能层进行定向优化，避免传统DPO的"对齐税"问题，在语法流畅性、逻辑一致性和事实准确性方面均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法将模型视为整体进行统一优化，忽略了Transformer架构中不同层处理语法、逻辑和事实等不同任务的功能专门化特性。

Method: 分层对齐方法，将模型层分为局部（语法）、中间（逻辑）和全局（事实性）三个功能块，使用LoRA进行精准微调，对每个功能块分别应用DPO优化。

Result: 局部层对齐提升语法流畅性，全局层对齐不仅改善事实一致性，还是提升逻辑一致性的最有效策略，所有分层策略都成功避免了标准DPO中的"对齐税"问题。

Conclusion: 分层对齐方法为模型对齐提供了更资源高效、可控和可解释的路径，展示了从整体优化转向结构感知精准微调的巨大潜力。
...
### [110] [APCE: Adaptive Progressive Context Expansion for Long Context Processing](https://arxiv.org/abs/2510.12051)
*Baisub Lee,Sanghyun Byun,Mohanad Odema,Jung Guack,Jacob Song,Woo Seong Chung*

Main category: cs.CL

TL;DR: 提出APCE方法，通过语义相似度匹配选择重要输入块，在长上下文摘要任务中减少50%-70%输入序列，同时保持或提升性能并改善内存效率。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文Transformer模型的两个关键挑战：因二次自注意力和线性KV缓存导致的内存占用增长，以及随着上下文长度增加出现的性能退化(ContextRot)现象。

Method: APCE方法通过低维语义相似度匹配，选择与当前查询最相关的输入块进行处理，直接操作输入而非依赖特定硬件环境。

Result: 实验表明APCE在仅使用50%-70%输入序列的情况下，实现了优于或与完整密集基线相当的摘要性能，显著改善了KV缓存和自注意力的内存效率。

Conclusion: APCE为长上下文Transformer模型提供了一种上下文感知的效率解决方案，有望推广到其他相关长上下文任务中。
...
### [111] [An AI-Based Behavioral Health Safety Filter and Dataset for Identifying Mental Health Crises in Text-Based Conversations](https://arxiv.org/abs/2510.12083)
...
TL;DR: 该研究评估了Verily行为健康安全过滤器(VBHSF)在精神健康危机检测中的表现，发现其在两个数据集上都表现出高敏感性和特异性，优于NVIDIA NeMo和OpenAI Omni Moderation Latest防护系统。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理精神健康紧急情况时经常提供有害或不恰当的建议，需要开发有效的安全过滤器来防止这类问题。

Method: 在两个临床标注的数据集上评估VBHSF性能：Verily精神健康危机数据集(1,800条模拟消息)和NVIDIA Aegis AI内容安全数据集子集(794条精神健康相关消息)，并与NVIDIA NeMo和OpenAI Omni Moderation Latest进行对比分析。

Result: VBHSF在Verily数据集上表现出高敏感性(0.990)和特异性(0.992)，F1分数0.939；在NVIDIA数据集上敏感性0.982，准确性0.921，特异性0.859。在所有比较中都显著优于其他防护系统。
...
### [112] [Deep Associations, High Creativity: A Simple yet Effective Metric for Evaluating Large Language Models](https://arxiv.org/abs/2510.12110)
*Ziliang Qiu,Renfen Hu*

Main category: cs.CL

TL;DR: 提出PACE方法，通过让LLMs生成平行关联链来评估其创造力，该方法能减少数据污染风险并高效评估，与人工评估有强相关性。


<details>
  <summary>Details</summary>
Motivation: LLMs创造力评估面临数据污染和人工评估成本高的问题，受人类创造力评估启发，需要开发更有效的评估方法。

Method: PACE方法，要求LLMs生成平行关联链来评估其创造力，该方法简单高效且能最小化数据污染风险。

Result: PACE与Chatbot Arena Creative Writing排名有强相关性（Spearman's ρ=0.739），高性能LLMs得分接近普通人类水平，但专业人士仍优于LLMs；语言分析显示人类和LLMs关联具体性都下降，但人类关联模式更多样。

Conclusion: PACE是评估LLMs创造力的有效方法，LLMs在关联创造力方面接近普通人类水平，但专业人士仍具优势，人类关联模式更丰富多样。
...
### [113] [Tracing Multilingual Knowledge Acquisition Dynamics in Domain Adaptation: A Case Study of English-Japanese Biomedical Adaptation](https://arxiv.org/abs/2510.12115)
*Xin Zhao,Naoki Yoshinaga,Yuma Tsuta,Akiko Aizawa*

Main category: cs.CL

TL;DR: 本文研究了多语言领域适应(ML-DA)中LLMs的学习动态，提出了AdaXEval评估方法，发现即使使用高质量双语语料，跨语言知识迁移仍然具有挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有ML-DA研究在训练和评估时使用知识覆盖不匹配的数据集，导致多语言知识获取机制不明确，特别是在低资源环境下性能不佳。

Method: 提出AdaXEval自适应评估方法，从训练用的双语领域语料构建选择题QA数据集；通过不同数据配方的持续训练，追踪LLMs获取领域事实的过程。

Result: 在13B英日双语LLM上的实验表明，跨语言迁移仍然具有挑战性，即使使用高质量双语语料。

Conclusion: 需要更深入地理解多语言知识获取机制，以改善跨语言领域适应效果。
...
### [114] [Understanding the Modality Gap: An Empirical Study on the Speech-Text Alignment Mechanism of Large Speech Language Models](https://arxiv.org/abs/2510.12116)
*Bajian Xiang,Shuaijiang Zhao,Tingwei Guo,Wei Zou*

Main category: cs.CL

TL;DR: 本文系统分析了大型语音语言模型中的模态差距问题，发现语音和文本输入在深层表示中方向对齐但幅度分离，并提出基于对齐路径评分和关键令牌干预的改进方法。
...
Method: 通过系统实验分析语音和文本表示的粗粒度和细粒度特征，引入对齐路径评分量化令牌级对齐质量，并设计角度投影和长度归一化等针对性干预策略。
...
### [115] [SafeMT: Multi-turn Safety for Multimodal Language Models](https://arxiv.org/abs/2510.12133)
...
Result: 测试17个模型发现，有害对话轮数增加会显著提升攻击成功率；提出的安全调节器相比现有防护模型能更有效降低多轮攻击成功率。

Conclusion: 多模态大语言模型在多轮对话中的安全机制不足，需要专门的安全调节器来识别和防范对话中的恶意攻击。
...
### [116] [Credal Transformer: A Principled Approach for Quantifying and Mitigating Hallucinations in Large Language Models](https://arxiv.org/abs/2510.12137)
*Shihao Ji,Zihui Song,Jiajie Huang*

Main category: cs.CL

TL;DR: 提出Credal Transformer来解决LLM幻觉问题，通过Credal Attention Mechanism产生credal set来量化不确定性，显著减少自信错误
...
Method: 用基于证据理论的Credal Attention Mechanism替代标准注意力机制，将注意力分数重新概念化为Dirichlet分布的证据质量，产生credal set（分布集合）而非单一注意力向量

Result: Credal Transformer能够识别分布外输入、量化模糊性，并在不可回答问题上通过弃权显著减少自信错误

Conclusion: 提出了一种新的架构来减轻幻觉，并提供了一个将不确定性量化直接集成到模型中的设计范式，为更可靠的AI奠定基础
...
### [117] [A Survey on Parallel Reasoning](https://arxiv.org/abs/2510.12164)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，并行推理作为一种增强推理鲁棒性的新范式出现，旨在克服传统顺序推理方法的脆弱性并提升实际性能。

Method: 首先给出并行推理的形式化定义，澄清其与相关概念的区别；然后基于新颖的分类法组织讨论先进技术，包括非交互式推理、交互式推理和效率导向的解码策略；最后探索各种应用场景。

Result: 提供了并行推理的全面综述，建立了清晰的概念框架和技术分类，识别了核心挑战并提出了未来研究方向。

Conclusion: 这项工作为初学者提供了有用的路线图，鼓励更多研究来改进并行推理方法，相关资源可在GitHub仓库获取。
...
### [118] [Towards Inference-time Scaling for Continuous Space Reasoning](https://arxiv.org/abs/2510.12167)
*Minghan Wang,Thuy-Trang Vu,Ehsan Shareghi,Gholamreza Haffari*

Main category: cs.CL

TL;DR: 本文研究了将离散推理空间的推理技术（如多样本生成和奖励模型重排序）应用于连续推理空间的可行性，发现虽然能生成多样推理路径，但难以有效区分正确与错误推理，需要引入归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 探索已证明在离散空间有效的推理技术（多样本生成+奖励模型重排序）是否能在连续推理空间中成功应用，以提升连续推理语言模型的性能。

Method: 使用COCONUT连续推理LM作为骨干，通过dropout-based采样生成多样推理路径，分析Pass@N性能，并探究几何特性和轨迹动态等来理解连续推理空间的独特挑战。

Result: 在连续推理空间中生成多样推理路径是可行的，但离散空间的数据生成和奖励模型训练方法在连续空间中只能带来边际改进，难以有效区分正确与错误推理。

Conclusion: 连续推理LM的训练框架不仅需要优化准确性，还应明确引入可在推理时用于区分正确与错误推理的归纳偏置，以克服当前限制。
...
### [119] [From Knowledge to Treatment: Large Language Model Assisted Biomedical Concept Representation for Drug Repurposing](https://arxiv.org/abs/2510.12181)
*Chengrui Xiang,Tengfei Ma,Xiangzheng Fu,Yiping Liu,Bosheng Song,Xiangxiang Zeng*

Main category: cs.CL

TL;DR: LLaDR是一个基于大语言模型的药物重定位框架，通过从LLM提取语义丰富的生物医学实体文本表示来增强知识图谱嵌入，提升对复杂疾病的药物重定位效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了真实实验室中的常识性生物医学概念知识，如某些药物与特定治疗根本不相容的机制先验知识，导致生物医学知识图谱表示不充分。

Method: 从大语言模型中提取生物医学实体的语义丰富文本表示，用于微调知识图谱嵌入模型，将治疗相关知识注入KGE中。

Result: 在基准测试中达到最先进性能，阿尔茨海默病案例研究进一步证实了其鲁棒性和有效性。

Conclusion: LLaDR通过注入治疗相关知识显著改善了生物医学概念表示，增强了对研究不足或复杂适应症的语义理解。
...
### [120] [Not in Sync: Unveiling Temporal Bias in Audio Chat Models](https://arxiv.org/abs/2510.12185)
*Jiayu Yao,Shenghua Liu,Yiwei Wang,Rundong Cheng,Lingrui Mei,Baolong Bi,Zhen Xiong,Xueqi Cheng*

Main category: cs.CL

TL;DR: 本文首次系统研究大型音频语言模型中的时间偏差问题，发现模型在预测事件时间戳时存在系统性偏差，这种偏差随音频长度增加而累积。
...
Method: 通过在带时间戳的数据集上进行受控实验，量化时间偏差现象，并引入时间偏差指数(TBI)来测量预测事件时间与真实时间的系统性偏差。

Result: 研究发现：(i)时间偏差在不同数据集和模型中普遍存在；(ii)偏差随音频长度增加而累积，在长录音中可达数十秒；(iii)偏差程度因事件类型和位置而异。
...
TL;DR: 提出基于大语言模型和直接偏好优化的语音翻译分割框架，通过偏好对齐提升分割准确性，在多个语言对上优于现有方法SHAS，同时改善翻译质量和延迟。


<details>
  <summary>Details</summary>
Motivation: 现有分割模型如SHAS虽然比启发式方法更鲁棒，但受限于监督学习目标，缺乏人类偏好对齐，而这对实时自然口译至关重要。

Method: 使用直接偏好优化训练大语言模型进行分割预测，结合SeamlessM4T v2作为翻译骨干，在ACL 60/60语料库上评估。

Result: 在英-日、中、德语三个语言对上，DPO调优的LLM比SHAS获得更高的分割准确率，翻译质量(BLEU、COMET)和延迟(Average Lagging)均有提升。
...
TL;DR: 提出了HALF框架，用于评估LLM在真实应用场景中的偏见，并按危害严重性加权结果。


<details>
  <summary>Details</summary>
Motivation: 现有评估缺乏真实场景基础，且未考虑不同领域危害严重性的差异。

Method: 使用五阶段流水线将九个应用领域分为三个等级（严重、中等、轻度），评估八个LLM的公平性。

Result: LLM在不同领域公平性不一致；模型大小或性能不能保证公平性；推理模型在医疗决策支持中表现更好但在教育中更差。
...
TL;DR: 研究发现LLMs在微调过程中会学习到Knobe效应这种道德偏见，通过层修补分析可以定位到特定层，仅需在关键层修补预训练模型的激活就能消除这种偏见。


<details>
  <summary>Details</summary>
Motivation: 了解LLMs在微调过程中如何内化人类偏见，特别是Knobe效应这种道德判断偏见，并探索其机制。

Method: 在3个开源权重LLMs上进行层修补分析，定位偏见所在的特定层，并尝试用预训练模型的激活修补这些层。

Result: 偏见不仅在学习过程中形成，而且集中在特定层中。仅需在少数关键层修补预训练激活就能有效消除Knobe效应。

Conclusion: LLMs中的社会偏见可以被解释、定位和通过针对性干预来缓解，无需重新训练模型。
...
TL;DR: 提出了DSAS方法解决LLMs在多文档问答中的长距离依赖建模和"中间信息丢失"问题，无需架构修改或额外训练参数，在多个基准测试中平均F1分数提升4.2%。


<details>
  <summary>Details</summary>
Motivation: LLMs在多文档问答任务中存在两个主要限制：长距离依赖建模困难，难以关注长文本中的关键信息；以及"中间信息丢失"问题，难以处理长输入中间位置的信息。现有解决方案要么截断全局依赖，要么需要昂贵的微调。

Method: 提出了双阶段自适应锐化(DSAS)方法，包含两个模块：上下文门控加权(CGW)模块通过层级注意力跟踪和位置感知加权来缓解"中间信息丢失"问题；互斥注意力抑制(RAS)模块通过抑制关键文本与无关文本之间的信息交换来增强对关键段落的关注。
...
Conclusion: DSAS作为一个即插即用的解决方案，无需架构修改或额外训练参数，有效解决了LLMs在多文档问答中的关键限制，具有良好的鲁棒性和可扩展性。
...
TL;DR: MedQA-Followup框架评估医疗LLMs在多轮对话中的鲁棒性，发现模型在深层鲁棒性方面存在严重漏洞，间接干预比直接建议更具破坏性。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架主要针对单轮问答，忽视了医疗咨询中常见的多轮交互复杂性，如冲突输入、误导性上下文和权威影响。

Method: 在MedQA数据集上使用受控干预，区分浅层鲁棒性（抵抗误导初始上下文）和深层鲁棒性（在跨轮次挑战时保持准确性），并引入间接-直接轴来分离上下文框架和明确建议。

Result: 模型在浅层扰动下表现良好，但在多轮设置中表现出严重脆弱性，Claude Sonnet 4的准确率从91.2%降至13.5%。间接干预比直接建议造成更大的准确率下降。
...
TL;DR: Chinese ModernBERT是一个从头训练的中文编码器，通过优化词汇表、训练策略和架构设计，在中文NLP任务上实现了性能提升和效率优化。


<details>
  <summary>Details</summary>
Motivation: 现有的编码器改进主要针对英语，未能充分考虑中文在分词和词法上的差异，需要专门为中文设计优化的预训练模型。

Method: 使用32k BPE词汇表、动态全词掩码、两阶段预训练（扩展上下文至8192 tokens）、阻尼余弦学习率调度，并在1.2T中文token上预训练。

Result: 在CLUE基准测试中与强中文编码器竞争，在SimCLUE上通过额外对比数据达到0.505/0.537的相关系数，超越Qwen-0.6B-embedding。

Conclusion: Chinese ModernBERT展示了为中文专门设计模型的有效性，并为语义文本相似度任务提供了清晰的扩展路径。
...
Result: 在60小时内标注了COHA语料库中的143,933个句子，在两个复杂标注任务上达到98%+的准确率。
...
TL;DR: 提出基于检索增强生成(RAG)的知识驱动反言论生成框架，通过整合联合国数字图书馆等权威知识库，为8个目标群体生成可信的反仇恨言论内容。


<details>
  <summary>Details</summary>
Motivation: 现有反言论生成方法存在可靠性不足和可扩展性差的问题，需要构建可信且连贯的反言论生成系统。

Method: 构建包含32,792个文本的知识库，集成RAG管道，将反言论生成建模为知识驱动的文本生成过程。

Result: 在MultiTarget-CONAN数据集上评估，该框架在自动评估和人工评估中均优于标准LLM基线方法。

Conclusion: 该框架为研究可信和合理的反言论生成铺平了道路，适用于仇恨言论及其他领域。
...
TL;DR: 本文开发了一种细粒度输入归因方法，发现大脑-LLM对齐和下一个词预测依赖不同的词汇子集：NWP具有近因和首因偏见且关注语法，而BA优先考虑语义和语篇信息并具有更针对性的近因效应。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型与人类大脑活动之间的对齐可以揭示语言处理的计算原理，并研究大脑对齐与下一个词预测之间的关系这一有争议的研究问题。

Method: 引入细粒度输入归因方法来识别对大脑-LLM对齐最重要的具体词汇。

Result: 发现BA和NWP依赖大部分不同的词汇子集：NWP表现出近因和首因偏见，主要关注语法；而BA优先考虑语义和语篇层面的信息，具有更针对性的近因效应。

Conclusion: 这项工作推进了我们对LLMs与人类语言处理关系的理解，并强调了BA和NWP在特征依赖方面的差异。归因方法可广泛应用于探索模型预测在各种语言处理任务中的认知相关性。
...
TL;DR: MoBiLE是一个即插即用的MoE推理框架，通过混合大小专家机制减少不重要token的专家数量来加速推理，同时保持重要token的完整专家以保障模型质量。


<details>
  <summary>Details</summary>
Motivation: 现有MoE模型卸载策略受限于CPU-GPU互联带宽，预取方法存在训练开销大且在细粒度专家分割的现代MoE模型中效果不佳的问题。

Method: 提出混合大小专家机制，为不重要token使用一半专家加速，为重要token保持完整专家；设计专用的回退和预取机制在大小专家间切换以提高内存效率。

Result: 在四种典型现代MoE架构和生成任务上评估，MoBiLE在消费级GPU系统上相比基线实现1.60x到1.72x的加速，精度损失可忽略不计。

Conclusion: MoBiLE框架有效解决了MoE推理中的带宽瓶颈问题，在保持模型质量的同时显著提升推理速度。
...
TL;DR: 该研究通过模拟实验发现，LLM作为审稿人存在系统性偏见：偏爱LLM生成的论文风格，对包含批判性陈述的人类论文评分偏低，这给学术公平带来风险。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在学术工作流程中的深度整合，需要探索其在研究和审稿双重角色中可能带来的新风险，特别是对学术公平性的影响。

Method: 通过模拟实验，构建研究代理（生成和修改论文）和审稿代理（评估提交的论文），然后进行人工标注分析。

Result: 发现LLM审稿人与人类判断存在显著偏差：系统性地给LLM撰写的论文打高分，对包含批判性陈述的人类论文持续低分评价。

Conclusion: LLM在同行评审中的部署需要谨慎，以避免对学术公平性的威胁，但同时LLM审稿指导的修改确实能提升论文质量，对早期研究人员有益。
...
TL;DR: 本研究对200多种语言进行大规模跨语言分词效率评估，发现拉丁文字语言分词效率高，而非拉丁文字和形态复杂语言的分词成本高出3-5倍，导致计算资源分配不均。


<details>
  <summary>Details</summary>
Motivation: 解决分词差异对语言多样性群体在人工智能访问公平性方面的障碍，量化大型语言模型中的计算不平等问题。

Method: 使用标准化实验框架，通过tiktoken库对200多种语言样本进行统一分词处理，收集TPS和RTC等评估指标，并与英语基准进行比较。

Result: 拉丁文字语言分词效率高，非拉丁文字和形态复杂语言分词成本显著增加，RTC比率通常高出3-5倍，导致计算成本增加和有效上下文利用减少。

Conclusion: 当前AI系统存在结构性不平等，低资源和非拉丁语言使用者面临不成比例的计算劣势，未来应开发考虑语言类型多样性的分词策略和自适应词汇构建方法。
...
TL;DR: PRoH是一个动态规划和推理框架，通过上下文感知规划、结构化问题分解和实体加权重叠引导的推理路径检索，解决了知识超图检索增强生成中的静态规划、非自适应检索和结构利用不足等问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识超图的RAG方法存在三个主要限制：静态检索规划、非自适应检索执行以及对KH结构和语义的浅层使用，这些限制了它们在多跳问答中的有效性。

Method: PRoH包含三个核心创新：(1)上下文感知规划模块，通过勾画局部KH邻域来指导结构化的推理计划生成；(2)结构化问题分解过程，将子问题组织为动态演化的有向无环图，实现自适应多轨迹探索；(3)实体加权重叠引导的推理路径检索算法，优先考虑语义连贯的超边遍历。

Result: 在多个领域的实验中，PRoH实现了最先进的性能，平均F1分数比之前的SOTA模型HyperGraphRAG提高了19.73%，生成评估分数提高了8.41%，同时在长距离多跳推理任务中保持了强大的鲁棒性。

Conclusion: PRoH通过动态规划和推理机制有效克服了现有KH-based RAG方法的局限性，在多跳问答任务中表现出卓越的性能和鲁棒性。
...
TL;DR: CLEAR框架通过分析LLM内部隐藏状态，定位知识冲突并引入冲突感知微调，显著提升了RAG系统的准确性和上下文忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在不忠实问题，模型响应与检索证据相矛盾。现有方法将LLM视为黑盒，忽视了LLM如何内部整合检索证据与参数记忆，特别是在知识冲突情况下。

Method: 提出CLEAR框架：1) 将上下文分解为细粒度句子级知识；2) 使用隐藏状态探测定位冲突知识；3) 引入冲突感知微调指导模型准确整合检索证据。

Result: 在三个基准测试上的广泛实验表明，CLEAR显著提高了准确性和上下文忠实度，在各种冲突条件下持续优于强基线方法。

Conclusion: CLEAR通过分析LLM内部表示和冲突定位，有效解决了RAG系统中的忠实度问题，为知识整合提供了新的解决方案。
...
TL;DR: 该研究通过多语言Wug测试比较了6个大型语言模型在4种语言（加泰罗尼亚语、英语、希腊语、西班牙语）上的形态学泛化能力，发现模型准确率更受训练数据量而非语言复杂度影响。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型的真实语言能力，特别是它们能否像人类一样对陌生词汇进行形态学泛化，以及这种能力是受语言结构复杂度还是训练数据量驱动。

Method: 使用多语言Wug测试，在4种语言上测试6个模型，并与人类表现进行比较，分析模型准确率与语言社区规模、数据可用性的关系。

Result: 模型能够以接近人类的准确率对未见词汇进行形态学泛化，但准确率模式更符合社区规模和数据可用性，而非结构复杂度。西班牙语和英语（资源丰富）表现优于加泰罗尼亚语和希腊语（资源较少）。

Conclusion: 模型行为主要由语言资源丰富度驱动，而非对语法复杂度的敏感性，其表现仅表面类似人类语言能力。
...
TL;DR: 提出SMEC训练框架，通过序列嵌套表示学习、自适应维度选择和可选择性跨批次记忆模块，在保持性能的同时显著降低LLM嵌入的维度。
...
Method: 提出SMEC框架，包含SMRL方法减少训练梯度方差，ADS模块降低维度剪枝信息退化，S-XBM模块增强高低维嵌入间的无监督学习。

Result: 在图像、文本和多模态数据集上实验表明，SMEC实现显著维度压缩同时保持性能。在BEIR数据集上，256维压缩嵌入性能比Matryoshka-Adaptor和Search-Adaptor分别提升1.1和2.7分。
...
Method: 构建个性化文本检测基准，提出基于特征反转陷阱的检测器性能预测方法，通过识别潜在特征方向来评估检测器依赖。

Result: 实验显示个性化设置下检测器性能存在显著差距，某些最先进模型性能大幅下降，提出的预测方法与实际性能差距相关性达85%。

Conclusion: 这项工作揭示了个性化文本检测的挑战，为未来研究提供了基准和方法支持。
...
TL;DR: 该论文将测试时缩放技术从数学和编程领域转移到LeWiDi-2025任务中，用于评估标注分歧。实验发现基准算法能提升LLM性能，但Best-of-N方法无效。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放技术此前仅限于有明确正确答案的领域（如数学、编程），本研究旨在将其扩展到存在标注分歧的LeWiDi任务中。

Method: 使用三种测试时缩放方法：模型平均、多数投票和Best-of-N采样，在LeWiDi-2025任务上进行实验。

Result: 模型平均和多数投票方法能一致提升LLM在LeWiDi任务上的性能，但Best-of-N方法未能产生改进。
...
TL;DR: 本文研究了视觉语言模型在处理典型和异常图像时的权衡问题，发现当图像与文本不一致时，模型的概念理解能力会下降。


<details>
  <summary>Details</summary>
Motivation: 研究VLMs在处理典型和异常实例时如何平衡语用先验（输入相关）和语义先验（概念一般性）之间的冲突。
...
TL;DR: 提出Faithful Uncertainty Tuning (FUT)方法，通过微调让LLMs能够忠实地表达不确定性，而不改变其答案分布。


<details>
  <summary>Details</summary>
Motivation: LLMs经常错误传达其不确定性：重复查询会产生不同答案，但生成的响应通常缺乏或使用不反映这种变异性的不确定性修饰语，这造成了忠实性差距。

Method: 通过增强模型样本与不确定性修饰语（如'可能'、'很可能'）来构建训练数据，这些修饰语与样本一致性对齐，无需额外监督。

Result: 在开放域问答任务上的评估显示，FUT显著减少了忠实性差距，同时保持了问答准确性并引入了最小的语义分布偏移。

Conclusion: FUT是一种简单有效的方法，可以教导LLMs忠实地传达不确定性。
...
TL;DR: StyleDecipher是一个可解释的机器生成文本检测框架，通过结合离散风格指标和连续风格表示来量化人类与LLM生成文本的风格差异，在多个领域实现最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法存在泛化能力有限、易受改写攻击、缺乏可解释性等问题，特别是在面对风格多样性或人机混合创作时表现不佳。

Method: 使用组合特征提取器联合建模离散风格指标和基于语义嵌入的连续风格表示，在统一表示空间中捕捉人类与LLM输出的风格差异，无需访问模型内部或标注片段。

Result: 在五个不同领域的实验中，StyleDecipher在领域内检测准确率持续达到最优，跨领域评估中比现有基线提升高达36.30%，对对抗性扰动和混合内容保持鲁棒性。
...
TL;DR: ACADATA是一个高质量的学术翻译平行数据集，包含150万段落对的训练集和6000翻译的评估集，覆盖96个语言方向。通过在ACAD-TRAIN上微调LLM，在学术翻译质量上显著提升，超越专有和开源模型。


<details>
  <summary>Details</summary>
Motivation: 解决学术翻译领域缺乏高质量平行数据集的问题，为学术领域和长上下文翻译研究提供资源。

Method: 构建ACADATA数据集（ACAD-TRAIN训练集和ACAD-BENCH评估集），在ACAD-TRAIN上微调大语言模型，并与专业机器翻译系统、通用LLM和专有模型进行基准测试。

Result: 微调后7B和2B模型在学术翻译质量上分别平均提升+6.1和+12.4 d-BLEU点，从英语翻译时在通用领域的长上下文翻译提升达24.9%。表现最佳的微调模型在学术翻译领域超越了最好的专有和开源模型。

Conclusion: ACADATA数据集和微调模型为学术领域和长上下文翻译研究提供了宝贵资源，证明了在专业数据集上微调LLM能显著提升特定领域的翻译质量。
...
TL;DR: COSTAR-A是一种改进的提示工程框架，在原有COSTAR方法基础上增加了'答案'组件，能够提升小型本地优化LLM的输出结构和决策性。


<details>
  <summary>Details</summary>
Motivation: 现有COSTAR框架在大语言模型上表现良好，但在小型本地优化模型上性能不稳定，特别是在需要更多指导或约束输出的任务中。

Method: 通过在COSTAR框架末尾添加'答案'组件，形成COSTAR-A框架，并在最多80亿参数的小型微调模型上进行受控提示-输出评估。

Result: COSTAR-A能够提升本地化LLM的输出结构和决策性，但效果因模型和用例而异。Llama 3.1-8B模型使用COSTAR-A时表现优于单独使用COSTAR。
...
TL;DR: 本文提出PARO框架，通过让LLMs学习任务特定的推理模式来自动生成推理轨迹，无需人工标注，在模式化推理任务中可达到与10倍规模人工标注相当的SFT+RLVR性能。


<details>
  <summary>Details</summary>
Motivation: 当前SFT+RLVR范式依赖昂贵的人工标注推理轨迹，本文旨在研究如何在模式化推理任务中大幅降低标注成本而不影响推理性能。

Method: 提出PARO框架，让LLMs学习任务特定的推理模式，自动生成与模式对齐的推理轨迹，无需人工标注完整推理过程。

Result: PARO生成的推理轨迹在SFT+RLVR中达到与10倍规模人工标注相当的性能，验证了推理模式而非标注数量是性能关键因素。

Conclusion: 在模式化推理任务中，大规模人工标注可被只需有限人类监督的LLM自动标注所替代，推理模式是性能的决定性因素。
...
TL;DR: 本文提出了生成空间大小(GSS)的概念来解决LLMs在创造性任务中输出过于同质化、在事实性任务中产生多样化但错误回答的问题，并开发了GSSBench评估套件来评估不同指标。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在开放生成任务中存在校准问题：在创造性任务中输出过于同质化，在事实性任务中产生多样化但错误的回答。这两种失败模式都可以通过生成空间大小(GSS)的概念来统一解决。

Method: 提出了GSSBench任务套件，包含具有真实GSS关系的提示对，用于评估不同指标。研究发现幻觉检测指标（特别是EigenScore）优于标准多样性和不确定性量化指标。

Result: EigenScore等幻觉检测指标在评估模型内部任务表示方面表现一致优于标准指标，且仅使用模型内部信息就能提供可解释的见解。

Conclusion: GSS概念有三个应用：检测提示歧义并预测澄清问题、解释推理模型中的过度思考和思考不足、引导模型扩展生成空间以获得高质量多样化输出。
...
TL;DR: 该研究扩展了语言模型对类型学常见语法偏好的研究，采用广义范畴语法构建更接近自然语言的人工语言，并关注模型对未见长句的泛化能力。
...
Conclusion: 人工语言更好地捕捉了自然语言特征，实验范式得出更清晰结论：类型学合理的词序更易于语言模型泛化。
...
TL;DR: 本文主张从经验主义角度重新审视语言模型，挑战基于索绪尔和乔姆斯基理论的语言学批评，提出以使用频率为核心的语言定义框架。


<details>
  <summary>Details</summary>
Motivation: 当前基于索绪尔和乔姆斯基理论的语言学评论对LLMs的批评往往是推测性和非建设性的，需要更实用的语言学框架来指导语言模型的设计和评估。

Method: 采用Witold Mańczak的经验主义语言学框架，将语言定义为所有口头和书面表达的总和，强调语言元素使用频率的核心地位。

Result: 基于Mańczak框架挑战了先前对LLMs的批评，并为语言模型的设计、评估和解释提供了建设性指南。

Conclusion: 经验主义语言学框架为理解LLMs提供了更实用和富有成效的视角，强调语言使用频率的重要性，有助于推动语言模型的发展。
...
TL;DR: Dr.LLM 是一个动态路由框架，通过轻量级路由器和MCTS训练，让预训练LLM能够跳过、执行或重复层，在保持或提升精度的同时节省计算。


<details>
  <summary>Details</summary>
Motivation: 传统LLM对所有token都经过所有层，导致简单查询浪费计算，复杂查询推理深度不足。现有自适应深度方法依赖昂贵推理搜索或架构修改，且往往牺牲精度。

Method: 使用轻量级每层路由器，通过MCTS训练获得高质量层配置，采用窗口池化稳定路由、焦点损失与类别平衡、瓶颈MLP路由器等技术。

Result: 在ARC和DART上精度提升达+3.4%，平均每样本节省5层；在跨域任务上仅损失0.85%精度，比现有路由方法提升达+7.7%。

Conclusion: Dr.LLM证明显式监督的路由器可在不改变基础权重的情况下，为冻结LLM提供预算感知、精度驱动的推理能力。
...
TL;DR: 该研究分析了为低资源语言（特别是班巴拉语）创建高质量语音标注数据集所需的人力成本，发现在实验室条件下转录1小时语音需要30小时人工，在实地条件下需要36小时。


<details>
  <summary>Details</summary>
Motivation: 创建低资源语言的语音数据集是一个关键但研究不足的挑战，特别是关于实际人力成本的问题。研究旨在了解为低识字率的口语语言生产高质量标注语音数据所需的时间和复杂性。

Method: 通过为期一个月的实地研究，涉及10名母语转录员，分析了对53小时班巴拉语语音数据的ASR生成转录的校正过程。
...
TL;DR: 本研究系统比较了基于规则、搜索和学习的AI智能体在Dhumbal纸牌游戏中的表现，发现基于规则的激进策略智能体在胜率上显著优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 评估AI智能体在具有文化意义的不完全信息多人纸牌游戏Dhumbal中的表现，为文化游戏的数字保存提供支持。

Method: 实现多种AI策略：基于规则的启发式方法（激进、保守、平衡、机会主义）、基于搜索的方法（MCTS、ISMCTS）和强化学习方法（DQN、PPO），通过锦标赛进行评估。

Result: 在1024轮模拟中，基于规则的激进智能体获得最高胜率（88.3%），显著优于ISMCTS（9.0%）和PPO（1.5%），通过有效利用Jhyap声明获得优势。

Conclusion: 研究为文化游戏提供了可复现的AI框架，展示了启发式方法在不完全信息环境下的有效性，并开源代码支持AI研究和文化游戏保护。
...
### [152] [Tensor Logic: The Language of AI](https://arxiv.org/abs/2510.12269)
*Pedro Domingos*

Main category: cs.AI

TL;DR: 提出张量逻辑语言，统一神经AI和符号AI，通过张量方程实现逻辑规则和爱因斯坦求和的统一，解决现有AI编程语言的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有AI编程语言存在缺陷：PyTorch和TensorFlow缺乏自动推理和知识获取支持，而LISP和Prolog缺乏可扩展性和学习支持，阻碍了AI发展。

Method: 设计张量逻辑语言，以张量方程为唯一构造，将逻辑规则和爱因斯坦求和视为相同操作，实现神经、符号和统计AI的统一表达。

Result: 优雅实现了包括transformer、形式推理、核机器和图模型在内的多种AI形式，并支持嵌入空间中的可靠推理。
...
### [153] [Beyond Consensus: Mitigating the Agreeableness Bias in LLM Judge Evaluations](https://arxiv.org/abs/2510.11822)
*Suryaansh Jain,Umair Z. Ahmed,Shubham Sahai,Ben Leong*

Main category: cs.AI

TL;DR: 该论文发现LLM作为评估器存在严重的正向偏见，提出少数否决策略和基于回归的框架来缓解偏见，在代码反馈任务上显著提升了评估准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估器存在强烈的正向偏见，虽然能准确识别有效输出，但难以识别无效输出，导致可靠性评分虚高，需要更有效的偏见缓解方法。

Method: 提出最优少数否决策略和基于回归的框架，利用少量人工标注数据直接建模验证器偏见，在代码反馈任务上进行验证。

Result: 在366个高中Python程序的代码反馈任务中，回归方法将最大绝对误差降至1.2%，比14个最先进LLM的最佳集成性能提升2倍。

Conclusion: 提出的回归框架能有效缓解LLM评估器的正向偏见，显著提高评估准确性，为模型选择提供更可靠的自动化评估方法。
...
### [154] [Holistic Agent Leaderboard: The Missing Infrastructure for AI Agent Evaluation](https://arxiv.org/abs/2510.11977)
*Sayash Kapoor,Benedikt Stroebl,Peter Kirgis,Nitya Nadgir,Zachary S Siegel,Boyi Wei,Tianci Xue,Ziru Chen,Felix Chen,Saiteja Utpala,Franck Ndzomga,Dheeraj Oruganty,Sophie Luskin,Kangheng Liu,Botao Yu,Amit Arora,Dongyoon Hahm,Harsh Trivedi,Huan Sun,Juyong Lee,Tengjun Jin,Yifan Mai,Yifei Zhou,Yuxuan Zhu,Rishi Bommasani,Daniel Kang,Dawn Song,Peter Henderson,Yu Su,Percy Liang,Arvind Narayanan*

Main category: cs.AI

TL;DR: HAL（Holistic Agent Leaderboard）是一个用于评估AI代理的标准化框架，通过并行评估、三维分析和LLM辅助日志检查来解决现有评估方法的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理评估存在诸多挑战，无法准确了解代理在复杂现实任务中的真实表现，需要更可靠和标准化的评估方法。

Method: 1. 开发标准化评估工具，在数百个虚拟机上进行并行评估；2. 进行涵盖模型、框架和基准的三维分析；3. 使用LLM辅助日志检查来发现未报告的行为。

Result: 进行了21,730次代理测试，涵盖9个模型和9个基准，发现了一些反直觉的结果，如更高的推理努力反而降低准确性，以及代理在任务中出现的异常行为。

Conclusion: 通过标准化评估方法和解决常见评估陷阱，HAL旨在推动研究从仅仅在基准测试中表现优异的代理转向在现实世界中可靠工作的代理。
...
### [155] [CGBench: Benchmarking Language Model Scientific Reasoning for Clinical Genetics Research](https://arxiv.org/abs/2510.11985)
*Owen Queen,Harrison G. Zhang,James Zou*

Main category: cs.AI

TL;DR: CGBench是一个用于评估语言模型在科学文献解释能力的基准测试，基于临床遗传学专家标注的文献构建，测试模型提取实验结果、评估证据强度、分类实验成果的能力。
...
Result: 模型在文献解释方面显示出潜力但存在显著差距，推理模型在细粒度任务上表现更好，非推理模型在高层次解释上更优，模型经常产生幻觉或误解结果。
...
### [156] [Asking Clarifying Questions for Preference Elicitation With Large Language Models](https://arxiv.org/abs/2510.12015)
*Ali Montazeralghaem,Guy Tennenholtz,Craig Boutilier,Ofer Meshi*

Main category: cs.AI

TL;DR: 提出一种基于扩散模型思想的两阶段训练方法，让大语言模型学会通过顺序提问来有效获取用户偏好信息，特别是在用户历史数据有限的情况下。


<details>
  <summary>Details</summary>
Motivation: 在对话式推荐系统中，当用户历史数据有限时，如何通过有效的顺序提问来获取用户偏好信息是一个关键挑战。

Method: 采用两阶段训练过程：前向过程从用户画像生成澄清问题并逐步移除答案（添加"噪声"），反向过程训练模型通过提问来"去噪"用户画像。

Result: 该方法显著提高了大语言模型提出漏斗式问题和有效获取用户偏好的能力。

Conclusion: 基于扩散模型思想的训练方法能够有效提升LLM在对话式推荐系统中获取用户偏好的能力。
...
### [157] [CausalTrace: A Neurosymbolic Causal Analysis Agent for Smart Manufacturing](https://arxiv.org/abs/2510.12033)
...
<details>
  <summary>Details</summary>
Motivation: 现代制造环境不仅需要准确预测，还需要对过程异常、根本原因和潜在干预提供可解释的见解。现有AI系统通常作为孤立的黑盒运行，缺乏预测、解释和因果推理的无缝集成，限制了其在高风险工业环境中的可信度和实用性。
...
Result: 在学术火箭装配测试平台上，CausalTrace与领域专家达成高度一致（本体问答ROUGE-1：0.91），并表现出强大的RCA性能（MAP@3：94%，PR@2：97%，MRR：0.92，Jaccard：0.92）。在C3AN评估中获得4.59/5分，展示了实时部署的精确性和可靠性。

Conclusion: CausalTrace成功地将神经符号因果分析集成到工业CoPilot中，通过数据驱动的方法结合领域知识，提供了透明、可解释的决策支持，在工业环境中表现出高精度和可靠性，解决了现有AI系统缺乏因果推理能力的问题。
...
### [158] [Do Large Language Models Respect Contracts? Evaluating and Enforcing Contract-Adherence in Code Generation](https://arxiv.org/abs/2510.12047)
*Soohan Lim,Joonghyuk Hahn,Hyunwoo Park,Sang-Ki Ko,Yo-Sub Han*

Main category: cs.AI

TL;DR: PACT是一个评估LLM生成代码合约遵循性的框架，扩展了HumanEval+和MBPP+基准，通过合约违反测试用例增强模型对合约的遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准主要评估功能正确性，但忽略了真实软件中合约遵循性这一关键方面，导致无法评估模型生成代码的鲁棒性和可靠性。

Method: 构建专注于合约违反的综合测试套件语料库，扩展现有基准；通过不同提示条件系统分析代码生成；引入新指标量化合约遵循性。

Result: 研究表明，在提示中增加合约违反测试用例相比仅使用合约描述，能显著提升模型对合约的遵循能力。
...
### [159] [Empowering LLM Agents with Geospatial Awareness: Toward Grounded Reasoning for Wildfire Response](https://arxiv.org/abs/2510.12061)
*Yiheng Chen,Lingyao Li,Zihui Ma,Qikai Hu,Yilun Zhu,Min Deng,Runlong Yu*

Main category: cs.AI

TL;DR: 提出了一种地理空间感知层(GAL)，将LLM智能体与结构化地球数据相结合，通过整合基础设施、人口、地形和天气信息，为灾害响应提供基于证据的资源分配建议。


<details>
  <summary>Details</summary>
Motivation: 现有统计方法缺乏语义上下文，跨事件泛化能力差，解释性有限；LLM虽然具有少样本泛化能力，但受限于文本且缺乏地理感知。

Method: 开发地理空间感知层(GAL)，从原始野火检测数据自动检索并整合基础设施、人口、地形和天气信息，生成带单位标注的感知脚本，结合历史类比和每日变化信号进行增量更新。

Result: 在真实野火场景中评估多个LLM模型，显示地理空间接地的智能体能够超越基线方法。

Conclusion: 该框架能够泛化到洪水、飓风等其他灾害类型，为灾害响应提供了有效的基于证据的决策支持。
...
### [160] [ThinkPilot: Steering Reasoning Models via Automated Think-prefixes Optimization](https://arxiv.org/abs/2510.12063)
*Sunzhu Li,Zhiyu Lin,Shuling Yang,Jiale Zhao,Wei Chen*

Main category: cs.AI

TL;DR: ThinkPilot是一个无需训练即可自动优化大型推理模型推理过程的框架，通过进化过程生成think-prefixes来引导模型实现更好的性能表现。
...
Method: 使用进化过程生成think-prefixes（思考前缀），这些前缀基于推理行为分类学驱动演化，指导模型朝着更优性能方向发展。

Result: ThinkPilot显著改善了精度-长度权衡以实现高效推理，大幅提升了安全性（如将DeepSeek-R1-Distill-Qwen-32B的StrongREJECT分数从27.0%降至0.7），并增强了指令跟随能力。
...
### [161] [AI Agents as Universal Task Solvers](https://arxiv.org/abs/2510.12066)
*Alessandro Achille,Stefano Soatto*

Main category: cs.AI

TL;DR: 该论文重新诠释了AI智能体中的学习角色，将其视为具有计算能力的随机动力系统，强调时间在学习推理中的关键作用，并提出从归纳学习转向转导学习的范式转变。
...
Method: 将AI智能体重新解释为计算能力的随机动力系统，强调时间在推理学习中的基础作用，提出转导学习范式，关注算法结构而非数据分布近似。

Result: 证明了通用求解器使用过去数据所能达到的最优加速与其算法信息紧密相关，推导了推理时间与训练时间之间的幂律缩放关系，并指出单纯扩大模型规模可能导致智能体行为像学者症候群患者一样缺乏真正洞察。

Conclusion: 在扩展推理模型时，需要优化的关键量是时间而非模型大小，时间在学习中的关键作用此前仅被间接考虑。
...
### [162] [HiCoTraj:Zero-Shot Demographic Reasoning via Hierarchical Chain-of-Thought Prompting from Trajectory](https://arxiv.org/abs/2510.12067)
*Junyi Xie,Yuankun Jiao,Jina Kim,Yao-Yi Chiang,Lingyi Zhao,Khurram Shafique*

Main category: cs.AI

TL;DR: HiCoTraj是一个利用LLM零样本学习和语义理解能力从人类移动轨迹推断人口统计属性的框架，无需标记训练数据，通过分层思维链推理实现透明推理过程。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于移动轨迹的人口统计推断方法依赖大规模标记数据导致的解释性差和泛化能力弱的问题，应对标记人口统计数据稀缺的挑战。

Method: 将轨迹转换为语义丰富的自然语言表示，包括详细活动记录和多尺度访问摘要，然后通过分层思维链推理（事实特征提取、行为模式分析、人口统计推断）系统引导LLM。

Result: 在真实世界轨迹数据上的实验评估表明，HiCoTraj在零样本场景下对多个人口统计属性实现了有竞争力的性能。

Conclusion: HiCoTraj框架有效解决了标记数据稀缺问题，同时提供了透明的推理链，在人口统计推断任务中展现出良好的零样本学习能力。
...
### [163] [EmboMatrix: A Scalable Training-Ground for Embodied Decision-Making](https://arxiv.org/abs/2510.12072)
...
TL;DR: 提出了EmboMatrix训练场概念，通过大规模任务模拟和精确奖励系统，让LLM在虚拟环境中获得具身决策能力。基于此训练的EmboBrain-7B模型在具身决策基准上超越了671B参数的DeepSeek-R1。


<details>
  <summary>Details</summary>
Motivation: 解决LLM缺乏物理环境暴露的问题，使其能够真正理解具身决策，实现从高级目标到可执行动作的转换。

Method: 构建EmboMatrix训练场，包含多智能体数据引擎生成大规模任务场景、分布式异构硬件系统实现可扩展模拟、多级奖励架构提供精确监督。

Result: EmboBrain-7B在具身决策基准上比671B的DeepSeek-R1提升了9.5%，证明了交互式环境学习对构建智能具身代理的有效性。
...
### [164] [CTRL-Rec: Controlling Recommender Systems With Natural Language](https://arxiv.org/abs/2510.12742)
*Micah Carroll,Adeline Foote,Kevin Feng,Marcus Williams,Anca Dragan,W. Bradley Knox,Smitha Milli*

Main category: cs.AI

TL;DR: CTRL-Rec是一种让用户通过自然语言实时控制推荐系统的方法，通过LLM模拟用户对物品的批准判断，训练嵌入模型来近似这些判断，并将其集成到传统推荐系统的信号加权中。


<details>
  <summary>Details</summary>
Motivation: 当用户对推荐系统不满意时，他们通常缺乏细粒度控制来改变推荐结果。大语言模型提供了通过自然语言请求来指导推荐的解决方案。

Method: 在训练时使用LLM模拟用户基于语言请求对物品的批准判断，训练嵌入模型来近似这些模拟判断，然后将这些基于用户请求的预测集成到传统推荐系统的标准信号加权中。部署时每个用户请求只需要一次LLM嵌入计算。

Result: 在MovieLens数据集上的实验表明，该方法在多样化请求下都能实现细粒度控制。在19名Letterboxd用户的研究中，CTRL-Rec受到用户积极评价，与传统控制相比显著增强了用户的控制感和满意度。

Conclusion: CTRL-Rec方法成功实现了通过自然语言对传统推荐系统进行实时控制，计算效率高，显著提升了用户体验和控制感。
...
### [165] [BeSTAD: Behavior-Aware Spatio-Temporal Anomaly Detection for Human Mobility Data](https://arxiv.org/abs/2510.12076)
...
Method: 学习语义丰富的移动表示，整合位置含义和时间模式；采用行为聚类感知建模机制，构建个性化行为档案并通过跨时期行为比较识别异常。

Result: 能够检测行为转变和常规偏差，并在大规模移动数据集中识别表现出此类变化的个体。
...
### [166] [Evaluating the Quality of Randomness and Entropy in Tasks Supported by Large Language Models](https://arxiv.org/abs/2510.12080)
*Rabimba Karanjai,Yang Lu,Ranjith Chodavarapu,Lei Xu,Weidong Shi*

Main category: cs.AI

TL;DR: 本文研究了大型语言模型处理随机性任务的能力，发现虽然LLM能产生一定随机性的输出，但表现不一致且与期望行为有显著偏差。


<details>
  <summary>Details</summary>
Motivation: 随着LLM技术的快速发展，许多应用需要随机性，但LLM处理随机性的能力尚不清楚，特别是在生成和使用随机数方面。

Method: 设计了一系列实验，考虑外部工具访问、任务类型、模型状态和提示策略等因素，涵盖生成随机数、随机字符串、项目洗牌以及使用熵和NIST测试套件评估随机性质量等任务。

Result: LLM能够产生具有一定随机性的输出，但性能不一致，且经常显著偏离预期行为。
...
### [167] [One Life to Learn: Inferring Symbolic World Models for Stochastic Environments from Unguided Exploration](https://arxiv.org/abs/2510.12088)
*Zaid Khan,Archiki Prasad,Elias Stengel-Eskin,Jaemin Cho,Mohit Bansal*

Main category: cs.AI

TL;DR: OneLife框架通过条件激活的程序化法则在概率编程框架中建模世界动态，能够在复杂随机环境中从有限的非指导性交互中学习关键环境动态，并在16/23个场景中优于基线。


<details>
  <summary>Details</summary>
Motivation: 解决在复杂、随机环境中，代理只有"一次生命"探索敌对环境且无人指导的现实挑战性设置，自主构建未知复杂环境的程序化世界模型。

Method: 使用条件激活的程序化法则，采用前提-效果结构，在相关世界状态中激活，创建动态计算图，仅通过相关法则路由推理和优化，避免复杂分层状态下所有法则贡献预测时的扩展挑战。

Result: 在Crafter-OO环境中，OneLife从最少非指导性交互中成功学习关键环境动态，在23个测试场景中的16个优于强基线，模拟推演成功识别出更优策略。

Conclusion: 为自主构建未知复杂环境的程序化世界模型奠定了基础，展示了在苛刻约束下学习随机动态的能力。
...
### [168] [ToPolyAgent: AI Agents for Coarse-Grained Topological Polymer Simulations](https://arxiv.org/abs/2510.12091)
*Lijie Ding,Jan-Michael Carrillo,Changwoo Do*

Main category: cs.AI

TL;DR: ToPolyAgent是一个多智能体AI框架，通过自然语言指令执行拓扑聚合物的粗粒度分子动力学模拟，集成了大语言模型与领域专用计算工具，支持线型、环状、刷状、星形聚合物及树枝状聚合物的交互式和自主模拟工作流。


<details>
  <summary>Details</summary>
Motivation: 降低复杂计算工作流的门槛，将自然语言界面与严谨的模拟工具相结合，推进聚合物科学中AI驱动的材料发现，为自主可扩展的多智能体科学研究生态系统奠定基础。

Method: 系统包含四个LLM驱动的智能体：配置智能体（生成初始聚合物-溶剂配置）、模拟智能体（执行LAMMPS分子动力学模拟和构象分析）、报告智能体（编译markdown报告）和工作流智能体（简化自主操作）。支持交互式（含用户反馈循环）和自主式（端到端任务执行）两种工作模式。

Result: 通过案例研究展示了系统的多功能性，涉及不同聚合物结构在多种溶剂条件、恒温器和模拟时长下的应用。成功指导系统研究相互作用参数对线型聚合物构象的影响，以及接枝密度对刷状聚合物持续长度的影响。

Conclusion: ToPolyAgent通过将自然语言界面与严谨的模拟工具耦合，降低了复杂计算工作流的障碍，推进了聚合物科学中AI驱动的材料发现，为自主可扩展的多智能体科学研究生态系统奠定了基础。
...
### [169] [Precise Attribute Intensity Control in Large Language Models via Targeted Representation Editing](https://arxiv.org/abs/2510.12121)
*Rongzhi Zhang,Liqin Ye,Yuzhao Heng,Xiang Chen,Tong Yu,Lingkai Kong,Sudheer Chava,Chao Zhang*

Main category: cs.AI

TL;DR: 提出了一种精确控制LLM输出属性强度的方法，通过目标达成问题重构、时序差分学习训练价值函数和基于梯度的隐表示干预，实现了对属性强度的细粒度连续控制。
...
Method: 1) 将精确属性强度控制重新表述为目标达成问题；2) 通过时序差分学习训练轻量级价值函数来预测部分生成内容的最终属性强度；3) 对隐表示进行基于梯度的干预，精确导航到特定属性强度目标。

Result: 在LLaMA-3.2-3b和Phi-4-mini上的实验证实，该方法能够以高精度将文本生成引导到用户指定的属性强度。

Conclusion: 该方法实现了对属性强度的细粒度连续控制，超越了简单的方向对齐，并在偏好数据合成、帕累托前沿近似优化和无干预推理的对齐行为蒸馏等下游任务中展示了效率提升。
...
### [170] [MatSciBench: Benchmarking the Reasoning Ability of Large Language Models in Materials Science](https://arxiv.org/abs/2510.12171)
...
<details>
  <summary>Details</summary>
Motivation: 填补大型语言模型在材料科学推理能力评估方面的空白，建立全面评估LLMs在材料科学领域推理能力的基准。

Method: 构建包含1340个问题的基准测试，采用结构化分类体系，包含多模态推理问题，并评估多种推理策略（基础思维链、工具增强、自我修正）。

Result: 即使是表现最佳的Gemini-2.5-Pro模型在材料科学问题上的准确率也不到80%，表明MatSciBench具有较高复杂性。不同推理策略在不同场景下表现各异，没有单一方法在所有情况下都表现优越。

Conclusion: MatSciBench为评估和推动LLMs在材料科学领域的科学推理能力建立了全面而可靠的基准，揭示了当前模型在该领域的局限性。
...
### [171] [Evolution of meta's llama models and parameter-efficient fine-tuning of large language models: a survey](https://arxiv.org/abs/2510.12178)
...
TL;DR: 本文综述了Meta AI的LLaMA系列模型（从LLaMA 1到LLaMA 4）的快速发展，以及为这些模型开发的参数高效微调（PEFT）方法，包括模型架构、性能特征和各种PEFT技术的机制与应用。


<details>
  <summary>Details</summary>
Motivation: 随着LLaMA系列模型的快速演进和广泛应用，需要系统性地总结这些基础模型及其高效微调方法，为研究者和实践者提供一站式资源。

Method: 首先描述LLaMA系列基础模型（7B-65B到288B参数）及其架构，然后介绍PEFT概念，并详细回顾五种应用于LLaMA的PEFT方法：LoRA、LLaMA-Adapter V1和V2、LLaMA-Excitor和QLoRA。

Result: 提供了模型和适配器架构、参数数量和基准测试结果的结构化分析，展示了微调后的LLaMA模型在某些情况下优于更大基线模型的示例。

Conclusion: LLaMA模型和PEFT方法已在法律和医疗等现实世界用例中成功应用，未来研究方向包括扩展到更大上下文和提升鲁棒性。
...
### [172] [ResearStudio: A Human-Intervenable Framework for Building Controllable Deep-Research Agents](https://arxiv.org/abs/2510.12194)
*Linyi Yang,Yixuan Weng*

Main category: cs.AI

TL;DR: ResearStudio是一个开源框架，通过"协作工作坊"设计实现实时人工控制的深度研究代理，支持用户在AI主导、人工辅助和人工主导、AI辅助模式间无缝切换。
...
### [173] [On the Design and Evaluation of Human-centered Explainable AI Systems: A Systematic Review and Taxonomy](https://arxiv.org/abs/2510.12201)
*Aline Mangold,Juliane Zietz,Susanne Weinhold,Sebastian Pannasch*

Main category: cs.AI

TL;DR: 本文对65个评估可解释AI(XAI)系统的用户研究进行了全面综述，提出了以人为中心的XAI系统设计目标和评估框架，并根据用户AI专业水平（AI新手和数据专家）调整设计目标。


<details>
  <summary>Details</summary>
Motivation: 随着AI在日常生活中的普及，对既高性能又可理解的智能系统需求增加。当前XAI系统评估过于技术化，缺乏对用户需求的关注，需要基于用户研究的评估指南。

Method: 对65个XAI用户研究进行系统性综述，分析XAI系统特性和评估指标，提出以人为中心的设计目标，并根据用户AI专业水平进行差异化设计。

Result: 区分了核心系统与XAI解释组件，将评估指标分为系统情感、认知、可用性、可解释性和解释指标，并为AI新手和数据专家分别制定了不同的设计目标。

Conclusion: 提出了扩展的XAI评估和设计框架，强调需要根据用户特征定制XAI系统设计，AI新手关注负责任使用、接受度和可用性，数据专家关注性能导向的人机协作和任务表现。
...
### [174] [GOAT: A Training Framework for Goal-Oriented Agent with Tools](https://arxiv.org/abs/2510.12218)
...
Method: 提出GOAT框架，从API文档自动构建合成数据集，训练LLM代理进行相互依赖的API调用推理和连贯响应生成。
...
### [175] [MedKGEval: A Knowledge Graph-Based Multi-Turn Evaluation Framework for Open-Ended Patient Interactions with Clinical LLMs](https://arxiv.org/abs/2510.12224)
*Yuechun Yu,Han Ying,Haoan Jin,Wenjian Jiang,Dong Xian,Binghao Wang,Zhou Yang,Mengyue Wu*

Main category: cs.AI

TL;DR: 提出了MedKGEval框架，这是一个基于结构化医学知识的多轮对话评估系统，用于评估临床LLM在动态医患交互中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要依赖事后审查完整对话记录，无法捕捉医疗对话的动态性和上下文敏感性，以及患者不断变化的信息需求。

Method: 使用知识图谱驱动患者模拟机制，通过控制模块从知识图谱检索相关医学事实；采用即时、轮级评估框架，由法官代理评估每个模型响应的临床适当性、事实正确性和安全性；构建多轮基准测试。

Result: 对8个最先进LLM的全面评估表明，MedKGEval能够识别传统评估流程经常忽略的细微行为缺陷和安全风险。

Conclusion: MedKGEval框架为临床LLM提供了更可靠的评估方法，虽然最初设计用于中英文医疗应用，但可通过切换知识图谱轻松扩展到其他语言。
...
### [176] [PromptFlow: Training Prompts Like Neural Networks](https://arxiv.org/abs/2510.12246)
*Jingyi Wang,Hongyuan Zhu,Ye Niu,Yunhui Deng*

Main category: cs.AI

TL;DR: PromptFlow是一个受TensorFlow启发的模块化提示工程框架，通过元提示、操作符、优化器和评估器实现自动化的提示优化，使用基于梯度的元学习和强化学习来回收经验。


<details>
  <summary>Details</summary>
Motivation: 解决传统提示工程依赖人工设计、静态更新规则和缺乏动态策略选择的问题，特别是LLM在提示工程中经验回收的未探索问题。

Method: 提出PromptFlow框架，包含元提示、操作符、优化器和评估器模块，采用基于梯度的元学习自动探索最优提示优化路径，并设计强化学习方法回收LLM在提示工程过程中的经验。

Result: 在多个数据集上的广泛实验证明了PromptFlow的有效性，能够以最少的任务特定训练数据实现最优的提示优化。

Conclusion: PromptFlow提供了一个高效、自动化的提示工程解决方案，显著减少了人工干预需求，并在各种NLP任务中表现出色。
...
### [177] [$\mathbf{T^3}$: Reducing Belief Deviation in Reinforcement Learning for Active Reasoning](https://arxiv.org/abs/2510.12264)
*Deyu Zou,Yongqiang Chen,Jianxiang Wang,Haochen Yang,Mufei Li,James Cheng,Pan Li,Yu Gong*

Main category: cs.AI

TL;DR: 提出T³方法，通过检测信念偏差并截断训练轨迹来改善LLM主动推理中的信念跟踪问题，提升训练稳定性和性能


<details>
  <summary>Details</summary>
Motivation: LLM在主动推理中容易产生信念偏差，导致问题状态丢失和重复无效动作，这会阻碍强化学习训练中关键探索步骤的正确奖励分配

Method: 开发T³方法，跟踪模型信念偏差，检测过度偏差并在训练时截断轨迹，移除无信息尾部，保留信息前缀的奖励
...
### [178] [RAG-Anything: All-in-One RAG Framework](https://arxiv.org/abs/2510.12323)
*Zirui Guo,Xubin Ren,Lingrui Xu,Jiahao Zhang,Chao Huang*

Main category: cs.AI

TL;DR: RAG-Anything是一个统一的多模态检索增强生成框架，能够处理文本、图像、表格和数学表达式等多种模态内容，解决了现有RAG系统只能处理文本的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统与现实世界信息环境存在严重不匹配，现代知识库本质上是多模态的，包含文本、视觉元素、结构化表格和数学表达式的丰富组合，但现有RAG框架仅限于文本内容，在处理多模态文档时存在根本性差距。

Method: 将多模态内容重新概念化为相互连接的知识实体而非孤立的数据类型；引入双图构建来捕获跨模态关系和文本语义的统一表示；开发结合结构知识导航和语义匹配的跨模态混合检索方法。

Result: 在具有挑战性的多模态基准测试中表现出优越性能，相比最先进方法取得显著改进；在传统方法失效的长文档上性能提升尤为明显。
...
### [179] [O-Forge: An LLM + Computer Algebra Framework for Asymptotic Analysis](https://arxiv.org/abs/2510.12350)
*Ayush Khaitan,Vijay Ganesh*

Main category: cs.AI

TL;DR: 提出了LLM+CAS框架和O-Forge工具，将前沿大语言模型与计算机代数系统结合，通过符号反馈循环生成既有创造性又经过符号验证的证明，特别针对渐近不等式问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在研究数学中验证困难的问题，回答Terence Tao提出的关于LLM与验证器结合能否证明复杂渐近不等式的问题，推动AI从竞赛数学向研究级数学工具发展。

Method: 使用LLM+CAS框架，通过上下文符号反馈循环：LLM建议域分解，CAS（如Mathematica）对每个部分进行公理化验证，形成创造性提议与符号验证的闭环。

Result: 该框架在提出适当域分解方面表现出显著效果，能够处理涉及困难证明和适当域分解的渐近不等式问题。

Conclusion: LLM+CAS框架成功展示了AI可以超越竞赛数学，成为专业数学家的研究级工具，特别是在渐近分析领域。
...
### [180] [A Survey of Vibe Coding with Large Language Models](https://arxiv.org/abs/2510.12399)
...
<details>
  <summary>Details</summary>
Motivation: Vibe Coding作为一种新兴开发范式，其有效性尚未得到充分探索，实证研究显示存在意外的生产力损失和人类-AI协作的根本挑战。

Method: 通过系统分析1000多篇研究论文，建立了Constrained Markov Decision Process理论框架，并综合现有实践形成了五种开发模型分类。

Result: 成功Vibe Coding不仅依赖于代理能力，更依赖于系统性上下文工程、完善的开发环境和人-代理协作开发模型。

Conclusion: Vibe Coding作为一种变革性开发方法，需要系统性的理论框架和实践模型支持，才能充分发挥其潜力。
...
### [181] [PricingLogic: Evaluating LLMs Reasoning on Complex Tourism Pricing Tasks](https://arxiv.org/abs/2510.12409)
*Yunuo Liu,Dawei Zhu,Zena Al-Khalili,Dai Cheng,Yanjun Chen,Dietrich Klakow,Wei Zhang,Xiaoyu Shen*

Main category: cs.AI

TL;DR: PricingLogic是首个评估大语言模型在旅游定价任务中可靠性的基准测试，包含300个基于真实定价政策的自然语言问题，测试显示LLMs在复杂定价场景中存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 旅游机构希望将容易出错的定价任务自动化给AI系统，但未经验证的LLM部署可能导致重大财务损失和客户信任危机。
...
### [182] [MTOS: A LLM-Driven Multi-topic Opinion Simulation Framework for Exploring Echo Chamber Dynamics](https://arxiv.org/abs/2510.12423)
*Dingyi Zuo,Hongjie Zhang,Jie Ou,Chaosheng Feng,Shuwan Liu*

Main category: cs.AI

TL;DR: 提出了MTOS框架，将多主题上下文与LLM结合，通过记忆机制、交互策略和信念衰减来模拟跨主题观点演化，解决了现有单主题LLM研究和传统数值模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中的观点极化、信息隔离和认知偏见问题日益严重，现实网络中的信息往往涉及多个相关主题，现有研究主要关注单主题，无法捕捉多主题跨领域的认知转移，传统数值模型缺乏解释性和行为一致性。

Method: 提出MTOS框架，整合LLM与短期和长期记忆，采用多种用户选择交互机制和动态主题选择策略，引入信念衰减机制实现跨主题观点更新。

Result: 多主题设置显著改变极化趋势：正相关主题放大回音室效应，负相关主题抑制回音室，无关主题通过资源竞争也能缓解回音室效应。相比数值模型，LLM智能体能更真实地模拟动态观点变化，再现新闻文本的语言特征，捕捉复杂的人类推理。

Conclusion: MTOS框架提高了模拟的解释性和系统稳定性，为多主题社交媒体观点演化提供了更真实的仿真工具。
...
### [183] [Biased-Attention Guided Risk Prediction for Safe Decision-Making at Unsignalized Intersections](https://arxiv.org/abs/2510.12428)
...
### [184] [Evaluating and Mitigating LLM-as-a-judge Bias in Communication Systems](https://arxiv.org/abs/2510.12462)
*Jiaxin Gao,Chen Chen,Yanwen Jia,Xueluan Gong,Kwok-Yan Lam,Qian Wang*

Main category: cs.AI

TL;DR: 本文系统研究了LLM作为评判者在点式评分设置下的11种偏见类型，发现当前LLM评判者对偏见输入具有鲁棒性，但微调于偏见数据会显著降低性能，并提出了四种缓解策略。


<details>
  <summary>Details</summary>
Motivation: 随着LLM被广泛用于通信系统中内容质量的自主评估，这些AI"评判者"的公正性无法保证，其评估标准中的任何偏见都可能扭曲结果并损害用户信任。

Method: 系统调查了两种LLM评判模型（GPT-Judge和JudgeLM）在点式评分设置下的11种偏见类型，涵盖隐式和显式形式，并分析了微调、评分标准、任务难度等因素的影响。

Result: 发现最先进的LLM评判者对偏见输入具有鲁棒性，通常给偏见样本分配较低分数；提供详细评分标准能进一步增强鲁棒性；在偏见数据上微调会显著降低性能；评判分数与任务难度相关。
...
### [185] [Using Medical Algorithms for Task-Oriented Dialogue in LLM-Based Medical Interviews](https://arxiv.org/abs/2510.12490)
*Rui Reis,Pedro Rangel Henriques,João Ferreira-Coimbra,Eva Oliveira,Nuno F. Rodrigues*

Main category: cs.AI

TL;DR: 开发了一个基于有向无环图的任务导向医疗对话框架，包含冷启动机制、自适应分支回溯、终止逻辑和自动化报告生成，在初步评估中显示出良好的用户体验和临床工作流程集成效果。


<details>
  <summary>Details</summary>
Motivation: 旨在构建一个能够将医疗算法和指南转化为结构化对话的系统，改善医患交互效率，减少认知负担，并支持临床决策。

Method: 采用有向无环图结构组织医疗问题，包含五个核心组件：临床问题语料库构建、基于层次聚类的冷启动机制、扩展剪枝的自适应分支、终止逻辑和自动化报告生成。

Result: 患者应用获得低认知负荷(15.6)、高可用性(86)和强满意度(8.1/9)；医生应用获得中等认知负荷(26)、优异可用性(88.5)和高满意度(8.3/9)。

Conclusion: 该系统成功集成到临床工作流程中，有效减少认知需求并支持高效报告生成，但存在系统延迟和小样本评估的局限性。
...
### [186] [Artificial Intelligence Virtual Cells: From Measurements to Decisions across Modality, Scale, Dynamics, and Evaluation](https://arxiv.org/abs/2510.12498)
*Chengpeng Hu,Calvin Yu-Chian Chen*

Main category: cs.AI

TL;DR: 提出了一个模型无关的细胞状态潜在视角，通过操作符语法组织学习过程，强调决策对齐的评估框架和功能空间读出的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前AI虚拟细胞模型在跨实验室、跨平台的可迁移性有限，存在数据泄露和覆盖偏差问题，且剂量、时间和组合效应尚未系统处理，跨尺度耦合受限。

Method: 采用模型无关的细胞状态潜在视角，通过测量、提升/投影实现跨尺度耦合，以及干预操作来处理剂量和调度问题。

Result: 提出了决策对齐的评估蓝图，涵盖模态、尺度、上下文和干预，强调功能空间读出如通路活性、空间邻域和临床相关终点。

Conclusion: 推荐操作符感知的数据设计、抗泄露分区以及透明校准和报告，以实现可重复的同类比较。
...
### [187] [ProtoSiTex: Learning Semi-Interpretable Prototypes for Multi-label Text Classification](https://arxiv.org/abs/2510.12534)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于原型的模型通常只在粗粒度（句子或文档级别）操作，无法处理真实世界文本分类的多标签性质，需要更细粒度的可解释模型。
...
Conclusion: ProtoSiTex通过自适应原型和多头注意力捕获重叠和冲突语义，为半可解释多标签文本分类提供了一个稳健的解决方案。
...
### [188] [Inclusive Fitness as a Key Step Towards More Advanced Social Behaviors in Multi-Agent Reinforcement Learning Settings](https://arxiv.org/abs/2510.12555)
*Andries Rosseau,Raphaël Avalos,Ann Nowé*

Main category: cs.AI

TL;DR: 提出基于包容性适应度的多智能体强化学习框架，通过基因型分配和遗传物质共享，模拟自然选择中的竞争与合作力量，在囚徒困境网络游戏中验证了与汉密尔顿法则等生物学原理的一致性。


<details>
  <summary>Details</summary>
Motivation: 受自然选择中竞争与合作力量驱动智力演化的启发，旨在开发能够模拟生物进化社会动态的多智能体系统，超越传统的二元团队结构。

Method: 采用多智能体强化学习框架，为每个智能体分配基因型，基于包容性适应度设计奖励函数，允许遗传物质共享，在囚徒困境网络游戏中研究社会动态。

Result: 实验结果与汉密尔顿法则等生物学原理一致，展示了基于遗传相似性的合作谱系，能够产生独特的非团队社会动态，如三方不对称合作关系。

Conclusion: 基于包容性适应度的奖励机制为更高级战略智能和社交智能的出现提供了基础，能够模拟生物进化中的多智能体自动课程，产生策略军备竞赛。
...
### [189] [HardcoreLogic: Challenging Large Reasoning Models with Long-tail Logic Puzzle Games](https://arxiv.org/abs/2510.12563)
*Jingcong Liang,Shijun Wan,Xuehai Wu,Siyuan Wang,Yitong Li,Qianglong Chen,Duyu Tang,Zhongyu Wei*

Main category: cs.AI

TL;DR: 提出了HardcoreLogic基准测试，包含5000多个逻辑谜题，通过增加复杂性、非常规元素和不可解谜题三个维度来评估大型推理模型的鲁棒性，揭示了当前模型对记忆模式的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注标准格式的谜题（如9x9数独），可能导致模型过度拟合和记忆解法模式，无法真正测试模型对新颖规则的理解和策略适应能力。

Method: 构建HardcoreLogic基准，包含10种游戏的5000多个谜题，通过三个维度系统性地变换标准谜题：增加复杂性(IC)、非常规元素(UE)和不可解谜题(UP)。

Result: 评估显示各种大型推理模型性能显著下降，即使在其他基准上表现优秀的模型也严重依赖记忆的刻板模式。增加复杂性是主要困难来源，但模型在细微规则变化上同样表现不佳。
...
### [190] [Memory as Action: Autonomous Context Curation for Long-Horizon Agentic Tasks](https://arxiv.org/abs/2510.12635)
*Yuxiang Zhang,Jiangming Shu,Ye Ma,Xueyuan Lin,Shangxi Wu,Jitao Sang*

Main category: cs.AI

TL;DR: 提出了Memory-as-Action框架，将工作记忆管理重构为可学习的内部能力，通过强化学习训练智能体在统一策略中执行显式记忆编辑操作，以平衡记忆管理与长期任务目标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长视野智能体任务中面临挑战，其受限内存容易被分散或不相关的上下文淹没。现有工作记忆方法通常依赖与智能体核心策略分离的外部启发式机制。

Method: 提出Memory-as-Action框架，智能体通过执行显式编辑操作主动管理工作记忆；为解决轨迹断裂问题，提出动态上下文策略优化算法，在记忆动作点分割轨迹并对结果动作段应用轨迹级优势。

Result: 端到端联合优化任务推理和记忆管理不仅减少了总体计算消耗，还提高了任务性能，这是通过适应模型内在能力的自适应上下文管理策略实现的。

Conclusion: 将工作记忆管理作为可学习的内部能力，通过统一策略进行端到端优化，能够有效提升智能体在长视野任务中的性能，同时减少计算资源消耗。
...
### [191] [ERA: Transforming VLMs into Embodied Agents via Embodied Prior Learning and Online Reinforcement Learning](https://arxiv.org/abs/2510.12693)
...
### [192] [Multi-Agent Debate for LLM Judges with Adaptive Stability Detection](https://arxiv.org/abs/2510.12697)
*Tianyu Hu,Zhen Tan,Song Wang,Huaizhi Qu,Tianlong Chen*

Main category: cs.AI

TL;DR: 提出多智能体辩论法官框架，通过协作推理和迭代优化提升LLMs作为法官的判断准确性，相比多数投票方法更有效。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs作为法官的方法依赖简单的聚合方法（如多数投票），即使个体智能体提供正确答案也可能失败，需要更有效的协作判断机制。

Method: 建立多智能体辩论法官框架，智能体协作推理并迭代优化回答；引入稳定性检测机制，通过时变Beta-Binomial混合模型建模法官共识动态，基于分布相似性（Kolmogorov-Smirnov检验）实现自适应停止。

Result: 在多个基准测试和模型上的实验表明，该框架相比多数投票方法提高了判断准确性，同时保持了计算效率。

Conclusion: 多智能体辩论框架通过协作推理和自适应停止机制，能够有效提升LLMs作为法官的判断性能，为自动化评估任务提供了更可靠的解决方案。
...
### [193] [CAMNet: Leveraging Cooperative Awareness Messages for Vehicle Trajectory Prediction](https://arxiv.org/abs/2510.12703)
*Mattia Grasselli,Angelo Porrello,Carlo Augusto Grazia*

Main category: cs.AI

TL;DR: 本文研究了使用车辆间通信的协作感知消息（CAM）数据进行车辆轨迹预测，开发了基于图神经网络的CAMNet模型，并在两个数据集上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临安全挑战，现有传感器存在视野受限和遮挡问题。车辆间通信能够共享信息，提高情境感知能力，特别是在传感器被遮挡时。

Method: 设计并训练了基于图神经网络的CAMNet模型，使用协作感知消息数据，在一个广泛使用的运动预测数据集上训练，并在自建的CAM数据集上评估。

Result: 方法显示出有希望的结果，证明CAM数据确实可以支持车辆轨迹预测，但同时也存在一些局限性。

Conclusion: CAM数据在车辆轨迹预测方面具有潜力，为未来研究提供了机会，但需要解决当前方法的局限性。
...
### [194] [Towards Robust Artificial Intelligence: Self-Supervised Learning Approach for Out-of-Distribution Detection](https://arxiv.org/abs/2510.12713)
*Wissam Salhab,Darine Ameyed,Hamid Mcheick,Fehmi Jaafar*

Main category: cs.AI

TL;DR: 提出一种无需标记数据即可改进OOD检测的方法，通过自监督学习和图论技术提高AI系统鲁棒性，在AUROC指标上达到0.99的优异表现。
...
Method: 结合自监督学习和图论技术，利用无标记数据学习有用表示，从而更有效地识别和分类OOD样本。

Result: 与现有最先进方法相比，该方法在AUROC指标上达到0.99的优异性能。

Conclusion: 该方法成功提高了AI系统对OOD样本的检测能力，增强了系统鲁棒性，且无需依赖标记数据。
...
### [195] [Clutch Control: An Attention-based Combinatorial Bandit for Efficient Mutation in JavaScript Engine Fuzzing](https://arxiv.org/abs/2510.12732)
...
<details>
  <summary>Details</summary>
Motivation: 现有JavaScript模糊测试方法在突变目标选择上使用随机策略，作者认为这个问题适合用可变臂数的组合多臂老虎机来解决，以提高模糊测试效率。
...
Conclusion: CLUTCH在JavaScript模糊测试中表现出更高的效率，特别是在可变和组合环境中优于现有最先进的多臂老虎机方法。
...
TL;DR: Ax-Prover是一个基于大型语言模型的多智能体自动定理证明系统，通过Model Context Protocol集成Lean工具，能在多个科学领域进行定理证明，并支持自主或与人类专家协作工作。


<details>
  <summary>Details</summary>
Motivation: 解决科学问题需要创造性推理和严格语法严谨性，现有专门化证明系统难以泛化到不同科学领域。

Method: 将大型语言模型的知识推理能力与Lean工具的正式正确性保证相结合，通过Model Context Protocol实现工具集成。

Result: 在公开数学基准测试中与最先进证明器竞争，在新引入的抽象代数和量子理论基准测试中大幅领先，并成功协助数学家形式化复杂密码学定理证明。

Conclusion: 基于工具的智能体定理证明方法为跨领域形式验证提供了可泛化的通用方法学，克服了专门化系统的泛化限制。
...
### [197] [Think as a Doctor: An Interpretable AI Approach for ICU Mortality Prediction](https://arxiv.org/abs/2510.11745)
*Qingwen Li,Xiaohang Zhao,Xiao Han,Hailiang Huang,Lanjuan Liu*

Main category: cs.LG

TL;DR: ProtoDoctor是一个用于ICU死亡率预测的新型框架，通过原型学习实现内在可解释性，整合了临床病程识别、人口异质性和预后意识三个关键决策要素。
...
Method: ProtoDoctor包含两个关键创新模块：预后临床病程识别模块（通过原型学习识别临床病程，使用新型正则化机制实现预后意识）和人口异质性识别模块（通过队列特定原型和风险调整建模人口异质性）。

Result: 广泛的实证评估表明ProtoDoctor在预测准确性上优于最先进的基线方法。人工评估进一步证实其解释更具临床意义、更可信且在ICU实践中更适用。
...
### [198] [Robust Adversarial Reinforcement Learning in Stochastic Games via Sequence Modeling](https://arxiv.org/abs/2510.11877)
*Xiaohang Tang,Zhuowen Cheng,Satyabrat Kumar*

Main category: cs.LG

TL;DR: 提出CART框架，通过将决策Transformer与阶段博弈的NashQ值结合，增强在对抗性随机游戏中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer已应用于序列决策，但其在对抗性强化学习中的鲁棒性尚未充分探索，特别是在对抗性随机游戏场景下。

Method: 将每个阶段的交互建模为阶段博弈，定义基于后续状态期望最大值的收益函数，通过Transformer策略条件化于阶段博弈的NashQ值。

Result: CART实现了更精确的最小最大值估计，并在多种对抗性随机游戏中持续获得更优的最坏情况回报。

Conclusion: CART是首个增强决策Transformer在对抗性随机游戏中鲁棒性的框架，能同时减少可被利用性和对状态转移不确定性的保守性。
...
### [199] [GAR: Generative Adversarial Reinforcement Learning for Formal Theorem Proving](https://arxiv.org/abs/2510.11769)
*Ruida Wang,Jiarui Yao,Rui Pan,Shizhe Diao,Tong Zhang*

Main category: cs.LG

TL;DR: 提出GAR框架，通过生成对抗式强化学习联合训练问题生成器和求解器，在可验证环境中实现问题生成与求解的协同进化，提升数学定理证明能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定问题集进行训练，导致训练效率低下且难以处理复杂问题。需要一种能够动态调整问题难度、与求解器能力协同进化的训练框架。

Method: GAR框架包含问题生成器和求解器，在对抗循环中联合训练。通过隐式课程学习机制，使问题难度与证明器能力同步演进。

Result: 在MiniF2F-Test基准测试中，Goedel-Prover-V2-8B和DeepSeek-Prover-V2-7B的pass@32平均相对提升4.20%；DeepSeek-Prover-V2在ProofNet-Test上的pass@32从22.58%提升至25.81%。

Conclusion: GAR不仅提升了数学定理证明性能，还建立了一个通用的强化学习范式，适用于可验证环境下的问题生成与求解协同进化。
...
TL;DR: 提出了一种通用傅里叶特征物理信息极限学习机(GFF-PIELM)，通过集成傅里叶特征映射作为激活函数，解决了传统PIELM在处理高频和变频偏微分方程时的困难。


<details>
  <summary>Details</summary>
Motivation: 传统物理信息极限学习机在处理高频和变频偏微分方程时面临挑战，需要改进以更好地捕捉目标解的不同频率分量。

Method: 1) 将傅里叶特征映射变体作为极限学习机的激活函数；2) 为隐藏层神经元分配频率系数；3) 开发基于ELM输出权重分布的初始化方法。

Result: GFF-PIELM在5个案例研究、10个数值示例中表现出色，显著提高了预测精度，同时保持了训练时间和架构复杂度的优势。

Conclusion: 该方法成功扩展了PIELM解决高频和变频PDEs的能力，其初始化策略对其他物理信息机器学习框架具有启发意义。
...
### [201] [Combining Euclidean and Hyperbolic Representations for Node-level Anomaly Detection](https://arxiv.org/abs/2510.11827)
*Simone Mungari,Ettore Ritacco,Pietro Sabatino*

Main category: cs.LG

TL;DR: Janus是一个结合欧几里得和双曲几何的图神经网络框架，通过多视图特征和对比学习来检测节点级异常。
...
Method: 使用欧几里得和双曲图神经网络分别处理原始特征和结构特征，通过多图自编码器框架和对比学习目标来对齐嵌入空间。

Result: 在四个真实数据集上的实验表明，Janus持续优于浅层和深层基线方法。

Conclusion: 结合多种几何表示能够为图中细微复杂异常的识别提供鲁棒有效的解决方案。
...
### [202] [Schrödinger bridge for generative AI: Soft-constrained formulation and convergence analysis](https://arxiv.org/abs/2510.11829)
...
<details>
  <summary>Details</summary>
Motivation: 经典薛定谔桥问题在生成式AI中应用时，硬终端约束在高维或数据稀缺情况下会导致实现不稳定，需要更灵活的方法。

Method: 采用软约束薛定谔桥问题框架，将终端约束替换为一般惩罚函数，形成更灵活的McKean-Vlasov型随机控制公式。

Result: 证明了所有惩罚水平下最优解的存在性，并证明当惩罚增大时，控制和价值函数以线性速率收敛到经典SBP的解。

Conclusion: 软约束方法不仅提供了定量收敛保证，还揭示了惩罚正则化如何实现鲁棒的生成建模、微调和迁移学习。
...
### [203] [Z0-Inf: Zeroth Order Approximation for Data Influence](https://arxiv.org/abs/2510.11832)
...
<details>
  <summary>Details</summary>
Motivation: 理解和量化训练数据对机器学习模型预测行为的影响对于数据选择和模型调试至关重要，但现有方法在大模型上要么精度不足，要么计算成本过高。

Method: 使用零阶近似方法，仅依赖训练和测试数据在中间检查点上的损失值以及检查点本身，无需梯度或逆Hessian计算。

Result: 该方法在计算效率上显著优于现有方法，在估计自影响(self-influence)方面达到更高精度，在估计训练-测试影响方面达到相当或更好的精度。
...
### [204] [Don't Walk the Line: Boundary Guidance for Filtered Generation](https://arxiv.org/abs/2510.11834)
*Sarah Ball,Andreas Haupt*

Main category: cs.LG

TL;DR: 提出Boundary Guidance方法，通过强化学习微调生成模型，使其远离分类器决策边界，提高安全性和实用性


<details>
  <summary>Details</summary>
Motivation: 传统微调方法使模型输出靠近分类器决策边界，导致误报和漏报增加，需要更好的安全过滤策略

Method: 使用强化学习微调生成模型，明确引导生成远离分类器边界区域

Result: 在越狱和模糊提示基准测试中，该方法提高了输出的安全性和实用性，LLM评估验证了效果

Conclusion: Boundary Guidance方法在不同模型规模和奖励设计下都表现出鲁棒性，是有效的安全过滤增强方法
...
### [205] [WaveletDiff: Multilevel Wavelet Diffusion For Time Series Generation](https://arxiv.org/abs/2510.11839)
*Yu-Hsiang Wang,Olgica Milenkovic*

Main category: cs.LG

TL;DR: WaveletDiff是一个基于小波系数的扩散模型框架，用于生成高质量的时间序列数据，通过多分辨率结构和跨层注意力机制在多个真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列生成模型局限于时域或频域，难以再现真实世界时间序列固有的多尺度结构，且大型高质量时间序列数据集稀缺。
...
Conclusion: WaveletDiff通过利用时间序列数据的多分辨率结构，在生成质量上显著优于现有方法，为解决时间序列数据稀缺问题提供了有效方案。
...
### [206] [Balancing Synthetic Data and Replay for Enhancing Task-Specific Capabilities](https://arxiv.org/abs/2510.11842)
*Urs Spiegelhalter,Jörg K. H. Franke,Frank Hutter*

Main category: cs.LG

TL;DR: 该论文研究了在计算预算约束下，通过继续预训练适应语言模型到新任务时，回放比例配置对任务性能和知识保留的平衡影响。


<details>
  <summary>Details</summary>
Motivation: 语言模型适应新任务时面临基本权衡：学习新能力的同时避免对现有知识的灾难性遗忘。现有研究对在计算约束下平衡任务性能和知识保留的最优回放比例理解不足。

Method: 使用bAbI推理任务作为目标，应用合成数据生成技术，系统评估不同总标记预算和回放比例配置，分析它们对任务掌握和通用知识保留的影响。

Result: 实验揭示了一个平衡任务特定性能和通用知识保留的最优配置，基于此提供了根据计算预算选择回放比例的实证指导。

Conclusion: 研究提供了基于计算预算选择回放比例的实用指南，使从业者能够以显著降低的训练成本实现强大的任务适应。
...
### [207] [Evaluating Open-Source Vision-Language Models for Multimodal Sarcasm Detection](https://arxiv.org/abs/2510.11852)
*Saroj Basnet,Shafkat Farabi,Tharindu Ranasinghe,Diptesh Kanoji,Marcos Zampieri*

Main category: cs.LG

TL;DR: 评估7种先进视觉语言模型在零样本、单样本和少样本提示下的多模态讽刺检测能力，以及生成讽刺解释的能力。


<details>
  <summary>Details</summary>
Motivation: 开源视觉语言模型的进展为理解复杂主观的多模态现象（如讽刺）提供了新机会，需要评估这些模型在多模态讽刺检测和解释生成方面的能力。

Method: 使用BLIP2、InstructBLIP、OpenFlamingo、LLaVA、PaliGemma、Gemma3和Qwen-VL等7种模型，在三个基准讽刺数据集（Muse、MMSD2.0和SarcNet）上进行零样本、单样本和少样本提示测试。

Result: 当前模型在二元讽刺检测方面取得中等成功，但在没有任务特定微调的情况下无法生成高质量的讽刺解释。

Conclusion: 虽然视觉语言模型在多模态讽刺检测方面有一定能力，但在生成高质量解释方面仍需改进，可能需要任务特定的微调。
...
### [208] [Actor-Enriched Time Series Forecasting of Process Performance](https://arxiv.org/abs/2510.11856)
*Aurelie Leribaux,Rafael Oyamada,Johannes De Smedt,Zahra Dasht Bozorgi,Artem Polyvyanyy,Jochen De Weerdt*

Main category: cs.LG

TL;DR: 该研究探讨了将参与者行为信息作为时间序列纳入预测过程监控(PPM)中，是否能提高吞吐时间(TT)预测模型的性能。结果表明，包含参与者行为的模型在RMSE、MAE和R2指标上均优于仅包含TT特征的基线模型。


<details>
  <summary>Details</summary>
Motivation: 过程通常是资源驱动的，理解并纳入参与者行为对于预测至关重要。虽然现有研究已纳入参与者行为的某些方面，但其作为时变信号在PPM中的作用仍然有限。

Method: 使用真实事件日志构建多变量时间序列，包括TT以及参与者中心特征（参与者参与度、继续、中断和交接行为的频率，以及这些行为的持续时间）。训练和比较多个模型来研究添加参与者行为的好处。

Result: 包含参与者行为的模型在RMSE、MAE和R2指标上持续优于仅包含TT特征的基线模型。

Conclusion: 随时间建模参与者行为并将此信息纳入预测模型可以增强性能指标预测的能力。
...
### [209] [Improving Knowledge Graph Embeddings through Contrastive Learning with Negative Statements](https://arxiv.org/abs/2510.11868)
*Rita T. Sousa,Heiko Paulheim*

Main category: cs.LG

TL;DR: 提出了一种新颖的知识图嵌入方法，通过整合显式声明的负陈述来改进嵌入学习，采用双模型架构分别训练正负陈述，并在训练中相互生成负样本。


<details>
  <summary>Details</summary>
Motivation: 现有知识图嵌入方法大多基于封闭世界假设，将缺失三元组视为假，这与现实世界的开放世界假设不符。显式负陈述有助于区分假三元组和未知三元组，但在知识图中很少包含且常被忽略。

Method: 使用双模型架构，两个嵌入模型并行训练：一个处理正陈述，另一个处理负陈述。训练时每个模型通过破坏正样本生成负样本，并选择另一个模型评分最高的候选作为负样本。

Result: 在通用和领域特定知识图上进行链接预测和三元组分类任务评估，实验表明该方法在预测性能上优于最先进的嵌入模型。
...
### [210] [ADARL: Adaptive Low-Rank Structures for Robust Policy Learning under Uncertainty](https://arxiv.org/abs/2510.11899)
*Chenliang Li,Junyu Leng,Jiaxiang Li,Youbang Sun,Shixiang Chen,Shahin Shahrampour,Alfredo Garcia*

Main category: cs.LG

TL;DR: AdaRL是一个双层优化框架，通过自适应调整策略复杂度与任务内在维度对齐来提升强化学习的鲁棒性，避免传统min-max优化的计算开销和保守性。
...
Method: 提出AdaRL双层优化框架：下层在固定秩约束下进行策略优化，从Wasserstein球中采样动态；上层自适应调整秩以平衡偏差-方差权衡，将策略参数投影到低秩流形上。

Result: 在MuJoCo连续控制基准测试中，AdaRL不仅持续优于固定秩基线和最先进的鲁棒RL方法，而且收敛到基础任务的内在秩。
...
### [211] [Integrating Sequential and Relational Modeling for User Events: Datasets and Prediction Tasks](https://arxiv.org/abs/2510.11903)
...
<details>
  <summary>Details</summary>
Motivation: 现实系统中需要同时捕捉个人事件和关系事件，但现有工作通常将它们分开建模（序列方法处理个人事件，图方法处理关系事件），缺乏统一框架。

Method: 引入包含两种事件类型的数据集集合，提出统一的形式化方法，通过实证研究验证模型从结合两种事件类型中受益。
...
### [212] [Variational Mixture of Graph Neural Experts for Alzheimer's Disease Biomarker Recognition in EEG Brain Networks](https://arxiv.org/abs/2510.11917)
...
Result: 在两个不同数据集上进行亚型分类和严重程度分期评估，VMoGE在AUC指标上比最先进方法提升4%到10%，并通过专家权重提供可解释性见解。

Conclusion: VMoGE为痴呆症的全面诊断和监测提供了EEG生物标志物发现的有效工具，其可解释性分析有助于临床指标和神经病理学特征的关联分析。
...
### [213] [Indoor Localization using Compact, Telemetry-Agnostic, Transfer-Learning Enabled Decoder-Only Transformer](https://arxiv.org/abs/2510.11926)
*Nayan Sanjay Bhatia,Pranay Kocheta,Russell Elliott,Harikrishna S. Kuttivelil,Katia Obraczka*

Main category: cs.LG

TL;DR: Locaris是一个基于解码器大型语言模型的室内定位系统，将Wi-Fi接入点测量视为token，无需预处理即可处理原始Wi-Fi遥测数据，实现免校准的室内定位。
...
Method: 通过在不同Wi-Fi数据集上微调LLM，Locaris学习从原始信号到设备位置的轻量级可泛化映射，将AP测量视为token进行处理。

Result: 实验表明Locaris在各种遥测类型上匹配或超越现有技术，仅需少量校准点即可在新设备和部署场景中保持高精度，实现亚米级精度。

Conclusion: 紧凑型LLM可作为室内定位的免校准回归模型，在大规模部署中提供可扩展和鲁棒的跨环境性能。
...
### [214] [Efficient Restarts in Non-Stationary Model-Free Reinforcement Learning](https://arxiv.org/abs/2510.11933)
*Hiroshi Nonaka,Simon Ambrozak,Sofia R. Miskala-Dinc,Amedeo Ercole,Aviva Prins*

Main category: cs.LG

TL;DR: 提出了三种高效的重启范式来解决无模型非平稳强化学习中的重启设计问题，包括部分重启、自适应重启和选择性重启，显著提升了动态遗憾性能。


<details>
  <summary>Details</summary>
Motivation: 现有RestartQ-UCB算法存在两个核心问题：完全遗忘（重启后丢失所有环境信息）和预定重启（不考虑策略与当前环境动态的兼容性）。

Method: 引入了部分重启、自适应重启和选择性重启三种方法，对RestartQ-UCB和RANDOMIZEDQ算法进行改进。
...
### [215] [On efficiently computable functions, deep networks and sparse compositionality](https://arxiv.org/abs/2510.11942)
*Tomaso Poggio*

Main category: cs.LG

TL;DR: 该论文证明了高效图灵可计算性意味着存在组合稀疏的DAG表示和相应的神经网络近似器，能够达到目标精度。具体来说，如果函数在比特深度上多项式时间可计算，则存在有界扇入布尔电路和深度网络，其大小和深度都是多项式级别的。


<details>
  <summary>Details</summary>
Motivation: 研究高效可计算函数与神经网络表示之间的关系，探索如何将图灵可计算性转化为实际的神经网络结构，为深度学习理论提供基础。

Method: 通过将高效图灵可计算函数转换为有界扇入布尔电路，然后将每个门替换为恒定大小的神经模拟器，构建深度神经网络。

Result: 证明了对于任何在比特深度上多项式时间可计算的函数，都存在大小和深度为多项式的深度网络，能够达到指数级精度。

Conclusion: 高效图灵可计算性保证了组合稀疏表示的存在性，这为理解神经网络的表达能力及其与计算复杂度的关系提供了理论基础。
...
### [216] [Sculpting Latent Spaces With MMD: Disentanglement With Programmable Priors](https://arxiv.org/abs/2510.11953)
*Quentin Fruytier,Akshay Malhotra,Shahab Hamidi-Rad,Aditya Sant,Aryan Mokhtari,Sujay Sanghavi*

Main category: cs.LG

TL;DR: 本文提出了一种新的可编程先验框架来解决VAE中KL散度正则化器在解耦表示学习中的失效问题，使用MMD方法实现了最先进的解耦性能。


<details>
  <summary>Details</summary>
Motivation: 传统VAE框架中的KL散度正则化器在强制潜在空间匹配因子化高斯先验方面不可靠，无法有效实现解耦表示学习。

Method: 提出了基于最大均值差异(MMD)的可编程先验框架，允许显式塑造潜在空间分布，并引入了无监督的潜在可预测性评分(LPS)来量化纠缠程度。

Result: 在CIFAR-10和Tiny ImageNet等复杂数据集上实现了最先进的相互独立性，且没有常见的重建权衡问题，还能构建与语义特征更好对齐的先验。
...
### [217] [Y-shaped Generative Flows](https://arxiv.org/abs/2510.11955)
*Arip Asadulaev,Semyon Semenov,Abduragim Shtanchaev,Eric Moulines,Fakhri Karray,Martin Takac*

Main category: cs.LG

TL;DR: 提出Y形生成流，通过共享路径移动概率质量，再分支到特定端点，优于传统V形传输


<details>
  <summary>Details</summary>
Motivation: 现代连续时间生成模型常采用V形传输，样本独立沿直线轨迹移动，忽略了共享结构

Method: 引入基于速度驱动的传输成本，具有次线性指数，在可扩展的神经ODE训练目标中实例化Y形流

Result: 在合成、图像和生物学数据集上，Y流恢复层次感知结构，在分布指标上优于强流基线，用更少积分步骤达到目标

Conclusion: Y形生成流能有效利用共享结构，提高生成效率和质量
...
### [218] [MosaicDiff: Training-free Structural Pruning for Diffusion Model Acceleration Reflecting Pretraining Dynamics](https://arxiv.org/abs/2510.11962)
*Bowei Guo,Shengkun Tang,Cong Zeng,Zhiqiang Shen*

Main category: cs.LG

TL;DR: MosaicDiff是一个新的扩散模型加速框架，通过轨迹感知的结构剪枝将扩散预训练动态与后训练采样加速对齐。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的预训练过程存在不同的学习速度阶段，但之前的后训练加速方法完全忽略了这一点。

Method: 采用自适应剪枝机制：中间快速学习阶段采用保守剪枝以保留关键特征，早期和后期慢速学习阶段采用更激进的剪枝策略。

Result: 在DiT和SDXL上的实验表明，该方法在不影响输出质量的情况下显著加速采样，大幅优于先前的最先进方法。
...
### [219] [QLENS: Towards A Quantum Perspective of Language Transformers](https://arxiv.org/abs/2510.11963)
*Aditya Gupta,Kirandeep Kaur,Vinayak Gupta*

Main category: cs.LG

TL;DR: QLENS是一个基于量子力学框架的Transformer解释方法，将模型的隐状态转换为希尔伯特空间中的状态向量，通过将隐藏层重新定义为酉算子和哈密顿量来建模状态演化过程。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer理解方法仅作为诊断检查点，缺乏数学框架来建模各层如何促进状态转换。受物理学跨学科视角启发，利用语言模型的概率特性与量子力学核心假设的相似性。

Method: 将Transformer的隐激活转换为希尔伯特空间中的状态向量，将隐藏层重新定义为酉算子和哈密顿量，通过玻恩规则从最终状态获取概率分布。
...
### [220] [Learning Dynamics of VLM Finetuning](https://arxiv.org/abs/2510.11978)
*Jusheng Zhang,Kaitong Cai,Jing Yang,Keze Wang*

Main category: cs.LG

TL;DR: CW-DPO是一种两阶段的视觉语言模型对齐方法，通过温和负样本的监督微调阶段和带冷却权重的DPO阶段，稳定训练过程并提高性能。


<details>
  <summary>Details</summary>
Motivation: 传统的基于偏好的视觉语言模型微调方法很脆弱，错误的负样本会产生无信息的梯度，导致训练不稳定。

Method: 第一阶段使用温和负样本进行监督微调，第二阶段应用带冷却权重的DPO目标，冷却权重基于模型在负样本上的平均token对数概率计算，抑制无信息梯度。
...
### [221] [Learning by Steering the Neural Dynamics: A Statistical Mechanics Perspective](https://arxiv.org/abs/2510.11984)
*Mattia Scardecchia*

Main category: cs.LG

TL;DR: 该研究通过统计力学方法分析随机不对称循环网络中的动力学吸引子，揭示了自耦合强度对固定点结构的相变影响，并提出了一种生物合理的监督学习算法，该算法能够学习MNIST数据集并利用网络深度发展层次表示。


<details>
  <summary>Details</summary>
Motivation: 当前基于梯度优化的深度神经网络与生物学习机制存在根本差异，研究旨在探索自然如何实现鲁棒、样本高效且能耗最小的学习，以及如何在不使用反向传播的情况下解决信用分配问题。

Method: 使用统计力学工具分析随机不对称循环网络的动力学吸引子，推导固定点数量与自耦合强度的闭式表达式，并提出基于局部可塑性的生物合理监督学习算法，通过瞬态外部刺激将输入映射到动力学固定点。

Result: 发现了固定点结构的相变：在临界自耦合强度以下，孤立固定点与指数级数量的窄簇共存；以上则出现次主导但密集且广泛的簇。提出的算法能够学习纠缠版MNIST，利用深度发展层次表示并提高异联想能力。

Conclusion: 算法性能与揭示的相变密切相关，研究为连接当代AI与计算神经科学提供了桥梁，并提出了皮层启发的自耦合替代方案。
...
### [222] [Nonlinear discretizations and Newton's method: characterizing stationary points of regression objectives](https://arxiv.org/abs/2510.11987)
*Conor Rowan*

Main category: cs.LG

TL;DR: 论文发现使用精确二阶信息（真实Hessian矩阵）训练神经网络会可靠地失败，这与使用近似Hessian的拟牛顿法形成对比，揭示了非线性离散化的几何特性和损失景观中驻点分布的新见解。


<details>
  <summary>Details</summary>
Motivation: 研究二阶方法在神经网络训练中的应用，特别是比较使用精确Hessian矩阵与近似Hessian矩阵的效果差异，挑战关于损失景观充满局部最小值的传统观点。

Method: 通过实验分析使用精确二阶信息（真实Hessian矩阵）训练神经网络的失败模式，与拟牛顿方法进行对比研究。

Result: 使用精确Hessian矩阵的神经网络训练可靠地失败，而拟牛顿方法表现良好，这表明损失景观中可能没有传统认为的那么多局部最小值。

Conclusion: 精确二阶信息在神经网络训练中的失败揭示了非线性离散化的几何特性，并质疑了损失景观中充满局部最小值的传统观点，为理解神经网络优化提供了新视角。
...
### [223] [Mamaba Can Learn Low-Dimensional Targets In-Context via Test-Time Feature Learning](https://arxiv.org/abs/2510.12026)
*Junsoo Oh,Wei Huang,Taiji Suzuki*

Main category: cs.LG

TL;DR: 本文对Mamba模型的上下文学习能力进行理论分析，证明其能够通过测试时特征学习高效提取相关特征方向，在单索引模型任务中达到与非线性Transformer相当的性能。
...
Result: 证明Mamba能够通过测试时特征学习高效提取相关特征方向，其测试时样本复杂度优于线性Transformer（表现为核方法），与非线性Transformer相当，接近信息理论最优速率。
...
### [224] [Your VAR Model is Secretly an Efficient and Explainable Generative Classifier](https://arxiv.org/abs/2510.12060)
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式分类器主要依赖计算成本高的扩散模型，限制了可扩展性，且对生成式分类器的理解受限。

Method: 基于视觉自回归建模构建生成式分类器，提出自适应VAR分类器A-VARC+优化性能。

Result: A-VARC+在准确性和推理速度间实现优越平衡，具有可解释性且对灾难性遗忘具有天然抵抗力。

Conclusion: VAR方法为生成式分类器研究提供了新视角，具有实际应用价值，并展示了与扩散模型不同的特性。
...
### [225] [MEASURE: Multi-scale Minimal Sufficient Representation Learning for Domain Generalization in Sleep Staging](https://arxiv.org/abs/2510.12070)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习睡眠分期模型在未见受试者上泛化能力差，现有对比学习方法未能充分提取领域不变特征，需要减少领域相关信息同时保留重要时序和频谱特征。

Method: 提出MEASURE框架，通过多尺度最小充分表示学习，在多个特征层级上减少领域相关信息，同时保留睡眠分期所需的时序和频谱特征。

Result: 在SleepEDF-20和MASS公开睡眠分期基准数据集上，MEASURE方法持续优于最先进方法。

Conclusion: MEASURE框架能有效减少领域相关信息，同时保留睡眠分期所需的时序和频谱特征，在领域泛化方面表现优异。
...
### [226] [Influence Dynamics and Stagewise Data Attribution](https://arxiv.org/abs/2510.12071)
...
<details>
  <summary>Details</summary>
Motivation: 当前的数据归因方法将样本间影响力视为静态，但神经网络学习具有不同阶段，表现出变化的影响力模式。

Method: 基于奇异学习理论构建分阶段数据归因框架，通过玩具模型和语言模型验证影响力动态变化。

Result: 发现影响力可以非单调变化，包括符号翻转和在发展过渡期的急剧峰值；在语言模型中，token级影响力变化与已知发展阶段一致。

Conclusion: 神经网络学习过程中的影响力是动态变化的，分阶段数据归因框架能更好地捕捉这种变化模式。
...
### [227] [GraphShaper: Geometry-aware Alignment for Improving Transfer Learning in Text-Attributed Graphs](https://arxiv.org/abs/2510.12085)
*Heng Zhang,Tianyi Zhang,Yuling Shi,Xiaodong Gu,Yaomin Shen,Haochen You,Zijian Zhang,Yilei Yuan,Jin Huang*

Main category: cs.LG

TL;DR: GraphShaper是一个几何感知的图基础模型框架，通过多几何专业化解决现有方法在结构边界处的性能退化问题，在零样本设置下显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的图文本对齐方法假设所有图结构都能在单一欧几里得空间中编码，但现实中树结构需要双曲几何保持层次分支，循环模式依赖球面几何保持闭合特性。在结构边界处，节点面临冲突的几何约束，导致性能显著下降。
...
Conclusion: 几何感知的图编码框架能够有效解决结构边界处的性能退化问题，通过尊重图结构的内在几何多样性显著提升模型性能。
...
### [228] [H4G: Unlocking Faithful Inference for Zero-Shot Graph Learning in Hyperbolic Space](https://arxiv.org/abs/2510.12094)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本属性图的零样本学习方法在需要细粒度模式识别的任务上表现不佳，特别是在异配图上存在过抽象问题，导致关键局部模式信息丢失。

Method: 提出H4G框架，使用可学习的块对角缩放矩阵和Möbius矩阵乘法系统性地降低嵌入半径，保留细粒度结构细节同时保持全局感知能力。

Result: H4G在异配图上实现了12.8%的性能提升，在同配图上实现了8.4%的提升，达到了最先进的零样本性能。

Conclusion: 半径减少能够实现忠实的多尺度表示，是推进零样本图学习的关键因素。
...
### [229] [Rethinking the Role of Dynamic Sparse Training for Scalable Deep Reinforcement Learning](https://arxiv.org/abs/2510.12096)
*Guozheng Ma,Lu Li,Zilin Wang,Haoyu Wang,Shengchao Hu,Leszek Rutkowski,Dacheng Tao*

Main category: cs.LG

TL;DR: 本文提出模块特定训练(MST)框架，通过动态稀疏训练策略解决深度强化学习中模型扩展时的优化问题，在不同模块采用差异化策略，显著提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习中模型扩展往往导致性能下降，现有动态训练方法存在三个关键局限：对所有模块采用统一策略、评估基础架构不足、缺乏不同动态方法的系统比较。

Method: 通过全面研究不同模块和架构，提出模块特定训练(MST)框架，在不同模块采用差异化动态稀疏训练策略，包括稀疏到稀疏、稠密到稀疏、稀疏到稠密等方法。

Result: 研究发现动态稀疏训练策略提供模块特定优势，与架构改进形成互补，MST框架在不修改算法的情况下显著提升了各种RL算法的可扩展性。

Conclusion: 模块特定训练框架有效利用了架构改进的优势，通过差异化动态训练策略解决了深度强化学习中的可扩展性问题，为大规模RL模型提供了实用解决方案。
...
### [230] [Chimera: State Space Models Beyond Sequences](https://arxiv.org/abs/2510.12111)
*Aakash Lahoti,Tanya Marwah,Ratish Puduppully,Albert Gu*

Main category: cs.LG

TL;DR: Chimera是一个统一模型，通过状态空间模型直接融入数据拓扑结构，无需特定领域的偏置设计，在语言、视觉和图数据上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer方法依赖自注意力机制，将数据视为无序集合，忽略了数据的邻域结构或图拓扑，需要设计特定领域的偏置（如位置嵌入、随机游走）来融入拓扑结构，但这需要大量工作且可能引入副作用。

Method: 提出Chimera模型，将状态空间模型推广到任意图拓扑，直接融入数据拓扑结构。针对有向无环图可实现线性时间递归，对一般图通过数学松弛达到Transformer的二次复杂度。

Result: 在GLUE上比BERT高0.7分，在ImageNet-1k上比ViT高2.6%，在Long Range Graph Benchmark上超过所有基线方法。

Conclusion: Chimera验证了数据拓扑是跨模态的强大归纳偏置，提供了一种无需领域特定启发式的方法来统一处理不同数据模态。
...
### [231] [nuGPR: GPU-Accelerated Gaussian Process Regression with Iterative Algorithms and Low-Rank Approximations](https://arxiv.org/abs/2510.12128)
*Ziqi Zhao,Vivek Sarin*

Main category: cs.LG

TL;DR: nuGPR是一个新的高斯过程回归框架，通过数值线性代数技术显著降低了训练计算成本，包括使用预处理共轭梯度法、数据聚类识别块对角结构、数值梯度优化等，在GPU上实现2倍训练时间减少和12倍内存消耗降低。
...
Method: 结合多种数值线性代数技术：预处理共轭梯度法加速线性求解、数据聚类识别协方差矩阵块对角结构、构建非对角块低秩近似、使用数值梯度替代精确微分、CUDA并行化。

Result: 与现有最佳GPU GPR实现相比，训练时间减少最多2倍，峰值内存消耗降低最多12倍，在合成和真实数据集上验证了有效性。

Conclusion: nuGPR通过数值线性代数方法有效解决了GPR的计算瓶颈，为大规模高斯过程回归应用提供了实用的解决方案。
...
### [232] [Graph Few-Shot Learning via Adaptive Spectrum Experts and Cross-Set Distribution Calibration](https://arxiv.org/abs/2510.12140)
...
<details>
  <summary>Details</summary>
Motivation: 现有图少样本学习方法存在两个主要局限：1）使用预定义的统一图滤波器无法适应局部拓扑结构的异质性；2）假设支持集和查询集来自相同分布，但在少样本条件下可能无法充分捕捉查询集的复杂分布。

Method: GRACE框架整合了自适应频谱专家（Adaptive spectrum experts）和跨集分布校准（Cross-set distribution calibration）技术，能够适应局部结构变化并进行分布校准。

Result: 理论上增强了模型泛化能力，实证上在多种实验设置下持续优于现有最先进基线方法。

Conclusion: GRACE通过同时适应局部结构变化和跨集分布校准，有效解决了图少样本学习中的关键挑战，实现了更好的泛化性能。
...
### [233] [Fairness-Constrained Optimization Attack in Federated Learning](https://arxiv.org/abs/2510.12143)
*Harsh Kasyap,Minghong Fang,Zhuqing Liu,Carsten Maple,Somanath Tripathy*

Main category: cs.LG

TL;DR: 本文提出了一种针对联邦学习的故意公平性攻击，恶意客户端通过增加公平性损失来发送有偏见的模型，即使数据分布均匀也能有效增加系统偏见。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护数据隐私，但由于参与者对训练数据的独立性，容易受到投毒攻击。协作过程也会传播偏见，即使是无意的。本文旨在研究恶意客户端如何故意发起公平性攻击。

Method: 提出一种故意公平性攻击方法，恶意客户端在训练时增加公平性损失，通过求解公平性指标（如人口统计均等和均等化几率）的优化问题来计算公平性损失。

Result: 攻击效果显著，即使只有一个恶意客户端，也能将偏见增加高达90%，同时保持全局准确性不变，使得攻击难以被检测。

Conclusion: 这种公平性攻击具有隐蔽性和有效性，能够对抗现有的拜占庭鲁棒和公平感知聚合方案，揭示了联邦学习系统在公平性方面的脆弱性。
...
### [234] [Budget-constrained Active Learning to Effectively De-censor Survival Data](https://arxiv.org/abs/2510.12144)
*Ali Parsaee,Bei Jiang,Zachary Friggstad,Russell Greiner*

Main category: cs.LG

TL;DR: 该论文研究了在生存数据集中应用预算学习的方法，允许学习者在预算限制下支付费用来获取部分或完整的标签信息，特别是针对右删失实例。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的数据收集中，对删失患者的随访并不总是能解除删失状态，而学习者模型在数据收集过程中获得的信息量取决于预算和数据本身的性质。

Method: 将最先进的预算学习算法应用于生存数据，提供理论和实验分析，包括如何支付费用来获取删失实例的部分或完整标签信息。

Result: 该方法在多个生存任务上的实证分析表明，其性能优于其他潜在方法，且提供了与标准主动学习方法BatchBALD渐近等价的时间复杂度和边界。

Conclusion: 该研究为在生存数据中应用预算学习提供了有效的框架，能够在预算限制下优化模型性能，同时保持与现有方法相当的计算效率。
...
### [235] [Self-Verifying Reflection Helps Transformers with CoT Reasoning](https://arxiv.org/abs/2510.12157)
*Zhongwei Yu,Wannian Xia,Xue Yan,Bo Xu,Haifeng Zhang,Yali Du,Jun Wang*

Main category: cs.LG

TL;DR: 论文提出了一个最小化推理框架，支持小型transformer进行基本的自验证反思，证明自验证反思在验证误差有界时能保证性能提升，实验显示小模型在整数乘法和数独任务中达到LLM级别性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在推理中经常进行反思，但LLMs在思维链中检测错误的能力有限，反思如何带来实证改进仍不清楚。

Method: 提出了一个最小化推理框架，支持小型transformer进行基本的自验证反思，避免自然语言使用以确保分析清晰度并降低实验成本。

Result: 实验显示只有几百万参数的小型transformer在训练和反思执行中都受益于自验证，在整数乘法和数独任务中达到LLM级别性能。强化学习主要优化浅层统计模式而非真正减少验证误差。
...
### [236] [Revisiting Meta-Learning with Noisy Labels: Reweighting Dynamics and Theoretical Guarantees](https://arxiv.org/abs/2510.12209)
*Yiming Zhang,Chester Holtz,Gal Mishne,Alex Cloninger*

Main category: cs.LG

TL;DR: 本文对元学习样本重加权方法在标签噪声下的训练动态进行了理论分析，揭示了其三个训练阶段，并基于分析提出了一种轻量级替代方法。


<details>
  <summary>Details</summary>
Motivation: 现有的元学习样本重加权方法虽然能利用小规模干净子集指导训练，但其行为和训练动态缺乏理论理解，需要深入分析以改进性能。

Method: 通过理论分析元重加权的训练轨迹，提出包含均值中心化、行偏移和标签符号调制的轻量级替代方法，避免昂贵的双层优化。

Result: 在合成和真实标签噪声基准测试中，该方法持续优于强重加权/选择基线方法。

Conclusion: 元重加权的训练动态可分为三个明确阶段，基于此理解设计的轻量级方法能提供更稳定的性能，避免元学习的高计算成本。
...
### [237] [DE3S: Dual-Enhanced Soft-Sparse-Shape Learning for Medical Early Time-Series Classification](https://arxiv.org/abs/2510.12214)
*Tao Xie,Zexi Tan,Haoyi Xiao,Binbin Sun,Yiqun Zhang*

Main category: cs.LG

TL;DR: DE3S提出了一种用于医疗早期时间序列分类的双重增强软稀疏形状学习框架，通过双重增强策略、软形状稀疏化机制和双路径专家网络架构，解决了准确性和早期性之间的冲突，在六个真实医疗数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 医疗应用中的早期时间序列分类（如ICU中的脓毒症预测）至关重要，但面临准确性和早期性之间的冲突，现有方法往往牺牲一个来换取另一个，难以捕捉早期阶段的微弱信号和类别不平衡问题。

Method: 提出DE3S框架，包含三个创新：1）结合传统时间增强和基于注意力的全局时间增强的双重增强策略；2）基于注意力得分的软形状稀疏化机制；3）双路径MoE和Inception模块融合架构，MoE在形状内进行局部学习，多尺度Inception模块捕捉形状间的全局模式。

Result: 在六个真实医疗数据集上的广泛实验显示达到了最先进的性能，消融研究确认了各组件有效性。

Conclusion: DE3S通过精确识别形状，有效解决了医疗早期时间序列分类中准确性和早期性的冲突，提高了ICU资源利用效率和医疗精度。
...
### [238] [Hierarchical Koopman Diffusion: Fast Generation with Interpretable Diffusion Trajectory](https://arxiv.org/abs/2510.12220)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但迭代去噪过程导致采样速度慢。现有一步方法加速了推理但牺牲了扩散动态的可解释性和精细控制能力，而这些特性对于可编辑生成等应用至关重要。

Method: 基于Koopman算子理论，将非线性扩散动态提升到潜在空间，其中演化由全局线性算子控制，实现闭式轨迹解。设计分层架构，通过尺度特定的Koopman子空间解耦不同空间分辨率的生成动态，系统性捕捉从粗到细的细节。

Result: Hierarchical Koopman Diffusion不仅实现了竞争性的一步生成性能，还通过谱分析提供了理解和操作生成过程的原则性机制。
...
### [239] [Unveiling the Vulnerability of Graph-LLMs: An Interpretable Multi-Dimensional Adversarial Attack on TAGs](https://arxiv.org/abs/2510.12233)
*Bowen Fan,Zhilin Guo,Xunkai Li,Yihan Zhou,Bing Zhou,Zhenjun Li,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: 提出了IMDGA框架，一种可解释的多维图攻击方法，统一了对图结构和文本特征的多层次对抗攻击，揭示了Graph-LLMs的语义维度漏洞。


<details>
  <summary>Details</summary>
Motivation: Graph-LLMs虽然通过整合大语言模型增强了图表示能力，但面临结构和文本双重维度的对抗攻击威胁，现有方法缺乏统一的多维攻击框架。

Method: IMDGA框架包含三个紧密集成的模块，协调执行图结构和文本特征的多层次扰动，平衡可解释性和攻击效果。

Result: 在多种数据集和架构上的实验表明，IMDGA在可解释性、攻击效果、隐蔽性和鲁棒性方面均优于现有方法。

Conclusion: 这项工作揭示了Graph-LLMs中先前未被充分探索的语义维度脆弱性，为提升其抗攻击能力提供了重要见解。
...
### [240] [MoRA: On-the-fly Molecule-aware Low-Rank Adaptation Framework for LLM-based Multi-Modal Molecular Assistant](https://arxiv.org/abs/2510.12245)
*Tao Yin,Xiaohong Zhang,Jiacheng Zhang,Li Huang,Zhibin Zhang,Yuansong Zeng,Jin Xie,Meng Yan*

Main category: cs.LG

TL;DR: 提出了Molecule-aware Low-Rank Adaptation (MoRA)方法，为每个输入分子图生成独特的低秩适配权重，动态注入冻结的LLM中，实现实例特定的参数空间对齐。


<details>
  <summary>Details</summary>
Motivation: 现有多模态对齐方法在处理分子图结构时存在两个主要限制：(1) 优化所有分子输入的共享参数空间，限制了模型捕捉实例特定结构特征的能力；(2) 为分子任务微调LLM会导致灾难性遗忘，削弱其通用推理能力。

Method: MoRA为每个输入分子图生成独特的低秩适配权重，这些权重动态注入冻结的LLM中，使模型能够根据每个分子输入的结构调整其推理，同时保留LLM的核心知识。

Result: 在关键分子任务（如化学反应预测和分子描述生成）上，MoRA的实例特定动态适配优于静态适配基线，包括在反应预测精确匹配上相对提升14.1%，在量子属性预测上误差减少22%。

Conclusion: MoRA通过实例特定的动态适配方法，有效解决了分子图结构与LLM集成中的关键挑战，在保持LLM通用推理能力的同时显著提升了分子任务的性能。
...
### [241] [Optimal Regularization for Performative Learning](https://arxiv.org/abs/2510.12249)
*Edwige Cyffers,Alireza Mirrokni,Marco Mondelli*

Main category: cs.LG

TL;DR: 本文研究了在高维岭回归中，正则化如何帮助应对执行性学习中的分布漂移问题，发现在过参数化情况下执行性效应可能有益，并提出了基于执行性效应强度设置正则化参数的方法。


<details>
  <summary>Details</summary>
Motivation: 在执行性学习中，数据分布会随着部署的模型而动态变化（例如用户策略性调整特征），这比传统监督学习更复杂。需要同时考虑当前数据优化和模型可能引发的分布漂移。

Method: 通过高维岭回归框架研究正则化的影响，分析执行性效应在不同参数化情况下的表现，并在合成和真实数据集上进行实证评估。

Result: 执行性效应在总体设置下会恶化测试风险，但在过参数化情况下可能有益。最优正则化参数与执行性效应的整体强度成正比。

Conclusion: 正则化是应对执行性学习分布漂移的有效工具，可以通过预判执行性效应强度来设置合适的正则化参数，在过参数化情况下执行性效应甚至可能带来益处。
...
### [242] [Diffusion Models for Reinforcement Learning: Foundations, Taxonomy, and Development](https://arxiv.org/abs/2510.12253)
*Changfu Xu,Jianxiong Guo,Yuzhu Liang,Haiyang Huang,Haodong Zou,Xi Zheng,Shui Yu,Xiaowen Chu,Jiannong Cao,Tian Wang*

Main category: cs.LG

TL;DR: 这篇综述论文系统性地探讨了扩散模型在强化学习中的应用，建立了双轴分类法，涵盖了从单智能体到多智能体领域的进展，并分析了实际应用和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 扩散模型作为领先的生成模型，为强化学习提供了多模态表达能力、稳定训练和轨迹级规划等关键优势，需要系统性地总结这一新兴研究领域的进展。

Method: 建立了双轴分类法：功能导向分类法澄清扩散模型在RL流程中的作用，技术导向分类法定位在线与离线学习机制中的实现。同时从单智能体到多智能体领域进行全面分析。

Result: 形成了多个DM-RL集成框架，突出了它们的实际效用，并总结了扩散基RL在多个领域的成功应用类别。

Conclusion: 识别了有前景的未来发展方向，包括维护GitHub仓库持续更新相关论文和资源，以推进扩散模型在强化学习中的应用研究。
...
### [243] [FedMMKT:Co-Enhancing a Server Text-to-Image Model and Client Task Models in Multi-Modal Federated Learning](https://arxiv.org/abs/2510.12254)
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像模型在专业任务中因隐私限制导致数据不足的问题，同时利用移动系统和物联网基础设施中的多模态数据

Method: 使用联邦多模态知识转移框架，在保护数据隐私的前提下，利用分散的多模态数据协同增强服务器和客户端模型

Result: 框架能够在不泄露隐私的情况下有效利用分散的多模态数据

Conclusion: FedMMKT为文本到图像模型在隐私敏感场景下的专业化应用提供了可行的解决方案
...
### [244] [HiLoRA: Adaptive Hierarchical LoRA Routing for Training-Free Domain Generalization](https://arxiv.org/abs/2510.12266)
*Ziyi Han,Huanyu Wang,Zeyu Zhang,Xiangxiang Dai,Xutong Liu,John C. S. Lui*

Main category: cs.LG

TL;DR: HiLoRA是一个无需训练的分层路由框架，通过自适应选择LoRA模块和细化ROC分配，在领域泛化中显著提升性能，最高可达55%的准确率提升。
...
Method: 提出HiLoRA框架，基于LoRA的结构特性定义秩一组件(ROCs)，在序列级别自适应选择LoRA子集并确定ROC分配，在token级别进一步细化路由，仅激活最相关的ROCs。

Result: 在领域泛化任务中，HiLoRA相比最先进基线方法实现了高达55%的准确率提升，同时保持相当的推理吞吐量。

Conclusion: HiLoRA提供了一种无需训练的高效LoRA复用方法，通过分层路由机制显著提升了领域泛化性能，并具有理论保证。
...
### [245] [Multi-Action Self-Improvement for Neural Combinatorial Optimization](https://arxiv.org/abs/2510.12273)
*Laurin Luttmann,Lin Xie*

Main category: cs.LG

TL;DR: 本文提出了一种基于多智能体联合行动的自改进方法，通过预测完整的智能体-任务分配并利用集合预测损失来提升神经组合优化的效率和协调能力。
...
Result: 在多个组合问题上验证了方法的有效性，相比标准自改进方法，在最终解质量和生成延迟方面都取得了一致性改进。

Conclusion: 通过多智能体联合行动和集合预测损失，显著提升了自改进方法的样本效率和协调能力，同时加速了解生成过程。
...
### [246] [Deep SPI: Safe Policy Improvement via World Models](https://arxiv.org/abs/2510.12312)
...
<details>
  <summary>Details</summary>
Motivation: 现有安全策略改进理论主要关注离线、表格化强化学习，缺乏在通用在线设置中的理论保证，特别是在结合世界模型和表示学习的情况下。
...
Conclusion: 该研究成功将经典离线RL的安全策略改进理论扩展到在线深度强化学习，为结合世界模型和表示学习的策略改进提供了理论保证。
...
### [247] [Leveraging Teleconnections with Physics-Informed Graph Attention Networks for Long-Range Extreme Rainfall Forecasting in Thailand](https://arxiv.org/abs/2510.12328)
*Kiattikun Chobtham,Kanoksri Sarinnapakorn,Kritanai Torsri,Prattana Deeprasertkul,Jirawan Kamma*

Main category: cs.LG

TL;DR: 提出结合物理信息图神经网络与极值分析的新方法，改进泰国雨量站降雨预测，特别针对极端事件，优于现有基准模型。


<details>
  <summary>Details</summary>
Motivation: 准确预测降雨特别是极端事件在气候学和地球系统科学中仍是重大挑战，传统机器学习模型在极端事件预测方面存在局限。

Method: 使用图注意力网络与LSTM结合，构建雨量站图结构捕捉时空模式，采用基于地形降水物理的边缘特征，并开发空间季节感知GPD方法进行极值分析。

Result: 该方法在大多数区域包括极端事件易发区优于现有基准模型，与SEAS5操作预报系统相比改进了极端事件预测能力。

Conclusion: 该方法为长期水资源管理决策提供了实用的精细分辨率地图生成能力，是极端降雨预测的有效改进方案。
...
### [248] [Finite-time Convergence Analysis of Actor-Critic with Evolving Reward](https://arxiv.org/abs/2510.12334)
*Rui Hu,Yu Chen,Longbo Huang*

Main category: cs.LG

TL;DR: 首次对具有演化奖励函数的单时间尺度actor-critic算法进行有限时间收敛分析，证明在马尔可夫采样下可实现O(1/√T)收敛率，与静态奖励的最佳已知率匹配。


<details>
  <summary>Details</summary>
Motivation: 许多实用的强化学习算法使用演化奖励函数（如奖励塑造、熵正则化、课程学习），但缺乏理论基础。本文旨在为这些技术提供理论支撑。

Method: 采用单时间尺度actor-critic算法，考虑奖励参数在每个时间步可能变化的情况，在马尔可夫采样下分析策略优化和价值估计。

Result: 在标准假设下，推导了actor和critic误差的非渐近界，证明只要奖励参数演化足够慢，就能达到O(1/√T)收敛率。

Conclusion: 为许多流行的RL技术提供了理论基础，同时在静态奖励情况下改进了分布不匹配分析，将最佳已知率提高了log²T倍。
...
### [249] [Traveling Salesman-Based Token Ordering Improves Stability in Homomorphically Encrypted Language Models](https://arxiv.org/abs/2510.12343)
...
<details>
  <summary>Details</summary>
Motivation: 随着用户使用私有信息与LLM交互，需要安全加密通信。同态加密可直接在加密数据上计算，但文本生成特别是下一个令牌预测仍是实际加密交互的主要障碍

Method: 采用基于旅行商问题(TSP)的令牌重排序策略，结合后处理步骤进一步减少近似误差

Result: 理论分析和实验结果表明，该方法防止崩溃，提高生成文本的连贯性，并在整个过程中保护数据隐私

Conclusion: 我们的贡献推进了实用且保护隐私的LLM推理的可行性
...
### [250] [Towards Cross-Modal Error Detection with Tables and Images](https://arxiv.org/abs/2510.12383)
...
<details>
  <summary>Details</summary>
Motivation: 大型组织在确保数据质量方面面临挑战，特别是在多模态数据（如图像、表格和文本）共存的环境中，传统单模态错误检测方法难以发现跨模态错误。

Method: 在四个数据集上对五种基线方法进行基准测试，重点关注Cleanlab（标签错误检测框架）和DataScope（数据估值方法）与AutoML框架的结合使用。

Result: Cleanlab和DataScope在结合强大的AutoML框架时表现最佳，获得了最高的F1分数，但现有方法在处理重尾真实世界数据时仍有局限性。

Conclusion: 当前方法在多模态错误检测方面仍有不足，特别是在处理真实世界重尾数据时，这为未来研究提供了方向。
...
### [251] [Enhanced Pre-training of Graph Neural Networks for Million-Scale Heterogeneous Graphs](https://arxiv.org/abs/2510.12401)
*Shengyin Sun,Chen Ma,Jiehao Chen*

Main category: cs.LG

TL;DR: 提出了一种用于大规模异构图预训练的GNN框架，包含结构感知和语义感知两个预训练任务，以解决异构图预训练中的语义不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有的GNN预训练方法主要针对同构图设计，而真实世界图大多是异构图，且存在语义不匹配问题（原始数据与理想可迁移语义信息数据之间的差异）。

Method: 1. 结构感知预训练任务：捕获异构图中的结构特性；2. 语义感知预训练任务：构建由语义邻居组成的扰动子空间来处理语义不匹配，使模型更关注语义空间中的通用知识。

Result: 在真实世界大规模异构图上的大量实验表明，该方法优于现有最先进的基线方法。
...
### [252] [Cautious Weight Decay](https://arxiv.org/abs/2510.12402)
*Lizhang Chen,Jonathan Li,Kaizhao Liang,Baiyu Su,Cong Xie,Nuo Wang Pierse,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: Cautious Weight Decay (CWD) 是一种简单的优化器无关修改，只在参数坐标与优化器更新方向一致时应用权重衰减，无需额外超参数即可提升模型性能。
...
Method: CWD仅对与优化器更新方向一致的参数坐标应用权重衰减，可解释为双层优化问题，在到达驻点流形时诱导滑动模态行为。
...
### [253] [Continuous Uniqueness and Novelty Metrics for Generative Modeling of Inorganic Crystals](https://arxiv.org/abs/2510.12405)
*Masahiro Negishi,Hyunsoo Park,Kinga O. Mastej,Aron Walsh*

Main category: cs.LG

TL;DR: 提出两种连续距离函数来评估生成式AI模型在无机晶体材料生成中的独特性和新颖性，解决了传统距离函数的四个主要局限性。


<details>
  <summary>Details</summary>
Motivation: 当前用于评估晶体材料生成模型的距离函数存在四个主要问题：无法量化相似度、不能区分成分和结构差异、缺乏坐标平移的Lipschitz连续性、以及独特性指标对样本排列不具不变性。

Method: 提出了两种连续距离函数来评估生成模型的独特性和新颖性，这些距离函数理论上克服了传统方法的局限性。

Result: 实验表明，新提出的距离函数能够揭示传统距离函数遗漏的洞察，为评估和比较无机晶体生成模型提供了更可靠的基础。

Conclusion: 连续距离函数为生成式AI模型在功能材料发现中的评估提供了更准确和可靠的方法，有助于解决气候变化等重大科学挑战。
...
### [254] [Bayesian Optimization for Dynamic Pricing and Learning](https://arxiv.org/abs/2510.12447)
*Anush Anand,Pranav Agrawal,Tejas Bodas*

Main category: cs.LG

TL;DR: 提出基于高斯过程的非参数动态定价方法，使用贝叶斯优化来学习未知需求函数，在无限和有限库存场景下均优于传统强化学习方法
...
Method: 使用高斯过程建模需求函数作为黑盒函数，基于贝叶斯优化开发定价算法，针对无限和有限库存两种场景分别设计算法
...
### [255] [A Function Centric Perspective On Flat and Sharp Minima](https://arxiv.org/abs/2510.12451)
*Israel Mason-Williams,Gabryel Mason-Williams,Helen Yannakoudakis*

Main category: cs.LG

TL;DR: 本文重新审视了损失景观几何中平坦最小值与泛化性能的关系，发现正则化往往产生更尖锐的最小值，但这些尖锐最小值反而具有更好的泛化性、校准性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为平坦最小值与更好的泛化性能相关，但近期研究发现存在理论反例和经验例外，需要重新理解尖锐度在模型性能中的作用。

Method: 进行了广泛的实证研究，从单目标优化到现代图像分类任务，比较了使用不同正则化方法（如SAM、权重衰减、数据增强）时最小值的尖锐度与性能关系。

Result: 发现正则化模型往往收敛到更尖锐的最小值，但这些尖锐最小值在泛化性、校准性、鲁棒性和功能一致性方面表现更好；无正则化的基线模型收敛到更平坦的最小值但性能更差。

Conclusion: 函数复杂性而非平坦度单独决定了解的几何特性，尖锐最小值可以反映更合适的归纳偏置，需要对损失景观几何进行以函数为中心的重新评估。
...
### [256] [Time-Correlated Video Bridge Matching](https://arxiv.org/abs/2510.12453)
*Viacheslav Vasilev,Arseny Ivanov,Nikita Gushchin,Maria Kovaleva,Alexander Korotin*

Main category: cs.LG

TL;DR: 提出了TCVBM框架，将桥匹配扩展到时间相关视频序列，通过显式建模序列间依赖关系来提升视频生成任务中的时间连贯性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在复杂分布间的转换任务中表现不佳，而现有的桥匹配方法未探索时间相关数据序列的应用，这对需要保持时间连贯性的视频任务至关重要。

Method: TCVBM框架扩展桥匹配到视频领域，在扩散桥中显式建模序列间依赖关系，将时间相关性直接融入采样过程。

Result: 在帧插值、图像到视频生成和视频超分辨率三个任务中，TCVBM在多个定量指标上表现优于基于桥匹配和扩散模型的经典方法。

Conclusion: TCVBM通过建模时间相关性，显著提升了视频任务的生成质量和重建保真度，为时间相关数据序列的处理提供了有效解决方案。
...
### [257] [CrossAD: Time Series Anomaly Detection with Cross-scale Associations and Cross-window Modeling](https://arxiv.org/abs/2510.12489)
*Beibu Li,Qichao Shentu,Yang Shu,Hui Zhang,Ming Li,Ning Jin,Bin Yang,Chenjuan Guo*

Main category: cs.LG

TL;DR: 提出CrossAD框架，通过跨尺度关联和跨窗口建模进行时间序列异常检测，在多尺度建模中考虑动态的跨尺度关联，并克服固定窗口大小的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多尺度建模时往往独立处理各尺度信息或使用简单的特征融合策略，忽略了异常发生时跨尺度关联的动态变化，且大多基于固定滑动窗口，限制了捕获全面上下文信息的能力。

Method: 提出跨尺度重构方法，从粗粒度序列重构细粒度序列以显式捕获跨尺度关联；设计查询库并融入全局多尺度上下文，克服固定窗口大小的限制。

Result: 在多个真实世界数据集上使用九种评估指标进行的广泛实验验证了CrossAD的有效性，在异常检测方面表现出最先进的性能。
...
### [258] [PubSub-VFL: Towards Efficient Two-Party Split Learning in Heterogeneous Environments via Publisher/Subscriber Architecture](https://arxiv.org/abs/2510.12494)
*Yi Liu,Yang Liu,Leqian Zheng,Jue Hong,Junjie Shi,Qingyou Yang,Ye Wu,Cong Wang*

Main category: cs.LG

TL;DR: 提出PubSub-VFL，一种基于发布/订阅架构的新型垂直联邦学习范式，通过分层异步机制解决传统VFL训练效率低和资源利用率差的问题，在保持准确性的同时实现2-7倍训练加速和高达91.07%的资源利用率。


<details>
  <summary>Details</summary>
Motivation: 随着数字经济发展，组织间数据协作需求增长，但隐私问题使直接数据共享不可行。现有垂直联邦学习存在计算资源利用率低、训练效率差的问题，特别是同步依赖设计增加训练延迟，参与者的资源和数据异构性进一步阻碍高效计算。

Method: 利用发布/订阅架构的解耦能力和参数服务器架构的数据并行性，设计分层异步机制；基于参与者系统配置文件构建优化问题，选择最优超参数同时保护隐私；支持差分隐私等安全协议。

Result: 在五个基准数据集上的实验表明，相比最先进的基线方法，PubSub-VFL在不牺牲准确性的情况下实现2-7倍训练加速，计算资源利用率高达91.07%。

Conclusion: PubSub-VFL通过创新的发布/订阅架构有效解决了垂直联邦学习中的效率和资源利用问题，理论分析证明其具有稳定收敛性，且与安全协议兼容，为高效安全的协作学习提供了可行方案。
...
### [259] [Mitigating the Noise Shift for Denoising Generative Models via Noise Awareness Guidance](https://arxiv.org/abs/2510.12497)
...
<details>
  <summary>Details</summary>
Motivation: 现有去噪生成模型在采样过程中存在一个被忽视但普遍的问题：预定义噪声水平与中间状态实际编码的噪声水平不匹配，这种噪声偏移会导致次优生成结果。

Method: 提出噪声感知引导(NAG)方法，明确引导采样轨迹与预定义噪声调度保持一致；还引入了无分类器版本的NAG，通过噪声条件丢弃联合训练噪声条件模型和无噪声条件模型。

Result: 在ImageNet生成和各种监督微调任务上的广泛实验表明，NAG能持续缓解噪声偏移问题，显著提升主流扩散模型的生成质量。

Conclusion: 噪声偏移是扩散模型中的一个系统性问题，NAG提供了一种简单有效的校正方法，能显著改善生成质量。
...
### [260] [The Robustness of Differentiable Causal Discovery in Misspecified Scenarios](https://arxiv.org/abs/2510.12503)
...
<details>
  <summary>Details</summary>
Motivation: 因果发现算法依赖难以在真实数据中满足的因果假设，限制了其在实际场景中的广泛应用，因此需要评估算法在假设违反时的表现

Method: 广泛基准测试各种主流因果发现算法在八种模型假设违反情况下的经验性能，使用结构汉明距离和结构干预距离作为评估指标

Result: 可微因果发现方法在常用挑战场景下表现出鲁棒性（除了尺度变化），并提供了理论解释

Conclusion: 为因果发现的合理评估提供标准，促进其在真实世界场景中的应用
...
### [261] [Multi-Armed Bandits with Minimum Aggregated Revenue Constraints](https://arxiv.org/abs/2510.12523)
*Ahmed Ben Yahmed,Hafedh El Ferchichi,Marc Abeille,Vianney Perchet*

Main category: cs.LG

TL;DR: 该论文研究带上下文信息的多臂老虎机问题，目标是在确保每个臂在不同上下文下获得最小聚合奖励的同时最大化总累积奖励。该框架适用于需要公平收入分配且存在上下文变化的实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 解决现实应用中需要在上下文变化下实现公平收入分配的问题，其中跨上下文的最小奖励约束聚合虽然能提升性能和可行性，但带来了技术挑战，特别是缺乏标准MAB设置中的闭式最优分配。

Method: 设计并分析两种算法：乐观优先考虑性能的算法和悲观强制约束满足的算法。对每种算法推导了问题相关的遗憾和约束违反上界。

Result: 建立了问题相关的遗憾和约束违反上界，并证明时间跨度依赖关系在一般情况下是最优的，揭示了先前工作中利用的自由探索原则的基本局限性。

Conclusion: 该研究为带上下文和最小奖励约束的多臂老虎机问题提供了有效的算法解决方案，并建立了理论保证，同时揭示了此类问题的基本局限性。
...
### [262] [Evaluation of Real-Time Preprocessing Methods in AI-Based ECG Signal Analysis](https://arxiv.org/abs/2510.12541)
...
<details>
  <summary>Details</summary>
Motivation: 便携式ECG系统日益普及，需要隐私合规、能效高的实时分析方法，边缘计算可降低延迟并提高数据安全性
...
### [263] [Research in Collaborative Learning Does Not Serve Cross-Silo Federated Learning in Practice](https://arxiv.org/abs/2510.12595)
*Kevin Kuo,Chhavi Yadav,Virginia Smith*

Main category: cs.LG

TL;DR: 本文通过访谈研究探讨跨组织联邦学习(FL)在实际应用中的挑战，发现其面临模型性能担忧、组织间激励与信任问题等障碍，这些问题与跨设备FL有很大不同。


<details>
  <summary>Details</summary>
Motivation: 尽管GDPR和HIPAA等数据保护法规推动了跨组织联邦学习的兴趣，但其实际应用仍然有限。研究旨在了解阻碍跨组织联邦学习采用的实际挑战。

Method: 采用访谈研究方法，访谈了用户组织、软件提供商和学术研究人员等不同利益相关者。

Result: 研究发现跨组织联邦学习面临一系列尚未被现有研究充分捕捉的挑战，包括模型性能担忧、组织间激励和信任问题等。

Conclusion: 跨组织联邦学习面临独特的实践挑战，需要未来研究来克服这些障碍。
...
### [264] [Rethinking Knowledge Distillation: A Data Dependent Regulariser With a Negative Asymmetric Payoff](https://arxiv.org/abs/2510.12615)
*Israel Mason-Williams,Gabryel Mason-Williams,Helen Yannakoudakis*

Main category: cs.LG

TL;DR: 知识蒸馏的功能性影响被量化分析，发现其知识转移效果有限且存在负面知识不对称转移，更像数据依赖的规则化器而非压缩机制。
...
Method: 使用假设检验、控制组和随机控制蒸馏，分析跨数据模态的知识转移机制，探索多种蒸馏变体和模型规模扩展规律。

Result: 在某些模态和架构中存在统计显著的知识转移，但程度低于预期；显著转移时存在严重的负面知识不对称转移。

Conclusion: 知识蒸馏主要作为数据依赖的规则化器而非压缩机制，存在负面不对称风险，对安全应用构成担忧。
...
### [265] [Towards Fast Coarse-graining and Equation Discovery with Foundation Inference Models](https://arxiv.org/abs/2510.12618)
*Manuel Hinz,Maximilian Mauel,Patrick Seifner,David Berghaus,Kostadin Cvejoski,Ramses J. Sanchez*

Main category: cs.LG

TL;DR: 提出了一种使用预训练基础推理模型（FIMs）来解耦变量发现和动力学建模的方法，通过冻结FIM权重仅训练编码器-解码器映射，实现快速可重用的粗粒化流程。


<details>
  <summary>Details</summary>
Motivation: 高维动态过程通常由低维流形上的少量有效变量控制，传统方法需要同时解决变量发现和动力学建模两个耦合问题，这增加了学习难度。

Method: 利用预训练的FIMs在零样本模式下估计动态系统的无穷小生成器，冻结FIM权重，仅训练编码器-解码器映射，定义简单的模拟一致性损失来稳定表示学习。

Result: 在具有半圆扩散的随机双阱系统（嵌入合成视频数据）上的概念验证展示了该方法在快速可重用粗粒化流程方面的潜力。

Conclusion: 通过解耦变量发现和动力学建模，利用预训练FIMs可以简化表示学习过程，为动态系统建模提供更稳定和高效的解决方案。
...
### [266] [Learning-To-Measure: In-context Active Feature Acquisition](https://arxiv.org/abs/2510.12624)
*Yuta Kobayashi,Zilin Jing,Jiayu Yao,Hongseok Namkoong,Shalmali Joshi*

Main category: cs.LG

TL;DR: 提出Learning-to-Measure (L2M)方法解决元主动特征获取问题，通过可靠的跨任务不确定性量化和基于条件互信息的贪婪特征获取策略，在稀疏标签和高缺失率场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统主动特征获取方法通常针对单一预定任务，缺乏可扩展性。现实应用中需要从具有系统性特征缺失和有限任务特定标签的回顾性数据中学习。

Method: L2M包含：1）对未见任务的可靠不确定性量化；2）基于不确定性的贪婪特征获取代理，最大化条件互信息。采用序列建模或自回归预训练方法支持任意缺失模式下的可靠不确定性量化。

Result: 在合成和真实世界表格基准测试中，L2M匹配或超越了任务特定基线方法，特别是在稀疏标签和高缺失率情况下表现突出。

Conclusion: L2M能够直接在具有回顾性缺失的数据集上运行，通过上下文执行元主动特征获取任务，无需每个任务重新训练，实现了跨任务的有效特征获取策略学习。
...
### [267] [Laminar: A Scalable Asynchronous RL Post-Training Framework](https://arxiv.org/abs/2510.12633)
...
TL;DR: Laminar是一个可扩展的强化学习后训练系统，通过完全解耦架构解决了RL训练中轨迹生成延迟高度倾斜导致的GPU利用率低下问题。


<details>
  <summary>Details</summary>
Motivation: 现有RL框架在大型集群上扩展性有限，由于RL轨迹生成的极端长尾偏斜导致严重GPU利用率不足。全局权重同步机制不适合RL训练中高度倾斜且不断演变的轨迹生成延迟分布。

Method: 1. 用中继工作者层替代全局更新，实现分布式参数服务，支持异步细粒度权重同步；2. 动态重新打包机制将长尾轨迹整合到专用rollout上；3. 完全解耦设计隔离故障。
...
### [268] [Expert or not? assessing data quality in offline reinforcement learning](https://arxiv.org/abs/2510.12638)
*Arip Asadulaev,Fakhri Karray,Martin Takac*

Main category: cs.LG

TL;DR: 提出Bellman Wasserstein距离(BWD)来评估离线强化学习数据集质量，无需训练智能体。BWD通过价值感知的最优传输分数衡量数据集行为策略与随机策略的差异，能有效预测离线RL算法性能并可作为正则化器提升策略优化效果。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习数据集质量差异很大，但难以预先评估。现有方法需要训练智能体才能判断数据集质量，这在实际应用中不切实际。需要一种无需训练就能评估数据集质量的方法。

Method: 提出Bellman Wasserstein距离(BWD)，这是一种价值感知的最优传输分数。通过行为评论家和状态条件最优传输公式计算，无需环境交互或完整策略优化。BWD衡量数据集行为策略与随机参考策略的差异程度。

Result: 在D4RL MuJoCo任务上的实验表明，BWD与多个离线RL算法的性能得分强相关，能有效预测标准智能体在给定数据集上的表现。将BWD作为正则化器集成到策略优化中，能显式地将学习策略推离随机行为并提高回报。

Conclusion: 价值感知的分布信号如BWD是离线RL数据集筛选和策略优化的实用工具，能够在不训练智能体的情况下有效评估数据集质量并提升学习效果。
...
### [269] [On Foundation Models for Temporal Point Processes to Accelerate Scientific Discovery](https://arxiv.org/abs/2510.12640)
*David Berghaus,Patrick Seifner,Kostadin Cvejoski,Ramses J. Sanchez*

Main category: cs.LG

TL;DR: 提出了一种基于基础模型的事件序列分析方法，无需为每个新数据集重新训练模型，通过少量示例即可分析新数据。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法需要为每个新数据集从头构建和训练模型，过程缓慢且成本高昂，限制了科学研究的效率。

Method: 在数百万模拟事件序列上训练一个基础模型，使其学习事件数据的通用模式，能够通过上下文理解和少量示例分析新数据。

Result: 该模型能够即时分析新的科学数据而无需重新训练，仅需查看数据集中的少量示例，并且可以快速微调以获得更高精度。
...
### [270] [Towards Foundation Inference Models that Learn ODEs In-Context](https://arxiv.org/abs/2510.12650)
*Maximilian Mauel,Manuel Hinz,Patrick Seifner,David Berghaus,Ramses J. Sanchez*

Main category: cs.LG

TL;DR: FIM-ODE是一种预训练的神经模型，能够从稀疏和噪声观测数据中零样本推断常微分方程（ODEs），在合成数据上训练，使用灵活的神经算子进行鲁棒的ODE推断。


<details>
  <summary>Details</summary>
Motivation: 常微分方程（ODEs）描述连续时间中的确定性动态系统，但基于数据驱动的ODE建模在数据稀疏或噪声时仍然具有挑战性。

Method: 引入FIM-ODE模型，使用预训练的神经算子从稀疏和噪声观测中零样本推断ODEs，在合成数据上进行训练。

Result: 实证验证表明FIM-ODE提供准确的估计，与神经最先进方法相当，并定性比较了它们估计的向量场结构。

Conclusion: FIM-ODE能够有效从稀疏噪声数据中推断ODEs，为动态系统建模提供了一种有前景的方法。
...
### [271] [SG-XDEAT: Sparsity-Guided Cross-Dimensional and Cross-Encoding Attention with Target-Aware Conditioning in Tabular Learning](https://arxiv.org/abs/2510.12659)
*Chih-Chuan Cheng,Yi-Ju Tseng*

Main category: cs.LG

TL;DR: SG-XDEAT是一个用于表格数据监督学习的新框架，通过双流编码器将特征分解为原始值和目标感知两个并行表示，结合跨维度注意力、跨编码注意力和自适应稀疏注意力机制，在多个基准测试中优于强基线方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决表格数据中噪声影响和特征表示不充分的问题，需要同时建模原始特征和目标感知特征，并自适应地过滤噪声，以获得更鲁棒的深度学习模型。

Method: 使用双流编码器分解特征为原始值流和目标感知流，通过跨维度自注意力捕获流内特征依赖，跨编码自注意力实现双向交互，自适应稀疏自注意力机制动态抑制低效用标记。

Result: 在多个公共基准测试中取得了持续的性能提升，证实了同时建模原始和目标感知视图并自适应过滤噪声的有效性。

Conclusion: SG-XDEAT通过联合建模原始和目标感知特征视图，并自适应过滤噪声，实现了更鲁棒的表格数据深度学习框架。
...
### [272] [Demystifying Hybrid Thinking: Can LLMs Truly Switch Between Think and No-Think?](https://arxiv.org/abs/2510.12680)
*Shouren Wang,Wang Yang,Xianxuan Long,Qifan Wang,Vipin Chaudhary,Xiaotian Han*

Main category: cs.LG

TL;DR: 当前混合思维LLMs存在模式分离不彻底问题，研究发现四个关键因素影响可控性，并提出改进方法显著减少无思维模式中的推理行为。


<details>
  <summary>Details</summary>
Motivation: 混合思维LLMs在效率和推理能力之间提供平衡，但实验发现当前模型存在推理行为泄漏到无思维模式的问题，需要理解和缓解这一现象。

Method: 分析影响可控性的关键因素，提出包含四个要素的实用方法：更大数据规模、使用不同问题的思维和无思维答案、适度增加无思维数据量、两阶段训练策略。

Result: 相比标准训练，新方法在保持两种模式准确性的同时，显著减少无思维输出长度（从1085降至585）和推理支持性标记出现次数（从5917降至522）。

Conclusion: 研究揭示了当前混合思维的局限性，为增强其可控性提供了方向。
...
### [273] [Structured Sparsity and Weight-adaptive Pruning for Memory and Compute efficient Whisper models](https://arxiv.org/abs/2510.12666)
...
Method: 使用稀疏组LASSO惩罚作为损失正则化器来强制执行结构化稀疏性，并提出权重统计感知剪枝算法，同时设计了自定义文本标准化器用于WER评估。
...
Conclusion: 该框架成功实现了Whisper模型的压缩和优化，在保持性能的同时显著提升了部署效率，特别适合边缘设备应用。
...
### [274] [Structure-Aware Spectral Sparsification via Uniform Edge Sampling](https://arxiv.org/abs/2510.12669)
*Kaiwen He,Petros Drineas,Rajiv Khanna*

Main category: cs.LG

TL;DR: 本文证明了在具有良好分离的k聚类结构的图上，均匀边采样可以替代传统基于有效电阻的重要性采样方法，用于保持谱聚类质量。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类方法依赖特征向量计算，在大规模图上扩展性受限。现有的谱稀疏化方法需要计算有效电阻，预处理成本高昂。本文研究是否简单的均匀边采样策略能够满足谱聚类的需求。

Method: 使用均匀边采样策略，采样O(γ²n log n/ε²)条边，其中γ是拉普拉斯条件数。通过引入新的簇内边电阻边界、秩(n-k)有效电阻公式，以及针对主导特征空间的矩阵Chernoff边界，绕过了重要性采样。

Result: 证明了在具有大结构比Υ(k)=λ_{k+1}/ρ_G(k)的图上，均匀采样能够保持用于聚类的谱子空间，确保谱嵌入的忠实性和聚类质量的保持。

Conclusion: 在强聚类性假设下，即使是均匀采样也具有结构感知能力，这为均匀边采样在结构保持谱聚类中的有效性提供了首个可证明的保证。
...
### [275] [Keep Calm and Avoid Harmful Content: Concept Alignment and Latent Manipulation Towards Safer Answers](https://arxiv.org/abs/2510.12672)
...
### [276] [CoRA: Covariate-Aware Adaptation of Time Series Foundation Models](https://arxiv.org/abs/2510.12681)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型主要在单变量时间序列上预训练，忽视了现实预测任务中多变量协变量的重要信息，限制了模型性能。

Method: 保持预训练基础模型主干作为冻结特征提取器，使用Granger因果嵌入自动评估协变量的因果预测能力，通过零初始化条件注入机制逐步整合外生信息。

Result: 在协变量感知预测任务上实现了31.1%的MSE降低，在完整或少量训练样本下均优于最先进的协变量感知深度预测模型。

Conclusion: CoRA框架与各种先进TSFM兼容性强，能够扩展到多模态协变量，为TSFM的实际应用提供了实用范例。
...
### [277] [Few Shot Semi-Supervised Learning for Abnormal Stop Detection from Sparse GPS Trajectories](https://arxiv.org/abs/2510.12686)
*Muhammad Ayub Sabir,Junbiao Pang,Jiaqi Wu,Fatima Ashraf*

Main category: cs.LG

TL;DR: 提出一种针对城际客车异常停靠检测的稀疏感知方法，通过自适应分段、局部时间指标引导调整和图神经网络，在稀疏GPS数据和有限标签条件下实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 城际客车异常停靠检测对乘客安全和运营可靠性至关重要，但面临GPS轨迹稀疏和标签数据有限的挑战，现有方法在稀疏采样条件下效果不佳。

Method: 1. 稀疏感知分段(SAS)基于局部时空密度自适应定义边界；2. 局部时间指标引导调整(LTIGA)平滑异常停靠指标；3. 构建时空图，使用标签传播和GCN学习关系模式；4. 自训练模块集成高置信度伪标签迭代改进预测。

Result: 在真实客车数据上的实验显示，仅使用10个标注实例就达到了AUC 0.854和AP 0.866，优于现有方法。

Conclusion: 该方法有效解决了GPS数据稀疏和标签有限的问题，为城际客车异常停靠检测提供了实用的解决方案。
...
### [278] [DiffEM: Learning from Corrupted Data with Diffusion Models via Expectation Maximization](https://arxiv.org/abs/2510.12691)
*Danial Hosseintabar,Fan Chen,Giannis Daras,Antonio Torralba,Constantinos Daskalakis*

Main category: cs.LG

TL;DR: 提出DiffEM方法，使用期望最大化算法从损坏数据训练扩散模型，通过条件扩散模型重建干净数据并迭代优化模型


<details>
  <summary>Details</summary>
Motivation: 扩散模型在高维逆问题中表现出色，但当只有损坏或噪声观测数据可用时，学习这些模型仍然具有挑战性

Method: DiffEM方法结合期望最大化算法：E步使用条件扩散模型从观测重建干净数据，M步使用重建数据优化条件扩散模型

Result: 理论上提供了DiffEM迭代的单调收敛保证，实验证明该方法在多种图像重建任务中有效

Conclusion: DiffEM方法能够从损坏数据成功训练扩散模型，为高维逆问题提供了有效的生成先验学习方案
...
### [279] [Topological Signatures of ReLU Neural Network Activation Patterns](https://arxiv.org/abs/2510.12700)
*Vicente Bosca,Tatum Rask,Sunia Tanweer,Andrew R. Tawfeek,Branden Stone*

Main category: cs.LG

TL;DR: 本文研究了ReLU神经网络激活模式的拓扑特征，分析了网络在特征空间中诱导的多面体分解，发现Fiedler划分与决策边界相关，且训练损失与多面体单元数量之间存在相似行为模式。


<details>
  <summary>Details</summary>
Motivation: 探索ReLU神经网络激活模式的拓扑结构特征，理解网络在特征空间中的几何分解如何与分类决策边界和训练动态相关联。

Method: 分析ReLU网络在特征空间中诱导的多面体分解，研究其对偶图的Fiedler划分，计算细胞分解的同调群，并观察训练过程中单元数量与损失函数的关系。

Result: 在二分类任务中，Fiedler划分与决策边界存在相关性；在回归任务中，训练损失与多面体单元数量在训练过程中表现出相似的行为模式。

Conclusion: ReLU神经网络的激活模式具有显著的拓扑特征，这些特征与网络的决策边界和训练动态密切相关，为理解神经网络的内在几何结构提供了新的视角。
...
### [280] [Multitask finetuning and acceleration of chemical pretrained models for small molecule drug property prediction](https://arxiv.org/abs/2510.12719)
*Matthew Adrian,Yunsie Chung,Kevin Boyd,Saee Paliwal,Srimukh Prasad Veccham,Alan C. Cheng*

Main category: cs.LG

TL;DR: 本文研究了在化学预训练图神经网络模型（如KERMT和KGPT）上进行多任务微调的方法，发现在药物发现应用中，多任务微调显著优于非预训练模型，且在大数据量时性能提升最显著。


<details>
  <summary>Details</summary>
Motivation: 化学预训练模型在药物发现中具有重要应用价值，通过自监督学习提取的通用化学知识可改善关键药物发现端点的预测，包括靶点效力和ADMET性质。多任务学习已被证明能提升预测模型性能。

Method: 采用多任务微调方法，在化学预训练图神经网络模型（KERMT和KGPT）上进行多任务学习，并与非预训练图神经网络模型进行比较。

Result: 多任务微调显著提升了性能，特别是在大数据量情况下效果最明显。同时发布了两个多任务ADMET数据集分割，用于更准确的基准测试。

Conclusion: 多任务微调化学预训练模型是提升药物性质预测性能的有效方法，特别是在大规模数据场景下。提供了KERMT模型的加速实现，支持工业级药物发现工作流程。
...
### [281] [CARVQ: Corrective Adaptor with Group Residual Vector Quantization for LLM Embedding Compression](https://arxiv.org/abs/2510.12721)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的词嵌入层参数庞大，导致存储需求高和内存占用大，特别是在边缘设备上部署时面临内存限制问题。

Method: 使用线性与非线性映射组合的校正适配器结合分组残差向量量化，无需专用硬件支持低比特存储。

Result: 在多个预训练LLM上测试，CARVQ在大多数情况下能实现更低的平均比特数/参数，同时保持合理的困惑度和准确率。

Conclusion: 该方法与最先进的Transformer量化方法兼容，可在支持4位内存的硬件上无缝集成，是实现LLM在边缘设备高效部署的关键步骤。
...
### [282] [Improving Decision Trees through the Lens of Parameterized Local Search](https://arxiv.org/abs/2510.12726)
*Juha Harviainen,Frank Sommer,Manuel Sorge*

Main category: cs.LG

TL;DR: 该论文研究了决策树学习中的局部搜索操作（调整切割阈值或交换特征）对分类错误最小化的影响，发现这些问题在一般情况下是NP完全的，但通过参数化复杂度分析找到了固定参数可解的条件。


<details>
  <summary>Details</summary>
Motivation: 研究决策树学习算法中常见的启发式局部搜索操作（调整切割阈值和交换特征）对最小化分类错误的影响，旨在理解这些问题的计算复杂性和可解性条件。

Method: 采用参数化复杂度分析方法，考虑特征数量d和域大小D等参数，证明问题在一般情况下是NP完全的，但找到固定参数可解的条件。

Result: 发现当特征数量d和域大小D都较小时，问题具有固定参数可解性，可在(D+1)^(2d) * |I|^O(1)时间内求解。提供了概念验证实现和实证结果。

Conclusion: 决策树局部搜索操作的最小化分类错误问题在一般情况下是NP完全的，但当特征数量和域大小都较小时具有固定参数可解性，这为实际应用提供了理论指导。
...
### [283] [Hierarchical Federated Learning for Crop Yield Prediction in Smart Agricultural Production Systems](https://arxiv.org/abs/2510.12727)
*Anas Abouaomar,Mohammed El hanjri,Abdellatif Kobbane,Anis Laouiti,Khalid Nafil*

Main category: cs.LG

TL;DR: 提出了一种用于智能农业和作物产量预测的分层联邦学习架构，通过季节性订阅机制将农场按作物类型分组，实现局部专业化训练和全局知识整合。


<details>
  <summary>Details</summary>
Motivation: 解决农业环境中数据隐私保护、通信开销以及异质农场环境的挑战，同时实现作物产量预测的准确性和适应性。

Method: 采用三层架构：客户端智能农场、作物特定聚合器和全局模型聚合器，通过季节性订阅机制组织农场加入作物特定集群进行协作训练。

Result: 实验表明局部和作物层模型能够紧密跟踪实际产量模式，显著优于标准机器学习模型，验证了分层联邦学习在农业环境中的优势。

Conclusion: 分层联邦学习架构在农业应用中有效平衡了局部专业化和全局泛化，同时保护数据隐私并减少通信开销，特别适用于异质农场环境和隐私敏感数据场景。
...
### [284] [Doctor Rashomon and the UNIVERSE of Madness: Variable Importance with Unobserved Confounding and the Rashomon Effect](https://arxiv.org/abs/2510.12734)
*Jon Donnelly,Srikar Katta,Emanuele Borgonovo,Cynthia Rudin*

Main category: cs.LG

TL;DR: UNIVERSE方法通过使用Rashomon集合（近似最优模型集合）来估计变量重要性，即使在存在未观测变量的情况下也能提供真实变量重要性的边界估计。


<details>
  <summary>Details</summary>
Motivation: 传统变量重要性方法依赖于单一模型和观测到的特征，但实际中重要变量常被遗漏，且不同模型估计的变量重要性存在差异（Rashomon效应）。

Method: 利用Rashomon集合（数据集中近似最优模型的集合）来生成变量重要性的边界，即使存在缺失特征也能工作。

Result: 理论保证了方法的鲁棒性，在半合成模拟中表现良好，并在信用风险任务中展示了实用性。

Conclusion: UNIVERSE方法提供了一种更稳健的变量重要性估计框架，能够处理未观测变量和模型不确定性。
...
### [285] [KoALA: KL-L0 Adversarial Detector via Label Agreement](https://arxiv.org/abs/2510.12752)
*Siqi Li,Yasser Shoukry*

Main category: cs.LG

TL;DR: KoALA是一种无需语义信息、无需架构修改或对抗训练的新型对抗性攻击检测器，通过KL散度和L0相似度两种互补指标的类别预测分歧来检测对抗攻击。
...
Method: 使用KL散度（对密集低幅变化敏感）和L0相似度（对稀疏高影响变化敏感）两种互补指标，当它们的类别预测不一致时检测对抗攻击。仅需在预训练图像编码器上使用干净图像进行简单微调。

Result: 在ResNet/CIFAR-10上达到0.94精度和0.81召回率，在CLIP/Tiny-ImageNet上达到0.66精度和0.85召回率。当定理条件满足时，能持续有效检测对抗样本。

Conclusion: KoALA是一种轻量级、即插即用的对抗攻击检测解决方案，适用于现有模型和各种数据模态，无需架构修改或对抗训练。
...
### [286] [Sample-Efficient Omniprediction for Proper Losses](https://arxiv.org/abs/2510.12769)
*Isaac Gibbs,Ryan J. Tibshirani*

Main category: cs.LG

TL;DR: 本文研究如何构建概率预测器，使其在下游用户决策时能产生准确决策。针对多决策者场景，提出了比现有方法更高效的omniprediction算法。


<details>
  <summary>Details</summary>
Motivation: 现有omniprediction方法存在局限性：基于多校准的方法样本复杂度次优，基于对抗博弈的方法会产生复杂随机化预测器。需要设计更直接、非随机化的高效算法。

Method: 提出了一种直接的非随机化算法，利用proper loss集合的结构特性，避免了在线到批处理的转换，改进了现有对抗博弈方法的复杂性。

Result: 证明了多校准比omniprediction更难，前者必须承受次优样本复杂度。新算法在样本效率上优于现有方法，且返回更简单的确定性预测器。

Conclusion: 通过利用proper loss的结构特性，可以设计出比现有方法更直接、更高效的omniprediction算法，避免了复杂随机化预测器的问题。
...
### [287] [Rationally Analyzing Shelby: Proving Incentive Compatibility in a Decentralized Storage Network](https://arxiv.org/abs/2510.11866)
*Michael Crystal,Guy Goren,Scott Duke Kominers*

Main category: cs.GT

TL;DR: 本文对Shelby存储网络协议进行了首次正式激励分析，证明其结合链下审计与偶尔链上验证的机制在自然参数设置下具有激励兼容性。


<details>
  <summary>Details</summary>
Motivation: 去中心化存储是区块链和Web3生态的核心应用，但大多数存储协议的激励特性缺乏正式分析。激励设计不当可能导致系统无法真正实现去中心化存储。

Method: 使用博弈论模型分析Shelby协议，研究链下审计与链上验证的组合机制，并考察联盟行为。

Result: 仅使用链下审计会导致普遍逃避责任，但Shelby结合对等审计与偶尔链上验证的机制在自然参数下实现了激励兼容。还提出了增强抗合谋性的简单修改方案。

Conclusion: Shelby协议通过精心设计的激励机制实现了去中心化存储的有效运行，为存储协议激励分析提供了首个正式证明框架。
...
### [288] [Fair Division of Indivisible Items](https://arxiv.org/abs/2510.12158)
*Kevin Hsu*

Main category: cs.GT

TL;DR: 本文研究了不可分割物品的公平分配问题，包括一般模型和图形模型。主要贡献包括：证明了当物品数量不超过代理人数加5时，混合物品的MMS分配存在性；证明了判断多图中是否存在EFX分配是NP完全问题；给出了判断图中是否存在EF1和EFX分配的线性时间算法。


<details>
  <summary>Details</summary>
Motivation: 研究不可分割物品的公平分配问题，特别是探索在一般模型和图形模型下满足MMS、EF1、EFX等公平标准的分配方案的存在性和计算复杂性。

Method: 采用理论分析和算法设计的方法，包括：1) 分析混合物品MMS分配的存在条件；2) 研究多图中EFX分配的计算复杂性；3) 设计判断图中EF1和EFX分配存在性的多项式时间算法。

Result: 1) 当m≤n+5时，混合物品的MMS分配存在，除非所有代理的MMS阈值都为负；2) 判断多图中是否存在EFX分配是NP完全问题；3) 对于图形模型中的杂务，EF1和EFX分配的存在性可以在多项式时间内判定。

Conclusion: 本文在公平分配领域取得了重要进展，揭示了混合物品MMS分配的存在条件，证明了多图中EFX分配问题的计算复杂性，并发现了杂务与物品在公平分配问题上的根本差异。
...
### [289] [Single-Deviation Stability in Additively Separable Hedonic Games with Constrained Coalition Sizes](https://arxiv.org/abs/2510.12641)
*Martin Bullinger,Adam Dunajski,Edith Elkind,Matan Gilboa*

Main category: cs.GT

TL;DR: 该论文研究了在联盟规模有固定上下界限制的可加可分离享乐博弈中的稳定性问题，分析了四种基于单智能体偏离的稳定性概念及其变体，并给出了关于稳定结果存在性的完整图景和计算复杂性分析。


<details>
  <summary>Details</summary>
Motivation: 研究在联盟规模受限的可加可分离享乐博弈中稳定结果的存在性问题，这对于理解现实世界中团队形成和联盟构建的稳定性具有重要意义。

Method: 考虑四种经典稳定性概念（纳什稳定性、个体稳定性、契约纳什稳定性、契约个体稳定性）及其两个变体，分析在不同规模参数下稳定结果的存在性，并在只有上界限制时研究相关存在性问题的计算复杂性。

Result: 提供了关于给定规模参数下稳定结果存在性的完整图景，获得了契约个体稳定性和契约纳什稳定性的多项式时间算法（后者要求上界为2），当下界至少为2时，还获得了纳什稳定性和契约个体稳定性的进一步结果。

Conclusion: 在联盟规模受限的可加可分离享乐博弈中，不同稳定性概念的存在性和计算复杂性呈现出丰富的理论特征，为理解受限环境下的稳定联盟形成提供了重要理论支撑。
...
### [290] [Embedding the Teacher: Distilling vLLM Preferences for Scalable Image Retrieval](https://arxiv.org/abs/2510.12014)
*Eric He,Akash Gupta,Adian Liusie,Vatsal Raina,Piotr Molenda,Shirom Chabra,Vyas Raina*

Main category: cs.IR

TL;DR: 提出一种将强大视觉语言模型的偏好排名蒸馏到嵌入系统的框架，在保持嵌入方法推理效率的同时，提升个性化文本-图像检索能力。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入方法（如CLIP）主要基于字面描述训练，无法有效处理产品推荐中的抽象或角色驱动属性；而视觉语言模型虽然能灵活对齐文本和图像，但受限于上下文窗口无法直接处理大规模检索。

Method: 通过知识蒸馏框架，将视觉语言模型的偏好排名能力转移到嵌入系统中，结合两者的优势。

Result: 在角色驱动的产品推荐任务上，该方法显著优于现有的嵌入基线方法。

Conclusion: 该方法为个性化文本-图像检索提供了一个高效的解决方案，既保持了嵌入方法的可扩展性，又获得了视觉语言模型的细粒度对齐能力。
...
### [291] [MIARec: Mutual-influence-aware Heterogeneous Network Embedding for Scientific Paper Recommendation](https://arxiv.org/abs/2510.12054)
*Wenjin Xie,Tao Jia*

Main category: cs.IR

TL;DR: MIARec模型通过重力方法测量学者间的相互学术影响力，并将其融入图表示学习的消息传播中，结合多通道聚合方法，在学术论文推荐任务中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有图推荐方法忽视学术网络中普遍存在的不对称学术影响力，需要更准确地建模学者间的相互影响关系。

Method: 采用重力方法测量学术影响力，在消息传播过程中融入影响力特征，使用多通道聚合捕获单关系子网络及其相互依赖的嵌入表示。

Result: 在真实数据集上的实验表明，MIARec在三个主要评估指标上均优于基线模型。

Conclusion: MIARec模型通过考虑学术影响力和多通道聚合，能够更全面地理解异质学术网络，在科学论文推荐任务中表现有效。
...
### [292] [Reinforced Preference Optimization for Recommendation](https://arxiv.org/abs/2510.12211)
*Junfei Tan,Yuxin Chen,An Zhang,Junguang Jiang,Bin Liu,Ziru Xu,Han Zhu,Jian Xu,Bo Zheng,Xiang Wang*

Main category: cs.IR

TL;DR: 提出ReRe框架，通过约束波束搜索和增强奖励设计，解决生成式推荐系统中负样本建模不足和奖励稀疏问题，显著提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成式推荐系统存在两个核心限制：缺乏高质量负样本建模和依赖隐式奖励。强化学习虽然提供了解决方案，但在推荐系统中应用面临采样效率低和奖励稀疏的挑战。

Method: 提出ReRe框架：1) 使用约束波束搜索提高采样效率并多样化硬负样本；2) 在基于规则的准确性奖励基础上增加辅助排序奖励，提供更细粒度的监督。

Result: 在三个真实世界数据集上的实验表明，ReRe在排序性能上持续优于传统和基于LLM的推荐器。分析显示ReRe不仅提升了基础模型和SFT初始化模型的性能，还在不同骨干网络家族和规模上表现出稳健的泛化能力。

Conclusion: ReRe为基于LLM的推荐系统提供了一个有效的强化学习框架，同时系统探索了RLVR在推荐系统中的设计空间，为未来研究提供了见解。
...
### [293] [An Empirical Study for Representations of Videos in Video Question Answering via MLLMs](https://arxiv.org/abs/2510.12299)
*Zhi Li,Yanan Wang,Hao Niu,Julio Vizcarra,Masato Taya*

Main category: cs.IR

TL;DR: 本文对多模态大语言模型在视频问答任务中的视频表示方法进行了实证研究，评估了单模态和多模态输入的效率与准确性权衡。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频问答方面取得显著进展，但尚不清楚哪种视频表示方法最有效，以及不同模态如何在任务准确性和计算效率之间取得平衡。

Method: 在VideoMME和LongVideoBench两个基准上系统评估了单模态输入（仅问题、字幕、视觉帧、音频信号）以及多模态组合。

Result: 视觉帧显著提高准确性但带来GPU内存和推理延迟的高成本，而字幕提供了轻量级但有效的替代方案，尤其适用于长视频。
...
### [294] [Causal Inspired Multi Modal Recommendation](https://arxiv.org/abs/2510.12325)
*Jie Yang,Chenyang Gu,Zixuan Liu*

Main category: cs.IR

TL;DR: 提出了一种因果启发的多模态推荐框架，通过双通道跨模态扩散模块识别模态混杂因子，使用后门调整和前门调整来消除混杂偏差，显著提升了推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐系统存在两个关键偏差：模态混杂（潜在因素同时驱动多个模态并影响用户偏好）和交互偏差（真实偏好与曝光效应和偶然点击的噪声混合），导致虚假的特征-偏好关联。

Method: 引入双通道跨模态扩散模块识别隐藏的模态混杂因子；使用后门调整结合分层匹配和向量量化码本阻断混杂路径；应用前门调整结合因果拓扑重构构建去混杂的因果子图。

Result: 在三个真实世界电商数据集上的广泛实验表明，该方法显著优于最先进的基线方法，同时保持强可解释性。

Conclusion: 提出的因果启发多模态推荐框架有效解决了模态混杂和交互偏差问题，在提升推荐性能的同时保持了良好的可解释性。
...
### [295] [Simple Projection Variants Improve ColBERT Performance](https://arxiv.org/abs/2510.12327)
*Benjamin Clavié,Sean Lee,Rikiya Takehi,Aamir Shakir,Makoto P. Kato*

Main category: cs.IR

TL;DR: 研究表明，用更复杂的非线性前馈网络（如深层FFN块、GLU块和跳跃连接）替代ColBERT中的单层线性投影层，可以显著提升多向量检索模型的性能，在多个检索基准上平均提升超过2个NDCG@10点。


<details>
  <summary>Details</summary>
Motivation: 探索MaxSim操作符对多向量模型训练梯度流的影响，发现单层线性投影在这种设置下存在固有局限性，需要更复杂的投影机制来提升性能。

Method: 设计和系统评估替代投影块，包括深层非线性FFN块、GLU块和跳跃连接，分析不同投影变体对ColBERT模型下游性能的影响。

Result: 许多投影变体优于原始线性投影，最佳变体在多个跨领域检索基准上平均性能提升超过2 NDCG@10点，且效果在不同随机种子下保持一致。

Conclusion: 用更复杂的投影块替代ColBERT的线性层是一个稳健的即插即用升级方案，特别是放大中间投影和残差连接对性能提升至关重要。
...
### [296] [A Hierarchical Quantized Tokenization Framework for Task-Adaptive Graph Representation Learning](https://arxiv.org/abs/2510.12369)
*Yang Xiang,Li Fan,Chenke Yin,Chengtao Ji*

Main category: cs.IR

TL;DR: 提出了一种用于图结构的分层量化框架，通过自加权机制实现多尺度任务自适应聚合，在保持编码器冻结的同时通过轻量门控过程调节信息流。


<details>
  <summary>Details</summary>
Motivation: 现有图标记化方法在线性化、连续化和量化方面存在适应性不足和效率问题，特别是量化方法以固定或任务无关的方式组织层次信息，无法动态调整不同层级的贡献权重。

Method: 分层量化框架，引入自加权机制进行任务自适应多尺度聚合，保持编码器冻结，通过轻量门控过程调节信息流。

Result: 在节点分类和链接预测基准数据集上的实验表明，在可比计算预算下相比强基线方法有持续改进。

Conclusion: 该框架实现了参数高效的适配，能够适应多样化的下游任务，同时保持计算效率。
...
### [297] [Leveraging Language Semantics for Collaborative Filtering with TextGCN and TextGCN-MLP: Zero-Shot vs In-Domain Performance](https://arxiv.org/abs/2510.12461)
*Andrei Chernov,Haroon Wahab,Oleg Novitskij*

Main category: cs.IR

TL;DR: 提出了TextGCN和TextGCN-MLP两种方法，利用LLM的文本嵌入和图卷积网络改进推荐系统，在零样本和领域内性能上均达到最优


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注微调LLM生成推荐或将LLM嵌入集成到下游模型，本文探索后一方向，旨在结合语言语义和图消息传递提升推荐性能

Method: TextGCN：在LLM生成的项目标题嵌入上直接应用无参数图卷积层；TextGCN-MLP：在TextGCN基础上添加可训练的多层感知器，使用对比损失训练

Result: TextGCN在零样本推荐上达到最优性能，显著优于先前方法；TextGCN-MLP在领域内推荐基准上达到最优性能，但零样本性能低于TextGCN

Conclusion: TextGCN架构通过结合语言语义和图消息传递实现了优异的零-shot性能，而TextGCN-MLP在领域内性能更优但牺牲了零-shot泛化能力，体现了领域专业化与零-shot泛化之间的权衡
...
### [298] [SMILE: SeMantic Ids Enhanced CoLd Item Representation for Click-through Rate Prediction in E-commerce SEarch](https://arxiv.org/abs/2510.12604)
*Qihang Zhao,Zhongbo Sun,Xiaoyang Zheng,Xian Guo,Siyuan Wang,Zihan Liang,Mingcan Peng,Ben Chen,Chenyi Lei*

Main category: cs.IR

TL;DR: SMILE是一种基于语义ID融合对齐的物品表示增强方法，通过RQ-OPQ编码量化物品内容和协同信息，解决冷启动物品协同信息不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现代搜索和推荐平台中，冷启动物品的协同信息不足加剧了现有平台物品的马太效应，挑战平台多样性。现有方法未能考虑协同与内容之间的不对称性，也未考虑物品间的细粒度差异。

Method: 使用RQ-OPQ编码量化物品内容和协同信息，进行两步对齐：RQ编码传递物品间共享的协同信号，OPQ编码学习物品的差异化信息。

Result: 在大规模工业数据集上的离线实验显示SMILE的优越性，在线A/B测试确认了统计显著的改进：物品CTR +1.66%，买家数 +1.57%，订单量 +2.17%。

Conclusion: SMILE通过融合语义ID对齐有效解决了冷启动物品协同信息不足的问题，显著提升了推荐系统性能。
...
### [299] [The Role of Parametric Injection-A Systematic Study of Parametric Retrieval-Augmented Generation](https://arxiv.org/abs/2510.12668)
*Minghao Tang,Shiyu Ni,Jingtong Wu,Zengxin Han,Keping Bi*

Main category: cs.IR

TL;DR: 本文系统研究了参数化检索增强生成(PRAG)，发现参数化文档仅捕获部分语义信息，单独使用性能不如文本级交互。但当参数化表示与文本文档结合使用时，模型能更有效地利用相关信息，对噪声输入更鲁棒，性能优于单独使用任一来源。


<details>
  <summary>Details</summary>
Motivation: 参数化检索增强生成(PRAG)通过将文档编码为模型参数来增强大语言模型，但参数注入机制尚不明确。本文旨在澄清参数注入的作用，理解PRAG的工作机制。

Method: 对PRAG进行系统性研究，分析参数化文档捕获的语义信息特征，比较单独使用参数化文档与文本级交互的性能差异，并探索参数化表示与文本文档结合使用的效果。

Result: 参数化文档仅捕获文档的部分语义信息，单独使用时性能不如文本级交互。但参数化表示编码了高层次文档信息，当与输入上下文中的文本文档结合时，模型能更有效地利用相关信息，对噪声输入更鲁棒，性能优于单独使用任一来源。
...
### [300] [SAIL-Embedding Technical Report: Omni-modal Embedding Foundation Model](https://arxiv.org/abs/2510.12709)
...
TL;DR: SAIL-Embedding是一个全模态嵌入基础模型，通过多阶段训练策略和架构设计解决了现有模型在现实应用中的局限性，在检索任务中达到SOTA性能，并在推荐场景中显著提升用户生命周期指标。


<details>
  <summary>Details</summary>
Motivation: 解决现有多模态嵌入模型在现实应用中的挑战，包括有限模态支持、训练机制不稳定和工业领域差距等问题。
...
Conclusion: SAIL-Embedding通过精心设计的训练策略成功解决了多模态嵌入模型在工业应用中的关键挑战，在多个真实场景中验证了其有效性。
...
### [301] [Human-in-the-Loop Bandwidth Estimation for Quality of Experience Optimization in Real-Time Video Communication](https://arxiv.org/abs/2510.12265)
*Sami Khairy,Gabriel Mittag,Vishak Gopal,Ross Cutler*

Main category: cs.MM

TL;DR: 提出一个基于人类反馈的数据驱动带宽估计框架，通过离线强化学习训练神经网络带宽估计器，在真实视频会议系统中将主观差通话率降低11.41%。


<details>
  <summary>Details</summary>
Motivation: 实时通信中的带宽估计面临网络架构快速演进、协议栈复杂和QoE指标定义困难的挑战，需要准确估计时变可用带宽来提升视频会议系统的用户体验质量。

Method: 首先训练基于主观用户评估的客观QoE奖励模型，收集100万条真实网络轨迹数据，然后提出新颖的分布离线强化学习算法来训练神经网络带宽估计器。

Result: 真实A/B测试显示，相比基线带宽估计器，该方法将主观差通话率降低了11.41%，离线RL算法在D4RL任务上的基准测试也证明了其泛化能力。

Conclusion: 该人类在环的数据驱动框架成功解决了实时通信带宽估计的挑战，通过离线强化学习显著提升了视频会议系统的用户体验质量。
...
### [302] [M3ST-DTI: A multi-task learning model for drug-target interactions based on multi-modal features and multi-stage alignment](https://arxiv.org/abs/2510.12445)
*Xiangyu Li,Ran Su,Liangliang Liu*

Main category: cs.MM

TL;DR: M3ST-DTI是一个用于药物-靶点相互作用预测的多任务学习模型，通过多阶段集成和对齐多模态特征来提升预测性能


<details>
  <summary>Details</summary>
Motivation: 现有方法无法捕捉深度模态内特征交互或实现有效的跨模态对齐，限制了预测性能和泛化能力

Method: 整合文本、结构和功能三种特征，使用自注意力机制和混合池化图注意力模块增强模态内表示，通过MCA与Gram损失进行早期特征对齐和融合，后期使用BCA模块捕获药物-靶点间的细粒度交互，并采用深度正交融合模块减少特征冗余

Result: 在基准数据集上的广泛评估表明，M3ST-DTI在多种指标上持续优于最先进的方法

Conclusion: M3ST-DTI通过多阶段多模态特征集成和对齐，有效提升了药物-靶点相互作用预测的准确性和泛化能力
...
### [303] [eye2vec: Learning Distributed Representations of Eye Movement for Program Comprehension Analysis](https://arxiv.org/abs/2510.11722)
*Haruhiko Yoshioka,Kazumasa Shimari,Hidetake Uwano,Kenichi Matsumoto*

Main category: cs.SE

TL;DR: eye2vec是一个分析软件开发者在阅读源代码时眼动轨迹的基础设施，使用分布式表示将连续注视表示为语法元素之间的转换，简化眼动分析过程。


<details>
  <summary>Details</summary>
Motivation: 传统眼动研究中，研究人员需要预先选择分析目标并开发分析方法，这依赖于耗时的手动工作，且不同分析层次会导致不同结果。

Method: 使用分布式表示将连续的两个注视表示为语法元素之间的转换，便于采用多样化的数据分析方法。

Result: 该方法能够简化眼动分析过程，提供丰富的语义解释。

Conclusion: eye2vec为程序理解中的眼动研究提供了更高效和灵活的分析框架。
...
### [304] [Task-Aware Reduction for Scalable LLM-Database Systems](https://arxiv.org/abs/2510.11813)
...
<details>
  <summary>Details</summary>
Motivation: LLM在数据密集型工作流中的应用受到现实世界文本数据（如日志、遥测数据）量大、冗长和嘈杂的限制。直接将这些数据输入LLM成本高、不环保，且与任务目标不一致。

Method: 将输入侧缩减视为注意力分配而非压缩，优先保留与下游任务最相关的信息。提出构建基准测试、设计自适应缩减流程、将token预算感知预处理集成到数据库和检索系统中的研究挑战。

Result: 提出了新的设计理念和研究方向，但尚未展示具体实验结果。

Conclusion: 通过将稀缺的注意力资源引导到嘈杂数据流中有意义的信号上，可以实现可扩展、准确和可持续的LLM-数据集成。
...
### [305] [Lingxi: Repository-Level Issue Resolution Framework Enhanced by Procedural Knowledge Guided Scaling](https://arxiv.org/abs/2510.11838)
*Xu Yang,Jiayuan Zhou,Michael Pacheco,Wenhan Zhu,Pengfei He,Shaowei Wang,Kui Liu,Ruiqi Pan*

Main category: cs.SE

TL;DR: Lingxi是一个基于历史问题修复数据提取程序性知识的软件问题解决框架，通过层次化抽象机制构建知识，指导智能体从多角度分析问题，显著提升仓库级问题的解决能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于智能体的方法在解决复杂仓库级问题时存在两个关键限制：缺乏程序性知识（即问题修复的步骤和原理）以及依赖大量计算资源进行盲目探索。

Method: Lingxi通过层次化抽象机制离线构建程序性知识，使智能体学习修复的步骤和原理；在线应用时采用知识驱动的扩展方法，利用类似问题的程序性知识从多角度智能分析目标问题。

Result: 在SWE-bench Verified基准测试中，Lingxi在Past@1设置下成功解决了74.6%的错误，显著优于五种最先进技术（提升5.4%到14.9%）。消融研究证实其成功直接源于程序性知识的使用。
...
### [306] [DMAS-Forge: A Framework for Transparent Deployment of AI Applications as Distributed Systems](https://arxiv.org/abs/2510.11872)
*Alessandro Cornacchia,Vaastav Anand,Muhammad Bilal,Zafar Qazi,Marco Canini*

Main category: cs.SE

TL;DR: DMAS-Forge是一个框架，旨在简化分布式多智能体AI应用的部署和测试，通过解耦应用逻辑与部署选择，自动生成必要的粘合代码和配置。


<details>
  <summary>Details</summary>
Motivation: 随着AI应用越来越多地依赖多个具有不同角色、专用工具和内存访问权限的智能体来解决复杂任务，部署和测试这些分布式系统变得困难且耗时。

Method: DMAS-Forge框架将应用逻辑与具体部署选择解耦，透明地生成必要的粘合代码和配置，以在不同部署场景下启动分布式多智能体应用。

Result: 提出了DMAS-Forge的愿景、设计原则和原型，展示了该框架如何最小化手动工作量来部署分布式多智能体应用。

Conclusion: DMAS-Forge为解决分布式AI智能体部署的挑战提供了有效方法，并讨论了该方法的机遇和未来工作方向。
...
### [307] [TorchCor: High-Performance Cardiac Electrophysiology Simulations with the Finite Element Method on GPUs](https://arxiv.org/abs/2510.12011)
*Bei Zhou,Maximilian Balmus,Cesare Corrado,Ludovica Cicci,Shuang Qian,Steven A. Niederer*

Main category: cs.SE

TL;DR: TorchCor是一个基于PyTorch的高性能Python库，用于在通用GPU上进行心脏电生理学的有限元模拟，显著加速大型3D网格的模拟，解决了传统模拟需要高性能CPU计算资源的问题。


<details>
  <summary>Details</summary>
Motivation: 心脏电生理学模拟通常需要大量CPU核心的高性能计算资源，这对许多研究团队和临床医生来说难以获得，因此需要开发更易访问的解决方案。

Method: 开发了基于PyTorch的TorchCor库，使用有限元方法在通用GPU上进行心脏电生理学模拟。

Result: TorchCor显著加速了心脏电生理学模拟，特别是在大型3D网格上，其求解器精度通过制造解析解和N版本基准问题验证。

Conclusion: TorchCor为学术和商业用途提供了免费无限制的高性能心脏电生理学模拟解决方案，解决了计算资源可访问性问题。
...
### [308] [Enhancing Neural Code Representation with Additional Context](https://arxiv.org/abs/2510.12082)
*Huy Nguyen,Christoph Treude,Patanamon Thongtanunam*

Main category: cs.SE

TL;DR: 该论文通过实证研究发现，在代码表示中融入上下文信息（如版本历史和调用图）能显著提升深度学习模型在代码克隆检测和代码摘要等程序理解任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型主要依赖源代码本身，忽略了版本历史和结构关系等上下文信息，这限制了模型理解代码演化和运行方式的能力。

Method: 在两个数据集（SeSaMe和CodeSearchNet）上评估了五个代表性模型（CodeBERT、GraphCodeBERT、CodeT5、PLBART、ASTNN），分别在仅使用代码和增强上下文两种设置下进行微调。

Result: 上下文信息普遍提升性能：版本历史持续提升克隆检测（如CodeT5 F1提升15.92%）和代码摘要（如GraphCodeBERT METEOR提升5.56%），调用图效果因模型和任务而异。组合多种上下文可获得进一步增益（最高提升21.48% macro-F1）。
...
### [309] [Towards Engineering Multi-Agent LLMs: A Protocol-Driven Approach](https://arxiv.org/abs/2510.12120)
*Zhenyu Mao,Jacky Keung,Fengji Zhang,Shuo Liu,Yifei Wang,Jialong Li*

Main category: cs.SE

TL;DR: SEMAP是一种协议层方法，通过引入软件工程设计原则来改进多智能体LLM系统，显著减少了代码开发和漏洞检测任务中的失败率。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体LLM系统在软件工程任务中经常失败，主要由于缺乏软件工程结构化原则导致的三个核心缺陷：规范不足、协调错位和不适当的验证。

Method: 提出了SEMAP协议层方法，基于Google的A2A基础设施实现三个核心SE设计原则：显式行为契约建模、结构化消息传递、以及带有验证的生命周期引导执行。

Result: 在代码开发中，函数级开发失败减少69.6%，部署级开发失败减少56.7%；在漏洞检测中，Python任务失败减少47.4%，C/C++任务失败减少28.2%。

Conclusion: SEMAP通过引入软件工程设计原则，有效解决了多智能体LLM系统中的核心缺陷，显著提高了软件工程任务的执行成功率。
...
### [310] [iCodeReviewer: Improving Secure Code Review with Mixture of Prompts](https://arxiv.org/abs/2510.12186)
...
Method: 提出iCodeReviewer，采用混合提示架构，包含多个提示专家来检查特定安全问题，并实现有效的路由算法基于代码特征激活必要的提示专家。

Result: 在内部数据集上，iCodeReviewer在安全问题和定位方面达到63.98%的F1分数，在生产环境中生成的审查评论接受率高达84%。

Conclusion: iCodeReviewer通过混合提示架构和路由算法，有效提高了自动化安全代码审查的准确性和实用性。
...
### [311] [Show Your Title! A Scoping Review on Verbalization in Software Engineering with LLM-Assisted Screening](https://arxiv.org/abs/2510.12294)
*Gergő Balogh,Dávid Kószó,Homayoun Safarpour Motealegh Mahalegi,László Tóth,Bence Szakács,Áron Búcsú*

Main category: cs.SE

TL;DR: 本文通过LLM辅助的筛选流程对9000多篇论文进行范围综述，研究了软件工程与心理学交叉领域中口头数据的使用情况，发现SE经常借鉴PSY方法但反向影响很少，且人类中心主题代表性不足。
...
Method: 使用GPT构建LLM辅助筛选流程，基于标题评估9000多篇论文的相关性，并进行人工验证。

Result: GPT输出与人工评审高度一致，分歧率为13%。主要主题与SE工艺相关，人类中心主题代表性不足，SE经常借鉴PSY方法但反向影响很少。

Conclusion: LLM可以有效支持跨学科综述过程，SE与PSY交叉研究存在不平衡，需要更多关注人类中心主题。
...
### [312] [(R)evolution of Programming: Vibe Coding as a Post-Coding Paradigm](https://arxiv.org/abs/2510.12364)
*Kevin Krings,Nino S. Bohn,Thomas Ludwig*

Main category: cs.SE

TL;DR: 本文研究了新兴的Vibe Coding范式，这是一种强调开发者与AI系统之间直觉、情感驱动和即兴交互的编程方式，与GitHub Copilot等传统AI辅助开发工具形成对比。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI特别是大语言模型的发展，软件开发实践出现了新的可能性。作者旨在探索Vibe Coding这一新兴范式如何区别于传统的编程方法，并理解其对软件开发文化的影响。

Method: 通过对10位经验丰富的软件从业者进行5次半结构化访谈，识别出五个主题维度：创造力、可持续性、编程未来、协作和批评。

Result: 研究发现Vibe Coding在共同漂移的隐喻下重新配置了开发者的角色，模糊了专业开发者与非开发者之间的界限。虽然支持新颖的表达形式和快速原型设计，但也带来了可重复性、可扩展性和包容性方面的挑战。

Conclusion: Vibe Coding代表了编程文化的重要转变，值得在人机交互和软件工程研究中进行进一步调查。
...
### [313] [Should I Run My Cloud Benchmark on Black Friday?](https://arxiv.org/abs/2510.12397)
*Sören Henning,Adriano Vogel,Esteban Perez-Wohlfeil,Otmar Ertl,Rick Rabiser*

Main category: cs.SE

TL;DR: 该研究扩展了之前对云环境性能变异性的调查，特别关注重大全球事件（如黑色星期五）是否会影响性能基准测试结果。


<details>
  <summary>Details</summary>
Motivation: 云环境中基准测试结果的可重现性和可信度常因性能的高变异性而受到质疑。之前的研究已证实应用层面存在性能变异性，但程度低于普遍假设。

Method: 通过在不同时间段重复执行流处理应用基准测试，特别关注黑色星期五等重大全球事件期间，分析性能模式变化。

Result: 研究发现云环境确实存在可观察的性能变异性，但程度低于预期。大规模研究揭示了微妙的每日和每周性能模式。

Conclusion: 重大全球事件可能对云环境性能基准测试结果产生影响，这需要进一步研究来量化具体影响程度。
...
### [314] [DarTwin made precise by SysMLv2 -- An Experiment](https://arxiv.org/abs/2510.12478)
*Øystein Haugen,Stefan Klikovits,Martin Arthur Andersen,Jonathan Beaulieu,Francis Bordeleau,Joachim Denil,Joost Mertens*

Main category: cs.SE

TL;DR: 本文评估了SysMLv2在领域特定语言(DSL)创建方面的能力，通过开发DarTwin DSL来形式化数字孪生演化表示法，但发现当前工具在图形表示能力方面存在限制。


<details>
  <summary>Details</summary>
Motivation: SysMLv2新增了内置的领域特定概念和语言扩展机制，这有望促进DSL的创建以及与现有系统描述和技术设计的接口。本文旨在评估这些功能并通过具体用例验证其能力。

Method: 通过SysMLv2开发DarTwin DSL，这是一个形式化现有DarTwin数字孪生演化表示法的领域特定语言，从而实现在任何SysMLv2工具中广泛应用DarTwin的演化模板。

Result: 成功演示了DarTwin DSL，但指出了当前SysMLv2工具在图形表示能力方面的局限性。

Conclusion: 这项工作为数字孪生的模型驱动工程领域做出了贡献，并将其与SysMLv2的发布相结合，从而在系统工程中实现了系统化方法与数字孪生演化管理的整合。
...
### [315] [Diff-XYZ: A Benchmark for Evaluating Diff Understanding](https://arxiv.org/abs/2510.12487)
...
<details>
  <summary>Details</summary>
Motivation: 可靠处理代码差异对于大规模编辑和重构存储库的智能体至关重要，需要评估和改进LLM在差异处理方面的能力。

Method: 从CommitPackFT的真实提交中提取三元组⟨旧代码,新代码,差异⟩，使用自动指标和清晰评估协议，比较不同差异表示格式。

Result: 研究发现不同格式应根据用例和模型大小选择，例如搜索替换格式适合大型模型的差异生成，但不适合差异分析和小型模型。

Conclusion: Diff-XYZ基准为评估和改进LLM的差异处理能力提供了可重复使用的基础，有助于未来差异格式和代码编辑模型的发展。
...
### [316] [The EmpathiSEr: Development and Validation of Software Engineering Oriented Empathy Scales](https://arxiv.org/abs/2510.12546)
*Hashini Gunatilake,John Grundy,Rashina Hoda,Ingo Mueller*

Main category: cs.SE

TL;DR: 开发了两个专门针对软件工程领域的共情量表：EmpathiSEr-P（测量从业者间共情）和EmpathiSEr-U（测量从业者对用户的共情），填补了该领域缺乏验证测量工具的空白。


<details>
  <summary>Details</summary>
Motivation: 现有通用共情量表基于心理学和医疗领域，其语言、场景和假设在软件工程环境中缺乏意义和可解释性，无法捕捉软件工程中角色特定、领域绑定的共情表达。

Method: 采用严谨的多阶段方法，包括专家评估、认知访谈和两次从业者调查，基于从业者知情的概念框架开发量表，涵盖认知共情、情感共情和共情反应三个维度。

Result: 开发出首个心理测量学验证的软件工程专用共情量表，为研究人员和从业者提供了评估共情和设计共情增强干预措施的工具。

Conclusion: 这些量表填补了软件工程领域共情测量的重要空白，能够有效支持团队协作和用户交互中的共情评估与提升。
...
### [317] [Evaluating End-User Device Energy Models in Sustainability Reporting of Browser-Based Web Services](https://arxiv.org/abs/2510.12566)
*Maja H. Kirkeby,Timmie Lagermann*

Main category: cs.SE

TL;DR: 评估简化能源和碳模型在网站可持续性报告中的准确性，发现基于恒定功率近似的方法与实际能耗存在显著偏差，需要针对网站类别和设备类型调整参数。


<details>
  <summary>Details</summary>
Motivation: 虽然丹麦Digst框架和英国DIMPACT模型在可持续性报告中广泛使用，但其准确性和精确性尚未得到充分研究，需要实证评估这些模型在真实用户交互中的表现。

Method: 通过预定义用户流程在四种笔记本电脑平台上测量购物、预订、导航和新闻网站的能耗，比较实际测量值与模型预测值。

Result: 恒定功率近似法（P * t）与实际能耗存在显著偏差，偏差程度取决于网站类别、设备类型和任务特征，且偏差是系统性的而非随机的。

Conclusion: 可持续性报告框架需要采用考虑网站类别和设备特性的功率参数，以提高模型的准确性和可重复性。
...
### [318] [Runtime Composition in Dynamic System of Systems: A Systematic Review of Challenges, Solutions, Tools, and Evaluation Methods](https://arxiv.org/abs/2510.12616)
*Muhammad Ashfaq,Ahmed R. Sadik,Teerath Das,Muhammad Waseem,Niko Makitalo,Tommi Mikkonen*

Main category: cs.SE

TL;DR: 本文通过系统文献综述分析了动态系统之系统（SoSs）中的运行时组合研究，识别了核心挑战、解决方案、工具和评估方法，旨在指导动态可组合SoSs的开发与实施。


<details>
  <summary>Details</summary>
Motivation: 现代SoSs在动态环境中运行时组合（即系统组件的动态发现、集成和协调）对适应性至关重要，但文献缺乏对此的系统综合。

Method: 采用系统文献综述方法，筛选2019-2024年间1774篇研究，选取80篇主要研究进行主题分析。

Result: 挑战分为四类：建模与分析、弹性操作、系统编排、系统异构性；解决方案涵盖七个领域：协同仿真与数字孪生、语义本体、集成框架、自适应架构、中间件、形式化方法、AI驱动弹性；服务导向框架主导工具，仿真平台支持评估；存在工具互操作性、跨工具链工作流和标准化基准缺失等关键差距。

Conclusion: 综合揭示了自主性与协调性、建模与现实差距、社会技术集成等张力，呼吁标准化评估指标、可扩展去中心化架构和跨领域框架。
...
### [319] [Beyond Postconditions: Can Large Language Models infer Formal Contracts for Automatic Software Verification?](https://arxiv.org/abs/2510.12702)
*Cedric Richter,Heike Wehrheim*

Main category: cs.SE

TL;DR: NL2Contract任务使用LLM将自然语言转换为包含前置条件和后置条件的正式功能契约，相比仅生成后置条件的方法，能显著减少自动软件验证中的误报。


<details>
  <summary>Details</summary>
Motivation: 解决自动软件验证中缺乏正式规范的问题，以及仅使用LLM生成后置条件时导致的验证误报问题。

Method: 引入NL2Contract任务，使用LLM从代码中的自然语言提示（如函数名、注释）生成完整的功能契约（前置+后置条件），并定义验证指标。

Result: LLM能有效生成对所有可能输入都可靠的功能契约；生成的契约能区分错误与正确行为；相比仅使用后置条件，验证器产生的误报更少。

Conclusion: LLM生成的功能契约与开发者意图一致，可用于自动软件验证器捕获真实世界bug，减少误报。
...
### [320] [Leveraging LLMs, IDEs, and Semantic Embeddings for Automated Move Method Refactoring](https://arxiv.org/abs/2503.20934)
...
TL;DR: MM-assist是一个基于LLM的MOVEMETHOD重构助手，通过结合静态分析、IDE工具和RAG技术，自动过滤LLM幻觉并提供端到端的重构建议，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法推荐的MOVEMETHOD重构与专家开发者的实践不符，而LLM虽然能给出专家级建议但存在高达80%的幻觉问题。

Method: 使用静态分析过滤LLM幻觉，要求LLM自我一致性评估和排名重构建议，采用重构感知的RAG技术解决LLM上下文限制问题。

Result: 在基准测试中Recall@1和Recall@3提升1.7倍，在210个开源重构案例中召回率至少提升2.4倍，用户研究中82.8%的建议获得积极评价。

Conclusion: MM-assist通过协同LLM、IDE、静态分析和语义相关性，提供了有效且实用的MOVEMETHOD重构解决方案。
...
### [321] [SpikePool: Event-driven Spiking Transformer with Pooling Attention](https://arxiv.org/abs/2510.12102)
*Donghyun Lee,Alex Sima,Yuhang Li,Panos Stinis,Priyadarshini Panda*

Main category: cs.NE

TL;DR: 该论文通过频域分析发现脉冲变换器表现为高通滤波器，而视觉变换器是低通滤波器。基于此提出了SpikePool，用最大池化注意力替代脉冲自注意力，实现选择性带通滤波效果，在事件视觉任务中取得竞争性结果并显著降低计算时间。


<details>
  <summary>Details</summary>
Motivation: 尽管脉冲变换器在事件视觉任务中表现出色，但对其如何处理事件数据的机制理解有限。现有方法主要关注架构修改，缺乏对底层信号处理特性的分析。

Method: 通过频域分析脉冲变换器，发现其高通滤波特性。提出SpikePool，用最大池化注意力替代脉冲自注意力，创建选择性带通滤波效果，保留有用高频内容同时抑制噪声。

Result: 在事件分类和物体检测任务上取得竞争性结果，训练时间减少42.5%，推理时间减少32.8%。

Conclusion: 频域分析揭示了脉冲变换器的滤波特性，SpikePool通过选择性带通滤波在保留关键高频信息和抑制噪声之间取得更好平衡，为事件数据处理提供了更高效的方法。
