...
TL;DR: PixCLIP是一个新框架，通过同时处理视觉提示输入和长文本描述来增强CLIP的细粒度图像-文本对齐能力，在像素级交互和长文本处理方面实现突破。
...
Method: 1) 建立自动标注流水线生成像素级定位的长文本描述，构建LongGRIT数据集；2) 用LLM替换CLIP原始文本编码器；3) 提出三分支像素-文本对齐学习框架，实现任意粒度的图像区域与文本对齐。

Result: PixCLIP在像素级交互和长文本处理方面取得突破，达到了最先进的性能。

Conclusion: PixCLIP通过同时增强视觉和文本处理的粒度，有效解决了CLIP在细粒度图像-文本对齐方面的局限性，为像素级视觉语言理解提供了新思路。
...
TL;DR: LoRA-Edge是一种参数高效微调方法，结合低秩适应和Tensor-Train分解，可在边缘设备上高效微调CNN，仅需更新1.49%参数即可达到接近全微调的精度。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备上CNN全微调不可行的问题，因为边缘设备有严格的内存、计算和能耗限制，需要参数高效的微调方法。

Method: 对预训练卷积层应用Tensor-Train SVD分解，选择性仅更新输出侧核心并采用零初始化，最后将更新融合回密集核中，保持推理成本不变。

Result: 在多个HAR数据集和CNN骨干上，LoRA-Edge仅更新最多1.49%参数，精度与全微调相差不到4.7%，收敛速度比基准方法快1.4-3.8倍。

Conclusion: LoRA-Edge实现了结构对齐、参数高效的边缘设备CNN适配，为边缘平台提供了实用的微调解决方案。
...
TL;DR: SILVI是一个开源标注软件，用于在视频数据中标注动物行为和互动，填补了现有工具在行为标注和个体定位之间的空白。


<details>
  <summary>Details</summary>
Motivation: 现有开源标注工具要么支持行为标注但不定位个体，要么能定位个体但不能捕捉互动，这限制了计算机视觉方法在动物行为分析中的应用。

Method: 开发了SILVI软件，整合行为标注和个体定位功能，让研究人员能在视频数据中直接标注行为和互动，生成结构化输出。

Result: SILVI软件已开发完成并开源，提供了文档和下载说明，支持训练和验证计算机视觉模型。

Conclusion: SILVI通过连接行为生态学和计算机视觉，促进了细粒度行为分析自动化方法的发展，虽然主要针对动物行为开发，但也可用于标注人类互动。
...
Result: 噪声注入技术将分布内和分布外评估的性能差距从0.10-0.20显著降低到0.01-0.06，在AUC、F1、准确率、召回率和特异性等关键指标上均有改善。

Conclusion: 噪声注入是一种有效提高深度学习模型在COVID-19检测中分布外泛化能力的技术，能够减少模型对源特定伪影的依赖。
...
TL;DR: 该研究探索了基于模仿学习的机器人控制策略在X射线引导脊柱手术中的应用，开发了一个高真实度的模拟环境，训练了仅基于视觉信息的插管对齐策略，在68.5%的案例中首次尝试成功，并展示了良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索模仿学习在复杂的多视角X射线引导脊柱手术（如椎弓根螺钉置入）中的适用性，因为X射线图像解释复杂，传统方法面临挑战。

Method: 开发了高真实度的X射线引导脊柱手术模拟环境，收集了正确轨迹和对应双平面X射线序列数据集，训练了基于视觉信息的模仿学习策略进行规划和开环控制。

Result: 策略在68.5%的案例中首次尝试成功，能够维持安全的椎弓根内轨迹，对复杂解剖结构（包括骨折）具有良好泛化能力，对不同的初始位置保持鲁棒性，在真实X射线上也能产生合理的轨迹。

Conclusion: 虽然初步结果有前景，但在入口点精度方面存在局限，闭环控制需要更频繁的反馈。通过更强的先验知识和领域知识，这类模型可为轻量级、无需CT的机器人术中脊柱导航提供基础。
...
TL;DR: 提出了一种基于剪枝轻量版YOLOv12结合自对抗训练和专用数据增强的实时目标检测框架，用于沙漠环境中的垃圾检测，在精度和效率方面取得显著改进。
...
Method: 使用剪枝轻量版YOLOv12，集成自对抗训练和专用数据增强策略，在DroneTrashNet数据集上进行训练和测试。

Result: 在精度、召回率和平均精度均值方面取得显著改进，同时实现低延迟和紧凑模型尺寸，适合在资源受限的无人机上部署。与最先进的轻量级YOLO变体相比，在准确性和效率方面达到最佳平衡。
...
TL;DR: 提出基于类别的图像合成方法(CoImg)，通过将同类多张图像融合成组合视觉合成图像，增强类内方差和信息密度，在OCT视网膜疾病数据集上显著提升诊断准确率至99.6%。


<details>
  <summary>Details</summary>
Motivation: 解决小样本、不平衡数据集和低质量输入图像导致深度学习模型高误报率的问题，特别是在医学影像诊断领域。

Method: 使用类基图像合成方法，将同类多张图像融合成3x1布局的合成图像(CoImg)，构建平衡数据集Co-OCTDL，并与原始数据集在相同VGG16模型架构和超参数下进行对比。

Result: 在OCTDL数据集上，合成图像方法将准确率提升至99.6%，F1-score达0.995，AUC达0.9996，显著降低了误报率。

Conclusion: 该方法能有效处理类别不平衡和小样本问题，即使在弱数据集上也能产生高质量预测，在医学影像诊断中具有重要应用价值。
...
TL;DR: 提出了一种无需标注异常的无监督医学图像异常检测框架，通过增量扩展可信正常样本集，结合轻量级适配器更新和不确定性门控样本准入机制，在多个医学影像数据集上显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像中异常检测面临标注异常稀缺和专家监督成本高的问题，需要开发无需异常标签的无监督方法来解决这一根本挑战。

Method: 使用冻结的预训练视觉骨干网络，添加小型卷积适配器进行快速域适应。通过k近邻异常评分和双概率门控机制（距离z-score阈值和SWAG认知不确定性边界）来安全扩展正常样本核心集。
...
Conclusion: 该框架在真实世界标签稀缺的医学影像应用中表现出高效性和有效性，能够持续优化正常性概念，无需依赖生成重建或重放缓冲区。
...
TL;DR: 提出了两种互补的时间动作定位方法：边界距离回归(BDR)通过符号距离回归实现信息理论最优定位，自适应时间细化(ATR)通过连续深度选择实现计算资源的自适应分配，在多个基准测试中显著提升了性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前时间动作定位方法对所有边界采用统一计算，但不同边界的难度存在显著差异，需要更智能的计算分配策略。

Method: 1. BDR：用符号距离回归替代分类，提供信息理论最优的边界定位；2. ATR：通过连续深度选择τ∈[0,1]实现端到端可微分优化，无需强化学习。

Result: BDR在现有方法上仅需约50行代码即可实现1.8-3.1%的mAP@0.7提升；ATR在THUMOS14上达到56.5% mAP@0.7（162G FLOPs），相比均匀处理的53.6%（198G FLOPs）提升2.9%同时减少18%计算量。
...
Method: 通过高斯引导的网格可微分渲染同时优化网格几何（顶点位置和面）和顶点颜色，利用输入图像的光度一致性和法线/深度图的几何正则化。

Result: 获得了高质量的三维重建结果，可用于下游编辑任务如重新照明和形状变形。

Conclusion: 提出的高斯-网格联合优化框架实现了几何和外观的统一优化，为三维编辑应用提供了高质量的重建结果。
...
TL;DR: 提出了一种基于线性分数变换参数的标定方法，用于解耦光场相机的主镜头和微透镜阵列，包含最小二乘解析解和非线性优化，在物理和模拟数据上验证了性能。
...
Method: 引入线性分数变换参数α来解耦主镜头和微透镜阵列，包括基于最小二乘的解析解和非线性优化，同时提出了从原始图像中检测特征的方法。

Result: 在物理和模拟数据上的实验验证了所提方法的性能，基于该模型的原始光场图像模拟速度更快，对数据驱动的深度学习方法至关重要。

Conclusion: 该方法有效解决了光场相机标定问题，提高了图像模拟效率，代码可从作者网站获取。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景重建方法只能恢复可见表面，忽略了被遮挡的结构元素（如墙壁、地板、天花板），而这些元素相对容易预测，因为它们通常是平面、重复且简单的。
...
TL;DR: 研究发现3D姿态信息在人类社交互动识别中起关键作用，简单的3D社交姿态特征能显著提升AI模型的社交判断能力。


<details>
  <summary>Details</summary>
Motivation: 理解人类如何从视觉输入中快速提取社交互动信息，以及为什么先进的AI视觉系统在这方面仍面临挑战。

Method: 结合最先进的姿态和深度估计算法提取视频中人物的3D关节位置，并与当前AI视觉模型进行对比分析。

Result: 3D关节位置在预测人类社交互动判断方面优于大多数AI视觉模型，简单的3D社交姿态特征（仅描述面部位置和方向）与完整3D关节集具有相同的预测能力。
...
TL;DR: CaRF是一个完全可微分的框架，直接在3D高斯空间中操作，通过引入相机感知机制和跨视图监督，解决了现有方法在多视图一致性方面的挑战。
...
Method: 提出了高斯场相机编码(GFCE)将相机几何融入高斯文本交互，以及训练配对视图监督(ITPVS)在训练期间对齐不同视图的高斯logits。
...
Method: 提出PhysCorr框架，包括PhysicsRM（首个双维度奖励模型，量化物体内部稳定性和物体间交互）和PhyDPO（新颖的直接偏好优化流程，利用对比反馈和物理感知重加权引导生成物理一致的输出）。

Result: 在多个基准测试上的广泛实验表明，PhysCorr在物理真实性方面取得显著改进，同时保持视觉保真度和语义对齐。

Conclusion: 这项工作向物理基础和可信的视频生成迈出了关键一步，提出的方法具有模型无关性和可扩展性，可无缝集成到各种视频扩散和基于Transformer的骨干网络中。
...
TL;DR: GNN-MoE：一种用于视觉Transformer领域泛化的参数高效微调方法，通过图神经网络路由器和专家混合框架，在保持参数效率的同时实现最先进的领域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 领域泛化(DG)要求视觉Transformer(ViT)在未见过的领域上具有鲁棒性能。标准微调方法成本高且可能损害泛化能力，需要参数高效的微调方法来适应预训练的ViT。

Method: 提出GNN-MoE框架，结合专家混合(MoE)和高效的Kronecker适配器。使用图神经网络(GNN)路由器（包括GCN、GAT、SAGE）在补丁间图上操作，动态分配补丁给专门化的专家，而不是基于令牌的路由。

Result: GNN-MoE在领域泛化基准测试中达到最先进或具有竞争力的性能，同时保持高参数效率。

Conclusion: 基于图的上下文路由对于实现鲁棒、轻量级的领域泛化具有重要价值，GNN-MoE展示了这种方法的有效性。
...
Method: 从10个公共来源收集120万张多模态胸部影像数据，从头预训练ViT模型，并提出引导随机调整裁剪的内容感知数据增强策略。

Result: MedDChest在多种下游诊断任务中显著优于公开可用的ImageNet预训练模型。

Conclusion: 大规模领域内预训练结合领域特定数据增强为胸部诊断任务提供了更强大的特征提取器，模型权重将公开以促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于等值面的体素化表示严重依赖水密化或渲染优化，不可避免地损害几何保真度，需要一种能保持锐利特征和内部结构的高保真表示方法。

Method: 采用稀疏体素化表示，设计双模式自编码器，支持直接表示和网格重建，无需场函数转换或等值面提取。

Result: 在直接表示中达到10^-5级别的距离误差；在网格重建中，与强基线相比，Chamfer Distance减少93%，F-score提高35%。
...
TL;DR: 提出基于低帧率指尖视频PPG信号的轻量级生物认证框架，使用CVT-ConvMixer-LSTM混合深度学习模型，在46个受试者上达到98%的认证准确率。


<details>
  <summary>Details</summary>
Motivation: PPG信号因其无创采集、活体检测特性和低成本可穿戴设备适用性，在生物认证中受到关注，但面临运动伪影、光照变化和个体生理差异等挑战。

Method: 使用CFIHSR数据集，对PPG信号进行预处理后通过CWT转换为时频标量图，开发CVT-ConvMixer-LSTM混合模型结合空间和时间特征。

Result: 在46个受试者上实验结果显示98%的认证准确率，验证了模型对噪声和个体差异的鲁棒性。
...
Method: Bratrix将视觉刺激解耦为视觉和语言语义组件，使用可学习的语言锚定语义矩阵增强跨模态相关性，采用两阶段训练策略（单模态预训练+多模态微调），并包含不确定性感知模块处理噪声神经信号。

Result: 在EEG、MEG和fMRI基准测试中，Bratrix在检索、重建和描述任务上优于最先进方法，特别是在200-way EEG检索任务中提升了14.3%。

Conclusion: Bratrix通过语言锚定的多模态对齐框架，有效解决了神经信号解码中的语义理解挑战，为视觉语义从神经信号中提取提供了新的解决方案。
...
TL;DR: 该论文比较了两种无配对自监督CT图像去噪方法：基于CycleGAN的残差翻译器和Noise2Score分数匹配去噪器。CycleGAN在最终图像质量上表现最佳，而Noise2Score在无配对情况下提供稳健的替代方案。


<details>
  <summary>Details</summary>
Motivation: 研究在无配对和自监督条件下进行CT图像去噪，评估两种训练数据效率高的方法，以解决实际应用中缺乏干净配对数据的问题。

Method: 使用CycleGAN残差翻译器和Noise2Score分数匹配去噪器两种范式，通过配置扫描确定最佳参数设置（lambda_cycle=30, lambda_iden=2, ngf=ndf=64），并采用标准U-Net骨干网络进行训练。

Result: CycleGAN将输入图像从34.66 dB/0.9234 SSIM提升到38.913 dB/0.971 SSIM，获得1.9441的估计分数和1.9343的未见集分数。Noise2Score在绝对PSNR/SSIM上稍逊，但在噪声较大的输入上表现突出。

Conclusion: CycleGAN提供最强的最终图像质量，而Noise2Score在无配对情况下提供具有竞争力的稳健替代方案，两者各有优势。
...
TL;DR: UKAST是一种U-Net架构，将基于有理函数的Kolmogorov-Arnold网络集成到Swin Transformer编码器中，解决了CNN难以建模长距离依赖和Transformer数据需求大、计算昂贵的问题，在多个医学图像分割基准上达到最先进性能。
...
Method: 提出UKAST架构，将基于有理函数的KANs集成到Swin Transformer编码器中，利用有理基函数和组有理KANs，解决了原始样条基KANs的低效问题。

Result: 在四个不同的2D和3D医学图像分割基准上达到最先进性能，一致超越CNN和Transformer基线，在数据稀缺设置下获得更高精度，缓解了标准Vision Transformers的数据饥饿限制。

Conclusion: KAN增强的Transformers在数据高效的医学图像分割方面具有巨大潜力，UKAST在减少FLOPs的同时仅轻微增加参数数量，实现了更表达性和数据高效的分割框架。
...
TL;DR: SpatialLock是一个新颖的文本到图像生成框架，通过位置参与注入和位置引导学习两个组件，实现了对生成图像中物体空间位置的精确控制。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成方法在物体定位控制方面存在不足，未能充分利用位置信息，导致对物体空间布局的理解不充分。

Method: 提出SpatialLock框架，包含位置参与注入(PoI)和位置引导学习(PoG)两个组件。PoI通过注意力层直接整合空间信息，PoG使用感知监督来细化物体定位。

Result: 实验表明SpatialLock在精确物体定位方面达到了新的最先进水平，在多个数据集上实现了超过0.9的IOU分数。

Conclusion: SpatialLock能够生成具有精确空间排列的物体，并提高了生成图像的视觉质量。
...
Conclusion: 多速率公式在扩散求解器中具有巨大潜力，为实现无需模型重训练的实时高质量图像合成铺平了道路。
...
Method: 使用参考特征作为辅助信息，结合线性平滑和风格-内容引导机制，避免内容泄漏；通过联合AdaIN模块支持多风格生成。

Result: 实验表明该方法能实现高质量的草图生成，具有准确的风格对齐和更好的风格控制灵活性。

Conclusion: M3S框架在草图风格控制方面表现出色，有效提升了生成质量和控制精度。
...
TL;DR: 提出一个完整的自动化网球比赛分析系统，集成多个深度学习模型进行实时球员检测、网球追踪和球场关键点识别，提供详细比赛分析数据。


<details>
  <summary>Details</summary>
Motivation: 为教练、广播公司和球员提供可操作的比赛洞察，通过自动化分析系统替代传统人工观察，提高比赛分析的效率和准确性。

Method: 使用YOLOv8进行球员检测，定制训练的YOLOv5模型进行网球追踪，基于ResNet50的架构进行球场关键点检测，集成这些模型构建完整分析流程。

Result: 实验结果显示系统在不同球场条件和比赛场景下表现出鲁棒性能，能够生成带注释的视频和详细性能指标。

Conclusion: 该系统成功实现了自动化网球比赛分析，为比赛参与者提供了有价值的分析工具，展示了深度学习在体育分析中的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 复杂海洋环境常导致相机运动和视觉退化，给多目标跟踪带来挑战，需要开发能够应对这些问题的鲁棒跟踪方法。

Method: 采用并行跟踪器架构，包含目标检测和重识别分支，以及专门的动态相机运动估计分支。集成可逆柱状检测网络和轻量级Transformer外观提取器，通过投影变换解耦平台和目标运动。

Result: 在新加坡海事数据集上的广泛评估显示，DMSORT达到最先进性能，在基于重识别的多目标跟踪框架中运行速度最快，同时保持高身份一致性和对抖动、遮挡的鲁棒性。

Conclusion: DMSORT在复杂海洋环境中实现了高效、鲁棒的多目标跟踪，为安全船舶导航和有效海事监视提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在执行需要特定领域程序知识的任务时仍落后于人类，而人类可以通过观看视频教程来学习特定应用的流程。

Method: 使用视觉语言模型推断UI动作，将视频分割为短动作子序列并为每个子序列分配文本目标。在执行时采用两阶段选择机制动态选择最相关的轨迹作为上下文指导。

Result: 在两个广泛使用的基准测试中，该框架始终优于仅使用文本教程或转录的强基线代理。

Conclusion: 在线视频可以被系统性地提炼为可操作的指导，从而在推理时改进计算机使用代理的性能。
...
TL;DR: 本文提出了OCR-Rotation-Bench（ORB）基准测试，用于评估OCR对图像旋转的鲁棒性，并开发了一个基于Phi-3.5-Vision的轻量级旋转分类管道，在旋转校正任务上达到96%和92%的准确率，显著提升了OCR性能。
...
Method: 构建ORB基准测试（包含英文和多语言Indic数据集），并基于Phi-3.5-Vision视觉编码器开发轻量级旋转分类管道，采用动态图像裁剪技术，专门针对4类旋转任务进行微调。

Result: 旋转分类在ORB数据集上分别达到96%和92%的准确率；在模拟真实世界设置中，显著提升了闭源OCR模型（最高14%）和开源权重模型（最高4倍）的性能。

Conclusion: 提出的旋转分类模块在文档方向校正方面表现出色，对提升OCR性能具有关键作用，特别是在处理旋转文档时。
...
Method: 使用20个组织样本对，应用多种预处理步骤（包括CycleGAN、Macenko、Reinhard、Vahadane等颜色变换方法），采用VALIS配准方法进行刚性和非刚性配准，通过相对目标配准误差(rTRE)评估性能。

Result: 在两种场景下（使用原始或反转多模态图像），CycleGAN颜色变换均实现了最低的配准误差，而其他方法显示出较高的误差。
...
TL;DR: 该论文研究了协方差描述符在医学图像分类中的应用，特别是结合预训练的通用视觉编码器（GVEs）和专门为对称正定矩阵设计的SPDNet网络。结果表明，基于GVE特征的协方差描述符优于手工特征，且SPDNet与DINOv2特征结合时性能最佳。


<details>
  <summary>Details</summary>
Motivation: 协方差描述符在通用计算机视觉任务中表现出色，但在医学成像领域仍未被充分探索。本文旨在研究其在医学图像分类中的有效性，特别是结合预训练视觉编码器和专门网络。

Method: 使用预训练的通用视觉编码器（DINOv2和MedSAM）提取特征构建协方差描述符，并与手工描述符进行比较。在MedMNSIT基准的11个数据集上评估SPDNet分类网络。

Result: 基于GVE特征的协方差描述符始终优于手工特征。SPDNet与DINOv2特征结合时，性能优于最先进的方法。
...
Result: 实验结果表明，AStF在运动风格迁移任务中表现出色，超越了现有最优方法。
...
TL;DR: 该论文提出MedSapiens模型，通过将面向人类姿态估计的Sapiens基础模型适配到医学影像中的解剖标志点检测任务，在多个数据集上建立了新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 传统解剖标志点检测依赖领域特定模型，而大规模预训练视觉模型的出现提供了新机会。作者探索人类中心基础模型在空间姿态定位方面的固有优势如何为医学影像中的解剖标志点检测提供强先验。

Method: 通过多数据集预训练将Sapiens人类中心基础模型适配到医学成像领域，创建MedSapiens模型。

Result: MedSapiens在平均成功检测率上比通用模型提升5.26%，比专业模型提升21.81%。在少样本设置下比现有少样本方法提升2.69%。

Conclusion: 人类中心基础模型固有的空间姿态定位优化为解剖标志点检测提供了强先验，这一潜力此前未被充分挖掘。MedSapiens证明了这种适配策略的有效性。
...
TL;DR: Proto-LeakNet是一个基于信号泄漏的深度伪造图像溯源框架，通过在扩散模型潜在空间中检测生成器特定的统计痕迹，实现可解释的源归属分析。


<details>
  <summary>Details</summary>
Motivation: 随着合成图像和深度伪造生成模型的日益复杂，源归属和真实性验证成为计算机视觉系统的关键挑战。扩散模型在输出中会无意中留下持久的统计痕迹（信号泄漏），特别是在潜在表示中。

Method: 在扩散模型的潜在域中操作，通过部分前向扩散重模拟来暴露生成器特定的残余线索。使用时序注意力编码器聚合多步潜在特征，并通过特征加权原型头构建嵌入空间，实现透明归属。结合闭集分类和基于密度的开集评估，无需重新训练即可分析未知生成器。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注模态不变的外观特征，但忽视了步态特征——这种特征不仅模态不变，还富含时间动态信息，限制了跨模态视频匹配的时空一致性建模能力。

Method: 提出DINOv2驱动的步态表示学习框架，包含语义感知轮廓和步态学习模型(SASGL)以及渐进双向多粒度增强模块(PBMGE)，通过DINOv2语义先验增强轮廓表示，并在步态和外观流之间进行双向多粒度交互。

Result: 在HITSZ-VCM和BUPT数据集上的大量实验表明，该方法显著优于现有最先进方法。

Conclusion: DinoGRL框架通过结合DINOv2的视觉先验学习步态特征，有效解决了跨模态视频行人重识别中的挑战，实现了更鲁棒的序列级表示。
...
TL;DR: FastGS是一个新颖的3D高斯溅射加速框架，通过基于多视图一致性的致密化和修剪策略，显著提升训练速度，在多个数据集上实现2-7倍的加速，同时保持可比的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射加速方法未能有效调控高斯数量，导致计算冗余和时间开销。需要一种能够平衡训练时间和渲染质量的新方法。
...
Conclusion: FastGS是一个简单通用的加速框架，在保持渲染质量的同时显著提升训练效率，具有强大的泛化能力。
...
TL;DR: 本研究将通用视觉基础模型针对除草剂试验进行领域特定适配，通过自监督学习在农业数据集上训练，显著提升了物种识别和损伤分类性能，特别是在未见过的条件和领域迁移场景下表现优异，同时大幅减少了标注需求。
...
Result: 领域特定模型在物种识别（F1从0.91提升至0.94）和损伤分类（从0.26提升至0.33）上显著优于通用模型；在未见条件下提升更大（物种识别从0.56到0.66，损伤分类从0.17到0.27）；在无人机图像等域迁移场景中保持强劲性能（物种分类从0.49到0.60）。

Conclusion: 领域特定预训练增强了分割准确性，特别是在低标注情况下，可减少80%的标注样本需求，为除草剂试验分析提供了可扩展的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着海上基础设施的扩张，需要有效的监测系统，但现有模型在样本稀缺时表现不佳，特别是对于代表性不足的对象类别、形状和尺寸。

Method: 结合合成和真实的Sentinel-1卫星图像训练YOLOv10目标检测模型，并在三个未见区域（墨西哥湾、北海、波斯湾）进行地理可迁移性评估。

Result: 模型检测到3,529个海上平台，F1分数为0.85，加入合成数据后提升至0.90，证明了模型在训练区域之外的泛化能力。
...
TL;DR: RISE-T2V是一个创新的文本到视频生成框架，通过集成提示词重述和语义特征提取来解决现有模型对简洁提示理解不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频扩散模型依赖预训练文本编码器，但对简洁提示理解有限，无法在线重述提示词以更好地匹配用户意图，限制了模型的扩展性和可用性。

Method: 提出重述适配器模块，将提示词重述和语义特征提取整合为一步，利用LLM在预测下一个token时的文本隐藏状态作为视频生成条件。

Result: RISE-T2V是一个通用框架，适用于不同的视频扩散模型架构，显著提升了生成与用户意图一致的高质量视频的能力。

Conclusion: RISE-T2V通过隐式重述基本提示为更全面的表示，有效解决了文本到视频模型对简洁提示理解不足的问题，增强了模型的通用性和实用性。
...
TL;DR: 提出一种新的两阶段方法，结合体素稀疏化和子流形稀疏卷积网络，用于医学影像中肿瘤的自动分割，在保持高分辨率输入的同时显著减少计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 放射影像中肿瘤的精确分割是专业且耗时的任务，目前是临床环境中定量分析的瓶颈。传统方法需要下采样或使用图像块来处理3D扫描，这限制了分割精度。

Method: 采用两阶段方法：首先进行体素稀疏化，然后使用子流形稀疏卷积网络。该方法允许使用高分辨率输入和原生3D模型架构进行分割。

Result: 在KiTS23挑战赛的肾癌CT图像上，获得了与挑战赛优胜者相当的结果：肾脏+肿块Dice相似系数95.8%，肿瘤+囊肿85.7%，单独肿瘤80.3%。计算效率显著提升，推理时间减少60%，VRAM使用减少75%。

Conclusion: 该方法在保持最先进分割精度的同时，显著降低了计算资源需求，为临床环境中的定量分析提供了可行的解决方案。
...
TL;DR: 评估9种CNN架构在VOC 2008数据集上进行马和摩托车二分类的性能，通过少数类数据增强解决类别不平衡问题，发现ConvNeXt-Tiny表现最佳。


<details>
  <summary>Details</summary>
Motivation: 解决目标检测中的类别不平衡问题，比较不同现代神经网络架构在二分类任务中的性能差异。

Method: 使用ResNet-50、ConvNeXt-Tiny、DenseNet-121、Vision Transformer等9种架构，实施少数类数据增强技术，在VOC 2008数据集上进行马和摩托车的二分类实验。

Result: ConvNeXt-Tiny表现最佳，马检测AP达95.53%，摩托车检测AP达89.12%。数据增强显著改善了少数类检测性能，对深层架构尤其有益。
...
TL;DR: 本文研究了传感器遮挡对BEVFusion架构3D物体检测性能的影响，发现在单模态下相机和LiDAR对遮挡的敏感性不同，而在融合模式下模型更依赖LiDAR数据。


<details>
  <summary>Details</summary>
Motivation: 虽然BEV融合架构通过多模态集成展示了强大性能，但环境条件（如雾、霾或物理障碍）导致的传感器遮挡对3D检测精度的影响尚未充分探索。

Method: 使用BEVFusion架构在nuScenes数据集上评估相机和LiDAR遮挡的影响，检测性能通过mAP和NDS指标衡量。

Result: 相机中度遮挡导致仅相机检测的mAP下降41.3%，而LiDAR仅在重度遮挡下性能急剧下降（mAP下降47.3%）。融合模式下，遮挡相机仅导致4.1%的mAP下降，而遮挡LiDAR导致26.8%的下降。

Conclusion: 研究结果强调了未来需要开发遮挡感知评估方法和改进的传感器融合技术，以在部分传感器失效或环境条件恶化时保持检测精度。
...
TL;DR: 本教程提供了使用开源深度学习模型从分析化学成像数据中提取空间信息的逐步指南，包括MATLAB代码演示，旨在降低深度学习在分析化学中的应用门槛。


<details>
  <summary>Details</summary>
Motivation: 分析化学中虽然常用成像技术获取材料空间信息，但传统化学计量学方法难以有效提取和分析这些信息。深度学习虽能增强图像处理能力，但由于缺乏结构化实施指南，在分析化学领域的应用仍然有限。

Method: 使用现有的开源深度学习模型从成像数据中提取深度特征，提供MATLAB代码教程演示，涵盖分析化学中常见的各种成像模态数据处理。

Result: 开发了一个包含逐步指南和代码演示的教程，使读者能够在自己的数据集上应用深度学习方法来提取空间信息。

Conclusion: 本教程成功填补了分析化学领域应用深度学习的知识空白，为研究人员提供了实用的工具和方法来整合空间信息与其他数据源。
...
TL;DR: 提出基于LoRA微调Florence-2模型的多任务框架，同时处理视觉问答、解释生成和视觉定位任务，在医学VQA应用中显著提升准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决医学视觉问答中需要同时获得准确答案、解释和视觉定位的需求，通过多任务学习提升模型的综合表现和可解释性。

Method: 使用LoRA微调的Florence-2模型，整合三个数据集：Kvasir-VQA-x1用于问答学习、合成的解释数据集提供结构化医学推理、文本-区域对链接视觉特征与分割掩码。

Result: 评估显示该方法在答案准确性和视觉定位方面显著优于单任务基线，证明了基于视觉定位的多任务学习在医学VQA中的有效性。

Conclusion: 多任务框架通过联合学习视觉定位、推理和解释，能够产生既准确又可解释的响应，为医学VQA应用提供了有效的解决方案。
...
### [45] [BoRe-Depth: Self-supervised Monocular Depth Estimation with Boundary Refinement for Embedded Systems](https://arxiv.org/abs/2511.04388)
*Chang Liu,Juan Li,Sheng Zhang,Chang Liu,Jie Li,Xu Zhang*

Main category: cs.CV

TL;DR: 提出BoRe-Depth轻量级单目深度估计模型，参数量仅8.7M，在嵌入式系统上实现50.7 FPS的高效运行，显著提升深度估计性能和边界质量。
...
TL;DR: DORAEMON是一个开源的PyTorch库，统一了不同尺度的视觉对象建模和表示学习，提供YAML驱动的工作流、1000多个预训练骨干网络、模块化损失函数和数据增强，支持一键导出到ONNX或HuggingFace。


<details>
  <summary>Details</summary>
Motivation: 为了整合视觉识别和表示学习中的数据集、模型和训练技术，提供一个可扩展的平台，加速实验并将研究成果高效转移到实际应用。

Method: 使用YAML驱动的单一工作流覆盖分类、检索和度量学习；通过timm兼容接口提供1000多个预训练骨干网络；包含模块化损失函数、数据增强和分布式训练工具。

Result: 在ImageNet-1K、MS-Celeb-1M和Stanford在线产品等基准测试中，可复现的配方达到或超过参考结果；支持一键导出到ONNX或HuggingFace。

Conclusion: DORAEMON通过将数据集、模型和训练技术整合到一个平台中，为视觉识别和表示学习的快速实验提供了可扩展的基础，促进了研究成果向实际应用的高效转移。
...
Method: 集成SAM2与自定义训练的YOLOv11目标检测器。用户提供点坐标生成初始分割掩码，这些掩码作为YOLO模型的训练数据。然后通过边界框提示完全自动化管道，无需进一步手动干预。引入两个无监督指标：时间一致性DICE_t和新组件计数NC_t。
...
TL;DR: 本文提出了一种解决凸多边形拼图的新方法，扩展了传统仅限于方形拼图的自动求解器，通过结合几何和图像兼容性来重构无序碎片。


<details>
  <summary>Details</summary>
Motivation: 现有自动拼图求解器大多局限于方形拼图，严重限制了实际应用。本文旨在扩展可处理的拼图类型，专注于凸分区这一多边形拼图的主要子集。

Method: 结合几何和图像兼容性，引入贪心求解器来处理凸分区拼图。
...
TL;DR: V-Thinker是一个通用的多模态推理助手，通过端到端强化学习实现交互式视觉中心思维，在视觉交互推理任务上优于现有LMM基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在深度整合图像交互与长程推理能力方面存在局限，视觉工具空间有限且任务特定工作流设计限制了进展。

Method: 包含数据进化飞轮（自动合成、演进和验证交互推理数据集）和视觉渐进训练课程（通过点级监督对齐感知，再通过两阶段强化学习框架整合交互推理）。
...
TL;DR: 提出了一个三轴分析框架（传感器、标签、域）来适配地理空间基础模型Prithvi-EO-2.0进行滑坡测绘，该模型在跨传感器、跨区域和有限训练数据条件下优于传统深度学习方法。
...
Method: 使用基于全局预训练、自监督和可适应微调的地理空间基础模型Prithvi-EO-2.0，通过三轴分析框架（传感器、标签、域）进行滑坡测绘。

Result: 模型在光谱变化下保持稳健，在标签稀缺时维持准确性，在不同数据集和地理环境中具有更好的泛化能力，表现优于任务特定的CNN、视觉变换器和其他GeoFMs。
...
TL;DR: 提出一个包含8个指标的新评估框架，用于评估说话头部视频生成的质量、自然性和同步性，通过大规模实验发现现有算法在唇部同步方面表现良好，但在表情生成和细节处理方面仍有挑战。
...
Method: 提出包含8个指标的三维评估框架（质量、自然性、同步性），重点关注效率和对人类偏好的对齐，分析头部、嘴部和眉毛的细粒度动态以及面部质量。

Result: 在17个最先进模型生成的85,000个视频上进行实验，发现大多数算法在唇部同步方面表现良好，但在生成表情和无伪影细节方面面临挑战。
...
Method: 提出STC-Net框架，通过定位、窗口提议和分级模块联合执行时间定位和分级。引入结合硬和软定位目标及背景感知分级监督的新型损失函数，在完整视频上操作而无需手动剪辑。

Result: 在1,859个LC视频的私有数据集上评估，STC-Net达到62.11%的准确率和61.42%的F1分数，比非定位基线在两个指标上都高出10%以上。

Conclusion: STC-Net展示了从完整LC视频中基于PGS自动评估手术复杂度的可扩展有效方法，对于术后分析和手术培训具有应用前景。
...
TL;DR: 提出了"Thinking with Video"新范式，利用视频生成模型（如Sora-2）在统一的时间框架中桥接视觉和文本推理，克服了纯文本和纯图像推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的"Thinking with Text"和"Thinking with Images"范式存在固有局限：(1)图像只能捕捉单一时刻，无法表示动态过程；(2)文本和视觉作为不同模态分离，阻碍了统一的多模态理解和生成。

Method: 引入"Thinking with Video"范式，利用Sora-2等视频生成模型进行推理，并开发了VideoThinkBench基准测试，包含视觉中心任务和文本中心任务。

Result: Sora-2在视觉中心任务上可与最先进的VLM相媲美，在某些任务（如Eyeballing Games）上甚至超越VLM；在文本中心任务上，MATH准确率达92%，MMMU准确率达75.53%。自一致性和上下文学习可进一步提升性能。
...
### [54] [UniSplat: Unified Spatio-Temporal Fusion via 3D Latent Scaffolds for Dynamic Driving Scene Reconstruction](https://arxiv.org/abs/2511.04595)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏、非重叠相机视图和复杂场景动态的联合挑战下表现不佳，需要一种能够鲁棒重建动态场景的通用框架。
...
Conclusion: UniSplat通过统一的潜在时空融合框架，成功解决了自动驾驶中动态场景重建的挑战，实现了鲁棒高质量的3D重建和渲染。
...
TL;DR: 提出了一种自动评估虚拟免疫组化染色图像质量的新框架，通过颜色反卷积生成IHC阳性像素掩码，直接量化染色准确性，而不是传统的图像保真度指标。


<details>
  <summary>Details</summary>
Motivation: 当前基于纹理和分布的图像质量评估方法只能衡量图像保真度，无法准确评估IHC染色的准确性，这阻碍了虚拟IHC技术在病理学中的实际应用。

Method: 使用颜色反卷积生成真实和虚拟IHC图像的棕色染色像素掩码，计算Dice系数、IoU、Hausdorff距离等染色准确性指标，无需专家手动标注。

Result: 传统图像保真度指标（FID、PSNR、SSIM）与染色准确性和病理学家评估相关性差；配对模型如PyramidPix2Pix和AdaptiveNCE表现最佳，非配对扩散和GAN模型可靠性较低；全玻片图像评估显示性能下降。

Conclusion: 该框架为评估虚拟IHC模型质量提供了可重复的方法，是加速虚拟IHC技术向病理学常规应用转化的关键步骤。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法存在局限：全参考方法需要干净参考视频，大多数无参考模型依赖昂贵的人工标注训练，而大多数无人工标注的无参考方法基于图像，忽略了视频对象检测的关键时域上下文。

Method: 利用DAVIS数据集的合成退化，训练时域感知卷积架构直接从退化视频预测全参考指标（LPIPS、PSNR、SSIM），在推理时无需参考视频。

Result: 流式方法优于基于图像的基线，能够泛化到多种退化类型，与全参考指标的相关性高于广泛使用的BRISQUE图像质量评估基线。
...
TL;DR: 该论文提出了一种基于偏振分辨近红外成像的眼动追踪系统(PET)，通过测量眼组织反射光的偏振状态来增强追踪能力，相比仅使用光强的方法显著降低了注视误差。


<details>
  <summary>Details</summary>
Motivation: 传统眼动追踪仅使用光强信息，在眼睑遮挡、眼距变化和瞳孔大小变化等情况下性能受限。偏振成像可以提供额外的光学对比度机制，揭示在强度图像中不可见的可追踪特征。

Method: 使用偏振滤光片阵列相机配合线性偏振近红外照明器构建PET系统，通过卷积神经网络机器学习模型处理偏振数据，在346名参与者的大规模数据集上进行训练和测试。

Result: PET系统将95百分位绝对注视误差的中位数降低了10-16%，在正常条件以及眼睑遮挡、眼距变化和瞳孔大小变化等挑战性情况下均表现出优于强度基线方法的性能。

Conclusion: 偏振-组织相互作用能够为眼动追踪带来实际性能提升，PET作为一种简单、鲁棒的传感模式，有望应用于未来的可穿戴设备中。
...
TL;DR: 论文提出了一种诊断和去偏多模态基准测试的方法，通过测试集压力测试和迭代偏置剪枝来识别和消除非视觉偏见，确保基准测试真正评估视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准测试存在严重问题，模型可以通过利用偏见、语言先验和表面模式来获得高分，而不需要真正的视觉理解能力，这违背了基准测试的初衷。

Method: 提出两阶段框架：1）测试集压力测试(TsT)，通过仅使用文本输入进行k折交叉验证来识别可被利用的模式；2）迭代偏置剪枝(IBP)，过滤高偏见样本。

Result: 在四个基准测试(VSI-Bench、CV-Bench、MMMU、VideoMME)中发现了普遍的非视觉偏见，并通过VSI-Bench-Debiased案例展示了去偏后的基准测试具有更低的非视觉可解性和更大的视觉盲性能差距。

Conclusion: 基准测试设计应采用诊断原则，主动识别和消除非视觉偏见，确保真正评估模型的视觉理解能力，而非利用表面模式。
...
TL;DR: 提出了SIMS-V框架，利用3D模拟器的特权信息生成空间丰富的视频训练数据，通过系统消融实验发现三个关键问题类别（度量测量、视角依赖推理和时间跟踪）对开发可迁移空间智能最有效，仅用25K模拟示例训练的7B参数视频LLM在真实世界空间推理基准上超越了72B基线模型。
...
Method: 开发了SIMS-V系统化数据生成框架，利用3D模拟器的特权信息创建空间丰富的视频训练数据，并通过系统消融实验分析问题类型、混合和规模对真实世界迁移的影响。

Result: 识别出三个最有效的问题类别，仅用25K模拟示例训练的7B参数模型在真实世界空间推理基准上超越了72B基线模型，并与专有模型竞争，同时在通用视频理解上保持性能。

Conclusion: 模拟数据可以有效开发可迁移的空间智能，通过精心选择的问题类别可以实现高效训练，在保持通用性能的同时显著提升空间推理能力。
...
TL;DR: 本文提出从反应式任务驱动系统转向空间超感知的新范式，包含语义感知、流式事件认知、隐式3D空间认知和预测性世界建模四个阶段，并开发了VSI-SUPER基准测试来推动这一方向的发展。


<details>
  <summary>Details</summary>
Motivation: 当前多模态智能系统主要关注反应式任务驱动方法，缺乏对空间认知和世界建模的深度理解，需要转向更全面的超感知范式。

Method: 提出了VSI-SUPER基准测试（包含VSR和VSC任务），构建了VSI-590K数据集训练Cambrian-S模型，并开发了基于预测误差的自监督下一潜在帧预测器方法。

Result: 在VSI-Bench上实现了+30%的绝对改进，但在VSI-SUPER上表现仍然有限，表明单纯的数据扩展不足以实现空间超感知。预测性感知方法显著优于领先的专有基线。

Conclusion: 空间超感知需要模型不仅能够感知，还需要能够预测、选择和组织经验，预测性感知是实现真正多模态智能的关键路径。
...
TL;DR: InfinityStar是一个统一的时空自回归框架，用于高分辨率图像和动态视频合成，在VBench上获得83.74分，超越所有自回归模型，甚至优于一些扩散模型，生成720p视频速度比扩散方法快10倍。
...
Result: 在VBench上获得83.74分，大幅超越所有自回归模型，甚至优于HunyuanVideo等扩散竞争对手，生成5秒720p视频速度比领先扩散方法快10倍。

Conclusion: InfinityStar是首个能够生成工业级720p视频的离散自回归视频生成器，为高效高质量视频生成研究开辟了新方向。
...
TL;DR: 提出了Track Any State任务，用于跟踪物体在状态转换过程中的变化，并引入TubeletGraph零样本系统来恢复转换后丢失的物体，构建状态演化图。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的物体经常发生状态转换（如苹果被切成块、蝴蝶破茧而出），现有方法在物体外观发生显著变化时容易丢失跟踪目标。

Method: TubeletGraph系统首先识别可能被忽略的轨迹，基于语义和邻近性先验决定是否整合这些轨迹，然后对添加的轨迹进行推理并生成描述每个观察到的转换的状态图。

Result: TubeletGraph在状态转换下的跟踪性能达到最先进水平，同时展示了对物体转换的深度理解能力，以及在时间定位和复杂物体转换语义推理方面的潜力。

Conclusion: 该方法成功解决了物体状态转换过程中的跟踪问题，提供了对物体状态演化的深层理解，并为复杂物体转换的语义推理开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动图像裁剪方法主要关注生成单一裁剪，而现代社交媒体应用需要多个不同且美观的裁剪版本。

Method: 使用图像分割算法作为预处理步骤，评估了多种单裁剪模型在生成多个不同裁剪方面的效果。

Result: 提出了一个包含277张图像和相关人工标注的数据集，可用于评估多裁剪生成方法。

Conclusion: 该研究为社交媒体应用中的多裁剪图像生成问题提供了数据集和初步评估框架。
...
<details>
  <summary>Details</summary>
Motivation: LLM在生成中表现出隐含的人格特质，但可靠地控制或对齐这些特质以满足特定需求仍是一个开放挑战。需要有效的机制来在生成过程中操纵模型行为。

Method: 从transformer层提取隐藏状态激活，应用大五人格特质框架，使用低秩子空间发现方法，识别特质特定的最优层，并通过动态层选择的灵活引导框架进行操作化。

Result: 发现人格特质占据低秩共享子空间，这些潜在结构可以通过精心设计的扰动转化为有效的引导机制，而不影响流畅性、方差和通用能力。
...
TL;DR: TextualVerifier是一个基于大语言模型的验证框架，通过思维链分解、变体生成、多数投票和共识聚合四阶段工作流程，为TextGrad提供自验证机制，显著提升文本推理的有效性和优化结果的可靠性。
...
Result: 实验显示，在PRM800K上推理步骤有效性提升29%；与TextGrad集成后在GPQA-Diamond、MMLU-ML和MMLU-CP基准上分别获得8.08、10.71和3.92个百分点的提升，损失函数集成使准确率从68.2%提升至70.4%。
...
<details>
  <summary>Details</summary>
Motivation: 现有希腊方言数据集覆盖范围有限，需要更大规模、更多样化的方言数据来提升语言模型对希腊方言的理解和生成能力。

Method: 构建扩展的希腊方言数据集GRDD+，包含10种希腊方言变体。在Llama-3-8B、Llama-3.1-8B、Krikri-8B三种模型架构上进行微调，并与Claude-3.7-Sonnet、Gemini-2.5、ChatGPT-5等前沿模型进行比较。

Result: 创建了迄今为止规模最大、变体最多的希腊方言数据集，总词数达6,374,939词。通过微调实验验证了高质量方言数据对语言模型性能的影响。

Conclusion: GRDD+数据集为希腊方言研究提供了重要资源，微调实验表明方言数据能有效提升语言模型在方言任务上的表现，为多方言语言模型开发奠定了基础。
...
Method: 构建了1400亿token的波兰语预训练语料库、77k自定义指令数据集和100k偏好优化数据集；采用负责任AI框架，包含严格数据治理和混合模块进行输出校正与安全过滤；详细描述了模型架构、训练过程和基础与指令调优变体的对齐技术。

Result: 开发了PLLuM模型家族，并在公共管理下游任务中展示了其实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型对齐方法如微调计算成本高且不够优化，而推理时方法如Best-of-N采样需要不切实际的计算量才能达到最优对齐。

Method: STARS采用分段级token对齐与拒绝采样，在解码时对短固定大小的token片段进行迭代采样、评分和拒绝/接受决策，实现早期生成路径修正。
...
Method: 将多标签分类分解为独立的二分查询，每个目标维度单独询问；采用LLM-to-SLM蒸馏技术，使用DeepSeek-V3生成标注来微调小模型；结合前缀缓存机制优化推理效率。

Result: 微调后的模型在训练过的维度上相比零样本基线有显著提升，特别是在情感文本分析的24个维度上表现出色。
...
TL;DR: 本文分析了三种低资源语言（阿法尔奥罗莫语、阿姆哈拉语和提格里尼亚语）机器翻译数据集中的性别表示质量问题，发现数据存在严重的男性偏向、刻板印象和有害内容。


<details>
  <summary>Details</summary>
Motivation: 随着低资源语言被纳入NLP研究，大规模数据收集往往重数量轻质量，可能导致技术性能差和产生有害内容，因此需要调查这些语言机器翻译数据集的质量问题。

Method: 研究分析了三种低资源语言的机器翻译数据集，重点关注数据集中的性别表示，包括人名、动词语法性别和刻板印象描绘等方面。

Result: 发现训练数据大量包含政治和宗教领域文本，而基准数据集主要关注新闻、健康、体育；数据中存在严重的男性偏向，以及针对女性的有害和有毒描绘，数据量最大的语言问题最突出。

Conclusion: 数量不能保证质量，希望这项工作能激发对低资源语言数据集的进一步调查，并促进及早减轻有害内容。
...
TL;DR: GRAD是一种解码时方法，通过构建稀疏标记转移图来缓解LLM幻觉问题，无需重新训练模型。该方法在检索语料库上累积下一个标记的对数概率，并在解码时自适应融合模型对数概率，偏好高证据的续写。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部知识源，但基于提示的接地方法脆弱且领域敏感，符号知识集成需要高昂的检索和格式化成本。受知识图谱启发，希望开发轻量级方法在解码时利用语料证据来引导生成。

Method: GRAD通过单次前向传播在检索语料库上累积下一个标记的对数概率构建稀疏标记转移图。解码时，图检索的对数概率经过最大归一化并与模型对数概率自适应融合，偏好高证据的续写同时保持流畅性。

Result: 在三个模型和多个问答基准测试中，GRAD始终优于基线方法，内在准确率提高9.7%，幻觉率降低8.6%，正确性提高6.9%，在所有方法中获得最高的真实性-信息性乘积分数。

Conclusion: GRAD提供了一种轻量级、即插即用的替代方案，证明来自语料级标记转移的统计证据可以有效引导生成更真实和可验证的输出。
...
TL;DR: 论文研究了迭代指称游戏中人类和视觉语言模型的表现差异，发现在缺乏相关上下文时模型表现远不如人类，但在有相关上下文时模型性能会显著提升。


<details>
  <summary>Details</summary>
Motivation: 测试智能体在多轮语言环境中进行上下文敏感语用推理的能力，通过迭代指称游戏比较人类和视觉语言模型的表现。

Method: 使用迭代指称游戏试验，改变上下文的数量、顺序和相关性，测试人类和视觉语言模型的表现。

Result: 没有相关上下文时，模型表现优于随机但远逊于人类；有相关上下文时，模型性能随试验次数显著提升。

Conclusion: 具有抽象指称对象的少样本指称游戏对机器学习模型来说仍然是困难任务。
...
TL;DR: 开发了人类繁荣地理指数(HFGI)，通过分析26亿条地理标记的美国推文，使用微调的大语言模型对48个繁荣相关指标进行分类，提供县级和州级的月度、年度繁荣相关话语数据。


<details>
  <summary>Details</summary>
Motivation: 量化人类繁荣这一多维概念（包括幸福、健康、目标、美德、关系和经济稳定），以超越经济指标理解社会福祉，现有测量方法缺乏精细的空间和时间分辨率。

Method: 分析2013-2023年约26亿条地理标记的美国推文，使用微调的大语言模型对48个指标进行分类，这些指标与哈佛全球繁荣研究框架一致，还包括对移民态度和腐败感知。

Result: 数据集提供了县级和州级的月度、年度繁荣相关话语指标，经验证这些测量准确代表了基础构念，并与已建立指标显示出预期的相关性。
...
TL;DR: 论文提出了一种通过向量翻译实现跨模型语义通信的方法，替代传统的token传递，减少计算开销并提升信息传输效率。


<details>
  <summary>Details</summary>
Motivation: 在多智能体场景中，LLMs通过token传递消息会丢弃大部分潜在语义，限制了信息传输并增加不必要的计算开销。

Method: 使用双编码器翻译器在Llama-2-7B和Mistral-7B-Instruct之间学习映射，实现表示空间的直接语义交换，并以30%的混合强度注入翻译向量。

Result: 获得了0.538的平均余弦对齐度，双向评估显示2.01:1的传输不对称性，通用模型比指令调优模型产生更可转移的表示。
...
TL;DR: RUST-BENCH是一个包含7966个问题的表格推理基准，使用2031个真实世界表格，涵盖科学和体育两个领域，旨在评估LLM在复杂表格数据上的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有表格推理基准主要测试模型在小规模、统一表格上的表现，未能充分反映真实世界数据的复杂性，无法全面评估LLM的推理能力。真实表格通常具有长文本、异构性和领域特定性，需要跨数千个token进行多跳推理。

Method: 引入RUST-BENCH基准，包含两个领域：RB-Science（NSF资助记录）和RB-Sports（NBA统计数据），共7966个问题来自2031个真实世界表格。该基准联合评估LLM在规模、异构性、领域特定性和推理复杂性四个维度上的表现。

Result: 实验表明，开源和专有LLM在处理异构模式和复杂多跳推理时都存在困难，揭示了当前架构和提示策略的持续弱点。

Conclusion: RUST-BENCH为推进表格推理研究建立了一个具有挑战性的新测试平台，突出了当前LLM在真实世界表格数据推理方面的局限性。
...
TL;DR: 提出了一种将溯因推理整合到检索增强LLMs中的框架，通过检测不完整证据、生成候选缺失前提并进行验证，提高了RAG系统在知识密集型任务中的准确性和推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成(RAG)管道在检索证据不完整时经常失败，导致推理过程中存在空白。溯因推理提供了一种原则性的方法来填补这些空白。

Method: 提出一个集成溯因推理的框架，包括检测不充分证据、生成候选缺失前提，以及通过一致性和合理性检查进行验证。

Result: 在溯因推理和多跳问答基准测试中，该方法提高了答案准确性和推理可靠性。
...
TL;DR: 提出了T-FIX基准，用于评估LLM在知识密集型领域生成的解释与专家判断的对齐程度


<details>
  <summary>Details</summary>
Motivation: 当前LLM解释评估主要关注合理性和内部忠实性，但无法衡量解释内容是否真正符合专家直觉，特别是在知识密集型领域（如手术、天文学、治疗等）中，用户是领域专家，需要反映专家级推理的解释

Method: 与领域专家合作开发了新的指标来测量LLM解释与专家判断的对齐度，并构建了T-FIX基准，涵盖七个知识密集型领域

Result: 开发了T-FIX基准和相应的评估指标

Conclusion: 提出了专家对齐作为评估解释的新标准，强调在知识密集型环境中需要确保LLM解释与专家直觉真正一致
...
TL;DR: 提出了PoK框架，通过知识规划和对比时序检索器来增强大语言模型在时序知识图谱问答中的能力，显著提升了检索精度和推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法充分理解时间约束的复杂语义信息，而大语言模型虽然具有强大的语义理解和推理泛化能力，但时序推理能力有限，容易产生幻觉和知识缺失。

Method: 提出PoK框架，包含知识规划模块和对比时序检索器。知识规划模块将复杂时序问题分解为子目标序列，对比检索框架构建时序知识存储库，选择性检索语义和时间对齐的事实。

Result: 在四个基准TKGQA数据集上的实验表明，PoK显著提升了检索精度和推理准确性，相比最先进方法性能提升最高达56.0%。

Conclusion: PoK通过结构化规划和时序知识检索的结合，有效增强了时序推理的可解释性和事实一致性，解决了大语言模型在时序推理中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能像人类一样形成词汇联想，特别是对情感词汇的联想机制，以及这种联想是否具有创造性。

Method: 通过比较人类参与者和大型语言模型对情感词汇的联想反应，分析联想内容的重叠度、情感强度和创造性。

Result: 人类与LLMs的联想重叠度中等，但LLMs倾向于放大刺激词的情感负载，联想更可预测且缺乏创造性。

Conclusion: 大型语言模型的词汇联想机制与人类存在显著差异，在情感处理和创造性联想方面表现出局限性。
...
TL;DR: 本研究开发了一个基于Transformer的医疗信息去识别模型，通过在大型放射学报告数据集上进行训练，显著提升了受保护健康信息(PHI)的检测性能，超越了现有学术和商业系统。


<details>
  <summary>Details</summary>
Motivation: 提高放射学报告中受保护健康信息的自动去识别能力，通过大规模训练数据集扩展基于Transformer的模型，并与商业云供应商系统进行性能比较。

Method: 在现有最先进的Transformer-based PHI去识别流水线基础上，使用斯坦福大学两个大型注释放射学语料库进行微调，引入额外的PHI类别(AGE)，并采用"隐藏式"合成PHI生成方法评估稳定性。

Result: 模型在Penn数据集上总体F1得分为0.973，在斯坦福数据集上为0.996，优于或保持先前最先进模型性能。合成PHI评估显示在50个独立去识别数据集上检测一致性良好(总体F1: 0.959)，在所有供应商系统中表现最佳。

Conclusion: 基于Transformer的去识别模型在多样化放射学数据集上训练，在PHI检测方面优于先前的学术和商业系统，为安全临床文本处理建立了新的基准。
...
TL;DR: 本文研究了语言识别的k列表识别问题，给出了k列表识别的精确特征刻画，并建立了统计设置下的识别速率界限。


<details>
  <summary>Details</summary>
Motivation: 经典的语言识别在极限下是不可能的，但最近在语言生成方面的积极结果促使重新审视这个问题。通过给学习者提供每次输出k个猜测的能力，探索是否能够实现有效的语言识别。

Method: 基于Angluin特征刻画的递归版本，给出了k列表识别的精确特征刻画。将语言集合分解为k个子集合，每个子集合都可以用大小为1的列表进行识别。

Result: 证明了语言集合可以被k列表识别的充要条件是该集合可以分解为k个子集合，每个子集合都可以用大小为1的列表进行识别。在统计设置下，k列表可识别的集合可以以指数速率识别，这是最优的。

Conclusion: k列表识别为语言识别问题提供了新的解决方案，通过允许学习者输出多个猜测，可以克服经典识别中的限制，并建立了最优的识别速率界限。
...
TL;DR: 批处理提示不仅降低LLM推理成本，还能作为推理时正则化器，提高大推理模型的准确性和效率，减少3-5倍推理token使用，抑制过度思考，促进更果断的回答。


<details>
  <summary>Details</summary>
Motivation: 探索批处理在大型语言模型中的额外益处，特别是作为多步推理过程中的正则化机制，而不仅仅是吞吐量优化。

Method: 在13个多样化基准测试上进行综合研究，通过详细的行为分析考察批处理对模型推理行为的影响。

Result: 批处理显著提高准确性，减少3-5倍推理token使用，抑制过度思考，减少犹豫语言，促进更果断回答，并观察到批次内示例间的集体效应。
...
TL;DR: RIDE是一个基于项目反应理论的对抗性问题重写框架，通过生成难度可控的数学问题变体来评估LLMs的真实数学推理能力，避免数据泄露和表面模式匹配的影响。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在数学推理任务上的高表现可能源于训练数据泄露或表面模式匹配，而非真正的推理能力，需要更严格的评估方法来衡量真实能力。

Method: 使用项目反应理论(IRT)测量问题难度，通过35个LLMs模拟学生构建难度排序器，在强化学习中指导问题重写模型生成难度可控的数学问题变体。

Result: 在竞赛级数学基准测试中，RIDE生成的扰动版本使先进LLMs性能平均下降21.73%，暴露了数学推理的有限鲁棒性。

Conclusion: RIDE框架有效评估了LLMs的真实数学推理能力，证实了对抗性评估方法的有效性，揭示了当前模型在数学推理鲁棒性方面的局限性。
...
TL;DR: 提出CantoASR框架，结合ASR和大型音频语言模型，通过声学特征提取和韵律感知校正，显著提升低资源粤语语音识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 粤语作为低资源语言，存在标注数据有限、六声调、变调和口音变化等挑战，现有ASR模型如Whisper在粤语识别中词错误率较高。

Method: 采用协作式ASR-LALM错误校正框架，整合强制对齐进行声学特征提取、LoRA微调的Whisper改进音调识别，以及指令调优的Qwen-Audio进行韵律感知校正。

Result: 在自发粤语数据评估中，相比Whisper-Large-V3实现了显著的字符错误率提升。
...
TL;DR: 本文探索了三种多智能体LLM流水线来改进文本到SQL的生成，发现多智能体讨论能提升小模型性能，而LLM规划器-编码器流水线效果最佳，可将准确率从52.4%提升至56.4%。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM在从自然语言生成SQL时面临模式规模大和复杂推理的挑战，现有工作多关注复杂不实用的流水线，而忽略了小型高效模型。

Method: 提出了三种多智能体LLM流水线：(1)多智能体讨论流水线，智能体迭代批判和优化SQL查询；(2)规划器-编码器流水线，思考模型规划器生成逐步SQL生成计划，编码器合成查询；(3)编码器-聚合器流水线，多个编码器独立生成SQL查询，推理智能体选择最佳查询。

Result: 在Bird-Bench Mini-Dev集上的实验显示，多智能体讨论能提升小模型性能，Qwen2.5-7b-Instruct经过三轮讨论后执行准确率提升10.6%。LLM规划器-编码器流水线效果最佳，DeepSeek-R1-32B和QwQ-32B规划器将Gemma 3 27B IT准确率从52.4%提升至56.4%。

Conclusion: 多智能体方法能有效提升文本到SQL任务的性能，特别是LLM规划器-编码器流水线在开源模型中表现最佳，为小型高效模型的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成内容泛滥导致通信剧场化，发送者用LLM将简单想法膨胀为冗长内容，接收者又用LLM压缩回摘要，双方都不接触真实内容。

Method: 采用多智能体架构，通过结构化对话捕获发送者意图，评估信息捕获保真度、可重现性和查询响应完整性三个信任维度。
...
### [88] [Computational Turing Test Reveals Systematic Differences Between Human and AI Language](https://arxiv.org/abs/2511.04195)
*Nicolò Pagan,Petter Törnberg,Christopher A. Bail,Anikó Hannák,Christopher Barrie*

Main category: cs.CL

TL;DR: 本文提出了一个计算图灵测试框架来评估LLM生成文本的人类相似性，发现在情感表达等方面LLM输出仍与人类文本有明显区别，且人类相似性与语义保真度之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 当前社会科学中广泛使用LLM模拟人类行为，但缺乏对LLM生成文本真实性的稳健评估工具，现有基于人类判断的验证方法存在局限性。

Method: 引入计算图灵测试框架，结合聚合指标（BERT检测性和语义相似度）与可解释语言特征（风格标记和主题模式），系统比较9个开源LLM在5种校准策略下的表现。

Result: 即使经过校准，LLM输出在情感基调和情感表达方面仍与人类文本明显可区分；指令调优模型表现不如基础模型；模型规模扩大不会提升人类相似性；存在人类相似性与语义保真度的权衡。
...
TL;DR: 评估当前大语言模型是否能通过波兰国家上诉庭的官方资格考试，测试发现模型在知识测试中表现尚可，但在实践写作部分均未通过，且LLM作为评判者的评估与官方委员会判断存在差异。


<details>
  <summary>Details</summary>
Motivation: 研究当前LLMs在法律专业考试中的实际能力，探索LLM作为考生和评判者的可行性，评估其在法律实践中的适用性。

Method: 构建混合信息恢复和提取管道，测试多个LLM（包括GPT-4.1、Claude 4 Sonnet和Bielik-11B-v2.6）在闭卷和各种检索增强生成设置下的表现，比较LLM作为评判者与官方委员会的评估结果。

Result: 模型在知识测试中取得满意分数，但在实践写作部分均未达到及格线；LLM作为评判者的评估经常与官方委员会判断不一致。

Conclusion: 尽管技术进步迅速，当前LLMs尚无法在波兰公共采购裁决中替代人类法官或独立考官，存在幻觉、法律条款引用错误、逻辑论证薄弱等问题，需要法律专家与技术团队的密切合作。
...
TL;DR: REMIND是一种评估机器学习遗忘效果的新方法，通过分析模型在输入微小变化时的损失模式来检测未学习数据的残余影响，比现有方法更敏感且实用。
...
Method: REMIND通过分析模型在小输入变化上的损失模式来检测未学习数据的残余影响，发现未学习数据产生更平坦、不陡峭的损失景观，而保留或无关数据则呈现更尖锐、波动性更强的模式。

Result: REMIND在仅需查询访问的条件下优于现有方法，在不同模型、数据集和改写输入上表现出鲁棒性，适用于实际部署。
...
TL;DR: 通过检索增强生成和测试时计算量化预训练过程在数据集价值提取方面的效率，发现预训练未能充分利用现有数据集信息，检索可带来显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 理解预训练机制从数据中提取知识和思想的效率，量化预训练过程遗留在数据集中的价值。

Method: 使用检索增强生成和测试时计算，从标准开源数据集中检索信息，分析预训练与检索结合的效率。

Result: 在MMLU、Math-500和SimpleQA上观察到显著准确率提升，检索相当于5倍计算乘数，LLaMA 3.1 8B模型在MMLU上提升10个百分点。

Conclusion: 当前预训练方法未能充分利用现有预训练数据集的信息，仍有显著改进空间。
...
TL;DR: 提出一种基于图的方法进行主题标注，该方法通过丰富主题词并分析其关系来生成有意义的标签，在保持计算效率的同时取得了与ChatGPT-3.5相当的效果。
...
Method: 采用图基方法，通过丰富主题词的语义相关词汇并分析词汇间的关系来推导合适的主题标签。

Result: 在两个数据集上的比较研究表明，该方法在BERTScore和余弦相似度指标上优于传统基准方法，且效果与ChatGPT-3.5相当，同时保持计算效率。

Conclusion: 图基方法为高效的主题标注提供了可行方案，未来可进一步探索增强可解释性和自动化的研究方向。
...
TL;DR: SSPO提出句子级别的重要性比率计算方法，在GRPO和GSPO之间取得平衡，避免训练崩溃和采样数据利用率低的问题，并在五个数据集上取得最佳性能。
...
Method: SSPO采用句子级别的重要性比率计算，避免token级别的方差问题和响应级别的极端值影响，并应用句子熵来动态调整PPO-CLIP的裁剪边界。
...
TL;DR: 提出一种用于机器翻译微调的数据选择方法，通过定义可学习性分数，结合学习器模型和预训练参考模型的协同作用，提高训练效率和翻译性能。
...
Method: 使用学习器模型和预训练参考模型的协同作用，定义可学习性分数来评估数据点的训练效用，并采用考虑数据点间相互依赖关系的批量选择策略。

Result: 在英语到波斯语等多个语言对上的实验表明，该方法比iid基线提高了5倍的数据效率，使用缓存嵌入时计算效率提高24%，并实现了优于随机选择方法的翻译性能。

Conclusion: 该方法能够显著提高数据效率和计算效率，同时增强泛化能力，为机器翻译系统的微调提供了有效的数据选择解决方案。
...
TL;DR: 本研究测试了LLMs在时间推理方面的能力，使用1940年挪威书籍中的常识问题，让LLMs以1940年的视角回答问题，并比较了英语和挪威语提示的效果。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs进行时间推理的能力，特别是处理历史时期知识的能力。

Method: 使用1940年挪威书籍中的常识问题，让LLMs以1940年的视角回答，测试了英语和挪威语两种语言提示，采用LLM-as-judge进行评分。

Result: 英语提示的效果始终优于挪威语提示，更大的LLMs表现更好，测试了多个模型家族。

Conclusion: LLMs在时间推理方面表现出一定能力，但语言选择对结果有显著影响，英语提示效果更好。
...
TL;DR: PTTSD是一个概率性文本时间序列抑郁症检测框架，通过临床访谈的逐句文本预测PHQ-8抑郁评分，同时建模时间不确定性。
...
Conclusion: 注意力机制和概率建模具有重要价值，不确定性感知预测具有临床相关性和可解释性，为抑郁症检测提供了可靠工具。
...
TL;DR: ThaiOCRBench是首个针对泰语文本视觉理解任务的综合基准测试，包含2,808个样本和13个任务类别，评估显示专有模型优于开源模型，特别是在细粒度文本识别和手写内容提取方面。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注高资源语言，泰语在多模态建模中代表性不足，尤其是在需要文档结构理解的任务中。
...
Conclusion: 多轮简化方法在可读性控制的文本简化任务中表现良好，特别是结合LLM简化候选作为起点的策略能有效提升性能。
...
TL;DR: 本文系统评估了6个大型语言模型在BFI-2人格框架下的表现，发现不同模型在四个主要人格维度上存在显著差异，神经质和外向性受温度参数影响明显。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在人类中心应用中的普及，理解其类似人格的行为模式对于负责任开发和部署至关重要。

Method: 应用BFI-2人格框架评估六个LLM，在不同采样温度下分析人格特质表达，并使用层次聚类识别模型分组。

Result: 发现模型在四个人格维度上存在显著差异，神经质和外向性对温度调整敏感，层次聚类显示模型具有稳定的人格特征分组。

Conclusion: 研究为理解LLM中人格模式的出现提供了新见解，为模型调优、选择和AI系统伦理治理提供了新视角。
...
TL;DR: RAGalyst是一个自动化、与人类标注对齐的代理框架，用于严格评估领域特定的RAG系统，通过生成高质量合成QA数据集和优化LLM-as-a-Judge指标来实现与人类判断的高度相关性。
...
Result: 在三个不同领域（军事行动、网络安全、桥梁工程）评估发现，RAG性能高度依赖上下文，没有单一嵌入模型、LLM或超参数配置普遍最优，并分析了RAG中低答案正确性的最常见原因。

Conclusion: RAGalyst系统评估框架的必要性得到证实，使从业者能够发现领域特定的权衡，并为构建可靠有效的RAG系统做出明智的设计选择。
...
<details>
  <summary>Details</summary>
Motivation: 放射学报告包含两种不确定性：显性不确定性（通过模糊表达反映对发现的怀疑）和隐性不确定性（由于省略推理部分导致的不确定性），现有基于规则的系统无法充分处理这些不确定性。

Method: 1. 量化显性不确定性：创建专家验证的LLM参考排名，将常见模糊短语映射到概率值；2. 建模隐性不确定性：通过扩展框架系统性地添加从专家定义诊断路径中推导的特征性子发现。

Result: 发布了Lunguage++，这是Lunguage细粒度结构化放射学报告基准的扩展、不确定性感知版本，支持14种常见诊断。
...
TL;DR: 通过控制语言模型的隐藏激活来预测和干预其在思维链推理中的不确定性，发现模型在不确定时更容易被引导，且隐藏激活能预测未来的输出分布。


<details>
  <summary>Details</summary>
Motivation: 语言模型在生成文本时，单个token的选择可能导致完全不同的推理路径，这使得不确定性难以量化。研究旨在探索语言模型是否在生成过程中表征了可能的替代路径。

Method: 使用隐藏激活来控制并预测语言模型在思维链推理过程中的不确定性，通过激活干预实验来测试模型的可操控性。

Result: 发现模型在不同token上的不确定程度与其被激活控制的可操控性存在明显相关性；隐藏激活能够预测模型未来的输出分布。

Conclusion: 激活干预在模型尚未确定最终答案时最为有效，表明语言模型隐式地表征了可能的推理路径空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动论文评分系统缺乏用户体验，IntelliProof旨在弥合议论文结构语义与用户理解之间的差距，提供更直观的分析工具。

Method: 将论文构建为论证图：节点代表论点，节点属性包含支持证据，边编码支持或攻击关系；使用LLM进行初始分类评分并可视化。

Result: 系统能够快速探索论证质量，提供分类理由和连贯性量化指标，支持自然语言理解工具，保持人工监督。

Conclusion: IntelliProof通过交互式论证图分析和自然语言工具，有效提升了议论文分析的用户体验和理解深度。
...
TL;DR: LLM代码助手生成的漏洞在网络安全中日益重要，但现有安全基准和改进方法对主流编码LLM的影响有限。研究发现最新开源模型在早期报告漏洞场景中仍然脆弱，安全-功能权衡阻碍了有效修复。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的编码助手在软件开发中作用日益关键，其生成的漏洞对网络安全的影响也愈发重要。现有安全基准和改进方法对主流编码LLM的实际影响尚不明确。

Method: 引入新的严重性度量Prompt Exposure(PE)，综合考虑漏洞严重性、生成概率和诱导漏洞代码生成的提示表述。基于PE定义Model Exposure(ME)评分，指示模型生成漏洞的严重性和普遍性。

Result: 研究发现即使最新的开源模型在现实使用场景中仍然对早期报告的漏洞场景脆弱，表明安全-功能权衡阻碍了漏洞的有效修补。

Conclusion: 需要新的严重性度量来鼓励缓解最严重和普遍的漏洞，提出的PE和ME评分有助于识别和优先处理高风险漏洞。
...
TL;DR: 本文介绍了首个孟加拉语生物医学问答数据集BanglaMedQA和BanglaMMedBench，并评估了多种检索增强生成(RAG)策略在低资源语言医疗AI中的表现。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言中生物医学问答系统开发面临的挑战，促进医疗知识的公平获取。

Method: 应用多种RAG策略（传统、零样本回退、代理、迭代反馈和聚合RAG），结合基于教科书的检索和网络检索，通过OCR构建孟加拉语医学教科书语料库，并实现动态选择检索与推理策略的代理RAG流程。

Result: 代理RAG在使用openai/gpt-oss-120b时达到最高准确率89.54%，优于其他配置，并展现出更优的推理质量。

Conclusion: RAG方法能够显著提升孟加拉语医疗问答的可靠性和可访问性，为多语言医疗AI研究奠定了基础。
...
TL;DR: DeReC是一个轻量级事实验证框架，用通用文本嵌入替代自回归LLM方法，结合密集检索和分类，在保持高准确率的同时显著提升效率
...
Result: 在RAWFC数据集上F1得分65.58%，超越L-Defense(61.20%)，运行时间减少95%(23分36秒 vs 454分12秒)，在LIAR-RAW上减少92%(134分14秒 vs 1692分23秒)
...
<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought提示虽然能实现复杂推理，但生成完整固定长度的推理过程在计算上是浪费的，会增加token使用和延迟。

Method: LEASH监控两个内在信号：token级熵的斜率和top-logit边际的改进，当两个信号都达到平稳状态时停止生成，表明模型已达到稳定的推理状态。

Result: 在四个指令调优模型上的GSM8K和AQuA-RAT基准测试中，LEASH平均减少30-35%的token生成和27%的延迟，相对于CoT只有10个百分点的准确率下降。
...
### [108] [On the Existence of Fair Allocations for Goods and Chores under Dissimilar Preferences](https://arxiv.org/abs/2511.03810)
*Egor Gagushin,Marios Mertzanidis,Alexandros Psomas*

Main category: cs.GT

TL;DR: 本文解决了多组代理人对多种不可分割物品的公平分配问题，为任意数量的组和物品类型提供了明确的μ上界，使得当每种物品出现至少μ次时存在无嫉妒分配。


<details>
  <summary>Details</summary>
Motivation: Gorantla等人的研究证明了当每种物品类型出现足够多次时存在无嫉妒分配，但他们的证明是非构造性的，且只对两组代理或两种物品类型给出了明确的μ上界。本文旨在解决他们提出的主要开放问题。

Method: 引入了一种显著更简单但功能强大的技术，不仅为不可分割物品提供构造性保证，还自然地扩展到杂务和连续域。

Result: 为任意数量的组和物品类型推导出了明确的μ上界，解决了Gorantla等人提出的开放问题。

Conclusion: 该技术不仅解决了不可分割物品的公平分配问题，还扩展到杂务和连续域，在蛋糕切割等相关公平分配设置中带来了新的结果。
...
### [109] [The Complexity of Equilibrium Refinements in Potential Games](https://arxiv.org/abs/2511.03968)
*Ioannis Anagnostides,Maria-Florina Balcan,Kiriaki Fragkia,Tuomas Sandholm,Emanuel Tewolde,Brian Hu Zhang*

Main category: cs.GT

TL;DR: 本文解决了计算势博弈中均衡精炼的复杂度问题，证明了纯策略完美均衡在多种博弈表示下是PLS完全的，正规形式适当均衡在扩展形式博弈中是FIXP_a完全的，但在多面体势博弈中是NP-hard和coNP-hard的。


<details>
  <summary>Details</summary>
Motivation: 计算均衡精炼的复杂度一直是算法博弈论研究的重点，但在经典的势博弈类中这一问题长期悬而未决。

Method: 通过分析不同博弈表示下的均衡计算复杂度，包括扩展形式博弈、多面体博弈、对称网络拥塞博弈和对称拟阵拥塞博弈等。

Result: 建立了纯策略完美均衡的PLS完全性，正规形式适当均衡的FIXP_a完全性，以及在结构化博弈类中多项式时间计算完美均衡的可能性。

Conclusion: 在势博弈中，均衡精炼的计算复杂度存在显著差异，某些情况下可以高效计算，而其他情况下则具有很高的计算难度。
...
### [110] [Fraud-Proof Revenue Division on Subscription Platforms](https://arxiv.org/abs/2511.04465)
*Abheek Ghosh,Tzeh Yuan Neoh,Nicholas Teh,Giannis Tyrovolas*

Main category: cs.GT

TL;DR: 研究订阅平台收入分配机制，发现现有机制易受欺诈操纵，提出满足三种防操纵公理的新规则ScaledUserProp，实验证明其公平性优于现有规则。


<details>
  <summary>Details</summary>
Motivation: 现有基于机器学习的欺诈检测方法在与恶意行为者的持续对抗中效果有限，需要设计能够从根本上抑制操纵的收入分配机制。

Method: 形式化三种防操纵公理，分析现有规则满足情况，提出新的ScaledUserProp规则，并在真实和合成流媒体数据上进行实验验证。

Result: 发现广泛使用的流媒体平台机制不仅无法防止欺诈，还使操纵检测计算不可行；ScaledUserProp满足所有三种防操纵公理，实验显示其公平性更优。

Conclusion: ScaledUserProp作为满足防操纵公理的新机制，为订阅平台提供了比现有规则更公平的收入分配方案。
...
### [111] [Fisher Meets Lindahl: A Unified Duality Framework for Market Equilibrium](https://arxiv.org/abs/2511.04572)
*Yixin Tao,Weiqiang Zheng*

Main category: cs.GT

TL;DR: 提出一个统一的对偶框架，将公共品的Lindahl均衡与私有品的Fisher市场均衡联系起来，通过交换分配和价格的角色实现对应关系。


<details>
  <summary>Details</summary>
Motivation: Fisher市场均衡已有深入研究，但Lindahl均衡的理论基础相对薄弱，需要建立统一框架来填补这一空白。

Method: 构建对偶框架，将Lindahl均衡映射到具有对偶效用的Fisher市场均衡，并利用此框架分析福利性质、市场动态和公共劳役问题。

Result: 对于凹齐次效用，Lindahl均衡最大化Nash社会福利；对于凹非齐次效用，获得(1/e)^(1/e)近似比且该比率为紧的。扩展了比例响应动态和tâtonnement动态到Lindahl均衡。

Conclusion: 对偶框架为Lindahl均衡提供了计算和动态分析的新工具，统一了私有品和公共品市场均衡理论，并扩展到劳役市场。
...
### [112] [Tutorial Debriefing: Applied Statistical Causal Inference in Requirements Engineering](https://arxiv.org/abs/2511.03875)
*Julian Frattini,Hans-Martin Heyn,Robert Feldt,Richard Torkar*

Main category: cs.SE

TL;DR: 软件工程研究需要将研究成果转化为实践，这需要证明工具、流程或指南与性能指标之间的因果关系。当随机对照试验不可行时，需要从观察数据中进行统计因果推断。


<details>
  <summary>Details</summary>
Motivation: 软件工程研究社区旨在改善软件生产者和消费者的状况，这需要将研究成果转化为实践工具、流程和指南。但这类贡献的价值取决于能否证明它们与性能改进之间的因果关系。

Method: 当随机对照试验在法律、伦理或后勤上不可行时，采用统计因果推断方法从观察数据中获取因果关系的证据。

Result: 论文提出了在无法进行随机对照试验的情况下，使用统计因果推断作为获取因果关系证据的可靠方法。

Conclusion: 统计因果推断为软件工程研究提供了一种在随机对照试验不可行时获取因果证据的可行替代方案，有助于研究成果向实践的转化。
...
### [113] [Collaborative Agents for Automated Program Repair in Ruby](https://arxiv.org/abs/2511.03925)
*Nikta Akbarpour,Mahdieh Sadat Benis,Fatemeh Hendijani Fard,Ali Ouni,Mohamed Aymen Saied*

Main category: cs.SE

TL;DR: RAMP是一个轻量级的多智能体框架，将程序修复构建为反馈驱动的迭代过程，专门针对Ruby语言，无需依赖大型多语言修复数据库或昂贵微调。


<details>
  <summary>Details</summary>
Motivation: 现有自动程序修复方法计算成本高且主要关注少数语言，Ruby作为广泛使用的Web开发语言在APR研究中关注不足。

Method: 采用协作智能体团队，通过生成针对性测试、错误反思和候选修复精炼的迭代过程，使用轻量级提示和测试驱动反馈直接在Ruby上操作。
...
### [114] [PEFA-AI: Advancing Open-source LLMs for RTL generation using Progressive Error Feedback Agentic-AI](https://arxiv.org/abs/2511.03934)
*Athma Narayanan,Mahesh Subedar,Omesh Tickoo*

Main category: cs.SE

TL;DR: 提出了一种多智能体协作的RTL生成流程，通过渐进式错误反馈系统实现无需人工干预的自动化RTL设计，在开源数据集上取得了最先进的通过率。


<details>
  <summary>Details</summary>
Motivation: 解决传统RTL生成方法需要人工干预的问题，通过智能体协作和自校正机制实现完全自动化的硬件设计流程。

Method: 采用多智能体架构，结合专用LLM和硬件仿真工具，通过渐进式错误反馈系统(PEFA)进行迭代式错误修正，逐步增加设计复杂度。

Result: 在两个开源自然语言到RTL数据集上的基准测试表明，该方法实现了最先进的通过率，且在token使用上更加高效，有效缩小了开源和闭源LLM之间的性能差距。

Conclusion: 该智能体流程为自动化硬件设计提供了可行的解决方案，通过自校正机制显著提升了RTL生成的质量和效率。
...
### [115] [PSD2Code: Automated Front-End Code Generation from Design Files via Multimodal Large Language Models](https://arxiv.org/abs/2511.04012)
*Yongxi Chen,Lei Chen*

Main category: cs.SE

TL;DR: PSD2Code是一种新颖的多模态方法，通过PSD文件解析和资源对齐生成生产就绪的React+SCSS代码，解决了现有设计到代码生成方法中的结构不一致、资源错位和有限的生产就绪性问题。


<details>
  <summary>Details</summary>
Motivation: 现有设计到代码生成方法存在结构不一致、资源错位和有限的生产就绪性问题，需要一种能够生成高质量、生产就绪前端代码的解决方案。

Method: 采用ParseAlignGenerate流水线，从PSD文件中提取层次结构、图层属性和元数据，为大型语言模型提供精确的空间关系和语义分组。使用基于约束的对齐策略确保生成元素与设计资源的一致性，并通过结构化提示构建增强可控性和代码质量。

Result: 综合评估显示，在代码相似性、视觉保真度和生产就绪性等多个指标上显著优于现有方法。该方法在不同大型语言模型上表现出强大的模型独立性。

Conclusion: 将结构化设计信息与多模态大型语言模型相结合，为工业级代码生成提供了有效方法，标志着设计驱动自动化前端开发的重要进展。
...
### [116] [Specification-Guided Vulnerability Detection with Large Language Models](https://arxiv.org/abs/2511.04014)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在漏洞检测方面表现有限，主要原因是缺乏对安全规范的理解——即代码应该如何表现才能保持安全。当代码行为与这些期望不符时，就形成了潜在漏洞，但这种知识在训练数据中很少明确存在。

Method: VulInstruct从两个角度构建规范知识库：(i) 从跨项目的高质量补丁中提取通用规范，捕捉基本的安全行为；(ii) 从与目标代码相关的特定仓库中重复违规提取领域特定规范。通过检索相关历史案例和规范，使LLM能够基于期望的安全行为进行推理。

Result: 在PrimeVul数据集上，VulInstruct达到45.0% F1分数（提升32.7%）和37.7%召回率（提升50.8%），独特检测到24.3%的漏洞——比任何基线多2.4倍。在成对评估中实现32.3%的相对改进。还发现了一个先前未知的高严重性漏洞(CVE-2025-56538)。

Conclusion: VulInstruct通过系统性地利用安全规范知识，显著提升了大语言模型在漏洞检测方面的能力，证明了基于规范推理的方法在实际漏洞发现中的实用价值。
...
### [117] [LLM-Driven Adaptive Source-Sink Identification and False Positive Mitigation for Static Analysis](https://arxiv.org/abs/2511.04023)
*Shiyin Lin*
...
Method: 结合LLM推理与符号验证，通过神经符号推理自适应推断源/汇规范，并将模型建议基于程序事实和约束验证。

Result: 在Juliet 1.3、SV-COMP风格C基准测试和三个大型真实项目中，相比CodeQL、Joern和纯LLM流水线，平均减少43.7%误报，提高11.2%召回率，同时保持有竞争力的运行时开销。
...
### [118] [Benchmarking and Studying the LLM-based Agent System in End-to-End Software Development](https://arxiv.org/abs/2511.04064)
*Zhengran Zeng,Yixin Li,Rui Xie,Wei Ye,Shikun Zhang*

Main category: cs.SE

TL;DR: 提出了E2EDevBench基准和混合评估框架，通过控制实验发现当前软件开发代理只能满足约50%需求，主要瓶颈是需求遗漏和验证不足。


<details>
  <summary>Details</summary>
Motivation: 解决LLM自主代理在软件开发中评估困难的问题，包括基准过于简单和不同架构比较不公平的挑战。

Method: 构建动态维护的E2EDevBench基准，提出结合测试用例功能评估和细粒度LLM需求验证的混合框架，在统一基础上实现三种代表性代理架构进行控制实验。

Result: 最先进的代理在基准上只能满足约50%需求，成功关键取决于任务分解和协作的架构策略，主要瓶颈是需求遗漏和自验证不足。

Conclusion: 为社区提供了更现实的基准、全面评估框架，以及对软件开发代理当前能力和核心挑战的关键见解，指导未来研究提升需求理解和规划能力。
...
### [119] [How Natural Language Proficiency Shapes GenAI Code for Software Engineering Tasks](https://arxiv.org/abs/2511.04115)
*Ruksit Rojpaisarnkit,Youmei Fan,Kenichi Matsumoto,Raula Gaikovina Kula*

Main category: cs.SE

TL;DR: 研究英语语言能力对LLM生成代码质量的影响，发现高级英语提示能提升代码正确率
...
Method: 使用HumanEval数据集，系统地将164个编程任务的提示从基础到高级英语水平进行变化，测量生成代码的熟练度和正确性
...
### [120] [Are We Aligned? A Preliminary Investigation of the Alignment of Responsible AI Values between LLMs and Human Judgment](https://arxiv.org/abs/2511.04157)
*Asma Yamani,Malak Baslyman,Moataz Ahmed*

Main category: cs.SE

TL;DR: 本研究评估了23个LLM在负责任AI价值观方面与人类群体的对齐程度，发现LLM与AI从业者的价值观更接近，但在声称的价值观与实际需求优先级之间存在不一致性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在软件工程任务中的广泛应用，需要评估其负责任AI价值观是否与人类判断一致，特别是在需求获取、设计和评估等关键环节。

Method: 评估23个LLM在四个任务上的表现：选择关键负责任AI价值观、在特定情境中评分重要性、解决竞争价值观间的权衡、以及优先体现这些价值观的软件需求。

Result: LLM总体上与AI从业者的价值观更接近，强调公平、隐私、透明度、安全和问责制，但在声称的价值观与实际需求优先级之间存在不一致性。

Conclusion: 在需求工程中依赖LLM存在实际风险，需要系统方法来基准测试、解释和监控AI辅助软件开发中的价值观对齐。
...
### [121] [Explaining Software Vulnerabilities with Large Language Models](https://arxiv.org/abs/2511.04179)
*Oshando Johnson,Alexandra Fomina,Ranjith Krishnamurthy,Vaibhav Chaudhari,Rohith Kumar Shanmuganathan,Eric Bodden*

Main category: cs.SE

TL;DR: SAFE是一个IDE插件，利用GPT-4o解释SAST工具检测到的漏洞的原因、影响和缓解策略，旨在提高SAST工具的可解释性和可用性。


<details>
  <summary>Details</summary>
Motivation: SAST工具通常提供通用的警告信息，无法充分向开发者传达重要信息，导致误解或忽略关键发现。LLM的文本生成能力为解决SAST可解释性挑战提供了新机会。

Method: 开发了SAFE IDE插件，采用混合方法利用GPT-4o生成漏洞解释，包括原因、影响和缓解策略，并通过专家用户研究进行评估。

Result: 专家用户研究发现，SAFE生成的解释能显著帮助初学者到中级开发者理解和解决安全漏洞，从而改善SAST工具的整体可用性。

Conclusion: 基于LLM的混合方法能有效解决SAST工具的可解释性问题，SAFE插件展示了在提高开发者理解和处理安全漏洞方面的实用价值。
...
### [122] [GITER: A Git-Based Declarative Exchange Model Using Kubernetes-Style Custom Resources](https://arxiv.org/abs/2511.04182)
*Christos Tranoris*

Main category: cs.SE

TL;DR: 提出了一种基于Git的轻量级、可审计的异步信息交换方法，用Git作为协调媒介替代传统API和消息代理，支持跨域、跨组织和隔离环境协作。


<details>
  <summary>Details</summary>
Motivation: 传统API和消息代理在分布式系统集成中存在复杂性、耦合度高和审计困难等问题，需要一种更透明、可追溯且松耦合的通信模式。

Method: 采用Kubernetes Operators和Custom Resources模式，通过共享Git仓库作为单一事实源，使用spec字段表示期望状态，status字段反映实际结果，利用Git原生特性实现版本控制、提交签名和访问控制。

Result: 构建了一个基于Git的声明式通信模型，确保透明度、可追溯性和可复现性，同时保持系统间的松耦合和自治性。

Conclusion: Git作为声明式通信基板具有显著优势，能够有效支持复杂分布式环境下的信息交换，但需要权衡其与传统RESTful和代理集成方式的利弊。
...
### [123] [A Tool for Benchmarking Large Language Models' Robustness in Assessing the Realism of Driving Scenarios](https://arxiv.org/abs/2511.04267)
*Jiahui Wu,Chengjie Lu,Aitor Arrieta,Shaukat Ali*

Main category: cs.SE

TL;DR: 提出了DriveRLR基准工具，用于评估大型语言模型在判断驾驶场景真实性方面的鲁棒性。该工具通过生成变异场景变体和构建提示词来测试LLM的能力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统安全性评估需求迫切，基于仿真的测试方法成本低但难以评估场景真实性。LLM具有强大的推理和泛化能力，可能通过文本提示有效评估场景真实性。

Method: 开发DriveRLR工具，生成变异场景变体，构建提示词，然后评估LLM在判断驾驶场景真实性方面的鲁棒性。在DeepScenario数据集上使用GPT-5、Llama 4 Maverick和Mistral Small 3.2三个先进LLM进行验证。

Result: DriveRLR有效揭示了不同LLM在鲁棒性方面的差异，证明了其在场景真实性评估中的有效性和实用价值。

Conclusion: DriveRLR不仅是LLM鲁棒性评估工具，还可作为实际应用组件，如作为目标函数指导场景生成，支持基于仿真的自动驾驶系统测试工作流。
...
### [124] [Where Do LLMs Still Struggle? An In-Depth Analysis of Code Generation Benchmarks](https://arxiv.org/abs/2511.04355)
*Amir Molzam Sharifloo,Maedeh Heydari,Parsa Kazerooni,Daniel Maninger,Mira Mezini*

Main category: cs.SE

TL;DR: 分析大型语言模型在代码生成任务中的失败模式，识别了四个主要弱点模式和导致失败的常见任务复杂性因素


<details>
  <summary>Details</summary>
Motivation: 现有基准测试和排行榜仅提供模型排名，但缺乏对LLMs持续失败任务的分析，这些信息对理解模型局限性和指导未来发展至关重要

Method: 通过分析四个流行基准测试中的代码生成任务，识别LLMs最可能失败的任务；研究解决方案代码的静态复杂性是否导致失败；系统检查114个LLMs持续失败的任务

Result: 发现了LLMs在代码生成中的四个重复出现的弱点模式，以及基准测试任务中最常导致失败的常见复杂性因素

Conclusion: 需要更深入地理解LLMs的失败模式，而不仅仅是性能排名，这对开发更强大的模型至关重要
...
### [125] [Speed at the Cost of Quality? The Impact of LLM Agent Assistance on Software Development](https://arxiv.org/abs/2511.04427)
*Hao He,Courtney Miller,Shyam Agarwal,Christian Kästner,Bogdan Vasilescu*

Main category: cs.SE

TL;DR: 采用Cursor LLM助手能显著但短暂提升开发速度，但会导致静态分析警告和代码复杂度显著持续增加，最终造成长期速度放缓。


<details>
  <summary>Details</summary>
Motivation: 缺乏关于LLM代理助手对软件开发生产力和质量影响的实证证据，需要量化评估其因果效应。

Method: 使用差异中的差异设计，比较采用Cursor的GitHub项目与匹配的未使用Cursor的控制组项目。

Result: Cursor采用导致项目级开发速度显著但短暂提升，同时静态分析警告和代码复杂度显著持续增加。

Conclusion: LLM助手虽然短期提升生产力，但可能以牺牲代码质量为代价，导致长期开发速度下降，这对实践者、工具设计者和研究者都有重要启示。
...
### [126] [EDIT-Bench: Evaluating LLM Abilities to Perform Real-World Instructed Code Edits](https://arxiv.org/abs/2511.04486)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏直接评估LLM代码编辑能力的基准测试，现有数据集往往依赖人工生成的数据，无法反映真实使用场景。

Method: 从真实使用场景收集用户指令和代码上下文，构建包含545个问题的基准测试，涵盖多种语言和真实用例，包括错误修复、功能添加等。

Result: 评估了40个不同的LLM，只有5个模型得分超过60%，表明该基准具有挑战性。模型性能在不同指令类别间差异显著，上下文信息对任务成功率影响很大（性能差异达11%）。

Conclusion: EDIT-Bench是一个具有挑战性的真实世界代码编辑基准，上下文信息对LLM代码编辑能力评估至关重要，需要基于真实上下文进行评估。
...
### [127] [Microservices Is Dying, A New Method for Module Division Based on Universal Interfaces](https://arxiv.org/abs/2511.04548)
*Qing Wang,Yong Zhang*

Main category: cs.SE

TL;DR: 提出了一种新的系统设计理念和软件工程方法，通过计算模块独立性和设计通用接口来消除模块间依赖，并实现了名为EIGHT的平台架构，证明在保证模块独立性的前提下，单进程单体应用也能在运行时动态加载、卸载或修改任何部分。
...
Method: 从模块变更影响评估方法出发，提出计算模块独立性的概念方法，推导模块独立的必要条件，采用特定模式设计一组通用接口作为模块间的通用边界。

Result: 实现了EIGHT平台架构，证明只要保证模块独立性，单进程单体应用也能在运行时动态加载、卸载或修改任何部分。
...
### [128] [Multiscale Astrocyte Network Calcium Dynamics for Biologically Plausible Intelligence in Anomaly Detection](https://arxiv.org/abs/2511.03993)
*Berk Iskar,Michael Taynnan Barros*

Main category: cs.LG

TL;DR: 提出基于星形胶质细胞Ca²⁺信号机制的在线学习框架，用于网络异常检测，解决传统离线训练检测器的概念漂移和新型威胁问题。


<details>
  <summary>Details</summary>
Motivation: 传统离线训练的网络异常检测器容易受到概念漂移和新型攻击（如零日攻击、多态攻击）的影响，需要能够快速适应变化环境的在线学习方法。

Method: 结合多细胞星形胶质细胞动力学模拟器和深度神经网络，模拟IP₃介导的Ca²⁺释放、SERCA泵摄取和间隙连接扩散三个关键机制。

Result: 在CTU-13网络流量数据上评估，Ca²⁺门控模型优于基线DNN，准确率高达约98%，假阳性和假阴性率降低，且运行时开销可忽略。

Conclusion: 该Ca²⁺调控学习框架为需要快速适应演化数据模式的流式检测任务提供了通用的生物启发解决方案。
...
### [129] [Applying Time Series Deep Learning Models to Forecast the Growth of Perennial Ryegrass in Ireland](https://arxiv.org/abs/2511.03749)
*Oluwadurotimi Onibonoje,Vuong M. Ngo,Andrew McCarre,Elodie Ruelle,Bernadette O-Briend,Mark Roantree*

Main category: cs.LG

TL;DR: 开发了基于深度学习的模型来预测多年生黑麦草生长，使用时间卷积网络在34年数据上表现出色，为可持续乳业提供支持


<details>
  <summary>Details</summary>
Motivation: 草地作为第二大碳汇对生态和碳循环至关重要，爱尔兰乳业面临盈利和可持续性挑战，现有草生长预测模型不实用

Method: 为单变量数据集设计深度学习模型，特别是时间卷积网络，利用历史草高数据进行多年生黑麦草生长预测

Result: 时间卷积网络在科克地区表现优异，RMSE为2.74，MAE为3.46，在34年1757周数据上验证了最优模型配置

Conclusion: 研究提高了对模型行为的理解，增强了草生长预测的可靠性，推动了可持续乳业实践的发展
...
### [130] [Enhancing Multimodal Protein Function Prediction Through Dual-Branch Dynamic Selection with Reconstructive Pre-Training](https://arxiv.org/abs/2511.04040)
...
<details>
  <summary>Details</summary>
Motivation: 多模态蛋白质特征包含广泛信息，从结构数据到相互作用网络，难以解析其复杂互连关系

Method: 使用重构预训练挖掘细粒度信息，提出双向交互模块促进多模态特征交互学习，设计动态选择模块选择最适合当前预测的特征表示

Result: 在人类数据集上的BPO、MFO和CCO指标显著提升，优于其他基准模型

Conclusion: DSRPGO模型通过动态选择和重构预训练机制有效提升了多模态蛋白质功能预测性能
...
### [131] [Federated Learning with Gramian Angular Fields for Privacy-Preserving ECG Classification on Heterogeneous IoT Devices](https://arxiv.org/abs/2511.03753)
*Youssef Elmir,Yassine Himeur,Abbes Amira*

Main category: cs.LG

TL;DR: 提出了一个基于联邦学习的隐私保护心电图分类框架，将1D ECG信号转换为2D GAF图像，使用CNN进行特征提取，在保护医疗数据隐私的同时实现高效分类。
...
Method: 将1D ECG信号转换为2D GAF图像，使用CNN进行特征提取，采用联邦学习框架让数据保留在本地设备，只共享模型更新。

Result: 在服务器、笔记本电脑和资源受限的Raspberry Pi 4上部署，FL-GAF模型在多客户端设置中达到95.18%的分类准确率，显著优于单客户端基线。
...
### [132] [Laugh, Relate, Engage: Stylized Comment Generation for Short Videos](https://arxiv.org/abs/2511.03757)
*Xuan Ouyang,Senan Wang,Bouzhou Wang,Siyuan Xiahou,Jinrong Zhou,Yuekang Li*

Main category: cs.LG

TL;DR: LOLGORITHM是一个模块化多智能体系统，用于生成符合平台规范且具有风格多样性的短视频评论，支持六种不同评论风格，在抖音和YouTube上获得超过90%和87.55%的用户偏好率。


<details>
  <summary>Details</summary>
Motivation: 短视频平台中，评论在促进社区参与和内容再创作方面起着重要作用，但生成既符合平台规范又具有风格多样性和上下文感知的评论仍是一个挑战。

Method: 采用模块化多智能体系统，整合视频分割、上下文和情感分析、风格感知提示构建，支持六种评论风格，基于多模态大语言模型直接处理视频输入，通过显式提示标记和少样本示例实现细粒度风格控制。

Result: 构建了双语数据集，评估显示LOLGORITHM显著优于基线模型，在抖音和YouTube上分别获得超过90%和87.55%的用户偏好率。

Conclusion: 提出了一个可扩展和文化适应的框架，用于在短视频平台上生成风格化评论，为增强用户参与度和创意互动提供了有前景的路径。
...
### [133] [What's in Common? Multimodal Models Hallucinate When Reasoning Across Scenes](https://arxiv.org/abs/2511.03768)
*Candace Ross,Florian Bordes,Adina Williams,Polina Kirichenko,Mark Ibrahim*

Main category: cs.LG

TL;DR: 提出了Common-O基准测试，评估多模态语言模型在真实场景中的推理能力，发现即使最佳模型在复杂场景推理中表现也很差（仅1%准确率），存在严重幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型在感知基准测试上表现良好，但在真实世界场景推理时仍存在严重幻觉问题，需要构建新基准来评估和改进模型的跨场景推理能力。

Method: 构建包含10,500+个全新图像的Common-O基准测试，基于人类认知测试设计"共同点"推理任务，评估领先多模态模型在单图像感知和跨场景推理中的表现。

Result: 最佳模型在Common-O上仅达35%准确率，在更复杂的Common-O Complex上仅1%；模型在场景中有相似物体时更容易产生幻觉；多图像训练模型表现更好。

Conclusion: 多模态模型在跨场景推理方面存在显著挑战，需要改进训练方法（如多图像训练）来解决幻觉问题，Common-O基准将促进相关研究。
...
### [134] [Contamination Detection for VLMs using Multi-Modal Semantic Perturbation](https://arxiv.org/abs/2511.03774)
*Jaden Park,Mu Cai,Feng Yao,Jingbo Shang,Soochahn Lee,Yong Jae Lee*

Main category: cs.LG

TL;DR: 提出了一种基于多模态语义扰动的新方法来检测视觉语言模型中的测试集污染问题，该方法通过观察模型在受控扰动下的泛化能力来识别污染。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型使用互联网规模预训练数据可能造成测试集泄露，导致性能虚高，但现有的污染检测方法效果不佳或行为不一致，需要开发有效的检测方法。

Method: 故意污染开源视觉语言模型，然后提出基于多模态语义扰动的检测方法，通过观察污染模型在受控扰动下泛化能力的失败来识别污染。

Result: 该方法在多种现实污染策略下都表现出鲁棒性和有效性，能够可靠检测出被污染的视觉语言模型。

Conclusion: 提出的多模态语义扰动方法为检测视觉语言模型中的测试集污染提供了一种简单而有效的解决方案，有助于评估模型的真实性能。
...
### [135] [FusionDP: Foundation Model-Assisted Differentially Private Learning for Partially Sensitive Features](https://arxiv.org/abs/2511.03806)
*Linghui Zeng,Ruixuan Liu,Atiquer Rahman Sarkar,Xiaoqian Jiang,Joyce C. Ho,Li Xiong*

Main category: cs.LG

TL;DR: FusionDP是一个特征级差分隐私框架，通过利用基础模型对敏感特征进行估算，在保持敏感特征隐私的同时提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 在隐私保护机器学习中，传统DP-SGD对所有特征统一添加噪声，导致模型性能显著下降。现实中只有部分特征（如人口统计信息）需要隐私保护，而其他特征（如实验室结果）敏感性较低。

Method: 两步框架：1）使用大型基础模型基于非敏感特征估算敏感特征，作为外部先验；2）改进的DP-SGD算法，在原始特征和估算特征上训练模型，同时保持原始敏感特征的正式隐私保护。

Result: 在PhysioNet的败血症预测任务和MIMIC-III的临床笔记分类任务上，FusionDP相比隐私保护基线显著提高了模型性能，同时保持了严格的特征级隐私。

Conclusion: FusionDP通过基础模型驱动的估算方法，有效改善了各种模态数据的隐私-效用权衡，为特征级差分隐私提供了实用解决方案。
...
### [136] [Fair and Explainable Credit-Scoring under Concept Drift: Adaptive Explanation Frameworks for Evolving Populations](https://arxiv.org/abs/2511.03807)
*Shivogo John*

Main category: cs.LG

TL;DR: 本文开发了自适应解释框架来解决信用评分系统中概念漂移导致的解释不稳定和不公平问题，通过三种自适应SHAP变体显著提高了时间稳定性和公平性。


<details>
  <summary>Details</summary>
Motivation: 传统可解释性技术（如SHAP）假设静态数据和固定背景分布，当概念漂移发生时，其解释变得不稳定且可能不公平，这在动态演变的信用模型中构成挑战。

Method: 整合XGBoost预测建模与三种自适应SHAP变体：(A)按切片解释重加权，(B)漂移感知SHAP重新基线化，(C)在线代理校准。使用多年度信用数据集进行基准测试。

Result: 自适应方法（特别是重新基线化和基于代理的解释）显著提高了时间稳定性，减少了跨人口群体的差异影响，且不降低预测准确性。鲁棒性测试确认了在真实世界漂移条件下的弹性。

Conclusion: 自适应可解释性是在数据驱动的信用系统中维持透明度、责任性和伦理可靠性的实用机制，适用于任何决策模型随人口变化而演变的领域。
...
### [137] [Learning Filter-Aware Distance Metrics for Nearest Neighbor Search with Multiple Filters](https://arxiv.org/abs/2511.04073)
*Ananya Sutradhar,Suryansh Gupta,Ravishankar Krishnaswamy,Haiyang Xu,Aseem Rastogi,Gopal Srinivasa*

Main category: cs.LG

TL;DR: 本文提出了一种学习最优权衡向量距离和过滤器匹配的方法，通过约束线性优化问题学习权重，相比固定惩罚方法在过滤近似最近邻搜索中提高了5-10%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于图的方法使用固定、数据无关的惩罚来整合过滤器意识，但由于标签和向量分布的多样性，这些方法往往无法在不同数据集上泛化。
...
Conclusion: 通过从数据中学习最优权衡权重，而不是依赖固定惩罚，可以更好地反映底层过滤器分布，更有效地解决过滤ANN搜索问题。
...
TL;DR: 提出了一种路由方法，将每个问题分配给最可能解决它的最小模型，在保持准确性的同时减少计算开销。
...
Result: 在多样化数学基准测试中，路由方法比随机分配提高了效率，匹配了s1.1-32B的性能，同时显著减少了计算量。
...
### [139] [One Size Does Not Fit All: Architecture-Aware Adaptive Batch Scheduling with DEBA](https://arxiv.org/abs/2511.03809)
*François Belias,Naser Ezzati-Jivan,Foutse Khomh*

Main category: cs.LG

TL;DR: DEBA是一种自适应批量大小调度器，通过监控梯度方差、梯度范数变化和损失变化来调整批量大小。研究发现架构决定适应效果：轻量级和中深架构获得45-62%训练加速和1-7%精度提升，而稳定架构如ViT-B16仅获得6%加速。


<details>
  <summary>Details</summary>
Motivation: 现有自适应批量大小方法对所有架构采用相同策略，假设通用解决方案，但实际效果因架构而异。

Method: 引入DEBA自适应批量调度器，监控梯度方差、梯度范数变化和损失变化来指导批量大小调整。使用滑动窗口统计和足够冷却期（5+周期）。

Result: 轻量级和中深架构获得45-62%训练加速和1-7%精度提升；浅残差网络精度提升2.4-4.0%，加速36-43%；深度残差网络表现高方差；ViT-B16仅6%加速。

Conclusion: 自适应批量大小方法不能跨架构通用，需要架构感知设计。梯度稳定性指标可预测哪些架构能从自适应调度中受益。
...
### [140] [Ground-Truth Subgraphs for Better Training and Evaluation of Knowledge Graph Augmented LLMs](https://arxiv.org/abs/2511.04473)
...
Method: 开发SynthKGQA框架，从知识图谱自动生成问答数据集，提供完整的真实事实集合用于问题推理。

Result: 应用SynthKGQA到Wikidata生成GTSQA数据集，用于测试知识图谱检索器在未见图谱结构和关系类型上的零样本泛化能力。

Conclusion: SynthKGQA框架能够生成高质量数据集，不仅支持更有效的信息检索基准测试，还能训练更好的模型。
...
### [141] [Sketch-Augmented Features Improve Learning Long-Range Dependencies in Graph Neural Networks](https://arxiv.org/abs/2511.03824)
...
<details>
  <summary>Details</summary>
Motivation: 图神经网络在聚合局部邻域信息时面临三个主要挑战：(i) 长距离信息压缩，(ii) 节点表示过度平滑，(iii) 表达能力有限。这些限制影响了GNN在捕获长距离依赖关系方面的性能。

Method: 向标准GNN注入随机全局节点特征嵌入（称为Sketched Random Features）。这些嵌入具有唯一性、距离敏感性和拓扑无关性，能够高效捕获长距离依赖关系。

Result: 在真实世界图学习任务上的实验结果表明，该方法相比基线GNN能持续提升性能，既可以作为独立解决方案，也可以作为现有技术（如图位置编码）的补充增强。

Conclusion: Sketched Random Features方法有效缓解了GNN的三个关键限制，为图神经网络提供了捕获长距离依赖关系的有效途径，且与现有技术具有良好的互补性。
...
### [142] [From Static to Dynamic: Enhancing Offline-to-Online Reinforcement Learning via Energy-Guided Diffusion Stratification](https://arxiv.org/abs/2511.03828)
*Lipeng Zu,Hansong Zhou,Xiaonan Zhang*

Main category: cs.LG

TL;DR: 提出了一种名为StratDiff的新方法，通过能量引导扩散分层技术解决离线到在线强化学习中的分布偏移问题，显著提升了现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习面临行为策略与在线学习策略之间的分布偏移挑战，现有方法很少明确利用离线数据的分布结构来适应不同类型的样本。

Method: 使用扩散模型从离线数据中学习先验知识，通过基于能量的函数改进策略模仿，生成离线类动作，并根据KL散度将训练批次分层为离线类和在线类样本，分别采用不同的学习策略。

Result: 在D4RL基准测试上的广泛实验表明，StratDiff与Cal-QL和IQL结合后显著优于现有方法，实现了更好的适应性和更稳定的性能。

Conclusion: StratDiff通过有效利用离线数据的分布结构，成功解决了离线到在线强化学习中的分布偏移问题，为这一领域提供了创新的解决方案。
...
### [143] [Higher-Order Causal Structure Learning with Additive Models](https://arxiv.org/abs/2511.03831)
*James Enouen,Yujia Zheng,Ignavier Ng,Yan Liu,Kun Zhang*

Main category: cs.LG

TL;DR: 本文扩展了因果加性模型(CAM)以处理高阶交互作用，引入了有向无环超图(hyper DAG)来表示这种结构，提供了可识别性结果，并开发了相应的学习算法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的许多过程都表现出高阶机制，但在因果发现中明确处理交互作用的研究很少。本文旨在扩展因果加性模型以包含高阶交互作用。

Method: 扩展因果加性模型(CAM)到包含高阶交互作用的加性模型，使用有向无环超图表示结构，开发了贪婪CAM算法的扩展版本来处理超DAG搜索空间。

Result: 提供了超DAG的可识别性结果，扩展了典型的马尔可夫等价类，并证明了学习更复杂的超图结构可能带来更好的经验结果，特别是在有限样本复杂度方面。

Conclusion: 通过引入有向无环超图来建模高阶交互作用，扩展了因果结构学习的能力，并开发了有效的学习算法，在合成实验中展示了其经验价值。
...
### [144] [Enhancing Q-Value Updates in Deep Q-Learning via Successor-State Prediction](https://arxiv.org/abs/2511.03836)
*Lipeng Zu,Hansong Zhou,Xiaonan Zhang*

Main category: cs.LG

TL;DR: 提出SADQ方法，通过建模环境动态和整合后继状态分布来改进DQN，减少训练方差并提高学习效率
...
Method: 使用随机转移模型显式建模环境动态，将后继状态分布整合到Q值估计中，并探索基于转移结构的更高效动作选择策略

Result: 在标准RL基准和实际向量控制任务中，SADQ在稳定性和学习效率上持续优于DQN变体

Conclusion: SADQ通过建模环境动态和整合后继状态分布，在保持无偏值估计的同时显著降低了训练方差
...
### [145] [Benchmark Datasets for Lead-Lag Forecasting on Social Platforms](https://arxiv.org/abs/2511.03877)
...
TL;DR: 本文提出了Lead-Lag Forecasting（LLF）这一新的预测范式，用于预测早期使用渠道（如浏览量）与时间上滞后的结果渠道（如引用量）之间的关系，并提供了arXiv和GitHub两个大规模基准数据集。


<details>
  <summary>Details</summary>
Motivation: 社交和协作平台中存在早期交互（如浏览、点赞）与滞后结果（如引用、销售）之间的时间序列关联模式，但缺乏标准化的数据集和统一的预测框架。

Method: 通过构建arXiv（230万篇论文的访问量->引用量）和GitHub（300万个仓库的推送/星标->分支）两个大规模基准数据集，验证了领先-滞后动态的存在，并建立了参数和非参数回归基线模型。

Result: 研究证实了领先-滞后动态在多个领域普遍存在，包括Wikipedia、Spotify、电商和LinkedIn等，数据集为长期动态预测提供了理想的测试平台。

Conclusion: 本研究确立了LLF作为一个新的预测范式，为在社交和使用数据中系统探索领先-滞后关系奠定了实证基础，并提供了公开的数据门户。
...
### [146] [DecoHD: Decomposed Hyperdimensional Classification under Extreme Memory Budgets](https://arxiv.org/abs/2511.03911)
...
<details>
  <summary>Details</summary>
Motivation: 传统的HDC压缩方法通常缩小特征轴，这会损害集中性和鲁棒性。现有的HDC分解方法使用固定的原子超向量进行解码，不适合压缩学习到的类别原型。

Method: 提出DecoHD方法，直接在分解的HDC参数化中学习：使用每层共享的小型通道集合，通过跨层乘法绑定和最终捆绑，从紧凑因子产生大的表示空间。通过轻量级捆绑头沿类别轴进行压缩，同时保持原生绑定-捆绑-评分机制。

Result: DecoHD实现了极度的内存节省，在严格的部署预算下仅有轻微精度下降。平均精度保持在强非缩减HDC基线的0.1-0.15%范围内（最差情况5.7%），对随机位翻转噪声更鲁棒，使用最多约97%更少的可训练参数达到精度平台，在硬件上相比CPU（AMD Ryzen 9 9950X）实现约277倍/35倍的能量/速度增益，相比GPU（NVIDIA RTX 4090）实现13.5倍/3.7倍增益，相比基线HDC ASIC实现2.0倍/2.4倍增益。

Conclusion: DecoHD通过分解HDC参数化实现了高效的模型压缩，在保持高精度的同时显著减少了内存占用和计算资源需求，特别适合内存/近内存加速器部署。
...
### [147] [On Predicting Sociodemographics from Mobility Signals](https://arxiv.org/abs/2511.03924)
*Ekin Uğurel,Cynthia Chen,Brian H. Y. Lee,Filipe Rodrigues*

Main category: cs.LG

TL;DR: 该论文提出了一种从移动数据推断社会人口属性的方法，通过引入基于有向移动图的高阶移动描述符、开发模型置信度与准确性的均衡度量工具，以及采用多任务学习框架来提升预测准确性、可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 从移动数据推断社会人口属性有助于交通规划者更好地利用被动收集的数据集，但由于移动模式与社会人口特征之间的弱相关性和不一致性，以及跨情境泛化能力有限，这一任务仍然具有挑战性。

Method: 1) 引入基于有向移动图的行为基础高阶移动描述符，捕捉行程序列、出行方式和社会共同出行的结构化模式；2) 开发度量指标和可视化诊断工具，确保模型置信度与准确性之间的均衡；3) 采用多任务学习框架，从共享表示中联合预测多个社会人口属性。

Result: 该方法在预测年龄、性别、收入和家庭结构方面显著优于基线特征。多任务学习方法在训练数据有限或测试集分布与训练集不同时表现尤其出色，优于单任务模型。

Conclusion: 提出的方法通过改进特征表示、不确定性量化和多任务学习，有效解决了从移动数据推断社会人口属性时的准确性、可解释性和泛化性挑战，为交通规划提供了更可靠的工具。
...
### [148] [SynQuE: Estimating Synthetic Dataset Quality Without Annotations](https://arxiv.org/abs/2511.03928)
*Arthur Chen,Victor Zhong*

Main category: cs.LG

TL;DR: 提出了SynQuE问题：使用有限的未标注真实数据来评估合成数据集的质量并排序，以选择最佳训练数据。提出了LENS代理指标，利用大语言模型推理来改进复杂任务上的评估效果。


<details>
  <summary>Details</summary>
Motivation: 解决在数据稀缺（收集成本高或隐私限制）情况下，如何有效选择高质量合成数据集进行训练的现实挑战。

Method: 1. 建立首个SynQuE基准测试；2. 引入基于分布和多样性的距离度量代理指标；3. 提出LENS代理指标，利用大语言模型推理来评估复杂任务。

Result: SynQuE代理指标与真实任务性能相关，在情感分析、Text2SQL、网页导航和图像分类等任务上表现良好。LENS在复杂任务上表现最佳，如在Text2SQL解析中，使用SynQuE选择的前3个合成数据集可将准确率从30.4%提升到38.4%（+8.1%）。

Conclusion: SynQuE为真实数据稀缺情况下的合成数据选择提供了实用框架，并推动了基于基础模型的数据表征和细粒度数据选择的未来研究。
...
### [149] [NVIDIA Nemotron Nano V2 VL](https://arxiv.org/abs/2511.03929)
...
TL;DR: Nemotron Nano V2 VL是最新的视觉语言模型，专为文档理解、长视频理解和推理任务设计，相比前代模型在所有视觉和文本领域都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 开发一个在真实世界文档理解、长视频理解和推理任务中表现强大的视觉语言模型，通过改进模型架构、数据集和训练方法来实现性能提升。

Method: 基于混合Mamba-Transformer LLM架构构建，采用创新的token减少技术来提高长文档和视频场景中的推理吞吐量。
...
### [150] [LogHD: Robust Compression of Hyperdimensional Classifiers via Logarithmic Class-Axis Reduction](https://arxiv.org/abs/2511.03938)
*Sanggeon Yun,Hyunwoo Oh,Ryozo Masukawa,Pietro Mercati,Nathaniel D. Bastian,Mohsen Imani*

Main category: cs.LG

TL;DR: LogHD是一种超维计算压缩方法，通过对类别轴进行对数压缩，将内存需求从O(CD)减少到O(D log_k C)，同时保持维度D不变，在保持准确性的同时显著提高能效和抗干扰能力。


<details>
  <summary>Details</summary>
Motivation: 标准超维计算需要为每个类别存储一个原型向量，导致O(CD)的内存需求，这在内存、能耗和可靠性受限的系统中成为瓶颈。现有方法主要压缩特征轴，虽然减少存储和计算量，但会削弱鲁棒性。

Method: 使用对数类别轴压缩，用约log_k C个捆绑超向量替代C个类别原型，在n维激活空间中进行解码；采用容量感知码本和基于配置文件的解码方法，可与特征轴稀疏化组合使用。

Result: 在各种数据集和注入比特翻转的情况下，LogHD在较小模型下达到竞争性准确度，在相同内存下具有更高弹性；在同等内存条件下，能承受比特征轴压缩高2.5-3.0倍的比特翻转率；ASIC实现相比CPU和GPU分别获得498倍和24.3倍的能效提升。

Conclusion: LogHD通过类别轴压缩有效解决了超维计算的内存瓶颈问题，在保持性能的同时显著提升了能效和抗干扰能力，为资源受限系统提供了可行的解决方案。
...
### [151] [RLHF: A comprehensive Survey for Cultural, Multimodal and Low Latency Alignment Methods](https://arxiv.org/abs/2511.03939)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型对齐研究从经典文本方法扩展到更广泛领域，需要系统性地总结多模态对齐、文化公平性和低延迟优化等关键进展，填补现有研究空白。

Method: 首先回顾PPO、DPO、GRPO等基础算法，然后详细分析最新创新技术，通过比较性综合梳理这些方法。

Result: 提供了对齐技术的系统性比较分析，识别了多模态对齐、文化公平性和低延迟优化等关键研究领域的进展和挑战。

Conclusion: 本工作为研究人员构建更鲁棒、高效和公平的AI系统提供了重要路线图，指明了未来对齐研究的发展方向。
...
### [152] [Conditional Score Learning for Quickest Change Detection in Markov Transition Kernels](https://arxiv.org/abs/2511.03953)
...
Method: 直接从样本对学习条件分数∇_y log p(y|x)，基于此开发了基于分数的CUSUM程序，使用条件Hyvarinen分数差异检测核变化，并提出截断版本确保有界增量。

Result: 证明了均匀遍历马尔可夫过程的指数级误报时间下界和检测延迟的渐近上界。

Conclusion: 该方法为高维马尔可夫模型中的基于分数的变化检测提供了理论保证和实际可行性。
...
### [153] [PrivacyCD: Hierarchical Unlearning for Protecting Student Privacy in Cognitive Diagnosis](https://arxiv.org/abs/2511.03966)
...
Method: 提出分层重要性引导遗忘算法，利用认知诊断模型参数重要性的分层特性，通过创新的平滑机制结合个体和层级重要性，精确识别与待遗忘数据相关的参数。

Result: 在三个真实世界数据集上的实验表明，HIF在关键指标上显著优于基线方法。

Conclusion: HIF为认知诊断模型提供了首个有效的数据删除解决方案，支持部署高性能、保护隐私的AI系统。
...
### [154] [Non-Asymptotic Optimization and Generalization Bounds for Stochastic Gauss-Newton in Overparameterized Models](https://arxiv.org/abs/2511.03972)
*Semih Cayci*

Main category: cs.LG

TL;DR: 本文分析了随机高斯-牛顿方法在过参数化深度神经网络训练中的收敛性和泛化性能，建立了有限时间收敛界限和泛化界限，发现高斯-牛顿矩阵的最小特征值越大，泛化性能越好。


<details>
  <summary>Details</summary>
Motivation: 研究高阶优化方法如何影响深度学习中的泛化性能，特别是在过参数化深度神经网络训练中。

Method: 使用带有Levenberg-Marquardt阻尼和mini-batch采样的随机高斯-牛顿方法，在回归设置中训练具有平滑激活函数的过参数化深度神经网络。

Result: 建立了参数空间中的有限时间收敛界限，并推导了非渐近泛化界限，表明高斯-牛顿矩阵的最小特征值越大，稳定性界限越紧。
...
### [155] [PETRA: Pretrained Evolutionary Transformer for SARS-CoV-2 Mutation Prediction](https://arxiv.org/abs/2511.03976)
*Xu Zou*

Main category: cs.LG

TL;DR: PETRA是一种基于进化轨迹而非原始RNA序列的Transformer方法，通过系统发育树构建进化轨迹，有效减轻测序噪声并捕捉病毒进化的层次结构，在SARS-CoV-2突变预测方面表现优异。


<details>
  <summary>Details</summary>
Motivation: SARS-CoV-2具有快速且不可预测的进化轨迹，持续出现免疫逃逸变种，对公共卫生和疫苗开发构成挑战。现有大规模生成预训练Transformer直接应用于嘈杂的病毒基因组序列存在局限性。

Method: 提出PETRA方法，基于系统发育树构建进化轨迹而非原始RNA序列，采用加权训练框架解决全球序列数据的地理和时间不平衡问题。

Result: 在SARS-CoV-2突变预测方面表现卓越，核苷酸突变的加权召回率@1达到9.45%，刺突蛋白氨基酸突变为17.10%，显著优于基线方法。还能实时预测主要进化支的突变。

Conclusion: PETRA通过进化轨迹建模有效解决了病毒基因组序列的噪声问题，在突变预测方面表现出色，为实时监测病毒进化提供了有力工具。
...
### [156] [Structural Priors and Modular Adapters in the Composable Fine-Tuning Algorithm of Large-Scale Models](https://arxiv.org/abs/2511.03981)
*Yuxiao Wang,Di Wu,Feng Liu,Zhimin Qiu,Chenrui Hu*

Main category: cs.LG

TL;DR: 提出一种可组合微调方法，通过图结构先验和模块化适配器解决大模型在多任务适应中的计算成本和结构不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 解决大规模预训练模型在多任务适应时面临的高计算成本和结构不稳定性问题。

Method: 引入关系矩阵建模任务间依赖关系，将节点和路径相关性编码为图结构先验；通过低秩映射和可插拔机制在不同层嵌入模块化适配器，实现高效跨任务组合和重用。

Result: 显著提升任务预测精度、适配器权重分配精度和整体计算效率，同时保持模型轻量化设计。

Conclusion: 图先验和模块化机制在可组合微调中具有协同优势，能够有效提高参数效率、训练稳定性并缓解路径冲突和冗余计算。
...
### [157] [TwIST: Rigging the Lottery in Transformers with Independent Subnetwork Training](https://arxiv.org/abs/2511.03983)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统稀疏化方法需要后训练校准或Hessian恢复的问题，提供训练时即可获得可部署稀疏模型的方法，避免额外微调开销。

Method: 并行训练多个子网络，定期聚合参数并重采样新子网络，识别高质量子网络（"黄金票券"），产生结构化密集矩阵。

Result: 在高稀疏度（50%+）下显著优于基线方法，达到23.14 PPL（对比31.64 PPL），在商用硬件上实现实际推理加速和内存减少。

Conclusion: TwIST提供了一种高效训练时路径，无需额外微调或恢复开销即可获得可部署的稀疏大语言模型。
...
### [158] [Use of Continuous Glucose Monitoring with Machine Learning to Identify Metabolic Subphenotypes and Inform Precision Lifestyle Changes](https://arxiv.org/abs/2511.03986)
*Ahmed A. Metwally,Heyjun Park,Yue Wu,Tracey McLaughlin,Michael P. Snyder*

Main category: cs.LG

TL;DR: 本文综述了利用连续血糖监测和可穿戴技术进行动态代谢表型分析的方法，通过机器学习模型从高分辨率血糖数据中预测胰岛素抵抗和β细胞功能，实现个性化代谢亚型识别和精准干预。


<details>
  <summary>Details</summary>
Motivation: 传统糖尿病和前驱糖尿病分类使用的静态血糖阈值掩盖了病理生理性血糖异常的异质性，这些异质性主要由胰岛素抵抗、β细胞功能障碍和肠促胰岛素缺乏驱动。

Method: 利用连续血糖监测和可穿戴技术收集高分辨率血糖数据，结合机器学习模型分析家庭口服葡萄糖耐量测试数据，预测肌肉胰岛素抵抗和β细胞功能，并整合饮食、睡眠和体力活动模式数据。

Result: 研究表明机器学习模型能够准确预测金标准测量的肌肉胰岛素抵抗和β细胞功能，个体对标准化餐食的餐后血糖反应可作为代谢亚型的生物标志物，饮食缓解剂对餐后血糖反应的减弱效果具有表型依赖性。
...
### [159] [Towards Scalable Meta-Learning of near-optimal Interpretable Models via Synthetic Model Generations](https://arxiv.org/abs/2511.04000)
*Kyaw Hpone Myint,Zhe Wu,Alexandre G. R. Day,Giri Iyengar*

Main category: cs.LG

TL;DR: 提出一种高效的合成预训练数据生成方法，用于决策树的元学习，通过采样近似最优决策树来创建大规模合成数据集，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 决策树在金融和医疗等高风险领域广泛应用，但传统方法需要大量真实数据或计算昂贵的优化决策树进行元学习，限制了其可扩展性和效率。

Method: 使用合成方法采样近似最优决策树，生成大规模、逼真的预训练数据集，并采用MetaTree transformer架构进行元学习。

Result: 该方法在性能上可与使用真实世界数据或计算昂贵的优化决策树相媲美，同时显著降低了计算成本。

Conclusion: 该策略为可扩展和高效的决策树元学习提供了新途径，提高了数据生成的灵活性，推动了可解释决策树模型的发展。
...
### [160] [Accelerating scientific discovery with the common task framework](https://arxiv.org/abs/2511.04001)
...
TL;DR: 提出科学和工程的通用任务框架(CTF)，为评估机器学习算法在科学工程领域的表现提供标准化基准和比较指标。


<details>
  <summary>Details</summary>
Motivation: 机器学习算法在科学工程领域快速发展，需要客观的评估指标来比较不同算法在预测、状态重构、泛化和控制等任务上的表现，特别是在有限数据和噪声测量的情况下。

Method: 引入通用任务框架，包含不断增长的数据集集合，涵盖科学工程中常见的实际任务目标，借鉴了语音识别、语言处理和计算机视觉领域的成功经验。

Result: CTF为科学工程领域提供了关键的使能技术，能够客观比较当前快速发展和部署的各种算法。

Conclusion: 科学工程领域迫切需要类似传统ML领域的标准化评估框架，CTF填补了这一空白，将促进该领域算法的快速发展。
...
### [161] [Memory- and Latency-Constrained Inference of Large Language Models via Adaptive Split Computing](https://arxiv.org/abs/2511.04002)
*Mingyu Sung,Vikas Palakonda,Suhwan Im,Sunghwan Moon,Il-Min Kim,Sangseok Yun,Jae-Mo Kang*

Main category: cs.LG

TL;DR: 提出了首个面向边缘设备LLM部署的自回归感知分割计算框架，通过混合精度量化、中间压缩和统一优化，在保持精度的同时显著降低通信开销和内存需求。


<details>
  <summary>Details</summary>
Motivation: LLMs在资源受限的IoT设备上部署不实用，现有分割计算方法无法解决自回归推理的独特挑战，特别是迭代token生成和扩展的KV缓存需求。

Method: 开发了三点关键贡献：1) 单点分割压缩(OPSC)混合精度量化方案；2) 两阶段中间压缩管道；3) 统一优化框架联合选择最优分割点、量化设置和序列长度。

Result: 在多种LLM和硬件平台上评估，相比最先进量化方法实现了1.49倍推理加速和显著通信开销减少，同时保持或提高了模型精度。

Conclusion: 该框架为边缘设备上的LLM部署提供了实用解决方案，有效解决了自回归推理的内存和通信瓶颈问题。
...
### [162] [DartQuant: Efficient Rotational Distribution Calibration for LLM Quantization](https://arxiv.org/abs/2511.04063)
*Yuantian Shao,Yuanteng Chen,Peisong Wang,Jianlin Yu,Jing Lin,Yiwu Yao,Zhihui Wei,Jian Cheng*

Main category: cs.LG

TL;DR: 提出了DartQuant方法，通过分布感知的旋转校准来高效优化量化性能，在70B模型上实现47倍加速和10倍内存节省，首次在单张3090 GPU上完成70B模型的旋转校准。


<details>
  <summary>Details</summary>
Motivation: 解决旋转矩阵量化中端到端微调计算成本高且容易过拟合的问题，使大语言模型量化在资源受限环境中可行。
...
Conclusion: DartQuant方法有效解决了大模型量化中的计算成本和过拟合问题，为资源受限环境下的模型量化提供了可行方案。
...
### [163] [Pediatric Appendicitis Detection from Ultrasound Images](https://arxiv.org/abs/2511.04069)
*Fatemeh Hosseinabadi,Seyedhassan Sharifi*

Main category: cs.LG

TL;DR: 开发基于预训练ResNet架构的深度学习模型，用于从超声图像自动检测小儿阑尾炎，在Regensburg小儿阑尾炎数据集上达到93.44%的准确率。
...
Method: 使用预训练ResNet架构进行微调，通过图像归一化、调整大小和数据增强等预处理，区分阑尾炎与非阑尾炎病例。

Result: 模型整体准确率为93.44%，精确率91.53%，召回率89.8%，在异质性超声图像中表现出色。

Conclusion: 该模型能有效学习判别性空间特征，克服儿科影像中低对比度、斑点噪声和解剖变异等挑战。
...
### [164] [Left Atrial Segmentation with nnU-Net Using MRI](https://arxiv.org/abs/2511.04071)
*Fatemeh Hosseinabadi,Seyedhassan Sharifi*

Main category: cs.LG

TL;DR: 使用nnU-Net框架自动分割心脏MRI中的左心房，在LA Segmentation Challenge 2013数据集上达到93.5的Dice分数，优于传统分割方法。


<details>
  <summary>Details</summary>
Motivation: 手动分割左心房耗时、主观性强，不适合大规模或时间敏感的临床应用，需要自动化解决方案。

Method: 应用nnU-Net自配置深度学习架构，自动适应MRI数据特性，包括预处理、网络配置和训练流程。

Result: 在30个MRI扫描数据集上达到平均Dice分数93.5，与专家标注高度重叠，在不同左心房形状、对比度和图像质量下表现稳健。

Conclusion: nnU-Net能准确分割左心房和近端肺静脉，为房颤消融和心脏生物物理模型构建提供可靠的自动化工具。
...
### [165] [DeNoise: Learning Robust Graph Representations for Unsupervised Graph-Level Anomaly Detection](https://arxiv.org/abs/2511.04086)
*Qingfeng Chen,Haojin Zeng,Jingyi Jie,Shichao Zhang,Debo Cheng*

Main category: cs.LG

TL;DR: DeNoise是一个针对训练数据被异常图污染的图级异常检测框架，通过联合优化图级编码器、属性解码器和结构解码器，结合对抗目标和对比学习，在噪声环境下学习鲁棒的图表示。


<details>
  <summary>Details</summary>
Motivation: 现有的图神经网络方法通常假设训练集是干净的，只包含正常图，但实际应用中训练数据往往被异常图污染，这会扭曲学习到的表示并严重降低性能。
...
### [166] [KoTaP: A Panel Dataset for Corporate Tax Avoidance, Performance, and Governance in Korea](https://arxiv.org/abs/2511.04094)
*Hyungjong Na,Wonho Song,Seungyong Han,Donghyeon Jo,Sejin Myung,Hyungjoon Kim*

Main category: cs.LG

TL;DR: 该研究构建了韩国避税面板(KoTaP)，这是一个包含2011-2024年KOSPI和KOSDAQ上市非金融企业的长期面板数据集，包含12,653个公司年度观测值，用于研究企业避税与盈余管理、盈利能力、稳定性、增长性和治理等多个领域的关系。


<details>
  <summary>Details</summary>
Motivation: 构建一个标准化的韩国企业避税面板数据集，既能与国际文献保持一致，又能反映韩国企业的独特制度特征，为会计、金融和跨学科研究提供关键开放资源。

Method: 从KOSPI和KOSDAQ上市企业中筛选非金融企业、12月财年结束、无资本减损、税前利润为正的企业，构建包含12,653个观测值的平衡面板数据集，使用现金有效税率(CETR)、GAAP有效税率(GETR)和账面税收差异指标(TSTA, TSDA)等互补指标衡量避税行为。

Result: 成功构建了KoTaP数据集，具有平衡面板结构、标准化变量，与国际文献在核心指标分布和相关性上保持一致，同时反映了韩国企业所有权集中、外资持股比例高、流动性比率高等独特特征。

Conclusion: KoTaP是一个具有国际可比性和情境独特性的关键数据集，支持计量经济学和深度学习模型基准测试、外部有效性检验、可解释AI分析，以及政策评估、审计规划和投资分析等应用。
...
### [167] [Decomposable Neuro Symbolic Regression](https://arxiv.org/abs/2511.04124)
*Giorgio Morales,John W. Sheppard*

Main category: cs.LG

TL;DR: 提出了一种可分解的符号回归方法，结合Transformer、遗传算法和遗传编程，从训练好的黑盒回归模型中提取可解释的数学表达式。
...
Method: 使用多集Transformer生成单变量符号骨架，通过遗传算法评估和选择高质量候选，再用遗传编程级联合并骨架，最后通过遗传算法优化系数。

Result: 在受控和噪声变化的问题上，相比其他方法具有更低或相当的插值和外推误差，并能一致学习匹配原始数学结构的表达式。

Conclusion: 该方法能够生成可解释的多元表达式，在保持准确性的同时更好地识别数据背后的控制方程。
...
### [168] [Exploring the Feasibility of End-to-End Large Language Model as a Compiler](https://arxiv.org/abs/2511.04132)
*Hongbin Zhang,Shihao Gao,Yang Liu,Mingjie Xing,Yanjun Wu,Chen Zhao*

Main category: cs.LG

TL;DR: 本文探索了将大型语言模型作为端到端编译器的可行性，设计了CompilerEval评估框架，发现LLMs具备基本编译能力但成功率较低，通过优化提示、模型缩放和推理方法可显著提升汇编代码质量。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs已被用于辅助编译器开发和维护，但其作为端到端编译器的潜力尚未充分探索。本文旨在研究LLM作为编译器（LaaC）的可行性及未来发展方向。

Method: 设计了专门的CompilerEval数据集和评估框架，评估主流LLMs在源代码理解和汇编代码生成方面的能力，分析各种错误类型，探索多种改进LLM生成代码的方法，并评估跨平台编译能力。

Result: 实验结果表明，LLMs展现出作为编译器的基本能力，但目前编译成功率较低。通过优化提示、扩大模型规模和引入推理方法，可以显著提升LLM生成的汇编代码质量。
...
### [169] [Exchange Policy Optimization Algorithm for Semi-Infinite Safe Reinforcement Learning](https://arxiv.org/abs/2511.04147)
*Jiaming Zhang,Yujie Yang,Haoning Wang,Liping Zhang,Shengbo Eben Li*

Main category: cs.LG

TL;DR: 提出交换策略优化(EPO)框架解决半无限安全强化学习问题，通过迭代求解有限约束子问题并自适应调整活动约束集，在保证确定有界安全性的同时实现最优策略性能。


<details>
  <summary>Details</summary>
Motivation: 实际应用中安全强化学习常涉及无限约束条件（半无限安全RL），如需要在连续参数空间上强制执行安全条件，确保每个空间位置的资源分配充足。现有方法难以处理此类问题。

Method: EPO通过迭代求解有限约束安全RL子问题，自适应调整活动约束集：超过预设容忍度的违规约束被添加以细化策略，零拉格朗日乘子的约束在策略更新后被移除。这种交换规则防止工作集无限增长并支持有效策略训练。

Result: 理论分析表明，在温和假设下，通过EPO训练的策略实现与最优解相当的性能，且全局约束违反严格保持在预定界限内。

Conclusion: EPO为半无限安全RL问题提供了有效的算法框架，能够在保证确定有界安全性的同时优化长期性能，解决了连续参数空间上的安全约束问题。
...
### [170] [Learning to Land Anywhere: Transferable Generative Models for Aircraft Trajectories](https://arxiv.org/abs/2511.04155)
*Olav Finne Praesteng Larsen,Massimiliano Ruocco,Michail Spitieris,Abdulmajid Murad,Martina Ragosta*

Main category: cs.LG

TL;DR: 该论文研究如何通过迁移学习将数据丰富机场训练的生成模型有效适配到数据稀缺机场，使用扩散模型和流匹配方法，在仅需目标机场5%数据的情况下就能达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 许多次级和区域机场面临严重的轨迹数据稀缺问题，这限制了机器学习方法的应用和大规模模拟分析能力。

Method: 采用最先进的扩散模型和流匹配架构，先在苏黎世机场预训练，然后在都柏林机场使用不同比例数据（0%-100%）进行微调。

Result: 扩散模型仅需都柏林机场5%数据就能达到竞争性性能，20%数据即可达到基线水平性能，始终优于从头训练的模型。
...
### [171] [Deep Learning Approach for Clinical Risk Identification Using Transformer Modeling of Heterogeneous EHR Data](https://arxiv.org/abs/2511.04158)
...
<details>
  <summary>Details</summary>
Motivation: 解决临床风险分类中异构EHR数据的挑战，包括不规则时间模式、大模态差异和复杂语义结构。

Method: 采用特征嵌入层统一表示结构化和非结构化数据，引入可学习时间编码机制捕获不均匀采样间隔下的动态演化，使用多头自注意力结构进行全局依赖建模，设计语义加权池化模块自适应分配医疗事件重要性。
...
TL;DR: 该论文研究了深度集成模型中联合调优权重衰减、温度缩放和早停策略对预测性能和不确定性量化的影响，并提出了一种部分重叠的保留策略作为实用解决方案。


<details>
  <summary>Details</summary>
Motivation: 深度集成是机器学习中的强大工具，虽然通常单独训练和调优模型，但证据表明联合调优集成可以带来更好的性能。本文旨在研究联合调优对预测性能和不确定性量化的具体影响。

Method: 提出部分重叠保留策略作为实用折中方案，在支持联合评估和最大化训练数据使用之间取得平衡。研究联合调优权重衰减、温度缩放和早停策略的效果。

Result: 联合调优集成通常能匹配或提高性能，但在不同任务和指标上效果大小存在显著差异。部分重叠保留策略提供了有吸引力的实用解决方案。

Conclusion: 研究结果揭示了深度集成训练中个体优化和联合优化之间的权衡，为实践者优化深度集成模型提供了有价值的见解和指导。
...
### [173] [ScaleDL: Towards Scalable and Efficient Runtime Prediction for Distributed Deep Learning Workloads](https://arxiv.org/abs/2511.04162)
*Xiaokai Wang,Shaoyuan Huang,Yuting Li,Xiaofei Wang*

Main category: cs.LG

TL;DR: ScaleDL是一个新颖的DNN运行时预测框架，结合非线性逐层建模和图神经网络交叉层交互机制，在保持准确性的同时显著降低数据收集成本，相比基线模型实现了6倍更低的MRE和5倍更低的RMSE。


<details>
  <summary>Details</summary>
Motivation: 随着DNN模型规模和复杂度的增加，训练和推理任务对分布式计算资源的需求急剧增长，传统基于加法计算单元模型的方法在准确性和泛化性上存在局限，而图增强建模虽然性能更好但数据收集成本过高，需要一种平衡准确性、泛化性和数据收集成本的方法。

Method: 提出ScaleDL框架，结合非线性逐层建模和基于图神经网络的交叉层交互机制，使用D-optimal方法降低数据收集成本，实现跨不同网络架构的分层泛化能力。

Result: 在五个流行DNN模型的工作负载上进行实验，证明ScaleDL显著提升了运行时预测准确性和泛化性，相比基线模型实现了6倍更低的平均相对误差和5倍更低的均方根误差。

Conclusion: ScaleDL框架成功解决了DNN运行时预测中准确性、泛化性和数据收集成本之间的平衡问题，为优化DNN开发和资源分配提供了有效的解决方案。
...
### [174] [Block Rotation is All You Need for MXFP4 Quantization](https://arxiv.org/abs/2511.04214)
*Yuantian Shao,Peisong Wang,Yuanteng Chen,Chang Xu,Zhihui Wei,Jian Cheng*

Main category: cs.LG

TL;DR: 本文对MXFP4格式下的后训练量化方法进行了系统评估，发现GPTQ等方法表现良好，而基于旋转的方法与MXFP4格式存在严重不兼容问题。作者提出了一种简单的块旋转策略来适配MXFP4格式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型规模快速增长带来了巨大的内存、计算和能耗成本。虽然后训练量化是高效部署的解决方案，但在W4A4量化方面仍面临挑战。随着MXFP4这种新型FP4格式的出现，需要评估现有量化方法在该格式下的适用性。

Method: 建立了MXFP4格式下PTQ方法的全面基准测试，通过系统评估发现旋转方法与MXFP4格式的不兼容性，并深入分析了冲突根源。提出了一种简单的块旋转策略来适配旋转方法到MXFP4格式。

Result: GPTQ等方法在MXFP4格式下表现稳定，而基于旋转的方法由于MXFP4的PoT块缩放与全局旋转能量重分布之间的不匹配而性能严重下降。提出的块旋转策略显著提升了各种LLM的量化精度。

Conclusion: 该研究为从业者提供了明确的指导，并为新兴低精度格式下的PTQ研究奠定了基础，提出的块旋转策略有效解决了旋转方法与MXFP4格式的兼容性问题。
...
### [175] [The Strong Lottery Ticket Hypothesis for Multi-Head Attention Mechanisms](https://arxiv.org/abs/2511.04217)
*Hikari Otsuka,Daiki Chijiwa,Yasuyuki Okoshi,Daichi Fujiki,Susumu Takeuchi,Masato Motomura*

Main category: cs.LG

TL;DR: 该论文证明了强彩票票假设在多头注意力机制和Transformer架构中的存在性，表明在随机初始化的网络中隐藏着高性能子网络。
...
Method: 通过理论分析证明，在随机初始化的多头注意力机制中，如果键和值的隐藏维度为O(dlog(Hd^{3/2}))，则高概率包含能够近似任意目标多头注意力的强彩票票。

Result: 理论证明和实验验证均表明，通过增加源模型的隐藏维度，源模型中的强彩票票与近似目标模型之间的误差呈指数级下降。

Conclusion: 该研究成功将强彩票票假设扩展到多头注意力机制和Transformer架构，为理解Transformer中高性能子网络的存在提供了理论基础。
...
### [176] [seqme: a Python library for evaluating biological sequence design](https://arxiv.org/abs/2511.04239)
*Rasmus Møller-Larsen,Adam Izdebski,Jan Olszewski,Pankhil Gawade,Michal Kmicikiewicz,Wojciech Zarzecki,Ewa Szczurek*

Main category: cs.LG

TL;DR: seqme是一个用于评估生物序列设计计算方法的开源Python库，包含序列、嵌入和属性三类模型无关的评估指标。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏一个统一的软件库来实现评估生物序列设计方法性能的指标，这些指标需要评估设计序列对目标分布的保真度和期望属性的实现程度。

Method: 开发了一个模块化、高度可扩展的开源Python库seqme，包含序列、嵌入和属性三类模型无关的评估指标，适用于多种生物序列类型。

Result: seqme库提供了多种生物序列的嵌入和属性模型，以及诊断和可视化功能，可用于评估一次性设计和迭代计算设计方法。
...
### [177] [Guided by Stars: Interpretable Concept Learning Over Time Series via Temporal Logic Semantics](https://arxiv.org/abs/2511.04244)
*Irene Ferfoglia,Simone Silvetti,Gaia Saveri,Laura Nenzi,Luca Bortolussi*

Main category: cs.LG

TL;DR: STELLE是一个神经符号框架，通过将时间序列直接嵌入到时序逻辑概念空间，统一分类和解释任务，提供可解释的时间序列分类。


<details>
  <summary>Details</summary>
Motivation: 时间序列分类在安全关键应用中很重要，但通常使用黑盒深度学习方法，难以理解其决策依据。需要开发既能保持竞争力又能提供可解释性的方法。

Method: 提出STL启发的核函数，将原始时间序列映射到与预定义STL公式的对齐程度，联合优化准确性和可解释性，每个预测都伴随最相关的逻辑概念。

Result: 实验证明STELLE在保持竞争力的准确性的同时，提供逻辑上可信的解释，在多样化的真实世界基准测试中得到验证。

Conclusion: STELLE框架成功地将神经网络的表示能力与时序逻辑的符号推理相结合，为时间序列分类提供了既准确又可解释的解决方案。
...
### [178] [Efficient Reinforcement Learning from Human Feedback via Bayesian Preference Inference](https://arxiv.org/abs/2511.04286)
...
Method: 将主动查询模块集成到RLHF流程中，结合RLHF的可扩展性和PBO的样本效率优势，实现主动和样本高效的偏好收集。
...
### [179] [Differentially Private In-Context Learning with Nearest Neighbor Search](https://arxiv.org/abs/2511.04332)
*Antti Koskela,Tejas Kulkarni,Laith Zumot*

Main category: cs.LG

TL;DR: 提出了一个结合最近邻搜索的差分隐私上下文学习框架，通过隐私过滤器跟踪累积隐私成本，在文本分类和文档问答任务上显著优于现有基线方法。
...
Result: 在所有评估基准测试中，该方法相比现有基线都有显著优势，实现了更优的隐私-效用权衡。

Conclusion: 提出的差分隐私上下文学习框架通过整合隐私感知的最近邻搜索，在保护隐私的同时提升了模型性能。
...
### [180] [LUME-DBN: Full Bayesian Learning of DBNs from Incomplete data in Intensive Care](https://arxiv.org/abs/2511.04333)
...
<details>
  <summary>Details</summary>
Motivation: 现有处理纵向临床数据缺失值的方法主要来自静态贝叶斯网络文献，未能充分考虑数据的时间特性，限制了时间不确定性的量化能力，这在重症监护等需要理解时间动态的场景中尤为重要。

Method: 采用吉布斯采样方法学习动态贝叶斯网络，将每个缺失值视为服从高斯分布的未知参数，在每次迭代中从未观测值的完全条件分布中采样，实现原则性的插补和不确定性估计。

Result: 在模拟数据集和真实重症监护患者数据上的评估表明，相比MICE等标准模型无关技术，该贝叶斯方法展现出更优的重建精度和收敛特性。

Conclusion: 该方法支持更安全、更知情的临床决策制定，特别是在缺失数据频繁且可能产生影响的场景中，强调了在时间模型中纳入完全贝叶斯推断的临床相关性。
...
### [181] [The Illusion of Certainty: Uncertainty quantification for LLMs fails under ambiguity](https://arxiv.org/abs/2511.04418)
...
<details>
  <summary>Details</summary>
Motivation: 现实语言存在固有歧义性（偶然不确定性），但现有UQ方法通常在无歧义任务上进行基准测试，无法反映真实部署场景。

Method: 引入MAQA*和AmbigQA*数据集，配备基于事实共现估计的真实答案分布；评估了三种不确定性估计范式：预测分布、内部表征和模型集成。

Result: 所有不确定性估计方法在歧义数据上都表现不佳，性能退化至接近随机水平；理论分析表明预测分布和集成方法在歧义情况下存在根本限制。

Conclusion: 研究揭示了当前LLM不确定性量化方法的关键不足，需要重新思考当前建模范式以适应现实语言的歧义性。
...
### [182] [Spurious Correlation-Aware Embedding Regularization for Worst-Group Robustness](https://arxiv.org/abs/2511.04401)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型容易依赖虚假相关性，在分布偏移下表现脆弱，特别是在子群体偏移场景中，模型在少数群体上表现不佳。现有方法缺乏连接嵌入空间表示与最差群体误差的理论框架。

Method: SCER方法直接在嵌入层面对特征表示进行正则化，抑制虚假线索。通过分析不同领域和类别间群体平均嵌入的差异，识别核心方向和虚假方向，并施加理论约束。

Result: 在多个视觉和语言任务上的系统评估表明，SCER在最差群体准确率上优于现有最先进方法。

Conclusion: SCER通过理论驱动的嵌入正则化，有效提升模型对虚假相关性的鲁棒性，在最差群体上取得更好性能。
...
### [183] [On the Equivalence of Regression and Classification](https://arxiv.org/abs/2511.04422)
...
Method: 通过证明回归问题与线性可分分类任务的等价性，将M个样本的回归问题转化为2M个样本的分类问题，并基于边界最大化推导出新的回归公式。

Result: 提出了可回归性度量方法，可以在不学习模型的情况下估计数据集的回归难度，并训练神经网络学习线性化映射，将输入变量转换到线性回归器适用的空间。

Conclusion: 建立了回归与分类的严格等价关系，为回归问题提供了新的理论基础和实用工具，包括可回归性度量和线性化映射方法。
...
### [184] [ForecastGAN: A Decomposition-Based Adversarial Framework for Multi-Horizon Time Series Forecasting](https://arxiv.org/abs/2511.04445)
...
<details>
  <summary>Details</summary>
Motivation: 现有transformer模型在短期预测中表现不佳且忽略分类特征，需要一种能适应不同预测跨度并整合各类特征的通用方法。

Method: 使用三个模块：分解模块提取季节性和趋势成分；模型选择模块根据预测跨度选择最优神经网络配置；对抗训练模块通过条件生成对抗网络增强预测鲁棒性。

Result: 在11个多元时间序列基准数据集上验证，ForecastGAN在短期预测中持续优于最先进的transformer模型，在长期预测中保持竞争力。
...
### [185] [Federated Stochastic Minimax Optimization under Heavy-Tailed Noises](https://arxiv.org/abs/2511.04456)
...
<details>
  <summary>Details</summary>
Motivation: 重尾噪声在非凸随机优化中日益受到关注，因为经验研究表明它比标准有界方差假设更符合现实情况。本文研究联邦学习环境下重尾梯度噪声的非凸-PL极小极大优化问题。

Method: 提出了两种新算法：Fed-NSGDA-M（整合归一化梯度）和FedMuon-DA（利用Muon优化器进行局部更新），两者都旨在在更温和条件下有效处理联邦极小极大优化中的重尾噪声。

Result: 理论上证明了两种算法都达到了$O({1}/{(TNp)^{rac{s-1}{2s}}})$的收敛速率。据我们所知，这是首个在重尾噪声下具有严格理论保证的联邦极小极大优化算法。

Conclusion: 广泛的实验进一步验证了所提算法的有效性，为联邦学习中的重尾噪声优化问题提供了理论支撑和实用解决方案。
...
### [186] [Towards Causal Market Simulators](https://arxiv.org/abs/2511.04469)
*Dennis Thumm,Luis Ontaneda Mijares*

Main category: cs.LG

TL;DR: 提出了TNCM-VAE模型，将变分自编码器与结构因果模型结合，生成具有因果推理能力的反事实金融时间序列数据。


<details>
  <summary>Details</summary>
Motivation: 现有的市场生成器缺乏因果推理能力，无法进行有效的反事实分析和风险评估。
...
### [187] [Q3R: Quadratic Reweighted Rank Regularizer for Effective Low-Rank Training](https://arxiv.org/abs/2511.04485)
...
Method: 基于迭代重加权最小二乘框架，使用二次正则化项来近似平滑对数行列式作为秩替代目标，实现低秩诱导训练。

Result: 在ViT-Tiny模型上能够截断60%和80%的参数，CIFAR-10准确率仅下降约1.3%和4%；在图像和语言任务的Transformer上均验证了有效性。

Conclusion: Q3R是一种有效的低秩训练方法，能够以较小的计算开销训练具有指定低目标秩的权重矩阵，与现有架构完全兼容。
...
### [188] [Distribution-Aware Tensor Decomposition for Compression of Convolutional Neural Networks](https://arxiv.org/abs/2511.04494)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络在图像任务中计算需求大，传统压缩方法在权重空间使用各向同性范数（如Frobenius范数）进行低秩近似，但这种方法不够高效且通常需要后压缩微调。

Method: 使用数据信息范数衡量函数空间误差，具体为最小化层输出分布的变化，表达为∥(W-W̃)Σ¹/²∥F，其中Σ¹/²是层输入协方差矩阵的平方根。提出了新的交替最小二乘算法用于Tucker-2和CPD张量分解。
...
Conclusion: 基于数据信息的压缩方法优于传统权重空间方法，无需微调且具有跨数据集的可迁移性，为神经网络压缩提供了更高效的解决方案。
...
### [189] [Alternative Fairness and Accuracy Optimization in Criminal Justice](https://arxiv.org/abs/2511.04505)
*Shaolong Wu,James Blume,Geshi Yeung*

Main category: cs.LG

TL;DR: 本文提出了一种改进的群体公平性方法，通过最小化加权错误损失同时将假阴性率差异控制在容忍范围内，为刑事司法系统中的算法公平性提供了实用框架。


<details>
  <summary>Details</summary>
Motivation: 算法公平性研究发展迅速，但在刑事司法等关键领域核心概念仍不明确，特别是不同公平性概念之间存在冲突，需要开发实用的解决方案。

Method: 开发了一种改进的群体公平性方法：不追求受保护群体间的精确平等，而是最小化加权错误损失，同时将假阴性率差异控制在小的容忍范围内。
...
### [190] [Linear Mode Connectivity under Data Shifts for Deep Ensembles of Image Classifiers](https://arxiv.org/abs/2511.04514)
*C. Hepburn,T. Zielke,A. P. Raulf*

Main category: cs.LG

TL;DR: 本文研究了数据偏移下的线性模式连通性(LMC)现象，发现通过小学习率和大批次大小可以减轻数据偏移的影响，并探讨了LMC在训练效率与模型多样性之间的平衡。


<details>
  <summary>Details</summary>
Motivation: 研究数据偏移对线性模式连通性的影响，理解数据偏移作为随机梯度噪声源的作用，以及如何通过训练参数调节来缓解这种影响。

Method: 将数据偏移解释为额外的随机梯度噪声源，通过实验研究学习率和批次大小对模型收敛到相同局部最小值或不同区域的影响。

Result: 小学习率和大批次大小可以减少数据偏移的影响，使模型收敛到相同的局部最小值；通过LMC采样的模型倾向于产生更相似的错误，但LMC在训练效率和模型多样性之间提供了平衡。

Conclusion: LMC在数据偏移下仍然有效，通过适当的训练参数可以减轻数据偏移的影响，LMC的价值在于平衡训练效率与更大、更多样化集成模型带来的收益。
...
### [191] [Comparing EPGP Surrogates and Finite Elements Under Degree-of-Freedom Parity](https://arxiv.org/abs/2511.04518)
*Obed Amo,Samit Ghosh,Markus Lange-Hegermann,Bogdan Raiţă,Michael Pokojovy*

Main category: cs.LG

TL;DR: 比较边界约束Ehrenpreis-Palamodov高斯过程(B-EPGP)代理模型与经典有限元方法结合Crank-Nicolson时间步进(CN-FEM)在求解二维波动方程时的性能，B-EPGP在匹配自由度下精度提高约两个数量级


<details>
  <summary>Details</summary>
Motivation: 开发新的基准测试来比较基于特征函数的高斯过程代理模型与传统有限元方法在求解偏微分方程方面的性能差异

Method: B-EPGP使用从特征簇导出的指数多项式基函数精确强制PDE和边界条件，采用惩罚最小二乘法估计系数；CN-FEM使用经典有限元方法结合Crank-Nicolson时间步进；引入自由度匹配协议确保公平比较

Result: 在匹配自由度条件下，B-EPGP始终获得比CN-FEM更低的时空L^2误差和最大时间L^2空间误差，精度提高约两个数量级

Conclusion: B-EPGP方法在求解二维波动方程方面显著优于传统的CN-FEM方法，在相同计算复杂度下提供更高的精度
...
### [192] [End-to-End Reinforcement Learning of Koopman Models for eNMPC of an Air Separation Unit](https://arxiv.org/abs/2511.04522)
*Daniel Mayfrank,Kayra Dernek,Laura Lang,Alexander Mitsos,Manuel Dahmen*

Main category: cs.LG

TL;DR: 本文提出了一种基于强化学习的Koopman代理模型训练方法，用于非线性模型预测控制应用，并在大规模空分装置案例中验证了其可扩展性和性能优势。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅在小规模案例中得到验证，需要证明该方法在更具挑战性的大规模需求响应案例中的可扩展性，特别是在仅能观测少量实际可测量变量的情况下。

Method: 使用基于强化学习的Koopman代理模型训练方法，针对大规模单产品（氮气）空分装置构建需求响应案例研究。

Result: 与基于系统辨识的Koopman eNMPC相比，该方法在保持相似经济性能的同时避免了约束违反，而传统方法虽然产生小的经济节约但频繁违反约束。

Conclusion: 该方法成功扩展到大规模案例，在仅观测少量实际可测量变量的条件下，实现了良好的经济性能且避免了约束违反，证明了其在实际工业应用中的潜力。
...
### [193] [Uncertainty Quantification for Reduced-Order Surrogate Models Applied to Cloud Microphysics](https://arxiv.org/abs/2511.04534)
*Jonas E. Katona,Emily K. de Jong,Nipun Gunawardena*

Main category: cs.LG

TL;DR: 提出了一种模型无关的后处理框架，用于潜在空间降阶模型中的预测不确定性量化，无需修改底层架构或训练过程。
...
Method: 使用保形预测方法，在潜在空间降阶模型的多个组件（潜在动态、重构和端到端预测）中估计统计预测区间。

Result: 在云微物理学的潜在空间动态模型上验证了该方法，能够准确预测液滴尺寸分布的演化，并在整个降阶模型流程中量化不确定性。

Conclusion: 该模型无关的后处理框架为降阶模型提供了灵活且通用的不确定性量化解决方案。
...
### [194] [Integrating Temporal and Structural Context in Graph Transformers for Relational Deep Learning](https://arxiv.org/abs/2511.04557)
*Divyansha Lachi,Mahmoud Mohammadi,Joe Meyer,Vinam Arora,Tom Palczewski,Eva L. Dyer*

Main category: cs.LG

TL;DR: 提出RGP模型，通过时间子图采样和跨注意力瓶颈机制，有效整合时空依赖关系，支持多任务预测，在多个数据集上达到SOTA性能。
...
Method: 提出时间子图采样器捕获时间相关关系；设计RGP图变换器架构，使用跨注意力潜在瓶颈整合结构和时间上下文信息；采用灵活跨注意力解码器支持多任务联合学习。

Result: 在RelBench、SALT和CTU数据集上的实验表明，RGP实现了最先进的性能表现。

Conclusion: RGP为关系深度学习提供了一个通用且可扩展的解决方案，能够有效支持多样化的预测任务。
...
### [195] [ARETE: an R package for Automated REtrieval from TExt with large language models](https://arxiv.org/abs/2511.04573)
*Vasco V. Branco,Jandó Benedek,Lidia Pivovarova,Luís Correia,Pedro Cardoso*

Main category: cs.LG

TL;DR: ARETE是一个基于大型语言模型的R包，用于自动从文献中提取物种出现数据，显著提高了数据获取效率并扩展了物种分布范围信息。


<details>
  <summary>Details</summary>
Motivation: 保护生物学面临的关键挑战是缺乏物种出现数据，而文献中的数据往往非机器可读，需要大量人工处理。

Method: 开发ARETE R包，利用chatGPT API进行光学字符识别、数据提取和验证，自动化处理物种出现数据。

Result: 对100种蜘蛛的测试显示，新提取数据将已知分布范围平均扩大了三个数量级，揭示了新的历史分布区域。
...
### [196] [Complexity as Advantage: A Regret-Based Perspective on Emergent Structure](https://arxiv.org/abs/2511.04590)
*Oshri Naparstek*

Main category: cs.LG

TL;DR: 提出了复杂性即优势(CAA)框架，将系统复杂性定义为相对于观察者家族的预测遗憾差异。系统复杂意味着对某些观察者容易预测，对其他观察者难以预测，从而创造信息优势。


<details>
  <summary>Details</summary>
Motivation: 传统复杂性度量将复杂性视为系统内在属性，但实际中复杂性价值往往体现在不同观察者之间的预测能力差异上。该框架旨在为复杂性提供功能价值基础。

Method: 通过定义观察者家族的预测遗憾来评估系统复杂性，系统复杂性体现在不同观察者预测难易程度的差异上。使用简单动力学模型进行演示。

Result: 该框架统一了多种涌现行为概念，包括多尺度熵、预测信息和观察者依赖结构。表明"有趣"系统是那些能在不同观察者间产生差异化遗憾的系统。

Conclusion: CAA框架为复杂性提供了功能性解释，表明复杂性价值在于创造观察者间的信息优势，对学习、进化和智能体设计具有启示意义。
...
### [197] [Regret Lower Bounds for Decentralized Multi-Agent Stochastic Shortest Path Problems](https://arxiv.org/abs/2511.04594)
*Utkarsh U. Chavan,Prashant Trivedi,Nandyala Hemachandra*

Main category: cs.LG

TL;DR: 本文研究了去中心化多智能体随机最短路径问题（Dec-MASSPs），在线性函数逼近框架下分析了学习复杂度，并首次建立了该设置的遗憾下界Ω(√K)。
...
Method: 应用基于对称性的新论证方法，识别最优策略的结构，并通过构建对任何数量智能体都难以学习的实例来建立遗憾下界。

Result: 证明了Dec-MASSPs的遗憾下界为Ω(√K)，揭示了去中心化控制中的固有学习难度。

Conclusion: 这些发现阐明了去中心化控制的学习复杂度，可为多智能体系统中高效学习算法的设计提供指导。
...
### [198] [Environment Agnostic Goal-Conditioning, A Study of Reward-Free Autonomous Learning](https://arxiv.org/abs/2511.04598)
*Hampus Åström,Elin Anna Topp,Jacek Malec*

Main category: cs.LG

TL;DR: 将常规强化学习环境转换为目标条件环境，使智能体能够自主、无奖励地学习解决任务。智能体通过自主选择目标进行学习，训练时间与外部引导的强化学习相当。


<details>
  <summary>Details</summary>
Motivation: 研究如何让智能体在无外部奖励的情况下自主学习解决任务，实现通用智能体训练，为特定应用场景提供预训练模型。

Method: 将常规强化学习环境转换为目标条件环境，让智能体自主选择目标进行学习，该方法与底层离策略学习算法无关。

Result: 虽然智能体不会对任何目标给予更高价值导致单个目标性能不稳定，但平均目标成功率得到改善和稳定。训练后的智能体可以寻求环境中的任何观测结果。

Conclusion: 该方法实现了环境无关的自主目标学习，能够通用地训练智能体，为后续特定应用提供基础。
...
### [199] [Addressing divergent representations from causal interventions on neural networks](https://arxiv.org/abs/2511.04638)
*Satchel Grant,Simon Jerome Han,Alexa Tartaglini,Christopher Potts*

Main category: cs.LG

TL;DR: 本文研究了因果干预在机制可解释性中可能导致的分布外表示问题，分析了无害和有害两种偏差类型，并提出了改进的CL损失函数来减轻有害偏差。


<details>
  <summary>Details</summary>
Motivation: 研究因果干预技术是否会产生分布外表示，以及这是否会影响解释方法对目标模型自然状态的忠实性。

Method: 首先实证验证常见因果干预技术会导致内部表示偏离自然分布，然后理论分析无害和有害两种偏差类型，最后改进CL损失函数来正则化干预。

Result: 发现因果干预确实会产生分布外表示，其中无害偏差发生在权重零空间和行为决策边界内，有害偏差会激活隐藏网络通路并导致休眠行为变化。

Conclusion: 通过改进的CL损失函数可以减轻有害偏差，同时保持干预的解释能力，为实现更可靠的可解释性方法指明了方向。
...
### [200] [Efficient probabilistic surrogate modeling techniques for partially-observed large-scale dynamical systems](https://arxiv.org/abs/2511.04641)
*Hans Harder,Abhijeet Vishwasrao,Luca Guastoni,Ricardo Vinuesa,Sebastian Peitz*

Main category: cs.LG

TL;DR: 本文研究用于偏微分方程描述的动力系统概率预测技术，比较了多种减少采样步骤的流匹配扩展方法，包括直接蒸馏、渐进蒸馏、对抗扩散蒸馏、Wasserstein GANs和整流流。


<details>
  <summary>Details</summary>
Motivation: 开发高效的动力系统预测方法，特别是针对偏微分方程描述的系统（如Navier-Stokes方程），减少采样步骤以提高计算效率。

Method: 比较了五种流匹配扩展方法：直接蒸馏、渐进蒸馏、对抗扩散蒸馏、Wasserstein GANs和整流流，并在具有挑战性的系统上进行实验验证。

Result: 实现了对大规模3D模拟的2D切片直接预测，为求解器的高效流入生成铺平了道路。

Conclusion: 多种流匹配扩展方法能够有效减少采样步骤，为偏微分方程动力系统的概率预测提供了高效解决方案。
...
### [201] [Optimal Inference Schedules for Masked Diffusion Models](https://arxiv.org/abs/2511.04647)
*Sitan Chen,Kevin Cong,Jerry Li*

Main category: cs.LG

TL;DR: 本文研究了扩散语言模型中的并行采样问题，通过函数逼近理论精确刻画了采样分布与真实分布之间的期望散度，并给出了基于信息论性质的新采样边界和调度策略。


<details>
  <summary>Details</summary>
Motivation: 标准自回归大语言模型的推理过程是顺序的，导致推理时间长且成本高。扩散语言模型（特别是掩码扩散模型）能够并行采样，但缺乏对其并行采样能力与性能损失关系的严格理解。

Method: 建立采样分布与真实分布之间期望散度的精确表征，将其与单变量函数逼近理论联系起来。基于此连接，推导出新的下界和上界，并设计新的采样调度策略。

Result: 发现一般情况下难以与最优解竞争，除非对分布有强先验知识。但在某些自然设置下，可以在O(log n)步内采样而不会出现明显性能损失，其中n是序列总长度。

Conclusion: 通过函数逼近理论为扩散语言模型的并行采样提供了严格的理论基础，并展示了在信息论约束下实现高效并行采样的可能性。
...
### [202] [TT-Prune: Joint Model Pruning and Resource Allocation for Communication-efficient Time-triggered Federated Learning](https://arxiv.org/abs/2511.04653)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习网络中设备数量不断增加且无线带宽有限，导致训练速度慢（stragglers）和通信开销大的问题。
...
### [203] [Nowcast3D: Reliable precipitation nowcasting via gray-box learning](https://arxiv.org/abs/2511.04659)
...
TL;DR: 提出了一种灰盒、完全三维的临近预报框架，直接处理体积雷达反射率数据，将物理约束的神经算子与数据驱动学习相结合，在极端降水临近预报中实现了更准确的3小时预报。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：数值天气预报及其深度学习模拟对快速演变的对流过程过于缓慢和粗糙；外推法和纯数据驱动模型存在误差累积和过度平滑问题；基于2D雷达的混合方法丢弃了关键的垂直信息。

Method: 学习垂直变化的3D平流场，使用保守平流算子；参数化空间变化的扩散；引入布朗运动启发的随机项表示未解析运动；残差分支捕捉小尺度对流启动和微物理变化；基于扩散的随机模块估计不确定性。

Result: 在3小时预报时间内实现了更准确的预测，在160名气象学家的盲评中，在57%的情况下排名第一，在各种降水机制中均表现优异。
...
### [204] [Forgetting is Everywhere](https://arxiv.org/abs/2511.04666)
*Ben Sanati,Thomas L. Lee,Trevor McInroe,Aidan Scannell,Nikolay Malkin,David Abel,Amos Storkey*

Main category: cs.LG

TL;DR: 提出了一个算法和任务无关的遗忘理论，将遗忘定义为学习者在未来经验预测分布中缺乏自一致性，表现为预测信息的损失。该理论自然产生了一种衡量算法遗忘倾向的通用方法。


<details>
  <summary>Details</summary>
Motivation: 开发通用学习算法面临的根本挑战是它们在适应新数据时容易忘记过去的知识。尽管经过数十年研究，仍缺乏统一的定义来理解学习的底层动态。

Method: 提出了算法和任务无关的遗忘理论，将遗忘表征为预测分布中缺乏自一致性。设计了一套全面的实验，涵盖分类、回归、生成建模和强化学习。

Result: 实证研究表明遗忘存在于所有学习设置中，并在决定学习效率方面发挥重要作用。

Conclusion: 这些结果为遗忘建立了原则性理解，并为分析和改进通用学习算法的信息保留能力奠定了基础。
...
### [205] [Multi-Method Analysis of Mathematics Placement Assessments: Classical, Machine Learning, and Clustering Approaches](https://arxiv.org/abs/2511.04667)
*Julian D. Allagan,Dasia A. Singleton,Shanae N. Perry,Gabrielle C. Morgan,Essence A. Morgan*

Main category: cs.LG

TL;DR: 本研究使用经典测验理论、机器学习和无监督聚类相结合的多方法框架评估了198名学生参加的40项数学分班考试。结果显示55%的题目具有优秀区分度，30%题目区分度差需要替换。机器学习模型达到97.5%准确率，聚类分析发现42.5%的自然能力分界点，建议优化分班标准。


<details>
  <summary>Details</summary>
Motivation: 通过多方法整合评估数学分班考试的有效性，为基于证据的分班优化提供实证基础，解决传统分班标准可能存在的过度分类问题。

Method: 结合经典测验理论分析题目区分度，使用随机森林和梯度提升等机器学习算法预测学生表现，采用K-means聚类识别自然能力结构。

Result: 55%题目区分度优秀，30%题目区分度差需替换；第6题（图表解读）是最佳区分项；机器学习模型准确率达96-97.5%；聚类发现42.5%的自然能力边界，与机构55%标准存在差异。

Conclusion: 多方法整合为数学分班优化提供了稳健的实证基础，建议替换低区分度题目、实施两阶段评估、结合随机森林预测与透明度机制，优化分班决策。
...
### [206] [On the Brittleness of CLIP Text Encoders](https://arxiv.org/abs/2511.04247)
*Allie Tran,Luca Rossetto*

Main category: cs.MM

TL;DR: 本文系统分析了CLIP等多模态嵌入模型在多媒体信息检索中对非语义查询扰动的敏感性，发现句法和语义扰动会导致最大的不稳定性，而表面编辑（如标点和大小写）则最为脆弱。


<details>
  <summary>Details</summary>
Motivation: 现有的对比对齐训练的多模态嵌入模型对小的输入扰动缺乏稳定性，特别是在处理手动表达的查询时，查询的微小变化可能导致最佳匹配结果排名的巨大差异。

Method: 在多媒体信息检索场景中，使用TRECVID Ad-Hoc Video Search查询和V3C1视频集合，评估多种CLIP变体对词汇、句法和语义扰动的响应。

Result: 研究发现句法和语义扰动导致最大的不稳定性，而脆弱性集中在表面的简单编辑（如标点和大小写变化）。

Conclusion: 鲁棒性是评估视觉语言模型超越基准准确性的关键维度，需要更多关注模型对查询扰动的稳定性。
...
### [207] [Caption Injection for Optimization in Generative Search Engine](https://arxiv.org/abs/2511.04080)
*Xiaolu Chen,Yong Liao*

Main category: cs.IR

TL;DR: 提出了首个多模态生成搜索引擎优化方法Caption Injection，通过从图像中提取标题并注入文本内容，在MRAG基准测试中显著优于仅文本的G-SEO基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有G-SEO方法仅限于文本优化，无法充分利用多模态数据，而随着多模态检索增强生成技术的发展，需要新的方法来提升内容在生成搜索中的主观可见性。

Method: 提出Caption Injection方法，从图像中提取标题并注入到文本内容中，整合视觉语义来增强生成搜索场景中内容的主观可见性。

Result: 在MRAMG基准测试中，Caption Injection在G-Eval指标下显著优于仅文本的G-SEO基线方法，证明了多模态整合在G-SEO中的必要性和有效性。

Conclusion: 多模态整合对于提升生成搜索引擎优化效果至关重要，Caption Injection方法有效提高了用户感知的内容可见性。
...
### [208] [E-CARE: An Efficient LLM-based Commonsense-Augmented Framework for E-Commerce](https://arxiv.org/abs/2511.04087)
*Ge Zhang,Rohan Deepak Ajwani,Tony Zheng,Hongjian Gu,Yaochen Hu,Wei Guo,Mark Coates,Yingxue Zhang*

Main category: cs.IR

TL;DR: 提出E-CARE方法，通过构建常识推理因子图来编码LLM的推理模式，在推荐系统中实现高效的常识增强，只需单次LLM前向传播即可获得推理能力，在2个下游任务中精度提升达12.1%。


<details>
  <summary>Details</summary>
Motivation: 解决电商平台中基于LLM的查询-产品相关性预测方法存在的高成本问题，包括实时LLM推理开销大、人工标注和微调需求，同时保留LLM的常识推理能力。

Method: 设计E-CARE框架，构建常识推理因子图来编码LLM的推理模式，在推理时只需单次LLM前向传播即可访问常识推理能力，避免了密集的实时LLM调用。

Result: 在2个下游任务上的实验显示，精度@5指标提升了最高12.1%，同时显著降低了推理成本。

Conclusion: E-CARE方法成功地在保持LLM常识推理能力的同时，大幅提升了推荐系统的效率，为电商平台提供了更实用的解决方案。
...
### [209] [Transforming Mentorship: An AI Powered Chatbot Approach to University Guidance](https://arxiv.org/abs/2511.04172)
...
Method: 构建数据摄取管道处理CSV文件和大学网页信息，采用BM25词汇排序与ChromaDB语义检索的混合方法，使用LLaMA-3.3-70B生成对话响应。

Result: 生成文本语义相关性高，BERTScore达0.831，METEOR得分0.809；数据管道更新效率高，更新耗时106.82秒，新数据处理耗时368.62秒。
...
### [210] [Coordination-Free Lane Partitioning for Convergent ANN Search](https://arxiv.org/abs/2511.04221)
*Carl Kugblenu,Petri Vuorimaa*

Main category: cs.IR

TL;DR: 提出了一种无协调的并行搜索分区方法，通过确定性候选池和伪随机排列，将重复计算转化为互补工作，在不增加成本的情况下提高搜索覆盖率。


<details>
  <summary>Details</summary>
Motivation: 现有的向量搜索系统在并行处理查询时，各线程/副本/分片会重复发现相同的候选结果，导致计算资源浪费而无法提高覆盖率。

Method: 对每个查询：(1)构建确定性的候选池，大小为总top-k预算；(2)应用每查询的伪随机排列；(3)为每个并行通道分配不相交的位置切片，从而实现无运行时协调的不同结果返回。

Result: 在4个并行通道、总候选预算64的情况下，SIFT1M数据集上recall@10从0.249提升到0.999，通道重叠率从近100%降至0%；MS MARCO数据集上hit@10从0.200提升到0.601，MRR@10从0.133提升到0.330。

Conclusion: 提出了简单的操作指南：将每查询候选池大小设为总预算，确定性地跨通道分区位置，在不改变预算或截止时间的情况下将冗余分发转化为互补覆盖。
...
### [211] [Denoised Recommendation Model with Collaborative Signal Decoupling](https://arxiv.org/abs/2511.04237)
*Zefeng Li,Ning Yang*
...
<details>
  <summary>Details</summary>
Motivation: 传统协同过滤算法因用户-物品交互矩阵中的噪声导致推荐性能不佳，现有去噪方法多在单一图上进行，会削弱协作信号

Method: 使用GNN-based CF模型DRCSD，包含协作信号解耦模块（按结构特征分解信号为不同阶次）和分阶去噪模块（对每阶进行针对性去噪），并改进传统GNN的信息聚合机制

Result: 在三个公开真实数据集上的实验表明，DRCSD对不稳定交互具有优越的鲁棒性，在推荐精度指标上相比最先进基线模型获得显著性能提升

Conclusion: DRCSD通过解耦和分阶去噪策略有效解决了协作信号衰减问题，显著提升了推荐系统的准确性和鲁棒性
...
### [212] [LLM-as-a-Judge: Toward World Models for Slate Recommendation Systems](https://arxiv.org/abs/2511.04541)
*Baptiste Bonin,Maxime Heuillet,Audrey Durand*

Main category: cs.IR

TL;DR: 研究探索了大型语言模型如何通过基于slate的成对推理来建模用户跨域偏好，在三个任务上的实证研究表明LLMs作为推荐系统世界模型的潜力。


<details>
  <summary>Details</summary>
Motivation: 建模用户跨域偏好是slate推荐研究中的关键挑战，需要探索LLMs如何有效作为用户偏好的世界模型。

Method: 使用多个LLMs在三个不同数据集的任务上进行实证研究，通过成对推理分析slate中的偏好关系。

Result: 研究结果揭示了任务性能与LLMs捕获的偏好函数特性之间的关系，指出了改进方向。

Conclusion: LLMs在推荐系统中作为世界模型具有显著潜力，为改进用户偏好建模提供了新的可能性。
...
### [213] [Scaling Agent Learning via Experience Synthesis](https://arxiv.org/abs/2511.03773)
...
TL;DR: DreamGym是一个统一的框架，通过基于推理的经验模型合成多样化经验数据，解决RL训练中昂贵的环境交互、任务多样性不足、奖励信号不可靠等问题，显著提升自主代理的在线强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习在大语言模型代理中实际应用面临的挑战：昂贵的环境交互成本、有限的任务多样性、不可靠的奖励信号以及基础设施复杂性，这些因素阻碍了可扩展经验数据的收集。

Method: 1. 将环境动态蒸馏到基于推理的经验模型中，通过逐步推理推导一致的状态转换和反馈信号；2. 使用离线真实数据初始化经验回放缓冲区，并通过新交互持续丰富；3. 自适应生成挑战当前代理策略的新任务，实现更有效的在线课程学习。

Result: 在多样化环境和代理骨干上的实验表明：在非RL就绪任务（如WebArena）上，DreamGym比所有基线方法性能提升超过30%；在RL就绪但成本高昂的设置中，仅使用合成交互就能匹配GRPO和PPO性能；在将纯合成经验训练的策略转移到真实环境RL时，DreamGym在需要更少真实世界交互的同时获得显著额外性能提升。

Conclusion: DreamGym为通用强化学习提供了一个可扩展的热启动策略，通过合成经验数据有效解决了RL训练中的可扩展性问题，在多种场景下都表现出显著优势。
...
### [214] [How Different Tokenization Algorithms Impact LLMs and Transformer Models for Binary Code Analysis](https://arxiv.org/abs/2511.03825)
*Ahmed Mostafa,Raisul Arefin Nahid,Samuel Mulder*

Main category: cs.AI

TL;DR: 评估NLP分词模型在汇编代码分析中的内在特性和参数选择，研究预处理定制和预分词规则，并分析其对下游任务的影响。


<details>
  <summary>Details</summary>
Motivation: 汇编代码分析中的分词技术研究不足，但分词对词汇量、语义覆盖和下游任务性能有重要影响，需要系统评估不同分词模型的效果。

Method: 系统研究多种分词模型，分析其在汇编指令编码和语义捕捉方面的效率，使用Llama 3.2、BERT和BART等预训练模型评估分词器在多个性能指标上的有效性。

Result: 分词器选择显著影响下游性能，内在指标只能部分预测外在评估结果，揭示了内在分词器特性与实际应用之间的复杂权衡。

Conclusion: 为低级代码分析优化分词模型提供了有价值的见解，有助于提升基于自然语言模型的二进制分析工作流程的鲁棒性和可扩展性。
...
### [215] [To See or To Read: User Behavior Reasoning in Multimodal LLMs](https://arxiv.org/abs/2511.03845)
*Tianning Dong,Luyi Ma,Varun Vasudevan,Jason Cho,Sushant Kumar,Kannan Achan*

Main category: cs.AI

TL;DR: BehaviorLens框架系统评估了用户行为数据的文本与图像表示对多模态大语言模型性能的影响，发现图像表示能显著提升模型预测准确率。
...
Method: 开发BehaviorLens基准框架，在六个MLLM上评估交易数据的三种表示方式：文本段落、散点图和流程图，使用真实购买序列数据集。

Result: 当数据表示为图像时，MLLM的下一次购买预测准确率比等效文本表示提高了87.5%，且无需额外计算成本。

Conclusion: 图像表示用户行为数据能显著提升多模态大语言模型的推理性能，为构建更高效的智能代理系统提供了重要启示。
...
### [216] [KnowThyself: An Agentic Assistant for LLM Interpretability](https://arxiv.org/abs/2511.03878)
*Suraj Prasai,Mengnan Du,Ying Zhang,Fan Yang*

Main category: cs.AI

TL;DR: KnowThyself是一个基于聊天的LLM可解释性工具，通过整合现有碎片化能力，提供自然语言交互和可视化解释，降低技术门槛。


<details>
  <summary>Details</summary>
Motivation: 现有LLM可解释性工具存在碎片化和代码密集的问题，需要更统一、易用的解决方案。

Method: 使用编排器LLM重述用户查询，通过代理路由器分发到专用模块，最后将输出整合为连贯解释。

Result: 开发了一个可扩展的聊天界面平台，支持模型上传、自然语言提问和交互式可视化。

Conclusion: KnowThyself为LLM可解释性提供了强大且易于访问的基础平台。
...
### [217] [Large language models replicate and predict human cooperation across experiments in game theory](https://arxiv.org/abs/2511.04500)
*Andrea Cera Palatsi,Samuel Martin-Gutierrez,Ana S. Cardenal,Max Pellert*

Main category: cs.AI

TL;DR: LLMs能够复制人类在博弈论实验中的合作行为模式，Llama模型能高保真地再现人类偏离理性选择理论的行为，而Qwen模型更接近纳什均衡预测。


<details>
  <summary>Details</summary>
Motivation: 理解LLMs在多大程度上能模拟人类决策行为，这对实际应用和社会模拟至关重要，因为不匹配可能导致有害结果，而无法复制人类行为则使LLMs在社会模拟中无效。
...
Conclusion: 适当校准的LLMs能够复制群体人类行为模式，并能系统探索未开发的实验空间，为社会科学研究提供补充方法，生成关于人类社交决策的新经验预测。
...
### [218] [Extracting Causal Relations in Deep Knowledge Tracing](https://arxiv.org/abs/2511.03948)
*Kevin Hong,Kia Karbasi,Gregory Pottie*

Main category: cs.AI

TL;DR: 本文挑战了关于深度知识追踪(DKT)模型的传统解释，证明其优势在于建模先决条件关系的因果结构，而非双向关系。通过将练习关系图修剪为有向无环图(DAG)并在因果子集上训练DKT，发现其预测能力与因果结构高度一致。


<details>
  <summary>Details</summary>
Motivation: 长期以来，计算教育研究的目标是开发可解释的知识追踪模型。虽然DKT被认为是传统KT方法的重大进步，但对其成功原因的解释存在争议。本文旨在挑战关于DKT建模双向关系的流行解释，探究其真正的优势来源。

Method: 通过将练习关系图修剪为有向无环图(DAG)，并在Assistments数据集的因果子集上训练DKT模型。同时提出了一种基于DKT学习表示提取练习关系DAG的替代方法。

Result: 实验表明DKT的预测能力与因果结构高度一致，其有效性主要源于对知识组件间因果依赖关系的近似建模能力，而非简单的双向关系映射。

Conclusion: DKT的成功主要归因于其能够近似建模知识组件间的因果依赖关系，这一发现对理解深度知识追踪模型的工作机制具有重要意义。
...
### [219] [LLMs and Cultural Values: the Impact of Prompt Language and Explicit Cultural Framing](https://arxiv.org/abs/2511.03980)
*Bram Bulté,Ayla Rigouts Terryn*

Main category: cs.AI

TL;DR: LLMs对提示语言和文化框架敏感，但存在系统性文化偏见，偏向荷兰、德国、美国和日本的价值观，无法充分代表文化多样性。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能够代表其全球用户的文化多样性，以及提示语言和文化框架如何影响模型响应与不同国家人类价值观的契合度。

Method: 使用10个LLMs测试63个来自霍夫斯泰德价值观调查模块和世界价值观调查的项目，翻译成11种语言，采用有无明确文化视角的提示方式。

Result: 提示语言和文化视角都会导致LLM输出变化，但所有模型都表现出相似模式：在大多数话题上保持中立，在社交宽容等议题上持进步立场，且系统性偏向特定国家的价值观。
...
### [220] [ArchPilot: A Proxy-Guided Multi-Agent Approach for Machine Learning Engineering](https://arxiv.org/abs/2511.03985)
...
Method: 采用三代理协作框架：编排代理使用MCTS启发算法协调搜索过程；生成代理迭代生成、改进和调试候选架构；评估代理执行代理训练并生成优化的代理函数。

Result: 在MLE-Bench上的实验表明，ArchPilot优于AIDE和ML-Master等SOTA基线方法。

Conclusion: ArchPilot通过多代理协作，在有限预算下实现了高效的ML工程，最小化了对昂贵完整训练的依赖。
...
### [221] [Detecting Silent Failures in Multi-Agentic AI Trajectories](https://arxiv.org/abs/2511.04032)
...
### [222] [Interpreting Multi-Attribute Confounding through Numerical Attributes in Large Language Models](https://arxiv.org/abs/2511.04053)
*Hirohane Takagi,Gouki Minegishi,Shota Kizawa,Issey Sukeda,Hitomi Yanaka*

Main category: cs.AI

TL;DR: 该研究揭示了大型语言模型在数值推理中的系统性错误，发现LLMs会编码并放大真实世界的数值相关性，且无关上下文会诱导表征偏移，这种影响随模型规模变化。


<details>
  <summary>Details</summary>
Motivation: 尽管行为研究已记录LLMs存在数值推理错误，但底层表征机制仍不清楚。研究旨在探究LLMs如何内部整合实体的多个数值属性，以及无关数值上下文如何干扰这些表征及其下游输出。

Method: 结合线性探测与偏相关分析，以及基于提示的脆弱性测试，在不同规模的模型上进行实验。

Result: LLMs编码真实世界的数值相关性但倾向于系统性放大它们；无关上下文诱导幅度表征的一致偏移，下游影响随模型规模变化。
...
### [223] [Agentmandering: A Game-Theoretic Framework for Fair Redistricting via Large Language Model Agents](https://arxiv.org/abs/2511.04076)
*Hao Li,Haotian Chen,Ruoyuan Gong,Juanjuan Wang,Hao Jiang*

Main category: cs.AI

TL;DR: 提出了Agentmandering框架，将选区重划重新构想为两个代表对立政治利益的智能体之间的回合制谈判，通过LLM智能体将战略互动嵌入重划过程，显著减少党派偏见和不公平性。
...
### [224] [KGFR: A Foundation Retriever for Generalized Knowledge Graph Question Answering](https://arxiv.org/abs/2511.04093)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理方面表现出色，但在处理知识密集型问题时受限于上下文长度和参数化知识，现有方法存在数据集特定调优和在大型或未见图谱上可扩展性差的问题。

Method: 提出LLM-KGFR协作框架，其中KGFR使用LLM生成的关系描述编码关系，基于问题中的角色初始化实体，采用非对称渐进传播策略高效处理大型图谱，通过节点、边和路径级接口支持LLM迭代推理。

Result: 实验表明LLM-KGFR在保持可扩展性和泛化能力的同时实现了强大性能，为知识图谱增强推理提供了实用解决方案。

Conclusion: LLM-KGFR框架通过结构化检索器与大语言模型的协作，有效解决了知识密集型推理问题，具备零样本泛化能力和可扩展性优势。
...
### [225] [Testing the Testers: Human-Driven Quality Assessment of Voice AI Testing Platforms](https://arxiv.org/abs/2511.04133)
*Miguel E. Andres,Vadim Fedorov,Rida Sadek,Enric Spagnolo-Arrizabalaga,Nadescha Trudel*

Main category: cs.AI

TL;DR: 提出了首个系统性评估语音AI测试质量的框架，通过人类中心化基准测试来验证测试平台的仿真质量和评估质量。


<details>
  <summary>Details</summary>
Motivation: 随着语音AI代理快速部署到生产环境，缺乏系统性的测试可靠性验证方法，组织无法客观评估测试工具是否真正有效，这在大规模部署时造成了关键的测量差距。

Method: 结合心理测量技术（成对比较产生Elo评分、自举置信区间和置换检验）与严格的统计验证，提供可重现的度量标准，适用于任何测试方法。

Result: 对三个领先商业平台进行实证评估，结果显示性能存在显著差异：最佳平台Evalion的评估质量F1得分为0.92（其他为0.73），仿真质量为0.61（其他为0.43）。
...
### [226] [When Empowerment Disempowers](https://arxiv.org/abs/2511.04177)
...
### [227] [Opus: A Quantitative Framework for Workflow Evaluation](https://arxiv.org/abs/2511.04220)
*Alan Seroul,Théo Fagnoni,Inès Adnani,Dana O. Mohamed,Phillip Kingston*

Main category: cs.AI

TL;DR: Opus工作流评估框架是一个概率-规范化的数学模型，用于量化工作流质量和效率，整合了正确性、可靠性和成本等概念，支持工作流的比较、评分和优化。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统性的方法来评估和优化工作流的质量和效率，特别是在现代自动化系统中，需要一个能够综合考虑成功概率、资源使用和输出收益的数学框架。

Method: 结合Opus工作流奖励（概率函数估计预期性能）和Opus工作流规范惩罚（衡量结构性和信息质量的函数），形成一个统一的优化公式，支持自动评估、排名和优化。

Result: 提出了一个能够直接比较、评分和优化工作流的数学框架，可以集成到强化学习循环中指导工作流的发现和改进。

Conclusion: Opus工作流评估框架为工作流质量评估和优化提供了一个系统性的数学基础，支持自动化系统中的工作流管理和改进。
...
### [228] [AdversariaLLM: A Unified and Modular Toolbox for LLM Robustness Research](https://arxiv.org/abs/2511.04316)
*Tim Beyer,Jonas Dornbusch,Jakob Steimle,Moritz Ladenburger,Leo Schwinn,Stephan Günnemann*

Main category: cs.AI

TL;DR: AdversariaLLM是一个用于LLM越狱鲁棒性研究的工具箱，旨在解决当前LLM安全研究生态系统碎片化、实现困难的问题，提供可重现、正确且可扩展的研究框架。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全和鲁棒性研究生态系统碎片化严重，实现经常存在bug，导致研究难以重现和比较，阻碍了有意义的进展。

Method: 设计了一个以可重现性、正确性和可扩展性为核心的工具箱，实现了12种对抗攻击算法，集成了7个基准数据集，支持通过Hugging Face访问各种开源LLM，并包含计算资源跟踪、确定性结果和分布评估等高级功能。

Result: 提供了一个完整的LLM安全研究框架，集成了攻击算法、基准数据集和评估方法，并通过JudgeZoo包提供独立的评判功能。

Conclusion: AdversariaLLM为LLM安全研究建立了透明、可比较和可重现的坚实基础，解决了当前研究生态系统的碎片化问题。
...
### [229] [Shared Spatial Memory Through Predictive Coding](https://arxiv.org/abs/2511.04235)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统中部分可观测性和有限带宽常导致协调失败，需要开发能够有效共享和重建一致空间记忆的方法。

Method: 使用信息瓶颈目标，让智能体学习何时、与谁、交流什么；基于网格细胞状度量的内部空间编码；分层强化学习策略主动探索以减少联合不确定性。
...
### [230] [RLoop: An Self-Improving Framework for Reinforcement Learning with Iterative Policy Initialization](https://arxiv.org/abs/2511.04285)
*Zeng Zhiyuan,Jiashuo Liu,Zhangyue Yin,Ge Zhang,Wenhao Huang,Xipeng Qiu*

Main category: cs.AI

TL;DR: RLoop框架通过迭代策略初始化解决RL训练中的过拟合问题，将训练过程转化为探索-利用的良性循环，显著提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习训练中的过拟合问题，即模型在训练中获得高奖励但失去泛化能力，这主要由策略过度专业化和灾难性遗忘引起。

Method: 提出RLoop框架：使用RL探索解空间→筛选成功轨迹创建专家数据集→通过拒绝采样微调(RFT)精炼初始策略→为下一次迭代创建更好的起点。

Result: RLoop有效缓解遗忘并显著改善泛化，相比标准RL平均准确率提升9%，pass@32提升超过15%。

Conclusion: 迭代策略初始化框架成功将瞬态策略变化转化为稳健性能增益，解决了RL训练中的关键挑战。
...
### [231] [GUI-360: A Comprehensive Dataset and Benchmark for Computer-Using Agents](https://arxiv.org/abs/2511.04307)
...
TL;DR: GUI-360°是一个大规模数据集和基准测试套件，用于推进计算机使用代理(CUAs)的研究，解决了真实任务稀缺、多模态轨迹自动收集和统一评估基准缺失的问题。


<details>
  <summary>Details</summary>
Motivation: 解决计算机使用代理研究中的三个关键差距：真实世界CUA任务稀缺、多模态轨迹自动收集标注流程缺乏、以及GUI定位、屏幕解析和动作预测联合评估统一基准的缺失。

Method: 采用LLM增强的自动化流程，包括查询来源、环境模板构建、任务实例化、批量执行和LLM驱动的质量过滤，收集了超过120万执行动作步骤的数据。

Result: 基准测试显示现有视觉-语言模型在GUI定位和动作预测方面存在显著不足，监督微调和强化学习虽有改善但未达到人类水平可靠性。
...
### [232] [Probing the Probes: Methods and Metrics for Concept Alignment](https://arxiv.org/abs/2511.04312)
*Jacob Lysnæs-Larsen,Marte Eggen,Inga Strümke*

Main category: cs.AI

TL;DR: CAV探测器的分类准确率不能可靠反映概念对齐度，探测器更可能捕捉虚假相关性而非目标概念。作者提出基于空间线性归因的新概念定位方法，并引入三类概念对齐评估指标。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI中，CAV探测器的高准确率被错误地等同于概念对齐度，但实际上探测器容易捕捉虚假相关性而非真正代表目标概念。

Method: 提出基于空间线性归因的概念定位方法，与现有特征可视化技术对比；引入三类概念对齐评估指标：硬准确率、分割分数和增强鲁棒性。

Result: 实验表明，故意构造的错位探测器利用虚假相关性也能达到接近标准探测器的准确率；具有平移不变性和空间对齐的探测器能提高概念对齐度。

Conclusion: 需要基于对齐度的评估指标而非探测器准确率，且探测器设计应考虑模型架构和目标概念特性。
...
### [233] [RxSafeBench: Identifying Medication Safety Issues of Large Language Models in Simulated Consultation](https://arxiv.org/abs/2511.04328)
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏真实世界数据集和隐私限制，现有LLMs在药物安全性方面的研究不足，特别是在真实临床咨询环境中的评估尚未充分探索。

Method: 构建包含药物安全数据库RxRisk DB，采用两阶段过滤策略确保临床真实性和专业质量，生成2,443个高质量咨询场景，通过结构化选择题评估LLMs推荐安全药物的能力。

Result: 当前LLMs在整合禁忌症和相互作用知识方面存在困难，特别是当风险是隐含而非明确时表现更差。

Conclusion: RxSafeBench提供了首个全面评估LLMs药物安全性的基准，为改进AI驱动的临床决策支持系统的可靠性提供了见解。
...
### [234] [Monitor-Generate-Verify (MGV):Formalising Metacognitive Theory for Language Model Reasoning](https://arxiv.org/abs/2511.04341)
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时推理架构缺乏监控过程，导致模型容易陷入'前缀主导陷阱'——过早承诺次优推理路径且难以恢复，造成约20%的准确率损失。

Method: 将Flavell以及Nelson和Narens的元认知理论形式化为计算规范，在Generate-Verify范式前添加显式监控模块，捕捉生成前的元认知体验（从难度评估到置信度判断），并通过验证反馈优化后续监控。
...
### [235] [Post-Training LLMs as Better Decision-Making Agents: A Regret-Minimization Approach](https://arxiv.org/abs/2511.04393)
*Chanwoo Park,Ziyang Chen,Asuman Ozdaglar,Kaiqing Zhang*

Main category: cs.AI

TL;DR: 提出迭代后悔最小化微调方法，通过反复蒸馏低后悔决策轨迹来增强大语言模型的决策能力


<details>
  <summary>Details</summary>
Motivation: 大语言模型在交互动态环境中作为决策代理时表现不佳，难以实现低后悔或有效的探索-利用权衡

Method: 迭代后悔最小化微调：模型多次展开决策轨迹，选择k个最低后悔轨迹，并在这些轨迹上微调自身

Result: 该方法提升了多种模型的决策性能，包括Transformer、开源LLM和GPT-4o mini等闭源模型，能泛化到不同任务设置

Conclusion: Iterative RMFT为增强LLM决策能力提供了一个原则性的通用后训练框架
...
### [236] [The Peril of Preference: Why GRPO fails on Ordinal Rewards](https://arxiv.org/abs/2511.04439)
*Anisha Garg,Ganesh Venkatesh*

Main category: cs.AI

TL;DR: CoRPO是一种改进的强化学习方法，解决了GRPO在序数奖励下会强化错误行为的问题，通过自适应基线确保失败轨迹不被正向强化，并在达到质量阈值后转向相对偏好模式。


<details>
  <summary>Details</summary>
Motivation: GRPO的简单性使其在序数奖励下存在问题，因为其组平均基线会给失败轨迹分配正优势值，从而强化错误行为。需要一种新方法来处理更丰富的非二元反馈。

Method: CoRPO使用自适应基线强制执行最低质量阈值，确保失败解决方案永远不会被正向强化。当策略持续满足阈值后，基线自动过渡到相对偏好模式，推动模型寻找最优解而非仅可接受的解。

Result: 在代码验证任务上的实证验证表明，CoRPO表现出更稳定的收敛性和更好的域外泛化能力。

Conclusion: 这项工作是在LLM通过强化学习学习真正新能力的研究计划中的关键一步，使LLM能够从丰富的多维反馈中学习，从二元奖励进展到序数奖励，并进一步向更密集的每步监督发展。
...
### [237] [Beyond Shortest Path: Agentic Vehicular Routing with Semantic Context](https://arxiv.org/abs/2511.04464)
*Carnot Braun,Rafael O. Jarczewski,Gabriel U. Talasso,Leandro A. Villas,Allan M. de Souza*

Main category: cs.AI

TL;DR: PAVe系统结合传统路径规划算法与LLM语义推理，通过多目标Dijkstra算法生成候选路径，再由LLM代理根据用户任务、偏好和规避规则进行上下文评估，实现个性化车辆路由。
...
Method: 使用多目标（时间、CO2）Dijkstra算法生成候选路径，LLM代理基于预处理的地理空间POI缓存评估路径是否符合用户任务、偏好和规避规则。

Result: 在现实城市场景基准测试中，PAVe成功将复杂用户意图转化为适当路径修改，使用本地模型时初始路径选择准确率超过88%。

Conclusion: 将经典路由算法与基于LLM的语义推理层相结合，是创建个性化、自适应和可扩展城市出行优化解决方案的稳健有效方法。
...
### [238] [Promoting Sustainable Web Agents: Benchmarking and Estimating Energy Consumption through Empirical and Theoretical Analysis](https://arxiv.org/abs/2511.04481)
*Lars Krupp,Daniel Geißler,Vishal Banwari,Paul Lukowicz,Jakob Karolus*

Main category: cs.AI

TL;DR: 该论文首次探索了网络代理的能源消耗和碳排放问题，通过理论和实证方法揭示了不同网络代理设计理念对能耗的显著影响，并呼吁在评估标准中加入能耗指标。


<details>
  <summary>Details</summary>
Motivation: 虽然网络代理研究蓬勃发展，但其引发的可持续性问题仍未被充分探索。研究者旨在揭示网络代理的能源和碳排放成本，强调这一问题的紧迫性。

Method: 采用理论估计和实证基准测试相结合的方法，从两个角度分析网络代理的能耗：通过估算进行理论分析，以及通过基准测试进行实证验证。

Result: 研究发现不同网络代理设计理念会严重影响能耗，且更高能耗并不一定带来更好结果。同时指出某些网络代理缺乏模型参数和流程透明度，限制了能耗估算的准确性。

Conclusion: 研究呼吁改变网络代理的评估方式，建议在基准测试中加入专门的能耗衡量指标，以促进更可持续的网络代理开发。
...
### [239] [Optimizing Sensor Placement in Urban Storm Sewers: A Data-Driven Sparse Sensing Approach](https://arxiv.org/abs/2511.04556)
*Zihang Ding,Kun Zhang*

Main category: cs.AI

TL;DR: 提出了一种数据驱动的稀疏传感框架，结合EPA-SWMM模型，通过优化传感器布置来重建城市雨水系统中的峰值流量，在资源受限条件下实现高效洪水监测。


<details>
  <summary>Details</summary>
Motivation: 城市地表洪水因强降雨超过排水系统容量而日益频发，但高时空分辨率洪水预测和监测面临时间、预算和技术限制，如何在资源受限条件下监测城市排水网络和预测流量状况是主要挑战。

Method: 使用EPA-SWMM模型生成训练数据集，应用数据驱动稀疏传感框架，利用奇异值分解进行降维和QR分解进行传感器分配，基于模拟训练数据集识别最优监测节点。

Result: 在77个节点中仅需3个优化布置的传感器即可实现满意的峰值流量重建性能，纳什-萨克利夫效率值达到0.92-0.95，模型对测量不确定性具有良好鲁棒性。
...
### [240] [Jr. AI Scientist and Its Risk Report: Autonomous Scientific Exploration from a Baseline Paper](https://arxiv.org/abs/2511.04583)
*Atsuyuki Miyai,Mashiro Toyooka,Takashi Otonari,Zaiying Zhao,Kiyoharu Aizawa*

Main category: cs.AI

TL;DR: 开发了Jr. AI Scientist系统，模拟新手研究者的工作流程，能够分析论文局限性、提出假设、实验验证并撰写论文，在评估中表现优于现有全自动系统，但仍有重要局限性。
...
Method: 开发Jr. AI Scientist系统，模拟新手研究者的核心研究流程：分析基线论文局限性、制定改进假设、通过严格实验验证、撰写结果论文，利用现代编码代理处理复杂多文件实现。

Result: 评估显示Jr. AI Scientist生成的论文获得比现有全自动系统更高的评审分数，但作者评估和Agents4Science评审都发现了重要局限性。

Conclusion: 当前AI科学家系统存在直接应用的风险和关键挑战，需要进一步研究，报告了开发过程中发现的各种风险，希望加深对AI科学家发展现状和风险的理解。
...
### [241] [Are We Asking the Right Questions? On Ambiguity in Natural Language Queries for Tabular Data Analysis](https://arxiv.org/abs/2511.04584)
*Daniel Gomm,Cornelius Wolff,Madelon Hulsebos*

Main category: cs.AI

TL;DR: 该论文将自然语言查询中的歧义重新定义为合作交互的特征，提出了区分可解析的合作查询与不可解析的非合作查询的框架，并分析了15个数据集中的查询类型混合问题。
...
### [242] [Question the Questions: Auditing Representation in Online Deliberative Processes](https://arxiv.org/abs/2511.04588)
*Soham De,Lodewijk Gelauff,Ashish Goel,Smitha Milli,Ariel Procaccia,Alice Siu*

Main category: cs.AI

TL;DR: 提出了一个基于合理代表制(JR)的审计框架来评估专家问答环节中问题选择的代表性，并开发了高效的审计算法。通过历史审议数据比较了主持人选择、整数线性规划选择和LLM生成问题三种方法的代表性。
...
Method: 基于合理代表制(JR)概念建立审计框架，开发O(mn log n)时间复杂度的审计算法，比较主持人选择、整数线性规划选择和LLM生成问题三种方法的代表性。
...
### [243] [DR. WELL: Dynamic Reasoning and Learning with Symbolic World Model for Embodied LLM-Based Multi-Agent Collaboration](https://arxiv.org/abs/2511.04646)
...
<details>
  <summary>Details</summary>
Motivation: 协作多智能体规划需要在部分信息和有限通信的情况下做出联合决策。轨迹级别的协调经常失败，因为时间或运动中的微小偏差会级联成冲突。符号规划通过提高抽象级别和提供最小动作词汇来缓解这一挑战，实现同步和集体进展。

Method: 采用两阶段协商协议：1）智能体提出候选角色并进行推理；2）在共识和环境约束下承诺联合分配。承诺后，每个智能体独立生成符号计划并执行。计划通过共享世界模型在执行结果中落地，该模型编码当前状态并随智能体行动而更新。

Result: 在协作推块任务上的实验表明，智能体能够在不同回合间适应，动态世界模型捕获可重用模式，提高了任务完成率和效率。通过协商和自我优化，动态世界模型以时间开销为代价，实现了不断演进、更高效的协作策略。

Conclusion: 通过推理符号计划而非原始轨迹，DR.WELL避免了脆弱的步骤级对齐，实现了可重用、可同步和可解释的更高级操作。符号规划方法使多智能体系统能够更稳健地协作。
...
### [244] [VeriCoT: Neuro-symbolic Chain-of-Thought Validation via Logical Consistency Checks](https://arxiv.org/abs/2511.04662)
*Yu Feng,Nathaniel Weir,Kaj Bostrom,Sam Bayless,Darion Cassel,Sapana Chaudhary,Benjamin Kiesl-Reiter,Huzefa Rangwala*

Main category: cs.AI

TL;DR: VeriCoT是一个神经符号方法，通过将思维链推理步骤形式化为一阶逻辑来自动验证LLM推理的有效性，识别推理中的逻辑缺陷，并利用验证信号提升推理质量。


<details>
  <summary>Details</summary>
Motivation: LLMs虽然能进行多步推理，但无法可靠验证自身逻辑，即使得出正确答案，底层推理可能存在缺陷，这在高风险场景中影响可信度。

Method: 提取思维链推理步骤，形式化为一阶逻辑，识别前提基础（源上下文、常识知识、先前推理步骤），使用自动求解器验证逻辑有效性。

Result: 在ProofWriter、LegalBench和BioASQ数据集上，VeriCoT能有效识别有缺陷的推理，并作为最终答案正确性的强预测指标。

Conclusion: VeriCoT不仅能识别推理缺陷，还能通过推理时自反思、监督微调和偏好微调等方式进一步提升推理有效性和准确性。
...
### [245] [Evolutionary Optimization Trumps Adam Optimization on Embedding Space Exploration](https://arxiv.org/abs/2511.03913)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型难以控制和优化特定目标，需要昂贵的重新训练。嵌入空间探索，特别是进化算法，为优化图像生成提供了有前景的方法。
...
Result: 在Parti Prompts数据集子集上的实验表明，sep-CMA-ES在美学和对齐指标上始终优于Adam。

Conclusion: 进化方法为扩散模型提供了高效的无梯度优化，无需微调即可增强可控性，凸显了进化方法在深度生成模型嵌入空间探索中的潜力。
...
### [246] [A Reinforced Evolution-Based Approach to Multi-Resource Load Balancing](https://arxiv.org/abs/2511.04183)
*Leszek Sliwko*

Main category: cs.NE

TL;DR: 提出了一种基于强化遗传方法来解决d-资源系统优化问题，通过引入迁移算子等改进来克服经典进化算法在严格可行性函数下的低效问题。


<details>
  <summary>Details</summary>
Motivation: 经典进化算法在处理具有严格可行性函数的d-资源系统优化问题时效率低下，需要改进标准遗传算法来应对这一挑战。

Method: 采用强化遗传方法，引入了迁移算子（模拟生物随机遗传漂变）等多种改进和适应标准遗传算法的操作。

Result: 开发了一种改进的遗传算法策略，能够更有效地处理具有严格可行性约束的优化问题。

Conclusion: 通过引入迁移算子等生物启发的改进，强化遗传方法能够有效解决经典进化算法在严格可行性约束下的性能问题。
...
### [247] [Neural Computation Without Slots: Steps Towards Biologically Plausible Memory and Attention in Natural and Artificial Intelligence](https://arxiv.org/abs/2511.04593)
*Shaunak Bhandarkar,James L. McClelland*

Main category: cs.NE

TL;DR: 本文提出扩展现代Hopfield网络(MHN)以增强其生物合理性，通过引入K-winner MHN使用神经元集合而非单个神经元存储记忆，并在持续学习环境中展示更好的旧记忆保留能力。同时借鉴语言模型中槽位存储的优势，扩展MHN以支持类似功能。


<details>
  <summary>Details</summary>
Motivation: 生物大脑可能缺乏数字计算机中的专用存储槽位，但需要实现类似功能。本文旨在探索生物合理机制如何支持AI系统中实现人类类似能力的关键计算功能。

Method: 1. 提出K-winner MHN，将存储从单个神经元扩展到稀疏神经元集合；2. 在持续学习环境中测试记忆保留能力；3. 借鉴语言模型槽位存储机制，扩展MHN以支持长序列输入存储和误差反向传播。

Result: 在持续学习环境中，基于集合的MHN比标准(单神经元)MHN表现出更好的旧记忆保留能力，通过分级敏感度指标d'衡量。

Conclusion: 这些建模方法为理解生物合理机制如何支持AI系统实现人类类似能力的关键计算功能迈出了重要步骤。
