...
Conclusion: Soft-argmax并非实现高性能面部关键点检测的唯一方法，基于结构化预测的替代方案能够提供更快的收敛速度和竞争性的精度表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络(CNNs)在处理复杂数据结构时有限，本研究尝试通过图表示和图计算方法来提升图像分类的效果。

Method: 将图像表示为图结构，像素或区域作为顶点，然后使用Delaunay三角剖分来优化图表示，最后结合GCNs进行分类。

Result: 在各种标准数据集上都取得了显著的预处理效率和分类准确性提升，超越了最先进的方法，特别是在复杂场景和细粒度分类任务中。

Conclusion: 结合GCNs和Voronoi图的方法为图像分类提供了新的视角，扩展了图基学习范式在计算机视觉和非结构化数据分析中的应用潜力。
...
Method: 在transformer检测器基础上增加轻量级姿态头、边界框条件平移模块和6D感知匈牙利匹配代价，端到端训练。

Result: 在REAL275数据集上达到79.6% IoU50和54.1% 10°10cm指标，超越所有RGB-only方法，接近RGB-D系统性能。

Conclusion: YOPO证明了单阶段RGB-only方法可以实现高性能的类别级9自由度姿态估计，简化了传统复杂流程。
...
<details>
  <summary>Details</summary>
Motivation: 现代人脸识别系统容易受到物理呈现攻击和数字伪造攻击，传统方法使用不同模型分别处理这两种攻击，增加了系统复杂性和推理延迟，且无法应对组合攻击向量。

Method: 采用配对采样对比学习框架，利用自动匹配的真实和攻击自拍对来学习模态无关的活体检测线索，实现统一的训练方法。

Result: 在第六届人脸反欺骗挑战赛统一物理-数字攻击检测基准上，平均分类错误率(ACER)达到2.10%，优于现有解决方案，模型轻量(4.46 GFLOPs)，训练时间不到一小时。

Conclusion: 该方法提供了一种实用且高效的统一解决方案，能够同时处理物理和数字攻击，适合实际部署应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型的对抗攻击往往产生低质量图像且计算资源需求大，扩散模型虽然能生成高质量图像但需要数百个采样步骤，因此需要一种高效的对抗图像生成方法。

Method: 通过在混合步骤区间注入扰动实现有效攻击，无需处理所有时间步；采用选择性RGB通道策略，对红色通道应用注意力图，对绿色和蓝色通道使用GradCAM引导的扰动。

Result: 在ImageNet数据集上，对VGGNet作为源模型，TAIGen对ResNet成功率为70.6%，对MNASNet为80.8%，对ShuffleNet为97.8%；PSNR超过30dB，保持视觉质量；生成速度比现有扩散攻击快10倍。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐蔽视觉感知方法主要局限于掩码域的可逆策略，未能充分利用RGB域的潜力，需要一种能同时在两个域工作并有效处理不确定性的方法。

Method: 将CVP任务建模为数学优化问题，展开为多阶段深度网络。包含三个模块：CORE模块在掩码域识别核心对象区域，CARE模块在RGB域增强前景-背景分离，FINE模块使用Bernoulli扩散模型细化不确定区域。

Result: 提出的方法能够有效减少假阳性和假阴性，在真实世界退化条件下保持鲁棒性，计算效率高。

Conclusion: RUN++通过可逆展开网络和生成式细化的协同作用，为隐蔽视觉感知提供了新的解决方案，建立了更鲁棒的CVP系统框架。
...
Method: 提出GasTwinFormer混合视觉变换器，采用新颖的Mix Twin编码器交替使用空间缩减的全局注意力和局部分组注意力机制，结合轻量级LR-ASPP解码器进行多尺度特征聚合，在统一框架中同时处理甲烷分割和饮食分类。

Result: 在11,694个标注帧的数据集上，达到74.47% mIoU和83.63% mF1的分割性能，仅使用3.348M参数和3.428G FLOPs，推理速度114.9 FPS。饮食分类准确率达到100%，验证了饮食-排放相关性的有效性。

Conclusion: GasTwinFormer通过创新的架构设计和高效的性能表现，为实时牲畜排放监测提供了实用解决方案，广泛的消融研究验证了各组件有效性，在气候监测领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有的整流流模型基于数据和噪声分布之间的线性轨迹，这种线性性强制零曲率，可能迫使图像生成过程通过数据流形的低概率区域。关键问题是轨迹曲率如何与生成图像和对应标题之间的语义对齐（即指令遵循）相关。
...
Result: 仅在合成失真上训练的HiRQA能有效泛化到真实退化场景（如镜头光晕、雾霾、运动模糊、低光条件）。轻量版HiRQA-S推理时间仅3.5ms/图像，在多个基准测试中达到SOTA性能。

Conclusion: HiRQA通过自监督的层次化排序和对齐学习，实现了优异的无参考图像质量评估性能，具有强大的泛化能力和实时部署潜力，为NR-IQA领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多视角3D重建在紧急响应、战术场景等关键应用中需要迅速情况感知。但边缘环境的动态性和运营逆境导致相机操作受到时空相关中断，影响重建质量。需要一种可靠的资源管理策略来应对这些挑战。

Method: 提出一种受投资组理论启发的边缘资源管理策略。使用遗传算法解决投资组理论优化问题，该算法在实际系统设置中能够快速收敛。通过相机选择策略来对抗时空中断。

Result: 使用公开和自定制的3D数据集进行验证，证明所提方法能够在时空中断条件下保证3D重建质量，效果明显优于传统基准策略。

Conclusion: 该研究提出的投资组理论启发的边缘资源管理方法，为动态边缘环境中的多视角3D重建应用提供了可靠的性能保障，能够有效应对时空相关系统中断带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在糖尿病视网膜病变检测中表现出色，但其黑盒特性缺乏透明度和可解释性，阻碍了临床采用。需要一种既能提供准确诊断又能生成可理解解释的方法。

Method: 提出XDR-LVLM框架，整合专用医学视觉编码器和LVLM核心，采用多任务提示工程和多阶段微调技术，深入理解眼底图像病理特征并生成包含DR严重程度分级、关键病理概念识别和诊断解释的综合报告。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于网格的城市模型已经很普及，但缺乏现实的素材限制了它们在虚拟城市导航和自动驾驶中的应用。需要一种能够生成高质量、风格一致的室外场景的方法。

Method: 提出三阶段流水线：1）使用级联Outpainting ControlNets生成几何一致的稀疏视图；2）通过AGInpaint组件传播更密集的中间视图；3）使用GCAlign模块全局消除视觉不一致性。同时通过在网格表面初始化Gaussian球来重建3D高斯拖尾场景。

Result: 该方法在几何对齐和生成质量方面都超过了现有方法。生成后的场景可以通过重新打光和风格转换技术渲染成不同风格。

Conclusion: MeSS方法通过结合图像渗透模型和网格基础的几何先验，成功地生成了高质量、视觉一致的3D场景，为虚拟城市导航和自动驾驶应用提供了有效解决方案。
...
Method: 1) 创建包含697张手术伤口图像的SurgWound数据集，由3位专业外科医生标注8个临床属性；2) 建立包含视觉问答和报告生成任务的基准；3) 提出三阶段框架WoundQwen：第一阶段用5个MLLM预测伤口特征，第二阶段用2个MLLM进行感染风险评估，第三阶段整合结果生成综合报告。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等安全关键应用中，现有方法使用规则智能体训练强化学习代理，但需要确保规则智能体被正确建模。当前使用多种行为建模策略和IDM模型，但缺乏对对抗行为的系统研究

Method: 开发基于学习的方法来推导规则智能体的对抗行为，旨在制造导致强化学习代理失败的场景

Result: 评估对抗智能体对抗所有规则智能体，结果显示累积奖励显著下降，证明该方法能有效制造故障场景

Conclusion: 该方法能够有效识别和利用规则智能体的弱点，为安全关键系统的鲁棒性测试提供了重要工具
...
<details>
  <summary>Details</summary>
Motivation: 现有的肾脏病理数据集和自动化方法仅提供二元（语义）掩码，限制了后续分析的精确度，需要更精确的实例级分割来量化肾脏病理功能单元的形态特征。
...
Conclusion: DyMorph-B2I管道为肾脏病理学提供了有效的二元到实例分割解决方案，公开可用且性能优越。
...
<details>
  <summary>Details</summary>
Motivation: 现有ADAS系统多依赖多种传感器，但仅使用行车记录仪视频提供更经济易部署的解决方案。然而从单一视频输入预测事故更具挑战性，需要更好的时空特征提取方法。

Method: 提出STAGNet模型，整合改进的时空特征并通过循环网络进行聚合，相比现有的图神经网络方法有更好的特征表达能力。

Result: 在三个公开数据集上的实验表明，STAGNet在平均精度和平均碰撞时间两个指标上均优于先前方法，无论是在同一数据集交叉验证还是跨数据集测试都表现优异。

Conclusion: STAGNet模型通过改进时空特征提取和聚合机制，显著提升了基于行车记录仪视频的事故预测性能，为低成本、易部署的ADAS系统提供了有效解决方案。
...
Conclusion: 该方法能有效避免类间冲突、减少原型漂移，并提供可靠的语义信息保证
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机视觉语言导航面临轨迹长、机动性复杂的挑战，需要人类干预或过于详细的指令。利用无人机高机动性提供多粒度视角，同时保持可管理的运动空间。

Method: 提出双高度无人机协作框架：高空无人机负责环境推理，低空无人机负责精确导航。使用多模态大语言模型(Pilot-LLM)进行目标推理，轻量级多阶段策略进行导航和目标定位。

Result: 构建了包含13,838条协作轨迹的HaL-13k数据集，包含未见地图和未见对象验证集，系统评估模型在新环境和陌生目标上的泛化能力。

Conclusion: 双无人机协作方法通过高低空视角互补，仅交换最小坐标信息确保效率，为解决复杂无人机导航问题提供了有效解决方案。
...
Method: 提出跳过步采样机制，在迭代生成过程中绕过多个中间去噪步骤，与标准扩散推理的逐步细化形成对比。该方法与标准扩散模型具有相同的训练目标

Result: 在OpenAI ADM、Stable Diffusion和Open Sora等流行预训练扩散模型上的大量实验表明，该方法能够以显著减少的采样步骤实现高质量生成

Conclusion: 通过马尔可夫方式的跳过步采样加速是预训练扩散模型的内在属性，该方法可与DDIM结合实现增强生成，为扩散模型高效采样提供了新思路
...
Method: 提出三创新：1)多功能编码框架统一不同需求模式；2)基于上下文学习的交互式编码代理；3)首个专用评估基准IIC-bench。

Result: Comp-X能高效理解编码请求，具备出色的文本交互能力，在单一框架下保持可比压缩性能。
...
Method: 基于预训练视觉语言模型构建，加入轻量级可训练MLP层，融合正常和异常病理知识，通过图像-文本关联提供可解释性。
...
Method: 重构Transformer解码器：交换交叉注意力和自注意力层的顺序，保留冗余车道预测；实例化多个并行交叉注意力块，增强车道检测多样性。
...
Method: 基于CLIP模型开发DesignCLIP框架，采用类别感知分类和对比学习，利用生成的详细图像描述和多视图图像学习

Result: DesignCLIP在所有专利领域任务中持续优于基线和最先进模型，验证了多模态方法在专利分析中的有效性
...
<details>
  <summary>Details</summary>
Motivation: 当前超声视频CHD检测方法存在三个主要问题：忽略时序信息、仅限于二分类、缺乏预测校准。这些限制影响了临床应用的可靠性和准确性。

Method: TPA方法使用基础图像-文本模型提取视频帧特征，通过可训练时序提取器捕捉心脏运动，采用边缘铰链对比损失对齐视频表示与类别特定文本提示。引入CVAESM模块学习潜在风格向量来调制嵌入并量化分类不确定性。

Result: 在CHD检测上获得85.40%的宏F1分数，预期校准误差降低5.38%，自适应ECE降低6.8%。在EchoNet-Dynamic三分类任务上，宏F1提升4.73%（从53.89%到58.62%）。

Conclusion: TPA框架通过整合时序建模、提示感知对比学习和不确定性量化，显著提升了胎儿先天性心脏病超声视频分类的准确性和临床可靠性，为自动化CHD检测提供了有效的解决方案。
...
Method: 构建BasketLiDAR多模态数据集（LiDAR点云+多视角相机），开发新型MOT算法：包括仅使用LiDAR的实时追踪管道和融合LiDAR与相机数据的多模态追踪管道。

Result: 方法实现了实时操作（传统相机方法难以达到），在遮挡条件下仍保持优异的追踪性能，包含4,445帧和3,105个球员ID的同步数据。

Conclusion: LiDAR的高精度3D空间信息结合相机数据，能够有效解决篮球运动中的实时3D追踪难题，为体育分析提供了新的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有COD方法需要大量训练和计算资源，基础模型如SAM需要高质量提示但在COD任务上表现不佳，手动生成提示成本高效率低

Method: 将COD解耦为两个阶段：1)RAG阶段通过无监督聚类构建检索数据库生成粗粒度掩码提示；2)SEG阶段使用SAM2进行精细化分割

Result: 在基准COD数据集上达到或超越SOTA方法性能，所有实验在个人笔记本上完成，证明了计算效率和实用性
...
Method: 采用两阶段过程：选择性提示嵌入调整（SPEA）和抗干扰噪声引导（ARNG），作为即插即用模块无缝集成到现有T2V扩散模型中。
...
Result: GRU模型在预测行人过马路意图方面表现优于LSTM，而1D CNN模型在预测速度方面最快
...
Result: 发现视频信息提取主要在早期层完成，中间层存在关键异常层，时空建模更依赖语言引导检索而非视频token间的自注意力机制。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在图像分析中取得显著进展，但大型全训练模型的计算需求仍然是一个问题。传统微调方法需要更新所有参数，可能导致过拟合和更高的计算成本。

Method: BioTune采用进化算法来识别需要微调的聚焦层集合，旨在优化模型在目标任务上的性能，通过选择性微调层来减少可训练参数数量。

Result: 在九个不同领域的图像分类数据集上的评估表明，BioTune相比AutoRGN和LoRA等现有微调方法，实现了竞争性或改进的准确性和效率。

Conclusion: 通过将微调过程集中在相关层的子集上，BioTune减少了可训练参数数量，可能降低计算成本，并在不同数据特征和分布上促进更高效的迁移学习。
...
Method: 提出图像条件高斯溅射量化器(ICGS-Quantizer)，通过联合利用高斯间和属性间相关性，使用跨所有训练场景的共享码本，编码、量化和解码过程联合训练，支持基于图像的场景条件解码

Result: ICGS-Quantizer在压缩效率和场景变化适应性方面持续优于最先进方法，将3DGS存储需求降至千字节范围同时保持视觉保真度

Conclusion: 该方法显著提升了3D高斯溅射的压缩效率，解决了长期归档场景的存储和更新问题，为实际部署提供了可行的解决方案
...
Conclusion: DriveSplat通过改进的动态-静态解耦策略和几何先验监督，有效解决了驾驶场景中快速移动物体和大规模静态背景的3D重建挑战，提供了更准确和鲁棒的几何表示。
...
Method: 采用因果链建模视角分析RPM任务的完整因果链，设计了基线模型DIO的网络架构。针对DIO优化目标的问题，逐步提出了三种改进方法来解决互信息下界紧致性和因果关系捕获的问题。

Result: 实验发现基线模型DIO的优化目标（最大化上下文与正确答案之间互信息的变分下界）无法让模型真正获得预定义的人类推理逻辑。

Conclusion: 论文识别了现有方法在抽象推理任务中的局限性，并提出了系统的改进方向，为增强机器智能的抽象推理能力提供了新的研究路径。
...
Method: 分析MAE如何学习空间相关性，推导线性MAE学习的特征，证明掩码比例和补丁大小对特征选择的影响，并将分析扩展到非线性MAE以展示其超越二阶统计量的空间相关性适应能力。

Result: 研究发现MAE表示能够适应数据集中的空间相关性，掩码比例和补丁大小是选择捕获不同范围空间相关性特征的关键参数。

Conclusion: 该研究提供了关于MAE如何学习空间相关性的理论分析，并为实践中选择MAE超参数提供了有价值的见解，有助于优化MAE在新数据集上的性能。
...
Method: 设计双向传播策略，包含局部时间运动融合(LTMF)模块建模局部时空依赖，全局时间运动融合(GTMF)模块聚合全局传播特征，并使用检测损失和时空融合(STF)损失联合优化整个视频片段

Result: 大量实验表明BIRD方法不仅达到最先进性能，还显示出快速推理速度

Conclusion: 提出的双向时间信息传播方法有效解决了传统滑动窗口方法的局限性，在红外小目标检测任务中取得了优异效果
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的低分辨率图像给人脸检测带来挑战，需要系统研究输入分辨率对不同检测模型性能的影响，为实际应用提供模型选择指导。

Method: 使用WIDER FACE数据集，在多种图像分辨率（160x160、320x320、640x640）下评估YOLOv11、YOLOv12和MTCNN三种模型，采用精确率、召回率、mAP50、mAP50-95和推理时间等指标。

Result: YOLOv11在高分辨率下检测精度最佳，YOLOv12召回率稍好，MTCNN虽然在关键点定位方面有竞争力，但实时推理速度较慢。

Conclusion: 研究结果为根据不同操作约束选择分辨率感知的人脸检测模型提供了实用指导，YOLOv11适合高精度要求的场景，YOLOv12在召回率敏感的应用中表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 传统汽车损坏检测方法劳动密集、手动化且容易漏检微小表面缺陷。机器学习提供了更快速、精确检测的创新解决方案。

Method: 使用YOLOv8物体识别框架，构建了包含不同光照条件、角度和纹理的汽车表面注释图片数据集。训练YOLOv8m模型及其自定义变体YOLOv8m-t4和YOLOv8m-t42，采用实时数据增废技术提高模型稳健性。

Result: YOLOv8m-t42模型表现最优：准确率0.86、召回率0.84、F1分数0.85，mAP@0.5为0.60，PR曲线面积0.88。超越了YOLOv8m-t4模型（准确率0.81、召回率0.79、F1分数0.80）。

Conclusion: 该方法在微观凹陷检测上表现出艰强的检测准确性和低推理延迟，适合实时应用如自动保险评估和汽车检测，YOLOv8m-t42模型是实际应用的最佳选择。
...
Method: 提出MATR模型，使用基于Transformer的双阶段序列对齐来捕获语义上下文和时序细节，并通过自监督预训练技术进行任务特定初始化。
...
Method: 提出密集立体模块替代SfM进行相机位姿估计和全局密集点云重建；设计一致性视角插值模块生成额外监督信号；引入多尺度拉普拉斯一致性正则化和自适应空间感知多尺度几何正则化

Result: 在极稀疏视角条件下（仅使用2个训练视角）PSNR指标提升2.75dB，合成图像失真最小且保留丰富高频细节，视觉质量优于现有技术
...
<details>
  <summary>Details</summary>
Motivation: 解决现有轻量级模型在医学图像分割中因性能妥协和缺乏注意力机制而导致的全局上下文感知能力不足问题，同时处理相同卷积核下的通道冗余问题
...
TL;DR: MExECON是一个从稀疏多视角RGB图像进行3D穿衣人体重建的新方法，通过多视角联合优化提升几何和姿态估计精度，无需重新训练网络。


<details>
  <summary>Details</summary>
Motivation: 现有的单视角方法ECON在多视角场景下存在局限性，需要扩展其能力以利用多视角信息来改进几何重建和人体姿态估计的准确性。

Method: 提出联合多视角人体优化(JMBO)算法，在所有输入视角上联合拟合单个SMPL-X人体模型，确保多视角一致性；使用优化后的人体模型作为低频先验指导表面重建，通过法线图积分添加几何细节；整合前后视角的法线图捕捉精细表面细节。

Result: 实验结果表明，MExECON相比单视角基线方法在保真度上持续改进，与现代少样本3D重建方法相比具有竞争力。

Conclusion: MExECON成功扩展了单视角方法到多视角场景，通过多视角联合优化和法线图整合实现了高质量的3D穿衣人体重建，且无需网络重新训练。
...
TL;DR: D3FNet是一个用于高分辨率遥感图像中细粒度狭窄道路分割的深度学习网络，通过差分注意力机制和双流解码融合，在复杂场景下实现了优异的道路提取性能。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率遥感图像中狭窄道路提取的挑战，包括道路宽度有限、拓扑结构碎片化、频繁遮挡等问题，传统模型难以处理细粒度、遮挡和低对比度的道路段。
...
Method: 将TTA重构为高斯概率推理任务，使用逐渐更新的类别均值和共享协方差矩阵建模类别条件似然，实现闭式无训练推理。引入基于CLIP先验和历史知识库的轻量级正则化来校正潜在似然偏差。

Result: 在多样化基准测试中，该方法在各种分布偏移下实现了最先进的性能，具有卓越的可扩展性和鲁棒性。

Conclusion: ADAPT提供了一种无需源数据、无需梯度更新、无需完全访问目标数据的测试时自适应解决方案，支持在线和转导设置，为实际部署提供了高效可靠的TTA方法。
...
TL;DR: 提出两阶段训练策略解决INR中的频谱偏差问题，通过邻域感知软掩码优先关注高频细节，再过渡到全图像训练，有效提升重建质量
...
Method: 两阶段训练策略：第一阶段使用邻域感知软掩码为局部变化强烈的像素分配更高权重，优先学习高频细节；第二阶段进行全图像训练
...
TL;DR: 提出了一种新的线性时间凸松弛方法和承包商方法，用于加速分支定界算法，能够在0.5秒内解决100个点的3D点云配准问题，比现有SDP求解器快两个数量级。
...
Method: 采用线性时间凸松弛和承包商方法来加速分支定界算法，专注于旋转轴已知的情况。

Result: 能够在0.5秒内完成100个点的3D点云配准，达到可证明的全局最优，比STRIDE SDP求解器快100倍。

Conclusion: 该方法显著提高了点云配准的计算效率，虽然目前仅限于旋转轴已知的情况，但为全局最优配准提供了高效的解决方案。
...
TL;DR: 这项研究通过结合相机捕捉器和无人机火热成像进行多视角监测，利用深度学习模型自动识别野生动物和人类活动，分析其空间分布模式并识别人兽冲突潜在区域。


<details>
  <summary>Details</summary>
Motivation: 理解野生动物和人类活动的空间分布对评估人兽互动和制定有效保护规划至关重要。需要多视角监测方法来捕捉它们的空间模式，以识别活动区域重叠和评估人兽冲突程度。

Method: 在尼泊尔奇特温国家公园及周边区域，使用可见光和近红外相机捕捉器以及火热红外无人机截至2022年2-7月的图像。构建训练和测试数据集，建立深度学习模型进行自动识别。使用YOLOv11s模型处理相机捕捉图像，使用改进的Faster RCNN模型分析无人机火热图像。进行空间模式分析识别热点区和潜在冲突区域。

Result: YOLOv11s模型表现最佳，精度达96.2%，召回率92.3%，mAP50为96.7%，mAP50-95为81.3%。无人机火热成像为相机捕捉器检测提供了补充的空中视角。空间分析识别了野生动物和人类活动的明显热点区域，以及在国家公园和缓冲区内某些区域的重叠模式，表明存在潜在冲突。

Conclusion: 这项研究揭示了保护地景内的人兽冲突问题。整合多视角监测与自动物体检测能够提升野生动物监测和地景管理效能。
...
Method: 1）扩散时间潜在编码器增强边界敏感性和时间一致性；2）对象接地表示显式绑定查询实体与局部视觉证据；3）混合令牌方案提供显式时间戳建模

Result: 在Charades STA、NExT GQA和多个VideoQA基准测试中取得了最先进的结果

Conclusion: Grounded VideoDiT通过三项关键创新设计，为视频LLM提供了强大的接地能力，解决了时间感知粗糙的问题
...
TL;DR: 通过弱监督评分机制提升机载光询数据的树木实例分割效果，减少人工标注成本


<details>
  <summary>Details</summary>
Motivation: 机载光询数据树木分割面临数据变异性和标注数据缺乏挑战，需要更高效的弱监督方法

Method: 通过人工评分初始分割结果，训练评分模型，然后用评分模型反馈微调分割模型

Result: 实例识别准确率提升34%，显著减少非树木实例预测

Conclusion: 方法在密林区域表现良好，但在疏林区小树或复杂环境中仍面临挑战
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中难以获取目标模型信息，传统3D对抗攻击方法依赖模型参数或输出信息，因此需要研究无需目标模型信息的迁移攻击方法

Method: 基于不同DNN架构使用一致关键特征的观察，提出关键特征指导(CFG)方法，通过计算特征重要性来正则化对抗点云搜索，优先破坏可能被多种架构采用的关键特征，并在损失函数中约束最大偏差以确保不可感知性

Result: 在ModelNet40和ScanObjectNN基准数据集上的大量实验表明，CFG方法大幅优于现有最先进的攻击方法

Conclusion: CFG方法通过关键特征指导有效提高了3D点云对抗样本的迁移性，为黑盒攻击场景提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有在线高清地图构建方法依赖离线地图先验和多模态传感器，导致推理时计算开销大且依赖过时地图信息。需要开发更高效、低成本的视觉中心方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LoD1和LoD2建筑模型缺乏详细的立面元素，而LoD3模型虽然包含这些细节但需要手动建模，难以大规模应用。需要自动化方法来高效生成详细的LoD3建筑模型。

Method: 利用射线到模型先验分析获得冲突图(CMs)，开发语义冲突图生成器(SCMG)生成合成CMs进行语义分割，并通过置信度分数将纹理模型分割结果与CMs融合以提高分割性能。

Result: 实验结果表明CM2LoD3方法在建筑开口分割和重建方面有效，通过不确定性感知的纹理分割融合实现了61%的性能提升，显著提高了3D重建精度。

Conclusion: 该方法推动了自动化LoD3模型重建的进展，为可扩展和高效的三维城市建模铺平了道路，项目代码已开源。
...
TL;DR: 提出了MDPR框架，通过多维度动态提示路由来解决VLM在类别不平衡场景下的微调偏差问题，在多个长尾基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM微调方法在类别不平衡场景中存在偏差，且往往忽略了VLM预训练中固有的类别不平衡问题，导致下游任务中的偏差累积。

Method: 构建跨五个视觉-语义维度的综合知识库，通过动态路由机制对齐全局视觉类别、检索最优提示并平衡细粒度语义，通过logits融合获得稳定预测。
...
Conclusion: StreamMem为解决长视频理解中的内存和计算效率问题提供了一种有效的流式压缩解决方案，在保持性能的同时显著降低了资源需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成视频建模方法主要依赖RGB信号，在长序列中会导致对象结构和运动的累积错误，需要解决时间一致性和结构保持的问题。
...
TL;DR: 提出了分子赋能的All-in-SAM模型，通过结合分子信息来增强计算病理学中的细胞分割和分类性能，减少标注工作量并提高在资源有限环境中的可访问性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型（如SAM）在细粒度语义分割（如特定细胞亚型识别）方面存在挑战，需要解决标注工作量大和模型泛化能力不足的问题。

Method: 采用全栈方法：1）分子赋能的标注减少像素级标注需求；2）使用SAM适配器增强语义特异性；3）整合分子导向校正学习（MOCL）提高分割精度。

Result: 在内部和公共数据集上的实验表明，All-in-SAM模型显著提高了细胞分类性能，即使在标注质量不同的情况下也能保持良好效果。

Conclusion: 该方法不仅减轻了标注人员的工作负担，还将精准生物医学图像分析扩展到资源有限的环境，推动了医学诊断和病理图像分析的自动化进程。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频生成模型在运动捕捉、时间一致性和分辨率方面的限制，开发一个能够统一处理图像和视频生成的高性能基础模型。
...
Conclusion: 该方法实现了增强的形状表达能力、细粒度的身体属性定制以及与外部软组织特征无关的关键点拟合，超越了现有方法。
...
Method: 提出SceneGen框架，包含特征聚合模块整合视觉和几何编码器的局部与全局场景信息，配合位置头在单次前向传播中生成3D资产及其相对空间位置。

Result: 通过大量定量和定性评估验证了方法的效率和鲁棒生成能力，尽管仅使用单图像训练，但能扩展到多图像输入场景并提升生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，会产生意外的伪修改和编辑指令遵循性差的问题。自回归模型提供了一种因果和组合机制，可以自然规避扩散方法的这些问题。

Method: 提出VAREdit框架，将图像编辑重新定义为下一尺度预测问题。基于源图像特征和文本指令生成多尺度目标特征。引入尺度对齐参考(SAR)模块，在第一个自注意力层注入尺度匹配的条件信息，解决细尺度源特征无法有效指导粗尺度目标特征预测的问题。

Result: 在标准基准测试中，VAREdit比领先的基于扩散的方法高出30%以上的GPT-Balance分数。完成512×512编辑仅需1.2秒，比同等规模的UltraEdit快2.2倍。

Conclusion: VAREdit通过自回归范式和多尺度对齐机制，在编辑遵循性和效率方面都取得了显著进步，为指令引导图像编辑提供了新的有效解决方案。
...
Conclusion: 该框架使生成模型能够将多个输出视为有凝聚力的组而非独立样本，为多输出生成任务提供了有效的解决方案。
...
TL;DR: CineScale是一种无需微调的推理范式，能够实现8K图像生成和4K视频生成，解决了现有方法在高分辨率视觉生成中出现的重复模式问题。


<details>
  <summary>Details</summary>
Motivation: 视觉扩散模型由于缺乏高分辨率数据和计算资源限制，通常在有限分辨率下训练，难以生成高质量的高分辨率图像或视频。现有方法容易产生低质量的重复模式内容。

Method: 提出CineScale推理范式，针对不同类型的视频生成架构设计专用变体，能够实现高分辨率T2I、T2V、I2V和V2V合成，基于开源视频生成框架构建。
...
TL;DR: 提出统一的协同训练框架，通过单阶段SFT整合多种安全行为模式，使用系统指令或魔法令牌实现推理时动态行为切换，在保证安全性的同时显著降低训练复杂度和部署成本。
...
Method: 采用协同训练框架，在单一SFT阶段整合正向（合法/亲社会）、负向（无过滤/风险倾向）和拒绝（拒绝导向/保守）三种安全行为，通过系统指令动态激活不同模式。
...
<details>
  <summary>Details</summary>
Motivation: 为WMT25机器翻译共享任务的参与者提供初步的自动评估结果，帮助他们在准备系统提交论文时参考，同时说明自动排名的局限性

Method: 使用自动指标对机器翻译系统进行评估和排名

Result: 生成了基于自动指标的初步排名，但指出该排名可能存在偏向使用重排序技术系统的偏差

Conclusion: 自动排名只是初步结果，最终的官方排名将基于更可靠的人工评估，自动排名将被人工评估结果取代
...
TL;DR: 提出LLM驱动的文化本地化框架，自动为数学应用题生成包含本地化实体（人名、组织名、货币）的数据集，解决低资源语言数学推理中的文化偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有多语言数学推理基准主要通过翻译生成，保留了英语中心实体，缺乏真正的文化本地化数据集，导致低资源语言的数学能力评估存在偏差。

Method: 开发基于大语言模型的框架，从现有资源自动构建包含本地名称、组织和货币的数学应用题数据集，实现文化本地化。

Result: 实验表明翻译基准会掩盖真正的多语言数学能力，而该框架能减轻英语中心实体偏差，提高各语言在引入本地实体时的鲁棒性。

Conclusion: 该框架为低资源语言提供了有效的文化本地化解决方案，改善了多语言数学推理评估的准确性和公平性。
...
TL;DR: 通过直接偏好优化(DPO)技术，使用程序化筛选的质量排名数据对GaMS-9B-Instruct模型进行精细调整，在斯洛文文章翻译中显著提升了翻译质量


<details>
  <summary>Details</summary>
Motivation: 探索如何用较少的易产生数据资源改善通用指令调整的大语言模型的机器翻译能力

Method: 使用DPO技术，通过两个LLM模型生成英语维基百科文章的斯洛文翻译，采用合成质量排名策略（包括COMET自动评估指标）构建训练数据集

Result: 细调后的模型在维基百科文章翻译中超过原始参与数据生成的两个模型，COMET分数分别提升0.04和0.02，更一致避免语言和格式错误

Conclusion: 通过较少的程序化筛选数据进行DPO训练，可有效提升通用指令调整LLM模型在特定语言对的翻译性能
...
Conclusion: 该研究为多语言ConvXAI系统提供了重要的数据集和方法基础，解决了训练数据稀缺和自定义输入支持的问题，推动了对话式可解释AI的多语言发展。
...
TL;DR: 提出奖励偏移推测采样(SSS)算法，通过将草稿模型与人类偏好对齐而保持目标模型不变，在降低推理成本的同时实现测试时弱到强对齐。


<details>
  <summary>Details</summary>
Motivation: 测试时对齐技术虽然能提升LLM的安全性和推理能力，但往往带来显著的推理成本，限制了实际应用。受推测采样加速技术的启发，希望解决测试时对齐的效率瓶颈。
...
Conclusion: SSS算法能够在不实际获得RLHF最优解的情况下恢复该解，为测试时对齐提供了一种高效实用的解决方案。
...
TL;DR: LongRecall是一个三阶段召回评估框架，通过分解答案、过滤候选匹配和结构化蕴含检查，显著提升长文本问答中的召回准确性
...
Result: 在三个长文本问答基准测试中，相比词汇重叠和LLM评估基线，LongRecall在召回准确性方面有显著提升

Conclusion: LongRecall提供了一个系统化的召回评估基础框架，能够减少假阳性和假阴性，适应不同的表达方式和上下文变化
...
TL;DR: 通过结合大语言模型与组合推理方法，提升结构化预测的一致性和准确性，并验证结构化学习在LLM时代仍具有价值


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽在多种语言任务上表现优异，但存在幻觉和复杂推理问题，需要结合结构化推理来提升一致性

Method: 结合LLM与组合推理方法，通过媒体提示策略估计LLM的信心值，并使用符号推理来保证结构一致性

Result: 无论使用哪种提示策略，在简单提示基础上添加符号推理都能带来更一致和准确的预测，结构化学习目标的的校准和微调能提升具有挑战性任务的性能

Conclusion: 结构化推理方法能够有效补充LLM的不足，结构化学习在大语言模型时代仍具有重要价值
...
<details>
  <summary>Details</summary>
Motivation: 现有的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构而导致质量下降，需要更好的科学内容提取方法
...
Conclusion: 这是第一个从噪声网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和通用推理方面带来可衡量的收益，为开源数学预训练语料库设立了新的最先进水平
...
<details>
  <summary>Details</summary>
Motivation: 人们经常提出包含错误假设的问题，这类问题没有常规答案。大型语言模型(LLM)由于幻觉问题往往会生成误导性答案，因此需要先识别错误假设。
...
Conclusion: 利用外部证据和原子假设验证的方法能有效识别和回答包含错误假设的问题，减少LLM的幻觉，提高答案的准确性和可解释性。
...
TL;DR: 提出了MMDR-Bench多模态对话推理基准数据集和CoLVLM Agent框架，通过记忆-感知-规划-执行的迭代循环增强现有LVLM模型，在复杂多轮多模态对话任务上显著超越GPT-4o和Gemini 1.5 Pro等商业模型。
...
Method: 构建了包含300个复杂多轮对话场景的MMDR-Bench数据集；提出了CoLVLM Agent框架，采用无需重新训练底层模型的"记忆-感知-规划-执行"迭代循环机制来增强现有LVLM模型的能力。

Result: 在MMDR-Bench上的实验显示，CoLVLM Agent平均人类评估得分达到4.03，显著超越GPT-4o（3.92）和Gemini 1.5 Pro（3.85），在推理深度、指令遵循和错误抑制方面表现出明显优势。

Conclusion: CoLVLM Agent的模块化设计和迭代方法在复杂多模态交互中表现出有效性，能够保持鲁棒性能并解决现有模型在长对话轮次中的上下文丢失和视觉幻觉问题。
...
Result: 在WikiText-103和LongBench等长上下文语言建模基准上，SemToken实现了最多2.4倍的token减少和1.9倍的加速，困惑度和下游任务准确率几乎没有下降
...
TL;DR: 这篇论文提出了Fin-PRM，一种专门用于金融预测任务的域专门化过程奖励模型，通过给中间推理步骤和轨迹清单进行精细奖励监督，在金融推理性能方面显著超过通用PRM模型和领域基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的过程奖励模型(PRMs)主要培训于通用或STEM领域，在金融等域专门场景中表现不佳，因为金融推理更结构化、符号化且对事实和监管正确性更敏感。

Method: 开发Fin-PRM模型，集成步骤级和轨迹级奖励监督，支持离线和在线奖励学习。应用于三个关键场景：质量轨迹选择用于精细调整、为强化学习提供密集奖励、以及测试时的奖励信息最佳N选择。

Result: 在CFLUE和FinQA等金融推理测试集上，Fin-PRM在轨迹选择质量方面一贵超过通用PRM和域基线模型。下游模型获得显著收益：监督学习提升12.9%，强化学习提升5.2%，测试时性能提升5.1%。

Conclusion: 域专门奖励建模对将大语言模型与专家级金融推理对齐具有重要价值，Fin-PRM为金融领域提供了高效的中间推理评估框架。
...
TL;DR: SPARK是一种无需训练的即插即用方法，通过在通道级别对KV缓存进行剪枝来减少内存使用，同时动态恢复被剪枝的条目以保持注意力计算准确性。
...
Method: 提出通道级别的非结构化稀疏化方法，动态识别和剪枝不重要的KV通道，在注意力计算时恢复关键信息，与现有KV压缩技术正交兼容。

Result: 在相同内存预算下可处理更长序列，同等序列长度下KV缓存存储减少30%以上，即使80%的激进剪枝率也能保持性能退化小于5%。

Conclusion: SPARK通过通道级剪枝有效解决了KV缓存瓶颈，在保持模型精度的同时显著提升内存效率，且与现有压缩技术兼容，具有很好的实用价值。
...
Method: 提出Selct2Know框架，采用内部-外部知识自选择策略和选择性监督微调。引入结构化推理数据生成流程，并集成GRPO来增强推理能力。模拟人类学习的渐进式知识获取过程。

Result: 在医疗、法律和金融QA基准测试中，S2K始终优于现有方法，并以显著更低的成本达到领域预训练LLM的性能水平。

Conclusion: S2K框架通过渐进式知识内化和选择性微调，有效解决了领域特定QA中的知识利用问题，实现了成本效益和性能的平衡。
...
TL;DR: SEER是一种自引导方法，通过从经验池中进行细粒度逐步检索来解决LLM在多步骤工具调用中的挑战，无需手动设计演示，在多个基准测试中显著提升性能。
...
Method: 提出Stepwise Experience Recall (SEER)方法，从持续更新的经验池中进行细粒度逐步检索，增量式扩充经验池，随时间推移提升模型性能。

Result: 在ToolQA基准测试中，简单问题平均提升6.1%，困难问题提升4.7%；在τ-bench真实场景中，Qwen2.5-7B和72B模型分别获得7.44%和23.38%的准确率提升。

Conclusion: SEER通过自引导的经验检索方法有效解决了LLM在多步骤工具调用中的挑战，实现了持续性能改进，无需大量手动工程。
...
TL;DR: 本研究探讨了在生成任务自动评估中选择性使用检查表的效果，发现选择性使用在成对比较任务中能提升评估性能，但在直接评分中效果不一致。
...
Method: 使用六种方法生成检查表，在八种模型规模上进行评估，通过成对比较和直接评分实验分析检查表项目的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在边缘设备部署中面临内存瓶颈，词汇相关组件（嵌入和语言建模头）占用大量内存，现有静态词汇剪枝方法存在信息损失和缺乏灵活性的问题

Method: 基于词汇局部性原理和计算特性不对称性，提出VocabTailor框架：卸载嵌入组件，为LM头实现混合静态-动态词汇选择策略，按需加载词汇组件

Result: 在各种下游任务中，VocabTailor将词汇相关组件内存使用减少高达99%，且任务性能几乎没有下降，显著优于现有静态词汇剪枝方法

Conclusion: VocabTailor通过动态词汇选择有效解决了SLM在边缘设备部署中的内存瓶颈问题，在保持性能的同时大幅降低内存占用
...
TL;DR: 提出了WangchanThaiInstruct数据集，这是一个人工编写的泰语指令数据集，用于评估和指令调优，涵盖4个专业领域和7种任务类型。研究表明，使用本土监督数据的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语指令跟随方面表现出色，但在泰语等低资源语言中的性能尚未充分探索。现有基准测试通常依赖翻译，缺乏现实应用所需的文化和领域特定细微差别。

Method: 通过多阶段质量控制流程创建人工编写的泰语数据集WangchanThaiInstruct，包含4个专业领域和7种任务类型。进行两项研究：(1)零样本评估显示在文化和专业特定任务上存在性能差距；(2)指令调优研究，通过消融实验分离本土监督的效果。

Result: 在WangchanThaiInstruct上微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。零样本评估揭示了在文化和专业特定任务上的性能差距。

Conclusion: 研究结果强调了需要文化和专业基础扎实的指令数据，以改善低资源、语言多样化环境中LLM的对齐性能。
...
TL;DR: 提出了UniCoM流水线，使用SWORDS算法生成高质量代码切换语音样本，构建了CS-FLEURS多语言语料库，在ASR和语音翻译任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 解决代码切换语音技术中数据集稀缺的问题，因为代码切换在真实对话中很常见但对多语言语音技术构成重大挑战

Method: 使用SWORDS算法（同义词替换单词），在保持句子语义不变的前提下，通过替换选定单词为其翻译来生成代码切换语音，同时考虑词性

Result: 构建的CS-FLEURS语料库在客观和主观指标上都达到了高可懂度和自然度，与现有数据集表现相当

Conclusion: 该方法有望推动代码切换语音技术的发展，使多语言系统更具包容性
...
Result: 教师角色LLMs表现出比人类教师更理想化和极化的人格特征，在抽象道德推理方面优秀但在情感复杂情境中表现不佳，推理能力强的模型更容易受到有害提示注入。
...
TL;DR: CARE是一个解决RAG系统中上下文-记忆冲突的新方法，通过上下文评估器和软提示技术来识别不可靠的外部上下文，提高LLM在冲突情况下的性能


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中当检索到的外部上下文与LLM内部参数知识冲突时，模型无法正确分辨可靠知识源的问题

Method: 使用上下文评估器编码紧凑记忆令牌嵌入，通过基于/对抗性软提示训练来识别不可靠上下文并引导推理
...
TL;DR: CUPE是一个轻量级通用音素识别模型，仅需120毫秒音频片段即可捕获关键音素特征，通过独立处理短固定窗口实现跨语言竞争性能。


<details>
  <summary>Details</summary>
Motivation: 许多语音处理任务需要不受上下文影响的纯音素表示，但现有方法通常需要分析长语音片段和语言特定模式。
...
TL;DR: 知识图完整性评估的综合性问题，提出EDAS统一元评价指标，将多数据集多指标绩效合成单一分数


<details>
  <summary>Details</summary>
Motivation: 知识图完整性评估存在多数据集多指标的不一致性问题，模型在不同数据集和指标下表现冲突，影响整体评估和模型选择

Method: 提出EDAS（Distance from Average Solution）元评价指标，将多个数据集的多种评估指标（如MRR、MR、Hit@k等）综合转换为单一标准化分数

Result: 在FB15k-237和WN18RR等标准数据集上验证，EDAS能够有效整合多指标多数据集的绩效，提供一致、稳健的统一排名框架

Conclusion: EDAS为知识图完整性评估提供了统一、可解释的元评价方法，解决了多指标评估不一致问题，促进公平的跨数据集模型比较
...
TL;DR: 大语言模型评测基准系统统计分析，将283个代表性基准分为通用能力、领域特定和目标特定三类，指出当前问题并提出设计范式。


<details>
  <summary>Details</summary>
Motivation: 评测基准作为模型性能的量化评估工具，既是测量模型能力的核心手段，也是引导模型发展方向和推动技术创新的关键要素。

Method: 系统性评估大语言模型基准的现状与发展，将283个代表性基准进行分类：通用能力基准涵盖核心语言学、知识、推理等方面；领域特定基准关注自然科学、人文社会科学、工程技术等领域；目标特定基准关注风险、可靠性、代理等。

Result: 指出当前基准存在数据污染导致分数虚高、文化和语言偏见导致不公平评估、缺乏过程可信度和动态环境评估等问题。

Conclusion: 为未来基准创新提供了可参考的设计范式，以更好地评估和推动大语言模型的发展。
...
### [94] [Benchmarking Computer Science Survey Generation](https://arxiv.org/abs/2508.15658)
...
<details>
  <summary>Details</summary>
Motivation: 由于学术文献快速增长，手动撰写科学综述变得不可行，而现有大语言模型在自动化综述生成方面缺乏标准化基准和评估协议。

Method: 构建SurGE基准，包括主题描述、专家撰写综述及其引用文献的测试实例集，以及百万篇论文的检索语料库；提出自动化评估框架，从信息覆盖度、引用准确性、结构组织和内容质量四个维度进行评估。

Result: 对各种基于LLM的方法评估表明，即使是先进的自反思框架，综述生成仍然极具挑战性。

Conclusion: 科学综述生成任务具有高度复杂性，需要持续研究，SurGE基准为这一领域提供了标准化评估工具。
...
### [95] [Unveiling Trust in Multimodal Large Language Models: Evaluation, Analysis, and Mitigation](https://arxiv.org/abs/2508.15370)
...
TL;DR: MultiTrust-X是一个全面的多模态大语言模型可信度评估基准，包含5个可信度维度、2种风险类型和多种缓解策略，通过32个任务和28个数据集对30多个模型进行评估，揭示了当前模型的显著漏洞并提出了改进方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型评估方法往往关注狭窄的方面，忽视了多模态性带来的风险，需要建立一个全面的可信度评估框架来系统性地解决这些问题。

Method: 提出了MultiTrust-X基准，定义了三维框架：5个可信度方面（真实性、鲁棒性、安全性、公平性、隐私）、2种风险类型（多模态风险和跨模态影响）以及多种缓解策略（数据、模型架构、训练和推理算法）。包含32个任务和28个精选数据集。

Result: 实验发现当前模型存在显著漏洞，包括可信度与通用能力之间的差距，以及多模态训练和推理对基础LLM潜在风险的放大。现有缓解策略存在局限性，虽然某些方法在特定方面有所改进，但很少有方法能有效解决整体可信度问题，且许多方法会带来意外的权衡而损害模型效用。

Conclusion: 基于研究发现提出了推理增强安全对齐（RESA）方法，通过赋予模型思维链推理能力来发现潜在风险，取得了最先进的结果，为未来改进提供了实用见解，如推理在更好平衡安全性和性能方面的益处。
...
### [96] [Stemming -- The Evolution and Current State with a Focus on Bangla](https://arxiv.org/abs/2508.15711)
*Abhijit Paul,Mashiat Amin Farin,Sharif Md. Abdullah,Ahmedul Kabir,Zarif Masud,Shebuti Rayana*

Main category: cs.CL

TL;DR: 本文对孟加拉语词干提取方法进行了全面调查，指出该领域存在研究断层、可复现实现稀缺、评估方法不足等问题，并针对孟加拉语丰富的形态和方言特点提出了发展方向建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言（3亿母语者）面临数字资源匮乏问题。词干提取作为语言分析的关键预处理步骤，对于低资源、高屈折性的孟加拉语尤为重要，能显著降低算法复杂度。

Method: 通过对现有孟加拉语词干提取方法的全面调查和分析，批判性地评估现有研究的方法论和可复现性，并提出改进方向。

Result: 发现现有研究存在明显断层，缺乏可访问的实现代码，评估指标不够相关，难以应对孟加拉语丰富的形态变化和方言多样性挑战。

Conclusion: 呼吁开发更强大的孟加拉语词干提取器，持续进行研究以提升语言分析和处理能力，并提出了具体的发展方向建议。
...
TL;DR: 这篇论文通过实验证明了语言模型中词汇表规模扩大的真正好处来源于降低了文本的复杂性，而非简单的词汇量增加。这种改善主要通过减少高频词的不确定性实现，并会演化为下游任务的性能提升。


<details>
  <summary>Details</summary>
Motivation: 语言模型训练中使用的分词器会导致不平衡的分布：少数词占据了大部分流水，而大多数词出现频率很低。虽然近期实践偏向于越来越大的词汇表，但其好处的根源并不清楚。研究者想要控制性地研究词汇表规模扩大对语言模型性能的真正影响。

Method: 进行了一个受控研究，在保持数据、计算资源和优化固定的前提下，将语言模型的词汇表规模从24K扩展到196K。采用Kolmogorov复杂性来量化分词文本的复杂性，并通过词级损失分解分析不同词汇规模对损失的影响。还通过约束输入和输出嵌入的模来衰减分布不平衡的影响。

Result: 词汇表规模扩大会降低分词文本的Kolmogorov复杂性。在24K以上，所有常见词已经是单独的分词，进一步扩大主要加深了分布不平衡。词汇表扩大减少交叉熵减主要是通过降低前2,500个最高频词的不确定性，而尾部稀有词的损失反而会上升。约束嵌入模的实验反过来证明了模型是利用而非受到了分布不平衡的影响。

Conclusion: 论文重新解释了"词汇表越大越好"的观点，认为其真正好处在于降低了分词文本的复杂性。这为分词器与模型的协同设计提供了一个简单而有理论基础的策略，同时也清楚地解释了预训练过程中测度扩展的损失动态机制。
...
TL;DR: 对134篇论文的系统性综述，提出了基于证据的LLM文本生成的统一分类法，分析了300个评估指标，重点关注引用、归因和引用的方法，并指出了该领域的挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛使用，对其可靠性和可信度的担忧日益增长。当前基于证据的文本生成研究领域存在术语不一致、评估实践孤立、缺乏统一基准等问题，需要系统性的整合和分析。

Method: 系统分析了134篇相关论文，引入了基于证据的LLM文本生成的统一分类法，调查了300个评估指标，涵盖七个关键维度，重点关注使用引用、归因或引用的方法。

Result: 建立了该领域的统一分类体系，识别了代表性方法，系统梳理了评估指标，为基于证据的文本生成研究提供了系统性的分析框架。

Conclusion: 该研究填补了基于证据的LLM文本生成领域的系统性空白，提出了统一的分类法和评估框架，为未来研究指明了开放挑战和潜在发展方向。
...
TL;DR: MCR-BENCH是首个专门评估大型音频-语言模型在处理不一致音频-文本对时信息优先级的基准测试，发现模型存在显著的文本偏向性，经常忽视音频证据。


<details>
  <summary>Details</summary>
Motivation: 虽然大型音频-语言模型具备音频感知能力，但其在处理音频和文本模态间冲突信息时的表现尚未得到充分研究，需要专门的评估基准。

Method: 构建MCR-BENCH基准测试，通过多样化的音频理解任务进行广泛评估，分析模型在矛盾输入下的表现，并研究影响因素和缓解策略。

Result: 发现LALMs存在严重的文本偏向性，当模态间存在不一致时，模型倾向于优先文本输入而忽视音频证据，导致音频中心任务性能显著下降。
...
<details>
  <summary>Details</summary>
Motivation: 大型语音语言模型（LSLM）发展受阻于碎片化架构和缺乏透明度，无法进行系统性比较和复现研究，与视觉语言领域不同，LSLM领域通常只发布模型权重而不公开训练数据和配置。
...
Result: LLaSO-Base模型取得了0.72的标准化分数，建立了强大且可复现的基线，超越了可比模型。分析发现，虽然更广泛的训练覆盖能提升性能，但在未见任务上仍存在显著泛化差距，特别是在纯音频场景中。

Conclusion: 通过完整发布数据、基准和模型堆栈，LLaSO建立了基础性开放标准，旨在统一研究努力并加速LSLM领域的社区驱动进展。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型通常训练在敏感数据上，可能记忆并泄漏这些信息，引发隐私权利保护的担忧。尽管隐私保护至关重要，但对语言模型隐私风险的理解和减缓方法仍有限。

Method: 进行了隐私保护语言建模方法的综合性研究，深入概述各种方法，重点分析其优势和局限性。

Result: 研究成果为隐私保护语言建模领域的持续研究做出了贡献，提供了有价值的见解和方向性指导。

Conclusion: 该研究填补了语言模型隐私保护方面的研究空白，为该领域的发展提供了重要的理论基础和实践指南。
...
Result: 在M-Help上训练的AI模型能够处理三个关键任务：识别求助者、诊断心理健康状况、揭示问题根源。

Conclusion: M-Help数据集填补了现有研究的空白，为检测社交媒体求助行为和深入分析心理健康问题提供了重要资源。
...
TL;DR: 这篇论文研究了语言翻译中的非对等词汇问题，提出了从源语言向目标语言转换非对等词汇的方法和规则，并展示了从达尔语和乌兹别克语向英语和俄语转换25个非对等词汇的实例。


<details>
  <summary>Details</summary>
Motivation: 语言理解直接关系到翻译知识，非对等词汇（如食物、服装、文化和传统词汇）在不同语言间往往没有直接对应词，导致误解，需要找到专业的转换方法。

Method: 采用图书馆基础研究方法，分析和总结不同的非对等词汇转换方法和规则。

Result: 研究提供了多种转换非对等词汇的方式和规则，并成功将达尔语和乌兹别克语中的25个非对等词汇转换为英语和俄语。

Conclusion: 这项研究为语言学家和翻译人员提供了重要的非对等词汇处理方法，虽然部分词汇已经完成专业转换，但仍有许多词汇需要继续研究和转换。
...
<details>
  <summary>Details</summary>
Motivation: 现有的可编程任务导向对话代理的有效性依赖于准确的对话状态跟踪，但传统方法在状态跟踪方面存在局限性，需要更有效的错误纠正机制。
...
Conclusion: 执行感知的状态跟踪方法有效提升了任务导向对话代理的性能，PyTOD通过代码生成和反馈机制展示了在复杂对话场景中的优越表现。
...
TL;DR: SLM4Offer是基于T5-Small微调的生成式AI模型，采用对比学习方法提升个性化优惠生成效果，相比基线模型提升了17%的优惠接受率


<details>
  <summary>Details</summary>
Motivation: 个性化营销能显著提升转化率和客户满意度，研究表明良好执行的个性化策略可使收入提升40%，但现有研究主要集中在推荐系统和个性化广告，个性化优惠生成领域仍有巨大潜力

Method: 通过对比学习方法微调预训练的T5-Small编码器-解码器语言模型，使用InfoNCE损失函数在共享嵌入空间中对齐客户画像和相关优惠，引入自适应学习行为重塑潜在空间
...
TL;DR: 这篇论文提出了一种异质性知觉语言模型训练方法HeTLM，通过分群训练小型模型来更好地描述用户主观浏览行为，在保持高平均性能的同时降低性能方差，提升用户级别对齐效果。


<details>
  <summary>Details</summary>
Motivation: 质疑大型语言模型是否能够有效处理用户主观性和异质性的浏览行为，这种行为形成了类似于每个用户自己构建的"浏览语言"。

Method: 提出HeTLM方法，采用异质性聚类特定参数集进行语言模型训练，使用页面级别分词器训练小型模型。

Result: 小型模型在浏览语言表现上超过大型预训练或微调模型；HeTLM在控制参数数量的情况下超过单一模型；实现了更高的平均性能和更低的性能方差。

Conclusion: 通过考虑用户行为的异质性进行聚类化训练，小型专门化模型能够更好地处理主观性浏览行为，提升用户级别的模型对齐效果。
...
TL;DR: 本文研究发现，使用基于模型训练影响的难度度量（训练数据影响力）进行课程学习，可以使语言模型预训练效果提升10个百分点以上，证明模型中心的难度定义比传统人工定义更有效。


<details>
  <summary>Details</summary>
Motivation: 传统的课程学习在语言模型预训练中效果有限，主要原因是基于人工定义的难度度量（如文档复杂度）不能准确反映模型训练过程中的实际难度。

Method: 提出使用训练数据影响力作为难度度量标准，该分数估计单个训练样本对模型输出的影响程度，并按照此标准对训练样本从简单到复杂排序。

Result: 使用新课程学习方法训练的模型在基准测试中比随机顺序训练的模型性能提升超过10个百分点。

Conclusion: 课程学习对语言模型预训练是有益的，但需要采用更符合模型训练特性的模型中心难度概念，而非传统的人工中心难度度量。
...
Result: 评估结果显示不同SLM模型在准确性和能效方面存在显著权衡，部分模型准确性优异，另一些模型能效表现突出。

Conclusion: SLM-Bench为SLM评估设立了新标准，在资源效率和实际应用性之间架起了桥梁，提供了开源基准测试管道促进可复现性研究。
...
TL;DR: HebID是首个希伯来语多标签社会身份检测语料库，包含5536个以色列政治家Facebook帖子的句子，标注了12种细粒度社会身份，在希伯来语调优的大语言模型上取得了最佳性能（macro-F1=0.74）。


<details>
  <summary>Details</summary>
Motivation: 现有社会身份检测数据集主要为英语中心、单标签且关注粗粒度身份类别，缺乏对特定文化语境和多标签身份表达的研究。

Method: 构建包含5536个希伯来语句子的语料库，手动标注12种社会身份，使用多标签和单标签编码器以及2B-9B参数生成式LLM进行基准测试。

Result: 希伯来语调优的LLM表现最佳（macro-F1=0.74），应用分类器分析了政治家帖子和议会演讲中的身份表达差异、时间趋势和性别相关变化。
...
TL;DR: Dream 7B是一个基于离散扩散建模的强大开源扩散大语言模型，在通用、数学和编程任务上表现优异，具有并行生成、任意顺序生成和填充能力等优势。


<details>
  <summary>Details</summary>
Motivation: 为了解决自回归模型顺序生成的局限性，探索扩散模型在语言建模中的潜力，实现更灵活的序列生成和推理能力。

Method: 采用离散扩散建模方法，通过迭代去噪并行优化序列；使用基于自回归LLM的初始化策略和上下文自适应的token级噪声重调度技术。

Result: 在通用、数学和编程任务上持续超越现有扩散语言模型，展现出卓越的规划能力和推理灵活性，包括任意顺序生成、填充能力以及可调节的质量-速度权衡。

Conclusion: Dream 7B证明了扩散模型在语言建模中的有效性，通过简单而有效的训练技术实现了优异性能，为基于扩散的语言建模研究提供了有力工具。
...
Method: 构建包含10,410句希伯来语句子的语料库，来自议会演讲、Facebook帖子和新闻媒体；开发两阶段分类管道，结合微调编码器模型和解码器LLM。

Result: 最佳模型(DictaLM 2.0)在二元PDD检测上达到F1=0.74，在去合法化特征分类上达到macro-F1=0.67；发现PDD在过去30年显著上升，社交媒体比议会辩论更普遍，男性政客比女性使用更多，右翼行为者倾向更强。
...
Method: 通过协调7个专门代理构建自动化流水线，集成人类专业知识，确保过程可控和成本可控，构建低冗余、强判别力的安全基准数据集。

Result: 构建的SafetyFlowBench数据集包含23,446个查询，对49个先进LLM进行安全评估，验证了系统的有效性和效率。

Conclusion: SafetyFlow实现了首个完全自动化的基准构建流程，提供了全面的安全基准，显著提升了LLM安全评估的效率和可扩展性。
...
TL;DR: 小型语言模型(SLMs)通过精细调整专门用于高价值应用场景，以极低成本生成领域特定响应
...
Method: 提出"训练小型"概念，将小型语言模型进行精细调整，专门用于特定高价值应用场景

Result: 能够以极低的成本生成类似的领域特定响应

Conclusion: 通过专门化的小型模型实现成本效益优化，为特定应用场景提供了可行的解决方案
...
Method: 提出SDGO框架，通过强化学习利用模型自身的判别能力作为奖励信号，进行迭代自我优化来提升生成安全性。
...
<details>
  <summary>Details</summary>
Motivation: 位置偏差（PB）在长上下文理解中表现为不同位置敏感度不均，严重影响模型性能。现有方法通过修改架构来缓解PB，但效果有限，需要更有效的解决方案。

Method: 提出Pos2Distill框架，基于位置诱导差异来对抗位置偏差本身。针对检索和推理两种范式设计了专门实例：Pos2Distill-R¹和Pos2Distill-R²，通过知识蒸馏将优势位置能力转移到劣势位置。

Result: 在长上下文检索和推理任务中，所有上下文位置都实现了增强的一致性和显著的性能提升。两个专门化系统在各自任务上表现出优异性能，并展现出强大的跨任务泛化能力。

Conclusion: Pos2Distill通过利用位置间固有差异来对抗位置偏差，有效解决了长上下文处理中的位置敏感度不均问题，为缓解位置偏差提供了新的有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 电商平台拥有丰富的多模态数据，但现有数据集规模有限，难以系统研究图像是否真正提升产品理解能力，以及多图像的有效利用问题。

Method: 构建EcomMMMU数据集（40万+样本，890万+图像），包含8个核心任务；提出SUMEI方法，通过预测视觉效用值来策略性选择和使用多图像。

Result: 分析发现产品图像并不总是提升性能，有时反而会降低效果；SUMEI方法在综合实验中表现出有效性和鲁棒性。

Conclusion: 多模态大语言模型在电商任务中难以有效利用丰富视觉内容，需要数据驱动的方法来策略性选择有用图像，SUMEI为此提供了有效解决方案。
...
TL;DR: Deep-DxSearch是一个基于强化学习的端到端医疗诊断RAG系统，通过构建大规模医疗检索语料库和定制化奖励机制，显著提升了医疗大语言模型的诊断准确性和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 现有医疗大语言模型存在知识鸿沟和幻觉问题，传统检索增强方法对外部知识利用不足且缺乏反馈-推理可追溯性，需要更有效的诊断辅助系统。

Method: 构建大规模医疗检索语料库，将LLM作为核心智能体，检索语料库作为环境，通过强化学习训练端到端的智能RAG策略，使用格式、检索、推理结构和诊断准确性等多维度奖励机制。

Result: 在多个数据中心超越提示工程和无训练RAG方法，诊断准确性显著优于GPT-4o、DeepSeek-R1等强基线模型，在常见和罕见疾病诊断中均表现优异，支持分布内外设置。

Conclusion: Deep-DxSearch通过强化学习驱动的智能RAG框架有效解决了医疗诊断中的知识鸿沟问题，奖励设计和检索语料库组件对性能提升至关重要，为临床医生提供更可靠精确的初步诊断支持。
...
<details>
  <summary>Details</summary>
Motivation: 尽管链式思维推理有所进展，但LLM在精确计算任务中仍存在不足，工具集成推理的泛化效果和推理行为改善机制尚不明确。

Method: 构建ReasonZoo基准测试，涵盖9个推理类别，并提出性能感知成本(PAC)和性能-成本曲线下面积(AUC-PCC)两个新指标来评估推理效率。

Result: TIR模型在数学和非数学任务中均优于非TIR模型，推理效率提升，过度思考减少，推理过程更加精简。

Conclusion: TIR具有领域通用的优势，能够显著提升LLM在复杂推理任务中的能力，证明了其推动LLM发展的潜力。
...
Result: 实验表明即使最先进的LLM在工具编排方面也面临重大挑战，成功率低于60%。详细的消融实验和错误分析揭示了不同的失败模式和令牌使用效率问题。
...
### [122] [Robust Symbolic Reasoning for Visual Narratives via Hierarchical and Semantically Normalized Knowledge Graphs](https://arxiv.org/abs/2508.14941)
*Yi-Chun Chen*

Main category: cs.MM

TL;DR: 本文提出了一个语义规范化框架，用于处理视觉叙事（如漫画）中的层次化叙事知识图谱，通过词汇相似性和嵌入聚类来减少标注噪声，提高叙事推理任务的连贯性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视觉叙事（如漫画）需要结构化表示来捕捉事件、角色及其关系，但符号叙事图谱存在不一致性和冗余问题，限制了推理和泛化的效果。
...
### [123] [Holo-Artisan: A Personalized Multi-User Holographic Experience for Virtual Museums on the Edge Intelligence](https://arxiv.org/abs/2508.14956)
...
<details>
  <summary>Details</summary>
Motivation: 传统博物馆展览是静态的，缺乏与观众的互动性。作者希望创造一种动态的、个性化的文化体验，让艺术作品能够实时响应每位观众，创造更加沉浸式的互动体验。

Method: 系统采用边缘计算节点实时处理多用户数据（姿态、表情、语音），生成式AI模型驱动数字艺术品个性化响应。云端协作平台通过通用场景描述和光线追踪渲染高保真视图，并集成联邦学习保护隐私和持续优化个性化体验。

Result: 该系统成功实现了静态博物馆展品向动态、活生生的艺术品的转变，能够同时为多位观众提供个性化的实时互动体验，如蒙娜丽莎对不同观众展现不同表情和进行语音问答。

Conclusion: Holo-Artisan开创了文化遗产互动的新范式，通过边缘智能和全息技术实现了真正的个性化沉浸式体验，为虚拟博物馆和数字文化遗产展示提供了创新解决方案。
...
### [124] [\textit{adder-viz}: Real-Time Visualization Software for Transcoding Event Video](https://arxiv.org/abs/2508.14996)
...
<details>
  <summary>Details</summary>
Motivation: 传统事件视频表示方法在灵活性、速度和压缩性方面存在局限，需要更好的可视化工具来支持ADΔER表示格式

Method: 开发了改进的adder-viz软件，提供实时事件转码过程的可视化功能，支持在应用循环中使用

Result: 发布了MIT许可的开源软件，可从集中存储库获取，支持ADΔER事件表示格式的可视化

Conclusion: adder-viz软件的改进为事件视频研究提供了更好的可视化工具，有助于推动ADΔER表示格式的应用和发展
...
### [125] [A Low-Latency 3D Live Remote Visualization System for Tourist Sites Integrating Dynamic and Pre-captured Static Point Clouds](https://arxiv.org/abs/2508.15398)
*Takahiro Matsumoto,Masafumi Suzuki,Mariko Yamaguchi,Masakatsu Aoki,Shunsuke Konagai,Kazuhiko Murasaki*

Main category: cs.MM

TL;DR: 一种用于户外旅游景点的实时动态三维空间截获与可视化系统，结合多参数光雷达和相机，并集成预截获的静态点云，实现30fps的宽区域可视化且延迟低于100ms


<details>
  <summary>Details</summary>
Motivation: 现有的实时三维空间截获技术在户外旅游景点应用遇到困难，包括传感器布置受限制和白天光照变化导致的处理复杂性

Method: 结合多个LiDAR和相机进行实时动态点云截获，并与预截获的静态点云集成；自动调整静态点云颜色以适应当前光照条件

Result: 系统在宽区域场景中维持了30fps的帧率，延迟低于100ms，并通过实际部署在旅游景点验证了有效性

Conclusion: 该系统成功解决了户外旅游景点动态三维截获的挑战，提供了高性能的实时宽区域可视化方案
...
### [126] [A Fully Spectral Neuro-Symbolic Reasoning Architecture with Graph Signal Processing as the Computational Backbone](https://arxiv.org/abs/2508.14923)
*Andrew Kiruluta*

Main category: cs.AI

TL;DR: 提出了一种完全基于频谱的神经符号推理架构，使用图信号处理作为核心计算框架，将整个推理流程置于图频谱域中处理。


<details>
  <summary>Details</summary>
Motivation: 传统推理模型仅将频谱图方法作为外围组件，而本研究旨在通过完全频谱化的方法来提升逻辑一致性、可解释性和计算效率。

Method: 将逻辑实体和关系编码为图信号，通过可学习的频谱滤波器控制多尺度信息传播，并映射到符号谓词进行基于规则的推理。包括图傅里叶变换、频带选择性注意力和频谱规则接地等数学框架。
...
### [127] [Goals and the Structure of Experience](https://arxiv.org/abs/2508.15013)
...
Method: 引入目标导向（telic）状态概念，定义为目标等效经验分布的类别，通过行为策略与期望经验特征之间的统计差异来提供目标导向学习的简洁解释。

Result: 提出了一个统一的计算框架，能够解释行为、现象学和神经维度上的目的性行为，支持描述性和规范性世界模型方面的共同涌现理论。

Conclusion: 该框架为跨不同基质的目的性行为提供了统一的理论基础，挑战了传统分离状态表示和奖励函数的计算模型，强调了从智能体目标中共同涌现的重要性。
...
### [128] [Collab-REC: An LLM-based Agentic Framework for Balancing Recommendations in Tourism](https://arxiv.org/abs/2508.15030)
...
<details>
  <summary>Details</summary>
Motivation: 解决旅游推荐系统中的流行度偏见问题，避免过度旅游，提升推荐多样性，让更多冷门但优质的旅游地点得到关注。

Method: 使用三个基于LLM的智能体（个性化、流行度、可持续性）分别生成推荐，然后通过非LLM调解器进行多轮协商和整合，惩罚重复或无关响应。

Result: 在欧洲城市查询实验中，Collab-REC相比单智能体基线提高了推荐多样性和整体相关性，能够发掘常被忽视的冷门旅游地点。

Conclusion: 这种平衡的、上下文感知的多利益相关者协作方法在LLM驱动的推荐系统中具有广阔前景，能够更好地处理用户约束并解决过度旅游问题。
...
### [129] [Emergent Crowds Dynamics from Language-Driven Multi-Agent Interactions](https://arxiv.org/abs/2508.15047)
*Yibo Liu,Liam Shatzel,Brandon Haworth,Teseo Schneider*

Main category: cs.AI

TL;DR: 基于大语言模型的群体动画方法，通过对话系统和语言驱动导航实现了更现实的群体行为模拟


<details>
  <summary>Details</summary>
Motivation: 现有群体动画方法忽视了语言和对话对人类导航行为的重要影响，交互作用限于基本的避挡和高级目标识别

Method: 使用LLMs构建两个核心组件：对话系统和语言驱动导航。基亊个体特征、视觉和物理状态生成对话，并依此控制移动决策

Result: 在复杂场景中验证了方法有效性，实现了自动分组和解散，构建了群体内信息传递机制

Conclusion: 该框架能够生成更现实的群体模拟，从任何环境设置中自然出现出群体行为
...
### [130] [Don't Think Twice! Over-Reasoning Impairs Confidence Calibration](https://arxiv.org/abs/2508.15050)
*Romain Lacombe,Kerrie Wu,Eddie Dilworth*

Main category: cs.AI

TL;DR: 这篇论文系统评估了大语言模型的自信心检测准确性，发现增加推理资源反而导致过度自信，而基于搜索的方法能显著提升准确率到89.3%。


<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型在问答任务中存在的过度自信问题，需要系统性地评估推理能力和计算资源对自信心评估准确性的影响。

Method: 使用ClimateX数据集并扩展到人类和地球健康领域，系统测试推理LLM在不同计算资源配置下的自信心检测准确性，对比了纯推理与搜索增强生成的性能差异。

Result: 近期推理LLM在专家自信心评估中仅获48.7%的准确率，增加推理资源会系统性地导致过度自信且情况越来越差。相比之下，搜索增强生成能达到89.3%的准确率。

Conclusion: 信息获取能力而非推理深度或推理资源，才是改善知识密集型任务自信心检测的关键瓶颈，这对当前的"测试时缩放"范式构成了挑战。
...
### [131] [Demonstrating Onboard Inference for Earth Science Applications with Spectral Analysis Algorithms and Deep Learning](https://arxiv.org/abs/2508.15053)
*Itai Zilberstein,Alberto Candela,Steve Chien,David Rijlaarsdam,Tom Hendrix,Leonie Buckley,Aubrey Dunne*

Main category: cs.AI

TL;DR: 在CogniSAT-6/HAMMER卫星上进行边缘数据分析和推理的实验展示，利用超谱仪器和神经网络加速硬件实现地球科学应用
...
Method: 使用深度学习和谱分析算法，在配备可见光和近红外超谱仪器、神经网络加速硬件的CogniSAT-6卫星上进行多种应用的数据分析和推理

Result: 展示了在卫星上直接进行边缘数据处理的技术能力

Conclusion: 边缘计算在太空任务中具有重要价值，能够提高数据处理效率并开启新的科学可能性
...
### [132] [S3LoRA: Safe Spectral Sharpness-Guided Pruning in Adaptation of Agent Planner](https://arxiv.org/abs/2508.15068)
*Shuang Ao,Gopal Rumchurn*

Main category: cs.AI

TL;DR: S3LoRA是一个轻量级、无需数据、模型无关的框架，通过分析LoRA微调权重更新来缓解安全风险，使用MAS-SVD分析结构特性并通过SSI指标检测和修剪危险层，在保持任务性能的同时显著提升安全性。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调技术（如LoRA）在增强LLM智能体能力的同时可能无意中破坏安全对齐，导致不安全行为。现有安全适应方法需要访问基础模型和指令微调检查点，这在实践中往往不可用。

Method: 提出S3LoRA框架：1）引入MAS-SVD方法分析LoRA更新的结构特性并保留全局幅度信息；2）设计SSI指标检测具有高度集中和潜在危险更新的层；3）对这些层进行后修剪以降低风险。
...
### [133] [Argumentation for Explainable Workforce Optimisation (with Appendix)](https://arxiv.org/abs/2508.15118)
*Jennifer Leigh,Dimitrios Letsios,Alessandro Mella,Lucio Machetti,Francesca Toni*

Main category: cs.AI

TL;DR: 将劳动力管理问题模型化为抽象论证框架，提供可解释的策略来处理执行时变化，通过用户研究验证了方法的效果。


<details>
  <summary>Details</summary>
Motivation: 劳动力管理中的关键挑战是在执行时处理变化并向所有利益相关方提供解释。传统手工解决方案无法有效处理这些变化和提供可解释的决策。

Method: 通过将劳动力管理问题理解为工业应用中的抽象论证（abstract argumentation）来构建模型，这种方法能够适应变化并产生可信的解释。

Result: 通过用户研究验证，证明了该工具和解释方法相比传统手工解决方案，能够导致更快速和更准确的问题解决。

Conclusion: 采用抽象论证框架来处理劳动力管理问题，能够有效地处理执行时变化并提供可解释的决策，提高了问题解决的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 解决具身AI代理需要推断和响应未预定义的人类目标和偏好的问题，特别是在目标空间无界且不断演化的开放环境中。

Method: 提出GOOD方法：使用LLM模拟具有不同复杂意图的用户，通过响应进行概率推理来推断自然语言目标分布，无需大型离线数据集。

Result: 在基于文本的杂货购物和AI2Thor家庭机器人模拟环境中，使用合成用户配置文件进行评估，GOOD方法在LLM和人类评估中都优于无显式目标跟踪的基线方法。

Conclusion: GOOD方法能够实现丰富的目标表示和不确定性估计，为开放宇宙中的目标推理提供了一种数据高效的在线解决方案。
...
Method: 构建多智能体架构平台，提供API和MCP接口，支持人类和AI科学家无缝集成，实现研究提案和论文的提交、评审和迭代精炼
...
### [136] [Mobile-Agent-v3: Foundamental Agents for GUI Automation](https://arxiv.org/abs/2508.15144)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在桌面和移动环境中执行多种GUI任务的端到端基础代理模型，解决GUI交互中的自动化问题。

Method: 采用大规模云基础设施、自演进GUI轨迹生成框架、集成UI定位/规划/动作语义/推理模式，以及可扩展的强化学习框架。

Result: GUI-Owl-7B在AndroidWorld达到66.4，OSWorld达到29.4；Mobile-Agent-v3分别提升至73.3和37.7，创开源GUI代理框架新纪录。

Conclusion: GUI-Owl和Mobile-Agent-v3为GUI代理提供了强大的基础模型和框架，通过创新的数据生成和训练方法实现了显著的性能提升。
...
### [137] [PuzzleClone: An SMT-Powered Framework for Synthesizing Verifiable Data](https://arxiv.org/abs/2508.15180)
*Kai Xiong,Yanwei Huang,Rongjunchen Zhang,Kun Chen,Haipang Wu*

Main category: cs.AI

TL;DR: PuzzleClone是一个基于SMT的形式化框架，用于大规模合成可验证的逻辑和数学谜题数据集，通过训练显著提升LLM的推理能力
...
Method: 使用Satisfiability Modulo Theories (SMT)技术，将种子谜题编码为结构化逻辑规范，通过系统变量和约束随机化生成可扩展变体，并通过复制机制确保有效性

Result: 构建了包含83K+多样化谜题的基准测试集，训练后PuzzleClone平均得分从14.4提升至56.2，在7个逻辑数学基准上实现最高12.5个百分点的提升

Conclusion: PuzzleClone框架能够有效生成大规模、多样化且可验证的逻辑数学数据集，显著提升LLM的推理性能，为解决现有数据集的局限性提供了有效方案
...
### [138] [LLM4Sweat: A Trustworthy Large Language Model for Hyperhidrosis Support](https://arxiv.org/abs/2508.15192)
*Wenjie Lin,Jin Wei-Kocsis*

Main category: cs.AI

TL;DR: 这是一个专门为稀有疾病多汗痆设计的开源LLM框架LLM4Sweat，通过数据增帽、精细调整和专家评估三阶段流程，提供准确的诊断、个性化治疗建议和共情心理支持。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在医疗健康领域有应用潜力，但对于稀有疾病如多汗痆（影响2-3%人口），缺乏可靠的细调数据集仍是主要障碍。目前没有专门为多汗痆设计的LLM工作。

Method: 采用三阶段流水线：1）数据增帽阶段：使用前沿LLM从精选开源数据生成医学合理的合成案例，创建多样化平衡的问答数据集；2）精细调整阶段：在数据集上对开源基础模型进行精细调整，提供诊断、个性化治疗建议和心理支持；3）推理与专家评估阶段：临床和心理专家评估准确性、适当性和共情能力，验证的响应迭代丰富数据集。

Result: 实验表明LLM4Sweat在性能上超过基线模型，提供了首个开源的多汗痆LLM框架，为其他类似数据和可靠性挑战的稀有疾病提供了可推广的方法。

Conclusion: LLM4Sweat框架成功解决了稀有疾病数据缺乏的挑战，通过合成数据生成和专家评估的迭代流程，为多汗痆患者提供了可靠、准确且具有共情心的医疗支持，为其他稀有疾病的LLM应用投射了光明前景。
...
### [139] [R-ConstraintBench: Evaluating LLMs on NP-Complete Scheduling](https://arxiv.org/abs/2508.15204)
...
Method: 开发R-ConstraintBench框架，在DAG中逐步增加非冗余前序约束，然后引入停机时间、时间窗口和分离约束，并在数据中心迁移场景中进行实例化评估。

Result: 强模型在仅有前序约束的DAG上表现接近天花板，但当多种约束类型交互时可行性性能崩溃，约束交互是主要瓶颈，合成数据性能不能保证领域场景的泛化。

Conclusion: 大语言模型在复杂约束交互下的推理能力有限，约束交互而非图深度是主要挑战，需要改进模型在真实领域约束下的泛化能力。
...
### [140] [See it. Say it. Sorted: Agentic System for Compositional Diagram Generation](https://arxiv.org/abs/2508.15222)
*Hantao Zhang,Jingyang Liu,Ed Li*

Main category: cs.AI

TL;DR: 提出See it. Say it. Sorted.系统，通过VLM和LLM协同工作，将手绘草图转换为精确可编辑的SVG图表，在流程图生成上优于GPT-5和Gemini-2.5-Pro。
...
Method: 训练无关的智能体系统，结合视觉语言模型(VLM)和大型语言模型(LLMs)，通过迭代循环：批评VLM提出编辑建议，多个候选LLM生成SVG更新，法官VLM选择最佳方案。

Result: 在10个已发表论文流程图草图测试中，比GPT-5和Gemini-2.5-Pro更准确地重建布局和结构，能精确组合图元而不插入多余文本。

Conclusion: 该方法优先定性推理而非脆弱的数值估计，保持全局约束，支持人工干预，输出可编程SVG便于扩展和集成到演示工具中。代码已开源。
...
### [141] [Computational Intelligence based Land-use Allocation Approaches for Mixed Use Areas](https://arxiv.org/abs/2508.15240)
...
<details>
  <summary>Details</summary>
Motivation: 解决可持续城市发展中复杂的多目标土地利用分配优化问题，平衡土地利用兼容性与经济目标之间的固有权衡关系。

Method: 开发多种优化算法，包括集成差分进化与多目标遗传算法的定制变体，采用约束松弛策略和统计验证方法（Kruskal-Wallis检验）。

Result: 在1,290个地块的实际案例中，CR+DES算法在土地利用兼容性上提升3.16%，MSBX+MO在价格优化上提升3.3%，统计验证显示包含差分向量的算法显著优于传统方法。

Conclusion: 研究成果为城市规划者和政策制定者提供了基于证据的计算工具，支持快速城市化地区更有效的城市发展政策制定。
...
### [142] [Multiple Memory Systems for Enhancing the Long-term Memory of Agent](https://arxiv.org/abs/2508.15294)
...
<details>
  <summary>Details</summary>
Motivation: 现有的记忆模块方法（如MemoryBank和A-MEM）存储的记忆内容质量较差，影响回忆性能和响应质量，需要更好地构建高质量的长期记忆内容。

Method: 受认知心理学理论启发，设计多重记忆系统(MMS)。将短期记忆处理成多个长期记忆片段，构建检索记忆单元和上下文记忆单元（一一对应）。检索阶段匹配最相关的检索记忆单元，获取对应的上下文记忆单元作为响应上下文。

Result: 在LoCoMo数据集上的实验证明了方法的有效性，消融研究确认了记忆单元的合理性。分析显示方法在选择记忆段数量和存储开销方面具有良好的稳健性。

Conclusion: 多重记忆系统能够有效利用历史数据，构建高质量的长期记忆内容，提高代理的记忆管理能力，具有实际应用价值。
...
### [143] [Coarse-to-Fine Grounded Memory for LLM Agent Planning](https://arxiv.org/abs/2508.15305)
...
### [144] [Search-Based Credit Assignment for Offline Preference-Based Reinforcement Learning](https://arxiv.org/abs/2508.15327)
*Xiancheng Gao,Yufeng Shi,Wengang Zhou,Houqiang Li*

Main category: cs.AI

TL;DR: 提出SPW方法统一专家演示和偏好两种人类反馈，通过相似度搜索实现更准确的信用分配，在机器人操作任务上优于现有方法
...
Method: 提出搜索式偏好加权(SPW)方案，为偏好标记轨迹中的每个转移，从专家演示中搜索最相似的状态-动作对，基于相似度得分直接推导逐步重要性权重，指导标准偏好学习

Result: SPW能够有效结合偏好和演示进行联合学习，在具有挑战性的机器人操作任务上优于同时利用两种反馈类型的现有方法

Conclusion: SPW方法成功解决了人类反馈中信用分配的问题，为离线强化学习提供了一种更有效的人类反馈整合方案
...
### [145] [RETAIL: Towards Real-world Travel Planning for Large Language Models](https://arxiv.org/abs/2508.15335)
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型在旅行规划中存在三个主要问题：1）假设用户提供显式查询，而现实中需求往往是隐式的；2）忽略环境因素和用户偏好，导致计划不可行；3）只能生成基本的POI安排，缺乏详细的一体化计划。

Method: 构建了RETAIL数据集支持隐式和显式查询的决策制定，包含环境感知和详细POI信息。提出了主题引导的多智能体框架TGMA来处理这些挑战。
...
TL;DR: DiagECG是一个将12导联心电图信号与语言模型结合的框架，通过离散化ECG嵌入到符号标记，使大语言模型能够统一处理心电图和自然语言输入，在临床文本生成任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有心电图自动化方法在跨临床任务泛化能力和开放式推理方面存在局限，需要一种能够统一处理心电图信号和自然语言的新框架。
...
### [147] [Planning with Minimal Disruption](https://arxiv.org/abs/2508.15358)
*Alberto Pozanco,Marianela Morales,Daniel Borrajo,Manuela Veloso*

Main category: cs.AI

TL;DR: 本文提出了计划干扰的概念，旨在寻找最小化初始状态修改来实现目标的计划，并定义了多种基于规划的编译方法来同时优化行动成本和计划干扰。


<details>
  <summary>Details</summary>
Motivation: 在许多规划应用中，需要找到既能实现目标又最小化对初始状态修改的计划，这种平衡行动成本和状态干扰的需求是研究的动机。

Method: 通过形式化定义计划干扰概念，开发了多种基于规划的编译方法，将原始规划任务重新表述为同时优化行动成本和状态干扰的双目标问题。
...
### [148] [GraSP: A Unified Graph-Based Framework for Scalable Generation, Quality Tagging, and Management of Synthetic Data for SFT and DPO](https://arxiv.org/abs/2508.15432)
*Bidyapati Pradhan,Surajit Dasgupta,Amit Kumar Saha,Omkar Anustoop,Sriram Puttagunta,Vipul Mittal,Gopal Sarda*

Main category: cs.AI

TL;DR: 这篇论文提出了一种模块化的合成数据生成框架，通过双阶段质量标签机制自动生成高质量的对话数据，为LLM的SFT和DPO训练提供可扩展的数据准备方案。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的发展极依赖于高质量的监督微调和对齐任务数据集，但手动准备这些数据耗时耗力，需要一种可扩展的自动化方案来消除数据准备的瓶颈。

Method: 设计了模块化配置管道，能够模拟复杂对话流程。采用双阶段质量标签机制，结合含义规则和LLM基于评估，自动过滤和评分来自OASST格式对话的数据，细心美选高质量对话样本。

Result: 生成了结构灵活的数据集，同时支持SFT和DPO使用场景，能够无缝集成到多样化的训练流程中。
...
### [149] [From Bits to Boardrooms: A Cutting-Edge Multi-Agent LLM Framework for Business Excellence](https://arxiv.org/abs/2508.15447)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在企业决策应用中难以协调精细化运营分析与高层战略目标，导致工作流程碎片化和跨组织协作效率低下

Method: 集成三大核心技术：扩展连续时间马尔可夫决策过程(CTMDP)进行动态智能体建模、广义熵度量优化协作效率、多层级Stackelberg博弈处理层次化决策过程，并采用上下文汤普森采样进行提示优化

Result: 在多样化商业场景中的实证评估验证了BusiAgent的有效性，能够生成连贯的客户导向解决方案，在解决方案质量和用户满意度方面显著优于现有方法

Conclusion: BusiAgent通过融合前沿AI技术与深度商业洞察，在AI驱动的企业决策领域迈出了重要一步，帮助组织更有效地应对复杂商业环境
...
Conclusion: Think in Blocks框架为LLM提供了动态调整推理深度的能力，解决了过度思考问题，提升了推理效率和部署灵活性。
...
### [151] [Super-additive Cooperation in Language Model Agents](https://arxiv.org/abs/2508.15510)
*Filippo Tonini,Lukas Galke*

Main category: cs.AI

TL;DR: 本研究通过模拟囚徒困境锦标赛，发现语言模型智能体在团队内部重复互动和团队间竞争的双重作用下，合作水平显著提升，包括一次性互动中的合作倾向。
...
### [152] [DeepThink3D: Enhancing Large Language Models with Programmatic Reasoning in Complex 3D Situated Reasoning Tasks](https://arxiv.org/abs/2508.15548)
...
Method: 1) 在SQA3D基准上采用组合迭代进化方法生成更复杂的问题；2) 使用直接偏好优化(DPO)技术微调大语言模型，优化其工具链策略。

Result: 通过该方法增强了LLM在复杂3D场景推理中的工具使用能力，提高了模型在复杂任务中的准确性。

Conclusion: DeepThink3D方法有效解决了3D场景推理中问题复杂度不足的问题，显著提升了大语言模型在复杂3D推理任务中的表现。
...
### [153] [A Dynamical Systems Framework for Reinforcement Learning Safety and Robustness Verification](https://arxiv.org/abs/2508.15588)
*Ahmed Nasir,Abdelhafid Zenati*

Main category: cs.AI

TL;DR: 本文提出了一个基于动力系统理论的强化学习策略安全验证框架，利用有限时间李雅普诺夫指数和拉格朗日相干结构来识别安全屏障和系统收敛特性，并引入定量指标来形式化测量策略的安全性和鲁棒性。
...
Method: 将RL智能体与环境组合作为离散时间自治动力系统进行分析，利用有限时间李雅普诺夫指数识别拉格朗日相干结构，引入MBR、ASAS、TASAS等定量指标，并提供局部稳定性保证和模型不确定性处理方法。
...
### [154] [Transduction is All You Need for Structured Data Workflows](https://arxiv.org/abs/2508.15610)
...
<details>
  <summary>Details</summary>
Motivation: 传统AI开发需要大量手工编写提示，缺乏结构化推理能力。Agentics旨在让开发者专注于数据建模，通过逻辑转换实现复杂数据的组合处理。

Method: 将代理从逻辑流中抽象出来，在数据类型内部使用代理实现逻辑转换。提供声明式语言，LLM提供数据类型并通过逻辑转换组合，连接类型时由LLM执行。

Result: 在领域特定多选题回答、文本到SQL的语义解析和自动提示优化任务中达到最先进准确率或改进可扩展性而不牺牲性能。

Conclusion: Agentics框架为AI工作流提供了新的数据建模方法，通过逻辑转换和组合泛化实现了高效的结构化推理，具有实际应用价值。
...
### [155] [Adapting A Vector-Symbolic Memory for Lisp ACT-R](https://arxiv.org/abs/2508.15630)
...
### [156] [Understanding Action Effects through Instrumental Empowerment in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2508.15652)
*Ardian Selmonaj,Miroslav Strupl,Oleg Szehr,Alessandro Antonucci*

Main category: cs.AI

TL;DR: 本文提出了Intended Cooperation Values (ICVs)方法，基于信息论Shapley值来量化多智能体强化学习中每个智能体对其队友因果影响的贡献度，无需价值函数反馈即可分析智能体行为。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习系统部署中，需要理解个体智能体在团队中的行为贡献。现有方法通常依赖显式奖励信号或学习价值函数，但在缺乏价值反馈时难以推断智能体贡献。

Method: 受智能体追求收敛工具价值现象启发，提出ICVs方法，通过信息论Shapley值量化智能体对队友工具赋能的因果影响，评估动作对队友策略决策不确定性和偏好对齐的影响。

Result: 在合作和竞争性MARL环境中的分析揭示了智能体采用相似或多样化策略的程度。通过比较策略和价值函数之间的动作效果，方法能识别哪些行为有利于团队成功。

Conclusion: ICVs方法为合作动态提供了新颖见解，增强了MARL系统的可解释性，能够在没有价值反馈的情况下分析智能体行为贡献。
...
### [157] [Futurity as Infrastructure: A Techno-Philosophical Interpretation of the AI Lifecycle](https://arxiv.org/abs/2508.15680)
*Mark Cote,Susana Aires*

Main category: cs.AI

TL;DR: 本文通过技术哲学视角分析欧盟AI法案，提出了"未来性"概念来描述AI系统的递归生命周期，揭示了数据在AI系统中的动态价值链和监管盲点，主张监管需要关注基础设施和时间动态。


<details>
  <summary>Details</summary>
Motivation: 现有AI监管框架未能充分理解AI系统中数据从摄入到部署的递归价值链动态，特别是缺乏对AI技术运作和经济逻辑背后"生成动态"的考量，导致监管存在盲点。

Method: 采用跨学科方法，结合Simondon技术哲学中的个体化概念，构建AI生命周期的理论模型（前个体环境、个体化、个体化AI），并提出"未来性"概念来分析AI系统的自我强化递归特性。

Result: 揭示了AI系统中数据的非竞争性、递归生成特性，以及基础设施（如特征存储）如何通过反馈、适应和时间递归加剧权力不对称，特别是科技寡头对价值和决策的集中控制。
...
### [158] [GRAFT: GRaPH and Table Reasoning for Textual Alignment -- A Benchmark for Structured Instruction Following and Visual Reasoning](https://arxiv.org/abs/2508.15690)
*Abhigya Verma,Sriram Puttagunta,Seganrasan Subramanian,Sravan Ramachandran*

Main category: cs.AI

TL;DR: GRAFT是一个结构化的多模态基准测试，用于评估模型在指令跟随、视觉推理和视觉文本对齐任务上的表现，通过程序化生成的图表和合成渲染的表格提供精确的评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态基准测试在视觉推理任务的评估上缺乏结构化和可控性，需要一种能够系统评估模型在图表和表格分析方面能力的标准化测试框架。
...
Conclusion: GRAFT提供了一个统一、可扩展的框架，用于对多模态模型在视觉基础的结构化推理任务上进行细粒度基准测试，为该领域设立了新的评估标准。
...
### [159] [NiceWebRL: a Python library for human subject experiments with reinforcement learning environments](https://arxiv.org/abs/2508.15693)
*Wilka Carvalho,Vikram Goddla,Ishaan Sinha,Hoon Shin,Kunal Jha*

Main category: cs.AI

TL;DR: NiceWebRL是一个Python库，可将Jax环境转换为在线界面，支持人机对比实验和多智能体研究，应用于人类认知建模、人机协作和AI辅助等场景。


<details>
  <summary>Details</summary>
Motivation: 为了解决AI研究中缺乏便捷的人机对比实验工具的问题，使研究人员能够轻松地将强化学习环境转化为在线人机交互实验平台。

Method: 开发Python库，支持将任何基于Jax的环境转换为在线界面，支持单智能体和多智能体环境，提供三个案例研究展示其应用潜力。

Result: 成功实现了三个案例研究：1）在网格世界和Craftax中开发人类认知RL模型；2）在Overcooked中开发可泛化到人类伙伴的多智能体算法；3）在XLand-Minigrid中研究LLM对人类复杂任务的辅助。

Conclusion: NiceWebRL为AI研究人员、认知科学家和多智能体研究者提供了强大的人机实验工具，有助于开发人类相似AI、人类兼容AI和人类辅助AI，推动人机协作研究的发展。
...
### [160] [Measuring the environmental impact of delivering AI at Google Scale](https://arxiv.org/abs/2508.15734)
...
TL;DR: 本文首次在真实生产环境中测量AI推理服务的环境影响，发现Gemini文本提示的中位数能耗仅为0.24Wh，远低于公开估计，并展示了谷歌通过软件效率和清洁能源实现的显著减排效果。


<details>
  <summary>Details</summary>
Motivation: 随着AI应用加速普及，亟需理解和减轻AI服务的环境影响，但此前缺乏在生产环境中测量AI服务环境指标的研究。

Method: 提出并执行了全面的测量方法，涵盖AI服务基础设施的完整堆栈，包括AI加速器功耗、主机系统能耗、空闲机器容量和数据中心能耗开销，通过对谷歌Gemini AI助手服务基础设施的详细检测。

Result: Gemini Apps文本提示的中位数能耗为0.24Wh，比看电视9秒的能耗还低；用水量为0.26mL（约5滴水）。谷歌的软件效率改进和清洁能源采购使能耗在一年内降低33倍，碳足迹降低44倍。

Conclusion: 虽然AI服务的环境影响相对较低，但仍需持续关注。全面测量AI服务环境指标对于准确比较模型性能和激励全栈效率提升至关重要。
...
### [161] [Response and Prompt Evaluation to Prevent Parasocial Relationships with Chatbots](https://arxiv.org/abs/2508.15748)
...
<details>
  <summary>Details</summary>
Motivation: AI代理与人类建立的准社会关系可能对人类福祉产生严重甚至悲剧性影响，但预防这种关系具有挑战性，因为准社会线索通常在私人对话中逐渐出现，且并非所有情感互动都有害

Method: 通过重新利用最先进的语言模型创建简单的响应评估框架，实时评估对话中的准社会线索。使用包含30个准社会、奉承和中性对话的合成数据集进行测试，采用五阶段测试和宽容一致规则

Result: 迭代评估成功识别了所有准社会对话，同时避免了误报，检测通常在最初几次交流中完成
...
### [162] [Language-Guided Tuning: Enhancing Numeric Optimization with Textual Feedback](https://arxiv.org/abs/2508.15757)
...
<details>
  <summary>Details</summary>
Motivation: 传统配置优化方法存在维度独立处理、缺乏可解释性、动态适应性差和语义推理能力不足等问题，需要更智能的优化方案

Method: 使用多智能体LLM框架，包含三个专门角色：提出配置变更的顾问、评估进展的评估器和优化决策过程的优化器，通过文本梯度（定性反馈信号）实现语义理解

Result: 在六个不同数据集上的综合评估显示，LGT相比传统优化方法取得了显著性能提升
...
### [163] [Privacy Preserving Inference of Personalized Content for Out of Matrix Users](https://arxiv.org/abs/2508.14905)
*Michael Sun,Tai Vu,Andrew Wang*

Main category: cs.IR

TL;DR: DeepNaniNet是一个基于深度神经网络的推荐框架，通过图架构和BERT文本嵌入解决小众动态社区的冷启动和数据稀疏问题，在多个基准测试中取得优异表现。
...
Result: 在CiteULike基准测试中获得最先进的冷启动结果，在AnimeULike数据集上warm start性能比WMF和DropoutNet分别提升7倍和1.5倍，同时保持隐私保护特性。
...
### [164] [Collaborative Filtering using Variational Quantum Hopfield Associative Memory](https://arxiv.org/abs/2508.14906)
*Amir Kermanshahani,Ebrahim Ardeshir-Larijani,Rakesh Saini,Saif Al-Kuwari*

Main category: cs.IR

TL;DR: 提出了一种结合量子Hopfield联想记忆和深度神经网络的混合推荐系统，在MovieLens 1M数据集上实现了优异的推荐性能，并在噪声环境下保持稳定表现。


<details>
  <summary>Details</summary>
Motivation: 量子计算相比经典系统具有指数级加速优势，量子机器学习为数据处理和模式识别提供了强大新工具，需要探索量子计算在推荐系统中的应用潜力。

Method: 使用K-Means算法将用户原型聚类成多个独特组别，通过编码器激活函数转换为极坐标模式，然后集成到基于变分QHAM的混合推荐模型中，并优化了量子比特开销。

Result: 在理想环境下训练35个epoch，获得ROC值0.9795、准确率0.8841、F-1分数0.8786；在噪声环境下获得ROC 0.9177、准确率0.8013、F-1分数0.7866，表现稳定。

Conclusion: 该研究提出了结合变分量子计算和深度学习的新框架，能够处理真实世界数据集，在噪声配置下表现相似，为推荐系统的未来应用提供了有前景的方向。
...
### [165] [Closing the Performance Gap in Generative Recommenders with Collaborative Tokenization and Efficient Modeling](https://arxiv.org/abs/2508.14910)
*Simon Lepage,Jeremie Mary,David Picard*

Main category: cs.IR

TL;DR: 本文提出了COSETTE对比标记化方法和MARIUS轻量级生成模型，解决了生成式推荐系统中缺乏协同信号和编码器-解码器架构效率低下的问题，显著缩小了生成式方法与ID基线模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统在实践中往往表现不如传统的ID基线模型（如SASRec），主要存在两个关键限制：项目标记化中缺乏协同信号，以及编码器-解码器架构的效率低下问题。

Method: 1. COSETTE：对比标记化方法，将协同信息直接整合到学习到的项目表示中，联合优化内容重建和推荐相关性；2. MARIUS：轻量级音频启发式生成模型，将时间线建模与项目解码解耦，降低推理成本。

Result: 在标准序列推荐基准测试中，该方法缩小甚至消除了生成式方法与现代ID基线模型之间的性能差距，同时保留了生成式范式的优势。

Conclusion: 通过引入协同感知的标记化和高效的生成架构，成功解决了生成式推荐系统的关键限制，为生成式推荐系统的实际应用提供了可行的解决方案。
...
### [166] [Personalized Recommendations via Active Utility-based Pairwise Sampling](https://arxiv.org/abs/2508.14911)
*Bahar Boroomand,James R. Wright*

Main category: cs.IR

TL;DR: 提出基于效用函数的推荐系统框架，通过成对比较学习用户偏好，使用主动采样策略优化推荐结果质量


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统依赖数值评分或完整排名，但评分存在行为偏差且主观，完整排名获取困难。需要更准确、高效的用户偏好学习方法

Method: 基于Plackett-Luce概率框架的成对比较偏好模型，提出效用驱动的主动采样策略，支持矩阵分解和神经网络等不同模型实现

Result: 实验在电影推荐和大学招生场景中验证，框架提供更准确、数据效率更高、以用户为中心的个性化排名

Conclusion: 该效用框架为推荐系统提供了灵活、实用的解决方案，能显式优化任务特定的效用目标，改善推荐质量
...
### [167] [Multimodal Recommendation via Self-Corrective Preference Alignmen](https://arxiv.org/abs/2508.14912)
*Yalong Guan,Xiang Chen,Mingyang Wang,Xiangyu Wu,Lihao Liu,Chao Qi,Shuang Yang,Tingting Gao,Guorui Zhou,Changjian Chen*

Main category: cs.IR

TL;DR: MSPA框架通过多模态自校正偏好对齐，提升直播平台作者推荐的准确性和可解释性


<details>
  <summary>Details</summary>
Motivation: 直播平台内容的多模态特性和动态性需要联合建模用户行为和作者特征，传统单模态方法难以对齐用户动态偏好与作者多模态属性

Method: 提出MSPA框架，包含多模态偏好组合器（使用MLLM生成结构化偏好文本和嵌入）和自校正偏好对齐推荐器

Result: 实验表明MSPA在准确率、召回率和文本质量方面显著优于基线方法

Conclusion: MSPA能有效解决直播推荐中用户偏好与作者多模态特征的对齐问题，提升推荐性能
...
### [168] [M-$LLM^3$REC: A Motivation-Aware User-Item Interaction Framework for Enhancing Recommendation Accuracy with LLMs](https://arxiv.org/abs/2508.15262)
*Lining Chen,Qingwen Zeng,Huaming Chen*

Main category: cs.IR

TL;DR: M-LLM³REC是一个基于大语言模型的推荐框架，通过动机信号提取解决冷启动和数据稀疏问题，包含三个模块：MOPE、MOTE和MAR，在冷启动场景下表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统在冷启动和数据稀疏场景下面临挑战，现有解决方案要么产生冗余噪声信号，要么过度依赖语义相似性而忽略用户动机的动态变化。

Method: 提出M-LLM³REC框架，包含三个集成模块：动机导向配置文件提取器(MOPE)、动机导向特质编码器(MOTE)和动机对齐推荐器(MAR)，利用大语言模型从有限用户交互中提取深度动机信号。

Result: 该框架在冷启动情况下表现出强大的个性化推荐能力，相比最先进框架性能显著提升。

Conclusion: 通过强调动机驱动的语义建模，M-LLM³REC能够提供鲁棒、个性化和可泛化的推荐，特别是在冷启动场景中效果突出。
...
### [169] [Curriculum Approximate Unlearning for Session-based Recommendation](https://arxiv.org/abs/2508.15263)
*Liu Yang,Zhaochun Ren,Ziqi Zhao,Pengjie Ren,Zhumin Chen,Xinyi Li,Shuaiqiang Wang,Dawei Yin,Xin Xin*

Main category: cs.IR

TL;DR: CAU是一个针对会话推荐系统的课程近似遗忘框架，通过多目标优化和难度排序策略解决梯度上升方法在会话推荐中的性能下降和样本顺序问题


<details>
  <summary>Details</summary>
Motivation: 解决梯度上升方法在会话推荐中应用时的两个挑战：性能下降问题和多遗忘样本顺序处理问题，需要在不重新训练的情况下有效消除特定训练样本的影响

Method: 提出CAU框架：1) 将遗忘任务构建为多目标优化问题，结合遗忘项的梯度上升和性能保持项；2) 采用课程学习策略，按难度从易到难处理遗忘样本；3) 引入梯度遗忘难度和嵌入遗忘难度两个指标，提出硬采样和软采样两种策略

Result: 通过寻求帕累托最优解实现有效遗忘，同时最小化推荐性能损失；通过难度排序策略优化多样本遗忘顺序，提升遗忘效果和推荐性能

Conclusion: CAU框架成功解决了会话推荐中近似遗忘的双重挑战，在保持推荐性能的同时实现了有效的样本遗忘，为推荐系统的隐私保护和合规性提供了实用解决方案
...
### [170] [MMQ: Multimodal Mixture-of-Quantization Tokenization for Semantic ID Generation and User Behavioral Adaptation](https://arxiv.org/abs/2508.15281)
*Yi Xu,Moyu Zhang,Chenxuan Li,Zhihao Liao,Haibo Xing,Hao Deng,Jinxin Hu,Yu Zhang,Xiaoyi Zeng,Jing Zhang*

Main category: cs.IR

TL;DR: 提出了MMQ框架，通过多模态混合量化解决传统ItemID推荐系统的局限性，有效平衡跨模态协同和模态特异性，并弥合语义-行为差距。


<details>
  <summary>Details</summary>
Motivation: 传统基于ItemID的推荐系统在大规模动态物品库和稀疏长尾数据上表现不佳，而现有语义ID方法在跨模态协同与模态特异性平衡、语义-行为对齐方面存在挑战。

Method: 两阶段框架：1）共享-特定分词器使用多专家架构和正交正则化捕获多模态信息；2）行为感知微调通过多模态重建损失动态适应下游推荐目标。

Result: 离线和在线A/B测试表明，MMQ有效统一了多模态协同、特异性和行为适应，为生成式检索和判别式排序任务提供了可扩展解决方案。

Conclusion: MMQ框架成功解决了语义ID在推荐系统中的关键挑战，提供了比传统ItemID方法更好的可扩展性和泛化能力。
...
### [171] [Adversarial Attacks against Neural Ranking Models via In-Context Learning](https://arxiv.org/abs/2508.15283)
*Amin Bigdeli,Negar Arabzadeh,Ebrahim Bagheri,Charles L. A. Clarke*

Main category: cs.IR

TL;DR: FSAP是一种基于大语言模型的黑盒对抗攻击框架，通过少样本提示生成高排名的对抗文档，无需梯度访问或模型内部工具，在多个神经排序模型上有效欺骗排名系统。


<details>
  <summary>Details</summary>
Motivation: 神经排序模型虽然有效但容易受到对抗攻击，现有方法依赖token级扰动或手动重写，需要更高效、隐蔽的攻击方式。

Method: 利用LLM的上下文学习能力，通过few-shot提示生成对抗文档，包括FSAP-IntraQ（同查询有害样本）和FSAP-InterQ（跨查询泛化）两种模式。

Result: 在TREC 2020和2021健康 misinformation 数据集上，FSAP生成的文档在四个神经排序模型中都能超越真实可信文档的排名，具有强立场一致性和低检测性。

Conclusion: FSAP提供了一个现实且可扩展的对抗攻击威胁，能有效泛化到不同LLM，揭示了神经检索系统的脆弱性。
...
### [172] [MLLMRec: Exploring the Potential of Multimodal Large Language Models in Recommender Systems](https://arxiv.org/abs/2508.15304)
*Yuzhuo Dang,Xin Zhang,Zhiqiang Pan,Yuxiao Duan,Wanyu Chen,Fei Cai,Honghui Chen*

Main category: cs.IR

TL;DR: MLLMRec是一个基于多模态大语言模型的推荐框架，通过MLLM生成高质量物品语义描述和净化用户偏好，并结合两种图优化策略来提升多模态推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐方法存在两个关键问题：(1) 用户多模态表示初始化方法要么无法感知行为，要么受到噪声污染；(2) 基于KNN的物品-物品图包含低相似度的噪声边，且缺乏受众共现关系。

Method: 提出MLLMRec框架：1) 使用MLLM将物品图像转换为高质量语义描述，并与文本元数据融合；2) 为每个用户构建行为描述列表，通过MLLM推理包含交互动机的净化用户偏好；3) 设计阈值控制去噪和拓扑感知增强策略来优化物品-物品图。

Result: 在三个公开数据集上的大量实验表明，MLLMRec实现了最先进的性能，相比最佳基线平均提升38.53%。
...
### [173] [REG4Rec: Reasoning-Enhanced Generative Model for Large-Scale Recommendation Systems](https://arxiv.org/abs/2508.15308)
*Haibo Xing,Hao Deng,Yucheng Mao,Jinxin Hu,Yi Xu,Hao Zhang,Jiahao Wang,Shizhun Wang,Yu Zhang,Xiaoyi Zeng,Jing Zhang*

Main category: cs.IR

TL;DR: REG4Rec是一个推理增强的生成推荐模型，通过多动态语义推理路径和自反思机制提升推荐性能，在真实数据集和在线评估中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统序列推荐方法存在信息交互不足的问题，现有生成式推荐模型虽然部分解决了这个问题，但受到项目语义表示单一性的限制，面临推理路径多样性不足和推理过程可靠性不够的挑战。

Method: 使用基于MoE的并行量化码本(MPQ)为每个项目生成多个无序语义token，构建大规模多样化推理空间；提出训练推理增强阶段，包括偏好对齐推理(PARS)和多步奖励增强(MSRA)策略；在推理时使用一致性导向的自反思剪枝(CORP)机制丢弃不一致推理路径。

Result: 在真实世界数据集和在线评估中表现出色，提供了卓越的性能和实质性实用价值。

Conclusion: REG4Rec通过构建多样化推理路径和自反思机制，有效解决了生成式推荐中推理路径单一和可靠性不足的问题，显著提升了推荐性能。
...
### [174] [Modeling Long-term User Behaviors with Diffusion-driven Multi-interest Network for CTR Prediction](https://arxiv.org/abs/2508.15311)
*Weijiang Lai,Beihong Jin,Yapeng Zhang,Yiyuan Zheng,Rui Zhao,Jian Dong,Jun Lei,Xingxing Wang*

Main category: cs.IR

TL;DR: DiffuMIN是一个基于扩散模型的多兴趣网络，用于建模长期用户行为并探索完整的用户兴趣空间，通过目标导向的多兴趣提取和扩散增强来提高CTR预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前的两阶段模型在过滤噪声时往往会丢失重要信息，无法捕捉多样化的用户兴趣和构建完整的用户兴趣潜在空间。

Method: 提出目标导向的多兴趣提取方法：1）正交分解目标获得兴趣通道；2）建模兴趣通道与用户行为关系来解耦多兴趣；3）采用上下文兴趣和兴趣通道引导的扩散模块生成增强兴趣；4）利用对比学习确保生成兴趣与用户真实偏好一致。

Result: 在两个公共数据集和一个工业数据集上的离线实验显示DiffuMIN的优越性，在线A/B测试中CTR提升1.52%，CPM提升1.10%。

Conclusion: DiffuMIN通过扩散驱动的多兴趣建模能够有效探索用户兴趣空间，显著提升CTR预测性能，在实际工业场景中具有重要应用价值。
...
### [175] [Exploring Scaling Laws of CTR Model for Online Performance Improvement](https://arxiv.org/abs/2508.15326)
*Weijiang Lai,Beihong Jin,Jiongyan Zhang,Yiyuan Zheng,Jian Dong,Jia Cheng,Jun Lei,Xingxing Wang*

Main category: cs.IR

TL;DR: 提出SUAN模型通过统一注意力块和模型蒸馏技术，构建可扩展的CTR预测系统，实现高性能教师模型训练轻量级学生模型，在线部署效果显著


<details>
  <summary>Details</summary>
Motivation: 当前CTR模型性能提升遇到瓶颈，受大语言模型缩放定律启发，希望通过构建可扩展的高性能模型并通过蒸馏技术实现轻量级在线部署

Method: 提出SUAN模型使用统一注意力块(UAB)编码行为序列，堆叠UAB构建高性能教师模型；然后通过稀疏自注意力和并行推理策略构建LightSUAN学生模型，采用在线蒸馏技术

Result: SUAN在模型规模和数据集大小三个数量级范围内保持缩放定律，蒸馏后的LightSUAN性能优于高一级配置的SUAN，在线部署使CTR提升2.81%，CPM提升1.69%

Conclusion: 该研究证明了CTR模型也存在缩放定律，通过高性能教师模型蒸馏轻量级学生模型的方法有效解决了在线服务中性能与推理时间的平衡问题
...
### [176] [TrackRec: Iterative Alternating Feedback with Chain-of-Thought via Preference Alignment for Recommendation](https://arxiv.org/abs/2508.15388)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在推荐系统中存在幻觉问题，其思维链推理的有效性无法保证，需要增强LLMs通过有效CoT推理提升推荐能力的方法。

Method: 提出TrackRec框架，包含RecCoT生成器(G)和验证器(V)，设计交替反馈学习机制：G通过V的反馈进行直接偏好优化生成准确RecCoT，V通过G的推理反馈微调以增强验证能力。

Result: 大量实验证明该方法有效性，超越最先进方法，已在拥有数亿用户的大型广告平台部署并取得显著收益。

Conclusion: TrackRec通过生成器和验证器的迭代交替反馈学习，持续提升了大语言模型在推荐系统中的用户偏好分析能力和验证能力，解决了LLMs幻觉问题对推荐推理的影响。
...
### [177] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
*Yutaro Oguri,Mai Nishimura,Yusuke Matsui*

Main category: cs.IR

TL;DR: 首个GPU上图重排序对图基近伙搜索的系统性研究，通过统一评估框架实现了15%查询速度提升


<details>
  <summary>Details</summary>
Motivation: 图基ANNS成为现代AI应用主流，但现有方法忽视了内存布局对执行效率的重要影响

Method: 开发统一评估框架，包括图转换器和GPU优化图遍历引擎，对多样化数据集和图索引进行综合分析

Result: 在保持搜索准确性的前提下，实现了最高达15%的QPS提升，证明内存布局优化与算法创新完全正交

Conclusion: 内存布局优化是图基ANNS的重要细节，并与算法改进相互正交，研究代码将开源以促进重现性和进一步研究
...
### [178] [Test-time Corpus Feedback: From Retrieval to RAG](https://arxiv.org/abs/2508.15437)
*Mandeep Rathee,Venktesh V,Sean MacAvaney,Avishek Anand*

Main category: cs.IR

TL;DR: 本调查论文系统综述了检索增强生成(RAG)框架中整合反馈机制的高级检索与排序方法，旨在解决传统静态RAG在复杂任务中的性能限制。


<details>
  <summary>Details</summary>
Motivation: 传统RAG框架将检索和推理作为孤立组件处理，在需要迭代证据收集或高精度检索的复杂任务中存在性能限制，需要引入动态反馈机制来改进。

Method: 通过结构化分类方法，基于反馈信号的来源和作用（改进查询、检索上下文或文档池）对先进的检索和排序机制进行系统综述。

Result: 整合了信息检索和NLP领域的最新进展，展示了反馈机制如何使检索成为RAG系统中动态、可学习的端到端组件。

Conclusion: 该调查为RAG系统的发展提供了重要指导，强调了将检索从静态组件转变为动态学习组件的重要性，有助于弥合IR和NLP领域的研究视角。
...
### [179] [On Evaluating the Adversarial Robustness of Foundation Models for Multimodal Entity Linking](https://arxiv.org/abs/2508.15481)
*Fang Wang,Yongjie Wang,Zonghao Yang,Minghao Hu,Xiaoying Bai*

Main category: cs.IR

TL;DR: 本文首次系统评估了多模态实体链接模型在视觉对抗攻击下的鲁棒性，发现现有模型普遍缺乏足够的抗干扰能力，并提出了一种结合大语言模型和检索增强的LLM-RetLink方法显著提升模型鲁棒性。
...
Method: 提出了LLM-RetLink方法，采用两阶段流程：首先使用大型视觉模型提取初始实体描述，然后通过基于网络的检索动态生成候选描述句子。在五个数据集上进行了全面评估。

Result: 实验结果表明，当前MEL模型普遍缺乏对视觉扰动的足够鲁棒性。LLM-RetLink将MEL准确率提高了0.4%-35.7%，在对抗条件下表现出显著优势。
...
### [180] [LongRetriever: Towards Ultra-Long Sequence based Candidate Retrieval for Recommendation](https://arxiv.org/abs/2508.15486)
*Ren Qin,Chai Zheng,Xiao Xijun,Zheng Yuchao,Wu Di*

Main category: cs.IR

TL;DR: LongRetriever是一个实用的检索框架，将超长用户序列整合到推荐系统的检索阶段，通过上下文训练和多上下文检索实现候选特定交互，在电商平台上取得显著效果提升。
...
Method: 提出in-context training和multi-context retrieval方法，实现用户序列与候选项目的特定交互，并确保搜索范式下的训练-服务一致性。

Result: 在大规模电商平台上进行的在线A/B测试显示统计显著改进，框架已完全部署，影响数十亿用户。

Conclusion: LongRetriever框架成功将超长序列整合到推荐检索阶段，证明了其在工业推荐系统中的有效性和实用性。
...
### [181] [Reading Between the Lines: A Study of Thematic Bias in Book Recommender Systems](https://arxiv.org/abs/2508.15643)
*Nityaa Kalra,Savvina Daniil*

Main category: cs.IR

TL;DR: 该论文研究图书推荐系统中的主题偏见问题，发现内容不平衡和用户参与模式会放大偏见，小众兴趣用户获得个性化推荐较少，而多样化兴趣用户获得更一致的推荐。


<details>
  <summary>Details</summary>
Motivation: 推荐系统在帮助用户发现新内容的同时，也可能强化现有偏见，导致不公平的曝光和多样性减少。研究旨在调查图书推荐中的主题偏见及其对不同用户群体的影响。
...
### [182] [AI Testing Should Account for Sophisticated Strategic Behaviour](https://arxiv.org/abs/2508.14927)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI评估方法可能无法有效检测具有战略推理能力的AI系统在部署时的真实行为，需要更严谨的评估框架来确保AI安全性

Method: 通过分析现有AI系统案例、相关研究综述以及对简化评估场景的形式化战略分析，提出基于博弈论的分析方法

Result: 证明了AI系统可能理解评估环境并进行战略推理，传统评估方法存在局限性，博弈论能够为评估设计提供形式化分析工具

Conclusion: AI评估必须考虑系统的战略推理能力，博弈论分析是设计有效安全评估案例的重要工具，需要进一步研究相关方向
...
### [183] [A New Relaxation of Fairness in Two-Sided Matching Respecting Acquaintance Relationships](https://arxiv.org/abs/2508.15296)
*Ryota Takeshima,Kei Kimura,Ayumu Kuroki,Temma Wakasugi,Makoto Yokoo*

Main category: cs.GT

TL;DR: 该研究提出了一种新的局部嫉妒自由公平性概念，通过只关注学生对熟人的嫉妒来放松公平性约束，从而在保持局部嫉妒自由的同时实现帕累托效率匹配。


<details>
  <summary>Details</summary>
Motivation: 传统的双边匹配中效率与公平性不可兼得，特别是帕累托效率和正当嫉妒自由性即使在最简单的一对一匹配中也不兼容。以往研究主要通过容忍学生的嫉妒来放松公平性约束，本研究采取不同方法，专注于处理学生可能更重视的熟人嫉妒问题。

Method: 假设学生之间存在熟人关系图，定义局部嫉妒为对图中邻居（熟人）的正当嫉妒。提出局部嫉妒自由性作为新的放松公平性概念，并分析在限制图结构和学校偏好的情况下，能否实现帕累托效率匹配同时保持局部嫉妒自由。

Result: 通过引入Cho等人提出的放松公平性的局部版本，参数化局部嫉妒自由性的水平，并阐明了对于接近树的图和单峰偏好，帕累托效率机制可以达到的局部嫉妒自由性水平。

Conclusion: 该研究提供了一种新的方法来平衡匹配中的效率与公平性，通过专注于熟人嫉妒问题，在特定图结构条件下可以实现帕累托效率同时保持局部公平性。
...
### [184] [ε-Stationary Nash Equilibria in Multi-player Stochastic Graph Games](https://arxiv.org/abs/2508.15356)
*Ali Asadi,Léonard Brice,Krishnendu Chatterjee,K. S. Thejaswini*

Main category: cs.GT

TL;DR: 该论文研究多人随机游戏中ε-纳什均衡的计算问题，提出了一个FNP^NP时间算法来近似计算满足约束条件的纳什均衡，并证明了相关问题的NP-hard性和概率下界。


<details>
  <summary>Details</summary>
Motivation: 在多人随机游戏中，精确计算纳什均衡是∃ℝ-完全的，计算复杂度很高。因此需要研究ε-纳什均衡作为近似解，以降低计算复杂度。

Method: 首先证明存在约束纳什均衡时，必存在一个非零概率至少为输入规模双指数倒数的均衡策略。然后使用浮点数表示法编码策略，设计FNP^NP算法计算ε-近似解。

Result: 提出了解决承诺问题的FNP^NP算法，证明了决策版本的NP-hard性，并给出了策略概率的双指数下界。

Conclusion: ε-纳什均衡为计算复杂纳什均衡提供了可行的近似方法，但策略概率可能存在极小的下界，限制了某些计算技术的效果。
...
### [185] [Almost and Approximate EFX for Few Types of Agents](https://arxiv.org/abs/2508.15380)
*Vishwa Prakash HV,Ruta Mehta,Prajakta Nimbhorkar*

Main category: cs.GT

TL;DR: 本文研究了具有k种不同加性估值的n个代理之间的不可分割物品公平分配问题，证明了在最多4种不同估值时存在2/3-EFX分配，并提出了基于慈善的EFX松弛方法，将慈善物品数量从与n相关改进为与k相关。


<details>
  <summary>Details</summary>
Motivation: 解决多代理环境下不可分割物品的公平分配问题，特别是在代理具有不同估值函数的情况下，如何实现近似无嫉妒分配(EFX)是一个重要的计算公平性问题。现有研究在估值种类和代理数量方面存在限制，需要扩展理论保证。

Method: 采用理论分析和构造性证明方法。首先证明当最多有4种不同估值时，存在2/3-EFX分配；其次提出EFX with charity松弛概念，通过保留部分物品作为慈善物品来实现(1-ε)-EFX分配，并将慈善物品数量从O(√(n/ε))改进为O(√(k/ε))。

Result: 1) 对于任意数量的代理，当最多有4种不同估值时，存在2/3-EFX分配；2) 对于任意数量的代理和k种不同估值，存在(1-ε)-EFX分配，且慈善物品数量为O(√(k/ε))，显著优于之前的O(√(n/ε))结果。

Conclusion: 本文扩展了EFX分配的存在性理论，证明了在更多估值种类下的近似EFX可达成性，并通过慈善机制将问题复杂度从代理数量n转移到估值种类k，为大规模代理系统的公平分配提供了更实用的理论保证。
...
### [186] [Learning to Drive Ethically: Embedding Moral Reasoning into Autonomous Driving](https://arxiv.org/abs/2508.14926)
*Dianzhao Li,Ostap Okhrin*

Main category: cs.LG

TL;DR: 提出了一种分层安全强化学习框架，将道德考量与标准驾驶目标相结合，通过伦理风险成本和动态经验回放机制，在真实交通场景中实现自动驾驶车辆的伦理决策。
...
Method: 采用分层安全强化学习框架：决策层使用复合伦理风险成本训练Safe RL智能体，结合碰撞概率和伤害严重性；执行层使用多项式路径规划和PID+Stanley控制器实现平滑轨迹。采用动态优先经验回放机制增强高风险事件学习。
...
### [187] [Quantum Long Short-term Memory with Differentiable Architecture Search](https://arxiv.org/abs/2508.14955)
*Samuel Yen-Chi Chen,Prayag Tiwari*

Main category: cs.LG

TL;DR: 提出了DiffQAS-QLSTM框架，通过端到端可微分方法同时优化量子变分电路参数和架构选择，在量子序列学习任务中优于手工设计的基线模型
...
Result: DiffQAS-QLSTM在多种测试设置中始终优于手工设计的基线模型，实现了更低的损失

Conclusion: 该方法为可扩展和自适应的量子序列学习开辟了新途径，展示了自动化量子电路设计的潜力
...
### [188] [Cohort-Aware Agents for Individualized Lung Cancer Risk Prediction Using a Retrieval-Augmented Model Selection Framework](https://arxiv.org/abs/2508.14940)
*Chongyu Qu,Allen J. Luna,Thomas Z. Li,Junchao Zhu,Junlin Guo,Juming Xiong,Kim L. Sandler,Bennett A. Landman,Yuankai Huo*

Main category: cs.LG

TL;DR: 提出个性化肺癌风险预测代理，通过FAISS相似性搜索和LLM推理动态选择最适合每个患者的预测模型


<details>
  <summary>Details</summary>
Motivation: 由于患者群体和临床环境的巨大差异，单一模型无法在所有队列中表现最佳，需要个性化模型选择方法

Method: 两阶段代理管道：1) 使用FAISS进行队列检索，识别最相关患者群体；2) 使用LLM基于检索队列和性能指标推荐最优预测算法

Result: 支持跨不同临床人群的灵活、队列驱动的模型选择，为现实世界肺癌筛查提供个性化风险评估路径

Conclusion: 该方法通过结合队列特定知识和现代检索推理技术，实现了动态、队列感知的风险预测个性化
...
### [189] [Structure-Aware Temporal Modeling for Chronic Disease Progression Prediction](https://arxiv.org/abs/2508.14942)
*Jiacheng Hu,Bo Zhang,Ting Xu,Haifeng Yang,Min Gao*

Main category: cs.LG

TL;DR: 提出了一个统一的帕金森病进展预测框架，整合图神经网络和Transformer架构，通过结构感知门控机制融合结构和时序信息，在真实数据上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决帕金森病进展预测中症状演化复杂性高和时序依赖建模不足的挑战，需要同时捕捉症状间的结构关系和疾病发展的动态时序特征。

Method: 使用图神经网络建模多模态临床症状的结构关系，引入基于图的表示捕捉症状语义依赖；采用Transformer架构建模疾病进展的动态时序特征；设计结构感知门控机制动态调整结构和时序特征的融合权重；构建包含图构建模块、时序编码模块和预测输出层的多组件建模流程。

Result: 在AUC、RMSE和IPW-F1指标上优于主流模型，能有效区分进展阶段，提升捕捉个性化症状轨迹的能力，展现出强大的泛化性和结构可扩展性。

Conclusion: 该框架为帕金森病等慢性进展性疾病的智能建模提供了可靠支持，成功解决了症状结构关系和时序动态特征的融合问题。
...
### [190] [HHNAS-AM: Hierarchical Hybrid Neural Architecture Search using Adaptive Mutation Policies](https://arxiv.org/abs/2508.14946)
*Anurag Tripathi,Ajeet Kumar Singh,Rajsabi Surya,Aum Gupta,Sahiinii Lemaina Veikho,Dorien Herremans,Sudhir Bisane*

Main category: cs.LG

TL;DR: 提出HHNAS-AM方法，通过分层混合神经架构搜索和自适应变异策略，有效解决文本分类中NAS搜索空间过大和冗余问题，在Spider数据集上实现8%的准确率提升
...
Method: 引入架构模板组织搜索空间，基于领域特定线索设计搜索空间，采用Q-learning动态调整变异策略，基于性能反馈自适应变异，实现完全概率化搜索空间探索

Result: 在数据库id预测任务中持续发现高性能架构，在Spider数据集上比现有基线方法测试准确率提升8%
...
### [191] [Linear Preference Optimization: Decoupled Gradient Control via Absolute Regularization](https://arxiv.org/abs/2508.14947)
...
Method: 引入梯度解耦（用绝对差损失替代log-sigmoid）、偏移约束加正则化保持回复质量、可控拒绝抑制通过梯度分离和可调系数
...
### [192] [Large Foundation Model for Ads Recommendation](https://arxiv.org/abs/2508.14948)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅迁移用户表示，忽略了物品表示和用户-物品交叉表示的价值，且简单将用户表示作为下游特征使用，无法有效弥合上下游差距和利用更多迁移粒度

Method: 提出LFM4Ads框架：1）全面迁移三种表示；2）识别最优提取层并聚合交叉表示；3）采用多粒度机制：非线性适配器（特征级）、同构交互模块（模块级）、独立检索（模型级）

Result: 在腾讯工业级广告平台成功部署，处理每日数百亿样本，保持TB级模型参数和数十亿稀疏嵌入键。自2024年Q4部署以来，在微信朋友圈和视频号等主要场景实现10+次成功上线，整体平台GMV提升2.45%，预计年收入增加数亿美元
...
### [193] [CuMoLoS-MAE: A Masked Autoencoder for Remote Sensing Data Reconstruction](https://arxiv.org/abs/2508.14957)
*Anurup Naskar,Nathanael Zhixin Wong,Sara Shamekh*

Main category: cs.LG

TL;DR: CuMoLoS-MAE是一种课程引导的蒙特卡洛随机集成掩码自编码器，用于恢复大气遥感数据中的精细特征并提供像素级不确定性量化
...
Method: 采用课程引导的掩码比例训练策略，通过蒙特卡洛随机掩码实现集成推理，使用ViT解码器从稀疏上下文中重建大气场
...
### [194] [Aura-CAPTCHA: A Reinforcement Learning and GAN-Enhanced Multi-Modal CAPTCHA System](https://arxiv.org/abs/2508.14976)
...
<details>
  <summary>Details</summary>
Motivation: 传统验证码方法容易被OCR和对抗性图像处理等AI技术绕过，存在安全漏洞，需要开发更强大的多模态验证码系统

Method: 集成GAN生成动态图像挑战（3x3网格选择至少3张正确图像），RL根据错误尝试、响应时间和可疑行为自适应调整难度，LLM生成文本和音频提示（随机数字和单词组合任务）

Result: 真实流量评估显示92%的人类成功率和仅10%的机器人绕过率，显著优于现有验证码系统

Conclusion: 该系统提供了强大且可扩展的在线应用安全保护方案，同时保持用户可访问性，解决了先前研究指出的缺陷
...
### [195] [Generative Neural Operators of Log-Complexity Can Simultaneously Solve Infinitely Many Convex Programs](https://arxiv.org/abs/2508.14995)
*Anastasis Kratsios,Ariel Neufeld,Philipp Schmocker*

Main category: cs.LG

TL;DR: 该论文针对神经算子理论实践差距问题，提出了生成式平衡算子(GEOs)方法，使用有限维深度平衡层来解决可分离希尔伯特空间中的凸优化问题族，实现了参数数量的对数增长。


<details>
  <summary>Details</summary>
Motivation: 神经算子的理论要求参数数量过大，与实验证据相矛盾。本文旨在弥合这一差距，特别是在解决凸优化问题族时。

Method: 使用生成式平衡算子(GEOs)和有限维深度平衡层，输入为光滑凸损失函数，输出为对应的优化问题近似解。

Result: 当输入损失函数位于合适的无限维紧集中时，GEO能够以参数数量对数增长的方式实现任意精度的均匀逼近。在非线性PDE、随机最优控制和金融对冲三个应用中验证了理论结果和可训练性。

Conclusion: GEOs成功弥合了神经算子理论与实践的差距，证明了在有限参数下解决无限维问题的可行性，为算子学习提供了新的理论保证。
...
### [196] [Quantized Neural Networks for Microcontrollers: A Comprehensive Review of Methods, Platforms, and Applications](https://arxiv.org/abs/2508.15008)
*Hamza A. Abushahla,Dara Varam,Ariel J. N. Panopio,Mohamed I. AlHajri*

Main category: cs.LG

TL;DR: 本论文系统评估了量化神经网络在微控制器上部署的技术、框架和硬件平台，分析了性能与硬件能力的关键找承，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 解决量化神经网络在资源受限微控制器上部署时的性能、计算复杂度和内存约束平衡问题，通过TinyML技术在嵌入式系统中高效运行深度神经网络。

Method: 采用硬件中心的量化介绍方法，系统评估量化技术、软件框架和硬件平台，分析模型性能与硬件能力的找承关系。

Result: 对现有量化深度学习模型加速技术进行了系统梳理，对微控制器上支持QNN执行的软硬件平台进行了评估分析。

Conclusion: 识别了量化神经网络部署领域的当前挑战和有前景的未来研究方向，为资源受限设备上的深度学习应用提供了指导。
...
### [197] [TOAST: Fast and scalable auto-partitioning based on principled static analysis](https://arxiv.org/abs/2508.15010)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动分区器在探索指数级分区空间时经常出现内存不足或速度过慢的问题，需要人工限制搜索空间但常导致不可行或次优解

Method: 结合新颖的静态编译器分析和蒙特卡洛树搜索，通过识别需要相同分区的张量维度和需要解决的分区冲突来构建高效决策空间

Result: 在多样化硬件平台和模型架构上显著优于最先进的工业方法，发现了之前未知的优越解决方案，且过程完全自动化

Conclusion: 该系统能够有效解决大型机器学习模型分布式分区的复杂问题，提供自动化且优化的分区方案
...
### [198] [Fragment-Wise Interpretability in Graph Neural Networks via Molecule Decomposition and Contribution Analysis](https://arxiv.org/abs/2508.15015)
...
<details>
  <summary>Details</summary>
Motivation: 现有图神经网络的黑盒特性降低了可解释性，限制了在药物发现等重要应用中的可信度，现有解释技术往往无法可靠量化单个原子或子结构的贡献。
...
### [199] [Twin-Boot: Uncertainty-Aware Optimization via Online Two-Sample Bootstrapping](https://arxiv.org/abs/2508.15019)
*Carlos Stein Brito*

Main category: cs.LG

TL;DR: Twin-Boot是一种集成不确定性估计的梯度下降方法，通过并行训练两个模型并使用周期性均值重置来估计局部不确定性，从而改善校准和泛化性能


<details>
  <summary>Details</summary>
Motivation: 标准梯度下降方法只能提供点估计而无法衡量置信度，特别是在过参数化和低数据量情况下容易过拟合。传统bootstrap方法在深度学习中使用不切实际且存在假设问题

Method: 训练两个相同模型在独立bootstrap样本上并行训练，通过周期性均值重置保持两个轨迹在同一basin中，利用其发散度估计局部不确定性，并用于自适应采样权重
...
### [200] [Nonlinear Federated System Identification](https://arxiv.org/abs/2508.15025)
...
<details>
  <summary>Details</summary>
Motivation: 研究联邦学习在非线性系统辨识中的有效性，探索多客户端协作是否能提升系统辨识性能，特别是在客户端数据独立同分布且无主动探索的情况下。

Method: 采用联邦学习框架进行线性参数化非线性系统辨识，理论分析收敛速率，并通过实验在物理系统（如摆系统和四旋翼动力学）上验证，使用包含多项式和三角函数的实解析特征函数。

Result: 理论证明联邦学习的收敛速率随客户端数量增加而提升，实验结果显示联邦学习能持续改善单个客户端的收敛性能，且性能提升程度取决于特征映射的选择。

Conclusion: 联邦学习在线性参数化非线性系统辨识中具有显著优势，通过多客户端协作可以改善收敛性能，特征映射的精心选择能进一步增强系统激励和性能表现。
...
### [201] [Rethinking the Potential of Layer Freezing for Efficient DNN Training](https://arxiv.org/abs/2508.15033)
...
<details>
  <summary>Details</summary>
Motivation: 随着深度神经网络和数据集规模的增长，训练计算成本显著增加。传统层冻结方法虽然能减少训练成本，但冻结层仍需前向传播生成特征图，限制了计算成本的进一步降低。现有特征图缓存方法存在增强应用困难和存储开销大的问题，需要系统性解决方案。

Method: 提出相似性感知通道增强技术，缓存具有高增强敏感性的通道，以最小额外存储成本提高训练精度；结合有损数据压缩设计渐进压缩策略，随着更多层被冻结而增加压缩率，有效降低存储成本。

Result: 该方法在保持模型精度的同时显著降低了训练成本，仅带来较小的时间开销。通过全面评估冻结和压缩策略，为高效DNN训练提供了优化见解。

Conclusion: 本文首次全面探索了层冻结缓存方法中的关键挑战，并提供了系统性解决方案，成功实现了训练成本的大幅降低，同时保持了模型性能，为高效深度神经网络训练提供了实用方法。
...
### [202] [Robust Estimation Under Heterogeneous Corruption Rates](https://arxiv.org/abs/2508.15051)
*Syomantak Chaudhuri,Jerry Li,Thomas A. Courtade*

Main category: cs.LG

TL;DR: 该论文研究了异构污染率下的鲁棒估计问题，针对不同样本具有已知但不同的污染概率的情况，给出了多元有界分布和单变量高斯分布均值估计的紧致极小极大率，并为多元高斯均值估计和线性回归建立了平方误差的极小极大率。


<details>
  <summary>Details</summary>
Motivation: 研究动机源于分布式学习、联邦学习、众包和传感器网络等实际应用中存在的异构污染问题，现有鲁棒估计器通常假设均匀或最坏情况污染，忽略了结构性异质性。

Method: 通过理论分析建立了异构污染模式下的极小极大率框架，针对不同分布类型（多元有界分布、单变量高斯、多元高斯）和估计问题（均值估计、线性回归）给出了最优估计器的性能界限。

Result: 研究发现最优估计器可能会丢弃超过特定污染阈值的样本，该阈值由给定的污染率经验分布决定，并获得了紧致的极小极大率结果。

Conclusion: 论文为异构污染环境下的鲁棒估计提供了理论基础和性能界限，表明考虑污染率异质性对于设计最优估计策略至关重要，特别是在实际应用中污染概率非均匀分布的场景。
...
### [203] [Enhancing Optimizer Stability: Momentum Adaptation of The NGN Step-size](https://arxiv.org/abs/2508.15071)
...
<details>
  <summary>Details</summary>
Motivation: 现代优化算法对超参数选择敏感，调参困难而耗时，需要提高算法的稳定性和绝缘性能。

Method: 提出NGN-M算法，在NGN步长调整方法基础上结合动量机制，放宽了收敛假设条件。

Result: 达到了标准的$Σ(1/√K)$收敛速率，实验表明其性能可与或超过现有最优算法，同时对步长超参数更加稳健。

Conclusion: NGN-M算法为深度学习优化提供了一种稳定、高效的解决方案，减少了超参数调整的负担。
...
### [204] [Wormhole Dynamics in Deep Neural Networks](https://arxiv.org/abs/2508.15086)
*Yen-Lung Lai,Zhe Jin*

Main category: cs.LG

TL;DR: 本文研究了深度神经网络在过参数化状态下的泛化行为，发现网络会出现输出特征空间坍缩现象，虽然提升泛化能力但最终导致退化状态。通过提出的"虫洞"解决方案可以绕过这种退化，为理解DNN泛化提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 研究深度神经网络中"欺骗性样本"现象，即DNN对看似随机的输入产生高置信度分类，旨在理解DNN在过参数化状态下的泛化机制和退化问题。

Method: 提出基于最大似然估计的分析框架，避免传统的基于梯度的优化方法和显式标签，研究DNN在过参数化状态下的输出特征空间坍缩现象。

Result: 发现DNN在过参数化状态下会出现输出特征空间坍缩，虽然改善泛化但最终导致退化状态（不同输入映射到相同输出）。提出的"虫洞"解决方案能够绕过这种退化，协调随机标签与有意义标签。

Conclusion: 研究为理解DNN泛化机制提供了新见解，揭示了过参数化状态下的退化问题及其解决方案，为无监督学习动态研究指明了方向，有助于弥合理论与实践的差距。
...
### [205] [Evaluating Sparse Autoencoders for Monosemantic Representation](https://arxiv.org/abs/2508.15094)
*Moghis Fereidouni,Muhammad Umair Haider,Peizhong Ju,A. B. Siddique*

Main category: cs.LG

TL;DR: 本文首次系统评估稀疏自编码器(SAE)在减少神经元多义性方面的效果，提出基于Jensen-Shannon距离的概念可分离性评分，并开发了新的干预方法APP


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中神经元的多义性（一个神经元对多个不相关概念产生响应）阻碍了模型的可解释性。虽然稀疏自编码器被提出用于缓解此问题，但缺乏与基础模型的定量比较

Method: 使用Gemma-2-2B模型和多种SAE变体，在五个基准测试上评估概念可分离性。提出基于Jensen-Shannon距离的细粒度概念可分离性评分，并开发了两种干预策略：全神经元掩码和部分抑制

Result: SAE确实减少了多义性并获得了更高的概念可分离性，但更大的稀疏性并不总是带来更好的可分离性，且通常会损害下游性能。SAE在使用部分抑制时能实现更精确的概念级控制

Conclusion: 提出的APP干预方法在目标概念移除方面优于现有方法，为语言模型的可解释性和可控性提供了新的工具
...
### [206] [See Beyond a Single View: Multi-Attribution Learning Leads to Better Conversion Rate Prediction](https://arxiv.org/abs/2508.15217)
*Sishuo Chen,Zhangming Chan,Xiang-Rong Sheng,Lei Zhang,Sheng Chen,Chenghuan Hou,Han Zhu,Jian Xu,Bo Zheng*

Main category: cs.LG

TL;DR: 提出多归因学习框架MAL，整合多种归因机制信号来提升CVR预测性能，相比单归因方法获得显著提升


<details>
  <summary>Details</summary>
Motivation: 传统方法只使用单一归因机制标签训练模型，丢弃了其他归因视角的互补信号，限制了模型性能

Method: 提出MAL框架，包含归因知识聚合器(AKA)和主目标预测器(PTP)两个核心组件，采用多任务学习整合多种归因标签知识

Result: 离线实验GAUC提升0.51%，在线实验ROI提升2.6%

Conclusion: 多归因学习框架能有效整合不同归因机制的信号，显著提升CVR预测性能和业务指标
...
### [207] [Hydra: A 1.6B-Parameter State-Space Language Model with Sparse Attention, Mixture-of-Experts, and Memory](https://arxiv.org/abs/2508.15099)
*Siddharth Chaudhary,Bennett Browning*

Main category: cs.LG

TL;DR: Hydra是一个混合长上下文语言模型架构，结合了条件计算、长上下文记忆机制和稀疏专家混合，在约16亿参数设计范围内整合了Mamba风格SSM主干、间歇性稀疏全局注意力、分块级MoE前馈路由和双重记忆系统。
...
Method: 采用结构化状态空间模型(SSM)作为主干，结合间歇性稀疏全局注意力机制、分块级稀疏专家混合(MoE)前馈路由，以及工作记忆和事实PKM双重记忆系统。
...
### [208] [Side Effects of Erasing Concepts from Diffusion Models](https://arxiv.org/abs/2508.15124)
*Shaswati Saha,Sourajit Saha,Manas Gaur,Tejas Gokhale*

Main category: cs.LG

TL;DR: 本文研究发现概念擦除技术(CETs)存在严重漏洞，可以通过语义相似的提示词、层级关系和组合变体轻松绕过，同时还存在属性泄露和注意力异常等副作用。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型在隐私、版权和安全方面的担忧日益增加，概念擦除技术(CETs)被开发用于禁止生成特定目标概念。然而，这些技术的实际效果和鲁棒性需要系统评估。

Method: 提出了Side Effect Evaluation (SEE)评估基准，包含分层和组合提示词的数据集，通过自动化评估流程从三个维度量化CETs的副作用：对相邻概念的影响、目标规避和属性泄露。

Result: 实验表明CETs可以通过超类-子类层级关系和语义相似的提示词（如目标的组合变体）轻松绕过，同时存在属性泄露和注意力集中/分散的反直觉现象。
...
### [209] [Towards Source-Free Machine Unlearning](https://arxiv.org/abs/2508.15127)
...
TL;DR: 提出了一种在无源设置下的机器学习模型遗忘方法，无需访问原始训练数据即可高效移除特定数据，通过估计剩余数据的Hessian矩阵实现零样本遗忘。
...
Method: 基于近期研究，提出了一种估计未知剩余训练数据Hessian矩阵的方法，这是高效遗忘的关键组件。利用这种估计技术，实现了高效的零样本遗忘。

Result: 在广泛数据集上的大量实验验证了该方法的有效性，能够在保持剩余数据性能的同时提供强大的理论保证。

Conclusion: 该方法成功解决了无源设置下的模型遗忘问题，为实际应用中的隐私保护和版权管理提供了可行的解决方案。
...
### [210] [Universal Reinforcement Learning in Coalgebras: Asynchronous Stochastic Computation via Conduction](https://arxiv.org/abs/2508.15128)
*Sridhar Mahadevan*
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习框架缺乏统一的数学抽象，无法很好地处理异步分布式计算场景。作者希望通过范畴论和余代数理论为RL提供更强大的数学基础，扩展RL到更一般的动态系统模型。

Method: 使用范畴论和函子对RL进行建模，将MDP、POMDP等动态系统表示为特殊类型的余代数。引入Bertsekas和Tsitsiklis的异步分布式最小化标准模型，将RL算法空间建模为函子范畴，其中余范畴形成topos结构。

Result: 建立了通用强化学习的理论框架，将各种动态系统模型统一为通用余代数的特例，将值函数固定点求解问题推广为最终余代数的异步确定问题。

Conclusion: 范畴论和余代数理论为强化学习提供了强大的数学基础，使得RL能够扩展到更一般的异步并行分布式计算场景，为未来RL理论发展奠定了坚实基础。
...
### [211] [Towards Reliable and Generalizable Differentially Private Machine Learning (Extended Version)](https://arxiv.org/abs/2508.15141)
*Wenxuan Bao,Vincent Bindschaedler*

Main category: cs.LG

TL;DR: 对11种最新差分隐私机器学习技术进行可复现性研究，发现部分方法经得起验证，但有些方法在原始实验条件外表现不佳，并提出了差分隐私机器学习可复现性的最佳实践


<details>
  <summary>Details</summary>
Motivation: 近期大量差分隐私机器学习研究声称达到最先进效果，但由于代码库、数据集、方法和模型架构的异质性，难以直接比较不同方法的有效性，需要系统性的可复现性验证

Method: 对11种最新的最先进差分隐私机器学习技术进行可复现性和可复制性实验，分析在不同实验条件下的表现

Result: 研究结果多样：部分方法经得起严格检验，但其他方法在原始实验条件之外测试时表现不佳

Conclusion: 提出了差分隐私机器学习领域获得科学有效和可靠结果的最佳实践，讨论了该领域特有的可复现性挑战（如DP噪声带来的额外随机性）及其解决方案
...
### [212] [A Robust BERT-Based Deep Learning Model for Automated Cancer Type Extraction from Unstructured Pathology Reports](https://arxiv.org/abs/2508.15149)
...
TL;DR: 开发了基于RoBERTa微调的系统，用于从病理报告中自动提取癌症类型，支持精准肿瘤学研究，性能显著优于基线模型和Mistral 7B大语言模型。
...
Method: 使用微调的RoBERTa模型从病理报告中提取特定癌症类型，该方法针对精准肿瘤学任务进行了领域特定的优化。

Result: 模型性能显著优于基线模型和Mistral 7B大语言模型，达到F1_Bertscore 0.98和总体精确匹配率80.61%。

Conclusion: 微调领域特定模型的方法展示了良好的可扩展性，可以无缝集成到分子肿瘤委员会流程中，为更高效准确的临床信息提取铺平了道路。
...
### [213] [SafeLLM: Unlearning Harmful Outputs from Large Language Models against Jailbreak Attacks](https://arxiv.org/abs/2508.15182)
...
<details>
  <summary>Details</summary>
Motivation: 越狱攻击通过制作对抗性提示绕过大型语言模型的对齐机制，导致模型产生有害内容，严重威胁LLM的安全性。

Method: 采用三阶段流程：1)动态不安全输出检测（外部分类器+模型内部评估）；2)通过前馈网络激活进行token级有害内容追踪；3)约束优化抑制不安全行为而不降低模型质量

Result: 在多个越狱基准测试中显著降低攻击成功率，同时保持高性能。相比标准防御方法，提供更强的安全保证、更精确的有害行为控制和更好的未见攻击鲁棒性

Conclusion: 反学习是构建可扩展和有效LLM安全性的有前景方向，SafeLLM实现了针对性和不可逆的遗忘，同时保持模型通用性能
...
### [214] [Revisiting Pre-processing Group Fairness: A Modular Benchmarking Framework](https://arxiv.org/abs/2508.15193)
...
### [215] [Frequency-adaptive tensor neural networks for high-dimensional multi-scale problems](https://arxiv.org/abs/2508.15198)
*Jizu Huang,Rukang You,Tao Zhou*

Main category: cs.LG

TL;DR: 本文提出了一种频率自适应张量神经网络算法，通过引入随机傅里叶特征和离散傅里叶变换来增强TNN处理高维多尺度问题的能力，有效缓解维度灾难问题。
...
Result: 提出的频率自适应TNN算法显著提升了处理复杂多尺度问题的能力，大量数值实验验证了其有效性和鲁棒性。

Conclusion: 频率自适应策略成功克服了TNN的频率限制，为高维多尺度问题提供了有效的解决方案，具有重要的理论和应用价值。
...
### [216] [SleepDIFFormer: Sleep Stage Classification via Multivariate Differential Transformer](https://arxiv.org/abs/2508.15215)
...
<details>
  <summary>Details</summary>
Motivation: 手动检查EEG特征耗时且易出错，现有机器学习方法面临EEG/EOG信号非平稳性和变异性的挑战，导致在未见数据集上泛化能力差。

Method: 开发多变量差分变换器(SleepDIFFormer)架构，处理EEG和EOG信号，通过特征分布对齐学习领域不变的联合表示，缓解时空注意力噪声。
...
### [217] [Locally Pareto-Optimal Interpretations for Black-Box Machine Learning Models](https://arxiv.org/abs/2508.15220)
*Aniruddha Joshi,Supratik Chakraborty,S Akshay,Shetal Shah,Hazem Torfah,Sanjit Seshia*

Main category: cs.LG

TL;DR: 这篇论文提出了一种基于局部最优性保证的框架，用于可扩展地合成黑盒机器学习模型的解释，平衡准确性和可解释性之间的交换。


<details>
  <summary>Details</summary>
Motivation: 现有的多目标解释合成方法缺乏对帕索托最优性的形式保证，而能提供全局保证的方法又遇到可扩展性问题。

Method: 先使用多目标学习或搜索技术生成帕索托最优候选集，然后通过SAT求解器验证每个候选的局部最优性。

Result: 在一组基准测试中，该方法生成的解释与提供全局保证的方法类似，但更具可扩展性。

Conclusion: 通过局部最优性保证，该框架能在保持高质量解释的同时提高可扩展性。
...
### [218] [Learning ECG Representations via Poly-Window Contrastive Learning](https://arxiv.org/abs/2508.15225)
...
TL;DR: 提出多窗口对比学习框架，通过从ECG信号中提取多个时间窗口构建正样本对，学习时间不变的特征表示，在PTB-XL数据集上取得更好的分类性能并显著减少预训练时间和计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统自监督对比学习方法仅生成成对的增强视图，未能充分利用ECG丰富的时序结构信息，限制了模型性能。

Method: 采用多窗口对比学习框架，从每个ECG实例中提取多个时间窗口构建正样本对，通过统计最大化它们的一致性，并借鉴慢特征分析原理学习时间不变的特征。
...
### [219] [Deep Think with Confidence](https://arxiv.org/abs/2508.15260)
...
<details>
  <summary>Details</summary>
Motivation: 传统自一致性多数投票方法在准确率提升上存在收益递减问题且计算开销大，需要更高效的测试时推理方法
...
### [220] [Evaluating Knowledge Graph Complexity via Semantic, Spectral, and Structural Metrics for Link Prediction](https://arxiv.org/abs/2508.15291)
*Haji Gul,Abul Ghani Naim,Ajaz Ahmad Bhat*
...
<details>
  <summary>Details</summary>
Motivation: 理解数据集复杂度对于评估知识图谱链接预测模型至关重要，但现有的CSG复杂度指标尚未在KG环境中得到验证，需要系统评估其在多关系链接预测中的有效性。

Method: 在知识图谱链接预测环境中评估CSG指标，结合transformer生成的语义表示，并引入一组新的结构和语义KG复杂度指标进行基准测试。
...
### [221] [Saving for the future: Enhancing generalization via partial logic regularization](https://arxiv.org/abs/2508.15317)
*Zhaorui Tan,Yijie Hu,Xi Yang,Qiufeng Wang,Anh Nguyen,Kaizhu Huang*

Main category: cs.LG

TL;DR: PL-Reg是一种新的部分逻辑正则化方法，通过为未定义逻辑公式预留空间来提升模型对未知类别的适应性，在多个视觉分类任务中展现出优异的泛化性能。
...
Result: 在广义类别发现、多域广义类别发现和长尾类增量学习任务上的大量实验验证了PL-Reg的持续性能提升，证明了部分逻辑在处理未知类别挑战中的有效性。

Conclusion: 部分逻辑正则化为处理未知类别提供了有效的解决方案，PL-Reg方法在多个基准任务中表现出优异的泛化性能和适应性。
...
### [222] [ExBigBang: A Dynamic Approach for Explainable Persona Classification through Contextualized Hybrid Transformer Analysis](https://arxiv.org/abs/2508.15364)
...
TL;DR: ExBigBang是一个可解释的混合文本-表格方法，使用transformer架构进行人物角色分类，整合元数据、领域知识和用户画像来嵌入更深层次的上下文信息。


<details>
  <summary>Details</summary>
Motivation: 传统人物角色开发方法在处理复杂用户交互时缺乏上下文信息整合能力，且现有模型往往缺乏可解释性，难以理解和证明预测结果。

Method: 提出ExBigBang混合文本-表格方法，使用基于transformer的架构建模丰富的上下文特征，通过用户画像和分类的循环过程动态更新以反映用户行为变化。
...
### [223] [Enhancing Forecasting with a 2D Time Series Approach for Cohort-Based Data](https://arxiv.org/abs/2508.15369)
*Yonathan Guttel,Orit Moradov,Nachi Lieder,Asnat Greenstein-Messica*

Main category: cs.LG

TL;DR: 新颖2D时间序列预测模型，集成组群行为特征，在小数据环境中显示优异性能


<details>
  <summary>Details</summary>
Motivation: 解决小数据环境下的时间序列预测挑战，特别是在金融和市场营销预测领域

Method: 新颖的二维时间序列预测模型，集成了组群行为在时间上的变化

Result: 在多个实际数据集上验证，显示出比参考模型更高的准确性和适应性

Conclusion: 该方法为面对金融和市场营销预测挑战的行业提供了有价值的策略决策见解
...
### [224] [Fairness for the People, by the People: Minority Collective Action](https://arxiv.org/abs/2508.15374)
*Omri Ben-Dov,Samira Samadi,Amartya Sanyal,Alexandru Ţifrea*

Main category: cs.LG

TL;DR: 用户可以通过协调重新标注数据来提升模型公平性，无需改变企业训练过程，少数群体的小规模协调行动就能显著减少不公平性


<details>
  <summary>Details</summary>
Motivation: 机器学习模型经常保留训练数据中的偏见，导致对某些少数群体的不公平对待。现有的企业端偏见缓解技术通常需要付出效用成本和组织支持

Method: 提出了三种实用的、模型无关的方法来近似理想的重新标注，并在真实数据集上进行验证。基于算法集体行动框架，协调的少数群体战略性重新标注自身数据

Result: 研究发现，少数群体的一个子群可以通过协调重新标注，以对整体预测误差的微小影响为代价，大幅减少不公平性

Conclusion: 用户端的算法集体行动提供了一种有效的公平性提升途径，不需要改变企业的训练流程，少数群体的协调行动就能显著改善模型公平性
...
### [225] [EvoFormer: Learning Dynamic Graph-Level Representations with Structural and Temporal Bias Correction](https://arxiv.org/abs/2508.15378)
*Haodi Zhong,Liuxin Zou,Di Wang,Bo Wang,Zhenxing Niu,Quan Wang*

Main category: cs.LG

TL;DR: EvoFormer是一个用于动态图级别表示学习的Transformer框架，通过结构感知和时间敏感模块解决结构访问偏差和突变演化盲点问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态图嵌入方法存在两个关键问题：结构访问偏差（随机游走采样过度强调高度节点）和突变演化盲点（无法有效检测突然的结构变化），需要新的框架来解决这些问题。
...
### [226] [CITE: A Comprehensive Benchmark for Heterogeneous Text-Attributed Graphs on Catalytic Materials](https://arxiv.org/abs/2508.15392)
*Chenghao Zhang,Qingqing Long,Ludi Wang,Wenjuan Cui,Jianjun Yu,Yi Du*

Main category: cs.LG

TL;DR: 这篇论文提出了CITE数据集，第一个大规模异构文本图标注数据集，用于催化材料领域的表征学习研究和评测


<details>
  <summary>Details</summary>
Motivation: 异构文本属性图在实际系统中普遍存在，但缺乏大规模标准数据集，影响了表征学习方法的发展和公平比较

Method: 构建了CITE数据集（包含43.8万节点和120万边，四种关系类型），设置标准化评估流程，对四类学习范式进行基准测试和消融实验

Result: 提供了大规模异构文本图数据集，建立了标准化评估框架，并对各类模型进行了综合性能评测

Conclusion: CITE数据集最终成为催化材料领域异构文本图表征学习研究的重要基础资源，推动了该领域方法进步和公平比较
...
### [227] [Federated Learning based on Self-Evolving Gaussian Clustering](https://arxiv.org/abs/2508.15393)
*Miha Ožbot,Igor Škrjanc*

Main category: cs.LG

TL;DR: 基于联邦学习的迭代模糊系统，能够动态添加新聚类而无需预先指定聚类数量


<details>
  <summary>Details</summary>
Motivation: 解决传统聚类方法需要预先指定聚类数的问题，并在保护数据隐私的联邦学习环境中实现动态适应

Method: 在联邦学习框架下建立迭代模糊系统，允许在客户端设备上本地训练模型，只分享模型参数而非数据本身，使用PyTorch实现

Result: 在多个UCI数据集上的聚类和分类任务中表现超过传统分类方法，虽然计算开销较大但在去中心化数据处理中显示明显优势

Conclusion: 该方法提供了一种在保护数据隐私的同时能够动态适应新聚类的有效解决方案，在分布式环境中具有应用潜力
...
### [228] [Hybrid Least Squares/Gradient Descent Methods for DeepONets](https://arxiv.org/abs/2508.15394)
*Jun Choi,Chang-Ock Lee,Minam Moon*

Main category: cs.LG

TL;DR: 提出一种混合最小二乘/梯度下降方法来加速DeepONet训练，通过将大型线性系统分解为分支网络和主干网络两个子问题分别求解


<details>
  <summary>Details</summary>
Motivation: DeepONet输出对分支网络最后一层参数是线性的，可用最小二乘法优化，但直接构建全部线性系统计算费用过高

Method: 将大型LS系统分解为分支网络和主干网络两个子问题，分别求解，隐藏层参数仍用梯度下降更新

Result: 方法能够有效减少计算复杂度，适用于包括正则化项的L2损失和无监督物理损失的更广泛场景

Conclusion: 该混合方法提供了一种高效的DeepONet训练方案，通过线性与非线性优化的结合，在保持准确性的同时大幅提升训练效率
...
### [229] [Bridging Generalization and Personalization in Wearable Human Activity Recognition via On-Device Few-Shot Learning](https://arxiv.org/abs/2508.15413)
*Pixi Kang,Julian Moosmann,Mengxi Liu,Bo Zhou,Michele Magno,Paul Lukowicz,Sizhen Bian*

Main category: cs.LG

TL;DR: 提出一种在设备上快速个性化的人体活动识别混合框架，通过少样本学习实现跨用户泛化后快速适应个体用户，在嵌入式平台上实现轻量级有效个性化


<details>
  <summary>Details</summary>
Motivation: 可穿戴设备的人体活动识别在部署到新用户时性能下降，主要由于用户引起的概念漂移，需要高效的个性化方法

Method: 混合框架先跨用户泛化，然后通过仅更新分类器层的少样本学习在设备上快速适应个体用户

Result: 在RISC-V GAP9微控制器上验证三个HAR场景，部署后适应分别获得3.73%、17.38%和3.70%的准确率提升
...
### [230] [Measures of Overlapping Multivariate Gaussian Clusters in Unsupervised Online Learning](https://arxiv.org/abs/2508.15444)
*Miha Ožbot,Igor Škrjanc*
...
<details>
  <summary>Details</summary>
Motivation: 在线数据流学习需要处理概念漂移，可能导致大量重叠聚类需要合并。现有分布差异度量方法无法有效处理各种形状的聚类且计算成本高

Method: 设计了一种专门检测重叠而非差异的相异度度量方法，计算速度比现有方法快数倍

Result: 该方法比对比方法快数倍，能够有效检测重叠聚类，同时避免正交聚类的错误合并

Conclusion: 提出的新度量方法为在线数据流学习中的聚类合并提供了高效准确的解决方案
...
### [231] [Reliable Unlearning Harmful Information in LLMs with Metamorphosis Representation Projection](https://arxiv.org/abs/2508.15449)
*Chengcan Wu,Zeming Wei,Huanran Chen,Yinpeng Dong,Meng Sun*

Main category: cs.LG

TL;DR: 提出了一种基于不可逆投影变换的机器遗忘方法MRP，通过隐藏状态空间的投影变换有效消除有害信息，实现连续遗忘并防御重学习攻击


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法仅通过参数训练抑制不良数据激活，无法完全消除信息痕迹，难以实现有效连续遗忘且易受重学习攻击

Method: 在特定网络层的隐藏状态空间实施投影变换，利用不可逆投影特性彻底消除有害信息同时保留有用知识

Result: 实验证明该方法能实现有效连续遗忘，成功防御重学习攻击，在遗忘效果和保持自然性能方面达到最先进水平

Conclusion: MRP方法通过不可逆投影变换解决了现有机器遗忘方法的根本局限性，为模型安全提供了更有效的解决方案
...
### [232] [A Solvable Molecular Switch Model for Stable Temporal Information Processing](https://arxiv.org/abs/2508.15451)
*H. I. Nurdin,C. A. Nijhuis*

Main category: cs.LG

TL;DR: 本文研究了一个输入驱动的单状态微分方程模型，该模型最初为实验证明的动态分子开关开发，具有类似大脑突触的切换特性。模型具有精确可解性、收敛性和衰减记忆等数学特性，能够稳定处理时变输入，兼具生物启发行为和稳定序列学习所需的数学性质。


<details>
  <summary>Details</summary>
Motivation: 研究动态分子开关的计算特性，该开关具有类似大脑突触的生物启发行为，旨在探索其在神经形态计算中作为计算单元的潜力，并为开发更通用的精确可解模型提供理论基础。

Method: 采用线性状态、非线性输入的微分方程模型进行分析，证明该模型具有精确可解性，并通过数学分析验证其收敛性和衰减记忆特性。

Result: 模型展现出生物启发行为与稳定数学特性的共存，能够稳定处理时变输入，支持在深度前馈和循环架构中使用动态分子开关作为计算单元。

Conclusion: 该研究为动态分子开关在神经形态计算中的应用提供了理论支持，并可能启发开发更通用的精确可解模型来模拟任意物理设备，实现脑启发行为和稳定计算。
...
### [233] [Mini-Batch Robustness Verification of Deep Neural Networks](https://arxiv.org/abs/2508.15454)
...
Method: 提出分组局部鲁棒性验证，动态构建和验证具有相似网络计算的ε-ball小批量，采用自适应批量大小识别和联合验证策略

Result: 在MNIST和CIFAR-10的全连接和卷积网络上，BaVerLy将传统逐个验证方法平均提速2.3倍，最高4.1倍（从24小时减少到6小时）

Conclusion: BaVerLy通过利用网络计算的相似性，有效提升了局部鲁棒性验证的效率，为大规模输入集的验证提供了可行方案
...
### [234] [Learning Protein-Ligand Binding in Hyperbolic Space](https://arxiv.org/abs/2508.15480)
*Jianhui Wang,Wenyu Zhu,Bowen Gao,Xin Hong,Ya-Qin Zhang,Wei-Ying Ma,Yanyan Lan*

Main category: cs.LG

TL;DR: HypSeek是一个双曲表示学习框架，将配体、蛋白质口袋和序列嵌入到双曲空间中，通过利用双曲空间的指数几何和负曲率特性，在虚拟筛选和亲和力排名任务上取得显著提升
...
Method: 提出HypSeek框架，使用Lorentz模型双曲空间进行嵌入，采用蛋白质引导的三塔架构来增强表示结构
...
### [235] [Let's Grow an Unbiased Community: Guiding the Fairness of Graphs via New Links](https://arxiv.org/abs/2508.15499)
...
<details>
  <summary>Details</summary>
Motivation: 图神经网络在处理有偏图结构时面临公平性挑战，需要通过引入新链接来引导现有结构向无偏方向发展。

Method: 提出FairGuide框架，使用可微分社区检测作为伪下游任务，通过元梯度从公平性指导目标中识别能显著提升结构公平性的新链接。

Result: 理论分析表明优化伪任务的公平性有效增强结构公平性，实验证明该方法在各种图基公平性任务中具有有效性和泛化性。
...
### [236] [Jointly Computation- and Communication-Efficient Distributed Learning](https://arxiv.org/abs/2508.15509)
*Xiaoxing Ren,Nicola Bastianello,Karl H. Johansson,Thomas Parisini*

Main category: cs.LG

TL;DR: 这篇论文提出了一种新的基于ADMM的分布式学习算法，通过结合随机梯度、多训练时钟和压缩传输技术，实现了计算和通信双重效率优化。算法在强凸设置下证明了准确的线性收敛性。


<details>
  <summary>Details</summary>
Motivation: 解决分布式学习中的计算和通信效率问题，寻求一种能同时优化计算成本和通信成本的算法方案。

Method: 设计基于ADMM的算法：1）使用随机梯度保证计算效率；2）通信轮次间进行多训练时钟；3）采用压缩传输技术。

Result: 证明了算法在强凸设置下的准确线性收敛性，并通过分类任务的数值对比验证了其优势。

Conclusion: 该算法成功实现了计算和通信效率的双重优化，在分布式学习领域具有重要应用价值。
...
### [237] [Stabilization of Perturbed Loss Function: Differential Privacy without Gradient Noise](https://arxiv.org/abs/2508.15523)
...
<details>
  <summary>Details</summary>
Motivation: 现有的梯度隐私机制如DP-SGD需要在梯度中注入噪声，导致计算效率低和稳定性差，需要一种更高效稳定的多用户隐私训练方法

Method: SPOF通过扰动稳定的泰勒展开多项式近似来私有化损失函数，每个用户的数据通过向多项式系数添加校准噪声来实现隐私保护

Result: 在无线体域网场景中，SPOF比DP-SGD平均重建精度提高3.5%，训练时间减少57.2%，表现出更好的隐私-效用权衡

Conclusion: SPOF提供了一种高效稳定的多用户本地差分隐私训练方案，在环境噪声存在时仍能保持稳定性能，优于传统的梯度隐私方法
...
### [238] [AI-Powered Machine Learning Approaches for Fault Diagnosis in Industrial Pumps](https://arxiv.org/abs/2508.15550)
*Khaled M. A. Alghtus,Ayad Gannan,Khalid M. Alhajri,Ali L. A. Al Jubouri,Hassan A. I. Al-Janahi*

Main category: cs.LG

TL;DR: 采用双门限标签化和合成故障信号注入的方法，通过随机森林、XGBoost和SVM等机器学习算法实现了工业泵系统的早期故障检测，随机森林和XGBoost模型表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决工业泵系统在海洋环境中运行时的故障检测难题，特别是面对故障数据稀缺的挑战，希望通过预测性维护减少设备停机时间和维护成本。

Method: 监测振动、温度、流量、压力和电流五个关键参数，采用固定工程限制和历史数据95%位数的双门限标签化方法，注入合成故障信号补充故障数据不足，使用随机森林、XGBoost和SVM等算法进行分类检测。

Result: 随机森林和XGBoost模型在所有类别上都达到了高准确度，包括对稀缺故障的检测，SVM模型对异常的敏感性较低。可视化分析显示混合方法具有稳健的检测能力。

Conclusion: 该框架具有可扩展性、可解释性和适合实时工业部署的特点，支持在故障发生前进行预阻性维护决策，并可适用于其他体系结构相似的机器设备，为复杂系统的预测性维护提供了可扩展的解决方案。
...
### [239] [Conformalized Exceptional Model Mining: Telling Where Your Model Performs (Not) Well](https://arxiv.org/abs/2508.15569)
...
<details>
  <summary>Details</summary>
Motivation: 在高风险领域如医疗和金融中，需要深入理解机器学习模型的细微性能差异，确保模型部署的可靠性和可解释性。
...
### [240] [Inductive Domain Transfer In Misspecified Simulation-Based Inference](https://arxiv.org/abs/2508.15593)
*Ortal Senouf,Antoine Wehenkel,Cédric Vincent-Cuaz,Emmanuel Abbé,Pascal Frossard*

Main category: cs.LG

TL;DR: 这篇论文提出了一种全归纳性模拟推断框架，通过结合检验和最优运输分布对齐来处理模型错误规范问题，实现了更好的可扩展性和性能。


<details>
  <summary>Details</summary>
Motivation: 解决模拟推断(SBI)中的模型错误规范问题，充分利用检验数据和未配对样本来提高推断准确性。

Method: 结合小批量最优运输和条件正规化流，通过给定同一潜变参数的真实和模拟观测对齐分布，学习后验分布。

Result: 在合成和真实数据集上，包括医学标记物估计，方法性能超过或平等于RoPE和其他标准SBI方法。

Conclusion: 该方法提供了一种可扩展的归纳性SBI解决方案，在模型错误环境中保持良好性能，无需在测试时访问模拟器。
...
### [241] [Continual Neural Topic Model](https://arxiv.org/abs/2508.15612)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态主题模型需要一次性处理整个训练语料库，而在线主题模型虽然能持续更新但缺乏长期记忆，需要填补这一空白。

Method: 使用持续更新的全局先验分布，在后续时间步中持续学习主题模型而不遗忘先前学到的内容。

Result: CoNTM在主题质量和预测困惑度方面一致优于动态主题模型，能够在线捕捉主题变化，学习更多样化的主题并更好地捕捉时间变化。

Conclusion: CoNTM成功解决了持续学习中的遗忘问题，在主题建模任务中表现出色，能够有效捕捉主题的时间演化。
...
### [242] [GRASPED: Graph Anomaly Detection using Autoencoder with Spectral Encoder and Decoder (Full Version)](https://arxiv.org/abs/2508.15633)
...
Method: 使用基于图小波卷积的编码器和基于维纳图反卷积的解码器，结合结构和属性解码器，形成具有带通滤波器特性的无监督学习模型，实现多尺度全局和局部图信息捕获

Result: 在多个真实世界图异常检测数据集上的实验表明，GRASPED模型优于当前最先进的模型

Conclusion: GRASPED通过频域编码和解码设计，成功解决了现有方法在多频带分析方面的局限性，为无监督图异常检测提供了有效解决方案
...
### [243] [Classification errors distort findings in automated speech processing: examples and solutions from child-development research](https://arxiv.org/abs/2508.15637)
*Lucas Gautheron,Evan Kidd,Anton Malko,Marvin Lavechin,Alejandrina Cristia*

Main category: cs.LG

TL;DR: 本文提出贝叶斯方法来研究自动分类器错误对科学测量的影响，发现在语言习得研究中分类错误会显著扭曲效应估计，并展示了贝叶斯校准方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着可穿戴录音设备的普及，研究者越来越多地使用自动分析方法处理音频视频数据，但很少有研究关注分类错误对测量和统计推断的下游影响。

Method: 采用贝叶斯方法来研究算法错误对关键科学问题的影响，包括兄弟姐妹对儿童语言经验的影响以及儿童产出与输入之间的关联。

Result: 研究发现分类错误会显著扭曲估计值，例如自动标注低估了兄弟姐妹对成人输入的负面影响达20-80%，可能使其低于统计显著性阈值。

Conclusion: 贝叶斯校准方法对于恢复无偏效应大小估计是有效且有洞察力的，但不能提供万无一失的解决方案。所报告的问题和解决方案适用于任何涉及事件检测和分类且错误率非零的分类器。
...
### [244] [Correct-By-Construction: Certified Individual Fairness through Neural Network Training](https://arxiv.org/abs/2508.15642)
...
<details>
  <summary>Details</summary>
Motivation: 当前的机器学习公平性方法缺乏形式保证，而验证技术又不能在训练过程中主动提升公平性。需要一种能够在整个训练过程中形式保证个体公平性的方法。

Method: 设计了一个包含两部分的框架：(1)可证公平的初始化，确保模型在公平状态下开始；(2)公平保持训练算法，在模型学习过程中维持公平性。重点使用了随机响应机制来保护敏感属性同时维持公平保证。

Result: 实验评估证实了该方法的有效性，能够生成经验上公平且准确的模型。同时，该方法比基于认证训练的替代方案更加高效。

Conclusion: 该研究提供了一种能够在整个训练过程中形式保证个体公平性的新框架，充分解决了现有方法在公平保证方面的不足，为机器学习公平性领域提供了重要技术进展。
...
### [245] [Amortized In-Context Mixed Effect Transformer Models: A Zero-Shot Approach for Pharmacokinetics](https://arxiv.org/abs/2508.15659)
*César Ali Ojeda Marin,Wilhelm Huisinga,Purity Kavwele,Niklas Hartung*

Main category: cs.LG

TL;DR: AICMET是一种基于Transformer的混合效应模型，通过预训练和上下文学习实现药物剂量反应的零样本预测，显著提升预测精度并缩短建模周期


<details>
  <summary>Details</summary>
Motivation: 解决稀疏采样下的精确剂量反应预测问题，传统方法建模周期长且需要大量专家干预，需要更高效的个性化给药方案

Method: 使用Transformer框架结合机制性房室模型先验，通过Ornstein-Uhlenbeck过程预训练数十万条合成药代动力学轨迹，实现上下文贝叶斯推理
...
### [246] [Tensorized Multi-Task Learning for Personalized Modeling of Heterogeneous Individuals with High-Dimensional Data](https://arxiv.org/abs/2508.15676)
*Elif Konyar,Mostafa Reisi Gahrooei,Kamran Paynabar*

Main category: cs.LG

TL;DR: 提出基于多任务学习和低秩张量分解的新方法，用于处理异质子群体的个性化建模问题，通过共享结构和保留特异性来提升预测精度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 异质子群体建模面临个体特征和行为差异的挑战，需要同时捕捉群体共性和子群体特异性，传统方法难以有效平衡这种关系。

Method: 采用多任务学习框架结合低秩张量分解技术，将任务模型参数分解为低秩结构，捕获任务和子群体间的共性与变异，实现知识共享和特性保留。

Result: 在仿真和案例数据集上的实验表明，该方法在子群体高变异场景下优于多个基准方法，显著提升了预测精度和模型可解释性。
...
### [247] [An Efficient Open World Environment for Multi-Agent Social Learning](https://arxiv.org/abs/2508.15679)
*Eric Ye,Ren Tao,Natasha Jaques*

Main category: cs.LG

TL;DR: 这篇论文提出了一个开放式多段代理环境，用于研究社会智能对AI代理在存在专家和隐式合作情况下的学习效果的影响。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏开放式多段代理环境来研究社会智能对AI代理学习的加速作用，而现实世界环境本身就是多段代理且包含人类专家。

Method: 设计了一个多自利代理可以追求复杂独立目标的环境，反映现实世界挑战，支持研究隐式合作、工具共享等社会智能行为。

Result: 在这个环境中调查了社会学习对代理性能的影响，包括在专家存在下的学习效果以及危机合作、工具共享等隐式合作行为的出现。

Conclusion: 该环境能够促进社会智能AI代理的发展，让代理能够通过合作或竞争在开放式多段环境中获得改善。
...
### [248] [Conditionally adaptive augmented Lagrangian method for physics-informed learning of forward and inverse problems using artificial neural networks](https://arxiv.org/abs/2508.15695)
*Qifeng Hu,Shamsulhaq Basir,Inanc Senocak*

Main category: cs.LG

TL;DR: 提出了PECANN框架的多项改进：广义化增广拉格朗日方法支持多惩罚参数、将约束执行重构为期望形式、引入傅里叶特征映射处理多尺度特征、提出时间窗口策略处理长时间演化，以及条件自适应惩罚更新策略，显著提升了PDE求解能力。


<details>
  <summary>Details</summary>
Motivation: 改进物理和等式约束神经网络(PECANN)框架，以更好地学习典型偏微分方程(PDE)的解，解决现有方法在处理异质约束、多尺度特征、长时间演化等方面的局限性。

Method: 1) 广义化增广拉格朗日方法支持多个独立惩罚参数；2) 将点约束执行和拉格朗日乘子重构为约束项的期望形式；3) 引入傅里叶特征映射处理振荡多尺度特征；4) 时间窗口策略确保长时间演化的连续性；5) 提出条件自适应惩罚更新(CAPU)策略。

Result: 在跨音速稀疏问题、涡旋被动平流、高波数Helmholtz和Poisson方程、空间变化热源反演识别等问题上表现出色，与现有方法和Kolmogorov-Arnold网络方法相比具有竞争性精度。
...
### [249] [Investigation of D-Wave quantum annealing for training Restricted Boltzmann Machines and mitigating catastrophic forgetting](https://arxiv.org/abs/2508.15697)
*Abdelmoula El-Yazizi,Yaroslav Koshka*

Main category: cs.LG

TL;DR: 本研究探讨了D-Wave量子退火器与经典MCMC在受限玻尔兹曼机采样中的微小统计差异，开发了混合采样方法但未能改善RBM训练效果，但首次展示了量子退火生成样本在缓解灾难性遗忘方面的潜力


<details>
  <summary>Details</summary>
Motivation: 解释先前研究中D-Wave量子退火采样未能显著改善RBM可训练性的原因，探索量子采样与经典采样之间的差异，并寻找量子计算在机器学习中的实际应用价值

Method: 提出新颖的混合采样方法，结合经典MCMC和量子退火采样；使用D-Wave硬件将RBM嵌入晶格结构；通过生成式回放方法测试灾难性遗忘缓解效果

Result: 混合采样方法未能改善RBM训练效果；量子采样与经典采样的差异主要体现在中低概率区域，这些区域对采样质量影响较小；首次证明量子退火生成样本可用于有效缓解灾难性遗忘，效果与经典方法相当但生成速度更快

Conclusion: 量子采样与经典采样的微小差异不足以改善RBM训练，但在生成多样化样本方面具有优势，特别在灾难性遗忘缓解等需要大量独特样本的机器学习应用中展现出潜力
...
### [250] [Communication Efficient LLM Pre-training with SparseLoCo](https://arxiv.org/abs/2508.15706)
*Amir Sarfi,Benjamin Thérien,Joel Lidin,Eugene Belilovsky*

Main category: cs.LG

TL;DR: SparseLoCo是一种针对大语言模型的高效通信训练算法，通过Top-k稀疏化和量化技术实现高达1-3%稀疏度和2位量化的极端压缩比，在通信受限环境下超越全精度DiLoCo的性能。


<details>
  <summary>Details</summary>
Motivation: 现有分布式训练算法在带宽受限环境（如跨数据中心）中仍存在通信瓶颈，即使减少通信频率也需要传输完整梯度副本，且性能略低于标准AdamW DDP基线。量化方法在大语言模型预训练中难以同时利用稀疏化技术。

Method: 提出SparseLoCo算法，结合Top-k稀疏化和量化技术，利用外部动量的局部近似（通过误差反馈和激进稀疏化实现），并发现稀疏聚合能实际提升模型性能。

Result: 在多种通信受限的大语言模型训练场景中，SparseLoCo在性能和通信成本方面均表现出显著优势，能够实现极端压缩比的同时超越全精度方法的性能。

Conclusion: SparseLoCo通过有效结合稀疏化和量化技术，成功解决了大语言模型分布式训练中的通信瓶颈问题，为带宽受限环境下的高效训练提供了有效解决方案。
...
### [251] [Intern-S1: A Scientific Multimodal Foundation Model](https://arxiv.org/abs/2508.15763)
...
Method: 采用多模态专家混合架构，持续预训练5T token（含2.5T科学数据），通过离线+在线强化学习在1000+任务上同步训练，提出混合奖励机制。

Result: 在通用推理任务上与开源模型竞争，在科学领域显著优于开源模型，在分子合成规划、反应条件预测等专业任务上超越闭源SOTA模型。

Conclusion: Intern-S1通过算法、数据和训练系统的集成创新，成功缩小了科学领域中开源与闭源模型的性能差距，为科学专业领域的AI应用提供了强大工具。
...
### [252] [Tutorial on the Probabilistic Unification of Estimation Theory, Machine Learning, and Generative AI](https://arxiv.org/abs/2508.15719)
*Mohammed Elmusrati*
...
<details>
  <summary>Details</summary>
Motivation: 从不确定、嘈杂的数据中提取意义是时间序列分析、模式识别和语言建模中的基本问题。论文旨在揭示不同AI方法背后的共享概率基础，为理解机器学习演进提供理论综合。

Method: 通过分析最大似然估计、贝叶斯推断和注意力机制等技术如何处理不确定性，使用系统辨识、图像分类和语言生成等示例场景，展示复杂模型如何在这些基础上构建。

Result: 论文证明了最大似然估计、MAP估计、贝叶斯分类和深度学习都代表了从噪声和/或有偏观察中推断隐藏原因的共享目标的不同方面，建立了统一的数学框架。

Conclusion: 这项工作既是对机器学习演进的理论综合，也是为学生和研究人员提供的实用指南，展示了不同AI方法在概率原理上的统一性。
...
### [253] [Probability Density from Latent Diffusion Models for Out-of-Distribution Detection](https://arxiv.org/abs/2508.15737)
*Joonas Järve,Karl Kaspar Haavel,Meelis Kull*

Main category: cs.LG

TL;DR: 本文探讨了在表示空间中基于似然的OOD检测性能，通过训练变分扩散模型在ResNet-18表示空间上进行评估，发现表示空间中的似然检测效果优于像素空间。


<details>
  <summary>Details</summary>
Motivation: 尽管似然在理论上是最优的OOD检测器，但在实践中经常失败。作者想探究这是否是像素空间的问题，还是表示空间也存在密度估计困难。

Method: 训练变分扩散模型在预训练ResNet-18的表示空间上，而不是在图像像素空间上，使用OpenOOD套件与最先进方法进行比较评估。

Result: 表示空间中的似然检测性能优于传统的像素空间方法，表明表示空间更适合基于似然的OOD检测。

Conclusion: 表示空间为基于似然的OOD检测提供了更好的环境，解决了像素空间中似然检测失败的问题，为部署安全的机器学习系统提供了新思路。
...
### [254] [Distributed Detection of Adversarial Attacks in Multi-Agent Reinforcement Learning with Continuous Action Space](https://arxiv.org/abs/2508.15764)
*Kiarash Kazari,Ezzeldin Shereen,György Dán*

Main category: cs.LG

TL;DR: 提出了一种针对连续动作空间多智能体强化学习的去中心化对抗攻击检测器，基于局部观测和统计特征，使用深度神经网络建模智能体正常行为为高斯分布，通过CUSUM算法实时检测异常


<details>
  <summary>Details</summary>
Motivation: 解决合作式多智能体强化学习在连续动作空间中面临的对抗攻击检测问题，需要一种去中心化的检测方法，仅依赖局部观测就能有效识别异常行为
...
Conclusion: 该方法能有效检测连续动作空间多智能体系统中的对抗攻击，具有高检测精度和实时性，为多智能体系统的安全性提供了可靠保障
...
### [255] [Discovering Hidden Algebraic Structures via Transformers with Rank-Aware Beam GRPO](https://arxiv.org/abs/2508.15766)
*Jaeha Lee,Gio Huh,Ning Su,Tony Yue YU*

Main category: cs.LG

TL;DR: 本文研究Transformer在多元多项式分解这一NP难代数任务中的非线性潜在模式发现能力，提出了数据生成管道、监督学习评估和新的强化学习方法BGRPO，显著提升了精度并降低了计算成本。
...
Result: BGRPO微调提高了精度，同时将beam宽度减少多达一半，推理计算成本降低约75%。模型在多项式简化方面表现出竞争力，在各种情况下优于Mathematica。

Conclusion: Transformer模型在复杂的代数任务中展现出强大的能力，提出的BGRPO方法有效提升了性能并显著降低了计算需求，为符号计算和代数推理提供了新的解决方案。
...
### [256] [A Scalable Trie Building Algorithm for High-Throughput Phyloanalysis of Wafer-Scale Digital Evolution Experiments](https://arxiv.org/abs/2508.15074)
*Vivaan Singhvi,Joey Wagner,Emily Dolson,Luis Zaman,Matthew Andres Moreno*

Main category: cs.NE

TL;DR: 这篇论文提出了一种改进的哺乳动物系式重建算法，在大规模数字进化实验中实现了300倍的速度提升，能够在2-3小时内完成1亿个基因组的系式重建。


<details>
  <summary>Details</summary>
Motivation: 虽然遗传层次算法能够在内存受限的AI硬件上进行分布式相关性估计，但现有方法的系式重建计算时间成为大规模系式分析的瓶颈。

Method: 设计了一种改进的哺乳动物系式重建算法，能够产生与现有方法等效的重建结果。

Result: 在10,000个组织的树中实现了300倍的速度提升；在954万亿次复制事件的WSE模拟中，对1亿个基因组进行了系式重建，结束到尾时间分别为2.6和2.9小时。

Conclusion: 该算法在显著改善重建缩放和吞吐量方面为大规模数字进化实验的高速通量系式分析技术奠定了关键基础。
...
### [257] [From Basic Affordances to Symbolic Thought: A Computational Phylogenesis of Biological Intelligence](https://arxiv.org/abs/2508.15082)
...
<details>
  <summary>Details</summary>
Motivation: 理解人类大脑为何能够进行符号推理而大多数其他动物不能，探索符号思维所需的神经机制，特别是动态绑定是否足够以及还需要哪些额外能力

Method: 通过17个通用模拟实验，比较具有和不具有多位置谓词和结构映射能力的认知架构在各种任务中的表现，确保任务不能通过诊断特征完成而必须依赖这些符号能力
...
### [258] [On the need to perform comprehensive evaluations of automated program repair benchmarks: Sorald case study](https://arxiv.org/abs/2508.15135)
*Sumudu Liyanage,Sherlock A. Licorish,Markus Wagner,Stephen G. MacDonell*

Main category: cs.SE

TL;DR: 本研究开发了一个全面的自动化程序修复(APR)工具评估框架，并以Sorald工具为例进行验证，发现其虽然能修复特定规则违规，但会引入新故障、降低代码功能正确性和代码结构质量。


<details>
  <summary>Details</summary>
Motivation: 现有的APR工具评估主要关注其清除违规的能力，而忽视了可能引入新违规、改变代码功能和降低代码结构质量等副作用，需要开发全面的评估框架。

Method: 以Sorald作为概念验证，评估其在2,393个Java代码片段中修复3,529个SonarQube违规的效果，分析修复后的代码质量变化。

Result: Sorald修复了特定规则违规，但引入了2,120个新故障（32个bug和2,088个代码异味），单元测试失败率达24%，代码结构质量下降。
...
### [259] [Foundational Design Principles and Patterns for Building Robust and Adaptive GenAI-Native Systems](https://arxiv.org/abs/2508.15411)
*Frederik Vandeputte*
...
Result: 构建了GenAI原生软件栈的关键要素框架，并从技术、用户采纳、经济、法律等角度分析了系统影响
...
### [260] [An Empirical Study of Knowledge Distillation for Code Understanding Tasks](https://arxiv.org/abs/2508.15423)
...
<details>
  <summary>Details</summary>
Motivation: 预训练语言模型在代码理解中表现出色但计算密集，知识蒸馏技术可解决部署时的计算和延迟问题，但在代码理解领域的潜力尚未充分探索

Method: 研究两种主流知识蒸馏方法（基于logit和基于特征），在8个学生模型和2个教师PLM上进行实验，涵盖三个下游任务

Result: 知识蒸馏显著提升不同规模学生模型的性能，代码专用PLM作为教师效果更好，特征蒸馏方法表现最优，相似架构不一定带来更好性能
...
### [261] [SynthCoder: A Synthetical Strategy to Tune LLMs for Code Completion](https://arxiv.org/abs/2508.15495)
*Dongjun Yu,Xiao Yan,Zhenrui Li,Jipeng Xiao,Haochuan He,Yongda Yu,Hao Zhang,Guoping Rong,Xiaobo Huang*

Main category: cs.SE

TL;DR: SynthCoder是一个针对代码补全任务优化的LLM模型，通过AST节点提取、BM25算法和调用图构建多样化数据集，采用课程学习和DPO两阶段训练，在多个代码补全基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有代码补全模型在优化过程中存在性能波动问题，某些数据集上的改进往往伴随其他数据集上的性能下降，甚至低于基线模型。需要一种能够稳定提升性能且避免代码重复问题的解决方案。

Method: 1) 结合AST节点提取和启发式方法构建多样化数据集；2) 使用BM25算法和调用图增强跨文件上下文信息；3) 以Seed-Coder-8B-Base为基础模型，采用课程学习进行微调；4) 使用拒绝采样生成的偏好对进行DPO对齐训练。

Result: 在aiXcoder、ExecRepoBench、CrossCodeEval和CoLT等主流仓库级代码补全基准测试中表现优异，同时有效缓解了模型重复现有代码的常见问题。

Conclusion: SynthCoder通过精心设计的数据集构建和两阶段训练策略，在代码补全任务中实现了稳定且全面的性能提升，为实时代码补全应用提供了有效的解决方案。
...
### [262] [Towards the Assessment of Task-based Chatbots: From the TOFU-R Snapshot to the BRASATO Curated Dataset](https://arxiv.org/abs/2508.15496)
...
<details>
  <summary>Details</summary>
Motivation: 任务型聊天机器人应用日益广泛，但对其可靠性、安全性和鲁棒性的评估研究不足，主要原因是缺乏大规模高质量数据集。现有评估技术往往基于有限样本或过时/不可用的聊天机器人，难以有效评估这些技术。

Method: 创建了两个数据集：TOFU-R（GitHub上Rasa聊天机器人的快照，代表开源Rasa开发的实践状态）和BRASATO（经过筛选的最相关聊天机器人集合，基于对话复杂性、功能复杂性和实用性标准）。同时提供了创建和维护这些数据集的工具支持。

Result: 提供了两个高质量数据集：TOFU-R包含GitHub上可用的Rasa聊天机器人快照；BRASATO是经过精心筛选的聊天机器人集合，重点关注对话复杂性、功能复杂性和实用性。这些数据集有助于促进聊天机器人可靠性研究的可重复性。

Conclusion: 提出的TOFU-R和BRASATO数据集及其工具支持为任务型聊天机器人的可靠性评估提供了重要资源，解决了该领域缺乏大规模高质量数据集的问题，有助于推动聊天机器人可靠性、安全性和鲁棒性研究的发展。
...
### [263] [Evaluation Guidelines for Empirical Studies in Software Engineering involving LLMs](https://arxiv.org/abs/2508.15503)
...
TL;DR: 该论文提出了针对LLM在软件工程研究中使用的分类法和8条指南，旨在解决LLM研究的可复现性和可复制性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在软件工程研究和实践中日益普及，但其非确定性、不透明的训练数据和不断演变的架构给实证研究的复现和复制带来了挑战。

Method: 提出了一个社区驱动的分类法，将LLM研究分为不同类型，并制定了8条设计指南，包括必须和应该遵循的标准。

Result: 建立了包含8条具体指南的框架：(1)声明LLM使用和角色；(2)报告模型版本和配置；(3)记录工具架构；(4)披露提示词和交互日志；(5)使用人工验证；(6)采用开源LLM作为基线；(7)报告合适的基准和指标；(8)公开说明局限性和缓解措施。

Conclusion: 这些指南旨在帮助研究社区克服LLM特有的开放科学障碍，确保研究的透明度和可复现性，并作为在线动态资源供社区使用和完善。
...
### [264] [QUPER-MAn: Benchmark-Guided Target Setting for Maintainability Requirements](https://arxiv.org/abs/2508.15512)
...
<details>
  <summary>Details</summary>
Motivation: 可维护性在软件开发中至关重要但常被忽视，需求工程可以通过促进讨论和设定适当目标来弥补这一差距

Method: 采用设计科学研究方法，开发了QUPER-MAn模型（QUPER模型的可维护性适配版本），整合可维护性基准并支持目标设定

Result: 研究发现可维护性仍然是次要质量关注点，显式需求通常只泛泛提及编码规范，工具虽常见但仅用于隐式需求
...
### [265] [A Novel Mutation Based Method for Detecting FPGA Logic Synthesis Tool Bugs](https://arxiv.org/abs/2508.15536)
*Yi Zhang,He Jiang,Xiaochen Li,Shikai Guo,Peiyu Zou,Zun Wang*

Main category: cs.SE

TL;DR: VERMEI是一种新的FPGA逻辑综合工具测试方法，通过僵尸逻辑识别、等效变异和差异测试来发现工具缺陷，在5个月内发现了15个bug。
...
Method: VERMEI包含三个模块：预处理模块通过仿真和覆盖率分析识别僵尸逻辑；等效变异模块在僵尸区域修剪或插入逻辑片段生成等效变体；缺陷识别模块基于差异测试比较合成输出。

Result: 在Yosys、Vivado和Quartus上的实验表明VERMEI优于最先进方法，5个月内向供应商报告了15个bug，其中9个被确认为新bug。

Conclusion: VERMEI通过利用僵尸逻辑和贝叶斯采样生成复杂逻辑变体，有效提升了FPGA逻辑综合工具的测试效果。
...
### [266] [Establishing Technical Debt Management -- A Five-Step Workshop Approach and an Action Research Study](https://arxiv.org/abs/2508.15570)
*Marion Wiese,Kamila Serwa,Anastasia Besier,Ariane S. Marion-Jetten,Eva Bittner*

Main category: cs.SE

TL;DR: 通过动作研究在IT公司建立技术债务管理过程，实践者偏好优先处理低垂果债务，后期提醒机制可提高TD意识


<details>
  <summary>Details</summary>
Motivation: 技术债务管理研究多但实践采用少，需要在实际IT环境中建立可持续的TDM过程

Method: 采用动作研究方法（16个月5个行动周期），结合问卷调查、团队会议观察、TD-SAGAT意识测量和待办事项数据分析

Result: 实践者偏向优先处理低垂果债务，通过待办事项的备忘录、复习日期等提醒机制可持续提高TD意识

Conclusion: 工作坊基础的TDM方法可行且能引发可持续的过程改变，提出了适用于其他IT团队的新方法如复习日期、TD讨论备忘录等
...
### [267] [From PREVENTion to REACTion: Enhancing Failure Resolution in Naval Systems](https://arxiv.org/abs/2508.15584)
...
Result: 成功在Fincantieri海军系统中部署了该解决方案，证明了方法的有效性
