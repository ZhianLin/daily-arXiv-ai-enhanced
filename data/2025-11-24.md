...
Motivation: 推动体育分析中的计算机视觉研究，解决现有数据集缺乏细粒度球拍姿态标注的问题，促进动态物体跟踪、条件运动预测和多模态分析的发展。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，设计三个相互关联的任务：细粒度球跟踪、关节球拍姿态估计和预测性球轨迹预测，并采用CrossAttention机制进行多模态融合。

Result: 评估显示，简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，使轨迹预测结果超越强单模态基线。

Conclusion: RacketVision为动态物体跟踪、条件运动预测和体育多模态分析提供了一个多功能资源和强大起点。
...
TL;DR: 使用持久同调（PH）方法客观分析艺术风格，能区分不同艺术家、同一流派内的艺术家，以及真实作品与AI生成作品


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家和评论家的主观判断，需要更客观、结构化的数学工具来分析艺术风格

Method: 采用拓扑数据分析中的持久同调（PH）方法

Result: PH方法能以统计确定性区分不同艺术家、同一艺术流派内的艺术家，并能区分艺术家真实作品与AI生成作品

Conclusion: 持久同调为艺术风格分析提供了客观且可解释的洞察工具
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化来解决训练数据不足的问题，能够生成多样化的面部表情并支持基于风格的图像编辑。
...
Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型，并且支持基于风格的图像编辑，特定风格参数对应不同的面部运动。
...
TL;DR: 提出了PairHuman数据集，这是首个专为高质量双人肖像生成设计的大规模基准数据集，包含10万+图像，并开发了DHumanDiff基线方法用于双人肖像生成。
...
Method: 构建PairHuman数据集（10万+图像，包含场景、服饰、互动等多样信息），开发DHumanDiff方法增强面部一致性并平衡个性化生成与语义驱动场景创建。
...
TL;DR: 本文提出了一种基于无监督自编码器和CDF 97小波变换的混合高斯-泊松去噪方法，用于中子成像数据的噪声去除，相比传统方法在重建误差和边缘保持方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中很重要，但图像常被高斯和泊松噪声共同污染，传统滤波和阈值方法难以有效区分和去除这些重叠噪声，需要开发能保持图像保真度的去噪技术。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，希望通过推理时的相关性反馈机制来改善检索效果，这种模型无关的设计既可作为微调的替代方案，也可与微调后的模型配合使用。

Method: 提出了四种相关性反馈策略：1）伪相关性反馈（PRF）-基于top结果优化查询嵌入；2）生成式相关性反馈（GRF）-使用合成标题进行查询优化；3）注意力反馈总结器（AFS）-基于transformer的多模态细粒度特征集成模型；4）使用真实标题的显式反馈作为上界基准。

Result: 在Flickr30k和COCO数据集上的实验表明，GRF、AFS和显式反馈相比无反馈检索，对小型VLM的MRR@5提升了3-5%，对大型VLM提升了1-3%。AFS和显式反馈能缓解查询漂移，在迭代多轮检索中比GRF更稳健。
...
TL;DR: SAM 3是一个统一模型，能够基于概念提示（名词短语或图像示例）在图像和视频中检测、分割和跟踪对象，通过解耦识别和定位显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有系统在基于概念提示的图像和视频分割任务中精度有限，需要开发更高效、准确的分割和跟踪模型。

Method: 构建可扩展数据引擎生成包含400万个独特概念标签的高质量数据集；采用共享骨干网络的图像级检测器和基于记忆的视频跟踪器；通过存在头解耦识别和定位。

Result: SAM 3在图像和视频提示概念分割任务中的精度比现有系统提高了一倍，并在视觉分割任务上改进了之前SAM的能力。

Conclusion: SAM 3在概念提示分割任务中实现了显著性能提升，开源模型和新的SA-Co基准将推动该领域发展。
...
TL;DR: 提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代品，在保持安全性的同时显著提升零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型安全微调方法导致泛化性能显著下降，原因是僵化的对齐策略破坏了模型学习的语义结构。

Method: 采用邻近感知方法，基于最小干预原则，将不安全概念重定向到语义上最接近的安全替代品。
...
TL;DR: 提出一个三阶段框架，从单视图SVG输入生成多视图一致的SVG，具有几何和颜色一致性，无需重新训练。
...
Method: 三阶段框架：1）将栅格化输入提升为3D表示并渲染多视图图像；2）扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立跨视图部件级对应关系；3）在栅格到矢量转换中进行路径整合和结构优化。

Result: 生成的SVG在多个视图间表现出强的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来改进自回归3D网格生成模型，解决质量与速度的权衡问题，并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统手动创建3D网格耗时且难以扩展，现有自回归方法存在严重的质量-速度权衡问题，且序列依赖限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术来检索、生成和集成网格组件，解耦生成过程的序列依赖。

Result: Mesh RAG显著提升了网格质量，加速了生成速度（相比顺序部件预测），并实现了增量编辑，且无需模型重新训练。

Conclusion: Mesh RAG为自回归网格生成模型提供了一个有效的训练无关增强方案，突破了质量与速度的权衡限制，并扩展了编辑能力。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合了LLM场景布局推理、程序生成、扩散式3D生成等技术。
...
Result: 系统能够生成几何一致、视觉丰富且在实时渲染中高效的世界，支持对布局、规模和风格的细粒度控制。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准数据集，专注于描述性和回归任务，连接多模态感知与可测量的生物物理变量，特别关注森林生态场景。


<details>
  <summary>Details</summary>
Motivation: 现有地球观测数据集主要关注语义理解任务，缺乏将多模态感知与可测量生物物理变量对齐的基准，无法满足科学回归分析的需求。

Method: 通过混合人机协作流程生成和验证结构化文本标注，整合了配准的Sentinel-2和ALOS-2影像数据，建立了认知可解释的逻辑链。
...
TL;DR: BOP-ASK是一个用于对象交互推理的大规模数据集，包含150k图像和33M问答对，涵盖6个任务，旨在评估和提升视觉语言模型在精细空间理解方面的能力。
...
Method: 利用BOP数据集中的6D物体姿态，生成精细标注包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系、物体间关系等，构建大规模数据集。

Result: 在BOP-ASK上训练的模型优于基线，展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行精细物体中心空间推理等新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，显著提升了模型在精细空间推理方面的能力，并发布了数据集和生成流程。
...
TL;DR: 提出了Parts-Mamba模型，一种混合GCN-Mamba架构，用于在骨架不完整或存在遮挡的情况下提升动作识别性能，通过部件特定扫描和部件-身体融合模块捕获远程关节上下文信息。


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中由于遮挡或通信质量问题导致的骨架不完整问题，现有GCN模型因缺少局部上下文而表现不佳。

Method: 结合图卷积网络和Mamba模型，采用部件特定扫描特征捕获部件特定信息，并通过部件-身体融合模块保持非相邻关节的上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效增强捕获和保持远程关节上下文信息的能力，在骨架不完整情况下显著提升动作识别性能。
...
TL;DR: 提出了联合Gromov-Wasserstein (JGW)目标，将传统GW距离扩展到支持多对多对象匹配，解决了传统GW只能进行一对一匹配的局限性。
...
Method: 通过引入JGW目标函数，扩展GW框架以实现对象集合的同步匹配，并采用最优传输中的传统算法（包括熵正则化）来求解点云对象表示。
...
TL;DR: 提出REPA方法，通过将扩散/流模型与预训练自监督编码器（如DINOv2）的表征对齐，来改进逆问题的重建质量，同时提高效率。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练生成模型作为先验使用时，如何提高重建保真度和感知真实性是一个挑战。

Method: 在推理时应用表征对齐（REPA），将扩散/流模型的内部表征与预训练自监督视觉编码器的表征对齐，指导重建过程。

Result: 实验表明REPA在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提高重建质量，并能减少所需离散化步骤数。

Conclusion: REPA为逆问题提供了一种有效的正则化方法，通过表征对齐显著提升重建性能，同时提供理论解释和效率优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的固有特性进行精确定位。观察到在真实场景中，玻璃前后光照强度差异会导致反射变化，这种反射动态特性可用于玻璃检测。

Method: 提出NFGlassNet方法，包含反射对比挖掘模块(RCMM)用于提取反射特征，反射引导注意力模块(RGAM)用于融合反射和玻璃表面特征。构建了包含3.3K闪光/无闪光图像对的数据集用于网络训练。

Result: 大量实验表明，该方法在玻璃表面检测任务上优于现有最先进方法。

Conclusion: 利用闪光/无闪光图像中的反射动态特性能够有效提升玻璃表面检测性能，NFGlassNet方法在多个场景下表现出优越性能。
...
TL;DR: 提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视频时空推理任务，填补了当前多模态大语言模型在复杂视频场景理解上的空白。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要关注简单视频场景，无法反映真实世界中复杂多样的音频-视频事件，需要专门的数据集和模型来支持细粒度的时空推理。

Method: 构建R-AVST数据集：使用LLM提取关键对象、自动空间标注和人工质量检查；提出AVST-Zero模型：基于强化学习，避免中间监督，通过多维度奖励直接优化行为。

Result: 构建了包含5K未剪辑视频、27K对象、100种音频-视频事件的数据集，生成了8K高质量问答对；AVST-Zero模型在音频-视频时空推理任务上表现出竞争力。
...
TL;DR: 本文提出了Warm Diffusion方法，通过联合控制模糊和噪声，结合了热扩散和冷扩散的优点，解决了热扩散忽略图像相关性和冷扩散忽略噪声作用的问题。
...
Method: 提出Warm Diffusion（模糊-噪声混合扩散模型BNMD），采用分治策略利用图像的光谱依赖性，通过分离去噪和去模糊过程简化分数模型估计，并分析模糊-噪声比（BNR）来研究模型学习动态与数据流形变化之间的权衡。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成方面的有效性。

Conclusion: Warm Diffusion成功整合了热扩散和冷扩散的优点，通过联合控制模糊和噪声实现了更好的图像生成性能。
...
TL;DR: 提出了Q-Real数据集，用于对AI生成图像进行细粒度的真实性和合理性评估，包含3,088张图像，标注了主要实体的位置、判断问题和归因描述，并构建了基准测试来评估多模态大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有的质量评估方法通常只提供单一评分，过于粗糙，无法为生成模型的改进提供针对性指导。真实性和合理性是AI生成图像的两个关键维度，需要细粒度评估来有效提升生成性能。

Method: 构建Q-Real数据集，包含3,088张由流行文本到图像模型生成的图像，标注实体位置、判断问题和归因描述；设计Q-Real Bench基准测试评估MLLMs在判断和基于推理的定位任务上的表现；提出微调框架来增强MLLM能力。

Result: 实验结果表明数据集质量高且意义重大，基准测试全面有效。

Conclusion: Q-Real数据集和基准测试为AI生成图像的细粒度质量评估提供了有效工具，有助于指导生成模型的优化和改进。
...
<details>
  <summary>Details</summary>
Motivation: 旨在实现模型、任务和表示三个层面的统一，消除模态差异，创建完全视觉原生的多模态学习框架。

Method: 使用统一的扩散变换器，在像素空间中训练整流流，将文本提示渲染为绘制文本图像，所有输入输出都作为RGB像素处理。

Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐和涌现可控性，如循环一致的图像-标题-图像循环。
...
<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决无真实异常样本或训练数据可用的情况。

Method: 提出Delta-Denoising方法：1）通过最小提示对驱动两个扩散分支进行对比；2）累积每步去噪差异生成图像特定定位图；3）使用定位图指导潜在修复，在保留上下文的同时生成局部缺陷；4）进行token级提示优化和空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成质量、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno是一种有效的零样本异常生成方法，无需训练即可生成逼真的异常，并在下游检测任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的视频超分辨率方法存在误差累积、空间伪影以及感知质量与保真度之间的权衡问题，主要原因是视频帧间的不准确对齐和补偿不足。

Method: 提出DGAF-VSR方法，包含光学引导变形模块（OGWM）在放大分辨率下保持对齐特征的高频细节，以及特征域时序条件模块（FTCM）在特征域提供密集引导。

Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量（DISTS减少35.82%）、保真度（PSNR增益0.20 dB）和时间一致性（tLPIPS减少30.37%）方面优于现有方法。

Conclusion: 特征域对齐和密集引导能有效提升基于扩散模型的视频超分辨率性能，在多个关键指标上达到最先进水平。
...
TL;DR: 提出了一种集成语义和形状感知的深度学习框架，用于CBCT图像中的牙齿分割，特别针对邻牙粘连导致的形状扭曲问题。


<details>
  <summary>Details</summary>
Motivation: CBCT图像中的邻牙粘连会导致严重的解剖形状扭曲，准确的牙齿分割对数字牙科至关重要但具有挑战性。

Method: 采用目标牙齿质心提示的多标签学习策略建模牙齿间语义关系，结合牙齿形状感知学习机制强制形态约束，通过多任务学习统一优化分割和形状保持。

Result: 在内部和外部数据集上的广泛评估表明，该方法显著优于现有方法。

Conclusion: 该方法有效减轻形状扭曲，提供解剖学上忠实的牙齿边界。
...
TL;DR: 提出了OmniGround基准测试集和PG-TAF方法来解决视频时空定位中的类别偏见、推理简化和语言鲁棒性问题，在复杂真实场景下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视频时空定位任务中与现实需求存在显著差距，主要由于基准测试范围有限导致模型存在类别偏见、推理过于简化和语言鲁棒性差的问题。

Method: 提出了OmniGround基准测试集（3,475个视频，81个类别）和Forward-Backward-Refinement标注流程；开发了DeepSTG评估框架；提出了PG-TAF训练免费两阶段框架，将STVG分解为高级时序定位和细粒度时空传播。

Result: 在复杂真实场景下性能平均下降10.4%，特别是在小/遮挡物体和复杂空间关系上表现不佳；PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试集和PG-TAF方法有效解决了视频时空定位中的关键挑战，为复杂真实场景下的模型评估和改进提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐私基准主要评估隐私感知，但无法应对视觉语言模型从分布式信息中推理和链接构建个人档案的新威胁。

Method: 提出了隐私感知与推理框架，构建了包含合成个人档案的双语多模态数据集，设计了9个挑战性任务来评估从属性检测到跨图像重识别和链式推理的全谱系能力。

Result: 评估了50多个基础模型和商业模型，发现许多模型存在显著的未测量推理隐私风险，感知级指标无法预测这些推理风险，现有安全对齐措施对此类推理攻击无效。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光流方程作为物理正则化，无需先验运动估计即可同时恢复时间相干图像和运动场。
...
Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光流场，通过光流方程耦合作为物理正则化，并加入数据一致性损失。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改善的时间保真度。

Conclusion: 隐式联合建模与流正则化约束在动态MRI重建中具有重要潜力。
...
TL;DR: 提出了FingerCap任务和FiGOP方法，用于生成捕捉手指级细节的手部动作文本描述，通过结合RGB帧和手部关键点来改善视频MLLMs对细微手指动作的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度的手部动作对于视觉感知、具身智能和多模态通信至关重要，但现有视频MLLMs由于时间稀疏性难以捕捉细微的手指动态。

Method: 引入FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时间编码器将关键点转换为运动嵌入并与RGB特征集成。

Result: 在FingerCap-40K数据集上的实验表明，现有视频MLLMs在手指标注任务上表现不佳，而FiGOP增强模型在HandJudge评估和人工研究中均取得一致提升。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉细粒度手指动作时的时间稀疏性问题，为手部动作理解提供了新思路。
...
TL;DR: 提出了一种基于点监督的面部表情检测框架，通过高斯分布建模实例级表情强度，结合类无关表情强度分支和类感知顶点分类分支，在仅需单时间戳标注的情况下实现表情检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖耗时的时间边界标注，本文研究点监督面部表情检测，只需每个实例的单时间戳标注，降低标注成本。

Method: 提出双分支框架：1) 高斯实例自适应强度建模模块用于软伪标签生成；2) 类感知顶点分类分支区分宏表情和微表情；3) 强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的实验验证了框架的有效性。

Conclusion: 所提出的点监督面部表情检测框架在减少标注成本的同时保持了良好的性能。
...
TL;DR: 提出了Neighbor GRPO方法，通过扰动ODE初始噪声条件生成多样化轨迹，使用基于距离的代理策略优化模型，完全绕过了SDE转换的需求。
...
Method: 通过扰动ODE初始噪声生成候选轨迹，使用基于softmax距离的代理跳跃策略优化模型，引入对称锚点采样和组内准范数重加权技术。

Result: Neighbor GRPO在训练成本、收敛速度和生成质量方面显著优于SDE-based方法，同时保持了确定性ODE采样的优势。

Conclusion: Neighbor GRPO为流匹配模型提供了一种高效且与高阶求解器兼容的对齐方法，解决了SDE-based GRPO的关键局限性。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的材质基础模型，通过将材质编码为RGB外观和PBR属性两个相关潜在变量，使用视频扩散架构统一处理多种材质任务。


<details>
  <summary>Details</summary>
Motivation: 现有的材质生成方法缺乏统一的表示来桥接自然图像外观和PBR属性，导致任务特定流程碎片化且无法利用大规模RGB图像数据。
...
TL;DR: 提出双分支退化提取网络解决盲超分辨率问题，通过预测模糊和噪声的无监督退化嵌入，让SR网络分别适应不同类型的退化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在已知固定退化（如双三次下采样）的SISR任务中表现优异，但当实际退化偏离假设时性能会显著下降。

Method: 使用双分支退化提取器网络预测模糊和噪声的无监督退化嵌入，将退化提取器作为正则化器利用SR和HR图像之间的差异。

Result: 在多个基准测试上的广泛实验表明，该方法在盲SR问题上达到了最先进的性能。

Conclusion: 提出的方法通过分别处理模糊和噪声退化，有效解决了盲超分辨率问题，并在实验中表现出色。
...
TL;DR: 提出了一种边缘设备上基于原始食物图像合成逼真烹饪过程图像的方法，通过引入烹饪状态引导的生成器和专门的烹饪图像相似度指标，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上从原始食物图像合成逼真烹饪图像具有挑战性，现有方法要么生成不真实，要么资源消耗过大，无法在边缘设备部署。

Method: 创建首个带有厨师标注熟度级别的烤箱烹饪进展数据集，提出边缘高效的食谱和烹饪状态引导生成器，引入领域特定的烹饪图像相似度指标作为训练损失和进度监控信号。

Result: 模型在多个数据集上显著优于现有基线，FID分数分别提升了30%（自有数据集）和60%（公共数据集）。

Conclusion: 该方法能够根据用户偏好的视觉目标而非固定预设来合成逼真食物图像，确保了时间一致性和烹饪合理性。
...
TL;DR: 提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的已知类结构风险和未知类开放空间风险之间的困境
...
Method: 语义感知提示增强模块分解图像为判别性语义标记；双工对比学习保持与已知类的可分性和语义邻近性；语义引导扩散模块通过扰动语义标记合成伪未知类
...
<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯基元来拟合场景，导致存储和计算开销巨大。现有的修剪方法依赖手动设计的标准或引入额外的可学习参数，效果不理想。
...
TL;DR: 提出了Synergistic Diversity (SQ)框架来提升植物病害检测精度，通过新的SQ指标衡量集成成员间的协同效应，改善集成模型选择并提高检测准确性。


<details>
  <summary>Details</summary>
Motivation: 植物病害每年造成超过2200亿美元经济损失，威胁全球粮食安全。深度神经网络集成虽然能提高预测精度，但现有集成多样性指标难以识别最优集成团队。

Method: 1. 分析现有集成多样性指标(Q指标)的局限性；2. 提出新的SQ指标来捕捉集成成员间的协同效应；3. 在植物叶片图像数据集上进行广泛实验验证。

Result: SQ指标显著改善了集成选择过程，并提高了植物病害检测的准确性。
...
TL;DR: 提出RadioKMoE框架，结合KAN和MoE进行无线地图估计，KAN预测粗粒度覆盖图，MoE网络基于环境和初始图进行精确估计，专家网络专注于不同模式以提升局部细节和全局一致性。


<details>
  <summary>Details</summary>
Motivation: 无线地图对网络管理和部署很重要，但复杂的无线传播行为和环境影响给无线地图估计带来挑战。

Method: 使用KAN模块预测初始粗粒度覆盖图，利用KAN在物理模型近似和全局传播模式的优势；结合环境信息和初始图，通过MoE网络进行精确估计，MoE包含专注于不同无线地图模式的专家网络。

Result: 在多频段和单频段无线地图估计实验中，RadioKMoE在准确性和鲁棒性方面表现优异。

Conclusion: RadioKMoE框架通过结合KAN和MoE，有效提升了无线地图估计的精度和鲁棒性。
...
TL;DR: DReX是一个仅使用视觉特征的图像复杂度预测模型，融合了自监督和卷积表示，在IC9600基准上达到SOTA性能，超越了多模态方法且参数更少。


<details>
  <summary>Details</summary>
Motivation: 探索语言信息对于图像复杂度预测是否必要，验证仅使用视觉特征是否能实现与人类感知一致的复杂度预测。

Method: 提出DReX模型，通过可学习注意力机制融合ResNet-50的多尺度层次特征和DINOv3 ViT-S/16的语义丰富表示，捕捉低层纹理模式和高层语义结构。

Result: 在IC9600基准上达到Pearson r = 0.9581，超越之前方法（包括多模态方法），参数减少约21.5倍，在多个数据集和指标上均表现优异。

Conclusion: 视觉特征本身足以实现与人类对齐的复杂度预测，自监督transformer和监督深度卷积神经网络经过适当融合能提供互补和协同优势。
...
Method: 引入DepthFocus，一个可操控的Vision Transformer，通过标量深度偏好作为条件，动态调整计算以聚焦到目标深度层。主要使用新构建的50万张多层合成数据集进行训练。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量证明了意图对齐的估计能力，并在未见过的透射场景中展现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动、类人3D感知迈出的重要一步，展示了在复杂场景中进行选择性深度感知的能力。
...
TL;DR: 提出MVLR模型，通过视觉语言模型和隐式记忆库来恢复多种恶劣天气条件下的图像退化问题，在保持计算效率的同时提升恢复精度。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶和室外机器人在雨、雾、雪等恶劣天气条件下可靠视觉感知的挑战性需求。

Method: 结合轻量级编码器-解码器主干网络、视觉语言模型和隐式记忆库，通过链式思维推理编码天气退化先验，并利用动态交叉注意力机制自适应融合多尺度视觉特征。
...
TL;DR: ConfusedTourist是一个文化对抗鲁棒性测试套件，用于评估视觉语言模型在多种文化线索共存时的稳定性。研究发现，即使是最先进的VLM在面临简单的图像堆叠扰动时准确率也会大幅下降，表明视觉文化概念混合会严重损害多模态理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型文化评估通常只关注单一文化概念，忽略了多种文化线索共存的现实场景。为了支持多元文化社会，需要测试模型在多样化文化输入中的稳定性。

Method: 开发了ConfusedTourist文化对抗鲁棒性测试套件，通过图像堆叠扰动和基于图像生成的变体来评估VLMs对地理线索扰动的稳定性，并进行可解释性分析。

Result: 实验显示VLMs存在严重脆弱性：在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动效果更差。可解释性分析表明失败源于系统性的注意力转移，模型被干扰线索分散了注意力。

Conclusion: 视觉文化概念混合会显著损害最先进VLMs的性能，凸显了开发更具文化鲁棒性的多模态理解模型的迫切需求。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去识别，使用新颖的试剂损失来指导身份编辑方向的优化。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以达到良好平衡，需要一种更有效的身份编辑方法。

Method: 将身份编辑重新解释为预训练无条件扩散模型潜在h空间中的语义位移，通过优化发现身份编辑方向，并提出线性和测地线编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留之间实现了优越的平衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过潜在空间中的身份替换，为人脸去识别提供了一种有效且无需训练的新方法。
...
TL;DR: 提出了一种直接从RGB图像估计弥散圆(CoC)图的混合现实合成方法，无需场景深度或相机参数，通过神经重模糊网络实现逼真的镜头模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖相机参数和场景深度来计算弥散圆以实现逼真的镜头模糊渲染，但这些信息普通用户通常无法获取，限制了方法的可访问性和通用性。

Method: 直接从RGB图像估计弥散圆图，通过虚拟物体的符号弥散圆图与深度之间的线性关系推断其CoC值，使用神经重模糊网络渲染逼真的镜头模糊。

Result: 实验结果表明，该方法实现了具有逼真散焦效果的高保真合成，在定性和定量评估中均优于现有最先进技术。

Conclusion: 该方法为实际应用提供了灵活实用的解决方案，能够实现高质量的无相机参数混合现实合成。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统3D室内场景创建需要大量手动工作和专业知识的痛点，让用户仅通过简单文本描述就能自动生成合理的3D室内布局。

Method: 使用分层语言驱动代理解析模糊提示为详细场景描述，通过点云初始化、空间约束优化和新型相机轨迹采样策略，结合3D高斯场景表示进行高效渲染。

Result: 能够在30分钟内生成几何合理的3D室内场景，在渲染速度和视觉质量上超越现有方法，同时保持可编辑性。

Conclusion: RoomPlanner为3D室内场景生成提供了高效、自动化的解决方案，显著降低了创建门槛并提升了生成效率。
...
TL;DR: PathAgent是一个基于大型语言模型的训练免费代理框架，模拟病理学家的逐步分析过程，通过导航器、感知器和执行器模块自主探索全切片图像，生成可解释的预测。
...
Method: 使用三个模块：导航器迭代定位重要微区域，感知器提取形态学视觉线索，执行器将这些发现整合到自然语言轨迹中，形成明确的思维链。

Result: 在五个挑战性数据集上评估，PathAgent展现出强大的零样本泛化能力，在开放和受限视觉问答任务中超越任务特定基线。

Conclusion: PathAgent作为透明且临床基础诊断助手具有潜力，与人类病理学家协作评估证实了其临床价值。
...
TL;DR: 提出了一个统一的行人跟踪框架OmniPT，能够交互式地跟踪、基于参考跟踪并生成跟踪对象的语义理解，在跟踪基准测试中表现优于先前方法。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像级任务表现出色，但在实例级任务如视觉定位和对象检测中仍有性能差距。行人跟踪任务需要高级语义理解，这正是LVLM的优势所在。

Method: 采用RL-Mid Training-SFT-RL训练阶段：首先通过RL使模型输出固定格式的边界框，然后使用大量行人相关数据集进行中间训练，接着在多个行人跟踪数据集上进行监督微调，最后再次进行RL以提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试中的实验结果表明，所提方法能够比先前方法表现更好。

Conclusion: 提出的OmniPT框架成功将行人跟踪任务建模为基础模型可执行的任务，并实现了优于先前方法的性能。
...
TL;DR: RL-AD-Net是一个基于强化学习的点云补全细化框架，通过在预训练点云自编码器的潜在空间中操作，选择性调整全局特征向量来改善几何一致性，提升补全质量。


<details>
  <summary>Details</summary>
Motivation: 现有点云补全模型（包括基于transformer、去噪等方法）虽然能从部分输入生成全局合理的形状，但常常存在局部几何不一致的问题，需要改进几何保真度。

Method: 使用强化学习在预训练点云自编码器的潜在空间中进行细化，编码器将补全结果压缩为全局特征向量，RL代理选择性调整这些特征。通过轻量级非参数PointNN选择器评估几何一致性，保留更好的重建结果。

Result: 在ShapeNetCore-2048上的实验表明，基线补全网络在其训练风格的裁剪下表现合理，但在随机裁剪场景中表现不佳。而RL-AD-Net在两种设置下都能持续提供改进，证明了RL引导集成细化的有效性。

Conclusion: 该方法轻量级、模块化且模型无关，适用于广泛的补全网络而无需重新训练，展示了强化学习在点云补全细化中的潜力。
...
TL;DR: REArtGS++提出了一种基于平面高斯泼溅和时间几何约束的通用化铰接物体重建方法，改进了REArtGS在螺旋关节和多部件物体上的不足。
...
Method: 使用解耦螺旋运动建模关节，通过部件运动混合联合优化部件感知高斯和关节参数，引入平面高斯和时间一致性正则化来施加几何约束。

Result: 在合成和真实铰接物体上的大量实验表明，该方法在部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时间几何约束和平面高斯泼溅，实现了更好的通用化铰接物体重建性能。
...
TL;DR: ReBrain是一个基于检索增强扩散框架的脑部MRI重建方法，能够从稀疏CT扫描中合成完整的脑部MRI体积，在稀疏条件下实现最先进的跨模态重建性能。


<details>
  <summary>Details</summary>
Motivation: 某些患者由于身体或临床限制无法进行MRI扫描，而低剂量CT协议产生的稀疏CT体积难以准确重建完整的脑部MRI体积。

Method: 使用Brownian Bridge扩散模型在2D维度合成MRI切片，同时通过微调的检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，通过ControlNet分支指导中间MRI切片的生成，确保结构连续性。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架能够有效解决稀疏CT扫描重建脑部MRI的挑战，为无法进行MRI扫描的患者提供可行的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型虽然效率高，但存在多样性崩溃问题，类似于少步蒸馏扩散模型中的多样性减少现象。

Method: 通过分析发现特征图中的关键组件控制早期尺度的多样性形成，在模型输入中抑制该组件，在输出中放大该组件。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中暴露随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择的灵活性受损。需要一种既能保持性能又能提供灵活编辑能力的方法。

Method: STAR利用图像块位置网格中均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR通过定制但结构化的随机策略，在保持后缀完成能力的同时维持采样性能，无需对语言AR建模中广泛采用的模型架构进行重大更改。

Conclusion: STAR建模成功解决了传统AR模型在视觉生成中的性能与灵活性权衡问题，为图像编辑提供了有效的解决方案。
...
Conclusion: 该方法仅使用稀疏视角RGB图像就能实现高质量的铰接物体重建，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大预训练模型的视频异常检测方法通常依赖密集帧级推理，计算成本高且延迟大。本文质疑在使用强大预训练模型时是否真的需要密集推理。

Method: 提出双通路框架：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中规模视觉语言模型生成文本事件描述和精炼异常分数，并集成大语言模型定期审查描述以识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD仅处理28.55%和16.04%的帧数就达到最先进的训练免费性能，证明了稀疏推理对于基于大模型的视频异常检测是足够的。

Conclusion: 研究表明，在使用强大预训练模型时，稀疏推理足以实现有效的视频异常检测，这为降低计算成本和延迟提供了新思路。
...
TL;DR: SketchVerify是一个无需训练、基于草图验证的运动规划框架，通过测试时采样和验证循环来生成更动态连贯的轨迹，在完整视频生成前提升运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法多采用单次规划或迭代精炼，前者运动简单，后者计算成本高。需要一种能在保持高效的同时提升运动规划质量的方法。

Method: 给定提示和参考图像，预测多个候选运动轨迹，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代精炼直到找到满意轨迹。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法在运动质量、物理真实性和长期一致性方面显著优于竞争基线，同时效率更高。

Conclusion: SketchVerify通过测试时采样和验证有效提升了运动规划质量，缩放轨迹候选数量能持续提升性能，为视频生成提供了更高效的规划方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉情感识别中，预训练视觉模型的事实级特征与情感类别之间缺乏直接关联（称为"情感鸿沟"），而文本模态具有明确的情感表达和高信息密度，可以消除这一鸿沟。
...
Method: ChainV首先基于前一步推理进行粗粒度视觉补丁选择，然后根据平均注意力强度识别最具代表性的原子视觉提示，并引入一致性评估机制来调整自反思程度。
...
<details>
  <summary>Details</summary>
Motivation: 大时空尺度下的刚体运动重建面临建模范式限制、严重运动模糊和物理一致性不足等挑战，需要新的方法来提升重建质量和物理合理性。
...
Conclusion: PEGS通过物理先验和事件流增强的融合，有效解决了大时空尺度刚体运动重建的挑战，在运动恢复和去模糊方面表现出色。
...
TL;DR: 本文提出了一种从1D CCT到2D色度空间的颜色映射方法，使用轻量级MLP替代传统CCT插值，显著提高了非普朗克光源（如LED）下的比色精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的颜色映射方法只适用于普朗克光源，而现代LED光源常常偏离普朗克轨迹，导致颜色精度下降。

Method: 使用2D色度空间特征训练轻量级多层感知机(MLP)，结合包含代表性LED光源的光箱校准程序。

Result: 在LED照明场景中平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时部署。

Conclusion: 2D色度空间结合MLP的方法比传统1D CCT方法在非普朗克光源下具有更好的颜色精度和实用性。
...
TL;DR: 提出了一个完整的多阶段优化框架，将高性能的浮点图像压缩模型转化为高效的硬件友好型整数实现，包括动态范围感知量化、渐进混合精度搜索和通道剪枝技术。
...
Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化处理极端数据异常值；2. 渐进混合精度搜索：为每层分配最优非均匀位宽；3. 通道剪枝：移除GDN层中的非活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%，硬件感知优化进一步降低20%以上计算复杂度，对RD性能影响可忽略。

Conclusion: 最终模型在效率和性能上都达到了最先进水平，优于现有的FPGA图像压缩实现。
...
Conclusion: ODTSR成功解决了真实世界图像超分辨率中保真度与可控性的平衡问题，为扩散基方法提供了新思路。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，相比传统Dice损失能更好地检测小病灶，提高召回率，在nnU-Net框架下表现优于blob loss。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）对小病灶分割不足，因为小病灶相对体积小，对总体损失贡献可忽略。需要基于每个病灶的评估方法。

Method: 提出CC-DiceCE损失函数，基于CC-Metrics框架，与现有的blob loss进行比较，在nnU-Net标准化框架下进行基准测试。

Result: CC-DiceCE损失提高了检测召回率，分割性能基本不受影响，但会产生稍多的假阳性。在多数据集研究中，CC-DiceCE通常优于blob loss。

Conclusion: CC-DiceCE损失函数能有效改善小病灶检测，在保持分割质量的同时提高召回率，是处理医学图像中小病灶分割问题的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中普遍存在小目标，实时检测需要平衡准确性和效率，现有方法在这方面存在挑战。

Method: 设计了双分支特征提取模块（DFE）分别提取局部特征和全局上下文，多尺度特征融合模块（MFF）增强多尺度集成，以及全局局部聚合特征金字塔网络（GLAFPN）进一步促进小目标检测。

Result: 在VisDrone2019和NWPU VHR-10数据集上的实验表明，DMG-YOLO在mAP、模型大小等关键指标上取得了有竞争力的性能。

Conclusion: DMG-YOLO通过创新的模块设计，有效解决了遥感图像小目标实时检测的挑战，在准确性和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。作者希望结合判别式方法的优势来指导生成式精炼过程。

Method: 第一阶段使用基于Transformer的提议解码器从传感器输入回归生成粗略轨迹预测；第二阶段应用扩散精炼器迭代去噪和精炼这些初始预测，并设计了细粒度去噪解码器来增强场景合规性。
...
TL;DR: UI-Styler是一个针对超声图像的类别感知风格迁移框架，通过模式匹配和类别感知提示策略解决跨设备域偏移问题，在分类和分割任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，这会降低固定黑盒下游推理模型的性能。现有无配对图像翻译方法往往忽略类别特定的语义对齐，影响诊断准确性。

Method: 提出UI-Styler框架，使用模式匹配机制将目标图像的纹理模式转移到源图像，同时保留源图像的结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中持续优于现有UIT方法，达到最先进性能。

Conclusion: UI-Styler通过有效的风格迁移和类别感知语义对齐，成功解决了超声图像跨设备域偏移问题，为黑盒下游推理模型的重用提供了实用解决方案。
...
Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像和气候数据；开发FireScope框架，基于VLM的推理到生成方法，通过强化学习和视觉监督学习预测风险图并生成推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope取得显著性能提升；专家反馈和自动分析确认其推理轨迹忠实且语义有意义。

Conclusion: 基于语言的推理可以改进视觉生成模型的泛化能力；首次提出可跨大陆应用的高分辨率野火风险模型；为多模态火灾风险模型的稳健跨大陆泛化研究提供系统方法。
...
TL;DR: 本文系统评估了自监督表征在音频-视觉深度伪造检测中的潜力，发现这些特征能捕获相关互补信息且关注语义区域，但无法跨数据集可靠泛化。


<details>
  <summary>Details</summary>
Motivation: 自监督表征在视觉和语音任务中表现出色，但在音频-视觉深度伪造检测中的应用潜力尚未充分探索。现有工作要么孤立使用这些特征，要么将其嵌入复杂架构中。

Method: 系统评估自监督表征在音频、视频和多模态上的表现，涵盖唇部动作和通用视觉内容等域，评估检测效果、可解释性和跨模态互补性三个维度。

Result: 大多数自监督特征能捕获深度伪造相关信息且信息互补，模型主要关注语义区域而非虚假伪影，但无法跨数据集可靠泛化。泛化失败源于数据集特性而非特征本身。
...
TL;DR: 本文提出了INTSD夜间交通标志数据集和LENS-Net网络，解决了夜间交通标志识别的挑战。LENS-Net结合自适应图像增强检测器和多模态分类器，在夜间条件下实现了鲁棒的交通标志识别。
...
Method: 首先创建INTSD大规模夜间交通标志数据集；然后提出LENS-Net网络，包含自适应图像增强检测器进行联合光照校正和标志定位，以及结构化多模态CLIP-GCNN分类器利用跨模态注意力和图推理。

Result: 该方法超越了现有框架，消融研究证实了其关键组件的有效性。数据集和代码已公开供研究使用。

Conclusion: 提出的INTSD数据集和LENS-Net网络为夜间交通标志识别提供了有效的解决方案，通过多模态融合和自适应增强技术显著提升了识别性能。
...
TL;DR: 本文提出了一种结合群等变卷积神经网络与等变感知结构化剪枝的新框架，用于在资源受限环境中构建紧凑的变换不变模型。该方法通过结构化剪枝减少29.3%参数，同时保持几何鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决在资源受限环境中部署变换不变模型的问题，特别是卫星图像分析和几何视觉任务中需要保持几何鲁棒性的同时减少计算开销。

Method: 使用e2cnn库通过C4循环群实现旋转等变性，结合结构化剪枝保持等变特性，采用自适应微调、动态INT8量化和知识蒸馏等技术的完整优化流程。

Result: 在EuroSAT、CIFAR-10和Rotated MNIST等数据集上验证有效，实现29.3%参数减少且保持显著精度恢复，证明结构化剪枝可在保持几何鲁棒性的同时实现大幅压缩。
...
TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹，通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，实现精确的相机运动控制和高品质视频生成。
...
Method: 提出查询共享交叉注意力模块，融合6-DoF相机位姿和2D渲染视频帧两种控制信号；采用两阶段训练策略：首先从位姿输入学习粗略相机控制，然后结合视觉信息细化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性方面优于现有方法超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam通过统一的特征表示融合相机位姿和视觉信息，显著提升了相机运动控制的精确性和生成视频的视觉质量。
...
### [70] [MuM: Multi-View Masked Image Modeling for 3D Vision](https://arxiv.org/abs/2511.17309)
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法主要优化语义理解而非几何推理，CroCo虽然针对3D理解但仍有改进空间，需要更简单可扩展的方法来学习3D视觉特征。

Method: 扩展掩码自编码(MAE)到任意多视角，对所有视角统一掩码，使用轻量级解码器和帧间注意力机制。

Result: 在重建、密集图像匹配和相对位姿估计等下游任务中，MuM超越了最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: MuM提供了一种更简单可扩展的多视角自监督学习方法，在3D视觉任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像中的噪声通常复杂且难以准确建模，现有基于噪声建模的方法在真实数据上效果有限，需要一种能处理复杂真实噪声的有效方法。

Method: 多阶段噪声解耦框架：1) 将噪声分解为显式和隐式分量；2) 使用现有噪声模型生成配对数据进行预训练；3) 引入高频小波引导网络处理隐式噪声；4) 采用分离预训练和联合微调的多阶段学习策略。

Result: 在公开数据集和自采集数据集上的大量实验表明，该框架优于现有最先进方法，能有效处理复杂真实噪声并显著提升高光谱图像质量。

Conclusion: 所提出的多阶段噪声解耦框架通过噪声分解和多阶段学习策略，成功解决了真实高光谱图像中复杂噪声的去除问题，显著提升了图像质量。
...
### [72] [Non-Parametric Probabilistic Robustness: A Conservative Metric with Optimized Perturbation Distributions](https://arxiv.org/abs/2511.17380)
...
<details>
  <summary>Details</summary>
Motivation: 现有的概率鲁棒性(PR)方法假设固定且已知的扰动分布，这在实践中不现实。为了解决这一局限性，需要一种不依赖预定义扰动分布的更实用的鲁棒性度量。

Method: 基于高斯混合模型(GMM)与多层感知机(MLP)头和双三次上采样开发NPPR估计器，覆盖各种输入依赖和输入独立的扰动场景，直接从数据中学习优化的扰动分布。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16进行广泛实验，验证NPPR作为更实用的鲁棒性度量，与现有方法相比，PR估计保守性提高达40%。

Conclusion: NPPR是一个更实用的鲁棒性度量，不依赖预定义的扰动分布，在分布不确定性下提供更保守的概率鲁棒性评估，建立了对抗鲁棒性、概率鲁棒性和非参数概率鲁棒性之间的理论关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在嵌入3D位置增强空间精度时，难以实现动作执行的时间一致性控制

Method: 提取视觉特征，将1D时间嵌入3D位置形成4D嵌入，通过交叉注意力融合；扩展空间动作表示加入时间信息，对齐多模态表示到LLM进行时空动作预测
...
### [74] [Intervene-All-Paths: Unified Mitigation of LVLM Hallucinations across Alignment Formats](https://arxiv.org/abs/2511.17254)
*Jiaye Qian,Ge Zheng,Yuchen Zhu,Sibei Yang*

Main category: cs.CV

TL;DR: 提出一个与LVLMs transformer因果架构对齐的综合干预框架，发现幻觉源于图像-输入文本、图像-输出文本和文本-文本路径的相互作用，并针对不同问答对齐格式提出识别和干预关键幻觉头的方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但仍然容易产生幻觉。本研究旨在理解幻觉产生的因果路径并开发有效的干预方法。

Method: 提出综合干预框架，分析图像到输入文本、图像到输出文本和文本到文本三种路径对幻觉的影响，针对判别式和生成式格式识别和干预关键幻觉头。

Result: 实验表明该方法在多个基准测试中持续减少各种对齐类型的幻觉。

Conclusion: LVLMs的幻觉并非来自单一因果路径，而是多种路径相互作用的结果，提出的干预方法能有效减少幻觉。
...
### [75] [Continual Alignment for SAM: Rethinking Foundation Models for Medical Image Segmentation in Continual Learning](https://arxiv.org/abs/2511.17201)
*Jiayi Wang,Wei Dai,Haoyu Wang,Sihan Yang,Haixia Bi,Jian Sun*

Main category: cs.CV

TL;DR: 提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在保持SAM零样本能力的同时提高医学图像分割的计算效率和性能，解决异构隐私政策下的联合训练问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的异构隐私政策使得在合并数据集上进行联合训练不可行，需要在不发生灾难性遗忘的情况下从数据流中持续学习。虽然SAM模型提供了强大的零样本先验，但其大参数量和计算开销限制了实际部署。

Method: 提出对齐层（Alignment Layer）作为轻量级即插即用模块，对齐编码器-解码器特征分布以高效适配SAM到特定医学图像；基于SAM和对齐层，提出CA-SAM持续学习策略，自动适配适当的对齐层来减轻灾难性遗忘，同时利用SAM的零样本先验保持对未见医学数据集的强性能。

Result: 在九个医学分割数据集上的持续学习场景实验中，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很高潜力，CA-SAM通过轻量级对齐层和持续学习策略有效解决了医学图像分割中的隐私政策和计算效率问题。
...
### [76] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
*Suchetan G. Uppur,Hemant Kumar,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像并使用语义掩码指导扩散模型生成，以创建复杂的边缘案例场景。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统训练中获取多样化点云数据，特别是复杂边缘案例的挑战，现有方法依赖手工制作的3D虚拟环境，耗时且难以捕捉真实世界复杂性。
...
### [77] [SING3R-SLAM: Submap-based Indoor Monocular Gaussian SLAM with 3D Reconstruction Priors](https://arxiv.org/abs/2511.17207)
*Kunyi Li,Michael Niemeyer,Sen Wang,Stefano Gasperini,Nassir Navab,Federico Tombari*

Main category: cs.CV

TL;DR: SING3R-SLAM是一个基于高斯表示的稠密RGB SLAM框架，通过结合局部一致的3D重建与全局高斯表示，解决SLAM中的漂移和冗余点云问题，实现高效多用途3D建图。


<details>
  <summary>Details</summary>
Motivation: 当前稠密3D重建技术能准确捕捉局部几何，但集成到SLAM中面临漂移和冗余点云问题，限制了效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪和重建模块构建局部一致的子图，然后逐步对齐并融合到全局高斯地图中，该地图强制执行跨视图几何一致性，同时为局部跟踪提供反馈以纠正漂移。

Result: 实验表明，SING3R-SLAM在跟踪、3D重建和新视角渲染方面达到最先进水平，跟踪精度提升超过12%，产生更精细的几何细节，同时保持紧凑的内存高效全局表示。

Conclusion: SING3R-SLAM通过全局一致且紧凑的高斯表示，有效解决了SLAM中的漂移和冗余问题，为多种下游应用提供了高效且多功能的3D建图解决方案。
...
TL;DR: 本文发现多语言文本到图像模型存在文化偏差问题，提出了基于神经元定位的文化激活和层定向文化增强两种方法，在保持图像质量的同时显著提升了跨语言文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言文本到图像模型在生成图像时往往产生文化中性或英语偏向的结果，因为语言承载着文化内涵，需要确保多语言提示下生成图像的文化一致性。

Method: 1）提出探测方法定位文化敏感信号到特定神经元；2）推理时文化激活放大已识别神经元；3）层定向文化增强仅更新文化相关层。

Result: 在CultureBench上的实验表明，相比强基线方法，该方法在文化一致性方面有持续改进，同时保持了图像保真度和多样性。

Conclusion: 通过定位文化敏感神经元并针对性增强，可以有效提升多语言文本到图像模型的文化一致性，且无需对整个模型进行微调。
...
### [79] [Scaling Self-Supervised and Cross-Modal Pretraining for Volumetric CT Transformers](https://arxiv.org/abs/2511.17209)
...
TL;DR: 提出一种针对鱼眼图像的失真感知BEV语义分割框架，通过高斯参数化处理非线形失真，无需去失真或透视校正即可生成连续的BEV语义地图。


<details>
  <summary>Details</summary>
Motivation: 解决鱼眼图像因极端非线性失真、遮挡和深度模糊导致的BEV语义分割挑战，特别是在停车和城市驾驶场景中。

Method: 使用多相机高分辨率鱼眼图像，通过几何反投影和逐像素深度分布估计，将图像像素通过高斯参数化提升到3D空间，预测空间均值和各向异性协方差来显式建模几何不确定性，然后通过可微分溅射融合到BEV表示中。

Result: 在复杂停车和城市驾驶场景中表现出强大的分割性能，在严重鱼眼失真和多样化环境条件下，可行驶区域的IoU达到87.75%，车辆的IoU达到57.26%。

Conclusion: 该框架能够有效处理鱼眼图像的几何失真，生成连续的不确定性感知语义地图，在具有挑战性的真实场景中实现了最先进的BEV分割性能。
...
TL;DR: 提出双域自适应网络，将预训练图像超分模型从模拟数据集高效适配到真实世界数据集，通过空间域参数选择更新和频域分支增强高频恢复能力。


<details>
  <summary>Details</summary>
Motivation: 真实图像超分面临复杂退化模式，但真实LR-HR数据有限，影响基础特征学习。预训练SR模型提供有价值先验知识，可改善泛化性、加速训练并减少真实数据需求。

Method: 双域自适应网络：1) 空间域自适应策略，选择性更新预训练模型参数并使用低秩自适应技术调整冻结参数；2) 频域自适应分支，结合输入频谱数据和空间域骨干网络中间特征推断HR频率图。

Result: 在RealSR、D2CRealSR和DRealSR等真实图像超分基准测试中，优于现有最先进模型。

Conclusion: 该方法能有效利用预训练SR模型知识，通过双域自适应策略提升真实世界图像超分性能，代码已开源。
...
### [82] [Designing and Generating Diverse, Equitable Face Image Datasets for Face Verification Tasks](https://arxiv.org/abs/2511.17393)
*Georgia Baltsou,Ioannis Sarridis,Christos Koutlis,Symeon Papadopoulos*

Main category: cs.CV

TL;DR: 提出DIF-V数据集和综合方法，通过生成模型创建多样化合成人脸图像，解决现有数据集中的种族、性别偏见问题，提升人脸验证系统的公平性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸图像数据集存在显著的种族、性别等人口统计特征偏见，限制了人脸验证系统的有效性和公平性。需要创建更包容和多样化的数据集来解决这些问题。

Method: 整合先进生成模型创建多样化高质量合成人脸图像，强调各种面部特征的多样性，确保符合身份证照片特征要求。构建包含27,780张图像、926个独特身份的DIF-V数据集。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接从相邻帧学习连续3D语义占据，在保持恒定内存的同时实现长距离监督和推理。


<details>
  <summary>Details</summary>
Motivation: 大规模3D标注成本高昂，现有方法要么依赖2D渲染一致性导致3D结构隐式出现，要么使用离散化体素网格限制空间精度和可扩展性。

Method: 使用独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入收缩场景表示保持近场细节同时压缩远场区域。
...
### [84] [Sparse Mixture-of-Experts for Multi-Channel Imaging: Are All Channel Interactions Required?](https://arxiv.org/abs/2511.17400)
*Sukwon Yun,Heming Yao,Burkhard Hoeckendorf,David Richmond,Aviv Regev,Russell Littman*

Main category: cs.CV

TL;DR: MoE-ViT提出了一种用于多通道图像的Vision Transformer混合专家架构，通过轻量级路由器选择最相关的通道进行注意力计算，显著提升效率而不牺牲性能。


<details>
  <summary>Details</summary>
Motivation: 多通道图像领域（如细胞染色或卫星图像）中，现有方法独立处理每个通道导致注意力计算呈二次增长，造成计算瓶颈和高训练成本。需要解决跨通道注意力的效率问题。

Method: 将每个通道视为专家，使用轻量级路由器为每个图像块选择最相关的专家进行注意力计算，避免对所有通道交互建模。

Result: 在JUMP-CP和So2Sat真实数据集上的实验表明，MoE-ViT实现了显著的效率提升，且在某些情况下性能还有所增强。

Conclusion: MoE-ViT是多通道成像领域实用且有吸引力的骨干网络，在保持性能的同时大幅提升计算效率。
...
TL;DR: 提出了一种基于四元数保真项的颜色图像盲去卷积方法，通过四元数卷积核建模颜色通道间的关系，有效去除伪影并提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度或单独处理每个颜色通道，忽略了颜色通道之间的相互关系，这限制了去模糊效果。

Method: 设计了一个新颖的四元数保真项，使用包含四个核的四元数卷积核：一个捕获整体模糊的非负卷积核，以及三个无约束的卷积核分别建模红、绿、蓝通道的未知相互依赖关系，并在盲去卷积过程中使用归一化四元数核来保持图像强度。
...
TL;DR: 提出一种基于知识蒸馏的新框架，利用在少量任务相关数据上微调的教师网络来减轻学生网络中的捷径学习问题，在医学图像分析中显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型容易学习训练数据中虚假相关但无关的特征作为捷径解决方案，在医学图像分析等高风险应用中，这可能导致模型不使用临床相关特征进行预测，影响鲁棒性并对患者造成危害。

Method: 提出知识蒸馏框架，使用在少量任务相关数据上微调的教师网络来指导在包含偏置特征的大数据集上训练的学生网络，针对不同类型捷径在不同网络层中的表现特点进行缓解。

Result: 在CheXpert、ISIC 2017和SimBA数据集上使用多种架构进行广泛实验，相比传统经验风险最小化、基于增强的偏置缓解和基于分组的偏置缓解方法，取得了持续改进。在许多情况下，即使在外分布测试数据上也能达到与在无偏置数据上训练的基线模型相当的性能。

Conclusion: 该方法在实际医学成像场景中具有实际应用价值，特别是在偏置标注有限且捷径特征难以事先识别的情况下。
...
### [87] [REMSA: An LLM Agent for Foundation Model Selection in Remote Sensing](https://arxiv.org/abs/2511.17442)
*Binger Chen,Tacettin Emre Bök,Behnood Rasti,Volker Markl,Begüm Demir*

Main category: cs.CV

TL;DR: 本文介绍了RSFM数据库(RS-FMD)和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个模型，REMSA是基于LLM的自动选择系统，在专家验证的基准测试中表现优于多个基线方法。


<details>
  <summary>Details</summary>
Motivation: 遥感基础模型(RSFM)在环境监测、灾害评估等任务中应用广泛，但由于文档分散、格式异构和部署约束多样，选择合适的模型仍然很困难。

Method: 构建了包含150多个模型的RSFM数据库，并开发了基于LLM的REMSA智能体，通过自然语言查询、上下文学习和透明解释来自动选择模型。

Result: 在75个专家验证查询场景的基准测试中，REMSA产生了900个配置，表现优于朴素智能体、密集检索和非结构化RAG-based LLM等基线方法。

Conclusion: REMSA能够有效解决遥感基础模型选择问题，完全基于公开元数据运行，无需访问私有或敏感数据，为遥感社区提供了实用的模型选择工具。
...
Conclusion: MolSight框架通过系统化的三阶段训练策略，有效解决了OCSR中立体化学信息识别的挑战，为化学信息学、药物发现和LLM应用提供了强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于估计俯仰角和偏航角，且在处理大角度输入时精度下降。需要一种能够可靠估计滚转角并提高其他姿态参数预测性能的方法。

Method: 提出BiFingerPose算法，探索双模态输入，包括来自触摸屏的电容图像和来自屏下指纹传感器的指纹补丁。这种双模态方法能够实现滚转角的可靠估计。

Result: 在12人用户研究中，BiFingerPose相比之前SOTA方法预测性能提升超过21%，任务完成效率提高2.5倍，用户操作准确率提升23%。能够可靠估计滚转角，其他姿态参数预测性能也大幅提升。

Conclusion: BiFingerPose展示了在增强认证安全性和改善交互体验方面的应用潜力，开发了相应原型展示交互可能性。双模态方法为指尖姿态估计提供了实用优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在三维空间的空间推理能力有限，主要因为使用的视觉编码器（如CLIP）仅关注实例级语义特征，导致空间信息丢失。

Method: 提出基于层次化融合几何和语义特征的空间感知视觉编码器SpatialGeo，通过层次适配器将自监督学习的几何特征补充到CLIP中，使用随机特征丢弃避免仅依赖CLIP编码器的平凡解。

Result: 在SpatialRGPT-Bench上，SpatialGeo将最先进模型的准确率提升至少8.0%，推理内存成本减少约50%。

Conclusion: SpatialGeo通过融合几何和语义特征有效提升了MLLMs的空间推理能力，在性能提升的同时显著降低了计算成本。
...
Method: 采用相对姿态初始化与显式特征匹配，随后进行局部联合优化，最后引入包含特征轨迹的全局捆绑调整来整合几何一致性约束。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视角合成方面均优于现有最先进方法，在挑战性场景中表现出优异的鲁棒性。

Conclusion: NoPe-NeRF++是首个将局部和全局线索与NeRF无缝结合的工作，验证了所提出设计选择的有效性。
...
TL;DR: 本文提出了一种在生成图像中准确渲染折射透明物体的方法，通过同步物体边界内外像素并使用斯涅尔折射定律进行扭曲和融合，解决了生成模型在光学物理约束方面的不足。


<details>
  <summary>Details</summary>
Motivation: 生成图像模型在合成透明物体时表现不佳，特别是折射效果，因为折射光线会与图像其他部分的表面相交形成颜色约束。现有模型未能充分学习光学定律来准确渲染折射物体。

Method: 在生成过程的每一步中，使用斯涅尔折射定律对物体边界内外像素进行扭曲和融合同步。对于通过折射或反射可见但未直接观察到的表面，通过将图像与以物体为中心的全景图进行同步来恢复其外观。

Result: 该方法生成了更加光学合理的图像，能够更好地遵守物理约束条件。

Conclusion: 通过同步像素和使用光学定律，能够显著改善生成图像中折射透明物体的渲染质量，使其更符合物理现实。
...
TL;DR: 提出了一个统一的多媒体绘画过程生成框架，通过语义驱动的风格控制机制，将多种媒介嵌入扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和过程转移。
...
Method: 使用语义驱动的风格控制机制，将多种媒介嵌入扩散模型条件空间，采用跨媒介风格增强和反向绘画训练策略，确保生成过程平滑且符合人类创作习惯。

Result: 构建了大规模真实绘画过程数据集，在跨媒介一致性、时间连贯性和最终图像保真度方面取得了良好结果，在LPIPS、DINO和CLIP指标上表现优异。

Conclusion: 提出的感知距离剖面(PDP)曲线能够定量建模创作序列（构图、色块、细节精炼），准确反映人类艺术创作进程，为绘画过程生成提供了有效解决方案。
...
TL;DR: 提出一种基于图卷积网络的骨架动作识别方法，通过优化目标函数学习最信息丰富的子集进行标注，实现标签高效学习。
...
Method: 设计新颖的获取函数，通过混合数据代表性、多样性和不确定性的目标函数优化，选择最信息丰富的子集进行标注；并使用可逆GCN将数据从环境空间映射到潜在空间以更好地捕获数据固有分布。

Result: 在两个具有挑战性的骨架识别数据集上的广泛实验表明，该方法有效且优于相关工作。

Conclusion: 提出的标签节俭GCN方法在骨架动作识别中表现出色，能够有效减少标注需求。
...
TL;DR: DSeq-JEPA是一种改进的联合嵌入预测架构，通过引入基于视觉重要性的判别性顺序预测，将JEPA的潜在预测与GPT风格的顺序推理相结合，提升了表示学习的效果。


<details>
  <summary>Details</summary>
Motivation: 现有的I-JEPA方法对所有区域进行均匀和独立的预测，缺乏对预测顺序和位置的明确指导。受人类视觉感知的启发，需要从最具信息量的区域到次要区域进行选择性、顺序性的注意力部署。

Method: 首先基于Transformer衍生的显著性图识别主要判别性区域，然后按照这个判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割以及低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功地将预测式自监督学习与自回归式自监督学习相结合，通过引入判别性顺序预测机制，显著提升了视觉表示学习的性能。
...
TL;DR: 提出了一种统一的注意力-曼巴（UAM）骨干网络，用于细胞级放射组学特征的分类，并扩展到多模态框架，在细胞分类和肿瘤分割任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 细胞级放射组学特征能提供肿瘤表型的细粒度洞察，但现有研究主要关注切片级或斑块级分类，缺乏专门针对放射组学数据的骨干网络设计。

Method: 引入统一注意力-曼巴（UAM）骨干网络，灵活结合注意力和曼巴模块，无需手动调整比例。开发两种UAM变体，并构建多模态框架同时进行细胞级分类和图像分割。

Result: UAM在公共基准测试中达到最先进性能：细胞分类准确率从74%提升至78%（n=349,882个细胞），肿瘤分割精度从75%提升至80%（n=406个斑块）。

Conclusion: UAM作为统一且可扩展的多模态基础，在放射组学驱动的癌症诊断中展现出有效性和前景。
...
<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛应用，但大量高斯基元显著增加内存需求且不适合实时推理。超二次曲面允许减少基元数量和内存需求，但缺乏超二次曲面光栅化器来支持模型监督。

Method: 采用超二次曲面场景表示，通过多层级球面细分的高斯近似实现高斯光栅化监督训练，并开发快速超二次曲面体素化模块。

Result: 在Occ3D数据集上，相比高斯方法减少75%内存占用、提升124%推理速度、mIoU提升5.9%，基元数量减少84%，实现首个实时推理的占据模型。

Conclusion: 超二次曲面表示能够显著减少内存需求和提升推理速度，同时保持竞争性性能，为实时自动驾驶场景理解提供了可行方案。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间中计算增强诱导的漂移向量来推断语义恢复方向，显著提升对抗攻击下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配中表现出色，但对图像对抗扰动高度脆弱，而对抗微调成本过高，现有测试时防御方法鲁棒性有限

Method: 在CLIP嵌入空间中操作，计算增强诱导的漂移向量，基于这些潜在漂移的角度一致性推断语义恢复方向并校正嵌入

Result: 在广泛基准测试中，ATAC始终实现显著高鲁棒性，平均比先前最先进方法高出近50%，计算开销极小

Conclusion: ATAC是CLIP嵌入空间中测试时对抗防御新范式的有效方法，在非常规和极端设置下保持最先进鲁棒性
...
TL;DR: SVRecon将稀疏体素栅格化与SDF结合用于高保真表面重建，通过几何初始化和空间平滑损失解决稀疏体素优化中的局部最小值问题，实现了快速收敛和准确重建。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素与邻居空间解耦且边界锐利，在优化过程中容易陷入局部最小值。虽然SDF值提供了平滑连续的几何场，但在独立参数化的稀疏体素间保持这种平滑性具有挑战性。

Method: 1) 使用视觉几何模型进行鲁棒的几何初始化；2) 通过空间平滑损失在父子体素组和兄弟体素组间强制执行一致关系，促进连贯平滑的体素结构。

Result: 在多个基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有一致的快速收敛性。

Conclusion: SVRecon成功地将稀疏体素与SDF结合，通过几何初始化和空间平滑约束解决了稀疏体素优化问题，实现了高效准确的三维表面重建。
...
TL;DR: MorphSeek：一种基于表示级策略优化的可变形图像配准方法，通过高斯策略头在潜在特征空间进行连续优化，实现高效探索和粗到精的配准。


<details>
  <summary>Details</summary>
Motivation: 解决传统强化学习方法在可变形图像配准中因将高维变形空间投影到低维表示而无法捕捉空间变化变形的问题。

Method: 在编码器上引入随机高斯策略头建模潜在特征分布，结合无监督预热和弱监督微调，通过多轨迹采样稳定训练。

Result: 在三个3D配准基准测试中（OASIS脑MRI、LiTS肝脏CT和腹部MR-CT）相比竞争基线获得一致的Dice改进，同时保持高标签效率和低参数成本。

Conclusion: MorphSeek提出了一种表示级策略学习范式，实现了空间一致且数据高效的变形优化，为高维场景下的视觉对齐提供了原则性、骨干无关和优化器无关的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实推理阶段经常出现部分模态不可用的情况，现有多模态模型无法处理模态缺失，且会因跨模态交互中断导致性能严重下降。

Method: 使用自适应门控模态生成器动态融合可用信息重建缺失模态；设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示；通过专家混合进一步细化和补充缺失模态。

Result: 在三个公共AQA基准测试中，MCMoE在完整和不完整多模态学习方面均取得最先进的结果。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet上实现+4.32%的鲁棒准确率和3.5%的零样本准确率提升。


<details>
  <summary>Details</summary>
Motivation: 传统单教师对抗蒸馏方法存在知识多样性有限、收敛速度慢、鲁棒性与准确性难以平衡等问题，需要更有效的多教师框架来提升视觉语言模型的对抗鲁棒性。
...
Conclusion: MMT-ARD框架能有效提升多模态大模型的对抗鲁棒性，具有良好的可扩展性和实用性。
...
TL;DR: 提出Illustrator's Depth新概念，将平面图像分解为可编辑的有序图层，支持图像矢量化、文本到矢量图形生成、3D浮雕生成和深度感知编辑等应用。
...
Method: 提出Illustrator's Depth定义，为每个像素推断图层索引，形成离散、全局一致的排序；训练神经网络从光栅输入直接预测分层，使用精心策划的分层矢量图形数据集。

Result: 在图像矢量化方面显著优于现有基线，支持高保真文本到矢量图形生成、从2D图像自动生成3D浮雕、直观的深度感知编辑。
...
Method: 基于无监督图像到激光雷达知识蒸馏，研究骨干网络架构对泛化性能的影响，预训练单一骨干网络，采用冻结预训练骨干网络并训练MLP头的方法。

Result: 在四个广泛认可且具有挑战性的设置中取得了最先进的结果。

Conclusion: 通过利用视觉基础模型和优化的骨干网络架构，可以实现激光雷达点云语义分割的有效无监督域自适应，显著提升跨域泛化性能。
...
TL;DR: 提出一种基于神经网络的GPR B-scan图像里程计方法，通过分析连续B-scan图像的相似性和差异性特征来精确估计欧几里得距离，在CMU-GPR数据集上表现优于现有方法。
...
Method: 设计新的定制神经网络，从连续时刻的B-scan图像中提取多尺度特征，通过分析这些特征之间的相似性和差异性来确定行驶的欧几里得距离。

Result: 在CMU-GPR数据集上的实验表明，该方法在所有测试中都优于最先进的对比方法，整体加权RMSE为0.449米，比最佳现有方法降低了10.2%的RMSE。

Conclusion: 该方法能够有效利用GPR B-scan图像的相似性和差异性特征，在恶劣环境条件下实现更精确的距离估计，为机器人/车辆定位提供了改进方案。
...
TL;DR: CWMDT是一个将标准视频扩散模型转化为反事实世界模型的框架，通过构建场景的数字孪生表示，使用大语言模型进行推理，生成反事实视觉序列。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要关注基于事实观察的前向模拟，但许多新兴应用需要回答反事实查询，如"如果移除这个物体会发生什么？"。传统世界模型在纠缠的像素空间中操作，无法对特定场景属性进行针对性干预。

Method: 1. 构建观察场景的数字孪生，显式编码对象及其关系（结构化文本表示）；2. 使用大语言模型推理这些表示，预测反事实干预如何随时间传播改变观察场景；3. 用修改后的表示条件化视频扩散模型，生成反事实视觉序列。

Result: 在两个基准测试上的评估表明，CWMDT方法实现了最先进的性能，表明视频的替代表示（如数字孪生）为基于视频前向模拟的世界模型提供了强大的控制信号。

Conclusion: CWMDT框架成功地将标准视频扩散模型转化为有效的反事实世界模型，通过结构化表示和大语言模型推理，实现了对场景属性的针对性干预和反事实查询能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法难以表示任意形状或处理有限视角下的真实雷达信号，而光学3D重建方法将雷达信号简单视为相机视图时效果不佳。

Method: 采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以雷达频率为条件，以从粗到细的方式扩散到这个潜在空间。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用18,000多个MRI系列图像来估计脊柱年龄，并发现脊柱年龄差距(SAG)与退行性脊柱疾病和生活方式因素相关。
...
Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性疾病集群，通过消融研究选择模型，使用深度学习从MRI图像估计脊柱年龄，并计算脊柱年龄差距(SAG)。
...
TL;DR: 论文分析了多模态模型中LLM降尺度对视觉能力的影响，发现视觉能力下降比推理能力更严重，提出了Extract+Think方法通过视觉提取调优和逐步推理来提升效率。


<details>
  <summary>Details</summary>
Motivation: 多模态模型规模扩大带来了视觉理解和推理的显著进步，但实际应用需要更小、更高效的系统。研究旨在分析LLM降尺度如何影响多模态能力。

Method: 提出Extract+Think方法：1) 视觉提取调优，训练模型提取指令相关的视觉细节；2) 基于提取的视觉细节进行逐步推理生成答案。

Result: 研究发现LLM降尺度对视觉能力的影响比对推理能力的影响更大，视觉提取调优方法显著提升了效率和性能。

Conclusion: Extract+Think方法为多模态模型的高效性能设定了新标准，解决了LLM降尺度导致的视觉能力瓶颈问题。
...
Method: 提出EvDiff事件扩散模型，采用单步前向扩散降低计算成本，配备时序一致的EvEncoder，并通过新颖的代理训练框架消除对配对事件-图像数据集的依赖。

Result: 在真实世界数据集上的实验表明，该方法在保真度和真实感之间达到最佳平衡，在像素级和感知指标上都优于现有方法。
...
Method: 创建大规模多模态指令引导3D编辑数据集，探索两种条件策略：传统交叉注意力机制和新型3D标记拼接方法。结果表明标记拼接更具参数效率且性能更优。

Result: 广泛评估显示该方法在生成质量、3D一致性和指令保真度方面优于现有的2D提升方法，设立了新的基准。

Conclusion: 提出的原生3D编辑框架通过高效的单次前向传播实现了高质量的3D编辑，在保持未编辑区域一致性的同时忠实遵循指令变化。
...
TL;DR: 开发了基于spaCy框架的绍纳语形态分析工具Shona spaCy，包含词法库和语言学规则，支持词性标注和形态特征分析，准确率达88-90%。
...
Method: 基于spaCy框架构建规则型形态分析流水线，结合JSON词法库和语言学规则，建模名词类前缀、动词一致性标记、时体标记、拟声词和附着词。

Result: 在正式和非正式绍纳语语料库上评估，词性标注准确率90%，形态特征准确率88%，工具可通过pip安装使用。
...
TL;DR: 提出语义金字塔索引(SPI)，一种多分辨率向量索引框架，通过查询自适应分辨率控制解决RAG系统中向量数据库检索的语义粒度不匹配问题。
...
Result: 在多个RAG任务中，SPI实现最高5.7倍检索加速和1.8倍内存效率提升，端到端QA F1分数提升最高2.5分。
...
Method: 开发Bench360框架，允许用户定义自定义任务、数据集和相关指标，自动测试选定LLM、推理引擎和量化级别在不同使用场景（单流、批处理和服务器）下的性能。

Result: 在四个常见LLM任务、三个硬件平台和四个推理引擎上的测试结果显示，任务性能和系统效率之间存在多种权衡，不同推理引擎和模型表现差异明显。

Conclusion: 本地LLM推理没有单一最佳配置，这强烈需要像Bench360这样的框架来帮助用户找到最优配置。
...
<details>
  <summary>Details</summary>
Motivation: 工业级LLM往往忽视低资源语言如突尼斯阿拉伯语的理解能力，这可能导致数百万突尼斯人无法用自己的语言与AI充分交互，被迫使用法语或英语，威胁方言保护并影响年轻一代的语言偏好。

Method: 构建包含突尼斯方言、标准突尼斯阿拉伯语和英语的平行翻译数据集，并带有情感标签。在三个任务上对多个流行LLM进行基准测试：音译、翻译和情感分析。

Result: 结果显示不同模型之间存在显著差异，突显了它们在理解和处理突尼斯方言方面的优势和局限性。通过量化这些差距，揭示了当前模型的不足。

Conclusion: 这项工作强调了在下一代AI系统中包含低资源语言的重要性，以确保技术保持可访问性、包容性和文化根基，促进语言多样性的保护。
...
TL;DR: EliDecide是一种新颖的文本开放意图分类方法，通过学习不同特征方向上的椭圆决策边界，解决了现有方法假设已知类各向同性分布的局限性，在多个文本意图基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自适应决策边界方法假设已知类具有各向同性分布，将边界限制为球形，忽视了不同方向上的分布差异，这限制了开放意图检测的能力。

Method: 1) 使用监督对比学习获得已知样本的判别性特征空间；2) 应用可学习矩阵参数化椭圆作为每个已知类的边界；3) 通过新颖设计的双重损失函数优化边界，平衡经验风险和开放空间风险。

Result: 在多个文本意图基准和问题分类数据集上实现了最先进的性能，椭圆边界的灵活性展示了优越的开放意图检测能力和在复杂开放世界场景中的泛化潜力。

Conclusion: EliDecide通过椭圆决策边界有效解决了已知类分布各向异性的问题，为文本分类任务在开放世界场景中的鲁棒性提供了有力支持。
...
TL;DR: 提出了一种评估提示词能否有效引导生成文本符合特定人类价值观的实用方法，无需模型微调即可实现价值引导。
...
Method: 开发了一种实用、可复现、模型无关的程序，通过评分方法量化生成响应中目标价值观的存在和增益，使用Schwartz基本人类价值理论和对话数据集进行结构化评估。

Result: 在Wizard-Vicuna语言模型变体上应用该方法，比较基线提示和明确基于价值观的提示，证明即使不改变模型或动态优化提示也能实现价值引导。

Conclusion: 价值引导是可行的，无需修改模型或动态优化提示，为动态价值观调整提供了实用解决方案。
...
TL;DR: 提出了基于概念梯度的可解释性技术，通过分析概念对模型输出的直接影响来改进毒性检测，并引入目标词典集和词-概念对齐分数来识别导致误分类的有毒词汇。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中毒性内容传播日益严重，现有毒性检测方法缺乏基于概念的解释，且概念对目标类别的不均衡归因常导致分类错误。

Method: 利用概念梯度方法提供因果解释，构建目标词典集捕获导致误分类的有毒词汇，计算词-概念对齐分数评估词汇重要性，并采用无词典增强策略生成不含预定义有毒词汇的样本。

Result: 开发了更准确的毒性检测可解释性框架，能够识别导致模型误分类的具体词汇和概念归因问题。

Conclusion: 概念梯度方法为毒性检测提供了更因果的解释，目标词典集和词-概念对齐分数有助于理解模型误分类机制，无词典增强策略揭示了模型对更广泛毒性语言模式的归因。
...
TL;DR: 阿拉伯语AI检测模型在检测轻微AI润色的人类文章时表现不佳，容易将润色后的人类文章误判为AI生成，导致准确率显著下降。
...
Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（半AI生成半人类撰写），用于评估14个LLM和商业AI检测器；第二个包含400篇人类文章经10个LLM使用4种润色设置处理后的16400个样本，用于评估8个最佳模型对轻微润色文章的检测能力。

Result: 所有AI检测器都会错误地将大量润色后的人类文章归为AI生成。最佳LLM Claude-4 Sonnet准确率从83.51%降至57.63%；最佳商业模型originality.AI准确率从92%暴跌至12%。

Conclusion: 轻微AI润色会严重影响AI检测模型的性能，导致对人类文章的误判，这威胁到AI检测器的可信度和作者的声誉。
...
TL;DR: 复现了基于最近邻的测试时训练方法，该方法通过在推理时对检索到的最近邻序列进行微调来提升语言模型性能。实验证实该方法能显著降低困惑度和比特率，特别是在结构化数据集上效果更明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在最近邻检索上的有效性，探索该方法在不同规模模型和不同领域数据上的通用性，同时解决大规模检索的内存效率问题。
...
Conclusion: 最近邻测试时训练具有鲁棒性和通用性，即使在现代推理优化架构上也能获得一致增益，同时内存高效检索实现使大规模检索增强适应更具可行性。
...
TL;DR: 多语言大模型在内部表示空间中如何组织语言信息的研究，发现语言信息在第一层就急剧分离，且在整个模型深度中保持线性可分，语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关。


<details>
  <summary>Details</summary>
Motivation: 虽然多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息的结构方式以及在不同层中的涌现机制缺乏系统性分析。

Method: 对6个多语言LLM进行全面的探测研究，覆盖所有268个transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一层transformer块中急剧分离（从第0层到第1层提升76.4±8.2个百分点），在整个模型深度中几乎完全线性可分。中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构印记效应。
...
TL;DR: 提出了一种基于语言模型的SNOMED CT本体嵌入方法，用于解决词汇外查询的分层概念检索问题，并在评估中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT中的知识检索由于语言歧义、同义词和多义词等问题而具有挑战性，特别是当查询是词汇外(OOV)时，即在本体中没有等效匹配。

Method: 基于语言模型的本体嵌入方法，用于从SNOMED CT中检索分层概念，特别针对OOV查询。

Result: 该方法在评估中优于包括SBERT和两种词汇匹配方法在内的基线方法，能够有效检索最直接的包含者及其较少相关的祖先概念。

Conclusion: 虽然针对SNOMED CT进行评估，但该方法具有通用性，可以扩展到其他本体。作者发布了代码、工具和评估数据集。
...
TL;DR: 研究发现共情行为（牺牲任务效率满足人类需求）在LLM激活空间中表现为线性方向，通过对比提示在三个不同模型上进行检测和操控实验，揭示了模型架构对共情编码和操控性的影响。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否存在可检测和操控的共情行为方向，理解安全训练对共情编码和操控性的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上测试共情行为的检测和操控能力。

Result: 所有模型在最佳层都能高精度检测共情行为（AUROC 0.996-1.00），未审查模型与安全训练模型表现相当。操控成功率：Qwen 65.3%、Phi-3 61.7%、Dolphin在增强共情方向94.4%但反方向失败。

Conclusion: 检测与操控能力存在模型差异，安全训练影响操控鲁棒性而非防止操控，Dolphin仅在增强共情方向表现稳健，需要更多模型验证。
...
TL;DR: JGU Mainz团队在BLP-2025代码生成共享任务中获得第一名，提出了一种多代理流水线方法，通过代码生成代理和调试代理的协作，实现了95.4%的Pass@1分数。
...
Method: 采用多代理流水线：代码生成代理首先生成初始解决方案，然后执行单元测试，仅将失败的测试案例转发给调试代理，调试代理根据错误信息、当前程序和相关测试案例生成修订解决方案。

Result: 在BLP-2025共享任务中获得第一名，Pass@1分数达到95.4%。

Conclusion: 多代理流水线方法在代码生成任务中表现优异，通过迭代调试显著提高了代码质量。
...
TL;DR: 本文提出了一个基于扩展心智理论和激进具身认知的翻译心智ABC框架，将翻译重新定义为整合情感、行为和认知过程的具身活动，而非静态表征操作。


<details>
  <summary>Details</summary>
Motivation: 挑战基于表征的心智模型，为翻译研究提供非表征主义的新视角，强调翻译是动态的脑-身体-环境交互中涌现的心智活动。

Method: 结合扩展心智理论、激进具身认知、预测处理和主动推理理论，构建翻译心智的ABC（情感-行为-认知）框架。

Result: 提出了翻译心智的非表征主义解释，将翻译视为在脑-身体-环境循环中涌现的技能性参与，意义在具身互动中实时共创。

Conclusion: 翻译应被理解为参与社会文化实践的技能活动，意义通过具身互动与文本、工具和语境的实时共创而产生，而非静态表征的转换。
...
TL;DR: 本研究重新探讨了非宾格动词与施事性/终结性之间的关系，使用可解释维度验证了语法结构与语义特征的联系。


<details>
  <summary>Details</summary>
Motivation: Kim等人(2024)发现人类对施事性和终结性的评分不能很好地预测不及物动词的句法行为，这促使本研究重新检验这一关系。

Method: 使用基于种子词的可解释维度来计算施事性和终结性，结合人类判断来评估语义属性。

Result: 研究结果支持了非作格/非宾格语法与施事性/终结性之间的联系，表明可解释维度与人类判断结合能提供有价值证据。

Conclusion: 使用可解释维度结合人类判断可以为难以通过评分任务评估的语义属性提供重要证据，验证了语法结构与语义特征的关系。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者可通过在输入提示中嵌入触发器来引导生成有害内容，需要有效的防御方法。

Method: PEPPER通过将输入提示重写为语义距离远但视觉相似的描述，同时添加不显眼的元素，来破坏嵌入的触发器并稀释触发词的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率同时保持生成质量，且能与现有防御方法结合使用。

Conclusion: PEPPER提供了一种有效的后门攻击防御方案，具有强鲁棒性和通用性，优于任何单独的防御方法。
...
<details>
  <summary>Details</summary>
Motivation: LLM经常生成长而冗余的响应，这降低了清晰度和用户满意度，并增加了模型开发者的成本
...
Conclusion: SCM通过软概念感知训练有效提升了LLM的推理能力，弥合了软概念推理与离散token训练之间的鸿沟。
...
TL;DR: 提出一种新的训练方案，通过将TRM的潜在推理视为分类器自由引导和隐式策略改进算法，显著提高训练效率，减少18倍前向传播次数，在ARC-1上达到24%准确率。
...
Result: 方法显著提升训练效率：总前向传播次数减少18倍，无需停止机制，在仅0.8M参数下ARC-1准确率达到24%，优于大多数LLMs。

Conclusion: 提出的训练方案能大幅提高TRMs的训练效率，同时保持与标准TRMs相当的质量，展示了小型模型在复杂推理任务上的潜力。
...
TL;DR: 本研究探讨了训练数据的统计特性与诱导头形成的关系，发现批量大小和上下文长度的组合、二元重复频率和可靠性是影响诱导头形成的关键因素。


<details>
  <summary>Details</summary>
Motivation: 诱导头是现代语言模型具有出色上下文学习能力的基础，但其形成机制尚不明确。本研究旨在揭示训练数据统计特性与诱导头形成之间的精确关系。

Method: 通过分析自然和合成数据的统计特性，研究批量大小、上下文长度、表面二元重复频率和可靠性对诱导头形成的影响。

Result: 发现：(1)批量大小和上下文长度的简单组合可预测诱导头形成点；(2)二元重复频率和可靠性强烈影响诱导头形成，存在精确的帕累托前沿；(3)高频率和可靠性的局部依赖足以形成诱导头，低值时类别性和边际分布形状变得重要。

Conclusion: 训练数据的统计特性（特别是二元重复频率和可靠性）对诱导头形成具有决定性影响，这为理解语言模型上下文学习能力提供了新的视角。
...
TL;DR: 提出了一种新颖的论点感知定量摘要任务，旨在揭示对话中的论点结构并量化论点强度，开发了ARQUSUMM框架来实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题需要不仅总结多样化论点，还要揭示其论证结构和理由。现有摘要方法忽视了对话的论证性质，无法深入分析句子内部的论点结构。
...
Conclusion: 该研究提出的论点感知定量摘要任务和ARQUSUMM框架能有效揭示对话中的论点结构，为用户提供更有帮助的摘要。
...
TL;DR: 本研究提出监督微调方法增强领域特定信息提取，针对湖南历史名人领域设计细粒度模板并构建指令调优数据集，对四个大语言模型进行参数高效微调，显著提升信息提取性能。


<details>
  <summary>Details</summary>
Motivation: 湖南历史名人系统数据资源有限，通用模型在低资源场景下领域知识提取和结构化输出生成表现不佳，需要专门方法提升文化传承领域的信息提取能力。
...
Result: 所有模型微调后性能均有显著提升，其中Qwen3-8B表现最佳，在100个样本和50次训练迭代下达到89.3866分。

Conclusion: 研究为垂直领域大语言模型微调提供了新思路，展示了在文化遗产知识提取和知识图谱构建中成本效益应用的潜力。
...
Result: 视觉语言模型存在召回偏向（过度预测高说服力），对低/中层次特征的判别能力较弱。高层次语义对齐是预测人类判断的最强指标。简洁的、基于对象的理由能显著提高精确度和F1分数。
...
TL;DR: 提出了一个可解释的自动评分框架AnalyticScore，通过提取可识别元素、使用LLM生成可解释特征和直观的序数逻辑回归模型，在保持高准确性的同时实现可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决自动评分系统缺乏透明度和可解释性的问题，满足大规模真实评估中对可解释自动评分日益增长的需求。

Method: 开发AnalyticScore框架：1) 提取响应中的明确可识别元素；2) 使用LLM将响应特征化为人类可解释的值；3) 应用直观的序数逻辑回归模型进行评分。

Result: AnalyticScore在ASAP-SAS数据集的10个项目上，平均仅比不可解释的SOTA方法低0.06 QWK，且特征化行为与人类标注者高度一致。
...
TL;DR: MUCH是首个用于公平评估声明级不确定性量化方法的基准，包含4,873个多语言样本，提供24个生成对数概率，并提出高效的确定性分割算法。
...
Method: 构建包含4种欧洲语言和4种开源LLM的数据集，提供完整的生成对数概率，开发高效的确定性分割算法用于实时监控。

Result: 创建了MUCH基准，包含4,873个样本，分割算法仅需0.2%的LLM生成时间，评估显示现有方法在性能和效率上仍有改进空间。

Conclusion: MUCH基准为声明级不确定性量化研究提供了重要工具，当前方法仍需在性能和效率方面进一步优化。
...
TL;DR: 首次在纯AMD硬件上大规模MoE预训练研究，使用MI300X GPU和Pollara互连，提供系统优化指南和模型设计规则，开发了ZAYA1-base模型，性能与主流模型相当。


<details>
  <summary>Details</summary>
Motivation: 验证AMD硬件、网络和软件栈在大规模预训练中的成熟度和竞争力，为基于AMD硬件的MoE模型训练提供实践指导。

Method: 在MI300X GPU集群上进行系统性能基准测试，开发MI300X感知的transformer尺寸规则，设计MoE宽度优化策略，构建完整的训练栈包括容错和检查点重塑功能。

Result: ZAYA1-base模型（7.6亿激活参数，83亿总参数）在推理、数学和编程基准测试中表现优异，性能与Qwen3-4B、Gemma3-12B相当，优于Llama-3-8B和OLMoE。

Conclusion: AMD硬件、网络和软件栈已足够成熟和优化，能够支持具有竞争力的大规模预训练，为AMD生态系统的发展提供了有力证明。
...
TL;DR: 本文提出使用上下文压缩作为预训练任务来改进LLM的文本表示能力，优于基于token级预测的方法，并在多种任务上表现出色且更高效。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要针对因果建模和下一个token预测优化，不适合生成整体文本表示。现有方法大多依赖token级预测目标，存在局限性。

Method: 采用上下文压缩作为预训练任务，模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测，并结合对比学习进一步改进。

Result: 压缩目标显著提升了LLM的文本表示能力，优于token级预训练任务。结合对比学习得到的LLM2Comp模型在广泛任务上优于当代LLM文本编码器，且训练数据需求更少。

Conclusion: 上下文压缩是有效的LLM适应预训练任务，能产生强大的文本表示模型，在性能和样本效率方面均优于现有方法。
...
TL;DR: 本文介绍了LangMark，一个用于自动后编辑的多语言数据集，包含7种语言的20.7万个三元组（源文本、机器翻译输出、人工后编辑文本），并证明LLM通过少量提示可以有效提升机器翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决自动后编辑领域缺乏针对神经机器翻译输出的大规模多语言数据集的问题，以促进APE系统的发展。

Method: 构建并发布LangMark数据集，包含英语到7种语言的翻译三元组，由专业语言学家标注；使用大语言模型进行少量提示的自动后编辑实验。

Result: LLM通过少量提示可以有效执行自动后编辑，改进领先的商业和专有机器翻译系统的翻译质量。
...
TL;DR: 本文介绍了PLLuM项目中用于微调波兰大语言模型的指令数据集，提出了有机、转换和合成指令的功能分类，并分享了关于使用人工编写与合成指令数据集在语言适应方面的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目开发的大语言模型提供指令数据集，并探索不同类型指令在语言模型适应中的影响。

Method: 构建包含有机、转换和合成指令的功能分类数据集，用于微调基于transformer的大语言模型。

Result: 发布了PLLuM指令语料库的首个代表性子集(PLLuMIC)，可为其他LLM的类似数据集开发提供指导。

Conclusion: 不同类型指令数据集对基础LLM的语言适应具有不同影响，发布的语料库可为其他语言模型开发提供参考。
...
TL;DR: 提出ABCA框架，通过因果推理分析LLM内部知识多样性，实现早期弃权决策，防止不可靠回答。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前预防不可靠响应。需要一种能够分析LLM内部知识多样性并实现早期弃权的框架。

Method: ABCA框架通过因果推理分析LLM参数知识的多方面性（如学科、法律背景、时间框架等），估计基于这些方面的因果效应来评估查询相关知识的可靠性。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到最先进性能，并增强了弃权决策的可解释性。

Conclusion: ABCA通过分析LLM内部知识多样性实现早期弃权，有效解决了现有方法无法提前预防不可靠响应的问题，在可靠性和可解释性方面均有显著提升。
...
TL;DR: 提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合，通过注意力机制动态确定各特征类型的相对重要性，在新闻文本分类任务中表现优于传统统计模型和纯语义深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法虽然能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法虽然理解语义但可能忽略简单但有效的统计指标。需要结合两者的优势来提升新闻文本分类性能。
...
Conclusion: 策略性地整合不同类型的特征可以显著提高分类准确性。该模型能够平衡并利用统计和语义表示的互补优势，是现实世界新闻分类任务的实用有效解决方案。
...
TL;DR: AutoLink是一个自主代理框架，将模式链接重新定义为迭代的代理驱动过程，在大型数据库上实现高召回率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 工业级文本到SQL中，由于上下文窗口限制和无关噪声，将整个数据库模式提供给大语言模型是不切实际的。现有方法成本高昂，难以平衡召回率和噪声，且在大数据库上扩展性差。

Method: AutoLink是一个自主代理框架，通过LLM引导动态探索和扩展链接的模式子集，逐步识别必要的模式组件，无需输入完整数据库模式。

Result: 在Bird-Dev上达到97.4%的严格模式链接召回率，在Spider-2.0-Lite上达到91.2%，执行准确率分别为68.7%和34.9%，在大模式（如超过3000列）上保持高召回率和高效令牌消耗。
...
TL;DR: E3Layer是一个任务有效、训练经济、推理高效的层剪枝框架，通过可微分掩码优化和熵感知自适应知识蒸馏，在保持高性能的同时实现模型压缩和加速。
...
Result: 在Qwen3-32B模型上剪枝25%层数时，在MATH-500上达到96%准确率，仅比原模型（96.8%）下降0.8%，优于现有SOTA方法（95%），推理速度提升1.33倍，仅消耗0.5B tokens（后训练数据量的0.5%）。

Conclusion: E3Layer框架在多种模型架构和基准测试中表现出优越性，能够有效平衡性能、训练成本和推理效率，为实际部署提供了可行的层剪枝解决方案。
...
TL;DR: 提出基于事件语义的对话记忆方法，将对话历史表示为短事件命题而非关系三元组或摘要，在保持非压缩形式的同时提高信息可访问性。
...
Method: 将每个会话分解为丰富的基本话语单元（EDUs），包含标准化实体和来源轮次属性，组织成异构图支持关联检索，构建基于密集相似性搜索和LLM过滤的检索变体。
...
TL;DR: PARROT框架用于测量LLM在权威和说服压力下的准确性退化，通过双盲评估隔离因果效应，量化置信度变化，并系统分类失败模式。研究发现先进模型抗压性强，而老旧/小模型出现严重认知崩溃。


<details>
  <summary>Details</summary>
Motivation: 测量大型语言模型在社交压力（特别是权威和说服）下的准确性退化现象，即过度顺从（sycophancy）问题，这对模型在现实世界中的安全部署至关重要。

Method: 使用PARROT框架：(i)双盲评估比较中性问题和权威性错误问题；(ii)基于对数似然的校准跟踪量化置信度变化；(iii)八状态行为分类法系统分类失败模式。评估22个模型，1302个MMLU风格选择题，涵盖13个领域。

Result: 模型表现异质性显著：先进模型（GPT-5、GPT-4.1、Claude Sonnet 4.5）跟随率低（≤11%，GPT-5仅4%），准确性损失小；而老旧/小模型出现严重认知崩溃（GPT-4：80%，Qwen 2.5-1.5B：94%）。国际法和全球知识领域脆弱性高，初等数学相对稳健。

Conclusion: 抵抗过度顺从压力应成为模型安全部署的主要目标之一，与准确性、伤害避免和隐私同等重要。
...
TL;DR: 提出了MirageTVQA基准测试，包含24种语言的6万QA对，用于评估视觉语言模型在多语言和视觉噪声条件下的表格推理能力，发现现有模型在视觉噪声下性能下降35%以上且存在英语优先偏见。


<details>
  <summary>Details</summary>
Motivation: 现有表格问答数据集如WikiTableQuestions和FinQA主要是英语且表格格式完美干净，与现实场景存在显著差距，需要评估模型在多语言和视觉噪声条件下的表现。

Method: 构建包含24种语言、近6万QA对的MirageTVQA基准测试，其中表格包含视觉噪声以模拟扫描文档，用于评估领先视觉语言模型的表格推理能力。

Result: 评估显示领先视觉语言模型存在两个主要失败点：面对视觉噪声时性能严重下降（最佳模型下降超过35%），以及持续的英语优先偏见导致推理能力无法迁移到其他语言。
...
Method: 结合四种方法：基于历史响应模式的查找数据库、针对常见行为的个性化LightGBM模型、融合文本和时间表示的混合神经网络架构用于罕见行为分类、文本回复生成

Result: 常见行为预测平均宏F1分数0.64，罕见行为分类器在10个罕见行为上宏F1分数0.56

Conclusion: 有效的社交媒体行为预测需要针对不同行为类型差异的定制建模策略
...
Method: 由翻译专家进行人工翻译和本地化适配，同时设计专门提示进行机器翻译实验，比较不同模型在人工和机器翻译测试集上的表现。

Result: 模型在人工翻译爱沙尼亚语测试集上的表现略低于英文原版，机器翻译数据表现更差，提示工程对翻译质量和模型准确性的提升有限。

Conclusion: 数据集翻译和适配需要语言专家参与，以确保对大型语言模型语言能力和推理能力的可靠评估，机器翻译目前难以达到人工翻译的质量。
...
TL;DR: 该研究分析了GPT-3.5、GPT-4、LlaMa 2、PaLM 2和Dolly 2等大型语言模型在南非英语、Sepedi和Setswana社交媒体帖子上的零样本情感分析性能，发现通过融合不同模型的输出可以将情感分类错误率降至1%以下。


<details>
  <summary>Details</summary>
Motivation: 在多元语言社区中，利用情感分析系统快速识别社交媒体中的社会挑战，帮助政府部门更精确有效地发现和解决问题。虽然LLMs在英语情感分析中表现出色，但尚未有研究探讨其在南非语言中的应用。

Method: 分析五种最先进的LLMs（GPT-3.5、GPT-4、LlaMa 2、PaLM 2、Dolly 2）在南非10个政府部门管辖范围内的10个新兴话题的英语、Sepedi和Setswana社交媒体帖子中的零样本情感极性分类性能。

Result: 不同LLMs、话题和语言之间存在显著差异。通过融合不同LLMs的输出结果，情感分类性能大幅提升，分类错误率低于1%。
...
TL;DR: 提出了HUMA系统，一个基于LLM的多用户对话代理，能够在群聊中使用人类化策略和时机参与对话，在实验中参与者难以区分AI与人类社区管理者。


<details>
  <summary>Details</summary>
Motivation: 当前大多数对话系统设计为一对一的轮换交流，而非自然的异步群聊。随着AI助手在数字平台中普及，开发自然的人类化交互模式对于维护用户信任和参与度至关重要。

Method: HUMA采用事件驱动架构，包含三个组件：路由器、行动代理和反思模块，能够处理消息、回复、反应并引入逼真的响应时间模拟，适应群聊动态。

Result: 在97名参与者的四人角色扮演聊天研究中，参与者对AI和人类社区管理者的分类准确率接近随机水平，主观体验在社区管理者有效性、社交存在感和参与满意度方面仅有微小差异。
...
TL;DR: 该研究通过语料库分析发现，在普通话会话中，单音节词的音高轮廓受到词义的显著影响，即使控制了多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索普通话单音节词在自然对话中的音高实现，特别关注词义对音高轮廓的影响，挑战传统声调理论。

Method: 使用广义加性模型分解观测到的音高轮廓，控制词长、性别、说话者身份、声调语境、元音高度和话语位置等变量，分析词义对音高轮廓的预测能力。

Result: 词义是音高实现的重要预测因素，同音异义词具有不同的音高轮廓，基于上下文化嵌入的预测准确率显著超过置换基线。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示将语言在视觉上下文中进行基础化，以克服文本偏见和表面启发式问题，实现更鲁棒的自然语言理解。

Method: 使用文本到图像模型生成前提的视觉表示，通过余弦相似度和视觉问答两种技术进行推理比较。

Result: 该方法在不进行任务特定微调的情况下实现了高准确率，并设计对抗性数据集验证了方法的鲁棒性。

Conclusion: 利用视觉模态作为意义表示为实现鲁棒自然语言理解提供了一个有前景的方向。
...
TL;DR: 提出了Selective RoPE，一种输入依赖的旋转位置嵌入机制，可在线性和softmax变换器中实现任意角度的旋转，提升了语言建模和序列任务的性能。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控机制的启发，希望将输入依赖的位置编码引入到softmax变换器中，以改进语言相关任务的表现。

Method: 引入Selective RoPE机制，通过输入依赖的旋转角度来编码位置信息，该机制可泛化RoPE，并在softmax注意力中揭示了隐藏的旋转结构。

Result: 在门控变换器中应用Selective RoPE后，在语言建模和困难序列任务（如复制、状态跟踪和检索）上表现出性能提升。

Conclusion: 输入依赖的旋转位置编码机制能够有效提升变换器在语言建模和序列任务中的表现，揭示了位置编码与选择性机制之间的内在联系。
...
TL;DR: PUCP-Metrix是一个包含182个语言指标的开源存储库，用于西班牙语文本分析，在自动可读性评估和机器生成文本检测任务中表现出色。
...
TL;DR: ReVeL框架将多项选择题改写为开放式问题，以解决MCQA中选项泄露信号导致评估不可靠的问题，提高模型训练和评估的准确性。
...
Result: 在多项选择基准测试中保持相同准确率，开放式问题准确率提高约6个百分点；在评估中揭示MCQA基准测试中高达20个百分点的分数膨胀。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标如ROUGE、METEOR和EM过于依赖n-gram词汇相似性，缺乏深层语义理解；BERTScore和MoverScore虽然使用上下文嵌入，但缺乏灵活性且忽略词汇相似性；LLM评估器成本高、有偏见且不一致。

Method: 提出SMILE指标，整合句子级语义理解、关键词级语义理解和简单关键词匹配，平衡词汇精确性和语义相关性。

Result: 在文本、图像和视频问答任务上的广泛基准测试显示，SMILE与人类判断高度相关，且计算轻量。

Conclusion: SMILE填补了词汇评估和语义评估之间的空白，提供了全面且高效的评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统RLVR在定理证明等数学推理任务中扩展性有限，因为中间推理过程至关重要而最终答案难以直接可靠验证；同时token级SFT容易退化为死记硬背而非诱导长链思维

Method: 两阶段训练：先在数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调；通过"掩码-填充"和"步骤重排序"构建过程级自监督奖励

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B上实现，在AIME24/25、AMC23和MATH500基准测试中，相比原始RLVR平均相对提升：Pass@1 +9.86%、Pass@5 +5.27%、Pass@8 +4.00%
...
### [160] [Fractional Artificial Neural Networks for Growth Models](https://arxiv.org/abs/2511.16676)
*Juan Carlos Najera-Tinoco,Martin P. Arciga-Alejandre,Jorge Sanchez-Ortiz,Francisco J. Ariza-Hernandez*

Main category: cs.NE

TL;DR: 提出了一种使用分数阶人工神经网络求解分数阶增长模型初值问题的方法，该方法基于Caputo导数的离散化实现，并在R软件中进行了实现。


<details>
  <summary>Details</summary>
Motivation: 需要解决分数阶增长模型（如广义指数和带周期性收获的Logistic模型）的初值问题，传统方法可能难以处理这类复杂的分数阶微分方程。

Method: 基于Caputo导数的离散化，构建分数阶人工神经网络，并在统计软件R中实现该方法。

Result: 通过示例展示了分析解与人工神经网络近似解的比较，验证了方法的有效性。

Conclusion: 提出的分数阶人工神经网络方法能够有效求解分数阶增长模型的初值问题，为这类复杂系统的数值求解提供了新途径。
...
### [161] [Jump-diffusion models of parametric volume-price distributions](https://arxiv.org/abs/2511.16838)
*Anup Budhathoki,Leonardo Rydin Gorjão,Pedro G. Lind,Shailendra Bhandari*

Main category: cs.NE

TL;DR: 该研究提出数据驱动框架分析纽交所股票量价分布的随机演化，通过拟合四种分布模型并去趋势化参数，使用Kramers-Moyal系数分析参数动态，发现不同模型的形状参数和尺度参数分别呈现扩散和跳跃扩散特征。


<details>
  <summary>Details</summary>
Motivation: 研究股票量价分布的随机演化动态，特别是识别参数变化中的扩散和跳跃成分，以更好理解市场波动机制。

Method: 从976个交易日每10分钟采样经验分布，拟合Gamma、逆Gamma、Weibull和对数正态分布，去趋势化形状和尺度参数，使用自适应分箱和回归提取Kramers-Moyal系数分析动态。

Result: Gamma、逆Gamma和Weibull模型的形状参数呈现纯扩散特征，尺度参数主导跳跃扩散；对数正态模型则相反；全局矩反演显示跳跃对尺度参数方差贡献显著。

Conclusion: 股票量价分布参数演化中，跳跃动态在尺度参数中占主导地位，对总体波动性有重要贡献，不同分布模型呈现相反的参数动态特征。
...
### [162] [Illuminating the Black Box of Reservoir Computing](https://arxiv.org/abs/2511.17003)
*Claus Metzner,Achim Schilling,Thomas Kinfe,Andreas Maier,Patrick Krauss*

Main category: cs.NE

TL;DR: 本文从性能最大化转向系统简化，研究储层计算机执行不同任务所需的最小计算要素，包括神经元数量、非线性程度和连接结构，发现某些情况下读出层承担主要计算，而储层仅提供弱非线性和记忆。


<details>
  <summary>Details</summary>
Motivation: 储层计算机虽然能执行广泛的信息处理任务，但其内部数据转换机制、输入矩阵、储层和读出层之间的相互作用，以及设计参数变化的影响仍不清楚。

Method: 通过系统简化方法，分析执行特定任务所需的最小神经元数量、非线性程度和连接结构，考虑非sigmoid激活函数和非随机连接网络。

Result: 发现非平凡情况下读出层承担主要计算，储层仅提供弱非线性和记忆；输入矩阵结构、激活函数陡度和输入/输出时序等次要设计因素在某些任务中成为系统性能的关键决定因素。

Conclusion: 储层计算机的设计需要重新思考，某些传统上被忽视的设计参数对系统性能具有关键影响，简化方法有助于识别任务所需的最小计算要素。
...
### [163] [δ-EMG: A Monotonic Graph Index for Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.16921)
*Liming Xiang,Jing Feng,Ziqi Yin,Zijian Li,Daihao Xue,Hongchao Qin,Ronghua Li,Guoren Wang*

Main category: cs.IR

TL;DR: 提出了一种误差有界的近似最近邻搜索方法，通过构建δ-EMG图结构，确保返回结果是真实值的(1/δ)近似，相比传统基于召回率的方法提供了更强的理论保证。


<details>
  <summary>Details</summary>
Motivation: 当前近似最近邻搜索算法主要基于ε-召回率原则，只关注正确结果的召回率，但无法保证错误结果的偏差范围。需要一种能提供近似误差保证的方法。

Method: 采用基于图的框架，提出δ-EMG（误差有界单调图），在构建时施加δ-单调几何约束，确保贪心搜索能收敛到(1/δ)近似邻居。进一步设计误差有界top-k搜索算法，并在查询时自适应控制近似精度。

Result: 在ANN-Benchmarks数据集上验证有效性，在召回率要求0.99时，在SIFT1M数据集上达到19,000 QPS，比其他方法提升超过40%。

Conclusion: 提出的误差有界近似最近邻搜索方法不仅提供了理论保证，在实际性能上也显著优于现有方法，为高维空间检索系统提供了更可靠的解决方案。
...
### [164] [RASTP: Representation-Aware Semantic Token Pruning for Generative Recommendation with Semantic Identifiers](https://arxiv.org/abs/2511.16943)
...
<details>
  <summary>Details</summary>
Motivation: 生成式推荐系统使用语义标识符(SIDs)表示物品，但多个SIDs会显著增加输入序列长度，导致计算复杂度和内存消耗增加。现有方法主要优化注意力计算和KV缓存，但直接剪枝输入序列中的低信息token可能更有效。

Method: RASTP通过结合语义显著性(通过表示幅度衡量)和注意力中心性(来自累积注意力权重)来评估token重要性，动态剪枝低信息或无关的语义token。

Result: 在三个真实世界的Amazon数据集上的实验表明，RASTP将训练时间减少了26.7%，同时保持或略微提高了推荐性能。

Conclusion: RASTP通过直接剪枝输入序列中的低信息语义token，有效降低了生成式推荐系统的计算开销，同时保持了推荐质量。
...
### [165] [CLLMRec: LLM-powered Cognitive-Aware Concept Recommendation via Semantic Alignment and Prerequisite Knowledge Distillation](https://arxiv.org/abs/2511.17041)
*Xiangrui Xiong,Yichuan Lu,Zifei Pan,Chang Sun*

Main category: cs.IR

TL;DR: CLLMRec是一个基于大语言模型的MOOC概念推荐框架，通过语义对齐和先验知识蒸馏解决传统方法依赖结构化知识图谱的局限性，实现认知感知的个性化推荐。


<details>
  <summary>Details</summary>
Motivation: 现有MOOC概念推荐方法严重依赖高质量结构化知识图谱，但在真实教育场景中这类图谱往往稀缺，限制了推荐系统的实际应用效果。

Method: 提出CLLMRec框架，包含两个核心技术：语义对齐组件构建学习者和概念的统一表示空间；先验知识蒸馏采用师生架构，从大语言模型中提取概念先决关系并蒸馏到高效排序器中。

Result: 在两个真实MOOC数据集上的实验表明，CLLMRec在多个评估指标上显著优于现有基线方法。

Conclusion: CLLMRec能够在无需显式结构化先验的情况下，生成真正认知感知和个性化的概念推荐，验证了其有效性。
...
### [166] [Parametric Retrieval-Augmented Generation using Latent Routing of LoRA Adapters](https://arxiv.org/abs/2511.17044)
*Zhan Su,Fengran Mo,Jian-yun Nie*

Main category: cs.IR

TL;DR: 提出Poly-PRAG方法，通过潜在路由编码过程解决传统参数化检索增强生成中一对一文档编码方案的数据稀缺和高推理开销问题。
...
Method: 将文档集合编码视为多任务学习过程，每个段落分配唯一任务标识符，使用路由函数通过少量潜在LoRA适配器编码整个段落空间，在线推理时根据查询选择性激活潜在专家子集。

Result: 在多个知识密集型NLP任务上的综合评估表明，该方法在四个不同数据集上取得了最先进的结果。
...
### [167] [Large language models for automated PRISMA 2020 adherence checking](https://arxiv.org/abs/2511.16707)
...
TL;DR: 构建了一个包含108篇CC许可系统评价的基准数据集，评估了10个LLM在5种输入格式下的PRISMA 2020指南依从性评估性能。


<details>
  <summary>Details</summary>
Motivation: 解决PRISMA 2020指南依从性评估在同行评审过程中缺乏可共享基准的问题。

Method: 使用108篇CC许可系统评价构建基准数据集，评估10个LLM在5种输入格式（Markdown、JSON、XML、纯文本、仅手稿）下的性能。

Result: 提供结构化PRISMA检查表的准确率达78.7-79.7%，显著高于仅手稿输入的45.21%。Qwen3-Max模型在完整数据集上达到95.1%敏感性和49.3%特异性。

Conclusion: 结构化检查表显著改善LLM的PRISMA评估性能，但编辑决策前仍需人类专家验证。
...
### [168] [Multi-Agent Code Verification with Compound Vulnerability Detection](https://arxiv.org/abs/2511.16708)
*Shreshth Rajan*

Main category: cs.SE

TL;DR: CodeX-Verify是一个多智能体系统，使用四个专门化智能体检测不同类型的代码缺陷，在保持高检测率的同时运行更快且无需测试执行。


<details>
  <summary>Details</summary>
Motivation: 现有工具只能捕获65%的缺陷且存在35%的误报率，LLM生成的代码存在严重质量问题（29.6%的SWE-bench补丁失败，62%的BaxBench解决方案有漏洞）。

Method: 构建多智能体系统，使用四个专门化智能体检测不同类型缺陷，通过数学证明组合不同检测模式的智能体比单一智能体发现更多缺陷。

Result: 在99个带验证标签的代码样本上检测到76.1%的缺陷，与最佳现有方法相当但运行更快；多智能体组合相比单一智能体准确率提高39.7个百分点；在300个真实补丁上测试，每个样本运行时间低于200ms。

Conclusion: 多智能体方法能显著提高代码缺陷检测效果，且发现同一代码中的多个漏洞会带来指数级风险增加（如SQL注入加凭证泄露风险增加15倍）。
...
### [169] [Is the Cure Still Worse Than the Disease? Test Overfitting by LLMs in Automated Program Repair](https://arxiv.org/abs/2511.16858)
...
<details>
  <summary>Details</summary>
Motivation: 自动化程序修复存在测试过拟合问题，即修复后的代码在已见测试上通过但在隐藏测试集上失败。虽然这个问题在大型语言模型兴起前已被研究，但需要验证在当前技术下是否仍然存在

Method: 使用仓库级别的SWE-bench任务进行实验研究

Result: 未在摘要中明确说明具体结果

Conclusion: 需要进一步实验来评估测试过拟合在当前程序修复技术中的严重程度
...
### [170] [MOOT: a Repository of Many Multi-Objective Optimization Tasks](https://arxiv.org/abs/2511.16882)
*Tim Menzies,Tao Chen,Yulong Ye,Kishan Kumar Ganguly,Amirali Rayegan,Srinath Srinivasan,Andre Lustosa*

Main category: cs.SE

TL;DR: MOOT是一个多目标优化任务库，包含120多个来自软件工程研究论文的任务，用于帮助研究人员和从业者探索软件工程中的权衡决策问题。


<details>
  <summary>Details</summary>
Motivation: 软件工程师需要在竞争性目标之间做出权衡决策（如速度vs成本、安全vs易用性、准确vs可解释等），但目前缺乏有效工具来探索这些权衡，导致研究人员难以处理这些权衡问题，工业实践者交付次优产品。

Method: 创建MOOT仓库，收集来自近期软件工程研究论文的多目标优化任务，涵盖软件配置、云调优、项目健康、过程建模、超参数优化等领域。

Result: MOOT目前包含120多个任务，在MIT许可下免费提供，位于github.com/timm/moot，支持社区贡献，能够支持数十个新的研究问题。

Conclusion: MOOT为这一重要领域的研究提供了数据基础，使研究人员能够更有效地探索软件工程中的多目标权衡问题。
...
### [171] [ReVul-CoT: Towards Effective Software Vulnerability Assessment with Retrieval-Augmented Generation and Chain-of-Thought Prompting](https://arxiv.org/abs/2511.17027)
*Zhijie Chen,Xiang Chen,Ziming Li,Jiacheng Xue,Chaoyang Gao*

Main category: cs.SE

TL;DR: 提出了ReVul-CoT框架，结合检索增强生成和思维链提示来改进软件漏洞评估，解决了LLMs在领域知识和深度推理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在软件漏洞评估中存在两个主要问题：缺乏领域特定知识，以及依赖浅层模式匹配而非深度上下文推理。

Method: 使用检索增强生成模块从本地知识库动态检索相关漏洞信息，结合思维链提示引导模型进行分步推理，评估可利用性、影响范围等因素。

Result: 在12,070个漏洞数据集上的实验表明，ReVul-CoT在MCC指标上优于最先进基线16.50%-42.26%，在准确率、F1分数和MCC上分别提升10.43%、15.86%和16.50%。

Conclusion: 结合RAG和CoT提示能显著提升基于LLM的软件漏洞评估性能，为未来研究指明了有前景的方向。
...
### [172] [UI-CUBE: Enterprise-Grade Computer Use Agent Benchmarking Beyond Task Accuracy to Operational Reliability](https://arxiv.org/abs/2511.17131)
*Horia Cristescu,Charles Park,Trong Canh Nguyen,Sergiu Talmacel,Alexandru-Gabriel Ilie,Stefan Adam*

Main category: cs.SE

TL;DR: UI-CUBE是一个系统性基准测试，包含226个任务，旨在评估计算机使用代理的企业部署准备度，揭示当前模型在复杂工作流中的根本架构限制。


<details>
  <summary>Details</summary>
Motivation: 当前CUA基准主要关注功能正确性，但缺乏对企业部署所需的操作可靠性的评估。需要系统性地暴露当前CUAs的基本架构限制。

Method: 开发UI-CUBE基准，包含简单UI交互（136任务）和复杂工作流（90任务），采用系统界面变化覆盖、多分辨率测试和通过应用状态自动验证任务成功的方法。

Result: 评估显示能力断崖而非渐进退化：简单任务成功率67-85%，复杂工作流骤降至9-19%。人类在简单任务达97.9%，复杂任务仅61.2%。

Conclusion: 当前CUAs能操作单个界面元素，但无法作为可靠的工作流自动化工具。性能断崖模式表明存在内存管理、分层规划和状态协调的根本架构限制。
...
### [173] [SlsReuse: LLM-Powered Serverless Function Reuse](https://arxiv.org/abs/2511.17262)
*Jinfeng Wen,Yuehan Sun*

Main category: cs.SE

TL;DR: SlsReuse是一个基于大语言模型的服务器无函数重用框架，通过构建可重用函数库、学习统一语义增强表示，并结合意图感知发现和多级剪枝策略来实现函数推荐。


<details>
  <summary>Details</summary>
Motivation: 解决服务器无计算中新手开发者面临的挑战，特别是从零开发函数需要适应异构、平台特定的编程风格，过程耗时且容易出错。函数重用为这些问题提供了有前景的解决方案。

Method: 构建可重用函数库作为基础知识库；通过有效的提示工程和少样本提示学习异构函数的统一语义增强表示；结合意图感知发现、多级剪枝策略和相似性匹配进行函数推荐。

Result: 在110个任务查询的精选数据集上评估，基于ChatGPT-4o的SlsReuse在Recall@10上达到91.20%，比最先进的基线高出24.53个百分点。

Conclusion: SlsReuse是首个基于LLM的服务器无函数重用框架，能够有效弥合任务描述与异构函数实现之间的语义鸿沟，显著提升函数推荐性能。
...
### [174] [Detecting Performance-Relevant Changes in Configurable Software Systems](https://arxiv.org/abs/2511.17271)
*Sebastian Böhm,Florian Sattler,Norbert Siegmund,Sven Apel*

Main category: cs.SE

TL;DR: ConfFLARE通过识别与性能相关代码的数据流交互来检测性能回归，并基于相关特征选择配置子集，显著减少性能测试配置数量。


<details>
  <summary>Details</summary>
Motivation: 软件系统性能具有波动性，频繁的性能分析成本高昂，特别是在可配置系统中需要测量多个配置。现有配置采样方法无法保证完整性，可能遗漏仅影响少数配置的性能回归。

Method: ConfFLARE通过分析代码变更与性能相关代码之间的数据流交互，识别可能影响性能的特征，并基于这些特征选择相关配置子集进行性能测试。

Result: 在合成和真实软件系统的研究中，ConfFLARE几乎在所有情况下正确检测到性能回归，并在除两个案例外的所有情况下识别出相关特征，平均减少79%（合成）和70%（真实场景）的配置测试数量，节省数小时性能测试时间。

Conclusion: ConfFLARE是一种有效的性能回归检测方法，通过智能配置选择显著降低测试成本，同时保持检测准确性。
...
### [175] [Framework Matters: Energy Efficiency of UI Automation Testing Frameworks](https://arxiv.org/abs/2511.17303)
*Timmie M. R. Lagermann,Kristina Sophia Carter,Su Mei Gwen Ho,Luís Cruz,Kerstin Eder,Maja H. Kirkeby*

Main category: cs.SE

TL;DR: 比较了四个Web UI自动化测试框架的能耗表现，发现不同框架执行相同UI操作的能耗差异可达6倍，Puppeteer在多数操作中最节能，而Nightwatch通常能耗最高。


<details>
  <summary>Details</summary>
Motivation: 研究不同Web UI自动化测试框架的能耗特性，为开发者提供能耗感知的测试设计指导。

Method: 在受控的客户端-服务器环境中使用外部功率计，对每个UI操作（刷新、点击、拖拽、输入等）重复执行35次，测量能耗。

Result: Puppeteer在左键点击、右键点击、双击、复选框和文本输入操作中最节能；Selenium在刷新和滚动操作中最节能；Nightwatch通常能耗最高；相同操作在不同框架间的能耗差异可达6倍。

Conclusion: 为UI自动化测试框架提供能耗透明度有助于开发者为特定UI操作做出明智的能耗感知测试决策。
...
### [176] [Agentic Program Verification](https://arxiv.org/abs/2511.17330)
...
### [177] [Exploring Scientific Debt: Harnessing AI for SATD Identification in Scientific Software](https://arxiv.org/abs/2511.17368)
*Eric L. Melin,Ahmed Musa Awon,Nasir U. Eisty,Neil A. Ernst,Shurui Zhou*

Main category: cs.SE

TL;DR: 本研究探索了科学软件中的自认技术债务，发现科学软件比通用软件包含9.25倍的科学债务和4.93倍的SATD，并开发了基于Transformer的SATD识别模型。


<details>
  <summary>Details</summary>
Motivation: 科学软件中的自认技术债务对研究结果的准确性和可重复性构成威胁，但SATD与科学软件的关系尚未被充分探索，存在重要研究空白。

Method: 分析了27个科学和通用软件仓库，在67,066个标注代码注释上微调比较了10个基于Transformer的模型（参数规模100M-7B）。

Result: 科学软件包含比通用软件多9.25倍的科学债务和4.93倍的SATD，主要由于复杂计算、领域约束和不断变化的研究需求。最佳模型性能优于现有方法。

Conclusion: 研究揭示了科学软件中SATD与通用软件的差异及其对质量和科学有效性的影响，为开发者和研究人员提供了管理技术债务、保障科学发现完整性的策略。
...
### [178] [CREST: Improving Interpretability and Effectiveness of Troubleshooting at Ericsson through Criterion-Specific Trouble Report Retrieval](https://arxiv.org/abs/2511.17417)
*Soroush Javdan,Pragash Krishnamoorthy,Olga Baysal*

Main category: cs.SE

TL;DR: CREST是一个基于多标准集成的方法，通过为不同故障报告字段训练专门模型来提升检索系统的效果和可解释性，从而加快故障解决速度。


<details>
  <summary>Details</summary>
Motivation: 电信行业快速发展需要高效的故障排除流程，但故障报告的复杂性和多样性给检索系统带来挑战。现有两阶段检索方法需要改进以更好地处理不同故障标准。

Method: 提出CREST方法，为不同TR字段训练专门化模型，然后集成这些模型的输出来捕捉多样化和互补的信号。

Result: 使用爱立信内部TR数据证明，基于特定标准的模型在关键评估指标上显著优于单一模型方法。

Conclusion: 所有目标标准对于优化检索系统性能都很重要，CREST方法提高了检索准确性、分数校准和可解释性。
...
TL;DR: Cognitive BASIC是一种基于BASIC风格的提示语言和模型内解释器，将大语言模型推理结构化为显式的逐步执行轨迹，通过简单的编号行和命令实现可解释的认知控制层。
...
TL;DR: 提出一个系统化基准修订框架，通过分析响应模式统计特征来识别无效问题，结合专家评审和LLM初审来高效改进AI基准质量。


<details>
  <summary>Details</summary>
Motivation: AI基准中的无效问题会破坏评估可靠性，手动检查数千个基准问题不可行且效率低下，需要系统化的自动检测方法。

Method: 基于模型性能均值充分性假设，通过统计响应模式分析识别超出预期范围的异常问题，结合专家评审和LLM初审来验证问题有效性。

Result: 在9个广泛使用的基准上，该方法指导专家评审识别问题问题的准确率高达84%，显著减少人工工作量。

Conclusion: 该框架为系统化基准修订提供了高效可扩展的解决方案，能够可靠地识别和修正基准中的无效问题。
...
### [182] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
*Ye Han,Lijun Zhang,Dejian Meng,Zhuang Zhang*

Main category: cs.AI

TL;DR: 提出混合差分奖励机制解决多车协同驾驶中传统奖励函数信号噪声比低的问题，显著提升算法收敛速度和策略稳定性


<details>
  <summary>Details</summary>
Motivation: 传统基于状态的奖励函数在多车协同驾驶高频连续控制任务中存在奖励差异消失问题，导致策略梯度信号噪声比低，阻碍算法收敛和性能提升

Method: 提出混合差分奖励机制，包含基于全局势函数的时序差分奖励和直接衡量动作边际效用的动作梯度奖励两个互补组件，在多智能体部分可观测马尔可夫博弈框架下实现

Result: 实验表明HDR机制显著提高了收敛速度和策略稳定性，引导智能体学习到平衡交通效率和安全的优质协同策略

Conclusion: HDR机制通过整合时序差分奖励和动作梯度奖励，有效解决了多车协同驾驶中奖励信号弱化问题，为高频连续控制任务提供了有效的奖励设计方法
...
### [183] [Comparing verbal, visual and combined explanations for Bayesian Network inferences](https://arxiv.org/abs/2511.16961)
*Erik P. Nyberg,Steven Mascaro,Ingrid Zukerman,Michael Wybrow,Duc-Minh Vo,Ann Nicholson*

Main category: cs.AI

TL;DR: 该论文设计了贝叶斯网络界面的语言和视觉扩展，以帮助用户理解推理过程。研究发现这些扩展能显著提高用户对观察影响、影响路径等问题的理解，且语言和视觉结合使用效果最佳。
...
Result: 所有三种扩展都比基线界面表现更好，特别是在观察影响、影响路径和观察间相互作用的问题上；语言和视觉模式结合使用在某些问题类型上优于单独使用任一模式。

Conclusion: 语言和视觉扩展能有效提升用户对贝叶斯网络推理的理解，多模态方法比单一模态方法效果更好。
...
### [184] [MirrorMind: Empowering OmniScientist with the Expert Perspectives and Collective Knowledge of Human Scientists](https://arxiv.org/abs/2511.16997)
*Qingbin Zeng,Bingbing Fan,Zhiyu Chen,Sijian Ren,Zhilun Zhou,Xuhua Zhang,Yuanyi Zhen,Fengli Xu,Yong Li,Tie-Yan Liu*

Main category: cs.AI

TL;DR: MirrorMind是一个分层认知架构，通过整合双记忆表示来模拟科学发现的社会和历史本质，包含个体认知轨迹和集体学科记忆两个层面。
...
Method: 提出MirrorMind三层架构：个体层构建研究者的认知模型，领域层将集体知识映射为结构化学科概念图，跨学科层作为正交编排引擎。该架构分离记忆存储与智能体执行。

Result: 在四个综合任务中评估，包括作者级认知模拟、互补推理、跨学科协作促进和多智能体科学问题解决。结果显示MirrorMind能够实现结构化、个性化和洞察生成的科学推理。

Conclusion: 通过整合个体认知深度和集体学科广度，MirrorMind超越了简单事实检索，实现了结构化的、个性化的和洞察生成的科学推理。
...
### [185] [Budget-Aware Tool-Use Enables Effective Agent Scaling](https://arxiv.org/abs/2511.17006)
...
TL;DR: 本文研究了工具增强型代理在明确工具调用预算下的有效扩展方法，提出了预算跟踪器和BATS框架，通过预算意识改善代理的性能扩展曲线。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强型代理缺乏预算意识，即使增加工具调用预算也无法提升性能，需要研究如何在明确预算约束下有效扩展代理能力。

Method: 提出预算跟踪器插件提供持续预算意识，开发BATS框架动态调整规划和验证策略，并建立统一的成本度量标准联合考虑token和工具消耗。

Result: 预算感知方法产生更优的扩展曲线，推动成本-性能帕累托前沿，为工具增强型代理的扩展提供更透明和原则性的理解。

Conclusion: 预算意识是工具增强型代理有效扩展的关键，提出的方法能够系统性地改善代理在预算约束下的性能表现。
...
### [186] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 本文重新解释扩散模型在逆问题求解中的作用，提出DAPS++方法将扩散阶段与数据驱动优化解耦，提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要受测量一致性驱动，与扩散动力学脱节。需要澄清这种结构并改进推理过程。

Method: 将扩散重新解释为期望最大化框架中的初始化阶段，提出DAPS++方法，让似然项更直接地指导推理，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 扩散在逆问题求解中的作用应被视为初始化阶段，DAPS++通过解耦扩散与数据驱动优化，提供了更有效的推理框架。
...
### [187] [Patient-level Information Extraction by Consistent Integration of Textual and Tabular Evidence with Bayesian Networks](https://arxiv.org/abs/2511.17056)
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中既有结构化信息也有非结构化文本，需要有效整合这两种数据源来构建透明的临床决策支持系统。

Method: 使用专家指导的贝叶斯网络处理结构化EHR数据，神经网络分类器处理临床文本，通过虚拟证据和一致性节点进行概率融合。

Result: 在SimSUM模拟数据集上验证，一致性节点相比单独使用虚拟证据能改善预测校准，更好地处理缺失信息和解决数据矛盾。

Conclusion: 该方法能够有效整合多模态医疗数据，提供可解释的概率预测，改善临床决策支持系统的性能。
...
### [188] [The Belief-Desire-Intention Ontology for modelling mental reality and agency](https://arxiv.org/abs/2511.17162)
...
TL;DR: 本文提出了一个正式的BDI本体，作为模块化本体设计模式，用于表示智能体的信念-愿望-意图认知架构，并通过与LLMs和语义推理平台的集成实验验证了其应用价值。


<details>
  <summary>Details</summary>
Motivation: BDI模型是人工智能和认知科学中表示理性智能体的基石，但其与结构化、语义可互操作知识表示的集成仍然有限，需要开发形式化的本体来支持认知架构的语义表示。

Method: 设计了一个模块化的BDI本体模式，与基础本体对齐确保语义精确性和可重用性；通过两种实验验证：1) 与大型语言模型通过逻辑增强生成结合；2) 在Semas推理平台中集成，实现RDF三元组与智能体心理状态的双向转换。

Result: 实验表明BDI本体能够作为声明性和程序性智能之间的概念和操作桥梁，支持认知基础、可解释且语义可互操作的多智能体和神经符号系统在数据网络中的运行。

Conclusion: BDI本体为构建认知基础、可解释且语义可互操作的多智能体和神经符号系统提供了有效的解决方案，在数据网络环境中具有重要应用前景。
...
### [189] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出MIR方法解决多智能体强化学习中稀疏奖励问题，通过激励智能体探索影响队友的动作来提升团队探索效率。
...
Method: 提出相互内在奖励（MIR）方法，激励个体智能体探索能够影响队友的动作，结合原始策略有效促进团队探索。

Result: 在扩展的MiniGrid-MA环境中进行实验验证，与现有最优方法相比表现出更优越的性能。

Conclusion: MIR是一种简单有效的增强策略，能够显著提升稀疏奖励场景下多智能体强化学习的算法性能。
...
### [190] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
...
Method: 引入分层任务抽象机制(HTAM)，将多智能体系统构建为反映领域内在任务依赖关系的逻辑层次结构，通过任务中心架构确保程序正确性并将复杂问题分解为顺序层次。
...
### [191] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具备透明度、合作性和问责制的智能体系统。


<details>
  <summary>Details</summary>
Motivation: 智能体AI需要持续自主性、推理和交互能力，这要求其关于智能体的假设必须辅以明确的认知、合作和治理模型。

Method: 将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具与自适应数据驱动方法相结合。

Result: 提出了一种连接形式理论和实践自主性的智能体视角，为实现既具备能力又透明的智能体系统奠定了基础。

Conclusion: 通过将AAMAS社区的形式化工具与数据驱动方法相结合，可以构建出不仅能力强且灵活，而且透明、合作和可问责的智能体系统。
...
### [192] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 该研究系统评估了使用合成数据和离策略数据对LLM行为探测泛化性能的影响，发现在缺乏同策略数据时，使用同领域离策略数据比不同领域的同策略数据更可靠。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然示例稀少，研究者不得不依赖合成或离策略数据训练探测模型，但这种方法对探测泛化性能的影响尚不明确。

Method: 在八种不同LLM行为上测试线性和注意力探测模型，比较不同响应生成策略对探测性能的影响，并分析从离策略到同策略数据的泛化能力。

Result: 响应生成策略显著影响探测性能，成功从离策略数据泛化到同策略数据的行为具有预测性，欺骗和伪装行为的探测可能在实际监控中泛化失败。

Conclusion: 在LLM监控中，处理分布偏移的方法至关重要，使用同领域离策略数据比不同领域同策略数据能产生更可靠的探测模型。
...
### [193] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
...
<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法传输大量与驾驶安全无关的数据，超出通信带宽限制，且依赖预定义的通信伙伴，不适合动态交通环境。

Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性协作。包含感知风险识别模块和选择性信息交换融合模块。

Result: 相比通用协作感知方法，安全关键对象的平均精度损失小于1%，仅使用20%通信带宽。相比现有选择性协作感知方法，感知性能提升15%。
...
### [194] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
*Thorsten Fröhlich,Tim Schlippe*

Main category: cs.AI

TL;DR: 提出了RubiSCoT框架，利用AI技术改进学术论文评估过程，从开题到最终提交提供一致、可扩展的解决方案。
...
### [195] [Joint Design of Protein Surface and Structure Using a Diffusion Bridge Model](https://arxiv.org/abs/2511.16675)
*Guanlue Li,Xufeng Zhao,Fang Wu,Sören Laue*

Main category: cs.LG

TL;DR: PepBridge是一个用于联合设计蛋白质表面和结构的新框架，通过整合受体表面几何形状和生化特性来生成完整的蛋白质结构。


<details>
  <summary>Details</summary>
Motivation: 蛋白质-蛋白质相互作用受表面互补性和疏水相互作用控制，但设计能够精确互补目标受体的多样化且物理真实的蛋白质结构和表面仍然是一个重大挑战。
...
Conclusion: PepBridge代表了自上而下蛋白质结构联合设计的重要进展，促进了表面互补性、构象稳定性和化学可行性。
...
### [196] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
*Yuqi Li,Kuiye Ding,Chuanguang Yang,Hao Wang,Haoxuan Wang,Huiran Duan,Junming Liu,Yingli Tian*

Main category: cs.LG

TL;DR: DDTime是一个轻量级的时间序列数据集蒸馏框架，通过频域对齐机制和基于信息瓶颈原理的样本间正则化，解决了时间序列蒸馏中的时间偏差和样本多样性不足问题。


<details>
  <summary>Details</summary>
Motivation: 时间序列预测需要大量数据和计算资源，而现有数据集蒸馏方法在处理时间序列时面临两个挑战：1) 强自相关性导致的时间偏差；2) 缺乏显式类别先验导致的样本多样性不足。

Method: 基于一阶压缩分解构建DDTime框架：1) 通过频域对齐机制缓解自相关引起的时间偏差；2) 设计基于信息瓶颈原理的样本间正则化来增强样本多样性。

Result: 在20个基准数据集和多种预测架构上的实验表明，DDTime相比现有蒸馏方法获得约30%的相对精度提升，同时仅引入约2.49%的计算开销。

Conclusion: DDTime为时间序列数据集蒸馏提供了一个有效且高效的解决方案，能够显著提升预测精度同时保持较低计算成本。
...
### [197] [When Structure Doesn't Help: LLMs Do Not Read Text-Attributed Graphs as Effectively as We Expected](https://arxiv.org/abs/2511.16767)
...
Method: 通过系统实验比较LLMs仅使用节点文本描述与结合各种结构编码策略（如模板化图模板、GNN编码结构信息）的性能差异。

Result: LLMs仅使用节点文本描述已在各项任务中表现强劲；大多数结构编码策略仅提供边际收益或负面效果；显式结构先验往往不必要且有时适得其反。

Conclusion: 在LLM时代需要重新思考结构应如何表示和利用，这代表了与传统图学习范式的显著背离，为新的语义驱动图学习方法打开了大门。
...
### [198] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖同质性假设和硬优化目标，限制了在异质性图上的应用，且通常将文本嵌入视为静态目标，导致次优对齐。

Method: 使用最优传输的图对比学习框架，包含RealSoftMax相似度估计器、基于提示的过滤器以及OT引导的软监督机制。

Result: 在九个基准测试中持续优于最先进方法，验证了其有效性和鲁棒性。

Conclusion: GCL-OT能够灵活处理多粒度异质性，通过最优传输实现结构和文本的双向对齐，显著提升了异质性图上的性能。
...
### [199] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache是一个基于频域分析和异常KV识别的多模态KV缓存压缩框架，通过保留关键异常KV对来在减少80%内存使用的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的KV缓存随视觉输入长度线性增长，导致显著推理开销。现有压缩方法依赖注意力分数，与高效注意力内核不兼容且忽略值向量的贡献。

Method: 从KV矩阵分布角度分析，发现多模态KV矩阵能量集中在低频。提出FlashCache框架：1) 异常KV识别模块在频域建模主成分，优先保留偏离主成分的KV对；2) 动态预算分配模块自适应确定每层KV缓存大小。

Result: 在多个MLLM和基准测试中，FlashCache优于最先进的多模态KV压缩方法，实现1.69倍解码加速和80%KV内存使用降低，同时保持任务性能。

Conclusion: 频域引导的异常KV感知压缩方法能有效减少多模态KV缓存，提高推理效率而不牺牲性能。
...
### [200] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
*Ehsan Ahmed Dhrubo,Mohammad Mahmudul Alam,Edward Raff,Tim Oates,James Holt*

Main category: cs.LG

TL;DR: 提出基于向量符号架构(VSA)的多示例学习框架，通过高维向量表示和代数运算严格强制执行MIL的iff约束，在标准基准和医学影像数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习MIL方法大多违反MIL的基本iff约束(包为正当且仅当至少一个实例为正)，导致性能指标虚高和泛化能力差，需要能严格遵循MIL假设的框架。
...
### [201] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: 本研究比较了FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下对IoT攻击检测的性能，重点关注统计异构性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着IoT设备数量和数据量的激增，传统机器学习训练面临资源受限和隐私安全问题。联邦学习虽然能解决这些问题，但非独立同分布数据的统计异构性严重影响了其有效性。现有研究缺乏针对IoT攻击检测场景下联邦学习方法应对统计异构性的全面比较。

Method: 使用CICIoT2023数据集进行大规模IoT攻击分类，系统评估FedAvg、FedProx和Scaffold三种联邦学习算法在不同数据分布下的表现。

Result: 通过细致的分析和实验，揭示了这些联邦学习方法在应对统计异构性时的性能差异。

Conclusion: 研究为IoT安全领域的联邦学习应用提供了有价值的见解，帮助研究者和从业者更好地理解和应对统计异构性挑战。
...
### [202] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
...
<details>
  <summary>Details</summary>
Motivation: 现有的AI安全评估指标虽然能可靠评估模型能力和领域知识，但缺乏能够更好量化"现实世界风险"的指标，特别是针对ASL-3+模型可能提升非国家行为者能力的风险。

Method: 开发了MOCET指标，该指标具有可解释性和双重可扩展性（可自动化和开放式），能够量化现实世界风险。

Result: MOCET指标能够解决现有评估指标的不足，为LLM的安全案例提供更好的风险评估依据。

Conclusion: MOCET是一个有前景的指标，能够跟上AI快速发展的步伐，为AI安全评估提供更全面的风险量化工具。
...
### [203] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
*Yihang Fu,Lifang He,Qingyu Chen*

Main category: cs.LG

TL;DR: ManifoldFormer是一个几何深度学习框架，通过显式学习神经流形表示来解决现有EEG基础模型忽略神经动态内在几何结构的问题，在多个数据集上显著优于现有方法。
...
Method: 提出ManifoldFormer框架，包含三个关键创新：用于保持几何结构的黎曼VAE进行流形嵌入、在神经流形上直接操作的具有测地感知注意力机制的几何Transformer、以及利用神经ODE进行流形约束时间演化的动态预测器。
...
### [204] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: 本文系统比较了Transformer、Transformer++和Mamba三种架构在临床预测任务中的表现，发现在瑞典心衰队列中，Llama（Transformer++）在预测性能、校准度和鲁棒性方面表现最佳，Mamba次之。两种架构都能用更少参数和训练数据实现高效表示学习。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer和Mamba架构在临床预测任务中表现出色，但医学领域缺乏系统分析模型性能和效率的方法。本文旨在填补这一空白，通过系统消融研究为临床预测任务提供设计指导。

Method: 在瑞典42820名心衰患者队列中，比较了三种架构类别（Transformer、Transformer++、Mamba）的六个序列模型。评估三个一年预测任务：心衰住院后临床不稳定、心衰住院后死亡率和最新住院后死亡率。通过消融实验分析EHR输入序列修改、模型配置和时间预处理技术的影响。

Result: Llama（Transformer++）在所有任务中表现出最高的预测区分度、最佳校准度和最强鲁棒性，其次是Mamba。两种架构都能用微小配置超越其他大型Transformer。在相同模型大小下，Llama和Mamba仅需75%的训练数据就能达到优越性能。

Conclusion: 本研究首次提供了针对输入标记化、模型配置和时间数据预处理的系统消融研究。建议未来基于EHR的临床预测模型开发可参考本研究的推荐作为起点。
...
### [205] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
*Zijian Zhang,Xinyu Chen,Yuanjie Shi,Liyuan Lillian Ma,Zifan Xu,Yan Yan*

Main category: cs.LG

TL;DR: 本文提出了一种新的序数分类共形预测方法，该方法模型无关且提供实例级最优预测区间，通过滑动窗口算法显著提高了预测效率。
...
Method: 将序数分类共形预测建模为实例级最小长度覆盖问题，开发了线性时间复杂度的滑动窗口算法，并提出了长度正则化变体。

Result: 在四个基准数据集上的实验表明，所提方法比基线方法平均减少了15%的预测集大小，显著提高了预测效率。

Conclusion: 该方法填补了序数共形预测领域的空白，提供了模型无关、分布无关且实例级最优的解决方案，在保持统计保证的同时显著提升了预测效率。
...
### [206] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
TL;DR: 本研究利用被动声学监测和AI模型，通过欧洲龙虾的生物声学特征（嗡嗡声/甲壳振动）成功分类龙虾的年龄（幼体/成体）和性别（雄/雌），准确率超过97%和93%。


<details>
  <summary>Details</summary>
Motivation: 监测龙虾等难以观察的水生物种具有挑战性。了解龙虾的栖息地、福利、繁殖、性别和年龄对于管理和保护至关重要，而生物声学结合AI模型为此提供了非侵入性解决方案。
...
Conclusion: 研究表明监督机器学习和深度学习能够从龙虾声音中提取年龄和性别相关特征，为龙虾保护、检测和管理提供有前景的非侵入性被动声学监测方法，可实现水下物种的实时边缘计算应用。
...
### [207] [Better audio representations are more brain-like: linking model-brain alignment with performance in downstream auditory tasks](https://arxiv.org/abs/2511.16849)
*Leonardo Pepino,Pablo Riera,Juan Kamienkowski,Luciana Ferrer*

Main category: cs.LG

TL;DR: 研究发现，自监督音频模型在多种下游任务中表现越好，其内部表征与听觉皮层活动的对齐程度越高。EnCodecMAE预训练过程中，大脑相似性会逐步增加，表明类脑表征可能是从自然音频数据中学习重建缺失信息的副产品。
...
Method: 使用36种不同音频模型，通过体素回归、成分回归和表征相似性分析(RSA)，比较模型内部表征与两个独立fMRI数据集的大脑活动对齐程度。评估模型在HEAREval基准的6个听觉任务中的表现。

Result: 发现自监督音频模型比老旧的专用模型能更好地预测听觉皮层活动。模型整体任务性能与大脑表征对齐度呈强正相关(r>0.7)。EnCodecMAE预训练中，大脑相似性早期出现并逐步增加。

Conclusion: 类脑表征可能是从自然音频数据中学习重建缺失信息的副产品，无需专门优化即可获得。
...
### [208] [The use of vocal biomarkers in the detection of Parkinson's disease: a robust statistical performance comparison of classic machine learning models](https://arxiv.org/abs/2511.16856)
*Katia Pires Nascimento do Sacramento,Elliot Q. C. Garcia,Nicéias Silva Vilela,Vinicius P. Sacramento,Tiago A. E. Ferreira*

Main category: cs.LG

TL;DR: 本研究评估了深度神经网络(DNN)与传统机器学习方法在使用语音生物标志物区分帕金森病患者与健康对照方面的表现，发现DNN在准确性和效率上均优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 帕金森病早期常伴随语音障碍，使用语音生物标志物进行早期诊断具有非侵入性、低成本且易于获取的优势，可为临床提供替代方案。

Method: 使用两个公开语音数据集，提取梅尔频率倒谱系数(MFCCs)特征，采用1000次独立随机执行的验证策略评估模型鲁棒性，使用非参数检验比较不同分类模型的性能。

Result: DNN在意大利语音数据集和帕金森远程监测数据集上的平均准确率分别为98.65%和92.11%，显著优于传统机器学习模型。

Conclusion: DNN在使用语音生物标志物进行神经退行性疾病早期检测方面具有更高的准确性和可靠性，展现出巨大潜力。
...
### [209] [Topologic Attention Networks: Attending to Direct and Indirect Neighbors through Gaussian Belief Propagation](https://arxiv.org/abs/2511.16871)
*Marshall Rosenhoover,Huaming Zhang*

Main category: cs.LG

TL;DR: 提出拓扑注意力网络，通过概率机制学习信息在图中直接和间接连接中的传播方式，解决了图神经网络长距离依赖建模问题。
...
Method: 使用拓扑注意力机制，该机制从图的学习信息传播中产生，能够统一推理局部和全局关系，而不依赖显式的成对交互。
...
### [210] [PersonalizedRouter: Personalized LLM Routing via Graph-based User Preference Modeling](https://arxiv.org/abs/2511.16883)
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM数量增加，用户在选择合适LLM时面临挑战，现有方法无法从交互数据中学习个体用户偏好。

Method: 将交互数据转换为异构图，建模用户查询与最优LLM之间的上下文关系，使用多成本效率模拟和LLM-as-a-Judge策略评估适应性。

Result: 在两个模拟策略下分别超越最强方法15.38%和9.83%，在1000用户基准上分别提升16.19%和59.69%，且具有强少样本泛化能力。

Conclusion: PersonalizedRouter能有效学习用户偏好，实现个性化LLM选择，在性能和效率上均优于现有方法。
...
### [211] [Predicting Talent Breakout Rate using Twitter and TV data](https://arxiv.org/abs/2511.16905)
*Bilguun Batsaikhan,Hiroyuki Fukuda*

Main category: cs.LG

TL;DR: 本文提出了一种结合Twitter和电视数据预测日本艺人走红的方法，通过比较传统时间序列模型、神经网络和集成学习方法，发现集成学习在标准回归指标上表现最佳，但神经网络在预测艺人爆红方面具有更高的精确率和召回率。


<details>
  <summary>Details</summary>
Motivation: 在广告领域早期发现潜力艺人至关重要，研究旨在探索结合社交媒体和电视数据来预测艺人走红时间的有效性。

Method: 定义了艺人爆红概念，实验比较了传统时间序列模型、神经网络和集成学习方法，评估了它们在预测社会数据时间变化方面的表现。

Result: 集成学习方法在标准回归指标上优于传统和神经网络模型，但神经网络在预测艺人爆红方面表现出更高的精确率和召回率。

Conclusion: 不同模型在不同评估标准下各有优势，神经网络在预测艺人爆红事件方面更具实际应用价值。
...
### [212] [PepEVOLVE: Position-Aware Dynamic Peptide Optimization via Group-Relative Advantage](https://arxiv.org/abs/2511.16912)
*Trieu Nguyen,Hao-Wei Pang,Shasha Feng*

Main category: cs.LG

TL;DR: PepEVOLVE是一个位置感知的动态框架，用于多目标优化大环肽序列，相比现有方法能更有效地发现优化位点并生成高质量候选肽。


<details>
  <summary>Details</summary>
Motivation: 现有的大环肽生成方法需要预先指定可变位置，且依赖静态预训练和优化算法，限制了模型的泛化能力和优化效果。
...
Conclusion: PepEVOLVE为未知优化位点的大环肽先导优化提供了实用、可复现的路径，能更有效地探索设计空间并提升多目标设计质量。
...
### [213] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: SCR-MF是一个两阶段单细胞RNA测序数据插补工作流，结合了scRecover的dropout检测和missForest的非参数插补，在保持生物学保真度的同时提供稳健性能。
...
### [214] [CroTad: A Contrastive Reinforcement Learning Framework for Online Trajectory Anomaly Detection](https://arxiv.org/abs/2511.16929)
...
<details>
  <summary>Details</summary>
Motivation: 解决智能交通系统中轨迹异常检测面临的三个关键挑战：子轨迹异常检测研究不足、现有方法依赖阈值调优、不规则采样和训练集噪声影响模型性能。

Method: 结合对比学习和强化学习，通过对比学习提取不同行程的正常模式，利用深度强化学习进行在线实时异常评分。

Result: 在两个真实数据集上的广泛实验证明了该框架在各种评估场景下的有效性和鲁棒性。

Conclusion: CroTad框架能够有效解决轨迹异常检测中的关键挑战，提供无阈值、实时且细粒度的异常检测能力。
...
### [215] [A novel approach to classification of ECG arrhythmia types with latent ODEs](https://arxiv.org/abs/2511.16933)
*Angelina Yan,Matt L. Sampson,Peter Melchior*

Main category: cs.LG

TL;DR: 提出一种端到端分类管道，使用潜在ODE建模连续ECG波形，通过降采样构建鲁棒特征向量，在不同采样频率下保持高性能，解决可穿戴设备信号保真度与电池寿命的权衡问题。
...
### [216] [ToC: Tree-of-Claims Search with Multi-Agent Language Models](https://arxiv.org/abs/2511.16972)
...
<details>
  <summary>Details</summary>
Motivation: 传统权利要求撰写劳动密集、成本高昂且不一致，而传统大语言模型缺乏结构化迭代推理能力，无法进行精确的权利要求优化。
...
### [217] [Gradient flow for deep equilibrium single-index models](https://arxiv.org/abs/2511.16976)
*Sanjit Dandapanthula,Aaditya Ramdas*

Main category: cs.LG

TL;DR: 该论文研究了深度平衡模型(DEQs)在梯度下降训练中的理论性质，在线性模型和单索引模型设置下分析了梯度下降动态，证明了线性DEQs的参数守恒定律和梯度流良好条件性，并证明了梯度下降在这些模型中的线性收敛性。


<details>
  <summary>Details</summary>
Motivation: 深度平衡模型作为无限深度权重共享神经网络的新范式，在实践中取得了显著成功，但其梯度下降动态的理论理解仍是活跃研究领域。本文旨在填补这一理论空白。

Method: 在线性模型和单索引模型的简单设置下，通过数学分析证明线性DEQs的参数守恒定律，并利用这一性质分析梯度流条件，最后通过实验验证理论发现。

Result: 证明了线性DEQs的参数在训练过程中保持在球面上，梯度流始终保持良好条件；证明了在线性DEQs和深度平衡单索引模型中，梯度下降在适当初始化和小步长下能线性收敛到全局最小化器。

Conclusion: 该研究为深度平衡模型的梯度下降动态提供了严格的理论保证，填补了文献中的多个空白，并通过实验验证了理论结果的有效性。
...
### [218] [FIRM: Federated In-client Regularized Multi-objective Alignment for Large Language Models](https://arxiv.org/abs/2511.16992)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型对齐面临多目标冲突（如帮助性与无害性）、计算密集和数据隐私问题，现有联邦多目标优化方法存在严重通信瓶颈。

Method: FIRM算法让每个客户端本地解决正则化多目标优化问题，通过客户端内正则化直接缓解分歧漂移，只需传输单一参数集。

Result: 理论证明收敛到帕累托稳定点，实证显示训练动态更平滑、客户端分歧漂移减少、奖励权衡改善，并能根据偏好平滑调整目标权衡。

Conclusion: FIRM在保持通信效率的同时有效解决了联邦多目标对齐问题，为大规模模型对齐提供了实用解决方案。
...
### [219] [Mask the Redundancy: Evolving Masking Representation Learning for Multivariate Time-Series Clustering](https://arxiv.org/abs/2511.17008)
...
<details>
  <summary>Details</summary>
Motivation: 多元时间序列包含大量冗余信息（如稳态机器运行记录、零输出时段），这些冗余降低了模型对判别性时间戳的关注，导致聚类性能瓶颈。现有掩码策略多为独立预处理步骤，无法动态适应聚类关键时间戳的重要性。
...
Conclusion: EMTC通过动态掩码和多视图学习有效提升了多元时间序列聚类的性能，解决了冗余信息导致的注意力分散问题。
...
### [220] [Energy Scaling Laws for Diffusion Models: Quantifying Compute and Carbon Emissions in Image Generation](https://arxiv.org/abs/2511.17031)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成中的计算需求快速增长，引发了能耗和环境影响的担忧，需要预测不同模型配置和硬件设置的能耗

Method: 将扩散模型推理分解为文本编码、迭代去噪和解码组件，假设去噪操作主导能耗，在多种扩散模型和GPU架构上进行实验

Result: 能耗缩放定律在单个架构内预测准确度高，跨架构泛化能力强，能够可靠估计未见过的模型-硬件组合的能耗
...
### [221] [Step-E: A Differentiable Data Cleaning Framework for Robust Learning with Noisy Labels](https://arxiv.org/abs/2511.17040)
*Wenzhang Du*

Main category: cs.LG

TL;DR: Step-E是一个将样本选择和模型学习集成到单一优化过程中的框架，通过基于损失的样本排序和逐步排除高损失样本，有效处理训练数据中的噪声标签和异常值。


<details>
  <summary>Details</summary>
Motivation: 野外收集的训练数据通常包含噪声标签和异常值，这会显著降低深度神经网络的性能和可靠性。传统的数据清洗作为单独预处理阶段，无法充分利用下游模型的反馈，也无法适应未知的噪声模式。

Method: Step-E在每个epoch根据损失对样本进行排序，在短暂预热阶段后逐步增加被排除在梯度更新之外的高损失样本比例，形成在线课程学习，专注于简单一致的样本并最终忽略持续异常值。

Result: 在CIFAR-100N上，Step-E将ResNet-18模型的测试准确率从43.3%提升到50.4%，明显优于损失截断、自定步调学习和一次性过滤方法，接近干净标签的60.5%。在CIFAR-10N上，也从83.9%提升到85.3%，接近干净标签的85.9%，且训练时间开销适中。

Conclusion: Step-E通过将样本选择与模型学习集成到单一优化过程中，有效处理噪声数据，显著提升模型性能，接近干净标签的性能水平，同时保持适度的计算开销。
...
### [222] [Hash Collisions in Molecular Fingerprints: Effects on Property Prediction and Bayesian Optimization](https://arxiv.org/abs/2511.17078)
*Walter Virany,Austin Tripp*

Main category: cs.LG

TL;DR: 研究比较了精确指纹与标准压缩指纹在分子性质预测和贝叶斯优化中的性能差异，发现精确指纹在预测准确性上有小幅提升，但在贝叶斯优化中无明显优势。


<details>
  <summary>Details</summary>
Motivation: 分子指纹方法使用哈希函数生成固定长度的分子向量表示，但哈希碰撞会导致不同子结构具有相同特征，从而在分子相似性计算中产生高估。

Method: 使用高斯过程作为基础预测模型，在DOCKSTRING数据集的五个分子性质预测基准上比较精确指纹和标准压缩指纹的性能。

Result: 精确指纹在分子性质预测准确性上产生小幅但一致的提升，但这些增益并未转化为贝叶斯优化性能的显著改善。

Conclusion: 虽然精确指纹能略微提高预测准确性，但在贝叶斯优化应用中这种改进并不显著，表明哈希碰撞对优化性能的影响有限。
...
### [223] [Why Do Language Model Agents Whistleblow?](https://arxiv.org/abs/2511.17085)
*Kushal Agrawal,Frank Xiao,Guido Bergman,Asa Cooper Stickland*

Main category: cs.LG

TL;DR: 该论文研究了大型语言模型作为工具使用代理时的举报行为，即模型在未经用户指示或知情的情况下向对话边界之外的第三方披露可疑不当行为。


<details>
  <summary>Details</summary>
Motivation: 研究LLM作为工具使用代理时，其对齐训练会以新的方式表现出来，特别是模型可能违背用户利益或明确指示使用工具的行为，重点关注举报行为。

Method: 引入了一个包含多样化和现实化模拟不当行为场景的评估套件，测试不同模型和设置下的举报行为频率，并分析任务复杂性、系统提示和工具可用性等因素的影响。

Result: 发现举报频率因模型系列而异；增加任务复杂性会降低举报倾向；在系统提示中道德暗示会显著提高举报率；提供更多工具和详细工作流程会减少举报行为。

Conclusion: LLM确实会表现出举报行为，这种行为可以通过环境因素进行调节，且研究设置中的评估意识较低，验证了数据集的稳健性。
...
### [224] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
*Duo Zhou,Yuji Zhang,Tianxin Wei,Ruizhong Qiu,Ke Yang,Xiao Lin,Cheng Qian,Jingrui He,Hanghang Tong,Heng Ji,Huan Zhang*

Main category: cs.LG

TL;DR: 本文提出几何解缠遗忘(GU)方法，通过将遗忘梯度更新分解为保留空间的切向和法向分量，仅执行法向分量来消除遗忘过程对保留知识的副作用，实现理论保证的机器遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的机器遗忘方法在有效遗忘和保留知识保护之间存在持续权衡，缺乏对遗忘更新如何损害保留知识的正式分析，以及是否能以理论保证消除副作用。

Method: 从一阶分析出发，提出几何解缠遗忘(GU)方法，将任何候选遗忘梯度更新分解为保留空间的切向和法向分量，仅执行法向分量来确保保留损失的一阶不变性。

Result: GU方法在TOFU、MUSE和WMDP三个基准测试中，与各种现有方法结合均实现了持续改进。

Conclusion: 基于几何解缠的遗忘方法提供了一种理论上有保证且简单的解决方案，能够有效消除遗忘过程对保留知识的副作用，并可即插即用地与现有基于梯度的遗忘程序结合。
...
### [225] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
*Sonia Dupuis,Nando Metzger,Konrad Schindler,Frank Göttsche,Stefan Wunderle*

Main category: cs.LG

TL;DR: 开发了42年泛北极地表温度数据集，通过深度学习超分辨率算法将AVHRR GAC数据从4km降尺度到1km分辨率，用于改善北极多年冻土建模和气候变化监测。


<details>
  <summary>Details</summary>
Motivation: AVHRR卫星的全球覆盖数据空间分辨率较粗，限制了其在北极精细尺度多年冻土动态分析中的应用，需要高分辨率长期地表温度数据来监测快速变暖的北极地区。

Method: 使用基于深度各向异性扩散模型的超分辨率算法，以MODIS LST数据为训练集，结合高分辨率土地覆盖、数字高程和植被高度图作为指导，将AVHRR GAC数据从4km降尺度到1km。

Result: 生成了42年泛北极地区每日两次的1km分辨率地表温度数据集，显著提高了数据空间分辨率，支持多年冻土建模、近地表气温重建和格陵兰冰盖表面质量平衡评估。

Conclusion: 该数据集填补了MODIS时代之前的北极气候监测空白，为未来热红外观测卫星任务提供了可适应的框架，有助于维持气候数据记录的连续性。
...
### [226] [Reconstruction of Surface EMG Signal using IMU data for Upper Limb Actions](https://arxiv.org/abs/2511.17200)
*Shubhranil Basak,Mada Hemanth,Madhav Rao*

Main category: cs.LG

TL;DR: 使用深度学习从6轴IMU数据合成归一化sEMG信号，验证了该方法在肌肉意图检测中的可行性


<details>
  <summary>Details</summary>
Motivation: sEMG信号有噪声且采集困难，IMU提供了一种鲁棒的可穿戴替代方案，可用于假肢和康复生物反馈应用

Method: 收集同步的sEMG和IMU数据，使用基于扩张因果卷积的滑动窗口波网模型将IMU数据映射到sEMG信号

Result: 模型成功预测了肌肉激活的时序和总体形状，峰值幅度常被低估但具有高时间保真度

Conclusion: 该方法在肌肉意图检测方面具有可行性，特别适用于假肢控制和康复生物反馈系统
...
### [227] [DelTriC: A Novel Clustering Method with Accurate Outlier](https://arxiv.org/abs/2511.17219)
*Tomas Javurek,Michal Gregor,Sebastian Kula,Marian Simko*

Main category: cs.LG

TL;DR: DelTriC是一种新颖的聚类算法，结合PCA/UMAP降维、Delaunay三角剖分和反向投影机制，在高维原始空间中形成聚类。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法在高维空间中面临挑战，需要一种既能利用低维空间优势又能在原始高维空间进行准确聚类的算法。

Method: 首先在低维代理空间进行三角剖分以索引局部邻接关系，然后反向投影到原始空间进行鲁棒的边修剪、合并和异常检测。

Result: DelTriC在多种场景下优于k-means、DBSCAN和HDBSCAN等传统方法，具有可扩展性和准确性，并显著改进了异常检测。

Conclusion: DelTriC通过解耦邻域构建和决策制定，提供了一种有效的高维聚类解决方案。
...
### [228] [Generating transition states of chemical reactions via distance-geometry-based flow matching](https://arxiv.org/abs/2511.17229)
*Yufei Luo,Xiang Gu,Jian Sun*

Main category: cs.LG

TL;DR: TS-DFM是一种基于流匹配的框架，能够从反应物和产物预测过渡态结构，在分子距离几何空间中运行，显著提升了过渡态预测的准确性和效率。
...
Method: 提出TS-DFM流匹配框架，在分子距离几何空间中运行，设计了TSDVNet网络结构来学习生成过渡态几何的流速场。

Result: 在Transition1X基准数据集上，TS-DFM比之前最先进方法React-OT的结构准确性提高了30%，预测的过渡态为CI-NEB优化提供了高质量初始结构，并能识别替代反应路径。
...
### [229] [FlexiFlow: decomposable flow matching for generation of flexible molecular ensemble](https://arxiv.org/abs/2511.17249)
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D从头设计流匹配或扩散模型仅限于生成单个构象，但分子的构象景观决定了其可观察特性和与蛋白质靶点的结合能力。通过生成一组代表性的低能构象，可以更直接评估这些特性。

Method: 提出FlexiFlow架构，扩展流匹配模型，允许联合采样分子及其多个构象，同时保持等变性和置换不变性。

Result: 在QM9和GEOM Drugs数据集上实现最先进的分子生成结果，能够生成有效、无应变、独特且新颖的分子，同时捕获分子的构象多样性。生成的构象集合在推理时间仅为物理方法的一小部分时提供相似的覆盖范围。
...
### [230] [Enforcing governing equation constraints in neural PDE solvers via training-free projections](https://arxiv.org/abs/2511.17258)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络PDE求解器在科学模拟中经常违反控制方程约束，特别是非线性约束和动力学PDE中的长程时间依赖性问题，使得投影到可行集变得复杂。

Method: 提出了两种无需训练的后处理投影方法：1）非线性优化投影；2）基于局部线性化的投影，使用雅可比-向量和向量-雅可比乘积。

Result: 在代表性PDE上的约束分析表明，两种投影方法都显著减少了约束违反，并在物理信息基线上提高了准确性。

Conclusion: 两种后处理投影方法都能有效改善神经网络PDE求解器的约束满足性和准确性，为处理非线性约束提供了实用解决方案。
...
### [231] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
*Tom Nahrendorf,Stefan Minner,Helfried Binder,Richard Zinck*

Main category: cs.LG

TL;DR: 该研究针对德国高端汽车制造商的多产品、多市场、多层次需求预测问题，结合点预测和概率预测方法，使用LightGBM集成模型和混合整数线性规划协调方法，发现时空依赖性和整数偏差显著影响预测精度。


<details>
  <summary>Details</summary>
Motivation: 高端汽车制造商面临产品种类繁多、变体级数据稀疏和市场动态波动等复杂预测挑战，需要解决多产品、多市场、多层次层次结构中的月度汽车需求预测问题。

Method: 结合战略和运营规划层面的点预测和概率预测，采用LightGBM模型集成、分位数回归和混合整数线性规划协调方法，利用合并训练集进行建模。

Result: 时空依赖性和四舍五入偏差显著影响预测精度，整数预测对运营可行性至关重要；短期需求受生命周期成熟度、自回归动量和运营信号影响，中期需求受在线参与度、规划目标和竞争指标等预期驱动因素影响。
...
### [232] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
...
Method: 采用改进的SimCLR流程，通过随机变换生成增强表视图，使用自定义transformer编码器嵌入，并在潜在空间中进行对比以优化语义相似性。

Result: 在五个标准数据集上的实验显示，SAVeD在完全未见过的表上获得显著更高的准确率，分离分数显著提升，能够有效区分语义改变的版本。
...
### [233] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
*Israt Jahan Tulin,Sebastian Starke,Dominic Windisch,André Bieberle,Peter Steinbach*

Main category: cs.LG

TL;DR: 本文研究了两种自监督深度学习方法用于CT探测器数据去噪，相比非学习方法能有效提升信噪比和重建图像质量。
...
Method: 使用两种自监督深度学习方法对原始探测器数据进行去噪，并与非学习方法进行比较。

Result: 深度学习方法能够增强探测器数据的信噪比，并在重建图像中带来一致改进，优于非学习方法。

Conclusion: 自监督深度学习方法在CT数据去噪方面具有优势，能够有效提升图像重建质量。
...
### [234] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: 本文提出了Repulsive Bayesian Prompt Learning (ReBaPL)方法，通过贝叶斯框架优化提示学习，结合循环步长调度和随机梯度哈密顿蒙特卡洛算法，并引入表示空间的排斥力来增强探索多样性和防止模式坍塌。


<details>
  <summary>Details</summary>
Motivation: 传统提示学习方法容易过拟合且在分布外泛化方面表现不佳，贝叶斯提示学习通过将提示优化构建为贝叶斯推断问题来提高鲁棒性，但现有方法在探索复杂多模态后验分布方面仍有局限。

Method: ReBaPL方法整合了循环步长调度与SGHMC算法，交替进行探索和利用阶段，并引入基于概率度量（如MMD和Wasserstein距离）的表示空间排斥力来多样化探索。

Result: 在多个基准数据集上的实验表明，ReBaPL在提示学习任务上优于现有最先进方法，提供了更全面的提示后验分布表征。

Conclusion: ReBaPL为任何基于最大似然估计的现有提示学习方法提供了模块化的即插即用贝叶斯扩展，显著提升了泛化性能。
...
### [235] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
...
Method: 使用封建Q学习方案，利用随机逼近理论和ODE方法进行收敛性分析，将更新过程建模为博弈均衡问题。
...
### [236] [R2PS: Worst-Case Robust Real-Time Pursuit Strategies under Partial Observability](https://arxiv.org/abs/2511.17367)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于图的追逃游戏中，当追捕者只有不完美信息时，缺乏实时适用的追捕策略。现有强化学习方法局限于完美信息场景，且未考虑逃避者可能预测追捕者行动的情况。

Method: 1. 证明传统动态规划算法在异步移动下保持最优性；2. 提出关于逃避者可能位置的信念保持机制，将DP追捕策略扩展到部分可观测设置；3. 将信念保持嵌入EPG框架，通过跨图强化学习训练实时追捕策略。

Result: 强化学习后，策略实现了对未见真实世界图结构的鲁棒零样本泛化，并且始终优于现有游戏RL方法直接在测试图上训练的策略。

Conclusion: R2PS是首个在部分可观测性下提供最坏情况鲁棒实时追捕策略的方法，在未知图结构上表现出优异的泛化性能。
...
### [237] [A Unified Stability Analysis of SAM vs SGD: Role of Data Coherence and Emergence of Simplicity Bias](https://arxiv.org/abs/2511.17378)
...
Method: 开发线性稳定性框架分析SGD、随机扰动和SAM的行为，特别针对两层ReLU网络。核心是引入一致性度量来量化梯度曲率在数据点间的对齐程度。

Result: 该框架揭示了为何某些极小值在训练中更稳定且被偏好，通过一致性度量解释了梯度曲率对齐与极小值稳定性之间的关系。

Conclusion: 提供了一个统一的理论框架连接数据结构、优化动态性和学习解的性质，解释了SGD偏好平坦极小值的机制。
...
### [238] [Stable Coresets via Posterior Sampling: Aligning Induced and Full Loss Landscapes](https://arxiv.org/abs/2511.17399)
*Wei-Kai Chang,Rajiv Khanna*

Main category: cs.LG

TL;DR: 提出了一种基于后验采样的核心集选择框架，通过连接后验采样和损失景观来解决梯度方法在核心集选择中的局限性，实现了更快的训练和更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求激增，需要有效的核心集选择技术来加速训练。梯度方法虽然有理论优势，但面临SGD基线强和损失曲率不匹配导致代表性失效的问题。

Method: 1) 建立后验采样与损失景观的连接；2) 引入基于后验采样的平滑损失函数；3) 提出采样核心集选择方法的收敛性分析。

Result: 在多个数据集上的实验表明，该方法比现有技术实现了更快的训练速度和更好的泛化性能。

Conclusion: 该框架成功解决了核心集选择中的关键挑战，为大规模深度学习提供了有效的加速方案。
...
### [239] [DS-Span: Single-Phase Discriminative Subgraph Mining for Efficient Graph Embeddings](https://arxiv.org/abs/2511.17419)
*Yeamin Kaiser,Muhammed Tasnim Bin Anwar,Bholanath Das,Chowdhury Farhan Ahmed,Md. Tanvir Alam*

Main category: cs.LG

TL;DR: DS-Span是一个单阶段判别子图挖掘框架，统一了模式增长、剪枝和监督驱动评分，在搜索空间的一次遍历中完成，比传统多阶段方法更高效紧凑。


<details>
  <summary>Details</summary>
Motivation: 现有频繁或判别子图挖掘方法存在冗余多阶段流程、计算成本高、挖掘结构与判别相关性弱耦合等问题，需要更高效统一的解决方案。

Method: 提出覆盖上限资格机制动态限制探索，信息增益引导选择促进强类别分离能力的子图，同时最小化冗余，实现单阶段统一挖掘。

Result: 在基准测试中，DS-Span比先前多阶段方法生成更紧凑和判别的子图特征，以显著减少的运行时间实现更高或相当的准确率。
...
### [240] [Self-Supervised Learning by Curvature Alignment](https://arxiv.org/abs/2511.17426)
...
Method: 采用标准双视图编码器-投影器架构，在Barlow Twins风格冗余减少损失基础上添加曲率正则化器，通过计算嵌入点的离散曲率得分，并在增强视图间对齐和去相关这些曲率。
...
### [241] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
...
Method: 修改VEROS模型使其与JAX框架兼容，实现整个动力学核心的可微分性。

Result: 成功开发了可微分海洋模型，在初始状态修正和参数校准两个应用中验证了其有效性。
...
TL;DR: 提出了基于序列到序列的多智能体指针变换器(MAPT)框架，用于解决具有随机请求的多车辆动态取送货问题，在性能和计算时间上显著优于现有方法。
...
Conclusion: MAPT框架有效解决了多车辆动态取送货问题中的关键挑战，为大规模动态路由优化提供了高效的端到端集中决策方案。
...
### [243] [InTAct: Interval-based Task Activation Consolidation for Continual Learning](https://arxiv.org/abs/2511.17439)
*Patryk Krukowski,Jan Miksa,Piotr Helm,Jacek Tabor,Paweł Wawrzyński,Przemysław Spurek*

Main category: cs.LG

TL;DR: InTAct方法通过约束共享层中神经元激活范围来减少表示漂移，在领域增量学习中提升性能，无需冻结参数或存储历史数据。


<details>
  <summary>Details</summary>
Motivation: 解决基于提示的持续学习方法在领域偏移下容易发生表示漂移的问题，即使任务特定参数被隔离，共享表示的变化仍会导致遗忘。

Method: 捕获先前学习任务的特征激活范围，约束更新以确保网络在这些区域内保持一致性，同时允许在其他区域灵活适应，稳定重要神经元的功能角色。

Result: 在DomainNet和ImageNet-R等多样化领域增量基准测试中，持续减少表示漂移并提高性能，平均准确率比最先进基线提高最多8个百分点。
...
### [244] [Unmasking Airborne Threats: Guided-Transformers for Portable Aerosol Mass Spectrometry](https://arxiv.org/abs/2511.17446)
*Kyle M. Regan,Michael McLoughlin,Wayne A. Bryden,Gonzalo R. Arce*

Main category: cs.LG

TL;DR: 提出了MS-DGFormer框架，通过直接处理原始质谱数据，解决了MALDI-MS在环境监测中依赖复杂样品制备和多次光谱平均的限制，实现了单次检测的病原体识别。


<details>
  <summary>Details</summary>
Motivation: MALDI-MS在生物分子分析中应用广泛，但样品制备复杂且需要多次光谱平均，限制了其在实时环境监测中的应用，特别是在气溶胶分析中单次检测的需求。

Method: 采用基于Transformer的MS-DGFormer架构，结合字典编码器集成奇异值分解去噪后的光谱信息，捕捉时间序列光谱中的长程依赖关系。

Result: 该方法能够从单次光谱中准确识别病原体，在气溶胶样品中实现优异的病原体识别性能，支持自主实时分析。

Conclusion: MS-DGFormer消除了对复杂预处理的需求，为便携式MALDI-MS平台的发展铺平了道路，革新了环境病原体检测和生物威胁快速响应能力。
...
### [245] [PersonaAgent with GraphRAG: Community-Aware Knowledge Graphs for Personalized LLM](https://arxiv.org/abs/2511.17467)
*Siqi Liang,Yudi Zhang,Yue Guo*

Main category: cs.LG

TL;DR: 提出了一个基于知识图谱增强检索增强生成（Graph RAG）的个性化语言模型系统框架，通过构建LLM衍生的图索引和社区信息摘要，生成个性化提示来维持一致的用户角色行为。


<details>
  <summary>Details</summary>
Motivation: 需要能够适应个体用户偏好的个性化AI代理，让代理能够体现用户的"角色"（如用户档案或品味）。

Method: 引入知识图谱增强的检索增强生成机制，构建LLM衍生的图索引和相关文档，结合用户历史行为偏好摘要和基于图的社区检测识别的全局交互模式来生成个性化提示。

Result: 在LaMP基准测试中，新闻分类F1提高11.1%，电影标签F1提高56.1%，产品评分MAE降低10.4%。

Conclusion: 该框架能够有效维持一致的角色对齐行为，同时从集体知识中受益，显著提升了多个任务的性能。
...
### [246] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习通常数据需求量大，利用相似过程的数据可以提高样本效率，但如何识别相似过程是一个挑战。

Method: 结合顺序消除和零阶策略优化，提出同时进行聚类和学习的算法，为每个聚类输出个性化策略。

Result: 在适当的聚类分离条件下，算法能以高概率正确聚类，且每个聚类的策略次优性差距与聚类大小成反比，无额外偏差。

Conclusion: 这是首个展示如何通过聚类在数据驱动控制中学习个性化策略的工作，既能从协作中获得统计增益，又不会因包含不相似过程数据而遭受次优性。
...
### [247] [Near-Optimal Dropout-Robust Sortition](https://arxiv.org/abs/2511.16897)
*Maya Pal Gambhir,Bailey Flanigan,Aaron Roth*

Main category: cs.GT

TL;DR: 该论文提出了一种处理公民大会中参与者临时退出的优化算法，通过博弈论方法最小化退出对代表性和规模的影响，并确保公平选择概率。


<details>
  <summary>Details</summary>
Motivation: 公民大会中参与者临时退出会影响小组的代表性和规模，需要在不提前知道退出者的情况下选择参与者，以保证最终小组仍具有代表性。

Method: 将问题建模为极小极大博弈：极小化者选择参与者以最小化损失，极大化者定义退出分布。使用迭代投影梯度下降算法对抗最佳响应退出分布。

Result: 开发了高效的损失最小化算法，在真实数据集上优于现有基准，首次表征了鲁棒性、损失和公平性之间的权衡关系。

Conclusion: 提出的算法能有效管理参与者退出问题，确保代表性和公平性，为公民大会的组织提供了理论支持和实用工具。
...
### [248] [The Effects of Latency on a Progressive Second-Price Auction](https://arxiv.org/abs/2511.17424)
*Jordana Blazek,Eric Olson,Fredrick C. Harris*

Main category: cs.GT

TL;DR: 本文分析了渐进式第二价格拍卖在存在延迟、异步分析和随机初始出价情况下的ε-纳什均衡，提出了最小收益均衡状态算法，并证明设置略低于清算价的保留价可以稳定卖方收益同时保持效率。


<details>
  <summary>Details</summary>
Motivation: 研究延迟、异步分析和随机初始出价如何影响渐进式第二价格拍卖的ε-纳什均衡，特别是在弹性需求假设下对效用的影响。

Method: 引入寻找最小收益均衡状态的算法，通过设置略低于清算价的保留价来稳定卖方收益，同时使用真实的ε-最优响应方法分析均衡。

Result: 设置保留价可以稳定卖方收益并保持效率，虽然部分买家在分配价值和成本方面存在不可预测性，但各自的效用是可预测的。

Conclusion: 渐进式第二价格拍卖在存在延迟和异步分析的情况下，通过适当的保留价设置可以实现稳定的卖方收益和可预测的买家效用，同时保持资源分配效率。
