...
Motivation: 现有数据集多依赖合成模型或难以获取的专业物品，缺乏真实世界常见商品的成本效益数据集

Method: 使用运动结构恢复技术和高分辨率成像采集超市商品的3D网格，生成水密网格模型

Result: 创建了包含10个类别、形状尺寸重量多样的50种商品的高质量3D数据集

Conclusion: ASOS数据集强调可获取性和实际应用性，对物体检测、姿态估计和机器人应用具有重要基准测试价值
...
Method: 采用双分支多模态编码器结构：图像分支使用ResNet和Transformer提取建筑物损坏特征，文本分支使用BERT检索器处理文本向量化；集成跨模态交互模块通过多头注意力实现语义对齐；引入模态注意力门控机制动态控制生成过程中视觉证据和文本先验信息的作用；端到端训练，结合对比损失、检索损失和生成损失进行多任务优化。

Result: 在检索准确率和损坏严重程度分类指标上表现出优异性能，Top-1检索准确率提高了9.6%。

Conclusion: 该MM-RAG框架通过有效的多模态融合和端到端训练，成功实现了自然灾害后房屋损坏的准确评估，为保险理赔和资源规划提供了可靠的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决噪声历史文档文本提取中的不稳定性问题，提高转录准确性并建立置信度评估机制。

Method: 使用Gemini 2.0 Flash对每个图像的多个增强变体进行转录，然后通过定制的Needleman Wunsch风格对齐器融合输出，生成共识转录和置信度分数。

Result: 在622份宾夕法尼亚州死亡记录数据集上，相比单次转录基线，准确率提高了4个百分点。填充和模糊处理对提升准确性最有效，网格扭曲扰动最能区分高低置信度情况。
...
Method: 收集17万张真实交通监控图像并标注8大类24小类对象和事件，通过系统化数据生成流程产生高质量图像描述和问答对，涵盖5个关键交通监控任务
...
Conclusion: MITS数据集有效解决了交通监控领域多模态数据缺失问题，显著提升了大模型在该领域的性能，为智能交通监控和大模型研究提供了宝贵资源
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本视觉分类方面表现出色，但在细粒度任务和大规模分层标签空间中的性能研究不足，需要探索结构化推理是否能提升性能。

Method: 引入一个框架，使用决策树将分类分解为可解释的决策，在细粒度(GTSRB)和粗粒度(CIFAR-10)数据集上进行评估，并探索用LLM生成的类别和图像描述来增强树状提示。

Result: 模型在理解树状知识方面达到98.2%准确率，但树状推理始终表现不如标准零样本提示。添加图像描述能提升树状方法和零样本方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 旨在开发能够从视频数据中学习丰富世界模型并支持灵活提示控制的系统，类似于大型语言模型的通用提示能力

Method: 三步循环：1)概率预测-构建随机访问自回归序列模型；2)结构提取-通过因果推断零样本提取低维属性；3)整合-将结构转换为新标记类型并重新训练

Result: 在1.4万亿个互联网视频标记上训练PSI实例，实现了最先进的光流估计、自监督深度估计和对象分割，支持完整的预测改进循环

Conclusion: PSI系统通过循环增强的方式成功构建了可提示的世界模型，能够从视频数据中提取有意义的结构并用于改进预测能力
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然通过交换模型更新而非原始数据来保护隐私，但梯度反转攻击能够从共享梯度中逆向推导出原始训练数据。目前对于图像、文本和表格数据的攻击影响已有研究，但视频数据的泄露风险尚未被探索

Method: 评估了两种常见的视频分类方法：使用预训练特征提取器的方法和处理原始视频帧的简单变换方法。测试了攻击者在拥有零个、一个或多个参考帧的情况下，利用图像超分辨率技术增强通过梯度反转攻击提取的帧质量

Result: 特征提取器对梯度反转攻击具有更强的抵抗力，但如果分类器复杂度不足，数据泄露仍然可能发生。超分辨率技术能够显著提高攻击者重建视频的质量

Conclusion: 视频数据在联邦学习中存在可行的泄露威胁，需要进一步研究其发生条件并开发更有效的防护措施
...
Conclusion: 该框架有效解决了密集零售环境中的目标检测问题，通过减少对人工标注的依赖降低了成本，并能适应零售场景的动态变化，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将地面图像转换为鸟瞰图表示再与航拍图像对齐，这种转换常因透视畸变或高度信息压缩导致信息损失，影响对齐质量。需要一种更直接有效的跨视角定位方法。

Method: 直接建立地面与航拍图像之间的对应关系，仅使用单目深度先验将匹配的关键点提升到鸟瞰图空间。支持度量深度和相对深度，采用尺度感知的Procrustes对齐来估计相机位姿，并在使用相对深度时可选地恢复尺度。

Result: 实验结果表明，仅需相机位姿的弱监督，该方法就能学习准确的局部特征对应关系，在跨区域泛化和未知方向等挑战性条件下实现优越的定位性能，且兼容各种相对深度模型而无需针对每个模型进行微调。

Conclusion: 该方法通过直接建立跨视角对应关系避免了信息损失，具有高精度、强泛化能力和实际部署灵活性，为现实世界的跨视角定位应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统Bruckner测试需要眼科医生在临床环境中进行，利用智能手机和AI技术可以创建移动设备版本，实现可及性儿童视力筛查

Method: 基于深度神经网络模型，训练数据来自眼科医生收集和标注的儿童瞳孔图像，开发免费应用KidsVisionCheck

Result: 在未见测试数据上达到90%的准确率，提供高可靠性性能，无需专业设备，并能确定最佳数据收集条件

Conclusion: 这项工作标志着向全球可及性儿童视力筛查和早期干预视力异常迈出的第一步
...
<details>
  <summary>Details</summary>
Motivation: 现有的传感器融合方法通常在整个输入空间范围内统一处理传感器数据，这在面对挑战性条件时会影响性能。需要一种能够根据空间变化条件动态调整传感器融合的方法。

Method: 提出深度引导的多模态融合网络DGFusion，将多模态分割作为多任务问题处理。利用激光雷达测量作为输入和深度学习的真值，通过辅助深度头学习深度感知特征，编码为局部深度标记来条件化跨模态融合。

Result: 在具有挑战性的MUSES和DELIVER数据集上实现了最先进的全景和语义分割性能。

Conclusion: 深度引导的融合方法能够有效处理传感器在不同空间位置的可信度变化，特别是在恶劣条件下，为自动驾驶的鲁棒语义感知提供了有效解决方案。
...
Method: 使用ResNet-18深度学习框架，从面部图像提取不同大小、形状和位置的各种图像斑块，研究局部视觉信息对模型性能的影响。

Result: 实验结果表明，基于斑块的策略在准确性和敏感性方面达到或优于全图像方法，同时引导模型关注临床相关区域，增强鲁棒性和可解释性。

Conclusion: 提出的斑块式策略不仅保护患者隐私，还为改进自动化皮肤病诊断提供了实用见解，具有重要的临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 玫瑰痤疮是一种常见但诊断不足的皮肤炎症，自动检测面临症状弥散、标注数据稀缺和面部图像隐私问题的挑战，需要开发隐私保护的检测方法

Method: 1) 基于红色通道强度构建固定红色掩码，聚焦脸颊、鼻子和前额等诊断相关区域，排除身份识别特征；2) 在掩码后的合成图像上训练ResNet-18深度学习模型

Result: 该方法在真实测试数据上相比全脸基线模型在准确率、召回率和F1分数方面均有显著提升，表现出优越性能
...
Method: 采用消融研究方法，包括：(1)移除可学习维纳滤波器模块；(2)选择性使用复合损失函数中的各个损失项（MSE、SSIM损失和感知损失）。所有变体在公开的腹腔镜图像数据集上进行基准测试。

Result: 研究使用定量指标（SSIM、PSNR、MSE和CIEDE-2000）和定性视觉比较来评估不同组件配置的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决在复杂环境条件下无人机检测的鲁棒性问题，研究者希望利用多模态信息（视觉+声学）来提升检测性能，特别是在小尺寸无人机检测方面。

Method: 基于Deformable DETR和Wav2Vec2架构，开发了四种不同的融合配置（门控机制、线性层、MLP和交叉注意力），将声学嵌入与多分辨率特征映射融合。使用了Drone-vs-Bird数据集和新生成的包含7500+同步图像和音频片段的ARDrone数据集。

Result: 最佳性能的门控融合方法将Deformable DETR检测器在ARDrone数据集上的mAP提升了11.1%-15.3%（小无人机），中等和大尺寸无人机的mAP也有提升，整体增益在3.27%-5.84%之间。

Conclusion: 声学信息能够显著提升无人机检测性能，特别是在小尺寸无人机检测方面，多模态融合方法在真实场景中表现出色，门控融合是最有效的融合策略。
...
TL;DR: 提出了一种称为代理监督的新训练范式，通过将估计的空间变换应用于代理图像，将输入域与监督域解耦，从而在异质输入上训练配准网络，提高其对图像伪影、视野不匹配和模态差异的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的可变形图像配准虽然精度高，但对输入图像特征（如伪影、视野不匹配或模态差异）的变化敏感，需要开发能够提高配准网络鲁棒性和泛化能力的通用训练范式。

Method: 引入代理监督方法，将输入域与监督域解耦，通过将估计的空间变换应用于代理图像来确保在相似性定义良好的域中进行监督计算，从而在异质输入上进行训练。

Result: 在三个代表性应用（抗伪影脑MR配准、掩码无关肺CT配准和多模态MR配准）中，代理监督表现出对输入变化的强韧性，包括不均匀场、不一致视野和模态差异，同时在精心策划的数据上保持高性能。

Conclusion: 代理监督提供了一个原则性框架，在不增加复杂性的情况下训练鲁棒且可泛化的深度学习配准模型，为更鲁棒和可泛化的医学图像配准提供了实用途径，使其在多样化的生物医学成像场景中具有更广泛的应用性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在法医牙龄估计等高风险应用中因'黑盒'特性而受限，需要提高模型性能和透明度来支持专家决策。
...
Conclusion: 该框架不仅提高了准确性，还提供了模型不确定性的证据，强调单一可解释性模式（如注意力图）的不足，为法医年龄估计提供了更可靠的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法依赖于全监督预训练的源模型，使用余弦相似度对齐实例级特征时会丢失源模型潜在流形的关键几何信息

Method: 1) 使用自监督预训练的教师模型而非监督预训练；2) 将几何流形对齐原理应用于SFDA设置；3) 结合实例级特征匹配和空间相似性损失的复合目标训练学生模型；4) 通过学生参数的指数移动平均更新教师参数以防止灾难性遗忘

Result: 在多个基准数据集上的广泛实验表明，SCoDA显著优于最先进的SFDA方法

Conclusion: SCoDA通过自监督预训练和几何流形对齐，成功解决了SFDA中几何信息丢失的问题，为无源域数据适应提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖昂贵的人工标注数据集，且对未见数据集的泛化能力有限，无法应对显微数据的巨大多样性

Method: 将大型基础模型SAM2集成到追踪流程中，作为完全无监督方法，不依赖特定训练数据集，无需微调即可泛化到不同显微数据集

Result: 在2D和大规模3D延时显微视频中达到竞争性精度，无需数据集特定适配

Conclusion: 该方法克服了传统方法的局限性，实现了跨数据集的零样本泛化能力，为细胞追踪提供了有效的无监督解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的多目标多摄像头跟踪系统主要基于2D空间，而3D跟踪需要完全重构系统组件，这对于现有系统来说不可行。需要一种方法能够利用现有2D跟踪系统，通过深度信息将其扩展到3D空间
...
Conclusion: 该方法成功地将2D多摄像头跟踪系统扩展到3D空间，无需完全重构系统组件，为现有系统提供了向3D感知升级的可行路径
...
<details>
  <summary>Details</summary>
Motivation: 传统的REC方法需要任务特定的训练，本文旨在探索零样本工作流是否能够在不进行REC特定训练的情况下实现竞争性甚至更好的性能。

Method: 使用COCO-clean通用检测器(YOLO-World)生成候选框，然后通过通用VLM对每个区域独立进行True/False查询验证，避免了跨框干扰并支持弃权和多匹配。

Result: 在RefCOCO、RefCOCO+和RefCOCOg数据集上，该方法不仅超越了零样本GroundingDINO基线，还超过了经过REC训练的GroundingDINO和GroundingDINO+CRG的报告结果。

Conclusion: 研究表明，工作流设计而非任务特定的预训练是驱动零样本REC性能的关键因素，验证式方法显著优于基于选择的方法。
...
<details>
  <summary>Details</summary>
Motivation: 小麦叶片病虫害分割中，虫害通常只占标注像素的极小部分，这种极端像素级不平衡会导致模型过拟合常见类别而忽略罕见类别，影响整体分割性能

Method: RPCP增强技术：从训练图像提取罕见虫害斑块，应用随机几何变换模拟变化，将变换后的斑块粘贴到合适区域并避免与现有病变重叠，使用随机投影滤波器优化局部特征使其与背景自然融合

Result: 实验表明该方法显著提高了虫害类别的分割性能，同时保持甚至略微提升了其他类别的准确率

Conclusion: 针对性数据增强能有效缓解极端像素不平衡问题，为农业分割任务提供了简单而有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有多目标跟踪方法在长期跟踪中因身份切换问题性能下降，而现实应用（如畜牧业）中可获得零星的身份识别信息，需要有效利用这些不确定身份信息来改善长期跟踪性能

Method: 使用隐马尔可夫模型（HMM）框架，将不确定的身份信息与跟踪过程相结合，通过概率模型处理身份识别的不确定性

Result: 在10分钟猪只跟踪数据集上，即使只有21次喂食站识别，也提升了ByteTrack的F1分数；在MOT17和MOT20基准数据集上验证了框架的有效性，性能随身份信息提供频率增加而提升

Conclusion: 提出的HMM框架能够有效利用不确定身份信息，显著改善长期多目标跟踪性能，对身份识别不确定性具有鲁棒性，适用于现实世界的长期跟踪应用
...
<details>
  <summary>Details</summary>
Motivation: 事件相机作为新兴的生物启发式传感器，具有低延迟、低功耗和高捕获率等优势，但需要与传统帧相机融合才能充分发挥其潜力。本文旨在系统梳理这一融合领域的最新进展。

Method: 通过系统文献综述方法，从两个维度分析深度学习贡献：时间增强（帧插值、运动去模糊）和空间增强（超分辨率、低光/HDR增强、伪影减少），并探讨3D重建领域的进展。

Result: 调查发现事件相机与传统相机融合能显著提升各种视频恢复任务的性能，特别是在挑战性条件下改善视觉质量，并整理了可用于复现研究和基准测试的开放数据集。

Conclusion: 事件相机系统，特别是与深度学习结合，在高级视觉媒体恢复和增强方面具有巨大潜力，本调查旨在激发该领域的进一步研究。
...
Method: 采用双分支模型：视觉Transformer提取RGB空间上下文，脉冲Transformer捕获事件流时空动态；设计基于ISTA的适配器进行双向特征交互，并加入时序下采样注意力模块对齐特征。

Result: 在FE240hz、VisEvent、COESOT和FELT等RGB-Event跟踪基准测试中实现了最先进的性能，同时保持高能效。

Conclusion: ISTASTrack展示了混合ANN-SNN设计在鲁棒视觉跟踪中的有效性和实用性，为跨模态特征融合提供了新思路。
...
Method: 使用多深度状态空间模型构建太阳耀斑预测模型，引入频率和局部边界感知可靠性损失函数（FLARE损失）来改善类别不平衡下的预测性能和可靠性

Result: 在覆盖完整11年太阳活动周期的多波长太阳图像数据集上，该方法在Gandin-Murphy-Gerrity分数和真实技能统计量等标准指标上均优于基线方法

Conclusion: 所提出的多深度状态空间模型结合FLARE损失函数能够有效解决太阳耀斑预测中的类别不平衡问题，提高预测准确性和可靠性
...
Method: 提出多块堆叠的RGB-T编码器，同时执行多模态特征提取和跨模态融合；利用RGB和伪热数据进行大规模预训练；通过精简热分支实现紧凑架构；引入RGB-T局部模块使用自适应余弦相似度选择性地强调跨模态的显著一致和差异局部特征。

Result: 在FMB、PST900和CART数据集上达到与最先进模型竞争的性能，参数量和计算成本更低；在Jetson Orin NX上实现27 FPS的推理速度，具备实时部署能力。

Conclusion: TUNI通过统一的编码器设计成功解决了RGB-T语义分割中的特征提取和融合问题，在保持高性能的同时实现了高效的实时部署。
...
<details>
  <summary>Details</summary>
Motivation: 传统少样本字体生成需要完整字符形状，而本方法旨在通过部分设计元素提高字体创建效率，并探索部分设计细节对整体字符结构的影响

Method: 设计了一个新颖的少部件字体生成模型，以部分形状作为输入来生成整个字体

Result: 该方法不仅提高了字体创建效率，还提供了关于部分设计细节如何影响整体字符结构的洞察

Conclusion: 该模型为字体设计提供了更高效的方法，同时深化了对设计元素与整体结构关系的理解
...
<details>
  <summary>Details</summary>
Motivation: 传统高精度VIO流水线需要强大计算系统，而微控制器上的轻量级实现精度不足。本文旨在弥合这一差距，为微型无人机开发既高效又准确的VIO解决方案。

Method: 采用先进的SuperPoint、PX4FLOW和ORB特征检测与跟踪方法，并进行优化和量化以适应RISC-V超低功耗SoC。使用刚体运动模型减少估计误差，在平面运动场景中提高精度。

Result: 在GAP9低功耗SoC上，使用ORB特征跟踪器时，优化流水线相比基准流水线平均RMSE降低达3.65倍。PX4FLOW在移动速度低于24像素/帧时，以更低运行时成本实现与ORB相当的跟踪精度。

Conclusion: 该VIO流水线成功实现了在超低功耗系统上的高精度实时视觉惯性里程计，为微型无人机应用提供了有效的解决方案，平衡了计算效率和跟踪精度。
...
Method: 使用预训练的层次化主干网络，在2D人脸特征提取不同阶段引入多级注意力机制，采用半监督训练策略结合3DMM参数和可微分渲染器实现端到端训练。

Result: 在AFLW2000-3D和MICC Florence基准数据集上进行了大量实验，包括对比和消融研究，在3D人脸重建和对齐任务上进行了定量和定性评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖文本思维链，对多语言多模态推理支持有限，限制了在实际应用中的部署。需要开发能够同时处理语言识别、空间关系和逻辑推理的多语言视觉推理框架。

Method: 设计包含文本摘要与边界框、语言识别、空间对象级描述和逐步逻辑推理的多阶段推理管道。采用自动化数据标注方法生成多语言CoT注释，结合监督微调和语言感知组相对策略优化的两阶段训练范式。

Result: 在MMMB、Multilingual MMBench和MTVQA等数据集上，比同规模开源基线准确率提升约9.5%，甚至超越规模大2倍的模型约2.6%，优于GPT-4o-0513和Gemini-2.5-flash等先进专有模型。
...
Result: 实验结果表明该方法提高了颜色对齐准确性，且不损害图像质量，弥合了文本语义与视觉生成之间的差距

Conclusion: 该训练免费框架能有效提升文本到图像生成中的颜色保真度，无需额外训练或参考图像，为颜色敏感应用提供了实用解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在无人机遥感任务中的数学推理能力（如距离计算、轨迹估计、空间分析）尚未得到充分测试，需要专门的基准来评估和改进这方面的能力。

Method: 构建AVI-Math数据集，包含从不同高度和角度采集的无人机图像，设计涵盖6个数学学科和20个主题的3,773个问题。对14个主流视觉语言模型进行全面评估，并探索思维链提示和微调技术。

Result: 尽管这些模型在以往的多模态基准测试中表现成功，但在AVI-Math的推理任务上表现不佳，暴露出当前视觉语言模型在数学推理能力上的显著局限性。

Conclusion: 研究不仅揭示了视觉语言模型在数学推理方面的局限性，还为推进无人机应用中可信视觉语言模型的发展提供了宝贵见解，思维链提示和微调技术显示出解决这些推理挑战的潜力。
...
Method: 提出BEVTraj框架，在BEV空间直接操作实时传感器数据；使用可变形注意力从密集BEV特征中高效提取相关上下文；引入稀疏目标候选提案(SGCP)模块实现完全端到端预测，无需后处理步骤。
...
<details>
  <summary>Details</summary>
Motivation: 现有最先进方法在公开数据集上表现良好，但在处理身体重叠的多人分割时效果显著下降，因为重叠的人体从不同视角看可能是分离的。

Method: 提出基于弱监督人类实例和多视角一致性损失的新训练框架，利用多视角信息改进遮挡下的多人解析。采用半自动标注策略从多视角RGB+D数据和3D人体骨架生成实例分割掩码。

Result: 实验表明该方法在遮挡场景下相比基线模型实现了4.20%的相对改进。

Conclusion: 多视角信息可以有效提升多人解析模型在遮挡情况下的性能，提出的训练框架和标注策略为解决遮挡问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够同时处理韩语和英语的双语视觉语言模型，支持复杂多模态输入（如文档、图表、表格），并具备布局感知能力，以推动双语VLM的实际应用发展。

Method: 采用四阶段课程训练和内存高效技术，通过偏好优化提升安全性，同时保持核心语言能力，训练出14B和1.7B两个规模的模型版本。

Result: 模型在OpenCompass VLM排行榜上位列同规模模型第8名，展现出强大的空间定位能力和双语竞争性表现，同时发布了适用于设备部署的轻量版模型。

Conclusion: VARCO-VISION-2.0系列模型显著推进了双语视觉语言模型的发展，为实际应用提供了高质量的开源解决方案，包括完整版和轻量版两个变体。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人脸图像质量评估方法要么无法有效捕捉人脸特有的退化特征，要么计算复杂度太高，限制了在实际应用中的部署。需要开发既准确又高效的方法。
...
Method: 提出RCOD框架，包含潜在域分组策略、退化感知采样策略和视觉提示注入模块，实现推理阶段的真实感控制

Result: 在定量指标和视觉质量上均优于现有单步扩散方法，同时保持计算效率

Conclusion: RCOD框架成功解决了单步扩散模型在真实图像超分辨率任务中保真度与真实感权衡的灵活控制问题
...
Method: 采用两阶段方法：第一阶段通过梯度机制提取类别特异性特征进行不确定性量化和原型估计来精化伪标签；第二阶段使用基于余弦相似度的对比损失来增强视杯和视盘特征间的类间可分性。

Result: 在具有挑战性的跨域眼底成像数据集上，Grad-CL超越了最先进的无监督和源自由域适应方法，实现了优异的分割精度和改善的边界描绘。

Conclusion: Grad-CL框架有效解决了医学图像分割中的域适应问题，无需源数据即可实现稳健的性能迁移，对视网膜疾病的早期诊断具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 解决向量量化训练中的不稳定性问题，包括直通估计偏差、一步延迟更新和稀疏代码本梯度，这些问题导致了次优的重建性能和低代码本使用率。

Method: 提出VQBridge方案，基于map函数方法构建了一个稳健、可扩展的投影器，通过压缩-处理-恢复流水线优化代码向量，结合学习逆温技术实现100%代码本使用率。

Result: FVQ在多种代码本配置下均达到100%代码本使用率，包括262k大规模代码本，达到了独创的重建性能，且随着代码本规模、向量通道数或训练时间增加而持续改善。与LlamaGen集成后，图像生成性能显著提升，超越VAR 0.5 rFID和DiT 0.2 rFID。

Conclusion: VQBridge提供了一种简单有效的方案来解决向量量化训练的核心挑战，高质量的标记化器对强大的自回归图像生成至关重要，该方法具有良好的可扩展性和通用性。
...
<details>
  <summary>Details</summary>
Motivation: 观察到在视频掩码自编码训练中，ViT层按深度顺序收敛：浅层早收敛，深层晚收敛，这一现象可被利用来加速训练

Method: 通过显式进度表逐步冻结模型层，实现从像素预测到潜在预测的渐进过渡，避免表示坍塌问题

Result: 在高达40亿参数的大模型上应用LayerLock，在4DS感知套件上的结果超越了非潜在掩码预测方法

Conclusion: LayerLock提供了一种简单有效的自监督学习框架，通过层冻结进度表实现了训练加速和性能提升
...
<details>
  <summary>Details</summary>
Motivation: 研究外观嵌入在空间机器人应用中3D重建的作用，特别是几何精度这一关键需求，因为空间应用需要高精度的几何信息用于交互和碰撞避免

Method: 使用SPEED+数据集，比较K-Planes、高斯溅射和凸面溅射三种方法，分析外观嵌入对光度保真度和几何精度的影响

Result: 外观嵌入主要减少显式方法所需的图元数量而非提升几何保真度，凸面溅射比高斯溅射获得更紧凑和无杂波的表示
...
<details>
  <summary>Details</summary>
Motivation: 胎儿脑MRI通常使用快速多视角2D切片采集以减少运动伪影，但这些堆栈分辨率低、可能受运动影响且不能充分捕捉3D解剖结构。超分辨率重建方法旨在解决这些限制，但不同方法的比较性能特别是在病理情况下的表现，以及对下游体积分析和诊断任务的影响尚未充分探索。
...
Result: NeSVoR在健康对照组和病理组都表现出最高且最一致的重建成功率(>90%)。虽然不同SRR方法之间的体积估计存在显著差异，但脑室扩大的分类性能不受SRR方法选择的影响。

Conclusion: 研究结果突显了NeSVoR的鲁棒性，以及尽管SRR引起的体积变异性，诊断性能仍具有弹性。这表明在临床诊断应用中，SRR方法的选择可能不会显著影响最终的诊断准确性。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在物体移除任务中存在两个主要挑战：掩码幻觉（在掩码区域生成无关内容）和掩码形状偏差（生成内容模仿掩码形状而非周围环境）。需要专门针对物体移除任务的训练策略来解决这些问题。

Method: 提出MCR训练策略，在训练过程中引入两种掩码扰动：扩张和重塑。扩张掩码帮助模型输出与周围内容对齐，重塑掩码鼓励模型打破掩码形状偏差，通过强制这些扰动分支输出与原始掩码的一致性来实现。

Result: 实验表明MCR显著减少了幻觉和掩码形状偏差，在物体移除任务中实现了更好的性能，产生了更鲁棒和上下文一致的修复结果。

Conclusion: MCR是一种有效的训练策略，专门针对物体移除任务设计，通过掩码一致性正则化成功解决了当前方法面临的两个关键挑战，提升了图像修复的质量和一致性。
...
Method: 1）建立生成图像伪影的详细分类法；2）手动标注340K图像的大规模数据集MagicData340K；3）训练Vision-Language模型MagicAssessor进行评估；4）设计新颖的数据采样策略和多级奖励系统的GRPO方法；5）构建自动化基准MagicBench

Result: 评估发现即使像GPT-image-1这样的顶级模型也持续存在显著伪影，凸显了伪影减少是未来T2I发展的关键前沿

Conclusion: MagicMirror框架填补了T2I生成伪影评估的空白，为未来模型改进提供了重要基准，表明伪影减少是亟待解决的关键问题
...
<details>
  <summary>Details</summary>
Motivation: 当前手语翻译方法主要关注手势信号，忽视了唇部运动等非手动线索，而这些线索对于消除视觉相似符号的歧义至关重要。

Method: 提出SignClip框架，融合空间手势和唇部运动特征，引入分层对比学习框架，实现手语-唇部和视觉-文本模态的多层次对齐。

Result: 在PHOENIX14T和How2Sign基准测试中表现优异，在PHOENIX14T无词汇表设置下，BLEU-4从24.32提升到24.71，ROUGE从46.57提升到48.38。
...
TL;DR: 该论文分析了开源和闭源大型视觉语言模型在文本篡改检测方面的性能，发现开源模型正在接近但仍落后于GPT-4o等闭源模型，并揭示了专门用于图像篡改检测的VLM在文本检测中存在泛化问题。


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型在图像篡改检测方面表现出色，但文本篡改检测的研究相对缺失，需要填补这一知识空白。

Method: 在多个文本篡改数据集上对闭源和开源VLM进行系统性分析，包括真实场景文本和模拟现实滥用的伪造ID卡片。

Result: 开源模型性能正在提升但仍落后于闭源模型，专门用于图像篡改检测的VLM在文本检测任务上存在泛化能力不足的问题。

Conclusion: 文本篡改检测是VLM能力评估的重要维度，需要开发更具泛化能力的模型来应对现实世界中的文本篡改挑战。
...
Method: 提出了多模态提示学习机制（MPLM），包括对象无关的解耦文本提示和多模态对比损失，以增强模态内表示能力和模态间协作学习。同时设计了协作调制机制（CMM），通过联合调制RGB图像引导和点云引导分支来充分利用点云和RGB图像的互补表示。

Result: 大量实验表明，MCL-AD框架在零样本3D异常检测中实现了最先进的性能。

Conclusion: 通过多模态协作学习，MCL-AD框架能够有效整合不同模态的信息，在零样本3D异常检测任务中取得了优异的表现，证明了多模态方法在该领域的价值。
...
TL;DR: 提出了一种基于Lipschitz约束的随机深度(DropPath)方法，通过深度依赖的丢弃概率来控制网络的有效Lipschitz常数，在保持清洁精度的同时提升对抗鲁棒性并减少计算量。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络和Vision Transformers在计算机视觉中表现优异但对抗扰动脆弱，现有防御方法计算成本高或缺乏形式化保证，需要一种既能提升鲁棒性又保持效率和精度的解决方案。

Method: 采用Lipschitz引导的随机深度方法，设计深度依赖的丢弃概率调度策略，使丢弃概率随深度增加而增大，从而控制网络的有效Lipschitz常数，正则化深层网络。

Result: 在CIFAR-10数据集上使用ViT-Tiny的实验表明，该方法在保持接近基线清洁精度的同时，显著提升了对抗FGSM、PGD-20和AutoAttack的鲁棒性，并大幅减少了FLOPs计算量。

Conclusion: 深度依赖的DropPath调度策略是一种有效的正则化方法，能够在保持模型精度的同时提升对抗鲁棒性并降低计算复杂度，为构建高效鲁棒的视觉模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决城市公共基础设施监测维护中的精确定位问题，为地方当局和私营利益相关者提供有效的资产映射解决方案

Method: 基于能量地图的概率框架，编码物体位置的空间似然性；采用随机生死优化算法推断最可能的资产配置；整合GIS图层、道路地图等外部地理空间信息

Result: 在都柏林市中心街灯基础设施数据集上的仿真评估显示，该方法具有可扩展性和高精度的城市资产映射潜力

Conclusion: 提出的概率框架和优化算法能够有效解决复杂城市环境中的街道家具精确定位问题，为城市基础设施管理提供可靠的技术支持
...
TL;DR: 提出Cluster-Driven Feature Caching (ClusCa)方法，通过空间聚类减少扩散变换器中90%以上的token计算，实现4.96倍加速且保持图像质量


<details>
  <summary>Details</summary>
Motivation: 扩散变换器虽然能生成高质量图像和视频，但迭代去噪过程计算成本巨大。现有特征缓存方法只利用了时间维度的相似性，忽略了空间维度的相似性

Method: ClusCa在每个时间步对token进行空间聚类，每个聚类只计算一个token，然后将信息传播给其他token，大幅减少计算量

Result: 在DiT、FLUX和HunyuanVideo上实验显示，ClusCa在文本到图像和文本到视频生成中均有效，FLUX上实现4.96倍加速，ImageReward达到99.49%

Conclusion: ClusCa提供了一种正交且互补的特征缓存视角，无需训练即可直接应用于任何扩散变换器，显著加速推理过程
...
TL;DR: I-Segmenter是首个完全整数化的ViT分割框架，通过系统替换浮点运算、提出λ-ShiftGELU激活函数等方法，在保持精度的同时显著减少模型大小和推理时间。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在语义分割中表现优异，但在资源受限设备上部署受限，主要由于高内存占用和计算成本。量化虽能提高效率，但ViT分割模型在低精度下表现脆弱。

Method: 基于Segmenter架构，系统替换浮点运算为整数运算；提出λ-ShiftGELU激活函数处理长尾分布；移除L2归一化层；用最近邻上采样替换双线性插值。

Result: 在精度损失较小（平均5.1%）的情况下，模型大小减少3.8倍，推理速度提升1.2倍；单图像校准的PTQ也能获得有竞争力的精度。
...
<details>
  <summary>Details</summary>
Motivation: OCT图像中的散斑噪声会掩盖细节影响诊断，现有方法难以平衡噪声去除和结构保持，需要更准确的噪声统计建模。

Method: 提出GARD方法：使用Denoising Diffusion Gamma Model更准确建模散斑统计特性；引入Noise-Reduced Fidelity Term用预处理图像指导去噪；采用DDIM框架加速推理。

Result: 在配对噪声/低噪声OCT B扫描数据集上，GARD在PSNR、SSIM和MSE指标上显著优于传统方法和先进深度学习模型，定性结果显示边缘更清晰、解剖细节保持更好。

Conclusion: GARD通过伽马扩散模型和噪声减少保真度项，成功实现了OCT图像的高质量去噪，在保持解剖结构完整性方面表现出色。
...
TL;DR: GLAM模型通过几何引导的全局和局部对齐方法，解决了乳腺X光多视图分析中的关键问题，在多个数据集上超越了现有基线模型


<details>
  <summary>Details</summary>
Motivation: 现有的乳腺X光视觉语言模型大多从自然图像迁移而来，忽略了乳腺X光的多视图特性和几何关系，无法像放射科医生那样同时分析多个视图来处理同侧对应关系
...
Conclusion: 通过几何引导的多视图对齐方法能够有效提升乳腺X光视觉语言模型的性能，为医学影像分析提供了新的思路
...
TL;DR: 这是一篇关于视觉定位的综述论文，探讨了现代通用视觉语言模型中视觉定位的重要性、核心组件、应用场景、评估方法以及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 视觉定位能力使模型能够根据文本描述识别视觉输入中的特定区域，这种能力对于多模态应用至关重要，包括指代表达理解、细粒度视觉问答、实体引用字幕生成以及环境控制等。

Method: 论文采用综述研究方法，首先阐述视觉定位在视觉语言模型中的重要性，然后分析现代视觉定位模型的核心组件，并考察其实际应用，包括基准测试和评估指标。

Result: 论文系统梳理了视觉定位领域的研究现状，探讨了视觉定位与多模态思维链、推理能力之间的多维关系，为相关研究提供了全面的理论框架。

Conclusion: 论文分析了视觉定位面临的主要挑战，并提出了未来研究的潜在方向，强调了视觉定位在推动多模态人工智能发展中的关键作用。
...
TL;DR: 提出了一种针对文本图像编辑方法的视觉组件攻击——Attention Attack，通过使用源图像的自动生成标题作为编辑提示的代理，破坏文本提示与视觉表示之间的跨注意力机制，无需了解编辑方法或编辑提示即可破坏图像内容与文本描述的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的图像编辑方法容易受到对抗攻击，特别是在视觉组件方面存在脆弱性。需要一种无需了解具体编辑方法或提示就能有效破坏编辑过程的新型攻击方式。

Method: 使用源图像的自动生成标题作为编辑提示的代理，通过破坏文本提示与视觉表示之间的跨注意力机制来实施攻击。提出了Caption Similarity和语义IoU两种新的评估策略来衡量攻击效果。

Result: 在TEDBench++基准测试上的实验表明，该攻击方法显著降低了编辑性能，同时保持攻击的不可感知性。

Conclusion: Attention Attack是一种有效的对抗攻击方法，能够破坏文本图像编辑系统的视觉组件，提出的新评估指标为衡量免疫化成功提供了更可靠的度量标准。
...
TL;DR: 该论文研究如何通过知识蒸馏技术降低神经网络图像压缩模型的计算资源需求，使其更适合资源受限平台上的实时应用。
...
Method: 采用知识蒸馏训练范式，让较小的神经网络通过学习更大、更复杂模型的输出来获得更好的性能，从而减少处理需求和能耗。

Result: 研究表明知识蒸馏可以有效地应用于图像压缩任务：适用于不同架构大小、实现不同的图像质量/比特率权衡，并能节省处理和能源资源。
...
TL;DR: 提出一种无需训练的本征图像分解方法，仅使用可见光和热成像图像对，通过热成像检测吸收光来建立可见光与热成像强度之间的序数关系，从而自监督恢复着色和反射率。
...
Method: 利用热成像相机检测被吸收光产生的热量，建立可见光和热成像图像强度之间的序数关系，这些关系可以密集自监督优化神经网络来恢复着色和反射率。

Result: 在自然和人工光照下对已知反射率和着色进行定量评估，并在多样化户外场景进行定性实验，结果显示优于近期基于学习的模型。
...
TL;DR: 本文提出了压缩视频质量增强(CVQE)的新分类法、统一基准测试框架和系统性能分析，以解决现有综述在分类体系、比较分析和基准测试方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有压缩视频质量增强的综述缺乏系统性的分类方法，无法将具体方法与编码标准和伪影类型关联，对不同编码类型的架构范式比较分析不足，基准测试实践不完善。

Method: 提出了三个关键贡献：1）新的分类法，按架构范式、编码标准和压缩域特征利用对CVQE方法进行分类；2）统一的基准测试框架，整合现代压缩协议和标准测试序列；3）系统分析重建性能与计算复杂度之间的权衡关系。

Result: 建立了全面的CVQE评估基础，为研究部署提供一致的评估标准和明智的模型选择指导。

Conclusion: 该综述为CVQE领域建立了系统性框架，识别了性能与复杂度的关键权衡，并指出了未来研究的有前景方向。
...
Method: 提出MM SAM-adapter框架，使用适配器网络将融合的多模态特征注入到SAM的丰富RGB特征中，既能保留RGB特征的强泛化能力，又能选择性利用辅助模态提供的额外线索。

Result: 在DeLiVER、FMB和MUSES三个挑战性基准测试中实现了最先进的性能。特别是在RGB-easy和RGB-hard子集上的实验表明，该方法在有利和不利条件下都优于竞争方法。
...
Result: InfGen能够将许多模型提升到任意高分辨率时代，同时将4K图像生成时间削减到10秒以内，显著降低了计算复杂度。

Conclusion: InfGen提供了一种简化的任意分辨率图像生成解决方案，无需重新训练扩散模型，可应用于使用相同潜在空间的任何模型，实现了计算效率的显著提升。
...
TL;DR: 本研究将三种最先进的时序自监督学习方法应用于3D脑MRI分析，通过处理变长输入和学习鲁棒空间特征，在阿尔茨海默病预测任务中超越了监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病预测的深度学习模型面临标注数据缺乏、跨数据集泛化能力差以及对不同扫描数量和扫描时间间隔不灵活的局限性。

Method: 采用时序自监督学习（SSL）方法，包括时序顺序预测和对比学习，处理变长输入并学习鲁棒空间特征，在四个公开数据集（3,161名患者）上进行预训练。

Result: SSL模型在七个下游任务中的六个任务上超越了监督学习，展示了跨任务和不同输入图像数量及时间间隔的适应性和泛化能力。
...
TL;DR: 本文提出使用知识图谱(KG)结构化表示临床文档，用于自动化ICD编码任务，相比纯文本方法在Macro-F1指标上提升3.20%，同时提高训练效率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 临床文档标准化编码对临床研究和管理至关重要，但人工编码耗时费力。自动化编码面临高维长尾目标空间的挑战，现有方法在输入文档的外部知识利用方面研究不足。

Method: 构建文档级知识图谱来结构化表示患者状况，使用PLM-ICD架构集成KG进行ICD-9编码，KG仅用23%的原始文本就保留了90%的信息。

Result: 在主流基准测试中Macro-F1分数提升最高达3.20%，训练效率提高，KG中的实体和关系类型对性能提升有贡献。

Conclusion: 知识图谱结构化表示能有效提升临床文档自动化编码性能，同时提供更好的可解释性，为临床信息处理提供了新思路。
...
TL;DR: 提出Cross-Layer Attention Probing (CLAP)方法，通过处理LLM整个残差流的激活作为联合序列，显著改进了幻觉检测性能，支持细粒度检测和检测后缓解策略。
...
Result: 在五个LLM和三个任务上的实验表明，CLAP在贪婪解码和高温度采样响应中都优于基线方法，支持细粒度检测，并能有效减少幻觉提高可靠性。

Conclusion: CLAP提供了一种有效的幻觉检测方法，即使在分布外情况下也能保持高可靠性，为LLM的可靠性提升提供了实用解决方案。
...
### [67] [MultimodalHugs: Enabling Sign Language Processing in Hugging Face](https://arxiv.org/abs/2509.09729)
*Gerard Sant,Zifan Jiang,Carlos Escolano,Amit Moryossef,Mathias Müller,Rico Sennrich,Sarah Ebling*

Main category: cs.CL

TL;DR: MultimodalHugs是一个基于Hugging Face构建的框架，旨在解决手语处理研究中存在的复杂代码、低可复现性和不公平比较问题，支持更多样化的数据模态和任务。


<details>
  <summary>Details</summary>
Motivation: 手语处理研究相比口语语言研究面临复杂临时代码、低可复现性和不公平比较的挑战，现有工具如Hugging Face不够灵活，无法无缝集成手语实验。

Method: 在Hugging Face基础上构建MultimodalHugs框架，增加抽象层以支持更多数据模态和任务，特别是手语姿态估计数据和文本字符像素数据等多样化模态。

Result: 通过定量实验证明MultimodalHugs能够有效适应多种模态数据，为手语处理研究提供了更灵活和可复现的实验框架。

Conclusion: MultimodalHugs扩展了Hugging Face生态系统，不仅适用于手语处理，还可广泛应用于其他不符合Hugging Face标准模板的多模态用例。
...
### [68] [Optimal Multi-Task Learning at Regularization Horizon for Speech Translation Task](https://arxiv.org/abs/2509.09701)
*JungHo Jung,Junhyun Lee*

Main category: cs.CL

TL;DR: 本文从正则化角度研究多任务学习，探索跨模态一致性正则化和同模态R-drop正则化，提出正则化地平线概念，在MuST-C数据集上达到接近SOTA性能


<details>
  <summary>Details</summary>
Motivation: 端到端语音翻译面临配对数据稀缺问题，需要利用机器翻译的双语数据通过多任务学习来克服这一不足

Method: 从正则化视角构建多任务学习框架，研究跨模态一致性正则化和同模态R-drop正则化，分析MT损失系数作为正则化源，提出正则化地平线概念

Result: 在MuST-C数据集上通过调节正则化地平线内的超参数实现了接近最先进的性能

Conclusion: 多任务学习中的三种正则化源（跨模态一致性、同模态R-drop、MT损失系数）共同构成正则化地平线，通过优化这些正则化参数可以有效提升语音翻译性能
...
### [69] [Creativity Benchmark: A benchmark for marketing creativity for LLM models](https://arxiv.org/abs/2509.09702)
*Ninad Bhat,Kieran Browne,Pip Bingemann*

Main category: cs.CL

TL;DR: Creativity Benchmark是一个评估大语言模型在营销创意领域表现的框架，包含100个品牌和3种提示类型，通过678名专业创意人员的11,012次匿名比较显示模型性能差异很小，自动化评估无法替代人类专家评审。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估缺乏针对营销创意领域的专业基准，需要建立能够准确衡量模型在品牌约束条件下创意生成能力的评估体系。

Method: 使用100个品牌（12个类别）和三种提示类型（洞察、想法、疯狂想法），通过678名专业创意人员进行11,012次匿名成对比较，采用Bradley-Terry模型分析，并计算余弦距离来评估模型多样性和对提示重构的敏感性。

Result: 模型性能紧密聚集，没有模型在所有品牌或提示类型中占主导地位（Δθ≈0.45，胜率61%）。自动化评估与人类排名相关性弱且不一致，传统创造力测试在品牌约束任务中仅部分适用。

Conclusion: 需要专家人类评估和多样性感知的工作流程，自动化评委无法替代人类评估，凸显了在营销创意领域专业人类评审的重要性。
...
### [70] [CTCC: A Robust and Stealthy Fingerprinting Framework for Large Language Models via Cross-Turn Contextual Correlation Backdoor](https://arxiv.org/abs/2509.09703)
*Zhenhua Xu,Xixiang Zhao,Xubin Yue,Shengwei Tian,Changting Lin,Meng Han*

Main category: cs.CL

TL;DR: CTCC是一种新颖的规则驱动指纹框架，通过编码多轮对话中的上下文相关性来实现LLM所有权验证，解决了现有方法在隐蔽性、鲁棒性和通用性方面的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)的广泛部署加剧了知识产权保护担忧，现有指纹方法在隐蔽性、鲁棒性和通用性方面存在固有权衡，容易被检测或攻击。

Method: 提出CTCC框架，基于规则驱动，编码多轮对话中的上下文相关性（如反事实），而不是依赖词级或单轮触发器，支持黑盒访问下的指纹验证。

Result: 在多个LLM架构上的广泛实验表明，CTCC相比先前工作始终实现更强的隐蔽性和鲁棒性，有效减少误报和指纹泄露。

Conclusion: CTCC为现实世界LLM部署场景中的所有权验证提供了可靠实用的解决方案，支持在共享语义规则下即使部分触发器暴露也能持续构建。
...
### [71] [Temporal Preferences in Language Models for Long-Horizon Assistance](https://arxiv.org/abs/2509.09704)
...
<details>
  <summary>Details</summary>
Motivation: 研究语言模型的时间偏好特征及其可操纵性，为AI助手与人类长期目标的对齐提供设计依据，特别是在个性化情境校准和社会意识部署方面。

Method: 采用改编的人类实验协议，在时间权衡任务中评估多个语言模型，并与人类决策者进行基准比较。定义操作性指标MTO来衡量模型在面向未来和面向当前提示下的时间偏好变化。

Result: 推理型模型（如DeepSeek-Reasoner和grok-3-mini）在面向未来的提示下更倾向于选择延迟选项，但在跨身份或地理位置的个性化决策方面表现有限。能够正确推理时间导向的模型会为自身作为AI决策者内化未来导向。

Conclusion: 研究强调了AI助手需要与异质化、长期视野的目标对齐的重要性，并提出了个性化情境校准和社会意识部署的研究议程，为未来AI系统的设计提供了重要启示。
...
### [72] [The Non-Determinism of Small LLMs: Evidence of Low Answer Consistency in Repetition Trials of Standard Multiple-Choice Benchmarks](https://arxiv.org/abs/2509.09705)
...
<details>
  <summary>Details</summary>
Motivation: 评估小型开源LLM在多次回答相同问题时的答案一致性，研究不同参数设置对一致性的影响，以及一致性与准确性之间的权衡关系。

Method: 使用MMLU-Redux和MedQA多选基准测试，让模型重复回答10次相同问题，分析不同推理温度、模型大小（小型vs中型50B-80B）、微调vs基础模型等参数的影响。

Result: 小型模型在低温设置下，能够一致回答的问题比例通常在50%-80%之间，一致性答案的准确性与总体准确性有合理相关性。中型模型显示出更高的一致性水平。

Conclusion: 不同模型在答案一致性方面差异显著，低温设置有助于提高一致性，中型模型比小型模型具有更好的一致性表现，一致性与准确性之间存在一定的正相关关系。
...
### [73] [Beyond I'm Sorry, I Can't: Dissecting Large Language Model Refusal](https://arxiv.org/abs/2509.09708)
*Nirmalendu Prakash,Yeo Wei Jie,Amir Abdullah,Ranjan Satapathy,Erik Cambria,Roy Ka Wei Lee*

Main category: cs.CL

TL;DR: 该论文通过稀疏自编码器分析指令微调大语言模型的安全拒绝行为，开发了一个三阶段流程来识别和操控导致模型从拒绝转向服从的关键特征，揭示了安全机制的冗余性和可干预性。


<details>
  <summary>Details</summary>
Motivation: 理解指令微调大语言模型中安全拒绝行为的内部机制，尽管这是关键的安全特性，但其内部成因仍不明确。

Method: 使用稀疏自编码器分析残差流激活，通过三阶段流程：(1)寻找拒绝介导方向并收集相关特征；(2)贪婪过滤得到最小特征集；(3)使用因子分解机发现非线性交互作用。

Result: 成功识别出导致越狱的关键特征集，发现了冗余特征的存在，这些特征在早期特征被抑制时会激活。
...
### [74] [Assisting Research Proposal Writing with Large Language Models: Evaluation and Refinement](https://arxiv.org/abs/2509.09709)
*Jing Ren,Weiqi Wang*

Main category: cs.CL

TL;DR: 本研究提出基于内容质量和参考文献有效性的量化评估指标，以及基于这些指标的迭代提示方法，用于客观评估和提升ChatGPT在学术写作中的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在学术写作中日益普及，但存在错误或伪造参考文献等伦理问题，且当前的内容质量评估主要依赖主观人工判断，缺乏客观性和一致性。

Method: 提出两个关键评估指标（内容质量和参考文献有效性），并基于这些指标的得分设计迭代提示方法。通过大量实验验证方法的有效性。

Result: 提出的指标为评估ChatGPT写作表现提供了客观的量化框架。迭代提示显著提升了内容质量，同时减少了参考文献的不准确和伪造问题。

Conclusion: 该方法有效解决了学术写作中的关键伦理挑战，为大型语言模型在学术环境中的可靠应用提供了实用解决方案。
...
### [75] [Generating Individual Travel Diaries Using Large Language Models Informed by Census and Land-Use Data](https://arxiv.org/abs/2509.09710)
*Sepehr Golrokh Amin,Devin Rhoads,Fatemeh Fakhrmoosavi,Nicholas E. Lownes,John N. Ivan*

Main category: cs.CL

TL;DR: 本研究提出使用大型语言模型生成个体出行日记的新方法，通过开源数据生成虚拟人物并合成日记，验证显示与经典方法效果相当但在出行目的识别方面更优


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖大量专有家庭出行调查数据，成本高且难以获取。本研究旨在利用开源数据和LLM技术，开发更高效、低成本的出行日记生成方案

Method: 使用美国社区调查和智能位置数据库的开源数据随机生成虚拟人物，通过直接提示LLM合成出行日记。采用新颖的一对多真实性评分系统（包含四个指标），并使用Jensen-Shannon散度验证生成日记与真实日记的分布相似性

Result: LLM生成的日记与基于验证集校准的经典方法（负二项式用于出行生成，多项Logit用于方式/目的）相比，整体真实性相当（LLM均值：0.485 vs 0.455）。LLM在确定出行目的方面表现优异，且一致性更高；经典方法在出行次数和活动时长数值估计方面领先

Conclusion: LLM在零样本条件下具有可行性，为未来合成日记评估系统建立了可量化的真实性度量标准，证明了使用开源数据和LLM技术生成出行日记的统计代表性
...
### [76] [Psychiatry-Bench: A Multi-Task Benchmark for LLMs in Psychiatry](https://arxiv.org/abs/2509.09711)
...
Method: 基于权威专家验证的精神病学教科书和案例集构建基准，包含诊断推理、治疗计划、随访管理等11个任务类型，使用传统指标和LLM-as-judge相似性评分框架评估多种前沿LLM。
...
### [77] [The Thinking Therapist: Training Large Language Models to Deliver Acceptance and Commitment Therapy using Supervised Fine-Tuning and Odds Ratio Policy Optimization](https://arxiv.org/abs/2509.09712)
*Talha Tahir*

Main category: cs.CL

TL;DR: 本研究比较了SFT和ORPO两种训练方法对小型LLM进行接纳承诺疗法(ACT)训练的效果，发现ORPO方法在治疗保真度和同理心方面显著优于SFT和基础模型，而思维链推理仅对SFT模型有显著帮助。


<details>
  <summary>Details</summary>
Motivation: 探索不同训练方法和显式推理对小型开源大语言模型提供接纳承诺疗法能力的影响，以提升心理治疗AI助手的效能。

Method: 使用50组由Mistral-Large生成的合成ACT对话记录，对Llama-3.2-3b-Instruct进行两种训练：监督微调(SFT)和odds ratio策略优化(ORPO)，每种方法都包含有无思维链推理的变体。通过模拟治疗会话评估模型在ACT保真度量和治疗师同理心量表上的表现。

Result: ORPO训练模型在ACT保真度(χ²=185.15, p<.001)和治疗同理心(χ²=140.37, p<.001)上显著优于SFT和基础模型。思维链推理仅对SFT模型有显著帮助，平均提升ACT-FM分数2.68分(p<.001)，但对ORPO模型无显著优势。
...
### [78] [HANRAG: Heuristic Accurate Noise-resistant Retrieval-Augmented Generation for Multi-hop Question Answering](https://arxiv.org/abs/2509.09713)
*Duolin Sun,Dan Yang,Yue Shen,Yihan Jiao,Zhehao Tan,Jie Feng,Lianzhen Zhong,Jian Wang,Peng Wei,Jinjie Gu*

Main category: cs.CL

TL;DR: HANRAG是一个基于启发式的检索增强生成框架，通过查询路由、子查询分解和噪声过滤来解决多跳查询中的迭代检索效率低和噪声累积问题。


<details>
  <summary>Details</summary>
Motivation: 当前RAG方法在处理多跳查询时面临挑战：过度依赖迭代检索浪费步骤，原始复杂查询检索可能无法捕获相关子查询内容，导致噪声累积问题。

Method: 提出HANRAG框架，使用强大的揭示器进行查询路由、将查询分解为子查询，并对检索到的文档进行噪声过滤，增强系统适应性和抗噪能力。

Result: 在多个基准测试中与行业领先方法比较，该框架在单跳和多跳问答任务中都获得了优越性能。

Conclusion: HANRAG框架有效解决了多跳查询处理中的效率问题和噪声累积问题，显著提升了RAG系统处理多样化查询的能力。
...
### [79] [How Small Transformation Expose the Weakness of Semantic Similarity Measures](https://arxiv.org/abs/2509.09714)
...
TL;DR: 本研究评估了18种语义相似度测量方法，发现常用指标存在严重问题，某些嵌入方法将语义相反内容误判为相似的概率高达99.9%，而转换器方法有时认为相反含义比同义词更相似。


<details>
  <summary>Details</summary>
Motivation: 评估不同方法在测量语义相似度方面的有效性，这对于代码搜索、API推荐、自动化代码审查和重构工具等软件工程应用至关重要，特别是验证大语言模型是否真正理解语义关系而非仅识别表面模式。

Method: 创建系统化测试框架，对文本和代码应用受控变化，测试18种不同相似度测量方法，包括基于词的方法、嵌入技术、LLM-based系统和结构感知算法。

Result: 嵌入方法在计算距离时表现不佳，从欧几里得距离切换到余弦相似度使结果改善24-66%；LLM-based方法在区分语义差异方面表现更好，对真正不同含义给出低相似度分数(0.00-0.29)，而嵌入方法错误地为不相似内容分配高分(0.82-0.99)。

Conclusion: 当前常用的语义相似度测量方法存在显著缺陷，需要更可靠的评估指标；LLM-based方法在语义理解方面表现优于传统嵌入方法，但仍有改进空间。
...
### [80] [Investigating Symbolic Triggers of Hallucination in Gemma Models Across HaluEval and TruthfulQA](https://arxiv.org/abs/2509.09715)
*Naveen Lamba,Sanju Tiwari,Manas Gaur*

Main category: cs.CL

TL;DR: 本研究识别并表征了导致大型语言模型产生幻觉的关键符号属性，发现模型规模增大能降低幻觉率但符号元素仍是根本弱点


<details>
  <summary>Details</summary>
Motivation: 虽然LLM幻觉问题已被广泛研究，但导致模型内在易产生幻觉的属性尚未被识别和系统研究

Method: 利用HaluEval和TruthfulQA数据集，将问答格式转换为多种其他格式来验证符号属性作为幻觉原因，测试Gemma-2系列不同规模模型

Result: Gemma-2-2B平均幻觉率79.0%，随模型规模增大降至63.9%，但修饰语和命名实体的幻觉率仍高达84-95%
...
### [81] [ALIGNS: Unlocking nomological networks in psychological measurement through a large language model](https://arxiv.org/abs/2509.09723)
*Kai R. Larsen,Sen Yan,Roland Müller,Lan Sang,Mikko Rönkkö,Ravi Starzl,Donald Edmondson*

Main category: cs.CL

TL;DR: ALIGNS是一个基于大型语言模型的系统，用于生成包含55万+指标的综合nomological网络，解决心理学测量中验证网络构建的长期挑战。


<details>
  <summary>Details</summary>
Motivation: 解决Cronbach和Meehl提出70年来nomological网络构建的难题，避免临床试验无法检测治疗效果和公共政策目标错误等实际问题。

Method: 使用经过验证的问卷测量数据训练大型语言模型，开发ALIGNS系统，提供三个综合nomological网络，涵盖心理学、医学、社会政策等领域。

Result: 1) NIH PROMIS焦虑抑郁工具收敛为单一情绪困扰维度；2) 儿童气质测量识别出四个新维度和质疑一个现有维度；3) 心理测量专家评估显示系统重要性、可访问性和适用性良好。

Conclusion: ALIGNS是首个应用大语言模型解决测量验证基础问题的系统，免费提供nomologicalnetwork.org，为传统验证方法提供大规模nomological分析补充。
...
### [82] [Towards Reliable and Interpretable Document Question Answering via VLMs](https://arxiv.org/abs/2509.10129)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在文档理解中虽然能准确提取文本信息，但在答案的精确定位方面存在挑战，这限制了模型的可解释性和实际应用。

Method: 引入DocExplainerV0模块，该模块与现有VLM解耦，专门负责边界框预测和空间定位，支持包括专有系统在内的各种VLM。

Result: 系统评估显示文本准确性与空间定位之间存在显著差距，正确答案往往缺乏可靠的定位信息。

Conclusion: 该框架标准化了评估方法，为未来开发更可解释和鲁棒的文档信息提取VLM建立了基准。
...
### [83] [DiTTO-LLM: Framework for Discovering Topic-based Technology Opportunities via Large Language Model](https://arxiv.org/abs/2509.09724)
*Wonyoung Kim,Sujeong Seo,Juhyun Lee*

Main category: cs.CL

TL;DR: 基于技术间时序关系识别新兴技术机会的框架，利用大语言模型从专利数据中提取主题并追踪技术演变趋势


<details>
  <summary>Details</summary>
Motivation: 技术机会是推动技术进步、产业发展和创新的关键信息，需要系统化方法来识别新兴技术机会以支持战略决策

Method: 提出基于专利文本分析的框架：1）从专利数据集提取文本 2）映射文本主题发现技术间关系 3）追踪主题随时间变化识别技术机会 4）利用大语言模型提取主题和聊天模型提示支持机会发现

Result: 使用美国专利商标局AI专利数据集评估，实验结果表明人工智能技术正朝着便于日常访问的形式演变

Conclusion: 该框架展示了识别未来技术机会的潜力，为技术发展和创新管理提供了有效工具
...
### [84] [BIBERT-Pipe on Biomedical Nested Named Entity Linking at BioASQ 2025](https://arxiv.org/abs/2509.09725)
*Chunyu Li,Xindi Zheng,Siqi Liu*

Main category: cs.CL

TL;DR: 提出了一个轻量级的两阶段检索-排序管道系统BIBERT-Pipe，用于多语言生物医学嵌套命名实体链接任务，通过保持原始实体链接模型不变，仅修改三个任务对齐组件来提升性能


<details>
  <summary>Details</summary>
Motivation: 现有的生物医学实体链接基准主要针对英语单语和平坦提及，缺乏对嵌套和多语言提及的现实场景探索，BioNNE 2025共享任务旨在填补这一空白

Method: 两阶段检索-排序：检索阶段使用原始预训练模型，排序阶段进行领域特定微调；边界提示：在排序阶段用可学习的[Ms]/[Me]标签包装提及；数据集增强：自动扩展排名训练语料库

Result: 在BioNNE 2025多语言赛道中排名第三，证明了这些最小但原则性修改的有效性和竞争力

Conclusion: 该方法通过轻量级修改实现了对多语言生物医学嵌套实体链接的有效处理，代码已公开
...
### [85] [Natural Language Translation of Formal Proofs through Informalization of Proof Steps and Recursive Summarization along Proof Structure](https://arxiv.org/abs/2509.09726)
*Seiji Hattori,Takuya Matsuzaki,Makoto Fujiwara*

Main category: cs.CL

TL;DR: 利用LLM的非形式化能力将机器可验证的形式证明翻译为自然语言证明的方法


<details>
  <summary>Details</summary>
Motivation: 为了解决形式证明难以阅读和理解的问题，通过LLM的总结和语言化能力将形式证明转换为更易读的自然语言证明

Method: 利用大型语言模型的非形式化（形式语言证明步骤的言语化）和总结能力，将机器可验证的形式证明翻译为自然语言证明

Result: 在本科教材自然语言证明数据上评估，生成的自然语言证明质量与原证明相当；在Lean证明助手的现有形式证明库上应用，能够输出高可读性和准确性的自然语言证明
...
### [86] [A Role-Aware Multi-Agent Framework for Financial Education Question Answering with LLMs](https://arxiv.org/abs/2509.09727)
*Andy Zhu,Yingjun Du*

Main category: cs.CL

TL;DR: 这篇论文提出了一种多段代理框架，通过角色提示和批判精炼来提升金融问答的准确性，在专业金融问题上较基线提升6.6-8.3%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在金融问答中常常无法处理专业的多步数量推理、领域术语和实际场景理解，需要更有效的解决方案。

Method: 设计了一个包含Base Generator、Evidence Retriever和Expert Reviewer代理的多段代理框架，采用单次迭代流程生成精炼答案，结合RAG技术从6本金融教科书中获取上下文证据。

Result: 在3,532道专家设计的金融教育问题上，该方法比零射链式思绪基线提升了6.6-8.3%的准确率，Gemini-2.0-Flash表现最佳，GPT-4o-mini能达到专门调整的FinGPT模型的类似性能。

Conclusion: 该研究提供了一种成本效益高的方法来提升金融问答性能，为多段代理金融LLM系统的进一步研究提供了见解。
...
### [87] [A meta-analysis on the performance of machine-learning based language models for sentiment analysis](https://arxiv.org/abs/2509.09728)
*Elena Rohde,Jonas Klingwort,Christian Borgs*

Main category: cs.CL

TL;DR: 本文通过荟萃分析评估机器学习在Twitter情感分析中的性能，发现平均准确率为0.80，并指出总体准确率作为评估指标存在误导性，需要标准化报告规范。
...
Method: 采用PRISMA指南进行文献检索，从20项研究中筛选195个试验，使用双反正弦变换和三水平随机效应模型分析总体准确率这一最常用性能指标。

Result: AIC优化模型的平均总体准确率为0.80 [0.76, 0.84]。研究发现总体准确率因对类别不平衡和情感类别数量的敏感性而经常产生误导，需要标准化性能报告规范。

Conclusion: 需要规范机器学习分类器的性能报告标准，包括报告独立测试集的混淆矩阵，以确保跨研究的可靠比较，但目前这一实践远未普及。
...
<details>
  <summary>Details</summary>
Motivation: 中文古籍作为中华历史文化的重要载体，在数字化和理解方面面临挑战。现有文档基准主要关注英文印刷文本或简体中文，缺乏对中文古籍文档的评估标准。

Method: 构建AncientDoc基准测试，包含页面级OCR、白话翻译、推理问答、知识问答和语言变体问答五个任务，涵盖多种古籍类型和大量内容。使用多指标评估主流视觉语言模型，并辅以人工对齐的大语言模型进行评分。

Result: 创建了包含14种文档类型、超过100本书籍、约3000页内容的综合基准测试集，为评估VLMs在中文古籍处理能力提供了标准化的测试平台。

Conclusion: AncientDoc填补了中文古籍文档评估的空白，为促进视觉语言模型在古籍数字化和理解方面的发展提供了重要的基准工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基准测试无法准确评估在MCP新范式下的真实代理性能，导致对其实际操作价值的认知偏差和能力差异无法可靠区分。
...
Conclusion: MCP-AgentBench为研究社区提供了一个标准化、可靠的框架，用于构建、验证和推进能够充分利用MCP变革性优势的代理系统，加速真正能力和互操作性AI系统的发展。
...
TL;DR: 本研究分析了GPT-3.5和GPT-4o在决策和摘要任务中的偏见问题，发现决策任务存在显著的性别、年龄和背景偏见，而摘要任务偏见较小。研究还探讨了跨语言偏见传播和提示指令缓解策略的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的快速集成，研究者关注其可能加剧社会不平等和信息偏见的问题，特别是对背景、性别和年龄等方面的偏见影响。

Method: 使用Tamkin等人(2023)数据集的荷兰语翻译版本，创建了151,200个决策任务提示和176,400个摘要任务提示，测试了不同人口统计变量、指令、显著度水平和语言在GPT-3.5和GPT-4o上的表现。

Result: 两个模型在决策任务中都表现出显著偏见，偏向女性、年轻年龄和特定背景（如非裔美国人）。摘要任务偏见较小，但GPT-3.5在英语中显示出显著的年龄相关差异。跨语言分析显示英语和荷兰语的偏见模式大体相似。提示指令缓解策略最多能将最有利和最不利人口统计群体间的差距平均减少27%。

Conclusion: 研究强调了谨慎采用大语言模型和进行情境特异性偏见测试的重要性，需要继续开发有效的缓解策略以确保AI的责任部署。GPT-4o在所有英语提示中都显示出减少的偏见，表明在新模型中基于提示的缓解具有特定潜力。
...
TL;DR: HEFT是一种分层高效微调策略，结合LoRA和ReFT两种PEFT方法，在BoolQ基准测试中仅用3个epoch就达到85.17%准确率，超越了单独使用LoRA或ReFT方法20个epoch的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的专业化推理任务适配受计算资源限制，不同参数高效微调方法在权重空间或表示空间中运作，研究假设这些范式的协同组合可以释放更好的性能和效率。

Method: 提出HEFT分层适配策略：首先在权重空间使用LoRA进行广泛基础适配，然后在表示空间使用ReFT对内部激活进行精确外科式精炼，形成从粗到细的层次化微调。

Result: 在Llama-2-7B模型和BoolQ推理基准测试中，HEFT仅用3个epoch训练就达到85.17%准确率，优于LoRA-only（85.05%）和ReFT-only（83.36%）方法20个epoch的结果，显示出显著的协同效应。

Conclusion: PEFT方法的精心组合是一种强大的算法创新，为提升语言模型推理能力提供了更高效有效的路径，以更少的计算预算获得优异结果，为大规模模型适配复杂认知任务提供了原则性方法。
...
TL;DR: 提出通过语言与交互手势相关性建模多模态对话轮次组织的框架，基于语用框架分析开发标注方法，丰富了Frame2数据集的手势标注，证实手势在对话轮次管理中的作用


<details>
  <summary>Details</summary>
Motivation: 填补多模态对话轮次组织中手势策略研究空白，特别是缺乏可用于机器学习的标注数据集，探索自然环境下交互手势的使用模式

Method: 开发语用框架标注方法，在Frame2数据集（巴西电视剧视频文本语义框架标注）基础上增加对话轮次组织手势标注，分析语言与手势的相关性

Result: 确认面对面交流中手势用于传递、获取和保持对话轮次的作用，发现先前未记录的手势变体，证明语用框架标注有助于深入理解人类认知和语言

Conclusion: 手势使用源于语用框架的概念化，涉及心理空间、概念整合和隐喻，语用框架标注为理解人类多模态交流提供了新视角
...
<details>
  <summary>Details</summary>
Motivation: 多文档摘要的关键挑战在于有效整合多源信息同时保持连贯性和主题相关性。虽然大语言模型在单文档摘要中表现优异，但在多文档摘要方面仍有改进空间

Method: 首先通过显式提示模型使用主题标签来增强生成摘要的信息量，然后在GRPO框架中提出新颖的主题奖励机制来衡量生成摘要与源文档之间的主题对齐度
...
TL;DR: 评估大型语言模型生成合成调查回复的可靠性，与智利概率抽样调查的真实人类回复进行比较，发现LLM在信任项目上表现优异，但存在项目级异质性和人口统计偏差。


<details>
  <summary>Details</summary>
Motivation: LLMs在调查研究中作为合成受访者具有创新潜力，但需要验证其是否能准确恢复项目分布，避免复制训练数据中的社会刻板印象和偏见。

Method: 使用128个提示-模型-问题三元组生成189,696个合成配置文件，通过元分析比较准确性、精确度、召回率和F1分数，测试关键社会人口统计维度的偏差。

Result: 合成回复在信任项目上表现优异（F1分数和准确率>0.90）；GPT-4o、GPT-4o-mini和Llama 4 Maverick表现相当；45-59岁受访者的合成-人类对齐度最高。

Conclusion: LLM合成样本可以近似概率样本的回复，但存在显著的项目级异质性，需要仔细校准和额外分布测试以确保算法保真度和减少错误。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在推动法律人工智能发展方面取得显著进展，需要系统梳理LLM在法律领域的应用现状，为研究者和从业者提供全面的资源指南。

Method: 通过系统性的文献综述方法，收集和分析现有的法律LLM模型、框架、基准测试和数据集，并进行分类整理和评估。

Result: 构建了一个包含16个法律LLM系列、47个框架、15个基准测试和29个数据集的完整资源体系，为法律AI研究提供了系统性的参考。

Conclusion: LLM在法律AI领域具有巨大潜力，但仍面临诸多挑战，需要进一步研究和发展。本文为初学者提供了系统介绍，并鼓励该领域的未来研究。
...
Method: 构建了包含10万条藏语、5万条维吾尔语和5万条蒙古语数据的标题生成数据集CMHG，并创建了由母语者标注的高质量测试集。

Result: 成功创建了一个专门针对少数民族语言标题生成任务的大规模数据集，为相关研究提供了宝贵的资源。

Conclusion: 该数据集将推动中国少数民族语言标题生成技术的发展，并为该领域未来的基准测试做出贡献。
...
<details>
  <summary>Details</summary>
Motivation: 现有的无监督幻觉检测方法依赖与事实正确性无关的代理信号，存在偏差且泛化能力有限，需要开发更有效的检测方法

Method: 通过提示LLM验证陈述的真实性，获取其情境化嵌入作为特征，并将响应不确定性作为真实性的软伪标签进行训练

Result: IRIS在实验中 consistently 优于现有无监督方法，计算成本低，即使在少量训练数据下也能良好工作

Conclusion: IRIS提供了一个完全无监督、计算高效且适用于实时检测的幻觉检测解决方案，能够有效利用LLM内部表征进行事实正确性判断
...
TL;DR: 本文对开源大语言模型在多标签意图分类任务上的表现进行了全面分析，使用MultiWOZ 2.1数据集比较了LLama2-7B、Mistral-7B和Yi-6B模型在少样本设置下的性能，并与BERT监督学习基线进行对比。
...
Result: Mistral-7B-v0.1在14个意图类别中的11个上F分数表现最佳，加权平均F分数为0.50，具有较低的Hamming Loss和较高的Jaccard相似度。但BERT监督分类器的性能仍优于最佳少样本生成式LLM。

Conclusion: 开源LLM在多标签意图分类任务中展现潜力，Mistral-7B表现最佳，但监督学习方法仍具有优势。研究为小规模开源LLM在复杂多意图对话检测中的应用提供了实用框架。
...
TL;DR: 该研究通过分析社交媒体语言来追踪双相障碍(BD)诊断前后的语言变化轨迹，发现诊断前后存在反映情绪障碍、精神病共病、物质滥用等的语言改变，并在诊断后20年内观察到周期性情绪相关语言变化


<details>
  <summary>Details</summary>
Motivation: 语言是情感障碍的重要标记物，但临床评估规模有限。社交媒体语言分析因其高时间分辨率和纵向范围而受到关注，需要开发方法来识别诊断时间并研究双相障碍的语言轨迹

Method: 引入确定用户诊断时间的方法，分析从诊断前3年到诊断后21年的语言轨迹，与单相抑郁(UD)和非受影响用户(HC)进行对比

Result: 发现双相障碍诊断伴随普遍的语言改变，反映情绪障碍、精神病共病、物质滥用等；诊断后20年内观察到周期性情绪相关语言变化，具有明显的12个月周期性；趋势证据表明估计为女性的用户周期性增加

Conclusion: 研究结果为双相障碍急性和慢性期的语言改变提供了证据，验证并扩展了利用社交媒体进行心理健康可扩展监测的最新努力
...
Method: 使用AraBERTv2、AraELECTRA、MARBERT和CAMeLBERT四个模型集成，采用不同损失函数训练，结合加权训练、数据预处理、SAMER语料库重标注和Gemini 2.5 Flash生成约10,000个稀有级别样本的数据增强技术。

Result: 系统在句子级别达到87.5%的QWK，文档级别达到87.4%的QWK，后处理带来6.3%的QWK提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究使用人类设计的心理问卷（如BFI、PVQ）来测量大语言模型的人格特质和价值观，但这些问卷缺乏生态效度，无法反映LLMs在真实用户查询场景中的文本生成特性，需要系统比较两种问卷的差异

Method: 对传统心理测量问卷和生态效度问卷进行全面的对比分析，评估它们在测量LLMs人格特质时的表现差异

Result: 研究发现传统问卷：1）产生与生态效度问卷显著不同的人格特征剖面；2）项目数量不足导致测量不稳定；3）造成LLMs具有稳定人格结构的误导印象；4）对角色提示的LLMs产生夸大的特征剖面

Conclusion: 研究警告不要使用传统心理问卷来评估大语言模型，因为它们在生态效度和测量稳定性方面存在严重问题
...
TL;DR: 本文构建了一个气候科学领域的知识图谱，支持语义查询和LLM集成，帮助研究者精确查找模型、数据集和区域间的关联信息


<details>
  <summary>Details</summary>
Motivation: 气候科学文献日益复杂和庞大，传统关键词搜索难以满足研究者对模型、数据集、区域和变量间精确关联信息的查找需求

Method: 从气候出版物和科学文本构建领域特定知识图谱，支持Cypher查询语言进行结构化语义查询，并与大语言模型集成构建RAG系统

Result: 知识图谱能够回答诸如特定区域验证的模型、常用数据集与遥相关模式的关联等精确问题，提高了信息检索的准确性和效率

Conclusion: 该知识图谱为气候研究者、模型开发者等提供了准确、上下文相关的科学信息访问方式，超越了传统知识图谱构建，展示了实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 现有医院管理系统缺乏对非正规输入和少数语言提供准确实时医疗建议的能力，特别是在阿拉伯语等语言环境下。为了解决医疗资源紧张、过度拥挤和紧急医疗服务不足等全球性挑战。

Method: 收集社交媒体上的真实医患对话数据集，进行数据清洗和预处理以处理多种阿拉伯方言。微调Mistral-7B-Instruct-v0.2、LLaMA-2-7B和GPT-2 Medium等先进生成模型。

Result: 微调后的Mistral-7B模型表现最佳，在精确率、召回率和F1分数上分别达到68.5%、69.08%和68.5%的平均BERT分数。定性和定量评估验证了系统生成连贯相关医疗回复的能力。

Conclusion: 生成式AI在医院管理系统中具有巨大潜力，为全球医疗挑战提供了可扩展和适应性强的解决方案，特别是在语言和文化多样性的环境中。
...
TL;DR: 本研究提出了一种可扩展的合成数据增强策略，使用ChatGPT-4o和Gemini 2.5 Pro生成80,000个阿拉伯语医疗问答对，将训练语料扩展到100,000条记录，显著提升了阿拉伯医疗聊天机器人的性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语医疗聊天机器人的发展受到大规模高质量标注数据集稀缺的限制，现有20,000条交互数据难以支持模型的扩展性和泛化能力。

Method: 使用ChatGPT-4o和Gemini 2.5 Pro生成80,000个上下文相关且医学连贯的合成问答对，经过语义过滤和人工验证后整合到训练流程中，对包括Mistral-7B和AraGPT2在内的五个LLM进行微调。

Result: ChatGPT-4o生成的数据在所有模型中 consistently 获得更高的F1分数和更少的幻觉现象，合成数据增强显著提升了模型性能。

Conclusion: 合成数据增强是提升低资源医疗NLP领域特定语言模型性能的可行解决方案，为构建更具包容性、可扩展性和准确性的阿拉伯医疗聊天机器人系统铺平了道路。
...
TL;DR: 该论文研究了结合重音检测和语音识别的奥地利德语对话语音识别系统，通过微调wav2vec2模型开发重音检测器，并训练同时转录单词及其重音级别的ASR系统。


<details>
  <summary>Details</summary>
Motivation: 研究如何将韵律重音信息整合到自动语音识别系统中，以提升对话系统的表现并为语言学研究提供支持。

Method: 首先微调wav2vec2模型开发词级重音检测器，然后用其自动标注大型语料库中的韵律重音，最后基于这些标注训练同时转录单词和重音级别的ASR系统。

Result: 整合重音信息后ASR性能与基线系统相当，在识别正确的语句中重音检测准确率达到85.53%。

Conclusion: 基于Transformer的模型能有效编码韵律信息，为韵律增强的ASR系统提供了新的贡献，在语言研究和韵律感知对话系统中具有应用潜力。
...
TL;DR: 本文提出了一个系统框架，用于生成高质量、与人口分布对齐的AI角色集，以解决大语言模型社会模拟中的人口偏见问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型社会模拟研究主要关注代理框架和模拟环境设计，往往忽视了角色生成的复杂性以及非代表性角色集引入的潜在偏见。需要构建能够真实反映现实世界人口多样性和分布的角色集。

Method: 利用大语言模型从长期社交媒体数据生成叙事角色，通过严格质量评估筛选低质量档案，应用重要性采样实现与参考心理测量分布（如大五人格特质）的全局对齐，并引入任务特定模块针对特定子群体进行适配。

Result: 大量实验表明，该方法显著减少了人口层面的偏见，能够为广泛的研究和政策应用提供准确、灵活的社会模拟。

Conclusion: 提出的系统框架能够合成高质量、与人口分布对齐的角色集，为大语言模型驱动的社会模拟提供了更真实、更少偏见的基础，具有重要的研究和应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在识别大型语言模型生成文本与人类写作在语域变异维度上的系统性差异，特别是针对训练数据中代表性不足的非英语语言（如捷克语）。

Method: 采用Biber多维分析法，使用AI-Brown语料库（对应BE-21布朗家族语料库）和AI-Koditex语料库（捷克语），分析16个前沿模型在不同提示和设置下的文本生成表现，比较基础模型和指令调优模型的差异。

Result: 研究发现LLM在特定语域变异维度上与人类写作存在显著系统性差异，并建立了可用于模型比较和排名的可解释维度基准。

Conclusion: 该研究为评估LLM文本生成质量提供了新的多维分析框架，揭示了模型在不同语言和语域中的表现差异，对自然语言生成模型的改进具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 研究情感支持对话中良好意图的积极回应可能适得其反的问题，特别是在线支持对话中LLM生成回应可能显得轻描淡写或不现实乐观，需要更好地平衡积极情感与情感认同。

Method: 收集Reddit真实用户-助手对话，按情感强度分类为轻度（关系紧张、一般建议）和重度（悲伤、焦虑），使用LLM生成额外回应，微调LLM并开发弱监督多标签分类器集成（DeBERTa和MentalBERT）来检测不协调积极性类型。

Result: LLM在高风险情境下更容易通过轻描淡写和最小化语气表现出不现实的积极性，开发的分类器集成在检测不同类型不协调积极性方面表现改善。

Conclusion: 需要超越生成通用积极回应，研究协调的支持措施来平衡积极情感与情感认同，为构建情境感知和信任保持的在线对话系统提供见解。
...
TL;DR: 本文比较了多种语言模型在处理长文本分类任务（特别是法律文件）时的表现，发现专门为长文本设计的Longformer模型并无明显优势，开源模型在某些方面表现优于GPT变体。


<details>
  <summary>Details</summary>
Motivation: 解决当前主流语言模型（如BERT、RoBERTa）在处理长文本输入时的限制，特别是在法律文件分类等需要处理数百页文本的任务中。

Method: 使用XLM-RoBERTa、Longformer、GPT-3.5、GPT-4等模型在5种语言上进行多类分类实验，采用Comparative Agendas Project的21个政策主题标签体系。

Result: Longformer模型在处理长文本方面没有显示出特别优势；开源模型在比较中表现优于GPT变体；类别级别的支持度和内容重叠是影响长文本处理性能的重要因素。

Conclusion: 专门为长文本预训练的模型不一定优于通用模型，开源模型在处理长文本分类任务中具有竞争力，类别间的相似性对模型性能有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在知识密集型任务中因知识冲突而生成不忠实回答的问题，即模型倾向于依赖内部参数知识而非提供的上下文

Method: 使用自指导机制自动生成高质量结构化对比学习数据（锚样本、语义等价正样本、模拟不忠实场景的负样本），然后应用对比学习训练模型，使忠实回答在表示空间中更接近，不忠实回答更远离
...
<details>
  <summary>Details</summary>
Motivation: 稀疏混合专家模型虽然计算高效，但仍需加载所有专家参数，导致内存使用高且部署困难。现有方法主要关注专家级操作，忽视了神经元级结构

Method: DERN采用三步法：1)基于路由器统计剪枝冗余专家；2)将专家分解为神经元级片段并分配到最兼容的保留专家；3)在保留专家内合并片段构建紧凑表示

Result: 在Mixtral、Qwen和DeepSeek模型上，DERN在50%专家稀疏度下使常识推理和MMLU基准性能提升超过5%，同时大幅减少专家数量和内存使用

Conclusion: DERN通过神经元级重组有效解决了专家间语义冲突问题，实现了无需重新训练的专家压缩，使稀疏混合专家模型更易于实际部署
...
TL;DR: 本文通过大规模实验分析发现，上下文学习(ICL)确实构成学习机制，但其泛化能力有限，对提示格式和分布变化敏感，自回归编码机制不够鲁棒。


<details>
  <summary>Details</summary>
Motivation: 研究上下文学习(ICL)是否真正构成学习机制，以及其在未见任务上的学习和泛化能力，澄清ICL只是基于先验知识进行推理还是真正学习的争议。

Method: 进行大规模ICL分析实验，通过消融实验控制记忆效应、预训练、分布偏移、提示风格和措辞等因素，考察不同条件下ICL的表现。

Result: ICL是一种有效的学习范式，但在学习未见任务和泛化方面能力有限；当示例数量足够多时，准确率对示例分布、模型、提示风格和语言特征不敏感；ICL从提示中的规律性推断模式，导致分布敏感性，特别是在思维链等提示风格中。

Conclusion: 自回归的临时编码机制不够鲁棒，表明其通用泛化能力有限，ICL虽然构成学习但存在明显局限性。
...
TL;DR: 本研究评估了多种改进的Transformer架构模型（XLNet、Longformer、ModernBERT、Mamba、Llama）来解决传统语言模型在自动作文评分中因文本长度限制而需要截断输入的问题，使用Kaggle ASAP 2.0数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 传统基于Transformer的语言模型有固定的最大文本长度限制，而高年级学生的作文经常超过这个限制。在自动作文评分中截断输入文本会严重影响模型对评分标准中组织结构要素的完整评估能力，存在严重的有效性担忧。

Method: 研究评估了多种经过架构改进的模型，包括微调版本的XLNet、Longformer、ModernBERT、Mamba和Llama模型，这些模型都对标准Transformer架构进行了修改以克服长度限制。使用Kaggle ASAP 2.0数据集进行实验。

Result: 论文通过实验比较了这些改进模型在长文本处理方面的表现，但具体结果数据需要在完整论文中查看。

Conclusion: 研究表明，通过架构改进的模型能够更好地处理长文本输入，解决了传统Transformer模型在自动作文评分中的长度限制问题，提高了评分系统的有效性。
...
TL;DR: 提出了一种云边协同的多智能体提示框架，包含GuideLLM、SolverLLM和JudgeLLM三个组件，并在RefactorCoderQA基准测试中取得了76.84%的SOTA准确率。


<details>
  <summary>Details</summary>
Motivation: 为了优化大语言模型的推理和问题解决能力，解决现有基准测试的局限性，需要一种结构化、多领域的评估框架。

Method: 采用云边协同架构，包含边缘部署的轻量级GuideLLM提供方法指导、云端强大SolverLLM生成代码解决方案、以及自动评估器JudgeLLM。使用RefactorCoderQA多领域编码基准进行验证。

Result: 微调模型RefactorCoder-MoE达到76.84%的整体准确率，显著优于开源和商业基线模型，人类评估验证了生成方案的可解释性、准确性和实用性。

Conclusion: 提出的云边协同多智能体框架有效提升了LLM的编码能力，RefactorCoderQA基准为多领域编码任务提供了全面评估标准，系统性能指标分析揭示了架构的性能特征和权衡。
...
TL;DR: DeepDive通过自动合成复杂问题和多轮强化学习训练，显著提升了开源大语言模型在深度搜索任务中的表现，在BrowseComp基准测试中取得了新的开源竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 当前开源大语言模型在深度搜索任务中表现不佳，主要受限于长时程推理能力和缺乏足够难度的监督数据。

Method: 1) 从开放知识图谱自动合成复杂难找的问题；2) 应用端到端多轮强化学习来增强LLMs的长时程深度搜索推理能力。

Result: DeepDive-32B在BrowseComp基准测试中超越了WebSailor、DeepSeek-R1-Browse和Search-o1等模型，多轮RL训练显著提升了深度搜索能力。

Conclusion: DeepDive证明了通过自动数据合成和多轮强化学习可以有效提升开源LLMs的深度搜索性能，支持测试时工具调用扩展和并行采样。
...
TL;DR: WhisTLE是一种仅使用文本数据进行语音识别模型领域适应的新方法，通过变分自编码器建模编码器输出，结合文本到语音技术，在不增加推理成本的情况下显著降低词错误率。


<details>
  <summary>Details</summary>
Motivation: 预训练的语音识别模型如Whisper在遇到未见词汇和方言时性能下降，而收集语音数据往往不切实际，因此需要仅使用文本数据进行领域适应。

Method: 提出WhisTLE方法：1）训练变分自编码器从文本建模编码器输出；2）使用学习的文本到潜在编码器微调解码器；3）可选结合文本到语音技术进行适应；4）推理时恢复原始编码器，不增加运行时成本。

Result: 在4个领域外数据集和4个ASR模型上，WhisTLE结合TTS相比仅使用TTS的方法相对降低12.3%的词错误率，在32个场景中的27个都优于所有非WhisTLE基线方法。

Conclusion: WhisTLE提供了一种有效的文本only领域适应方案，能够显著提升预训练ASR模型在未见领域上的性能，且不增加推理成本，具有很好的实用价值。
...
TL;DR: SAM-BG是一个两阶段的自监督学习框架，通过结构语义保护来学习脑图表示，在精神病诊断中优于现有方法，特别是在小样本标注数据场景下。


<details>
  <summary>Details</summary>
Motivation: 脑网络标注数据有限，现有自监督学习方法的数据增强策略可能破坏脑图的关键结构语义，需要开发能够保护结构语义的新方法。

Method: 提出两阶段框架：预训练阶段在少量标注数据上训练边缘掩码器捕捉关键结构语义；自监督学习阶段利用结构先验指导结构感知的数据增强过程。

Result: 在两个真实精神病数据集上的实验表明，SAM-BG优于最先进方法，特别是在小标注数据设置下，并能发现临床相关的连接模式。

Conclusion: SAM-BG通过结构语义保护有效解决了脑图数据有限的问题，提高了诊断准确性和可解释性，为精神病诊断提供了新工具。
...
TL;DR: D-CAT是一种解耦跨注意力迁移学习框架，能够在推理阶段仅使用单一传感器的情况下实现跨模态知识迁移，解决了传统方法需要配对传感器数据的限制。


<details>
  <summary>Details</summary>
Motivation: 现有的跨模态迁移学习方法在训练和推理阶段都需要配对的传感器数据，这在资源受限环境中（如经济和技术上无法部署完整传感器套件）难以实现。需要一种能够在推理时仅使用单一传感器但仍能利用跨模态知识的方法。

Method: 提出了D-CAT框架，结合自注意力模块进行特征提取和新型跨注意力对齐损失，强制对齐不同传感器的特征空间，而不需要耦合两种模态的分类管道。

Result: 在三个多模态人类活动数据集（IMU、视频和音频）上评估，在分布内场景中，从高性能模态（如视频到IMU）迁移可获得高达10%的F1分数提升；在分布外场景中，即使较弱的源模态也能改善目标性能。

Conclusion: D-CAT通过实现单传感器推理的跨模态知识迁移，减少了感知系统的硬件冗余，同时保持准确性，这对于成本敏感或自适应部署（如家庭辅助机器人）至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 加密货币收益预测极其困难，价格受多种快速变化因素影响（链上活动、新闻流、社交情绪），且标注训练数据稀缺昂贵

Method: 从指令调优的LLM开始，代理在闭环架构中迭代扮演三个角色（执行者、评判者、元评判者），利用多模态市场输入和内部偏好反馈，持续精炼交易策略和评估标准

Result: 在不同市场机制下的实验表明，Meta-RL-Crypto在真实市场技术指标上表现良好，且优于其他基于LLM的基线方法

Conclusion: 该研究提出了一个无需人工监督的自改进交易代理框架，成功解决了加密货币预测的挑战，并在实际市场中展现出优越性能
...
TL;DR: LAVa是一个统一的KV缓存压缩框架，通过最小化Transformer残差流中的信息损失来实现动态预算分配，在长上下文推理中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的KV缓存压缩方法大多是启发式的，缺乏动态预算分配机制，无法根据不同任务需求灵活调整缓存资源。

Method: 通过分析层注意力输出损失，提出新的度量标准来比较不同头的缓存条目，实现层级压缩和动态头预算分配；通过对比跨层信息，实现动态层预算分配。

Result: 在多个基准测试（LongBench、Needle-In-A-Haystack、Ruler、InfiniteBench）上表现出优越性能，发现动态层预算对生成任务关键，动态头预算对抽取任务重要。

Conclusion: LAVa是首个统一的缓存淘汰和动态预算分配策略，无需训练或多策略组合，在各种任务类型中保持最佳性能。
...
TL;DR: 提出了HACO框架，结合保形风险校准和离线强化学习，为医疗补助人群的健康管理提供安全、公平且可审计的决策支持


<details>
  <summary>Details</summary>
Motivation: 医疗补助人群的健康管理项目需要协调纵向服务和干预措施，同时必须确保安全性、公平性和可审计性，需要一种能够控制不良事件风险的方法

Method: HACO框架：1）训练轻量级风险模型预测不良事件；2）使用保形阈值在目标风险水平下屏蔽不安全行动；3）在安全子集上学习偏好策略，并通过FQE评估策略性能

Result: HACO实现了强大的风险区分能力（AUC约0.81），校准阈值在α=0.10时为τ≈0.038，同时保持高安全覆盖率。亚组分析显示不同人口统计特征间存在系统性价值差异

Conclusion: 保形风险门控与离线强化学习相结合，能够为人口健康管理团队提供保守且可审计的决策支持，强调了公平性审计的重要性
...
TL;DR: 提出基于单头交叉注意力机制的统一路由框架，通过联合建模查询和模型嵌入实现动态选择最优LLM，在RouterBench基准上实现6.6%的质量提升和2.9%的最大性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型部署中不同计算成本和性能配置的挑战，实现可扩展、成本效益高的实时应用部署。
...
Conclusion: 该框架为成本感知的LLM路由建立了新标准，具有轻量级、高效和跨领域泛化能力强的特点。
...
TL;DR: 本研究使用机器学习和多模态融合策略，基于生理信号区分惊吓和惊讶事件，最高准确率达85.7%，并成功区分惊吓、惊讶和基线状态，准确率达74.9%。


<details>
  <summary>Details</summary>
Motivation: 意外事件会损害注意力和延迟决策，在航空等高危环境中构成严重安全风险。惊吓和惊讶反应以不同方式影响飞行员表现，但在实践中难以区分。现有研究大多单独研究这些反应，对其组合效应或如何使用生理数据区分它们的关注有限。

Method: 使用机器学习和多模态融合策略，基于生理信号区分惊吓和惊讶事件。采用SVM和XGBoost等算法，以及Late Fusion等多模态融合方法。

Result: 惊吓和惊讶事件可以可靠预测，SVM和Late Fusion组合达到最高平均准确率85.7%。在包含基线条件的扩展评估中，XGBoost和Late Fusion组合成功区分惊吓、惊讶和基线状态，最高平均准确率达74.9%。

Conclusion: 生理信号结合机器学习方法能够有效区分惊吓和惊讶反应，为高风险环境中飞行员表现监测和安全风险评估提供了有效工具。
...
TL;DR: 本文提出了一种解耦actor和critic熵的离散动作actor-critic框架，解决了DSAC在离散动作环境中的性能问题，在Atari游戏中达到了与DQN相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的离散动作强化学习方法中，值函数方法（如DQN）是主流，而策略方法要么无法有效利用离策略数据（如PPO），要么在离散动作设置中表现不佳（如SAC/DSAC）。DSAC性能差的主要原因是actor和critic熵的耦合。

Method: 提出了一个灵活的离策略actor-critic框架：1）解耦actor和critic的熵组件；2）使用m步Bellman算子进行critic更新；3）将标准策略优化方法与熵正则化结合来实例化actor目标。

Result: 理论证明在表格设置中可以收敛到最优正则化值函数。实证结果表明，在标准Atari游戏中可以达到DQN的性能水平，甚至在没有熵正则化或显式探索的情况下也能实现。

Conclusion: 通过解耦actor-critic熵组件，离散动作actor-critic方法可以达到与值函数方法相当的性能，为离散动作环境提供了有效的策略优化替代方案。
...
Method: 使用元路径结合随机丢弃创建等位基因GNN，通过残差注意力机制校准不同元路径的等位基因GNN，并使用相关性正则化项增大不同元路径生成的嵌入矩阵差异

Result: 在五个异质网络上的实验验证HGEN始终以显著优势优于最先进的竞争对手

Conclusion: HGEN通过有效的集成学习框架成功解决了异质图学习中的挑战，提高了分类性能
...
TL;DR: 本文提出了一种动态计算分配框架，在推理时根据查询需求选择最佳生成策略（如beam search或best-of-N），同时考虑token成本和延迟时间，在推理基准测试中优于静态策略。


<details>
  <summary>Details</summary>
Motivation: 现有推理时扩展方法主要关注并行生成方法如best-of-N，忽略了增量解码方法如beam search，且大多只关注token使用而忽略了延迟时间，这对于用户体验和智能体工作流至关重要。

Method: 将推理时扩展建模为动态计算分配和方法选择问题，系统需要基于每个查询决定应用哪种策略以及分配多少计算资源，明确考虑token成本和时钟延迟。

Result: 在推理基准测试中，该方法持续优于静态策略，实现了有利的准确率-成本权衡，同时保持部署的实用性。

Conclusion: 动态计算分配框架能够有效提升LLM性能，在考虑延迟和成本的情况下实现更好的推理效果，具有实际部署价值。
...
### [128] [Variational Neural Networks for Observable Thermodynamics (V-NOTS)](https://arxiv.org/abs/2509.09899)
*Christopher Eldred,François Gay-Balmaz,Vakhtang Putkaradze*

Main category: cs.LG

TL;DR: 提出基于热力学拉格朗日量的数据驱动计算框架，用于仅使用可观测变量预测耗散动力系统的相空间演化，确保熵不减特性


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法需要相空间变量信息，但耗散系统中动量和熵等变量通常无法直接观测，需要开发仅基于可观测变量的计算方法

Method: 构建基于热力学拉格朗日量的神经网络框架，通过神经网络确保热力学约束和熵不减演化特性，使用有限数据点和少量参数进行相空间演化描述

Result: 所提出的网络能够基于有限数据点和相对较少的系统参数，有效描述相空间演化

Conclusion: 该框架为仅使用可观测变量进行耗散系统相空间演化预测提供了有效解决方案，具有实际应用价值
...
### [129] [LoFT: Parameter-Efficient Fine-Tuning for Long-tailed Semi-Supervised Learning in Open-World Scenarios](https://arxiv.org/abs/2509.09926)
...
<details>
  <summary>Details</summary>
Motivation: 现有的长尾半监督学习方法通常从零开始训练模型，容易导致过度自信和低质量伪标签问题，需要将基础模型微调范式引入长尾半监督学习

Method: 提出LoFT框架，通过参数高效微调基础模型来生成更可靠的伪标签；针对开放世界场景提出LoFT-OW，处理未标注数据中可能存在的分布外样本

Result: 在多个基准测试中取得了优于先前方法的性能，即使仅使用先前方法1%的未标注数据也能实现优越结果

Conclusion: 基础模型微调能够为长尾半监督学习生成更可靠的伪标签，LoFT框架在标准设置和开放世界场景下都表现出色，为实际应用提供了有效解决方案
...
### [130] [Multi-Play Combinatorial Semi-Bandit Problem](https://arxiv.org/abs/2509.09933)
*Shintaro Nakamura,Yuko Kuroki,Wei Chen*

Main category: cs.LG

TL;DR: 提出了多选择组合半赌博机(MP-CSB)模型，扩展了传统组合半赌博机到非负整数动作空间，解决了最优运输和背包等问题。提出了两个算法：基于Thompson采样的算法和best-of-both-worlds算法，分别在不同场景下达到对数或次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 传统组合半赌博机(CSB)局限于二元决策空间，无法处理非负整数流或分配问题(如最优运输和背包问题)，需要扩展模型到更一般的动作空间。

Method: 提出MP-CSB模型，允许选择非负整数动作并从单臂获得多个反馈。设计了两个算法：1)基于Thompson采样的算法，计算高效且适用于指数大动作空间；2)best-of-both-worlds算法，在随机和对抗环境下都能良好表现。

Result: Thompson采样算法在随机环境下达到O(log T)分布依赖遗憾界；best-of-both-worlds算法在随机环境下达到O(log T)方差依赖遗憾界，在对抗环境下达到Õ(√T)最坏情况遗憾界，且具有数据依赖性。数值实验显示优于现有方法。

Conclusion: MP-CSB成功扩展了CSB到非负整数动作空间，提出的算法在理论和实验上都表现出色，为解决更广泛的组合优化问题提供了有效工具。
...
### [131] [SciML Agents: Write the Solver, Not the Solution](https://arxiv.org/abs/2509.09936)
*Saarth Gaonkar,Xiang Zheng,Haocheng Xi,Rishabh Tiwari,Kurt Keutzer,Dmitriy Morozov,Michael W. Mahoney,Amir Gholami*

Main category: cs.LG

TL;DR: 本文探讨使用LLMs生成科学计算代码来求解ODE问题，而非直接学习解函数。提出了包含误导性问题和1000个ODE任务的新基准，评估LLMs在数值算法选择和稳定性检查方面的能力。


<details>
  <summary>Details</summary>
Motivation: 传统科学机器学习方法直接预测目标值存在精度和鲁棒性挑战，本文探索使用LLMs编写代码来利用成熟的数值算法，将学习负担转移到领域感知的数值选择上。

Method: 引入两个新数据集：诊断性误导问题和1000个ODE任务的大规模基准。评估开源和闭源LLM模型，比较无引导vs领域知识引导提示，以及现成模型vs微调变体。

Result: 研究发现，在充分上下文和引导提示下，较新的指令跟随模型在两个评估标准（可执行性和数值有效性）上都达到高准确率。开源系统表现强劲，而较老或较小模型仍能从微调中受益。

Conclusion: 初步结果表明，通过精心设计的提示和微调，可以产生能够可靠解决简单ODE问题的专用LLM代理。
...
### [132] [DyKen-Hyena: Dynamic Kernel Generation via Cross-Modal Attention for Multimodal Intent Recognition](https://arxiv.org/abs/2509.09940)
*Yifei Wang,Wenbin Wang,Yong Luo*

Main category: cs.LG

TL;DR: DyKen-Hyena模型通过将音频视觉线索转换为动态的逐标记卷积核来直接调制文本特征提取，而不是简单的特征融合，在多模态意图识别任务中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态意图识别方法通过多头注意力等机制融合多模态特征，但可能因模态间的意图无关和冲突信息而污染主要语言特征，无法捕捉细粒度的标记级影响。

Method: 提出DyKen-Hyena模型，将问题从特征融合重新定义为处理调制，将音频视觉线索转换为动态的逐标记卷积核来直接调制文本特征提取过程。
...
### [133] [Adaptive Token Merging for Efficient Transformer Semantic Communication at the Edge](https://arxiv.org/abs/2509.09955)
...
Result: 在ImageNet分类上，用30%更少的FLOPs和低于20%的原始通信成本达到未修改transformer的精度；在VQA任务上，以不到三分之一计算量和十分之一带宽达到完整LLaVA模型的竞争性能

Conclusion: 该框架为资源受限的边缘智能场景部署强大transformer模型提供了实用且通用的解决方案，具有跨信道条件的鲁棒性和隐私保护优势
...
### [134] [Limited Reference, Reliable Generation: A Two-Component Framework for Tabular Data Generation in Low-Data Regimes](https://arxiv.org/abs/2509.09960)
*Mingxuan Jiang,Yongxin Wang,Ziyue Dai,Yicun Liu,Hongyi Nie,Sen Liu,Hongfeng Chai*

Main category: cs.LG

TL;DR: ReFine是一个合成表格数据生成框架，通过从可解释模型中提取符号规则嵌入提示词来指导生成，并采用双粒度过滤策略减少分布不平衡，在稀缺数据场景下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有表格生成方法（如GANs、扩散模型、微调LLMs）需要充足参考数据，在领域特定数据库记录稀缺时效果有限。基于提示的LLMs虽然灵活但难以捕捉数据集特定的特征-标签依赖关系，且生成冗余数据导致下游任务性能下降。
...
Conclusion: ReFine通过符号规则引导和双粒度过滤，有效解决了稀缺数据场景下的表格生成问题，显著提升了生成数据的质量和下游任务性能。
...
### [135] [Data-Driven Energy Estimation for Virtual Servers Using Combined System Metrics and Machine Learning](https://arxiv.org/abs/2509.09991)
*Amandip Sangha*

Main category: cs.LG

TL;DR: 提出了一种基于机器学习的虚拟服务器能耗估计方法，无需物理功率测量接口，仅使用虚拟机资源利用率指标就能准确预测能耗
...
Result: 在多样化工作负载实验中实现了高预测精度（R²在0.90到0.97之间），首次证明了仅基于客户机资源的能耗估计可行性

Conclusion: 该方法可用于实现能源感知调度、成本优化和独立于物理主机的能耗估计，为虚拟化环境提供了实用的能耗监控解决方案
...
### [136] [Neural Scaling Laws for Deep Regression](https://arxiv.org/abs/2509.10000)
*Tilen Cadez,Kyoung-Min Kim*

Main category: cs.LG

TL;DR: 本文实证研究了深度回归模型中的神经缩放定律，发现在扭曲范德瓦尔斯磁体参数估计模型中，损失与训练数据集大小和模型容量之间存在幂律关系，缩放指数范围为1到2。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型成功证明了神经缩放定律的重要性，但这些定律在深度回归模型中的应用仍未被充分探索。本文旨在填补这一空白，研究深度回归中的缩放行为。

Method: 使用扭曲范德瓦尔斯磁体的参数估计模型，在不同架构（包括全连接网络、残差网络和视觉变换器）上实证研究损失与训练数据集大小和模型容量之间的幂律关系。

Result: 观察到在广泛数值范围内存在幂律关系，缩放指数在1到2之间变化，具体取决于回归参数和模型细节。一致的缩放行为和大缩放指数表明深度回归模型性能可随数据量增加而显著提升。

Conclusion: 深度回归模型展现出与语言模型类似的神经缩放定律特性，大缩放指数意味着增加训练数据可以显著改善模型性能，这为资源有限的深度回归模型开发提供了重要指导。
...
### [137] [Intrinsic Dimension Estimating Autoencoder (IDEA) Using CancelOut Layer and a Projected Loss](https://arxiv.org/abs/2509.10011)
*Antoine Orioua,Philipp Krah,Julian Koellermeier*

Main category: cs.LG

TL;DR: 提出了IDEA方法，能够准确估计线性和非线性流形数据的内在维度，并通过重构损失指导训练，在理论基准和实际流体力学数据上都表现出色


<details>
  <summary>Details</summary>
Motivation: 现有方法在估计复杂数据集的内在维度方面存在局限性，特别是对于非线性流形数据。需要一种既能准确估计内在维度，又能保持数据重构能力的统一框架

Method: 使用带重加权双CancelOut层的自编码器结构，引入投影重构损失项，通过连续评估去除潜在维度后的重构质量来指导模型训练

Result: 在理论基准测试中显示出良好的准确性和高适应性，在流体力学数值解数据上成功估计了内在维度并实现了原始解的重构

Conclusion: IDEA方法为估计复杂数据集的内在维度提供了有效解决方案，特别适用于科学计算和工程应用中的高维数据降维和特征提取
...
### [138] [Exploring Expert Specialization through Unsupervised Training in Sparse Mixture of Experts](https://arxiv.org/abs/2509.10025)
*Strahinja Nikolic,Ilker Oguz,Demetri Psaltis*

Main category: cs.LG

TL;DR: 提出稀疏专家混合变分自编码器(SMoE-VAE)，在QuickDraw数据集上发现无监督专家路由比有监督基线重建性能更好，专家学习到的子类别结构超越了人工定义的类别边界。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习中神经网络内部组织的理解挑战，探索如何让模型自动发现数据的内在结构而非依赖人工标签。

Method: 使用稀疏专家混合变分自编码器架构，在QuickDraw数据集上比较无监督专家路由和有监督基线，通过t-SNE可视化和重建分析研究模型发现的数据结构。

Result: 无监督路由始终获得更优的重建性能，专家学习到有意义的子类别结构，这些结构往往超越人类定义的类别边界。数据集规模研究揭示了数据量与专家专业化之间的权衡。

Conclusion: MoE模型能够发现与模型目标更一致的基础数据结构，而非依赖预定义标签，为设计高效的MoE架构提供了指导。
...
### [139] [Sparse Coding Representation of 2-way Data](https://arxiv.org/abs/2509.10033)
*Boya Ma,Abram Magner,Maxwell McNeil,Petko Bogdanov*

Main category: cs.LG

TL;DR: 提出了一种用于双字典稀疏编码的低秩编码模型AODL，通过凸松弛和交替优化方法学习可泛化的字典，在保持重建质量的同时获得更稀疏的解，并揭示数据中的可解释模式。
...
Result: AODL在合成和真实数据集上实现了高质量的数据重建和缺失值插补，相比非低秩和固定字典基线，学习到稀疏度高达90%的解决方案。

Conclusion: AODL方法能够有效学习泛化性强的字典，提供更稀疏且可解释的解决方案，在多字典稀疏编码场景中表现出优越性能。
...
### [140] [Symbolic Feedforward Networks for Probabilistic Finite Automata: Exact Simulation and Learnability](https://arxiv.org/abs/2509.10034)
...
Result: 证明了PFAs与特定类别神经网络的等价性，通过标准梯度下降优化可以学习并恢复真实PFAs的精确行为。

Conclusion: 该工作建立了概率自动机理论与神经架构的统一框架，符号模拟器不仅具有表达性而且可学习，为连接符号计算与深度学习提供了理论基础。
...
### [141] [FedRP: A Communication-Efficient Approach for Differentially Private Federated Learning Using Random Projection](https://arxiv.org/abs/2509.10041)
*Mohammad Hasan Narimani,Mostafa Tavassolipour*

Main category: cs.LG

TL;DR: FedRP是一种新颖的联邦学习算法，结合随机投影和ADMM优化框架，通过降维传输模型参数来增强隐私保护和降低通信成本，提供强差分隐私保证。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护用户隐私的同时面临隐私攻击风险和通信成本管理的挑战，需要开发既能保持模型准确性又能有效保护隐私的高效算法。

Method: 提出FedRP算法，整合随机投影技术和ADMM优化框架，在参数传输前使用随机投影进行降维处理，减少通信量并增强隐私保护。

Result: 实验结果显示FedRP不仅保持高模型准确性，在隐私保护和通信效率方面均优于传统差分隐私方法和FedADMM，提供强(ε,δ)-差分隐私保证。
...
### [142] [Uncertainty-Aware Tabular Prediction: Evaluating VBLL-Enhanced TabPFN in Safety-Critical Medical Data](https://arxiv.org/abs/2509.10048)
...
Conclusion: VBLL集成并未提升TabPFN的不确定性校准性能，原始TabPFN表现出更好的校准能力
...
### [143] [KAN-SR: A Kolmogorov-Arnold Network Guided Symbolic Regression Framework](https://arxiv.org/abs/2509.10089)
...
Method: 使用Kolmogorov Arnold Networks (KANs)构建符号回归框架，采用分治法，结合平移对称性和可分离性等简化策略，并与神经控制微分方程结合来建模动态系统。

Result: 成功恢复了Feynman SRSD数据集的真实方程，并精确建模了硅内生物过程系统的动力学，为其他工程系统的动态建模开辟了道路。

Conclusion: KAN-SR框架通过结合深度学习、专门网络架构和简化策略，在符号回归任务中表现出色，特别是在科学方程发现和动态系统建模方面具有重要应用价值。
...
### [144] [Cost-Free Personalization via Information-Geometric Projection in Bayesian Federated Learning](https://arxiv.org/abs/2509.10132)
*Nour Jamoussi,Giuseppe Serra,Photios A. Stavrou,Marios Kountouris*

Main category: cs.LG

TL;DR: 提出了一种基于信息几何投影的贝叶斯联邦学习个性化框架，通过将全局模型投影到用户本地模型的邻域，实现可调节的全局泛化与本地特化权衡，并获得闭式解的无成本个性化。


<details>
  <summary>Details</summary>
Motivation: 现有贝叶斯联邦学习方法通常依赖MCMC采样或变分推断，需要复杂的个性化机制来适应异构数据分布。本文旨在开发一种更高效、可调节的个性化方法。

Method: 采用信息几何投影框架，将全局模型投影到用户本地模型的统计流形邻域，证明该投影等价于计算统计流形上的重心，从而获得闭式解。使用改进变分在线牛顿优化器，并扩展到贝叶斯联邦学习的一般聚合方案。

Result: 在异构数据分布下的实证评估表明，该方法能有效平衡全局和本地性能，且计算开销极小。

Conclusion: 提出的信息几何投影框架为贝叶斯联邦学习提供了一种高效、可调节的个性化方法，实现了全局泛化与本地特化的最优权衡，具有闭式解和最小计算开销的优势。
...
### [145] [BenchECG and xECG: a benchmark and baseline for ECG foundation models](https://arxiv.org/abs/2509.10151)
*Riccardo Lunelli,Angus Nicolson,Samuel Martin Pröll,Sebastian Johannes Reinstadler,Axel Bauer,Clemens Dlaska*

Main category: cs.LG

TL;DR: 提出了BenchECG标准化基准和xECG模型，通过统一评估框架解决ECG基础模型比较不一致的问题，xECG在多个数据集和任务上表现最优


<details>
  <summary>Details</summary>
Motivation: 现有ECG基础模型研究缺乏一致的评估标准，使用不同的任务选择和数据集，导致难以进行公平比较，阻碍了该领域的发展

Method: 开发BenchECG标准化基准，包含全面的公开ECG数据集和多样化任务；提出基于xLSTM的xECG模型，采用SimDINOv2自监督学习方法进行训练

Result: xECG在BenchECG基准测试中获得最佳分数，是唯一在所有数据集和任务上都表现优异的公开可用模型，性能优于现有最先进方法
...
### [146] [FedBiF: Communication-Efficient Federated Learning via Bits Freezing](https://arxiv.org/abs/2509.10161)
*Shiwei Li,Qunwei Li,Haozhao Wang,Ruixuan Li,Jianbin Lin,Wenliang Zhong*

Main category: cs.LG

TL;DR: FedBiF是一种新颖的联邦学习框架，通过在本地训练期间直接学习量化模型参数，每次只更新一个比特位，实现了1bpp上行和3bpp下行的通信压缩，同时保持与FedAvg相当的精度。


<details>
  <summary>Details</summary>
Motivation: 联邦学习存在显著的通信开销问题，现有量化方法通常在本地训练后应用量化，导致量化误差影响模型精度。需要一种在训练过程中直接学习量化参数的方法来减少通信成本同时保持精度。

Method: 提出FedBiF框架：服务器先量化模型参数并发送给客户端；每个客户端每次只更新多比特参数表示中的单个比特，冻结其余比特；采用逐比特更新策略。

Result: 在5个常用数据集上的IID和Non-IID设置下进行实验，FedBiF不仅实现了优异的通信压缩，还促进了模型稀疏性，在使用1bpp上行和3bpp下行通信时达到与FedAvg相当的精度。

Conclusion: FedBiF通过直接学习量化参数和逐比特更新策略，有效解决了联邦学习的通信开销问题，在保持模型精度的同时显著降低了通信成本。
...
### [147] [Federated Multi-Agent Reinforcement Learning for Privacy-Preserving and Energy-Aware Resource Management in 6G Edge Networks](https://arxiv.org/abs/2509.10163)
*Francisco Javier Esono Nkulu Andong,Qi Min*

Main category: cs.LG

TL;DR: 提出了一种联邦多智能体强化学习框架Fed-MARL，用于6G超密集边缘网络中的隐私保护、实时资源管理，通过跨层协调和加密聚合协议实现能效优化。
...
Conclusion: Fed-MARL框架为6G边缘网络提供了一种有效的隐私保护资源管理解决方案，能够同时优化多个性能指标并在动态资源受限环境中保持良好性能。
...
### [148] [A Symmetry-Integrated Approach to Surface Code Decoding](https://arxiv.org/abs/2509.10164)
*Hoshitaro Ohnishi,Hideo Mukai*

Main category: cs.LG

TL;DR: 提出了一种通过神经网络连续函数近似来重新优化表面码解码器的方法，提高了量子纠错解码的准确性


<details>
  <summary>Details</summary>
Motivation: 量子纠错对实用量子计算至关重要，但传统解码器因正确预测的非唯一性而只能获取误差概率分布，需要改进解码精度

Method: 使用神经网络数学插值近似综合征测量，将解码问题重新构建为回归问题，通过连续函数重新优化解码器模型

Result: 在码距5和7的多层感知机解码器，以及卷积神经网络、循环神经网络和变换器的码距5解码器上，重新优化的解码器都表现出比原始模型更好的准确性
...
### [149] [The Hidden Width of Deep ResNets: Tight Error Bounds and Phase Diagrams](https://arxiv.org/abs/2509.10167)
*Lénaïc Chizat*
...
<details>
  <summary>Details</summary>
Motivation: 研究深度残差网络在标准随机初始化下的训练动态，特别是当网络深度很大时的收敛行为，以及不同参数缩放对特征学习能力的影响。

Method: 采用数学分析方法，通过随机近似和传播混沌理论，证明深度残差网络的前向和后向传播行为可以近似为平均ODE，并分析不同残差缩放因子下的收敛性质。

Result: 获得了模型输出与其极限之间的误差界O(1/L + α/√(LM))，证明了当α=Θ(1)时可以实现完全特征学习，而α→∞时进入惰性ODE机制。对于两层感知器块，完全特征学习需要残差缩放因子为Θ(√D/(LM))。

Conclusion: 深度残差网络的训练动态可以通过平均ODE来刻画，不同的参数缩放策略会导致不同的特征学习能力，为理解深度网络训练提供了新的数学视角。
...
### [150] [P3D: Scalable Neural Surrogates for High-Resolution 3D Physics Simulations with Global Context](https://arxiv.org/abs/2509.10186)
*Benjamin Holzschuh,Georg Kohl,Florian Redinger,Nils Thuerey*

Main category: cs.LG

TL;DR: 提出了一种可扩展的框架，用于学习高分辨率3D物理模拟的确定性和概率性神经代理模型，采用混合CNN-Transformer架构，在速度和精度上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 为了解决高分辨率3D物理模拟中计算成本高、内存需求大的问题，需要开发能够有效学习多种偏微分方程动力学的高效神经代理模型。

Method: 采用混合CNN-Transformer主干架构，支持在小块模拟域上进行预训练，然后融合获得全局解，可选地通过序列到序列模型包含长程依赖关系。

Result: 在14种不同类型3D PDE的动力学学习任务中显著优于基线方法，能够扩展到512^3空间分辨率的高分辨率各向同性湍流，并能作为扩散模型生成不同雷诺数下高度湍流3D通道流的概率样本。

Conclusion: 该框架为高分辨率3D物理模拟提供了一种高效、可扩展的神经代理解决方案，在保持精度的同时大幅降低了计算和内存需求。
...
### [151] [Hadamard-Riemannian Optimization for Margin-Variance Ensemble](https://arxiv.org/abs/2509.10189)
*Zexu Jin*

Main category: cs.LG

TL;DR: 提出了一种新的集成学习框架，通过将边际方差显式纳入损失函数来提升模型鲁棒性和泛化性能，并通过权重重参数化提高计算效率。
...
Method: 提出了一种新的集成学习框架，将边际方差显式纳入损失函数，联合优化负期望边际及其方差。通过将集成权重重新参数化到单位球面上，简化了优化过程并提高了计算效率。

Result: 在多个基准数据集上的广泛实验表明，所提出的方法 consistently 优于传统的基于边际的集成技术，证明了其有效性和实用性。

Conclusion: 该研究通过显式考虑边际方差和优化权重参数化，显著提升了集成学习的性能和计算效率，为解决传统方法的局限性提供了有效解决方案。
...
### [152] [A Certifiable Machine Learning-Based Pipeline to Predict Fatigue Life of Aircraft Structures](https://arxiv.org/abs/2509.10227)
...
TL;DR: 提出基于机器学习的飞机机翼疲劳寿命预测管道，替代传统耗时复杂的有限元模拟方法，实现快速准确的疲劳寿命估计


<details>
  <summary>Details</summary>
Motivation: 传统飞机疲劳寿命预测方法耗时且复杂，需要多团队协作和大量有限元模拟。机器学习方法可以快速提供疲劳寿命估计，减少计算资源和人力需求

Method: 开发基于机器学习的管道，根据飞行参数预测飞机机翼不同位置的疲劳寿命，并进行统计验证和不确定性量化

Result: 在真实疲劳寿命估计用例中验证了管道的有效性，获得了准确的预测结果

Conclusion: 该机器学习管道是对传统方法的有效补充，能够显著减少昂贵的模拟需求，降低计算和人力资源要求
...
### [153] [Prompt Injection Attacks on LLM Generated Reviews of Scientific Publications](https://arxiv.org/abs/2509.10248)
*Janis Keuper*

Main category: cs.LG

TL;DR: 本文通过系统评估发现：简单的提示词注入攻击在LLM学术评审中极其有效（可达100%接受率），且LLM评审普遍存在接受偏向（>95%），这对LLM在同行评审中的使用具有重大影响。


<details>
  <summary>Details</summary>
Motivation: 针对作者使用隐藏提示词注入操纵评审分数的现象，研究这种攻击的可行性和技术成功率，为关于LLM在科学同行评审中使用的持续讨论提供实证依据。

Method: 使用多种LLM对2024年ICLR会议的1000篇论文进行系统评审评估，分析提示词注入攻击的效果和LLM评审的偏向性。

Result: 1) 非常简单提示词注入攻击高度有效，最高可达100%接受率；2) LLM评审普遍偏向接受（许多模型>95%接受率）。
...
### [154] [Property prediction for ionic liquids without prior structural knowledge using limited experimental data: A data-driven neural recommender system leveraging transfer learning](https://arxiv.org/abs/2509.10273)
...
Method: 采用两阶段方法：首先在固定温压下使用COSMO-RS模拟数据预训练神经推荐系统模型，学习阳离子和阴离子的性质特异性结构嵌入；然后使用这些嵌入和变温变压实验数据微调简单前馈神经网络。

Result: 该框架支持性质和跨性质知识迁移，预训练模型显著提升了四个目标性质的预测性能，能够稳健外推到未见过的离子液体，并为超过70万种离子液体组合提供性质预测。
...
### [155] [Proof of AutoML: SDN based Secure Energy Trading with Blockchain in Disaster Case](https://arxiv.org/abs/2509.10291)
...
Method: 采用SDN架构实现灵活的数据流和能源路由控制，使用五种AutoML选择的回归模型（梯度提升、LightGBM、随机森林、额外树和K近邻）生成随机nonce，通过9000样本数据集评估模型的随机性而非预测精度

Result: 随机森林和额外树回归器表现出完全的随机依赖性，梯度提升、K近邻和LightGBM分别具有97.6%、98.8%和99.9%的随机性得分，树基集成模型特别适合作为轻量级nonce生成器
...
### [156] [Generalizing Beyond Suboptimality: Offline Reinforcement Learning Learns Effective Scheduling through Random Data](https://arxiv.org/abs/2509.10303)
...
Method: CDQAC结合分位数critic和延迟策略更新，估计每个机器-操作对的回报分布而非直接选择，支持从多样化数据源学习

Result: CDQAC显著超越原始数据生成启发式方法和最先进的离线/在线RL基线，仅需10-20个训练实例即可学习高质量策略

Conclusion: CDQAC是高效的离线RL调度方法，意外发现在随机启发式数据上训练效果优于遗传算法和优先级调度规则生成的高质量数据
...
### [157] [GraphCSVAE: Graph Categorical Structured Variational Autoencoder for Spatiotemporal Auditing of Physical Vulnerability Towards Sustainable Post-Disaster Risk Reduction](https://arxiv.org/abs/2509.10308)
*Joshua Dimasaka,Christian Geiß,Robert Muir-Wood,Emily So*

Main category: cs.LG

TL;DR: 提出了GraphCSVAE框架，通过深度学习、图表示和分类概率推理整合卫星数据和专家知识，用于建模灾害后物理脆弱性的时空变化
...
Result: 在两个灾害频发地区(孟加拉国Khurushkul社区和塞拉利昂弗里敦市)成功揭示了灾后物理脆弱性的区域动态变化

Conclusion: 该框架为局部时空审计和灾后风险减少的可持续策略提供了有价值的见解，有助于改进灾害风险评估
...
### [158] [ARMA Block: A CNN-Based Autoregressive and Moving Average Module for Long-Term Time Series Forecasting](https://arxiv.org/abs/2509.10324)
*Myung Jin Kim,YeongHyeon Park,Il Dong Yun*

Main category: cs.LG

TL;DR: 提出基于ARIMA模型启发的简单卷积模块ARMA，用于长时序预测，包含趋势捕获和局部变化修正两个卷积组件，直接进行多步预测，在多个基准数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统ARIMA模型需要迭代多步预测且难以扩展到多变量设置，需要一种简单有效的长时序预测方法。

Method: 设计双卷积组件模块：一个卷积捕获趋势（自回归），另一个卷积修正局部变化（移动平均），直接进行多步预测。

Result: 在9个基准数据集上达到有竞争力的准确率，特别在趋势变化明显的数据集上表现优异，同时保持架构简洁。

Conclusion: ARMA模块不仅预测性能好，还能自然编码绝对位置信息，有潜力作为序列模型中位置编码的轻量级替代方案。
...
### [159] [Physics-informed sensor coverage through structure preserving machine learning](https://arxiv.org/abs/2509.10363)
*Benjamin David Shaffer,Brooks Kinch,Joseph Klobusicky,M. Ani Hsieh,Nathaniel Trask*

Main category: cs.LG

TL;DR: 提出了一个基于结构保持数字孪生的自适应源定位机器学习框架，结合有限元外微积分和基于transformer的算子学习，实现实时轨迹规划和数据同化
...
Method: 使用条件神经Whitney形式(CNWF)构建数字孪生，结合FEEC的数值保证和transformer算子学习，采用交错方案交替评估数字孪生和应用Lloyd算法指导传感器部署

Result: 实验表明，与物理无关的transformer架构相比，在复杂几何形状中强制物理约束时精度更高，结构保持为源识别提供了有效的归纳偏置

Conclusion: 结构保持的数字孪生框架能够有效实现自适应源定位，物理约束的保持对复杂环境中的源识别至关重要，正则性为定位提供了充分条件
...
TL;DR: 本文提出了一个统一的框架来形式化数据集压缩问题，将任务特定的压缩概念扩展到基于差异度量的更一般定义，并支持包括鲁棒性、隐私性在内的多种目标。


<details>
  <summary>Details</summary>
Motivation: 现有数据集压缩方法缺乏统一的理论框架，且主要关注泛化性能，需要扩展到更一般的概率分布近似问题和更多目标属性。

Method: 基于差异度量（discrepancy）构建统一框架，量化概率分布之间的距离，将数据集压缩形式化为分布近似问题。

Result: 提出了一个能够涵盖现有数据集压缩方法的统一理论框架，扩展了压缩任务的定义范围。

Conclusion: 该框架为数据集压缩提供了更严格的形式化基础，支持超越泛化性能的多种应用目标，为未来研究提供了理论基础。
...
TL;DR: 本研究探讨了对比学习在ECG预训练中对队列组成的依赖性，发现多中心多样化队列预训练虽然提高分布内准确性，但会编码队列特异性伪影降低分布外泛化能力，并提出了IDB策略来增强OOD鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 对比学习作为广泛采用的自监督预训练策略，但其对队列组成的依赖性尚未得到充分探索。研究者希望了解队列人口统计学、健康状况和人群多样性如何影响下游预测任务的性能。

Method: 提出了CAPE基础模型，在来自三大洲（北美、南美、亚洲）的四个队列（n=5,203,352）上进行预训练，并系统评估了预训练队列分布特性对下游性能的影响。为解决多中心预训练导致的OOD泛化问题，提出了In-Distribution Batch (IDB)策略。

Result: 发现下游性能取决于预训练队列的分布特性，包括人口统计学和健康状况。多中心、人口多样化队列预训练提高了分布内准确性，但通过编码队列特异性伪影降低了对比学习方法的分布外泛化能力。IDB策略在预训练期间保持队列内一致性，增强了OOD鲁棒性。

Conclusion: 这项工作为开发临床公平和可泛化的基础模型提供了重要见解，强调了预训练队列组成对模型性能的关键影响，并提出了有效的解决方案来改善分布外泛化能力。
...
TL;DR: 本文建立了无限维希尔伯特空间中整流流的严格函数形式化框架，扩展了有限维生成模型到无限维空间，并展示了在函数流匹配和概率流ODE方面的应用，实验证明性能优于现有函数生成模型。


<details>
  <summary>Details</summary>
Motivation: 许多在有限维欧几里得空间中开发的生成模型在无限维设置中具有函数泛化，但整流流向无限维空间的扩展尚未被探索。

Method: 基于无限维空间中连续性方程的叠加原理，建立了无限维希尔伯特空间中整流流的严格函数形式化框架，并将其扩展到函数流匹配和函数概率流ODE。

Result: 实验证明该方法相比现有函数生成模型具有更优越的性能，同时移除了现有理论中的限制性测度理论假设。

Conclusion: 成功建立了无限维空间中整流流的数学框架，为函数生成模型提供了更通用的理论基础，并在实践中展现出更好的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 解决相机陷阱生物多样性监测中物种识别标注资源有限的问题，传统主动学习方法只关注个体预测不确定性，而忽略了整个数据集的不确定性。

Method: 引入Vendi信息增益(VIG)主动学习策略，选择图像时同时考虑信息性和多样性，基于对数据集整体预测不确定性的影响来进行选择。

Result: 在Snapshot Serengeti数据集上，VIG使用不到10%的标签就达到了接近全监督的预测精度，在各种指标和批次大小下均优于标准基线方法，在特征空间中收集了更多样化的数据。
...
TL;DR: IGPO是一种针对掩码扩散大语言模型的强化学习框架，利用inpainting能力指导探索，通过插入部分真实推理轨迹来提升样本效率和性能，在数学推理任务上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 解决传统RL方法在LLM对齐中的探索挑战：稀疏奖励信号和样本浪费问题。dLLM的inpainting能力为引导探索提供了独特机会

Method: 提出IGPO框架，在在线采样时策略性地插入部分真实推理轨迹，结合监督微调与强化学习。还包括基于熵的过滤和合成重写简洁轨迹等技术

Result: 在GSM8K、Math500和AMC三个数学基准测试中取得显著提升，为全注意力掩码dLLM实现了新的最先进结果

Conclusion: IGPO有效解决了dLLM强化学习中的探索效率问题，inpainting指导的探索策略能够恢复有意义的梯度并提高样本效率，为dLLM的RL对齐提供了新思路
...
TL;DR: MuSe是一种高效的softmax注意力近似方法，通过语义聚类和多极展开技术降低Transformer的计算复杂度，在保持性能的同时显著提升训练效率。
...
Method: 结合语义聚类和计算物理中的多极展开技术，分别对查询和键在学习的表示空间中进行聚类，采用层次化的两阶段注意力机制，并在质心近似基础上增加偶极校正来捕捉簇内方向方差。

Result: 在8k上下文长度下比CUDNN Flash Attention快3倍，相对平方误差低于20%；在16k上下文的30M参数模型预训练中，运行时减少12.2%，性能损失仅0.36%。

Conclusion: 多极近似方法为高效Transformer预训练提供了可行方案，能够在不修改架构的情况下作为标准注意力的即插即用替代方案，显著提升长序列处理效率。
...
<details>
  <summary>Details</summary>
Motivation: 随着铁路系统复杂性和关键性增加，需要增强系统韧性来应对设计时未知的残余故障、系统环境变化和网络威胁

Method: 采用过程挖掘从执行轨迹学习实际控制流，进行在线一致性检查，并结合无监督机器学习进行异常定位

Result: 在ERTMS/ETCS L2 RBC切换场景中测试，显示出高精度、高效性和可解释性的异常检测与定位能力
...
TL;DR: 本文研究了Local SGD中外部优化器的作用，证明了新的收敛保证，发现调整外部学习率可以在优化误差和随机梯度噪声方差之间进行权衡，并弥补内部学习率的不良调整。理论表明外部学习率有时应大于1，且外部优化器中的动量可以改善收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习需要在大批量、分布式数据和并行计算硬件上训练，通信成为主要瓶颈。虽然Local SGD能减少通信开销，但关于外部优化器及其超参数选择的研究较少，需要深入理解外部优化器在Local SGD中的作用。

Method: 通过理论分析证明Local SGD的收敛保证，研究外部学习率调整、动量机制和加速方法的作用，并进行数据依赖性分析。使用标准语言模型和各种外部优化器进行综合实验验证理论。

Result: 理论表明调整外部学习率可以在优化误差和随机梯度噪声方差之间权衡，并能弥补内部学习率的不良调整。外部优化器中的动量可以改善收敛速度，加速方法提高了通信轮数的收敛率。实验验证了理论发现。

Conclusion: 外部优化器在Local SGD中起着关键作用，适当调整外部学习率（有时需要大于1）和使用动量机制可以显著改善算法性能，为分布式机器学习中的通信效率优化提供了重要指导。
...
### [168] [LLM-Based Instance-Driven Heuristic Bias In the Context of a Biased Random Key Genetic Algorithm](https://arxiv.org/abs/2509.09707)
*Camilo Chacón Sartori,Martín Isla Pino,Pedro Pinacho-Davidson,Christian Blum*

Main category: cs.NE

TL;DR: 提出了一种将大语言模型与遗传算法结合的框架，通过LLM分析问题实例特征来生成定制化的启发式偏差，用于解决最长运行子序列问题，在复杂实例上取得了显著改进


<details>
  <summary>Details</summary>
Motivation: 现有方法大多利用LLM进行代码生成来创建或改进特定启发式算法，但往往忽视了单个问题实例的结构特性，需要一种能够针对具体实例特征生成定制化启发式偏差的方法

Method: 引入人类-LLM协作过程共同设计和实现计算高效的度量指标，LLM分析这些实例特定指标来生成定制的启发式偏差，指导BRKGA算法在搜索空间中寻找有希望的区域

Result: 在1,050个不同复杂度的生成实例上进行测试，最佳混合方法BRKGA+Llama-4-Maverick相比标准BRKGA基线取得了统计显著的改进，特别是在最复杂的实例上表现突出

Conclusion: 利用LLM产生先验的、实例驱动的启发式偏差是增强复杂优化领域中元启发式算法的有效方法
...
### [169] [Predictive Spike Timing Enables Distributed Shortest Path Computation in Spiking Neural Networks](https://arxiv.org/abs/2509.10077)
*Simen Storesund,Kristian Valset Aars,Robin Dietrich,Nicolai Waniek*

Main category: cs.NE

TL;DR: 提出了一种基于局部脉冲时序的生物可信最短路径算法，通过抑制-兴奋消息对的时序压缩机制，无需全局状态或反向追踪即可计算所有最短路径


<details>
  <summary>Details</summary>
Motivation: 解决现有图算法（如Dijkstra、A*）需要全局状态和生物不可信操作，以及强化学习方法依赖缓慢梯度更新的问题，寻找与生物计算兼容的快速路径规划方法

Method: 利用局部脉冲消息传递和实际处理延迟，通过脉冲时序重合识别最优路径节点。神经元根据比预期更早收到的抑制-兴奋消息对减少响应延迟，形成从目标向源反向传播的时间压缩

Result: 通过理论证明和在随机空间网络上的仿真验证，算法能够收敛并发现所有最短路径，仅使用时序机制完成计算

Conclusion: 该工作展示了短期时序动态如何单独计算最短路径，为理解生物网络如何通过纯局部计算和相对脉冲时间预测解决复杂计算问题提供了新见解，对计算神经科学、AI和神经形态系统有重要意义
...
### [170] [Evolution of Coordination Through Institutional Incentives: An Evolutionary Game Theory Approach](https://arxiv.org/abs/2509.10112)
*Ndidi Bianca Ogbo,Zhao Song,The Anh Han*

Main category: cs.GT

TL;DR: 本文开发了一个进化博弈论模型，研究在有限预算下如何最优分配制度激励资源（奖励或惩罚）来促进承诺参与和确保承诺遵守，发现奖励方法比惩罚方法更有效。


<details>
  <summary>Details</summary>
Motivation: 认识到承诺机制在促进协调和合作行为方面的重要性，但现有研究大多停留在定性分析，缺乏对稀缺制度资源在促进参与和确保遵守之间分配策略的定量研究。

Method: 开发了一个进化博弈论模型，明确研究在预承诺框架内，有限预算的制度激励（奖励或惩罚）在这两个关键目标之间的战略分配。

Result: 研究发现奖励激励方法比惩罚方法始终产生更大的协调成功率，当资源在促进参与和确保遵守之间适当分配时获得最优结果。

Conclusion: 这些发现为设计制度激励以促进新技术的广泛协调采用提供了新的见解，强调奖励方法和资源优化分配的重要性。
...
### [171] [DB3 Team's Solution For Meta KDD Cup' 25](https://arxiv.org/abs/2509.09681)
...
<details>
  <summary>Details</summary>
Motivation: 解决CRAG-MM挑战赛中的多模态、多轮问答基准问题，特别是处理图像索引知识图谱、网络资源和多轮对话的复杂检索需求，以及控制LLM幻觉问题

Method: 开发了包含(1)针对不同任务的领域特定检索管道，处理图像索引知识图谱、网络资源和多轮对话；(2)使用SFT、DPO和RL进行高级拒绝训练的统一的LLM调优方法

Result: 在Task 1获得第2名，Task 2获得第2名，Task 3获得第1名，凭借对第一人称视角挑战的卓越处理赢得了自我中心查询的优秀大奖

Conclusion: 该综合框架成功整合了专门检索管道和统一的LLM调优方法，在多模态多轮问答任务中表现出色，特别是在处理自我中心查询方面具有优势
...
### [172] [TalkPlayData 2: An Agentic Synthetic Data Pipeline for Multimodal Conversational Music Recommendation](https://arxiv.org/abs/2509.09685)
*Keunwoo Choi,Seungheon Doh,Juhan Nam*

Main category: cs.IR

TL;DR: TalkPlayData 2是一个通过多智能体LLM管道生成的合成多模态对话音乐推荐数据集，包含音频和图像的多模态对话，用于训练生成式音乐推荐模型。


<details>
  <summary>Details</summary>
Motivation: 为了解决多模态对话音乐推荐数据稀缺的问题，通过智能体数据管道生成高质量的合成对话数据，以支持生成式推荐模型的训练。

Method: 创建多个具有不同角色和专门提示的LLM智能体，让Listener LLM和Recsys LLM进行对话并记录数据，每个对话基于微调的对话目标，所有LLM都支持音频和图像的多模态处理。

Result: 在LLM-as-a-judge和主观评估实验中，TalkPlayData 2在训练生成式音乐推荐模型的各个方面都达到了预期目标。

Conclusion: TalkPlayData 2成功构建了一个高质量的多模态对话音乐推荐数据集，并通过开源方式提供数据集和生成代码，为相关研究提供了重要资源。
...
### [173] [Faster and Memory-Efficient Training of Sequential Recommendation Models for Large Catalogs](https://arxiv.org/abs/2509.09682)
...
TL;DR: 提出了CCE-方法，一种GPU高效的交叉熵损失实现，可加速训练2倍并减少10倍以上内存消耗，支持更多负样本和批量大小来提升推荐模型性能


<details>
  <summary>Details</summary>
Motivation: 解决基于transformer的顺序推荐模型训练时因交叉熵损失导致的高计算成本和内存消耗问题，特别是在大规模商品目录场景下

Method: 开发CCE-方法，通过GPU高效实现交叉熵损失与负采样，使用Triton内核优化内存使用和计算效率

Result: 训练速度提升2倍，内存消耗减少10倍以上，在大规模商品目录数据集上相比原始PyTorch实现获得更好的准确率

Conclusion: CCE-方法有效解决了推荐系统训练中的内存瓶颈问题，同时证明了同时增加负样本数量和批量大小比单独优化任一参数能获得更好的模型性能
...
### [174] [Forecasting Clicks in Digital Advertising: Multimodal Inputs and Interpretable Outputs](https://arxiv.org/abs/2509.09683)
*Briti Gangopadhyay,Zhao Wang,Shingo Takamatsu*

Main category: cs.IR

TL;DR: 提出了一种结合点击数据和文本日志的多模态预测框架，使用强化学习提升文本理解能力，在准确性和可解释性方面优于传统方法


<details>
  <summary>Details</summary>
Motivation: 传统时间序列模型仅依赖数值数据，忽略了关键词更新等文本信息中的丰富上下文，影响了点击量预测的准确性和可解释性

Method: 多模态预测框架，结合点击数据和广告活动的文本日志，使用强化学习来改善文本信息理解和多模态融合，同时生成人类可解释的预测解释

Result: 在大规模行业数据集上的实验表明，该方法在准确性和推理质量方面均优于基线模型

Conclusion: 该多模态框架通过融合文本和数值信息，显著提升了点击量预测性能，并为数字广告提供了更可解释的预测结果
...
### [175] [Text-to-SQL Oriented to the Process Mining Domain: A PT-EN Dataset for Query Translation](https://arxiv.org/abs/2509.09684)
*Bruno Yui Yamate,Thais Rodrigues Neubauer,Marcelo Fantinato,Sarajane Marques Peres*

Main category: cs.IR

TL;DR: 本文介绍了text-2-SQL-4-PM，一个用于流程挖掘领域文本到SQL任务的双语（葡萄牙语-英语）基准数据集，包含1,655个自然语言语句和205个SQL语句，通过专家人工策划和专业翻译构建。


<details>
  <summary>Details</summary>
Motivation: 文本到SQL转换可以促进自然语言查询数据库，提高非SQL专家用户的可访问性和专家用户的生产力。针对流程挖掘领域的特殊挑战（如专业词汇和单表关系结构）需要定制化的数据集。

Method: 采用专家人工策划、专业翻译和详细标注流程的方法构建数据集，包括手动整理自然语言语句、SQL语句和限定词，并使用GPT-3.5 Turbo进行基线研究。

Result: 数据集支持文本到SQL实现的评估，展示了在流程挖掘领域应用的可行性和实用性，为语义解析和其他自然语言处理任务提供更广泛的适用性。

Conclusion: text-2-SQL-4-PM是一个专门为流程挖掘领域设计的有效基准数据集，能够促进文本到SQL技术在该领域的应用和发展。
...
### [176] [GeoGPT.RAG Technical Report](https://arxiv.org/abs/2509.09686)
*Fei Huang,Fan Wu,Zeqing Zhang,Qihao Wang,Long Zhang,Grant Michael Boquet,Hongyang Chen*

Main category: cs.IR

TL;DR: GeoGPT是一个开源的地球科学大语言模型系统，通过检索增强生成(RAG)技术从专业地学知识库中检索信息，结合用户个性化知识库，并优化嵌入和排序模型，显著提升了地学领域问答的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 为了解决地球科学领域专业知识的准确获取问题，开发一个能够整合专业地学知识库并支持个性化知识检索的开源AI系统，促进地学研究的开放协作。

Method: 采用检索增强生成(RAG)技术，构建GeoGPT专业地学知识库；允许用户上传个人出版物创建个性化知识库；微调嵌入模型(GeoEmbedding)和排序模型(GeoReranker)来优化检索质量和领域对齐。

Result: 系统能够生成准确、上下文相关的地学答案，检索质量和领域对齐显著提升，提供了精确可靠的专业输出。

Conclusion: GeoGPT通过开源核心RAG组件，体现了对开放科学的承诺，为全球地学科学家、研究人员和专业人士提供了强大易用的AI工具，推动了地学研究的协作和透明发展。
...
### [177] [Demonstrating Narrative Pattern Discovery from Biomedical Literature](https://arxiv.org/abs/2509.09687)
*Hermann Kroll,Pascal Sackhoff,Bill Matthias Thang,Christin Katharina Kreutz,Wolf-Tilo Balke*

Main category: cs.IR

TL;DR: 本文介绍了一种名为叙事模式挖掘的新搜索功能，允许用户在数字图书馆中探索上下文相关的实体和实体交互。


<details>
  <summary>Details</summary>
Motivation: 数字图书馆需要为用户提供有效的访问路径，PubPharm作为德国药学的专业信息服务，需要开发新的方法来探索生物医学文档集合中的实体关系。

Method: 开发了叙事模式挖掘功能，通过访谈五位领域专家来验证原型的实用性。

Result: 通过专家访谈验证了该搜索功能的有用性。

Conclusion: 叙事模式挖掘是一种有前景的新搜索功能，能够有效支持用户在数字图书馆中探索实体关系和交互。
...
### [178] [AI-Powered Assistant for Long-Term Access to RHIC Knowledge](https://arxiv.org/abs/2509.09688)
*Mohammad Atif,Vincent Garonne,Eric Lancon,Jerome Lauret,Alexandr Prozorov,Michal Vranovsky*

Main category: cs.IR

TL;DR: RHIC数据与分析保存计划开发了基于大语言模型的AI助手系统，通过检索增强生成技术提供自然语言访问文档、工作流和软件的能力，旨在支持科学数据的可重现性、教育和未来发现。


<details>
  <summary>Details</summary>
Motivation: 随着RHIC运行25年结束，保存其海量数据（约1EB）和嵌入的科学知识成为关键优先事项，需要确保这些宝贵科学遗产的长期可访问性和可用性。

Method: 基于大语言模型，采用检索增强生成（RAG）和模型上下文协议（MCP），索引RHIC实验的结构化和非结构化内容，实现领域适应的交互。

Result: 成功部署了AI助手系统，报告了计算性能、多实验集成进展，以及为可持续和可解释的长期AI访问设计的架构特性。

Conclusion: 现代AI/ML工具能够显著提升科学遗产数据的可用性和可发现性，为大型科学设施的长期数据保存和知识传承提供了有效解决方案。
...
### [179] [Personas within Parameters: Fine-Tuning Small Language Models with Low-Rank Adapters to Mimic User Behaviors](https://arxiv.org/abs/2509.09689)
*Himanshu Thakur,Eshani Agrawal,Smruthi Mukund*

Main category: cs.IR

TL;DR: 使用冻结的大语言模型提取鲁棒的文本用户表示，然后通过微调的小语言模型来模拟用户行为，在可扩展性和性能之间取得平衡


<details>
  <summary>Details</summary>
Motivation: 解决用户行为模拟的长期挑战，特别是如何有效处理大规模表格用户-物品交互数据、克服预训练模型的归纳偏差，以及为百万级用户实现规模化

Method: 提出使用冻结的LLM提取文本用户表示，然后用微调的SLMs构建成本效益高的用户代理，并训练多个低秩适配器用于用户群体或人物角色

Result: 实验提供了有力的经验证据，表明该方法开发出的用户代理能够弥合推荐系统离线指标与实际性能之间的差距
...
### [180] [Powering Job Search at Scale: LLM-Enhanced Query Understanding in Job Matching Systems](https://arxiv.org/abs/2509.09690)
...
TL;DR: 提出了一个基于大语言模型的统一查询理解框架，用于替代传统的多任务NER模型，解决查询理解中的碎片化架构问题，提高推荐准确性和系统可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统查询理解方法依赖多个任务特定的命名实体识别模型，这种碎片化架构脆弱、维护成本高，且难以适应不断变化的分类体系和语言模式。

Method: 使用大语言模型构建统一查询理解框架，联合建模用户查询和上下文信号（如个人资料属性），生成结构化解释来驱动更准确和个性化的推荐。

Result: 在线A/B测试中提高了相关性质量，同时显著降低了系统复杂性和运营开销。
...
### [181] [Wave-Based Semantic Memory with Resonance-Based Retrieval: A Phase-Aware Alternative to Vector Embedding Stores](https://arxiv.org/abs/2509.09691)
*Aleksandr Listopad*

Main category: cs.IR

TL;DR: 提出基于波的语义记忆框架，用波模式表示知识并通过共振干扰进行检索，相比传统向量方法能更好地保留振幅和相位信息，实现更强的语义区分能力
...
Method: 将知识建模为波模式ψ(x)=A(x)e^{iφ(x)}，通过基于共振的干扰进行检索，保留振幅和相位信息
...
### [182] [A Research Vision for Web Search on Emerging Topics](https://arxiv.org/abs/2509.10212)
*Alisa Rieger,Stefan Dietze,Ran Yu*

Main category: cs.IR

TL;DR: 本文提出了一个研究愿景，旨在开发支持用户在网络上搜索新兴主题信息时有效获取知识、了解主题动态性并形成负责任观点的搜索系统和界面。


<details>
  <summary>Details</summary>
Motivation: 新兴主题的信息通常稀疏、动态演变，质量参差不齐且容易受到操纵、错误信息和偏见的影响，当前搜索系统难以有效支持用户在这些情况下的知识获取和观点形成。

Method: 提出了三个总体研究问题：理解现状、确定符合愿景的系统需求、以及构建这些系统。通过文献综述指出了解决这些问题的方法路径。

Result: 提出了一个完整的研究框架和愿景，但没有具体的实验结果，因为这是一篇愿景性论文。

Conclusion: 开发支持新兴主题搜索的系统面临诸多挑战，但通过系统性的研究问题和方法路径，可以朝着这一愿景努力，帮助用户在信息不确定的环境中形成负责任的观点。
...
### [183] [Model-agnostic post-hoc explainability for recommender systems](https://arxiv.org/abs/2509.10245)
*Irina Arévalo,Jose L Salmeron*

Main category: cs.IR

TL;DR: 该研究提出了一种基于删除诊断的推荐系统分析方法，通过比较完整模型与排除特定用户/物品后模型的性能差异，来量化观察值对推荐系统的影响，从而提高模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 推荐系统虽然通过复杂的特征嵌入和深度学习算法提升了推荐效果，但往往降低了系统的可解释性和透明度。为了解决这个问题，需要开发一种能够量化单个用户或物品对推荐系统影响的方法。

Method: 采用删除诊断方法，通过训练一个完整模型和一个排除特定用户或物品的相似模型，比较两者的性能差异来量化该观察值对推荐系统的影响。该方法具有模型无关性，在神经协同过滤(NCF)和奇异值分解(SVD)两种推荐技术上进行了验证。

Result: 在MovieLens和Amazon Reviews数据集上的实验表明，该方法能够有效揭示模型行为，提供对推荐系统影响的量化分析，并且在不同推荐范式下都具有良好的通用性。

Conclusion: 删除诊断方法为推荐系统提供了一种有效的可解释性分析工具，能够量化单个用户或物品对系统的影响，有助于提高推荐系统的透明度和可信度，适用于不同类型的推荐算法。
...
### [184] [Diversified recommendations of cultural activities with personalized determinantal point processes](https://arxiv.org/abs/2509.10392)
*Carole Ibrahim,Hiba Bederina,Daniel Cuesta,Laurent Montier,Cyrille Delabre,Jill-Jênn Vie*

Main category: cs.IR

TL;DR: 使用个性化DPP采样方法在推荐系统中平衡相关性和多样性，通过线下线上指标评估效果，并开源代码


<details>
  <summary>Details</summary>
Motivation: 在优化用户参与度的同时，有效提升推荐多样性而不影响核心业务指标是行业挑战，旨在扩大受众的文化实践

Method: 基于质量-多样性分解的个性化Determinantal Point Processes (DPPs)采样方法，给予用户偏好更多权重

Result: 评估了相关性与多样性之间的权衡关系，提供了生产环境应用的实践洞察

Conclusion: 个性化DPP方法能够有效平衡推荐系统的相关性和多样性需求，适合在生产环境中应用
...
### [185] [RecoWorld: Building Simulated Environments for Agentic Recommender Systems](https://arxiv.org/abs/2509.10397)
...
TL;DR: RecoWorld是一个为智能推荐系统设计的模拟环境框架，通过双视图架构让模拟用户和推荐代理进行多轮交互，利用LLM的推理能力优化用户留存率。


<details>
  <summary>Details</summary>
Motivation: 为智能推荐系统提供安全的训练环境，避免在真实用户上试错，同时探索用户与推荐系统协同优化信息流的新交互范式。

Method: 采用双视图架构：模拟用户审查推荐项目并更新心态，当检测到用户可能流失时生成反思指令；推荐代理整合用户指令和推理痕迹来调整推荐策略，形成动态反馈循环。支持多种内容表示（文本、多模态、语义ID）和多轮强化学习。

Result: 构建了一个支持多代理模拟的环境框架，能够模拟目标用户群体的响应，为推荐系统提供了安全的迭代优化空间。

Conclusion: RecoWorld是向用户与代理协同塑造个性化信息流的推荐系统迈出的重要第一步，开创了"用户指导、推荐响应"的新交互范式，共同优化用户留存和参与度。
...
### [186] [MatSKRAFT: A framework for large-scale materials knowledge extraction from scientific tables](https://arxiv.org/abs/2509.10448)
*Kausik Hira,Mohd Zaki,Mausam,N. M. Anoop Krishnan*

Main category: cs.IR

TL;DR: MatSKRAFT是一个从表格数据中自动提取和整合材料科学知识的计算框架，通过图神经网络处理表格数据，在性能和速度上显著优于大型语言模型
...
Result: 在性能提取上达到88.68的F1分数，在成分提取上达到71.35的F1分数，处理速度比现有模型快19-496倍，构建了包含超过535,000个条目的综合数据库

Conclusion: 该方法能够揭示先前被忽视的材料及其独特属性组合，支持数据驱动的成分-属性关系发现，为材料科学发现奠定基础
...
### [187] [Human-AI Collaboration Increases Efficiency in Regulatory Writing](https://arxiv.org/abs/2509.09738)
*Umut Eser,Yael Gozin,L. Jay Stallons,Ari Caroline,Martin Preusse,Brandon Rice,Scott Wright,Andrew Robertson*

Main category: cs.AI

TL;DR: AutoIND LLM平台可将IND申请文件的初稿起草时间减少约97%，从约100小时缩短至3-4小时，质量评分达69-78%，无关键监管错误，但仍需专家进一步完善。
...
Method: 通过AutoIND LLM平台生成IND非临床书面总结，记录起草时间并与人工起草时间对比，由盲法监管写作评估员使用7个预设类别进行质量评估。

Result: 起草时间减少97%（从~100小时降至3.7小时和2.6小时），质量评分分别为69.6%和77.9%，无关键监管错误，但在重点强调、简洁性和清晰度方面存在不足。

Conclusion: AutoIND能显著加速IND起草过程，但专家监管作者仍需将输出完善至提交就绪质量，发现的系统性缺陷为模型改进提供了路线图。
...
### [188] [Executable Ontologies: Synthesizing Event Semantics with Dataflow Architecture](https://arxiv.org/abs/2509.09775)
*Aleksandr Boldachev*
...
<details>
  <summary>Details</summary>
Motivation: 传统业务流程管理(BPM)系统和面向对象语义技术在处理复杂动态系统时存在局限性，需要一种能够将事件语义与数据流架构整合的新方法。

Method: 提出了boldsea架构和BSL语义语言，包括其BNF语法，通过boldsea-engine直接解释语义模型作为可执行算法，无需编译。

Result: 实现了运行时修改事件模型、确保时间透明度，并在统一语义框架内无缝融合数据和业务逻辑。

Conclusion: Boldsea架构通过可执行本体成功解决了传统系统的限制，为复杂动态系统建模提供了有效的语义事件方法。
...
### [189] [How well can LLMs provide planning feedback in grounded environments?](https://arxiv.org/abs/2509.09790)
*Yuxuan Li,Victor Zhong*

Main category: cs.AI

TL;DR: 本文评估了大型语言模型和视觉语言模型在不同环境中提供规划反馈的能力，发现基础模型能提供高质量多样化反馈，但复杂动态或连续空间会降低反馈质量。


<details>
  <summary>Details</summary>
Motivation: 传统基于环境的规划学习需要精心设计的奖励函数或高质量标注演示，而预训练基础模型可能捕获有助于规划的背景知识，减少对奖励设计和演示的需求。

Method: 评估LLMs和VLMs在符号、语言和连续控制环境中提供反馈的能力，包括二元反馈、偏好反馈、动作建议、目标建议和增量动作反馈等类型，并考察上下文学习、思维链和环境动态访问等推理方法。

Result: 基础模型能在多个领域提供高质量多样化反馈；更大和具备推理能力的模型反馈更准确、偏差更小，且能从增强推理方法中获益更多；但在复杂动态或连续状态动作空间中反馈质量会下降。

Conclusion: 预训练基础模型是规划任务中有价值的反馈来源，模型规模和推理能力对反馈质量有重要影响，但在复杂环境中的应用仍面临挑战。
...
### [190] [A Modular and Multimodal Generative AI Framework for Urban Building Energy Data: Generating Synthetic Homes](https://arxiv.org/abs/2509.09794)
*Jackson Eshbaugh,Chetan Tiwari,Jorge Silveyra*

Main category: cs.AI

TL;DR: 提出基于生成式AI的模块化多模态框架，从公开的住宅信息和图像生成能源建模所需数据，解决数据获取难题


<details>
  <summary>Details</summary>
Motivation: 计算模型需要大量数据，但许多数据难以获取、昂贵或涉及隐私问题，阻碍了能源建模研究的可及性和可重复性

Method: 开发模块化多模态框架，利用生成式人工智能从公开可获取的住宅信息和图像中合成所需数据，并提供完整的处理流程

Result: 实验表明该框架能生成真实、标注良好的数据，避免了生成模型的常见问题，降低了数据获取成本

Conclusion: 该框架通过减少对昂贵或受限数据源的依赖，为更易获取和可重复的研究铺平了道路
...
### [191] [Towards a Common Framework for Autoformalization](https://arxiv.org/abs/2509.09810)
*Agnieszka Mensfelt,David Tena Cucala,Santiago Franco,Angeliki Koutsoukou-Argyraki,Vince Trencsenyi,Kostas Stathis*

Main category: cs.AI

TL;DR: 本文综述了自动形式化领域的研究现状，提出了统一框架来促进不同领域间的交叉融合，以推动下一代AI系统的发展。


<details>
  <summary>Details</summary>
Motivation: 自动形式化领域虽然发展迅速，但由于不同研究领域独立发展，缺乏共享的方法论、基准和理论框架，限制了该领域的整体进步。

Method: 通过回顾显性或隐性的自动形式化实例，分析现有研究成果，提出统一的框架来整合不同领域的方法和技术。

Result: 识别了自动形式化在数学定理证明、推理、规划和知识表示等多个领域的应用，揭示了这些领域之间的相似性和潜在的合作机会。

Conclusion: 建立统一的自动形式化框架可以促进不同研究领域的交叉融合，加速下一代AI系统的开发进程。
...
### [192] [Towards an AI-based knowledge assistant for goat farmers based on Retrieval-Augmented Generation](https://arxiv.org/abs/2509.09848)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在畜牧业应用受限，主要由于知识源的可用性、多样性和复杂性不足，特别是在山羊养殖健康管理领域缺乏专门的知识支持系统。
...
Conclusion: 该系统展现了在实际山羊养殖应用中的鲁棒性和可靠性，结构化知识融合的模块化设计有效提升了模型性能，检索覆盖范围和上下文整合仍有改进空间。
...
### [193] [LLMs as Agentic Cooperative Players in Multiplayer UNO](https://arxiv.org/abs/2509.09867)
*Yago Romano Matinez,Jesse Roberts*

Main category: cs.AI

TL;DR: LLM在UNO游戏中作为助手代理的表现测试，发现虽然模型能超越随机基准，但很少能有效帮助其他玩家获胜


<details>
  <summary>Details</summary>
Motivation: 测试基于大语言模型的代理是否能作为主动参与者真正帮助用户实现目标，特别是在协作游戏场景中

Method: 构建工具让仅解码器LLM在RLCard游戏环境中作为代理参与UNO游戏，接收完整游戏状态信息，使用两种不同的提示策略进行响应，评估从1B到70B参数的不同规模模型

Result: 所有模型在玩UNO时都能成功超越随机基准，但很少有模型能够显著帮助其他玩家获胜

Conclusion: 虽然LLM在单独游戏任务中表现良好，但在协作助人任务中的能力仍然有限，模型规模对性能有影响但并非决定性因素
...
### [194] [The (R)evolution of Scientific Workflows in the Agentic AI Era: Towards Autonomous Science](https://arxiv.org/abs/2509.09915)
*Woong Shin,Renan Souza,Daniel Rosendo,Frédéric Suter,Feiyi Wang,Prasanna Balaprakash,Rafael Ferreira da Silva*

Main category: cs.AI

TL;DR: 论文提出了一个概念框架，将科学工作流从静态、单一的系统演进为智能化的群体系统，最终实现完全自主的分布式科学实验室，旨在加速科学发现100倍。
...
Method: 提出了一个概念框架，工作流沿着两个维度演进：智能性（从静态到智能）和组合性（从单一到群体），并提供了架构蓝图来指导从当前工作流管理系统到完全自主分布式科学实验室的演进路径。

Result: 提出了一个系统性的演进框架和架构设计，为社区提供了向自主科学发展的重要指导，展示了实现100倍发现加速和变革性科学工作流的潜力。

Conclusion: 该框架为利用AI智能体技术实现科学发现的革命性加速提供了清晰的演进路径和实现蓝图，有望从根本上改变科学研究的工作方式。
...
### [195] [A Markovian Framing of WaveFunctionCollapse for Procedurally Generating Aesthetically Complex Environments](https://arxiv.org/abs/2509.09919)
...
Method: 将WaveFunctionCollapse重新表述为马尔可夫决策过程，使外部优化算法专注于目标最大化，利用WFC传播机制强制执行约束满足

Result: 在多个不同难度的领域中，联合优化方法随着任务复杂度增加而表现不佳，且始终不如在WFC-MDP上的优化表现

Conclusion: 将局部约束满足与全局目标优化解耦的方法具有明显优势，WFC-MDP框架能更有效地处理程序化内容生成中的复杂约束和优化问题
...
### [196] [Evaluation of Black-Box XAI Approaches for Predictors of Values of Boolean Formulae](https://arxiv.org/abs/2509.09982)
*Stav Armoni-Friedmann,Hana Chockler,David A. Kelly*

Main category: cs.AI

TL;DR: 提出基于实际因果关系的变量重要性度量方法，评估XAI工具性能，并开发新的B-ReX工具在布尔函数预测任务上表现优异
...
Method: 提出基于实际因果关系的正式变量重要性度量方法，评估现有XAI工具，并开发基于ReX的新工具B-ReX

Result: B-ReX在大型基准测试中优于其他黑盒XAI工具，在随机10值布尔公式上获得0.072±0.012的Jensen-Shannon散度

Conclusion: 基于实际因果关系的度量方法为XAI评估提供了更精确的标准，B-ReX工具在布尔函数预测任务中表现出优越性能
...
### [197] [GAMA: A General Anonymizing Multi-Agent System for Privacy Preservation Enhanced by Domain Rules and Disproof Method](https://arxiv.org/abs/2509.10018)
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在多智能体系统中的广泛应用，当任务涉及隐私数据时，需要在不牺牲性能的前提下实现隐私保护，但现有方法难以平衡隐私保护和语义完整性。

Method: 提出GAMA系统，划分私有和公共空间，在私有空间处理敏感数据，公共空间使用匿名化数据。采用基于领域规则的知识增强(DRKE)和基于反证的逻辑增强(DLE)来减少匿名化带来的语义损失。

Result: 在两个公开问答数据集(Trivia Creative Writing和Logic Grid Puzzle)上表现优于最先进模型。在专门设计的隐私保护数据集(Knowledge Privacy Preservation和Logic Privacy Preservation)上展现出卓越的隐私保护能力。

Conclusion: GAMA系统在保持任务处理性能的同时，有效实现了隐私保护，为隐私敏感的多智能体系统应用提供了可行的解决方案。
...
### [198] [XAgents: A Unified Framework for Multi-Agent Cooperation via IF-THEN Rules and Multipolar Task Processing Graph](https://arxiv.org/abs/2509.10054)
*Hailong Yang,Mingxian Gu,Jianqi Wang,Guanjin Wang,Zhaohong Deng*

Main category: cs.AI

TL;DR: XAgents是一个基于多极任务处理图和IF-THEN规则的多智能体协作框架，用于解决复杂任务规划中的不确定性，在知识型和逻辑型问答任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然提升了多智能体系统的能力，但在处理高度复杂且具有不确定性的任务时，仍然存在任务规划效果不佳的问题，导致误导性或错误输出阻碍任务执行。
...
### [199] [AI Harmonics: a human-centric and harms severity-adaptive AI risk assessment framework](https://arxiv.org/abs/2509.10104)
*Sofia Vei,Paolo Giudici,Pavlos Sermpezis,Athena Vakali,Adelaide Emma Bernardelli*

Main category: cs.AI

TL;DR: 提出AI Harmonics框架，使用基于序数严重性数据的新型AI危害评估指标(AIH)，通过实证事件数据分析识别政治和物理危害为最紧迫风险，为政策制定者提供针对性缓解策略


<details>
  <summary>Details</summary>
Motivation: 现有AI风险评估模型过于关注内部合规性，忽视了多元利益相关者视角和真实世界后果，需要转向以人为本、危害严重性自适应的评估方法

Method: 开发AI Harmonics框架，包含新型AI危害评估指标(AIH)，利用序数严重性数据捕捉相对影响而无需精确数值估计，结合稳健的通用方法和数据驱动的利益相关者感知框架

Result: 实验验证政治危害和物理危害具有最高集中度，政治危害侵蚀公众信任，物理危害造成严重甚至危及生命的风险，AI Harmonics能一致识别不均匀的危害分布

Conclusion: AI Harmonics为政策制定者和组织提供了有效的危害缓解目标定位工具，强调了基于实证数据的人类中心主义AI风险评估的重要性
...
### [200] [Virtual Agent Economies](https://arxiv.org/abs/2509.10147)
*Nenad Tomasev,Matija Franklin,Joel Z. Leibo,Julian Jacobs,William A. Cunningham,Iason Gabriel,Simon Osindero*

Main category: cs.AI

TL;DR: 论文提出了"沙盒经济"框架来分析AI自主代理形成的新经济层，探讨了其自发涌现特性、与人类经济的渗透性，以及通过拍卖机制、使命经济设计等技术手段来引导这一新兴经济体系安全发展的方法。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理的快速普及，正在形成一个超越人类直接监管尺度和速度的新经济层，这既带来了前所未有的协调机会，也带来了系统性经济风险和加剧不平等的挑战，需要建立分析框架和设计方法来引导其发展。

Method: 提出"沙盒经济"分析框架，从起源（涌现性vs意向性）和分离程度（渗透性vs非渗透性）两个维度进行表征；探讨拍卖机制实现公平资源分配、AI"使命经济"设计实现集体目标协调，以及确保信任、安全和问责的社会技术基础设施。

Result: 分析表明当前发展轨迹指向一个自发涌现的、高度渗透的AI代理经济体系；提出了可引导的AI代理市场设计选择，包括拍卖机制、使命经济架构等技术解决方案。

Conclusion: 需要主动设计可引导的代理市场，通过技术手段确保即将到来的技术转变与人类长期集体繁荣相一致，避免系统性风险和不平等加剧。
...
### [201] [Online Robust Planning under Model Uncertainty: A Sample-Based Approach](https://arxiv.org/abs/2509.10162)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于采样的在线规划方法（如Sparse Sampling和MCTS）在处理从有限数据学习的生成模型时，会因近似误差导致性能下降或不安全行为。现有鲁棒MDP方法计算量大，不适合实时应用。

Method: RSS算法通过结合Sample Average Approximation (SAA)的高效性和理论特性，计算鲁棒价值函数而非名义价值函数，实现了在在线设置中可处理的鲁棒策略计算。

Result: RSS适用于无限或连续状态空间，其样本和计算复杂度与状态空间大小无关。理论分析提供了性能保证，实证表明RSS在不确定动态环境中优于标准Sparse Sampling。

Conclusion: RSS是第一个具有理论保证的在线鲁棒规划算法，能够有效处理模型不确定性，适用于大规模动态环境的实时决策需求。
...
### [202] [Towards Fully Automated Molecular Simulations: Multi-Agent Framework for Simulation Setup and Force Field Extraction](https://arxiv.org/abs/2509.10210)
*Marko Petković,Vlado Menkovski,Sofía Calero*

Main category: cs.AI

TL;DR: 提出了一个基于LLM的多智能体框架，用于自动化多孔材料表征，包括文献提取力场参数和自动设置RASPA模拟
...
### [203] [Compartmentalised Agentic Reasoning for Clinical NLI](https://arxiv.org/abs/2509.10222)
...
<details>
  <summary>Details</summary>
Motivation: 传统假设认为扩大数据和参数规模会带来更结构化、可泛化的内部表示，但本文质疑这一假设在临床NLI任务中的有效性，特别是在推理过程不明确时LLMs倾向于使用启发式方法

Method: 提出CARENLI框架，将前提-陈述对路由到特定推理家族的求解器，并通过规划器、验证器和精炼器强制执行可审计程序，分解为因果归因、组合基础、认知验证和风险状态抽象四个推理家族

Result: 在四个LLM上，CARENLI将保真度提高了多达42个百分点，在因果归因中达到98.0%，在风险状态抽象中达到81.2%。验证器以接近天花板的可靠性标记违规，精炼器纠正了大量认知错误
...
### [204] [Investigating Language Model Capabilities to Represent and Process Formal Knowledge: A Preliminary Study to Assist Ontology Engineering](https://arxiv.org/abs/2509.10249)
*Hanna Abi Akl*
...
Result: 研究发现使用更紧凑的逻辑语言替代自然语言可以在保持推理任务强性能的同时提升效率

Conclusion: 该结果为小语言模型在ontology工程中的角色细化提供了基础，证明了形式化方法在提升语言模型推理能力方面的潜力
...
### [205] [The Morality of Probability: How Implicit Moral Biases in LLMs May Shape the Future of Human-AI Symbiosis](https://arxiv.org/abs/2509.10297)
*Eoin O'Doherty,Nicole Weinrauch,Andrew Talone,Uri Klempner,Xiaoyuan Yi,Xing Xie,Yi Zeng*

Main category: cs.AI

TL;DR: 该研究通过量化实验分析6个大型语言模型在18个道德困境中的表现，发现所有模型都一致偏好关怀和美德价值观，而惩罚自由主义选择。具备推理能力的模型对情境更敏感，提供更丰富的解释。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能快速发展，需要解决如何使机器决策与人类道德价值观对齐的紧迫问题，探索AI系统如何优先考虑道德结果以及人类-AI共生的前景。

Method: 对6个大型语言模型进行定量实验，在代表5种道德框架的18个困境中对结果进行排序和评分，分析模型架构、文化起源和可解释性对道德偏好的影响。

Result: 发现所有模型存在显著一致的价值偏见：关怀和美德价值观结果被评为最道德，而自由主义选择被一致惩罚。推理模型对情境更敏感且提供更丰富解释，非推理模型产生更统一但不透明的判断。
...
### [206] [State Algebra for Propositional Logic](https://arxiv.org/abs/2509.10326)
*Dmitry Lesnik,Tobias Schäfer*

Main category: cs.AI

TL;DR: State Algebra是一个新颖的代数框架，用于表示和操作命题逻辑，通过三种层次化表示（集合、坐标、行分解）提供灵活性和计算能力，在牺牲规范性的同时获得更紧凑的问题表示。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够使用代数方法有效表示和操作命题逻辑的框架，为搜索算法和知识编译提供工具支持，并扩展到概率逻辑和加权模型计数领域。

Method: 构建三层表示层次结构（Set、Coordinate、Row Decomposition），通过代数引擎进行计算，采用固定变量顺序来获得唯一规范形式，平衡规范性和灵活性。

Result: 框架展示了在放弃保证规范性的情况下可以获得更大的灵活性，能够为某些问题类提供更紧凑的表示，并支持搜索和知识编译算法的表达。

Conclusion: State Algebra提供了一个强大而灵活的代数框架，能够有效处理命题逻辑，具有良好的扩展性，为概率逻辑和加权模型计数等应用奠定了基础。
...
### [207] [Abduct, Act, Predict: Scaffolding Causal Inference for Automated Failure Attribution in Multi-Agent Systems](https://arxiv.org/abs/2509.10401)
*Alva West,Yixuan Weng,Minjun Zhu,Zhen Lin,Yue Zhang*

Main category: cs.AI

TL;DR: A2P Scaffolding框架通过结构化因果推理方法，将多智能体系统中的故障归因从模式识别任务转变为因果推断任务，显著提高了步骤级准确率（2.85倍提升）。
...
Method: 提出Abduct-Act-Predict（A2P）Scaffolding框架，通过三步推理过程：溯因推理（推断行动背后的隐藏原因）、行动（定义最小纠正干预）、预测（模拟后续轨迹验证干预效果），在单次推理过程中实现结构化因果分析。
...
### [208] [Mutual Information Tracks Policy Coherence in Reinforcement Learning](https://arxiv.org/abs/2509.10423)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的RL智能体面临传感器故障、驱动器磨损和环境变化等问题，但缺乏内在机制来检测和诊断这些故障，需要开发有效的故障诊断方法。

Method: 使用信息论框架分析状态-动作互信息模式，通过控制扰动实验验证信息指标对系统故障的差异诊断能力，包括状态熵、状态-动作互信息和状态-动作-下一状态联合互信息等指标。

Result: 成功学习表现出特征性信息特征：状态-动作互信息从0.84比特增长到2.83比特（增长238%）；状态-动作-下一状态联合互信息呈倒U型曲线；信息指标能够区分传感器故障（广泛崩溃）和驱动器故障（选择性破坏动作-结果可预测性）。

Conclusion: 信息模式既是学习的特征，也是系统健康的诊断工具，为基于信息论原理的自适应RL系统提供了自主故障检测和政策调整的基础，无需架构修改或性能下降即可实现精确故障定位。
...
### [209] [SWE-Effi: Re-Evaluating Software AI Agent System Effectiveness Under Resource Constraints](https://arxiv.org/abs/2509.09853)
*Zhiyu Fan,Kirill Vasilevski,Dayi Lin,Boyuan Chen,Yihao Chen,Zhiqing Zhong,Jie M. Zhang,Pinjia He,Ahmed E. Hassan*

Main category: cs.SE

TL;DR: SWE-Effi：针对AI软件工程系统的新评估指标，综合考虑准确性和资源消耗（token和时间），重新评估现有AI系统在问题解决任务中的整体效能。


<details>
  <summary>Details</summary>
Motivation: 现有AI软件工程排行榜（如SWE-bench）仅关注解决方案准确性，忽视了资源受限环境下的成本效益问题。任何AI系统不仅需要正确，还必须具备成本效益。

Method: 提出SWE-Effi多维指标，在SWE-bench基准的子集上重新评估流行的AI问题解决系统，定义效能为结果准确性与资源消耗之间的平衡。

Result: 发现系统效能不仅取决于框架本身，还取决于与基础模型的集成程度；识别出"token雪球"效应和"昂贵失败"模式；观察到token预算和时间预算下的效能权衡。

Conclusion: 需要综合考虑准确性和资源消耗的多维评估指标；模型集成质量对效能至关重要；资源效率问题限制了实际部署和RL训练成本；需要在不同预算约束下优化系统设计。
...
### [210] [From Hugging Face to GitHub: Tracing License Drift in the Open-Source AI Ecosystem](https://arxiv.org/abs/2509.09873)
*James Jewitt,Hao Li,Bram Adams,Gopi Krishnan Rajbahadur,Ahmed E. Hassan*

Main category: cs.SE

TL;DR: 对Hugging Face平台上的开源AI模型、数据集及其下游软件应用进行首次端到端许可证审计，发现35.5%的模型到应用转换中存在系统性许可证违规，通过重新许可消除限制性条款。开发了可扩展规则引擎，可解决86.4%的软件应用许可证冲突。


<details>
  <summary>Details</summary>
Motivation: 开源AI生态系统中隐藏的许可证冲突存在严重的法律和伦理风险，但缺乏数据驱动的理解，需要了解这些冲突的发生频率、来源和受影响社区。

Method: 对Hugging Face上的36.4万个数据集、160万个模型以及14万个GitHub项目进行端到端许可证审计，开发可扩展规则引擎编码近200个SPDX和模型特定条款用于检测许可证冲突。

Result: 发现35.5%的模型到应用转换通过重新许可消除限制性条款，原型引擎可解决86.4%的许可证冲突。发布了数据集和原型引擎以支持未来研究。

Conclusion: 许可证合规性是开源AI中的关键治理挑战，研究提供了实现自动化、AI感知的大规模合规性所需的数据和工具。
...
### [211] [SLD-Spec: Enhancement LLM-assisted Specification Generation for Complex Loop Functions via Program Slicing and Logical Deletion](https://arxiv.org/abs/2509.09917)
...
### [212] [WALL: A Web Application for Automated Quality Assurance using Large Language Models](https://arxiv.org/abs/2509.09918)
*Seyed Moein Abtahi,Akramul Azim*

Main category: cs.SE

TL;DR: WALL是一个集成SonarQube和大型语言模型的Web应用，通过三个模块实现软件问题的自动检测、修复和评估，在563个文件7599个问题上验证了有效性
...
Result: 在563个文件7599个问题上实验证明，采用经济型和高级LLMs混合方法能显著降低成本并提高修订率，有效减少人工工作量同时保持高质量修订
...
### [213] [Toward Green Code: Prompting Small Language Models for Energy-Efficient Code Generation](https://arxiv.org/abs/2509.09947)
*Humza Ashraf,Syed Muhammad Danish,Zeeshan Sattar*

Main category: cs.SE

TL;DR: 研究表明，针对特定的小型语言模型（SLMs），链式思维（CoT）提示工程可以显著提高代码生成的能效，但效果因模型而异，需要精心设计的提示策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在软件开发中的高能耗和碳足迹问题日益受到关注，小型语言模型作为更可持续的替代方案，需要通过提示工程来提升其能效表现。

Method: 评估4个开源SLMs在150个LeetCode Python问题上的表现，使用4种提示策略（角色提示、零样本、少样本、链式思维），测量运行时、内存使用和能耗，并与人工编写基准对比。

Result: CoT提示为Qwen2.5-Coder和StableCode-3B带来持续节能效果，但CodeLlama-7B和Phi-3-Mini-4K在任何提示策略下都无法超越基准。

Conclusion: 提示工程的效益具有模型依赖性，精心设计的提示可以引导SLMs实现更环保的软件开发，但需要针对特定模型进行优化。
...
### [214] [Development of Automated Software Design Document Review Methods Using Large Language Models](https://arxiv.org/abs/2509.09975)
...
Method: 分析设计文档评审方法并组织11个评审视角，针对可评审视角开发新技术使LLM能够理解包含表格数据的复杂设计文档，使用GPT进行实验评估设计项和描述在不同设计文档中的一致性

Result: 实验结果表明LLM能够在评审过程中有效识别软件设计文档中的不一致性

Conclusion: LLM可以替代人工完成软件设计文档的某些评审任务，特别是在识别设计不一致性方面具有实用价值
...
### [215] [Sustaining Research Software: A Fitness Function Approach](https://arxiv.org/abs/2509.10085)
*Philipp Zech,Irdin Pekaric*

Main category: cs.SE

TL;DR: 提出基于进化架构中适应度函数概念的新方法，通过自动化持续评估指标来提升研究软件的长期可持续性和FAIR原则（可发现、可访问、可互操作、可重用）


<details>
  <summary>Details</summary>
Motivation: 研究软件通常面临可维护性差、缺乏适应性、最终过时等可持续性挑战，需要系统化方法来确保其长期科学价值

Method: 定义针对研究软件FAIR原则的适应度函数集，作为自动化持续评估指标，集成到开发生命周期中，促进模块化设计、全面文档、版本控制和与不断发展的技术生态系统的兼容性

Result: 案例研究和实验结果表明该方法能有效增强研究软件的长期FAIR特性，弥合短期项目开发与持久科学影响之间的差距

Conclusion: 通过适应度函数方法可以培养研究社区的可持续性文化，为研究软件提供主动保护措施，确保其长期科学价值
...
### [216] [Generating Energy-Efficient Code via Large-Language Models -- Where are we now?](https://arxiv.org/abs/2509.10099)
*Radu Apsan,Vincenzo Stoico,Michel Albonico,Rudra Dhar,Karthik Vaidhyanathan,Ivano Malavolta*

Main category: cs.SE

TL;DR: LLM生成的Python代码在能源效率上不如人类专家编写的代码，绿色软件专家编写的代码在所有硬件平台上都比LLM生成代码节能17-30%


<details>
  <summary>Details</summary>
Motivation: 评估LLM生成的Python代码与人类编写代码在能源效率方面的差异，特别是在绿色软件开发背景下

Method: 使用6个主流LLM和4种提示技术生成363个解决方案，在服务器、PC和树莓派三种硬件平台上测量能源消耗，总计约881小时

Result: 人类解决方案在服务器上节能16%，树莓派上节能3%，但LLM在PC上比人类开发者节能25%。绿色软件专家的代码在所有平台上都比所有LLM节能17-30%

Conclusion: 尽管LLM代码生成能力良好，但目前仍需要人类专家来开发高能效的Python代码
...
### [217] [Stencil-Lifting: Hierarchical Recursive Lifting System for Extracting Summary of Stencil Kernel in Legacy Codes](https://arxiv.org/abs/2509.10236)
...
<details>
  <summary>Details</summary>
Motivation: 针对现有验证提升系统在效率上的瓶颈，需要开发一种能够高效转换遗留代码中低级模板内核到DSL实现的方法，以弥合传统优化技术与现代DSL范式之间的差距。

Method: 采用分层递归提升理论，使用不变子图表示模板内核，每个顶点关联基于谓词的摘要来编码计算语义；开发分层递归提升算法，通过收敛递归过程保证终止性，避免基于搜索的合成低效问题。

Result: 在两个不同测试套件的多样化模板基准和四个实际应用上评估，相比最先进的验证提升系统STNG和Dexter分别实现31.62倍和5.8倍的速度提升，同时保持完全语义等价。

Conclusion: Stencil-Lifting显著提高了低级模板内核到DSL实现的转换效率，有效连接了传统优化技术与现代DSL范式，为遗留代码现代化提供了高效解决方案。
...
### [218] [Targeted Test Selection Approach in Continuous Integration](https://arxiv.org/abs/2509.10279)
*Pavel Plyusnin,Aleksey Antonov,Vasilii Ermakov,Aleksandr Khaybriev,Margarita Kikot,Ilseyar Alimova,Stanislav Moiseev*

Main category: cs.SE

TL;DR: T-TS是一种基于机器学习的工业测试选择方法，通过将代码提交表示为变更文件的词袋，结合跨文件和预测特征，无需覆盖率映射，能选择15%的测试用例，减少5.9倍执行时间，加速5.6倍流水线，检测95%以上的测试失败。
...
Method: 提出Targeted Test Selection (T-TS)机器学习方法，使用变更文件的词袋表示代码提交，整合跨文件和额外预测特征，避免使用覆盖率映射。

Result: 在生产环境中部署后，T-TS仅选择15%的测试用例，减少5.9倍执行时间，加速5.6倍流水线，检测超过95%的测试失败。

Conclusion: T-TS是一种有效的工业测试选择解决方案，已在生产环境中验证其高效性，实现公开可用以支持进一步研究和实际应用。
...
### [219] [Developer-LLM Conversations: An Empirical Study of Interactions and Generated Code Quality](https://arxiv.org/abs/2509.10402)
*Suzhen Zhong,Ying Zou,Bram Adams*

Main category: cs.SE

TL;DR: 基于82,845个真实开发者-LLM对话的分析显示，LLM响应比开发者提示长14倍，68%为多轮对话，Python和JavaScript代码存在大量未定义变量问题，Java缺乏注释，C++缺少头文件，C#存在命名空间问题，但通过多轮交互可以改善文档质量和导入处理。


<details>
  <summary>Details</summary>
Motivation: 了解开发者如何在实际工作中与LLM进行交互，以及这些对话动态如何影响任务结果、代码质量和软件工程工作流程，填补对LLM在实际开发中应用理解的空白。

Method: 利用CodeChat数据集（包含82,845个真实开发者-LLM对话，368,506个代码片段，覆盖20多种编程语言），分析对话模式、主题分布，并在五种主要编程语言（Python、JavaScript、C++、Java、C#）中评估代码质量问题。

Result: 发现LLM响应长度中位数是开发者提示的14倍；68%的对话为多轮交互；最常见任务为网页设计（9.6%）和神经网络训练（8.7%）；不同语言存在特定代码质量问题；多轮对话可以改善某些问题（Java文档质量提升14.7%，Python导入处理改善3.7%）。

Conclusion: LLM在代码生成中存在语言特定的系统性质量问题，但通过多轮交互和明确指出错误的提示可以显著改善代码质量，为优化开发者-LLM协作提供了重要见解。
