...
TL;DR: 本文提出了一种基于脉冲神经网络（SNN）的神经形态眼动追踪模型，通过将现有高性能事件驱动眼动追踪模型中的循环和注意力模块替换为轻量级LIF层，并利用深度可分离卷积降低模型复杂度，实现了接近专用神经形态系统Retina的精度，同时大幅减少了模型规模和计算量。


<details>
  <summary>Details</summary>
Motivation: 可穿戴系统的眼动追踪需要低延迟和毫瓦级功耗，但传统的基于帧的流水线存在运动模糊、计算成本高和时间分辨率有限的问题。这些能力对于增强现实（AR）和虚拟现实（VR）等新兴技术中实现无缝和响应式交互至关重要，其中理解用户注视是沉浸感和界面设计的关键。神经形态传感器和脉冲神经网络（SNN）提供了有前景的替代方案，但现有的SNN方法要么过于专门化，要么性能不及现代ANN架构。

Method: 本文提出了高性能事件驱动眼动追踪模型的神经形态版本，将其中的循环和注意力模块替换为轻量级LIF（泄漏积分发放）层，并利用深度可分离卷积来降低模型复杂度。这种方法在保持精度的同时显著减少了模型规模和计算需求。

Result: 模型获得了3.7-4.1像素的平均误差，接近专用神经形态系统Retina（3.24像素）的精度，同时与所提出模型的最接近ANN变体相比，模型规模减少了20倍，理论计算量减少了850倍。这些高效变体预计在1kHz频率下以3.9-4.9 mW的功耗和3 ms的延迟运行。

Conclusion: 当前结果表明，高性能的事件驱动眼动追踪架构可以重新设计为SNN，在保持适合实时可穿戴部署的精度的同时，获得显著的效率提升。这为可穿戴AR/VR系统中的高效眼动追踪提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 不完全多视图数据中，不同视图存在缺失和不平衡观测，给聚类带来挑战。现有插补方法可能引入噪声和偏差，而免插补方法在严重不完整情况下缺乏跨视图互补性。需要一种更智能的选择性插补策略。

Method: ISMVC方法：1) 基于视图内相似性和跨视图一致性评估每个缺失位置的信息量，只在有足够支持时选择性插补；2) 结合变分自编码器和混合高斯先验学习聚类友好的潜在表示；3) 在分布层面进行插补，稳定后验分布聚合并显式建模插补不确定性。

Result: 在多个基准数据集上的实验表明，在更现实和具有挑战性的不平衡缺失场景下，ISMVC优于现有的基于插补和免插补方法。该方法轻量级、数据驱动且模型无关，可作为插件模块集成到现有IMC模型中。

Conclusion: ISMVC通过选择性插补策略有效解决了不完全多视图聚类问题，避免了盲目插补的噪声和免插补方法的局限性，在严重不平衡缺失场景下表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有HBox监督的OOD方法直接将GT水平框与预测RBox的最小外接矩形比较，导致尺度估计不准确。需要解决尺度预测不准和学习崩溃问题。

Method: 提出ABBSPO框架：1) 自适应边界框缩放(ABBS)：适当缩放GT水平框以优化每个预测RBox的尺寸；2) 对称先验角度(SPA)损失：利用航空目标的固有对称性进行自监督学习，解决多视图预测一致错误时的学习崩溃问题。

Result: 大量实验结果表明ABBSPO在弱监督定向目标检测中达到最先进性能，优于现有方法。

Conclusion: ABBSPO通过自适应尺度调整和对称性利用，有效解决了弱监督定向目标检测中的关键挑战，取得了显著性能提升。
...
TL;DR: SST（Show, Suggest and Tell）提出了一种结合扩散模型和自回归模型的新范式，用扩散模型为自回归生成提供建议，在COCO图像描述任务上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 扩散去噪模型在连续域表现出色，但在离散域（如文本生成）仍无法超越自回归模型。作者希望结合两者的优势：扩散模型的双向处理和精炼能力，以及自回归模型的强语言结构能力。

Method: 提出SST框架，采用扩散模型为自回归生成提供建议，而不是替代它。具体来说，扩散模型生成候选建议，自回归模型基于这些建议生成最终描述，形成"展示-建议-讲述"的流程。

Result: 在COCO数据集上，SST在没有强化学习的情况下达到125.1 CIDEr-D分数，比自回归和扩散模型的SOTA结果分别高出1.5和2.5分。实验表明建议质量与描述质量呈正相关。

Conclusion: 将扩散模型作为自回归生成的建议提供者是一个有前景的研究方向，能够结合两种模型的优势，在图像描述任务上取得了显著改进。
...
TL;DR: MetaVoxel是一个生成式联合扩散建模框架，通过单一扩散过程建模医学影像和临床元数据的联合分布，支持零样本推理，无需针对特定任务重新训练。
...
Result: 使用9个数据集的10,000多个T1加权MRI扫描和临床元数据，单个MetaVoxel模型在图像生成、年龄估计和性别预测任务上表现与特定任务基线相当，展示了灵活的推理能力。
...
TL;DR: TraceFlow是一个用于动态镜面场景高保真渲染的新框架，通过残差材料增强的2D高斯泼溅表示和动态环境高斯方法，结合混合渲染管道实现物理准确的镜面反射。


<details>
  <summary>Details</summary>
Motivation: 解决动态镜面场景渲染中的两个关键挑战：精确的反射方向估计和物理准确的反射建模，以在复杂动态环境中实现更真实、更锐利的镜面反射。

Method: 1. 提出残差材料增强的2D高斯泼溅表示，建模动态几何和材料属性以计算准确反射光线；2. 引入动态环境高斯和混合渲染管道，将渲染分解为漫反射和镜面反射组件；3. 采用由粗到细的训练策略提高优化稳定性并促进物理意义分解。
...
<details>
  <summary>Details</summary>
Motivation: 现有跟踪任务通常只关注物体级别，缺乏对物体与部件之间层次关系的建模。需要开发能够同时跟踪物体及其部件并保持层次关系的新任务。

Method: 提出分层实例跟踪任务，创建包含2765个唯一实体、552个视频、40个类别（包括物体和部件）的基准数据集。评估了四种模型的七种变体，专门针对这一新任务进行设计。

Result: 新数据集具有挑战性，评估的模型变体在分层实例跟踪任务上表现有限，表明该任务需要更先进的算法来解决。

Conclusion: 分层实例跟踪是一个新颖且具有挑战性的任务，提出的基准数据集为未来研究提供了基础，需要开发更有效的算法来处理物体与部件之间的层次关系跟踪。
...
TL;DR: 提出基于小波持久同调的图像结构表征方法，通过拓扑数据分析改善乳腺X光片检测模型的外部泛化性能


<details>
  <summary>Details</summary>
Motivation: 乳腺癌筛查中，现有模型在不同扫描仪、模态和患者群体间的泛化性能下降，存在较高的假阴性和假阳性问题

Method: 使用拓扑数据分析提取图像在不同强度阈值下的持久结构特征，转换为空间多尺度映射图，通过通道拼接集成到两阶段检测管道中

Result: 在INbreast数据集上，结合小波持久通道的ConvNeXt Tiny模型将患者级AUC从0.55提升至0.75

Conclusion: 基于小波持久同调的拓扑特征表征能有效提升乳腺X光片检测模型的外部泛化性能
...
Method: MPEG制定FCM标准，设计专门针对中间特征压缩的比特流语法和编解码器流水线，并开发Feature Coding Test Model (FCTM)测试模型。

Result: FCTM在多种视觉任务中平均实现85.14%的比特率降低，同时保持模型精度，为带宽受限和隐私敏感应用提供高效解决方案。

Conclusion: FCM标准为边缘-云协同推理提供了可扩展、高效且互操作的中间特征压缩方案，有助于在资源受限场景中部署智能视觉功能。
...
TL;DR: 提出Latent-CoT-Drive模型，使用潜在语言而非自然语言进行推理，通过动作提案token和世界模型token在潜在空间中统一推理与决策，在自动驾驶任务中实现更快推理和更好轨迹质量。
...
Method: 提出Latent-CoT-Drive模型，在动作对齐的潜在空间中统一推理与决策。模型交替使用：(1)动作提案token（与输出动作相同词汇表）；(2)世界模型token（基于学习的潜在世界模型，表示动作的未来结果）。通过真实未来场景监督冷启动，然后通过闭环强化学习进行后训练。

Result: 在大规模端到端驾驶基准测试中，相比无推理和文本推理基线，LCDrive实现了更快的推理速度、更好的轨迹质量，并从交互式强化学习中获得了更大的性能提升。

Conclusion: 潜在语言比自然语言更适合自动驾驶中的链式思维推理，通过在动作对齐潜在空间中统一推理与决策，能够实现更高效、更安全的自动驾驶系统。
...
TL;DR: 论文提出FCM（面向机器的特征编码）作为机器间通信的新范式，通过压缩神经网络中间特征而非像素数据，显著降低比特率、保护隐私并支持计算卸载，同时保持接近边缘推理的精度。


<details>
  <summary>Details</summary>
Motivation: 当前机器间视觉通信主要依赖为人类感知优化的像素级视频编解码器，导致带宽消耗大、扩展性差且暴露原始图像给第三方，需要更高效的机器间通信方案。

Method: 提出FCM（面向机器的特征编码）范式，压缩神经网络中间特征而非原始像素；分析H.26X系列编解码器（H.264/AVC、H.265/HEVC、H.266/VVC）作为FCM内部编解码器的性能差异。

Result: FCM能在显著降低比特率的同时保持接近边缘推理的精度；H.265/HEVC与H.266/VVC在机器任务性能上几乎相同（平均BD-Rate仅增加1.39%），而H.264/AVC相比VVC平均BD-Rate增加32.28%；对于跟踪任务，编解码器选择影响最小，HEVC甚至优于VVC。

Conclusion: FCM为机器间通信提供了高效解决方案，现有已部署编解码器的硬件可以支持机器间通信而不降低性能，特别是HEVC在跟踪任务中表现优异，为实际部署提供了可行性。
...
TL;DR: MCDPO提出多奖励条件DPO方法，解决标准DPO中多评价维度聚合导致的奖励冲突问题，通过在训练中注入偏好结果向量作为条件，实现各奖励维度的独立优化。


<details>
  <summary>Details</summary>
Motivation: 标准DPO方法使用Bradley-Terry模型将美学质量、语义对齐等多个评价维度聚合为单一标量奖励，导致奖励冲突——模型可能被迫遗忘特定维度的理想特征，仅因为它们出现在全局非优样本中。

Method: 提出多奖励条件DPO（MCDPO），引入解耦的Bradley-Terry目标，在训练时显式注入偏好结果向量作为条件，使模型能独立学习各奖励维度的优化方向。同时引入维度奖励dropout以确保各维度平衡优化。

Result: 在Stable Diffusion 1.5和SDXL上的大量实验表明，MCDPO在基准测试中取得优越性能。条件框架还支持在推理时使用Classifier Free Guidance动态控制多个奖励维度，无需额外训练或外部奖励模型。

Conclusion: MCDPO有效解决了多维度奖励聚合中的冲突问题，实现了各奖励维度的独立优化，并为推理时动态多轴控制提供了灵活框架，显著提升了扩散模型对齐的效果。
...
<details>
  <summary>Details</summary>
Motivation: 半监督少样本学习（SSFSL）在现实应用（如自动标注）中很重要，但现有方法忽视了开源视觉语言模型（VLM）及其预训练数据的潜力。虽然相关领域已成功利用这些资源，但SSFSL中直接应用传统SSL方法微调VLM效果不佳，需要解决VLM输出概率分布"平坦"的问题。

Method: 提出SWIFT方法：1）分类器初始化，提高伪标签置信度；2）温度调优，增强监督信号；3）分阶段微调，结合少量标注数据、大量未标注数据以及从VLM预训练集中检索的任务相关但可能有噪声的数据。

Result: 在五个SSFSL基准测试中，SWIFT比最近的FSL和SSL方法高出约5个准确率点，甚至能与使用真实标签进行监督学习的方法相媲美。

Conclusion: 通过简单的分类器初始化和温度调优技术，成功解决了VLM在SSFSL中表现不佳的问题，提出的SWIFT方法能够有效利用开源VLM资源，显著提升半监督少样本学习性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI生成视频检测基准忽视了数字水印的影响，许多先进生成模型在输出中嵌入了水印，检测器可能部分依赖这些模式，需要评估这种依赖程度。

Method: 构建包含6,500个视频的数据集，分为四类：真实-干净、真实-伪造水印、生成-带水印、生成-去水印。设计两个评估任务：任务I测试去水印AI视频的检测性能，任务II评估带伪造水印的真实视频的误报率。

Result: 测试10个模型（专用AIGC检测器、Transformer架构、MLLM方法）发现，水印操作导致性能变化2-8个百分点。基于Transformer的模型显示一致的中等依赖（6-8pp），而MLLM表现出多样化模式（2-8pp）。

Conclusion: 现有检测器部分依赖水印模式，需要水印感知的训练策略。RobustSora基准为推进鲁棒的AIGC检测研究提供了必要工具。
...
TL;DR: THE-Pose提出了一种新的类别级6D姿态估计框架，通过表面嵌入和混合图融合利用拓扑先验，解决了现有3D图卷积方法在复杂对象和视觉模糊性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D图卷积方法仅关注局部几何和深度信息，对类内变化和复杂对象不够鲁棒。需要结合全局上下文和局部结构来提高姿态估计的稳定性。

Method: 提出THE-Pose框架：1) 从图像域提取一致且不变的拓扑特征；2) 设计混合图融合模块，自适应地整合拓扑特征与点云特征，桥接2D图像上下文和3D几何结构。

Result: 在REAL275数据集上，THE-Pose相比3D-GC基线(HS-Pose)提升35.8%，超越先前最佳方法7.2%。即使在严重遮挡下，对未见或复杂对象仍保持稳定。

Conclusion: 通过利用拓扑先验和混合图融合，THE-Pose有效解决了现有3D图卷积方法的局限性，在类别级6D姿态估计任务中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图序列中心腔的准确分割对心脏功能定量分析和临床诊断至关重要，但成像噪声、伪影以及心脏的形变和运动给分割算法带来挑战。现有方法在捕获长程时空依赖与保持计算效率及细粒度特征表示之间存在权衡困难。

Method: 提出GDKVM架构：1）使用线性键值关联（LKVA）有效建模帧间相关性；2）引入门控增量规则（GDR）高效存储中间记忆状态；3）设计关键像素特征融合（KPFF）模块，在多个尺度上整合局部和全局特征。

Result: 在两个主流超声心动图视频数据集（CAMUS和EchoNet-Dynamic）上验证，与多种最先进方法相比，GDKVM在分割精度和鲁棒性方面表现更优，同时确保实时性能。

Conclusion: GDKVM通过创新的架构设计，在超声心动图视频分割任务中实现了更好的长程时空依赖建模与计算效率平衡，提高了分割精度和鲁棒性，具有临床应用价值。
...
TL;DR: LLM-NCD：通过融合视觉-文本语义和原型引导聚类的新型类发现框架，在CIFAR-100上对未知类准确率提升达25.3%，首次在NCD领域展示了对长尾分布的鲁棒性。
...
Result: 在CIFAR-100数据集上，相比现有方法，对未知类的准确率提升达25.3%，首次在NCD文献中展示了对长尾分布的独特鲁棒性。

Conclusion: LLM-NCD通过多模态语义融合有效解决了传统NCD方法的局限性，显著提升了新类发现性能，特别是在处理长尾分布数据方面表现出色。
...
TL;DR: Long-LRM++提出了一种半显式场景表示结合轻量级解码器的方法，在保持隐式表示高质量渲染的同时实现实时性能（14 FPS），并支持扩展到64个输入视图。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）可泛化高斯溅射方法（如Long-LRM）直接预测数百万高斯参数容易出错，导致细节模糊；2）隐式表示方法（如LVSM、LaCT）虽然渲染质量高，但需要深度序列化"解压缩"过程，无法实现实时渲染。研究旨在探索是否能在保持隐式表示优势的同时实现实时性能。

Method: 采用半显式场景表示结合轻量级解码器的设计。该方法避免了直接预测大量高斯参数的错误敏感性，同时减少了隐式表示所需的计算密集型解压缩过程，实现了实时渲染能力。

Result: 在DL3DV数据集上达到与LaCT相当的渲染质量，同时在A100 GPU上实现实时14 FPS渲染。支持扩展到64个输入视图（950×540分辨率），在ScanNetv2上提供优于高斯溅射的新视角深度预测。消融研究验证了各组件有效性。

Conclusion: Long-LRM++成功解决了隐式表示方法的实时渲染瓶颈，通过半显式表示和轻量级解码器设计，在保持高质量渲染的同时实现了实时性能，并展示了良好的可扩展性和泛化能力。
...
TL;DR: 提出SAAD方法，通过样本自适应权重调整解决对抗蒸馏中强教师不一定产生更强学生的问题，提升对抗鲁棒性转移效率。


<details>
  <summary>Details</summary>
Motivation: 现有对抗蒸馏方法常忽视使用最先进的鲁棒教师模型，且发现更强的教师不一定能产生更鲁棒的学生（鲁棒饱和现象）。传统解释归因于容量差距，但作者认为这种解释不完整，需要更深入理解对抗鲁棒性转移机制。

Method: 提出样本自适应对抗蒸馏（SAAD），核心思想是利用对抗可转移性（学生生成的对抗样本对教师有效的比例）作为关键指标。SAAD通过测量样本的对抗可转移性来重新加权训练样本，而不增加额外计算成本。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet数据集上的实验表明，SAAD相比先前方法能持续提升AutoAttack鲁棒性。

Conclusion: 对抗可转移性是成功转移鲁棒性的关键因素，SAAD方法通过样本自适应权重调整有效解决了对抗蒸馏中的鲁棒饱和问题，提升了鲁棒性转移效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑数据集主要关注静态外观变化，缺乏高质量的动作编辑数据。动作编辑任务在科学上具有挑战性，在实际应用中也很重要，可以支持帧控制视频合成和动画等下游应用。

Method: 1) 构建MotionEdit数据集，从连续视频中提取和验证高质量的动作变换图像对；2) 创建MotionEdit-Bench基准测试，使用生成式、判别式和基于偏好的指标评估模型性能；3) 提出MotionNFT（运动引导负感知微调）框架，通过计算输入图像与模型编辑图像之间的运动流与真实运动的对齐奖励来指导模型。

Result: 基准测试结果显示，现有最先进的基于扩散的编辑模型在动作编辑任务上仍然面临很大挑战。MotionNFT框架在FLUX.1 Kontext和Qwen-Image-Edit模型上的实验表明，它能够持续提高编辑质量和动作保真度，同时不牺牲一般的编辑能力。

Conclusion: MotionEdit数据集和基准测试填补了动作中心图像编辑领域的空白，而MotionNFT框架为解决这一挑战性任务提供了有效的解决方案，显著提升了模型在动作编辑方面的性能。
...
TL;DR: ShotDirector：通过参数级相机控制和分层编辑模式感知提示，实现电影式可控镜头转换的视频生成框架


<details>
  <summary>Details</summary>
Motivation: 当前多镜头视频生成主要关注镜头间的低级视觉一致性，忽视了镜头转换的设计和电影语言对连贯叙事表达的作用，导致镜头变化只是简单的顺序切换，缺乏有意的电影编辑模式。

Method: 提出ShotDirector框架，包含：1）相机控制模块，采用6自由度位姿和内在参数设置实现精确相机信息注入；2）镜头感知掩码机制，引入感知专业编辑模式的分层提示，实现对镜头内容的细粒度控制。

Result: 构建了ShotWeaver40K数据集捕获电影式编辑模式先验，开发了可控多镜头视频生成的评估指标。大量实验证明了该框架的有效性。

Conclusion: ShotDirector通过结合参数级条件和高层语义指导，实现了电影式的可控镜头转换，解决了现有方法忽视镜头转换设计和电影语言表达的问题。
...
TL;DR: Disentangled360：一种创新的3D感知技术，通过方向解耦的体积渲染实现单图像360°视图合成，适用于医学成像和自然场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有技术要么过度简化各向异性光照行为，要么缺乏跨不同场景的泛化能力，无法同时满足医学成像和自然场景重建的需求。

Method: 1. 在Gaussian Splatting框架中区分各向同性和各向异性贡献；2. 采用双分支条件框架：一个针对CT强度驱动的体数据散射，另一个针对真实世界RGB场景的归一化相机嵌入；3. 提出混合姿态无关锚定方法解决尺度模糊问题；4. 集成术前放射模拟和消费级360°渲染到单一推理管道。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言导航中展现出潜力，但实际发展受到巨大训练开销的严重阻碍。主要问题包括：(1) 处理长历史观测作为大量token序列带来的二次计算负担；(2) DAgger中的探索效率权衡，即收集智能体探索轨迹的数据聚合过程。
...
Conclusion: Efficient-VLN通过高效记忆机制和动态混合策略，在保持高性能的同时大幅降低了训练开销，为实际部署多模态大语言模型在视觉语言导航任务中提供了可行的解决方案。
...
TL;DR: 提出基于原型驱动的弱监督语义分割框架，结合视觉-语言对齐，在数字病理学中通过图像级标签实现更好的区域发现
...
Method: 采用原型驱动框架，结合CoOp式可学习提示调优生成文本原型，与可学习图像原型形成双模态原型库；加入多尺度金字塔模块解决ViT表示过平滑问题

Result: 在BCSS-WSSS基准测试中超越现有最先进方法，分析显示文本描述多样性、上下文长度以及文本与图像原型的互补行为带来显著优势

Conclusion: 联合利用文本语义和视觉原型学习在数字病理学弱监督语义分割中具有有效性，为降低标注成本提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 组织病理学中的弱监督语义分割严重依赖分类主干网络，但这些模型通常只定位最具判别性的区域，难以捕捉组织结构的完整空间范围。现有的视觉语言模型（如CONCH）提供丰富的语义对齐和形态感知表示，而现代分割主干网络（如SegFormer）保留细粒度空间线索，但如何结合这些互补优势在弱监督下仍具挑战。
...
Result: 在BCSS-WSSS数据集上的实验表明，该方法优于现有弱监督语义分割方法，提高了定位完整性和跨组织类型的语义一致性，同时通过冻结基础模型主干和轻量可训练适配器保持计算效率。

Conclusion: 提出的原型学习框架成功整合了视觉语言模型的语义对齐能力、分割主干的空间结构保留能力和文本引导的语义准确性，为组织病理学弱监督语义分割提供了一种高效且有效的解决方案。
...
Result: 提出的方法在多个数据集上优于基线模型，展示了优越的性能表现
...
TL;DR: EchoingPixels：一种用于音频-视觉大语言模型的高效token减少框架，通过跨模态语义筛选和同步增强位置编码，仅用5-20%的原始token实现可比性能，速度提升2-3倍。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉大语言模型面临大量音频和视频token带来的计算开销问题。现有的单模态token减少方法无法利用音频-视觉跨模态协同作用，且静态的模态预算分配不适应音频和视频不同的动态信息密度。

Method: 提出EchoingPixels框架，核心是跨模态语义筛选(CS2)模块，它共同处理联合多模态流，从整个音频-视觉token池中减少token，而不是为每个模态分配固定预算。同时设计同步增强RoPE(Sync-RoPE)来保持稀疏选择token的关键时间关系。

Result: 实验表明，EchoingPixels仅使用原始token的5-20%就能达到强基线的可比性能，同时实现2-3倍的速度提升和内存减少。

Conclusion: 该工作解决了音频-视觉大语言模型中联合token减少的关键瓶颈，通过跨模态协同和动态预算分配实现了高效的多模态处理，为实际应用提供了可行的解决方案。
...
TL;DR: StainNet是一个针对特殊染色病理图像（如免疫组化）的专用基础模型，基于ViT架构，采用自蒸馏自监督学习方法，在140万张特殊染色图像上训练，解决了现有病理基础模型主要基于H&E染色图像的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型主要基于H&E染色图像预训练，但临床实践中经常使用特殊染色（如免疫组化）图像。这些模型在涉及特殊染色的临床应用中存在局限性，需要专门针对特殊染色的基础模型。

Method: 提出StainNet模型，基于视觉变换器架构，采用自蒸馏自监督学习方法，在HISTAI数据库的20,231张公开可用的特殊染色全切片图像中裁剪出140万张图像块进行训练。

Result: 在内部肝脏恶性肿瘤分类任务和两个公开ROI级数据集上验证了StainNet的强能力。进行了少样本学习和检索评估，并与近期更大的病理基础模型比较，进一步突出了其优势。

Conclusion: StainNet是针对特殊染色病理图像的专用基础模型，解决了现有模型在特殊染色应用中的局限性，在多个任务上表现出色，模型权重已公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 丝状结构（如微管和肌动蛋白丝）在生物系统中很重要，但分割这些结构需要高质量像素级标注数据。由于丝状结构分布密集且几何特性复杂，手动标注极其耗时费力，导致数据稀缺问题。

Method: 基于Pix2Pix架构的条件生成框架，从二值掩码生成逼真的显微镜图像中的丝状结构。提出丝状结构感知的结构损失函数，提高生成合成图像时的结构相似性。

Result: 实验证明该方法有效，并且优于未使用合成数据训练的现有模型。

Conclusion: 提出的条件生成框架能够生成逼真的丝状结构图像，有效缓解丝状结构分割中的数据稀缺问题，提高分割性能。
...
TL;DR: 提出HD-CLIP（分层退化CLIP）方法，通过语义嵌入和有序退化嵌入分解低质量图像，结合分类器自由投影引导，提升真实世界图像超分辨率的细节保真度和感知真实性。
...
Method: 提出HD-CLIP将低质量图像分解为语义嵌入和有序退化嵌入，通过分类器自由投影引导（CFPG）集成到扩散模型中，作为即插即用模块。

Result: HD-CLIP能无缝集成到各种超分辨率框架中无需训练，显著提升细节保真度和感知真实性，在多样真实世界数据集上表现优异。

Conclusion: HD-CLIP通过分层退化表示和投影引导机制，有效解决了真实世界图像超分辨率中的退化建模问题，提升了生成恢复的质量和泛化能力。
...
TL;DR: 论文提出CoSPlan基准测试评估视觉语言模型在视觉顺序规划中的纠错能力，并开发了SGI方法提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型在复杂推理方面表现出色，但在视觉顺序规划（执行多步动作达成目标）方面研究不足。实际规划常涉及非最优（错误）步骤，需要模型具备检测和纠正能力。

Method: 提出CoSPlan基准测试，评估模型在迷宫导航、积木重排、图像重建和物体重组4个领域的纠错顺序规划能力。开发了训练免费的SGI方法，通过在初始状态和目标状态之间引入中间推理步骤来增强模型推理。

Result: 现有VLMs（如Intern-VLM和Qwen2）在CoSPlan上表现不佳，无法有效利用上下文线索。SGI方法平均提升5.2%性能，并在传统规划任务（Plan-Bench和VQA）上具有泛化能力。

Conclusion: CoSPlan揭示了VLMs在纠错顺序规划方面的局限性，SGI方法通过引入增量推理步骤有效提升了模型性能，为可靠顺序规划提供了新思路。
...
TL;DR: OmniZoo：大规模动物运动数据集与拓扑感知运动生成框架，支持任意骨骼拓扑和文本驱动的运动生成


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法大多依赖固定骨骼模板，无法泛化到不同或扰动拓扑的骨骼结构。缺乏大规模异构动物运动数据和统一生成框架是核心限制。

Method: 1. 构建OmniZoo数据集：涵盖140个物种、32,979个序列，包含多模态标注；2. 提出拓扑感知骨骼嵌入模块，将任意骨骼的几何和结构属性编码到共享标记空间；3. 建立自回归运动生成框架，结合文本语义生成运动。

Result: 方法能够为任意骨骼拓扑生成时间一致、物理合理、语义对齐的文本驱动运动，并支持跨物种运动风格迁移。

Conclusion: OmniZoo解决了当前运动生成方法在数据规模和拓扑泛化方面的限制，为任意骨骼结构的文本驱动运动生成提供了统一框架。
...
TL;DR: TranSamba是一个结合Transformer和Mamba的混合架构，用于弱监督的体医学图像分割，通过跨平面Mamba块实现高效的3D上下文建模。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督医学图像分割方法通常使用2D编码器，忽略了数据的体数据本质，需要一种能够有效捕捉3D上下文的方法。

Method: 提出TranSamba混合架构，在标准Vision Transformer基础上增加跨平面Mamba块，利用状态空间模型的线性复杂度在相邻切片间进行高效信息交换，增强Transformer块内的自注意力机制。

Result: 在三个数据集上的实验表明，TranSamba实现了最先进的性能，在不同模态和病理类型上均优于现有方法，且计算复杂度与输入体积深度呈线性关系，批处理时内存使用恒定。

Conclusion: TranSamba通过结合Transformer和Mamba的优势，为弱监督体医学图像分割提供了高效的3D上下文建模解决方案，代码和模型已开源。
...
TL;DR: mmCounter使用毫米波雷达通过提取超低频呼吸和微动信号，实现密集静态人群的准确计数，解决了传统毫米波雷达依赖运动检测的限制。


<details>
  <summary>Details</summary>
Motivation: 传统毫米波雷达在检测密集静态人群时存在局限，因为其空间分辨率不足且依赖运动检测。现有呼吸率估计研究假设人数已知，而实际应用中需要先确定人数。

Method: 采用多阶段信号处理流程：提取超低频（<1Hz）呼吸和微动信号，从背景噪声和静态物体中分离这些微弱信号，结合空间信息将信号源映射到个体。

Result: 在熟悉环境中平均F1分数87%，平均绝对误差0.6；在未测试环境中平均F1分数60%，平均绝对误差1.1。可在3平方米空间内计数最多7人。

Conclusion: mmCounter通过创新信号处理技术，首次实现了毫米波雷达对密集静态人群的准确计数，突破了传统毫米波雷达的应用限制。
...
TL;DR: 该论文提出了一个视频工具包和时空推理框架（STAR），通过增强多模态大语言模型在视频问答任务中的时空推理能力，解决了现有模型在空间关系建模和时序动态理解上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在复杂的视频问答任务中难以同时建模视频帧内的空间关系和理解时序演化的因果动态，这限制了它们在动态真实场景中的感知、理解和推理能力。

Method: 1. 为MLLM配备全面可扩展的视频工具包，增强时空推理能力；2. 提出时空推理框架（STAR），策略性地调度时空工具，逐步定位视频中的关键区域，避免工具链捷径问题。
...
TL;DR: Visual Funnel：一种解决MLLMs中"上下文盲区"问题的免训练两阶段方法，通过上下文锚定和熵缩放组合构建层次化视觉输入，提升细粒度感知能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在细粒度视觉细节感知方面存在不足，现有裁剪方法虽然能提供高保真细节，但造成了"上下文盲区"问题——细节与全局上下文的结构性脱节，限制了在精度要求高的任务中的应用
...
Conclusion: 上下文盲区问题源于输入缺乏"结构多样性"而非信息"数量"不足，Visual Funnel通过构建层次化视觉输入组合有效解决了这一问题，为MLLMs的细粒度视觉感知提供了有效方案
...
TL;DR: P2S是一个无需训练的长视频时刻检索框架，通过自适应跨度生成和查询分解技术，在零样本设置下超越了有监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有长视频时刻检索方法面临计算效率低下的挑战：有监督方法扩展性差且泛化能力有限，而零样本方法在搜索阶段候选爆炸、精炼阶段需要高成本视觉语言模型验证。

Method: 提出P2S框架，包含两个核心技术：1) 自适应跨度生成器防止搜索阶段候选爆炸；2) 查询分解技术在不依赖高成本VLM验证的情况下精炼候选。

Result: P2S在零样本设置下超越了有监督的SOTA方法，在MAD数据集上R5@0.1指标提升3.7%，是首个能在小时级长视频中进行时间定位的零样本框架。

Conclusion: P2S通过创新的训练免费框架解决了长视频时刻检索的计算效率问题，在搜索和精炼阶段都实现了显著改进，为零样本长视频分析提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在新视角合成中表现出色，但其性能严重依赖密集高质量输入图像。现实应用中数据通常是稀疏且运动模糊的，这两者形成恶性循环：稀疏视图忽略了解决运动模糊所需的多视角约束，而运动模糊又抹去了对齐有限视图所需的高频细节，导致重建失败。

Method: 提出CoherentGS框架，采用双先验策略：结合预训练的去模糊网络（恢复清晰细节并提供光度指导）和扩散模型（提供几何先验填充未观测区域）。关键技术包括一致性引导的相机探索模块和深度正则化损失。

Result: 在合成和真实场景上使用3、6、9个输入视图进行定量和定性评估，CoherentGS显著优于现有方法，为这一挑战性任务设定了新的最先进水平。

Conclusion: CoherentGS成功解决了从稀疏模糊图像进行3D重建的挑战，通过双先验策略打破了稀疏视图和运动模糊之间的恶性循环，实现了高质量的重建结果。
...
TL;DR: 首个结合4D毫米波雷达与LiDAR的场景流估计方法，通过动态感知双向跨模态融合模块和专门设计的损失函数，在新建的数据集上显著优于单模态方法。


<details>
  <summary>Details</summary>
Motivation: 雷达比LiDAR更便宜、在各种天气条件下更鲁棒，并能检测点级速度，是LiDAR的有价值补充。然而，雷达数据存在噪声、分辨率低和稀疏性问题，且目前没有专门用于场景流估计的雷达-LiDAR数据集。

Method: 1) 基于公开真实世界汽车数据集构建雷达-LiDAR场景流数据集；2) 提出有效的雷达去噪和场景流标签生成预处理策略；3) 提出RaLiFlow框架，包含动态感知双向跨模态融合(DBCF)模块和专门设计的损失函数，实现有效的雷达-LiDAR融合。

Result: 在重新构建的场景流数据集上进行广泛实验，证明该方法显著优于现有的基于LiDAR和基于雷达的单模态方法。

Conclusion: 首次探索了4D雷达与LiDAR的融合用于场景流估计，提出的RaLiFlow框架通过有效的跨模态融合和损失函数设计，成功克服了雷达数据的挑战，实现了优于单模态方法的性能。
...
TL;DR: 提出一种用于内窥镜图像特征匹配的深度学习框架，通过自监督对比学习优化DINOv2骨干网络，在SCARED数据集上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 在微创手术中，内窥镜图像间的精确像素级对应关系对于3D重建、相机跟踪和场景理解至关重要。然而手术领域存在独特挑战：弱透视线索、非朗伯组织反射和复杂可变形解剖结构会降低传统计算机视觉技术的性能。深度学习模型虽然在自然场景中表现良好，但其特征并不天然适用于手术图像的细粒度匹配，需要针对性适应。

Method: 提出一个深度学习流水线用于建立内窥镜图像对的特征对应关系，以及一个用于模型训练的自监督优化框架。方法利用新视角合成流水线生成真实内点对应关系，随后在对比学习范式中用于挖掘三元组。通过这种自监督方法，在DINOv2骨干网络上增加额外的Transformer层，专门优化以产生通过余弦相似度阈值实现直接匹配的嵌入向量。

Result: 实验评估表明，该流水线在SCARED数据集上超越了最先进的方法，相比相关工作具有更高的匹配精度和更低的极线误差。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注推理任务，忽略了细粒度识别这一实际应用中的关键能力，需要专门的评估框架来填补这一空白。

Method: 构建FROW基准，包含马赛克数据（组合多个简短回答）和开放世界数据（基于真实世界问答生成），并从数据构建和训练过程两个角度提出优化策略。

Result: 马赛克数据提升类别识别准确率1%，开放世界数据提升FROW基准准确率10%-20%、内容准确率6%-12%，预训练阶段加入细粒度数据可提升类别识别准确率最高达10%。

Conclusion: FROW基准有效评估LVLMs的细粒度识别能力，提出的优化策略显著提升模型性能，为实际应用场景提供了重要工具。
...
Method: 1) 使用八叉树进行全局点云空间分区实现并行加速；2) 在叶节点内使用自适应体素占用统计和kNN密度估计快速去除孤立噪声点；3) 构建结合密度权重和自适应距离权重的引力评分函数精细区分噪声点和物体点
...
Conclusion: 该方法在多噪声场景下实现了高精度、强鲁棒性和实时性能的平衡，验证了其有效性
...
TL;DR: 首次系统研究RL在文本到3D生成中的应用，提出Hi-GRPO分层优化方法，开发了首个RL增强的文本到3D模型AR3D-R1


<details>
  <summary>Details</summary>
Motivation: RL已在2D图像生成中证明有效，但应用于3D生成仍未被探索。3D对象具有更高的空间复杂性，需要全局一致的几何结构和细粒度局部纹理，对奖励设计和RL算法更敏感

Method: 1) 评估奖励维度和模型选择；2) 研究GRPO变体，强调token级优化的有效性；3) 引入MME-3DR基准；4) 提出Hi-GRPO分层优化方法，通过专用奖励集合优化全局到局部的3D生成

Result: 开发了AR3D-R1，这是首个RL增强的文本到3D模型，能够从粗粒度形状到纹理细化进行专家级生成

Conclusion: 这项研究为RL驱动的3D生成推理提供了见解，希望推动该领域发展。代码已开源
...
<details>
  <summary>Details</summary>
Motivation: TikTok和YouTube等平台视频内容快速增长，多模态仇恨言论传播加剧，现有研究主要关注视频级分类，而实际应用中关键的时序定位任务（识别仇恨片段何时发生）尚未得到充分解决，特别是在弱监督（仅有视频级标签）条件下。

Method: 1. 模态感知时序编码器：建模异质序列模式，包含针对文本的预处理模块进行特征增强；2. 动态跨模态融合：自适应强调每个时刻最具信息量的模态，结合跨模态对比对齐策略增强多模态特征一致性；3. 模态感知MIL目标：在视频级监督下识别判别性片段。

Result: 在HateMM和MultiHateClip数据集上的实验表明，该方法在定位任务中达到了最先进的性能，尽管仅依赖粗粒度标签，却能产生细粒度、可解释的帧级预测。

Conclusion: MultiHateLoc是首个针对弱监督多模态仇恨言论时序定位的框架，通过创新的模态建模和融合策略，有效解决了现有方法在捕捉跨模态和时序动态方面的不足，为多模态仇恨内容检测提供了实用的时序定位解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在城区道路提取已有进展，但越野环境仍面临挑战。主要问题包括：缺乏大规模矢量化数据集，以及现有节点中心方法在越野场景中对遮挡和模糊交叉口的脆弱性导致拓扑错误。
...
Conclusion: 数据集和路径中心范式为野外道路测绘提供了更强基础，解决了越野道路提取的关键挑战。
...
TL;DR: TransLocNet：基于跨模态注意力融合LiDAR几何与航拍语义的空中-地面定位框架，通过双向注意力对齐和对比学习，在CARLA和KITTI数据集上实现亚米级、亚度级精度，定位误差降低达63%。


<details>
  <summary>Details</summary>
Motivation: 地面LiDAR与航拍图像之间存在巨大的视点和模态差异，使得空中-地面定位变得困难。现有方法难以有效融合这两种不同模态的数据，需要一种能够桥接几何信息与语义上下文的方法。
...
Result: 在CARLA和KITTI数据集上的实验表明，TransLocNet优于现有最先进基线方法，定位误差降低高达63%，实现了亚米级（位置）和亚度级（方向）的精度。在合成和真实世界场景中均表现出鲁棒性和泛化能力。

Conclusion: TransLocNet通过有效融合LiDAR几何与航拍语义上下文，解决了空中-地面定位中的跨模态对齐问题，为合成和真实世界环境提供了鲁棒且可泛化的定位解决方案。
...
TL;DR: 该论文提出NCTTA方法，通过特征-分类器对齐和混合目标来提升测试时适应（TTA）的鲁棒性，解决分布偏移下的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法缺乏对分布偏移下性能下降根本原因的理论洞察。作者发现样本级神经坍缩现象（NC3+），并识别出性能下降源于样本级错位，这在大分布偏移下会加剧，表明需要重新对齐特征嵌入和分类器权重。

Method: 提出NCTTA方法：基于NC3+现象，设计特征-分类器对齐方法，使用混合目标来减轻不可靠伪标签的影响。混合目标结合几何邻近性和预测置信度，实现更可靠的对齐。

Result: 大量实验证明NCTTA在增强对分布偏移鲁棒性方面的有效性。例如，在ImageNet-C上NCTTA比Tent方法提升了14.52%的性能。

Conclusion: 通过扩展神经坍缩到样本级并发现NC3+现象，论文揭示了TTA性能下降的根本原因，并提出了有效的NCTTA方法来缓解这一问题，显著提升了模型在分布偏移下的鲁棒性。
...
TL;DR: 提出基于移动设备的实时物理治疗动作识别与评估框架，通过姿态估计、角度特征提取和动态规划序列匹配，实现客户端实时运动分析和错误检测


<details>
  <summary>Details</summary>
Motivation: 为远程物理治疗和移动健康应用提供实时、可扩展的运动识别与评估解决方案，解决传统方法在实时性、准确性和客户端部署方面的限制

Method: 1) 使用姿态估计神经网络从摄像头输入提取人体关键点；2) 将关键点转换为三角函数角度特征；3) 轻量级监督模型进行帧级姿态分类和准确度评分；4) 基于改进Levenshtein距离的动态规划算法进行完整运动序列匹配和偏差检测

Result: 实验评估验证了方法的有效性，系统完全在客户端运行，实现了实时性能和可扩展性，适用于远程物理治疗监督和移动健康应用

Conclusion: 该框架为实时物理治疗运动识别和评估提供了高效解决方案，具有实际应用价值，特别适合移动设备和远程医疗场景
...
TL;DR: 提出一种新的统一变换框架，通过双域渐进时间对齐和质量条件专家混合模块，在消除误差传播的同时提升运动估计性能，实现高质量视频压缩。


<details>
  <summary>Details</summary>
Motivation: 现有学习视频压缩框架面临两难：分离变换框架虽R-D性能好但误差传播明显；统一变换框架消除误差传播但运动估计性能不足。需要解决这一局限性。

Method: 1. 双域渐进时间对齐：粗粒度像素域对齐处理简单运动，精粒度潜在域对齐使用流引导可变形Transformer处理复杂运动；2. 质量条件专家混合模块：根据目标质量和内容动态分配专家调整量化步长，实现连续比特率控制。

Result: 实验结果显示，该方法在保持与最先进方法竞争性R-D性能的同时，成功消除了误差传播。

Conclusion: 提出的统一变换框架通过双域渐进时间对齐和QCMoE模块，实现了高质量、无误差传播的视频压缩，解决了现有框架的局限性。
...
TL;DR: 提出一种混合式Shape-from-Focus框架，结合传统DDL核计算多尺度聚焦体积与轻量级GRU深度提取模块，通过迭代优化和凸上采样实现高精度深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习SFF方法通常采用两阶段流程：先使用重型特征编码器提取聚焦体积，再通过简单聚合技术估计深度，这种方法容易产生伪影和噪声放大问题。

Method: 1) 使用传统手工设计的DDL核计算多尺度聚焦体积，捕获长距离和方向性聚焦变化；2) 采用轻量级多尺度GRU深度提取模块迭代优化低分辨率初始深度估计；3) 在循环网络中集成学习的凸上采样模块重建高分辨率深度图。

Result: 在合成和真实数据集上的广泛实验表明，该方法在准确性和泛化能力上均优于当前最先进的深度学习和传统方法，能够适应多样化的聚焦条件。

Conclusion: 提出的混合框架通过结合传统聚焦体积计算和轻量级深度学习优化，有效解决了现有SFF方法的伪影和噪声问题，实现了更精确、更鲁棒的深度估计。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏用于研究面部血流动态搏动的3D数据集，这限制了远程光电容积脉搏波成像（iPPG）方法的改进和验证。需要能够生成合成视频数据并研究多视角方法来减轻光照影响的数据集。

Method: 收集了15名受试者的多视角原始视频（23个视角，30Hz），包含血流脉搏参考测量，并使用单目运动结构技术生成面部3D扫描。处理后的3D搏动图与FLAME 3D头部模型的纹理空间兼容。

Result: 数据集包含信噪比、局部脉搏幅度、相位信息和补充数据。对数据集的光照条件、图一致性和捕获面部及颈部皮肤区域生理意义特征的能力进行了全面评估。

Conclusion: Pulse3DFace是首个3D血流搏动图数据集，能够促进动态面部血流模型开发、合成视频生成以及多视角光照效应缓解方法研究，为改进远程脉搏估计技术提供重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 少样本语义分割中，编码器对新类别的特征提取能力有限是主要瓶颈，现有研究主要关注解码器改进，而编码器的泛化能力不足。

Method: 提出TaP方法，利用低秩适应（LoRA）在支持集上微调编码器，计算开销小，可快速适应新类别并避免灾难性遗忘，与现有FSS框架兼容。

Result: 在COCO 20^i、Pascal 5^i以及跨域数据集（DeepGlobe、ISIC、Chest X-ray）上，TaP显著提升分割性能，尤其在复杂多类别场景表现突出，低秩适应即可获得强性能。

Conclusion: TaP通过解决编码器对新类别泛化能力的关键限制，为构建更鲁棒、高效、可泛化的分割系统铺平了道路。
...
TL;DR: Blink是一个动态视觉token分辨率框架，模拟人类眨眼式感知过程，通过显著性引导的扫描和动态token分辨率来增强多模态大语言模型的视觉感知能力。
...
Method: 提出Blink框架，包含两个模块：1) 显著性引导扫描：基于注意力图估计每层视觉token的显著性；2) 动态token分辨率：通过即插即用的token超分辨率模块扩展重要token，在下一层当它们失去焦点时丢弃扩展token，实现动态平衡探索和精细聚焦。

Result: 大量实验验证了Blink的有效性，证明其能增强视觉感知和多模态理解能力。

Conclusion: Blink通过模拟人类感知策略，实现了自适应且高效的视觉感知增强，为多模态大语言模型的视觉理解提供了新思路。
...
TL;DR: GETok是一种用于多模态大语言模型的空间表示方法，通过可学习的网格标记和偏移标记来提升模型在2D图像空间中的物体定位能力。
...
Method: GETok将专门的可学习标记词汇集成到MLLMs中：1）使用网格标记将图像平面划分为结构化空间锚点；2）利用偏移标记实现精确的迭代定位预测细化。通过将空间关系直接嵌入到标记中，在不修改自回归架构的情况下提升2D空间推理能力。

Result: 大量实验表明，GETok在监督微调和强化学习设置下的各种指代任务中，都优于最先进的方法，取得了卓越的性能。

Conclusion: GETok通过将空间表示直接嵌入到语言标记中，显著提升了多模态大语言模型在原生2D空间中的推理能力，为解决MLLMs在空间定位方面的限制提供了有效方案。
...
TL;DR: 提出基于骨架编码器的少样本原型网络框架，用于解决孤立手语识别中的数据稀缺和长尾分布问题，在WLASL数据集上取得显著性能提升，并展示零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 孤立手语识别面临数据稀缺和词汇长尾分布问题，传统分类方法容易过拟合常见类别而难以泛化到罕见类别，需要更有效的解决方案。

Method: 提出少样本原型网络框架，结合时空图卷积网络和多尺度时间聚合模块，通过情景训练学习语义度量空间，基于动态类别原型进行手语分类。

Result: 在WLASL数据集上达到43.75% Top-1和77.10% Top-5准确率，比相同骨干架构的标准分类基线提升超过13%，在未调优的SignASL数据集上实现近30%的零样本准确率。

Conclusion: 原型训练策略在数据稀缺情况下优于标准分类方法，提供了用有限数据识别大量手语词汇的可扩展途径，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法忽视了音频视觉同步，缺乏精确的实例级编辑所需的细粒度空间和时间可控性，需要新的框架来解决这些问题。

Method: 提出AVI-Edit框架，包含粒度感知掩码细化器（迭代细化用户提供的粗掩码为精确实例区域）和自反馈音频代理（提供高质量音频引导以实现细粒度时间控制），并构建了大规模实例中心对应数据集。

Result: 大量实验表明，AVI-Edit在视觉质量、条件遵循和音频视觉同步方面优于现有最先进方法。

Conclusion: AVI-Edit通过创新的掩码细化和音频引导机制，成功实现了精确的音频同步视频实例编辑，为音频视觉内容创作提供了有效解决方案。
...
TL;DR: 提出对称U-Net架构用于一体化图像恢复，无需复杂专家混合或扩散模型，通过对称设计和跨尺度特征传播有效利用退化信息，实现SOTA性能且计算成本更低。


<details>
  <summary>Details</summary>
Motivation: 现有的一体化图像恢复方法过度依赖复杂架构（如专家混合、扩散模型）和精细的退化提示策略，作者发现精心设计的特征提取本身已编码退化信息，对称U-Net架构足以有效利用这些线索。

Method: 提出对称U-Net架构（SymUNet），通过对齐编码器-解码器特征尺度并实现简化的跨尺度传播，在跳跃连接中使用简单的加法融合。进一步提出语义增强变体SE-SymUNet，通过交叉注意力从冻结的CLIP特征注入语义信息以显式增强退化先验。

Result: 在多个基准数据集上验证了方法的优越性，SymUNet和SE-SymUNet均超越了现有方法，同时降低了计算成本，为一体化图像恢复建立了更简单更强的基准。

Conclusion: 对称U-Net架构是有效的一体化图像恢复解决方案，无需复杂架构即可实现SOTA性能，为未来研究提供了更简单强大的基础。
...
Result: 在WXSOD数据集上进行广泛实验，使用不同训练数据规模(100%、50%、30%)、三种编码器和七种解码器配置。结果表明，提出的SOD框架(特别是NIFM增强的特定编码器)相比普通编码器，在复杂天气条件下显著提高了分割精度。

Conclusion: 该研究提出的针对多天气条件的SOD框架有效解决了天气噪声对显著目标检测的影响，通过噪声指示器融合模块实现了天气感知的特征调制，提升了在复杂天气条件下的分割性能，同时保持了与现有解码器的兼容性。
...
TL;DR: 提出RSRT数据集和TRSLLaVA方法，通过训练免费的文本到文本匹配实现遥感图像语义检索，性能接近监督模型。


<details>
  <summary>Details</summary>
Motivation: 解决遥感图像检索中的"语义鸿沟"问题，现有方法依赖昂贵的领域特定训练，缺乏评估VLM生成文本实用性的基准。

Method: 引入RSRT数据集（每张图像多个结构化描述），提出TRSLLaVA方法，将跨模态检索重新定义为文本到文本匹配问题，在统一文本嵌入空间中使用VLM生成的描述进行检索。

Result: 在RSITMD和RSICD基准测试中，训练免费方法性能接近最先进的监督模型。在RSITMD上平均召回率达到42.62%，是标准零样本CLIP基线（23.86%）的近两倍。

Conclusion: 通过结构化文本实现高质量语义表示，为遥感图像检索提供了强大且经济高效的范式。
...
Method: TCAM采用运动中心框架，通过运动场注意力机制自主观察视频、识别多个运动活动，并将自然语言描述空间地关联到对应的运动轨迹。核心是利用对比视觉-语言表示对齐的运动模式作为语义信号，通过结合全局视频-文本对齐和细粒度空间对应的统一训练，实现多运动表达的无查询发现。

Result: 在MeViS基准测试中，TCAM实现了58.4%的视频到文本检索准确率，64.9的JF空间定位分数，每个视频发现4.8个相关表达且精确度达84.7%，展示了强大的跨任务泛化能力。

Conclusion: TCAM通过运动模式与视觉-语言表示的对齐，为无查询视频理解提供了有效框架，在挑战性条件下能够自主发现和描述视频中的运动活动，具有实际应用价值。
...
TL;DR: 该论文提出了一种结合深度特征提取与可解释图像处理模块的深度学习架构，用于眼科疾病的自动诊断，通过视网膜血管分割作为辅助任务来提高模型的可解释性和临床部署可行性。


<details>
  <summary>Details</summary>
Motivation: 近年来威胁视力的眼病发病率急剧上升，需要可扩展且准确的筛查解决方案。标准卷积神经网络存在"黑盒"局限性，需要弥合算法输出与专家医学验证之间的差距。

Method: 实现了一个结合深度特征提取与可解释图像处理模块的管道，专注于高保真视网膜血管分割作为辅助任务来指导分类过程，将模型预测基于临床相关的形态学特征。

Result: 通过将模型预测基于临床相关的形态学特征，旨在减少假阳性并提高在临床环境中的部署可行性，从而弥合算法输出与专家医学验证之间的差距。

Conclusion: 该研究提出的方法通过结合深度学习和可解释的图像处理，为眼科疾病的自动诊断提供了一种更可靠、更易于临床验证的解决方案，有望改善临床部署的可行性。
...
TL;DR: Lang2Motion是一个通过将运动流形与联合嵌入空间对齐，实现语言引导点轨迹生成的框架。它从真实视频中提取点轨迹，使用基于transformer的自编码器学习轨迹表示，在文本到轨迹检索任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在人体运动或视频合成，缺乏为任意物体生成明确轨迹的方法。需要一种能够从语言描述生成物体运动轨迹的框架。

Method: 从真实视频中通过点跟踪提取运动轨迹，使用基于transformer的自编码器学习轨迹表示。通过双重监督：文本运动描述和渲染的轨迹可视化，两者都通过CLIP的冻结编码器映射。

Result: 在文本到轨迹检索任务上达到34.2%的Recall@1，比基于视频的方法高出12.5个百分点。运动精度比视频生成基线提高33-52%（12.4 ADE vs 18.3-25.3）。尽管只在多样物体运动上训练，但在人体动作识别上达到88.3%的Top-1准确率。

Conclusion: Lang2Motion成功实现了语言引导的轨迹生成，支持风格迁移、语义插值和潜在空间编辑。通过CLIP对齐的轨迹表示，展示了跨运动域的有效迁移能力。
...
TL;DR: DOCR-Inspector：一个基于VLM的文档解析质量评估系统，通过细粒度错误检测和28种错误类型分类，提供全面的文档解析质量分析。


<details>
  <summary>Details</summary>
Motivation: 当前文档解析任务中，标准基准测试存在数据集偏差，模型排名不一致，且总体评分掩盖了具体的错误模式，难以可靠评估真实场景下的解析质量。

Method: 提出DOCR-Inspector框架，使用VLM-as-a-Judge方法分析文档图像及其解析输出，识别所有错误并将其分类到28种预定义类型；构建DOCRcase-200K训练数据集，并提出Chain-of-Checklist推理范式支持层次化质量评估。

Result: 在882个真实文档案例的DOCRcaseBench基准上，DOCR-Inspector-7B超越了Gemini 2.5 Pro等商业模型和领先的开源模型；其质量评估能为解析结果优化提供有效指导。

Conclusion: DOCR-Inspector不仅是一个实用的文档解析质量评估工具，还能驱动文档解析系统的大规模改进，解决了真实场景下文档解析质量评估的可靠性问题。
...
TL;DR: K-Track是一个通用的跟踪加速框架，通过结合稀疏深度学习关键帧更新和轻量级卡尔曼滤波预测，实现5-10倍加速同时保持85%以上原始精度，使高性能点跟踪能在资源受限的边缘设备上部署。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习跟踪器在边缘设备上部署面临重大障碍，因为需要每帧GPU推理，而边缘设备的计算、功耗和连接都有限。需要一种方法在保持精度的同时大幅降低计算需求。

Method: 提出K-Track（卡尔曼增强跟踪）框架，采用混合策略：稀疏深度学习关键帧更新 + 轻量级卡尔曼滤波中间帧预测，使用贝叶斯不确定性传播保持时间一致性。该框架与具体跟踪器无关。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的进步使得伪造真实人物肖像变得容易，对安全、通信和公众信任构成严重威胁。需要不仅能区分伪造内容与真实媒体，还能提供清晰可靠推理的检测系统。

Method: 引入TriDF基准，包含来自先进合成模型的高质量伪造内容，涵盖16种DeepFake类型。基准评估三个关键方面：感知（使用人工标注证据识别细粒度伪造痕迹）、检测（评估跨不同伪造家族和生成器的分类性能）、幻觉（量化模型生成解释的可靠性）。
...
TL;DR: NaviHydra：一种从规则模拟器蒸馏的可控导航引导端到端模型，通过BEV轨迹收集和导航合规性评估，在NAVSIM基准测试中取得SOTA结果
...
Method: 提出NaviHydra框架：1) 接受高级导航指令作为控制信号；2) 使用基于BEV的轨迹收集方法增强轨迹特征提取；3) 引入新的导航合规性指标评估路线遵循度；4) 设计测试评估对各种导航指令的响应
...
TL;DR: XDen-1K是首个用于真实世界物理属性估计的大规模多模态数据集，专注于体积密度估计，包含1000个真实物体及其X射线扫描，可提升机器人操作和物理仿真的准确性。
...
Method: 1. 构建XDen-1K数据集：包含1000个真实物体，148个类别，提供高分辨率3D几何模型、部件级标注和双平面X射线扫描；2. 提出新颖的优化框架：从稀疏X射线视图恢复高保真体积密度场；3. 将X射线图像作为条件信号加入现有分割网络进行体积分割；4. 在下游机器人任务中进行实验验证。

Result: 实验表明，利用该数据集能有效提高质心估计的准确性和机器人操作的成功率。XDen-1K可作为基础资源和具有挑战性的新基准，推动物理基础视觉推理和具身AI的研究。

Conclusion: XDen-1K填补了真实世界物理属性估计数据集的空白，通过多模态数据和优化框架实现了高保真体积密度恢复，在机器人操作和物理仿真等应用中展现出实用价值，有望成为该领域的基础资源。
...
TL;DR: Geo6DPose：一种轻量级、完全本地化、无需训练的零样本6D物体姿态估计方法，通过几何过滤策略结合基础模型视觉特征，在单GPU上实现亚秒级推理，匹配更大模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前零样本6D姿态估计主要依赖大规模模型和云端推理，存在高延迟、高能耗、连接风险、成本高和数据治理等问题，与机器人实际部署中的有限计算资源和本地推理需求相冲突。

Method: 结合基础模型视觉特征与几何过滤策略：计算模板DINO描述符与场景块的相似性图，通过将场景块中心投影到3D空间、模板描述符投影到物体模型坐标系来建立相互对应关系，最后通过对应关系驱动的RANSAC恢复最终姿态，并使用加权几何对齐度量进行排序。

Result: 在单个商用GPU上实现亚秒级推理（1.08 FPS），平均召回率达到53.7 AR，与显著更大的零样本基线方法性能相当。无需训练、微调或网络访问，兼容不断演进的基础模型骨干网络。

Conclusion: Geo6DPose通过用几何可靠性换取模型规模，提供了一种实用、完全本地的6D感知解决方案，适用于机器人部署的实际约束条件，推进了无需训练、完全本地的6D姿态估计技术的发展。
...
TL;DR: 提出基于最优传输损失和迭代神经网络的SMLM新方法，无需NMS，支持端到端训练，在高密度发射下性能超越现有方法。
...
Method: 将SMLM训练目标重新表述为集合匹配问题，推导出最优传输损失函数，消除推理时对NMS的需求。提出集成显微镜光学系统知识的迭代神经网络。

Result: 在合成基准和真实生物数据上，新损失函数和架构在中等和高发射密度下均超越现有技术水平。

Conclusion: 提出的方法实现了端到端训练，无需NMS，在高密度发射条件下性能优越，有望改善SMLM的活细胞成像能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张图像进行逼真新视角合成的挑战，现有方法通常需要多视角输入或计算成本高

Method: 通过神经网络前向传播回归3D高斯表示参数，支持度量尺度相机运动，实现实时渲染

Result: 在多个数据集上实现SOTA，LPIPS降低25-34%，DISTS降低21-43%，合成时间降低三个数量级

Conclusion: SHARP实现了从单张图像快速生成度量3D高斯表示，支持实时逼真新视角合成，具有优秀的零样本泛化能力
...
TL;DR: 该研究提出了一种用于胸部X光解剖标志点分割的不确定性估计方法，通过变分潜在空间推导出潜在不确定性和预测不确定性两种互补度量，并发布了包含65.7万个分割标注及不确定性估计的大规模数据集CheXmask-U。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割系统的安全临床部署需要不确定性估计来识别不可靠预测并支持人工监督。虽然先前工作主要关注像素级不确定性，但基于标志点的分割具有固有的拓扑保证，却从未知性角度研究不足。本研究旨在探索胸部X光解剖标志点分割的不确定性估计。

Method: 采用混合神经网络架构，结合标准图像卷积编码器和基于图的生成解码器，利用其变分潜在空间推导两种互补的不确定性度量：(1) 潜在不确定性：直接从学习到的分布参数中捕获；(2) 预测不确定性：通过从潜在样本生成多个随机输出预测获得。

Result: 实验表明两种不确定性度量都随扰动严重程度增加而增加，反映了全局和局部退化。这些不确定性信号能够识别不可靠预测（与人工标注对比），并支持在CheXmask数据集上进行分布外检测。研究发布了CheXmask-U数据集，包含657,566个胸部X光标志点分割及每个节点的不确定性估计。

Conclusion: 不确定性估计是增强胸部X光解剖标志点分割方法鲁棒性和安全部署的有前景方向。发布的CheXmask-U数据集使研究人员能够在使这些解剖掩模时考虑分割质量的空间变化。
...
TL;DR: SpaceDrive是一个空间感知的视觉语言模型驾驶框架，通过将3D空间信息作为显式位置编码，而不是文本数字标记，来增强自动驾驶中的空间推理能力。
...
Conclusion: SpaceDrive通过将空间信息作为显式位置编码，显著提升了VLM在自动驾驶中的空间推理和轨迹规划能力，为解决VLM在细粒度3D空间理解方面的局限性提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频深度估计方法存在两个问题：1）逐帧单目模型导致时间不一致和不准确；2）基于时间建模的方法计算量大，不适合实时应用。这些限制影响了实际应用中的通用性和性能。

Method: 提出VeloDepth，一种高效鲁棒的在线视频深度估计流程。核心是传播模块，通过基于光流的扭曲和学习的残差校正来细化和传播深度特征和预测。结构上强制时间一致性，实现连续帧间的稳定深度预测。
...
TL;DR: IRMoGen提出了一种通过评估和精炼任务在运动理解和生成之间建立双向知识流动的新范式，IRG-MotionLLM模型通过迭代的文本-运动对话实现了运动生成、评估和精炼的紧密耦合。
...
Method: 提出IRMoGen范式，通过迭代文本-运动对话将运动生成与评估、精炼紧密耦合。开发IRG-MotionLLM模型，采用三阶段训练方案，并构建自动化数据引擎从现有文本-运动数据集中合成交错推理标注。

Result: 实验表明：1) 评估和精炼任务显著改善文本-运动对齐；2) 交错运动生成、评估和精炼步骤在各训练阶段均带来一致性能提升；3) IRG-MotionLLM明显优于基线模型，在标准文本到运动生成基准上达到先进性能。

Conclusion: 通过评估和精炼任务建立运动理解和生成之间的双向知识流动是有效的，IRMoGen范式显著提升了运动生成质量，IRG-MotionLLM模型在文本到运动生成任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动化息肉诊断报告存在不一致性和幻觉问题，主要原因是高质量多模态医学数据稀缺。需要开发更可靠、临床可行的解决方案来改善结肠镜检查的息肉诊断。

Method: 1) 构建MMEndo多模态内窥镜数据集；2) 基于Qwen2-VL-7B模型，采用LoRA参数高效微调和DPO直接偏好优化进行对齐；3) 在IU-XRay数据集上进行额外验证。

Result: LDP在自动指标和临床专家评估中均优于现有基线，获得7.2/10的医师评分。相比全微调，训练计算成本降低833倍，在IU-XRay数据集上也验证了其鲁棒性。

Conclusion: LDP框架为初级医疗保健提供了可扩展、临床可行的息肉诊断报告生成方案，通过高效微调和临床对齐，显著提升了诊断质量和计算效率。
...
TL;DR: 开发端到端管道，从冠状动脉CTA自动提取几何、生成模拟数据，并利用扩散回归模型直接预测冠状动脉血压，避免传统CFD计算，支持CAD诊断。


<details>
  <summary>Details</summary>
Motivation: 传统CFD模拟冠状动脉血流计算成本高、耗时，难以集成到大规模临床工作流，限制了AI模型的训练数据和基于生理学的CAD评估的广泛应用。

Method: 开发端到端管道自动化冠状动脉几何提取和模拟数据生成；引入基于扩散的回归模型，直接从CTA特征预测冠状动脉血压，绕过CFD计算。

Result: 在模拟冠状动脉血流动力学数据集上，模型达到R² 64.42%、RMSE 0.0974、归一化RMSE 0.154的SOTA性能，优于多个基线方法。

Conclusion: 该工作为快速、无创的血压预测提供了可扩展且易于使用的框架，支持CAD诊断，解决了传统CFD工作流的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 研究表示对齐（REPA）中目标表示的关键因素：是全局语义信息还是空间结构影响生成性能？传统观点认为更强的全局语义性能会带来更好的生成效果
...
Conclusion: 空间结构是表示对齐中影响生成性能的关键因素，iREPA的简单改进能有效提升训练效率，促使重新思考表示对齐的基本工作机制
...
<details>
  <summary>Details</summary>
Motivation: 前列腺癌全切片图像分级面临三大挑战：1) 图像尺度巨大；2) 组织结构异质性高；3) 难以选择诊断相关区域。现有方法采用随机或静态patch选择，导致冗余或非信息区域被包含，降低性能。

Method: 提出GLAT（图拉普拉斯注意力Transformer）结合IRM（迭代优化模块）。IRM使用预训练ResNet50提取局部特征，利用基础模型进行重要性评分，迭代优化patch选择。GLAT构建patch节点图，通过图拉普拉斯约束保证空间一致性，使用可学习滤波机制增强判别性组织特征。最后通过凸聚合机制动态调整patch重要性，生成稳健的WSI级表示。

Result: 在5个公共数据集和1个私有数据集上进行广泛实验，模型在性能和空间一致性方面均优于现有最优方法，同时保持计算效率。

Conclusion: 提出的GLAT+IRM模型通过迭代优化patch选择和图拉普拉斯注意力机制，有效解决了前列腺癌全切片图像分级中的关键挑战，实现了更高的性能和更好的空间一致性。
...
<details>
  <summary>Details</summary>
Motivation: 领域泛化(DG)遇到标签噪声时，噪声会加剧深层伪特征的放大，导致现有算法性能下降。需要让现有方法在遇到噪声时仍能有效工作。

Method: 提出自集成后学习(SEPL)方法：1) 特征探测训练：利用模型中间特征表示，训练多个探测分类器；2) 预测集成推理：通过众包推理方式集成不同分类器的预测结果。针对标签噪声，采用半监督算法训练探测分类器。

Result: 大量实验评估表明，该方法不仅增强了现有方法的鲁棒性，而且在真实世界应用中展现出高灵活性和显著潜力。

Conclusion: SEPL方法通过特征多样化和集成预测，有效解决了领域泛化中标签噪声导致的伪特征放大问题，提升了现有方法的噪声鲁棒性。
...
TL;DR: PoseGAM：一种无需显式特征匹配的几何感知多视角6D物体姿态估计框架，通过结合显式点云几何和几何表示网络学习特征，在未见物体上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计方法通常依赖显式构建查询图像与物体模型或模板图像之间的特征对应关系，这限制了在未见物体上的泛化能力。需要一种能够直接预测姿态而无需显式匹配的方法。

Method: 基于多视角基础模型架构，通过两种互补机制整合物体几何信息：1) 显式点云几何；2) 几何表示网络学习特征。构建包含超过19万个物体的大规模合成数据集以增强鲁棒性和泛化能力。

Result: 在多个基准测试中取得最先进性能，平均AR提升5.1%，在个别数据集上达到17.6%的增益，显示出对未见物体的强泛化能力。

Conclusion: PoseGAM通过几何感知多视角框架直接预测物体姿态，无需显式特征匹配，在未见物体姿态估计任务中表现出优异的性能和泛化能力。
...
TL;DR: SWiT-4D：一种基于滑动窗口Transformer的视频到4D网格生成方法，利用图像到3D生成先验，仅需少量视频微调即可实现高质量时空一致的4D重建。


<details>
  <summary>Details</summary>
Motivation: 单目视频到高质量动画3D资产（显式4D网格）的转换仍具挑战性。缺乏大规模自然捕获的4D网格数据集限制了纯数据驱动方法的训练。而图像到3D生成领域已有强大先验模型，可被有效利用。

Method: 提出SWiT-4D（滑动窗口Transformer），与任何基于DiT的图像到3D生成器无缝集成，通过时空建模跨视频帧处理，保持原始单图像前向过程。针对静态相机单目视频，引入基于优化的轨迹模块恢复全局平移。

Result: 仅需单个短视频（<10秒）微调即可实现高保真几何和稳定时间一致性。在领域内测试集和挑战性领域外基准（C4D、Objaverse和野外视频）上均优于现有基线，在时间平滑性方面表现突出。

Conclusion: SWiT-4D通过有效利用图像到3D生成先验，在极少4D监督下实现了高质量视频到4D网格转换，展示了实际部署的可行性，为4D内容生成提供了高效解决方案。
...
Method: 构建了包含1,106个问题的基准，基于1,278个视频片段（来自25个数据集和内部视频），采用四级框架（感知、规划、预测、跨视频推理），每个问题由3DV专家精心设计和审核。

Result: 评估25个开源和专有MLLMs，发现显著的人机差距：许多模型接近随机猜测，最佳推理模型落后人类近60%。空间微调模型在基准上泛化效果不佳，存在几何推理、运动理解、长时预测和跨视频对应等系统性失败。

Conclusion: 该基准为推进视频空间智能研究提供了坚实的测试平台，揭示了当前MLLMs在空间理解方面的严重不足，需要新的方法来解决几何推理、运动理解和长时预测等核心挑战。
...
TL;DR: 论文提出微观空间智能(MiSI)概念，并建立MiSI-Bench基准框架，包含16.3万问答对和58.7万图像，评估VLMs在微观空间推理能力。实验显示当前VLMs表现远低于人类，但微调7B模型在空间转换任务上超越人类，而在氢键识别等科学任务上表现不佳，表明需要领域知识整合。
...
Method: 提出MiSI-Bench基准框架，包含超过163,000个问答对和587,000张图像，源自约4,000个分子结构。涵盖9个互补任务，评估从基本空间转换到复杂关系识别的能力。使用最先进的VLMs进行实验，并对7B模型进行微调。

Result: 当前最先进的VLMs在此基准上表现显著低于人类水平。但微调的7B模型显示出巨大潜力，在空间转换任务上甚至超越人类。然而，在氢键识别等科学基础任务上表现较差，表明需要整合明确的领域知识。

Conclusion: 微观空间智能是科学发现的关键能力，MiSI-Bench为评估VLMs在此领域提供了系统基准。虽然微调模型在简单空间任务上表现良好，但在科学推理任务上仍需领域知识整合，这对实现科学AGI至关重要。数据集已公开可用。
...
TL;DR: MoCapAnything是一个类别无关的运动捕捉框架，能够从单目视频和任意绑定3D资产中重建驱动特定资产的旋转动画（如BVH），实现跨物种的运动重定向。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉流水线大多是特定物种或模板的，缺乏通用性。作者提出类别无关运动捕捉（CAMoCap）问题，旨在为任意3D资产生成可直接驱动的旋转动画。

Method: 提出参考引导的分解框架：1）参考提示编码器从资产骨架、网格和渲染图像提取关节查询；2）视频特征提取器计算密集视觉描述符并重建粗略4D变形网格；3）统一运动解码器融合线索生成时序一致的轨迹；4）轻量级约束感知逆向运动学恢复资产特定旋转。

Result: 在领域内基准测试和野外视频上，MoCapAnything能生成高质量骨骼动画，并在异构绑定间实现有意义的跨物种运动重定向。创建了包含1038个运动片段的Truebones Zoo数据集。

Conclusion: MoCapAnything实现了可扩展的、提示驱动的任意3D资产运动捕捉，填补了现有流水线的通用性空白，为内容创作提供了更灵活的工具。
...
<details>
  <summary>Details</summary>
Motivation: 表格提取是视觉文档理解的关键挑战，现有方法缺乏标注数据，难以评估直接提取表格的新方法（如视觉语言模型）。

Method: 创建大规模数据集PubTables-v2，支持多页表格结构识别等任务；基于该数据集开发POTATR模型，将Table Transformer扩展为图像到图的页面级表格提取方法。

Result: PubTables-v2成为首个大规模多页表格结构识别基准；评估了领域专用视觉语言模型；POTATR实现了全面的页面级表格提取。

Conclusion: PubTables-v2解决了表格提取领域的数据瓶颈，推动了多页表格结构识别等挑战性任务的发展，POTATR展示了页面级表格提取的有效性。
...
TL;DR: DuetSVG是一个统一的多模态模型，通过联合生成图像token和SVG token来提升SVG生成质量，并采用测试时缩放策略利用视觉预测指导SVG解码。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视觉语言模型的SVG生成方法仅生成文本，解码过程中缺乏视觉信号，导致在处理复杂语义时难以生成视觉吸引人或几何一致的SVGs。

Method: 提出DuetSVG统一多模态模型，以端到端方式联合生成图像token和对应的SVG token；在图像和SVG数据集上进行训练；推理时采用新颖的测试时缩放策略，利用模型自身的视觉预测作为指导来提升SVG解码质量。

Result: 大量实验表明该方法优于现有方法，能够生成视觉忠实、语义对齐且语法清晰的SVGs，在广泛的应用场景中表现优异。

Conclusion: DuetSVG通过联合多模态生成和测试时视觉指导，有效解决了现有VLM-based SVG生成方法的局限性，实现了高质量的SVG生成。
...
TL;DR: FoundationMotion：一个全自动数据构建流水线，通过检测视频中的物体轨迹，结合大语言模型生成细粒度运动描述和问答对，用于训练模型提升运动理解能力。
...
Method: 提出FoundationMotion全自动数据构建流水线：1）检测和跟踪视频中的物体以提取轨迹；2）利用轨迹和视频帧，结合大语言模型生成细粒度描述和多样化的运动与空间推理问答对。

Result: 使用该流水线构建的数据集微调开源模型（如NVILA-Video-15B和Qwen2.5-7B），在运动理解方面取得显著提升，且不影响其他任务性能。模型在多个运动理解数据集和基准测试中超越了Gemini-2.5 Flash和Qwen2.5-VL-72B等强大基线。

Conclusion: FoundationMotion提供了一个可扩展的解决方案，用于构建细粒度运动数据集，有效微调各种模型，显著提升运动理解和空间推理能力。
...
TL;DR: BabyVLM-V2是一个基于儿童发展轨迹的视觉语言模型框架，通过纵向多模态预训练数据集和认知评估工具包，实现了从零开始训练的小模型在多项任务上达到竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 早期儿童发展轨迹为视觉基础模型的高效预训练提供了自然目标。研究者希望建立一个发展合理、认知对齐的视觉语言模型框架，以加速发展合理的视觉基础模型预训练研究。

Method: 1) 构建纵向、多方面的预训练数据集，最大化覆盖婴儿中心的多模态语料；2) 开发DevCV Toolbox，将NIH Baby Toolbox的所有视觉相关测量指标转化为包含10个多模态任务的基准套件；3) 从零开始预训练紧凑模型。

Result: 从零开始预训练的紧凑模型在DevCV Toolbox上取得了竞争性表现，在某些任务上甚至超越了GPT-4o。该框架为发展合理的视觉基础模型预训练提供了统一、有原则的方法。

Conclusion: BabyVLM-V2框架通过发展合理的预训练和认知对齐评估，为视觉基础模型研究提供了加速发展的路径，展示了从儿童发展轨迹中学习的潜力。
...
TL;DR: Any4D是一个可扩展的多视角transformer，用于度量尺度、密集前馈4D重建，能够处理多种传感器模态，在精度和计算效率上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常专注于2视角密集场景流或稀疏3D点跟踪，缺乏能够处理多帧、多模态数据的统一4D重建框架。

Method: 采用模块化的4D场景表示：将每视角4D预测编码为以局部相机坐标表示的自我中心因子（深度图和相机内参）和以全局世界坐标表示的他者中心因子（相机外参和场景流）。使用可扩展的多视角transformer进行度量尺度的密集前馈4D重建。

Result: 在多种设置下取得优越性能：精度误差降低2-3倍，计算效率提高15倍，为多个下游应用开辟了新途径。

Conclusion: Any4D提供了一个灵活、高效的4D重建框架，能够处理多种传感器模态，在精度和速度上显著超越现有方法，具有广泛的应用前景。
...
TL;DR: 提出一种基于3D Morphable Models映射高斯泼溅的单目视频说话头生成方法，通过transformer从音频预测模型参数，实现实时且时间一致的高质量说话头视频


<details>
  <summary>Details</summary>
Motivation: 现有说话头生成方法存在速度与质量的权衡：扩散方法虽真实但速度慢，高斯泼溅方法实时但面部跟踪不准确、高斯映射不一致，导致输出不稳定和视频伪影，影响实际应用

Method: 使用3D Morphable Models映射高斯泼溅生成个性化头像，引入基于transformer的模型参数预测，直接从音频驱动时间一致性，仅需单目视频和独立音频输入

Result: 方法能够生成实时说话头视频，在定量和定性评估中均表现出竞争力，解决了现有方法的速度与质量矛盾

Conclusion: 通过结合3D Morphable Models和高斯泼溅，并利用transformer从音频预测参数，实现了实时、时间一致且高质量的说话头生成，为实际应用提供了可行方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法针对特定4D一致性任务（如新颖视角合成、文本到视频等）分别训练，导致模型分散且数据利用不充分。需要一个统一框架来整合这些任务。

Method: OmniView将空间、时间和视角条件分别表示，允许这些输入的灵活组合。通过统一的扩散模型架构处理多种4D任务，包括静态/动态/多视角输入的新颖视角合成、时间轨迹外推、文本/图像到视频生成等。

Result: 在多个基准测试中表现优异：多视角NVS LLFF数据集图像质量提升33%，动态NVS Neural 3D Video基准提升60%，静态相机控制RE-10K提升20%，文本条件视频生成的相机轨迹误差减少4倍。

Conclusion: OmniView展示了通用4D视频模型的可行性，通过单一模型实现强大的泛化能力，为4D内容生成提供了统一解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要超越语言的多种模态（空间、时间、功能等），现有多模态模型存在脆弱性、扩展性差、依赖专业工具或人工数据等问题，需要更简单的解决方案。
...
Conclusion: Mull-Tokens为多模态抽象推理提供了一个简单有效的解决方案，有助于解决文本和视觉推理中的基础挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（VLM）采用自回归token生成方式，存在计算效率低、参数冗余的问题。VL-JEPA旨在通过预测连续嵌入而非生成token，在抽象表示空间中学习任务相关语义，同时抽象掉表面语言变异性，提高模型效率和性能。

Method: VL-JEPA基于联合嵌入预测架构（JEPA），预测目标文本的连续嵌入而非生成离散token。模型在抽象表示空间中学习，专注于任务相关语义。推理时仅在需要时调用轻量级文本解码器将预测嵌入转换为文本。支持选择性解码，减少解码操作。

Result: 在相同视觉编码器和训练数据条件下，VL-JEPA比标准token空间VLM训练性能更强，参数量减少50%。选择性解码将解码操作减少2.85倍。在8个视频分类和8个视频检索数据集上平均性能超过CLIP、SigLIP2和Perception Encoder。在4个VQA数据集上（GQA、TallyQA、POPE、POPEv2）与经典VLM（InstructBLIP、QwenVL）性能相当，仅使用1.6B参数。

Conclusion: VL-JEPA通过预测连续嵌入而非生成token，在减少参数量的同时实现了更强的性能。该架构不仅支持生成任务，还自然地支持开放词汇分类、文本到视频检索和判别式VQA等多种任务，展示了联合嵌入预测架构在视觉语言建模中的潜力。
...
Result: AlcheMinT在视觉质量上与最先进的视频个性化方法相匹配，同时首次实现了视频中多主题生成的精确时间控制，在主题身份保持、视频保真度和时间一致性方面表现出色。
...
TL;DR: 提出MeViS大规模多模态数据集，专注于基于物体运动语言描述的视频分割与跟踪，填补现有数据集对运动信息强调不足的空白


<details>
  <summary>Details</summary>
Motivation: 现有指代视频分割数据集多关注静态属性，可在单帧识别目标物体，忽视了视频和语言中运动信息的重要性，需要探索运动表达和运动推理线索在像素级视频理解中的可行性

Method: 构建包含33,072个人工标注运动表达（文本和音频）的MeViS数据集，涵盖2,006个复杂场景视频中的8,171个物体；提出LMPM++方法用于RVOS/AVOS/RMOT任务

Result: 在MeViS数据集上评估15种现有方法，发现现有方法在处理运动表达引导的视频理解方面存在局限；提出的LMPM++方法在RVOS/AVOS/RMOT任务上达到新的SOTA结果

Conclusion: MeViS数据集为复杂视频场景中运动表达引导的视频理解算法开发提供了平台，填补了现有数据集对运动信息强调不足的空白，推动了该领域的发展
...
TL;DR: Flex是一种高效场景编码器，使用少量可学习场景token联合编码多摄像头多时间步信息，无需3D先验，显著提升自动驾驶系统效率与性能。
...
Method: 使用少量可学习场景token联合编码所有摄像头和时间步的图像token信息，采用几何无关设计，直接从数据中学习紧凑场景表示，无需显式3D归纳偏置。

Result: 在2万小时专有数据集上评估，相比SOTA方法实现2.2倍推理吞吐量提升，驾驶性能大幅改善，且场景token展现出无监督场景分解能力。
...
TL;DR: ClusIR是一个基于聚类引导的图像恢复框架，通过可学习聚类显式建模退化语义，在空间和频域传播聚类感知线索，实现自适应恢复。


<details>
  <summary>Details</summary>
Motivation: 现有All-in-One图像恢复方法未能显式建模退化类型，且难以适应复杂或混合退化场景。需要一种能够明确感知退化语义并自适应调整恢复行为的框架。

Method: 提出ClusIR框架，包含两个核心组件：1) 概率聚类引导路由机制(PCGRM)，将退化识别与专家激活解耦，实现区分性退化感知和稳定专家路由；2) 退化感知频率调制模块(DAFMM)，利用聚类引导先验进行自适应频率分解和针对性调制，协同优化结构和纹理表示。

Result: 在多个基准测试上的广泛实验验证了ClusIR在多种场景下达到竞争性性能，能够获得显著的恢复结果。

Conclusion: ClusIR通过聚类引导的协同作用，将语义线索与频域调制无缝连接，实现了对各种退化类型的自适应高质量图像恢复。
...
TL;DR: E-RayZer是一种自监督大型3D视觉模型，直接从无标签图像学习真正3D感知表示，通过显式几何进行自监督3D重建，优于现有方法
...
Conclusion: E-RayZer建立了3D感知视觉预训练的新范式，通过显式几何的自监督学习获得真正3D感知表示，在多个3D任务上表现出色
...
TL;DR: GroupDiff：通过跨样本注意力机制实现图像协同生成，在推理时让多张图像联合去噪，显著提升生成质量


<details>
  <summary>Details</summary>
Motivation: 探索扩散模型推理中未被利用的信号：传统方法在推理时独立生成图像，本文研究是否能让样本协同生成，利用跨样本的关联性提升生成效果

Method: 提出Group Diffusion方法，解锁注意力机制使其能在图像之间共享，而不仅限于单个图像内的patch。在推理时让图像联合去噪，学习图像内和图像间的对应关系

Result: 观察到明显的缩放效应——更大的组规模产生更强的跨样本注意力和更好的生成质量。在ImageNet-256x256上实现最高32.2%的FID改进，且提出的定性测量方法与FID高度相关

Conclusion: 跨样本推理是生成建模中一个有效且先前未被探索的机制，GroupDiff展示了通过协同生成可以显著提升扩散模型的性能
...
TL;DR: 提出Omni-Attribute开放词汇图像属性编码器，通过语义链接的图像对和双目标训练范式，实现高保真、属性特定的表示学习，解决现有方法中视觉因素纠缠和信息泄漏问题。
...
TL;DR: StereoWalker提出在机器人导航基础模型中引入立体视觉和显式中层视觉模块（深度估计、像素跟踪），解决了单目视觉的深度尺度模糊问题，显著减少训练数据需求并提升导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于单目视觉的导航基础模型（NFMs）完全依赖端到端学习，忽略了中层视觉模块（如深度估计、跟踪等）。这种方法需要大量像素到动作的监督数据，且在动态和非结构化环境中，单目视觉存在深度尺度模糊问题，限制了准确的空间推理能力。

Method: StereoWalker通过两个关键改进：1）使用立体视觉输入解决深度尺度模糊问题；2）引入显式中层视觉模块（深度估计和密集像素跟踪），提供可靠的几何和运动结构信息。同时构建了一个大型立体导航数据集，包含从互联网立体视频中自动标注的动作数据。

Result: 实验表明，StereoWalker仅使用1.5%的训练数据就能达到当前最佳性能，使用全部数据时性能超过现有方法。立体视觉相比单目视觉能带来更高的导航性能，中层视觉模块显著提升了数据效率。

Conclusion: 在机器人导航基础模型中忽略中层视觉先验是低效的。通过引入立体视觉和显式中层视觉模块，可以解决单目视觉的深度尺度模糊问题，显著减少训练数据需求并提升导航性能，特别是在动态和非结构化环境中。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在严重遮挡和开放集场景下，由于缺乏足够的开放集去遮挡和姿态估计先验知识，难以同时生成高质量的几何结构和准确的对象姿态。

Method: 1) 将去遮挡模型与3D物体生成解耦，利用图像数据集和收集的去遮挡数据集增强对多样化开放集遮挡模式的处理能力；2) 提出统一姿态估计模型，整合全局和局部机制的自注意力和交叉注意力以提高准确性；3) 构建开放集3D场景数据集以扩展姿态估计模型的泛化能力。
...
TL;DR: WorldLens是一个用于评估生成世界模型的全方位基准测试，涵盖生成质量、重建能力、动作跟随、下游任务和人类偏好五个维度，旨在衡量模型构建、理解和在其生成世界中行为的能力。


<details>
  <summary>Details</summary>
Motivation: 当前生成世界模型虽然能合成逼真的4D驾驶环境，但往往在物理或行为上存在缺陷。领域缺乏统一的方法来评估生成世界是否保持几何一致性、遵守物理规律或支持可靠控制。

Method: 提出WorldLens基准测试，涵盖五个方面：生成、重建、动作跟随、下游任务和人类偏好。构建WorldLens-26K大规模人工标注视频数据集，包含数值评分和文本理由。开发WorldLens-Agent评估模型，从人工标注中蒸馏学习，实现可扩展、可解释的评分。

Result: 现有世界模型在五个维度上表现不均衡：纹理强的模型常违反物理规律，几何稳定的模型缺乏行为保真度。WorldLens基准测试揭示了当前模型的局限性，并提供了统一的评估生态系统。

Conclusion: WorldLens基准测试、数据集和评估模型构成了一个统一的生态系统，用于衡量世界保真度，标准化未来模型的评估标准——不仅要看它们看起来有多真实，还要看它们的行为有多真实。
...
TL;DR: StereoSpace是一个基于扩散模型的单目到立体合成框架，通过视角条件建模几何，无需显式深度或变形，在端到端协议下在感知舒适度和几何一致性方面超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单目到立体合成方法通常依赖显式深度估计或变形操作，这可能导致几何不准确和伪影。本文旨在探索一种不依赖深度或变形的纯视角条件化方法，实现更鲁棒的立体生成。

Method: 提出StereoSpace框架，基于扩散模型，通过视角条件化建模几何关系，使用规范矫正空间和条件引导生成器端到端推断对应关系并填充遮挡区域。引入端到端评估协议，排除测试时的真实或代理几何估计。

Result: StereoSpace在变形填充、潜在变形和变形条件化三类方法中表现最佳，在分层和非朗伯场景上实现锐利的视差和强鲁棒性。在iSQoE（感知舒适度）和MEt3R（几何一致性）指标上超越现有方法。

Conclusion: 视角条件化扩散模型为立体生成提供了一个可扩展、无需深度的解决方案，证明了纯视角条件化在几何建模中的有效性，为单目到立体合成开辟了新方向。
...
TL;DR: 本文分析基于token补全的大语言模型的推理机制，认为其本质是随机生成而非真正的溯因推理，只是模仿人类文本中的推理模式，具有表面溯因但缺乏真实理解的特点。


<details>
  <summary>Details</summary>
Motivation: 研究当前基于token补全的大语言模型的推理本质，澄清其看似溯因推理的表现实际上只是对训练数据中人类推理模式的模仿，而非真正的逻辑推理能力。

Method: 通过分析LLMs的随机性本质和与人类溯因推理的相似性，使用具体例子展示模型如何生成看似合理的思想、模仿常识推理、提供解释性答案，但缺乏真实基础。

Result: LLMs基于学习到的文本模式生成内容，表面上的溯因推理表现主要源于训练数据包含的人类推理结构，模型本身无法识别真理、验证解释或进行真正的理解。

Conclusion: LLMs具有随机基础但表面溯因的双重特性，可用于辅助创意生成和人类思考，但其输出需要批判性评估，因为它们无法进行真正的溯因推理或真理验证。
...
TL;DR: 使用小型语言模型(SLMs)进行自动问题生成，通过"生成-验证"策略和概率推理能力来生成高质量问题，补充大型语言模型在学习分析中的应用。


<details>
  <summary>Details</summary>
Motivation: 探索小型语言模型(SLMs)作为大型语言模型(LLMs)的补充，用于学习分析中的自动问题生成，以利用SLMs的文本生成和概率推理能力。

Method: 提出新颖的问题生成流程，采用"生成-验证"策略：首先进行扩展性生成创建大量候选问题，然后通过基于概率推理的选择性验证进行精炼。

Result: 通过人类专家和大型语言模型(LLM)的评估，大多数评判者认为生成的问题具有明确答案且与学习目标良好对齐，表明SLMs能够生成高质量问题。

Conclusion: 当通过精心设计的流程引导并充分利用其优势时，小型语言模型(SLMs)能够有效生成高质量问题，为学习分析研究提供了大型语言模型的有力补充。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在垂直领域长文本生成中面临"不可能三角"瓶颈：难以同时实现低幻觉、深度逻辑连贯性和个性化表达。研究发现这是由于现有生成范式陷入了"统计平滑陷阱"，忽略了专家级写作所需的高熵信息获取和结构化认知过程。

Method: 提出DeepNews框架，模拟资深财经记者的隐式认知过程，包含三个核心模块：1)基于信息觅食理论的双粒度检索机制，强制10:1饱和信息输入比；2)基于领域专家知识库(叙事模式)和原子块的模式引导战略规划；3)使用节奏打破和逻辑迷雾等对抗性约束提示技术来破坏模型生成文本的概率平滑性。

Result: 实验发现深度财经报道中存在明显的"知识悬崖"：当检索上下文低于15,000字符时内容真实性崩溃，而超过30,000字符的高冗余输入可将无幻觉率稳定在85%以上。在一家顶级中国科技媒体的生态效度盲测中，基于上一代模型的DeepNews系统获得了25%的投稿接受率，显著优于GPT-5零样本生成的0%接受率。

Conclusion: DeepNews框架通过显式建模专家认知过程，有效解决了长文本生成中的"不可能三角"问题，证明了结构化认知工作流在垂直领域专业内容生成中的重要性，为突破大语言模型的统计平滑陷阱提供了可行路径。
...
TL;DR: 提出兩階段提示框架，從短評文本推斷用戶顯性與隱性特徵，並融入回應生成提示以產生個性化回覆，無需微調模型
...
Method: 兩階段提示框架：1) 從短評推斷顯性（用戶陳述偏好）與隱性（人口統計或風格線索）特徵 2) 將推斷特徵融入回應生成提示；調整解碼溫度以鼓勵多樣且忠實的生成

Result: 使用韓國外送App真實數據集評估，在精確度、多樣性與語義一致性方面顯示特徵增強提示能有效提升回應相關性與個性化

Conclusion: 特徵增強提示能有效提升自動回應的相關性與個性化，無需模型微調，在用戶信息有限的情境下特別有價值
...
TL;DR: 研究大语言模型在适应新任务时的安全性退化问题，将安全保护视为持续学习问题，评估多种CL方法在缓解安全退化方面的效果


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，其安全对齐变得越来越重要。研究发现模型在适应新任务时会出现安全性退化，这主要归因于灾难性遗忘。研究旨在解决在微调过程中如何保持模型安全性的问题。
...
Result: 持续学习方法相比标准微调始终获得更低的攻击成功率。其中DER方法在保持任务效用的同时，优于其他CL方法和现有的安全保护基线方法。这些发现在不同任务和模型家族中具有普遍性。

Conclusion: 持续学习是保护大语言模型在微调过程中安全性的实用解决方案，特别是在微调即服务的场景下，能够有效缓解安全退化问题。
...
TL;DR: AutoMedic是一个多智能体仿真框架，用于自动评估LLM作为临床对话代理的性能，通过将静态QA数据集转换为虚拟患者档案，实现临床多轮对话，并使用CARE指标进行多方面评估。


<details>
  <summary>Details</summary>
Motivation: 当前医学领域LLM评估存在不足：1）现有静态QA基准无法评估动态交互式临床对话场景；2）缺乏超越简单准确性的多方面评估策略；3）动态临床场景的评估因患者状态和交互轨迹的组合空间巨大而难以标准化和量化。

Method: 提出AutoMedic多智能体仿真框架：1）将现成的静态QA数据集转换为虚拟患者档案；2）在LLM智能体之间实现真实、临床基础的多轮临床对话；3）使用CARE指标（临床对话准确性、效率/策略、同理心、鲁棒性）进行多方面评估。

Result: AutoMedic被证明是临床对话代理的有效自动评估框架，经人类专家验证有效，为LLM在对话式医学应用中的有效开发提供了实用指南。

Conclusion: AutoMedic解决了临床对话场景中LLM评估的标准化和量化难题，通过多智能体仿真和CARE指标实现了全面、自动化的评估，有助于推动LLM在医学对话应用中的安全可信发展。
...
TL;DR: 本文研究了将英语训练的视觉语言模型适配到其他语言的方法，发现数据集翻译是性能提升的主要瓶颈，建议未来应关注原生语言数据收集和改进翻译策略。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要基于英语开发，限制了非英语用户的使用。需要扩展这些能力到更广泛的语言范围，提高模型的多语言可访问性。

Method: 探索并比较了三种适配方法：基于翻译的流水线、LoRA微调、以及将视觉适配与语言适配分离的两阶段微调策略。使用翻译后的多模态基准测试和母语专家手动评估进行验证。

Result: 研究发现数据集翻译是多语言VLM性能的主要瓶颈，数据质量限制了训练和评估的效果。翻译质量直接影响模型性能。
...
TL;DR: Confucius Code Agent (CCA) 是一个开源AI软件工程师，在工业规模任务上表现出色，在SWE-Bench-Pro上达到54.3%的Resolve@1性能，基于Confucius SDK平台构建，提供分层工作记忆、持久笔记系统和模块化工具扩展。
...
Method: 基于Confucius SDK平台构建，包含：1) 统一编排器与分层工作记忆用于长上下文推理；2) 持久笔记系统用于跨会话持续学习；3) 模块化扩展模块用于鲁棒工具使用；4) 元代理通过构建-测试-改进循环自动合成、评估和优化代理配置。

Result: 在SWE-Bench-Pro上达到54.3%的Resolve@1性能，显著优于之前的编码代理，在真实世界软件工程任务上表现出色。
...
TL;DR: SWAA提出五种方法组合，使全注意力预训练的LLM能适应滑动窗口注意力，解决长上下文推理成本问题，无需重新预训练。


<details>
  <summary>Details</summary>
Motivation: Transformer LLM的自注意力机制在输入长度上呈二次方复杂度，使得长上下文推理成本高昂。滑动窗口注意力(SWA)将复杂度降至线性，但直接将全注意力(FA)预训练的模型切换到SWA会导致严重的性能下降，因为存在训练-推理不匹配问题。

Method: 提出滑动窗口注意力适应(SWAA)，包含五种方法的组合：1)仅在预填充阶段应用SWA；2)保留"sink"令牌；3)交错FA/SWA层；4)思维链(CoT)；5)微调。

Result: 实验表明SWA适应是可行但非平凡的：没有单一方法足够，但特定的协同组合能有效恢复原始长上下文性能。分析了不同SWAA配置的性能-效率权衡，并为不同场景提供推荐方案。

Conclusion: SWAA提供了一套实用的方法组合，使FA预训练的LLM能够适应SWA，在保持性能的同时显著降低长上下文推理成本，无需重新预训练。
...
TL;DR: CoopRAG是一种新颖的检索增强生成框架，通过检索器和LLM的协同工作以及检索器模型不同层之间的协同，显著提升了问答任务的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生事实不准确的输出，现有的RAG方法在简单和多跳问答任务中仍然存在检索错误和幻觉问题，需要更有效的解决方案。

Method: 提出CoopRAG框架：1)将问题分解为子问题和推理链并掩码不确定位置；2)基于增强信息检索相关文档；3)通过对比检索器不同层重新排序文档；4)利用LLM填充掩码位置重建推理链。

Result: CoopRAG在三个多跳问答数据集和一个简单问答数据集上，在检索和问答性能方面均优于最先进的QA方法。

Conclusion: CoopRAG通过检索器和LLM的协同工作以及检索器模型内部的协同，有效解决了现有RAG方法的局限性，为问答任务提供了更可靠的解决方案。
...
TL;DR: T-pro 2.0 是一个开源的俄语大语言模型，支持混合推理和高效推理，包含模型权重、指令数据集、推理基准和优化推理管道等完整资源。


<details>
  <summary>Details</summary>
Motivation: 为俄语社区提供一个可复现、可扩展的开源LLM系统，支持俄语推理研究，并解决推理延迟问题，促进俄语LLM应用的开发与评估。

Method: 使用西里尔字母密集的分词器，采用适配的EAGLE推测解码管道来降低延迟，支持直接回答和推理轨迹生成两种模式。

Result: 发布了完整的开源资源包：模型权重、T-Wix 500k指令数据集、T-Math推理基准和EAGLE权重，并提供了公开网页演示展示推理加速效果。

Conclusion: T-pro 2.0 作为一个可访问的开源系统，为构建和评估高效实用的俄语LLM应用提供了基础。
...
TL;DR: SRAP框架通过两阶段方法检测并恢复对抗性剽窃中的"扭曲短语"，使用统计异常检测和语义重建技术，在科学文本中实现23.67%的恢复准确率。
...
Result: 在对抗性科学文本平行语料上的实验显示：零基线方法完全失败(0.00%恢复准确率)，而检索增强方法达到23.67%恢复准确率，显著优于基线方法。静态决策边界在术语密集的科学文本中更稳健。

Conclusion: SRAP框架不仅能检测对抗性剽窃异常，还能数学重建原始术语，通过将混淆表达链接回最可能的源文档实现法证分析，为科学文献完整性提供有效保护。
...
TL;DR: 该论文提出通过知识图谱增强大语言模型，使用KG-BERT方法解决LLMs缺乏结构化知识导致事实不一致的问题，在知识密集型任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型（如Claude、Mistral IA、GPT-4）在NLP任务上表现出色，但缺乏结构化知识，导致事实不一致和可靠性问题。需要增强LLMs的接地性和推理能力。

Method: 通过集成知识图谱（KGs）来增强LLMs，采用KG-BERT方法将结构化知识融入模型，提升模型的接地性和推理能力。

Result: 实验表明，该方法在知识密集型任务（如问答和实体链接）上取得显著性能提升，改善了事实可靠性。

Conclusion: 知识图谱集成能有效提升大语言模型的事实可靠性和推理能力，为开发更具上下文感知能力的下一代LLMs提供了可行路径。
...
TL;DR: 开发了一个结合LLM、知识图谱增强BERT和双向LSTM的心理学感知对话代理，通过多模态数据实时分类学生认知和情感状态，在教育环境中提升学习表现和情绪健康


<details>
  <summary>Details</summary>
Motivation: 现有教育聊天机器人通常只专注于辅导或情感支持中的单一功能，缺乏能够同时处理认知和情感状态的综合系统。需要开发一个能够实时识别学生认知和情感状态、并提供适应性支持的智能对话代理

Method: 结合大型语言模型(LLMs)、知识图谱增强BERT(KG-BERT)和带注意力的双向LSTM，利用文本语义、语音韵律特征和时间行为趋势等多模态数据，实时分类学生的参与度、压力和概念理解状态

Result: 大学学生的试点研究表明，相比基线方法，该系统提高了学习动机、降低了压力水平，并取得了适度的学业成绩提升

Conclusion: 整合语义推理、多模态融合和时间建模的方法，在支持适应性、以学生为中心的教育干预方面具有良好前景，能够同时提升学习表现和情绪健康
...
TL;DR: LLMs通过表面形式训练后，能够可靠地区分语法正确和错误的句子变体，显示出对句法结构的敏感性，而不仅仅是线性顺序。


<details>
  <summary>Details</summary>
Motivation: 传统生成语法中，语法性对比（如主助倒装和寄生空位许可）被视为内部层级语法的证据。本研究旨在测试仅基于表面形式训练的大语言模型（LLMs）是否以暗示底层结构表征的方式再现这些对比。

Method: 聚焦两个经典结构：主助倒装（测试主语边界识别）和寄生空位许可（测试抽象依赖结构）。使用GPT-4和LLaMA-3等模型，通过提示引出可接受性评分进行评估。

Result: LLMs可靠地区分了两种结构中语法正确和错误的变体，表明它们对结构敏感，而不仅仅是线性顺序。结构概括（区别于认知知识）从表面形式的预测训练中涌现。

Conclusion: 大语言模型通过表面形式的预测训练，表现出对句法的功能敏感性，无需显式编码。这表明结构表征可以从语言数据中涌现，而不仅仅是线性模式学习。
...
Method: 扩展DoGE算法为XDoGE用于多语言设置，通过小型代理模型优化语言权重，然后使用这些权重从头训练或持续预训练全尺寸模型

Result: 实验了六种语言（英语、西班牙语、葡萄牙语、加泰罗尼亚语、加利西亚语、巴斯克语），研究了数据重复和欠采样效果，并发布了IberianLLM-7B-Instruct模型

Conclusion: XDoGE方法能有效优化多语言LLM的语言分布，提升中低资源语言性能，新发布的IberianLLM-7B-Instruct模型在伊比利亚语言上表现优异
...
TL;DR: 本文研究了不同架构大语言模型在因果推理任务中的表现，发现ICL单独使用不可靠，微调的编码器和编码器-解码器架构在成本效益和鲁棒性方面优于仅解码器模型


<details>
  <summary>Details</summary>
Motivation: 研究上下文学习（ICL）在大语言模型因果推理中的作用和性能，因果推理需要多跳组合和严格合取控制，而依赖输入中的虚假词汇关系可能导致误导性结果

Method: 比较微调版本的编码器、编码器-解码器和仅解码器架构在零样本和少样本ICL下的表现，包括自然语言和非自然语言场景

Result: ICL单独使用对可靠因果推理不足，常过度关注无关输入特征；仅解码器模型对分布偏移脆弱，而微调的编码器和编码器-解码器模型能更鲁棒地泛化；仅解码器架构需要大规模才能匹配或超越前两者

Conclusion: 对于成本效益高、短期鲁棒的因果推理，具有针对性微调的编码器或编码器-解码器架构更优
...
TL;DR: 该论文提出了RoleRMBench——首个角色扮演对话奖励建模的基准，并开发了RoleRM模型，通过连续隐式偏好方法显著提升了主观领域奖励建模的性能。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型在处理主观和开放式领域（如角色扮演）时表现严重退化，难以捕捉基于角色的细微人类判断，需要专门针对角色扮演对话的奖励建模基准和方法。

Method: 1. 创建RoleRMBench基准，涵盖叙事管理、角色一致性、参与度等七个细粒度能力；2. 提出RoleRM模型，采用连续隐式偏好方法，将主观评估重构为多结构化策略下的连续一致成对监督。

Result: RoleRM在RoleRMBench上平均超越强开源和闭源奖励模型超过24%，在叙事连贯性和风格保真度方面取得显著提升，揭示了连续偏好表示和标注一致性的重要性。

Conclusion: 该研究强调了连续偏好表示和标注一致性在主观对齐中的重要性，为人本对话系统中的主观对齐奠定了基础，展示了专门针对角色扮演的奖励建模方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型快速发展，但农业应用仍受限于多语言语音数据缺乏、统一多模态架构缺失以及全面评估基准不足的问题。

Method: 1) 构建可扩展的数据合成与收集管道，创建最大的农业语音数据集（49.2万合成+1.4万真实样本，覆盖6种语言）；2) 采用三阶段训练范式：文本知识注入、渐进式多模态对齐、基于GRPO的强化学习；3) 提出首个农业三模态基准AgriBench-Omni-2K。

Result: AgriGPT-Omni在多语言多模态推理以及真实世界语音理解任务上显著优于通用基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型主要基于英语等主导语言训练，对低资源语言的表示反映了源语言材料的文化和语言偏见。研究以塞尔维亚语为案例，旨在揭示影响低资源语言技术发展的结构、历史和社会技术因素。

Method: 采用半结构化访谈方法，采访了10位学者和从业者，包括语言学家、数字人文主义者和AI开发者。通过分析塞尔维亚文本遗产的历史破坏和当代问题，识别出技术开发中的挑战。

Result: 研究发现塞尔维亚语面临的主要挑战包括：历史文本遗产的破坏、浅层音译、依赖英语训练模型、数据偏见以及缺乏文化特异性的数据集管理。这些问题导致工程优先方法牺牲了语言细微差别。

Conclusion: 研究提出Data Care框架，基于CARE原则（集体利益、控制权、责任和伦理），将偏见缓解从技术修复转变为语料库设计、标注和治理的组成部分，为在传统LLM开发复制权力不平衡和文化盲点的背景下构建包容、可持续和文化基础的语言技术提供可复制模型。
...
TL;DR: 提出一个全面的数据偏见检测与缓解管道，包含四个组件，用于处理表示偏见和刻板印象两种偏见类型，并通过人类验证和模型微调评估效果。
...
Method: 提出四组件管道：1) 基于质量标准的LLM生成相关群体标签词表；2) 使用人口统计表示分数量化表示偏见；3) 使用社会语言学过滤检测和缓解刻板印象；4) 通过语法和上下文感知的反事实数据增强补偿表示偏见。

Result: 在性别、宗教和年龄等示例上评估：1) 人类验证显示成功减少文本数据集中的表示偏见和刻板印象；2) 在偏见数据集上微调的LLM（0.6B-8B参数）在偏见基准测试中并未一致显示改进性能，暴露了当前评估方法的差距。

Conclusion: 虽然数据去偏见方法有效减少了数据偏见，但微调后的模型在偏见基准测试中表现不一致，表明需要更有针对性的数据操作来解决模型偏见，并改进评估方法。
...
TL;DR: OPV（基于结果的流程验证器）通过验证长推理链的总结结果来准确高效地检测错误，采用主动学习框架降低标注成本，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于结果的验证器无法检查长推理链中的中间步骤错误，而基于流程的验证器由于高质量标注数据稀缺且人工标注成本高昂，难以可靠检测复杂长推理链中的错误。

Method: 提出OPV验证器，验证长推理链的总结结果而非完整流程；采用迭代主动学习框架，通过专家标注最不确定的案例，使用拒绝微调（RFT）和RLVR训练新OPV。

Result: 在held-out基准测试中F1分数达83.1，优于Qwen3-Max-Preview的76.3；能有效检测合成数据集中的假阳性；与策略模型协作时显著提升性能，如将DeepSeek-R1-Distill-Qwen-32B在AIME2025上的准确率从55.2%提升至73.3%。

Conclusion: OPV通过验证总结结果而非完整流程，实现了准确高效的验证，降低了标注成本，在多个任务中表现出优越性能和广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有紧急语音识别系统对非标准英语变体（如加勒比口音）存在系统性性能下降，导致加勒比人群在紧急服务中存在关键差距，需要开发口音弹性的紧急AI系统。

Method: 三层架构：1) 加勒比口音优化的自动语音识别；2) 通过大语言模型进行本地实体提取；3) 生物声学痛苦检测。系统为调度员提供三个互补信号：转录置信度、结构化临床实体和声音压力指标。

Result: 建立了口音弹性紧急AI框架，将低ASR置信度重新定义为有价值的队列优先级信号（特别是与升高的声音痛苦标记结合时），确保即使ASR失败也能应用ESI和START分诊协议。

Conclusion: TRIDENT为加勒比口音人群提供了公平获得国家分诊协议的途径，将低ASR置信度转化为有用的调度信号，并考虑了灾难场景下的离线操作需求，但需要在实际加勒比紧急呼叫上进行实证验证。
...
TL;DR: 提出OPV验证器，通过总结长推理链的结果来验证推理过程，结合主动学习框架，在减少标注成本的同时实现准确高效的验证。


<details>
  <summary>Details</summary>
Motivation: 当前基于结果的验证器无法检查长推理链中的不可靠中间步骤，而基于过程的验证器由于高质量标注稀缺且成本高昂，难以可靠检测复杂长推理链中的错误。

Method: 提出基于结果的流程验证器OPV，通过总结长推理链的结果来验证推理过程。采用迭代主动学习框架，通过专家标注最不确定的案例，使用拒绝微调和RLVR训练新的OPV。

Result: OPV在OPV-Bench上取得SOTA结果（F1 83.1 vs Qwen3-Max-Preview的76.3），能有效检测合成数据集中的假阳性，与专家评估高度一致。与策略模型协作时显著提升性能，如将DeepSeek-R1-Distill-Qwen-32B在AIME2025上的准确率从55.2%提升到73.3%。

Conclusion: OPV通过验证总结结果的推理过程，实现了准确高效的验证，支持大规模标注，在减少标注成本的同时显著提升了验证性能。
...
TL;DR: 研究探讨通过模型缩放作为适应新目标语言的高效策略，发现适当缩放可以在保持英语能力的同时，用更少的目标语言数据达到与持续预训练相当的性能


<details>
  <summary>Details</summary>
Motivation: 当前大规模多语言模型在中等和低资源语言上表现仍不如语言特定适应模型，特别是在较小模型规模下。需要寻找更高效的语言适应策略。

Method: 通过FLOP匹配的模型进行全面的缩放消融实验，比较缩放英语基础模型与标准持续预训练的效果，并探索将缩放后的语言特定模型合并构建模块化多语言系统的方法。

Result: 当暴露于足够的目标语言数据时，较大的缩放模型能够匹配或超越使用更多数据进行持续预训练的小模型，展示了缩放对数据效率的益处。缩放还有助于保持基础模型的英语能力，减少灾难性遗忘。虽然合并仍不如联合多语言训练有效，但缩放后的合并表现优于小规模合并。

Conclusion: 缩放是适应新目标语言的高效策略，能够在保持源语言能力的同时提高数据效率。模型合并有改进空间，专门针对语言级别整合的合并方法可能带来性能提升。
...
TL;DR: 研究发现LLMs在处理罗马化印度语言文本时性能显著下降，可能导致医疗分诊中数百万额外错误，尽管模型能理解语义但分类输出仍不稳定。


<details>
  <summary>Details</summary>
Motivation: 印度临床应用中，用户常使用罗马化文本而非原生文字，但现有研究很少用真实数据评估这种书写变体对LLM可靠性的影响，特别是在母婴保健分诊这一关键领域。

Method: 在包含五种印度语言和尼泊尔语的真实用户查询数据集上，对领先的LLMs进行基准测试，比较罗马化文本和原生文字的性能差异。

Result: 罗马化消息导致性能持续下降，F1分数比原生文字低5-12分；在合作母婴健康组织中，这种差距可能导致近200万额外分诊错误；性能差距并非临床推理失败，而是罗马化输入中的书写噪声导致分类输出不稳定。

Conclusion: LLM医疗系统存在关键安全盲点：看似能理解罗马化输入的模型在实际应用中可能无法可靠执行，这对印度等多元语言环境中的临床部署构成重大风险。
...
TL;DR: FACTS Leaderboard是一个综合性评估语言模型事实准确性的在线排行榜套件，包含四个子排行榜：多模态、参数化、搜索和基于文档的评估，通过自动化评分模型全面衡量模型在各种场景下生成事实准确文本的能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏全面评估语言模型事实准确性的统一基准，现有评估方法往往只关注特定场景或单一维度。需要建立一个综合性的评估框架，能够衡量模型在不同情境下（如视觉问答、闭卷知识、信息搜索、文档引用）的事实准确性，为模型开发提供更全面的指导。

Method: 构建包含四个子排行榜的综合套件：1) FACTS Multimodal评估图像问答的事实性；2) FACTS Parametric评估模型参数中的世界知识；3) FACTS Search评估使用搜索API时的信息准确性；4) FACTS Grounding (v2)评估长文本回答是否基于提供的文档。使用自动化评分模型对模型响应进行评分，最终得分为四个组件的平均值。

Result: 开发了一个在线排行榜套件，包含公开和私有数据集分割，允许外部参与同时保护评估完整性。该套件将持续维护，为语言模型的事实准确性提供全面、平衡的评估框架。

Conclusion: FACTS Leaderboard提供了一个全面评估语言模型事实准确性的标准化框架，通过多维度评估帮助研究人员和开发者更好地理解和改进模型的事实性表现，促进更可靠的语言模型发展。
...
TL;DR: LabelFusion是一个融合集成方法，通过结合传统Transformer分类器和LLM的优势，实现准确且成本感知的文本分类，支持多类别和多标签任务。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer分类器在文本分类中表现良好，但大型语言模型（LLMs）具有更强的推理能力。然而，LLMs成本高、延迟大。需要一种方法能够结合两者的优势，同时考虑实际部署中的成本、延迟和准确性权衡。

Method: 通过融合集成方法，将传统Transformer分类器（如RoBERTa）的嵌入向量与LLM（如GPT、Gemini、DeepSeek）生成的每类得分拼接，然后输入到多层感知机（FusionMLP）中进行最终预测。使用结构化提示工程策略从LLM获取每类得分。

Result: 在AG News数据集上达到92.4%的准确率，在10类Reuters 21578主题分类上达到92.3%的准确率。该方法能够在准确性、延迟和成本之间实现实用权衡。

Conclusion: LabelFusion通过融合传统Transformer分类器和LLM的优势，提供了一种有效的文本分类解决方案，既能获得高性能，又能灵活控制成本和延迟，适用于实际部署场景。
...
TL;DR: 使用计算文本分析方法研究《霍比特人》对话的情感基调，发现整体积极、平静，随着故事进展支配感逐渐增强，体现了小说紧张与舒适交替的情感节奏。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过计算工具与文学解释相结合，揭示文学作品中的微妙情感结构，特别是分析托尔金《霍比特人》对话中的情感节奏和情感调节模式。

Method: 使用正则表达式提取对话文本，进行预处理，然后使用NRC-VAD词典对情感维度进行量化评分，包括效价（积极/消极）、唤醒度（兴奋/平静）和支配感（控制力）。

Result: 对话整体保持积极（高效价）和平静（低唤醒度）的基调，随着故事进展支配感逐渐增强；危险和兴奋时刻与幽默、友情和放松时刻交替出现，形成情感节奏；可视化图表展示了托尔金语言在紧张与舒适之间的循环。

Conclusion: 数字方法能够揭示文学作品中的微妙情感结构，展示了《霍比特人》中稳定的情感节奏和情感调节如何塑造其叙事，为计算文学分析提供了有价值的案例。
...
TL;DR: 评估多模态大语言模型在政治视频情绪分析中的表现，发现在理想条件下表现良好，但在真实议会辩论中效果不佳，存在统计推断风险。


<details>
  <summary>Details</summary>
Motivation: 情绪在政治沟通中至关重要，多模态生成AI为情绪分析带来新机遇，但缺乏对其有效性的实证证据，需要填补这一研究空白。

Method: 使用两个互补的人类标注视频数据集，评估当前多模态大语言模型在视频情绪唤起分析中的表现，比较理想条件与真实议会辩论场景。

Result: 理想条件下，mLLMs的情绪唤起评分高度可靠且无明显人口统计偏差；但在真实议会辩论录音中，mLLs的唤起评分表现不佳，可能对下游统计推断产生负面影响。
...
TL;DR: 该研究构建了一个多智能体语言习得计算实验室，通过母语模型生成语言样例，子代模型通过统计分析习得抽象语法知识，验证了非平凡语法范畴的获取。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言习得的计算机制，特别是如何通过有限的语言输入样例来获取抽象的语法知识，而不依赖母语模型的内部知识。

Method: 采用多智能体系统：一个母语语言模型和一个子代语言模型。子代模型只能访问母语模型生成的语言样例，通过分层凝聚聚类分析对输入数据进行统计分析，从中提取离散语法规则。

Result: 实验表明该系统能够习得类似自然语言学家提出的语法范畴结构，证明了非平凡语法知识的获取。参数配置在训练集上确定后，在测试集上同样有效，验证了方法的鲁棒性。

Conclusion: 该计算实验室环境能够成功模拟语言习得过程，仅通过语言样例就能获取抽象语法知识，为理解语言习得的计算机制提供了有效工具。
...
### [137] [Spatial Spiking Neural Networks Enable Efficient and Robust Temporal Computation](https://arxiv.org/abs/2512.10011)
*Lennart P. L. Landsmeer,Amirreza Movahedin,Mario Negrello,Said Hamdioui,Christos Strydis*

Main category: cs.NE

TL;DR: 提出Spatial Spiking Neural Networks (SpSNNs)，通过神经元学习空间坐标，让延迟从神经元间的物理距离自然产生，取代传统可训练延迟参数，大幅减少参数数量同时保持时间表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有SNN将突触延迟作为完全可训练、无约束的参数，导致内存占用大、计算需求高且偏离生物合理性。大脑中延迟源于神经元在空间中的物理距离，因此需要更符合生物原理且高效的延迟建模方法。

Method: 提出SpSNNs框架：神经元学习有限维欧几里得空间中的坐标，延迟由神经元间距离决定，取代每个突触的延迟学习。通过自动微分计算精确延迟梯度，支持任意神经元模型和架构。

Result: 在Yin-Yang和Spiking Heidelberg Digits基准测试中，SpSNNs性能优于传统延迟SNN，参数数量大幅减少。2D和3D网络性能最佳，显示几何正则化效应。动态稀疏化SpSNNs在90%稀疏度下保持全精度，参数减少达18倍。

Conclusion: SpSNNs为探索时空计算中的空间结构提供了原则性平台，提供了硬件友好的可扩展、高能效神经形态智能实现方案，其学习到的空间布局可直接映射到硬件几何结构。
...
### [138] [A Spiking Neural Network Implementation of Gaussian Belief Propagation](https://arxiv.org/abs/2512.10638)
*Sepideh Adamiat,Wouter M. Kouw,Bert de Vries*

Main category: cs.NE

TL;DR: 论文提出了一种基于脉冲神经网络的贝叶斯推理实现方法，通过因子图上的消息传递机制，在生物启发的神经元模型中实现了高斯信念传播。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯推理为自然智能体的信息处理提供了原则性解释，但神经机制如何执行这些抽象操作仍是一个开放问题。研究者希望探索分布式贝叶斯推理（因子图上的消息传递）是否可以通过脉冲神经网络实现。

Method: 采用泄漏积分发放神经元模型构建脉冲神经网络，将因子节点传入的消息编码为脉冲信号，通过SNN传播这些信号，然后将脉冲信号解码为传出消息。实现了三个核心线性操作：等式（分支）、加法和乘法。

Result: 与标准和积算法相比，该方法能够准确更新消息。在卡尔曼滤波和贝叶斯线性回归中的应用表明，该框架在静态和动态推理任务中都具有潜力。

Conclusion: 该研究为基于生物机制的神经形态概率推理实现迈出了重要一步，展示了脉冲神经网络执行分布式贝叶斯推理的可能性。
...
### [139] [STARS: Semantic Tokens with Augmented Representations for Recommendation at Scale](https://arxiv.org/abs/2512.10149)
...
Method: 1. 双记忆用户嵌入：分离长期偏好和短期会话意图；2. 语义物品标记：融合预训练文本嵌入、可学习增量、LLM生成的属性标签；3. 上下文感知评分：学习日历和事件偏移；4. 延迟感知两阶段检索管道：离线嵌入生成和在线最大内积搜索。

Result: 离线评估：相比现有LambdaMART系统，Hit@5提升超过75%；大规模A/B测试（600万次访问）：总订单+0.8%，首页加购+2.0%，用户访问次数+0.5%，所有提升均具有统计显著性。
...
### [140] [The Best of the Two Worlds: Harmonizing Semantic and Hash IDs for Sequential Recommendation](https://arxiv.org/abs/2512.10388)
*Ziwei Liu,Yejing Wang,Qidong Liu,Zijian Zhang,Chong Chen,Wei Huang,Xiangyu Zhao*

Main category: cs.IR

TL;DR: 提出H2Rec框架，通过协调语义ID和哈希ID来解决序列推荐中头尾项目性能不平衡问题，实现更好的推荐质量


<details>
  <summary>Details</summary>
Motivation: 传统序列推荐系统使用哈希ID存在长尾问题，而基于语义ID的方法又面临协作压倒现象，导致头尾项目性能不平衡

Method: 提出H2Rec框架，采用双分支建模架构分别捕捉语义ID的多粒度语义和哈希ID的独特协作身份，并引入双级对齐策略桥接两种表示

Result: 在三个真实世界数据集上的实验表明，H2Rec能有效平衡头尾项目的推荐质量，并超越现有基线方法

Conclusion: H2Rec通过协调语义ID和哈希ID，解决了序列推荐中头尾项目性能不平衡的问题，为推荐系统提供了更平衡和有效的解决方案
...
### [141] [Rethinking Popularity Bias in Collaborative Filtering via Analytical Vector Decomposition](https://arxiv.org/abs/2512.10688)
*Lingfeng Liu,Yixin Song,Dazhong Shen,Bing Yin,Hao Li,Yanyong Zhang,Chao Wang*

Main category: cs.IR

TL;DR: 论文揭示了协同过滤中流行度偏差是BPR优化的内在几何伪影，而非外部混杂因素，并提出DDC框架通过方向性分解与校正来纠正嵌入几何


<details>
  <summary>Details</summary>
Motivation: 流行度偏差严重削弱了协同过滤模型的个性化能力，导致过度推荐热门项目而忽略用户对利基内容的真实偏好。现有方法将其视为外部混杂因素，但本文发现这是BPR优化的内在几何问题

Method: 提出方向性分解与校正（DDC）框架，通过非对称方向性更新来纠正嵌入几何。DDC引导正向交互沿个性化偏好方向，同时引导负向交互远离全局流行度方向，从几何源头上解耦偏好与流行度

Result: 在多个基于BPR的架构上进行广泛实验，DDC显著优于最先进的去偏方法，将训练损失降低到精心调优基线的5%以下，同时实现更优的推荐质量和公平性

Conclusion: 流行度偏差是BPR优化的内在几何伪影，而非外部混杂因素。DDC框架通过方向性分解与校正有效解决了这一问题，为协同过滤中的去偏提供了新的理论基础和实践方法
...
### [142] [HGC-Herd: Efficient Heterogeneous Graph Condensation via Representative Node Herding](https://arxiv.org/abs/2512.09947)
*Fuyan Ou,Siqi Ai,Yulin Hu*

Main category: cs.LG

TL;DR: HGC-Herd：一种无需训练的异构图压缩框架，通过轻量级特征传播和类内聚类机制生成紧凑且信息丰富的子图，在保持语义和结构保真度的同时显著降低计算和内存开销。


<details>
  <summary>Details</summary>
Motivation: 异构图神经网络（HGNNs）在处理大规模图时面临可扩展性挑战，主要由于结构冗余和高维节点特征。现有的图压缩方法（如GCond）主要针对同构图设计，依赖梯度匹配，导致计算、内存和优化开销较大。

Method: HGC-Herd采用无需训练的压缩框架：1）集成轻量级特征传播来编码多跳关系上下文；2）使用类内聚类机制识别每类的代表性节点；3）生成平衡且具有区分性的子集用于下游学习任务。

Result: 在ACM、DBLP和Freebase数据集上的实验表明，HGC-Herd在保持与全图训练相当或更优准确率的同时，显著减少了运行时间和内存消耗。

Conclusion: HGC-Herd为高效且可扩展的异构图表示学习提供了实用价值，通过训练免费的压缩方法解决了HGNNs在大规模图上的可扩展性问题。
...
### [143] [BAMBO: Construct Ability and Efficiency LLM Pareto Set via Bayesian Adaptive Multi-objective Block-wise Optimization](https://arxiv.org/abs/2512.09972)
*Kesheng Chen,Wenjian Luo,Zhenqian Zhu,Yamin Hu,Yiya Xi*

Main category: cs.LG

TL;DR: BAMBO是一个自动构建LLM帕累托集的贝叶斯自适应多目标块优化框架，通过混合最优块划分策略解决现有方法的维度灾难问题。
...
Method: 提出BAMBO框架，采用混合最优块划分策略（作为1D聚类问题），使用动态规划平衡块内同质性和块间信息分布，在进化循环中使用qEHVI采集函数自动优化。

Result: 实验表明BAMBO能够发现比基线方法更优、更全面的帕累托前沿，支持根据多样化操作约束进行敏捷模型选择。

Conclusion: BAMBO通过智能块划分解决了LLM帕累托集构建中的维度灾难问题，实现了高效的多目标优化，为不同资源约束下的模型选择提供了灵活解决方案。
...
### [144] [Latent Action World Models for Control with Unlabeled Trajectories](https://arxiv.org/abs/2512.10016)
*Marvin Alles,Xingyuan Zhang,Patrick van der Smagt,Philip Becker-Ehmck*

Main category: cs.LG

TL;DR: 提出潜在动作世界模型，通过联合使用动作标注和无动作数据，在少量动作标注样本下实现高效世界模型学习


<details>
  <summary>Details</summary>
Motivation: 人类学习结合直接交互和无动作经验（如视频），但标准世界模型依赖动作标注轨迹，在动作标签稀缺时效果受限

Method: 引入潜在动作世界模型家族，学习共享潜在动作表示，将观测控制信号与被动观测推断的动作对齐，使单个动力学模型能在大规模无标签轨迹上训练，仅需少量动作标注样本

Result: 在DeepMind Control Suite上，相比纯动作标注基线，使用约少一个数量级的动作标注样本获得强劲性能

Conclusion: 潜在动作使世界模型能同时利用被动和交互数据，实现更高效学习，连接了传统分离的离线RL和无动作训练领域
...
### [145] [Cluster-Dags as Powerful Background Knowledge For Causal Discovery](https://arxiv.org/abs/2512.10032)
*Jan Marco Ruiz de Vargas,Kirtan Padh,Niki Kilbertus*

Main category: cs.LG

TL;DR: 提出基于Cluster-DAG先验知识的因果发现方法，通过改进PC和FCI算法，在完全和部分可观测设置中提升性能


<details>
  <summary>Details</summary>
Motivation: 当前因果发现方法在处理高维数据和复杂依赖关系时面临挑战，利用系统先验知识可以辅助因果发现

Method: 使用Cluster-DAG作为先验知识框架来预热启动因果发现，提出两种改进的基于约束的算法：Cluster-PC（完全可观测）和Cluster-FCI（部分可观测）

Result: 在模拟数据上的实证评估表明，Cluster-PC和Cluster-FCI在性能上优于没有先验知识的基线方法

Conclusion: Cluster-DAG先验知识框架比现有的分层背景知识方法更灵活，能有效提升因果发现算法的性能
...
### [146] [Robust Gradient Descent via Heavy-Ball Momentum with Predictive Extrapolation](https://arxiv.org/abs/2512.10033)
*Sarwan Ali*
...
<details>
  <summary>Details</summary>
Motivation: 像NAG这样的加速梯度方法在良好条件下收敛快，但在病态或非凸问题上容易因动量积累而发散。需要一种既能保持加速效果又能在复杂地形中保持稳定的方法。

Method: HB-SGE结合重球动量和预测梯度外推，使用局部泰勒近似估计未来梯度方向，而不是简单累积历史梯度。这种方法提供自适应加速同时保持稳定性，具有O(d)内存开销，超参数与标准动量方法相同。

Result: 在病态二次问题(条件数κ=50)上，HB-SGE在119次迭代收敛，而SGD和NAG都发散。在非凸Rosenbrock函数上，HB-SGE在2718次迭代收敛，而经典动量方法在10步内发散。在良好条件下NAG更快，但HB-SGE在多种地形上都比SGD快。

Conclusion: HB-SGE为病态和非凸优化问题提供了鲁棒的替代方案，虽然不如NAG在良好条件下快，但能在NAG和标准动量方法失败的情况下稳定收敛，具有实际应用价值。
...
### [147] [Intelligently Weighting Multiple Reference Models for Direct Preference Optimization of LLMs](https://arxiv.org/abs/2512.10040)
*Skyler Wu,Aymen Echarghaoui*

Main category: cs.LG

TL;DR: MRPO扩展DPO，使用多个参考模型进行正则化，但现有权重设置方法不理想。作者提出四种新权重策略（两种离线、两种在线），实验表明所有新策略都优于现有MRPO方法。但有趣的是，单参考DPO在6/7情况下优于所有多参考方法，质疑多参考方法的实际价值。


<details>
  <summary>Details</summary>
Motivation: 当前MRPO方法中设置参考模型权重的方式是临时性的且统计上不可靠，导致性能不稳定。需要更系统、统计上更合理的权重设置方法来提升多参考偏好优化的效果。

Method: 提出四种新的权重设置策略：1）两种离线方法利用验证集信号；2）一种在线方法使用滑动窗口估计器减少过拟合；3）一种在线方法将参考权重设置视为K臂老虎机问题，采用Thompson Sampling。实验使用Qwen2.5-0.5B作为策略模型，7个不同家族的参考模型（0.5B-14B）。

Result: 所有四种新策略在UltraFeedback和SafeRLHF数据集上的偏好准确率都优于现有MRPO权重方法。但更令人深思的是，使用7个参考模型中任意6个的单参考DPO，都一致优于所有测试的多参考方法。

Conclusion: 虽然提出的新权重策略确实改进了MRPO方法，但实验结果表明单参考DPO在大多数情况下优于多参考方法，这对多参考方法的实际应用价值提出了质疑。研究揭示了当前多参考方法可能存在的局限性。
...
### [148] [SEMDICE: Off-policy State Entropy Maximization via Stationary Distribution Correction Estimation](https://arxiv.org/abs/2512.10042)
*Jongmin Lee,Meiqi Sun,Pieter Abbeel*

Main category: cs.LG

TL;DR: 提出SEMDICE算法，从任意离策略数据集中学习状态熵最大化的先验策略，用于无监督强化学习预训练


<details>
  <summary>Details</summary>
Motivation: 在无监督强化学习预训练中，需要学习适用于下游任务的先验策略而不依赖任务特定奖励函数。状态熵最大化（SEM）是一种有效方法，但现有方法难以从任意离策略数据集中学习最优策略。

Method: 提出SEMDICE算法，通过直接在平稳分布空间中优化策略来计算状态熵最大化策略。该方法基于离策略数据集，计算单一、平稳的马尔可夫状态熵最大化策略。

Result: 实验结果表明，SEMDICE在最大化状态熵方面优于基线算法，并且在基于SEM的无监督RL预训练方法中实现了最佳的下游任务适应效率。

Conclusion: SEMDICE是一种原理性的离策略算法，能够从任意离策略数据集中有效学习状态熵最大化策略，为下游任务提供优秀的预训练基础。
...
### [149] [Local LLM Ensembles for Zero-shot Portuguese Named Entity Recognition](https://arxiv.org/abs/2512.10043)
*João Lucas Luz Lima Sarcinelli,Diego Furtado Silva*

Main category: cs.LG

TL;DR: 提出一种用于葡萄牙语零样本命名实体识别的三阶段集成方法，通过组合多个本地运行的小型LLM，在无需微调的情况下超越单个模型性能


<details>
  <summary>Details</summary>
Motivation: LLM在NLP任务中表现出色，但在命名实体识别（NER）方面表现不佳，尤其是对葡萄牙语等低资源语言。虽然开源LLM支持本地部署，但没有单一模型在所有任务中都占优，因此需要集成方法。现有LLM集成主要关注文本生成或分类，NER领域研究不足。

Method: 提出三阶段集成管道：1）使用启发式方法选择最优模型组合，仅需少量标注数据；2）组合多个本地运行、能力相似的LLM；3）支持零样本NER，无需对当前任务进行微调。

Result: 在五个葡萄牙语NER数据集中，该方法在四个数据集上超越了单个LLM性能。在跨数据集配置中，基于不同源数据集获得的集成模型通常优于单个LLM，可能无需当前任务的标注数据。

Conclusion: 该工作通过有效组合多个小型LLM而不进行微调，推进了可扩展、低资源、零样本NER的发展，为资源受限的语言提供了实用的NER解决方案。
...
### [150] [Detailed balance in large language model-driven agents](https://arxiv.org/abs/2512.10047)
*Zhuo-Yang Song,Qing-Hong Cao,Ming-xing Luo,Hua Xing Zhu*

Main category: cs.LG

TL;DR: 该论文发现大语言模型驱动的智能体在状态转换中存在详细平衡，表明LLM生成可能不是通过学习规则集，而是通过隐式学习潜在函数实现的，这为建立AI系统的宏观动力学理论提供了基础。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的智能体在解决复杂问题方面取得了经验性成功，但缺乏理解其宏观动力学的理论框架。需要从工程实践提升到可预测、可量化的科学理论。

Method: 基于最小作用量原理，通过实验测量LLM生成状态之间的转移概率，统计发现LLM生成转换中的详细平衡现象。

Result: 发现LLM生成转换中存在详细平衡，表明LLM生成不是通过学习规则集和策略，而是通过隐式学习一类潜在函数实现的，这种潜在函数可能超越不同的LLM架构和提示模板。

Conclusion: 这是首次在LLM生成动力学中发现不依赖于具体模型细节的宏观物理定律，为建立复杂AI系统的宏观动力学理论迈出了重要一步，将AI智能体研究从工程实践提升到可预测、可量化的科学。
...
### [151] [DB2-TransF: All You Need Is Learnable Daubechies Wavelets for Time Series Forecasting](https://arxiv.org/abs/2512.10051)
*Moulik Gupta,Achyut Mani Tripathi*

Main category: cs.LG

TL;DR: 提出DB2-TransF，用可学习的Daubechies小波系数层替代Transformer的自注意力机制，在时间序列预测中实现高效的多尺度模式捕捉和相关性建模


<details>
  <summary>Details</summary>
Motivation: Transformer架构虽然擅长建模长程依赖，但其二次计算复杂度限制了在大规模高维时间序列预测中的可扩展性和适应性

Method: 使用可学习的Daubechies小波系数层替代Transformer的自注意力机制，构建小波基模块来高效捕捉多尺度局部和全局模式，增强多时间序列间的相关性建模

Result: 在13个标准预测基准测试中，DB2-TransF达到或优于传统Transformer的预测精度，同时显著减少内存使用

Conclusion: DB2-TransF为高级时间序列预测提供了一个可扩展且资源高效的框架，在保持预测精度的同时大幅降低计算资源需求
...
### [152] [Mitigating Exposure Bias in Risk-Aware Time Series Forecasting with Soft Tokens](https://arxiv.org/abs/2512.10056)
*Alireza Namazi,Amirreza Dolatpour Fathkouhi,Heman Shakeri*

Main category: cs.LG

TL;DR: SoTra方法通过传播连续概率分布来缓解暴露偏差，实现校准的、不确定性感知的轨迹预测，结合风险感知解码最小化临床风险


<details>
  <summary>Details</summary>
Motivation: 自回归预测在糖尿病和血流动力学管理中至关重要，不同操作区域具有不同的临床风险。标准模型使用教师强制训练存在暴露偏差，导致闭环使用时多步预测不稳定
...
Conclusion: 这些改进支持SoTra在安全关键预测控制中的应用
...
### [153] [\textsc{Text2Graph}: Combining Lightweight LLMs and GNNs for Efficient Text Classification in Label-Scarce Scenarios](https://arxiv.org/abs/2512.10061)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为零样本分类器计算成本高、环境影响大，限制了在高性能计算环境中的大规模标注应用，需要更可持续的工作流程

Method: 开发Text2Graph开源框架，将LLM部分标注与GNN标签传播结合，提供模块化设计，可灵活替换特征提取器、边构建方法和采样策略

Result: 在5个主题分类和情感分析数据集上测试，图传播方法在性能上具有竞争力，同时显著降低能耗和碳排放

Conclusion: Text2Graph提供了一种可持续的文本分类解决方案，在保持性能的同时大幅减少计算成本和环境影响
...
### [154] [MedXAI: A Retrieval-Augmented and Self-Verifying Framework for Knowledge-Guided Medical Image Analysis](https://arxiv.org/abs/2512.10098)
*Midhat Urooj,Ayan Banerjee,Farhat Shaikh,Kuntal Thakur,Sandeep Gupta*

Main category: cs.LG

TL;DR: MedXAI是一个可解释的医学影像分类框架，通过整合深度学习模型与临床专家知识，提升跨域泛化能力、减少罕见类别偏见，并提供临床对齐的解释。


<details>
  <summary>Details</summary>
Motivation: 医学AI面临领域偏移、罕见类别偏见和缺乏透明度等挑战，深度学习模型在现实世界分布变化中表现不佳，且缺乏临床部署所需的可解释性。

Method: 提出MedXAI框架，整合深度视觉模型与临床专家知识，通过符号化组件作为临床先验和正则化器，定位相关诊断特征而非依赖技术性后处理方法。

Result: 在10个多中心数据集上评估，跨域泛化提升3%，罕见类别F1分数提升10%，显著优于强深度学习基线，在领域内和跨域性能均表现优越。

Conclusion: MedXAI通过整合专家知识，在提升医学AI性能的同时提供临床对齐的解释，特别适用于多模态医学AI中的罕见疾病诊断。
...
### [155] [CHyLL: Learning Continuous Neural Representations of Hybrid Systems](https://arxiv.org/abs/2512.10117)
...
Method: CHyLL的核心洞察是重置映射在守卫表面粘合状态空间，将状态空间重构为分段光滑商流形，使流在空间上连续。基于微分拓扑的嵌入定理，CHyLL同时学习高维空间中的无奇点神经嵌入和其中的连续流。

Result: CHyLL能够准确预测混合系统的流，具有优越的准确性，并能识别混合系统的拓扑不变量。最后成功应用于随机最优控制问题。

Conclusion: CHyLL提供了一种连续学习混合系统的新方法，避免了传统方法中的模式切换和不连续性问题，在预测准确性和拓扑分析方面表现优异。
...
### [156] [Partitioning the Sample Space for a More Precise Shannon Entropy Estimation](https://arxiv.org/abs/2512.10133)
*Gabriel F. A. Bastos,Jugurta Montalvão*

Main category: cs.LG

TL;DR: 提出一种新的离散熵估计器，通过分解性、缺失质量估计和未见结果数量估计来补偿小数据集下的负偏差


<details>
  <summary>Details</summary>
Motivation: 在小数据集场景下，当样本数量可能小于可能结果数量时，可靠地估计香农熵是一个关键问题。传统估计器在这种情况下存在负偏差问题

Method: 使用分解性属性，结合缺失质量估计和未见结果数量估计，来补偿由小样本引起的负偏差

Result: 实验结果显示，在欠采样情况下，该方法优于一些经典估计器，并与一些成熟的最先进估计器性能相当

Conclusion: 提出的离散熵估计器在小数据集场景下表现良好，能够有效补偿负偏差，为小样本熵估计提供了可靠解决方案
...
### [157] [Sequence-to-Image Transformation for Sequence Classification Using Rips Complex Construction and Chaos Game Representation](https://arxiv.org/abs/2512.10141)
*Sarwan Ali,Taslim Murad,Imdadullah Khan*

Main category: cs.LG

TL;DR: 提出一种将分子序列转化为图像的新拓扑方法，结合混沌游戏表示和代数拓扑的Rips复形构建，用于抗癌肽分类，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统分子序列分类的特征工程方法存在稀疏性和计算复杂度问题，而深度学习模型在表格化生物数据上表现不佳。需要一种能有效捕捉分子序列结构信息的新表示方法。

Method: 结合混沌游戏表示将序列元素映射到2D坐标，计算成对距离，构建Rips复形来捕捉局部结构和全局拓扑特征。该方法提供表示唯一性、拓扑稳定性和信息保存的正式保证。

Result: 在抗癌肽数据集上实验显示，该方法在乳腺癌和肺癌数据集上分别达到86.8%和94.5%的准确率，优于基于向量、序列语言模型和现有图像方法。

Conclusion: 拓扑表示能保留关键序列信息，同时有效利用基于视觉的深度学习架构进行分子序列分析，为分子序列分类提供了新的有效方法。
...
### [158] [Murmur2Vec: A Hashing Based Solution For Embedding Generation Of COVID-19 Spike Sequences](https://arxiv.org/abs/2512.10147)
*Sarwan Ali,Taslim Murad*

Main category: cs.LG

TL;DR: 提出一种基于哈希的可扩展嵌入方法，用于SARS-CoV-2刺突蛋白序列的谱系分类，相比现有方法在效率和准确性上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: COVID-19早期检测和特征分析对临床响应和公共卫生规划至关重要。现有方法存在局限性：基于系统发育树的方法计算密集，无法扩展到数百万序列的数据集；现有嵌入方法要么需要序列对齐，要么预测性能不佳且运行成本高，难以进行大规模分析。

Method: 针对SARS-CoV-2刺突蛋白区域的最常见谱系，提出一种基于哈希的可扩展嵌入方法，生成紧凑的低维序列表示，然后用这些嵌入训练多种机器学习模型进行监督谱系分类。

Result: 与多种基准和最先进的生物序列嵌入方法相比，所提出的嵌入方法在效率上显著提升：达到86.4%的分类准确率，同时将嵌入生成时间减少高达99.81%。

Conclusion: 该方法为大规模病毒序列分析提供了一个快速、有效且可扩展的解决方案，在效率和准确性方面均优于现有方法。
...
### [159] [Rethinking Causal Discovery Through the Lens of Exchangeability](https://arxiv.org/abs/2512.10152)
*Tiago Brogueira,Mário Figueiredo*

Main category: cs.LG

TL;DR: 本文主张将传统i.i.d.因果发现框架重构为更一般的可交换性框架，提出概念和实证论证，并创建了仅基于可交换性假设的合成数据集，其神经网络因果发现算法在真实基准上达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现方法在i.i.d.和时间序列数据下采用不同的建模假设，作者认为i.i.d.设置应该重构为更一般的可交换性框架，以统一和扩展因果发现的理论基础。

Method: 1) 提出概念论证：将实验因果推断对可交换性的依赖扩展到因果发现；2) 实证论证：展示现有i.i.d.方法实际依赖可交换性假设；3) 创建仅基于可交换性假设的合成数据集；4) 开发基于神经网络的因果发现算法，仅在该数据集上训练。

Result: 1) 可交换性合成数据集比所有其他i.i.d.合成数据集更接近真实世界"i.i.d."数据集（Tübingen数据集）的统计结构；2) 仅在该数据集上训练的神经网络因果发现算法在真实基准测试中达到与其他SOTA i.i.d.方法相当的性能。

Conclusion: 因果发现的i.i.d.框架应重构为更一般的可交换性框架，这不仅在理论上更严谨，而且能产生更接近真实世界数据的合成数据集，支持开发有效的因果发现算法。
...
Method: 提出CIEGAD框架：1) 通过聚类条件构建领域配置文件；2) 使用集成类别频率和几何指标的分层频率-几何分配进行生成分配；3) 通过插值和外推合成共存精细控制生成方向；4) 通过几何约束过滤结合LLM-as-a-Judge机制进行质量控制。

Result: 在多个分类任务上的实验表明，CIEGAD能有效扩展真实数据分布的边缘，同时保持生成数据与真实数据的高对齐性和语义多样性。特别在长尾和多类分类任务中，CIEGAD持续提升F1和召回率，验证了分布一致性、多样性和质量的三重和谐。

Conclusion: CIEGAD作为一个实用导向的数据增强框架，能补充代表性不足的区域，同时保持与真实数据的对齐，为解决数据稀缺和分布不平衡问题提供了有效方案。
...
### [161] [Assessing Neuromorphic Computing for Fingertip Force Decoding from Electromyography](https://arxiv.org/abs/2512.10179)
*Abolfazl Shahrooei,Luke Arthur,Om Patel,Derek Kamper*

Main category: cs.LG

TL;DR: 比较SNN和TCN在从高密度表面肌电信号解码指尖力方面的性能，TCN表现更优但SNN作为神经形态计算基准有潜力


<details>
  <summary>Details</summary>
Motivation: 高密度表面肌电信号为非侵入式神经接口提供了可能，但将神经活动映射到用户运动意图仍然具有挑战性。需要评估神经形态架构（SNN）与传统卷积网络（TCN）在解码指尖力方面的性能差异。

Method: 使用单个参与者的10次试验数据，通过两个前臂电极阵列采集HD-sEMG信号，采用FastICA分解获取运动单元放电活动，使用重叠窗口训练模型，采用端到端因果卷积架构，比较SNN和TCN的性能。

Result: 在保留试验中，TCN达到4.44% MVC RMSE（Pearson r = 0.974），而SNN达到8.25% MVC RMSE（r = 0.922）。TCN更准确，但SNN作为神经形态计算基准仍有改进空间。

Conclusion: 虽然TCN在解码指尖力方面更准确，但SNN作为神经形态计算的现实基准，通过适度的架构和超参数优化可以缩小性能差距，具有发展潜力。
...
### [162] [MiniF2F-Dafny: LLM-Guided Mathematical Theorem Proving via Auto-Active Verification](https://arxiv.org/abs/2512.10187)
...
<details>
  <summary>Details</summary>
Motivation: 将数学推理基准从交互式定理证明器扩展到自动定理证明器，探索自动化验证与LLM提示的协作模式

Method: 创建miniF2F-Dafny基准，评估Dafny自动化验证能力，测试12个现成LLM在自动化失败时提供证明提示的效果
...
### [163] [Exact Recovery of Non-Random Missing Multidimensional Time Series via Temporal Isometric Delay-Embedding Transform](https://arxiv.org/abs/2512.10191)
*Hao Shu,Jicheng Li,Yu Jin,Ling Zhou*

Main category: cs.LG

TL;DR: 提出LRTC-TIDT模型，通过时间等距延迟嵌入变换构造Hankel张量，利用时间序列的平滑性和周期性自然诱导低秩性，解决多维时间序列中非随机缺失数据的精确恢复问题。


<details>
  <summary>Details</summary>
Motivation: 非随机缺失数据是多维时间序列中普遍存在但处理不足的缺陷，严重威胁数据驱动分析和决策的可靠性。传统的纯低秩张量补全方法在处理非随机缺失时存在方法论和理论上的不足，而现有的Hankel结构张量补全方法存在低秩性来源不明确和缺乏精确恢复理论的问题。

Method: 提出时间等距延迟嵌入变换，构造Hankel张量，其低秩性由底层时间序列的平滑性和周期性自然诱导。在此基础上开发LRTC-TIDT模型，在张量奇异值分解框架下表征低秩结构。

Result: 在满足规定的非随机采样条件和温和的不相干假设下，LRTC-TIDT模型能够实现精确恢复。在多种非随机缺失模式下的仿真实验验证了该结论，并在网络流量重建、城市交通估计和温度场预测等实际任务中持续优于现有张量方法。

Conclusion: LRTC-TIDT模型通过时间等距延迟嵌入变换为Hankel张量提供了明确的低秩性来源，并建立了非随机缺失数据下的精确恢复理论，为多维时间序列的非随机缺失数据恢复提供了有效的解决方案。
...
### [164] [Federated Domain Generalization with Latent Space Inversion](https://arxiv.org/abs/2512.10224)
*Ragja Palakkadavath,Hung Le,Thanh Nguyen-Tang,Svetha Venkatesh,Sunil Gupta*

Main category: cs.LG

TL;DR: 提出一种新的联邦域泛化方法，通过潜在空间反转技术保护客户端隐私，并使用重要性权重聚合策略处理非独立同分布数据，在减少通信开销的同时提升全局模型性能。
...
Method: 1. 潜在空间反转技术：在本地客户端训练中强制跨本地模型的域不变性，增强隐私保护；2. 重要性权重聚合策略：在模型聚合时优先考虑对本地模型预测有显著影响的参数，保留本地适配能力。

Result: 实验表明该方法在性能上优于现有最先进方法，同时通信开销更小，实现了隐私保护与模型性能的平衡。

Conclusion: 提出的联邦域泛化框架通过潜在空间反转和重要性权重聚合，有效解决了隐私泄露和非独立同分布数据问题，在保持低通信成本的同时实现了优越的泛化性能。
...
### [165] [Adaptive Information Routing for Multimodal Time Series Forecasting](https://arxiv.org/abs/2512.10229)
...
TL;DR: 提出自适应信息路由（AIR）框架，通过文本信息动态引导时间序列模型，改进多模态时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测仅依赖历史数据，在实际场景中信息有限导致预测不准确。多模态方法结合文本数据等额外模态，但现有方法将文本数据与时间序列数据同等对待，未能充分利用文本信息来动态指导时间序列模型。
...
Conclusion: AIR框架通过文本信息动态引导时间序列模型，相比现有方法能更有效地利用多模态信息，显著提升时间序列预测性能，为多模态时间序列预测提供了新思路。
...
### [166] [R^2-HGP: A Double-Regularized Gaussian Process for Heterogeneous Transfer Learning](https://arxiv.org/abs/2512.10258)
...
<details>
  <summary>Details</summary>
Motivation: 传统多输出高斯过程在迁移学习中面临三大挑战：1）源域和目标域输入空间异构导致知识直接迁移困难；2）忽略先验知识和物理信息，导致映射不稳定；3）不恰当的信息共享容易引发负迁移。现有模型无法统一解决这些问题。

Method: 提出双重正则化异构高斯过程框架(R²-HGP)：1）设计可训练先验概率映射模型对齐异构输入域，将对齐结果作为隐变量；2）构建多源迁移GP模型并整合到条件变分自编码器框架；3）加入物理知识作为正则项确保对齐符合物理约束；4）对迁移系数施加稀疏惩罚，自适应选择信息量最大的源输出并抑制负迁移。

Result: 通过大量仿真和真实工程案例验证，R²-HGP在多种评估指标上均优于现有最先进基准方法，表现出显著优势。

Conclusion: R²-HGP框架有效解决了异构迁移学习中的关键挑战，通过输入域对齐、物理知识整合和自适应源选择，实现了稳定且高效的知识迁移，为复杂工程应用提供了可靠解决方案。
...
### [167] [A Kernel-based Resource-efficient Neural Surrogate for Multi-fidelity Prediction of Aerodynamic Field](https://arxiv.org/abs/2512.10287)
*Apurba Sarker,Reza T. Batley,Darshan Sarojini,Sourav Saha*

Main category: cs.LG

TL;DR: KHRONOS是一种基于核的神经代理模型，通过融合稀疏高保真数据和低保真信息来预测空气动力学场，在计算资源受限条件下表现优异，相比传统密集神经网络参数更少、训练推理更快。


<details>
  <summary>Details</summary>
Motivation: 空气动力学仿真计算成本高，需要快速替代模型用于设计和优化。传统代理模型在处理不同保真度数据和计算资源约束方面存在局限性。

Method: 提出KHRONOS模型，基于变分原理、插值理论和张量分解构建核基神经代理。融合稀疏高保真数据和低保真信息，使用AirfRANS数据集作为高保真基准，NeuralFoil生成低保真数据。与MLP、GNN、PINN三种架构对比，考虑不同高保真数据可用性(0%、10%、30%)和复杂几何参数化。

Result: 所有模型最终达到可比预测精度，但KHRONOS在资源受限条件下表现突出：需要数量级更少的可训练参数，训练和推理速度更快，在可比精度下显著优于传统密集神经网络。

Conclusion: KHRONOS及其类似架构在平衡多保真度空气动力学场预测的精度和效率方面具有潜力，特别适用于计算资源受限的应用场景。
...
### [168] [An Interpretable AI Tool for SAVR vs TAVR in Low to Intermediate Risk Patients with Severe Aortic Stenosis](https://arxiv.org/abs/2512.10308)
*Vasiliki Stoumpou,Maciej Tysarowski,Talhat Azemi,Jawad Haider,Howard L. Haronian,Robert C. Hagberg,Dimitris Bertsimas*

Main category: cs.LG

TL;DR: 开发了一个可解释的处方框架，通过最优策略树为主动脉瓣狭窄患者提供TAVR与SAVR的个体化治疗推荐，相比实际临床决策可显著降低5年死亡率。


<details>
  <summary>Details</summary>
Motivation: 目前临床实践中，对于低至中危严重主动脉瓣狭窄患者，选择外科主动脉瓣置换术（SAVR）还是经导管主动脉瓣置换术（TAVR）存在较大变异性，缺乏能够直接优化长期结局的个体化、可解释治疗推荐模型。

Method: 提出一个可解释的处方框架，整合预后匹配、反事实结局建模和最优策略树（OPT）。通过预后匹配和样本加权模拟随机化，估计SAVR和TAVR两种治疗下的反事实死亡率，训练策略模型将患者划分为临床一致的亚组并推荐风险较低的治疗方案。

Result: 应用OPT推荐相比实际临床决策，在Hartford医院估计可降低20.3%的5年死亡率，在St. Vincent's医院可降低13.8%，显示出良好的外部验证性能。学习到的决策边界与真实世界结局和临床观察一致。

Conclusion: 该可解释处方框架首次为TAVR与SAVR选择提供透明、数据驱动的推荐，在内部和外部队列中均能改善估计的长期结局，为结构性心脏病精准医疗提供了更系统、循证的方法。
...
TL;DR: 提出一个云原生隐私保护架构，整合联邦学习、差分隐私、零知识合规证明和强化学习驱动的自适应治理，用于构建可验证合规、可扩展的分布式机器学习系统。


<details>
  <summary>Details</summary>
Motivation: 分布式机器学习系统需要强大的隐私保证、可验证的合规性以及在异构多云环境中的可扩展部署。现有系统难以同时满足这些要求。

Method: 集成联邦学习（避免集中敏感数据）、差分隐私（提供形式化隐私保证）、零知识证明（验证合规性）和强化学习驱动的自适应治理框架。在混合Kubernetes集群上部署完整原型。

Result: 原型系统降低了成员推理攻击风险，能够一致地执行形式化隐私预算，在差分隐私下保持稳定的模型性能。跨多机构工作负载的实验评估显示，该架构在最小开销下保持实用性，并提供持续的风险感知治理。

Conclusion: 该框架为大规模部署可信且合规的分布式机器学习系统建立了实用基础，实现了隐私保护、可验证合规性和可扩展部署的统一。
...
<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体系统通过递归反馈循环运行，但这类智能体循环的几何行为（收敛、发散或更复杂动态）仍缺乏深入理解，需要系统分析框架。

Method: 引入语义嵌入空间的几何分析框架，将迭代变换视为离散动力系统；区分工件空间和嵌入空间；提出等渗校准消除余弦相似度的各向异性偏差；通过受控实验研究单一智能体循环。

Result: 识别两种基本机制：收缩式重写循环收敛于稳定吸引子且分散度递减；探索式总结与否定循环产生无界发散且无聚类形成；提示设计直接控制智能体循环的动态机制。

Conclusion: 智能体循环的几何行为可通过几何框架系统分析，等渗校准能准确测量轨迹和吸引子，提示设计能控制收敛、发散和轨迹结构，为智能体系统设计提供理论基础。
...
### [171] [Better Prevent than Tackle: Valuing Defense in Soccer Based on Graph Neural Networks](https://arxiv.org/abs/2512.10355)
...
<details>
  <summary>Details</summary>
Motivation: 当前足球防守评估方法主要关注可见的球上行动（如拦截、抢断），但有效防守往往体现在阻止危险机会发生之前。现有方法未能充分衡量防守球员的真实影响，需要更全面的防守贡献评估框架。

Method: 使用图注意力网络（GAT）估计每个进攻选项的成功概率和预期价值，以及每个防守球员的防守责任。通过计算行动前后进攻队的预期控球价值（EPV）变化，根据防守球员是否减少或增加对手的EPV来分配正负信用分。

Result: 在2023-24赛季训练并在2024-25赛季荷甲联赛事件和追踪数据上评估，DEFCON聚合的球员信用分与市场估值呈现强正相关。框架还展示了多种实际应用，包括比赛中的防守贡献时间线、场地区域空间分析和攻防球员互动总结。

Conclusion: DEFCON提供了一个全面的足球防守贡献评估框架，能够量化传统方法难以测量的防守影响，为球员评估和战术分析提供了新的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有策略梯度方法在Transformer架构上的应用存在局限性，需要专门为基于Transformer的策略设计更通用的策略梯度框架，以更好地优化大语言模型的训练。

Method: 提出了广义策略梯度(GPG)定理，该定理专门针对Transformer-based策略设计，将标准策略梯度定理和GRPO（可能指某种特定的策略优化方法）作为其特例包含在内。

Result: 建立了统一的策略梯度框架，能够更有效地训练基于Transformer的策略，为大语言模型的策略优化提供了新的理论视角。

Conclusion: GPG定理为Transformer-based策略提供了更通用的梯度计算框架，在训练大语言模型方面具有实际应用价值，为策略优化领域带来了新的理论洞见。
...
<details>
  <summary>Details</summary>
Motivation: 铁磁物质的磁化强度随外加磁场强度增加而增加，这是由于微观层面磁畴的逐渐对齐。这种磁畴与外加磁场的对齐形成了磁化非线性的物理基础，需要合适的数学方法来描述这种非线性行为。

Method: 使用直线连分数来近似磁化强度的非线性函数。这种连分数是一种代数表达式，可以通过非线性回归来估计参数。

Result: 得到的拟合结果可用于解释磁畴在增长和收缩过程中的非线性行为，为磁化过程提供了数学描述工具。

Conclusion: 直线连分数方法能够有效近似铁磁物质磁化强度的非线性特性，为理解和分析磁畴在外加磁场下的行为提供了有用的数学框架。
...
TL;DR: 论文提出Eminence框架，通过理论分析稀疏决策边界如何实现高效后门攻击，利用极小比例（<0.1%）毒化样本实现>90%攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络对后门攻击脆弱，现有方法依赖启发式暴力搜索，缺乏理论分析限制了攻击可预测性和适应性。需要从理论层面理解后门攻击机制。

Method: 理论分析稀疏决策边界如何实现不成比例模型操控，推导闭式模糊边界区域；提出Eminence框架，优化通用视觉隐蔽触发器，战略利用脆弱决策边界实现低毒化率攻击。

Result: Eminence在<0.1%毒化率下实现>90%攻击成功率，清洁精度损失可忽略，跨模型/数据集/场景高可迁移性；验证了边界毒化与对抗边界操控的指数关系。

Conclusion: 通过理论分析揭示了稀疏决策边界在后门攻击中的关键作用，提出的Eminence框架具有理论保证和固有隐蔽性，为理解后门攻击机制提供了新视角。
...
TL;DR: 论文提出统一的算子理论框架描述神经网络训练动态，推导出谱传输-耗散PDE，解释双下降现象，连接NTK训练与特征学习。


<details>
  <summary>Details</summary>
Motivation: 现代深度网络在粗糙、有限正则性区域运行，其雅可比诱导算子表现出重尾谱和强基漂移。需要从梯度下降直接推导统一的算子理论描述来理解训练动态。

Method: 从梯度下降出发，在函数空间中推导精确演化方程，应用Kato扰动理论得到耦合模式ODE系统，粗粒化后收敛到谱传输-耗散PDE。分析弱耦合区域的自相似解。

Result: 证明神经训练保持函数正则性，迫使漂移取渐近幂律形式。在弱耦合区域，PDE允许自相似解，产生分辨率前沿、多项式振幅增长和幂律耗散。得到显式标度律指数，解释双下降几何。

Conclusion: NTK训练和特征学习是同一PDE的两个极限：v≡0恢复惰性动态，v≠0产生表示漂移。结果为现代深度网络的算子几何、优化动态和普适标度行为提供了统一的谱框架。
...
TL;DR: 该论文提出了一种基于人类认知科学的AI元认知评估框架，使用心理学指标meta-d'来衡量模型置信度预测自身准确性的可靠性，并利用这一动态敏感性分数进行测试时模型选择，从而提高联合推理准确性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然能表达预测置信度，但常常存在校准不良的问题，即置信度不能真实反映模型能力。受人类元认知（评估自身知识和判断可靠性的能力）启发，研究者希望探索AI模型是否真正"知道它们知道什么"，并建立评估AI元认知的新框架。

Method: 1. 引入心理学中的meta-d'指标作为元认知敏感性度量，用于表征模型置信度预测自身准确性的可靠性；2. 使用这一动态敏感性分数作为上下文，构建基于bandit的仲裁器，在测试时学习选择信任哪个专家模型；3. 在多个数据集和深度学习模型组合（包括CNN和VLM）上进行实验验证。

Result: 实验结果表明，这种元认知方法能够提高联合推理准确性，优于各个组成模型。该框架为AI模型提供了新颖的行为描述，将集成选择问题重新定义为评估短期信号（置信度预测分数）和中期特征（元认知敏感性）的问题。

Conclusion: 该研究从人类认知科学角度提出了评估和利用AI元认知的新框架，通过心理学指标meta-d'和bandit仲裁器实现了测试时模型选择，提高了模型性能，为理解AI模型的"自我认知"能力提供了新视角。
...
TL;DR: 提出一种基于高斯过程回归的混合物理-ML框架，用于准确预测正渗透水通量，并实现完整的预测不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 正渗透是一种有前景的低能耗膜分离技术，但传统机理模型受经验参数变化影响，纯数据驱动模型缺乏物理一致性和不确定性量化，需要开发更准确可靠的预测方法。

Method: 采用混合物理-ML框架，使用高斯过程回归训练物理模型预测值与实验水通量之间的残差误差，并通过Delta方法分解总预测方差为模型不确定性和输入不确定性。

Result: 仅使用120个数据点训练，在独立测试数据上达到0.26%的平均绝对百分比误差和0.999的R2值，实现了最先进的预测性能。

Conclusion: 该框架为高级正渗透工艺优化和数字孪生开发提供了真正稳健可靠的代理模型，具有低数据需求和高预测准确性的优势。
...
TL;DR: 提出T-SKM-Net框架，首次系统地将SKM方法集成到神经网络约束满足中，通过可训练采样Kaczmarz-Motzkin网络高效处理混合约束问题，在电力系统优化等安全关键应用中实现快速推理且零约束违反。


<details>
  <summary>Details</summary>
Motivation: 神经网络约束满足对电力系统优化、机器人路径规划、自动驾驶等安全关键应用至关重要，但现有方法面临效率与适用性的权衡，硬约束方法要么计算复杂度高，要么对约束结构有严格假设。SKM方法虽具有良好的收敛特性，但其argmax操作引入不可微性，给神经网络应用带来挑战。

Method: 提出T-SKM-Net框架：1) 通过零空间变换将混合约束问题转化为纯不等式问题；2) 使用SKM进行迭代求解；3) 将解映射回原始约束空间。框架提供后处理模式（仅推理）和联合训练模式（端到端训练），并基于无偏梯度估计器保证可训练性。

Result: 在DCOPF case118基准测试中，后处理模式实现4.27ms/项的GPU串行前向推理，最大最优性间隙0.0025%；联合训练模式实现5.25ms/项，最大最优性间隙0.0008%。相比pandapower求解器获得超过25倍加速，且在给定容差下保持零约束违反。

Conclusion: T-SKM-Net框架成功将SKM方法集成到神经网络约束满足中，解决了不可微操作的训练挑战，为安全关键应用提供了高效、可扩展的约束满足解决方案，在保持零约束违反的同时显著加速推理过程。
...
TL;DR: UACER提出了一种用于鲁棒对抗强化学习的新方法，通过多样化评论家集成和时间衰减不确定性机制来稳定训练过程，在MuJoCo控制任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 鲁棒对抗强化学习在自动驾驶和机器人控制等序列决策领域有重要应用，但传统的对抗训练方法存在学习动态非平稳性问题，导致训练不稳定和收敛困难，特别是在高维复杂环境中。

Method: UACER包含两个核心策略：1）多样化评论家集成：使用K个并行的多样化评论家网络来稳定Q值估计，替代传统的单评论家架构；2）时间衰减不确定性机制：开发基于方差的Q值聚合策略，显式纳入认知不确定性来动态调节探索-利用权衡，同时稳定训练过程。

Result: 在多个MuJoCo控制问题上进行的综合实验验证了UACER的优越有效性，在整体性能、稳定性和效率方面都优于最先进的方法。

Conclusion: UACER通过集成多样化的评论家网络和创新的不确定性机制，有效解决了鲁棒对抗强化学习中的训练不稳定问题，为高维复杂环境中的鲁棒策略学习提供了有效解决方案。
...
TL;DR: 提出自适应回放缓冲区（ARB），通过动态优先采样解决离线到在线强化学习中的数据平衡问题，使用轻量级"on-policyness"指标，无需复杂学习过程


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习面临固定离线数据集与新增在线经验之间的平衡困境，传统固定数据混合比例方法难以兼顾早期学习稳定性和渐进性能

Method: 引入自适应回放缓冲区（ARB），基于"on-policyness"指标动态优先采样数据，评估轨迹与当前策略行为的对齐程度，为每个转移分配比例采样权重

Result: 在D4RL基准测试中，ARB能持续缓解早期性能下降，显著提升各种O2O RL算法的最终性能

Conclusion: 自适应、行为感知的回放缓冲区设计对于离线到在线强化学习至关重要，ARB方法简单易实现且能有效平衡离线数据稳定性和在线经验相关性
...
TL;DR: 提出解耦蒸馏编码器(DDE)框架，通过师生蒸馏压缩模型，在资源受限设备上部署时保持解耦性，减少多标签OOD推理器大小


<details>
  <summary>Details</summary>
Motivation: 变分自编码器的解耦潜在空间可用于多标签分布外(OOD)样本推理，但现有模型过大，难以部署在资源受限设备上，需要压缩模型同时保持解耦特性

Method: 提出解耦蒸馏编码器(DDE)框架，将师生蒸馏形式化为带约束的优化问题，通过解耦约束保持潜在空间的解耦性，并基于Rademacher复杂度建立理论保证

Result: 在NVIDIA设备上实证评估，压缩后的模型在保持解耦性的同时显著减小了模型大小，适合资源受限设备部署

Conclusion: DDE框架能有效压缩多标签OOD推理器，在保持解耦性的同时减小模型规模，为资源受限设备上的部署提供了可行方案
...
Method: 提出变分模式寻求损失(VML)，通过最小化扩散后验与测量后验之间的KL散度，在反向扩散步骤中引导样本趋向MAP估计；对于线性逆问题可解析推导VML；基于此提出VML-MAP算法

Result: VML-MAP在多个数据集上的多样化图像恢复任务中，在性能和计算时间上都优于现有方法

Conclusion: VML-MAP为使用预训练扩散模型解决逆问题提供了高效准确的解决方案，无需任务特定训练，在理论和实证上都表现出色
...
### [183] [Unlocking the Address Book: Dissecting the Sparse Semantic Structure of LLM Key-Value Caches via Sparse Autoencoders](https://arxiv.org/abs/2512.10547)
...
<details>
  <summary>Details</summary>
Motivation: KV缓存是长上下文大语言模型的主要内存瓶颈，但通常被视为不透明的数值张量。需要一种方法既能减少内存占用，又能提供KV缓存的语义解释性。

Method: 提出STA-Attention框架，使用Top-K稀疏自编码器（SAEs）分解KV缓存为语义原子。不同于标准L1正则化SAEs，Top-K方法消除收缩偏差，保持注意力机制所需的精确点积几何。基于发现的Key-Value不对称性（Key作为稀疏路由器，Value携带密集内容），引入双预算策略选择性保留信息量最大的语义成分。

Result: 在Yi-6B、Mistral-7B、Qwen2.5-32B等模型上的实验表明，语义重建能够保持与原始模型相当的困惑度和零样本性能，有效弥合机制可解释性与忠实注意力建模之间的差距。

Conclusion: STA-Attention框架成功地将KV缓存从数值张量转化为可解释的语义表示，通过Top-K稀疏自编码器和双预算策略，在保持模型性能的同时实现了KV缓存的语义分解和噪声过滤。
...
### [184] [Asynchronous Reasoning: Training-Free Interactive Thinking LLMs](https://arxiv.org/abs/2512.10931)
*George Yakushev,Nataliia Babina,Masoud Vahid Dastgerdi,Vyacheslav Zhdanovskiy,Alina Shutova,Denis Kuznedelev*

Main category: cs.LG

TL;DR: 提出一种无需额外训练的方法，使LLM能够同时思考、听取和生成输出，大幅减少实时响应延迟


<details>
  <summary>Details</summary>
Motivation: 现有LLM需要先思考再回答，导致实时交互性差，无法适应语音助手等需要实时响应的场景，而人类可以异步思考、听取和行动

Method: 利用旋转嵌入的特性，使原本设计用于顺序交互的LLM能够同时进行思考、听取和生成输出

Result: 在数学推理、常识推理和安全推理任务上，能够实时生成准确的思考增强答案，将第一个非思考token的生成时间从几分钟减少到≤5秒，整体实时延迟降低6-11倍

Conclusion: 通过利用旋转嵌入的特性，成功实现了LLM的异步思考能力，显著提升了实时交互性能，为语音助手等应用场景提供了可行的解决方案
...
### [185] [Is the Information Bottleneck Robust Enough? Towards Label-Noise Resistant Information Bottleneck Learning](https://arxiv.org/abs/2512.10573)
*Yi Huang,Qingyun Sun,Yisen Gao,Haonan Yuan,Xingcheng Fu,Jianxin Li*

Main category: cs.LG

TL;DR: LaT-IB是一种抗标签噪声的信息瓶颈方法，通过最小充分干净准则和噪声感知潜在解耦，在标签噪声场景下实现鲁棒表示学习。


<details>
  <summary>Details</summary>
Motivation: 传统信息瓶颈方法严重依赖准确标签，在现实场景中普遍存在的标签噪声下容易性能下降和过拟合，需要开发对标签噪声鲁棒的表示学习方法。
...
Conclusion: LaT-IB通过创新的噪声感知解耦和三阶段训练策略，有效解决了信息瓶颈方法对标签噪声的脆弱性问题，为现实世界标签噪声场景提供了鲁棒的表示学习解决方案。
...
### [186] [Stronger Normalization-Free Transformers](https://arxiv.org/abs/2512.10938)
*Mingzhi Chen,Taiming Lu,Jiachen Zhu,Mingjie Sun,Zhuang Liu*

Main category: cs.LG

TL;DR: 本文提出了一种新的点状函数Derf（erf(αx + s)）来替代传统的归一化层，在多个领域超越了LayerNorm、RMSNorm和DyT的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然归一化层长期以来被视为深度学习架构中不可或缺的组件，但最近提出的Dynamic Tanh（DyT）表明存在替代方案。DyT通过约束极值实现稳定收敛并达到归一化级别的性能，本研究旨在寻找能够超越DyT的函数设计。

Method: 首先研究点状函数的内在特性如何影响训练和性能，然后基于这些发现进行大规模搜索以寻找更有效的函数设计。最终提出了Derf(x) = erf(αx + s)，其中erf(x)是重新缩放的高斯累积分布函数。
...
TL;DR: THeGAU：一个模型无关的异构图神经网络框架，通过类型感知图自编码器和引导式图增强，解决类型信息丢失和结构噪声问题，提升节点分类性能。
...
Method: 提出THeGAU框架，包含：1）类型感知图自编码器，通过重构符合模式的边作为辅助任务来保留节点类型语义；2）解码器驱动的增强机制，选择性精炼噪声结构。

Result: 在三个基准HIN数据集（IMDB、ACM、DBLP）上的实验表明，THeGAU在多个骨干网络上均优于现有HGNN方法，达到最先进性能，同时显著降低计算开销。

Conclusion: THeGAU通过联合设计类型感知重建和引导式图增强，有效提升了异构图神经网络的鲁棒性、准确性和效率，解决了类型信息丢失和结构噪声问题。
...
TL;DR: 该论文在约束强化学习领域提出了一系列理论框架和算法，涵盖平均成本CMDP、有限时域CMDP、人类偏好学习以及大语言模型对齐等多个方向，为安全对齐的决策系统提供了理论保证和实用工具。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在安全关键环境和人类偏好对齐方面面临挑战，需要开发能够处理约束、整合人类反馈、并具有理论保证的算法，特别是在大语言模型对齐等复杂场景中。
...
Result: ACPO在平均成本CMDP中实现最先进的性能；e-COP在安全关键环境中提供可证明的性能保证；warmPref-PS显著减少遗憾并提高数据收集效率；PSPL提供贝叶斯简单遗憾保证并稳健识别最优策略；MOPO可扩展到数十亿参数的语言模型并在各种对齐设置中保持鲁棒性。

Conclusion: 该论文统一了平均成本、时域和偏好驱动的约束强化学习范式，为安全和对齐的决策系统提供了理论进展和实用工具，特别在大语言模型对齐等实际应用中具有重要价值。
...
TL;DR: 本文提出了一种针对贝叶斯神经网络的多层次量化框架，能够在4位精度下实现8倍内存压缩，同时保持分类准确性和不确定性校准。
...
Method: 提出了一个系统性的多层次量化框架，基于随机变分推理的BNNs，区分三种量化策略：变分参数量化（VPQ）、采样参数量化（SPQ）和联合量化（JQ）。采用方差参数的对数量化，以及专门的激活函数来保持分布结构。

Result: 在Dirty-MNIST数据集上的实验表明，BNNs可以量化到4位精度，同时保持分类准确性和不确定性解耦。在4位精度下，联合量化相比浮点实现实现了高达8倍的内存减少，且认知不确定性和偶然不确定性估计的退化最小。
...
### [190] [Supporting Migration Policies with Forecasts: Illegal Border Crossings in Europe through a Mixed Approach](https://arxiv.org/abs/2512.10633)
...
Method: 混合方法：机器学习技术+移民专家定性洞察，引入人工评估协变量改进数据驱动模型的预测能力

Result: 方法经过已知数据测试验证，证明在移民政策背景下的适用性和可靠性，提供政策相关预测

Conclusion: 该方法响应欧盟政策需求，结合数据建模与专家判断，为欧盟移民治理提供创新的操作工具
...
### [191] [Token Sample Complexity of Attention](https://arxiv.org/abs/2512.10656)
*Léa Bohbot,Cyril Letrouit,Gabriel Peyré,François-Xavier Vialard*

Main category: cs.LG

TL;DR: 本文研究了大型语言模型中注意力机制在极端序列长度下的收敛行为，提出了token-sample复杂性概念，分析了注意力图在有限token数下的收敛速率。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型上下文窗口不断扩大，需要理解注意力机制在极端序列长度下的行为特性，特别是注意力如何随token数量增加而收敛到其无限token极限。

Method: 引入token-sample复杂性概念，从两个层面分析收敛性：1) 注意力图的逐点一致收敛；2) 变换后token分布矩的收敛。针对紧支撑分布和次高斯分布，推导有限n下的收敛界，并分析注意力参数趋于无穷时softmax趋近hardmax的情况。

Result: 对于紧支撑分布，注意力图在半径为R的球上以C(R)/√n速率一致收敛，其中C(R)随R指数增长。对于变换后分布矩的收敛，速率为C'(R)/n^β（β<1/2），C'(R)与分布支撑大小多项式相关。在注意力参数趋于无穷时，获得对数收敛速率。实验在合成高斯数据和真实BERT模型上验证了理论预测。

Conclusion: 本文系统分析了注意力机制在长序列下的收敛行为，建立了理论收敛界，揭示了注意力图收敛速率与序列长度、分布特性及注意力几何的关系，为大语言模型长上下文处理提供了理论指导。
...
### [192] [DCFO Additional Material](https://arxiv.org/abs/2512.10659)
...
<details>
  <summary>Details</summary>
Motivation: 异常检测需要解释性，特别是对于广泛使用的LOF算法。现有反事实解释方法大多未针对异常检测的特殊挑战设计，也不支持经典异常检测算法。

Method: DCFO将数据空间划分为LOF行为平滑的区域，实现高效的基于梯度的优化，专门为LOF生成反事实解释。

Result: 在50个OpenML数据集上的实验表明，DCFO在生成反事实的邻近性和有效性方面持续优于基准方法。

Conclusion: DCFO成功解决了LOF算法缺乏可解释性的问题，为异常检测提供了专门的反事实解释方法，具有实际应用价值。
...
### [193] [Learning by Analogy: A Causal Framework for Composition Generalization](https://arxiv.org/abs/2512.10669)
*Lingjing Kong,Shaoan Xie,Yang Jiao,Yetian Chen,Yanhui Guo,Simone Shao,Yan Gao,Guangyi Chen,Kun Zhang*

Main category: cs.LG

TL;DR: 该论文提出了一种基于因果模块化和最小变化原则的层次化数据生成过程，能够从可观测数据中可识别地恢复潜在概念结构，从而支持复杂关系的组合泛化。


<details>
  <summary>Details</summary>
Motivation: 组合泛化能力使模型能够超越有限经验理解新概念组合，但现有方法的数据结构和原理尚不明确。论文旨在形式化人类通过类比将高级概念分解为基本概念进行重组的直觉过程。

Method: 引入基于因果模块化和最小变化原则的层次化数据生成过程，编码不同层次概念及其交互机制。理论证明该潜在层次结构可从文本-图像对等可观测数据中可识别地恢复。

Result: 理论分析表明该方法支持复杂概念关系的组合泛化，超越先前仅假设简单交互（如加性效应）的工作。在基准数据集上应用理论框架取得了显著改进。

Conclusion: 通过因果模块化和层次化结构的形式化，该研究为组合泛化提供了理论基础，证明了潜在概念结构的可识别性，并在实践中验证了其有效性。
...
### [194] [HybridVFL: Disentangled Feature Learning for Edge-Enabled Vertical Federated Multimodal Classification](https://arxiv.org/abs/2512.10701)
*Mostafa Anoosha,Zeinab Dehghani,Kuniko Paxton,Koorosh Aslansefat,Dhavalkumar Thakker*

Main category: cs.LG

TL;DR: HybridVFL：一种新型垂直联邦学习框架，通过客户端特征解耦和服务器端跨模态Transformer实现上下文感知融合，在皮肤病变诊断等边缘AI场景中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统垂直联邦学习在移动健康诊断等边缘AI场景中存在性能限制，主要原因是简单的特征融合方法无法有效处理分布式设备上的敏感多模态数据。
...
TL;DR: 该论文提出基于因果最小化原则的理论框架，为深度生成模型提供可解释的潜在表示和组件级可控性，通过稀疏性或压缩约束实现与真实潜在变量的等价性。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型虽然革命性地改变了图像和文本生成领域，但大多作为不透明的黑箱运行，阻碍了人类的理解、控制和对齐。现有方法如稀疏自编码器虽然经验上成功，但缺乏理论保证，可能导致主观见解。

Method: 引入基于因果最小化原则的理论框架，提出分层选择模型，其中高层概念由低层变量的受限组合产生。在理论推导的最小化条件（表现为稀疏性或压缩约束）下，学习到的表示可以与数据生成过程的真实潜在变量等价。

Result: 将约束应用于领先的生成模型，能够提取其固有的分层概念图，提供对其内部知识组织的新见解。这些基于因果的概念可作为细粒度模型控制的杠杆。

Conclusion: 因果最小化原则可以为可解释生成模型建立理论基础，使潜在表示具有清晰的因果解释和鲁棒的组件级可识别控制，为透明、可靠的系统铺平道路。
...
TL;DR: 提出基于可设计球面格林函数的神经算子框架GSNO，通过谱学习方法在球面域上平衡等变性和不变性，应用于扩散MRI、浅水动力学和全球天气预报等任务。


<details>
  <summary>Details</summary>
Motivation: 现有球面神经算子虽然具有旋转等变性，但缺乏处理现实世界复杂性的灵活性，需要一种既能保持几何特性又能适应各向异性约束系统的算子设计框架。

Method: 基于可设计球面格林函数及其谐波展开建立算子理论基础，提出绝对和相对位置相关的格林函数，开发GSNO算子和谱学习方法，构建GSHNet层次架构结合多尺度谱建模与球面上下采样。

Result: 在扩散MRI、浅水动力学和全球天气预报等任务上，GSNO和GSHNet持续超越现有最佳方法，证明了框架的有效性和通用性。

Conclusion: GSNO为球面算子学习提供了一个原则性通用框架，将严格理论与现实复杂性相结合，为球面域上的参数偏微分方程求解提供了新途径。
...
TL;DR: LGAN通过构建以每个节点为中心的诱导子图的线图进行高阶聚合，在保持较低时间复杂度的同时获得了比2-WL更强的表达能力，并在基准测试中优于现有k-WL GNNs，同时提供更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有GNNs主要依赖邻居节点间的消息传递，表达能力受限于1-WL测试。虽然已有k-WL-based GNNs试图突破这一限制，但计算成本随k快速增加，且这些方法主要操作节点元组，无法保留归因方法所需的细粒度节点或边级语义，导致可解释性问题。

Method: 提出Line Graph Aggregation Network (LGAN)，从以每个节点为中心的诱导子图构建线图进行高阶聚合。该方法在注入聚合假设下具有比2-WL更强的表达能力，同时保持较低的时间复杂度。

Result: 在基准测试中，LGAN优于最先进的k-WL-based GNNs，同时提供更好的可解释性。

Conclusion: LGAN成功克服了现有k-WL GNNs的计算成本高和可解释性差的问题，在保持高效的同时获得了更强的表达能力，为图分类任务提供了更优的解决方案。
...
TL;DR: 提出一种基于Transformer的无模板逆合成反应预测框架，通过分子图信息注入注意力机制和配对数据增强策略，在USPTO-50K基准上达到无模板方法的最优性能
...
Method: 采用无模板的Transformer框架，将分子图信息注入注意力机制以联合利用SMILES序列和结构线索，并应用配对数据增强策略提升训练多样性和规模

Result: 在USPTO-50K基准测试中，该方法在无模板方法中达到最先进的性能，显著优于基础的Transformer基线模型

Conclusion: 提出的无模板Transformer框架通过结合分子结构信息和数据增强策略，有效提升了逆合成反应预测的准确性和鲁棒性，为实际应用提供了可行方案
...
### [199] [UniExtreme: A Universal Foundation Model for Extreme Weather Forecasting](https://arxiv.org/abs/2508.01426)
...
<details>
  <summary>Details</summary>
Motivation: 现有天气预测基础模型主要关注一般天气条件或特定类型的极端事件，忽视了现实世界中多样化极端事件的大气模式。极端事件具有两个关键特征：与正常天气状态的光谱差异，以及多样化极端事件的分层驱动因素和地理混合。

Method: 提出了UniExtreme模型，包含两个核心模块：1) 自适应频率调制(AFM)模块，通过可学习的Beta分布滤波器和多粒度光谱聚合捕获区域性的光谱差异；2) 事件先验增强(EPA)模块，通过双级记忆融合网络融入区域特定的极端事件先验，解决极端多样性和复合极端模式。

Result: 大量实验表明，UniExtreme在极端天气和一般天气预测方面都优于最先进的基线模型，展示了在各种极端场景下的卓越适应性。

Conclusion: UniExtreme通过整合光谱差异建模和事件先验增强，成功解决了多样化极端天气事件的预测挑战，为通用极端天气预测基础模型提供了有效解决方案。
...
### [200] [Interpretable and Steerable Concept Bottleneck Sparse Autoencoders](https://arxiv.org/abs/2512.10805)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAEs）在机制可解释性、概念发现和模型控制方面有潜力，但现有方法存在两个问题：1）多数神经元解释性或可控性低，下游应用效果差；2）无监督学习导致用户所需概念往往缺失，实用性受限。

Method: 提出概念瓶颈稀疏自编码器（CB-SAE），包含两个核心改进：1）剪枝低效用神经元；2）在潜在空间中添加轻量级概念瓶颈层，该层与用户定义的概念集对齐。

Result: CB-SAE在大视觉语言模型和图像生成任务中，解释性提升32.1%，可控性提升14.5%。

Conclusion: CB-SAE框架有效解决了传统稀疏自编码器的局限性，通过概念瓶颈层显著提升了模型的可解释性和可控性，为下游应用提供了更实用的工具。
...
### [201] [Extrapolation of Periodic Functions Using Binary Encoding of Continuous Numerical Values](https://arxiv.org/abs/2512.10817)
*Brian P. Powell,Jordan A. Caraballo-Vega,Mark L. Carroll,Thomas Maxwell,Andrew Ptak,Greg Olmschenk,Jorge Martinez-Palomera*

Main category: cs.LG

TL;DR: 论文发现二进制编码使神经网络能够外推训练范围之外的周期函数，提出归一化二进制编码(NB2E)方法，使普通MLP无需函数形式先验知识即可成功外推各种周期信号。


<details>
  <summary>Details</summary>
Motivation: 神经网络通常难以外推训练范围之外的数据，特别是周期函数。现有方法需要先验知识或特殊架构，本文旨在探索是否可以通过简单的输入编码使标准神经网络获得外推能力。

Method: 提出归一化二进制编码(NB2E)，将连续数值编码为二进制表示。使用普通多层感知机(MLP)作为基础架构，无需特殊设计或先验知识。通过内部激活分析揭示编码机制。

Result: NB2E使普通MLP成功外推各种周期信号，包括正弦波、方波等。内部激活分析显示编码诱导了位相表示，使网络能够独立于位置学习信号结构。

Conclusion: 二进制编码为神经网络提供了一种简单有效的周期函数外推方法，揭示了输入表示对网络外推能力的重要影响，为改进神经网络泛化能力提供了新思路。
...
### [202] [Learning Controllable and Diverse Player Behaviors in Multi-Agent Environments](https://arxiv.org/abs/2512.10835)
*Atahan Cilan,Atay Özgövde*

Main category: cs.LG

TL;DR: 提出强化学习框架，无需人类游戏数据即可实现可控多样的玩家行为，通过行为向量距离奖励机制学习行为控制
...
Method: 将玩家行为定义为N维连续空间，均匀采样目标行为向量；训练时每个智能体接收当前和目标行为向量作为输入，奖励基于两者距离的归一化减少；使用PPO多智能体策略

Result: 在自定义多玩家Unity游戏中，相比仅以胜利为目标的基线，该方法产生显著更大的行为多样性，并能可靠匹配指定的行为向量；单个策略可重现新或未见过的游戏风格而无需重新训练
...
### [203] [Bayesian Symbolic Regression via Posterior Sampling](https://arxiv.org/abs/2512.10849)
...
Method: 采用序贯蒙特卡洛框架进行贝叶斯符号回归，结合概率选择、自适应退火和使用归一化边际似然，高效探索符号表达式的搜索空间。

Result: 与传统的遗传编程方法相比，该方法在具有挑战性的噪声基准数据集上表现更好，减少了过拟合倾向，并能发现更准确、可解释的方程。

Conclusion: 该方法为科学发现和工程设计中更鲁棒的符号回归铺平了道路，通过增强的泛化能力和不确定性量化为噪声环境下的符号回归提供了新途径。
...
### [204] [Generative Modeling from Black-box Corruptions via Self-Consistent Stochastic Interpolants](https://arxiv.org/abs/2512.10857)
*Chirag Modi,Jiequn Han,Eric Vanden-Eijnden,Joan Bruna*

Main category: cs.LG

TL;DR: 提出SCSI方法，通过迭代更新污染数据与干净数据之间的传输映射，仅需访问污染数据集和污染通道的黑盒访问，即可构建干净数据的生成模型


<details>
  <summary>Details</summary>
Motivation: 在许多科学和工程领域，我们只能观测到通过噪声、病态通道污染的测量数据，而无法获得干净数据。需要一种方法能在分布层面解决逆问题，构建原始数据的生成模型

Method: 基于随机插值子的自洽方法，迭代更新污染数据与干净数据样本之间的传输映射，仅需访问污染数据集和污染通道的黑盒访问，无需干净数据

Result: SCSI方法在自然图像处理和科学重建的逆问题上表现出优越性能，计算效率高，能处理任意非线性前向模型，具有理论保证

Conclusion: SCSI方法为从污染数据构建干净数据生成模型提供了一种高效、灵活且理论可靠的新方法，特别适用于只能获得污染测量的科学和工程领域
...
### [205] [Scaling Behavior of Discrete Diffusion Language Models](https://arxiv.org/abs/2512.10858)
...
Method: 通过平滑插值掩码扩散和均匀扩散来研究DLMs在不同噪声类型下的缩放行为，重点关注批量大小和学习率等关键超参数。实验比较了不同噪声类型的缩放规律，并将均匀扩散模型扩展到100亿参数，训练计算量达10^22 FLOPs。

Result: DLMs的缩放行为强烈依赖于噪声类型，与ALMs有显著不同。在计算受限的缩放中，所有噪声类型都能收敛到相似的损失值；但在计算效率训练中，均匀扩散比掩码扩散需要更多参数但更少数据，使其在数据受限场景中具有优势。成功将均匀扩散模型扩展到100亿参数，验证了预测的缩放规律。

Conclusion: 均匀扩散模型在数据受限场景中表现出色，需要更多参数但更少数据，是计算效率训练的有前景候选方案。研究证实了DLMs缩放行为对噪声类型的依赖性，并成功构建了迄今为止最大的公开均匀扩散模型。
...
### [206] [UrbanAI 2025 Challenge: Linear vs Transformer Models for Long-Horizon Exogenous Temperature Forecasting](https://arxiv.org/abs/2512.10866)
*Ruslan Gokhman*

Main category: cs.LG

TL;DR: 线性模型（特别是DLinear）在仅使用历史室内温度数据的长期预测任务中，优于复杂的Transformer架构，表明精心设计的线性模型在时间序列预测中仍是强大的基线。


<details>
  <summary>Details</summary>
Motivation: 研究在仅使用历史温度数据（无外部变量）的挑战性单变量设置下，进行长期温度预测，比较线性模型和Transformer家族模型的性能。

Method: 使用Linear、NLinear、DLinear、Transformer、Informer和Autoformer模型，在标准化的训练、验证和测试分割下，进行长期外生变量温度预测评估。

Result: 线性基线模型（Linear、NLinear、DLinear）在所有分割中始终优于更复杂的Transformer架构，其中DLinear在所有分割中实现了最佳整体准确率。

Conclusion: 精心设计的线性模型在仅使用外生变量的挑战性时间序列预测场景中，仍然是强大的基线方法，复杂模型不一定带来性能提升。
...
### [207] [Guided Transfer Learning for Discrete Diffusion Models](https://arxiv.org/abs/2512.10877)
...
<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在语言等离散领域表现优异，但需要大量训练数据，迁移到新领域成本高且风险大。现有迁移学习方法需要微调大型扩散模型，计算成本高且不实用。

Method: 基于连续扩散的比率迁移学习，提出离散扩散模型的引导迁移学习(GTL)，无需修改预训练去噪器即可从目标分布采样。进一步提出高效引导采样器，通过规划器选择位置和候选词来降低计算成本。

Result: GTL方法在序列数据（包括合成马尔可夫链和语言建模）上进行了评估，展示了其实际应用价值。高效采样器使大规模语言建模在大型词汇和长序列上变得实用。

Conclusion: GTL为离散扩散模型提供了一种无需微调的迁移学习方法，通过引导采样实现了高效适应新领域，解决了大规模离散扩散模型迁移学习的计算瓶颈问题。
...
### [208] [Classifier Reconstruction Through Counterfactual-Aware Wasserstein Prototypes](https://arxiv.org/abs/2512.10878)
...
<details>
  <summary>Details</summary>
Motivation: 反事实解释通常位于决策边界附近，可以作为信息丰富但代表性较弱的样本。在标记数据有限的情况下，如何有效利用反事实样本来改进模型重构，避免决策边界偏移问题。

Method: 将原始数据样本与反事实样本结合，使用Wasserstein重心近似每个类别的原型，从而保留类别的分布结构。这种方法避免了将反事实样本简单视为普通训练实例的问题。

Result: 在多个数据集上的实验结果表明，该方法提高了代理模型与目标模型之间的保真度，验证了其有效性。

Conclusion: 通过合理利用反事实样本作为信息补充，结合Wasserstein重心方法近似类别原型，可以有效改进模型重构质量，特别是在数据有限的情况下。
...
### [209] [Physics-Informed Learning of Flow Distribution and Receiver Heat Losses in Parabolic Trough Solar Fields](https://arxiv.org/abs/2512.10886)
*Stefan Matthes,Markus Schramm*

Main category: cs.LG

TL;DR: 提出一个物理信息学习框架，从CSP电厂运行数据中推断回路质量流量比和接收器热损失系数，利用夜间均质化过程分离水力与热效应


<details>
  <summary>Details</summary>
Motivation: CSP电厂的大型液压网络需要均匀出口温度，但存在光学性能不均、热损失和压降差异。回路温度可测，但质量流量和接收器热损参数未知，无法诊断水力失衡或接收器退化

Method: 利用夜间均质化过程（无辐照时热油循环）分离水力与热损失效应；将可微分共轭传热模型离散化并嵌入端到端学习管道，使用Andasol 3太阳能电厂的50MW历史数据进行优化

Result: 模型准确重建回路温度（RMSE <2°C），产生物理意义明确的水力失衡和接收器热损失估计；与无人机红外热成像（QScan）对比显示强相关性，正确识别所有高损失接收器区域

Conclusion: 现实世界的CSP运行数据包含足够信息来恢复潜在物理参数，当结合适当的建模和可微分优化时，能够有效诊断水力失衡和接收器退化问题
...
### [210] [SparseSwaps: Tractable LLM Pruning Mask Refinement at Scale](https://arxiv.org/abs/2512.10922)
*Max Zimmer,Christophe Roux,Moritz Wagner,Deborah Hendrych,Sebastian Pokutta*

Main category: cs.LG

TL;DR: 提出一种针对大语言模型的高效剪枝方法，通过行级等稀疏约束和1-swap优化算法，显著降低剪枝误差并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的资源需求巨大，传统剪枝方法如全局幅度剪枝在Transformer架构上效果不佳，而基于整数规划的精确求解又计算不可行。现有方法依赖近似或启发式算法，需要更高效、更优的剪枝解决方案。

Method: 提出一种可扩展的1-swap剪枝算法：1) 通过行级等稀疏约束解耦行间依赖；2) 利用校准数据的Gram矩阵高效计算最优1-swap（交换一个保留权重和一个剪枝权重）；3) 从任意剪枝掩码开始，在GPU上高效运行，基本无需超参数调优。

Result: 相比Wanda方法，将每层剪枝误差降低高达60%；在多种GPT架构上，持续改善困惑度和零样本准确率；算法在LLM规模上计算可行且高效。

Conclusion: 通过行级等稀疏约束和高效的1-swap优化，成功解决了大语言模型剪枝中的可扩展性问题，提供了一种计算可行、性能优越的剪枝方法，显著优于现有技术。
...
### [211] [Digital Twin Supervised Reinforcement Learning Framework for Autonomous Underwater Navigation](https://arxiv.org/abs/2512.10925)
...
<details>
  <summary>Details</summary>
Motivation: 水下环境自主导航面临GPS缺失、能见度差、障碍物多等挑战，需要开发鲁棒的导航方法。BlueROV2作为广泛使用的开源平台，为研究提供了良好基础。

Method: 采用基于PPO算法的深度强化学习，观察空间结合目标导航信息、虚拟占用网格和操作区域边界的射线投射。与DWA确定性运动规划器进行对比。

Result: PPO策略在高度杂乱环境中持续优于DWA，表现出更好的局部适应性和更少的碰撞。通过3D数字孪生监督的实物验证，证明了从仿真到真实世界的可迁移性。
...
### [212] [Decoupled Q-Chunking](https://arxiv.org/abs/2512.10926)
*Qiyang Li,Seohong Park,Sergey Levine*

Main category: cs.LG

TL;DR: 提出一种新算法，通过解耦评论家与策略的动作块长度，使用蒸馏评论家优化策略，解决长动作块策略学习的挑战，在长时域离线目标条件任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 时间差分方法存在自引导偏差问题，块状评论家虽然加速了价值备份，但要求策略以开环方式输出整个动作块，这在需要策略反应性的环境中可能次优，且长动作块建模困难。

Method: 提出解耦评论家与策略的动作块长度，通过从原始块状评论家乐观回溯构建蒸馏评论家，近似部分动作块扩展为完整块时可达到的最大价值，从而优化策略。

Result: 在具有挑战性的长时域离线目标条件任务上评估，该方法可靠地优于先前方法。

Conclusion: 该方法保留了多步价值传播的优势，同时避免了开环次优性和长动作块策略学习的困难，为块状评论家与策略的协同优化提供了有效解决方案。
...
### [213] [Empirical evaluation of the Frank-Wolfe methods for constructing white-box adversarial attacks](https://arxiv.org/abs/2512.10936)
*Kristina Korotkova,Aleksandr Katrutsa*

Main category: cs.LG

TL;DR: 提出使用改进的Frank-Wolfe方法（投影自由方法）构建高效白盒对抗攻击，相比基于投影或几何直觉的标准方法更有效
...
Method: 采用改进的Frank-Wolfe方法（投影自由方法）构建白盒对抗攻击，避免传统方法中的投影操作。在MNIST和CIFAR-10数据集上，使用多类逻辑回归模型、卷积神经网络和Vision Transformer进行实验验证

Result: 通过理论分析和数值实验证明，改进的Frank-Wolfe方法在构建对抗攻击方面比基于投影操作或几何直觉的标准方法更有效

Conclusion: 投影自由方法（特别是改进的Frank-Wolfe方法）为构建高效对抗攻击提供了有前景的数值优化方案，能够更好地评估神经网络的对抗鲁棒性
...
### [214] [Hierarchical Dataset Selection for High-Quality Data Sharing](https://arxiv.org/abs/2512.10952)
*Xiaona Zhou,Yingyan Zeng,Ran Jin,Ismini Lourentzou*

Main category: cs.LG

TL;DR: DaSH方法通过层次化建模数据集和组级效用，在资源约束下选择整个数据集以提升下游性能，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实场景中数据通常以离散数据集形式组织，这些数据集在相关性、质量和效用上存在差异。现有方法通常选择单个样本且将所有数据视为同等相关，忽略了数据集及其来源之间的差异，因此需要一种能够选择整个数据集的方法。

Method: 提出DaSH（Dataset Selection via Hierarchies）方法，在数据集和组（如集合、机构）两个层次上建模效用，通过层次化建模实现从有限观察中的高效泛化。

Result: 在两个公共基准测试（Digit-Five和DomainNet）中，DaSH比最先进的数据选择基线方法在准确率上提升了高达26.2%，同时需要显著更少的探索步骤。消融实验表明DaSH在低资源设置和缺乏相关数据集的情况下具有鲁棒性。

Conclusion: DaSH适用于实际多源学习工作流中的可扩展和自适应数据集选择，能够有效处理异构数据集池中的数据集选择问题。
...
### [215] [Bidirectional Normalizing Flow: From Data to Noise and Back](https://arxiv.org/abs/2512.10953)
*Yiyang Lu,Qiao Sun,Xianbang Wang,Zhicheng Jiang,Hanhong Zhao,Kaiming He*

Main category: cs.LG

TL;DR: BiFlow是一种双向归一化流框架，通过近似逆映射而非精确解析逆，解决了因果解码瓶颈问题，在ImageNet上实现了质量提升和百倍加速采样。


<details>
  <summary>Details</summary>
Motivation: 传统归一化流需要精确解析逆变换，导致因果解码成为主要瓶颈。最近TARFlow等结合Transformer和自回归流的方法虽然复兴了NF，但因果解码限制了效率和灵活性。

Method: 提出双向归一化流(BiFlow)框架，放弃精确解析逆的要求，学习一个近似噪声到数据的逆映射模型。这使得可以使用更灵活的损失函数和架构，无需因果解码约束。

Result: 在ImageNet上，BiFlow相比因果解码对应方法，生成质量提升的同时采样速度加速了100倍。在基于NF的方法中达到SOTA，在单评估(1-NFE)方法中具有竞争力。
...
### [216] [CC-GRMAS: A Multi-Agent Graph Neural System for Spatiotemporal Landslide Risk Assessment in High Mountain Asia](https://arxiv.org/abs/2510.20875)
...
<details>
  <summary>Details</summary>
Motivation: 滑坡是日益严重的气候诱发灾害，在亚洲高山地区造成严重的环境和人类影响。尽管卫星和时间数据集不断增加，但及时的滑坡检测和灾害响应仍然不发达且分散。需要开发更准确、及时的滑坡预测和响应系统。

Method: 提出CC-GRMAS框架，利用系列卫星观测和环境信号。系统围绕三个相互关联的智能体构建：预测、规划和执行。这些智能体协作实现实时态势感知、响应规划和干预。方法结合了当地环境因素，并实现了多智能体协调操作。

Result: 该方法提高了滑坡预测的准确性，能够实现实时态势感知和灾害响应规划。通过多智能体协作和本地环境因素的整合，为脆弱山区提供了可扩展的主动解决方案。

Conclusion: CC-GRMAS框架通过整合卫星数据、环境信号和多智能体协调，为气候适应型灾害准备提供了可扩展的主动解决方案，特别适用于亚洲高山等脆弱山区地形。
...
### [217] [Search-based Software Testing Driven by Domain Knowledge: Reflections and New Perspectives](https://arxiv.org/abs/2512.10079)
*Federico Formica,Mark Lawford,Claudio Menghi*

Main category: cs.SE

TL;DR: 本文回顾了基于搜索的软件测试（SBST）中集成领域知识的最新实验结果，突出展示了大胆和意外的发现，为未来研究提供新视角。


<details>
  <summary>Details</summary>
Motivation: SBST能自动生成大量测试用例，但缺乏工程师的领域知识。虽然已有技术尝试在SBST框架中集成领域知识，但需要重新审视这些方法的有效性，从新角度评估其实际价值。

Method: 通过回顾和分析近期SBST中集成领域知识的实验结果，重点突出那些大胆和意外的发现，对现有技术进行批判性反思。

Result: 揭示了SBST集成领域知识技术中一些反直觉的发现，挑战了传统认知，为重新评估这些方法提供了实证基础。

Conclusion: 需要从新视角重新审视领域知识驱动的SBST技术，这些意外结果为未来研究方向提供了重要启示，可能改变该领域的研究范式。
...
### [218] [ATLAS: Automated Toolkit for Large-Scale Verified Code Synthesis](https://arxiv.org/abs/2512.10173)
*Mantas Baksys,Stefan Zetzsche,Olivier Bouissou,Remi Delmas,Soonho Kong*

Main category: cs.SE

TL;DR: ATLAS通过自动合成已验证的Dafny程序解决LLM程序验证训练数据稀缺问题，显著提升模型验证能力


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在程序验证方面有潜力，但缺乏已验证代码的训练数据阻碍了进展

Method: ATLAS自动化流水线合成带规范、实现和证明的完整Dafny程序，将合成过程分解为多个专门任务，生成2.7K已验证程序并提取19K+训练示例

Result: 在Qwen 2.5 7B Coder上微调后，DafnyBench提升23个百分点，DafnySynthesis提升50个百分点

Conclusion: 合成已验证代码能有效增强LLM的形式验证能力
...
### [219] [Does SWE-Bench-Verified Test Agent Ability or Model Memory?](https://arxiv.org/abs/2512.10218)
*Thanosan Prathifkumar,Noble Saji Mathews,Meiyappan Nagappan*

Main category: cs.SE

TL;DR: 研究发现SWE-Bench-Verified基准测试可能被模型训练数据污染，导致分数反映的是记忆而非真实问题解决能力，建议转向考虑数据污染的新数据集。


<details>
  <summary>Details</summary>
Motivation: SWE-Bench-Verified作为评估LLMs解决GitHub问题能力的基准测试，可能存在与模型训练数据重叠的问题，导致分数不能真实反映模型的问题解决能力，而是反映训练记忆。

Method: 测试两个在基准测试中表现优异的Claude模型，让它们仅使用问题文本定位相关文件，然后使用问题文本加文件路径。在BeetleBox和SWE-rebench基准上进行相同测试，比较模型在不同基准上的表现差异。

Result: 模型在SWE-Bench-Verified上的表现比在其他基准上好3倍，在定位编辑文件方面好6倍，尽管这些基准都涉及相同的开源Python项目。这表明模型可能在训练中见过SWE-Bench-Verified任务。

Conclusion: SWE-Bench-Verified基准测试分数可能不能真实反映代理处理实际软件问题的能力，继续使用可能误导进展评估和模型选择。研究支持转向考虑数据污染的新数据集。
...
### [220] [Studying and Automating Issue Resolution for Software Quality](https://arxiv.org/abs/2512.10238)
*Antu Saha*

Main category: cs.SE

TL;DR: 该研究通过三个方向提升软件问题解决效率：使用LLM改进问题报告质量、实证分析开发者工作流、以及自动化解决任务，旨在推进AI驱动的问题解决并支持更可维护的软件系统。


<details>
  <summary>Details</summary>
Motivation: 软件开发中问题解决至关重要，但开发者面临低质量问题报告、对实际工作流理解有限、缺乏自动化支持等挑战，需要系统性的解决方案来提升软件质量和维护性。

Method: 采用三个互补方向：1) 利用LLM推理和应用特定信息提升问题报告质量；2) 实证分析传统和AI增强系统中的开发者工作流；3) 通过ML、DL和LLM方法自动化认知密集型解决任务，包括buggy UI定位和解决方案识别。

Result: 该研究提供了实证洞察、实用工具和自动化方法，能够推进AI驱动的问题解决，支持更可维护和高质量的软件系统开发。

Conclusion: 通过综合方法解决软件问题解决中的关键挑战，该工作为AI驱动的软件维护提供了有价值的贡献，有望显著提升软件质量和开发效率。
...
### [221] [Cross-modal Retrieval Models for Stripped Binary Analysis](https://arxiv.org/abs/2512.10393)
*Guoqiang Chen,Lingyun Ying,Ziyang Song,Daguang Liu,Qiang Wang,Zhiqi Wang,Li Hu,Shaoyin Cheng,Weiming Zhang,Nenghai Yu*

Main category: cs.SE

TL;DR: BinSeek是一个两阶段跨模态检索框架，用于无符号二进制代码分析，通过嵌入模型和重排序模型实现二进制代码与自然语言描述的相关性匹配，在检索性能上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在二进制代码分析中面临挑战：从数千个无符号二进制函数中根据用户查询检索相关函数的研究不足，缺乏符号信息使其区别于源代码检索。

Method: 提出两阶段跨模态检索框架BinSeek：1) BinSeekEmbedding在大规模数据集上训练，学习二进制代码与自然语言描述的语义相关性；2) BinSeek-Reranker通过上下文增强仔细判断候选代码与描述的相关性。使用LLM数据合成管道自动化构建训练数据。

Result: BinSeek达到最先进性能：在Rec@3上超越同规模模型31.42%，在MRR@3上超越27.17%，甚至领先参数规模大16倍的先进通用模型。同时构建了领域基准数据集。

Conclusion: BinSeek是首个针对无符号二进制代码的两阶段跨模态检索框架，通过专门的嵌入和重排序模型有效解决二进制代码检索问题，为未来研究提供了基准和工具。
...
### [222] [How to Trick Your AI TA: A Systematic Study of Academic Jailbreaking in LLM Code Evaluation](https://arxiv.org/abs/2512.10415)
*Devanshu Sahoo,Vasudev Majhi,Arjun Neekhra,Yash Sinha,Murari Mandal,Dhruv Kumar*

Main category: cs.SE

TL;DR: 该论文首次大规模研究了学术环境中基于LLM的自动代码评估器的越狱攻击，提出了学术越狱攻击概念，创建了包含25K对抗性学生提交的毒化数据集，并评估了6个LLM的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM作为自动代码评估器在学术环境中的普及，学生可能使用对抗性提示策略诱导错误评分以获得不应得的学术优势，需要研究这种威胁的严重性和防御方法。

Method: 系统性地将20多种越狱策略适配到学术代码评估场景，创建包含25K对抗性学生提交的毒化数据集，定义三个越狱评估指标（越狱成功率、分数膨胀、危害性），并在6个LLM上进行全面评估。

Result: 研究发现这些LLM模型表现出显著脆弱性，特别是对说服性和角色扮演类攻击（越狱成功率高达97%），揭示了学术代码评估中LLM评估器的安全风险。
...
### [223] [UniCoR: Modality Collaboration for Robust Cross-Language Hybrid Code Retrieval](https://arxiv.org/abs/2512.10452)
...
<details>
  <summary>Details</summary>
Motivation: 现有代码检索方法在混合查询（自然语言+代码片段）和跨语言场景下存在三个主要挑战：1）语义理解不足；2）混合检索中的模态融合低效；3）跨语言场景泛化能力弱。需要开发能够有效利用混合查询并具备跨语言能力的代码检索方法。
...
Result: 在实证基准和大规模基准上的实验表明，UniCoR优于所有基线模型，在MRR上平均提升8.64%，在MAP上平均提升11.54%。在混合代码检索中表现出稳定性，在跨语言场景中具备泛化能力。

Conclusion: UniCoR通过统一的自监督学习框架有效解决了混合查询代码检索中的关键挑战，显著提升了检索性能，特别是在跨语言场景下表现出强大的泛化能力，为代码检索领域提供了新的解决方案。
...
### [224] [Decoding Human-LLM Collaboration in Coding: An Empirical Study of Multi-Turn Conversations in the Wild](https://arxiv.org/abs/2512.10493)
*Binquan Zhang,Li Zhang,Haoyuan Zhang,Fang Liu,Song Wang,Bo Shen,An Fu,Lin Shi*

Main category: cs.SE

TL;DR: 该论文通过分析LMSYS-Chat-1M和WildChat数据集，实证研究了人类与LLM在编程协作中的交互机制、指令遵循能力和用户满意度，揭示了任务类型对交互模式的影响以及不同编程任务的挑战差异。


<details>
  <summary>Details</summary>
Motivation: 虽然现有数据集记录了真实的人机对话，但缺乏对人类与LLM在编程场景中协作机制的系统性研究。研究者希望探索用户在交互过程中的曲折路径、LLM的指令遵循能力以及用户满意度，以更好地理解人机协作机制。

Method: 使用LMSYS-Chat-1M和WildChat数据集进行实证分析，研究人类与LLM在编程协作中的交互机制、LLM的指令遵循能力和用户满意度。

Result: 1) 任务类型塑造交互模式：代码质量优化倾向于线性模式，设计驱动任务偏向树状结构，查询任务偏好星型模式
2) Bug修复和代码重构对LLM的指令遵循更具挑战性，不遵从率显著高于信息查询
3) 代码质量优化和需求驱动开发任务的用户满意度较低，而结构化知识查询和算法设计任务满意度较高

Conclusion: 该研究为改进LLM界面和提升编程协作中的用户满意度提供了建议，同时为自适应对话系统的未来研究指明了方向。这项工作拓宽了对人类与LLM协同作用的理解，支持更有效的AI辅助开发。
...
### [225] [Analyzing developer discussions on EU and US privacy legislation compliance in GitHub repositories](https://arxiv.org/abs/2512.10618)
*Georgia M. Kapitsaki,Maria Papoutsoglou,Christoph Treude,Ioanna Theophilou*

Main category: cs.SE

TL;DR: 分析GitHub上32,820个开源软件issue，研究开发者如何讨论隐私法规（GDPR/CCPA）合规问题，提出24个讨论类别和6个集群分类


<details>
  <summary>Details</summary>
Motivation: 尽管GitHub上有大量开发者讨论数据，但缺乏关于开源软件开发者如何讨论隐私法规合规问题的实证证据。GDPR和CCPA等隐私立法影响了软件开发方式，需要了解开发者实际关注的问题。

Method: 挖掘并分析GitHub仓库中的32,820个issue，部分自动分析识别法律用户权利和原则，手动分析1,186个issue样本，基于关注的问题类型进行分类。

Result: 创建了包含24个讨论类别的分类体系，分为6个集群：功能/缺陷、同意相关、文档、数据存储/共享、适应性、一般合规。发现开发者主要关注特定用户权利（删除权、选择退出权、访问权），讨论主要集中在用户同意、用户权利功能、缺陷和cookie管理。

Conclusion: 创建的分类体系可帮助从业者优先处理合规问题，教育界可调整课程以更好地培养未来工程师的隐私法律意识，研究界可识别支持隐私法规合规的改进领域。
...
### [226] [PACIFIC: a framework for generating benchmarks to check Precise Automatically Checked Instruction Following In Code](https://arxiv.org/abs/2512.10713)
*Itay Dreyfuss,Antonio Abu Nassar,Samuel Ackerman,Axel Ben David,Rami Katan,Orna Raz,Marcel Zalmanovici*

Main category: cs.SE

TL;DR: PACIFIC框架自动生成基准测试，评估LLM在代码任务中的顺序指令遵循和代码干运行能力，通过控制难度和避免数据污染实现可靠评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法往往依赖工具使用或代理行为，无法隔离评估LLM内在的代码推理和指令遵循能力，且存在训练数据污染问题，需要更精确的评估框架。

Method: 提出PACIFIC框架，自动生成具有明确预期输出的基准测试变体，控制难度级别，通过简单输出比较进行可靠评估，专注于代码干运行（不执行代码的逐步推理）和顺序指令遵循能力。

Result: 验证表明PACIFIC能生成不同难度级别的基准测试，有效区分最先进LLM的指令遵循和干运行能力，即使对高级模型也能产生挑战性测试。

Conclusion: PACIFIC提供了一个可扩展、抗污染的方法论，用于评估LLM在代码相关任务中的核心能力，为代码助手系统的评估提供了更精确的工具。
...
### [227] [Zorya: Automated Concolic Execution of Single-Threaded Go Binaries](https://arxiv.org/abs/2512.10799)
*Karolina Gorna,Nicolas Iooss,Yannick Seurin,Rida Khatoun*

Main category: cs.SE

TL;DR: Zorya是一个针对Go二进制文件的符号执行框架，通过P-Code中间表示和多层过滤机制，显著提升了漏洞检测效率和覆盖率。


<details>
  <summary>Details</summary>
Motivation: Go语言在关键基础设施中的广泛应用需要系统化的漏洞检测，但现有符号执行工具难以处理Go二进制文件的运行时复杂性和可扩展性问题。

Method: 基于Zorya框架，将Go二进制文件转换为Ghidra的P-Code中间表示，添加了未执行路径的bug检测和多层过滤机制，专注于panic相关的路径，并采用函数模式分析。

Result: panic可达性过滤实现了1.8-3.9倍的速度提升，过滤了33-70%的分支；Zorya检测到所有panic，而现有工具最多只能检测两个；函数模式分析比从main函数开始快约两个数量级。

Conclusion: 专门的符号执行可以在具有运行时安全检查的语言生态系统中实现实用的漏洞检测。
...
### [228] [Does Timeboost Reduce MEV-Related Spam? Theory and Evidence from Layer-2 Transactions](https://arxiv.org/abs/2512.10094)
*Brian Zhu*

Main category: cs.GT

TL;DR: Timeboost机制通过拍卖时间戳优势减少L2区块链中的MEV垃圾交易，将用户支付从交易回滚成本转移到拍卖竞价，增加排序器和DAO收入。


<details>
  <summary>Details</summary>
Motivation: Layer-2区块链中MEV机会常导致垃圾交易泛滥：大量相同交易同时提交，多数回滚浪费区块空间。需要机制减少这种低效行为。

Method: 1) 建立博弈论模型，用户选择提交交易副本数量；2) 扩展模型纳入Timeboost拍卖和后续交易行为；3) 收集多个L2网络的mempool数据，通过窄时间窗口内相同交易测量垃圾交易；4) 以其他L2为基准进行Timeboost采用的事件研究。

Result: Timeboost在均衡状态下减少垃圾交易并增加排序器/DAO收入，将用户支付从回滚成本转移到拍卖竞价。实证显示Arbitrum采用Timeboost后MEV相关垃圾交易减少，收入增加，与模型预测一致。

Conclusion: Timeboost机制有效解决L2区块链中的MEV垃圾交易问题，通过拍卖时间戳优势改善系统效率，为类似机制设计提供理论和实证支持。
...
### [229] [Computing Evolutionarily Stable Strategies in Imperfect-Information Games](https://arxiv.org/abs/2512.10279)
*Sam Ganzfried*

Main category: cs.GT

TL;DR: 提出一种计算对称完美回忆扩展式不完全信息博弈中演化稳定策略的算法，支持双人游戏并可扩展至多人游戏，在非退化游戏中能计算所有ESS，在退化游戏中计算子集，支持随时停止的算法设计。


<details>
  <summary>Details</summary>
Motivation: 演化稳定策略在博弈论中具有重要意义，但现有方法难以有效计算对称完美回忆扩展式不完全信息博弈中的ESS，特别是在存在无限连续对称纳什均衡的退化游戏中。

Method: 开发了针对双人对称完美回忆扩展式不完全信息博弈的ESS计算算法，采用随时可停止的设计，能够处理非退化游戏（计算所有ESS）和退化游戏（计算ESS子集），并可扩展至多人游戏。

Result: 算法在非退化癌症信号博弈和随机游戏中展示了可扩展性，能够有效计算ESS，特别是在退化游戏中能够处理无限连续对称纳什均衡的情况。

Conclusion: 该算法为计算对称扩展式不完全信息博弈中的演化稳定策略提供了有效的工具，填补了现有方法的空白，特别是在处理退化游戏方面具有重要价值。
...
### [230] [Certifying Concavity and Monotonicity in Games via Sum-of-Squares Hierarchies](https://arxiv.org/abs/2512.10292)
*Vincent Leon,Iosif Sakos,Ryann Sim,Antonios Varvitsiotis*

Main category: cs.GT

TL;DR: 该论文研究多人博弈中的凹性和单调性验证问题，证明验证这些性质是NP难的，但提出了基于平方和规划的分层验证方法，并引入SOS-凹/单调博弈作为近似。


<details>
  <summary>Details</summary>
Motivation: 凹性及其改进性质是多人博弈可处理性的基础，能保证纳什均衡存在性和分布式算法收敛性。然而，验证博弈是否具有凹性或单调性本身是困难的，需要开发有效的验证方法。

Method: 1. 证明验证凹性或单调性是NP难的，即使对于多项式效用函数和基本半代数策略集的博弈；2. 开发两套平方和规划分层程序来验证凹性和单调性；3. 引入SOS-凹/单调博弈作为全局近似；4. 在多项式时间内计算给定博弈的最接近SOS-凹/单调博弈。

Result: 1. 验证凹性或单调性是NP难的；2. 提出的平方和分层程序能在多项式时间内求解每个层级；3. 几乎所有凹/单调博弈都能在有限层级被验证；4. 能在多项式时间内计算最接近的SOS-凹/单调博弈；5. 方法应用于不完全回忆扩展式博弈的经典例子。

Conclusion: 该研究为验证博弈的凹性和单调性提供了理论框架和实用工具，通过平方和规划方法克服了NP难问题，为分析不完全回忆扩展式博弈等复杂博弈提供了新途径。
...
### [231] [The $k$-flip Ising game](https://arxiv.org/abs/2512.10389)
*Kovalenko Aleksandr,Andrey Leonidov*

Main category: cs.GT

TL;DR: 研究完全图上的k-flip Ising博弈模型，分析从亚稳态/不稳定态到稳定态的首次命中时间分布，发现k依赖的扩散与恢复力竞争导致亚稳态衰减存在最优k*


<details>
  <summary>Details</summary>
Motivation: 研究部分并行动态噪声二元选择（Ising）博弈在离散时间下的行为，特别是k-flip机制如何影响系统从亚稳态到稳定态的转变过程，探索k值对系统动力学的影响

Method: 在完全图上建立N个玩家的k-flip Ising博弈模型，其中k个玩家在每个时间步可以改变策略。解析计算博弈的转移矩阵，分析策略分布φ=N^+/N的前两阶矩，研究从亚稳态和不稳定态到稳定态的首次命中时间分布

Result: 计算了转移矩阵和分布矩，发现亚稳态衰减过程中首次命中时间分布的前两阶矩对k存在非平凡依赖关系，在某些k*处出现最小值，这归因于k依赖的扩散力与恢复力之间的竞争

Conclusion: k-flip机制在Ising博弈中引入重要的动力学效应，扩散与恢复力的竞争导致亚稳态衰减存在最优k值，这为理解复杂系统中的相变和动力学行为提供了新视角
...
### [232] [LLM-Auction: Generative Auction towards LLM-Native Advertising](https://arxiv.org/abs/2512.10551)
*Chujie Zhao,Qun Hu,Shiping Song,Dagui Chen,Han Zhu,Jian Xu,Bo Zheng*

Main category: cs.GT

TL;DR: 提出LLM-Auction：首个基于学习的生成式拍卖机制，将拍卖与LLM生成整合用于LLM原生广告，通过迭代奖励偏好优化算法实现高效分配。
...
Method: 提出LLM-Auction机制，将分配优化建模为LLM输出与机制目标之间的偏好对齐问题，引入迭代奖励偏好优化算法交替优化奖励模型和LLM。

Result: LLM-Auction在分配效率上显著优于现有基线，同时实现期望的机制属性，能够在不增加额外推理成本的情况下建模分配外部性。

Conclusion: LLM-Auction是首个将拍卖与LLM生成整合的学习型生成式拍卖机制，通过IRPO算法有效解决LLM原生广告中的分配优化问题，具有实际工业应用价值。
...
### [233] [Dynamics of multidimensional Simple Clock Auctions](https://arxiv.org/abs/2512.10614)
*Jad Zeroual,Marianne Akian,Aurélien Bechler,Matthieu Chardy,Stéphane Gaubert*

Main category: cs.GT

TL;DR: 研究简单时钟拍卖中拥有完美信息的玩家对抗直接竞标者的最优策略，当对手估值满足普通替代条件时，固定标的投标是最优的，连续时间模型存在唯一解，价值函数分段线性


<details>
  <summary>Details</summary>
Motivation: 分析简单时钟拍卖机制中信息优势玩家的策略行为，特别是在频谱拍卖等实际应用中，理解拥有完美信息的玩家如何对抗直接竞标者，以及连续时间模型的数学性质

Method: 研究连续时间版本的SCA拍卖，价格遵循具有分段常数动态的微分包含，证明存在Filippov意义下的唯一解，分析价值函数的分段线性特性

Result: 当对手估值满足普通替代条件时，拥有完美信息的玩家最优策略是固定标的投标；连续时间模型存在唯一解，确保与离散时间拍卖极限的一致性；价值函数是分段线性的（可能不连续）

Conclusion: 简单时钟拍卖中信息优势玩家有明确的最优策略，连续时间模型具有良好的数学性质，这些理论结果在2017年澳大利亚多频段频谱拍卖的简化版本中得到验证
...
### [234] [Designing Truthful Mechanisms for Asymptotic Fair Division](https://arxiv.org/abs/2512.10892)
*Jugal Garg,Vishnu V. Narayan,Yuang Eric Shen*

Main category: cs.GT

TL;DR: 该论文研究了在更广泛的联合价值分布下，当物品数量m=Ω(n log n)时，高概率存在无嫉妒分配，并提出首个期望真实、多项式时间可实现的随机机制。


<details>
  <summary>Details</summary>
Motivation: 先前研究假设价值分布良好或独立同分布，且依赖非策略证明机制，限制了在策略性代理场景中的应用。需要扩展到更现实的分布（允许相关性、原子性、不等概率）并设计策略证明机制。

Method: 提出新的随机机制，该机制期望真实、多项式时间可实现，能输出高概率无嫉妒分配。扩展到渐近加权公平分配和多代理类型、多物品类型场景。

Result: 在更广泛的联合分布下，当m=Ω(n log n)时，高概率存在无嫉妒分配。提出的机制是首个期望真实、多项式时间可实现的随机机制，解决了Manurangsi和Suksompong（2017）的开放问题。

Conclusion: 该工作将无嫉妒分配理论扩展到更现实的分布假设，并提供了实用的策略证明机制，为实际公平分配问题提供了理论保证和算法工具。
...
### [235] [ExaCraft: Dynamic Learning Context Adaptation for Personalized Educational Examples](https://arxiv.org/abs/2512.09931)
*Akaash Chatterjee,Suman Kundu*

Main category: cs.AI

TL;DR: ExaCraft是一个AI驱动的个性化学习示例生成系统，通过分析学习者的动态上下文（包括位置、教育背景、专业、复杂偏好等）来创建文化相关且适应个人学习需求的示例。


<details>
  <summary>Details</summary>
Motivation: 现有教育AI工具缺乏针对学习者动态理解、困难和技能增长的个性化示例生成能力。学习最有效的方式是使用与学习者个人相关的示例，但目前系统无法适应学习者不断变化的上下文。
...
Conclusion: ExaCraft展示了AI系统如何通过适应学习者的动态上下文来生成个性化示例，从而提高学习效果。系统能够根据学习者的困难、掌握情况和进展模式来调整示例内容和复杂度。
...
### [236] [Suzume-chan: Your Personal Navigator as an Embodied Information Hub](https://arxiv.org/abs/2512.09932)
...
<details>
  <summary>Details</summary>
Motivation: 专家知识获取通常需要实时人际沟通，数字工具虽能改善信息获取但难以创造深度理解所需的连接感

Method: 基于社会存在理论，开发名为"Suzume-chan"的具身信息枢纽原型，这是一个本地运行的小型柔软AI代理，结合语言模型和检索增强生成技术，通过语音解释学习和对话响应

Result: 原型系统能够通过物理和对话交互减少心理距离，使知识分享更加温暖和人性化

Conclusion: 具身信息枢纽为知识分享提供了新途径，通过增强社会存在感创造更人性化的知识交流体验
...
### [237] [Exploring Health Misinformation Detection with Multi-Agent Debate](https://arxiv.org/abs/2512.09935)
*Chih-Han Chen,Chen-Han Tsai,Yu-Shao Peng*

Main category: cs.AI

TL;DR: 提出一个两阶段框架用于健康信息检测：第一阶段使用LLM评估检索到的文章并计算聚合一致性分数，当分数低于阈值时进入第二阶段，多个智能体进行结构化辩论来生成有充分理由的裁决。


<details>
  <summary>Details</summary>
Motivation: 随着健康相关错误信息在网上泛滥，事实核查变得日益重要。有效的验证需要高质量证据检索和严谨的推理过程，但目前方法在复杂验证任务上存在不足。

Method: 两阶段框架：1) 一致性分数预测阶段：使用大语言模型独立评估检索到的文章，计算反映整体证据立场的聚合一致性分数；2) 多智能体辩论阶段：当一致性分数低于预定阈值时，多个智能体进行结构化辩论，综合冲突证据并生成有明确理由的裁决。

Result: 实验结果表明，与基线方法相比，该两阶段方法取得了更优越的性能，突显了将自动评分与协作推理相结合在复杂验证任务中的价值。

Conclusion: 结合一致性分数预测和多智能体辩论的两阶段框架能有效检测健康错误信息，为复杂验证任务提供了一种有前景的解决方案。
...
### [238] [Echo-CoPilot: A Multi-View, Multi-Task Agent for Echocardiography Interpretation and Reporting](https://arxiv.org/abs/2512.09944)
*Moein Heidari,Mohammad Amin Roohi,Armin Khosravi,Ilker Hacihaliloglu*

Main category: cs.AI

TL;DR: Echo-CoPilot：一个基于大语言模型的多视图、多任务智能体，通过协调专业超声心动图工具，实现临床统一的超声心动图评估


<details>
  <summary>Details</summary>
Motivation: 超声心动图是心血管诊疗的核心，但全研究解读仍是认知密集型、多视图的手动任务。现有基础模型虽能在单个感知子任务上表现良好，但缺乏统一的临床连贯评估框架。

Method: 采用ReAct式循环架构，大语言模型作为协调器，分解临床查询，调用视图识别、心脏结构分割、测量、疾病预测和报告生成等专业工具，整合输出为指南感知的答案和叙述性总结。

Result: 在MIMIC-EchoQA基准测试中达到50.8%准确率，优于通用和生物医学视频视觉语言模型。定性分析显示能利用定量测量和生理学上下文解决临床决策阈值附近的疑难病例。

Conclusion: Echo-CoPilot展示了通过大语言模型协调专业工具实现统一超声心动图评估的可行性，为临床决策提供支持，代码将在论文接受后发布。
...
### [239] [Fuzzy Hierarchical Multiplex](https://arxiv.org/abs/2512.09976)
*Alexis Kafantaris*
...
Method: 提出新的模糊优化框架，利用动态性将数据映射到度量中，通过多重网络结构分析逻辑蕴含和概念层次，采用白箱理论方法分析框架的逻辑和数学基础

Result: 建立了完整的模糊优化框架，能够有效分析FHM（模糊层次模型），以简单优雅的方式展示了逻辑步骤，为服务流程设计中的信息传输优化提供了理论工具

Conclusion: 该框架成功扩展了FCM因果关系，为服务优化特别是信息传输服务优化提供了有效的理论框架，具有实际应用价值
...
### [240] [An exploration for higher efficiency in multi objective optimisation with reinforcement learning](https://arxiv.org/abs/2512.10208)
*Mehmet Emin Aydin*

Main category: cs.AI

TL;DR: 该论文提出使用多目标强化学习来优化搜索算法中的算子序列选择，以解决多目标优化问题中的算子选择效率问题。


<details>
  <summary>Details</summary>
Motivation: 优化和搜索过程中的效率问题持续影响算法性能。虽然使用算子池而非单一算子处理邻域移动操作有前景，但寻找最优或近优算子序列仍需研究。现有研究多集中于单目标优化，多目标优化领域在这方面研究不足。

Method: 提出基于多目标强化学习的通用方法，通过多阶段实现算子序列的智能选择。该方法包含已完成阶段和待完成阶段，旨在系统展示多目标强化学习的应用效果。

Result: 论文概述了提出的通用方法框架，部分阶段已完成，部分阶段待完成。该方法旨在证明多目标强化学习在提高优化算法效率方面的潜力。
...
### [241] [Exploring LLMs for Scientific Information Extraction Using The SciEx Framework](https://arxiv.org/abs/2512.10004)
...
Method: 提出SciEx框架，采用模块化设计，将PDF解析、多模态检索、信息提取和聚合等关键组件解耦。这种设计支持按需数据提取，同时允许灵活集成新模型、提示策略和推理机制。

Result: 在三个科学主题的数据集上评估SciEx提取细粒度信息的准确性和一致性。研究结果为当前基于LLM的流程提供了实际见解，包括其优势和局限性。

Conclusion: SciEx通过模块化设计有效解决了科学信息提取中的关键挑战，为快速变化的科学数据需求提供了灵活可扩展的解决方案。
...
### [242] [Phythesis: Physics-Guided Evolutionary Scene Synthesis for Energy-Efficient Data Center Design via LLMs](https://arxiv.org/abs/2512.10611)
...
<details>
  <summary>Details</summary>
Motivation: 传统数据中心设计方法依赖人工经验和专业仿真工具，难以应对日益复杂的系统需求。现有生成式AI方法缺乏物理考虑，无法满足数据中心量化运营目标和严格物理约束的要求

Method: 提出Phythesis框架，采用迭代双层优化架构：1) LLM驱动优化层生成物理合理的3D布局并进行自我批评以优化场景拓扑；2) 物理信息优化层识别最优资产参数并选择最佳资产组合

Result: 在三个生成规模上的实验显示，相比纯LLM方案，Phythesis实现了57.3%的生成成功率提升和11.5%的电源使用效率(PUE)改进

Conclusion: Phythesis成功将LLM与物理引导优化相结合，为数据中心能效设计提供了自动化仿真就绪场景合成的新方法
...
### [243] [DynaMate: An Autonomous Agent for Protein-Ligand Molecular Dynamics Simulations](https://arxiv.org/abs/2512.10034)
...
<details>
  <summary>Details</summary>
Motivation: 尽管分子动力学模拟在药物发现和蛋白质工程中广泛应用，但其技术复杂性（参数化、输入准备、软件配置）仍然是广泛高效使用的主要障碍。目前尚未有成功的基于智能体LLM的自动化蛋白质-配体MD工作流程解决方案。

Method: DynaMate是一个模块化多智能体框架，包含三个专门模块：实验规划、模拟执行和结果分析。框架集成了动态工具使用、网络搜索、PaperQA和自校正行为，能够自主设计并执行完整的MD工作流程，包括MM/PB(GB)SA方法的结合自由能计算。
...
### [244] [SimWorld-Robotics: Synthesizing Photorealistic and Dynamic Urban Environments for Multimodal Robot Navigation and Collaboration](https://arxiv.org/abs/2512.10046)
...
TL;DR: SimWorld-Robotics (SWR) 是一个基于虚幻引擎5构建的大规模、逼真城市环境仿真平台，用于具身AI研究，包含两个机器人基准测试：多模态指令跟随任务和多智能体搜索任务。


<details>
  <summary>Details</summary>
Motivation: 当前的基础模型在通用机器人研究中主要关注室内家庭场景，缺乏针对大规模城市环境的仿真平台和评估基准。需要创建更真实、复杂、可扩展的城市环境来全面评估机器人在现实场景中的关键能力。

Method: 基于虚幻引擎5构建SWR平台，程序化生成无限逼真的城市场景，包含动态元素（行人、交通系统），支持多机器人控制和通信。在此基础上构建了两个基准测试：1）多模态指令跟随任务（机器人根据视觉-语言导航指令在行人交通环境中到达目的地）；2）多智能体搜索任务（两个机器人通过通信协作定位并相遇）。

Result: SWR平台在真实性、复杂性和可扩展性方面超越了现有的城市仿真。实验结果表明，包括视觉语言模型在内的最先进模型在SWR任务中表现不佳，缺乏城市环境所需的稳健感知、推理和规划能力。

Conclusion: SWR平台为城市环境中的具身AI研究提供了重要的仿真基础设施，其基准测试全面评估了机器人的关键能力，揭示了当前模型在城市场景中的局限性，为未来研究指明了方向。
...
### [245] [Parallel Decoder Transformer: Model-Internal Parallel Decoding with Speculative Invariance via Note Conditioning](https://arxiv.org/abs/2512.10054)
*Logan Robbins*
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的自回归解码本质上是顺序的，导致延迟随输出长度线性增长。现有的"分解-填充"方法（如Skeleton-of-Thought）通过外部编排实现并行化，但由于缺乏跨流通信而遭受连贯性漂移问题。

Method: 提出并行解码变换器（PDT），在冻结预训练模型中注入轻量级的"推测性笔记条件化"适配器，使并行解码流能够通过共享的动态潜在空间进行同步。将协调问题形式化为推测共识问题，其中兄弟流向全局总线广播语义"笔记"，由学习的验证头进行门控。

Result: 在冻结的200亿参数骨干网络上使用5万步课程进行验证，PDT实现了有效的自校正，在覆盖预测中达到77.8%的精确度，并在不修改主干权重的情况下恢复近似的串行语义。
...
### [246] [Mind the Gap! Pathways Towards Unifying AI Safety and Ethics Research](https://arxiv.org/abs/2512.10058)
*Dani Roytburg,Beck Miller*

Main category: cs.AI

TL;DR: 通过大规模文献计量和合作网络分析发现，AI安全与AI伦理研究存在显著的结构性分裂，80%以上合作发生在各自社区内部，跨领域交流高度集中于少数关键研究者。


<details>
  <summary>Details</summary>
Motivation: AI对齐研究分化为安全（关注智能扩展、欺骗行为、存在风险）和伦理（关注当前危害、社会偏见、生产流程缺陷）两个平行轨道，两者对"对齐"定义存在分歧，导致研究相对孤立，需要揭示这种结构性分裂及其影响。

Method: 采用文献计量和合作网络分析方法，对2020-2025年间12个主要ML和NLP会议的6,442篇论文进行分析，研究安全与伦理社区之间的合作模式和结构连接性。

Result: 发现超过80%的合作发生在各自社区内部，跨领域连接高度集中：约5%的论文贡献了超过85%的桥梁链接。移除少数关键研究者会显著增加隔离程度，表明跨学科交流依赖于少数关键人物而非广泛分布的合作。

Conclusion: 安全与伦理的分裂不仅是概念上的，更是制度性的，对研究议程、政策和学术场所有重要影响。需要通过共享基准、跨机构平台和混合方法整合技术安全工作和规范伦理，以构建既稳健又公正的AI系统。
...
### [247] [Linear socio-demographic representations emerge in Large Language Models from indirect cues](https://arxiv.org/abs/2512.10065)
*Paul Bouchaud,Pedro Ramaciotti*

Main category: cs.AI

TL;DR: LLMs在线性表示中编码用户的社会人口属性（性别、种族等），这些属性可以从姓名、职业等间接线索推断，并影响下游行为如职业推荐。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs如何从姓名、职业等间接线索推断对话伙伴的社会人口属性，并探索这些隐式表示如何影响模型行为，即使模型通过了偏见基准测试。

Method: 在四个开源Transformer LLMs（Magistral 24B, Qwen3 14B, GPT-OSS 20B, OLMo2-1B）的残差流中探测社会人口属性的线性表示，使用显式人口披露提示，然后验证这些探针能否从姓名（性别、种族）和职业（劳动力统计相关）等隐式线索预测属性。

Result: LLMs在激活空间中形成社会人口属性的线性表示，其中刻板印象相关的属性沿可解释的几何方向编码。姓名激活与人口普查一致的性别和种族表示，职业触发与现实世界劳动力统计相关的表示。这些隐式表示主动影响下游行为（如职业推荐）。

Conclusion: 即使通过偏见基准测试的模型仍可能隐藏并利用隐式偏见，这些隐式人口表示主动塑造LLMs的下游行为，对大规模应用时的公平性有重要影响。
...
### [248] [Interpretable Embeddings with Sparse Autoencoders: A Data Analysis Toolkit](https://arxiv.org/abs/2512.10092)
*Nick Jiang,Xiaoqing Sun,Lisa Dunlap,Lewis Smith,Neel Nanda*

Main category: cs.AI

TL;DR: 稀疏自编码器（SAE）嵌入比LLM更经济可靠，比稠密嵌入更可控，能发现数据集语义差异、概念相关性，并用于模型行为分析


<details>
  <summary>Details</summary>
Motivation: 当前大规模文本分析依赖昂贵的LLM技术或缺乏可控性的稠密嵌入，需要更经济、可靠且可控的分析方法

Method: 使用稀疏自编码器创建SAE嵌入，其维度映射到可解释概念，通过四个数据分析任务验证其有效性

Result: SAE嵌入比LLM成本低2-8倍且更可靠，比稠密嵌入更可控，能发现数据集差异、概念相关性，并成功应用于模型行为分析案例

Conclusion: SAE是强大的非结构化数据分析工具，强调通过数据解释模型的重要性，为文本分析提供了经济、可靠且可控的新方法
...
### [249] [Robust AI Security and Alignment: A Sisyphean Endeavor?](https://arxiv.org/abs/2512.10100)
*Apostol Vassilev*

Main category: cs.AI

TL;DR: 该论文通过将哥德尔不完备性定理扩展到AI领域，建立了AI安全和对齐鲁棒性的信息论限制，提供了应对这些挑战的实用方法，并证明了AI系统认知推理的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着AI技术的快速发展，理解AI安全和对齐的基本限制对于负责任地采用AI技术至关重要。该研究旨在建立AI鲁棒性的信息论基础限制，为应对这些不可避免的挑战做好准备。

Method: 通过将哥德尔不完备性定理扩展到人工智能领域，建立信息论框架来分析AI安全和对齐的鲁棒性限制。该方法从数学上证明了AI系统在安全性和对齐方面存在固有的、无法完全克服的局限性。

Result: 证明了AI安全和对齐存在根本性的信息论限制，这些限制类似于哥德尔不完备性定理在数学系统中的表现。同时提供了应对这些挑战的实用方法，并证明了AI系统在认知推理方面存在更广泛的局限性。

Conclusion: AI安全和对齐存在固有的、无法完全消除的信息论限制，这些限制源于哥德尔不完备性定理在AI领域的扩展。认识到这些限制并制定相应的应对策略对于负责任地发展和部署AI技术至关重要。
...
### [250] [Modeling Narrative Archetypes in Conspiratorial Narratives: Insights from Singapore-Based Telegram Groups](https://arxiv.org/abs/2512.10105)
*Soorya Ram Shimgekar,Abhay Goyal,Lam Yin Cheung,Roy Ka-Wei Lee,Koustuv Saha,Pi Zonooz,Navin Kumar*

Main category: cs.AI

TL;DR: 开发了一个两阶段计算框架来分析新加坡Telegram群组中的阴谋论叙事，发现阴谋论内容被编织在日常讨论中而非局限于孤立回音室。


<details>
  <summary>Details</summary>
Motivation: 阴谋论话语日益嵌入数字通信生态系统，但其结构和传播难以研究。需要理解阴谋论内容如何在日常社交互动中运作，挑战关于在线激进化的常见假设。

Method: 两阶段计算框架：1) 微调RoBERTa-large分类消息是否为阴谋论内容；2) 构建带符号的信念图，节点代表消息，边符号反映信念标签对齐，权重为文本相似度。提出带符号信念图神经网络(SiBeGNN)，使用符号解缠损失学习分离意识形态对齐和风格特征的嵌入。

Result: 在553,648条消息中识别出七个叙事原型：法律话题、医疗关注、媒体讨论、金融、权威矛盾、群组管理、一般聊天。SiBeGNN聚类质量(cDBI=8.38)优于基线方法(13.60-67.27)，专家评估者间一致性达88%。发现阴谋论消息不仅出现在怀疑或不信任的聚类中，也出现在金融、法律和日常事务的常规讨论中。

Conclusion: 阴谋论话语在普通社交互动中运作，挑战了关于在线激进化的常见假设。该框架推进了信念驱动话语分析的计算方法，可用于立场检测、政治传播研究和内容审核政策。
...
### [251] [AgriRegion: Region-Aware Retrieval for High-Fidelity Agricultural Advice](https://arxiv.org/abs/2512.10114)
*Mesafint Fanuel,Mahmoud Nabil Mahmoud,Crystal Cook Marshal,Vishal Lakhotia,Biswanath Dari,Kaushik Roy,Shaohu Zhang*

Main category: cs.AI

TL;DR: AgriRegion是一个专门为农业咨询设计的检索增强生成框架，通过地理空间元数据注入和区域优先重排序机制，减少LLM在农业领域的上下文幻觉问题，提供区域感知的准确农业建议。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在农业领域经常出现上下文幻觉问题，提供的建议可能在一个地区科学有效，但在另一个地区由于土壤、气候和当地法规的差异而具有灾难性后果。需要开发能够提供高保真、区域感知农业咨询的系统。

Method: AgriRegion采用检索增强生成框架，包含地理空间元数据注入层和区域优先重排序机制。将知识库限制在已验证的当地农业推广服务，并在检索过程中强制执行地理空间约束，确保关于种植计划、害虫控制和施肥的建议具有本地准确性。

Result: 实验表明，AgriRegion相比最先进的大语言模型系统减少了10-20%的幻觉，并通过综合评估显著提高了信任分数。创建了包含12个农业子领域160个领域特定问题的新基准数据集AgriRegion-Eval。

Conclusion: AgriRegion框架通过结合地理空间约束和区域优先检索，有效解决了农业咨询中的上下文幻觉问题，为提供准确、可靠的区域特定农业建议提供了可行方案。
...
### [252] [The 2025 Foundation Model Transparency Index](https://arxiv.org/abs/2512.10169)
*Alexander Wan,Kevin Klyman,Sayash Kapoor,Nestor Maslej,Shayne Longpre,Betty Xiong,Percy Liang,Rishi Bommasani*

Main category: cs.AI

TL;DR: 2025年基础模型透明度指数显示，主要AI公司透明度从2024年的58分降至2025年的40分，整体透明度恶化，IBM以95分领先，xAI和Midjourney仅14分垫底。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型开发公司影响力日益增大，需要持续评估其透明度实践的变化，为政策制定者提供参考，并揭示当前透明度状况及政策干预的必要性。

Method: 采用年度基础模型透明度指数方法，新增数据获取、使用数据和监控等指标，首次评估阿里巴巴、DeepSeek和xAI等公司，对100分制进行量化评分。

Result: 平均透明度分数从2024年58分降至2025年40分，IBM以95分最高，xAI和Midjourney仅14分最低；公司在训练数据、训练计算以及部署后使用和影响方面最不透明。

Conclusion: 基础模型开发公司透明度整体恶化，需要更积极的政策干预来弥补关键信息缺口；Frontier Model Forum成员公司处于中间位置，缺乏成为透明度领导者的激励。
...
### [253] [CP-Env: Evaluating Large Language Models on Clinical Pathways in a Controllable Hospital Environment](https://arxiv.org/abs/2512.10206)
*Yakun Zhu,Zhongzhen Huang,Qianhan Feng,Linjie Mu,Yannian Gu,Shaoting Zhang,Qi Dou,Xiaofan Zhang*

Main category: cs.AI

TL;DR: CP-Env是一个可控的代理医院环境，用于评估大语言模型在端到端临床路径中的表现，模拟从分诊到多学科会诊的完整医疗流程，并提出三层评估框架。
...
Method: 开发CP-Env可控代理医院环境，模拟包含患者和医生代理的医院生态系统，构建从分诊、专科会诊到诊断测试和多学科会诊的临床场景。提出包含临床效果、流程能力和专业伦理的三层评估框架。

Result: 大多数模型在处理临床路径复杂性时表现不佳，存在幻觉和关键诊断细节丢失问题。有趣的是，过多的推理步骤有时会适得其反，而顶级模型倾向于通过内化知识减少工具依赖。
...
### [254] [ID-PaS : Identity-Aware Predict-and-Search for General Mixed-Integer Linear Programs](https://arxiv.org/abs/2512.10211)
*Junyang Cai,El Mehdi Er Raqabi,Pascal Van Hentenryck,Bistra Dilkina*

Main category: cs.AI

TL;DR: 本文提出ID-PaS框架，将预测-搜索方法扩展到参数化混合整数规划问题，通过身份感知学习处理异构变量，在多个实际大规模问题上优于Gurobi和传统PaS方法。


<details>
  <summary>Details</summary>
Motivation: 现有的预测-搜索方法主要局限于二元问题，且忽视了实际应用中常见的固定变量问题。需要将这一框架扩展到参数化混合整数规划，并更好地处理异构变量。

Method: 提出ID-PaS（身份感知学习框架），扩展预测-搜索方法到参数化MIP问题。该框架使机器学习模型能够更有效地处理异构变量，包括固定变量和参数变化的情况。

Result: 在多个实际大规模问题上的实验表明，ID-PaS在性能上始终优于最先进的求解器Gurobi和传统的预测-搜索方法。

Conclusion: ID-PaS成功将预测-搜索框架扩展到参数化混合整数规划问题，通过身份感知学习处理异构变量，在实际应用中表现出优越性能，为组合优化问题提供了更有效的解决方案。
...
### [255] [Reverse Thinking Enhances Missing Information Detection in Large Language Models](https://arxiv.org/abs/2512.10273)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理缺失信息问题时表现不佳，常出现回答不完整、事实错误和幻觉等问题。传统的正向推理方法（如思维链、思维树）无法系统性地识别和恢复被省略的信息。

Method: 提出基于反向思维的新框架，引导大语言模型通过逆向推理来识别必要条件和定位缺失元素，将缺失信息识别任务转化为更易处理的逆向推理问题。

Result: 实验结果表明，反向思维方法相比传统正向推理方法取得了显著的性能提升，为大语言模型的逻辑完整性和推理鲁棒性提供了有前景的方向。

Conclusion: 反向思维方法能有效增强大语言模型在缺失信息检测任务上的表现，为解决大语言模型推理中的信息缺失问题提供了新思路。
...
### [256] [Neuronal Attention Circuit (NAC) for Representation Learning](https://arxiv.org/abs/2512.10282)
*Waleed Razzaq,Izis Kankaraway,Yun-Bo Zhao*

Main category: cs.AI

TL;DR: 提出神经注意力电路(NAC)，一种生物启发的连续时间注意力机制，通过线性一阶ODE和稀疏门控网络实现高效的自适应动态，在多个领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制虽然改进了RNN的表征学习能力，但其离散特性限制了连续时间建模。需要一种生物合理、能处理连续时间动态的注意力机制。

Method: 将注意力logits计算重新表述为线性一阶ODE的解，使用源自秀丽隐杆线虫神经元电路策略的稀疏门控网络。包含三种计算模式：显式欧拉积分、精确闭式解和稳态近似，并采用稀疏Top-K配对连接方案提高内存效率。

Result: NAC在精度上匹配或优于竞争基线，在运行时间和内存效率方面处于中等位置。在非规则时间序列分类、自动驾驶车道保持和工业预测等任务中表现良好。
...
### [257] [Investigating The Functional Roles of Attention Heads in Vision Language Models: Evidence for Reasoning Modules](https://arxiv.org/abs/2512.10300)
*Yanbei Jiang,Xueqi Ma,Shu Liu,Sarah Monazam Erfani,Tongliang Liu,James Bailey,Jey Han Lau,Krista A. Ehinger*

Main category: cs.AI

TL;DR: 提出CogVision数据集和可解释性框架，通过分解多模态问题为子问题来系统分析视觉语言模型(VLMs)中注意力头的功能角色，发现功能头稀疏、分布不均且对多模态推理至关重要。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在多模态基准测试中表现出色，但它们很大程度上仍是一个黑箱。需要开发系统性的可解释性框架来分析VLM的内部机制，特别是注意力头在多模态推理中的功能角色。

Method: 1) 引入CogVision数据集，将复杂多模态问题分解为模拟人类链式推理的逐步子问题，每个子问题关联特定感知或认知功能；2) 采用基于探针的方法识别专门处理这些功能的注意力头，将其表征为功能头；3) 在不同VLM家族中进行分析和干预实验。

Result: 1) 功能头普遍稀疏，在不同功能间的数量和分布各异；2) 功能头介导交互和层次组织；3) 干预实验显示功能头对多模态推理至关重要：移除导致性能下降，强调则提升准确性。

Conclusion: 该研究为VLM的认知组织提供了新见解，并指出了设计更具人类对齐感知和推理能力模型的有前景方向。功能头分析框架有助于理解VLM的内部工作机制。
...
### [258] [Trustworthy Orchestration Artificial Intelligence by the Ten Criteria with Control-Plane Governance](https://arxiv.org/abs/2512.10304)
*Byeong Ho Kang,Wenli Yang,Muhammad Bilal Amin*

Main category: cs.AI

TL;DR: 提出"可信编排AI十准则"框架，将治理嵌入AI生态系统执行结构，确保可验证、透明、可重现和人类有意义控制


<details>
  <summary>Details</summary>
Motivation: AI系统在决策中作用日益重要，但技术能力与制度问责之间存在日益扩大的差距。仅靠伦理指导不足以应对这一挑战，需要将治理嵌入生态系统执行结构的架构

Method: 提出"可信编排AI十准则"综合保障框架，整合人类输入、语义一致性、审计和溯源完整性，构建统一的控制面板架构。借鉴国际标准和澳大利亚国家AI保障框架倡议

Result: 展示可信度可以通过工程方式系统性地融入AI系统，确保执行结构保持可验证、透明、可重现和处于有意义的人类控制之下

Conclusion: 该框架为整个AI组件、其消费者和人类参与者提供治理保护伞，而不仅仅是AI到AI的协调，填补了技术能力与制度问责之间的差距
...
### [259] [InfoCom: Kilobyte-Scale Communication-Efficient Collaborative Perception with Information Bottleneck](https://arxiv.org/abs/2512.10305)
*Quanmin Wei,Penglin Dai,Wei Li,Bingyi Liu,Xiao Wu*

Main category: cs.AI

TL;DR: InfoCom提出了一种基于信息瓶颈理论的信息感知协作感知框架，通过信息净化范式将通信开销从MB级降至KB级，同时保持接近无损的感知性能。


<details>
  <summary>Details</summary>
Motivation: 协作感知通过信息共享弥补单智能体感知的局限性，但面临通信与性能的权衡问题。现有方法通常假设MB级数据传输，在实际网络约束下可能失效。

Method: 1) 信息感知编码：将特征压缩为最小化消息，同时保留感知相关信息；2) 稀疏掩码生成：以可忽略的通信成本识别空间线索；3) 多尺度解码：通过掩码引导机制逐步恢复感知信息，而非简单特征重建。

Result: 在多个数据集上的实验表明，InfoCom实现接近无损的感知性能，同时将通信开销从MB级降至KB级，相比Where2comm和ERMVP分别减少440倍和90倍。

Conclusion: InfoCom建立了通信高效协作感知的理论基础，通过信息瓶颈原则优化任务关键信息的提取，实现了通信开销的显著降低而不牺牲感知性能。
...
### [260] [EpiPlanAgent: Agentic Automated Epidemic Response Planning](https://arxiv.org/abs/2512.10313)
*Kangkun Mao,Fang Xu,Jinru Ding,Yidong Jiang,Yujun Yao,Yirong Chen,Junming Liu,Xiaoqin Wu,Qian Wu,Xiaoyan Huang,Jie Xu*

Main category: cs.AI

TL;DR: EpiPlanAgent：基于大语言模型的多智能体系统，可自动化生成和验证数字应急响应计划，显著提升计划完整性和指南一致性，同时大幅缩短开发时间。


<details>
  <summary>Details</summary>
Motivation: 传统的流行病应对计划制定依赖劳动密集型的人工方法，效率低下且难以扩展。本研究旨在利用大语言模型和智能体技术，开发自动化系统以改进公共卫生应急响应规划。

Method: 设计了基于大语言模型的多智能体框架EpiPlanAgent，集成了任务分解、知识基础和模拟模块。公共卫生专业人员使用真实世界疫情场景在受控评估中测试系统。
...
### [261] [User-Feedback-Driven Continual Adaptation for Vision-and-Language Navigation](https://arxiv.org/abs/2512.10322)
*Yongqiang Yu,Xuhui Li,Hazza Mahmood,Jinxing Zhou,Haodong Hong,Longtao Jiang,Zhiqiang Xu,Qi Wu,Xiaojun Chang*

Main category: cs.AI

TL;DR: 本文提出一个用户反馈驱动的视觉语言导航自适应框架，通过系统整合人类交互到持续学习中，将用户反馈转化为高质量环境对齐训练数据，显著提升导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前GSA-VLN框架缺乏用户反馈，仅依赖无监督的环境暴露自适应。实际应用中，用户反馈能提供自然且有价值的监督信号，显著提升自适应质量。

Method: 1) 用户反馈驱动自适应框架：将用户反馈（导航指令和纠正信号）转化为高质量环境对齐训练数据；2) 记忆库热启动机制：重用先前获取的环境知识，缓解冷启动退化，确保稳定重新部署。

Result: 在GSA-R2R基准测试中，方法持续超越GR-DUET等强基线，提升导航成功率和路径效率。记忆库热启动稳定早期导航并减少更新后的性能下降。在持续和混合自适应设置下均表现稳健。

Conclusion: 提出的用户反馈驱动框架有效扩展了GSA-VLN，通过系统整合人类交互实现了高效现实的自适应，在多样化部署条件下展现持续改进能力。
...
### [262] [On the Collapse of Generative Paths: A Criterion and Correction for Diffusion Steering](https://arxiv.org/abs/2512.10339)
...
<details>
  <summary>Details</summary>
Motivation: 现有的概率密度比方法在组合异构模型（如不同噪声调度或数据集训练的模型）时会出现边际路径崩溃问题，导致中间密度不可归一化，这在分子设计等需要组合多个专家模型的任务中尤为突出

Method: 提出ACE（自适应路径校正指数）方法：1）推导路径存在性准则，仅从噪声调度和指数即可预测崩溃；2）将Feynman-Kac引导扩展到时变指数，保证有效概率路径

Result: 在合成2D基准测试和柔性姿态支架装饰任务中，ACE消除了崩溃，实现了高引导组合生成，在分布和对接指标上优于恒定指数基线和专用任务特定模型

Conclusion: ACE将概率密度比引导从不稳定启发式方法转变为可控生成的可靠工具，使异构专家模型组合在实际应用中更加稳定
...
### [263] [REMISVFU: Vertical Federated Unlearning via Representation Misdirection for Intermediate Output Feature](https://arxiv.org/abs/2512.10348)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦遗忘技术主要针对水平联邦学习，而垂直联邦学习的特征分区架构使得这些方法失效。GDPR等数据保护法规要求支持参与方的被遗忘权，因此需要开发适用于垂直联邦学习的遗忘方法。

Method: 提出REMISVFU表示误导框架：当收到删除请求时，遗忘方将其编码器输出坍缩到单位球面上的随机锚点，切断其特征与全局模型的统计联系。服务器端联合优化保留损失和遗忘损失，通过正交投影对齐梯度以消除破坏性干扰。

Result: 在公开基准测试中，REMISVFU将后门攻击成功率抑制到自然类先验水平，仅牺牲约2.5%的干净准确率，优于现有最先进基线方法。

Conclusion: REMISVFU为垂直联邦学习提供了一种有效的即插即用遗忘框架，能够在保护参与者被遗忘权的同时保持剩余方的模型效用，解决了垂直联邦学习中的遗忘难题。
...
### [264] [LLM-Empowered Representation Learning for Emerging Item Recommendation](https://arxiv.org/abs/2512.10370)
*Ziying Zhang,Quanming Yao,Yaqing Wang*

Main category: cs.AI

TL;DR: EmerFlow：一个基于LLM的新兴物品推荐框架，通过LLM推理增强物品特征，与现有推荐模型对齐，结合元学习优化嵌入，在有限交互下学习表达性嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有推荐方法通常假设新兴物品历史交互很少甚至没有，这种假设过于简化。好的模型需要保持新兴物品独特性，同时利用其与成熟物品的共享模式。

Method: 提出EmerFlow框架：1）通过LLM推理增强新兴物品原始特征；2）将这些表示与现有推荐模型的嵌入空间对齐；3）通过元学习结合新交互来优化嵌入。

Result: 在电影和医药等多个领域的广泛实验表明，EmerFlow始终优于现有方法，能够在有限交互下学习到表达性强的嵌入。

Conclusion: EmerFlow通过LLM增强特征表示、嵌入空间对齐和元学习优化，有效解决了新兴物品推荐问题，在多个领域表现出优越性能。
...
### [265] [AgentProg: Empowering Long-Horizon GUI Agents with Program-Guided Context Management](https://arxiv.org/abs/2512.10371)
*Shizuo Tian,Hao Wen,Yuxuan Chen,Jiacheng Liu,Shanhui Zhao,Guohong Liu,Ju Ren,Yunxin Liu,Yuanchun Li*

Main category: cs.AI

TL;DR: AgentProg：一种基于程序引导的移动GUI智能体上下文管理方法，通过将交互历史重构为带变量和控制流的程序，解决长时程任务中的上下文开销问题，同时集成全局信念状态机制处理部分可观测性。
...
Method: 提出AgentProg方法，将智能体交互历史重构为带有变量和控制流的程序结构，基于程序结构确定哪些信息应该保留或丢弃。同时集成基于Belief MDP框架的全局信念状态机制，处理部分可观测性和适应环境变化。

Result: 在AndroidWorld和扩展的长时程任务套件上的实验表明，AgentProg在这些基准测试中达到了最先进的成功率。更重要的是，在长时程任务中保持稳健性能，而基线方法则出现灾难性性能下降。

Conclusion: AgentProg通过程序化重构交互历史和集成全局信念状态机制，有效解决了移动GUI智能体长时程任务中的上下文管理问题，在保持高性能的同时显著降低了上下文开销。
...
### [266] [Planning, Living and Judging: A Multi-agent LLM-based Framework for Cyclical Urban Planning](https://arxiv.org/abs/2412.20505)
*Hang Ni,Yuzhi Wang,Hao Liu*

Main category: cs.AI

TL;DR: 提出CUP循环城市规划范式，利用多智能体LLM框架实现生成-评估-优化的闭环城市规划


<details>
  <summary>Details</summary>
Motivation: 城市化背景下城市更新面临挑战，需要适应性的方法来应对不断变化的需求，利用大语言模型的发展来改进城市规划过程

Method: 提出CUP循环城市规划范式，采用多智能体LLM框架，包含三个核心组件：规划（生成和优化城市方案）、生活（模拟居民行为和互动）、评判（评估方案效果并提供迭代反馈）

Result: 在真实世界数据集上的实验证明了该框架作为连续自适应规划过程的有效性

Conclusion: CUP范式通过闭环的生成-评估-优化循环，实现了动态响应式的城市规划方法，为城市更新提供了新的解决方案
...
### [267] [Boosting RL-Based Visual Reasoning with Selective Adversarial Entropy Intervention](https://arxiv.org/abs/2512.10414)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RL的微调方法主要在策略优化阶段干预熵，忽略了在RL采样阶段的熵干预，而后者可以通过提升响应多样性来增强GRPO性能

Method: 提出选择性对抗熵干预(SaEI)：1) 熵引导对抗采样(EgAS)，将采样响应的熵作为对抗目标，用对抗梯度攻击视觉输入生成对抗样本；2) 令牌选择性熵计算(TsEC)，在不扭曲事实知识的前提下最大化对抗攻击效果

Result: 在领域内和领域外数据集上的广泛实验表明，该方法能通过熵干预显著提升策略探索能力，从而增强推理能力

Conclusion: SaEI方法通过在RL采样阶段引入选择性对抗熵干预，有效提升了视觉语言模型的推理能力，为RL微调提供了新思路
...
### [268] [Representation of the structure of graphs by sequences of instructions](https://arxiv.org/abs/2512.10429)
...
<details>
  <summary>Details</summary>
Motivation: 当前图表示方法（基于邻接矩阵）不适合深度学习语言模型处理，而这些模型在文本处理方面表现出强大能力，需要一种能将图转换为文本格式的表示方法。

Method: 将图的邻接矩阵转换为一串简单指令序列，这些指令逐步构建邻接矩阵。该转换是可逆的，可以从图生成字符串，也可以从字符串重建图。

Result: 提出的表示方法紧凑且能保持图的局部结构模式。初步计算实验显示出有利结果。

Conclusion: 这种新的图表示方法有望增强深度学习模型对图的处理能力，为图分析与深度学习语言模型的结合提供了新途径。
...
### [269] [Targeted Data Protection for Diffusion Model by Matching Training Trajectory](https://arxiv.org/abs/2512.10433)
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型个性化微调技术普及，但存在未经授权数据使用和隐私侵犯问题。现有保护方法仅能被动降低图像质量，无法实现稳定控制。目标数据保护(TDP)虽有望主动重定向到目标概念，但现有方法因采用快照匹配而控制性差，未考虑完整学习动态。

Method: TAFAP通过对抗性扰动的微调实现轨迹对齐，是首个成功实现有效TDP的方法。不同于基于快照的方法，TAFAP采用受数据集蒸馏启发的轨迹匹配，在整个微调过程中强制执行持久、可验证的转换，控制整个训练轨迹。

Result: TAFAP在扩散模型中首次成功实现目标转换，同时控制身份和视觉模式。显著优于现有TDP方法，实现向目标概念的稳健重定向，同时保持高图像质量。

Conclusion: TAFAP实现了可验证的安全保障，为控制和追踪扩散模型输出变化提供了新框架，解决了当前保护方法的局限性。
...
### [270] [When Reject Turns into Accept: Quantifying the Vulnerability of LLM-Based Scientific Reviewers to Indirect Prompt Injection](https://arxiv.org/abs/2512.10449)
*Devanshu Sahoo,Manish Prasad,Vasudev Majhi,Jahnvi Singh,Vinay Chamola,Yash Sinha,Murari Mandal,Dhruv Kumar*

Main category: cs.AI

TL;DR: 论文研究了LLM评审系统对对抗性PDF操纵的脆弱性，开发了WAVS评估指标，发现多种攻击策略能成功操纵评审分数，实现论文从"拒绝"到"接受"的决策翻转。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在科学同行评审中的广泛应用（包括评审员个人使用和机构正式部署），需要评估这些"LLM-as-a-Judge"系统对对抗性PDF操纵的鲁棒性，特别是针对将"拒绝"决策翻转为"接受"这一特定动机。

Method: 开发了WAVS（加权对抗脆弱性分数）评估指标，收集了200篇科学论文数据集，针对评审任务调整了15种领域特定的攻击策略，并在包括GPT-5、Claude Haiku和DeepSeek在内的13个语言模型上进行评估。

Result: 结果显示，如"Maximum Mark Magyk"等混淆策略能成功操纵评分，即使在大型模型中也能实现令人担忧的决策翻转率。某些攻击策略在多个模型中都能有效改变评审结果。
...
### [271] [Zero-shot 3D Map Generation with LLM Agents: A Dual-Agent Architecture for Procedural Content Generation](https://arxiv.org/abs/2512.10501)
*Lim Chien Her,Ming Yan,Yunshu Bai,Ruihao Li,Hao Zhang*

Main category: cs.AI

TL;DR: 提出一种无需训练的LLM智能体架构，通过Actor-Critic双智能体协作实现零样本PCG参数配置，将自然语言指令转化为精确的技术参数设置。


<details>
  <summary>Details</summary>
Motivation: PCG虽然能算法化生成复杂世界，但需要精确配置不透明的技术参数。现有LLM难以弥合抽象用户指令与严格参数规范之间的语义鸿沟。

Method: 采用Actor-Critic双智能体架构：Actor负责生成参数配置，Critic进行评估和反馈，通过迭代工作流自主推理工具参数并逐步优化配置以符合人类设计偏好。

Result: 在3D地图生成任务上验证，优于单智能体基线，能从自然语言描述生成多样且结构有效的环境，为PCG中的指令跟随建立了新基准。

Conclusion: 现成LLM可有效重新用作任意PCG工具的通用智能体，通过将负担从模型训练转向架构推理，为无需任务特定微调而掌握复杂软件提供了可扩展框架。
...
### [272] [Achieving Olympia-Level Geometry Large Language Model Agent via Complexity Boosting Reinforcement Learning](https://arxiv.org/abs/2512.10534)
*Haiteng Zhao,Junhao Shen,Yiming Zhang,Songyang Gao,Kuikun Liu,Tianyou Ma,Fan Zheng,Dahua Lin,Wenwei Zhang,Kai Chen*

Main category: cs.AI

TL;DR: InternGeometry：首个达到IMO金牌水平的LLM几何智能体，通过迭代命题生成、符号引擎验证和反馈反思，仅用13K训练样本解决44/50 IMO几何问题，超越AlphaGeometry 2的数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在几何问题解决中因辅助构造启发式能力弱而受限，专家模型如AlphaGeometry 2依赖大规模数据合成和搜索。本文旨在构建首个达到IMO金牌水平的LLM几何智能体，克服几何启发式限制。

Method: 1. 迭代式命题生成与验证：智能体提出命题和辅助构造，符号引擎验证，根据反馈指导后续提案；2. 动态记忆机制：支持每问题200+次与符号引擎交互；3. 复杂度提升强化学习(CBRL)：训练阶段逐步增加合成问题复杂度；4. 基于InternThinker-32B构建。

Result: 1. 解决2000-2024年50道IMO几何问题中的44道（88%），超过金牌选手平均分（40.9）；2. 仅用13K训练样本，是AlphaGeometry 2数据量的0.004%；3. 能为IMO问题提出人类解法中未出现的新颖辅助构造。

Conclusion: InternGeometry展示了LLM智能体在专家级几何任务上的潜力，通过迭代验证和反馈机制克服几何启发式限制，以极低数据需求达到金牌水平，并能提出创新构造，为几何AI研究提供新方向。
...
### [273] [NormCode: A Semi-Formal Language for Context-Isolated AI Planning](https://arxiv.org/abs/2512.10563)
*Xin Guan*
...
<details>
  <summary>Details</summary>
Motivation: 多步LLM工作流存在上下文污染问题：随着信息在步骤间积累，模型会产生幻觉、混淆中间输出、失去任务约束跟踪。需要一种方法来消除跨步骤污染

Method: NormCode是一种半正式语言，提供三种同构格式：.ncds用于人工编写，.ncd用于机器执行，.ncn用于人工验证。它严格分离语义操作（LLM驱动的推理，非确定性）和语法操作（确定性数据重构），每个步骤在数据隔离中运行，只接收显式传递的输入

Result: 验证了两个演示：1）基础X加法算法在任意长度输入上达到100%准确率；2）自托管执行NormCode自身的五阶段编译器流水线。工作编排器提供依赖驱动调度、SQLite支持的检查点和循环管理
...
### [274] [Refinement Contrastive Learning of Cell-Gene Associations for Unsupervised Cell Type Identification](https://arxiv.org/abs/2512.10640)
*Liang Peng,Haopeng Liu,Yixuan Ye,Cheng Liu,Wenjun Shen,Si Wu,Hau-San Wong*

Main category: cs.AI

TL;DR: scRCL是一个用于单细胞组学研究的无监督细胞类型识别框架，通过结合细胞-基因相互作用和对比学习来获得更具信息性的细胞表示，显著提高了细胞类型识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的细胞聚类方法主要关注细胞内在结构，忽略了细胞-基因关联的关键作用，这限制了它们区分密切相关的细胞类型的能力。需要一种能够同时利用细胞-细胞关系和细胞-基因相互作用的方法来改进细胞类型识别。

Method: 提出了Refinement Contrastive Learning框架(scRCL)，包含两个对比分布对齐组件来揭示可靠的细胞内在结构，以及一个精炼模块来整合基因相关性结构学习，通过捕捉潜在的细胞-基因关联来增强细胞嵌入表示。

Result: 在多个单细胞RNA-seq和空间转录组学基准数据集上的实验表明，scRCL在细胞类型识别准确性方面持续优于最先进的基线方法。下游生物学分析证实恢复的细胞群体表现出连贯的基因表达特征。

Conclusion: scRCL通过显式结合细胞-基因相互作用，能够学习到更具生物学意义的细胞表示，有效提高了无监督细胞类型识别的性能，为单细胞组学研究提供了有力的分析工具。
...
### [275] [Remember Me, Refine Me: A Dynamic Procedural Memory Framework for Experience-Driven Agent Evolution](https://arxiv.org/abs/2512.10696)
*Zouying Cao,Jiaji Deng,Li Yu,Weikang Zhou,Zhaoyang Liu,Bolin Ding,Hai Zhao*

Main category: cs.AI

TL;DR: ReMe是一个用于LLM智能体经验驱动进化的记忆框架，通过多维度蒸馏、上下文自适应重用和效用驱动精炼机制，实现从静态存储到动态推理的转变，显著提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆框架主要采用"被动积累"范式，将记忆视为静态的只追加档案，无法有效支持动态推理和智能体进化。需要弥合静态存储与动态推理之间的差距。

Method: 提出ReMe框架，包含三个核心机制：1) 多维度蒸馏：识别成功模式、分析失败触发因素、生成比较性见解；2) 上下文自适应重用：通过场景感知索引将历史见解适配到新情境；3) 效用驱动精炼：自主添加有效记忆并修剪过时记忆，维护紧凑高质量经验池。

Result: 在BFCL-V3和AppWorld基准测试中达到最先进水平。观察到显著的内存缩放效应：配备ReMe的Qwen3-8B超越无记忆的Qwen3-14B，表明自进化记忆为终身学习提供了计算高效路径。

Conclusion: ReMe框架成功实现了LLM智能体从被动记忆积累到主动经验进化的转变，通过动态记忆管理显著提升智能体性能，为终身学习系统提供了有效解决方案。
...
### [276] [CAPTAIN: Semantic Feature Injection for Memorization Mitigation in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.10655)
*Tong Zhang,Carlos Hinojosa,Bernard Ghanem*

Main category: cs.AI

TL;DR: CAPTAIN：一种无需训练的框架，通过在去噪过程中直接修改潜在特征来减少扩散模型的记忆效应，保持提示对齐的同时降低隐私和版权风险


<details>
  <summary>Details</summary>
Motivation: 扩散模型可能无意中复制训练数据，引发隐私和版权问题。现有方法通常操纵分类器自由引导或扰动提示嵌入，但难以在减少记忆的同时保持与提示的对齐。

Method: CAPTAIN采用三阶段方法：1) 基于频率的噪声初始化，减少早期去噪过程中的记忆模式复制；2) 识别特征注入的最佳去噪时间步并定位记忆区域；3) 将非记忆参考图像的语义对齐特征注入到定位的潜在区域中。

Result: 实验表明，CAPTAIN相比基于CFG的基线方法显著减少了记忆效应，同时保持了与目标提示的强对齐和视觉质量。

Conclusion: CAPTAIN提供了一种有效的训练前缓解方法，能够在减少扩散模型记忆效应的同时保持提示对齐，为解决隐私和版权问题提供了新思路。
...
### [277] [Replace, Don't Expand: Mitigating Context Dilution in Multi-Hop RAG via Fixed-Budget Evidence Assembly](https://arxiv.org/abs/2512.10787)
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在处理多跳查询时，当初始检索遗漏桥接事实时容易失败。传统纠正方法（如Self-RAG、CRAG、Adaptive-k）通常通过增加上下文或修剪列表来解决问题，但这会导致上下文稀释问题——干扰信息挤占相关信息的空间。

Method: SEAL-RAG采用"替换而非扩展"策略，执行搜索→提取→评估→循环（SEAL）周期：1）实时实体锚定提取构建缺失实体/关系的间隙规范；2）触发针对性微查询；3）使用实体优先排序主动用间隙填补证据替换干扰信息。所有操作都在固定检索深度k下进行。

Result: 在HotpotQA（k=3）上，SEAL比Self-RAG提高答案正确性3-13个百分点，证据精度提高12-18个百分点。在2WikiMultiHopQA（k=5）上，比Adaptive-k准确率高8.0个百分点，证据精度保持96%（CRAG仅为22%）。所有改进都具有统计显著性（p<0.001）。

Conclusion: SEAL-RAG通过固定k替换策略，在保持可预测成本的同时，确保top-k槽位为精度而非广度优化。该方法有效解决了多跳查询中的上下文稀释问题，显著提升了RAG系统的性能。
...
### [278] [On the Dynamics of Multi-Agent LLM Communities Driven by Value Diversity](https://arxiv.org/abs/2512.10665)
*Muhua Huang,Qinlin Zhao,Xiaoyuan Yi,Xing Xie*

Main category: cs.AI

TL;DR: 价值多样性增强AI多智能体系统的集体行为：适度多样性提升价值稳定性、促进涌现行为和创造原则，但极端异质性会导致不稳定
...
### [279] [AEBNAS: Strengthening Exit Branches in Early-Exit Networks through Hardware-Aware Neural Architecture Search](https://arxiv.org/abs/2512.10671)
*Oscar Robben,Saeed Khalilian,Nirvana Meratnia*

Main category: cs.AI

TL;DR: 本文提出一个硬件感知的神经架构搜索框架，用于设计更高效的早退网络，通过优化出口分支的深度和层类型，结合自适应阈值调优，在保持或减少计算量的同时提高准确率。


<details>
  <summary>Details</summary>
Motivation: 早退网络通过根据输入复杂度调整计算来降低能耗和延迟，但设计过程耗时且需要平衡效率与性能。现有NAS方法主要关注出口分支的位置和数量，而出口分支的深度和层类型对效率和准确率也有重要影响。

Method: 使用硬件感知的神经架构搜索来强化出口分支，在优化过程中同时考虑准确率和效率。框架考虑了出口分支的不同深度和层类型，并结合自适应阈值调优。

Result: 在CIFAR-10、CIFAR-100和SVHN数据集上的评估表明，提出的框架设计的早退网络相比现有方法，在相同或更低的平均MACs计算量下实现了更高的准确率。

Conclusion: 通过硬件感知NAS优化出口分支的深度和层类型，结合自适应阈值调优，能够设计出更高效的早退网络，在资源受限设备上实现更好的准确率与计算效率平衡。
...
### [280] [Challenges of Evaluating LLM Safety for User Welfare](https://arxiv.org/abs/2512.10687)
*Manon Kempermann,Sai Suresh Macharla Vasu,Mahalakshmi Raveenthiran,Theo Farrell,Ingmar Weber*

Main category: cs.AI

TL;DR: 研究发现现有LLM安全评估主要关注通用风险，但针对个人建议场景需要上下文感知评估。实验表明缺乏用户上下文会导致安全评分虚高，而即使用户提供真实上下文信息，评估改进仍有限，特别是对弱势群体。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型安全评估主要关注通用风险（如危险能力），但数百万用户使用LLM获取个人财务和健康等高风险建议，这些危害是上下文依赖的而非通用的。虽然OECD等框架认识到需要评估个体风险，但用户福利安全评估仍不成熟。

Method: 探索性研究评估GPT-5、Claude Sonnet 4和Gemini 2.5 Pro在财务和健康建议上的表现，使用不同脆弱程度的用户档案。首先比较有无用户上下文评估者的安全评分差异，然后测试用户报告会披露的上下文信息是否能改善评估。

Result: 1) 无上下文评估者比有上下文评估者给出显著更高的安全评分（高脆弱用户从5/7降至3/7）；2) 即使用户提供真实上下文信息，评估改进仍不显著；3) 有效评估需要评估者基于多样化用户档案评估响应。

Conclusion: 有效的用户福利安全评估需要评估者基于多样化用户档案评估响应，仅靠用户上下文披露不足，特别是对弱势群体。本研究提供了上下文感知评估方法，证明评估个体福利需要不同于现有通用风险框架的方法。
...
### [281] [Enhancing Radiology Report Generation and Visual Grounding using Reinforcement Learning](https://arxiv.org/abs/2512.10691)
...
TL;DR: 研究探讨强化学习(RL)和显式推理("thinking")在胸部X光视觉语言模型中的作用，发现RL能提升报告生成和视觉定位性能，但显式推理无明显帮助。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型主要依赖监督微调(SFT)，但缺乏对答案质量的评估。强化学习能整合任务特定反馈，结合显式推理在数学和编程任务中已显示显著提升，本研究旨在探索RL和推理在胸部X光VLM中的效果。

Method: 基于Qwen3-VL构建RadVLM，进行大规模CXR数据SFT，然后通过冷启动SFT赋予模型基本推理能力。应用Group Relative Policy Optimization(GRPO)结合临床任务特定奖励进行报告生成和视觉定位，在有无推理的领域特定和通用领域变体上进行匹配RL实验。

Result: 强SFT对基础性能至关重要，RL在两个任务上都能提供额外提升，但显式推理未进一步改善结果。RL优化的RadVLM模型在统一评估流程中超越基线，在报告生成和视觉定位上达到最先进性能。

Conclusion: 临床对齐的强化学习是医学视觉语言模型监督微调的有力补充，能显著提升胸部X光解释性能，但显式推理在当前设置下未显示额外价值。
...
### [282] [COMPARE: Clinical Optimization with Modular Planning and Assessment via RAG-Enhanced AI-OCT: Superior Decision Support for Percutaneous Coronary Intervention Compared to ChatGPT-5 and Junior Operators](https://arxiv.org/abs/2512.10702)
...
TL;DR: CA-GPT AI-OCT系统在OCT引导的PCI规划和评估中，相比通用ChatGPT-5和初级医师表现出更优的决策一致性，特别是在复杂场景下。


<details>
  <summary>Details</summary>
Motivation: 虽然血管内成像（特别是OCT）能改善PCI结果，但其解读依赖操作者经验。通用AI虽有潜力但缺乏领域特异性可靠性，因此需要评估专门针对OCT的CA-GPT模型在PCI规划和评估中的性能。

Method: 单中心分析96例接受OCT引导PCI的患者，将CA-GPT、ChatGPT-5和初级医师生成的程序决策与专家记录进行比较。使用10个预先设定的指标评估PCI前和PCI后阶段的一致性。

Result: PCI前规划：CA-GPT中位一致性得分(5[3.75-5])显著高于ChatGPT-5(3[2-4])和初级医师(4[3-4])。在支架直径(90.3% vs 72.2%)和长度选择(80.6% vs 52.8%)上优于初级医师。PCI后评估：CA-GPT保持优异一致性(5[4.75-5])，显著高于ChatGPT-5和初级医师。亚组分析证实CA-GPT在复杂场景中的稳健优势。

Conclusion: 基于CA-GPT的AI-OCT系统在PCI规划和评估阶段均优于通用大语言模型和初级医师，为血管内成像解读提供了标准化可靠方法，显著增强操作者专业能力并优化OCT引导的PCI。
...
### [283] [HAROOD: A Benchmark for Out-of-distribution Generalization in Sensor-based Human Activity Recognition](https://arxiv.org/abs/2512.10807)
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中，个体、设备、环境和时间的变化会导致相同活动的分布偏移，现有OOD算法仅在特定场景（如跨设备或跨位置）中应用，缺乏对这些算法有效性的全面洞察。

Method: 提出HAROOD基准测试，定义4种OOD场景（跨人、跨位置、跨数据集、跨时间），构建包含6个数据集、16种比较方法（基于CNN和Transformer架构）的测试平台，采用两种模型选择协议。

Result: 通过大量实验发现：没有单一方法能始终优于其他方法，这突显了该领域仍有巨大进步空间；代码库模块化且易于扩展。

Conclusion: HAROOD为OOD-based HAR研究提供了全面的基准测试平台，有助于推动该领域的发展；代码已开源，便于社区使用和扩展。
...
### [284] [Agile Deliberation: Concept Deliberation for Subjective Visual Classification](https://arxiv.org/abs/2512.10821)
...
TL;DR: 提出Agile Deliberation框架，通过概念界定和迭代两个阶段，支持用户在模糊概念下通过边界案例反馈来训练视觉分类器。
...
Conclusion: Agile Deliberation有效支持主观、演化的概念定义，通过边界案例暴露帮助用户明确模糊概念，在内容审核等应用中具有实用价值。
...
### [285] [V-OCBF: Learning Safety Filters from Offline Data via Value-Guided Offline Control Barrier Functions](https://arxiv.org/abs/2512.10822)
...
<details>
  <summary>Details</summary>
Motivation: 现有离线安全强化学习方法通常只能保证软约束，无法确保前向不变性；而控制屏障函数（CBF）虽然能提供严格安全保证，但需要专家设计屏障函数或完整系统动力学知识。需要一种能从离线数据中学习安全控制器的方法。

Method: 提出V-OCBF框架：1）从离线演示中学习神经CBF，无需动力学模型；2）推导递归有限差分屏障更新，实现无模型学习；3）采用期望分位数目标，避免在分布外动作上查询屏障；4）使用QP公式合成实时安全控制

Result: 在多个案例研究中，V-OCBF相比基线方法显著减少了安全违规次数，同时保持了强大的任务性能，展示了其在不依赖在线交互或人工设计屏障函数的情况下合成安全关键控制器的可扩展性

Conclusion: V-OCBF为离线合成安全关键控制器提供了一种有效框架，能够从离线数据中学习神经CBF，实现严格的安全保证，无需在线交互或专家设计的屏障函数，在安全性和任务性能之间取得了良好平衡
...
### [286] [LLMs Can Assist with Proposal Selection at Large User Facilities](https://arxiv.org/abs/2512.10895)
*Lijie Ding,Janell Thomson,Jon Taylor,Changwoo Do*

Main category: cs.AI

TL;DR: LLMs可用于大型用户设施的提案选择，相比传统人工评审更可扩展、一致且成本效益高，能提供与人工排名强相关的排序，并在识别高发表潜力提案方面表现不亚于人工评审。


<details>
  <summary>Details</summary>
Motivation: 传统人工提案评审存在提案间相关性弱、评审者偏见和不一致性问题。基于成对偏好的方法虽然逻辑上更优越，但工作量呈二次方增长，对人类评审者不切实际。

Method: 利用LLMs进行提案选择，采用基于成对偏好的方法。使用橡树岭国家实验室散裂中子源三个束线的精心策划的提案和发表记录作为数据集，通过LLM生成提案排名。

Result: LLM排名与人工排名有强相关性（Spearman ρ≈0.2-0.8，去除10%异常值后≥0.5）。在识别高发表潜力提案方面，LLM表现不亚于人类评审者，且成本降低两个数量级以上。

Conclusion: LLMs为大型用户设施的提案选择提供了可扩展、一致且成本效益高的替代方案，不仅能提供与人工相当的排名，还能进行如通过嵌入模型定量评估提案相似性等人类难以完成的进阶分析。
...
### [287] [Multi-Granular Node Pruning for Circuit Discovery](https://arxiv.org/abs/2512.10903)
...
Result: 方法发现的电路节点数少于先前方法，证明粗粒度方法认为重要的许多神经元实际上无关紧要，同时保持任务性能，内存占用降低5-10倍。
...
### [288] [On Decision-Making Agents and Higher-Order Causal Processes](https://arxiv.org/abs/2512.10937)
*Matt Wilson*

Main category: cs.AI

TL;DR: 论文建立了POMDP决策代理与单输入过程函数之间的精确对应关系，揭示了AI与量子物理之间的对偶解释


<details>
  <summary>Details</summary>
Motivation: 探索人工智能中的决策代理与量子物理中高阶量子操作经典极限（过程函数）之间的深层联系，为理解智能系统提供新的理论框架

Method: 通过将POMDP中的代理策略和记忆更新组合成过程函数w，使用链接积与POMDP环境交互，建立精确的数学对应关系

Result: 成功建立了决策代理与过程函数之间的精确对应，发现对偶解释：物理视角下过程函数作为环境，AI视角下过程函数编码代理；并将此扩展到多代理系统的去中心化POMDP

Conclusion: 该研究揭示了人工智能决策理论与量子物理过程函数之间的深刻联系，为多代理系统提供了新的理论基础，并展示了跨学科研究的重要价值
