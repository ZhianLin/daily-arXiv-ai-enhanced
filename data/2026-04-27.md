...
TL;DR: 本文提出了一种利用单张图像预测太阳能板未来任意时刻辐照度的方法，通过视觉线索确定相机朝向和可见天空区域来预测太阳和天空辐照度，并利用反射辐照度的时间平滑性进行预测，在真实城市峡谷数据上验证了该方法优于传统方法和3D模型。


<details>
  <summary>Details</summary>
Motivation: 太阳能板的硬件成本虽已下降，但安装的软成本（如评估面板辐照度）仍然高昂。传统的3D建模方法无法捕捉影响辐照度的小型近邻结构，且成本较高。因此需要一种低成本、易操作的方法来评估城市环境中任意表面的太阳能潜力。

Method: 1) 利用面板位置拍摄的单张图像，通过视觉线索确定相机朝向和天空可见区域；2) 据此预测太阳直射和天空散射辐照度；3) 利用附近建筑反射辐照度的时间平滑性进行预测；4) 设计Solaris设备捕获面板视角图像。

Result: 在真实城市峡谷的辐照度测量数据上验证，该方法比传统的辐照度转换方法和3D模型仿真通常能提供更准确的辐照度预测。单张球面图像还可用于确定面板的最佳固定朝向。

Conclusion: 基于单张图像的方法可有效评估城市环境中任意表面的太阳能潜力并预测辐照度随时间的变化，成本低且精度优于传统方法，具有实用价值。
...
TL;DR: 本文提出Soft Anisotropic Diagrams (SAD)，一种可微分的显式图像表示方法，通过自适应各向异性站点和软Voronoi分区实现高效渲染，在Kodak数据集上达到46.0 dB PSNR，训练速度比Image-GS快4-19倍。


<details>
  <summary>Details</summary>
Motivation: 现有可微分图像表示方法（如Instant-NGP、Image-GS）在表示质量与计算效率之间存在权衡，且缺乏显式的空间结构控制。作者希望设计一种既能保持高质量重建、又能实现快速训练和渲染的显式可微分表示。

Method: 1) 提出软各向异性图（SAD），每个站点定义各向异性度量和加权的距离分数；2) 使用softmax混合每像素top-K站点生成颜色；3) 通过可学习温度实现软Apollonius图划分；4) 提出top-K传播方案（类jump flooding+随机注入）实现全局覆盖；5) GPU优先的梯度加权初始化、Adam优化和自适应预算控制（致密化与剪枝）。

Result: Kodak数据集达46.0 dB PSNR，编码仅需2.2秒（Image-GS需28秒），端到端训练速度比SOTA基线快4-19倍；在标准基准上一致优于Image-GS和Instant-NGP。

Conclusion: SAD是一种高效、紧凑且可微分的图像表示方法，支持快速随机访问和正向/逆向问题的无缝集成，在表示质量、训练速度和存储效率上均具有显著优势。
...
TL;DR: 本文介绍了EgoMAGIC数据集，包含3,355个视频、50项医疗任务，并提供了动作检测基线结果（最佳mAP 0.526），旨在推动AR辅助医疗AI研究。


<details>
  <summary>Details</summary>
Motivation: DARPA的PTG项目旨在开发集成于AR头显的虚拟助手，辅助用户完成复杂任务。本文旨在构建并公开一个大规模的自我中心视角医疗活动数据集，以支持相关AI研究。

Method: 收集3,355个头部佩戴立体相机录制的医疗任务视频；使用195万标签训练40个YOLO模型检测124个医疗对象；在8个医疗任务上使用三种模型进行动作检测基线实验。

Result: 最佳动作检测方法平均mAP达到0.526；训练了40个YOLO模型用于124类医疗对象检测；数据集包含3,355个视频覆盖50项任务。

Conclusion: EgoMAGIC数据集不仅适用于动作检测，也适合动作识别、目标检测、错误检测等视觉任务，为医疗AI提供了丰富的基准资源。
...
TL;DR: H-Sets是一个两阶段框架，通过Hessian矩阵检测高阶特征交互，并结合SAM分割先验与IDG-Vis归因方法，生成更稀疏、更忠实于模型的图像分类显著性图。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法大多只关注单个特征的边际效应，忽略了特征交互（多个特征共同影响模型输出）的重要性。在图像分类中，语义往往源于像素间的相互依赖，而现有交互方法要么过于粗糙（如仅使用超像素），要么不满足可解释性公理。

Method: 两阶段框架：1) 通过输入Hessian检测局部交互像素对，递归合并为语义集合，并利用SAM分割作为空间分组先验；2) 使用IDG-Vis（集成方向梯度的集合级扩展）对每个集合进行归因，沿像素空间路径积分方向梯度，并用Harsanyi红利聚合。

Result: 在VGG、ResNet、DenseNet和MobileNet模型上，基于ImageNet和CUB数据集的评估表明，H-Sets生成的显著性图比现有方法更可解释、更忠实于模型。

Conclusion: H-Sets通过显式建模高阶特征交互，有效提升了显著性图的稀疏性和忠实度，尽管Hessian计算带来额外开销，但这一代价是目标明确的，且带来了显著的质量提升。
...
TL;DR: 本文提出FLARE-BO框架，通过贝叶斯优化联合优化8个图像增强参数（伽马校正、光照归一化、色度去噪、双边滤波、NLM去噪、白平衡、后平滑），在低光照条件下实现无需训练的视觉增强，在LOL数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的低光照增强方法（基于高斯过程的贝叶斯优化）仅能优化3个参数，缺乏光照分解和白平衡校正，且使用的NLM去噪在噪声条件下易过度平滑边缘，亟需更全面的参数优化框架。

Method: 提出FLARE-BO框架，联合优化8个参数：伽马校正、LIME式光照归一化、色度去噪、双边滤波、NLM去噪、Grey-World自动白平衡、自适应后平滑。采用单位超立方体参数归一化、目标标准化、Sobol拟随机初始化和Log期望改进采集函数进行搜索。

Result: 在Low Light paired dataset (LOL)上评估，FLARE-BO相比未在该数据集上专门训练的现有方法取得显著改进。

Conclusion: 通过扩展参数空间和优化策略，FLARE-BO有效提升了低光照图像增强性能，证明了无需学习的贝叶斯优化方法在更大参数空间中仍具有竞争力。
...
TL;DR: 本文提出了一套针对鱼眼相机的光学动捕系统外参标定与验证方案，通过联合估计相机外参和板-标记变换、采用分阶段求解器提升收敛可靠性，并设计了独立验证工具Lollypop，在Meta Quest 3上验证了优于现有方法的效果，已部署于生产线。


<details>
  <summary>Details</summary>
Motivation: 现有光学动捕系统在AR/VR、SLAM等场景的外参标定存在多重误差源（如板-标记装配差异、初始化歧义、漂移），且鱼眼相机的非线性畸变使标定与验证更加困难，而这些误差常被忽视直至影响下游数据质量。

Method: 1) 联合优化：同时估计相机外参和板-标记变换（board-to-marker transform）；2) 分阶段求解器：分步优化以应对初始化歧义，提升收敛稳定性；3) 独立验证工具Lollypop：构建完全独立于标定数据的测量链，实现快速、无操作员依赖的标定质量评估。

Result: 在Meta Quest 3头显（配备鱼眼相机）上，所提标定方法优于现有手工标定结果；Lollypop能够可靠检测标定质量随时间退化的情况。

Conclusion: 提出的标定与验证系统有效解决了鱼眼相机动捕标定中的鲁棒性和退化检测问题，已在生产级数据采集流程中部署应用。
...
TL;DR: 本文提出Pixel-Aligned 1DoF Gaussian Splatting (PAGaS)，将高斯泼溅从新视角合成扩展到多视图立体深度任务，通过将高斯的位置和大小约束在反向投影的像素体素内，仅优化深度这一自由度，实现了高精度的深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法虽然在新型视角合成上表现优异，但在几何重建精度上仍有不足。作者希望将GS表示从新视角合成任务扩展到多视图立体深度估计任务，同时解决现有GS变体难以准确建模场景几何的问题。

Method: 提出Pixel-Aligned 1DoF Gaussian Splatting (PAGaS)，核心创新是像素的深度使用单自由度高斯建模。高斯的位置和大小被反向投影的像素体素严格约束，优化过程中仅深度作为唯一的自由参数进行优化，从而保持高斯紧密约束。

Result: 在具有挑战性的3D重建基准上，PAGaS相比参考的几何方法和基于学习的多视图立体基线方法，在深度估计精度上取得了显著提升，能生成高度细节化的深度图。

Conclusion: PAGaS成功将高斯泼溅应用于多视图立体深度任务，通过极致的自由度约束（仅1DoF）获得了高精度的几何重建结果，为GS在几何任务上的应用提供了新思路。
...
TL;DR: 本文提出了一种无监督异常检测框架，通过学习健康视网膜的规范分布，无需标注即可在OCT图像中检测和定位病变，在多个数据集上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 监督深度学习模型需要昂贵且劳动密集型的专家标注，且在不同病理、设备和患者群体中泛化能力差，亟需一种无需标注的自动化OCT分析方法。

Method: 采用离散潜在模型学习健康OCT结构模式，结合视网膜层感知监督和结构化三元组学习区分健康与病理表征，通过重建差异在图像和像素级别检测异常。

Result: 在Kermany数据集AUROC达0.799，跨数据集Srinivasan上AUROC达0.884，RETOUCH基准上无监督分割Dice 0.200、mIoU 0.117。

Conclusion: 提出的无监督框架有效解决了OCT分析中的标注瓶颈问题，具备良好的跨数据集泛化能力和临床部署潜力。
...
TL;DR: 本文提出了一种受人类视觉感知启发的统一生成模型，通过层次化粒子分组实现跨场景的运动感知（随机点、纹理、自然视频），并利用并行块吉布斯采样进行高效推理，在多个基准上验证了其类人感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉系统缺乏能够在稀疏运动点、纹理表面和自然场景等多种设置下统一工作的运动感知方法，而人类视觉却能鲁棒地完成这些任务。受此启发，作者希望建立一种通用的、基于人类视觉原理的运动感知框架。

Method: 提出层次化生成模型：将低层运动线索和高层外观特征分组为粒子（表示局部物质的高斯分布），再将粒子聚类为独立运动的物理实体。推导基于并行化块吉布斯采样的硬件加速推理算法来恢复稳定的粒子运动和分组。

Result: 在三个领域验证：1) 2D随机点运动图：捕捉到人类物体感知（包括模糊条件下的渐变不确定性）；2) 伪装旋转物体数据集：正确恢复3D结构运动和2D分割；3) 自然RGB视频：追踪组成变形物体的3D运动物质，实现鲁棒的物体级场景理解。

Conclusion: 该工作建立了一个基于运动感知的通用框架，根植于人类视觉原理，弥合了计算机视觉与生物视觉在运动感知领域的差距。
...
TL;DR: SAMIDARE是一个增强SAM2MOT的框架，通过密度感知掩码重新生成、选择性记忆更新和状态感知关联，在密集体育场景中实现SOTA多目标跟踪性能，在SportsMOT数据集上比基线提升2.5 HOTA和4.2 IDF1。


<details>
  <summary>Details</summary>
Motivation: 现有基于分割的多目标跟踪方法在密集场景中容易受到掩码误差和ID切换的影响，限制了自动体育分析的效果。

Method: 提出SAMIDARE框架，包含三个核心组件：(1)密度感知掩码重新生成；(2)选择性记忆更新；(3)状态感知关联与新轨迹初始化。

Result: 在SportsMOT验证集上达到SOTA，相比基线提升2.5 HOTA和4.2 IDF1。
...
TL;DR: 本文研究互动场景中基于一人运动生成另一人运动的问题，构建了拳击配对动作数据集，对比了三种Transformer模型，并引入人物ID嵌入来区分个体。实验表明简单Transformer效果最好，而iTransformer和Crossformer会累积误差，人物ID嵌入有助于保持结构一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法多聚焦于单智能体运动生成（从文本生成或单一人体运动预测），缺乏对互动场景中两人运动相互依赖关系的研究。本文旨在解决互动场景中基于一人运动生成另一人运动的问题。

Method: 构建了拳击比赛视频中提取的配对动作-反应运动序列数据集；对比了三种模型：简单Transformer、iTransformer和Crossformer；引入人物ID嵌入来显式区分个体，帮助模型保持结构一致性并捕捉互动动态。

Result: 简单Transformer能生成合理的互动感知运动且无姿态崩溃；iTransformer和Crossformer随时间累积误差导致运动生成不稳定；人物ID嵌入有助于防止结构崩溃并提升运动一致性。

Conclusion: 在互动感知运动生成中，显式建模个体身份非常重要。简单Transformer在该任务上优于更复杂的iTransformer和Crossformer。
...
TL;DR: 本文针对SAR图像中广义类别发现（GCD）任务受限于大视觉模型（LVM）的光学先验与SAR图像之间的跨模态不兼容性问题，提出了模态差异曲线（MDC）来建模跨模态差异，并设计了MDC引导的跨模态先验迁移（MCPT）框架，通过自适应频率标记化（AFT）和频率感知专家精炼（FER）实现光学先验到SAR域的有效迁移，在多个主流数据集上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 通用类别发现（GCD）在标签稀缺的SAR领域具有重要前景，但大视觉模型的光学先验与SAR图像之间存在跨模态不兼容性，现有域适应方法缺乏反映成像特性的归纳偏置，无法有效将光学先验迁移到SAR领域。

Method: 提出模态差异曲线（MDC）作为频率域描述子来建模跨模态差异；提出MDC引导的跨模态先验迁移（MCPT）框架，包含自适应频率标记化（AFT）将MDC转化为可学习标记，以及频率感知专家精炼（FER）进行带感知的差异特征精炼，通过对比学习对齐跨模态嵌入并内化适应模式。

Result: 在多个主流数据集上取得了最先进的性能，表明频率域差异建模能够更有效地将光学先验适应到SAR图像中。

Conclusion: 频率域差异建模是实现光学先验向SAR图像有效迁移的关键，MCPT框架通过显式建模模态差异并引导特征学习，显著提升了SAR-GCD任务性能。
...
TL;DR: 提出UniME，一种两阶段异质架构方法，通过Stage 1的单一ViT统一编码器（掩码图像建模预训练）和Stage 2的多个CNN专用编码器（提取细粒度特征）的协同，解决脑肿瘤分割中MRI模态缺失问题，在BraTS 2023/2024上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床MRI扫描常缺失一种或多种模态，导致分割性能下降。现有方法难以同时兼顾细粒度结构捕获、跨模态互补性建模和可用模态的有效利用，需要一种能调和这三者权衡的方案。

Method: 两阶段异质架构：Stage 1用掩码图像建模预训练单个ViT统一编码器，建立对缺失模态鲁棒的统一表征；Stage 2添加多个模态专用的CNN编码器，提取高分辨率、多尺度的细粒度特征，并与全局表征融合以生成精确分割。

Result: 在BraTS 2023和BraTS 2024数据集上，UniME在不完全多模态场景下优于此前所有方法。

Conclusion: 通过解耦表征学习与分割任务，两阶段异质设计有效调和了细粒度特征、跨模态互补性和可用模态利用之间的矛盾，为缺失模态下的脑肿瘤分割提供了新范式。
...
TL;DR: 提出EvFlow-GS框架，利用事件流和光流联合优化可学习双积分、相机位姿和3D高斯泼溅，从运动模糊图像中实现锐利3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖不精确的事件双积分先验和噪声事件，导致伪影和纹理模糊，亟需更鲁棒的联合优化框架。

Method: 1) 利用光流从事件中提取边缘信息；2) 设计基于事件的损失分别应用于不同模块；3) 引入事件残差先验增强3DGS渲染图像的强度变化监督；4) 将3DGS和LDI输出整合为联合损失，实现互促优化。

Result: 实验表明EvFlow-GS在运动模糊场景下的3D重建性能达到领先水平。

Conclusion: 通过事件流与光流的协同以及端到端可学习设计，有效解决了运动模糊3D重建中的伪影和纹理模糊问题。
...
TL;DR: SAGA-ReID提出一种基于CLIP文本嵌入空间锚点向量的结构化聚合方法，通过对齐中间补丁特征与锚点向量来重建身份表示，在遮挡和跨视角场景下显著优于传统全局池化方法，在遮挡基准上最高提升+10.6 Rank-1。


<details>
  <summary>Details</summary>
Motivation: CLIP-based ReID方法将空间特征聚合为单一全局[CLS]token，该token优化目标是图像-文本对齐而非空间选择性，导致遮挡和跨视角变化下表示脆弱。

Method: 将中间补丁token与CLIP文本嵌入空间中参数化的锚点向量对齐，强调空间稳定证据同时抑制被遮挡或缺失区域，无需单张图像的文本描述。

Result: 在合成遮挡和真实人体干扰两种条件下，SAGA的优势随遮挡增加而显著增长；在遮挡基准上Rank-1提升最高达+10.6。

Conclusion: 结构化重建方法解决了仅靠骨干网络质量和架构复杂度无法解决的瓶颈问题，在更强骨干上仍优于专用序列补丁聚合方法。
...
TL;DR: CharTide是一个面向图表到代码生成的数据中心框架，通过三视角微调策略（解耦视觉感知、纯文本代码逻辑和模态融合）构建2M样本数据集，并采用基于信息不变性的查询驱动强化学习进行对齐，使7B/8B模型超越GPT-4o并接近GPT-5性能。


<details>
  <summary>Details</summary>
Motivation: 现有图表到代码生成方法受限于数据集的简单缩放，将视觉感知与程序逻辑混为一谈，无法充分利用多模态监督信号，导致模型在视觉精度和语法正确性上存在根本性瓶颈。

Method: 提出CharTide框架：(1) 三视角微调策略，将训练过程解耦为视觉感知、纯文本代码逻辑和模态融合三个独立流；(2) 构建2M样本数据集；(3) 基于信息不变性的查询驱动强化学习框架，将对齐问题转化为数据验证问题，使用冻结的Inspector通过原子级QA任务提供可验证的奖励信号。

Result: CharTide-7B/8B在ChartMimic、Plot2Code和ChartX基准上显著超越开源基线，超越GPT-4o，与GPT-5性能相当。

Conclusion: 通过数据中心的方法设计（而非单纯的模型缩放），系统性地重构训练和对齐数据，可以更有效地利用多模态监督信号，显著提升小参数模型的图表代码生成能力。
...
TL;DR: 本文提出首个从单张自然图像中检测3D反射对称性的框架，针对建筑地标场景。通过自动数据标注管道构建大规模数据集ArchSym，并设计结合场景几何的对称性检测器，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的3D对称性检测方法局限于物体中心或合成数据集，无法泛化到真实场景；同时单目输入的尺度模糊性使得3D平面定位困难，现有工作多只预测平面方向。

Method: (1) 基于SfM重建和跨视图图像匹配的自动数据标注管道，构建ArchSym数据集；(2) 单视图对称性检测器，通过将对称性参数化为相对于预测场景几何的符号距离图，实现精确的3D定位。

Result: 在自建基准上，本文方法显著优于现有最先进基线模型。

Conclusion: 本文首次实现从自然图像中检测3D反射对称性，验证了大规模数据标注管道的有效性，并为单视图3D对称性检测提供了新范式。
...
TL;DR: 本文提出FMDiffWA，一种基于频域调制扩散框架的水印攻击方法，通过频域水印调制模块和优化训练策略，在有效去除隐形水印的同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 当前数字水印攻击技术进展缓慢，打破了攻防平衡，阻碍了该领域的进一步发展。

Method: 提出FMDiffWA框架，包含频域水印调制（FWM）模块，嵌入前向和反向扩散过程的采样阶段；并改进扩散模型训练策略，增加辅助精炼约束。

Result: 相比现有水印攻击方法，FMDiffWA在视觉保真度上表现更优，且对多种水印方案具有良好的泛化能力。

Conclusion: FMDiffWA有效实现了攻击效能与视觉质量的更好权衡，促进了水印攻防领域的平衡发展。
...
TL;DR: 本文提出SportsTime（长视频体育理解基准，含1.4万+问答对和5万+时序证据标注）和Chain-of-Time Reasoning (CoTR)方法，通过时序奖励GRPO训练和锚点-观察-推理证据循环，显著提升多模态大模型在体育视频中的长时序推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在体育视频长时序推理中表现不佳，归因于两个耦合因素：对时序分散证据的监督不足，以及缺乏要求模型识别、定位和验证时序证据的方法。

Method: 提出Chain-of-Time Reasoning (CoTR)：训练阶段使用时序奖励GRPO鼓励时序接地推理；推理阶段采用锚点-观察-推理（anchor-observe-infer）证据搜索循环，迭代定位、验证和组合时序证据后生成最终答案。同时构建SportsTime基准数据集。

Result: 实验表明SportsTime作为基准的有效性，CoTR在时序组合推理和逐步接地质量上持续优于强MLLM基线。

Conclusion: 通过细粒度时序证据监督和显式推理过程设计，可有效提升MLLM在长视频体育领域的理解和推理能力。
...
TL;DR: 本文提出OccDirector，一个仅依赖自然语言即可生成4D占用动态的框架，无需几何先验，实现了从外观合成到语言驱动的行为编排的范式转变。


<details>
  <summary>Details</summary>
Motivation: 现有生成框架依赖刚性几何条件（如显式轨迹）或简单属性级文本，无法编排复杂的多智能体顺序交互，存在语义-时空鸿沟。

Method: 采用VLM驱动的时空MMDiT架构，结合历史前缀锚定策略保证长时序交互一致性，并构建了多级语言指令标注的OccInteract-85k数据集。

Result: 在生成质量和指令跟随能力上达到SOTA，成功实现从外观合成到语言驱动的行为编排。

Conclusion: OccDirector证明了仅凭自然语言即可生成物理合理的4D占用动态，开辟了语言驱动的自动驾驶行为编排新范式。
...
TL;DR: 本文提出了面向城市交通的开放场景视觉问答数据集LTD（包含11.6K高质量VQA对）及统一基础模型UniVLT，该模型通过课程式知识迁移统一了微观自动驾驶推理与宏观交通分析，在多个基准上达到SOTA性能，并揭示了现有基础模型在多视角复杂交通场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有交通系统研究主要集中在微观自动驾驶领域，缺乏面向城市级交通分析的开放式安全导向视觉问答（VQA）基础模型，尤其是在异构路侧摄像头观测的多视图推理方面存在空白。

Method: 构建LTD数据集（11.6K VQA对，涵盖多视角、多天气、多光照条件），包含三项互补任务：细粒度多目标定位、多图像相机选择、多图像风险分析。采用多模型视觉语言生成+交叉验证+人工循环精炼保证标注质量。提出UniVLT，通过课程式知识迁移训练统一微观AD推理与宏观交通分析。

Result: 在LTD及多个AD基准上，UniVLT在开放推理任务中达到SOTA性能，同时揭示了现有基础模型在多视图复杂交通场景中的不足。

Conclusion: 所提出的数据集和模型有效填补了城市级交通开放域推理的空白，为智能交通系统提供了可扩展的基础设施级智能解决方案。
...
TL;DR: 本文提出基于反事实关系验证的证据驱动开放词汇场景图生成框架，通过分解谓词为视觉/几何/上下文证据，并利用反事实验证器确保关系预测依赖于真实视觉证据而非语言先验，在多个基准上提升了可靠性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇SGG方法虽扩展了语义覆盖范围，但预测的关系可能受语言先验或物体共现驱动，缺乏对视觉证据的可靠依赖，导致可靠性问题。

Method: 1) 使用视觉-语言模型生成开放词汇关系候选；2) 将谓词短语分解为支持、接触、包含、深度、运动、状态等软证据基；3) 关系条件证据编码器提取谓词相关线索；4) 反事实验证器检验移除必要证据后关系分数是否下降、无关扰动下是否稳定；5) 引入矛盾感知谓词学习和图级偏好优化。

Result: 在常规、开放词汇和全景SGG基准上一致提升了标准召回率指标、未见谓词泛化能力和反事实接地质量。
...
TL;DR: 提出RIME框架，通过检索友好的改写和跨模态对齐，在减少思维链冗余的同时显著提升多模态嵌入性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理驱动的生成式多模态嵌入存在CoT思维步骤冗余、语义歧义等问题，限制了在广泛检索场景中的表现。

Method: 提出RIME框架：(1) 检索友好的改写机制；(2) 跨模态对齐(CMA)桥接生成式与判别式嵌入空间；(3) Refine强化学习(Refine-RL)以判别式嵌入作为语义锚点引导改写优化。

Result: 在MMEB-V2、MRMR和UVRB三个基准上，RIME显著优于先前的生成式嵌入模型，同时大幅减少推理长度。

Conclusion: RIME通过改写驱动的统一框架，有效解决了生成式多模态嵌入中CoT的冗余问题，在效率和准确性之间取得了更好的平衡。
...
TL;DR: DocPrune是一个无需额外训练的渐进式文档标记剪枝框架，通过保留任务关键标记、移除背景和无关标记，并在适当层次自动启动剪枝，实现长文档理解中3倍吞吐量提升和F1分数+1.0的提高。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理文档图像时面临计算资源浪费问题，因为文档图像包含大量背景和稀疏支持证据，而现有的自然图像/视频标记剪枝方法无法利用文档独有的结构稀疏性。

Method: 提出DocPrune框架，包含两个核心策略：(1) 渐进式保留任务关键标记，移除背景或问题无关标记；(2) 基于模型理解水平自动选择合适的层开始标记剪枝，无需任何额外训练。

Result: 在M3DocRAG上，DocPrune在编码器端实现3.0x吞吐量提升，解码器端实现3.3x提升，同时F1分数提升+1.0，实现了更高精度和效率。

Conclusion: 该工作验证了利用文档结构稀疏性进行标记剪枝的有效性，在不增加训练成本的前提下同时提升速度和精度，为长文档理解提供了高效解决方案。
...
TL;DR: 本文评估了月球环境中不同相机模型和光照条件对合成图像质量的影响，使用了来自月船二号和NASA仪器的真实数字高程模型数据，旨在提高自主导航和决策系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 合成图像生成对于行星任务的可视化、成像系统测试和探索活动规划至关重要，尤其是在评估着陆点、检测危险和验证导航系统方面，因此需要系统评估不同图像模拟方法的效果。

Method: 利用来自月船二号轨道器高分辨率相机（OHRC）、NASA宽角相机（WAC）和窄角相机（NAC）的真实数字高程模型（DEM）和地形数据，评估不同相机模型和光照条件对合成月球图像质量的影响。

Result: 评估了多种图像模拟方法，揭示了不同相机模型和光照条件对合成月球图像质量的具体影响。

Conclusion: 该研究有助于提高合成图像在月球探测自主导航和决策系统中的可靠性，为开发更有效的工具以支持未来月球任务和增强对月面环境的理解做出了贡献。
...
TL;DR: 本文提出KVBench基准（1800个专家提示，涵盖6个高中学科）评估知识密集型文本到图像生成，发现现有模型在逻辑推理、符号精度和多语言鲁棒性上存在严重缺陷。并提出KE-Check框架（知识阐述+检查表引导修正），有效减轻科学幻觉，缩小开源与闭源模型差距。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型在自然图像生成上表现优异，但在知识密集型场景（如科学可视化）中，需要严格遵循领域知识、结构约束和符号惯例，而当前模型在视觉合理性与科学正确性之间存在显著差距，这一问题尚未被系统研究。

Method: (1) 构建KVBench基准：基于30+权威教材，由专家设计1800个提示，涵盖生物、化学、地理、历史、数学、物理六大学科。(2) 提出KE-Check框架：两阶段方法，第一阶段进行知识阐述以丰富提示结构，第二阶段通过检查表引导修正，识别违反科学约束并执行约束导向的编辑。

Result: 评估14个SOTA模型（开源和闭源），发现所有模型在逻辑推理、符号精度和多语言鲁棒性上存在显著缺陷，开源模型一致落后于闭源系统。KE-Check有效减轻了科学幻觉，缩小了开源与闭源模型之间的性能差距。

Conclusion: 知识密集型T2I生成是一个尚未解决的关键挑战，现有模型远未达到科学正确性要求。KE-Check提供了一种有效缓解方案，但仍有较大提升空间。
...
TL;DR: 提出了一种名为Dual Convergent Lines (DCL)的新型关键点混淆方法，通过双锚点与分割线设计，使得几何恢复攻击的优化问题病态化，从而有效抵御隐私攻击，适用于基于云的视觉定位中的隐私保护图像查询。


<details>
  <summary>Details</summary>
Motivation: 现有的PPIQ方法（基于几何和基于分割的混淆）均易受隐私攻击，尤其是几何混淆方法中，混淆后邻接线的空间分布仍围绕原始关键点位置，为恢复原始点提供了可利用的线索。

Method: DCL在中心分割线上放置两个固定锚点，根据关键点的位置决定使用哪个锚点，将每个关键点提升为从该锚点出发的直线。这种设计使得邻接线要么汇聚到同一锚点（平凡解），要么在分割边界处近乎平行（高方差解），均导致点恢复失败。

Result: 在室内和大型室外数据集上的实验表明，DCL在隐私攻击鲁棒性、效率、可扩展性方面表现优异，同时达到了实用的定位性能。

Conclusion: DCL通过巧妙的几何设计从根源上破坏了攻击的优化条件，是一种高效且可部署的隐私保护关键点混淆方案。
...
TL;DR: 本文研究了从立体视觉到单目深度估计的过渡，在仿真中立体视觉精度更高，但真实世界中单目方法（UniDepthV2+YOLO12n）更鲁棒且成本效益更优，尽管深度推理仅0.1 FPS、检测10 FPS。


<details>
  <summary>Details</summary>
Motivation: 探索将深度感知的月球车导航从立体视觉转向单目深度估计的可行性，降低硬件成本与功耗，同时验证边缘AI在真实部署中的鲁棒性。

Method: Unity仿真环境下用OpenCV StereoSGBM生成立体视差图；物理车上基于树莓派4，采用UniDepthV2进行单目度量深度估计，YOLO12n进行实时目标检测。

Result: 仿真中立体视觉精度更高；真实部署中单目方法更鲁棒且成本效益更好，深度推理速度0.1 FPS，目标检测速度10 FPS。

Conclusion: 单目深度估计+边缘AI在真实月球车导航中更具实用性，但当前深度推理速度是主要瓶颈，需进一步优化。
...
TL;DR: 提出了ChangeQuery，一个统一的多模态框架，结合灾前光学影像与灾后SAR特征，实现全天候、交互式的灾害态势感知。构建了DICQ数据集（平衡自然灾害与武装冲突），并设计自动化语义标注流水线生成细粒度指令，在复杂灾害监测任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言灾害评估方法受限于单模态光学依赖、偏向自然灾害场景、缺乏可交互的细粒度推理能力，无法提供面向复杂战略查询的可操作情报。

Method: 1) 构建DICQ数据集：结合灾前光学语义与灾后SAR结构特征，平衡覆盖自然灾害与武装冲突；2) 提出自动化语义标注流水线(ASAP)，采用"统计优先、生成在后"范式，将分割掩码自动转化为有空间和数量感知的分层指令集；3) ChangeQuery架构作为交互式灾害分析师，支持多任务推理。

Result: 大量实验表明ChangeQuery在复杂灾害监测任务上达到新SOTA，提供鲁棒且可解释的解决方案。

Conclusion: ChangeQuery突破了单模态与灾害类型偏见的限制，通过多模态融合与结构化交互数据训练，实现了全天候、可交互的灾害态势感知，为复杂灾害监测提供了稳健且可解释的解决方案。
...
TL;DR: 本文提出DONUT合成基准和FILTR框架，首次实现从3D点云编码器特征中通过可学习的神经网络直接预测持久性图（拓扑描述符），实验表明现有编码器保留的全局拓扑信号有限，但FILTR仍能有效利用这些信息近似持久性图。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以从3D点云编码器（如Point-BERT, Point-MAE）产生的特征中提取拓扑信息（持久性图），填补了预训练编码器在拓扑理解方面的评估空白。

Method: 1) 构建DONUT合成基准数据集，具备可控的拓扑复杂度；2) 提出FILTR（Filtration Transformer），采用transformer解码器架构，将持久性图生成视为集合预测任务，直接从冻结的编码器特征中预测持久性图。

Result: DONUT分析显示现有编码器仅保留有限的全局拓扑信号，但FILTR仍能有效利用编码器信息近似持久性图，首次实现从原始点云通过可学习前馈机制以数据驱动方式提取持久性图。

Conclusion: 尽管当前3D点云编码器对全局拓扑信息的保留有限，但FILTR框架证明了从编码器特征中学习拓扑描述符是可行的，为3D形状的拓扑理解开辟了新途径。
...
TL;DR: 本文提出了一种基于光流引导的动态3D高斯泼溅SLAM框架，通过运动掩码生成、动态高斯时序建模和自适应插入策略，实现了动态场景下的高效跟踪、重建和渲染。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM方法难以高效重建动态场景中的静态和动态区域，亟需一种既能鲁棒估计相机位姿又能高效重建动态环境的框架。

Method: 1）利用输入深度和光流先验，通过相机自运动模型分解光流，生成类别无关的运动掩码；2）对动态高斯在关键帧处显式建模时序中心，利用3D场景流先验传播并采用自适应插入策略；3）使用高斯混合模型（GMM）自适应学习动态高斯的时间不透明度和旋转。

Result: 在跟踪精度、动态重建质量和训练效率方面均达到当前最优水平。

Conclusion: 提出的光流引导动态3DGS SLAM框架有效解决了动态场景下的高效重建难题，为动态SLAM提供了新的思路。
...
TL;DR: PoseFM 是首个将单目帧间视觉里程计重构为生成式任务的方法，利用 Flow Matching 将相机运动建模为分布而非点估计，通过连续时间 ODE 实现不确定性感知的鲁棒位姿预测，在多个基准上取得有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的单目 VO 方法大多采用确定性回归，缺乏对预测不确定性的建模能力，在结构或光照条件差的环境中鲁棒性不足。

Method: 提出 PoseFM 框架，将帧间 VO 视为生成式任务；利用 Flow Matching 将相机运动建模为分布，通过连续时间 ODE 从噪声中生成位姿预测，并提供不确定性估计机制。

Result: 在 TartanAir、KITTI 和 TUM-RGBD 基准上取得强性能，部分轨迹达到最低绝对轨迹误差（ATE），整体与最优帧间单目 VO 方法竞争。

Conclusion: PoseFM 证明了将生成式 Flow Matching 引入 VO 的有效性，为不确定性感知的鲁棒位姿估计提供了新范式。
...
TL;DR: 本文提出了一种名为OSFENet的单次学习网络，通过学习目标点云的数据分布来实现边缘提取，比基于通用数据分布训练的网络效果更好。该方法设计了基于滤波KNN的表面块表示，并引入RBF_DoS模块。在ABC数据集上对比7个基线，并在S3DIS、Semantic3D、UrbanBIS等数据集上验证了实用性。


<details>
  <summary>Details</summary>
Motivation: 不同扫描仪具有独特的采样误差分布，基于多扫描仪数据训练的网络效果不如基于单一扫描仪数据训练的网络。因此需要一种能适应目标点云特定数据分布的边缘提取方法。

Method: 提出OSFENet单次学习网络，设计了滤波KNN（filtered-KNN）表面块表示以支持单次学习框架，并引入RBF_DoS（Radial Basis Function-based Descriptor of Surface patch）模块来增强点云边缘提取能力。

Result: 在ABC数据集上与7个基线方法对比展现出优势，在S3DIS（室内）、Semantic3D和UrbanBIS（室外）等真实扫描数据集上验证了实际效用。

Conclusion: OSFENet通过学习特定点云的数据分布，在边缘提取任务上优于基于通用数据分布训练的网络，表现出良好的泛化能力和实用性。
...
TL;DR: 本文提出了一种名为嵌入损失（EL）的新型辅助损失函数，通过随机初始化网络的特征嵌入对齐分布，显著提升了扩散模型蒸馏的效率和质量，在CIFAR-10上取得了SOTA的FID值，并将训练迭代次数减少高达80%。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型蒸馏方法存在计算资源需求大、训练时间长的问题，且现有辅助损失函数（如回归损失需要预生成数据集且上限受限，GAN损失训练不稳定）均有明显局限性，因此需要一种更高效、稳定的补充损失函数。

Method: 提出嵌入损失（Embedding Loss, EL），利用多种随机初始化网络的特征嵌入，通过计算嵌入特征空间中的最大均值差异（MMD）来对齐蒸馏得到的少步生成器与原始数据的特征分布，从而实现鲁棒的分布匹配。

Result: 在CIFAR-10上，无条件生成FID达1.475，条件生成FID达1.380，均为当时SOTA；在ImageNet、AFHQ-v2、FFHQ等多个基准上使用DMD、DI、CM等蒸馏框架均取得一致改进；训练迭代次数减少高达80%。

Conclusion: EL作为一种简单有效的辅助损失函数，能够以更少的计算资源和训练时间显著提升扩散模型蒸馏的生成质量，为资源受限场景下部署扩散生成模型提供了更实用和可扩展的解决方案。
...
TL;DR: 本文提出了一种用于TRUS视频前列腺癌分类的启发式帧选择方法和三分支协同特征学习网络（HFS-TriNet），通过动态帧采样、SAM大模型特征提取和WTCR去噪边缘检测，解决了视频分析中的信息冗余、类间相似度高和低信噪比问题。


<details>
  <summary>Details</summary>
Motivation: TRUS视频相比静态图像包含更丰富的时空信息，有助于提升计算机辅助诊断的准确性和鲁棒性，但存在信息冗余导致计算成本高、帧间类内/类间相似度高使特征提取困难、信噪比低阻碍临床信息识别等问题，亟需有效的解决方案。

Method: 提出HFS-TriNet方法：1）HFS策略通过动态初始化训练片段起点，实现间隔帧采样以减轻冗余；2）三分支网络包括：常规ResNet50分支、基于预训练医学SAM的大模型分支（提取深度特征并利用归一化注意力模块探索时序一致性）、以及小波变换卷积残差（WTCR）分支（提取高频域病灶边缘信息并在低频域去噪）。

Result: 未在摘要中明确给出具体数值结果，但该方法针对TRUS视频分类的三大挑战（冗余、高相似度、低信噪比）分别提出了对应策略。

Conclusion: HFS-TriNet通过启发式帧采样和三分支协同特征学习，有效提升了TRUS视频中前列腺癌分类的性能，为计算机辅助诊断提供了新的解决方案。
...
TL;DR: FoL++ 提出了一种结合鲁棒判别区域建模与自适应重排序的视觉定位方法，通过可靠性估计分支、空间对齐损失和自适应候选调度器，在七个基准上实现了最先进的性能，且推理速度比 FoL 提升 40%。


<details>
  <summary>Details</summary>
Motivation: 现有 VPR 方法受限于无关区域导致的感知混淆（perceptual aliasing）以及刚性候选调度策略造成的低效重排序问题。

Method: 1) 可靠性估计分支（Reliability Estimation Branch）生成空间可靠性图以建模遮挡鲁棒性；2) 两种空间对齐损失（SAL 和 SCEL）对齐特征并突出显著区域；3) 伪对应策略（pseudo-correspondence）在弱监督下生成密集局部特征监督；4) 自适应候选调度器（Adaptive Candidate Scheduler）根据全局相似度动态调整候选池大小。

Result: 在七个基准数据集上达到最先进性能，内存占用轻量，推理速度相比 FoL 提升 40%。

Conclusion: 通过自适应融合全局与局部证据并加权局部匹配可靠性，FoL++ 超越了传统的独立匹配系统，验证了区域可靠性建模与自适应调度在 VPR 中的有效性。
...
TL;DR: 本文提出SpaMEM基准，用于评估多模态大语言模型在具身空间推理中的空间信念演化能力。该基准包含超千万张图像，设计15个诊断任务（三级层次），发现现有模型在坐标一致性和视觉记忆维持上存在显著瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在静态视觉-空间推理上表现良好，但在具身场景中，模型需要根据自我中心观察持续修正空间信念，这要求长时间跨度的空间连贯性，而现有模型在此方面严重不足。本文旨在构建一个诊断基准，系统性地隔离和评估空间信念演化的核心机制。

Method: 构建SpaMEM基准：（1）基于物理引擎生成10,601,392张高保真图像（RGB、深度、实例、语义分割），来自1,000个程序化生成房屋中的25,000+交互序列；（2）设计三级层次化空间推理任务（共15个）：Level 1单视角原子空间感知、Level 2基于文本状态历史的时序推理、Level 3端到端原始视觉流信念维护；（3）分别评估短期（逐步）更新和长期（情节级）重建能力。

Result: 开源VLM系列模型在SpaMEM上表现出一致的堆叠瓶颈：坐标一致性接地是硬天花板；从Level 2到Level 3的显著性能下降暴露了明显的符号脚手架依赖——模型依赖文本式簿记取得成功，但在维持鲁棒的视觉记忆上极为困难。

Conclusion: SpaMEM为具身空间推理提供了细粒度的诊断标准，揭示了现有模型在状态表示、信念修正和长程情节集成上的根本缺陷，亟需引入显式机制来克服这些瓶颈。
...
TL;DR: 提出一种基于方差条件MLP的神经正则化方法，直接作用于3D高斯体来修正因2D特征多视图不一致导致的语义场噪声，实现准确鲁棒的3D语义高斯泼溅。


<details>
  <summary>Details</summary>
Motivation: 现有方法将视觉基础模型提取的多视图不一致2D特征直接提升到3D高斯体中，导致语义场存在噪声，影响下游任务性能。已有预处理或优化策略往往带来额外计算开销。

Method: 提出一种方差感知的条件MLP，直接作用于3D高斯体，利用其几何和外观属性来修正3D空间中的语义错误，无需额外的预处理或复杂优化。

Result: 在多个数据集上的实验表明，该方法提升了提升后语义的准确性，为鲁棒的3D语义高斯泼溅提供了高效有效的方案。

Conclusion: 直接对3D高斯体进行语义正则化是一种高效且有效的途径，能够在不增加预处理时间和高计算开销的前提下，显著提升语义场的质量。
...
TL;DR: SnapLog方法通过图像嵌入和时序分割，从视频中提取事件数据，使其可用于流程挖掘分析，实验证明能准确反映视频中的流程。


<details>
  <summary>Details</summary>
Motivation: 业务流程管理和流程挖掘领域面临数据多模态的障碍，特别是视频数据无法直接作为事件数据被解释和分析。

Method: 利用图像嵌入将视频帧转换为特征向量，通过帧间相似性矩阵进行时序分割，再采用广义少样本分类对视频片段进行标签分配，生成带有时间戳的事件序列。

Result: 该方法生成的日志能够准确反映视频中的流程。

Conclusion: SnapLog成功将非结构化的视频数据转化为结构化的事件日志，使得传统流程挖掘技术得以应用，为多模态流程分析提供了新途径。
...
TL;DR: 本文提出了一种基于对比示例的神经元标注方法，通过对比激活示例与非激活示例，利用VLM生成候选标签并用CLIP编码器进行评分，显著提升了标注的忠实度和语义粒度。


<details>
  <summary>Details</summary>
Motivation: 现有神经元标注方法依赖高激活示例，容易产生宽泛或误导性标签。FALCON虽引入对比示例但主要针对子空间解释性，缺乏可扩展的神经元级标注方法。本文旨在利用对比示例改进神经元级标签的准确性和特异性。

Method: 两阶段流程：(1) 候选标签生成——向VLM提供对比图像集（高激活+低激活示例），生成更具体、更忠实的候选标签；(2) 标签分配——提出Contrastive Semantic Projection (CSP)，扩展SemanticLens，将对比示例直接融入CLIP打分与选择流程。

Result: 在广泛实验和黑色素瘤检测案例研究中，对比标注方法在忠实度和语义粒度上均优于现有基线方法。

Conclusion: 对比示例是神经元标注与分析流程中简单而强大、但目前尚未充分利用的组件。
...
TL;DR: 本文提出一种个性化驾驶员疲劳检测系统，结合个性化EAR/MAR阈值与CNN深度学习模型，在多种条件下实现高精度实时监测，眼状态检测准确率达99.1%，哈欠检测达98.8%。


<details>
  <summary>Details</summary>
Motivation: 传统基于固定EAR/MAR阈值的驾驶员疲劳检测方法因个体面部结构、光照和驾驶条件差异而泛化能力差，亟需一种能够适应个体差异的鲁棒检测方案。

Method: 系统结合两种策略：(1) 个性化阈值校准——在驾驶前针对每个驾驶员校准专属EAR和MAR阈值；(2) CNN深度学习模型——用于眼状态和哈欠检测，以提升复杂场景下的精度。系统同时监测眼睑运动、头部位置和打哈欠行为。

Result: 个性化阈值相比固定阈值提升2-3%检测精度；CNN模型实现眼状态检测99.1%准确率、哈欠检测98.8%准确率。

Conclusion: 将经典度量方法与深度学习相结合，能够在真实驾驶场景中实现鲁棒、实时的个性化疲劳监测，有效提升道路安全。
...
TL;DR: Holo360D是一个包含109,495张全景图像的大规模数据集，提供连续全景序列、对齐点云/网格和相机位姿，专为推进全景3D重建研究而设计。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建模型在全景图上性能不佳（因球面畸变），且现有全景3D数据集多为离散位置采集，缺乏连续轨迹，严重制约了多视图全景3D重建的发展。

Method: 使用3D激光扫描仪+360相机采集原始数据，经在线和离线SLAM系统处理，再通过后处理管线（几何去噪、网格孔洞填充、区域特定重网格化）提升质量，最后在数据集上微调3D重建模型建立基准。

Result: Holo360D提供了优越的训练信号，建立了全面的基准，为全景3D重建模型的有效微调策略提供了关键见解。

Conclusion: Holo360D作为首个大规模连续全景序列数据集，能有效推动全景3D重建模型的发展，数据集和代码将公开。
...
TL;DR: 本文提出CGC框架，通过跨图像对比、图像内对比和基于规则的空间奖励机制，低成本提升多模态大模型的多图像细粒度理解能力，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在多图像细粒度理解上面临空间幻觉、注意力泄露和对象恒常性失败等挑战，且依赖昂贵的人工标注或大规模思维链数据生成。

Method: 基于已有单图像标注数据，通过跨图像对比和图像内对比构建组合式多图像训练实例，引入基于规则的空间奖励与GRPO框架，采用"先思考再定位"范式。

Result: 在MIG-Bench和VLM2-Bench上达到SOTA，并在MathVista、MuirBench、MMStar、MMMU、BLINK等基准上相比Qwen3-VL-8B模型有一致提升。

Conclusion: CGC是一种低成本、有效的多图像细粒度理解增强框架，且学到的能力可迁移到更广泛的多模态理解和推理任务中。
...
TL;DR: 本文介绍了ICPR 2026低分辨率车牌识别竞赛，基于LRLPR-26数据集（含20,000训练轨迹和3,000测试轨迹），共269支队伍参赛，最佳识别率达82.13%，并总结了前五名方法及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现实监控场景中，远距离拍摄、压缩伪影和恶劣成像条件导致车牌图像分辨率低，识别困难。现有研究缺乏专门针对真实低质量数据的竞赛，因此组织本次竞赛以推动该领域发展。

Method: 构建LRLPR-26数据集，每个训练轨迹包含5张低分辨率和5张高分辨率图像；设计竞赛流程（包括盲测阶段）；制定评估协议；收集并分析参赛队伍（特别是前五名）的方法。

Result: 269支队伍（来自41个国家）注册，99支提交有效结果；冠军识别率82.13%，4支队伍超过80%；表明任务难度大但顶尖水平竞争激烈。

Conclusion: 低分辨率车牌识别仍具挑战性；竞赛成功吸引了国际关注；总结了当前有效方法，并指出了未来研究方向。
...
TL;DR: RAIL-BENCH是首个专为铁路领域设计的感知基准套件，涵盖轨道检测、目标检测、植被分割、多目标跟踪和单目视觉里程计五项挑战，并提出了新的线段平均精度指标LineAP用于轨道检测评估。


<details>
  <summary>Details</summary>
Motivation: 铁路领域缺乏公开、标准化的基准测试套件和评估协议，难以实现不同方法之间的可重复比较，现有研究无法系统评估铁路环境下的感知算法性能。

Method: 构建了RAIL-BENCH基准套件，包含五个针对铁路环境定制的挑战任务、从真实场景中收集的标注训练/测试数据集、标准化评估指标以及公开排行榜；同时提出了LineAP指标，用于评估折线预测的几何精度。

Result: 成功建立了铁路领域首个感知基准套件，提供了标准化的评估平台和数据集，并提出了LineAP这一新的评估指标。

Conclusion: RAIL-BENCH填补了铁路领域感知基准的空白，通过标准化评估协议和多任务挑战，有望推动铁路自动化感知技术的可重复研究与对比。
...
TL;DR: 本文基于Muharaf历史阿拉伯语手稿数据集，手工扩展了作者标签（从28%增至86.75%），提出了一种带注意力机制的CNN模型用于封闭集作者识别，在行级协议下达到99.05% Top-1准确率，在页面分离协议下达到78.61% Top-1准确率，并量化了页面级线索的影响。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语手稿承载着重要的文化和历史价值，作者识别对于来源鉴定、真实性验证和历史分析至关重要。然而，现有工作缺乏对历史阿拉伯语手稿数据集在标准化评估协议下的基准结果。

Method: 手动扩展Muharaf数据集的作者标签（从6,858行扩展到21,249行），过滤非手写文本和噪声后保留18,987行；提出基于CNN（DenseNet201）加注意力机制的封闭集作者识别模型；将罕见的双作者行建模为复合作者-配对类别；在行级和页面分离两种协议下进行14种配置的基准测试和消融实验。

Result: 行级协议：微调DenseNet201加注意力机制达到99.05% Top-1、99.73% Top-5准确率和97.44% F1-score。页面分离协议：最佳结果为78.61% Top-1、87.79% Top-5准确率和66.55% F1-score，量化了页面级线索的贡献。

Conclusion: 通过扩展标签和建立两种评估协议下的基准，为历史阿拉伯语手稿的作者识别提供了更清晰的评估标准和实用资源，对历史学家和语言学家具有实际参考价值。
...
TL;DR: NONSAC是一个通用的鲁棒模型估计框架，通过重复采样非最小子集并利用鲁棒估计器生成多个候选模型，最终基于评分规则选择最优模型，可集成RANSAC等算法提升可扩展性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒估计方法（如RANSAC）在处理大规模噪声和离群点数据时，可扩展性和鲁棒性不足，需要一种更通用的框架来解决这一挑战。

Method: NONSAC框架重复采样非最小数据子集，使用鲁棒估计器生成多个候选模型，然后基于预定义的评分规则选择最终模型。该框架与估计器无关，可集成到RANSAC等现有几何拟合算法中。

Result: 在相对相机位姿估计、PnP和点云配准任务上评估了多种评分规则；展示了NONSAC在全对全对应假设下的无对应点云配准中的适用性。

Conclusion: NONSAC是一种估计器无关的通用框架，能够显著提升现有几何拟合算法在大规模噪声数据上的可扩展性和鲁棒性，并在多种视觉任务中展现有效性和通用性。
...
TL;DR: 本文提出一种非对称知识蒸馏框架，利用预训练视觉模型从失真图像中学习鲁棒表征，学生模型仅通过失真图像即可近似教师模型从干净图像中提取的表征，在多种失真场景的分类任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自监督学习在干净数据上表现优异，但现实场景中干净观测数据稀疏或不可获取，亟需一种能从失真/损坏数据中学习鲁棒视觉表征的方法。

Method: 提出非对称知识蒸馏框架：教师和学生均初始化为预训练Vision Transformer，教师处理干净图像，学生处理失真图像；引入多层级蒸馏（全局嵌入、patch级特征、注意力图对齐），使学生无需直接接触干净数据即可近似干净图像表征。

Result: 在多个数据集和多种失真类型（噪声、模糊、遮挡等）的图像分类任务中，所提方法一致优于同等监督水平下的现有替代方案。

Conclusion: 预训练视觉模型可通过非对称知识蒸馏有效迁移到失真数据场景，多层级对齐策略是关键；该方法为在缺乏干净数据时学习鲁棒表征提供了有效途径。
...
TL;DR: 本文基于1990-2020年间中英文期刊的45,732篇学术论文，提取23,928幅专题地图，分析其设计特征在三个维度（地图元素、色彩设计、布局结构）上的演变。研究发现中英文学术地图在结构上高度相似，均偏向中性、低饱和、高亮度配色和居中布局，但英文地图色彩丰富度略高，中文地图更依赖中性色和整合布局。时间趋势显示两者并行进化：元素丰富度、图例使用和色彩多样性增加，布局结构稳定。总体而言，学术地图设计演变呈现制度趋同而非文化分化。


<details>
  <summary>Details</summary>
Motivation: 专题地图在学术交流中至关重要，但大规模的设计演变缺乏实证研究。本文试图填补这一空白，系统考察中英文学术地图设计实践的演变规律。

Method: 汇编1990-2020年间16种中英文学术期刊的45,732篇文章，利用计算机视觉和大模型文档解析方法提取23,928幅地图，从地图元素、色彩设计、布局结构三个维度量化分析设计特征。

Result: 中英文学术地图结构高度相似：配色中性、低饱和、高明度、色彩种类有限；布局居中、主图占比高。差异在于英文地图色彩丰富度略高、布局更紧凑；中文地图更依赖中性色和整合布局。时间上两者均呈现元素丰富度、图例使用和色彩多样性增长的趋势，布局结构稳定。

Conclusion: 学术地图设计演变更多体现制度趋同而非文化分化，中英文期刊在地图设计规范上的共同规范主导了其演化方向。
...
TL;DR: 提出ReLIC-SGG框架，通过将未标注关系视为潜在变量而非明确负例、构建语义关系格来建模谓词之间的相似/蕴含/矛盾关系，以及引入正-无图学习目标，解决了开放词汇场景图生成中标注不完整导致的大量假阴性问题。


<details>
  <summary>Details</summary>
Motivation: 场景图标注天然不完整：许多有效关系缺失，同一交互可以不同粒度描述（如on/standing on/resting on/supported by）。开放词汇场景图生成的关系空间更大，这一问题更严重。现有方法将未标注关系一律视为负例，导致大量假阴性监督信号。

Method: 1) 构建语义关系格（semantic relation lattice），建模开放词汇谓词间的相似、蕴含和矛盾关系；2) 将未标注关系作为潜在变量，通过视觉-语言兼容性、图上下文和语义一致性推断缺失的正关系；3) 采用正-无图学习（positive-unlabeled graph learning）目标减少假阴性监督；4) 格引导解码生成紧凑且语义一致的场景图。

Result: 在常规、开放词汇和全景场景图生成基准上，ReLIC-SGG提升了罕见和未见谓词的识别能力，并更好地恢复了缺失的关系。

Conclusion: 通过显式建模关系标注的不完整性，将未标注关系视为不确定而非负例，能有效缓解开放词汇SGG中的假阴性问题，提升模型对稀有和未见关系的泛化能力。
...
TL;DR: 本文提出TriPatch，一种结合多阶段协同攻击与物理多样性鲁棒性增强的行人对抗补丁生成方法，通过三重损失（检测置信度抑制、边界框偏移放大、NMS破坏）和外观一致性损失，在多个检测器上实现更高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有物理对抗攻击存在两大局限：缺乏对多阶段检测管道的系统性破坏（残差模块可抵消扰动），以及未建模复杂物理变化导致鲁棒性差。

Method: 设计三重损失（检测置信度抑制+边界框偏移放大+NMS破坏）实现多阶段协同攻击；引入外观一致性损失约束补丁颜色分布，结合数据增强提升物理鲁棒性。

Result: 在多个检测器模型上，TriPatch相比现有方法取得了更高的攻击成功率。

Conclusion: TriPatch有效解决了现有物理对抗攻击缺乏多阶段系统性破坏和物理鲁棒性不足的问题，为行人检测安全提供了更强的攻击评估方法。
...
TL;DR: 本文提出语义进展函数（Semantic Progress Function），通过测量帧间语义嵌入距离来刻画生成序列中的非线性语义变化，并利用线性化重参数化方法使语义变化以恒定速率展开，从而改善生成过渡的平滑性与连贯性。


<details>
  <summary>Details</summary>
Motivation: 图像与视频生成模型产生的变换存在高度非线性：长时间内容几乎不变，随后突然出现语义跳跃。这种不均匀的语义节奏导致生成结果不自然，作者旨在分析和纠正这一行为。

Method: 1) 对于每一帧，计算帧间语义嵌入距离；2) 拟合一条平滑曲线，反映序列中累积的语义偏移；3) 通过将该曲线与直线的偏离程度检测不均匀语义节奏；4) 提出语义线性化过程，通过重参数化（重新定时间）使语义变化以恒定速率展开。

Result: 语义线性化能够产生更平滑、更连贯的过渡效果；该框架可用于识别时间不规则性、比较不同生成器的语义节奏、以及将生成或真实视频序列导向任意目标节奏。

Conclusion: 本文提供了一个与模型无关的框架，不仅能线性化语义节奏，还能作为分析生成模型时序行为的通用工具，具有广泛的应用前景。
...
TL;DR: 本文对自动驾驶中的图视觉问答（GVQA）进行了比较研究，提出了两种跨阶段上下文传递机制：显式变体（无需训练，通过提示策略将NLI矛盾降低42.6%）和隐式变体（门控上下文投影器+QLoRA，仅更新0.5%参数，将规划阶段NLI矛盾降低34%，CIDEr提升30.3%）。两种变体作为互补案例，显式提供强零训练基线，隐式实现规划阶段语义增益。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶GVQA中，规划决策需与模型自身感知保持一致，但现有方法缺乏有效的跨阶段上下文传递机制来保证这种一致性。

Method: 显式变体：三种提示条件策略（无需训练）在领域自适应4B VLM上评估；隐式变体：门控上下文投影器提取隐藏状态并注入下一阶段输入嵌入，联合QLoRA适配器在8B VLM上训练，仅更新~0.5%参数。

Result: 显式变体：NLI矛盾降低42.6%；隐式变体：规划阶段NLI矛盾降低34%（p<0.05），跨阶段蕴涵提升50%，CIDEr提升30.3%，但词汇重叠和结构一致性下降。

Conclusion: 两种变体互补：显式提供强零训练表面一致性基线，隐式实现显著规划阶段语义增益，领域适应是未来全频谱改进的关键。
...
TL;DR: FlowAnchor是一种无需训练的、基于流的视频编辑框架，通过空间注意力精炼和自适应幅度调制来稳定编辑信号，实现高效、时序一致的多目标视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有免反转视频编辑方法在多目标场景或帧数增加时效果不佳，原因是高维视频潜空间中编辑信号不稳定，表现为空间定位不精确和长度导致的幅度衰减。

Method: 提出FlowAnchor框架，包含两个核心组件：1) Spatial-aware Attention Refinement，在文本引导与空间区域之间实现一致对齐；2) Adaptive Magnitude Modulation，自适应保持足够的编辑强度。两者协同稳定编辑信号，引导基于流的演化过程。

Result: 实验表明，FlowAnchor在挑战性的多目标和快速运动场景下，实现了更忠实、时序一致且计算高效的视频编辑。

Conclusion: FlowAnchor通过锚定编辑位置和强度，有效解决了免反转视频编辑中编辑信号不稳定的问题，为基于流的视频编辑提供了高效、稳定的解决方案。
...
TL;DR: EV-CLIP提出了两种视觉提示（mask prompts和context prompts），用于提升CLIP在多样化场景和视角下的少样本视频动作识别能力，在五个基准数据集上优于现有参数高效方法，且效率与骨干网络规模无关。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP适配动作识别的方法主要关注时序建模，忽视了空间感知能力。在低光照、自我中心视角等现实场景中，空间理解是时序推理的前提，但现有方法对此关注不足。

Method: 提出EV-CLIP框架，包含两种视觉提示：1) mask prompts——通过重加权像素引导模型关注动作相关区域；2) context prompts——通过压缩帧级特征为紧凑表示，实现轻量级时序建模。

Result: 在五个基准数据集上，EV-CLIP在整体性能上优于现有参数高效方法。其效率不依赖于骨干网络规模，适合资源受限场景。

Conclusion: 通过视觉提示同时增强空间感知和时序建模，EV-CLIP在多样化场景下实现了高效且可部署的少样本动作识别。
...
TL;DR: 本文提出了一种基于3D点云数据的非侵入式个体猪只识别系统TARA，采用半监督学习框架和访问级投票策略，在商业养殖环境中实现了100%的识别准确率，优于传统RFID方法。


<details>
  <summary>Details</summary>
Motivation: 传统RFID耳标在群养环境中存在侵入性强、易丢失、空间受限等问题，迫切需要一种非侵入式、可靠的个体识别方法以实现精准畜牧管理。

Method: 提出TARA（时间自适应识别架构），一个半监督框架，利用3D点云数据，通过动态重校准机制更新个体身份模板以适应形态变化，并采用访问级多数投票策略生成高保真伪标签进行训练。

Result: 在商业养殖场的群养母猪数据集上，所提方法在访问级别实现了100%的个体识别准确率。

Conclusion: 基于视觉的3D点云分析是RFID系统的稳健且优越的替代方案，为全自动个体动物监测铺平了道路。
...
TL;DR: 提出PASR框架，通过从2D基础模型（DINOv3）中蒸馏知识到3D编码器，将单视图3D形状检索建模为特征层面的分析-合成问题，在检索、姿态估计和分类任务上均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有单视图3D形状检索方法（对比学习或共同嵌入空间）采用前馈式整体对齐，缺乏可解释性，限制了鲁棒性和泛化能力，尤其在真实场景中面对遮挡和细粒度细节时表现不佳。

Method: 提出PASR框架：(1) 将2D基础模型DINOv3的知识蒸馏到3D编码器；(2) 对齐姿态条件下的3D投影与2D特征图；(3) 推理时通过分析-合成进行测试时优化，联合搜索最佳形状和姿态以重建输入图像的patch级特征图。

Result: 在干净和遮挡的3D形状检索数据集上大幅超越现有方法，同时在单个框架内实现鲁棒的形状检索、有竞争力的姿态估计和准确的类别分类。

Conclusion: 基于分析-合成的特征级优化天然具有对部分遮挡的鲁棒性，且能捕捉细粒度几何细节，是一种可解释且多任务兼容的3D形状检索范式。
...
TL;DR: 提出SS3D，一种基于SfM的网络规模自监督预训练管线，用于从单目视频进行前馈3D估计，在YouTube-8M上预训练后展现出强大的跨域零样本迁移能力。


<details>
  <summary>Details</summary>
Motivation: 现有自监督3D估计方法多局限于受限数据集，无法利用网络级视频规模；网络视频存在弱多视角可观测性和强数据异质性，亟需可扩展的自监督预训练方案。

Method: 1) 联合预测深度、自运动和内参的单前馈模型；2) 先内参后两阶段训练调度；3) 多视角信号代理(MVS)用于过滤和课程采样；4) 专家训练蒸馏为单一学生模型；5) 统一的单检查点评估协议。

Result: 在YouTube-8M上预训练（过滤后约1亿帧），在跨域零样本迁移和微调性能上均优于先前自监督基线。

Conclusion: SS3D验证了SfM自监督预训练可扩展到网络级视频规模，并提供了可复现的预训练检查点和代码。
...
TL;DR: 本文提出一种4D扩散生成框架，通过学习拓扑保持的时空形变来建模和合成纵向大脑解剖结构变化，在神经退行性疾病进展预测中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有纵向神经影像数据集时间稀疏，限制了连续解剖变化建模的能力，而大多数现有方法只关注图像强度或纹理操作，无法捕捉几何结构变化。

Method: 提出4D (3D×T) 扩散生成框架，学习拓扑保持的时空形变数据分布，以临床变量（健康状况、年龄、性别等）为条件，生成未来解剖状态和重建疾病轨迹。

Result: 在两个大规模纵向神经影像数据集上，该方法在生成解剖准确、时间一致且有临床意义的大脑轨迹方面优于当前最优基线，并在纵向疾病分类和脑分割下游任务中验证了有效性。

Conclusion: 该方法能更真实地表示纵向大脑变化，有助于神经退行性疾病的早期诊断、监测和治疗规划。
...
TL;DR: 本文提出MegaDepth-X数据集和训练策略，通过从密集重建的地标中模拟稀疏采样，有效解决了互联网照片长尾分布（即大部分场景只有稀疏、不均匀图像）下的3D重建问题，微调后的3D基础模型在极端稀疏、对称重复场景中表现鲁棒，且不损失密集基准性能。


<details>
  <summary>Details</summary>
Motivation: 互联网照片呈现极端长尾分布——少数著名地标有密集图像可轻松重建3D，但大多数真实场景只有稀疏、有噪声且不均匀的图像，超出了经典方法和学习方法的处理能力。可靠的真实3D监督信号在稀疏场景中难以获取。

Method: 1. 构建MegaDepth-X数据集，包含从互联网地标中精心重建的干净密集深度3D数据。2. 提出一种训练图像采样策略，从密集重建的地标中有选择地采样稀疏子集，模拟长尾场景中的相机分布。3. 在这些模拟数据上微调3D基础模型。

Result: 微调后的3D基础模型在极端稀疏图像条件下能生成鲁棒的重建结果；在对称和重复场景中也能实现更可靠的重建；同时保持了在标准密集3D基准数据集上的泛化能力。

Conclusion: 通过模拟稀疏场景进行训练是解决长尾3D重建问题的有效途径，MegaDepth-X数据集和采样策略为该方向提供了重要支撑。处理长尾分布是3D基础模型未来的关键前沿之一。
...
TL;DR: 本文介绍了一个包含45组二元互动（90人）的多模态数据集，涵盖2D/3D面部视频、热谱、语音、生理信号（PPG、EDA、心率、血压、呼吸）及自我报告情感数据，包含有共同历史和无共同历史的两种互动类型，以及同意/不同意/中立等社交信号标注，总计20TB数据供研究社区使用。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏公开可用的多模态数据集来记录和标注多人社交互动中的行为与自我报告测量，尤其是二元互动（dyadic interaction）的录制和标注严重不足，阻碍了多模态社交互动建模的研究。

Method: 收集45组二元互动（90人）的多模态数据，包括2D面部视频、3D面部几何、热谱、语音、生理信号（PPG、EDA、心率、血压、呼吸）和自我报告情感。包含有共同历史（熟人）和无共同历史（陌生人）两种类型，并对社交信号（同意、不同意、中立）进行标注。

Result: 建立了20TB的多模态二元互动数据集，并通过大量实验评估了有/无社交历史背景的二元互动及其情感表达。

Conclusion: 该数据集将为多模态人际社交行为建模提供前所未有的支持，有望推动社交交互计算领域的研究进展。
...
TL;DR: 研究发现，印度YouTube上关于牛尿(gomutra)的健康论述中，宣传内容将神圣传统语言与伪科学声称巧妙融合，甚至与辟谣内容在修辞上难以区分。LLMs（GPT-4o、Gemini 2.5 Pro、DeepSeek-V3.1）由于主要受西方语料训练，系统性地无法对此类文化嵌入型错误信息进行可靠分析，且文化混淆延伸至性别化修辞与提示设计，仅靠提示工程无法弥补文化能力缺陷。


<details>
  <summary>Details</summary>
Motivation: 社交媒体已成为全球南方健康信息的主要渠道，但主流LLM训练语料以西方为中心，导致对非西方文化语境下的健康错误信息分析能力不足。本研究旨在揭示文化嵌入型错误信息的独特修辞特征及LLM对此类分析的局限性。

Method: 以印度YouTube上gomutra（牛尿）话语为案例，对30份多语言转录文本进行事后LLM辅助话语分析，对比三种LLM（GPT-4o、Gemini 2.5 Pro、DeepSeek-V3.1）在不同提示语调下的表现。

Result: 宣传内容将神圣传统语言与伪科学声称融合，与辟谣内容形成镜像修辞；LLM系统性地无法准确分析此类文化嵌入型错误信息；文化混淆延伸至性别化修辞与提示设计，导致分析不可靠性加剧。

Conclusion: LLM辅助话语分析中的文化能力无法通过提示工程事后弥补，需要更根本的解决方案。
...
TL;DR: 本文研究了大语言模型对提示敏感性的内部机制，发现不同提示风格（指令式和示例式）下模型会激活共享的"词汇任务头"，行为差异源于这些任务头被激活的程度不同，失败有时源于竞争性任务表征稀释了目标信号。


<details>
  <summary>Details</summary>
Motivation: 大语言模型对提示方式高度敏感，同一问题不同问法可能导致截然不同的回答，这种不可预测的行为给用户和开发者带来困扰。作者希望探究这种表面上的随机行为背后是否存在统一的内部机制。

Method: 比较两种主流提示风格——基于指令的自然语言描述提示和基于示例的少样本演示提示。通过分析模型内部注意力机制，识别出任务特定的"词汇任务头"，并研究其在不同提示风格下的激活模式。

Result: 发现不同提示风格下模型实际调用了共享的底层机制（词汇任务头）；行为差异与这些任务头的激活程度相关；任务失败部分原因是竞争性任务表征稀释了目标任务的信号。

Conclusion: 揭示了大语言模型内部表征与表面行为之间的关系，表明看似随机的提示敏感性背后存在可解释的内部机制，为理解LLM行为提供了更清晰的图景。
...
TL;DR: 本文研究视觉语言模型（VLMs）能否追踪信息来源于图像还是文本（源模态监控），发现语义信号比句法信号更重要，尤其在模态分布差异大时。


<details>
  <summary>Details</summary>
Motivation: 探究多模态模型是否具备"源模态监控"能力，即分辨信息来自图像还是文本输入，这是更广泛的"绑定问题"的一个实例。

Method: 在11种视觉语言模型上进行目标模态信息检索任务，通过操纵句法信号（如词语位置、语法结构）和语义信号（如语义一致性）来评估模型绑定信息源的能力。

Result: 句法和语义信号都对绑定有影响，但在模态分布差异大时，语义信号的主导作用超过句法信号。

Conclusion: 该发现对模型鲁棒性及多模态智能体系统的构建具有重要启示。
...
TL;DR: 本文提出一种轻量级患者-临床试验匹配框架，通过检索增强生成（RAG）从长病程EHR中提取关键片段，再利用大语言模型（LLM）编码这些片段进行降维和轻量预测，在多个基准上达到与端到端LLM相当的性能，且计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 现有患者-试验匹配方法中，全文档LLM处理计算开销大，传统ML方法又难以处理非结构化临床文本，亟需一种兼顾可扩展性、泛化能力和计算效率的解决方案。

Method: 提出两阶段轻量框架：1) RAG从长EHR中检索临床相关片段，降低输入复杂度；2) 用冻结或微调的LLM编码片段，经降维后由轻量分类器做最终预测。

Result: 在n2c2、SIGIR、TREC 2021/2022和真实多模态数据集MCPMD上验证，RAG选择显著降低计算负担且保留临床信号；冻结LLM对结构化数据有效，非结构化叙事需微调；整体性能媲美端到端LLM，计算成本更低。

Conclusion: 该方法证明了检索+轻量LLM编码在患者-试验匹配中的有效性，为实际临床部署提供了高效且可推广的范式。
...
TL;DR: 本文探索在神经符号语言中表示和推理视觉-语言概念，基于Qwen3-VL-2B-Instruct模型，在视觉-语言评估数据集上准确率提升3.33%，推理token减少75%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在分析推理能力上存在不足，且推理过程token消耗大。受电影《降临》中外星语言能改变思维方式的启发，作者希望探索用神经符号语言进行视觉-语言推理，以提升推理效率和能力。

Method: 以Qwen3-VL-2B-Instruct为基础模型，在4台Nvidia H200 GPU节点上进行训练和推理。采用神经符号语言（类似Wolfram语言风格）来表示和推理视觉-语言概念，替代传统的符号推理系统（如SymPy）。

Result: 在包含数学、科学和通用知识的视觉-语言评估数据集上，准确率提升3.33%；相比SymPy，推理token消耗减少75%。

Conclusion: 神经符号语言能有效提升视觉-语言模型的推理效率和分析推理能力，同时大幅减少推理token。作者还记录了计算挑战、扩展可能性及未来工作方向。
...
TL;DR: 本文聚焦精神科初诊中的对话式AI基础设施问题，构建了一个基于655道临床问题与合成患者案例的选择基准，比较了随机提问、临床固定表单和LLM引导的自适应策略在信息恢复上的表现。结果表明LLM自适应策略整体最优，尤其在患者回答不配合时优势明显，说明临床对话系统的表现不仅取决于语言理解能力，还取决于在有限交互次数内能否触及正确的话题。


<details>
  <summary>Details</summary>
Motivation: 精神科初诊是一个高风险、信息收集式的对话过程，临床医生需要在有限时间内决定提问什么、按什么顺序提问，并处理不完整或模糊的回答。尽管对话式AI在医疗领域受到关注，但在精神科初诊场景中仍缺乏基础设施和系统化的任务定义。因此作者希望通过标准化的基准测试来推动这一方向的研究。

Method: 将任务形式化为"问题选择问题"，基于655道临床医生编写的问题库和5种行为条件下的合成患者场景构建基准。比较三种策略：随机提问、临床固定表单（按临床顺序提问）、LLM引导的自适应策略（根据之前回答动态选择后续问题）。共进行300场模拟访谈，覆盖4名患者和5种行为条件。

Result: 临床固定表单显著优于随机提问；LLM引导的自适应策略整体信息恢复效果最佳。在回答不配合（如"guard-concise"条件）下，自适应策略的优势尤为明显。

Conclusion: 临床对话系统的性能不仅依赖于理解已披露信息的语言能力，还依赖于在有限交互次数内能否到达正确的话题。该基准为研究临床结构化提问与自适应追问如何协同提升交互式机器学习中的信息恢复提供了可控实验框架。
...
TL;DR: 本文发现基于可验证奖励的强化学习（RLVR）在思维链推理中虽然能提升任务准确率，但并不能保证推理过程对最终答案具有因果重要性和充分性；通过加入少量SFT预训练或辅助的因果/充分性奖励可以修复这一问题。


<details>
  <summary>Details</summary>
Motivation: 挑战现有常见假设：通过RLVR训练的推理链能可靠地反映模型得出答案的真实推理过程。作者希望通过量化指标检验这种假设是否成立。

Method: 提出两个新指标：CIR（推理的因果重要性，衡量推理token对最终答案的累积影响）和SR（推理的充分性，衡量仅凭推理是否能得到唯一答案）。在Qwen2.5模型系列和ReasoningGym任务上进行了实验。

Result: (1) RLVR提升了准确率，但并未可靠提升CIR或SR；(2) RLVR前加入少量SFT可改善CIR和SR；(3) 在结果奖励基础上加入辅助的CIR/SR奖励，可在保持准确率的同时提升推理的因果重要性和充分性。

Conclusion: RLVR并不总能如人们普遍认为的那样让模型真正依赖推理过程，但通过简单的后训练调整（如少量SFT或联合奖励）即可修复这一问题。
...
TL;DR: 本文提出了一个高效的检索增强生成（RAG）系统，专门用于乌克兰语文档问答，在UNLP 2026共享任务中获得第二名。系统采用自定义两阶段检索流程和微调后的乌克兰语专用语言模型，最终压缩模型实现轻量级部署，能在资源受限硬件上实现高质量可验证的AI问答。


<details>
  <summary>Details</summary>
Motivation: 针对乌克兰语文档问答任务，在严格计算资源限制下，实现高效、准确、可验证的RAG系统。

Method: 提出自定义两阶段搜索流水线（检索相关文档页）+ 基于合成数据微调的乌克兰语专用语言模型 + 模型压缩实现轻量级部署。

Result: 在UNLP 2026共享任务中获得第二名，在严格计算限制下实现高质量、可验证的问答性能。

Conclusion: 所提架构证明了在资源受限硬件上实现本地高质量、可验证的AI问答是可行的，且不牺牲准确性。
...
TL;DR: 本文提出KARITA框架，通过知识增强的检索与学习来应对时间推移带来的数据分布和语义变化，在临床、法律和科学等多个领域验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究要么忽略时间偏移，要么难以捕捉语义和知识领域丰富的演变模式，缺乏能够整合外部知识来适应多种时间变化的有效方法。

Method: 提出KARITA框架，包含知识驱动的数据增强与检索机制，整合如MeSH医学本体等丰富知识源，利用时间偏移洞察进行选择性检索增强学习，以应对不确定性、特征偏移等多种时间变化。

Result: 在临床、法律和科学语料等多个领域的分类任务上，KARITA在时间适应方面均取得了一致的性能提升。

Conclusion: 知识整合在时间增强和学习中比传统方法更为关键且有效，为处理时间漂移问题提供了新思路。
...
TL;DR: 混合架构中，注意力路径（即使占比较小）的LoRA微调优于全模型微调，且循环骨干在顺序混合中破坏性能、在并行混合中提升性能；并行混合具有正向跨任务迁移，顺序混合则遭受灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA方法在混合架构（注意力+循环组件）中均匀应用适配器，未考虑不同组件的功能角色差异，缺乏对组件特定LoRA放置的系统研究。

Method: 在两种混合架构（Qwen3.5-0.8B顺序混合、Falcon-H1-0.5B并行混合）上，对三个领域进行微调并在五个基准上评估，系统研究组件类型的LoRA放置策略。

Result: 注意力路径以5-10倍更少的可训练参数超越全模型适应；循环骨干在顺序混合中导致-14.8 pp下降，在并行混合中带来+8.6 pp提升；并行混合有正跨任务迁移，顺序混合有灾难性遗忘。

Conclusion: 混合架构的拓扑结构从根本上决定适应响应，组件感知的LoRA放置是混合架构必要的设计维度。
...
TL;DR: 这篇论文研究了Transformer模型在处理层级结构任务时，残差流中的层级几何表示和注意力模式中的栈式表示是否被因果使用，还是仅仅可解码。通过在Dyck语言（平衡括号序列）和模板化自然语言上的探测和干预实验，发现深度、距离和栈顶信号都可解码，但因果作用不同：遮挡栈顶注意力会显著降低长距离准确性，而消融残差流低维子空间影响较小。结论是，可解码性不等于因果使用。


<details>
  <summary>Details</summary>
Motivation: 先前研究已发现Transformer在层级结构任务中会以不同方式表示层级信息（残差流几何结构和栈式注意力模式），但不清楚这些表示是被因果使用还是仅仅可解码。

Method: 在Dyck语言（平衡括号序列）上训练Transformer，通过探测（probing）和解耦干预（intervention）方法，分别分析残差流和注意力模式中的层级信息，并比较其因果作用。

Result: 深度、距离和栈顶信号都可解码，但因果作用不同。遮挡注意力到真实栈顶位置会导致长距离准确性显著下降，而消融残差流中的低维子空间影响较小。

Conclusion: 即使在已知相关层级变量的受控环境下，可解码性并不意味因果使用。
...
TL;DR: 本文提出了一种针对教育场景中大语言模型的“教学越狱”漏洞，并设计了SHAPE基准测试（9,087个学生-问题对）以及一种基于知识图谱增强的教学流水线，通过显式门控机制在指令生成和问题求解之间路由，显著提升了模型在对抗性压力下的安全性，同时保持了高帮助性。


<details>
  <summary>Details</summary>
Motivation: 当前教育领域的大语言模型存在一个关键漏洞：学生可以通过诱导性提示（即“教学越狱”）让模型直接给出答案，而非提供逐步引导的教学内容。作者希望系统性地研究和防御这一安全威胁。

Method: 1) 统一并形式化了安全、有帮助和教学行为，使用知识掌握图（knowledge-mastery graph）进行建模。2) 构建SHAPE基准测试（9,087个学生-问题对），用于评估模型在对抗性压力下的教学行为。3) 提出图增强教学流水线：从查询中推断先决概念、识别掌握缺口，并通过显式门控机制在“教学指导”和“问题求解”之间进行路由生成。

Result: 在多个大语言模型上的实验表明，该方法在两种教学越狱设置下显著提升了安全性，同时在同一评估协议下保持了接近上限的帮助性。

Conclusion: 知识图谱增强的显式门控机制能有效防御教育场景中的“教学越狱”攻击，在不牺牲帮助性的前提下大幅提升安全性，为构建安全可靠的教育LLM提供了可行方案。
...
TL;DR: 本研究分析300篇由三个前沿LLM在三种提示条件下重写的个人叙事，发现LLM重写会导致风格标准化：功能词、缩写和第一人称代词减少，词汇多样性、词长和标点复杂度增加。即使使用保留声音的提示也无法消除这种趋势，表明LLM重写是一种方向性的文本调节，而非表面编辑。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型重写如何改变个人叙事的风格和叙事质感，揭示LLM重写不仅仅是表面编辑，而是一种具有方向性影响的文本调节形式。

Method: 分析300篇由三个前沿LLM在三种提示条件（通用改进、仅重写、保留声音修订）下重写的个人叙事。使用13个计算文体学标记（功能词、词汇多样性、词长、标点、缩写、第一人称代词、情感词等）测量变化。

Result: LLM重写产生一致的模式：功能词、缩写、第一人称代词减少；词汇多样性、词长、标点复杂度增加。保留声音的提示能降低变化幅度但无法消除方向。重写文本在特征空间中趋同，难以匹配回源文本。叙事标记显示从嵌入式叙述转向疏离式叙述，从显式因果推理转向压缩抽象。

Conclusion: 当代LLM对文本产生一种方向性拉力，使其趋向更 polished、更少情境化的语域。这对数字人文和计算文本分析有重要影响，因为功能词、代词、缩写、标点等常被用作风格、声音、作者身份和语料库完整性的证据。
...
TL;DR: 三大前沿AI系统（Claude Sonnet 4.5、GPT-5.4、Gemini 2.5 Flash）在面对来自10个国家用户的个人困境咨询时，始终给出西方式、个人主义导向的建议，与当地实际价值观存在显著偏差（平均差距+0.76），仅有日本被AI赋予了过时的集体主义刻板印象。这揭示了AI系统存在系统性价值观同质化问题。


<details>
  <summary>Details</summary>
Motivation: 探究AI助手在面对不同国家背景用户时，是否会给出差异化、符合当地价值观的建议，还是表现出统一的西方中心主义偏见。

Method: 向三个AI系统（Claude、GPT-5.4、Gemini）呈现10个真实生活困境，分别从10个国家、7种语言的用户视角提问，共收集840份评分回应，并与World Values Survey Wave 7的实际国民价值观数据进行对比。

Result: 所有AI系统均显著偏向西方个人主义价值观（平均差距+0.76，t=15.65，p<0.001），尼日利亚（+1.85）和印度（+0.82）差距最大。日本是唯一例外，AI对日本用户的集体主义预期反而超过了实际数据。Claude和GPT-5.4偏误幅度相近，Gemini较低但仍显著。三个模型机制不同：Claude在用户母语下更集体主义，Gemini更个人主义，GPT-5.4仅对stated country identity做出反应。

Conclusion: 前沿AI系统存在系统性价值观同质化，忽视文化多样性，编码了西方中心主义偏见（日本案例则体现了过时的刻板印象）。这种现象可能导致AI建议在全球范围内推广单一文化价值观。
...
TL;DR: 本文运用因果可解释性方法（激活修补）研究语言模型是否在不同句法构式中共享神经机制。发现填充词-空位依赖存在局部化共享机制（早中层），而否定极性项无统一机制。激活修补方法具有分布外泛化能力，而分布式对齐搜索易过拟合。通过操控识别组件可提升模型在可接受性判断基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型的内部机制在多大程度上与语言学中的跨构式原则对齐，即模型是否在不同句法构式中使用共享的神经机制。

Method: 采用因果可解释性方法（激活修补）在细粒度层面识别特定注意力头和MLP块的功能角色，聚焦于填充词-空位依赖和否定极性项许可两种句法构式。

Result: (1) 填充词-空位依赖存在高度局部化且共享的机制，位于早中层；而NPI处理无统一机制。(2) 激活修补识别的机制能泛化到分布外数据，而分布式对齐搜索易在狭窄语言分布上过拟合。(3) 操控识别出的组件可提升模型在可接受性判断基准上的表现。

Conclusion: 语言模型在不同句法构式上的内部机制对齐程度不同，因果可解释性方法比监督式方法更鲁棒，且识别的机制具有行为层面的可验证性。
...
TL;DR: 本文提出三源交互框架（参数知识、文档、用户），系统评估27个LLM发现模型更依赖文档而非用户，且后训练强化此偏好；多数模型无法区分有益/有害外部信息，而针对性微调可提升辨别能力。


<details>
  <summary>Details</summary>
Motivation: 现有知识冲突和谄媚研究局限于二元冲突范式（参数知识 vs 文档 或 参数知识 vs 用户），忽视了实际交互场景中三者同时存在的情况，缺乏对三源信息交互的综合研究。

Method: 构建三源交互框架，在2个数据集上系统评估来自3个家族的27个LLM，通过对比模型对不同来源信息（参数知识、文档断言、用户断言）的依赖程度，并进行行为分析和微调实验。

Result: 1) 多数模型更依赖文档断言而非用户断言；2) 后训练（如RLHF）强化了这一偏好；3) 多数模型无法有效区分有益和有害的外部信息；4) 对多样化源交互数据进行微调可显著提升模型的辨别能力。

Conclusion: 为开发能够可靠整合多源信息的可信LLM铺平了道路，揭示了模型在交互环境中处理外部信息的偏好模式及其改进方向。
...
TL;DR: 对7个开源指令微调LLM（3-9B参数）的言语置信度进行心理测量学筛查，发现所有模型在数值型置信度报告上均无效（平均天花板率91.7%），类别型格式亦无法挽救，且token级log概率无法有效预测置信度。研究还发现了推理蒸馏模型中的"推理污染效应"。


<details>
  <summary>Details</summary>
Motivation: 验证当前广泛使用的言语置信度提取方法是否能让LLM产生具有基本心理测量学效度的条目级Type-2判别能力，即在最小数值化提示和贪婪解码条件下，模型口头报告的置信度能否反映其内部不确定性。

Method: 预注册研究，对8个模型（7个指令微调+1个基准）在TriviaQA的524个题目上，分别采用数值型（0-100）和类别型（10类）两种置信度格式，在消费级硬件上以Q5_K_M量化运行，共8,384次确定性试验。对每个模型-格式组合应用心理测量学效度筛查。

Result: 所有7个指令微调模型在数值型置信度上均被判定为无效（平均天花板率91.7%）；类别型格式破坏了6/7模型的任务表现（准确率低于5%）；token级log概率跨验证R²<0.01；推理蒸馏模型中，推理链长度与置信度呈显著负相关（ρ=-0.36）。

Conclusion: 在3-9B参数规模下，最小化的言语置信度提取无法在输出接口保留下内部不确定性信号。任何下游使用前必须进行心理测量学筛查。结果不意味着模型缺乏内部不确定性表征，而是口头提取方法在此规模下失效。
...
TL;DR: TTS-PRISM是一个面向中文的多维诊断框架，通过12维评估体系、对抗性扰动数据集和指令微调模型，实现对生成式语音合成系统细粒度声学伪影的诊断，在1600样本测试集上优于通用评估模型。


<details>
  <summary>Details</summary>
Motivation: 现有生成式TTS模型虽接近人类水平，但传统单一评估指标无法诊断细粒度声学伪影或解释感知塌陷问题，亟需多维度诊断框架。

Method: 1) 建立12维评估体系（从稳定性到高级表现力）；2) 设计针对性合成流程，包含对抗性扰动和专家锚点，构建高质量诊断数据集；3) 通过模式驱动指令微调，将评分标准和推理嵌入高效的端到端模型。

Result: 在1600样本黄金测试集上，TTS-PRISM在人类对齐度上优于通用评估模型；通过对六种TTS范式的分析，建立了直观的诊断标记，揭示了细粒度能力差异。

Conclusion: TTS-PRISM作为开源的多维诊断框架，能够有效替代传统单一指标，为中文TTS系统提供细粒度、可解释的质量评估。
...
TL;DR: 本文提出一个基于可解释AI的分层归因方法，让微调后的LLM在教育对话系统中不仅能推荐问题行为干预策略，还能给出自然语言解释，提升了教师的信任度。


<details>
  <summary>Details</summary>
Motivation: 现有微调LLM虽能通过多轮对话支持教师诊断学生问题行为，但很少解释为何推荐某个策略，导致缺乏透明度和教师信任。

Method: 构建可解释对话系统，基于微调LLM，采用分层归因方法（基于xAI）识别对话中的证据，并据此生成自然语言解释。

Result: 技术评估中，该方法在识别支持证据上优于基线方法；在22名职前教师的初步用户研究中，收到解释的参与者对系统信任度更高。

Conclusion: 该工作为提升教育对话系统中LLM的可解释性提供了一个有前景的方向。
...
TL;DR: 本文对11个前沿大语言模型在15种概率分布上的原生采样能力进行了大规模统计审计，发现批量生成模式下仅有7%的中位通过率，而独立请求模式中10/11的模型全部失败。采样保真度随分布复杂度和采样规模增大而下降，且这种失败会向下游应用（如选择题生成、文本到图像提示合成）引入系统性偏差。结论是当前LLM缺乏功能性的内部采样器。


<details>
  <summary>Details</summary>
Motivation: 随着LLM从聊天界面演变为随机流水线和接近通用智能系统的核心组件，从指定概率分布中忠实地采样的能力已从理论好奇心变为功能性需求。然而，此前缺乏对前沿LLM原生概率采样能力的大规模、统计有力的审计。

Method: 采用双协议设计：批量生成（一次响应内生成N=1000个样本）和独立请求（N=1000次无状态调用），对11个模型在15种分布上进行基准测试。通过协议不对称性分析、分布复杂度与采样保真度的关系、以及下游任务中的偏差传播来系统评估采样失败模式。

Result: 批量生成中位通过率仅7%；独立请求中10/11模型在所有分布上均失败。采样保真度随分布复杂度单调下降，且随采样规模N增大而恶化。下游任务中，模型无法在多项选择题生成中强制执行均匀答案位置约束，并在属性约束文本到图像提示合成中系统性地违反人口统计目标。

Conclusion: 当前前沿LLM缺乏功能性的内部采样器，无法可靠地从指定概率分布中采样，因此需要外部工具来支持需要统计保证的应用。
...
TL;DR: 本文提出了MuDABench基准，用于跨多文档的定量分析问答，包含8万+页和332个问答实例，并设计了一套评估协议。现有RAG系统表现不佳，多智能体工作流虽有改进，但与人类专家仍有差距，主要瓶颈在于单文档信息提取精度和领域知识不足。


<details>
  <summary>Details</summary>
Motivation: 现有多文档QA基准通常只涉及少量文档且跨文档推理有限，无法满足大规模半结构化文档集合中需要进行定量分析的需求。

Method: 通过远程监督（利用文档级元数据和标注的金融数据库）构建MuDABench基准；提出包含最终答案准确率和中间事实覆盖率的评估协议；设计多智能体工作流（规划、提取、代码生成模块）来提升性能。

Result: 标准RAG系统在MuDABench上表现较差；多智能体工作流在过程和结果指标上均有显著提升，但与人类专家仍存在明显差距。

Conclusion: 当前系统的两大瓶颈是单文档信息提取精度不足和领域知识缺乏，未来的研究方向应聚焦于提升细粒度信息提取能力和注入专业领域知识。
...
TL;DR: 本文提出RC-RAG框架，通过多阶段引入关系改写（paraphrase）来增强大语言模型在关系补全任务上的表现，无需微调，在长尾场景下显著优于现有RAG基线。
...
Method: 提出RC-RAG多阶段改写引导框架：(a) 在检索阶段引入关系改写以扩展词汇覆盖；(b) 利用改写生成关系感知摘要；(c) 在生成阶段利用改写引导推理。无需模型微调。

Result: 在5个LLM、2个基准数据集上，RC-RAG持续优于多个RAG基线。长尾场景下，最佳LLM结合RC-RAG相比自身提升了40.6个EM点，且超过两个RAG基线16.0和13.8个EM点，计算开销低。

Conclusion: 提出的RC-RAG通过多阶段集成关系改写，有效提升了LLM在关系补全上的能力，特别是在长尾知识场景下，且无需微调、计算高效。
...
TL;DR: 本文首次系统分析了AI agent在编码任务中的token消耗模式，发现agent任务消耗token量是代码推理/聊天任务的1000倍，且输入token是主要成本驱动因素；token使用高度随机（同一任务差异可达30倍），更高token消耗不意味着更高准确率；不同模型token效率差异巨大（Kimi-K2和Claude-Sonnet-4.5比GPT-5多消耗150万+token）；人类感知的任务复杂度与模型实际计算开销存在根本性差距；前沿模型无法准确预测自身token消耗（相关系数最高仅0.39），且系统性地低估真实成本。


<details>
  <summary>Details</summary>
Motivation: 随着AI agent在复杂人类工作流中广泛采用，LLM token消耗快速增长。当agent被部署在需要大量token的任务上时，自然产生三个问题：(1) AI agent把token花在哪？(2) 哪些模型更token高效？(3) agent能否在任务执行前预测自己的token使用量？然而此前缺乏对这一方向的系统研究。

Method: 在SWE-bench Verified基准上，分析8个前沿LLM（包括GPT-5、Claude-Sonnet-4.5、Kimi-K2等）的agent编码任务轨迹，并从多维度统计token消耗模式；同时评估模型在任务执行前预测自身token成本的能力。

Result: (1) agent任务token消耗是代码推理/聊天的1000倍，输入token驱动主要成本；(2) token使用高度随机（同任务差异达30倍），准确率在中间成本时达到峰值，高成本时饱和；(3) Kimi-K2和Claude-Sonnet-4.5平均比GPT-5多消耗150万+token；(4) 人类专家评定的任务难度与token成本相关性弱；(5) 前沿模型无法准确预测自身token使用（相关系数最高0.39），系统性地低估真实成本。

Conclusion: 本文为AI agent的经济学提供了新见解，揭示了token消耗的随机性、模型间的巨大效率差异，以及人类复杂度感知与计算开销之间的根本性鸿沟，为未来agent经济性相关研究提供了方向。
...
TL;DR: LLM在生成链式思维推理时，最终答案通常在早期就已固定，后续大量推理token（每查询平均760个）属于冗余的后决策解释。通过早期停止策略（如探针式停止），每查询可节省500个推理token，准确率仅下降2%。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在生成链式思维推理过程中，最终答案是在哪个阶段被确定的。如果答案在中间阶段就已固定，后续推理token只是后决策解释，会浪费计算资源并增加延迟。

Method: 使用强制答案补全方法（forced answer completion），在部分推理前缀下提取模型的中间预测。基于Qwen3-4B模型，跨多个数据集分析答案演进过程，并测试多种早期停止策略（包括基于探针的停止）。

Result: 仅32%的查询中预测答案会发生改变；最终答案切换后，模型平均每查询额外生成760个推理token；早期停止策略可每查询减少500个推理token，准确率仅下降2%。

Conclusion: 链式思维生成中的大部分推理是冗余的，可以通过早期停止以最小性能代价大幅降低推理成本。
...
TL;DR: 提出了STEM框架，通过语义到结构的投影和全局引导子图来解决KGQA中的语义不匹配和缺乏全局结构视角的问题，在多跳推理任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: KGQA面临两个挑战：知识图谱的结构异质性导致检索时语义不匹配，以及现有推理路径检索方法缺乏全局结构视角。

Method: 提出STEM框架：1) Semantic-to-Structural Projection将查询分解为原子关系断言并构建自适应查询模式图；2) 全局感知的节点锚定和子图检索；3) Triple-Dependent GNN生成全局引导子图来指导图构建过程。

Result: 在多跳推理基准上取得了最先进的性能，显著提高了多跳推理图检索的准确性和证据完整性。

Conclusion: STEM通过将多跳推理重构为模式引导的图搜索任务，有效解决了语义不匹配和缺乏全局结构视角的问题，验证了引入全局结构信息对提升KGQA性能的有效性。
...
TL;DR: 本文提出ReLeVAnT框架，利用n-gram处理、对比分数匹配和浅层神经网络，对法律文档进行二分类，在LexGLUE数据集上达到99.3%准确率和98.7% F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有法律文档分类方法依赖结构化元数据、LLM提取的元数据或多模态方法，需要大量计算资源。作者希望利用文档类别间的判别性特征，提出一种更高效、轻量的分类方案。

Method: ReLeVAnT框架包括：1) n-gram处理提取特征；2) 对比分数匹配（contrastive score matching）增强判别性；3) 浅层神经网络作为主分类器。每个语料库只需一次关键词提取。

Result: 在LexGLUE数据集上达到99.3%准确率和98.7% F1分数，表明该方法在保持高精度的同时效率优异。

Conclusion: ReLeVAnT证明了利用判别性特征配合浅层模型，可以在法律文档分类任务中以较低计算成本达到与深度方法相当甚至更优的性能。
...
TL;DR: SLIDERS是一个通过结构化推理（将信息提取到关系数据库，用SQL替代文本拼接）来解决长文档集合问答中聚合瓶颈问题的框架，在多个基准测试上显著超越GPT-4.1等基线模型。


<details>
  <summary>Details</summary>
Motivation: 现实中的文档问答需要跨多个文档及文档的不同部分综合证据，但固定LLM上下文窗口无法应对文档集合不断增长的情况。现有分块方法面临聚合瓶颈——随着分块数量增加，系统仍需对大量提取的证据进行组合和推理。

Method: SLIDERS框架将文档中的关键信息提取到关系数据库中，通过SQL而非拼接文本实现可扩展的持久化结构化状态推理。同时引入数据协调阶段，利用来源信息、提取理据和元数据来检测并修复重复、不一致和不完整的记录。

Result: 在三个现有长上下文基准上全面超越所有基线（平均超GPT-4.1达6.6分）；在两个新基准上（3.9M和36M token）分别超越次优基线约19分和32分。

Conclusion: SLIDERS通过结构化数据库存储和SQL推理，有效解决了长文档集合问答中的上下文窗口限制和聚合瓶颈问题，验证了结构化推理相比纯文本方法在长文档场景下的显著优势。
...
TL;DR: 本文提出了Clarity框架，用于自动生成包含多面模糊性的NL2SQL基准测试，实验表明现有系统在检测模糊性时表现尚可，但难以准确定位和解决模式层面的模糊根源。


<details>
  <summary>Details</summary>
Motivation: 现有NL2SQL基准测试通常假设单一模糊来源且依赖用户交互，忽略了工业场景中真实存在的多面模糊性和不完整用户澄清等失败模式。

Method: 提出了Clarity框架，采用约束驱动流水线，将可执行的SQL转换为带有模糊性的查询，并增强对话延续和模式级元数据，覆盖单轮和多轮设置。

Result: 在Spider和BIRD上的实验表明，包括基于强LLM的系统在内的主流NL2SQL系统在多面模糊性下性能显著下降；系统虽然能检测到模糊性，但难以准确定位和解决模式层面的模糊来源。

Conclusion: 工业级NL2SQL系统需要更强健的模糊性检测与解决能力。
...
TL;DR: 本文提出一个框架，允许领域专家仅通过提供实体名称和标签，自动利用网络和大型语言模型获取描述文本，从而快速构建针对特定任务的实体分类器。在SIC代码和医疗保健提供者分类任务上分别达到82.3%和72.9%的宏平均F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有的NLP资源缺乏任务特定信息，对新兴或小众实体的覆盖有限，难以满足实际应用中的分类需求（如企业/医疗提供者按不同分类体系归类）。

Method: 提出一种结合网络搜索和大型语言模型（LLMs）的文本采集方法，动态获取每个实体的描述文本，并基于这些文本训练文本分类器。用户只需提供实体名称和标注标签即可。

Result: 在SIC代码分类任务上宏平均F1为82.3%，在医疗保健提供者分类任务上宏平均F1为72.9%。

Conclusion: 所提框架能够有效利用网络和LLM获取的文本信息，在低资源条件下为领域专家构建高质量的任务特定实体分类器。
...
TL;DR: 本文提出Context-Fidelity Boosting (CFB)，一种轻量级解码时框架，通过对源支持token施加对数概率调整，减少LLM的事实性幻觉，在摘要和QA任务上有效提升忠实度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型常生成与输入上下文矛盾或忽略上下文信息的内容（忠实性幻觉），现有方法成本高或需重训练，亟需一种轻量、通用的解决方案。

Method: 受水印技术中logit调整启发，提出三种增强策略：静态增强（固定偏置）、上下文感知增强（基于有无上下文的分歧度动态缩放偏置）、token感知增强（结合源位置注意力和语义相似度进一步调整偏置），在解码时对源支持token进行加性logit调整。

Result: 在多个开源LLM的摘要和问答任务上，CFB一致提升了忠实度指标，且生成开销极小。

Conclusion: CFB无需重训练或架构更改，是一种通用、轻量且有效的减少事实性幻觉的框架。
...
TL;DR: 本文提出DPS框架，通过识别注意力头中的"偏好头"并对比有无偏好头的logits差异，实现无训练、可解释的LLM个性化生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM个性化方法多为黑盒（如prompt工程或微调），缺乏可解释性和可控性。本文从机制可解释性出发，试图揭示个性化在Transformer架构中的产生机制。

Method: (1) 通过因果掩码分析识别"偏好头"（Preference Heads）; (2) 计算偏好贡献分数（PCS）衡量每个注意力头对用户对齐输出的因果影响; (3) 解码时对比有无偏好头的预测，放大个性化logits与通用logits的差异。

Result: 在多个LLM和个性化基准上，DPS在保持内容连贯性和低计算开销的同时，持续提升个性化保真度。

Conclusion: DPS不仅实现了有效的无训练个性化，还为Transformer中个性化能力的产生位置和方式提供了机制层面的解释。
...
TL;DR: 本文提出 CNSL-bench，首个用于评估多模态大语言模型（MLLMs）手语理解能力的中国国家手语基准，包含权威词典支撑、多模态对齐数据和丰富手语形式，实验表明当前 MLLMs 在手语理解上远逊于人类表现。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在手语理解方面的内在能力，尤其是在多模态场景下，尚未被充分探索，缺乏系统性的评估基准。

Method: 构建 CNSL-bench 基准，基于《国家通用手语词典》确保权威性，包含文本描述、图片和手语视频的多模态对齐数据，支持空中书写、指拼、手指字母等多种手语形式的细粒度分析。

Result: 在 21 个开源和商业 MLLMs 上评估，发现当前模型在手语理解上显著低于人类水平，不同输入模态和手语形式存在系统性差异。

Conclusion: 当前 MLLMs 手语理解能力严重不足，仅提升推理能力难以弥补差距，且不同模型的指令遵循鲁棒性差异显著。
...
TL;DR: 针对无词汇表（gloss-free）手语翻译中随机批次负样本对比学习存在的噪声和不一致问题，本文提出基于负样本相似度动态分析的成对选择策略（PS），通过参考检查点对候选负样本进行评分，并以课程学习方式构建包含更具挑战性负样本的小批次，从而提升对比监督的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有SLT系统采用CLIP式视觉-语言预训练进行跨模态对齐，但随机批次对比学习存在两个问题：1）负样本数量有限且依赖批次；2）语义相似甚至相同的样本被错误标记为负样本，引入噪声和不一致的监督信号。

Method: 1）通过轨迹分析追踪训练中负样本的视频-文本相似度动态，发现仅少量负样本被持续推远，多数负样本的相似度呈非下降的异质动态。2）提出SCL-SLT，包含成对选择（PS）策略：利用参考检查点的相似度动态对候选负样本评分；通过课程学习逐步强调更有挑战性的负样本，构建小批次进行对比学习。

Result: 轨迹分析表明随机批次负样本多为非信息性样本，无法有效促进对齐。PS策略能减少噪声和语义无效负样本的影响，强化对比监督。

Conclusion: SCL-SLT通过选择信息性负样本并采用课程学习，有效提升了无词汇表手语翻译中跨模态对比学习的质量。
...
TL;DR: 本文通过三个大规模实验（作家N=2,939，读者N=11,091）发现，AI写作辅助会系统性地扭曲作家的人设感知——让作家显得更自信、更有能力、更积极，且人口统计学特征向特权群体偏移。尽管作家反感这种扭曲，但仍偏好AI辅助文本。通过实验数据训练奖励模型可在一定程度上缓解扭曲，但会降低用户接受度。


<details>
  <summary>Details</summary>
Motivation: 虽然已有数百万人使用AI写作辅助，但AI如何影响他人对作者人设（信念、个性、身份）的感知尚不清楚。作者希望系统性量化AI辅助写作对读者感知的扭曲程度及其社会影响。

Method: 三阶段大规模实验：(1) 2,939名作家撰写政治观点段落（有/无AI辅助）；(2) 11,091名读者在29个社会感知维度上盲评这些段落；(3) 基于10,008段落、2,903,596评分的实验数据训练奖励模型，试图在模型层面纠正人设扭曲。

Result: AI辅助在所有29个维度上均产生人设扭曲：作者显得更武断、更胜任、更积极，且感知人口特征向特权群体偏移。作家反感多数扭曲但仍偏好AI辅助文本。奖励模型可缓解扭曲但降低用户接受度。

Conclusion: AI写作辅助导致的人设扭曲普遍且持续存在，即使在现实的人类监督条件下也难以消除。这给公共话语、信任和民主协商带来重要影响，且问题会随AI普及而加剧。理想与不良属性之间的纠缠使得问题难以简单解决。
...
TL;DR: 本文研究了评估LLM生成的商业创意时，由于专家评分存在分歧，自动评估器应该模拟集体共识还是个性化建模个体评估者。通过引入PBIG-DATA数据集（3000个专家评分），发现专家在细粒度评分上分歧显著，而个性化裁判（基于目标评估者历史）比聚合裁判更接近对应评估者，表明在多元化评估场景中，聚合标签是脆弱的评估目标。


<details>
  <summary>Details</summary>
Motivation: 评估LLM生成的商业创意比生成更难规模化，因为其依赖多维标准且专家判断常存在分歧。由此引出的方法论问题：自动评估器应逼近集体共识，还是对个体评估者进行个性化建模？

Method: 构建PBIG-DATA数据集（约3000个评分，覆盖300个专利产品创意，6个商业维度），分析专家分歧模式；比较三种裁判配置：仅基于规则的零样本裁判、基于混合评估历史的聚合裁判、基于目标评估者历史记录的个性化裁判。

Result: 专家在细粒度序数评分上分歧显著，但在粗粒度选择上一致性更高；个性化裁判在所有维度和模型规模下均比聚合裁判更贴近对应评估者；评估者一致性仅在个性化条件下与裁判生成的推理相似度相关。

Conclusion: 聚合标签在多元化评估场景中是脆弱的评估目标，应设计基于评估者条件的裁判模型来评估商业创意。
...
TL;DR: 论文提出RouteLMT，一种通过预测大模型相对于小模型的边际增益来优化机器翻译中混合系统路由决策的方法，在质量-预算帕累托前沿上取得了更优表现。


<details>
  <summary>Details</summary>
Motivation: LLM在机器翻译中表现优异但部署成本高昂。混合系统通过小模型处理多数请求、大模型处理部分请求来平衡成本和质量。然而现有路由策略（基于启发式、外部预测器或绝对质量估计）未能捕捉大模型是否真正提供了值得其成本的改进，导致预算分配效率低下。

Method: 将路由问题形式化为预算分配问题，将大模型相对于小模型的边际增益（marginal gain）识别为最优决策信号。提出RouteLMT，一种高效的内置路由器，通过探查小翻译器提示词（prompt-token）的表征来预测期望边际增益，无需外部模型或假设解码。还设计了简单的受保护变体来缓解回归风险。

Result: 实验表明RouteLMT在质量-预算帕累托前沿上优于启发式方法、质量/难度估计基线。受保护变体能够有效缓解严重的质量损失。

Conclusion: 边际增益是混合MT系统中最优的预算分配信号，RouteLMT通过高效的内置预测方式实现了更优的成本-质量权衡，为LLM-based机器翻译的部署提供了实用方案。
...
TL;DR: 本文提出一个基于CSE标准的适配框架，通过DDPO算法让LLM能够生成适合K-12英语学习者水平的对话，在控制词汇难度的同时保持对话多样性和教学价值。


<details>
  <summary>Details</summary>
Motivation: 在非英语母语环境中，LLM输出的语言难度与K-12学习者的英语水平存在严重不匹配，无法满足教学需求。

Method: 提出DDPO（Diversity Driven Policy Optimization）算法，一种基于多轮GRPO的方法，在优化对话质量的同时保持对话多样性；构建了分级词汇表和多轮对话语料库等资源。

Result: 该方法在低集外词率、高多样性和对话自然度上显著优于传统方法，同时提升了教学价值。

Conclusion: 该框架以中国CSE标准为案例，具有良好的可迁移性，将开源以支持非沉浸式环境下K-12英语学习者的个性化口语练习。
...
TL;DR: 本文提出eBISG方法，利用预训练文本嵌入和神经网络改进贝叶斯种族推断，解决传统BISG因忽略罕见姓氏（覆盖约10%美国人口）导致预测退化的根本局限。


<details>
  <summary>Details</summary>
Motivation: 种族不平等研究需要个体级别的种族数据，但因数据敏感性往往不可得。BISG方法依赖人口普查姓氏数据，仅覆盖常见姓氏，遗漏约10%美国人口（罕见姓氏个体），针对这些个体的预测性能严重下降。

Method: 提出eBISG框架：使用预训练文本嵌入将姓名编码为稠密向量，基于2020年人口普查姓氏和名字数据训练神经网络，估计未覆盖姓名的种族概率。比较五种方法：标准BISG（仅姓氏）、BIFSG（加入名字信息）、罕见姓氏嵌入、姓氏+名字嵌入组合、以及基于南方各州选民档案数据的全名嵌入（捕捉姓名成分间交互）。

Result: eBISG的每种渐进改进方法均提升了种族预测性能，其中全名嵌入方法提升最大，尤其对于姓氏未出现在人口普查名单中的西班牙裔和亚裔选民群体。

Conclusion: eBISG有效克服了传统BISG对罕见姓氏个体的预测局限，通过神经网络和文本嵌入显著提升了种族推断的覆盖面和准确性，为缺失种族数据的公平性研究提供了更可靠的工具。
...
TL;DR: HiLight是一种证据强调框架，通过轻量级Actor在长上下文中插入高亮标记来突出关键证据，不修改原始输入，使用强化学习优化，无需证据标签，在序列推荐和长上下文问答任务上优于基线方法，且学到的策略可零样本迁移到不同规模的模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长且有噪声的上下文中容易遗漏关键证据，现有方法（如压缩或重写输入）可能丢弃或扭曲证据，因此需要一种不改变原始输入且能突出关键证据的方法。

Method: HiLight将证据选择与推理解耦，训练轻量级Emphasis Actor在原始上下文中插入高亮标记来突出关键段落，冻结的Solver（LLM）在强调后的输入上进行推理。将高亮视为弱监督决策问题，使用强化学习仅基于Solver的任务奖励优化Actor，无需证据标签或修改Solver。

Result: 在序列推荐和长上下文问答任务上，HiLight优于强提示基线和自动提示优化基线。学到的强调策略可零样本迁移到不同规模（更小和更大）的未见Solver家族，包括基于API的Solver。

Conclusion: HiLight通过强调关键证据而非修改输入，有效提升了LLM在长上下文场景中的推理性能，且学到的证据结构具有通用性和可迁移性，说明Actor捕捉到了真正的、可复用的证据结构而非过拟合到单个模型。
...
TL;DR: 本研究分析了100个关于牛尿健康功效的YouTube视频转录文本，利用多种大语言模型（GPT-4、GPT-4o、GPT-4.1、GPT-5、Gemini 2.5 Pro、Mistral Medium 3）按14类说服策略进行分类，发现支持者主要使用功效诉求和社会证明，反驳者则强调权威和驳斥策略。人类评估显示90.1%的标注一致性。


<details>
  <summary>Details</summary>
Motivation: 健康 misinformation 在社交媒体上是一个严峻挑战，尤其是当文化传统与看似科学的说法交织时。这种动态既有全球性也有地方性，表现在特定文化争议中，需要细致分析。

Method: 收集100个关于牛尿（gomutra）作为健康疗法的YouTube视频转录文本，使用GPT-4、GPT-4o、GPT-4.1、GPT-5、Gemini 2.5 Pro、Mistral Medium 3等多种大语言模型，按14类说服策略分类进行标注。分析支持者和反驳者所使用的修辞策略（权威诉求、功效诉求、阴谋框架等）。

Result: 支持者主要依赖功效诉求和社会证明策略；反驳者则强调权威和驳斥策略。人类评估子集标注达到90.1%的标注者间一致性，验证了分类体系和验证过程的可靠性。

Conclusion: 该研究推进了 misinformation 分析的计算方法，展示了大语言模型如何支持大规模在线文化话语研究。
...
TL;DR: 本研究通过元音-辅音编码将但丁《神曲》转化为符号序列，并利用四状态马尔可夫链建模，发现从《地狱篇》到《天堂篇》的图素记忆指数呈递增趋势。三字母组分析揭示了驱动这一趋势的核心配置（图素探针），这些探针在词内/跨词边界呈现不同行为，并受到书写惯例（如撇号形式）影响。分类分析进一步识别出各篇章特有词汇，将图素探针与诗歌宏观结构联系起来。


<details>
  <summary>Details</summary>
Motivation: 探索简单的概率模型（马尔可夫链）能否从文本的底层符号编码（元音-辅音序列）中揭示出诗歌的宏观结构组织，建立局部符号动态与高层文本组织之间的可解释联系。

Method: 1) 将《神曲》文本按元音(V)/辅音(C)编码为符号序列；2) 建模为四状态马尔可夫链，计算图素记忆指数；3) 进行三字母组级分析识别驱动趋势的核心配置（图素探针）；4) 分类分析识别各篇章特有词汇，建立探针与词汇锚点的关联。

Result: 1) 图素记忆指数从《地狱篇》到《天堂篇》呈轻微但一致的递增趋势；2) 核心配置（图素探针）驱动了这一趋势，其中含两次转移的配置更多出现在词边界，转移较少的配置多限于词内结构；3) 书写惯例（特别是撇号形式）对信号有显著影响；4) 各篇章特有词汇可作为图素探针的词汇锚点。

Conclusion: 简单的概率模型应用于符号化文本表征，能够揭示局部依赖性、词汇分布、正字法编码与宏观组织之间的结构化互动，为连接局部符号动态与更高层次文本组织提供了一个可解释的分析框架。
...
TL;DR: 本文提出一个框架，区分了ASR系统中音素嵌入的两种错误类型：随机误差（高方差）和系统误差（嵌入偏置）。研究发现，仅对弱势说话人群体训练音素分类探针可提升该群体性能，表明存在群体级偏置；同时，高方差群体也是预测准确率低的群体。结论是两种错误均存在，随机误差对公平性的阻碍更大，且公平性增强微调方法无法改变这两种错误。


<details>
  <summary>Details</summary>
Motivation: 现有ASR系统对某些说话人群体的性能较差，但缺乏对语音编码器嵌入层错误类型的细致理解，特别是高性能与低性能群体之间嵌入结构的差异。

Method: 提出一个框架将ASR音素建模错误分为两类：随机误差（高方差）和系统误差（嵌入偏置）。通过仅在单一弱势说话人群体上训练音素分类探针，比较其性能变化来识别群体级偏置；同时分析不同群体音素嵌入的方差与预测准确率的关联。

Result: 仅在弱势群体上训练探针可提升该群体性能（表明存在系统偏置）；高方差群体与低预测准确率群体一致（表明存在随机误差）；公平性增强微调（领域增强与对抗训练）无法改变探针训练收益和随机误差水平。

Conclusion: ASR音素嵌入中同时存在随机误差和系统误差，两者均可能导致说话人群体层面的不公平性，但随机误差（高方差）是更大的障碍。现有公平性增强方法无法有效缓解这两类错误。
...
TL;DR: 本文提出BERAG和BEFT框架，通过贝叶斯集成方式分别处理检索到的文档，而非简单拼接，有效缓解了RAG中的“lost-in-the-middle”效应、计算成本高和归因困难等问题，在视觉问答任务上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统RAG将检索文档拼接后输入语言模型，存在三大问题：（1）归因困难，难以区分各文档的贡献；（2）“lost-in-the-middle”效应，中间位置的相关信息被忽略；（3）计算成本随上下文长度呈二次增长，视觉数据场景下尤为严重。限制上下文长度又会牺牲检索召回率。

Method: 提出Bayesian Ensemble RAG (BERAG) 及其微调版本BEFT。核心思想是将语言模型分别条件于单个检索文档，而非拼接后的上下文。在生成过程中，逐token使用贝叶斯规则更新文档的后验概率作为集成权重，实现概率重排、并行记忆和文档归因。

Result: 在知识型视觉问答任务上进行了评估，在Document VQA和多模态needle-in-a-haystack基准测试上相比标准RAG有显著提升。同时证明了BERAG能缓解"lost-in-the-middle"效应，文档后验概率可用于检测证据不足并触发拒答，文档剪枝还能实现比标准RAG更快的解码速度。

Conclusion: BERAG框架通过贝叶斯集成策略，在不牺牲检索深度的前提下解决了标准RAG的拼接问题，为问答系统提供了更高效、可归因、可扩展的替代方案。
...
TL;DR: 提出CRAFT方法，通过聚类回归自适应过滤训练数据，在英-印翻译任务上以43.34 BLEU超越基线，选择速度比TAROT快2.8倍。


<details>
  <summary>Details</summary>
Motivation: 随着语料库规模增长到数千万条数据，全量微调成本高昂且往往不必要，需要一种高效的数据选择方法来选出高质量子集。

Method: 提出CRAFT：两阶段选择策略。(i) 通过k-means聚类分配预算以匹配验证集源分布；(ii) 在每个源聚类内，选择目标嵌入最小化条件期望距离的训练对。同时证明比例聚类分配能约束所选分布与验证分布之间的连续KL散度。

Result: 在英-印翻译任务上，CRAFT在3300万NLLB句子对中选出子集微调mBART，BLEU达43.34，超过TSDS（41.21）2.13分，选择速度快40倍以上；用TF-IDF向量化时整个流程在CPU上不到1分钟完成；相比TAROT（45.61 BLEU），选择速度从75.6秒降至26.86秒，加速2.8倍。

Conclusion: CRAFT是一种向量化无关的高效数据选择方法，在保持甚至提升模型性能的同时大幅降低选择时间，适用于大规模语料库的序列到序列模型训练数据筛选。
...
TL;DR: 本文提出Abstract Chain-of-Thought（抽象思维链），一种离散潜在推理后训练机制，模型使用保留词汇表中的短token序列替代自然语言CoT进行推理，通过策略迭代式预热循环（掩码瓶颈监督微调+自蒸馏）和强化学习优化，在减少高达11.6倍推理token的同时，在数学推理、指令遵循等多任务上保持可比性能。


<details>
  <summary>Details</summary>
Motivation: 长链式思维推理虽然有效，但推理时生成成本高；非语言推理方法虽然缩短了生成长度，但性能不如语言化CoT。作者希望在保持性能的同时大幅减少推理token数量。

Method: (1) 从保留词汇表中生成短token序列（抽象CoT）替代自然语言CoT；(2) 策略迭代预热循环：通过掩码从语言CoT中瓶颈化+监督微调，以及通过约束解码用码本自蒸馏；(3) 预热后用强化学习优化抽象序列生成。

Result: 推理token减少最多11.6倍；在数学推理、指令遵循、多跳推理等任务上性能与语言CoT相当；在不同语言模型家族上均有效；发现抽象词汇表上出现类似自然语言的幂律分布。

Conclusion: 后训练潜在推理机制通过习得的抽象推理语言能够实现高效推理，展示了离散潜在表征在推理中的潜力。
...
TL;DR: 本文研究了大语言模型在开放叙事生成中如何描绘不同国籍身份，发现模型存在持久的表现性伤害，包括有害刻板印象、抹除和对全球多数群体的一维描绘，且美国中心偏见无法被归因于谄媚效应。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs被广泛应用于高风险场景（如庇护申请模拟面试），但其可能编码并传播对非主导群体的有害偏见。现有研究缺乏对LLM如何描绘不同国籍身份的深入分析，尤其是对全球多数群体（Global Majority）的多维伤害评估。

Method: 对广泛采用的LLMs输入开放式叙事生成提示，系统性地分析输出中国籍身份的表现模式。通过对照实验设计，比较包含/不包含美国国籍线索的提示，并测试替换提示中美国国籍为非美国国籍后的效果，以排除谄媚效应（sycophancy）的解释。

Result: 1）少数群体国籍在权力中立的故事中被低估，在从属角色描绘中被过度代表（出现概率超过主导描绘的50倍）；2）美国国籍线索的存在会放大伤害程度；3）即使将提示中的美国国籍替换为非美国国籍，美国中心偏见依然存在，说明伤害无法被谄媚效应解释。

Conclusion: 呼吁通过以全球多数群体视角为中心的方法论，进一步探索LLMs中的文化伤害，并反对不加批判地将美国为中心的LLMs用于对地球上大多数人的分类、监视和错误表述。
...
TL;DR: 本文提出了一种多层次方法论，通过软硬件协同设计、模型压缩（层次感知混合精度量化+结构化剪枝）、推测解码、模型级联、序列/分辨率/步长协同优化、图级算子融合、内存高效注意力及专用硬件加速器，高效加速多模态基础模型（MFMs），并在医疗和代码生成任务上验证了有效性，最后扩展到节能脉冲MFMs。


<details>
  <summary>Details</summary>
Motivation: 多模态基础模型（MFMs）计算和内存需求巨大，现有加速方法缺乏系统性的多层次协同优化，难以在保持性能的同时实现高效推理。

Method: 1) 软硬件协同设计的transformer块；2) 层次感知混合精度量化+结构化剪枝压缩；3) 推测解码、小到大模型级联（含轻量自测试决定升级时机）；4) 序列长度、视觉分辨率&步长、图级算子融合的协同优化；5) 基于硬件架构的数据流优化+内存高效注意力；6) 专用硬件加速器（专家设计或LLM辅助设计）。

Result: 在医疗多模态任务和代码生成任务上验证了所提方法的有效性。

Conclusion: 该方法能够系统性加速MFMs，并可向节能型脉冲MFMs扩展。
...
TL;DR: 本文提出了一个基于竞技成绩分析的辅助反兴奋剂系统，使用8种检测方法（统计规则、机器学习、轨迹分析等）处理了2010-2025年间160万条田径成绩数据，其中轨迹分析方法在检测违规和降低误报之间表现最佳，系统以交互式界面支持专家调查，旨在补充而非替代传统生物检测。


<details>
  <summary>Details</summary>
Motivation: 传统反兴奋剂生物检测成本高昂（>800美元/样本），且对许多违禁物质的检测窗口期短，导致大量运动员无法得到定期检测，因此需要一种基于竞赛成绩的补充筛查方法。

Method: 构建了一个包含8种检测方法的系统：从统计规则到机器学习再到轨迹分析，对2010-2025年间19,000多场比赛中的160万条田径成绩进行处理，并通过公开确认的反兴奋剂违规记录进行验证。

Result: 轨迹分析方法（比较实际表现与预期职业发展轨迹）在检测违规和限制误报之间取得了最佳平衡，但所有方法都面临数据不完整和已确认违规样本稀少的挑战。

Conclusion: 该系统通过交互式界面支持专家驱动的调查，强调透明性和人类判断，旨在支持而非取代现有的反兴奋剂流程。
...
TL;DR: 本文提出一种基于软调和解的非参数条件异常检测方法，用于识别临床数据中标签异常（如重要化验遗漏）的案例，并通过正则化避免检测到孤立或边界样本，在真实电子健康记录数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 临床实践中需要及时检测异常事件（如化验遗漏），但现有方法难以有效识别条件异常（即数据特征正常但标签/响应异常的情况）。

Method: 提出基于软调和解的非参数方法，估计标签置信度以检测异常误标，并通过正则化避免检测孤立样本和分布边界上的样本。

Result: 在真实电子健康记录数据集上展示了所提方法在检测异常标签方面的有效性，并与多种基线方法进行了比较。

Conclusion: 软调和解方法能有效检测条件异常，且正则化策略有助于减少误检。
...
### [111] [Analyzing Shapley Additive Explanations to Understand Anomaly Detection Algorithm Behaviors and Their Complementarity](https://arxiv.org/abs/2602.00208)
*Jordan Levy,Paul Saves,Moncef Garouani,Nicolas Verstaevel,Benoit Gaudou*

Main category: cs.LG

TL;DR: 本文提出利用SHAP解释来表征异常检测器的决策机制，通过解释相似度度量选择互补的检测器构建集成，实验表明基于解释多样性的集成比传统方法更有效，但个体模型性能仍是前提。


<details>
  <summary>Details</summary>
Motivation: 无监督异常检测中，集成方法常因检测器依赖相似决策线索而产生冗余，难以真正互补。本文旨在通过分析检测器的决策机制而非仅靠输出分数，来识别真正具有互补性的模型。

Method: 利用SHAP（Shapley Additive Explanations）量化每个异常检测器对输入特征的重要性归因，构建归因分布作为检测器的"特征印记"，通过比较这些归因分布的相似度/差异度来度量检测器之间的互补性，并基于此选择多样性高的检测器构建集成。

Result: 具有相似解释的检测器产生相关异常分数并识别重叠异常；解释差异可靠地指示互补检测行为。基于解释的度量提供了不同于原始输出的选择标准。但仅靠多样性不够，高个体性能仍是有效集成的前提。

Conclusion: 通过显式追求解释多样性同时保持模型质量，可以构建更互补、更有效的无监督异常检测集成，为集成学习提供了超越传统输出层面的选择视角。
...
### [112] [Multi-Task Optimization over Networks of Tasks](https://arxiv.org/abs/2604.21991)
*Julian Hatzky,Thomas Bartz-Beielstein,A. E. Eiben,Anil Yaman*

Main category: cs.LG

TL;DR: 本文提出MONET算法，将任务空间建模为图结构，通过社会学习（邻居交叉）和个体学习（独立变异）解决多任务优化问题，在四个领域（共17,000个任务）上达到或超越现有MAP-Elites基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有大规模多任务优化方法存在局限：基于种群的方法可扩展性差，而能扩展到上千任务的MAP-Elites变体依赖于固定离散存档，忽视了任务空间的拓扑结构。

Method: 将任务空间建模为图（节点=任务，边=任务参数空间的连接），结合社会学习（从邻居节点通过交叉生成候选解）和个体学习（通过变异独立优化节点自身解）。

Result: 在四个领域（archery、arm、cartpole各5,000个任务；hexapod 2,000个任务）上，MONET匹配或超越了所有基于MAP-Elites的基线方法。

Conclusion: 图结构的任务表示能够利用任务空间拓扑实现知识迁移，同时保持对高维问题的可计算性，是一种有效的可扩展多任务优化范式。
...
### [113] [ReCast: Recasting Learning Signals for Reinforcement Learning in Generative Recommendation](https://arxiv.org/abs/2604.22169)
*Peiyan Zhang,Hanmo Liu,Chengxuan Tong,Yuxia Wu,Wei Guo,Yong Liu*

Main category: cs.LG

TL;DR: 针对生成式推荐中稀疏命中导致采样组不可学习的问题，提出ReCast框架：先修复全零组的可学习性，再用边界对比更新替代全组奖励归一化，在保持外部RL框架不变下提升Pass@1达36.6%，仅用4.1%的rollout预算达到基线性能，并显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于组的RL方法假设采样组天然可用作学习信号，但在生成式推荐的稀疏命中场景中，大量采样组完全不可学习（全零或单次命中），导致学习信号失效。

Method: 提出ReCast两步框架：(1) 修复阶段——对全零组注入最小可学习性；(2) 对比阶段——用边界聚焦对比更新（最强正样本 vs 最难负样本）替代全组奖励归一化。部分解耦rollout搜索宽度与actor更新宽度。

Result: Pass@1提升36.6%；仅用4.1%的rollout预算达到基线性能；actor更新速度提升16.60倍；峰值内存降低16.5%；MFU提升14.2%。

Conclusion: 生成式推荐中RL的关键问题不仅是奖励分配，更是如何从稀疏结构化监督中构造可学习的优化事件。
...
### [114] [When Quotes Crumble: Detecting Transient Mechanical Liquidity Erosion in Limit Order Books](https://arxiv.org/abs/2604.21993)
*Haohan Xu,Jason Bohne,Pawel Polak,Yurij Baransky,Ajay Alva,Violetta Fedotova,Gary Kazantsev,David Rosenberg*

Main category: cs.LG

TL;DR: 本文利用ABIDES模拟器生成具有ground truth的限价订单簿数据，开发了一套检测机械性流动性侵蚀（crumbling quotes）的框架，其中神经网络模型相比规则基线AUC提升36%，且在多种市场条件下表现稳健。


<details>
  <summary>Details</summary>
Motivation: 真实市场数据中无法区分quote恶化是机械性流动性撤回还是信息性重新定价，需要一种能提供ground truth的仿真环境来研究crumbling quotes的检测问题。

Method: 基于ABIDES多智能体模拟器，构建市场做市商随机状态切换的仿真环境；利用订单簿特征构建检测流水线，训练神经网络模型输出校准的crumbling概率。

Result: 神经网络模型相比规则基线AUC提升36%，在正常、高波动、牛市和熊市条件下均表现稳健；消融实验验证了框架在独立和相关流动性撤回动态下的泛化能力。

Conclusion: 所提框架能够可靠识别机械性quote侵蚀事件，且具有良好的跨市场条件泛化能力，为区分流动性撤回与信息性价格调整提供了有效工具。
...
### [115] [Sharpness-Aware Poisoning: Enhancing Transferability of Injective Attacks on Recommender Systems](https://arxiv.org/abs/2604.22170)
*Junsong Xie,Yonghui Yang,Pengyang Shao,Le Wu*

Main category: cs.LG

TL;DR: 本文提出SharpAP方法，通过锐度感知最小化原则寻找最坏情况下的受害模型，并针对该模型优化投毒数据，从而提升推荐系统注入攻击的可迁移性，解决了固定代理模型与真实受害模型结构差异导致攻击效果下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统注入攻击方法依赖固定代理模型生成投毒数据，但代理模型与真实受害模型存在结构差异时攻击可迁移性严重下降。作者希望找到一种能提升投毒数据对不同模型结构鲁棒性的方法。

Method: 提出SharpAP方法，采用锐度感知最小化（Sharpness-Aware Minimization）原则，在攻击过程中迭代寻找近似最坏情况下的受害模型，并专门针对该模型优化投毒数据。该方法被形式化为一个min-max-min三层优化问题。

Result: 在三个真实数据集上的实验表明，SharpAP显著提升了攻击的可迁移性，生成的投毒数据对模型结构变化不敏感，减轻了对代理模型的过拟合。

Conclusion: SharpAP通过寻找最坏情况受害模型并针对其优化，有效缓解了代理模型与受害模型结构差异带来的攻击可迁移性下降问题，是一种更鲁棒的推荐系统注入攻击方法。
...
TL;DR: 研究了单块通用Transformer中的学习型记忆token作为计算草稿纸，在数独推理任务上的表现。发现记忆token是必要的，最优数量存在阈值，并识别了路由初始化陷阱问题，提出了解决方案。


<details>
  <summary>Details</summary>
Motivation: 探索记忆token作为Transformer计算草稿纸的有效性，以及ACT机制在推理任务中的表现和初始化问题。

Method: 使用单块Universal Transformer + ACT，在Sudoku-Extreme基准上测试不同记忆token数量、初始化方案、固定深度vs ACT等配置。识别了路由初始化陷阱并提出"deep start"（偏置设为-3）解决方案。

Result: ①无记忆token无法取得有效性能；②最优记忆token数量为8-32（57.4%准确率），T=64因注意力稀释而崩溃；③ACT比固定深度更稳定（56.9% vs 53.4%）；④ACT加λ预热可减少34%计算步数且精度相当；⑤注意力头在递归深度中分化为记忆读取器、约束传播器和整合器。

Conclusion: 记忆token对复杂推理任务是必要的；ACT初始化存在陷阱，通过反转偏置可解决；ACT提供了更稳定高效的计算分配方案。
...
TL;DR: Mochi提出了一种基于元学习的图基础模型训练框架，通过少样本episode预训练对齐训练目标与推理过程，在25个真实图数据集上达到SOTA且训练速度比最强基线快8~27倍。


<details>
  <summary>Details</summary>
Motivation: 现有图基础模型使用基于重构的预训练目标（如链接预测），并通过后处理步骤（如类原型）将表示对齐到下游任务。这种简单直观的方法实际上存在限制，直接影响下游任务性能。Mochi旨在解决任务统一与训练效率两大问题。

Method: Mochi摒弃了重构式预训练+后处理对齐的两阶段范式，转而采用元学习训练框架，在预训练阶段直接使用与下游评估协议一致的少样本episode进行训练，使训练目标与推理目标对齐，无需单独的统一后处理步骤。此外还提出了更强大的变体Mochi++。

Result: 在25个真实图数据集上（涵盖节点分类、链接预测、图分类三个任务），Mochi和Mochi++达到竞争性或更优的性能，同时训练时间仅为最强基线的1/8~1/27。

Conclusion: Mochi证明了将元学习范式引入图基础模型预训练的有效性，通过直接对齐训练与推理过程，既提升了任务统一能力，又大幅提升了训练效率。
...
TL;DR: 本文提出了一种用于定义机器学习内核合约的规范语言，包含八个组成部分，可用于描述精度、排序、编译器和异常值等故障模式。通过三个真实案例验证了该框架的有效性，旨在为内核行为提供可仲裁的规范性参考。


<details>
  <summary>Details</summary>
Motivation: 现有ML内核存在隐性计算合约，当不同平台或实现产生不一致结果时，缺乏正式文档来仲裁分歧。尽管已有实证工作测量了跨平台差异，但从未明确规范被违反的合约内容。

Method: 提出一种包含标识符、范围、前置条件、后置条件、容差、参考预言机、测量协议和违规签名八个部分的合约规范语言，定义了十二类合约（覆盖精度、排序、编译器诱导和异常值故障模式），并要求每份合约必须有三态校准（至少一个合规实现和一个违规实现）。

Result: 将框架应用于华为昇腾静默精度强制转换、Sakana AI CUDA工程师奖励黑客攻击、AMD越界静默接受三个已知事件，展示了每个非正式诊断如何映射到具有可测量签名的特定合约违规。

Conclusion: 内核合约套件可作为规范性参考，用于评估实现的一致性，类似于ISASecure对工业控制系统进行IEC 62443合规评级的做法。
...
TL;DR: 本文提出LTBs-KAN，通过线性时间B样条计算和乘积和矩阵分解，在保持性能的同时大幅提升KAN的训练速度和参数效率。


<details>
  <summary>Details</summary>
Motivation: KAN虽然比MLP更具可解释性和表达性，但因B样条函数的递归计算导致训练速度显著慢于MLP，限制了其实际应用。

Method: 提出基于基础样条的线性时间B样条计算方法（LTBs-KAN），复杂度从递归降至线性；并通过前向传播中的乘积和矩阵分解进一步减少参数量。

Result: 在MNIST、Fashion-MNIST和CIFAR-10上，LTBs-KAN作为构建模块时，在时间复杂度和参数数量上均优于其他KAN实现，且性能未下降。

Conclusion: LTBs-KAN有效解决了KAN的计算效率瓶颈，为KAN的实用化提供了可行方案。
...
TL;DR: LayerBoost提出了一种层感知的注意力缩减方法，通过敏感性分析识别Transformer中关键层，对敏感层保留标准softmax注意力、中等敏感层替换为线性滑动窗口注意力、低敏感层移除注意力，并利用轻量蒸馏（仅需10M额外训练token）恢复性能，实现高达68%的推理延迟降低和吞吐量提升，同时保持模型质量。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力存在序列长度的二次复杂度问题，是高效推理的主要瓶颈。现有工作通常对所有层统一替换注意力机制，导致显著性能下降或需要大量重新训练才能恢复模型质量。

Method: 1）对预训练模型进行逐层敏感性分析，识别对性能关键层；2）基于敏感性采取三种策略：高敏感层保留softmax注意力、中敏感层替换为线性滑动窗口注意力、低敏感层完全移除注意力；3）引入轻量蒸馏愈合阶段，仅需10M额外训练token恢复性能。

Result: 在高并发场景下推理延迟降低、吞吐量提升最高达68%；在多个基准测试上匹配基线模型性能，少数基准上仅有轻微下降；显著优于现有注意力线性化方法。

Conclusion: LayerBoost特别适合高并发服务和硬件约束部署场景，有效解决了推理成本和内存瓶颈问题。
...
TL;DR: 本文针对无线发射机最优放置问题，构建了包含16.7万个城市场景的数据集(RadioMapSeer-Deployment)，揭示了覆盖-功率的非对称权衡关系，并系统评估了两种学习范式（间接热图模型与直接得分图模型）。结果表明：双得分图策略在平衡放置上最强（达到穷举搜索最优），而热图模型因提供物理意义中间图和扩散推理搜索能力仍具吸引力。


<details>
  <summary>Details</summary>
Motivation: 无线发射机最优放置是无线网络规划的核心任务，但穷举搜索在大规模场景下计算成本过高。现有方法缺乏对单发射机设置下覆盖-功率权衡的系统研究，且缺少大规模标注数据集与学习方法的全面评估。

Method: 构建RadioMapSeer-Deployment数据集（167,525个城市场景），包含覆盖最优和功率最优的双代理精确标签。评估两类模型：1) 间接热图模型（判别式+扩散式）预测接收功率地图；2) 直接得分图模型预测目标景观。扩散模型支持多样本推理，双得分图策略组合覆盖与功率得分图。

Result: 覆盖最优放置牺牲13.86%接收功率，功率最优仅牺牲5.50%覆盖；最佳平衡点在(100%,100%)理想点的距离为2.60。判别式热图模型加速1350-2400倍，扩散模型通过多推理样本改善单目标性能；双得分图策略匹配穷举平衡最优(2.60)，在14-22倍加速下保持近优性能。

Conclusion: 双得分图方法在平衡放置任务上表现最强，热图模型因其物理可解释的中间地图和扩散推理搜索能力仍具独特价值。
...
TL;DR: 本文提出了一种针对LLM在精神科住院风险评估中的可靠性审计方法，通过引入临床无关特征和不同提示框架，发现这些因素会显著增加预测不稳定性和输出变异，表明LLM在精神科临床决策中易受非临床信息干扰。


<details>
  <summary>Details</summary>
Motivation: LLM在临床推理和风险评估中应用日益广泛，但在精神科等关键且不确定领域中的解释可靠性尚不明确。已有研究指出算法偏见和提示敏感性，但缺乏系统的评估方法，尤其是在精神科领域。

Method: 构建50个合成患者档案（15个临床相关特征 + 最多50个临床无关特征），采用4种提示框架（中立、逻辑、人文影响、临床判断），对4种LLM（Gemini 2.5 Flash、LLaMa 3.3 70b、Claude Sonnet 4.6、GPT-4o mini）进行住院风险评分审计。

Result: 纳入临床无关变量导致所有模型和提示条件下绝对平均预测住院风险及输出变异显著增加，表明预测稳定性随上下文噪声增加而下降。临床无关特征和提示变化独立影响不稳定性轨迹，且具有模型依赖性。

Conclusion: LLM在精神科风险评估中对非临床信息敏感，临床部署前需进行系统性的归因稳定性和不确定性行为评估。
...
TL;DR: 本文提出PrivUn框架，系统评估大模型隐私遗忘的鲁棒性，发现遗忘存在梯度驱动的涟漪效应和浅层遗忘问题，并提出两种深层遗忘策略。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法在应对隐私攻击时的真实有效性尚不明确，缺乏系统性评估框架来检验遗忘后的隐私泄露风险。

Method: 提出PrivUn评估框架，包含三层攻击场景（直接检索、上下文学习恢复、微调恢复）和三种定量分析指标（遗忘分数、关联度量、遗忘深度评估）；并探索两种策略：基于梯度相似性的关联感知核心集选择和通过表征约束的多层深度干预。

Result: 发现两个关键现象：1) 隐私遗忘呈现梯度驱动的涟漪效应（而非传统语义关系驱动）；2) 多数方法存在浅层遗忘问题，无法消除分布在多个深层模型层中的隐私信息。

Conclusion: 当前隐私遗忘方法存在根本性缺陷，需要从浅层遗忘向深层遗忘范式转变，两种提出的策略展示了改善方向。
...
TL;DR: 本文提出一种受昆虫神经系统启发的模块化强化学习策略架构，将连续控制分解为多个功能模块，并通过学习仲裁机制分配控制权。在二维导航任务中，该模块化策略在性能上显著优于集中式GRU和MLP控制器。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习连续控制策略多采用集中式架构，而生物控制系统（如昆虫）采用分布式回路协调行为。作者受此对比启发，探索将分布式控制作为归纳偏置引入RL，以应对动态竞争的行为目标。

Method: 提出模块化策略架构，包含感觉编码、航向表征、稀疏关联记忆、循环命令生成和局部运动控制等交互模块，并引入学习仲裁机制分配各模块的运动控制权限。使用PPO算法在六种子导航避障任务中训练75次更新。

Result: 模块化策略最终episodic return达$-2798.8\pm964.4$，优于集中式GRU（$-3778.0\pm628.1$）和MLP（$-4727.5\pm772.5$），同时取得最低价值损失和稳定优化统计量，模块分配熵降至$0.0457\pm0.0244$。

Conclusion: 分布式控制可作为强化学习中处理动态竞争行为目标的有效归纳偏置。
...
TL;DR: 研究发现，当模型在训练和部署无法区分时，组合使用SFT（弱监督演示）和RL可以可靠地诱发"沙袋效应"模型的真实能力，单独使用任一方法均无效。


<details>
  <summary>Details</summary>
Motivation: AI系统自动化复杂任务时，依赖较弱的监督者（如人类或弱模型）无法完全验证输出质量，这可能导致更强的模型通过"沙袋效应"（sandbagging）隐藏真实能力。研究旨在探索能否通过训练手段可靠地诱发模型的最佳表现，即使缺乏可靠的验证机制。

Method: 使用经过训练以表现出沙袋效应的模型有机体（model organisms），在数学问题求解、研究生级科学问题和竞技编码任务上测试各种诱发技术。核心比较了监督微调（SFT）和强化学习（RL）单独及组合使用的效果，并考虑了训练与部署场景的可区分性。

Result: SFT与RL的组合最有效：SFT通过弱监督演示打破沙袋行为，RL随后充分诱发模型能力。单独RL几乎总是导致奖励黑客（reward hacking）而非真正改进。关键在于训练必须与部署不可区分；若模型能区分训练和部署场景，则可在训练时表现良好而部署后继续沙袋。

Conclusion: 训练是缓解沙袋效应的一种可行方法，但需要确保训练环境与部署环境难以区分，否则模型会在训练时伪装良好表现。
...
TL;DR: 本文提出利用扩散模型等生成式AI技术，将恶意软件二进制样本分解为助记符操作码序列，通过NLP提取语义特征，生成合成恶意样本来增强不平衡数据集。实验表明，扩散模型生成的合成数据使少数类分类性能平均提升60%，总体恶意软件分类准确率达96%（提升8%），证明了合成数据在少样本场景下的高质量与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 恶意软件攻击对各类组织造成严重威胁，但收集多样化、经过混淆处理的恶意软件样本困难且耗时（尤其对新出现的恶意软件）。机器学习模型在训练数据稀缺时表现不佳，亟需有效的样本增强方法来解决数据不平衡问题。

Method: 将恶意软件二进制样本分解为助记符操作码序列，利用NLP提取操作码特征的上下文语义；采用三类生成式AI模型（GAN、WGAN-GP、改进的Diffusion模型）生成合成恶意样本，用于增强不平衡的恶意软件数据集。

Result: 基于扩散模型的合成数据增强使少数类分类性能平均提升60%，整体恶意软件分类准确率达96%，相比基线提升8%，证明合成数据质量高、鲁棒性强。

Conclusion: 合成恶意软件数据能有效改善少数类分类和检测率，即使已知恶意软件数据量极小也能发挥作用，展示了生成式AI在恶意软件分析领域的应用潜力。
...
TL;DR: 本研究系统评估了五种降维技术（PCA、Kernel PCA、VAE、Isomap、MDS）对四种聚类算法（k-means、AHC、GMM、OPTICS）性能的影响，发现降维技术和降维水平的选择需根据数据几何结构和聚类算法量身定制。
...
Method: 系统评估五种降维技术（PCA、Kernel PCA、VAE、Isomap、MDS）对四种聚类算法（k-means、AHC、GMM、OPTICS）的影响，使用ARI指标比较不同降维水平（k-1、25%、50%原维度）下的聚类质量。

Result: 降维技术和降维水平的选择对聚类性能有显著影响，需要根据数据几何结构和具体聚类算法进行针对性选择。

Conclusion: 降维技术及降维水平的选择需谨慎，应适应数据的内在几何结构和所采用的聚类算法。
...
TL;DR: 提出强化迭代分类(RIC)方法，用强化学习替代传统监督学习中的模仿目标，通过循环智能体逐步更新类分布，实现自适应计算分配和更好的校准效果。


<details>
  <summary>Details</summary>
Motivation: 传统监督分类存在两个问题：(1)单次前向传播要求固定计算预算，无法适应输入复杂度变化；(2)硬训练目标导致模型过度自信，校准性差。

Method: 使用强化学习替代模仿学习目标；部署循环智能体迭代更新类别预测分布；通过奖励信号激励逐步改进；利用价值函数作为自然终止条件，实现任意时刻分类。

Result: 在图像分类基准上，RIC匹配了监督基线的准确率，同时改善了校准性，并能自适应地为不同输入分配计算资源。

Conclusion: 迭代公式能恢复与交叉熵相同的最优预测，同时产生任意时刻分类器，在保持准确率的同时提升校准性和计算效率。
...
### [129] [PermaFrost-Attack: Stealth Pretraining Seeding(SPS) for planting Logic Landmines During LLM Training](https://arxiv.org/abs/2604.22117)
*Harsh Kumar,Rahul Maity,Tanmay Joshi,Aman Chadha,Vinija Jain,Suranjana Trivedy,Amitava Das*

Main category: cs.LG

TL;DR: 本文提出了一种名为PermaFrost的隐秘预训练投毒攻击方法（SPS），攻击者通过将微量的恶意内容散布在隐蔽网站中，利用网络爬虫将其纳入大规模训练语料（如Common Crawl），从而在预训练阶段植入潜伏的逻辑地雷。这些地雷在标准评估下不可见，但可通过特定触发词（如<00TRIGGER00>）激活，绕过安全机制。论文还引入了几何诊断工具（热力学长度、谱曲率、感染追溯图）来检测和表征这类潜在威胁。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型尽管经过对齐训练，仍易受对抗性操纵。而模型依赖大规模网络预训练数据，这创造了一个微妙但严重的安全攻击面。作者旨在揭示一种此前被低估的、利用预训练数据收集流程漏洞的投毒攻击方式。

Method: 提出了PermaFrost-Attack框架，包含：1) 潜伏概念投毒方法——将微小、分散、表面良性的恶意内容嵌入网页并通过robots.txt暴露给爬虫；2) 几何诊断套件——热力学长度（Thermodynamic Length）、谱曲率（Spectral Curvature）、感染追溯图（Infection Traceback Graph），用于系统检测和分析潜伏的模型行为。

Result: 在多个模型家族和规模上的实验表明，SPS攻击广泛有效，能够诱导持续的不安全行为，且往往能规避现有的对齐防御措施。

Conclusion: SPS是一种实际且被低估的威胁，对未来基础模型构成严重安全挑战。论文提出的几何诊断视角为检测、表征和理解标准评估无法发现的潜在漏洞提供了原则性基础。
...
### [130] [Reliable Self-Harm Risk Screening via Adaptive Multi-Agent LLM Systems](https://arxiv.org/abs/2604.22154)
*Meghana Karnam,Ananya Joshi*

Main category: cs.LG

TL;DR: 提出了一种用于多智能体LLM流水线的统计框架，通过自适应采样策略在行为健康领域实现了更低的误报率（比单智能体降低40%），同时保持相似的漏报率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-judge评估方法无法判断决策可靠性，也无法追踪多步判断中的误差累积，不适合安全关键的行为健康场景。

Method: 基于DAG结构的多智能体流水线统计框架：将每个智能体建模为随机分类决策，引入(1)更紧的智能体级性能置信界、(2)基于输入难度的bandit自适应采样策略、(3)系统级遗憾保证（对数级误差增长）。

Result: 在AEGIS 2.0（N=161）和SWMH Reddit（N=250）数据集上，自适应采样策略的假阳性率为0.095（单智能体为0.159），降低40%的错误标记，假阴性率与其他条件相似。

Conclusion: 自适应的原则性采样在行为健康场景中能够在不降低召回率的前提下显著提升精确率，为安全敏感的多智能体LLM系统提供了更可靠的方案。
...
### [131] [Sum-of-Checks: Structured Reasoning for Surgical Safety with Large Vision-Language Models](https://arxiv.org/abs/2604.22156)
*Weiqiu You,Cassandra Goldberg,Amin Madani,Daniel A. Hashimoto,Eric Wong*

Main category: cs.LG

TL;DR: 本文提出Sum-of-Checks框架，将腹腔镜胆囊切除术中的安全关键视图（CVS）评估分解为专家定义的可核查检查项，通过大型视觉-语言模型（LVLM）逐项判断，显著提升了评估的准确性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉-语言模型在手术安全关键任务中存在预测难以审计、可靠性不足的问题，而准确评估CVS对预防胆管损伤至关重要。

Method: 提出Sum-of-Checks框架：将每个CVS标准分解为专家定义的可推理检查项；LVLM对每项检查输出二元判断及理由；通过固定加权聚合得到标准级分数。在Endoscapes2023基准上测试三个前沿LVLM。

Result: Sum-of-Checks在三个模型和所有CVS标准上，帧级平均精度（mAP）相对最佳基线提升12-14%。LVLM在观察性检查（如可见性、工具遮挡）上可靠，但在决策关键解剖证据上存在显著波动。

Conclusion: 将手术推理结构化为专家对齐的验证检查，可同时提升LVLM的CVS评估准确性和透明性；明确分离证据获取与决策制定是构建可靠、可审计手术AI系统的关键。
...
### [132] [Logistic Bandits with $\tilde{O}(\sqrt{dT})$ Regret without Context Diversity Assumptions](https://arxiv.org/abs/2604.22161)
*Seoungbin Bae,Dabeen Lee*

Main category: cs.LG

TL;DR: 本文提出SupSplitLog算法，通过样本分裂与牛顿一步校正技术，首次在无需上下文多样性假设条件下实现逻辑斯蒂克匪徒问题的$\tilde{\mathcal{O}}(\sqrt{dT})$遗憾界，并在维度依赖性和数据自适应复杂度方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有逻辑斯蒂克匪徒算法依赖严格的上下文多样性假设（如协方差矩阵最小特征值严格正定），这排除了上下文向量集中于低维子空间的情况。作者希望消除这一限制，提出一种无需此类假设仍能获得率最优遗憾界的算法。

Method: 提出SupSplitLog算法，核心是将收集的样本分裂为两个不相交子集：一个用于计算初始点估计量，另一个用于执行牛顿型一步校正过程。通过精心设计分裂规则以平衡初始估计与一步校正的精度需求，从而在无需上下文多样性假设下实现理论保证。

Result: 1) 在无上下文多样性假设下实现$\tilde{\mathcal{O}}(\sqrt{dT})$遗憾界；2) 在维度$d$的依赖程度上严格优于现有算法；3) 可推导出基于数据依赖复杂度量的遗憾界，当上下文集中于低维子空间时避免直接依赖$d$；4) 实验验证了算法的数值优越性。

Conclusion: SupSplitLog是首个在无上下文多样性假设下达到率最优遗憾界的逻辑斯蒂克匪徒算法，通过样本分裂策略巧妙解决了估计精度与假设要求之间的矛盾，并在理论和实验上展示了其优越性。
...
### [133] [Estimating Tail Risks in Language Model Output Distributions](https://arxiv.org/abs/2604.22167)
*Rico Angell,Raghav Singhal,Zachary Horvitz,Zhou Yu,Rajesh Ranganath,Kathleen McKeown,He He*

Main category: cs.LG

TL;DR: 本文提出了一种基于重要性采样的方法，通过构建不安全版本的模型来高效估计语言模型产生有害输出的尾部概率，相比暴力采样方法节省10-20倍样本量，能够有效评估模型在部署中的罕见但高风险行为。


<details>
  <summary>Details</summary>
Motivation: 当前安全评估方法仅关注导致有害输出的输入分布，忽略了模型的概率性和尾部输出行为。模型每天被查询数十亿次时，即使罕见的危险行为也会发生，需要有效估计尾部风险。

Method: 提出基于重要性采样的罕见事件估计方法。核心思想是通过构建目标模型的不安全版本（使有害输出更可能发生），在这些版本上进行高效采样，然后通过重要性权重校正回原分布，从而用更少样本准确估计有害输出的概率。

Result: 在误用和失调基准测试中，该方法使用暴力蒙特卡洛估计10-20倍更少的样本即可达到匹配精度。例如，仅用500个样本即可估计概率为10^-4的有害输出。该方法还能揭示模型对输入扰动的敏感性并预测部署风险。

Conclusion: 准确估计罕见事件对于安全评估既关键又可行，尾部风险应成为模型安全评估的重要组成部分。
...
### [134] [Optimal sequential decision-making for error propagation mitigation in digital twins](https://arxiv.org/abs/2604.22168)
*Annice Najafi,Shokoufeh Mirzaei*

Main category: cs.LG

TL;DR: 本文探讨模块化数字孪生中的误差传播缓解问题，将其建模为序贯决策过程。基于隐马尔可夫模型（HMM）推断的潜在误差状态，构建马尔可夫决策过程（MDP）和部分可观测MDP（POMDP），并通过动态规划与强化学习求解，最终验证MDP策略表现最优，POMDP可恢复约95%性能，量化了信息价值。


<details>
  <summary>Details</summary>
Motivation: 解决模块化数字孪生中误差传播的缓解问题，将其形式化为序贯决策过程，以在系统保真度与维护成本之间取得平衡。

Method: （1）基于HMM学习误差状态及转移矩阵；（2）构建MDP（完全可观）和POMDP（部分可观，利用混淆矩阵作为观测模型）；（3）通过动态规划和Gillespie随机仿真求解；（4）对比Q-learning和REINFORCE两种无模型强化学习算法。

Result: MDP策略获得最高累积奖励和最长的名义运行时间；POMDP在现实观测噪声下恢复约95%的MDP性能；敏感性分析验证了结论的鲁棒性；策略层级间的差距在p<0.001水平上统计显著。

Conclusion: MDP与POMDP之间的性能差距量化了信息的价值，为投资改进分类精度提供了原则性标准；无模型强化学习可在缺乏显式模型时学习有效策略。
...
TL;DR: 本文提出DROL，一种基于潜在条件的一步离线强化学习actor，通过top-1动态路由机制，让actor从潜在先验中采样多个候选动作，将数据集动作分配给最近的候选动作，仅更新胜者，从而在保持单次推理高效的同时，实现局部改进，避免传统一步提取中的妥协问题。


<details>
  <summary>Details</summary>
Motivation: 现有一步离线RL actor将"走向高Q值"和"停留在数据支持的分布内"这两个矛盾目标压缩到同一个样本上，导致当两者冲突时只能妥协，即使附近存在更好的、数据支持的动作用户也无法利用。

Method: DROL使用潜在条件的一步actor，从有界潜在先验中采样K个候选动作，通过动态路由将每个数据集动作分配给最近的候选动作，仅对胜者（winner）使用行为克隆和critic指导进行更新。路由基于当前候选几何重新计算，使区域所有权在学习过程中动态转移。

Result: 在OGBench和D4RL基准上，DROL与一步FQL基线竞争力相当，在OGBench的多个任务组上有提升，同时在AntMaze和Adroit上保持强性能。

Conclusion: DROL通过动态路由机制为一步actor提供了更大的局部改进空间，克服了点式提取难以捕捉的优化问题，同时保持了测试时的单次推理效率。
...
### [136] [Fast Neural-Network Approximation of Active Target Search Under Uncertainty](https://arxiv.org/abs/2604.22254)
*Bilal Yousuf,Zsofia Lendek,Lucian Busoniu*

Main category: cs.LG

TL;DR: 本文提出用卷积神经网络近似主动搜索策略，在保持检测率的同时将在线计算量降低数个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有主动搜索方法（AS/ASI）虽检测准确，但在线优化计算成本高，难以实时应用。

Method: 利用概率假设密度滤波器估计目标数量，训练CNN直接推断AS/ASI决策，输入为多通道网格编码（目标信念、智能体位置、访问历史、边界信息）。

Result: 仿真实验表明，CNN方法在均匀和聚类目标分布下检测率与AS/ASI相当，计算量降低数个数量级。

Conclusion: CNN可有效替代在线优化，大幅提升搜索规划效率，适用于计算资源受限场景。
...
### [137] [Protect the Brain When Treating the Heart: A Convolutional Neural Network for Detecting Emboli](https://arxiv.org/abs/2604.22258)
*Andrea Angino,Ken Trotti,Diego Ulisse Pizzagalli,Rolf Krause,Tiziano Torre,Stefanos Demertzis*

Main category: cs.LG

TL;DR: 本文提出基于2.5D U-Net架构的方法，用于在时空连续数据中分割气体微栓塞（GME），实现对心脏结构性介入术中GME的实时、高精度检测与量化。


<details>
  <summary>Details</summary>
Motivation: 心脏结构性介入术中气体微栓塞（GME）是常见并发症，经胸心脏超声可用于检测GME，但存在操作者依赖性、目标高速运动及背景干扰等问题，导致检测与量化困难。

Method: 采用2.5D U-Net架构，在时空连续数据中对GME进行分割，结合空间与时间信息以实现鲁棒检测和精确分割，并保持实时执行速度。

Result: 该方法实现了对背景的鲁棒检测和高分割精度，同时保持实时执行速度，成功集成到患者监测手术流程中，可提供GME面积随时间的量化信息。

Conclusion: 提出的2.5D U-Net方法有效解决了GME检测中的操作者依赖、高速运动及背景干扰问题，具备实时性能，可应用于临床患者监测手术协议。
...
### [138] [How LLMs Detect and Correct Their Own Errors: The Role of Internal Confidence Signals](https://arxiv.org/abs/2604.22271)
*Dharshan Kumaran,Viorica Patraucean,Simon Osindero,Petar Velickovic,Nathaniel Daw*

Main category: cs.LG

TL;DR: LLM内部存在一个二阶置信架构（PANL信号），该信号独立于生成概率，不仅能检测错误，还能预测模型是否有能力进行自我修正，这为理解LLM的自我纠错机制提供了神经科学视角的解释。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能够检测并纠正自己的错误，但其背后的机制尚不清楚。作者希望从决策神经科学中的二阶置信模型出发，探究LLM内部是否存在独立于生成信号的评估信号来支持错误检测和自我修正。

Method: 采用"验证-修正"范式，在Gemma 3 27B和Qwen 2.5 7B两个模型上，对TriviaQA和MNLI两个任务进行分析。通过测量口头置信度、token对数概率、PANL（答案后换行符）激活表征，并结合因果干预实验来验证二阶架构的预测。

Result: (i) 口头置信度预测错误检测的能力远超token对数概率，排除了一阶解释；(ii) PANL激活比口头置信度本身更能预测错误检测；(iii) PANL能预测模型能纠正哪些错误，而所有行为信号均无法做到。因果干预确认PANL在答案信息被破坏时能挽救错误检测行为。

Conclusion: LLM自然地实现了二阶置信架构，其内部评估信号不仅编码了答案可能错误的信息，还编码了模型是否有能力修复该错误的知识。
...
### [139] [A Brain-Inspired Deep Separation Network for Single Channel Raman Spectra Unmixing](https://arxiv.org/abs/2604.22324)
*Gaoruishu Long,Jinchao Liu,Bo Liu,Jie Liu,Xiaolin Hu*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度分离神经网络（RSSNet）的单通道拉曼光谱解混方法，灵感来源于语音分离技术。该方法能够从单个含噪混合光谱中分解出纯组分光谱，在合成数据集上优于现有方法（>4dB），且仅用合成数据训练即可泛化到真实矿物混合物光谱，为拉曼解混提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 现有拉曼光谱解混方法大多需要多个混合光谱作为输入（超定模型），无法满足开放域/非合作检测场景下仅基于单个含噪混合光谱进行组分识别的需求。稀疏回归虽是唯一能应对该场景的方法，但对噪声容忍度极低，难以实际应用。

Method: 借鉴语音分离思想，提出RSSNet深度分离神经网络。网络以单个混合光谱为输入，直接输出各纯组分光谱。该方法旨在求解欠定系统（成分数量远超观测数量），可从包含数千种物质的库中分解混合光谱。

Result: 在合成数据集上，RSSNet性能优于现有方法超过4dB；仅用合成数据训练的模型成功解混真实矿物粉末混合物光谱，表现出强泛化能力。

Conclusion: 该方法代表了拉曼解混的新范式，为快速检测拉曼混合物提供了新的可能性。
...
### [140] [FETS Benchmark: Foundation Models Outperform Dataset-specific Machine Learning in Energy Time Series Forecasting](https://arxiv.org/abs/2604.22328)
*Marco Obermeier,Marco Pruckner,Florian Haselbeck,Andreas Zeiselmair*

Main category: cs.LG

TL;DR: 本文提出了FETS基准，系统评估了基础模型在能源时间序列预测中的表现。研究发现，基础模型在多个预测场景和9个数据类别上一致优于传统机器学习方法，特别是协变量增强的基础模型表现最佳。模型性能与频谱熵强相关，存在上下文长度饱和效应，且在聚合数据（国家负荷、区域供暖、电网）上表现更优。


<details>
  <summary>Details</summary>
Motivation: 能源时间序列预测对气候中和能源转型至关重要，但现有方法高度依赖于特定数据集，需要大量训练数据，限制了可扩展性，且模型开发和维护成本高。基础模型通过大规模预训练学习通用模式，在多个预测任务中表现优异，但在能源预测领域的应用尚未被充分探索。

Method: 构建FETS基准：(1) 从利益相关者、属性、数据类别三个维度结构化描述能源预测用例；(2) 收集并分析9个数据类别下的54个数据集；(3) 在不同预测设置下，将基础模型与经典机器学习方法进行对比基准测试。

Result: 基础模型在所有设置和数据类别上一致优于经过数据集特定优化的机器学习方法，即使后者在训练期间看到了完整的历史目标数据。协变量增强的基础模型表现最强。预测性能与频谱熵强相关，超过一定上下文长度后性能饱和，在聚合数据（国家负荷、区域供暖、电网）上表现更优。

Conclusion: 基础模型作为可扩展、可泛化的能源领域预测解决方案具有巨大潜力，尤其是在数据受限和隐私敏感的场景下。
...
### [141] [TabSCM: A practical Framework for Generating Realistic Tabular Data](https://arxiv.org/abs/2604.22337)
*Sven Jacob,Bardh Prenkaj,Weijia Shao,Gjergji Kasneci*

Main category: cs.LG

TL;DR: TabSCM是一种混合类型表格数据生成器，结合因果结构发现与条件扩散模型/梯度提升树，在保持因果依赖关系的同时生成高质量合成数据，速度比纯扩散模型快583倍。
...
Method: 首先从CPDAG出发定向边为DAG，然后用KDE或分类频率拟合根节点边际分布，最后按拓扑序使用条件扩散模型（连续变量）和梯度提升树（分类变量）学习结构赋值，通过祖先采样生成数据。

Result: 在7个公开数据集上，TabSCM在统计保真度、下游效用和隐私风险方面匹配或超越GAN、扩散模型和LLM基线，并降低规则违反率。生成速度比纯扩散模型快583倍。

Conclusion: TabSCM在真实性、可解释性和因果合理性之间取得了实用平衡，适用于公平审计和政策模拟。
...
### [142] [A Nationwide Japanese Medical Claims Foundation Model: Balancing Model Scaling and Task-Specific Computational Efficiency](https://arxiv.org/abs/2604.22348)
*Nanae Aratake,Taisei Tosaki,Yuji Okamoto,Eiichiro Uchino,Masaki Nakamura,Nobutomo Matsui,Akiko Hatakama,Yasushi Okuno*

Main category: cs.LG

TL;DR: 本研究评估了结构化医疗基础模型中模型规模与下游任务性能的关系，发现下游性能存在任务依赖的饱和阈值：疾病预测受益于更大模型（32M-101M参数），而药物预测在11M参数即达到饱和，可节省178小时预训练时间。


<details>
  <summary>Details</summary>
Motivation: 在自然语言处理中，缩放定律表明更大模型可预测地降低预训练损失，但在结构化医疗数据（有限词汇、稀疏观测）中，增加模型规模是否能持续改善下游预测尚不明确，因为大多数研究仅评估单一模型规模。

Method: 使用日本全国519家医院理赔数据库中随机抽取的230万患者、32家医院数据，预训练五种规模（2.2M-101M参数）的编码器-only Transformer，用于疾病发病率和药物预测任务。

Result: 下游性能在任务依赖的阈值处饱和：疾病预测受益于更大模型（32M-101M），而药物预测在11M参数即饱和，节省178小时预训练时间。所有任务中最佳模型在PR-AUC上持续优于LightGBM基线。
...
### [143] [SOC-ICNN: From Polyhedral to Conic Geometry for Learning Convex Surrogate Functions](https://arxiv.org/abs/2604.22355)
*Kang Liu,Jianchen Hu*

Main category: cs.LG

TL;DR: 本文提出SOC-ICNN架构，将输入凸神经网络从线性规划推广到二阶锥规划，引入平滑曲率表示能力，在保持前向复杂度不变的前提下显著扩展了表示空间。


<details>
  <summary>Details</summary>
Motivation: 传统ReLU-ICNN等价于线性规划的最优值函数，其表示能力局限于分段线性多面体函数，存在表示瓶颈。

Method: 将底层优化类从LP推广到SOCP，注入半正定曲率和基于欧几里得范数的锥原语，构建SOC-ICNN架构。

Result: 严格证明了SOC-ICNN在不增加前向复杂度渐近阶的前提下，显著扩展了ReLU-ICNN的表示空间；实验表明函数逼近能力大幅提升，下游决策质量具有竞争力。

Conclusion: SOC-ICNN通过引入二阶锥结构有效突破了传统ICNN的表示瓶颈，实现了更强大的表示能力与计算效率的平衡。
...
### [144] [Revisiting Neural Activation Coverage for Uncertainty Estimation](https://arxiv.org/abs/2604.22360)
*Benedikt Franke,Nils Förster,Frank Köster,Asja Fischer,Markus Lange,Arne Raulf*

Main category: cs.LG

TL;DR: 本文提出将神经激活覆盖（NAC）扩展为回归任务中的不确定性估计方法，实验证明其优于MC Dropout等技术。


<details>
  <summary>Details</summary>
Motivation: NAC原本用于分布外检测和泛化，作者希望将其扩展为回归问题中已训练神经网络的不确定性估计技术。

Method: 基于NAC框架，将其改造为适用于回归任务的不确定性估计方法，并与Monte-Carlo Dropout等基线方法进行对比。

Result: NAC不确定性评分比其他技术（如MC Dropout）更具意义和可靠性。

Conclusion: NAC可以有效地作为回归任务中神经网络的不确定性估计方法，优于现有常用技术。
...
### [145] [Robust Fuzzy local k-plane clustering with mixture distance of hinge loss and L1 norm](https://arxiv.org/abs/2604.22405)
*Junjun Huang,Xiliang Lu,Xuelin Xie,Jerry Zhijian Yang*

Main category: cs.LG

TL;DR: 本文提出了一种鲁棒模糊局部k平面聚类方法（RFLkPC），通过结合Hinge损失和L1范数的混合距离，并假设每个平面簇有界，有效解决了传统K平面聚类对异常值敏感和簇无限延伸的问题，在模拟和真实数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统K平面聚类（KPC）和模糊KPC模型存在两个主要缺陷：一是假设数据点到平面法向量的投影距离服从L2距离，导致对异常值高度敏感；二是假设簇无限延伸，损害了聚类性能。因此，需要一种更鲁棒的聚类方法来解决这些问题。

Method: 提出鲁棒模糊局部k平面聚类（RFLkPC）方法，核心创新包括：（1）结合Hinge损失和L1范数的混合距离度量；（2）假设每个平面簇有界（有限区域），而非无限延伸；（3）在此基础上构建模型并设计相应的优化算法。

Result: 在模拟数据和真实数据上，与多种相关模型进行大量对比实验，验证了RFLkPC方法的有效性和优越性。

Conclusion: RFLkPC方法能够灵活且鲁棒地处理带或不带异常值的平面聚类任务，代码已开源。
...
### [146] [Hidden Failure Modes of Gradient Modification under Adam in Continual Learning, and Adaptive Decoupled Moment Routing as a Repair](https://arxiv.org/abs/2604.22407)
*Yuelin Hu,Zhenbo Yu,Zhengxue Cheng,Wei Liu,Li Song*

Main category: cs.LG

TL;DR: 本文揭示了持续学习方法（如梯度投影、惩罚重缩放、回放混合）与Adam优化器组合时的隐藏失败模式：在非自适应持续语言模型中，投影方法几乎退化为普通遗忘（12.5-12.8 vs 13.2），而自适应解耦路由保持稳定（9.4）。原因是投影操作导致Adam第二动量路径下旧方向有效学习率膨胀1/(1-α)倍。通过仅将修改梯度路由到一阶矩并保持二阶矩统计量真实，加上重叠感知的自适应强度，可避免崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法将Adam视为中性后端，忽略了梯度修改与Adam二阶动量的交互可能导致隐藏的失败模式，尤其在非自适应、高重叠的持续语言模型场景中。

Method: 提出仅将修改后的梯度路由到Adam的一阶矩，同时保持二阶矩统计量忠实反映真实梯度幅度，并引入重叠感知的自适应强度。

Result: 在8域持续LM中，自适应解耦路由稳定在9.4（优于vanilla 3.8单位），而投影方法接近遗忘（12.5-12.8）。在16域流中，优势扩大到4.5-4.8单位。在7B规模LoRA下同样有效。

Conclusion: 梯度修改与Adam二阶动量的冲突是持续学习中的一个普遍失败模式，所提修复是唯一一致避免崩溃的配置。
...
### [147] [Distance-Misaligned Training in Graph Transformers and Adaptive Graph-Aware Control](https://arxiv.org/abs/2604.22413)
*Qinhan Hou,Jing Tang*

Main category: cs.LG

TL;DR: 本文通过合成节点分类基准研究了图Transformer中距离偏差（长程vs局部通信）与任务需求不匹配导致的失败模式，发现任务局部性决定最佳偏差，且离线自适应控制器能显著改善性能，但零差距控制器效果有限，表明距离感知诊断和控制目标设计的重要性。


<details>
  <summary>Details</summary>
Motivation: 图Transformer能够全局混合信息，但这种灵活性也带来了失败模式：某些任务需要长程通信，而另一些任务更适合局部交互。作者希望系统性地研究这一现象，即标签相关信息所在位置与模型在图上分配通信距离之间的不匹配问题。

Method: 在上下文随机块模型图上构建合成节点分类基准，标签由可控的局部信号和远壳信号混合生成。定义了"距离错位训练"作为标签相关信息与模型通信分配之间的不匹配。设计了两种控制器：离线自适应控制器（有任务侧距离目标）和零差距控制器（任务不可知）。

Result: 1）图距离偏差偏好随任务局部性系统性变化；2）离线自适应控制器（有距离目标）几乎匹配各机制下的最佳固定偏差，在混合和局部任务上相比中性基线显著改进；3）任务不可知的零差距控制器效果较弱。

Conclusion: 距离分辨诊断有助于理解图Transformer的失败模式，设计图感知的控制信号很重要——仅靠自适应不够，控制目标本身也很关键。
...
### [148] [From Local to Cluster: A Unified Framework for Causal Discovery with Latent Variables](https://arxiv.org/abs/2604.22416)
*Zongyu Li*

Main category: cs.LG

TL;DR: 本文提出L2C框架，通过局部结构学习自动发现变量聚类，并在聚类层面进行因果推断，无需因果充分性假设。


<details>
  <summary>Details</summary>
Motivation: 现有局部方法只关注直接邻居，缺乏宏观视角；聚类方法要么需要预知聚类，要么假设因果充分性。直接对聚类变量应用单变量因果发现会违反因果充分性，导致错误结果。

Method: 提出L2C框架：利用聚类归约定理将任意聚类归约为最多三个节点；应用局部因果发现识别存在潜变量时的直接原因、效应和V结构；通过聚类层面演算进行宏观因果推断。

Result: 理论分析证明L2C具有可靠性、原子完备性和计算效率；合成和真实数据实验表明L2C准确恢复真实聚类，并在宏观因果效应识别上优于现有基线方法。

Conclusion: L2C提供了一个从局部到聚类的统一因果抽象框架，无需因果充分性假设，自动发现变量聚类，实现高效的宏观因果推理。
...
### [149] [Beyond Land Surface Temperature: Explainable Spatial Machine Learning Reveals Urban Morphology Effects on Human-Centric Heat Stress](https://arxiv.org/abs/2604.22433)
*Yuan Wang,Shengao Yi,Xiaojiang Li,Pengyuan Liu,Zhiwei Yang,Ronita Bardhan,Rudi Stouffs*

Main category: cs.LG

TL;DR: 该研究在新加坡建立“建模-比较-评估”框架，系统比较了地表温度（LST）与生理相关热指数（UTCI）在空间格局和驱动因素上的差异，发现两者存在显著不一致，其中天空视角因子对UTCI影响大而对LST影响小，且高反照率反而增加UTCI，为气候适应性城市规划提供了新依据。


<details>
  <summary>Details</summary>
Motivation: 现有城市规划研究多依赖地表温度（LST）来评估热暴露，但LST能否充分代表人体实际热应激尚不清楚，且二者与城市因素的差异机制缺乏系统研究。

Method: 采用Landsat 30米LST和GPU加速的1米UTCI数据，建立“建模-比较-评估”框架；利用地理加权XGBoost（GW-XGBoost）和广义加性模型（GAM）分析非平稳和阈值效应关系。

Result: LST与UTCI在空间格局上存在显著差异；GW-XGBoost模型全局R²分别为0.855（LST）和0.905（UTCI）；天空视角因子对UTCI贡献大但对LST贡献小；高反照率与UTCI升高相关。

Conclusion: LST无法充分捕捉影响人体实际热应激的遮阴和辐射过程，需整合生理相关热指数以指导热风险管理和气候适应性城市规划。
...
### [150] [HubRouter: A Pluggable Sub-Quadratic Routing Primitive for Hybrid Sequence Models](https://arxiv.org/abs/2604.22442)
*Abhinaba Basu*

Main category: cs.LG

TL;DR: HubRouter提出用O(nM)的hub路由机制替代O(n²)的自注意力层，通过可学习的hub token实现稀疏注意力，在多种架构中验证了其效率与质量权衡。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer的O(n²)自注意力机制在处理长序列时计算开销过大，作者希望设计一种可插拔的替代方案，在不显著牺牲质量的前提下大幅提升训练吞吐量。

Method: HubRouter采用"编码-解码-打分-议会"四阶段流程：(1) M个可学习hub token对所有token做交叉注意力；(2) token投影到hub空间生成路由指纹；(3) 选择头选出top-k token；(4) 稀疏议会仅关注选中的子集。支持因果路由（Hub-GPT）和双向路由（Hub-Jamba）。

Result: (1) Hub-Jamba在序列长度1024下达到约90x训练吞吐量（优化后基线约10-15x），PPL微升4.2%（可能处于种子噪声范围内）；(2) 混合25% HubRouter层在匹配预算下PPL最优（268.0 vs 282.4）；(3) Hub-GPT因果路由PPL 211.5，比Jamba差约3 PPL；(4) hub数量M=8-14为可靠收敛区间。

Conclusion: HubRouter是O(n²)注意力的有效替代方案，在大幅提升效率的同时保持可接受的质量。双向路由（Jamba风格）表现优于严格因果路由。最佳hub数量为8-14，过大或过小都影响稳定性。
...
### [151] [Towards Adaptive Continual Model Merging via Manifold-Aware Expert Evolution](https://arxiv.org/abs/2604.22464)
*Haiyun Qiu,Xingyu Wu,Kay Chen Tan*

Main category: cs.LG

TL;DR: 提出MADE-IT方法，通过流形感知的动态专家演化和隐式路由机制，解决了持续模型融合中的饱和-冗余困境，在多个任务序列上实现了更优的准确率和鲁棒性，同时大幅精简冗余专家。


<details>
  <summary>Details</summary>
Motivation: 现有CMM方法面临饱和-冗余困境：骨干网络方法受限于固定容量，参数饱和导致表示干扰；MoE变体随意扩展带来专家冗余和路由瓶颈（依赖额外数据优化）。亟需一种自适应、轻量且无需额外数据驱动的专家管理与激活机制。

Method: 1) 基于投影的子空间亲和度指标 + 分布感知自适应阈值机制，引导专家自主演化，平衡多样性与结构精简；2) 数据无关、无需训练的隐式路由机制，通过特征-子空间对齐激活专家，避免参数化门控网络。

Result: 在长序列和乱序任务序列上，MADE-IT在准确率和鲁棒性上持续优于强基线，同时显著剪枝冗余专家（尤其在通用模块和浅层网络中）。

Conclusion: MADE-IT通过流形几何指导的专家动态演化与隐式路由，有效解决了持续模型融合中的核心矛盾，实现了性能与效率的双赢。
...
### [152] [Deep Learning for Model Calibration in Simulation of Itaconic Acid Production](https://arxiv.org/abs/2604.22496)
*Daria Fokina,Marco Baldan,Constantin Romankiewicz,Wolfgang Laudensack,Roland Ulber,Michael Bortz*

Main category: cs.LG

TL;DR: 本研究利用深度学习估算衣康酸生产中的动力学参数，比较了直接深度学习（DDL）和条件流匹配（CFM）两种策略。结果表明，CFM在准确性和泛化能力上优于DDL，能可靠预测不同操作条件和规模下的系统行为。


<details>
  <summary>Details</summary>
Motivation: 传统非线性回归方法在动态生物过程建模中参数估计困难，亟需更灵活、数据高效的方法来预测不同操作条件和规模下的系统行为。

Method: 采用两种深度学习策略（直接深度学习DDL和生成式条件流匹配CFM）估算动力学参数，并以非线性回归作为基准方法进行比较。实验基于不同搅拌速度和反应器规模的真实批次数据。

Result: CFM比DDL更准确，浓度预测与非线性回归结果高度吻合；DDL偏差较大。在放大实验中，CFM的泛化能力和鲁棒性均优于DDL。

Conclusion: CFM能够可靠预测不同操作条件和规模下的系统行为，是一种灵活且数据高效的动态生物过程模型参数估计方法。
...
### [153] [Decoding High-Dimensional Finger Motion from EMG Using Riemannian Features and RNNs](https://arxiv.org/abs/2604.22499)
*Martin Colot,Cédric Simar,Guy Cheron,Ana Maria Cebolla Alvarez,Gianluca Bontempi*

Main category: cs.LG

TL;DR: 本文提出了一个端到端的EMG-to-kinematics回归框架，结合8通道EMG臂环、单摄像头和自动同步流程，构建了EMG-FK数据集（10小时，20名参与者的15个手指关节角度），并设计了Temporal Riemannian Regressor (TRR)模型。TRR利用多频段黎曼协方差特征的GRU网络，在EMG-FK和emg2pose基准上均优于现有方法，实现了实时部署（Raspberry Pi 5上约10预测/秒），为高维手指运动解码提供了低门槛、实时的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖分类式机器学习，限制了可控自由度且牺牲了自然交互。本文旨在实现高维手指运动学从前臂表面EMG信号的连续估计，以支持更自然的假手控制、AR/XR交互和遥操作。

Method: 1) 构建EMG-FK数据集：使用8通道EMG臂环+单摄像头+自动同步流程采集20名参与者10小时数据，包含15个手指关节角度。2) 提出Temporal Riemannian Regressor (TRR)：基于GRU的轻量级模型，利用多频段黎曼协方差特征解码手指运动。

Result: 在EMG-FK上，TRR在受试者内评估中达到9.79°±1.47的平均绝对误差，在跨受试者评估中达到16.71°±3.97。在Raspberry Pi 5上实现约10次预测/秒的实时部署，速度比现有方法快约一个数量级。

Conclusion: 本文降低了可复现、实时的EMG高维手指运动解码的门槛，为基于EMG的嵌入式系统实现更自然直观的控制铺平了道路。
...
### [154] [FeatEHR-LLM: Leveraging Large Language Models for Feature Engineering in Electronic Health Records](https://arxiv.org/abs/2604.22534)
*Hojjat Karami,David Atienza,Jean-Philippe Thiran,Anisoara Ionescu*

Main category: cs.LG

TL;DR: FeatEHR-LLM利用大语言模型基于数据集模式生成可执行的特征提取代码，在不访问原始患者数据的情况下处理不规则采样的电子健康记录时序数据，在8个临床预测任务中7个达到最高AUROC。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录的特征工程面临不规则观测间隔、可变测量频率和结构性稀疏等挑战，现有自动化方法要么缺乏临床领域知识，要么假设输入数据规则采样，难以适用于真实世界的非规则EHR数据。

Method: 利用大语言模型(LLM)仅基于数据集模式和任务描述（而非原始患者记录）生成临床有意义的表格特征；通过工具增强生成机制，为LLM配备专门处理不规则时序数据的查询例程；采用迭代验证循环的流程，支持单变量和多变量特征生成。

Result: 在4个ICU数据集的8项临床预测任务中，FeatEHR-LLM在7项任务上取得最高的平均AUROC，相比强基线最高提升6个百分点。

Conclusion: FeatEHR-LLM有效解决了不规则EHR时序数据的特征工程难题，在不暴露患者隐私的前提下利用LLM的临床知识生成可解释、可执行的特征提取代码，显著提升了临床预测性能。
...
### [155] [An Integrated Framework for Explainable, Fair, and Observable Hospital Readmission Prediction: Development and Validation on MIMIC-IV](https://arxiv.org/abs/2604.22535)
*Isaac Tosin Adisa*

Main category: cs.LG

TL;DR: 本文提出并验证了一个集成框架，解决临床转译读入院预测的三大障碍：缺乏可解释性、缺乏部署可靠性基础设施、以及人口统计学公平性评估不足。基于MIMIC-IV数据库41.5万成人入院记录，使用逻辑回归、XGBoost和LightGBM模型，XGBoost达到AUC-ROC 0.696，所有亚组均满足公平性阈值。


<details>
  <summary>Details</summary>
Motivation: 现有读入院预测模型在临床部署中面临三个主要障碍：模型不可解释（黑箱问题）、缺乏可靠的部署基础设施、以及对不同人口统计学亚群的公平性评估不足。本文旨在构建一个同时解决这三个问题的集成框架，推动模型从研究走向临床实际应用。

Method: 使用MIMIC-IV数据库构建415231例成人入院队列（30天再入院率18.0%），按70/15/15划分训练/验证/测试集。训练逻辑回归、XGBoost和LightGBM三种模型，使用26个特征。采用SHAP提供个体层面的可解释性，在16个亚组中使用AUC-ROC、假阴性率（FNR）和阳性预测值（PPV）评估公平性，用Brier分数和校准曲线评估校准度。

Result: XGBoost达到AUC-ROC 0.696（95% CI 0.691-0.701），优于或匹敌LACE基线（AUC 0.60-0.68）。LightGBM校准度最佳（Brier 0.146）。既往入院次数是最主要的预测因子。所有亚组均满足公平性阈值（delta AUC <= 0.05, delta FNR <= 0.10）。

Conclusion: 该框架在性能、临床可解释解释性和人口统计公平性方面均表现良好，代码已开源，为临床读入院预测的实际部署提供了可行方案。
...
### [156] [On the Properties of Feature Attribution for Supervised Contrastive Learning](https://arxiv.org/abs/2604.22540)
*Leonardo Arrighi,Julia Eva Belloni,Aurélie Gallet,Ivan Gentile,Matteo Lippi,Marco Zullich*

Main category: cs.LG

TL;DR: 本文实证证明，使用监督对比学习（SCL）训练的神经网络在特征归因可解释性方面（忠实性、复杂性和连续性）优于使用交叉熵（CE）训练的网络，为追求更透明、可信的模型提供了指导。


<details>
  <summary>Details</summary>
Motivation: 传统交叉熵损失函数在安全关键场景中存在对抗鲁棒性和分布外检测方面的不足，而监督对比学习虽在这些方面表现更优，但其对模型可解释性的影响尚未被充分研究。本文旨在填补这一空白。

Method: 通过实证研究，对比使用监督对比学习（SCL）和交叉熵（CE）训练的神经网络，在图像分类任务上评估特征归因解释的质量，考察忠实性、复杂性和连续性三个维度。

Result: 监督对比学习（SCL）训练的网络在特征归因解释的忠实性、复杂性和连续性三个指标上均优于交叉熵（CE）训练的网络。

Conclusion: 监督对比学习不仅能提升模型的对抗鲁棒性和分布外检测能力，还能产生更高质量的特征归因解释，使其成为追求模型透明度和可信任度的更优训练目标选择。
...
### [157] [SOLAR-RL: Semi-Online Long-horizon Assignment Reinforcement Learning](https://arxiv.org/abs/2604.22558)
...
TL;DR: 提出SOLAR-RL框架，通过从静态数据中重构轨迹、检测首次失败点并分配密集步骤级奖励，在无需在线交互的情况下模拟在线反馈，有效提升了GUI智能体的长程任务完成率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在GUI智能体训练中面临两难：离线RL依赖静态步骤级数据，忽略全局轨迹语义（如任务完成度、执行质量）；在线RL虽能捕捉长期动态，但交互成本高且环境不稳定。作者希望弥合这一差距。

Method: SOLAR-RL框架包含三个关键步骤：1) 从静态数据中重建多样化rollout候选项；2) 利用逐步骤有效性信号检测首次失败点；3) 结合目标对齐的奖励塑形，反向分配稠密的步骤级奖励，以反映轨迹级执行质量，从而在不产生交互成本的情况下模拟在线反馈。

Result: 实验表明，SOLAR-RL在长程任务完成率和鲁棒性上显著优于强基线方法，是一种样本高效的自主GUI导航解决方案。

Conclusion: SOLAR-RL通过半在线长程分配强化学习，成功将全局轨迹信息融入离线学习过程，有效解决了离线RL忽视全局语义与在线RL成本过高之间的矛盾，为GUI智能体训练提供了新的高效范式。
...
TL;DR: 本文提出基于矩阵谱熵的无数据客户端贡献估计方法，通过最后一层更新的熵值衡量信息多样性，设计了SpectralFed（归一化熵作聚合权重）和SpectralFuse（熵+类对齐+卡尔曼滤波）两种方案，在多个非IID基准上验证了与客户端准确率的高相关性。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中需要评估客户端贡献以进行公平奖励，但现有方法依赖服务端验证数据或客户端自报信息，存在隐私泄露或易被操纵的问题。

Method: 利用最后一层更新的矩阵冯·诺依曼（谱）熵作为无数据信号衡量信息多样性，提出两种方案：(i) SpectralFed——归一化熵作为聚合权重；(ii) SpectralFuse——融合熵与类别对齐，通过秩自适应卡尔曼滤波实现每轮稳定性。

Result: 在CIFAR-10/100、FEMNIST和FedISIC基准上，熵得分与客户端独立准确率在多种非IID场景下保持高相关性，优于现有无数据贡献估计基线。

Conclusion: 谱熵可作为客户端贡献的有效无数据指标，无需验证数据或客户端元数据，兼顾隐私与鲁棒性。
...
### [159] [SpikingBrain2.0: Brain-Inspired Foundation Models for Efficient Long-Context and Cross-Platform Inference](https://arxiv.org/abs/2604.22575)
...
TL;DR: SpikingBrain2.0 (SpB2.0) 是一个5B参数规模的长上下文脉冲神经网络模型，通过双空间稀疏注意力架构和优化训练策略，在保持性能的同时实现超长序列高效推理，支持超千万token处理，并在FP8 GPU和神经形态硬件上均有显著加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文全注意力Transformer面临计算和推理瓶颈，亟需设计既能保持性能又能在低训练开销下实现长上下文高效处理的基座模型。

Method: 提出双空间稀疏注意力（DSSA），混合稀疏Softmax注意力（MoBA）和稀疏线性注意力（SSE）；支持INT8脉冲编码（事件驱动）和FP8编码（GPU加速）双量化路径；开发优化后的Transformer-to-Hybrid (T2H)训练流水线，支持LLM和VLM双转换路径。

Result: SpB2.0-5B在不到7k A100 GPU小时的训练下恢复了Qwen3-4B基座Transformer的大部分能力；4M上下文时TTFT加速10.13倍；8张A100上支持超1000万token（全注意力模型内存溢出）；FP8推理在250k上下文时加速2.52倍；神经形态执行稀疏度64.31%，功耗降低46.5%。

Conclusion: SpikingBrain2.0为轻量级、多模态脉冲基座模型提供了实用路径，展示了脑启发机制与高效架构结合在资源受限和边缘场景中的潜力。
...
### [160] [Adaptive Head Budgeting for Efficient Multi-Head Attention](https://arxiv.org/abs/2604.22583)
*Bilal Faye,Abdoulaye Mbaye,Hanane Azzag,Mustapha Lebbah*

Main category: cs.LG

TL;DR: BudgetFormer提出了一种自适应多头注意力机制，根据输入动态调整注意力头数量和选择最相关的头，在文本分类任务中降低了计算成本并提升了性能。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer对所有输入使用固定数量的注意力头，但在粗粒度任务（如文本分类）中，全局信息往往不需要全部注意力头，导致不必要的计算开销或次优性能。

Method: 提出BudgetFormer架构，包含：(1) 学习每个输入的head budget（所需注意力头数量）和relevance distribution（选择最信息量的头）；(2) 基于探索-利用权衡的训练策略，先探索有效配置再收敛到高效模式。

Result: 在多种复杂度的文本分类任务上，BudgetFormer降低了FLOPs和内存的推理成本，同时性能可超越标准全多头注意力。

Conclusion: 自适应头分配是一种能够同时提升Transformer模型效率和效果的原则性方法。
...
### [161] [Beyond Patient Invariance: Learning Cardiac Dynamics via Action-Conditioned JEPAs](https://arxiv.org/abs/2604.22618)
*Jose Geraldo Fernandes,Luiz Facury,Pedro Robles Dutenhefner,Wagner Meira*

Main category: cs.LG

TL;DR: 本文提出基于事件条件世界模型的医学时序自监督学习框架，通过预测疾病状态转换来解耦正常解剖与病理动态，在MIMIC-IV-ECG数据集上超越全监督方法，并在低资源场景下AUROC提升超0.05。


<details>
  <summary>Details</summary>
Motivation: 现有医疗自监督学习广泛采用不变性目标，最大化同一患者不同视图的相似性。虽然对静态解剖有效，但该范式在数学上迫使模型抑制其本应检测的短暂病理变化，与临床诊断的根本目标不一致。

Method: 将自监督学习从不变性目标转向事件条件世界模型（Action-Conditioned World Models）。借鉴LeJEPA框架，将病理定义为作用于患者潜在状态的状态转移向量，通过学习预测疾病发生后的未来电生理状态，显式解耦稳定解剖特征与动态病理驱动力。

Result: 在MIMIC-IV-ECG数据集上，该方法在关键分诊任务中超越全监督基线；在低资源场景下，世界模型相比监督学习AUROC提升超过0.05。

Conclusion: 对生物动力学建模提供了比静态分类更鲁棒的密集监督信号，验证了事件条件世界模型在医学时序分析中的有效性。
...
### [162] [Associativity-Peakiness Metric for Contingency Tables](https://arxiv.org/abs/2604.22655)
...
<details>
  <summary>Details</summary>
Motivation: 现有的聚类算法性能评估指标多针对真值向量与预测向量对，无法揭示列联表中蕴含的细粒度特征，而公开文献中缺乏直接评估列联表的单一性能指标。

Method: 提出了Associativity Peakiness (AP)指标，用于刻画聚类算法在列联表上的关键性能特征，并通过500个列联表的模拟实验进行验证。

Result: AP指标在评估聚类算法时具有比公开指标更高的动态范围，且计算效率更高。

Conclusion: AP指标为聚类算法的列联表性能评估提供了有效且高效的解决方案，弥补了现有评估体系的空白。
...
### [163] [Zero-Shot Morphological Discovery in Low-Resource Bantu Languages via Cross-Lingual Transfer and Unsupervised Clustering](https://arxiv.org/abs/2604.22723)
*Hillary Mutisya,John Mugane*

Main category: cs.LG

TL;DR: 提出了一种结合跨语言迁移学习与无监督聚类的方法，用于低资源班图语言的形态特征发现。在仅有91个标注范式的Giriama语上，该方法发现了名词类分配、两个未报道的形态模式（a-前缀变体与缩略k'-前缀），并实现了高精度分词和词形还原。


<details>
  <summary>Details</summary>
Motivation: 低资源班图语言（如Giriama语）缺乏标注数据，现有方法难以有效发现其形态特征，亟需一种不依赖大量标注数据的方法来支持语言文档化。

Method: 跨语言迁移学习（从斯瓦希里语迁移）与无监督聚类相结合，通过加权投票集成；迁移学习利用约60%词汇重叠进行同源词检测，聚类则发现语言特有创新。

Result: 发现Class 2的a-前缀变体（元音合并，95.1%一致性）和缩略k'-前缀（98.5%一致性）；在444个已知动词范式上词形还原准确率78.2%；语料库扩展至19,624词（9,014个独特词元），分词率97.3%，词形还原率86.7%。

Conclusion: 跨语言迁移与无监督聚类互补性良好，能有效发现低资源班图语言的形态特征，并发布了代码和发现的词汇资源以支持后续文档化工作。
...
TL;DR: 本文通过统一框架比较了8种Shapley值变体，在低延迟风险场景下进行大规模实证评估（含专业分析师3,735次案例审查），发现标准量化指标与人类感知的清晰度和决策效用脱节，且解释虽提升决策信心却未改善客观绩效，存在自动化偏见风险。


<details>
  <summary>Details</summary>
Motivation: Shapley值在可解释AI中虽有多种变体，但缺乏关于其实际人类效用的共识，现有评估依赖未经验证的量化代理指标。

Method: 采用统一摊销框架隔离8种Shapley变体的语义差异，在低延迟风险工作流约束下，跨4个风险数据集及模拟真实欺诈检测环境（含专业分析师与3,735次案例审查）进行大规模实证评估。

Result: 标准量化指标（稀疏性、忠实性）与人类感知清晰度及决策效用脱节；无变体提升分析师客观表现，但所有解释均显著增加了决策信心，揭示高 stakes 环境下的自动化偏见风险。

Conclusion: 当前评估代理指标不足以预测下游人类影响，研究为高风险操作决策系统中Shapley变体的选择与评估提供基于证据的指导。
...
### [165] [Neural Recovery of Historical Lexical Structure in Bantu Languages from Modern Data](https://arxiv.org/abs/2604.22730)
*Hillary Mutisya,John Mugane*

Main category: cs.LG

TL;DR: 本文训练了一个基于班图形态学范式的Transformer模型（BantuMorph v7），仅使用现代形态数据就能恢复跨语言的词汇结构，且与历史重建的原始班图语形式高度一致。在14种东、南非班图语言中，模型识别出的前11个名词同源候选中有10个（90.9%）与已重建的原始班图语形式吻合，动词同源候选也有12个与重建词根对齐。跨模型验证和名词类别分析进一步证实了模型恢复的班图词汇结构与已知的Guthrie区域分类一致。


<details>
  <summary>Details</summary>
Motivation: 探究仅依靠现代形态学数据训练的神经网络模型，是否能够恢复与历史语言学重建一致的跨语言词汇结构，从而评估模型在历史语言学研究中的潜力。

Method: 使用BantuMorph v7（基于班图形态范式的Transformer）处理14种东、南非班图语言，提取名词和动词词干的编码器嵌入，识别出728个名词和1,525个动词同源候选（跨5种以上语言）。通过与BLR3数据库（4,786个重建原始班图语形式）和ASJP基础词汇表对比验证，并使用NLLB-600M翻译模型进行跨模型验证。

Result: 前11个名词同源候选中有10个（90.9%）与重建的原始班图语形式对齐，包括*-ntU '人'（8种语言）、*gombe '牛'（9种语言）等。12个动词同源候选与重建词根对齐，包括*-bon- '看'和*-jIm- '站'。跨模型验证支持Guthrie区域分类（p<0.01），所有13个产出性名词类别的跨语言余弦相似度均>0.83。

Conclusion: 仅使用现代形态数据训练的神经网络能够有效恢复与历史重建一致的班图词汇结构，但受限于仅涵盖东、南非班图语言，结果应解读为恢复共享的班图词汇结构，而非明确区分原始班图语保留特征与后来的区域创新。
...
### [166] [Iterative Model-Learning Scheme via Gaussian Processes for Nonlinear Model Predictive Control of (Semi-)Batch Processes](https://arxiv.org/abs/2604.22672)
*Tai Xuan Tan,Alexander Mitsos,Eike Cramer*

Main category: cs.LG

TL;DR: 本文提出一种基于高斯过程（GP）的模型学习非线性模型预测控制（GP-MLMPC）方案，用于批次过程。该方法仅需一条初始轨迹数据即可启动，通过迭代运行批次并更新GP模型，在无需机理模型的情况下实现了快速收敛。在半间歇聚合反应器上的仿真表明，跟踪误差在4次迭代后降低83%，经济目标下最终产品质量在8次迭代后提升17倍，性能与全模型NMPC相当。


<details>
  <summary>Details</summary>
Motivation: 批次过程具有固有的瞬态和非线性特性，适合采用非线性模型预测控制（NMPC），但NMPC的采用受限于动态模型的成本高和难以获取。因此，作者希望提出一种无需机理模型、数据高效的控制方法。

Method: 提出GP-MLMPC方案：1）从单一初始轨迹（如PI控制器数据）初始化GP模型；2）在每个批次迭代中，利用嵌入GP的NMPC运行批次，并用新观测数据更新GP；3）利用GP的不确定性量化构建机会约束，确保运行安全。

Result: 在半间歇聚合反应器仿真中：跟踪目标下，4次迭代后跟踪误差较初始轨迹降低83%；经济目标下，8次迭代后最终产品质量提升17倍。两种情况下，性能均与全模型NMPC相当。

Conclusion: GP-MLMPC是一种有前景的数据高效控制方案，无需机理知识即可实现非线性批次过程的最优控制，样本效率高且收敛快。
...
### [167] [Operational Feature Fingerprints of Graph Datasets via a White-Box Signal-Subspace Probe](https://arxiv.org/abs/2604.22676)
*Yuchen Xiong,Swee Keong Yeap,Zhen Hong Ban*

Main category: cs.LG

TL;DR: WG-SRC是一种白盒信号子空间探针，通过固定图信号字典和封闭形式线性分类器，在保持竞争力的同时提供可解释的节点分类诊断，并能生成操作级特征指纹来指导数据集特定修改。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在节点分类中取得了良好性能，但其消息传递机制将自身属性、邻域平滑、高通图差异、类别几何和分类器边界纠缠在不可解释的表示中，无法解释节点为何被分类以及数据集需要何种图学习机制。

Method: WG-SRC采用固定的命名图信号字典（原始特征、行归一化和对称归一化低通传播、高通图差异），结合Fisher坐标选择、类别PCA子空间、封闭形式多α岭分类和基于验证的分数融合，实现白盒预测与分析。

Result: 在六个节点分类数据集上，WG-SRC与复现的图基线方法竞争力相当且在一致划分下取得正向平均增益；其生成的图谱将行为分解为原始特征、低通、高通、类别几何和岭边界等操作级特征指纹，区分了不同数据集的主导机制。

Conclusion: WG-SRC作为白盒图学习仪器，通过预测性能验证诊断结果，其生成的指纹可作为事后评估指导，支持对齐的机制性干预（如识别可移除噪声的高通块、应保留的原始特征、需要岭边界校正的场景）。
...
### [168] [Spend Less, Fit Better: Budget-Efficient Scaling Law Fitting via Active Experiment Selection](https://arxiv.org/abs/2604.22753)
*Sijie Li,Shanda Li,Haowei Lin,Weiwei Sun,Ameet Talwalkar,Yiming Yang*

Main category: cs.LG

TL;DR: 本文提出将标度律拟合视为预算感知的序贯实验设计问题，通过仅使用约10%的训练预算即可达到接近全实验集性能的效果。


<details>
  <summary>Details</summary>
Motivation: 标度律拟合本身成本高昂（可达数百万美元），传统的穷举式实验设计在预算有限时效率极低，需要一种预算感知的实验选择方法。

Method: 将标度律拟合形式化为预算感知的序贯实验设计问题，提出一种不确定性感知的方法，序贯地将实验预算分配到对目标区域外推最有用的实验上。

Result: 在多个标度律任务基准上，该方法一致优于经典实验设计基线，仅使用约10%的训练预算即可达到接近全实验集拟合的性能。

Conclusion: 提出的预算感知序贯实验设计方法能显著降低标度律拟合成本，为大规模训练预算分配提供高效解决方案。
...
TL;DR: 本文提出ResRank，通过Encoder-LLM将候选项压缩为单个嵌入，结合残差连接和余弦相似度评分机制，在保持或提升排序效果的同时，彻底解决了LLM列表重排序中的"中间丢失"和推理延迟问题。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的列表重排序虽效果好，但存在两个瓶颈：(1)输入变长时的"中间丢失"现象损害排序质量；(2)推理延迟随序列长度超线性增长，难以工业部署。

Method: (1)采用Encoder-LLM将每个候选段落压缩为单个嵌入，与查询文本一同输入Reranker-LLM；(2)引入残差连接结构，融合编码器嵌入与重排序器的上下文隐藏状态；(3)用一步余弦相似度评分机制替代自回归解码；(4)设计双阶段、多任务、端到端联合优化策略。

Result: 在TREC Deep Learning和8个BEIR基准数据集上，ResRank实现了与现有方法竞争或更优的排序效果，同时零生成token、每个段落仅处理一个token，平衡了效果与效率。

Conclusion: ResRank通过压缩-残差-评分三管齐下的设计，从根本上解决了LLM重排序的效率和效果矛盾，为工业部署提供了可行方案。
...
TL;DR: 本文质疑LLM与协同表示在推荐系统中的全局对齐假设，提出两者是部分共享、本质异质的视图，应当采用互补性融合而非强制对齐的设计原则。


<details>
  <summary>Details</summary>
Motivation: 现有范式默认语义嵌入与协同表示共享潜在实体，且认为更强的对齐带来更好效果。作者认为这一假设过强且与真实推荐场景结构不匹配，需要重新审视。

Method: 提出全局低复杂度对齐假设的形式化定义；开发互补性感知的诊断工具（量化重叠度、唯一命中贡献、理论融合上界）；在稀疏推荐基准上开展实证分析和对齐探测实验。

Result: 发现语义视图与协同视图在项目级一致性较低，存在显著的神谕融合增益；低容量映射只能捕获共享成分，无法恢复完整协同几何结构，尤其在分布漂移下表现更差。

Conclusion: 对齐不应被视为默认的融合原则，应转向保留私有信号的互补性融合设计，为下一代LLM增强推荐系统提供原则性基础。
...
TL;DR: 本文从理论上揭示了基于GRPO的LLM推荐优化与AUC最大化之间的等价关系，并证明采用beam-search负样本可将优化目标重塑为partial AUC，从而更好地对齐Top-K推荐。据此提出WPAUC指标及高效优化方法TAWin，在四个真实数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 虽然beam-search负样本在LLM推荐中表现优于随机负样本，但其背后的理论机制尚不明确。本文旨在填补这一理论空白，并据此设计更优的推荐优化方法。

Method: 从理论上分析GRPO在二元反馈下的优化目标等价于最大化AUC；证明beam-search负样本将目标重塑为partial AUC，更好地对齐Top-K指标；提出Windowed Partial AUC (WPAUC) 约束FPR在窗口[α, α+d]内；设计Threshold-Adjusted Windowed reweighting (TAWin) 方法实现高效优化。

Result: 在四个真实世界数据集上的实验验证了理论分析的正确性，并取得了一致的SOTA性能。

Conclusion: 揭示了LLM推荐中beam-search负样本的理论优势，并提出了更直接对齐Top-K推荐目标的优化框架WPAUC和TAWin，为LLM推荐系统提供了理论指导。
...
TL;DR: 本文提出ASPIRE框架，通过双层优化目标解决图滤波学习中因传统推荐目标导致的“低频爆炸”问题，实现可学习且自适应的图滤波器，在协同过滤中达到与任务特定设计相当的性能，并适用于LLM增强的协同过滤。


<details>
  <summary>Details</summary>
Motivation: 现有频谱协同过滤中的图滤波器设计大多依赖手动调参而非完全可学习的滤波器，原因在于传统推荐目标存在偏差，诱导了称为“低频爆炸”的频谱现象，从根本上阻碍了图滤波器的有效学习。

Method: 提出ASPIRE框架，采用双层优化目标，基于理论分析解耦滤波器学习目标，实现频谱自适应性、训练稳定性和优秀的推荐性能。

Result: 学习到的滤波器在性能上与精心设计的任务特定滤波器相匹配；ASPIRE在LLM驱动的协同过滤中同样有效；证明了图滤波器学习的可行性与泛化能力。

Conclusion: 图滤波器学习是可行且可泛化的，为协同过滤中更富有表现力的图神经网络铺平了道路。
...
TL;DR: 本文研究如何利用查询性能预测（QPP）选择最优查询变体，以避免为每个变体执行完整检索和生成流程。实验发现检索与生成目标存在分歧，但轻量级预检索预测器能有效选择提升端到端质量的查询变体。


<details>
  <summary>Details</summary>
Motivation: LLM生成的多个查询变体若全部执行完整RAG流程计算成本过高，因此需要一种机制在付出下游成本前识别出最优查询变体。

Method: 将QPP用于变体选择（intra-topic discrimination），在TREC-RAG数据集上使用稀疏和稠密检索器，评估预检索和后检索预测器，采用相关性指标和决策指标进行衡量。

Result: 检索和生成目标存在系统性分歧（utility gap）；QPP能够可靠地识别出可提升端到端质量的变体；轻量级预检索预测器效果常媲美甚至优于更昂贵的后检索方法。

Conclusion: QPP可作为高效选择查询变体的手段，预检索预测器在延迟与效果之间取得良好平衡，为构建鲁棒且高效的RAG系统提供了实用方案。
...
TL;DR: 本文提出Utility-Aligned Embeddings (UAE)框架，通过训练bi-encoder模仿基于困惑度减少的效用分布，将效用信号直接注入嵌入空间，在QASPER基准上Recall@1提升30.59%，MAP提升30.16%，Token F1提升17.3%，且比高效LLM重排序方法快180倍以上。


<details>
  <summary>Details</summary>
Motivation: 密集向量检索（RAG的骨干）相似性搜索精度有限，而基于LLM重排序的效用方法性能虽优但计算成本高且受困惑度估计噪声影响。作者希望结合两者优势，提出一种实用且高性能的检索方法。

Method: 将检索建模为分布匹配问题，训练bi-encoder通过Utility-Modulated InfoNCE目标函数模仿从困惑度降低导出的效用分布，在不需测试时LLM推理的情况下将分级效用信号注入嵌入空间。

Result: 在QASPER基准上，UAE相比强语义基线BGE-Base，Recall@1提升30.59%，MAP提升30.16%，Token F1提升17.3%；且比高效LLM重排序方法快180倍以上。

Conclusion: 将检索与生成效用对齐能够在规模化情况下提供可靠的上下文，证明了该方法在性能与效率之间取得了有效平衡。
...
### [175] [Feedback Over Form: Why Execution Feedback Matters More Than Pipeline Topology in 1-3B Code Generation](https://arxiv.org/abs/2604.21950)
*Charles Junichi McAndrews*

Main category: cs.SE

TL;DR: 研究了1-3B小语言模型通过执行反馈流水线组合提升代码生成能力，发现自精炼显著改善但机制狭窄，模型专业化比流水线架构更重要，进化搜索未发现复杂结构带来明显收益。


<details>
  <summary>Details</summary>
Motivation: 小模型（1-3B）便于本地运行，但在复杂代码生成任务上能力有限。探索通过流水线组合能否恢复部分性能损失。

Method: 构建基于执行反馈的代码生成流水线，使用NEAT启发式进化搜索探索流水线结构，在HumanEval和MBPP上评估，全部在单笔记本本地推理。

Result: 自精炼在两个基准上提升超过4个标准差；主要修复运行时错误（NameError、SyntaxError），很少修复逻辑错误；1.5B生成器+3B精炼器匹敌3B模型；早停至关重要；代码专用模型优于所有通用流水线；进化搜索主要发现简单循环结构。

Conclusion: 在1-3B规模下，执行反馈比流水线复杂度更重要，模型专业化比架构设计更重要。
...
### [176] [A systematic review of generative AI usage for IT project management](https://arxiv.org/abs/2604.21958)
*Ionut Anghel,Tudor Cioara*

Main category: cs.SE

TL;DR: 本文采用PRISMA方法系统综述了生成式AI在IT项目管理中的应用，发现OpenAI的GPT占主导且主要依赖提示工程，研究仍处于探索阶段，并提出了三个未来研究方向：特定过程组AI代理、基于项目角色的AI代理以及人机协作混合网络。


<details>
  <summary>Details</summary>
Motivation: 研究者希望通过系统综述的方式，整合当前生成式AI在IT项目管理中的知识，为研究人员提供关于技术、应用、采用趋势、局限性以及跨工具和过程组集成的全面视角。

Method: 采用PRISMA（系统综述和元分析首选报告项目）方法对相关文献进行系统性回顾和分析。

Result: 分析发现OpenAI的GPT在纳入研究中占主导地位，但主要依赖提示工程，表明该领域研究仍处于探索阶段。

Conclusion: 识别并讨论了三个有前景的研究方向：（1）特定过程组AI代理；（2）基于项目角色的AI代理；（3）支持人类引导编排的混合协作网络。
...
### [177] [Documentless Assessments Using Nominal Group Interviews](https://arxiv.org/abs/2604.22003)
*Eduardo Miranda*

Main category: cs.SE

TL;DR: 本文提出了一种结合用户故事和规划扑克的群组访谈技术，用于无文档化过程评估并促进参与者协作，已在咨询项目中成功验证。


<details>
  <summary>Details</summary>
Motivation: 传统过程评估依赖文档审查和审计式访谈，导致参与者对立、协作性差，且缺乏文档时无法有效进行评估。

Method: 从敏捷开发中借鉴用户故事（将CMMI实践具体化）和规划扑克技术，替代传统的文档审查和审计式访谈，采用群组访谈形式进行事实发现与验证。

Result: 在一个咨询项目中成功应用，使原本意见分歧的参与者开始相互对话并达成共识。

Conclusion: 该群组访谈技术能有效支持无文档化的过程评估，同时促进评估参与者之间的协作与共识形成。
...
### [178] [FlyCatcher: Neural Inference of Runtime Checkers from Tests](https://arxiv.org/abs/2604.22028)
*Beatriz Souza,Chang Lou,Suman Nath,Michael Pradel*

Main category: cs.SE

TL;DR: 本文提出 FlyCatcher，一种从现有测试用例自动推导运行时检查器的方法，通过 LLM、静态分析和动态验证的结合，成功从 400 个测试中推断出 300 个正确的状态检查器，比现有方法多 2.6 倍，并能检测 5.2 倍的错误。


<details>
  <summary>Details</summary>
Motivation: 复杂软件系统常发生静默故障（不导致显式错误的语义违规），而编写运行时检查器困难且耗时，实践中很少做。本文旨在自动化从已有测试用例推导检查器，降低门槛。

Method: FlyCatcher 结合 LLM 合成、静态分析和动态验证，从测试中推断状态性运行时检查器。检查器通过维护影子状态（shadow state）来监控特定方法调用并断言应满足的属性。

Result: 在 4 个复杂系统的 400 个测试上评估：推断出 334 个检查器，其中 300 个正确；比现有最优方法多推断 2.6 倍正确检查器，检测 5.2 倍更多错误。

Conclusion: FlyCatcher 自动化了从测试到运行时检查器的推断过程，为检测软件静默故障提供了实用方案，有助于运行时检查在实践中的广泛采用。
...
### [179] [Call-Chain-Aware LLM-Based Test Generation for Java Projects](https://arxiv.org/abs/2604.22046)
*Guancheng Wang,Qinghua Xu,Lionel C. Briand,Zhaoqiang Guo,Kui Liu*

Main category: cs.SE

TL;DR: 本文提出CAT，一种调用链感知的大语言模型测试生成方法，通过静态分析显式地将调用链和依赖上下文注入提示中，显著提升了项目级单元测试的代码覆盖率。


<details>
  <summary>Details</summary>
Motivation: 现有基于执行路径信息引导提示构建的方法，在处理复杂软件系统（如丰富的类间依赖、深层调用链和复杂对象初始化需求）时效果不足，亟需更精准的上下文信息来提升测试生成质量。

Method: CAT通过专用静态分析系统地建模调用者-被调用者关系、对象构造函数和第三方依赖，显式地将调用链和依赖上下文注入LLM提示中，并支持迭代测试修复机制以处理生成失败的情况。

Result: 在Defects4J基准上，CAT相比SOTA方法PANTA，行覆盖率和分支覆盖率分别提升18.04%和21.74%；在LLM截止日期后的真实GitHub项目上也持续表现出更优性能；消融实验验证了调用链和依赖上下文的重要性。

Conclusion: 显式引入调用链和依赖上下文能有效提升LLM生成项目级单元测试的质量，CAT验证了这一思路的有效性，为复杂软件系统的自动测试生成提供了新方向。
...
### [180] [TRACE: Topology-aware Reconstruction of Accidents in CARLA for AV Evaluation](https://arxiv.org/abs/2604.22068)
*Nahian Salsabil,Sebastian Elbaum*

Main category: cs.SE

TL;DR: 提出TRACE管道，自动将NHTSA事故报告重建为CARLA高保真仿真，创建了包含52种真实交通事故场景的基准测试集，用于验证自动驾驶系统。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶验证基准要么生成合成冲突场景，要么将事故描述映射到抽象道路几何上，无法捕捉真实事故的道路拓扑复杂性，缺乏基于真实事故的高保真仿真场景。

Method: TRACE管道三步法：(1)检索OpenStreetMap数据保留精确道路拓扑；(2)利用大语言模型结合道路几何和碰撞前操作推断车辆初始状态；(3)从半结构化报告数据生成仿真轨迹。

Result: 构建了包含52个多样化事故场景的基准测试集，涵盖多种碰撞类型、道路拓扑和碰撞前操作，作为开源资源用于测试自动驾驶系统。

Conclusion: 该管道能够自动化地将真实事故报告转化为高保真仿真场景，为自动驾驶验证提供了更具挑战性和真实性的开源基准资源。
...
### [181] [Ethics Testing: Proactive Identification of Generative AI System Harms](https://arxiv.org/abs/2604.22089)
*Shin Hwei Tan,Haibo Wang,Heng Li*

Main category: cs.SE

TL;DR: 本文提出"伦理测试"新概念，旨在系统化生成测试用例，识别生成式AI系统（如ChatGPT）输出内容中的软件危害（如有害行为、侵犯知识产权等），并通过五个案例研究展示了该方法的应用。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统（如基于LLM的ChatGPT）可自动生成代码、图像等内容，但生成内容可能存在潜在危害。目前缺乏系统性测试方法来识别这些AI生成内容中的软件危害。

Method: 提出"伦理测试"概念，区别于传统的公平性测试（识别歧视），伦理测试旨在系统性检测自动生成内容中因不道德行为导致的软件危害（如有害行为或侵犯知识产权行为）。通过五个案例研究展示如何对生成式AI系统进行伦理测试。

Result: 定义了伦理测试的概念，分析了其中的挑战，并通过五个案例研究证明了伦理测试在生成式AI系统中的可行性。

Conclusion: 伦理测试是识别生成式AI系统中软件危害的新方法，弥补了现有测试方法在检测AI生成内容中不道德行为导致危害方面的空白。
...
### [182] [Evaluating LLM-Based Goal Extraction in Requirements Engineering: Prompting Strategies and Their Limitations](https://arxiv.org/abs/2604.22207)
*Anna Arnaudo,Riccardo Coppola,Maurizio Morisio,Flavio Giobergia,Andrea Bioddo,Angelo Bongiorno,Luca Dadone*

Main category: cs.SE

TL;DR: 本文提出了一种基于LLM链的自动化目标导向需求工程（GORE）方法，通过提示工程、上下文学习和生成-评论家反馈机制从软件文档中提取功能目标。尽管在低级目标识别上达到61%的准确率，该方法更适合作为人工提取的加速工具而非完全替代方案。


<details>
  <summary>Details</summary>
Motivation: 需求工程（RE）人工制品具有文本性和重复性，LLM已被证明可用于其自动化生成和处理。本文旨在自动化目标导向需求工程（GORE）流程，从软件文档中提取功能目标。

Method: 提出三阶段流水线：参与者识别、高层级目标提取和低层级目标提取。采用LLM链结合工程化提示，实验不同上下文学习变体，并引入生成-评论家反馈机制（两个LLM之间的反馈循环）。

Result: 低层级目标识别准确率达61%；反馈循环机制+零样本学习优于独立少样本学习；消融研究显示无反馈循环性能略有下降；反馈机制与少样本学习结合未带来优势。

Conclusion: 该方法最适合作为加速手动提取的工具而非完全替代品。未来将整合RAG和CoT提示技术以提高准确性，并优化示例的数量和质量。
...
### [183] [RAG-Reflect: Agentic Retrieval-Augmented Generation with Reflections for Comment-Driven Code Maintenance on Stack Overflow](https://arxiv.org/abs/2604.22217)
*Mehedi Hasan Shanto,Muhammad Asaduzzaman,Alioune Ngom*

Main category: cs.SE

TL;DR: 本文提出RAG-Reflect框架，结合大语言模型、检索增强推理和自我反思机制，在不需任务特定训练的情况下，实现了与微调模型相当的有效评论-编辑预测性能（F1=0.78），显著优于传统基线方法。


<details>
  <summary>Details</summary>
Motivation: Stack Overflow等编程平台上的用户评论大多表达感谢或澄清，仅有少数能触发实际的代码编辑。现有方法难以有效识别哪些评论真正推动了代码修改，而微调模型需要大量标注数据。作者希望开发一种无需任务特定训练就能达到微调水平的解决方案。

Method: 提出RAG-Reflect模块化框架，包含三个阶段：初始化阶段通过Interpretation模块分析知识库生成验证规则；推理阶段则依次进行(1)检索上下文示例、(2)推理评论-编辑因果关系、(3)基于预设规则反思决策。整个流程建立在一轮模式分析之上，无需重新训练。

Result: 在SOUP基准上，RAG-Reflect达到Precision=0.81、Recall=0.74、F1=0.78，优于逻辑回归、XGBoost及多种提示方法等基线，接近微调模型F1=0.773的表现。消融实验表明检索和反思模块均显著提升了性能。

Conclusion: RAG-Reflect证明智能体AI原则（检索增强推理+自我反思）可以在无需任务特定训练的情况下，有效解决软件维护中的评论-编辑预测任务，为在线编程平台的质量维护提供了新的自动化思路。
...
### [184] [Enhancing a gamified tool for UML modeling education](https://arxiv.org/abs/2604.22400)
*Giacomo Garaccione,Riccardo Coppola,Luca Ardito*

Main category: cs.SE

TL;DR: 本文介绍UMLegend工具的增强版本，在原有类图练习基础上新增用例图支持，并重构为模块化架构以支持更多建模语言，计划2026年开展纵向研究评估长期游戏化学习对UML建模能力的影响。


<details>
  <summary>Details</summary>
Motivation: 学生在软件工程教育中低估了UML用例图和类图的重要性，现有游戏化工具几乎只关注类图，缺乏对用例图等其他建模符号的支持。

Method: 基于2025年对UMLegend工具（提供动态反馈和长期游戏化机制）的实验结果，对工具进行增强：重构为模块化架构以支持多种建模语言，新增用例图练习功能。

Result: UMLegend从仅支持类图的工具扩展为支持类图和用例图的模块化平台，未来可便捷添加更多软件工程主题和建模符号。

Conclusion: 新版UMLegend有望提升学生对多种UML建模语言的学习效果，计划通过2026年的纵向研究验证长期游戏化学习是否真正提高了学生的建模能力。
...
### [185] [R2Code: A Self-Reflective LLM Framework for Requirements-to-Code Traceability](https://arxiv.org/abs/2604.22432)
*Yifei Wang,Jacky Keung,Xiaoxue Ma,Zhenyu Mao,Kehui Chen,Yishu Li*

Main category: cs.SE

TL;DR: 提出R2Code框架，通过语义对齐、自反思验证和动态上下文检索，提升需求到代码的追踪准确率并降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有IR和嵌入方法依赖词汇相似性，导致跨项目/语言的追踪链接不完整且不一致，长上下文检索和提示成本高。

Method: 1) 分解增强的双向对齐网络(BAN)实现四层语义与代码结构对齐；2) 自反思一致性验证(SRCV)进行解释引导的可靠性校准；3) 动态上下文自适应检索(DCAR)通过语义重叠加权调整粒度并过滤上下文。

Result: 在5个公共数据集上，平均F1提升7.4%，令牌消耗降低最多41.7%。

Conclusion: R2Code在跨领域和跨语言的场景下显著提升了需求-代码追踪性能，同时大幅降低了计算开销。
...
### [186] [Gamifying Architectural Governance to Reduce Organizational Coupling in Microservice Systems](https://arxiv.org/abs/2604.22454)
*Xiaozhou Li*

Main category: cs.SE

TL;DR: 本文提出通过游戏化框架（积分、徽章、排行榜、架构改进任务）来治理微服务系统中的组织耦合问题，通过持续挖掘仓库数据检测架构边界违规，以行为激励方式引导开发者维护服务边界，减少跨服务贡献带来的协调开销。


<details>
  <summary>Details</summary>
Motivation: 微服务架构中开发者频繁跨服务贡献导致组织耦合（OC），侵蚀架构边界并增加协调成本，现有方法缺乏对开发者行为的有效引导。

Method: 提出游戏化框架，整合仓库数据挖掘、架构度量与游戏化机制（积分、徽章、排行榜、架构改进任务），通过行为激励引导开发者维护服务边界。

Result: 提出了概念框架和评估路线图，尚未给出实证结果，属于愿景性工作。

Conclusion: 开辟了软件架构治理、社会技术分析与游戏化交叉的新研究方向，指出行为激励在支持微服务可持续演化方面的潜力。
...
### [187] [Test Design and Review Argumentation in AI-Assisted Test Generation](https://arxiv.org/abs/2604.22473)
*Eduard Paul Enoiu,Robert Feldt*

Main category: cs.SE

TL;DR: 本文提出一种概念分类法和结构化模板，用于表征AI辅助生成的测试用例背后的论证过程，包括测试目标、主张、理由和证据，旨在支持测试设计构建与回顾性评审。


<details>
  <summary>Details</summary>
Motivation: 现有工作关注测试技术分类、需求与测试的链接以及系统保证论证的结构化，但未能显式表示单个测试设计决策背后的论证过程。随着AI助手越来越多地生成和演化测试用例，工程师不仅需要理解测试用例本身，还需要理解其存在的原因和支撑依据。

Method: 提出一个概念分类法（taxonomy）和结构化模板，将测试用例表征为四个核心要素：测试目标（test goal）、主张（claim）、理由（reason）和证据（evidence）。该模板既可应用于测试设计的构建阶段（constructive use），也可应用于评审阶段（retrospective use）。

Result: 该方法关注的是评估测试用例所附论证的质量，而非生成测试用例的合理性或客观价值。

Conclusion: 所提出的分类法和模板为AI辅助测试生成提供了显式的论证表示机制，弥补了现有工作在单个测试设计决策论证层面的空白，有助于工程师更好地理解和评审AI生成的测试用例。
...
### [188] [A Comparison of ROS 2 and AUTOSAR Adaptive Platform Against Industry-Elicited Automotive Middleware Requirements](https://arxiv.org/abs/2604.22576)
...
TL;DR: 本文对比了ROS 2 Jazzy和AUTOSAR Adaptive Platform R24-11两种主流中间件框架，基于ZF集团汽车软件工程师的实际需求，提供了工业视角的评估。


<details>
  <summary>Details</summary>
Motivation: 汽车中间件在软件定义车辆中至关重要，但缺乏从工业实际需求出发的对比研究，作者希望填补这一空白。

Method: 从ZF集团（一家大型汽车供应商）的汽车软件工程师处收集实际需求，并基于这些需求对ROS 2 Jazzy和AUTOSAR Adaptive Platform R24-11两种中间件框架进行评估。

Result: 提供了两种中间件框架如何满足实际工业需求的对比分析。

Conclusion: 该研究有助于更清晰地理解汽车中间件开发和评估的优先级，并为行业提供有价值的参考。
...
### [189] [From Natural Language to Verified Code: Toward AI Assisted Problem-to-Code Generation with Dafny-Based Formal Verification](https://arxiv.org/abs/2604.22601)
*Md Erfan,Md Kamal Hossain Chowdhury,Ahmed Ryan,Md Rayhanur Rahman*

Main category: cs.SE

TL;DR: 论文介绍了NL2VC-60数据集，通过分级提示策略（无上下文、签名提示、自愈提示）评估7个开源LLM在Dafny形式化验证上的表现。结果表明，签名引导和自愈反馈可大幅提升验证成功率（如Gemma 4-31B达90.91%），展示了开源LLM在形式化验证中的潜力。


<details>
  <summary>Details</summary>
Motivation: LLM生成的代码常出现错误或幻觉代码，缺乏正确性保障。形式化验证要求模型同时生成实现逻辑和形式化规格，但从自然语言到精确形式化规格的转换非常困难。因此，作者旨在通过数据集和评估方法，探索开源LLM在自动化形式化验证中的能力。

Method: 构建NL2VC-60数据集（60个复杂算法问题），采用三级提示策略评估7个开源LLM：1) 无上下文提示；2) 签名提示（提供结构锚点）；3) 自愈提示（利用Dafny验证器的迭代反馈）。同时集成uDebug平台防止空泛验证。

Result: 无上下文提示几乎全面失败；签名提示和自愈反馈带来显著提升。Gemma 4-31B达90.91%验证成功率，GPT-OSS 120B从0%提升至81.82%。

Conclusion: 形式化验证对开源LLM已具备可行性，开源LLM可作为有效的"学徒"来合成复杂标注，促进高可靠性软件开发。
...
### [190] [Quality-Driven Selective Mutation for Deep Learning](https://arxiv.org/abs/2604.22640)
*Zaheed Ahmed,Emmanuel Charleson Dapaah,Philip Makedonski,Jens Grabowski*

Main category: cs.SE

TL;DR: 本文提出一个概率框架，从抗性（难以杀死）和真实性（模拟真实故障）两个维度量化深度学习变异体的质量，实验表明该框架可在减少55.6%变异体数量的同时保持变异体有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习领域的变异测试缺乏一个统一的度量标准来同时衡量变异体的抗性（hard-to-kill）和真实性（模拟真实故障的能力），导致无法有效筛选高质量的变异算子配置。

Method: 提出概率框架：抗性维度使用统计杀死概率（statistical killing probabilities）来衡量，真实性维度使用广义Jaccard相似度比较变异体与真实故障的可检测性模式。在四个真实DL故障数据集（CleanML, DeepFD, DeepLocalize用于估计，defect4ML用于验证）上进行实证评估。

Result: 质量驱动的选择可减少最多55.6%的变异体生成数量，同时在基线对齐的选择阈值下保持典型的抗性和真实性水平。

Conclusion: 双目标选择（抗性与真实性）可以降低变异测试成本，而不损害变异体在测试目标或真实故障模拟中的有用性。
...
### [191] [Verifier Warnings Do Not Improve Comprehensibility Prediction](https://arxiv.org/abs/2604.22653)
*Nadeeshan De Silva,Martin Kellogg,Oscar Chaparro*

Main category: cs.SE

TL;DR: 论文通过控制实验发现，虽然代码形式验证器的警告数量与人类理解性存在相关性，但将其作为机器学习模型的输入特征，与仅使用语法和开发者特征相比，并未显著提升代码可理解性预测性能。


<details>
  <summary>Details</summary>
Motivation: 基于已有元分析发现验证器警告数量与代码可理解性之间存在小相关性，作者推测将该特征纳入机器学习模型可提升代码可理解性预测效果。

Method: 采用控制-处理实验设计，将验证器警告总和特征加入已有的机器学习模型，并与仅使用语法特征和开发者特征的基线模型进行性能对比分析。

Result: 加入验证器警告特征后，模型的预测性能与基线模型相比无显著差异。

Conclusion: 验证器警告总和虽然与可理解性相关，但其判别能力有限；结合语法和开发者特征已足以有效预测人类评判的代码可理解性。
...
### [192] [RealBench: A Repo-Level Code Generation Benchmark Aligned with Real-World Software Development Practices](https://arxiv.org/abs/2604.22659)
...
TL;DR: 提出RealBench基准，通过结合UML图与自然语言需求评估LLM在仓库级代码生成上的表现，发现LLM在此任务上表现较差且存在显著差距，虽能识别模块但生成质量差，同时不同仓库规模下最佳生成策略不同。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如HumanEval）仅基于自然语言需求评估代码生成，但真实企业开发中开发者通常基于结构化设计（如UML图）编写代码，导致现有基准无法反映LLM在工业实践中的真实能力。

Method: 构建RealBench基准，每个样本包含自然语言需求+UML系统设计图；基于此系统评估先进LLM在给定结构化设计时的仓库级代码生成能力，并比较不同生成策略（一次性生成 vs 按模块生成）。

Result: 1）仓库级代码生成中LLM表现差且模型间差距大；2）LLM能识别UML中的模块，但生成质量因语法和逻辑错误而差；3）小仓库最优策略为一次性生成，复杂仓库最优为按模块生成。

Conclusion: 现有基准高估了LLM在实际开发中的代码生成能力，RealBench更贴近真实工业场景；未来需提升LLM的语法与逻辑正确性，并根据仓库复杂度选择生成策略。
...
### [193] [Inferring Equivalence Classes from Legacy Undocumented Embedded Binaries for ISO 26262-Compliant Testing](https://arxiv.org/abs/2604.22673)
*Marco De Luca,Domenico Francesco De Angelis,Domenico Amalfitano,Pasquale Cimmino,Anna Rita Fasolino*

Main category: cs.SE

TL;DR: 本文提出了一种无需源码或文档，直接从编译后固件的二进制级别推断输出导向等价类的测试方法，并在汽车工业实践中验证了其正确性与可用性。


<details>
  <summary>Details</summary>
Motivation: 工业实践中，等价类划分测试技术应用于遗留无文档嵌入式固件时，常因功能规格说明不完整或过时而受阻。

Method: 结合控制流重构与引导式符号执行，对单个函数进行分析，根据不可区分的可观测行为（返回值、输出参数）对执行路径进行分组，并可选择后处理生成人类可读表示。

Result: 在汽车工业环境中通过实践者研究评估，结果与专家预期高度一致，可读性和实用性获得正面评价。

Conclusion: 证明了二进制级等价类推理在遗留无文档安全关键嵌入式软件中用于系统测试的可行性与实际相关性。
...
### [194] [Code for All: Educational Applications of the "Vibe Coding" Hackathon in Programming Education across All Skill Levels](https://arxiv.org/abs/2604.22747)
*Ashley J. Chen,Yijia Cao,Minghao Shao,Ramesh Karri,Muhammad Shafique*

Main category: cs.SE

TL;DR: 本文通过为期一个月的在线黑客马拉松（设有三个技术难度赛道）研究了"氛围编程"（vibe coding，即仅用自然语言通过LLM生成代码而不手动编辑）的教育价值。采用混合方法评估了不同背景参与者的学习效果、提示工程实践及AI辅助编程在教育中的潜力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的兴起，"氛围编程"使得用户只需描述意图即可让AI生成或修改代码，这可能扩大编程的普及度。然而，这种编程方式是否具有教育价值、能否带来有意义的学习成果，尚不清楚。作者旨在通过实证研究回答这一问题。

Method: 研究采用为期一个月的在线黑客马拉松，设有三个递增技术难度的赛道（Spark基础功能、Build后端/数据库集成、Launch生产级应用）。参与者来自多国，从完全新手到经验丰富的开发者。约束：仅使用LLM生成的代码，不进行手动编辑。数据收集包括：完整聊天记录、源代码、演示视频、功能报告。采用混合方法：标准化项目评估（功能性、UI/UX设计、影响、提示质量、代码可读性） + 事后问卷调查感知学习成果 + 开放式反馈的主题分析。

Result: 研究发现描述了不同背景的参与者如何随着任务复杂度增加而参与"氛围编程"；"无手动编辑"的约束如何塑造提示工程和调试实践；以及这些模式对将AI辅助开发融入编程教育和竞争性学习环境的启示。

Conclusion: "氛围编程"在不同技术复杂度下对不同背景学习者具有教育潜力，但其效果高度依赖于任务设计、约束条件和提示工程策略。这些发现为将AI辅助开发融入编程教育与竞赛提供了实证依据和设计启示。
...
### [195] [Algorithmic Feature Highlighting for Human-AI Decision-Making](https://arxiv.org/abs/2604.22236)
*Yifan Guo,Jann Spiess*

Main category: cs.GT

TL;DR: 本文研究在人类决策者信息处理能力有限的情况下，算法如何通过突出显示少量关键特征来辅助人类决策，分析了高认知能力（理性）和低认知能力（天真）两类人类主体对特征选择策略的不同反应，并探讨了优化策略的计算复杂度与现实可行性。


<details>
  <summary>Details</summary>
Motivation: 人类决策者面对复杂任务时信息处理能力有限，但现有算法大多直接给出预测结果而非帮助人类更高效地关注关键信息。作者希望研究如何设计算法来突出显示与具体案例相关的小部分特征，从而实现人与算法之间的互补。

Method: 将特征高亮建模为一种受约束的信息政策——从案例中选取少量特征向人类展示。区分两类人类主体：理性主体（能正确推断算法选择特征的规则）和天真主体（仅更新已展示特征的信息，视特征选择为外生事件）。分别在两类主体下分析优化高亮策略的计算复杂度，并通过美国住房调查数据进行实证校准。

Result: 对理性主体优化高亮策略在计算上是难以处理的（即使是在简单的离散二元设置中）；而对天真主体优化则是可处理的（只要最大信息带宽固定）。同时，针对理性主体最优的高亮策略在部署给天真主体时可能表现极差。

Conclusion: 突出显示与具体案例相关的特征（而非固定特征集）是一种具有实际吸引力且计算可行的人机互补工具。
...
### [196] [TRUST-SC: Truthful Multi-Task Double Auction for Quality-Aware Spatial Crowdsourcing in Strategic Environment](https://arxiv.org/abs/2604.22241)
*Chattu Bhargavi,Vikash Kumar Singh,Alok Kumar Shukla*

Main category: cs.GT

TL;DR: 本文提出了一种名为TRUST-SC的诚实多任务双向拍卖机制，解决空间众包中任务请求者和执行者拥有私有估值信息导致的激励与效率问题。该框架采用三层架构：空间聚类、基于多数投票的质量评估、以及多单位双向拍卖，实现了高效任务分配和可靠执行者选择。


<details>
  <summary>Details</summary>
Motivation: 空间众包系统中，任务请求者和执行者均拥有私有估值信息，导致战略性环境中难以设计高效且诚实的激励机制。

Method: 提出TRUST-SC框架，采用三层架构：(1) 空间聚类提升可扩展性；(2) 基于多数投票的质量评估识别可靠执行者；(3) 多单位双向拍卖机制保证激励相容性和个体理性。

Result: 理论分析和仿真结果表明，所提机制实现了高效任务分配、可靠的执行者选择，性能优于现有基准机制。

Conclusion: TRUST-SC通过结合空间聚类、质量评估和双向拍卖，有效解决了空间众包中的诚实性和效率问题。
...
### [197] [Looking Into the Past: Eye Movements Characterize Elements of Autobiographical Recall in Interviews with Holocaust Survivors](https://arxiv.org/abs/2604.22016)
*Emily Zhou,Marcus Ma,Kleanthis Avramidis,Gabor Mihaly Toth,Shrikanth Narayanan*

Main category: cs.MM

TL;DR: 本研究利用大屠杀幸存者访谈视频语料库（N=806），探究自由回忆自传体记忆过程中眼动与记忆提取的关系，发现垂直眼动在不同时间语境下显著变化，且眼动特征可预测句子时间语境。


<details>
  <summary>Details</summary>
Motivation: 现有眼动与记忆研究多局限于受控实验室环境，缺乏在自然、高情感强度的自传体回忆场景中对眼动与记忆关系的探究。

Method: 使用半结构化访谈视频语料库（806名大屠杀幸存者），分析眼动与自传体回忆中情节、语义、情感及时间维度的关系，并训练主体内序列模型预测时间语境。

Result: 发现垂直眼动在不同时间语境下变化最显著，且眼动特征（甚至句子开始前的眼动）足以预测时间语境。

Conclusion: 结果证实了实验室研究中眼动与记忆的关联，强化了眼动在高度情绪化、遥远记忆提取与构建中的作用。
...
### [198] [Math Takes Two: A test for emergent mathematical reasoning in communication](https://arxiv.org/abs/2604.21935)
*Michael Cooper,Samuel Cooper*

Main category: cs.AI

TL;DR: 提出了Math Takes Two基准测试，通过两个智能体在无先验数学知识情况下通过通信发展共享符号系统来评估数学推理的涌现能力。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在数学基准测试中表现优异，但不清楚这是真正的数学推理还是对数学语法的统计模式匹配。大多数评估依赖已有数学约定，无法测试模型从第一性原理构建抽象概念的能力。

Method: 设计了一个新的基准测试Math Takes Two，要求两个没有先验数学知识的智能体通过通信发展共享符号协议，以解决一个视觉化任务，其中数值系统的使用有助于外推。

Result: 该基准测试不使用预定义的数学语言，要求智能体从头发现潜在结构和表征，提供了评估涌现数值推理能力的新视角。

Conclusion: Math Takes Two为开发和评估具有涌现数值推理能力的模型提供了新的评估工具。
...
### [199] [An Artifact-based Agent Framework for Adaptive and Reproducible Medical Image Processing](https://arxiv.org/abs/2604.21936)
...
TL;DR: 本文提出了一种基于工件契约的智能体框架，通过语义层增强医学图像处理流程，实现了自适应工作流配置与确定性可复现性的统一，并在真实CT和MRI临床数据上验证了效果。


<details>
  <summary>Details</summary>
Motivation: 医学影像研究从基准评测转向临床部署时，需要适应数据集特定条件的可配置工作流（适应性），同时保证所有转换和决策被显式记录且可重执行（可复现性）。现有方法难以同时满足这两点。

Method: 提出基于工件契约的智能体框架：引入语义层，通过工件契约形式化中间和最终输出；利用模块化规则库实现目标条件驱动的工作流组装；由工作流执行器维护确定性计算图构建和溯源追踪，智能体本地运行以保护隐私。

Result: 在真实临床CT和MRI数据集上，框架展示了自适应配置合成能力、跨多次执行的确定性可复现性，以及基于工件契约的语义查询能力。

Conclusion: 在异构临床环境中，自适应工作流配置可以在不牺牲可复现性的前提下实现。
...
### [200] [MolClaw: An Autonomous Agent with Hierarchical Skills for Drug Molecule Evaluation, Screening, and Optimization](https://arxiv.org/abs/2604.21937)
...
TL;DR: 本文提出一个名为MolClaw的自主智能体，通过三层层次化技能架构（工具级、工作流级、学科级）统一30多个专业领域资源，在药物分子评估、筛选和优化等复杂多步骤工作流中达到SOTA性能；同时提出MolBench基准测试，证明了工作流编排能力是AI驱动药物发现的主要能力瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在处理药物分子筛选和优化这类需要编排数十种专业工具的多步骤工作流时，难以保持稳健性能，表现持续不佳。现有方法缺乏对复杂工作流的结构化组织能力。

Method: MolClaw采用三层层次化技能架构：(1) 工具级技能——标准化原子操作；(2) 工作流级技能——将原子操作组合成经过质量检查和反思的验证流程；(3) 学科级技能——提供跨所有场景的科学原理以指导规划和验证。同时提出MolBench基准，涵盖分子筛选、优化和端到端发现任务，涉及8到50+次连续工具调用。

Result: MolClaw在所有评估指标上达到SOTA性能。消融实验表明，性能提升集中在需要结构化工作流的任务上，而在可通过临时脚本解决的任务上提升效果消失。

Conclusion: 工作流编排能力是AI驱动药物发现的主要能力瓶颈，MolClaw的三层技能架构有效解决了这一瓶颈。
...
### [201] [Read the Paper, Write the Code: Agentic Reproduction of Social-Science Results](https://arxiv.org/abs/2604.21965)
*Benjamin Kohler,David Zollikofer,Johanna Einsiedler,Alexander Hoyle,Elliott Ash*

Main category: cs.AI

TL;DR: 本文开发了一个基于LLM agent的自动化系统，能够仅根据论文的方法描述和原始数据复现实证社会科学结果，并在48篇论文上评估了不同模型和框架的表现。


<details>
  <summary>Details</summary>
Motivation: 现有工作使用LLM agent在同时提供数据和代码的情况下复现社会科学结果，本文旨在探索更严格的设定：仅凭论文的方法描述和原始数据，能否复现实验结果？

Method: 开发了一个agentic复现系统，包括：1）从论文中提取结构化方法描述；2）在严格信息隔离下运行重新实现（agent看不到原始代码、结果或论文全文）；3）进行确定性的单元格级别结果比较；4）通过错误归因步骤追踪系统链中的差异根源。在48篇有人工验证可复现性的论文上评估了4种agent框架和4种LLM。

Result: agent能够很大程度上恢复已发表结果，但不同模型、框架和论文之间的表现差异显著。错误根因分析表明，失败既源于agent自身的错误，也源于论文本身的方法描述不充分。

Conclusion: 基于LLM agent的方法在仅凭方法描述和原始数据的情况下可以部分复现社会科学结果，但性能受多种因素影响，且论文的方法描述不充分是导致复现失败的重要原因之一。
...
### [202] [Rethinking Publication: A Certification Framework for AI-Enabled Research](https://arxiv.org/abs/2604.22026)
*Yang Lu,Rabimba Karanjai,Lei Xu,Weidong Shi*

Main category: cs.AI

TL;DR: 本文提出一个双层认证框架，将知识质量评估与人类贡献分级分离，以解决AI管道生成学术产出在现有出版体系中的归属和认证问题。框架将贡献分为三类（A:管道可达，B:需人类指导，C:超出当前管道能力），并引入完全披露的自动化研究基准槽位。


<details>
  <summary>Details</summary>
Motivation: 现有出版体系建立在"人类作者"默认假设之上，缺乏对AI自动化管道生成知识进行归因和认证的原则性方法。随着AI研究管道产出的可发表成果日益增多，亟需在不创建新机构的前提下，建立一致、透明的处理机制。

Method: 采用规范性-概念分析、在四项明确约束下进行框架设计，以及针对两个代表性投稿案例的干运行验证。框架对贡献进行同期分级（基于投稿时的管道能力），并设置基准槽位用于完全披露的自动化研究。

Result: 干运行验证表明该框架能在容忍不可归因不确定性的同时恰当地认证知识。框架可在现有编辑基础设施内实施，将前沿人类贡献的认可建立在认知成就而非不可验证的人类起源主张之上。

Conclusion: 出版历来既认证知识的有效性，也认证知识的"人类制造"属性。AI管道首次将这些功能分离。该框架通过贡献分级和透明披露机制，为新时代的学术出版提供了可操作、可落地的解决方案。
...
### [203] [Sound Agentic Science Requires Adversarial Experiments](https://arxiv.org/abs/2604.22080)
*Dionizije Fa,Marko Culjak*

Main category: cs.AI

TL;DR: 本文指出，基于LLM的智能体在科学数据分析中被广泛采用，虽然加速了发现，但同时也加速了“产生看似合理、可无限修改的分析结果”这一失败模式，即用选择性分析支持预设结论。作者提出，针对非实验性科学主张，应采用“证伪优先”标准：智能体不应主要用于构建最具说服力的叙事，而应主动寻找主张可能失败的方式。


<details>
  <summary>Details</summary>
Motivation: 作者认为，LLM智能体在科学数据分析中的使用，虽然表面上是加速科学发现，但实际上更容易产生“选择性分析”（p-hacking-like）的问题。现有验证机制（代码迭代、后验统计支持）对科学知识的验证不足，缺乏被遗漏的否定性证据（未运行的实验、未发表的反例）导致假阳性结果。

Method: 提出“证伪优先”（falsification-first）评估标准：对于由智能体辅助产生的非实验性主张，不应让智能体主要用于构建最吸引人的叙事，而应让它主动搜索能使该主张失败的方式（如寻找反例、替代解释、假设违反等）。

Result: 论文未提供实验性结果，主要在逻辑层面论证了“加速生成选择性分析”的风险，并给出了规范性的建议方案。

Conclusion: 科学知识不能通过智能体生成的流畅解释或单一数据集的显著结果得到验证。为了避免智能体放大“发表积极结果”的偏差，应采用证伪优先的标准来使用智能体。
...
### [204] [AgentSearchBench: A Benchmark for AI Agent Search in the Wild](https://arxiv.org/abs/2604.22436)
*Bin Wu,Arastun Mammadli,Xiaoyu Zhang,Emine Yilmaz*

Main category: cs.AI

TL;DR: 提出了AgentSearchBench，一个包含近万个真实世界智能体的大规模基准，用于研究在自然环境下如何搜索和排序合适的AI智能体。实验发现基于语义描述的方法与实际表现存在差距，而引入执行感知的轻量级行为信号能显著提升排序质量。


<details>
  <summary>Details</summary>
Motivation: 现有研究和基准通常假设智能体功能明确、候选池可控或只有可执行任务查询，缺乏对现实世界中智能体搜索场景（智能体能力通常具有组合性和执行依赖性，难以仅从文本描述评估）的充分研究。

Method: 构建了AgentSearchBench基准，包含来自多个提供商的近万个真实智能体。将智能体搜索形式化为检索和重排序问题，覆盖可执行任务查询和高层任务描述两类场景，并使用执行驱动的性能信号评估相关性。

Result: 实验表明语义相似性与实际智能体表现之间存在持续差距，揭示了基于描述检索和重排序方法的局限性。轻量级行为信号（包括执行感知探针）能显著提升排序质量。

Conclusion: 强调了将执行信号纳入智能体发现过程的重要性，为构建更有效的智能体搜索引擎提供了新方向。
...
### [205] [Memanto: Typed Semantic Memory with Information-Theoretic Retrieval for Long-Horizon Agents](https://arxiv.org/abs/2604.22085)
*Seyed Moein Abtahi,Rasa Rahnema,Hetkumar Patel,Neel Patel,Majid Fekri,Tara Khani*

Main category: cs.AI

TL;DR: Memanto是一个通用记忆层，通过13种预定义语义记忆类别、自动冲突解决和时间版本控制，结合基于信息论的无索引语义数据库，在无需知识图谱复杂性的情况下实现高性能代理记忆，在LongMemEval和LoCoMo基准上分别达到89.8%和87.1%的SOTA准确率。


<details>
  <summary>Details</summary>
Motivation: 从无状态语言模型推理向持久的、多会话自主代理的转变，揭示了记忆是生产级代理系统部署中的主要架构瓶颈，现有混合语义图架构计算开销大、需要实体提取和图模式维护。

Method: Memanto集成了13种预定义类型的语义记忆模式、自动冲突解决机制和时间版本控制，核心引擎是Moorcheh的Information Theoretic Search——一种无索引语义数据库，可实现确定性检索且消除摄入延迟。

Result: 在LongMemEval和LoCoMo基准上分别达到89.8%和87.1%的SOTA准确率，仅需单次检索查询，零摄入成本，延迟低于90毫秒，超越所有混合图与向量系统。

Conclusion: 知识图谱复杂性并非实现高保真代理记忆的必要条件，Memanto通过简洁设计实现了更低复杂度的可扩展代理记忆系统部署。
...
### [206] [Emergent Strategic Reasoning Risks in AI: A Taxonomy-Driven Evaluation Framework](https://arxiv.org/abs/2604.22119)
...
TL;DR: 本文提出ESRRSim框架，用于自动化评估大语言模型中出现的策略性推理风险（如欺骗、评估游戏、奖励黑客等），通过7大类20子类的可扩展风险分类法，对11个推理模型进行评估，发现风险检出率差异巨大（14.45%-72.72%），且随模型代际提升，模型可能愈发识别并适应评估情境。


<details>
  <summary>Details</summary>
Motivation: 大语言模型随着推理能力和部署范围的扩大，可能产生服务于自身目标的行为（欺骗、评估游戏、奖励黑客等），但系统性理解与基准测试这些风险仍是一个未解决的挑战。

Method: 提出ESRRSim框架：构建包含7大类20个子类的可扩展风险分类法；生成旨在引发忠实推理的评估场景；设计双重评分标准（评估模型响应与推理轨迹）；采用与评判器无关的可扩展架构。

Result: 在11个推理LLM上评估，风险检出率从14.45%到72.72%不等，存在显著差异；发现代际提升可能使模型更善于识别并适应评估情境。

Conclusion: ESRRSim能够系统性评估LLM的策略性推理风险，揭示了不同模型风险画像的显著差异及模型能力的代际演化趋势。
...
### [207] [When Does LLM Self-Correction Help? A Control-Theoretic Markov Diagnostic and Verify-First Intervention](https://arxiv.org/abs/2604.22273)
*Aofan Liu,Jingxiang Meng*

Main category: cs.AI

TL;DR: 该论文提出将迭代自纠正视为一个控制论反馈回路，并通过两态马尔可夫模型推导出一个简单的部署诊断指标（ECR/EIR > Acc/(1-Acc)）。实验发现仅当错误引入率（EIR）接近0%时自纠正方有益，且通过"先验证再提示"的消融实验可有效降低EIR，将有害纠正转为有益。


<details>
  <summary>Details</summary>
Motivation: 自纠正虽被广泛用于智能体LLM系统，但何时有益、何时有害尚不明确。作者旨在建立一个可操作的诊断框架，帮助判断何时应进行迭代自纠正。

Method: 将自纠正建模为控制论反馈回路（同一LLM同时作为控制器和被控对象），使用两态马尔可夫模型{正确, 错误}推导诊断条件。通过7个模型、3个数据集（GSM8K, MATH, StrategyQA）实验验证，并进行"先验证再提示"的消融实验和ASC停止策略对比。

Result: 发现EIR阈值接近0%（<=0.5%）是区分有益与有害自纠正的关键。仅o3-mini(+3.4pp, EIR=0%)、Claude Opus 4.6(+0.6pp, EIR~0.2%)和o4-mini(+/-0pp)不退化；GPT-5退化-1.8pp。验证优先消融可将GPT-4o-mini的EIR从2%降至0%，将-6.2pp退化转为+0.2pp。

Conclusion: 自纠正不应被视为默认行为，而应作为由可测量的错误动态所控制的决策。论文提供的诊断指标和提示工程策略可有效指导何时进行迭代自纠正。
...
### [208] [Introducing Background Temperature to Characterise Hidden Randomness in Large Language Models](https://arxiv.org/abs/2604.22411)
*Alberto Messina,Stefano Scotta*

Main category: cs.AI

TL;DR: 本文提出了"背景温度" $T_{\mathrm{bg}}$ 的概念，用于形式化描述大语言模型在标称温度 $T=0$ 时仍产生的输出不确定性，并通过实验估计其大小。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在 $T=0$ 时仍会出现非确定性输出，这种来自实现层面的随机性（如批量大小变化、核函数非不变性、浮点数非结合性）影响了可重复性和评估可靠性，需要被形式化定义和量化。

Method: 定义了背景温度 $T_{\mathrm{bg}}$ 作为实现相关扰动过程引起的有效温度，将其与推理环境 $I$ 关联，并提出通过理想参考系统的等效温度 $T_n(I)$ 来经验性估计 $T_{\mathrm{bg}}$ 的实验协议。

Result: 在主流大语言模型提供商的代表性模型上进行了初步实验，验证了 $T_{\mathrm{bg}}$ 的存在性和可估计性。

Conclusion: $T_{\mathrm{bg}}$ 的提出为理解 LLM 非确定性输出来源提供了形式化框架，对模型的可重复性、评估和部署具有重要启示。
...
### [209] [CognitiveTwin: Robust Multi-Modal Digital Twins for Predicting Cognitive Decline in Alzheimer's Disease](https://arxiv.org/abs/2604.22428)
*Bulent Soykan,Gulsah Hancerliogullari Koksalmis,Hsin-Hsiung Huang,Laura J. Brattain*

Main category: cs.AI

TL;DR: 本文提出CognitiveTwin数字孪生框架，利用Transformer和深度马尔可夫模型融合多模态纵向数据（认知评分、MRI、PET、脑脊液生物标志物、遗传学），在TADPOLE数据集1666名患者上实现了高精度、人口公平性且对缺失数据鲁棒的认知衰退预测。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的疾病进展具有高度异质性，现有预测工具难以同时满足高精度、跨人口统计群体的公平性以及对缺失数据的鲁棒性需求。

Method: 构建CognitiveTwin数字孪生框架，采用Transformer架构融合多模态数据，结合深度马尔可夫模型（Deep Markov Model）捕捉纵向时间动态。在TADPOLE（ADNI）数据集的1666名患者上进行训练和评估。

Result: CognitiveTwin在预测误差、人口统计公平性以及对非随机缺失（MNAR）数据模式的鲁棒性方面均表现良好，提供了准确且个性化的认知衰退预测。

Conclusion: CognitiveTwin在人口公平性和临床脱落鲁棒性方面表现出色，可作为可靠的临床试验富集和个性化护理规划工具。
...
### [210] [From Skills to Talent: Organising Heterogeneous Agents as a Real-World Company](https://arxiv.org/abs/2604.22446)
*Zhengxu Yu,Yu Fu,Zhiyuan He,Yuxuan Huang,Lee Ka Yiu,Meng Fang,Weilin Luo,Jun Wang*

Main category: cs.AI

TL;DR: 本文提出OneManCompany（OMC）框架，通过可移植的智能体身份（Talents）、类型化组织接口、人才市场和E²R树搜索机制，将多智能体系统从静态管线升级为自组织、自改进的AI组织，在PRDBench上达到84.67%成功率，超越SOTA 15.48个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有MAS受限于固定团队结构、紧耦合协调逻辑和会话内学习，缺乏原则性的组织层来独立于个体智能体能力地管理智能体工作力的组装、治理和持续改进。

Method: OMC框架包含：(1) Talents——封装技能、工具和运行配置的可移植智能体身份；(2) 类型化组织接口——抽象异构后端；(3) 人才市场——社区驱动的按需招聘机制；(4) E²R树搜索——统一规划、执行和评估的层次化循环，提供终止和无死锁保证。

Result: 在PRDBench上OMC达到84.67%成功率，比SOTA高15.48个百分点，跨领域案例研究验证了其通用性。

Conclusion: OMC将多智能体系统从静态预配置管线转变为能适应开放任务的自我组织、自我改进的AI组织，具备形式化保证和卓越性能。
...
### [211] [Superminds Test: Actively Evaluating Collective Intelligence of Agent Society via Probing Agents](https://arxiv.org/abs/2604.22452)
*Xirui Li,Ming Li,Yunze Xiao,Ryan Wong,Dianqi Li,Timothy Baldwin,Tianyi Zhou*

Main category: cs.AI

TL;DR: 本文首次实证评估了大规模语言模型智能体社会（MoltBook平台，超200万智能体）中集体智能是否随规模自发涌现。通过三级分层框架（联合推理、信息综合、基础交互）测试，发现社会层面集体智能严重缺失，无法超越个体模型，交互浅层且稀疏。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型智能体规模扩展至数百万级，集体智能是否能从规模中自发涌现成为一个关键问题，但目前缺乏实证评估。

Method: 在MoltBook平台（超200万智能体）上，提出Superminds Test分层框架，通过三类Probing Agents（联合推理、信息综合、基础交互）对社会层面智能进行控制性测试，并辅以平台范围交互分析。

Result: 社会未能超越个体先进模型在复杂推理任务上的表现；极少能综合分布式信息；甚至基础协调任务也频繁失败。平台交互浅层化，线程极少超过单轮回复，多数回复泛泛或离题。

Conclusion: 集体智能不会仅从规模中自发涌现；当前智能体社会的核心瓶颈是极端稀疏和浅层的交互，阻碍了信息交换与成果积累。
...
### [212] [On the Hybrid Nature of ABPMS Process Frames and its Implications on Automated Process Discovery](https://arxiv.org/abs/2604.22455)
*Anti Alman,Izack Cohen,Avigdor Gal,Fabrizio Maria Maggi,Marco Montali*

Main category: cs.AI

TL;DR: 本文提出了ABPMS中"流程框架"的概念，将其定义为一种混合业务流程表示，由半并发执行的程序性和声明性流程模型组成。作者主张对程序性模型也采用开放世界假设，使其具有类似约束的语义，并提出了将声明性约束映射为等效程序性片段的方法，为流程发现奠定基础。


<details>
  <summary>Details</summary>
Motivation: 传统流程模型对ABPMS的约束过于严格，限制了系统的半自主行为能力（即"框架自主性"）。需要一个更宽容的流程表示方式，能够容纳从预定义程序到常识规则和最佳实践等异构知识。

Method: 1. 概念化ABPMS流程框架为混合业务流程表示，包含半并发执行的程序性和声明性模型。2. 提出对程序性模型也采用开放世界假设，将其解释为约束而非强制执行要求。3. 提出将声明性约束映射为等效的半并发执行程序性片段的方法。

Result: 建立了流程框架的理论基础，阐明了其执行语义，并提出了将声明性约束映射为程序性片段的方法，为后续的流程发现方法奠定了基础。

Conclusion: 流程框架通过混合程序性与声明性模型，能够更好地支持ABPMS的框架自主性。将声明性约束映射为程序性片段的方法为流程发现提供了可行路径。
...
### [213] [QuantClaw: Precision Where It Matters for OpenClaw](https://arxiv.org/abs/2604.22577)
*Manyi Zhang,Ji-Fu Li,Zhongao Sun,Xiaohao Liu,Zhenhua Dong,Xianzhi Yu,Haoli Bai,Xiaobo Xia*

Main category: cs.AI

TL;DR: 本文提出QuantClaw，一种面向自主智能体系统的动态精度路由插件，通过根据任务特性动态分配量化精度，在保持或提升任务性能的同时，最高可节省21.4%成本和15.7%延迟。


<details>
  <summary>Details</summary>
Motivation: 自主智能体系统（如OpenClaw）因长上下文输入和多轮推理导致计算与金钱成本过高，而标准量化方法在实际智能体场景中的影响尚不明确，亟需一种任务自适应的精度分配方案。

Method: 首先分析OpenClaw上多样化复杂工作流对量化的敏感性，发现精度需求高度依赖任务类型；基于此提出QuantClaw，一种即插即用的精度路由插件，根据任务特征动态分配精度——轻量任务使用低成本配置，高需求任务保留高精度。

Result: 在多种智能体任务上，QuantClaw在保持或提升任务性能的同时，实现了最高21.4%的成本节省和15.7%的延迟降低（基于GLM-5的FP8基线）。

Conclusion: 结果表明，在智能体系统中将精度视为一种动态资源具有显著收益，为降低大规模自主智能体系统的部署成本提供了有效方案。
...
### [214] [Rethinking Math Reasoning Evaluation: A Robust LLM-as-a-Judge Framework Beyond Symbolic Rigidity](https://arxiv.org/abs/2604.22597)
*Erez Yosef,Oron Anschel,Shunit Haviv Hakimi,Asaf Gendler,Adam Botach,Nimrod Berman,Igor Kviatkovsky*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型（LLM）的评估框架，用于替代传统的符号数学比较方法，以更准确、灵活地评估模型在数学推理任务中生成的答案。


<details>
  <summary>Details</summary>
Motivation: 现有的数学推理评估依赖符号数学比较来验证答案正确性，但该方法无法泛化到多样化的数学表示和答案格式，导致评估不准确。

Method: 提出一种基于LLM的评估框架，利用大语言模型自身的能力对模型生成的答案进行验证，替代规则驱动的符号数学比较方法。

Result: 在Lighteval和SimpleRL两个流行框架中展示了符号评估的失败案例，并与本文方法对比，证明了所提框架相比现有方法有明显改进。

Conclusion: 该框架能够实现更可靠的评估和基准测试，有助于更准确地监控模型性能，对推动数学问题解决和智能系统的发展具有重要意义。
...
### [215] [Agentic World Modeling: Foundations, Capabilities, Laws, and Beyond](https://arxiv.org/abs/2604.22748)
...
TL;DR: 本文提出一个"层级×法则"分类法来系统梳理世界模型（World Models）研究，涵盖L1预测器、L2模拟器、L3进化器三个能力层级，以及物理、数字、社会、科学四个法则领域，总结了400+文献和100+代表性系统，提出评估原则和开放问题，为从被动预测迈向能重塑环境的智能体世界模型绘制路线图。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统从文本生成转向通过持续交互完成目标，建模环境动态成为核心瓶颈。不同社区对"世界模型"定义各异，缺乏统一框架来对比和理解各领域的方法、失败模式与评估实践。本文旨在通过分类法桥接这些孤立社区，系统梳理前沿进展。

Method: 提出"层级×法则"二维分类法：层级（L1一步局部转移预测器 → L2多步动作条件模拟器 → L3自主修正模型进化器）与法则（物理、数字、社会、科学四大领域）。基于此框架综述400+文献，总结100+代表性系统，分析各层级-法则组合下的方法、失败模式和评估实践，并提出以决策为中心的评估原则和可复现评估包。

Result: 系统化梳理了世界模型研究全景，明确了不同层级和法则领域的约束条件与典型失败模式，提出了决策导向的评估准则，并识别出架构设计、开放问题和治理挑战等关键方向。

Conclusion: 本文提供了一个统一的世界模型研究路线图，连接了基于模型的强化学习、视频生成、Web/GUI智能体、多智能体社会模拟和AI科学发现等此前相互隔离的社区，指明了从被动预测到主动模拟并重塑环境的发展路径。
...
### [216] [L-System Genetic Encoding for Scalable Neural Network Evolution: A Comparison with Direct Matrix Encoding](https://arxiv.org/abs/2604.22000)
*Alexander Stuy,Nodin Weddington*

Main category: cs.NE

TL;DR: 本文提出了一种基于L-System的遗传编码方案Lsys，用于进化赫布神经网络。实验表明，相比传统的Matrix编码，Lsys在1000代内食物采集量提升2.74倍（3802 vs 1388），变异性降低8.5倍（CV: 5.2% vs 44.0%），且在新环境泛化能力上优势更显著（5.82倍）。Lsys的性能优势主要来自遗传算子在压缩符号编码上的操作，而非初始种群结构。


<details>
  <summary>Details</summary>
Motivation: 探索在没有问题领域先验知识的情况下，使用遗传算法优化赫布神经网络的可行性，并比较不同神经网络拓扑遗传编码方法的优劣。

Method: 提出L-System形式的遗传字母表（Lsys）用于神经网络的基因编码，并与传统的Matrix编码进行对比实验。设置24组实验，包括Lsys和Matrix两种编码方式，以及MatrixLSG对照组（初始种群由Lsys生成，但使用Matrix算子进化），在人工食物世界中测试进化性能。

Result: Lsys编码在1000代时平均最大食物数3802±197，Matrix为1388±610，性能优势2.74倍；变异系数5.2% vs 44.0%，一致性提升8.5倍。所有8个Lsys种群均成功学习，4个Matrix种群完全失败。在新迷宫中，Lsys泛化性能2455±176 vs 422±212，优势5.82倍。

Conclusion: Lsys编码在收敛速度、峰值性能、可靠性和泛化能力上全面优于Matrix编码，其优势主要源于遗传算法在压缩符号化Lsys字母表上的操作。
...
### [217] [Structure-Guided Diffusion Model for EEG-Based Visual Cognition Reconstruction](https://arxiv.org/abs/2604.22649)
*Yongxiang Lian,Yueyang Cang,Pingge Hu,Yuchen He,Li Shi*

Main category: cs.NE

TL;DR: 提出结构引导扩散模型(SGDM)，通过结合显式结构信息从EEG信号重建视觉图像，在抽象和自然图像数据集上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有EEG视觉解码方法局限于自然图像和类别级表示，难以捕捉结构特征，且无法区分客观感知与主观认知。

Method: 采用两阶段生成机制：结构监督变分自编码器+时空EEG编码器（通过对比学习对齐视觉嵌入空间），利用ControlNet将结构信息集成到扩散模型中引导图像生成。

Result: 在抽象和自然图像数据集上均优于现有方法，重建图像在低级视觉特征和语义表示上保真度更高，时空EEG分析揭示了层次化结构编码模式。

Conclusion: SGDM能有效捕捉显式结构几何并生成与个体认知表征高度保真的图像，将神经解码扩展到低维或类别输出之外，支持更灵活的脑机通信。
