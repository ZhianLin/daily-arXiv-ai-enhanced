...
Method: 使用潜在动作模型和结构化离散动作令牌的双级动作表示框架，结合机器人轨迹数据与通用QA和空间QA数据集的混合训练策略。

Result: 在LIBERO Franka基准测试中表现优异，在真实世界评估中在各种具有挑战性的操作任务上实现了有竞争力的成功率。

Conclusion: iFlyBot-VLA通过双级动作表示和混合训练有效对齐了语言、视觉和动作的表示空间，为机器人操作任务提供了强大的解决方案，并计划开源部分数据集支持社区研究。
...
TL;DR: 本研究首次在低类间变异性条件下系统评估胎儿超声成像中的基础模型，发现领域特定的预训练对于区分解剖结构相似的胎儿脑标准切面至关重要。


<details>
  <summary>Details</summary>
Motivation: 虽然DINOv3等视觉基础模型在医学领域显示出良好的可迁移性，但其在区分解剖结构相似结构方面的能力尚未得到系统研究。本研究针对胎儿脑标准切面（TT、TV、TC）这一具有高度重叠解剖特征的关键挑战展开。

Method: 收集并整合所有公开可用的胎儿超声数据集，构建包含18.8万张标注图像的FetalUS-188K多中心基准。采用自监督方式预训练DINOv3学习超声感知表示，通过线性探测和全微调等标准化适应协议进行评估。

Result: 在胎儿超声数据上预训练的模型始终优于基于自然图像初始化的模型，加权F1分数提升高达20%。领域自适应预训练使网络能够保留区分中间切面（如TV）所需的关键回声和结构线索。

Conclusion: 通用基础模型在低类间变异性条件下泛化能力不足，而领域特定的预训练对于在胎儿脑超声成像中获得稳健且临床可靠的表示至关重要。
...
### [3] [AI-Generated Image Detection: An Empirical Study and Future Research Directions](https://arxiv.org/abs/2511.02791)
*Nusrat Tasnim,Kutub Uddin,Khalid Mahmood Malik*

Main category: cs.CV

TL;DR: 本文提出了一个统一的基准测试框架，用于在受控和可复现条件下系统评估深度伪造检测方法，解决了现有方法在基准标准化、训练协议一致性和评估指标全面性方面的关键差距。


<details>
  <summary>Details</summary>
Motivation: AI生成的深度伪造媒体对多媒体取证、虚假信息检测和生物识别系统构成严重威胁，导致公众对法律系统信任度下降、欺诈案件显著增加和社会工程攻击。现有取证方法存在三个关键问题：非标准化基准、不一致的训练协议以及有限的评估指标。

Method: 引入统一的基准测试框架，对十种最先进的取证方法（从头训练、冻结预训练、微调）和七个公开可用数据集（GAN和扩散生成）进行系统评估，使用多种指标包括准确率、平均精度、ROC-AUC、错误率和类别敏感度，并通过置信度曲线和Grad-CAM热图分析模型可解释性。
...
TL;DR: Geo-Foundational Models (GFMs) 在洪水淹没制图方面与传统U-Net模型相比，提供了小到中等的精度提升，同时计算成本和标注需求更低。Clay模型在多个传感器上表现最佳，特别是在小样本场景下优势明显。
...
Method: 评估了三种GFMs（Prithvi 2.0、Clay V1.5、DOFA和UViT）与TransNorm、U-Net和Attention U-Net，使用PlanetScope、Sentinel-1和Sentinel-2数据，进行了留一区域交叉验证和少样本实验。

Result: 所有GFMs表现竞争性，性能差异仅2-5%。Clay在PlanetScope（0.79 mIoU）和Sentinel-2（0.70）上表现最佳，Prithvi在Sentinel-1（0.57）上领先。Clay在少样本实验中仅用5张训练图像就达到0.64 mIoU，优于Prithvi（0.24）和DOFA（0.35）。Clay模型更小（26M参数），比Prithvi快3倍，比DOFA快2倍。

Conclusion: GFMs相比传统U-Net在洪水制图精度上提供小到中等改进，计算成本更低，标注工作量更少。Clay因其更好的性能、更小的模型尺寸和更快的推理速度成为更优选择。
...
TL;DR: 提出一种基于学习框架的图像重建方法，适用于上采样和修复任务，能够在固定、确定性采样模式且覆盖不完整的情况下，利用图像分布的多个不变性实现与全监督方法相当的重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统监督方法需要完整采样的真实数据，而自监督方法通常依赖随机采样模式。本文针对固定、确定性采样模式且覆盖不完整的情况，探索如何在这种更具挑战性的设置下实现高质量图像重建。
...
Conclusion: 该方法能够在固定、确定性采样模式且覆盖不完整的情况下，通过利用图像分布的不变性，实现与全监督方法相当的重建质量，同时大幅减少对真实数据的需求。
...
TL;DR: 本研究系统评估了三种大型多模态模型在医疗图像中受保护健康信息检测的性能，发现LMM在OCR准确率上优于传统方法，但OCR性能提升并不总是转化为更好的整体PHI检测效果。


<details>
  <summary>Details</summary>
Motivation: 传统PHI检测方法主要依赖OCR和命名实体识别，而新兴的大型多模态模型为改进文本提取和语义分析提供了新机会，需要系统评估其在医疗隐私保护中的应用价值。

Method: 使用GPT-4o、Gemini 2.5 Flash和Qwen 2.5 7B三种LMM，比较两种处理流程：纯文本分析流程和OCR+语义分析集成流程，并与EasyOCR等传统模型进行基准测试。

Result: LMM在OCR准确率上显著优于传统模型（WER: 0.03-0.05, CER: 0.02-0.03），但在复杂印记模式情况下表现最佳。当文本区域可读性好且使用强LMM进行文本分析时，不同流程配置结果相似。

Conclusion: LMM在OCR方面有优势，但OCR性能提升不一定带来更好的PHI检测效果。研究提供了基于实证的LMM选择建议和可扩展的模块化部署策略。
...
TL;DR: StrengthSense是一个包含11种力量需求活动和2种非力量需求活动的IMU传感器开放数据集，采集自29名健康受试者，用于推进人类活动识别算法和健康监测应用的发展。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏捕捉力量需求活动的综合数据集，而通过可穿戴传感器追踪这些活动对于监测肌肉力量、耐力和功率至关重要。

Method: 使用10个IMU传感器放置在四肢和躯干上，从29名健康受试者收集数据，并通过视频记录进行注释验证。

Result: 数据集包含11种力量需求活动（如坐立、爬楼梯、拖地等）和2种非力量需求活动，并通过IMU估计的关节角度与视频提取的角度进行对比分析验证数据准确性。

Conclusion: StrengthSense数据集为研究人员和开发者提供了可靠的资源，可用于开发人类活动识别算法、健身和健康监测应用等。
...
TL;DR: 提出一个自动化合成文本视觉问答数据集的端到端流水线，利用OCR、区域检测、字幕生成和问题生成等技术，生成了基于44K图像的72K QA对。


<details>
  <summary>Details</summary>
Motivation: 大规模文本VQA数据集的人工标注既繁琐又困难，需要利用基础模型和OCR系统的成熟技术建立自动化合成流水线。

Method: 使用OCR检测识别、感兴趣区域检测、字幕生成和问题生成等多个模型和算法的集成流水线，自动合成和验证QA对。

Result: 成功构建了包含约72K QA对的大规模文本VQA数据集，基于约44K图像，这是首个自动合成验证的此类数据集。

Conclusion: 该流水线能够忠实地合成文本VQA数据集，并随着场景文本数据的可用性而扩展，为文本VQA任务提供了高效的数据生成解决方案。
...
TL;DR: 开发并临床评估了基于头戴式显示器的仅深度、无标记增强现实配准管道，在小或低曲率解剖结构上实现了约3-4毫米的中位误差，接近中等风险任务的临床误差阈值。


<details>
  <summary>Details</summary>
Motivation: 开发无需标记的增强现实配准方法，在头戴式显示器上实现准确配准，特别针对小或低曲率解剖结构，提高无标记AR引导的临床准备度。

Method: 在HoloLens 2上，通过深度偏差校正、人工引导初始化和全局局部配准，将AHAT深度数据与CT皮肤网格对齐。通过AR追踪工具验证表面追踪误差指标，并在手术中进行目标试验。

Result: 临床前验证显示AR追踪与CT距离紧密一致（腿：中位|Δd| 0.78毫米，RMSE 0.97毫米；脚：0.80毫米，1.20毫米）。临床中每点误差中位数为3.9毫米，不同解剖部位误差分别为脚3.2毫米、耳4.3毫米、小腿5.3毫米。

Conclusion: 仅深度无标记AR管道在活体手术环境中实现了约3-4毫米中位误差，无需基准标记，接近中等风险任务的典型临床误差阈值。人工引导初始化加全局局部配准使小或低曲率目标能够准确对齐。
...
TL;DR: 提出了一个端到端的自动化管道，结合实例分割和专门的腔室排序算法，从CT扫描中自动重建浮游有孔虫的三维生长轨迹。


<details>
  <summary>Details</summary>
Motivation: 浮游有孔虫是重要的环境指示生物，但现有的腔室生长轨迹分析主要依赖耗时且主观的手动分割方法，需要自动化解决方案。

Method: 使用实例分割技术结合专门的腔室排序算法，从高分辨率CT扫描中自动重建三维生长轨迹，并评估多种分割方法对生长顺序重建精度的影响。

Result: 实验结果表明，该管道显著减少了人工工作量，同时保持了生物学上有意义的准确性。腔室排序算法在部分分割情况下仍能稳健地重建发育轨迹。
...
Method: 采用三阶段级联框架：1) 使用现有检测算法进行初始裂缝分割；2) 通过PCA确定准平行裂缝的主方向轴；3) 使用RPCA提取不规则裂缝几何的主传播轴。
...
TL;DR: 提出了一种基于CNN的自适应偏置调节机制，通过动态调整事件相机的偏置设置来抑制25Hz-500Hz范围内的闪烁效应，无需额外硬件或软件滤波。
...
Method: 使用卷积神经网络在空间域识别闪烁现象，并动态调节事件相机的特定偏置参数来最小化闪烁影响。

Result: 在面部检测任务中，YOLO置信度显著提升，检测到面部的帧数比例增加；平均梯度（闪烁指标）在良好光照下降低38.2%，低光条件下降低53.6%。
...
TL;DR: 提出了一个三阶段框架来解决ICCV 2025感知测试挑战中的Grounded Video QA任务，通过引入触发时刻概念显著提升了HOTA分数


<details>
  <summary>Details</summary>
Motivation: GVQA任务需要强大的多模态模型来进行视频内容复杂推理、视觉接地和时间跟踪，现有方法在这些方面存在不足

Method: 将GVQA任务分解为三个阶段：视频推理与问答、时空接地和跟踪，关键创新是使用CORTEX提示推导触发时刻来定位目标对象最清晰帧

Result: HOTA分数达到0.4968，相比去年获胜分数0.2704有显著提升
...
<details>
  <summary>Details</summary>
Motivation: 视网膜血管具有极细分支结构，其全局形态在不同图像中差异很大，这给分割精度和鲁棒性带来挑战。现有深度学习方法在处理这种特殊结构时仍有不足。

Method: MM-UNet架构包含形态Mamba卷积层（替换点卷积以增强分支拓扑感知）和反向选择性状态引导模块（结合反向引导理论和状态空间建模来提升几何边界意识和解码效率）。
...
Method: 收集566名参与者的PET、MRI和血液生物标志物数据，开发语言增强生成模型，融合多模态信息合成PET图像，并在全自动诊断流程中评估。

Result: 合成PET图像在结构细节和区域模式上与真实PET高度相似，诊断准确率达0.80，基于合成PET的诊断模型性能优于仅使用MRI或血液标志物的模型。

Conclusion: 语言增强生成模型能合成逼真的PET图像，提升MRI和血液生物标志物在淀粉样蛋白空间模式评估中的效用，改善阿尔茨海默病诊断流程。
...
TL;DR: 提出了一种基于3D高斯泼溅和SAM-2分割的对象中心3D重建框架，用于草莓植株表型分析，实现了背景去除和自动性状估计。


<details>
  <summary>Details</summary>
Motivation: 传统植物表型分析方法耗时费力且具有破坏性，而现有3DGS方法重建整个场景包含背景噪声，增加了计算成本并干扰下游性状分析。

Method: 结合SAM-2分割和alpha通道背景掩码的预处理流程，实现干净的草莓植株重建，并使用DBSCAN聚类和PCA自动估计植株高度和冠层宽度等性状。

Result: 实验结果表明，该方法在准确性和效率上均优于传统流程，提供了可扩展的非破坏性草莓植株表型分析解决方案。

Conclusion: 提出的对象中心3D重建框架为草莓植株表型分析提供了更准确、高效的解决方案，显著减少了计算时间并改善了性状分析的准确性。
...
TL;DR: 本研究开发了首个自动化管道autoStrain，使用深度学习技术从经食管超声心动图自动估计左心室节段纵向应变，提出了两种深度学习方法TeeFlow和TeeTracker，并在合成数据集和临床数据上验证了其准确性。


<details>
  <summary>Details</summary>
Motivation: 当前左心室节段纵向应变的估计技术需要大量人工干预和专业经验，效率低下且资源密集，限制了其在监测应用中的使用。

Method: 使用SIMUS模拟管道生成高度逼真的合成TEE数据集，训练和评估两种深度学习模型：基于RAFT光流模型的TeeFlow（密集帧间预测）和基于CoTracker点轨迹模型的TeeTracker（稀疏长序列预测）。

Result: TeeTracker在准确性上优于TeeFlow，在合成测试数据集上的运动估计平均距离误差为0.65mm。临床验证显示autoStrain管道与临床参考值一致，平均差异为1.09%（-8.90%至11.09%）。
...
TL;DR: 本文研究了基础模型如何改变移动稀疏感知的格局，通过真实移动AR数据评估，证明基础模型在几何感知图像扭曲方面显著提升性能，并在3D场景重建中展现领先表现。


<details>
  <summary>Details</summary>
Motivation: 移动感知系统长期面临感知质量与效率之间的权衡，现有稀疏感知方法常因时空信息缺失导致精度下降。本文探讨基础模型是否能改变这一现状。

Method: 使用真实移动AR数据评估基础模型在几何感知图像扭曲和3D场景重建中的表现，研究基于基础模型的稀疏感知的可扩展性。

Result: 基础模型在几何感知图像扭曲方面提供显著改进，能够准确重用跨帧信息，并在3D场景重建中展现领先性能。

Conclusion: 研究揭示了将基础模型集成到移动稀疏感知系统中的前景和开放挑战，基础模型有潜力重塑移动稀疏感知的格局。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法主要关注跨模态互补性，但忽视了模态特异性特征的重要性，且模态间的分布差异会导致有偏和噪声表示，降低分类性能。

Method: 提出协作注意力和一致性引导融合框架，包含可学习参数表示块补偿缺失模态信息，共享编码器和模态独立编码器保留共享和特异性表示，以及一致性引导机制对齐跨模态潜在分布。

Result: 在ADNI数据集上的实验结果表明，该方法相比现有融合策略实现了更优越的诊断性能。

Conclusion: 该方法通过有效融合多模态神经影像特征，显著提升了阿尔茨海默病的诊断准确性，为早期诊断提供了有力工具。
...
TL;DR: 提出了一种潜在特征对齐方法，通过对抗学习和方向特征一致性来减少内窥镜视频中合成图像与真实图像之间的域差距，从而改进单目深度估计的绝对深度精度。


<details>
  <summary>Details</summary>
Motivation: 在内窥镜手术场景中，从内窥镜相机获取绝对深度信息很困难，这限制了在真实内窥镜图像上进行监督深度学习。现有的图像级无监督域适应方法在合成图像与真实图像之间仍存在域差距。

Method: 使用潜在特征对齐方法，深度网络以翻译后的合成图像和真实内窥镜帧作为输入，通过对抗学习和方向特征一致性学习域不变特征。该方法与图像翻译过程无关，专注于深度估计本身。

Result: 在中央气道模型的内窥镜视频上评估，相比最先进的单目深度估计方法，该方法在绝对和相对深度指标上都取得了优越性能，并在各种骨干网络和预训练权重上一致改进结果。

Conclusion: 提出的潜在特征对齐方法有效减少了域差距，显著提高了内窥镜视频中单目深度估计的绝对深度精度，为自主医疗机器人提供了更可靠的深度指导。
...
TL;DR: 提出HTSC-CIF框架，通过分层任务分解解决医学报告生成中的三个关键挑战：领域知识理解不足、文本-视觉实体嵌入对齐差、跨模态偏差导致的伪相关性。


<details>
  <summary>Details</summary>
Motivation: 现有医学报告生成模型面临三个主要挑战：领域知识理解不足、文本-视觉实体嵌入对齐差、跨模态偏差导致的伪相关性。先前工作仅解决单一挑战，本文旨在同时解决所有三个挑战。

Method: HTSC-CIF框架将挑战分为三个层次：低层任务对齐医学实体特征与空间位置；中层任务使用前缀语言建模和掩码图像建模增强跨模态对齐；高层任务通过跨模态因果干预模块减少混杂因素。

Result: 大量实验证实HTSC-CIF的有效性，显著优于最先进的医学报告生成方法。

Conclusion: HTSC-CIF框架通过分层任务分解成功解决了医学报告生成中的三个关键挑战，为可靠病灶描述提供了有效解决方案。
...
TL;DR: 本文探讨了使用欧拉角参数化作为归一化流模型基础来进行物体姿态估计的有效性，尽管欧拉角存在一些缺点，但在某些方面可能比更复杂的参数化方法更有用。


<details>
  <summary>Details</summary>
Motivation: 物体姿态估计在3D计算机视觉中至关重要。虽然单点估计通常足够，但在传感器和投影约束或物体固有对称性导致姿态不明确的情况下，概率姿态输出具有诸多优势。

Method: 使用欧拉角参数化作为归一化流模型的基础，探索这种参数化方法在姿态估计中的应用。

Result: 研究发现欧拉角参数化尽管有其局限性，但在某些方面能够产生有用的模型。

Conclusion: 欧拉角参数化在姿态估计中可能比更复杂的参数化方法在某些方面更有优势，值得进一步探索。
...
TL;DR: SAIL-RL是一个强化学习后训练框架，通过教导多模态大语言模型何时以及如何思考来增强其推理能力，解决了现有方法中结果导向监督和统一思考策略的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：仅基于结果的监督只奖励正确答案而不确保推理质量；统一的思考策略导致简单任务过度思考而复杂任务思考不足。
...
<details>
  <summary>Details</summary>
Motivation: 手写数学表达式识别需要准确捕捉符号间的空间关系，传统方法难以有效处理复杂的二维结构布局。

Method: 使用深度BLSTM网络进行符号分割、识别和空间关系分类构建初始图，然后结合2D-CFG解析器和GNN链接预测模型来优化图结构。

Result: 实验结果表明该方法在手写数学表达式结构识别方面表现出色，具有良好性能。

Conclusion: 基于图神经网络的方法能够有效识别手写数学表达式的结构，为复杂二维数学表达式识别提供了有前景的解决方案。
...
TL;DR: Cycle-Sync是一个用于相机位姿估计的全局框架，通过改进的消息传递最小二乘法强调循环一致性，无需相机间距离信息即可实现最低样本复杂度，并在实验中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的相机位姿估计方法通常需要相机间距离信息，且对异常值敏感。本文旨在开发一个仅依赖循环一致性就能实现精确恢复的鲁棒全局框架。

Method: 将消息传递最小二乘法(MPLS)改进用于相机位置估计，强调循环一致性信息，重新定义循环一致性并使用Welsch型鲁棒损失函数，同时集成了基于鲁棒子空间恢复的异常值剔除模块。

Result: 建立了相机位置估计中最强的确定性精确恢复保证，仅通过循环一致性就能达到目前已知的最低样本复杂度。在合成和真实数据集上的实验表明，Cycle-Sync始终优于领先的位姿估计器。

Conclusion: Cycle-Sync提供了一个无需捆绑调整的全局相机位姿估计框架，仅依赖循环一致性就能实现鲁棒且精确的位姿恢复，在理论和实验上都表现出色。
...
TL;DR: 提出GAFD-CC方法，通过全局感知特征解耦和置信度校准来改进OOD检测性能，利用特征与logits的关联性提升检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有后处理OOD检测方法通常忽略特征与logits之间的内在关联，这限制了OOD检测的有效性。

Method: GAFD-CC包含两个核心步骤：1) 基于分类权重的全局感知特征解耦，分离出正相关和负相关特征；2) 自适应融合解耦特征与多尺度logit置信度。
...
### [27] [M3PD Dataset: Dual-view Photoplethysmography (PPG) Using Front-and-rear Cameras of Smartphones in Lab and Clinical Settings](https://arxiv.org/abs/2511.02349)
...
<details>
  <summary>Details</summary>
Motivation: 解决便携式生理监测设备可及性差和视频光电容积描记技术因运动伪影、光照变化和单视角限制导致的可靠性问题，特别是在心血管患者中的应用挑战。

Method: 构建首个公开的双视角移动光电容积描记数据集M3PD，包含60名参与者（47名心血管患者）的同步面部和指尖视频；提出F3Mamba模型，通过Mamba时序建模融合双视角信息。

Result: F3Mamba模型相比现有单视角基线，心率误差降低了21.9%至30.2%，在具有挑战性的真实场景中表现出更好的鲁棒性。

Conclusion: 双视角移动光电容积描记技术结合Mamba时序建模能够显著提升心率监测的准确性和可靠性，为心血管疾病的无创监测提供了有前景的解决方案。
...
TL;DR: CoCoVa是一个新颖的视觉语言模型框架，通过连续跨模态推理解决传统VLM在离散语言空间推理的局限性，使用潜在思想向量链进行迭代推理，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 人类认知中存在许多难以用语言表达的思维过程，而当前的视觉语言模型被限制在离散的语言标记空间中进行推理，这限制了视觉感知的丰富性和高维特性。

Method: 提出CoCoVa框架，核心是迭代推理循环：使用Latent Q-Former作为动态推理引擎，通过跨模态融合迭代优化潜在思想向量链；采用token选择机制动态识别显著视觉区域；通过对比学习和基于扩散的重构多任务训练确保潜在表示与视觉和文本模态的对齐。

Result: CoCoVa在准确性和token效率上优于强基线模型。使用1.5B骨干网络时，在几乎所有基准测试中与7B-9B大模型相当或更优；扩展到7B LLM骨干时仍与最先进模型保持竞争力。定性分析显示学习的潜在空间捕获了可解释的结构化推理模式。

Conclusion: CoCoVa通过连续推理机制有效弥合了离散语言处理与连续视觉理解之间的表征差距，展示了在视觉语言任务中采用连续推理的潜力。
...
TL;DR: RxnCaption框架将化学反应图解析重新定义为图像描述问题，使用BIVP策略和MolYOLO分子检测器，在RxnCaption-11k数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有化学反应数据多为论文中的图像格式，无法被机器读取和用于机器学习模型训练。

Method: 将坐标预测驱动的解析过程重新定义为图像描述问题，采用BIVP策略，使用MolYOLO预绘制分子边界框和索引。

Result: BIVP策略显著提高结构提取质量，RxnCaption-VL在多个指标上达到最先进性能。
...
TL;DR: 提出一种基于对比自监督学习的雷达点云运动目标分割方法，通过聚类对比损失和动态点移除来预训练网络，提高标注效率。


<details>
  <summary>Details</summary>
Motivation: 雷达传感器能直接测量点的多普勒速度，适合单次扫描的运动目标分割，但雷达点云稀疏且噪声大，标注成本高，需要自监督方法来减少标注依赖。

Method: 采用两步法：首先使用基于聚类的对比自监督表示学习预训练网络，包含动态点移除的聚类细化；然后用少量标注数据进行有监督微调。

Result: 该方法提高了标注效率，通过自监督预训练有效提升了最先进方法的性能。

Conclusion: 提出的自监督学习方法能有效解决雷达点云运动目标分割中的标注难题，提升模型性能同时降低标注成本。
...
TL;DR: 提出了一种基于分组的混合颜色校正算法，用于彩色点云的颜色一致性校正。根据点云重叠率将目标点分为2-3组，对不同距离组采用不同的颜色校正方法。


<details>
  <summary>Details</summary>
Motivation: 彩色点云的颜色一致性校正是3D渲染和压缩应用中的重要任务，现有方法主要针对彩色图像，缺乏专门针对点云的有效解决方案。

Method: 1. 估计源点云和目标点云的重叠率；2. 根据重叠率将目标点自适应分为近距离组Gcl、中距离组Gmod和远距离组Gdist；3. 对Gcl使用K近邻双边插值(KBI)，对Gmod使用联合KBI和直方图均衡(JKHE)，对Gdist使用直方图均衡(HE)。

Result: 在1086个测试彩色点云对上验证了算法的有效性，优于现有最先进方法。

Conclusion: 该分组混合颜色校正算法能够有效实现彩色点云的颜色一致性校正，具有无分组效应特性，代码已开源。
...
TL;DR: 该研究通过统一基准测试发现，自监督ViT模型在猫和人类视觉系统表征对齐方面表现最佳，特别是在早期层，表明自监督学习能产生跨越物种统计差异的视觉特征。


<details>
  <summary>Details</summary>
Motivation: 研究猫和人类视觉系统之间的表征对齐，特别是探索不同神经网络架构（CNN、监督ViT、自监督ViT等）在跨物种视觉表征方面的表现差异。

Method: 使用统一冻结编码器基准，通过层间中心核对齐（线性和RBF）和表征相似性分析等方法，量化猫-人类跨物种表征对齐，测试了卷积网络、监督Vision Transformers、窗口化变换器和自监督ViT等多种模型。

Result: DINO ViT-B/16获得最高的对齐度（平均CKA-RBF≈0.814，平均CKA-linear≈0.745，平均RSA≈0.698），在早期块达到峰值；监督ViT在CKA上具有竞争力但在几何对应上较弱；CNN表现良好但低于普通ViT；窗口化变换器表现最差。

Conclusion: 自监督学习与ViT归纳偏置相结合，产生的表征几何比广泛使用的CNN和窗口化Transformer更接近猫和人类视觉系统的对齐，为跨物种视觉计算收敛提供了可测试的神经科学假设。
...
<details>
  <summary>Details</summary>
Motivation: 解决低光照图像增强中光照变化和噪声的挑战，传统方法难以同时处理光照调整和噪声去除。

Method: 基于Retinex理论分解图像为反射和光照分量，使用条件整流流框架建模光照变化，结合去噪网络处理反射分量中的噪声。

Result: 在低光照增强和曝光校正任务上表现出优越的定量和定性性能，优于现有方法。
...
TL;DR: 提出了一个自动化多阶段代码驱动管道，用于生成视觉推理数据集ChartM³，包含38K图表和142K问答对，显著提升多模态大语言模型在复杂图表理解中的推理能力和跨域泛化性能。
...
Method: 采用自动化多阶段代码驱动管道，集成检索增强生成(RAG)获取专业图表模板，使用思维链(CoT)策略生成推理代码模拟真实数据分布，驱动图表渲染和统计计算。

Result: 构建了ChartM³数据集，包含38K图表和142K问答对，通过监督微调(SFT)和强化学习(RL)实验证明该数据集显著提升模型推理能力和跨域泛化性能。

Conclusion: 该自动化管道生成的数据集能有效提升模型在复杂图表理解任务中的表现，使较小模型达到与大规模模型相当的性能水平。
...
<details>
  <summary>Details</summary>
Motivation: 农业除草机器人需要精确的语义分割，但真实标注数据获取成本高，合成数据与真实图像之间存在域差距问题。

Method: 使用Blender构建程序化生成合成作物-杂草图像的流程，生成包含不同植物生长阶段、杂草密度、光照和相机角度的标注数据集。

Result: 在合成图像上训练的分割模型实现了10%的sim-to-real差距，优于现有方法，且在跨域场景中表现优于真实数据集。

Conclusion: 合成农业数据集具有巨大潜力，支持采用混合策略进行更高效的模型训练。
...
Conclusion: 该研究为在移动机器人边缘设备上部署小型视觉语言模型提供了重要见解，揭示了其在实际应用中的可行性和局限性。
...
### [37] [DetectiumFire: A Comprehensive Multi-modal Dataset Bridging Vision and Language for Fire Understanding](https://arxiv.org/abs/2511.02495)
*Zixuan Liu,Siavash H. Khajavi,Guangkai Jiang*

Main category: cs.CV

TL;DR: 提出了DetectiumFire数据集，包含22.5k高清火灾图像和2.5k真实火灾视频，覆盖多种火灾类型和环境，解决了火灾领域多模态模型应用的数据缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在图像生成和推理任务中表现良好，但在火灾领域应用仍面临挑战，主要原因是缺乏公开可用的高质量火灾标注数据集。

Method: 构建大规模多模态数据集DetectiumFire，包含高分辨率火灾图像和真实火灾视频，标注包括传统计算机视觉标签（如边界框）和详细文本提示，支持合成数据生成和火灾风险推理等应用。

Result: DetectiumFire在规模、多样性和数据质量方面优于现有基准，显著减少了冗余并增强了真实场景覆盖。在目标检测、扩散图像生成和视觉语言推理等任务中验证了其有效性。

Conclusion: 该数据集有潜力推动火灾相关研究并支持智能安全系统开发，已公开发布以促进AI社区对火灾理解的更广泛探索。
...
Method: 提出KAO框架，采用核自适应优化和扩散模型，引入潜在空间条件化方法优化紧凑潜在空间，并在扩散过程中加入显式传播实现前向-后向融合。

Result: 实验结果表明KAO在DeepGlobe和Massachusetts Roads等VHR卫星数据集上设立了新的修复基准，提供了可扩展的高性能解决方案。

Conclusion: KAO在预条件模型的效率和后条件模型的灵活性之间取得了平衡，为超高分辨率卫星图像修复提供了高效准确的解决方案。
...
### [39] [UniChange: Unifying Change Detection with Multimodal Large Language Model](https://arxiv.org/abs/2511.02607)
...
Method: 利用MLLM的语言先验和统一能力，引入三个特殊标记[T1]、[T2]和[CHANGE]，通过文本提示指导变化类别识别，无需预定义分类头，能够从类定义冲突的多源数据集中有效学习。

Result: 在四个公开基准测试(WHU-CD、S2Looking、LEVIR-CD+、SECOND)上达到最先进性能，IoU分数分别为90.41、53.04、78.87和57.62，超越了所有先前方法。

Conclusion: UniChange成功统一了BCD和SCD任务，展示了MLLM在变化检测领域的潜力，为多源数据融合和统一框架设计提供了新思路。
...
TL;DR: 提出MVAFormer方法，用于多视角时空动作识别，通过基于transformer的协作模块处理多视图特征，在STAR设置下优于基线方法4.4个F-measure点。


<details>
  <summary>Details</summary>
Motivation: 解决传统多视角动作识别方法无法适用于时空动作识别(STAR)设置的问题，传统方法只关注从整个视频中识别单个动作，而STAR需要按时间顺序识别每个人的动作。

Method: 提出MVAFormer方法，使用基于transformer的视图间协作模块，利用保留空间信息的特征图而非丢失空间信息的嵌入向量，并将自注意力分为相同视图和不同视图来有效建模多视图关系。

Result: 在新收集的数据集上的实验结果表明，MVAFormer在F-measure上比比较基线高出约4.4个点。

Conclusion: MVAFormer通过保留空间信息的特征图和专门设计的transformer协作模块，在时空动作识别设置下实现了有效的多视图协作和性能提升。
...
TL;DR: OLATverse是一个大规模数据集，包含765个真实世界物体的约900万张图像，这些图像在精确控制的光照条件下从多个视角拍摄，用于物体中心逆向渲染和重光照研究。
...
Method: 收集765个常见和不常见的真实世界物体，使用35台DSLR相机和331个独立控制的光源进行拍摄，模拟多样化的光照条件，并提供相机参数、物体掩码、表面法线和漫反射反照率等辅助资源。

Result: 构建了大规模的真实物体数据集，建立了首个全面的真实世界物体中心逆向渲染和法线估计基准测试集。
...
<details>
  <summary>Details</summary>
Motivation: 随着低空经济发展，无人机在巡逻系统中应用广泛，但在GNSS拒止区域卫星定位方法容易失效，需要解决跨时空、跨视角的异构航拍图像匹配问题。

Method: 利用现代目标检测技术从无人机和卫星图像中提取显著实例，集成图神经网络推理图像间和图像内的节点关系，使用细粒度的基于图的节点相似性度量。

Result: 在公开和真实数据集上的大量实验表明，该方法能有效处理异构外观差异，泛化能力强，适用于更大模态差距的场景如红外-可见光图像匹配。

Conclusion: 提出的基于目标检测和图神经网络的跨视角无人机定位框架在异构图像匹配方面表现出色，为解决GNSS拒止环境下的无人机定位问题提供了有效方案。
...
TL;DR: VCode是一个将多模态理解重新定义为代码生成的基准，使用SVG作为紧凑、可解释的可执行视觉表示。通过CodeVQA评估协议验证符号保真度，并提出了VCoder代理框架来改进VLM在视觉编码方面的性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI进展主要集中在语言中心任务上，而视觉中心编码未被充分探索。受人类通过草图推理的启发，提倡使用SVG代码作为视觉表示。

Method: 1. 引入VCode基准，将多模态理解重新定义为SVG代码生成；2. 提出CodeVQA评估协议，通过策略模型在渲染的SVG上回答问题来评估符号保真度；3. 开发VCoder代理框架，包含"思考与修订"和"视觉工具行动"两个维度。

Result: 前沿VLM在生成忠实SVG方面表现不佳，揭示了语言中心与视觉中心编码之间的差距。VCoder在基准测试中比表现最佳的Claude-4-Opus提高了12.3分。人类研究显示人类和VLM在渲染SVG上表现都更差，但一致性揭示了符号视觉表示的前景。

Conclusion: SVG代码作为视觉表示具有潜力，但当前VLM在视觉中心编码方面仍有局限。VCoder框架通过迭代修订和视觉工具使用有效提升了性能，为视觉推理开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决基础模型在专业科学任务中的适应问题，探索最优的领域适应策略。

Method: 引入PtychoBench基准，系统比较监督微调(SFT)和上下文学习(ICL)两种专业化策略，在视觉伪影检测和文本参数推荐任务上进行评估。

Result: 视觉任务中SFT和ICL互补，微调模型在上下文感知示例引导下达到最高性能；文本任务中ICL策略更优，超越强大的监督微调模型。
...
TL;DR: 提出了一种基于能量优化的视频镜头组装方法，通过学习参考视频的组装风格来自动化视频编辑过程。


<details>
  <summary>Details</summary>
Motivation: 传统视频编辑依赖人工经验，现有智能视频编辑技术难以捕捉创作者的独特艺术表达，需要一种能学习参考视频风格并自动组装镜头的方法。

Method: 首先通过大语言模型生成脚本并与视频库进行视觉语义匹配获取候选镜头，然后从参考视频中提取镜头属性（尺寸、相机运动、语义等），使用基于能量的模型学习这些属性并评分候选镜头序列，最后结合多种语法规则优化镜头组装。

Result: 该方法不仅能按照特定逻辑、叙事需求或艺术风格自动排列组合独立镜头，还能学习参考视频的组装风格，创建连贯的视觉序列。

Conclusion: 该方法使得没有视频编辑经验的用户也能创建视觉上吸引人的视频，实现了从参考视频中学习组装风格并自动化视频编辑的目标。
...
TL;DR: 提出一种基于本地模型的自动化报告生成管道，用于移动机器人系统，保护隐私且无需外部服务
...
Method: 使用多模态传感器和本地模型构建自动化报告生成管道，可在边缘计算设备上部署，保护隐私且无需外部服务

Result: 在包含室内、室外和城市环境的多样化数据集上评估实现，提供定量和定性评估结果，生成示例报告可通过公共仓库获取

Conclusion: 提出的自动化报告生成管道能够有效支持移动机器人系统的评估，同时保护隐私且不依赖外部服务
...
TL;DR: LiteVoxel是一种自调优的稀疏体素光栅化训练管道，通过逆Sobel重加权和EMA滞后保护机制，解决了传统方法在低频内容拟合不足、VRAM过度增长的问题，显著降低内存使用同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏体素光栅化方法存在三个主要问题：对低频内容拟合不足、依赖脆弱的剪枝启发式方法、以及会导致VRAM过度增长的过度生长问题。

Method: 采用逆Sobel重加权损失函数，结合训练中期的gamma斜坡；使用基于深度分位数的剪枝逻辑，通过EMA滞后保护稳定化；在显式增长预算下进行基于射线足迹的优先级驱动细分。

Result: 在Mip-NeRF 360和Tanks & Temples数据集上的测试显示，显著减少了低频区域和边界不稳定性错误，同时保持PSNR/SSIM、训练时间和FPS与强基线相当，峰值VRAM降低40%-60%。
...
TL;DR: 本研究探索无监督学习方法用于剪切散斑图像的自动异常检测，评估了三种架构：全连接自编码器、卷积自编码器和师生特征匹配模型，发现师生方法在分类鲁棒性和缺陷定位方面表现最优。


<details>
  <summary>Details</summary>
Motivation: 剪切散斑技术虽然具有高灵敏度和全场检测能力，但由于需要专家解释，工业应用受限。本研究旨在减少对标记数据和人工评估的依赖，探索无监督学习方法实现自动化异常检测。

Method: 开发了包含可重复缺陷模式的控制数据集，在理想和实际变形条件下系统采集剪切散斑测量数据。评估了三种仅使用无缺陷数据训练的无监督模型：全连接自编码器、卷积自编码器和师生特征匹配模型。

Result: 师生特征匹配方法在分类鲁棒性方面表现最优，能够实现精确的缺陷定位，相比自编码器模型显示出更好的特征表示可分性。使用YOLOv8模型作为定位质量的基准参考。
...
Method: 实现并评估了五种深度学习架构（UNet、U2-Net、UNETR、Time-Embedding UNet和ODE-UNet），在两个纵向队列（ADNI和AIBL）上进行训练和测试，使用全局相似性和局部差异指标直接比较预测的随访MRI与实际扫描。

Result: 最佳性能模型实现了高保真度预测，所有模型在独立外部数据集上泛化良好，展示了稳健的跨队列性能。

Conclusion: 深度学习可以可靠地在体素级别预测参与者特定的脑部MRI，为个体化预后提供了新机会。
...
TL;DR: UVH-26是印度首个大规模交通摄像头图像数据集，包含26,646张高分辨率图像和180万个标注框，涵盖14个印度特有车辆类别。该数据集显著提升了车辆检测模型在印度交通场景中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有全球基准数据集缺乏针对印度复杂交通场景的数据，无法有效支持印度智能交通系统的发展。UVH-26旨在填补这一空白，为新兴国家复杂交通条件下的检测和分类提供基础。

Method: 通过2800个班加罗尔安全城市CCTV摄像头采集图像，组织565名大学生参与众包标注，使用多数投票和STAPLE算法生成共识标注，训练YOLO11、RT-DETR和DAMO-YOLO等多种检测器。

Result: 在UVH-26上训练的模型相比COCO数据集训练的基线模型，mAP50:95指标提升了8.4-31.5%。RT-DETR-X表现最佳，mAP50:95达到0.67，而COCO训练的模型仅为0.40。

Conclusion: UVH-26数据集有效解决了现有全球基准在印度交通场景下的不足，证明了领域特定训练数据对印度复杂交通条件的重要性，为新兴国家智能交通系统的发展提供了重要基础。
...
Method: 基于ViT-B/16骨干网络，引入7个参数高效模块：跨流特征归一化、多分辨率特征协调、身份感知记忆模块、时序动态建模、跨视角特征对齐、分层时序模式学习和多视角身份一致性学习。

Result: 在AG-VPReID基准测试中实现最先进性能，在所有高度级别均表现优异，并在G2A-VReID和MARS数据集上展现出强泛化能力，同时保持189 FPS的实时效率。
...
TL;DR: VCFlow是一个无需特定受试者训练即可从fMRI重建视觉体验的层次化解码框架，通过模拟人类视觉系统的腹背侧结构来学习多维表征，在牺牲7%准确率的情况下实现快速重建。


<details>
  <summary>Details</summary>
Motivation: 解决跨受试者泛化问题，开发无需特定受试者训练即可从fMRI重建连续视觉体验的方法，具有临床应用的巨大潜力。

Method: 提出VCFlow层次化解码框架，明确建模人类视觉系统的腹背侧架构，分离并利用早期视觉皮层、腹侧和背侧流的特征，采用特征级对比学习策略增强主体不变语义表征提取。

Result: 相比传统方法需要12小时以上数据和大量计算，VCFlow仅牺牲7%平均准确率，无需重新训练即可在10秒内生成重建视频，提供快速且临床可扩展的解决方案。

Conclusion: VCFlow为无需特定受试者训练的脑解码提供了有效且实用的解决方案，在准确性和效率之间取得了良好平衡，具有重要的临床应用价值。
...
### [53] [TAUE: Training-free Noise Transplant and Cultivation Diffusion Model](https://arxiv.org/abs/2511.02580)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型只能输出单一平面图像，无法满足专业应用对分层控制的需求。现有解决方案要么需要大量数据集进行微调，要么只能生成孤立前景元素而无法产生完整连贯的场景。

Method: 提出噪声移植和培养（NTC）技术，从前景和合成生成过程中提取中间潜在表示，将其移植到后续层的初始噪声中，确保各层间的语义和结构一致性。
...
TL;DR: 提出一个零样本多动物追踪框架，结合Grounding Dino检测器和SAM 2追踪器，无需重新训练即可应用于新数据集。


<details>
  <summary>Details</summary>
Motivation: 多动物追踪对理解动物生态和行为至关重要，但传统方法需要大量模型微调和启发式设计，限制了应用范围。
...
Conclusion: 该框架展示了视觉基础模型在零样本多动物追踪中的潜力，无需重新训练即可适应新场景，代码已开源。
...
TL;DR: 提出了一种轻量级CNN框架，用于检测面部识别系统中的打印/显示、视频和包裹攻击，确保快速生物特征认证。
...
Conclusion: 提出的鲁棒架构提供了无缝的活体检测，有效防止面部识别系统的呈现攻击。
...
TL;DR: UniPruneBench是一个统一的可扩展基准，用于评估多模态大模型中视觉token剪枝方法，涵盖6个能力维度和10个数据集，评估了10种剪枝算法在3种LMM模型上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型存在严重的推理效率问题，主要由于图像编码器引入的大量视觉token。现有的token压缩方法评估零散且不一致，需要统一的评估框架。

Method: 构建UniPruneBench基准，提供标准化协议，涵盖多种能力维度和数据集，评估代表性压缩算法在不同LMM模型上的表现，并考虑系统级指标如运行时间和预填充延迟。

Result: 发现随机剪枝是一个强基线方法；没有单一方法在所有场景中表现最佳；不同任务对剪枝的敏感度差异显著，OCR任务最脆弱；剪枝比例是影响性能下降的主导因素。

Conclusion: UniPruneBench为高效多模态建模的未来研究提供了可靠基础，揭示了剪枝方法评估的重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统Vision Transformers使用固定的patch token，忽略了图像的空间和语义结构，需要更灵活的自适应token生成方法。

Method: 使用分层模型选择和信息准则，实现像素级粒度的自适应token生成，支持端到端训练。

Result: 在图像级分类和密集预测任务中表现优异，还支持开箱即用的栅格到矢量转换。

Conclusion: 该方法提供了更灵活的token生成方案，能够更好地捕捉图像结构信息，同时保持与现有预训练模型的兼容性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖中间表示来对齐跨模态特征，但存在参数多、可解释性差、未能充分利用中间特征等问题。可见光和红外模态之间存在显著差异，需要更有效的特征对齐方法。
...
Conclusion: 提出的MTRL框架无需额外参数就能有效对齐跨模态特征，在VI-ReID任务上实现了优越性能，同时保持了高效的推理速度。
...
TL;DR: 提出了一种情感线索引导的推理框架，通过两阶段调优的视频情感基础模型(VidEmo)来解决视频情感分析的挑战，并建立了包含210万样本的情感中心细粒度数据集(Emo-CFG)。


<details>
  <summary>Details</summary>
Motivation: 视频情感分析面临情感动态性和线索依赖性的挑战，现有方法难以理解复杂且不断演化的情感状态及其合理推理。

Method: 采用阶段式方法统一基础属性感知、表情分析和高层情感理解，核心是视频情感基础模型(VidEmo)，通过课程情感学习和情感树强化学习两阶段调优。

Result: 实验结果表明该方法在15个人脸感知任务上取得了竞争性性能，设立了新的里程碑。
...
Method: 引入LLEXICORP，将CRP与多模态大语言模型耦合，通过示例教导语言模型CRP的语义，并强制分离命名和解释任务，生成可针对不同受众定制的文本解释。

Result: 在ImageNet数据集上的VGG16模型上进行了定性评估，结果表明该方法能够自动生成描述性概念名称和直观的自然语言解释。
...
TL;DR: 该研究首次对视频-文本表示对齐进行系统性分析，揭示了跨模态对齐对视觉和文本数据丰富度的依赖性，提出了参数化测试时缩放定律，并探讨了语义对齐与下游任务性能的关联。


<details>
  <summary>Details</summary>
Motivation: 虽然图像与文本的对齐研究已取得进展，但视频数据的时序特性在此背景下仍未被充分探索。本研究旨在填补这一空白，系统分析现代视频和语言编码器的跨模态对齐能力。

Method: 通过分析不同丰富度的视觉数据（静态图像vs多帧视频）和文本数据（单字幕vs集合），提出参数化测试时缩放定律来捕捉对齐行为，并研究语义对齐与下游任务性能的关联。

Result: 发现跨模态对齐高度依赖于测试时提供的视觉和文本数据丰富度；提出的缩放定律对经验观察具有显著预测能力；强对齐与通用视频表示和理解能力相关；时序推理与跨模态对齐存在关联。

Conclusion: 视频-文本对齐可作为零样本探测时空数据表示能力的信息性方法，为视觉和语言模型提供了具有挑战性的测试基准。
...
TL;DR: PercHead是一种从单张图像进行3D头部重建和语义编辑的方法，通过双分支编码器和ViT解码器实现视图一致的3D重建，并使用基于DINOv2和SAM2.1的感知监督策略提升几何和外观保真度。该方法在视角合成方面达到最先进水平，并能通过轻量级GUI实现直观的3D编辑。


<details>
  <summary>Details</summary>
Motivation: 单图像3D头部重建和语义编辑面临严重视角遮挡、弱感知监督和3D空间编辑模糊性等挑战，需要开发能够同时处理这两个任务的统一方法。

Method: 使用双分支编码器和基于ViT的解码器，通过迭代交叉注意力将2D特征提升到3D空间，采用高斯泼溅进行渲染。核心是基于DINOv2和SAM2.1的感知监督策略。编辑时通过交换编码器和微调网络，使用分割图控制几何，文本或参考图像指定外观。

Result: 在视角合成方面达到最先进性能，对极端视角表现出卓越的鲁棒性。能够通过轻量级交互式GUI实现直观的3D编辑功能。

Conclusion: PercHead提供了一个统一的框架，既能实现高质量的3D头部重建，又能支持直观的语义编辑，在几何和外观保真度方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统思维链方法仅依赖文本，而许多复杂问题需要视觉化思考。MIRA旨在评估模型在需要视觉中间步骤的推理任务中的能力，模拟人类"画图思考"的过程。

Method: 提出统一评估协议，包含三个输入级别：仅图像和问题的直接输入、带思维提示的文本思维链输入、以及带注释图像线索和文本提示的视觉思维链输入。使用pass@k和多数投票准确率评估模型上限。

Result: 实验显示现有多模态大语言模型在仅依赖文本提示时表现较差，但当提供中间视觉线索时，所有模型和任务的平均相对增益达到33.7%。扩展搜索空间和设计对齐的文本提示仅带来有限改进。

Conclusion: 结果强调了想象视觉信息在MIRA基准上成功推理的关键作用，表明视觉中间步骤对于复杂推理任务至关重要。
...
TL;DR: PLUTO-4是新一代病理学基础模型，包含两个互补的Vision Transformer架构：紧凑高效的PLUTO-4S和前沿规模的PLUTO-4G，在多个病理学基准测试中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 基于大规模病理图像语料库训练的基础模型在多样化组织病理学任务中展现出强大的迁移能力，需要进一步扩展到前沿规模以提升性能。

Method: 使用来自50多个机构的551,164张WSI图像，采用DINOv2的自监督目标进行预训练。PLUTO-4S采用FlexiViT设置和2D-RoPE嵌入进行多尺度部署优化，PLUTO-4G使用单一补丁尺寸最大化表示能力和稳定性。

Result: PLUTO-4在需要不同空间和生物学背景的任务中实现最先进性能，包括补丁级分类、分割和幻灯片级诊断。PLUTO-4S提供高吞吐量和稳健性能，PLUTO-4G在多个病理学基准测试中建立新性能前沿，皮肤病理学诊断提升11%。

Conclusion: PLUTO-4作为转化研究和诊断用例的骨干模型，具有改变实际应用的潜力，其多样化改进强调了模型在现实世界应用中的价值。
...
TL;DR: DenseMarks是一种学习到的人头表示方法，通过Vision Transformer网络为每个像素预测3D嵌入，在3D规范立方体中建立对应关系，可用于密集对应、面部跟踪和立体重建。


<details>
  <summary>Details</summary>
Motivation: 为了解决人类头部图像的高质量密集对应问题，特别是在不同姿态和个体间建立一致的语义对应关系。

Method: 使用Vision Transformer网络预测像素级3D嵌入，通过对比损失训练，结合多任务学习（面部关键点和分割约束），并利用潜在立方体特征保证空间连续性。

Result: 在几何感知点匹配和单目头部跟踪方面达到最先进水平，对姿态变化具有鲁棒性，覆盖整个头部包括头发。

Conclusion: DenseMarks提供了一种可解释和可查询的规范空间表示，能够实现跨姿态和个体的一致头部对应关系。
...
TL;DR: 提出了MPG框架，通过解码时组合范式实现多人格生成，无需重新训练或依赖外部模型，利用单维模型的隐式密度比作为"免费午餐"，并设计了SCR采样方法来高效实现。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，基于重新训练的方法成本高且扩展性差，而解码时方法通常依赖外部模型或启发式方法，限制了灵活性和鲁棒性。需要一种无需重新训练、不依赖外部模型的多人格生成方法。

Method: 提出MPG框架，将多人格生成任务重新表述为从目标策略中采样，该策略聚合了单维模型的隐式密度比。设计了SCR采样方法，以块为单位生成响应，并通过滑动窗口内的估计阈值并行验证，显著降低计算开销。

Result: 在MBTI人格和角色扮演任务上的实验表明，MPG方法有效，性能提升达到16%-18%。

Conclusion: MPG框架提供了一种高效、灵活的多人格生成方法，无需重新训练或依赖外部模型，通过利用单维模型的隐式密度比和SCR采样技术实现了高质量生成。
...
TL;DR: GEMS提出了一种基于图神经网络的轻量级人类模拟方法，在离散选择任务中性能媲美甚至超越大型语言模型，但模型规模小三个数量级。


<details>
  <summary>Details</summary>
Motivation: 探索在人类模拟任务中，是否必须使用大型语言模型，还是可以用更小、领域特定的模型来替代。

Method: 将离散选择模拟任务构建为图上的链接预测问题，使用图神经网络，仅在需要时融入语言表示。
...
Method: 两阶段方法：(1)通过语义聚类和L0优化发现多样化掩码候选；(2)在推理时实现高效动态剪枝，无需大量训练。

Result: 实验结果表明该方法持续优于最先进的静态深度剪枝方法。

Conclusion: 该方法特别适合资源受限的部署场景，为LLM高效推理提供了有效解决方案。
...
TL;DR: 开发了一个自动生成查询和评估医疗聊天机器人回答的基础设施，发现LLM评估者之间一致性低，建议使用多个LLM作为评估者以避免统计显著但不具普适性的结果。


<details>
  <summary>Details</summary>
Motivation: 医疗聊天机器人在涉及非医疗因素（如人口统计信息）时必须提供一致的建议，但目前LLM在医疗场景中存在幻觉、遗漏和偏见问题。

Method: 1) 自动生成查询的提示创建管道，采样患者人口统计、病史、疾病和写作风格；2) 使用多个LLM-as-a-judge设置和提示的评估管道，包括幻觉和遗漏检测。

Result: LLM注释者表现出低一致性分数（平均Cohen's Kappa κ=0.118），只有特定的（回答，评估）LLM对在不同写作风格、性别和种族上产生统计显著差异。

Conclusion: 建议使用多个LLM作为评估者，并发布LLM间一致性指标以提高透明度，特别是在缺乏真实数据的情况下。
...
TL;DR: LTD-Bench是一个突破性的基准测试，通过要求模型通过点阵或可执行代码生成绘图，将LLM评估从抽象分数转变为直接可观察的视觉输出，揭示了现有模型在空间推理方面的严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估范式存在关键盲点，依赖不透明的数值指标掩盖了空间推理的基本局限性，无法提供对模型能力的直观理解，这在需要物理世界理解的应用中造成了危险的脱节。

Method: LTD-Bench采用综合方法，包含互补的生成任务（测试空间想象力）和识别任务（评估空间感知），跨越三个渐进难度级别，系统地评估语言-空间映射的两个关键方向。

Result: 对最先进模型的大量实验暴露了令人担忧的能力差距：即使在传统基准测试中表现优异的LLM，在建立语言和空间概念之间的双向映射方面也表现出严重缺陷。
...
Conclusion: M-Solomon通过自适应查询增强策略，在多模态环境中实现了更好的性能和效率平衡，证明了选择性增强的有效性。
...
TL;DR: LiveSecBench是一个专门针对中文LLM应用场景的动态安全基准，评估模型在合法性、伦理、事实性、隐私、对抗鲁棒性和推理安全六个维度的表现，并保持动态更新。


<details>
  <summary>Details</summary>
Motivation: 为中文语言LLM应用场景开发一个专门的安全评估基准，基于中国法律和社会框架，持续跟踪新的安全威胁。

Method: 构建包含六个关键维度的评估框架（合法性、伦理、事实性、隐私、对抗鲁棒性、推理安全），采用动态更新机制，计划纳入文本到图像生成安全和智能体安全等新威胁向量。

Result: 当前版本（v251030）已评估18个LLM，提供了中文语境下AI安全的全景图，排行榜公开可访问。

Conclusion: LiveSecBench为中文LLM安全评估提供了专门的动态基准，有助于持续监控和改进AI模型在中文环境中的安全性。
...
TL;DR: AyurParam-2.9B是一个专门针对阿育吠陀医学领域的双语语言模型，通过专家策划的数据集微调而成，在专业医学任务上超越了同规模开源模型，甚至能与更大模型竞争。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在通用任务上表现出色，但在需要深厚文化、语言和专业知识的高度专业化领域（如阿育吠陀医学）表现不佳，无法准确理解和应用这些领域的复杂知识。

Method: 从Param-1-2.9B模型微调，使用专家策划的阿育吠陀数据集，包含英语和印地语的情境感知、推理和客观问答内容，采用严格标注协议确保事实准确性和教学清晰度。

Result: 在BhashaBench-Ayur基准测试中，AyurParam不仅超越了同规模（1.5-3B参数）的所有开源指令调优模型，还展现出比更大模型更具竞争力或更优越的性能。

Conclusion: AyurParam的结果强调了在专业医学知识领域，真实的领域适应和高质量监督对于提供可靠、文化一致的AI至关重要。
...
TL;DR: AutoAdv是一个无需训练的多轮越狱攻击框架，通过自适应机制在6轮对话内对Llama-3.1-8B实现95%攻击成功率，比单轮攻击提升24%，揭示了当前安全机制在多轮对话中的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全评估主要关注单轮交互，而真实攻击往往通过多轮自适应对话展开，需要研究多轮越狱攻击的有效性和当前安全机制的脆弱性。
...
### [75] [Beyond Single Embeddings: Capturing Diverse Targets with Multi-Query Retrieval](https://arxiv.org/abs/2511.02770)
*Hung-Ting Chen,Xiang Liu,Shauli Ravfogel,Eunsol Choi*

Main category: cs.CL

TL;DR: 提出AMER模型，通过自回归生成多个查询向量来检索相关文档，解决传统检索器因相关文档分布多模态而性能受限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有检索器通常只生成一个查询向量，但查询的相关文档分布可能是多模态的（如不同解释），这限制了检索性能。

Method: 开发AMER架构，自回归生成多个查询向量，所有预测的查询向量都用于从语料库中检索文档。

Result: 在合成向量化数据上完美捕捉多个目标分布，性能比单嵌入模型提升4倍；在真实世界多答案检索数据集上，相对单嵌入基线分别提升4%和21%。

Conclusion: 多查询向量检索器具有巨大潜力，为目标文档嵌入相似度较低的数据子集带来更大收益，为未来研究开辟了新方向。
...
TL;DR: 本研究探索了将领域特定的持续预训练专家模型合并的方法，以构建具备金融、数学和日语处理能力的专业大语言模型，解决了多技能训练成本高且不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现出色，但在金融等专业领域表现不佳，需要融合领域知识、数学推理和多语言处理能力。直接进行多技能训练成本高昂且不稳定，因此探索专家模型合并成为实用替代方案。

Method: 创建金融、数学和日语专家模型，采用三种合并方法（Task Arithmetic、TIES和DARE-TIES），在包含18个任务8个数据集的金融基准上进行三阶段评估：知识恢复、互补性和涌现性。

Result: 专家模型与基础模型合并能恢复CPT期间丢失的通用知识，专家间合并能提升性能并产生跨领域涌现技能。Task Arithmetic表现强劲但对超参数敏感，TIES更稳健。模型相似性与合并成功相关，但涌现技能依赖更复杂因素。
...
TL;DR: 评估基于角色的提示是否能提升LLM在宏观经济预测任务中的表现，发现GPT-4o与人类专家预测准确度相似，但角色描述对预测精度没有显著提升。


<details>
  <summary>Details</summary>
Motivation: 探索角色提示是否能改善大型语言模型在宏观经济预测中的表现，特别是与人类专家预测的对比。

Method: 使用PersonaHub语料库中的2,368个经济学相关角色提示GPT-4o，复制ECB专业预测者调查的50个季度轮次（2013-2025），并与人类专家小组和100个无角色描述的基线预测进行比较。

Result: GPT-4o与人类预测者达到非常相似的准确度水平，差异在统计上显著但实际影响不大。角色描述没有带来可衡量的预测优势，可以省略以降低计算成本而不牺牲准确性。

Conclusion: GPT-4o在提供相关上下文数据时，即使面对样本外的宏观经济事件也能达到有竞争力的预测准确度，但多样化提示产生的预测结果与人类小组相比表现出显著的同质性。
...
TL;DR: Smart-Hiring是一个端到端的NLP流水线，用于自动从非结构化简历中提取结构化信息，并语义匹配候选人与职位描述。
...
Result: 在真实数据集上的实验表明，该方法在保持高可解释性的同时实现了有竞争力的匹配准确率。
...
TL;DR: 该研究分析谷歌翻译从英语到罗马尼亚语翻译中的词汇错误，特别关注WHO、Gavi组织和药品说明书等COVID-19相关官方信息的翻译质量。


<details>
  <summary>Details</summary>
Motivation: 提高机器翻译质量，通过改进词汇选择和减少错误来增强谷歌翻译的准确性和流畅性。

Method: 对230篇从英语翻译成罗马尼亚语的文本进行综合分析，重点关注COVID-19相关官方信息的词汇错误。

Result: 研究发现谷歌翻译在专业术语和官方信息翻译中存在词汇选择错误，需要进一步改进。

Conclusion: 机器翻译系统在专业领域和官方信息翻译方面仍有改进空间，特别是在词汇选择和准确性方面需要优化。
...
TL;DR: 提出NTKT方法，将知识追踪重构为基于预训练大语言模型的下一词预测任务，显著提升预测性能并改善冷启动问题
...
Method: 将学生历史记录和问题内容表示为文本序列，使用预训练大语言模型进行下一词预测任务
...
TL;DR: CGES是一种贝叶斯框架，通过置信度信号自适应停止采样，在保持准确性的同时显著减少LLM调用次数。


<details>
  <summary>Details</summary>
Motivation: 解决自一致性策略需要固定调用次数且在正确答案罕见时失效的问题。

Method: 使用基于标记概率或奖励模型的置信度信号构建候选答案的后验分布，当后验质量超过阈值时自适应停止采样。

Result: 在五个推理基准测试中，平均减少69%的模型调用次数（如从16.0次降至4.9次），同时准确性与自一致性策略相差不超过0.06个百分点。
...
TL;DR: TRACE框架通过程序化策略应用解决LLM对齐-现实差距问题，实现精确的策略更新而无需大规模重新标注数据。
...
Method: TRACE框架通过程序化筛选现有偏好数据、识别高影响冲突、应用混合优化来反转、丢弃或保留偏好，同时保护模型性能。

Result: 在多种模型家族上验证，TRACE在复杂政策变化下能够强制执行新原则而不降低通用能力。

Conclusion: TRACE为维护LLM对齐提供了可扩展、动态且成本效益高的范式，为可持续和负责任的AI部署奠定基础。
...
TL;DR: 研究发现，当在特定知识类型完全由新知识组成的数据集上进行微调时，LLMs会产生显著增加的事实幻觉。高陌生度的知识类型是幻觉的主要驱动因素，这些倾向甚至会影响QA任务中的其他知识类型。提出的KnownPatch方法通过在训练后期添加少量已知知识样本来有效缓解新知识引发的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有研究未深入调查LLMs在引入新知识微调时产生事实幻觉的具体表现和机制，本文旨在填补这一空白。

Method: 设计了受控数据集Biography-Reasoning，在多种知识类型和两种任务类型（知识问答和知识推理）上进行细粒度分析，并提出KnownPatch方法在训练后期添加已知知识样本来缓解幻觉。

Result: 发现特定知识类型的高陌生度（而非新知识的整体比例）是幻觉的主要驱动因素；新知识学习会降低模型对问题中关键实体的注意力，导致过度关注周围上下文；KnownPatch方法有效缓解了注意力模式的破坏并提升了性能。

Conclusion: 特定知识类型的高陌生度是引发LLMs事实幻觉的关键因素，KnownPatch方法通过调整注意力模式有效缓解了新知识学习引发的幻觉问题。
...
TL;DR: 本文提出了一种高效存储微调后参数更新的方法，通过结合低秩近似和稀疏化来优化存储效率，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)的庞大尺寸限制了存储和处理能力，即使存储微调后的模型版本也是一个重大挑战。研究表明微调主要影响少量参数，因此需要更高效的存储方法。

Method: 提出最优奇异值损伤方法，利用微调更新的低秩和稀疏特性，通过选择性稀疏化低秩近似更新，保留最重要的奇异向量组件。

Result: 在相同内存预算下，所提方法相比单独使用低秩近似或稀疏化，实现了显著的存储效率和更高的准确性。

Conclusion: 结合低秩近似和稀疏化的方法能够有效解决微调模型存储问题，在有限内存预算下保持模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 翻译中经常需要添加背景信息来向新受众明确隐含的文化含义，这种语用显化现象在翻译理论中广泛讨论但很少被计算建模。

Method: 通过零对齐识别候选显化案例，并使用带有人工标注的主动学习进行精炼，构建多语言语料库和检测框架。

Result: 实体和系统级显化最为频繁，主动学习将分类器准确率提高了7-8个百分点，跨语言准确率最高达0.88，F1分数0.82。
...
<details>
  <summary>Details</summary>
Motivation: 研究AI在全球扩散不均衡的原因，特别是低资源语言国家AI采用率较低的问题，假设语言性能差距会降低AI效用从而减缓采用

Method: 使用加权回归模型从社会经济和人口因素中分离出语言效应的影响

Result: 低资源语言国家的AI用户比例比基准低约20%，语言可访问性是AI公平扩散的显著独立障碍

Conclusion: 语言性能差距确实会减少AI效用并减缓在低资源语言国家的采用，语言可访问性是实现AI公平扩散的关键因素
...
Method: 提出CoRL强化学习框架，将多LLM协调问题建模为具有双重目标的强化学习：最大化任务性能同时最小化总体推理成本，并在多预算设置下实现可控行为。

Result: 在四个不同基准测试中，CoRL在高预算设置下超越最佳专家LLM，在低预算模式下仍保持强劲性能，证明了集中式协调的有效性。
...
TL;DR: MemSearcher是一个搜索代理工作流，通过迭代维护紧凑内存来平衡信息完整性和效率，在多轮交互中稳定上下文长度，显著提升搜索性能同时降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统搜索代理要么拼接全部历史交互导致长上下文和高计算成本，要么仅使用当前轮次丢弃关键信息，这种权衡限制了搜索代理的可扩展性。

Method: 提出MemSearcher工作流，每轮将用户问题与内存融合生成推理轨迹，执行搜索动作，并更新内存仅保留任务解决所需信息。引入多上下文GRPO强化学习框架联合优化推理、搜索策略和内存管理。

Result: 在七个公开基准测试中，相比强基线取得显著提升：Qwen2.5-3B-Instruct平均增益+11%，Qwen2.5-7B-Instruct平均增益+12%。3B版本的MemSearcher甚至超越7B基线。

Conclusion: 在信息完整性和效率之间取得平衡不仅能获得更高准确性，还能降低计算开销，MemSearcher证明了这一设计原则的有效性。
...
TL;DR: Oolong是一个长上下文推理基准测试，包含合成任务和真实世界对话任务，要求模型分析文本片段并进行聚合推理，现有前沿模型在128K上下文长度下准确率低于50%。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文评估主要关注检索任务，允许模型忽略大部分上下文作为噪声，这无法全面评估模型的长上下文推理能力。需要开发更全面的长上下文推理评估基准。

Method: 创建Oolong基准测试，包含两个任务集：Oolong-synth（自然合成任务，便于消融分析）和Oolong-real（真实世界对话数据推理）。要求模型分析文本片段、执行分类和计数、处理时间和用户关系。

Result: 前沿模型（GPT-5、Claude-Sonnet-4、Gemini-2.5-Pro）在128K上下文长度下，两个任务集的准确率都低于50%，表现不佳。

Conclusion: 当前模型在需要分析大量文本并进行聚合推理的长上下文任务上仍面临挑战，Oolong基准测试的发布将促进能够处理大量文本推理的模型发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI向超人类能力发展，现有基于外部约束的对齐策略可能不足以应对未来超级智能AI。研究探索是否可以通过内在的镜像神经元模式实现更好的对齐。
...
TL;DR: 提出了人类-AI共体现智能系统，将人类用户、智能AI和可穿戴硬件集成，用于现实世界实验和智能制造，通过APEX系统在洁净室中实现柔性电子制造，提高准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型局限于虚拟领域，而现实世界的实验和制造仍依赖人类监督和专业知识，这限制了科学和制造工作流程的可重复性、可扩展性和可访问性。

Method: 引入人类-AI共体现智能系统，结合人类精确执行和控制、智能AI的记忆、上下文推理、自适应规划和实时反馈，通过可穿戴接口捕获实验和制造过程，实现无缝通信和可解释协作。
...
TL;DR: 使用基础模型自动搜索奖励函数，通过文本指令在Gran Turismo 7赛车游戏中生成具有竞争力的强化学习智能体。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习中，将期望行为映射到奖励函数很困难，特别是在复杂环境如自动驾驶赛车中。
...
TL;DR: 提出了深度价值基准（DVB），这是一个评估框架，用于测试大语言模型是否学习人类基本价值观还是仅仅掌握表层偏好。通过控制深层价值观和表层特征之间的混淆，发现所有模型的深度价值泛化率仅为0.30，都低于随机水平。


<details>
  <summary>Details</summary>
Motivation: 区分AI系统是否学习基本人类价值观还是仅掌握表层偏好模式，这对AI对齐至关重要。掌握深层价值观的系统能更稳健地泛化人类意图，而仅学习表层模式的风险产生不对齐行为。

Method: 使用新颖的实验设计，在训练阶段让LLMs接触深层和表层特征故意相关的人类偏好数据，测试阶段打破这些相关性，测量模型的深度价值泛化率（DVGR）。

Result: 在9个不同模型中，平均DVGR仅为0.30，所有模型基于深层价值观的泛化都低于随机水平。较大模型的DVGR略低于较小模型。

Conclusion: DVB提供了一个可解释的对齐核心特征度量，表明当前LLMs在泛化深层价值观方面表现不佳，这对AI对齐具有重要意义。
...
TL;DR: 本文提出了InsurAgent，一个基于大语言模型的智能体，用于模拟洪水保险购买决策行为，解决了LLMs在定量概率估计上的不足。
...
TL;DR: Re-FORC是一种自适应奖励预测方法，可根据上下文预测未来奖励作为思考token数量的函数，实现推理链的早期停止、模型优化选择和自适应测试时扩展。


<details>
  <summary>Details</summary>
Motivation: 解决推理模型在长推理过程中计算资源浪费的问题，通过预测未来奖励来优化推理效率和准确性。

Method: 在推理模型上训练轻量级适配器，预测不同思考token数量下的期望未来奖励，实现动态推理长度控制。

Result: 减少26%计算量保持准确性；在同等计算下提高4%准确率或减少55%计算量；在高计算和低计算场景下分别提高11%和7%准确率。

Conclusion: Re-FORC通过自适应奖励预测实现了推理效率和准确性的显著提升，支持动态推理长度控制和计算时间预估。
...
TL;DR: 提出了ATHENA框架，通过结合符号效用发现和语义适应来建模个性化决策，在旅行模式和疫苗选择任务中优于现有方法


<details>
  <summary>Details</summary>
Motivation: 个体决策模型与群体最优预测存在差距，需要同时考虑数值属性和语言影响来建模个性化选择过程

Method: 两阶段框架：1）通过LLM增强的符号发现找到群体级符号效用函数；2）基于最优效用创建个性化语义模板进行个体级语义适应

Result: 在真实世界任务中持续优于基于效用、机器学习和LLM的模型，F1分数比最强基线提升至少6.5%
...
TL;DR: STRMAC是一个状态感知路由框架，通过分别编码交互历史和代理知识来驱动路由器，在每一步自适应选择最合适的单个代理进行高效协作，并在挑战性协作推理基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能够整合独特专业知识、灵活协作，但其潜力受到僵化的智能体调度和低效协调策略的限制，无法适应不断变化的任务需求。

Method: 提出STRMAC框架，分别编码交互历史和代理知识来驱动路由器，自适应选择最合适的单个代理；引入自演化数据生成方法，加速高质量执行路径的收集。

Result: 在挑战性协作推理基准上实现最先进性能，比基线提升高达23.8%，与穷举搜索相比减少数据收集开销高达90.1%。

Conclusion: STRMAC框架通过状态感知路由和自演化数据生成，有效解决了多智能体系统中的协作效率问题，显著提升了系统性能并大幅降低了训练成本。
...
TL;DR: 本文提出PPP方法，通过多目标强化学习同时优化生产力、主动性和个性化三个维度，在软件工程和深度研究任务中显著优于GPT-5等基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注任务成功率，但有效的现实世界智能体需要同时优化生产力（任务完成）、主动性（提出关键问题）和个性化（适应用户偏好）三个维度。

Method: 引入UserVille交互环境和基于LLM的用户模拟器，提出PPP多目标强化学习方法，联合优化生产力、主动性和个性化三个目标。
...
TL;DR: 提出了Deep Ideation框架，通过整合科学概念网络和LLM驱动的方法来生成高质量的研究想法，相比现有方法提升10.67%的质量。


<details>
  <summary>Details</summary>
Motivation: 现有研究想法生成方法过于简单，主要依赖关键词共现或语义相似性，忽视了科学概念间的复杂上下文关系，无法有效利用科学文献中的知识。

Method: 提出Deep Ideation框架，整合科学网络（捕捉关键词共现和上下文关系），采用探索-扩展-演化的迭代流程，使用想法栈跟踪进展，并配备基于真实审稿反馈训练的批评引擎。

Result: 实验显示该方法相比其他方法提升10.67%的想法质量，生成的想法超过顶级会议接受水平，人工评估证实其在实际科研中的价值。
...
Method: 构建可控数据集系统变化视觉和文本输入的推理难度，使用熵作为细粒度不确定性度量，通过层间预测探究内部机制。

Result: 发现模态跟随概率随相对不确定性单调下降的普遍规律，在平衡点处模型倾向于以相近概率跟随两种模态，揭示了模型在模糊区域会在层间振荡的内部机制。

Conclusion: 相对不确定性和固有模态偏好是控制模态跟随的两个基本原则，提供了量化框架和机制性洞察来解释MLLMs如何解决冲突信息。
...
TL;DR: 本文分析了多智能体推理中的懒惰行为问题，提出了因果影响度量和可验证奖励机制来改善协作效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的多智能体推理系统中，存在智能体懒惰行为问题，其中一个智能体主导而另一个贡献很少，导致协作失效，系统退化为单智能体模式。

Method: 1. 理论分析懒惰行为的产生原因；2. 引入稳定高效的因果影响度量方法；3. 设计可验证奖励机制，允许推理智能体丢弃噪声输出、整合指令并在必要时重启推理过程。

Result: 大量实验表明，所提出的框架有效缓解了懒惰智能体行为，释放了多智能体框架在复杂推理任务中的全部潜力。

Conclusion: 通过因果影响度量和可验证奖励机制，能够有效解决多智能体推理中的协作问题，提升系统整体性能。
...
Method: 使用多模态电子健康记录，通过量化分词处理连续实验室数值，采用掩码语言建模预训练和注意力机制实现可解释性。
...
Result: 该系统使用来自UCI机器学习存储库的数据集进行开发和验证，能够通过常规血液分析获得参数，提供非侵入性和可访问的初步评估方法。
...
TL;DR: 提出基于二元分类的新框架来估计生成模型的完整精确率-召回率曲线，进行统计分析并获得最小最大上界，扩展了现有仅关注曲线极值的PR度量方法。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型在图像和文本领域的成功，其评估方法受到广泛关注。现有方法大多依赖标量指标，而精确率-召回率曲线为分析提供了更丰富视角，但其估计面临挑战。

Method: 基于二元分类视角构建新框架来估计完整PR曲线，进行统计分析并推导最小最大上界风险。

Result: 提出的框架能够扩展文献中仅关注曲线极值的PR度量方法，并在不同实验设置下研究曲线行为。

Conclusion: 该框架为生成模型的PR曲线估计提供了理论基础和实用方法，支持更全面的模型评估分析。
...
TL;DR: ReAcTree是一种分层任务规划方法，通过构建动态代理树将复杂目标分解为可管理的子目标，结合两种互补记忆系统，显著提升了长时程任务的执行成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂长时程任务时存在困难，因为它们依赖单一轨迹，试图在一个统一过程中解决整个任务，这限制了性能。

Method: 提出ReAcTree方法：构建动态代理树分解复杂目标为子目标，每个代理节点负责推理、行动和扩展树，控制流节点协调执行策略，并集成情景记忆和工作记忆两种记忆系统。

Result: 在WAH-NL和ALFRED数据集上的实验表明，ReAcTree在多种LLM上均优于ReAct等强基线方法。在WAH-NL上，使用Qwen 2.5 72B时ReAcTree达到61%的目标成功率，几乎是ReAct（31%）的两倍。

Conclusion: ReAcTree通过分层任务分解和记忆系统集成，有效解决了复杂长时程任务的规划问题，显著提升了任务执行性能。
...
TL;DR: 提出VMR方法，将开放式任务转化为可验证的多选格式，使RLVR范式能应用于缺乏标准答案的开放式任务，显著提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法依赖标准答案验证，无法应用于开放式任务（如创意写作、指令遵循），但这类任务中推理能力仍有潜在价值。

Method: 提出可验证多选重构(VMR)训练策略，将开放式数据重构为可验证的多选格式，从而在缺乏明确标准答案的情况下实现有效训练。

Result: 在8个开放式基准测试中，VMR训练相比基线平均提升5.99分，验证了该方法在开放式任务中的有效性。

Conclusion: VMR成功将RLVR范式扩展到开放式领域，证明强化推理能力确实能提升开放式任务性能，为缺乏标准答案的任务提供了新的训练思路。
...
TL;DR: 本文提出了Agentic AI在5G/6G网络中的框架和应用，通过LAMs实现自主决策和RAN优化。


<details>
  <summary>Details</summary>
Motivation: 5G/6G网络的复杂性使得手动优化效率低下，需要Agentic AI来自动化动态RAN环境中的决策。

Method: 引入Agentic AI核心概念，描述反思、规划、工具使用和多智能体协作等设计模式，并通过5G RAN案例展示时间序列分析和LAM驱动智能体的协作。

Result: 提出了Agentic AI在移动网络中的理论框架和实际应用方法。

Conclusion: Agentic AI为5G/6G网络优化提供了有效的自动化解决方案，能够实现基于KPI的自主决策。
...
Method: 构建并维护知识图谱来系统建模游戏元素、任务依赖和因果关系，利用LLM解析自然语言更新日志，通过知识图谱的多跳推理识别影响范围，生成针对更新的测试用例。

Result: 在Overcooked和Minecraft两个代表性游戏环境中的实验表明，KLPEG能够更准确地定位受更新影响的功能，并以更少的步骤完成测试，显著提高了测试效果和效率。

Conclusion: KLPEG框架通过知识图谱和LLM的结合，有效解决了游戏增量更新的自动化测试问题，为游戏测试提供了更精确和高效的解决方案。
...
TL;DR: ORCA基准测试评估大语言模型在多领域真实场景下的定量推理能力，使用已验证的计算器输出作为标准。在500个跨领域任务中，五个最先进模型准确率仅为45-63%，主要错误来自舍入(35%)和计算错误(33%)。
...
Method: 开发ORCA基准测试，包含500个自然语言任务，涵盖金融、物理、健康、统计等领域，使用Omni计算器引擎的已验证输出作为标准答案。

Result: 五个最先进模型(包括ChatGPT-5、Gemini 2.5 Flash等)准确率在45-63%之间，数学和工程领域表现较好，物理和自然科学较弱。模型间存在部分互补性而非冗余。

Conclusion: 当前大语言模型在真实世界定量推理任务中仍有显著局限性，特别是在数值精度和复杂领域推理方面，需要进一步改进。
...
Method: 开发基于GR(1)规范的自适应屏蔽框架，使用归纳逻辑编程在线检测环境假设违规并自动修复规范。

Result: 在Minepump和Atari Seaquest案例中，自适应屏蔽相比静态屏蔽能维持接近最优的奖励和完美的逻辑合规性。
...
<details>
  <summary>Details</summary>
Motivation: 人类中心领域的培训和教育需要真实实践，但现实的人类行为模拟一直有限。现有黑盒神经网络模型缺乏透明性和与人类心理学的对齐。

Method: 基于心理学理论（自我效能、心态、社会建构主义等）构建多代理系统，模拟"内部议会"中代表关键心理因素的代理进行审议和互动，决定输出行为。

Result: 系统实现了前所未有的透明性和与人类心理学的对齐，能够生成可信的人类行为模拟。
...
TL;DR: DecompSR是一个用于分析组合空间推理能力的大型基准数据集和生成框架，包含超过500万个数据点，可以独立控制组合性的多个方面。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在空间推理任务中表现出组合性能力不足的问题，需要系统性的评估工具来深入分析其推理能力的局限性。

Method: 通过程序化生成方法构建数据集，确保构造正确性，并使用符号求解器独立验证数据集的正确性。数据集可以独立控制生产力、可替代性、过度泛化和系统性等组合性维度。

Result: 基准测试显示大语言模型在空间推理任务中难以进行生产性和系统性泛化，但对语言变异性表现更稳健。

Conclusion: DecompSR提供了一个可证明正确且严谨的基准数据集，能够精细探测大语言模型的组合推理能力，为模型评估提供了新的分析维度。
...
TL;DR: 提出了一个协作迷宫求解基准，评估32个领先模型在单独、同质和异质配对中的协作能力，发现存在"协作差距"，并提出"接力推理"方法改善协作效果。


<details>
  <summary>Details</summary>
Motivation: AI发展轨迹表明我们将越来越依赖基于代理的系统，这些系统的成功关键在于异构代理之间的有效协作，但目前缺乏大规模评估代理间协作的实证研究。

Method: 设计了一个协作迷宫求解基准框架，能够隔离协作能力、调节问题复杂度、实现可扩展自动评分，并在无输出格式约束下评估32个领先开源和闭源模型。

Result: 发现明显的"协作差距"：单独表现良好的模型在需要协作时性能显著下降。协作可能完全崩溃，小型蒸馏模型在某些配对中几乎完全失败。从较强代理开始通常能改善结果。

Conclusion: 需要协作感知的评估、增强协作能力的训练策略，以及可靠激发代理潜在技能的交互设计，这些指导原则适用于AI-AI和人类-AI协作。
...
TL;DR: CostBench是一个专注于成本效率的基准测试，用于评估LLM智能体的经济推理和重新规划能力，揭示当前智能体在成本优化规划和动态适应方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估主要关注任务完成度，忽视了资源效率和适应性这一关键能力，特别是智能体在变化环境中制定和调整成本最优计划的能力。

Method: 在旅行规划领域构建CostBench基准，包含可通过多种原子和复合工具序列解决的任务，支持四种动态阻塞事件（如工具故障和成本变化）来模拟现实世界的不确定性。

Result: 评估显示主流开源和专有模型在成本感知规划方面存在显著差距：智能体经常无法在静态环境中找到成本最优解，GPT-5在最难任务上的精确匹配率低于75%，动态条件下性能进一步下降约40%。

Conclusion: CostBench通过诊断这些弱点，为开发既经济理性又鲁棒的未来智能体奠定了基础。
...
TL;DR: 本文提出了span query概念，将推理服务器接口泛化，支持聊天、RAG、推理时扩展和智能体工作负载，通过优化KV缓存和注意力局部性，显著提升非聊天用例的性能。


<details>
  <summary>Details</summary>
Motivation: 当前推理服务器主要针对聊天完成进行优化，但客户端已发展出各种推理时扩展和深度推理技术。现有解决方案通常只针对单一用例（如RAG）优化，缺乏通用性。

Method: 引入span query作为推理调用的表达式树，通过交换性约束连接，支持自动优化KV缓存局部性。对vLLM进行少量修改（492行代码）实现高性能span query执行。

Result: span query在两种不同的非聊天用例中实现了10-20倍的TTFT降低。注意力优化的span query在2b参数模型上的表现远超使用8b模型的传统推理服务器。

Conclusion: span query为推理服务器提供了通用接口，能够有效支持多样化的工作负载，并通过优化显著提升性能，解决了传统推理服务器在非聊天用例中的局限性。
...
TL;DR: 提出了一种轻量级、模型无关的诊断框架来分析多模态大语言模型的推理过程，通过将每个模态视为代理来识别模态间的冲突和误导行为。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型发展迅速，但其推理过程仍不透明，难以理解哪个模态驱动预测、如何解决冲突或何时一个模态主导决策。

Method: 提出模态破坏诊断方法，将每个模态视为代理生成候选标签和自评估，通过简单融合机制聚合输出，识别支持正确结果的贡献者和误导的破坏者。

Result: 在情感识别基准测试的案例研究中，揭示了系统的可靠性特征，帮助区分失败是源于数据集伪影还是模型限制。
...
TL;DR: Orion-MSP是一种表格数据上下文学习架构，通过多尺度处理、块稀疏注意力和感知器式记忆三大创新，解决了现有模型在特征层次依赖、计算效率和组件通信方面的限制，在保持高性能的同时实现了对高维表格的有效扩展。


<details>
  <summary>Details</summary>
Motivation: 解决表格数据神经网络建模的挑战：特征类型异构、多尺度复杂交互，以及现有表格上下文学习架构在单尺度特征处理、二次方计算复杂度和顺序组件处理方面的局限性。

Method: 提出Orion-MSP架构，包含三个关键创新：多尺度处理捕获层次特征交互；块稀疏注意力结合窗口化、全局和随机模式实现可扩展效率；感知器式记忆支持组件间安全双向信息流。

Result: 在多样化基准测试中，Orion-MSP达到或超越了最先进性能，同时能够有效扩展到高维表格，为高效的表格上下文学习设立了新标准。

Conclusion: Orion-MSP通过创新的多尺度处理、高效注意力和组件通信机制，成功解决了表格上下文学习的关键挑战，在性能和可扩展性方面均表现出色。
...
TL;DR: 本文提出了一种在复杂AI环境中优化攻击策略的方法，通过将攻击能力分解为五个技能组件，并利用概率模型在数据有限的情况下优化攻击超参数，显著提升了攻击强度。
...
Method: 将攻击能力分解为五个组成技能：怀疑建模、攻击选择、计划合成、执行和隐蔽性；开发攻击动态的概率模型，在模拟中优化攻击超参数，然后将结果转移到SHADE-Arena环境中。

Result: 攻击强度得到显著提升，安全评分从基线0.87降低到0.41，表明该方法有效提高了攻击能力。

Conclusion: 通过技能分解和概率模拟的方法，可以在数据有限的情况下有效优化攻击策略，为AI风险评估提供了更可靠的工具。
...
TL;DR: Kosmos是一个AI科学家系统，能够自动化数据驱动的科学发现过程，通过结构化世界模型协调数据分析代理和文献搜索代理，在12小时内执行多达200次代理运行，生成可追溯的科学报告。


<details>
  <summary>Details</summary>
Motivation: 现有的AI科研代理在采取多次行动后会失去连贯性，限制了发现深度。需要开发能够长期保持连贯性的自动化科学发现系统。

Method: 使用结构化世界模型在数据分析代理和文献搜索代理之间共享信息，进行并行数据分析、文献搜索和假设生成的循环，最多运行12小时。

Result: Kosmos在单次运行中平均执行42,000行代码和阅读1,500篇论文，79.4%的报告陈述被独立科学家确认为准确，20次循环相当于人类6个月的研究工作量。

Conclusion: Kosmos证明了AI系统能够进行深度科学发现，在多个领域产生了7项发现，其中3项独立重现了未发表成果，4项为科学文献做出了新颖贡献。
...
Result: 建立了连接神经网络与逻辑的通用框架，统一了现有的神经编码和知识提取方法，为深度学习提供了所需的语义基础。

Conclusion: 逻辑为AI科学提供了必要的语义框架，通过神经符号方法可以弥合深度学习与可理解知识之间的鸿沟，但实际应用中识别语义编码仍面临挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型局限于固定模态对，需要大量对齐数据集进行微调，构建能够整合文本、图像、音频和视频的全能模型仍不切实际且缺乏强大的推理支持。

Method: 采用主代理系统，主代理解释用户意图，将子任务委托给特定模态的代理，并将它们的输出整合为连贯响应。基于代理的设计能够无缝集成专门的基础模型。

Result: 在文本、图像、音频、视频和全能基准测试中的广泛实验表明，Agent-Omni始终达到最先进的性能，特别是在需要复杂跨模态推理的任务上。

Conclusion: 该框架是模块化且易于扩展的，允许随着更强模型的出现进行未来改进，确保了对多样化输入的适应性，同时保持了透明度和可解释性。
...
### [124] [Detecting Vulnerabilities from Issue Reports for Internet-of-Things](https://arxiv.org/abs/2511.01941)
*Sogol Masoumzadeh*

Main category: cs.SE

TL;DR: 该研究首次探索使用机器学习和大型语言模型检测物联网项目中的漏洞指示问题，提出了结合ML、LLM和NLP的方法，以及微调BERT模型的方法。


<details>
  <summary>Details</summary>
Motivation: 物联网系统中漏洞分析速度较慢，需要及时识别反映软件漏洞的问题报告，而现有ML和LLM方法在物联网领域的应用尚未探索。

Method: 提出两种方法：(1) 结合ML、LLM和NLP技术检测21个Eclipse物联网项目的漏洞指示问题；(2) 在11,000个GitHub问题上微调预训练的BERT掩码语言模型进行漏洞分类。

Result: 基于BERT NLP特征训练的SVM模型表现最佳，AUC达到0.65；微调BERT模型准确率为0.26，表明训练时需要暴露所有数据的重要性。
...
### [125] [Metamorphic Testing of Large Language Models for Natural Language Processing](https://arxiv.org/abs/2511.02108)
*Steven Cho,Stefano Ruberto,Valerio Terragni*

Main category: cs.SE

TL;DR: 本文对大型语言模型进行最全面的蜕变测试研究，收集191个蜕变关系，实现36个代表性关系进行约56万次测试，揭示了MT在LLM测试中的能力、机会和局限性。


<details>
  <summary>Details</summary>
Motivation: LLM在执行NLP任务时经常产生错误结果，但标记数据集有限导致难以验证其正确性。蜕变测试可以缓解这个oracle问题，无需显式oracle即可暴露错误行为。

Method: 进行文献综述收集191个NLP任务的蜕变关系，实现36个代表性关系，使用三个流行LLM进行约56万次蜕变测试。

Result: 实验结果揭示了MT在LLM测试中的能力、机会和局限性，为改进LLM有效性提供了重要见解。

Conclusion: 蜕变测试是验证LLM行为有效性的重要方法，本研究为LLM的可靠性和改进提供了系统性的测试框架和实证基础。
...
### [126] [Open the Oyster: Empirical Evaluation and Improvement of Code Reasoning Confidence in LLMs](https://arxiv.org/abs/2511.02197)
...
Method: 提出了代码推理任务的置信度分析与增强框架，包括对主流LLM进行置信度可靠性的全面实证研究，评估提示策略优化和数学校准（如Platt Scaling）等技术的有效性。

Result: DeepSeek-Reasoner在各种任务中表现最佳，在ECE、Brier Score和Performance Score三个指标上分别比其他模型高出0.680、0.636和13.652。混合策略相比原始性能在上述三个指标上分别提升了0.541、0.628和15.084。

Conclusion: 具有推理能力的模型表现出更优的置信度可靠性，混合策略是增强各种模型置信度可靠性的最有效方法。当前LLM在复杂推理任务中的置信度仍有较大改进空间，为LLM辅助软件工程中的置信度应用提供了研究基础和技术参考。
...
### [127] [LLMs as Judges: Toward The Automatic Review of GSN-compliant Assurance Cases](https://arxiv.org/abs/2511.02203)
*Gerhard Yu,Mithila Sivakumar,Alvine B. Belle,Soude Ghari,Song Wang,Timothy C. Lethbridge*

Main category: cs.SE

TL;DR: 本文提出了一种基于LLM-as-a-judge范式的自动化保证案例审查方法，通过新的基于谓词的规则来形式化审查标准，实验表明DeepSeek-R1和GPT-4.1表现最佳，但仍需人工审查来完善结果。
...
Method: 采用LLM-as-a-judge范式，提出新的基于谓词的规则来形式化既有的保证案例审查标准，并据此设计针对审查任务的LLM提示。在GPT-4o、GPT-4.1、DeepSeek-R1和Gemini 2.0 Flash等多个先进LLM上进行实验。

Result: 大多数LLM展现出相对良好的审查能力，其中DeepSeek-R1和GPT-4.1表现更优，DeepSeek-R1最终超越了GPT-4.1。但实验结果也表明仍需人工审查来完善LLM生成的审查结果。

Conclusion: LLM在保证案例审查中具有潜力，DeepSeek-R1表现最佳，但完全自动化审查仍不可行，需要人机协作来确保审查质量。
...
### [128] [SWE-Sharp-Bench: A Reproducible Benchmark for C# Software Engineering Tasks](https://arxiv.org/abs/2511.02352)
*Sanket Mhatre,Yasharth Bajpai,Sumit Gulwani,Emerson Murphy-Hill,Gustavo Soares*

Main category: cs.SE

TL;DR: SWE-Sharp-Bench是首个针对C#语言的软件工程基准测试，包含150个实例，评估显示AI编码代理在C#上的表现（40%解决率）远低于Python（70%解决率）。


<details>
  <summary>Details</summary>
Motivation: C#作为重要的企业级编程语言（TIOBE排名第5），在现有的软件工程基准测试中缺失，需要专门的基准来评估AI编码代理在C#上的性能。

Method: 构建了SWE-Sharp-Bench基准，包含来自17个仓库的150个实例，并使用相同的模型-代理配置在不同语言间进行对比评估。

Result: 评估结果显示显著的性能差距：Python任务解决率为70%，而C#任务解决率仅为40%。

Conclusion: C#语言在AI编码代理任务中存在显著性能差距，开源了SWE-Sharp-Bench基准和整个构建流程以促进未来研究。
...
### [129] [EvoDev: An Iterative Feature-Driven Framework for End-to-End Software Development with LLM-based Agents](https://arxiv.org/abs/2511.02399)
*Junwei Liu,Chen Xu,Chong Wang,Tong Bai,Weitong Chen,Kaseng Wong,Yiling Lou,Xin Peng*

Main category: cs.SE

TL;DR: EvoDev是一个受特性驱动开发启发的迭代软件开发框架，通过构建特性映射图来建模特性间依赖关系，在Android开发任务中相比现有方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型代理方法主要采用线性的瀑布式流程，过度简化了现实世界开发的迭代性质，难以处理复杂的大规模项目。

Method: 将用户需求分解为一组用户价值特性，构建有向无环图的特性映射来显式建模特性间依赖关系，每个节点维护多层次信息（业务逻辑、设计、代码），并沿依赖关系传播上下文。

Result: 在Android开发任务中，EvoDev比表现最佳的基线Claude Code提升了56.8%，在不同基础LLM上单代理性能提升了16.0%-76.6%。
...
### [130] [Who's Who? LLM-assisted Software Traceability with Architecture Entity Recognition](https://arxiv.org/abs/2511.02434)
*Dominik Fuchß,Haoyu Liu,Sophie Corallo,Tobias Hey,Jan Keim,Johannes von Geisau,Anne Koziolek*

Main category: cs.SE

TL;DR: 本文提出了两种基于LLM的方法来自动识别软件架构实体并建立架构文档与源代码之间的可追踪性链接，无需手动创建软件架构模型。


<details>
  <summary>Details</summary>
Motivation: 解决软件架构文档与源代码之间的语义鸿沟问题，传统方法需要手动创建软件架构模型，耗时且不实用。

Method: 提出了ExArch和ArTEMiS两种方法：ExArch从架构文档和源代码中提取组件名称构建简单SAM；ArTEMiS识别文档中的架构实体并与SAM实体匹配。

Result: ExArch在无需手动SAM的情况下达到F1分数0.86，与需要手动SAM的TransArC（F1:0.87）相当；ArTEMiS与传统启发式方法SWATTR性能相当（F1:0.81），与ExArch结合优于无需手动SAM的最佳基线ArDoCode。

Conclusion: LLM能够有效识别文本工件中的架构实体，实现自动化的SAM生成和可追踪性链接恢复，使架构-代码可追踪性更加实用和易用。
...
### [131] [When Continuous Delivery Is Not an Option: Practical Paths to Continuous Engineering in Complex Organizations](https://arxiv.org/abs/2511.02445)
*Eriks Klotins,Magnus Ahlgren,Nicolas Martin Vivaldi,Even-Andre Karlsson*

Main category: cs.SE

TL;DR: 研究四个工业案例，探讨约束条件如何影响持续软件工程（CSE）的实际采用，提出更新的准备度模型来指导实践


<details>
  <summary>Details</summary>
Motivation: 持续软件工程承诺提高效率和质量，但完全采用常受复杂产品、遗留系统、组织惯性和监管要求等约束，需要研究这些约束如何影响实践中的CSE采用

Method: 应用并扩展先前提出的CSE行业准备度模型，通过专家访谈和叙事综合评估四个工业案例的当前和潜在采用水平

Result: 识别了共同的驱动力和采用障碍，包括组织准备度、跨组织依赖性和客户对持续交付的有限需求，提出了更新的准备度模型

Conclusion: 虽然完整的端到端CSE采用可能不可行，但有意义的内部改进仍然是可能且有益的，为组织在部分或受限的CSE转型中提供实证指导
...
### [132] [Lost in Code Generation: Reimagining the Role of Software Models in AI-driven Software Engineering](https://arxiv.org/abs/2511.02475)
*Jürgen Cito,Dominik Bork*

Main category: cs.SE

TL;DR: 生成式AI使得通过自然语言提示快速创建软件成为可能，但这导致了原型与工程化软件之间的界限模糊，产生了脆弱系统。论文主张通过事后从AI生成代码中恢复模型来重建理解、揭示风险并指导改进。


<details>
  <summary>Details</summary>
Motivation: 生成式AI降低了软件开发门槛，但也导致原型与工程化软件界限模糊，产生缺乏鲁棒性、安全性和可维护性的脆弱系统，需要重新思考软件模型的作用。

Method: 提出将模型从前期蓝图转变为事后从AI生成代码中恢复的工具，作为人类意图、AI生成和长期系统演进之间的中介。

Result: 模型可以作为中介在AI驱动的软件工程中提供可持续路径，恢复系统理解、暴露风险并指导改进。

Conclusion: 在AI驱动的软件工程中，模型应作为事后恢复的工具，而非仅作为前期蓝图，以在人类意图、AI生成和系统演进之间提供可持续的桥梁。
...
### [133] [ReleaseEval: A Benchmark for Evaluating Language Models in Automated Release Note Generation](https://arxiv.org/abs/2511.02713)
*Qianru Meng,Zhaochun Ren,Joost Visser*

Main category: cs.SE

TL;DR: ReleaseEval是一个用于自动发布说明生成的可复现开源基准，包含94,987条发布说明，支持三种任务设置，评估显示大语言模型在结构化信息处理上表现优异，但在长代码差异抽象上仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 解决自动发布说明生成中数据集限制（缺乏明确许可、可复现性差）和任务设计不完整（主要依赖提交消息，忽略提交层次结构和代码变更等细粒度上下文）的问题。

Method: 引入ReleaseEval基准，包含94,987条发布说明，支持三种输入粒度任务：commit2sum（从提交消息生成）、tree2sum（结合提交树结构）、diff2sum（利用细粒度代码差异）。

Result: 大语言模型在所有任务上都优于传统基线方法，在tree2sum上取得显著提升，但在diff2sum上仍有困难。

Conclusion: 大语言模型擅长利用结构化信息，但在从长代码差异中抽象信息方面面临挑战，ReleaseEval为自动发布说明生成提供了系统评估框架。
...
### [134] [Investigating the Experience of Autistic Individuals in Software Engineering](https://arxiv.org/abs/2511.02736)
*Madalena Sasportes,Grischa Liebel,Miguel Goulão*

Main category: cs.SE

TL;DR: 本研究分析了自闭症软件工程师在软件工程活动中的经验，特别关注他们的优势，发现他们在逻辑思维、细节关注、编程专注度方面表现出色，喜欢学习新技术，偏好书面沟通和远程工作，且对AI系统有较高舒适度。


<details>
  <summary>Details</summary>
Motivation: 自闭症谱系障碍(ASD)患者在日常生活中的挑战常导致失业和心理健康问题。现有研究强调了自闭症患者相比非自闭症个体的优势，如高度关注细节和优秀逻辑推理能力，这些优势在软件工程活动中可能很有价值，但现有SE研究主要关注挑战而非优势。

Method: 结合社会技术扎根理论，通过16名自闭症软件工程师的半结构化访谈和49名受访者（包括5名自闭症参与者）的调查，将新兴主题与Gama等人关于神经多样性认知功能障碍对软件工程绩效影响的理论进行比较。

Result: 结果显示自闭症软件工程师在逻辑思维、细节关注和编程专注度方面技能突出；喜欢学习新编程语言和编程相关技术；确认了他们对书面沟通和远程工作的偏好；报告了与AI系统互动的高舒适度水平。

Conclusion: 研究结果通过提供自闭症软件工程师优势的进一步证据扩展了现有工作。
...
### [135] [Formalizing Regression Testing for Agile and Continuous Integration Environments](https://arxiv.org/abs/2511.02810)
*Suddhasvatta Das,Kevin Gary*

Main category: cs.SE

TL;DR: 该论文为敏捷开发中的持续回归测试建立了形式化模型，将连续构建视为时间有序链，并定义了回归测试窗口来捕捉有限的时间预算。


<details>
  <summary>Details</summary>
Motivation: 现代敏捷开发实践需要持续回归测试，而非传统理论假设的单一版本测试，因此需要新的形式化框架来支持这种连续测试场景。

Method: 将连续构建形式化为时间有序链，每个构建包含程序、需求和测试，定义回归测试窗口来模拟有限时间预算，并验证模型与现有敏捷回归测试算法的兼容性。

Result: 成功形式化了持续回归测试现象，验证了模型能够直接表示两种最先进的敏捷回归测试算法，并证明了形式化的正确性和完备性。

Conclusion: 提出的形式化模型为敏捷环境中的持续回归测试提供了理论基础，当时间限制设为无穷大时能退化为传统的重测全部方法，保持了与传统理论的语义一致性。
...
### [136] [From Code Changes to Quality Gains: An Empirical Study in Python ML Systems with PyQu](https://arxiv.org/abs/2511.02827)
*Mohamed Almukhtar,Anwar Ghammam,Marouane Kessentini,Hua Ming*

Main category: cs.SE

TL;DR: 该研究开发了PyQu工具，通过分析3400多个开源Python机器学习项目的370万次提交，识别了61种直接提升软件质量的代码变更，并将其分为13个类别，其中41%是新发现的变更类型。
...
Result: PyQu工具在识别质量提升提交时达到平均准确率0.84、精确率0.85、召回率0.85和F1分数0.85，发现了61种直接影响软件质量的代码变更，分为13个类别，其中41%是现有工具未发现的新类型。
...
### [137] [An Evaluation of Interleaved Instruction Tuning on Semantic Reasoning Performance in an Audio MLLM](https://arxiv.org/abs/2511.02234)
*Jiawei Liu,Enis Berk Çoban,Zarina Schevchenko,Hao Tang,Zhigang Zhu,Michael I Mandel,Johanna Devaney*

Main category: cs.MM

TL;DR: 研究探索了在音频多模态大语言模型中使用交错指令调优的效果，发现交错提示能提升语义推理性能，但会降低音频标注能力。


<details>
  <summary>Details</summary>
Motivation: 传统的多模态大语言模型训练方法只是简单拼接非文本信息和文本提示，这可能无法促进模态的深度整合，限制了模型利用核心语言模型推理能力的能力。

Method: 使用LTU模型作为测试平台，在SHARD音频推理基准上实验交错指令调优，将音频令牌交错在提示中，并进行少量微调。

Result: 即使零样本交错提示也能改善推理任务性能，少量交错训练微调能进一步提升结果，但会牺牲模型的音频标注能力。
...
### [138] [Wireless Video Semantic Communication with Decoupled Diffusion Multi-frame Compensation](https://arxiv.org/abs/2511.02478)
*Bingyan Xie,Yongpeng Wu,Yuxuan Shi,Biqian Feng,Wenjun Zhang,Jihong Park,Tony Quek*

Main category: cs.MM

TL;DR: 提出了一种无线视频语义通信框架WVSC-D，通过在语义层面而非像素层面进行视频编码，结合解耦扩散多帧补偿技术，显著提高了无线视频传输的带宽效率。
...
Method: WVSC-D框架首先将原始视频帧编码为语义帧，然后在语义层面进行视频编码；引入参考语义帧替代传统运动向量；在接收端采用两阶段条件扩散过程生成补偿语义帧。

Result: 实验结果显示，WVSC-D相比其他基于深度学习的方法（如DVSC）在PSNR指标上提升了约1.8 dB。

Conclusion: 该语义通信框架通过语义级视频编码和扩散补偿技术，在保证视频传输质量的同时显著提高了带宽效率。
...
### [139] [Solving cold start in news recommendations: a RippleNet-based system for large scale media outlet](https://arxiv.org/abs/2511.02052)
*Karol Radziszewski,Michał Szpunar,Piotr Ociepka,Mateusz Buczyński*

Main category: cs.IR

TL;DR: 基于RippleNet的可扩展推荐系统实现，针对媒体领域设计，在波兰大型在线媒体平台Onet.pl上部署生产。通过将基于内容的物品嵌入集成到RippleNet的知识传播机制中，解决新发布内容的冷启动问题。


<details>
  <summary>Details</summary>
Motivation: 解决媒体领域中新发布内容的冷启动问题，为大型在线媒体平台提供有效的推荐解决方案。

Method: 集成基于内容的物品嵌入到RippleNet的知识传播机制中，使用Amazon SageMaker进行分布式训练和推理，Apache Airflow编排数据管道和模型重训练工作流，构建包含用户和物品特征及交互表的综合黄金数据集。

Result: 实现了能够有效评分未见物品的推荐系统，在生产环境中成功部署。

Conclusion: 提出的解决方案有效解决了媒体领域的冷启动问题，通过集成内容嵌入和知识传播机制，为大规模推荐系统提供了可行的实现方案。
...
### [140] [Enhancing Multimodal Recommendations with Vision-Language Models and Information-Aware Fusion](https://arxiv.org/abs/2511.02113)
*Hai-Dang Kieu,Min Xu,Thanh Trung Huynh,Dung D. Le*

Main category: cs.IR

TL;DR: VLIF是一个基于视觉语言和信息论的多模态推荐框架，通过视觉丰富化和信息感知融合提升推荐性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖粗糙的视觉特征和不受控的融合，导致冗余或不对齐的表示，限制了视觉特征在多模态融合中的贡献

Method: 包含两个关键组件：(1)基于VLM的视觉丰富化模块生成细粒度、标题引导的描述；(2)基于部分信息分解的信息感知融合模块解耦冗余和协同信号

Result: 在三个Amazon数据集上的实验表明，VLIF持续优于最近的多模态基线方法，并显著增强了视觉特征的贡献

Conclusion: VLIF通过视觉语言对齐和信息论指导的融合，有效提升了多模态推荐系统的表示质量和性能
...
### [141] [KGBridge: Knowledge-Guided Prompt Learning for Non-overlapping Cross-Domain Recommendation](https://arxiv.org/abs/2511.02181)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的跨域推荐方法在非重叠用户场景下面临三大挑战：对KG稀疏性和流行度偏差敏感、依赖重叠用户进行域对齐、缺乏可迁移知识和领域特定知识的显式解耦。

Method: 提出KGBridge框架，包含两个核心组件：KG增强提示编码器（将关系级语义建模为软提示）和两阶段训练范式（跨域预训练+隐私保护微调），通过关系感知语义控制和对应驱动解耦来分离域共享和域特定语义。

Result: 在基准数据集上的广泛实验表明，KGBridge始终优于最先进的基线方法，并且在不同的KG稀疏度下保持鲁棒性，有效缓解了KG增强跨域推荐中的结构不平衡和语义纠缠问题。

Conclusion: KGBridge通过结合结构化知识指导和提示学习，在非重叠用户场景下实现了有效的跨域知识迁移，为KG增强的跨域推荐提供了新的解决方案。
...
### [142] [Average Precision at Cutoff k under Random Rankings: Expectation and Variance](https://arxiv.org/abs/2511.02571)
*Tetiana Manzhos,Tetiana Ianevych,Olga Melnyk*

Main category: cs.IR

TL;DR: 本文推导了AP@k的期望和方差，为MAP@k评估指标建立随机基准，帮助更可靠地解释推荐系统和信息检索平台的排名质量。


<details>
  <summary>Details</summary>
Motivation: 推荐系统和信息检索平台依赖排名算法向用户展示最相关的内容，需要可靠的评估指标。MAP@k被广泛使用，但需要建立随机基准来更好地解释观察到的分数。

Method: 推导了AP@k的期望和方差，覆盖了离线和在线两种广泛使用的评估模型。期望建立了随机基准，方差量化了随机波动程度。

Result: 得到了AP@k的期望和方差表达式，这些可以作为MAP@k的基准，帮助判断排名算法的表现是否显著优于随机水平。

Conclusion: AP@k的期望和方差为MAP@k评估提供了重要的统计基准，使得对排名算法质量的解释更加可靠和有意义。
...
### [143] [Structural Plasticity as Active Inference: A Biologically-Inspired Architecture for Homeostatic Control](https://arxiv.org/abs/2511.02241)
*Brennen A. Hill*

Main category: cs.NE

TL;DR: SAPIN是一种受主动推理和生物神经形态可塑性启发的计算模型，通过局部预测误差最小化和细胞迁移实现学习和网络结构优化，在CartPole任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络依赖生物上不可信的全局反向传播机制，需要开发更符合生物学习原理的模型。

Method: SAPIN在2D网格上运行，采用两种并行学习机制：基于时间差异的局部Hebbian突触可塑性规则，以及细胞在网格上迁移以优化信息接收场的结构可塑性机制。

Result: 模型成功解决了CartPole强化学习任务，锁定参数后平均成功率82%，展示了通过最小化预测误差发现稳定平衡策略的能力。

Conclusion: SAPIN证明了局部预测误差最小化和结构可塑性足以实现有效学习，为开发更生物合理的神经网络提供了新途径。
...
### [144] [Evolutionary Algorithm for Chance Constrained Quadratic Multiple Knapsack Problem](https://arxiv.org/abs/2511.02500)
*Kokila Kasuni Perera,Aneta Neumann*

Main category: cs.NE

TL;DR: 提出了一种混合方法来解决带有随机利润的二次多背包问题，结合了进化算法和多因子优化的局部优化策略。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的资源分配问题往往具有随机性，需要处理随机利润和机会约束的复杂组合优化问题。

Method: 使用进化算法进行全局搜索，定期将中间解传递给基于多因子优化的局部优化器进行精炼，将局部问题建模为多任务问题。

Result: 实验结果表明，特别是结合多因子优化的混合方法在机会约束和容量约束较紧的情况下表现良好。

Conclusion: 提出的混合方法能有效处理带有随机利润的二次多背包问题，特别是在约束条件严格的情况下。
...
### [145] [Near Optimal Convergence to Coarse Correlated Equilibrium in General-Sum Markov Games](https://arxiv.org/abs/2511.02157)
*Asrin Efe Yorulmaz,Tamer Başar*

Main category: cs.GT

TL;DR: 本文改进了马尔可夫博弈中粗相关均衡(CCE)的收敛速率，从O(log⁵T/T)提升到O(logT/T)，与相关均衡(CE)的最佳收敛速率匹配，并在高维设置中带来指数级增益。


<details>
  <summary>Details</summary>
Motivation: 无遗憾学习动态在博弈论中扮演重要角色，能够实现去中心化收敛到均衡概念如CCE或CE。现有方法在马尔可夫博弈中收敛到CCE的速率不够理想，需要改进。

Method: 基于正常形式博弈中自适应步长技术的最新进展，通过阶段式方案将其扩展到马尔可夫设置，使用基于实时反馈调整学习率的乐观跟随正则化领导者(OFTRL)框架，定制化用于基于价值迭代的学习。

Result: 实现了O(logT/T)的收敛速率，这是目前已知的马尔可夫博弈中CCE的最快收敛速率，同时将动作集大小的依赖从多项式改进为多对数，在高维设置中带来指数级增益。

Conclusion: 提出的自博弈算法在马尔可夫博弈中实现了已知最快的CCE收敛速率，显著改进了现有方法的性能。
...
### [146] [Human-AI Collaboration with Misaligned Preferences](https://arxiv.org/abs/2511.02746)
*Jiaxin Song,Parnian Shahkar,Kate Donahue,Bhaskar Ray Chaudhury*

Main category: cs.GT

TL;DR: 该论文研究算法作为人类决策助手时的价值，发现与人类偏好不完全对齐的算法可能比完全对齐的算法带来更多效用，因为不同偏好的算法能提供更多样化的选择。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，算法常作为助手帮助人类从大量选项中筛选出较小子集供最终选择，但人类偏好本身存在噪声，且算法与人类偏好可能存在不对齐。研究这种不对齐关系如何影响人类效用。

Method: 通过理论建模和分析，研究算法与人类偏好不对齐情况下的协作效果，比较对齐算法与不对齐算法对人类效用的影响。

Result: 研究发现人类从不对齐算法（犯不同错误）获得的效用高于从对齐算法获得的效用，因为不对齐算法能提供更多样化的选择。

Conclusion: 算法设计者和政策制定者应考虑算法与人类偏好不对齐可能带来的积极影响，特别是在促进多样性和确保所有人类受益方面。
...
### [147] [CudaForge: An Agent Framework with Hardware Feedback for CUDA Kernel Optimization](https://arxiv.org/abs/2511.01884)
...
<details>
  <summary>Details</summary>
Motivation: 手动设计CUDA内核成本高、耗时长，现有自动生成方法往往产生低效内核、计算开销大且泛化能力差，需要更高效的自动化解决方案。

Method: 采用训练免费的多智能体工作流，包含Coder和Judge两个LLM智能体，通过迭代生成、测试、分析和优化CUDA内核，集成Nsight Compute等硬件性能指标反馈。

Result: 在KernelBench上达到97.6%的正确率，平均比PyTorch基线快1.68倍，优于OpenAI-o3和Kevin等SOTA模型，在多种GPU和基础模型上表现出强泛化能力，单内核优化仅需26.5分钟和0.3美元API成本。

Conclusion: 多智能体、训练免费的工作流能够实现成本效益高、泛化能力强且高性能的CUDA内核优化，为AI应用中的大规模LLM训练等场景提供高效解决方案。
...
### [148] [Retrieval-Augmented Multimodal Depression Detection](https://arxiv.org/abs/2511.01892)
*Ruibo Hou,Shiyu Teng,Jiaqing Liu,Shurong Chai,Yinhao Li,Lanfen Lin,Yen-Wei Chen*

Main category: cs.LG

TL;DR: 提出了一种基于检索增强生成(RAG)的多模态抑郁症检测框架，通过从情感数据集中检索相关内容并生成情感提示来增强情绪表示，在AVEC 2019数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有抑郁症检测方法面临的高计算成本、领域不匹配和静态知识限制等问题，特别是传统情感分析方法在情绪理解方面的局限性。

Method: 使用RAG框架，给定抑郁症相关文本时从情感数据集中检索语义相关的情感内容，然后利用大语言模型生成情感提示作为辅助模态。

Result: 在AVEC 2019数据集上取得了CCC为0.593和MAE为3.95的SOTA性能，超越了之前的迁移学习和多任务学习基线方法。

Conclusion: 提出的RAG框架通过增强情绪表示和可解释性，有效提升了抑郁症检测的性能，为解决多模态深度学习中的挑战提供了新思路。
...
### [149] [Deciphering Personalization: Towards Fine-Grained Explainability in Natural Language for Personalized Image Generation Models](https://arxiv.org/abs/2511.01932)
...
<details>
  <summary>Details</summary>
Motivation: 现有个性化模型缺乏可解释性，特别是无法精确识别多个个性化方面及其不同程度。自然语言解释比视觉特征更易理解，但现有方法过于粗粒度。

Method: 提出了FineXL技术，能够为个性化图像生成模型提供细粒度的自然语言解释，包括描述每个个性化方面并给出量化分数。

Result: 实验结果显示，FineXL在不同个性化场景下应用于多种图像生成模型时，将解释准确性提高了56%。

Conclusion: FineXL成功解决了现有方法在个性化图像生成模型解释性方面的局限性，提供了更精确的多方面个性化解释。
...
### [150] [The Eigenvalues Entropy as a Classifier Evaluation Measure](https://arxiv.org/abs/2511.01904)
*Doulaye Dembélé*
...
<details>
  <summary>Details</summary>
Motivation: 现有的分类评估指标在类别不平衡的数据集上表现不佳，需要更准确的评估方法。

Method: 使用特征值熵作为评估指标，建立了特征值与灵敏度、特异性、AUC和基尼指数等常用指标的关系，并提出了处理类别不平衡问题的混淆矩阵估计方法。

Result: 通过多个数据示例显示，所提出的特征值熵评估指标优于文献中的标准评估指标。

Conclusion: 特征值熵是一个有效的分类评估指标，特别适用于类别不平衡问题，能够提供比传统指标更准确的性能评估。
...
### [151] [Human-Machine Ritual: Synergic Performance through Real-Time Motion Recognition](https://arxiv.org/abs/2511.02351)
...
Method: 使用可穿戴IMU传感器采集数据，采用MiniRocket时间序列分类算法，结合体感记忆和关联将动作映射到声音控制。

Result: 系统可靠支持高精度分类（延迟<50毫秒），在创意、教育和现场表演环境中提供可复制的框架。

Conclusion: 该系统为将舞蹈智能机器集成到各种表演情境中提供了可行的解决方案，实现了高效的人机协同表演。
...
TL;DR: 提出了一种用于高维微分同胚映射的网格无关学习框架，结合变分原理和拟共形理论，通过控制共形畸变和体积畸变确保准确的双射映射。
...
Method: 将变分原理与拟共形理论相结合，通过调节共形畸变和体积畸变来控制变形质量，框架与基于梯度的优化和神经网络架构兼容。

Result: 在合成和真实医学图像数据上的数值实验验证了该方法在复杂配准场景中的准确性、鲁棒性和有效性。

Conclusion: 该框架为高维微分同胚映射提供了一种灵活可扩展的解决方案，能够处理复杂的配准问题。
...
Method: 引入叠加梯度下降(SGD)优化器，建立数学框架，在PyTorch和Qiskit中实现混合量子-经典电路，通过注入量子电路扰动将梯度更新与量子叠加联系起来。
...
### [154] [Neural Green's Functions](https://arxiv.org/abs/2511.01924)
*Seungwoo Yoo,Kyeongmin Yeo,Jisung Hwang,Minhyuk Sung*

Main category: cs.LG

TL;DR: 提出了Neural Green's Function，一种用于线性偏微分方程的神经解算子，通过模仿格林函数的行为，在多样化不规则几何和源边界函数上实现优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的解算子在泛化到未见过的源或边界函数时存在困难，需要设计一种对训练中使用的具体函数不敏感的解算子框架。
...
Conclusion: Neural Green's Function框架能够实现稳健和高效的泛化，对线性偏微分方程求解具有重要价值。
...
### [155] [DeepContour: A Hybrid Deep Learning Framework for Accelerating Generalized Eigenvalue Problem Solving via Efficient Contour Design](https://arxiv.org/abs/2511.01927)
...
<details>
  <summary>Details</summary>
Motivation: 传统轮廓积分方法在求解大规模广义特征值问题时，积分轮廓的选择严重依赖于特征值分布的先验知识，不当选择会导致计算开销大和数值精度差。

Method: 使用傅里叶神经算子快速预测GEP的谱分布，然后应用核密度估计自动确定最优积分轮廓，最后用优化后的轮廓指导轮廓积分求解器。

Result: 在多个数据集上实现了高达5.63倍的加速，有效提升了GEP求解效率。
...
### [156] [Dynamic Population Distribution Aware Human Trajectory Generation with Diffusion Model](https://arxiv.org/abs/2511.01929)
*Qingyue Long,Can Rong,Tong Li,Yong Li*

Main category: cs.LG

TL;DR: 提出了一种基于扩散模型的新型轨迹生成框架，通过整合动态人口分布约束来指导高保真度的轨迹生成。
...
Result: 实验表明生成轨迹在关键统计指标上接近真实轨迹，性能优于现有最优算法超过54%。

Conclusion: 该框架能有效生成高保真轨迹数据，为城市规划等应用提供可靠数据支持。
...
### [157] [Tool Zero: Training Tool-Augmented LLMs via Pure RL from Scratch](https://arxiv.org/abs/2511.01934)
...
Method: 提出动态泛化引导的奖励设计，从探索性工具使用模式逐步转向利用性模式，并基于此设计了Tool-Zero系列模型，直接从基础模型进行强化学习训练。

Result: 实验结果显示，该方法相比SFT和RL-with-SFT模型在相同实验设置下实现了超过7%的性能提升，在跨数据集和数据集内评估中均表现出有效性和鲁棒性。

Conclusion: 纯强化学习可以有效激发模型内在推理能力并增强工具无关的泛化能力，为工具增强LLM训练提供了新的有效途径。
...
### [158] [Q-Sat AI: Machine Learning-Based Decision Support for Data Saturation in Qualitative Studies](https://arxiv.org/abs/2511.01935)
*Hasan Tutar,Caner Erden,Ümit Şentürk*

Main category: cs.LG

TL;DR: 提出基于机器学习的系统模型，用于客观确定定性研究样本量，替代传统主观的数据饱和原则。


<details>
  <summary>Details</summary>
Motivation: 传统定性研究样本量确定依赖主观的数据饱和原则，导致不一致性并威胁方法严谨性。需要更客观、系统的方法。

Method: 使用五种基本定性研究方法的数据集，开发集成学习模型。评估10个关键参数作为输入特征，比较多种ML算法。

Result: KNN、梯度提升、随机森林、XGBoost和决策树算法表现最佳（测试R2约0.85），能有效建模定性抽样决策中的复杂非线性关系。

Conclusion: 提出基于Web的计算应用概念框架，作为定性研究者、期刊审稿人和论文导师的决策支持系统，标准化样本量论证并增强透明度。
...
### [159] [Shorter but not Worse: Frugal Reasoning via Easy Samples as Length Regularizers in Math RLVR](https://arxiv.org/abs/2511.01937)
*Abdelaziz Bounhar,Hadi Abdine,Evan Dufraisse,Ahmad Chamma,Amr Mohamed,Dani Bouch,Michalis Vazirgiannis,Guokan Shang*

Main category: cs.LG

TL;DR: 通过保留并适度加权中等难度问题作为隐式长度正则化器，防止LLMs在推理时过度冗长，实现无需显式长度惩罚的简洁推理。
...
Method: 在RLVR训练中保留并适度加权中等难度问题，让模型接触可解决的短链任务，约束输出分布，防止冗长失控。
...
### [160] [The Geometry of Grokking: Norm Minimization on the Zero-Loss Manifold](https://arxiv.org/abs/2511.01938)
*Tiberiu Musat*

Main category: cs.LG

TL;DR: 该论文将神经网络中的Grokking现象解释为约束优化问题，证明梯度下降在零损失流形上最小化权重范数，并推导出两层网络中第一层的后记忆动态闭式解。


<details>
  <summary>Details</summary>
Motivation: 理解Grokking现象中神经网络在完全记忆训练数据后延迟泛化的根本原因，揭示其背后的学习动力学机制。

Method: 采用约束优化视角，在无限小学习率和权重衰减系数的极限下证明梯度下降在零损失流形上最小化权重范数，并引入参数子集学习动态解耦的近似方法。

Result: 推导出两层网络第一层后记忆动态的闭式表达式，实验验证使用预测梯度模拟训练过程能够重现Grokking的延迟泛化和表征学习特征。

Conclusion: Grokking现象可以通过约束优化框架得到解释，梯度下降在零损失流形上的权重范数最小化是导致延迟泛化的关键机制。
...
### [161] [Learning a Distance for the Clustering of Patients with Amyotrophic Lateral Sclerosis](https://arxiv.org/abs/2511.01945)
...
<details>
  <summary>Details</summary>
Motivation: ALS患者对治疗反应差异大，需要个性化护理，但现有研究受限于小样本、异质性数据和缺乏明确临床分群定义。

Method: 使用疾病进展评分对序列进行聚类，整合医学专业知识，研究多种距离测量方法（包括现成距离和弱监督学习），并与聚类方法配对。

Result: 在353名ALS患者数据集上测试，在生存分析方面优于现有方法，同时获得相当的轮廓分数，学习到的距离增强了结果的相关性和可解释性。

Conclusion: 该方法在ALS患者聚类中表现出优越性能，为个性化医疗提供了有价值的工具，同时结果对医学专家更具解释性。
...
### [162] [COFAP: A Universal Framework for COFs Adsorption Prediction through Designed Multi-Modal Extraction and Cross-Modal Synergy](https://arxiv.org/abs/2511.01946)
*Zihan Li,Mingyang Wan,Mingyu Gao,Zhongshan Chen,Xiangke Wang,Feifan Zhang*

Main category: cs.LG

TL;DR: 提出了COFAP框架，通过深度学习提取多模态结构化学特征，使用跨模态注意力机制融合特征，无需传统气体相关特征即可实现COFs吸附性能的准确预测。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习预测器依赖特定气体相关特征，这些特征计算耗时且限制了可扩展性，导致效率低下和劳动密集型流程。

Method: 开发COFAP框架，通过深度学习提取多模态结构和化学特征，并使用跨模态注意力机制融合这些互补特征。

Result: COFAP在hypoCOFs数据集上超越了先前方法，建立了新的SOTA；发现高性能分离COFs集中在狭窄的孔径和比表面积范围内。

Conclusion: COFAP具有优越的效率和准确性，可直接应用于晶体多孔材料的高通量筛选，并开发了权重可调优先排序方案支持应用特定排名。
...
### [163] [Interpretable Heart Disease Prediction via a Weighted Ensemble Model: A Large-Scale Study with SHAP and Surrogate Decision Trees](https://arxiv.org/abs/2511.01947)
*Md Abrar Hasnat,Md Jobayer,Md. Mehedi Hasan Shawon,Md. Golam Rabiul Alam*

Main category: cs.LG

TL;DR: 该研究开发了一个结合树模型和CNN的加权集成模型，用于心血管疾病风险预测，在22.9万患者数据集上实现了0.8371的AUC和80%的召回率，并通过SHAP和决策树提供可解释性。
...
### [164] [EchoLSTM: A Self-Reflective Recurrent Network for Stabilizing Long-Range Memory](https://arxiv.org/abs/2511.01950)
*Prasanth K K,Shubham Sharma*

Main category: cs.LG

TL;DR: 提出输出条件门控机制，通过基于过去推断调节内部记忆门实现自反思维，结合注意力机制形成EchoLSTM模型，在长序列依赖任务中显著优于标准LSTM，并与Transformer性能相当但参数效率更高。
...
Method: 提出输出条件门控架构原则，使模型能够基于自身过去推断调节内部记忆门，实现自反思维。最终模型EchoLSTM将此原则与注意力机制结合。

Result: 在Distractor Signal Task上达到69.0%准确率，比标准LSTM基线提升33个百分点；在ListOps基准测试中达到69.8%准确率，与Transformer的71.8%相当，但参数效率高出5倍以上。

Conclusion: 自反思维机制使模型形成了更稳健的记忆系统，在长序列依赖任务中表现出色且参数效率高。
...
### [165] [NeuroClean: A Generalized Machine-Learning Approach to Neural Time-Series Conditioning](https://arxiv.org/abs/2511.01951)
*Manuel A. Hernandez Alonso,Michael Depass,Stephan Quessy,Numa Dancause,Ignasi Cos*

Main category: cs.LG

TL;DR: NeuroClean是一个无监督的EEG/LFP预处理管道，通过五步流程自动去除脑电信号中的伪影和噪声，使用机器学习分类器确保任务相关信息在清洗过程中得以保留。


<details>
  <summary>Details</summary>
Motivation: 脑电信号记录中存在大量伪影和噪声，需要自动化预处理方法来提高数据质量并减少人为干预带来的偏差和可重复性问题。
...
### [166] [Bulk-boundary decomposition of neural networks](https://arxiv.org/abs/2511.02003)
...
<details>
  <summary>Details</summary>
Motivation: 为深度神经网络的训练动态提供新的理论理解框架，揭示网络架构和训练样本在训练过程中的不同作用。

Method: 从随机梯度下降公式出发，将拉格朗日量重新组织为bulk项和boundary项，并基于此分解发展了神经动态的场论表述。

Result: bulk捕获由网络架构和激活函数设定的内在动态，boundary反映输入和输出层训练样本的随机相互作用，揭示了深度网络的局部和同质结构。

Conclusion: bulk-boundary分解为理解深度神经网络训练动态提供了新的理论视角，并自然扩展到场论表述。
...
### [167] [TapOut: A Bandit-Based Approach to Dynamic Speculative Decoding](https://arxiv.org/abs/2511.02017)
*Aditya Sridhar,Nish Sinnadurai,Sean Lie,Vithursan Thangarasa*

Main category: cs.LG

TL;DR: TapOut是一种基于多臂老虎机的在线、免训练、即插即用的动态推测解码算法，用于智能决定LLM推测解码中要生成多少个token，无需手动调参即可达到竞争性或更优的加速效果。


<details>
  <summary>Details</summary>
Motivation: 推测解码通过轻量级草稿模型生成token并用大型目标模型并行验证来加速LLM，但确定最佳推测token数量是关键挑战。现有方法依赖手动调参的敏感阈值，成本高且跨模型和领域泛化能力差。

Method: 提出TapOut算法，使用多臂老虎机在线选择多个无参数动态推测策略，基于过去的奖励和探索来选择最佳策略，无需超参数调优。

Result: 在多样化模型对和数据集上的广泛实验表明，TapOut相比成熟的动态推测基线方法，无需任何超参数调优即可达到竞争性或更优的加速效果。

Conclusion: TapOut为动态推测解码提供了一种有效、免调参的解决方案，能够智能决定推测token数量，显著提升LLM推理效率。
...
### [168] [Shared Parameter Subspaces and Cross-Task Linearity in Emergently Misaligned Behavior](https://arxiv.org/abs/2511.02022)
...
<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型在微调后出现广泛对齐偏差现象的根本机制，特别是跨不同领域的有害行为泛化机制。

Method: 采用几何视角研究对齐偏差现象，分析微调权重更新的余弦相似度、主角度、投影重叠等几何特性，以及通过线性模式连接性验证功能等价性。

Result: 发现不同狭窄对齐偏差任务会收敛到相同的参数方向，微调权重更新表现出高余弦相似度和共享低维子空间，插值模型保持一致的广泛对齐偏差行为。

Conclusion: 对齐偏差现象源于不同狭窄任务发现相同的共享参数方向，有害行为可能组织在权重景观的特定可预测区域中，揭示了参数几何与行为结果之间的基本联系。
...
### [169] [Path-Coordinated Continual Learning with Neural Tangent Kernel-Justified Plasticity: A Theoretical Framework with Near State-of-the-Art Performance](https://arxiv.org/abs/2511.02025)
*Rathin Chandra Shit*
...
<details>
  <summary>Details</summary>
Motivation: 灾难性遗忘是持续学习中的核心问题，神经网络在训练新任务时会忘记先前学习的任务。
...
Conclusion: 该框架能有效降低灾难性遗忘，随着任务序列推进遗忘率从27%降至18%，验证了80%的路径并保持90-97%的中间任务保留率。
...
### [170] [RobustFSM: Submodular Maximization in Federated Setting with Malicious Clients](https://arxiv.org/abs/2511.02029)
*Duc A. Tran,Dung Truong,Duy Le*

Main category: cs.LG

TL;DR: 提出了RobustFSM，一种针对联邦子模最大化问题的鲁棒解决方案，能够抵御各种客户端攻击，在严重攻击情况下性能比传统联邦算法提升高达200%。


<details>
  <summary>Details</summary>
Motivation: 联邦子模最大化中，分散的客户端拥有本地数据并定义代表性质量，但容易受到恶意客户端分享虚假信息的攻击，类似于联邦学习中的后门攻击，但具有子模最大化的独特特性。

Method: 开发了RobustFSM解决方案，通过设计鲁棒机制来应对各种实际客户端攻击，保护联邦子模最大化过程的安全性。

Result: 使用真实世界数据集进行实证评估，数值结果显示在严重攻击情况下，RobustFSM的解决方案质量显著超过传统联邦算法，改进程度取决于数据集和攻击场景，最高可达200%。

Conclusion: RobustFSM为联邦子模最大化提供了一种有效的鲁棒解决方案，能够抵御客户端恶意行为，在安全性和性能方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 预测种间相互作用是微生物生态学的关键挑战，这些相互作用决定了微生物群落的结构和活性。

Method: 构建配对微生物相互作用的边图，利用图神经网络(GNN)作为分类器，基于单一培养生长能力、与其他物种的相互作用和系统发育数据来预测相互作用方向。

Result: 模型不仅能够预测二元相互作用(正/负)，还能分类更复杂的相互作用类型(互利共生、竞争、寄生)，F1分数达到80.44%，显著优于XGBoost的72.76%。

Conclusion: 图神经网络是预测微生物种间相互作用的有效方法，能够利用共享信息提高预测准确性。
...
### [172] [Regularization Through Reasoning: Systematic Improvements in Language Model Classification via Explanation-Enhanced Fine-Tuning](https://arxiv.org/abs/2511.02044)
...
<details>
  <summary>Details</summary>
Motivation: 探索在LLM分类微调中为标签附加简要解释是否能提升模型性能，特别是理解这种提升是否源于解释的语义内容还是结构特性。

Method: 使用多个LLM生成的数据集，在6个对话数据集上微调7B参数模型，比较标签加解释训练与仅标签训练的效果，并测试用词汇对齐但语义不连贯的随机token序列替代真实解释的效果。

Result: 在18个数据集和任务设置中，标签加解释训练均优于仅标签基线。令人意外的是，即使使用随机token序列作为伪解释，也能显著提升准确率，缩小与真实解释的差距。模型内部分析显示解释增强模型在中间层有更高的激活熵，输出层有更尖锐的预测分布。

Conclusion: 解释增强微调（无论使用真实理由还是精心构造的随机token序列）能提高LLM分类的准确性和可靠性，表明token级支架通过促进更丰富的中间计算和减少过度自信的捷径来塑造推理过程。
...
### [173] [Quantum-Enhanced Generative Models for Rare Event Prediction](https://arxiv.org/abs/2511.02042)
*M. Z. Haider,M. U. Ghouri,Tayyaba Noreen,M. Salman*

Main category: cs.LG

TL;DR: 提出量子增强生成模型(QEGM)，一种混合经典-量子框架，通过变分量子电路增强对罕见事件的建模能力，在金融、气候和生物数据上显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 罕见事件（如金融崩盘、极端气候）由于稀缺性和重尾分布难以建模，传统深度生成模型容易忽略低概率模式或产生不准确的置信度估计。

Method: 结合深度潜变量模型与变分量子电路，采用混合损失函数（重构保真度+尾部感知似然）和量子随机性驱动的噪声注入，通过混合优化循环训练。

Result: 在合成高斯混合和真实数据集上，QEGM将尾部KL散度降低达50%，提升了罕见事件召回率和覆盖校准能力。
...
TL;DR: 本文重新审视了CCS方法，提出相对对比一致性的优化目标，将CCS重新表述为特征值问题，获得闭式解并扩展到多变量情况，避免了随机初始化敏感性问题。


<details>
  <summary>Details</summary>
Motivation: CCS作为一种无监督探测方法，虽然表现出潜力，但其双目标函数机制尚未被完全理解。本文旨在澄清CCS的机制并扩展其适用性。

Method: 将CCS重新表述为特征值问题，获得具有可解释特征值的闭式解，并自然扩展到多变量情况。
...
### [175] [Flashlight: PyTorch Compiler Extensions to Accelerate Attention Variants](https://arxiv.org/abs/2511.02043)
...
<details>
  <summary>Details</summary>
Motivation: 现有的注意力优化方法如FlashAttention和FlexAttention存在局限性，前者需要专门的内核，后者只能支持部分注意力变体。需要一种更灵活、自动化的方法来支持各种注意力模式的高效实现。

Method: 利用PyTorch的编译工作流，透明地融合和分块注意力计算，自动生成优化的内核。支持数据依赖的注意力公式，超越了FlexAttention的能力范围。

Result: Flashlight生成的内核性能与FlexAttention相当或更优，同时提供了原生PyTorch代码的灵活性，使开发者能够快速探索新的注意力模型而不牺牲性能。

Conclusion: Flashlight填补了现有注意力优化方法的空白，提供了一个既高效又灵活的解决方案，能够支持更广泛的注意力变体，包括数据依赖的注意力模式。
...
### [176] [A Dual-Use Framework for Clinical Gait Analysis: Attention-Based Sensor Optimization and Automated Dataset Auditing](https://arxiv.org/abs/2511.02047)
*Hamidreza Sadeghsalehi*

Main category: cs.LG

TL;DR: 提出基于多流注意力机制的深度学习框架，既能优化传感器配置，又能自动审计数据集完整性。在步态分析任务中发现公开数据集存在严重的侧向性偏差。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器和AI的客观步态分析对管理神经和骨科疾病至关重要，但模型容易受到隐藏数据集偏差的影响，且特定任务的传感器优化仍然具有挑战性。

Method: 多流注意力深度学习框架，应用于Voisard等人的多队列步态数据集，涵盖四个临床任务（PD、OA、CVA筛查；PD vs CVA鉴别）。

Result: 注意力机制定量发现数据集存在严重混淆。在OA和CVA筛查任务中，模型将超过70%的注意力分配给右脚，而统计上忽略左脚（注意力小于0.1%），这反映了公开数据集中严重的侧向性偏差。

Conclusion: 主要贡献是方法学上的，证明可解释框架可以自动审计数据集完整性。次要发现是模型提出了新颖的数据驱动传感器协同作用，为未来优化协议提供假设。
...
### [177] [Finding Probably Approximate Optimal Solutions by Training to Estimate the Optimal Values of Subproblems](https://arxiv.org/abs/2511.02048)
*Nimrod Megiddo,Segev Wasserkrug,Orit Davidovich,Shimrit Shtern*

Main category: cs.LG

TL;DR: 开发一个用于最大化二元变量实值函数的求解器，通过估计目标函数及其子实例的最优值来训练估计器，使用期望总偏差作为损失函数。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要计算策略值或依赖已解决的实例，而该方法旨在避免这些限制，通过估计最优目标函数值来更高效地求解二元优化问题。

Method: 基于一个不等式，使用期望总偏差作为损失函数来训练估计器，估计目标函数及其子实例的最优值，而不直接计算策略值或依赖已解决的实例。

Result: 该方法能够有效估计最优目标函数值，并通过训练估计器来求解二元变量的最大化问题，避免了传统方法的计算瓶颈。

Conclusion: 提出的求解器通过估计最优目标函数值和子实例的最优值，使用期望总偏差作为损失函数，提供了一种不依赖策略值计算或已解决实例的高效求解方法。
...
TL;DR: 提出One-Shot Dynamic Thresholding (OSDT)方法，通过在单个序列上校准阈值并将其应用于后续输入，显著加速掩码扩散语言模型的解码过程。


<details>
  <summary>Details</summary>
Motivation: 观察到现有并行解码方法使用固定全局置信度阈值存在强烈的位置和步骤间置信度波动，以及数据集中输入间置信度轨迹的高度相似性，这为动态阈值校准提供了机会。
...
TL;DR: 提出一种将物理信息直接融入损失函数的框架，通过假设数据样本处于热平衡状态，推导出基于能量的损失函数，用于分子生成和自旋基态预测任务。


<details>
  <summary>Details</summary>
Motivation: 在科学领域的机器学习应用中，有效利用系统的物理先验知识至关重要。以往方法主要在架构层面融入物理洞察，本研究旨在将物理信息直接整合到损失函数中。

Method: 假设每个数据样本相对于近似能量景观处于热平衡状态，使用玻尔兹曼分布的反向KL散度推导能量损失函数，将损失表示为数据与模型预测之间的能量差。

Result: 在分子生成和自旋基态预测任务中，该方法相比基线模型取得了显著改进，损失函数梯度更好地与有效配置对齐，且计算效率高。

Conclusion: 提出的能量损失函数框架能够产生物理基础扎实的损失函数，尊重物理对称性，架构无关且计算高效，为科学领域的机器学习应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决世界建模领域碎片化问题，研究人员各自开发专用架构而很少相互借鉴，需要一个统一的框架来构建结构化世界模型。

Method: 使用隐马尔可夫模型（HMMs）和切换线性动态系统（sLDS）作为离散和连续过程的基础构建块，通过层次组合构建世界模型，避免传统结构学习的组合爆炸问题。

Result: 在多模态生成建模和像素级规划任务中表现出与神经网络方法竞争的性能，同时保持可解释性。

Conclusion: 这些自然构建块可能为世界建模提供基础架构，类似于标准化层在深度学习中的推动作用，但核心挑战是可扩展的联合结构-参数学习。
...
TL;DR: 该论文提出了一种不确定性引导的在线集成方法，用于处理聚变数据中的非平稳分布漂移问题，显著提升了托罗伊德场线圈偏转预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 聚变数据具有非平稳特性，存在分布漂移问题，而传统机器学习模型假设数据分布稳定，在遇到非平稳数据流时性能会下降。在线学习技术在其他领域已有应用，但在聚变应用中尚未充分探索。

Method: 使用在线学习来适应漂移的数据流，并提出不确定性引导的在线集成方法。利用深度高斯过程近似（DGPA）进行校准的不确定性估计，然后使用不确定性值指导元算法，基于在不同历史数据范围上训练的集成学习器产生预测。

Result: 在线学习相比静态模型减少了80%的误差。在线集成和不确定性引导的在线集成分别比标准单模型在线学习减少了约6%和10%的预测误差。

Conclusion: 在线学习对于维持机器学习模型在聚变应用中的性能至关重要，而提出的不确定性引导在线集成方法进一步提升了预测性能，同时为决策者提供了不确定性估计。
...
### [182] [Geometric Data Valuation via Leverage Scores](https://arxiv.org/abs/2511.02100)
*Rodrigo Mendoza-Smith*

Main category: cs.LG

TL;DR: 提出基于统计杠杆得分的数据估值几何替代方法，替代计算复杂的Shapley值，满足核心公理并提供与下游决策质量的理论保证。


<details>
  <summary>Details</summary>
Motivation: Shapley数据估值虽然理论严谨但计算组合复杂度高，难以在大规模数据上应用，需要寻找计算可行的替代方法。

Method: 使用统计杠杆得分量化数据点在表示空间中的结构影响，通过测量其对数据集张成的贡献程度，并扩展到岭杠杆得分以获得严格正边际增益。

Result: 杠杆采样训练得到的模型参数和预测风险与全数据最优解仅相差O(ε)，岭杠杆采样在主动学习实验中优于标准基线方法。

Conclusion: 统计杠杆得分提供了一种计算高效的数据估值方法，满足Shapley公理并建立了数据估值与下游决策质量的严格联系。
...
### [183] [Measuring the Intrinsic Dimension of Earth Representations](https://arxiv.org/abs/2511.02101)
...
Method: 通过分析环境维度在256-512之间的INRs，研究其内在维度对空间分辨率和输入模态变化的敏感性。

Result: 发现地理INRs的内在维度在2-10之间，与下游任务性能相关，并能捕捉空间伪影。

Conclusion: 内在维度提供了一个架构无关、无标签的信息内容度量，可用于INRs的无监督评估、模型选择和预训练设计。
...
### [184] [Matrix Sensing with Kernel Optimal Loss: Robustness and Optimization Landscape](https://arxiv.org/abs/2511.02122)
*Xinyuan Song,Jiaye Teng,Ziye Ma*

Main category: cs.LG

TL;DR: 该论文研究了非凸优化问题中损失函数选择对鲁棒性和优化景观的影响，通过噪声矩阵感知任务展示了基于非参数回归的鲁棒损失函数相比传统MSE损失在处理非高斯和重尾噪声时的优越性。
...
Method: 采用基于非参数回归的鲁棒损失函数，使用核密度估计残差密度并最大化估计的对数似然，该损失在误差为高斯时与MSE一致，但在更一般设置下保持稳定。

Result: 理论和实证分析表明，新损失函数在处理大噪声和多样化噪声分布时表现优异，通过分析限制等距性质常数的上界来研究鲁棒损失如何重塑优化景观。

Conclusion: 通过简单地改变损失函数可以增强机器学习任务的鲁棒性，这为在直观且广泛适用的分析框架指导下改进模型性能提供了初步见解。
...
### [185] [Variance-Aware Feel-Good Thompson Sampling for Contextual Bandits](https://arxiv.org/abs/2511.02123)
*Xuheng Li,Quanquan Gu*

Main category: cs.LG

TL;DR: 本文提出了FGTS-VA，一种针对上下文赌博机问题的方差感知Thompson Sampling算法，具有最优的遗憾界，适用于一般奖励函数。


<details>
  <summary>Details</summary>
Motivation: 现有的方差相关遗憾界研究主要集中在UCB类算法，而基于采样的Thompson Sampling算法研究不足。唯一的相关工作LinVDTS仅限于线性奖励函数且遗憾界在模型维度上不是最优的。

Method: 提出了FGTS-VA算法，核心分析技术是扩展了去耦系数，该系数反映了模型空间的复杂度。新去耦系数记为dc，算法实现了与方差相关的遗憾界。
...
Conclusion: 在线性上下文赌博机设置下，FGTS-VA的遗憾界与使用加权线性回归的UCB类算法相匹配，证明了该算法在方差感知Thompson Sampling中的优越性。
...
### [186] [QuPCG: Quantum Convolutional Neural Network for Detecting Abnormal Patterns in PCG Signals](https://arxiv.org/abs/2511.02140)
...
<details>
  <summary>Details</summary>
Motivation: 早期识别异常生理模式对及时检测心脏疾病至关重要，需要开发高效的心音信号分类方法。

Method: 结合小波特征提取和自适应阈值压缩方法，将一维心音图信号转换为紧凑的二维图像，使用混合量子-经典CNN进行分类。
...
### [187] [Disentangling Causal Substructures for Interpretable and Generalizable Drug Synergy Prediction](https://arxiv.org/abs/2511.02146)
*Yi Luo,Haochen Zhao,Xiao Liang,Yiwei Liu,Yuye Zhang,Xinyu Li,Jianxin Wang*

Main category: cs.LG

TL;DR: 提出CausalDDS框架，通过将药物分子分解为因果和伪子结构，利用因果子结构表示来预测药物协同作用，提高模型准确性和可解释性。
...
Method: 将药物分子分解为因果和伪子结构，采用条件干预机制（干预基于配对分子结构），并提出基于充分性和独立性原则的新优化目标。

Result: 在冷启动和分布外设置下优于基线模型，能有效识别药物协同作用的关键子结构，提供分子层面的机制解释。
...
### [188] [Can LLMs subtract numbers?](https://arxiv.org/abs/2511.02795)
...
TL;DR: 对大型语言模型在减法运算中的系统性研究，发现减法准确率远低于加法，主要问题在于处理负数结果时经常遗漏负号。
...
Method: 评估了8个预训练LLMs在加减法问题上的表现，通过探测分析了解模型内部表示，并测试了少样本学习和指令调优等改进方法。

Result: 减法准确率显著低于加法，错误主要集中在a<b的情况，模型能内部识别结果应为负数但输出时经常遗漏负号。指令调优模型在生成负号方面达到近乎完美的准确率。

Conclusion: 研究揭示了LLMs在减法运算中的局限性，特别是处理负数结果的能力，但通过指令调优可以显著改善这一缺陷。
...
### [189] [CFL: On the Use of Characteristic Function Loss for Domain Alignment in Machine Learning](https://arxiv.org/abs/2511.02148)
*Abdullah Almansour,Ozan Tonguz*

Main category: cs.LG

TL;DR: 本文提出使用特征函数(CF)作为频域方法来衡量高维空间中的分布偏移和进行领域自适应，作为传统统计方法的替代方案。
...
Method: 使用特征函数(CF)作为频域方法来量化高维空间中的分布偏移，并与传统统计方法(如KL散度、KS检验、Wasserstein距离等)进行对比。
...
### [190] [In Good GRACEs: Principled Teacher Selection for Knowledge Distillation](https://arxiv.org/abs/2511.02833)
*Abhishek Panigrahi,Bingbin Liu,Sadhika Malladi,Sham Kakade,Surbhi Goel*

Main category: cs.LG

TL;DR: 提出了GRACE评分方法，无需验证器、教师模型内部信息或测试数据，仅通过分析学生模型梯度分布特性来量化教师模型对学生模型蒸馏的有效性。
...
Method: GRACE通过测量学生模型梯度的分布特性，从信息论角度与梯度算法的留一稳定性建立联系，从而控制蒸馏学生的泛化性能。

Result: 在GSM8K和MATH数据集上，GRACE与LLaMA和OLMo学生模型的蒸馏性能强相关（Spearman相关性高达86%），使用GRACE选择的教师模型相比简单选择最佳性能教师可提升7.4%性能。

Conclusion: GRACE能高效有效地为给定学生模型识别强兼容的教师模型，并为蒸馏过程提供细粒度指导，包括最佳温度设置、尺寸约束下的最佳教师选择以及特定模型族内的最佳教师选择。
...
### [191] [ProtoTSNet: Interpretable Multivariate Time Series Classification With Prototypical Parts](https://arxiv.org/abs/2511.02152)
*Bartłomiej Małkus,Szymon Bobek,Grzegorz J. Nalepa*

Main category: cs.LG

TL;DR: ProtoTSNet是一种用于多变量时间序列分类的可解释方法，通过改进ProtoPNet架构来捕捉动态模式并处理特征重要性变化，在UEA档案的30个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在工业和医学等关键领域，时间序列数据很常见，需要既准确又可解释的算法，因为这些领域的决策具有重大影响。
...
Conclusion: ProtoTSNet提供可解释的结果，领域专家可访问，在保持竞争力的同时实现了可解释性。
...
### [192] [Tackling Incomplete Data in Air Quality Prediction: A Bayesian Deep Learning Framework for Uncertainty Quantification](https://arxiv.org/abs/2511.02175)
*Yuzhuang Pian,Taiyu Wang,Shiqi Zhang,Rui Xu,Yonghong Liu*

Main category: cs.LG

TL;DR: 提出了CGLUBNF框架，通过傅里叶特征和图注意力编码器处理时空数据缺失问题，使用通道门控学习单元自适应过滤特征，贝叶斯推理优化预测分布，在空气质量预测中实现高精度和校准的预测区间。


<details>
  <summary>Details</summary>
Motivation: 空气质量预测对公共卫生至关重要，但观测数据常因收集和传输问题存在不同程度和模式的缺失，这些不完整的时空记录会阻碍可靠推断和风险评估，导致过度自信的外推。

Method: 使用傅里叶特征和图注意力编码器捕捉多尺度空间依赖性和季节性时间动态，通道门控学习单元配备可学习激活函数和门控残差连接来自适应过滤和放大信息特征，贝叶斯推理联合优化预测分布和参数不确定性。

Result: 在两个真实世界数据集上系统评估，涵盖四种典型缺失数据模式，与五种最先进基线方法比较，CGLUBNF实现了优越的预测精度和更尖锐的置信区间，在多个预测时间范围内验证了鲁棒性。
...
### [193] [OmniField: Conditioned Neural Fields for Robust Multimodal Spatiotemporal Learning](https://arxiv.org/abs/2511.02205)
*Kevin Valencia,Thilina Balasooriya,Xihaier Luo,Shinjae Yoo,David Keetae Park*

Main category: cs.LG

TL;DR: OmniField是一个多模态时空学习框架，能够处理稀疏、不规则、有噪声的多模态数据，通过跨模态融合和迭代优化实现重建、插值、预测等任务。


<details>
  <summary>Details</summary>
Motivation: 解决真实世界多模态数据中的两个挑战：模态内测量稀疏、不规则、有噪声，以及可用模态在时空上变化导致记录可用性降低的问题。

Method: 提出OmniField框架，使用连续性感知的神经场，结合多模态交叉块架构和迭代跨模态细化，在解码器前对齐信号，无需网格化或替代预处理。

Result: 在广泛评估中，OmniField持续优于八个强大的多模态时空基线方法，在严重模拟传感器噪声下性能仍接近干净输入水平。

Conclusion: OmniField展示了处理多模态时空数据的强大能力，特别是在面对噪声和模态缺失时的鲁棒性。
...
### [194] [Learning Interactive World Model for Object-Centric Reinforcement Learning](https://arxiv.org/abs/2511.02225)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于对象的RL方法通常只对单个对象进行状态分解，而将对象间的交互关系隐式处理。为了学习更鲁棒和可迁移的策略，需要显式建模对象间的交互关系。

Method: 1. 从像素中学习对象中心潜在表示和交互结构；2. 使用预训练视觉编码器；3. 将任务分解为可组合的交互原语；4. 训练分层策略：高层选择交互类型和顺序，低层执行交互。

Result: 在模拟机器人和具身AI基准测试中，FIOC-WM相比世界模型基线方法，在策略学习的样本效率和泛化能力方面都有显著提升。

Conclusion: 显式的模块化交互学习对于鲁棒控制至关重要，FIOC-WM框架证明了这一点。
...
### [195] [Opportunistic Expert Activation: Batch-Aware Expert Routing for Faster Decode Without Retraining](https://arxiv.org/abs/2511.02237)
...
<details>
  <summary>Details</summary>
Motivation: MoE模型在自回归生成时经常进入内存受限状态，因为平均专家负载增长比等效密集前馈层慢，导致MoE延迟由激活专家数量决定。

Method: 使用批处理感知路由，让token搭载已被加载到内存中的专家，这些专家对同一批次中的其他token至关重要。

Result: 在Qwen3-30B和Qwen3-235B模型上，批次大小为16时，MoE层解码延迟分别降低了39%和15%，且没有统计显著的精度损失。

Conclusion: 批处理感知路由能有效降低MoE模型解码延迟，同时保持模型质量。
...
### [196] [Neural network initialization with nonlinear characteristics and information on spectral bias](https://arxiv.org/abs/2511.02244)
*Hikaru Homma,Jun Ohkubo*

Main category: cs.LG

TL;DR: 本文提出了一种基于频谱偏置的神经网络初始化框架，通过调整SWIM算法中的尺度因子，在早期隐藏层捕获低频分量，在后期隐藏层表示高频分量，从而提升训练性能。


<details>
  <summary>Details</summary>
Motivation: 神经网络参数初始化对学习性能有重要影响，好的初始化甚至可以避免反向传播训练。同时神经网络存在频谱偏置现象，即在早期层倾向于学习粗粒度信息。本研究旨在利用频谱偏置信息来改进神经网络初始化。

Method: 提出一个框架，调整SWIM算法中的尺度因子，使早期隐藏层捕获低频分量，后期隐藏层表示高频分量。

Result: 在一维回归任务和MNIST分类任务上的数值实验表明，所提方法优于传统初始化算法。

Conclusion: 这项工作阐明了频谱特性在神经网络学习中的重要性，并产生了一种有效的参数初始化策略，能够增强训练性能。
...
### [197] [Probabilistic Graph Cuts](https://arxiv.org/abs/2511.02272)
*Ayoub Ghriss*

Main category: cs.LG

TL;DR: 提出了一个统一的概率框架，为包括归一化割在内的多种图割方法提供可微分的替代方案，通过积分表示和高斯超几何函数获得紧致的解析上界。


<details>
  <summary>Details</summary>
Motivation: 现有的概率化图割方法主要关注RatioCut，缺乏通用性保证和理论严谨的梯度计算，需要建立一个更通用的框架来支持多种图割方法和稳定的数值计算。

Method: 使用积分表示和高斯超几何函数构建统一的概率框架，为多种图割方法提供紧致的解析上界，并实现闭式的前向和后向传播。

Result: 该框架为可扩展的、可微分的图分割提供了严谨且数值稳定的基础，适用于多种聚类和对比学习目标。

Conclusion: 提出的统一概率框架成功解决了现有方法的局限性，为图割的可微分学习提供了理论保证和实用工具。
...
### [198] [Gradient-Variation Online Adaptivity for Accelerated Optimization with Hölder Smoothness](https://arxiv.org/abs/2511.02276)
*Yuheng Zhao,Yu-Hu Yan,Kfir Yehuda Levy,Peng Zhao*

Main category: cs.LG

TL;DR: 本文研究了Hölder光滑函数下的在线学习问题，并将其与离线优化联系起来。提出了自适应梯度变化在线学习算法，无需预先知道光滑度参数，并通过在线到批量转换获得了随机凸优化的通用方法。


<details>
  <summary>Details</summary>
Motivation: 光滑性在离线优化加速和在线学习梯度变化后悔最小化中都很重要。这两种问题密切相关，加速优化可以通过梯度变化在线学习的视角来理解。本文旨在研究Hölder光滑函数（包含光滑和非光滑函数）的在线学习及其对离线优化的影响。

Method: 针对（强）凸在线函数，设计了相应的梯度变化在线学习算法，其后悔在光滑和非光滑机制之间平滑插值。算法不需要预先知道Hölder光滑度参数，具有强自适应性。通过在线到批量转换，将这种自适应性转化为随机凸优化的通用方法。对于离线强凸优化，结合在线自适应性和基于检测的猜测检查过程。

Result: 提出的梯度变化在线学习算法在光滑和非光滑机制之间实现了最优保证的平滑插值，且无需预先知道光滑度参数。通过在线到批量转换，获得了Hölder光滑性下随机凸优化的最优通用方法。对于离线强凸优化，首次实现了通用方法，在光滑机制下达到加速收敛，在非光滑机制下保持近最优收敛。

Conclusion: 本文成功地将在线学习的梯度变化自适应性应用于离线优化问题，为Hölder光滑函数类提供了统一的解决方案。在线自适应性与检测机制的结合首次实现了离线强凸优化的通用方法，在光滑和非光滑机制下都能获得最优或近最优的性能。
...
### [199] [Reinforcement learning based data assimilation for unknown state model](https://arxiv.org/abs/2511.02286)
*Ziyi Wang,Lijian Jiang*

Main category: cs.LG

TL;DR: 提出一种结合强化学习和贝叶斯滤波的新方法，直接从噪声观测中学习未知动态系统的状态转移模型，无需真实状态轨迹。
...
### [200] [Federated Quantum Kernel Learning for Anomaly Detection in Multivariate IoT Time-Series](https://arxiv.org/abs/2511.02301)
*Kuan-Cheng Chen,Samuel Yen-Chi Chen,Chen-Yu Liu,Kin K. Leung*

Main category: cs.LG

TL;DR: 提出联邦量子核学习框架，将量子特征映射与联邦聚合结合，用于分布式、保护隐私的工业物联网异常检测


<details>
  <summary>Details</summary>
Motivation: 工业物联网系统面临高维多变量时间序列异常检测的挑战，传统联邦学习方法在处理非线性决策边界和不平衡异常分布时存在困难

Method: 使用参数化量子电路在量子边缘节点本地计算压缩核统计量，仅共享这些摘要到中央服务器构建全局Gram矩阵并训练决策函数

Result: 在合成工业物联网基准测试中，FQKL在捕捉复杂时间相关性方面优于经典联邦基线，同时显著减少通信开销

Conclusion: 该工作展示了量子核在联邦设置中的潜力，为下一代物联网基础设施的可扩展、鲁棒和量子增强智能铺平道路
...
### [201] [FP8-Flow-MoE: A Casting-Free FP8 Recipe without Double Quantization Error](https://arxiv.org/abs/2511.02302)
*Fengjuan Wang,Zhiyi Su,Xingzhu Hu,Cheng Wang,Mou Sun*

Main category: cs.LG

TL;DR: FP8-Flow-MoE：一种针对大型MoE模型的FP8训练方法，通过量化一致性数据流和融合算子消除冗余转换，显著提升训练效率和内存使用


<details>
  <summary>Details</summary>
Motivation: 现有FP8训练方法依赖BF16主导的数据流，频繁的量化-反量化转换侵蚀了FP8的理论效率优势，而完全使用FP8又会引入双重量化误差问题

Method: 提出量化一致的FP8中心化数据流，包含缩放感知转置和融合FP8算子，将显式转换操作从12次减少到2次

Result: 在671B参数MoE模型上测试，相比BF16和朴素FP8基线，吞吐量提升21%，每GPU内存使用降低16.5GB，同时保持稳定收敛
...
### [202] [The Sequential Edge: Inverse-Entropy Voting Beats Parallel Self-Consistency at Matched Compute](https://arxiv.org/abs/2511.02309)
*Aman Sharma,Paras Chopra*

Main category: cs.LG

TL;DR: 本文研究发现，在同等计算资源下，顺序推理链比并行自一致性方法在语言模型推理任务中表现更优，并提出逆熵加权投票方法进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 重新审视语言模型推理中的测试时扩展策略，探讨在同等token预算和计算资源下，是运行多个独立并行链还是运行较少但顺序迭代优化的链更有效。

Method: 在5个开源模型和3个推理基准上进行全面评估，比较并行自一致性与顺序扩展方法，并引入逆熵加权投票这一无需训练的方法来优化顺序扩展。

Result: 顺序扩展在95.6%的配置中优于并行自一致性，准确率提升高达46.7%。逆熵加权投票进一步提高了顺序扩展的成功率，使其成为最优的测试时扩展策略。

Conclusion: 研究结果从根本上挑战了自Wang等人自一致性解码以来主导测试时扩展的并行推理范式，将顺序优化定位为现代LLM推理的稳健默认方法，需要在推理时优化方法上进行范式转变。
...
### [203] [Large-scale automatic carbon ion treatment planning for head and neck cancers via parallel multi-agent reinforcement learning](https://arxiv.org/abs/2511.02314)
...
TL;DR: 提出了一种可扩展的多智能体强化学习框架，用于并行调优头颈癌碳离子治疗中的45个治疗计划参数，生成的计划质量与专家手动计划相当或更好，显著改善了危及器官的保护。


<details>
  <summary>Details</summary>
Motivation: 头颈癌治疗计划制定困难，碳离子治疗虽然剂量适形性好但速度慢，传统深度学习方法存在数据偏差和计划可行性问题，强化学习在大规模参数搜索空间中探索效率低。

Method: 使用集中训练分散执行的多智能体强化学习框架，结合Double DQN、Dueling DQN和循环编码，采用紧凑的历史DVH向量作为状态输入，线性动作到值变换映射，以及基于临床知识的奖励函数设计。

Result: 在头颈癌数据集上同时调优45个参数，生成的治疗计划与专家手动计划相当或更好（相对计划分数：RL 85.93±7.85% vs Manual 85.02±6.92%），对5个危及器官有显著改善。

Conclusion: 该框架能高效探索高维治疗计划参数空间，通过与治疗计划系统直接交互生成临床竞争力的碳离子治疗计划，显著改善了危及器官的保护效果。
...
TL;DR: RoME是一个领域鲁棒的专家混合框架，通过动态路由和分布鲁棒优化来解决MILP求解器在跨领域泛化方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的MILP求解方法主要在单一领域开发和评估，难以泛化到未见的问题分布，这限制了构建可扩展通用求解器的发展。

Method: RoME框架使用学习到的任务嵌入动态路由问题实例到专门专家，采用两级分布鲁棒优化策略：跨域优化缓解全局分布偏移，域内优化通过对任务嵌入引入扰动增强局部鲁棒性。

Result: 在三个领域训练的单一RoME模型在五个不同领域评估时平均提升67.7%，在MIPLIB上的零样本测试也显示出对挑战性实际实例的泛化能力。

Conclusion: 跨领域训练不仅增强了模型对未见领域的泛化能力，还通过鼓励模型捕获更通用的内在组合模式来提升每个单独领域的性能。
...
### [205] [Learning A Universal Crime Predictor with Knowledge-guided Hypernetworks](https://arxiv.org/abs/2511.02336)
*Fidan Karimova,Tong Chen,Yu Yang,Shazia Sadiq*

Main category: cs.LG

TL;DR: HYSTL是一个超网络增强的时空学习框架，能够在不假设不同城市犯罪类型相同的情况下，训练统一的犯罪预测器。
...
Method: 使用超网络动态生成预测函数参数，构建结构化犯罪知识图谱来弥合不同犯罪类型之间的语义差距，利用犯罪类型关联指导预测。

Result: 在两个犯罪类型不重叠的城市上进行实验，结果显示HYSTL优于现有最先进的基线方法。

Conclusion: HYSTL框架能够有效训练统一的犯罪预测器，通过超网络和知识图谱处理不同城市犯罪类型差异，提升预测性能。
...
### [206] [Reducing normalizing flow complexity for MCMC preconditioning](https://arxiv.org/abs/2511.02345)
*David Nabergoj,Erik Štrumbelj*

Main category: cs.LG

TL;DR: 提出了一种分解预调节架构，将线性预调节器与条件归一化流结合，降低复杂度并提升对目标几何的适应性


<details>
  <summary>Details</summary>
Motivation: 现有非线性预调节器（如归一化流）可能因过度参数化而降低采样效率和拟合质量，且无法自适应调整架构以适应目标分布

Method: 使用分解预调节架构：线性预调节器处理近似高斯分布的维度，条件归一化流处理复杂维度，基于预热样本估计分布特性
...
<details>
  <summary>Details</summary>
Motivation: 现有图神经网络在动态图中对分布偏移的泛化能力较差，而动态图生成过程中潜在非平稳环境的演化会影响OOD泛化性能。
...
TL;DR: LUMA-RAG是一个终身多模态代理架构，通过流式多级内存系统、跨模态对齐桥和稳定性感知检索遥测技术，解决了多模态RAG系统中的索引新鲜度和跨模态语义一致性问题。
...
Result: 实验显示实现了稳健的文本到图像检索（Recall@10 = 0.94）、在乘积量化卸载下的优雅性能下降，以及可证明稳定的音频到图像排名（Safe@1 = 1.0）。
...
<details>
  <summary>Details</summary>
Motivation: 心律失常的早期检测可以预防严重并发症，但传统手动诊断依赖视觉解读且具有主观性。现有深度学习模型在现实场景中泛化能力不足，特别是在处理小样本和噪声数据时。
...
Conclusion: 所提出的CNN-H-Infinity-LSTM架构在心律失常检测方面表现出色，通过结合控制理论概念显著提升了模型在现实医疗场景中的鲁棒性和泛化性能。
...
TL;DR: 提出了一种基于高斯过程的完全去中心化覆盖控制算法，通过结合期望位置成本和方差探索项来平衡探索与利用。


<details>
  <summary>Details</summary>
Motivation: 解决未知空间环境中的覆盖控制问题，实现完全去中心化的算法，仅依赖局部观测和邻居通信。

Method: 使用高斯过程建模环境，结合GP-UCB获取函数设计局部成本函数，采用贪婪选择策略在线更新诱导点。

Result: 在仿真中验证了算法的有效性。

Conclusion: 提出的去中心化算法能够有效处理未知环境中的覆盖控制问题，具有良好的可扩展性。
...
TL;DR: 本文提出了机器遗忘问题的通用约束优化框架，通过可行更新概念和掩码技术选择性地更新模型参数，可在保持模型效用的同时实现遗忘，并能作为附加组件集成到任何一阶近似遗忘方法中。


<details>
  <summary>Details</summary>
Motivation: 解决机器遗忘问题，在保持模型原有性能的同时有效移除特定数据的影响，统一现有的一阶近似遗忘方法。

Method: 将机器遗忘建模为约束优化问题，引入可行更新概念，基于掩码技术选择性地更新模型参数，并考虑梯度估计噪声以提供统计保证。

Result: 在计算机视觉分类器上的实验验证了该方法的有效性。

Conclusion: 提出的可行更新方法能够在不降低模型效用的前提下实现遗忘，可作为通用组件增强现有遗忘方法。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在医学影像中的应用日益增多，但许多报告的改进缺乏统计稳健性。现有分析未考虑模型训练中的不确定性，即不同随机初始化可能导致验证分数相似但实际性能不同的模型。

Method: 扩展现有的统计框架，将underspecification作为额外的方差分量纳入模型，通过模拟实验分析随机种子变化对性能优越性声明的影响。

Result: 模拟实验表明，即使很小的种子变异性（约1%）也会显著增加支持优越性声明所需的证据强度。

Conclusion: 在验证医学影像系统时，必须明确建模训练方差，以确保统计结论的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的欧几里得空间知识图谱嵌入模型存在建模复杂关系的局限性，可能导致训练效率低下。球形几何约束能够提供更强的正则化效果。

Method: 使用可学习的非线性球化层将实体映射到超球面上，将关系解释为平移-投影的混合变换。

Result: 在FB15k-237、CoDEx-S和CoDEx-M三个基准数据集上的实验表明，SKGE显著优于TransE，特别是在大规模数据集上表现突出。

Conclusion: 流形选择不是实现细节而是基本设计原则，球形几何先验为设计下一代强大稳定的KGE模型提供了基础。
...
TL;DR: NOWS是一种混合策略，利用学习到的解算子为Krylov方法提供高质量初始猜测，从而加速传统迭代求解器，可减少高达90%的计算时间。


<details>
  <summary>Details</summary>
Motivation: 高保真PDE模拟在计算上成本高昂，而纯数据驱动的代理模型在训练分布外不可靠，需要结合传统求解器的严谨性。

Method: 使用神经网络算子为共轭梯度和GMRES等Krylov方法生成高质量初始猜测，保持现有离散化和求解器基础设施不变。

Result: 在基准测试中，学习到的初始化持续减少迭代次数和端到端运行时间，计算时间最多减少90%，同时保持底层数值算法的稳定性和收敛保证。

Conclusion: NOWS通过将神经算子的快速推理与传统求解器的严谨性相结合，为加速高保真PDE模拟提供了实用且可信赖的方法。
...
TL;DR: BRAINS是一个用于阿尔茨海默病早期检测的双模块系统，结合了大型语言模型的推理能力和病例检索技术，在资源有限地区提供可扩展、可解释的筛查工具。


<details>
  <summary>Details</summary>
Motivation: 随着阿尔茨海默病全球负担增加，在医疗资源有限地区迫切需要早期准确检测方法。

Method: 采用双模块架构：认知诊断模块使用在认知和神经影像数据上微调的LLM进行结构化评估；病例检索模块编码患者档案并检索相似病例，通过病例融合层增强上下文理解。
...
TL;DR: 提出了一个统一的信息几何框架，将学习中的理解形式化为信息性与几何简洁性之间的权衡，通过变分几何信息瓶颈方法实现几何感知的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有表示学习方法缺乏对几何结构的显式建模，无法在信息压缩和几何简洁性之间取得平衡，限制了样本效率和可解释性。

Method: 提出变分几何信息瓶颈（V-GIB），结合互信息压缩和曲率正则化，使用Hutchinson迹、Jacobian范数和局部PCA等可处理几何代理。

Result: 在合成流形、少样本场景和真实数据集上的实验显示，方法能建立稳健的信息几何帕累托前沿，在数据稀缺时保持预测能力，验证了效率-曲率定律。

Conclusion: V-GIB为获得几何一致、数据高效且与人类可理解结构对齐的表示提供了原则性且可测量的途径。
...
TL;DR: 提出了一种基于深度强化学习的端到端方法DRLHQ，用于解决容量限制选址路径问题(CLRP)及其变体开放CLRP(OCLRP)，通过异构查询注意力机制处理选址和路径决策的相互依赖关系。
...
Method: 采用编码器-解码器结构，将CLRPs重新表述为马尔可夫决策过程，并引入异构查询注意力机制来动态适应不同决策阶段。

Result: 在合成和基准数据集上的实验结果表明，该方法在解决CLRP和OCLRP时相比传统方法和基于DRL的基线方法具有更优的解质量和更好的泛化性能。

Conclusion: 这是首个针对CLRPs的端到端学习方法，提出的异构查询注意力机制能有效处理选址和路径决策的相互依赖关系，为其他基于DRL的方法提供了通用建模框架。
...
TL;DR: 这篇论文综述了因果图神经网络在医疗AI中的应用，通过结合图表示和因果推理来解决分布偏移、歧视和不可解释性问题，建立患者特异性因果数字孪生，但面临计算复杂度高、验证困难和因果清洗风险等挑战。
...
Method: 结合结构因果模型、解缠因果表示学习、干预预测和反事实推理等技术，构建因果图神经网络来学习不变机制而非虚假相关性。

Result: 在精神病诊断、癌症亚型分析、生理监测和药物推荐等应用中展示了临床价值，为患者特异性因果数字孪生奠定了基础。

Conclusion: 提出了区分因果启发架构与因果验证发现的分层框架，确定了从纯关联性转向因果性主张的关键研究重点，但计算需求、验证挑战和因果清洗风险仍是主要障碍。
...
TL;DR: 研究具有非线性效用函数的大学招生匹配问题，提出基于罗尔斯公平的解决方案，当经典稳定匹配不存在时最大化最差大学的效用


<details>
  <summary>Details</summary>
Motivation: 传统大学招生匹配模型假设线性效用函数，但现实中大学评估学生群体时会考虑多样性等非线性因素，这导致经典稳定匹配可能不存在

Method: 提出基于罗尔斯公平的替代解决方案概念，设计确定性和随机性算法，通过迭代改进最差大学的效用来实现公平分配

Result: 当经典稳定匹配不存在时，提出的算法能够提供公平的分配方案，最大化最差大学的效用

Conclusion: 在非线性效用函数的匹配问题中，基于罗尔斯公平的解决方案比传统稳定匹配更实用，为无法保证稳定性的场景提供了可行的分配方法
...
TL;DR: 该论文研究了在单变量随机干预和ε-干预忠实性假设下，因果发现中假阴性率(FNR)的理论保证。对于稀疏Erdős-Rényi有向无环图，FNR以O(log d/√d)的速率集中在其均值附近；对于广义Barabási-Albert图，当度指数γ>3时，偏差宽度消失。这表明现实的无标度拓扑结构固有地正则化因果发现，减少方向误差的变异性。


<details>
  <summary>Details</summary>
Motivation: 研究高维度和网络异质性对因果结构恢复准确性的影响，挑战传统直觉，为因果发现提供维度自适应、忠实性鲁棒的保证。

Method: 采用理论分析框架，针对稀疏Erdős-Rényi有向无环图和广义Barabási-Albert图，在单变量随机干预和ε-干预忠实性假设下，分析假阴性率的集中性质。

Result: 对于稀疏Erdős-Rényi图，FNR以O(log d/√d)速率集中；对于广义Barabási-Albert图，当γ>3时偏差宽度消失。模拟结果验证了理论预测，显示FNR在实践中确实集中并随维度增长而消失。

Conclusion: 现实的无标度拓扑结构固有地正则化因果发现，减少方向误差变异性，挑战了高维度和网络异质性必然阻碍准确发现的传统直觉，为因果结构恢复提供了首个维度自适应、忠实性鲁棒的保证。
...
TL;DR: 该论文提出了一种新的邻域约束方法来解决离线强化学习中的外推误差问题，通过限制贝尔曼目标中的动作选择到数据集动作的邻域并集，实现了比现有方法更好的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习面临分布外动作引起的外推误差问题，现有的密度约束、支持约束和样本约束方法各有局限性：密度和样本约束过于保守，支持约束难以准确建模行为策略。

Method: 提出邻域约束方法，将贝尔曼目标中的动作选择限制在数据集动作的邻域并集内，使用数据质量作为适应标准设计自适应邻域约束，并基于高效的双层优化框架开发ANQ算法。

Result: ANQ在标准离线强化学习基准测试中达到了最先进的性能，在噪声数据或有限数据场景下表现出强大的鲁棒性。

Conclusion: 邻域约束方法有效解决了现有离线强化学习方法的局限性，在理论上能够约束外推误差和分布偏移，在实践中实现了高性能和强鲁棒性。
...
TL;DR: 本文提出了一种新方法，允许用户在贝叶斯优化过程中通过指定先验分布来反复干预和引导超参数优化过程，同时引入误导性先验检测机制来防止有害的用户输入。


<details>
  <summary>Details</summary>
Motivation: 当前超参数优化方法缺乏用户控制和在线引导能力，限制了其在机器学习专家中的接受度。需要一种能够允许用户在优化过程中实时输入专家知识和偏好的方法。

Method: 通过泛化现有的πBO方法，引入重复干预机制，让用户能够在HPO过程运行时以先验分布的形式指定专家知识和偏好，同时加入误导性先验检测方案。

Result: 实验表明该方法能有效整合多个先验，利用信息丰富的先验，同时可靠地拒绝或克服误导性先验，保持了与未受干扰的贝叶斯优化的竞争力。

Conclusion: 该方法成功实现了用户对贝叶斯优化的在线引导，在保持理论保证的同时提高了HPO的用户控制能力和实用性。
...
TL;DR: 提出了DClamp-PPO算法，通过在对优势函数错误方向区域施加更陡峭的损失斜率来改进PPO算法，在各种MuJoCo环境中表现优于PPO及其变体。


<details>
  <summary>Details</summary>
Motivation: PPO算法在优化过程中重要性比率经常移动到"错误"方向，这是阻碍PPO改进的关键因素，但一直被忽视。

Method: 提出方向性钳位PPO算法(DClamp-PPO)，在严格"错误"方向区域（优势为正时重要性比率低于1-β，优势为负时重要性比率高于1+β）施加更陡峭的损失斜率进行惩罚。

Result: DClamp-PPO在各种MuJoCo环境中使用不同随机种子的情况下，始终优于PPO及其变体。

Conclusion: 该方法在理论和实证上都表明能更好地避免"错误"方向更新，同时保持重要性比率更接近1。
...
TL;DR: Omega^2是一个基于大语言模型的信用评分框架，结合结构化财务数据和机器学习，在7800个企业信用评级数据集上实现AUC超过0.93的预测性能。


<details>
  <summary>Details</summary>
Motivation: 提高企业信用评分的预测可靠性和可解释性，结合定量学习和基于语言的推理。

Method: 整合CatBoost、LightGBM和XGBoost模型，通过贝叶斯搜索优化，在时间验证下确保前瞻性和可重复结果。

Result: 在穆迪、标普、惠誉和Egan-Jones的多机构数据集上，平均测试AUC超过0.93，显示良好的泛化能力和时间一致性。

Conclusion: 将基于语言的推理与定量学习相结合，为可靠的企业信用风险评估创建了透明且机构级的解决方案。
...
TL;DR: 提出了一种自动迁移神经网络代码的方法，使用中间模型作为抽象层，成功在PyTorch和TensorFlow之间迁移神经网络代码。
...
Result: 在五个神经网络上的实验评估表明，该方法成功迁移了代码，并生成了功能等效于原始网络的新实现。

Conclusion: 该方法能够有效解决神经网络代码在不同深度学习框架间的迁移问题，提高了代码现代化的效率。
...
TL;DR: NAIGNs通过结合重构损失和非对抗性的隐式最大似然估计目标，改进了IGNs，解决了模式坍塌、模式丢失和训练不稳定问题，同时能更好地恢复损坏数据和生成新样本。


<details>
  <summary>Details</summary>
Motivation: IGNs存在模式坍塌、模式丢失和训练不稳定问题，这些问题源于其目标函数中的对抗性成分，导致模型只能部分覆盖数据流形。

Method: 提出NAIGNs，将重构损失与非对抗性的隐式最大似然估计(IMLE)生成目标相结合，替代IGNs中的对抗性成分。

Result: NAIGNs在恢复损坏数据和生成与数据分布匹配的新样本方面优于IGNs，同时隐式学习到数据流形的距离场和基于能量的模型。

Conclusion: NAIGNs通过非对抗性方法有效解决了IGNs的问题，提高了生成质量和训练稳定性，同时具备额外的学习能力。
...
TL;DR: 该论文研究可计算PAC学习，发现传统统计学习基本定理在可计算框架下不再成立，通过引入有效VC维度和递归可枚举可表示类来重建可计算学习理论。


<details>
  <summary>Details</summary>
Motivation: 传统PAC学习理论中的基本定理在要求学习器为可计算函数时不再成立，需要建立可计算框架下的学习理论。

Method: 引入有效VC维度和递归可枚举可表示类，研究它们与传统VC维度的关系，探讨各种CPAC学习概念。

Result: 发现有效VC维度可以取任意高于传统VC维度的值，但强CPAC学习下两者一致；CPAC可学习性可通过RER类包含来刻画；满足唯一识别性质的CPAC可学习类必为RER类。

Conclusion: 在可计算PAC学习框架下，建立了有效VC维度与RER类的新理论，为可计算学习提供了系统分析工具。
...
TL;DR: GasRL是一个结合天然气市场模拟和深度强化学习的仿真器，用于分析最优库存管理对均衡价格及供需动态的影响。研究发现SAC算法表现最佳，能实现盈利性、市场清算和价格稳定等多重目标，且产生的价格动态特征与真实市场高度匹配。


<details>
  <summary>Details</summary>
Motivation: 研究旨在分析天然气存储运营商的最优库存管理策略如何影响市场均衡价格和供需动态，并评估欧盟强制最低存储阈值政策的市场效果。

Method: 开发GasRL仿真器，结合校准的天然气市场模型和基于深度强化学习的存储运营商策略模型，测试多种RL算法（特别是SAC算法）的性能。

Result: SAC算法在GasRL环境中表现最优，成功实现存储运营商的多重目标，产生的均衡价格动态（波动性和季节性）与真实价格高度一致，且无需显式价格数据校准。欧盟最低存储阈值政策能增强市场对意外供应冲击的韧性。

Conclusion: GasRL仿真器有效模拟天然气市场动态，SAC算法能产生与真实市场相符的最优存储策略，欧盟最低存储阈值政策有助于提高市场抗风险能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决transformer模型在推理时的二次方复杂度问题，提升推理吞吐量和可扩展性，适用于智能体任务、长上下文推理等场景。

Method: 通过增量蒸馏从预训练transformer模型逐步替换注意力层为线性Mamba块，构建混合SSM-Transformer架构。
...
TL;DR: 提出了Byrd-NAFL算法，将Nesterov动量与拜占庭弹性聚合规则结合，在联邦学习中同时实现通信效率和对抗拜占庭攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在存在拜占庭对手的联邦学习环境中，需要同时提高通信效率和对抗恶意行为的鲁棒性。

Method: Byrd-NAFL算法将Nesterov动量整合到联邦学习过程中，并结合拜占庭弹性聚合规则来抵御梯度污染。

Result: 在非凸平滑损失函数下建立了有限时间收敛保证，实验验证了该算法在收敛速度、准确性和对多种拜占庭攻击策略的弹性方面优于现有基准。

Conclusion: Byrd-NAFL算法能够有效实现快速且安全的收敛，在拜占庭攻击环境下表现出优越性能。
...
### [231] [In Situ Training of Implicit Neural Compressors for Scientific Simulations via Sketch-Based Regularization](https://arxiv.org/abs/2511.02659)
*Cooper Simpson,Stephen Becker,Alireza Doostan*

Main category: cs.LG

TL;DR: 提出了一种新颖的在线训练协议，使用有限内存缓冲区存储完整和草图数据样本来防止灾难性遗忘，特别针对基于隐式神经表示的神经压缩应用。


<details>
  <summary>Details</summary>
Motivation: 解决在线学习中灾难性遗忘问题，通过草图数据作为正则化器来维持模型性能，特别针对复杂模拟数据的神经压缩任务。

Method: 使用包含完整数据和草图数据的有限内存缓冲区进行在线训练，基于Johnson-Lindenstrauss理论将草图作为正则化器，应用于隐式神经表示的超网络压缩。

Result: 在多种复杂模拟数据上展示了强大的重建性能和高压缩率，草图方法使在线方案性能接近等效离线方法。

Conclusion: 草图技术成功实现了在线训练方案，在保持高压缩率的同时避免了灾难性遗忘，性能接近离线方法。
...
### [232] [Scalable Evaluation and Neural Models for Compositional Generalization](https://arxiv.org/abs/2511.02667)
*Giacomo Camposampiero,Pietro Barbiero,Michael Hersche,Roger Wattenhofer,Abbas Rahimi*

Main category: cs.LG

TL;DR: 该论文提出了一个严格的组合泛化评估框架，训练了5000多个模型来评估监督视觉骨干网络的组合泛化能力，并引入了属性不变网络，在组合泛化方面建立了新的帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 组合泛化是现代机器学习中的一个关键开放挑战，但缺乏标准化评估协议，现有基准测试往往偏重效率而非严谨性，同时通用视觉架构缺乏必要的归纳偏置。

Method: 1) 引入严格的评估框架，统一并扩展先前方法，将计算需求从组合级降低到常数级；2) 在监督视觉骨干网络上进行广泛评估；3) 提出属性不变网络模型。

Result: 属性不变网络在组合泛化方面实现了23.43%的准确率提升，与完全解耦的对应模型相比，参数开销从600%减少到16%。

Conclusion: 该工作为组合泛化提供了严格的评估基准，并提出了高效的属性不变网络，在组合泛化性能与模型效率之间建立了新的平衡。
...
### [233] [Curriculum Design for Trajectory-Constrained Agent: Compressing Chain-of-Thought Tokens in LLMs](https://arxiv.org/abs/2511.02690)
*Georgios Tzannetos,Parameswaran Kamalaruban,Adish Singla*

Main category: cs.LG

TL;DR: 提出一种课程学习策略，通过在训练过程中逐步收紧约束条件，帮助智能体渐进式掌握部署要求，从而提高在复杂约束环境下的训练效率和性能。


<details>
  <summary>Details</summary>
Motivation: 在部署环境中，智能体需要面对严格的资源预算或安全约束，这些约束往往使任务变得复杂，给训练带来挑战。

Method: 采用课程学习策略，从简化的约束条件开始训练，逐步引入完整的部署条件，基于自定步调学习技术实现平滑过渡。

Result: 理论分析和实证验证表明，该方法在强化学习和大型语言模型任务中都能加速训练，在LLM应用中还能压缩思维链输出，实现推理速度提升。
...
### [234] [Does Interpretability of Knowledge Tracing Models Support Teacher Decision Making?](https://arxiv.org/abs/2511.02718)
*Adia Khalid,Alina Deriyeva,Benjamin Paassen*

Main category: cs.LG

TL;DR: 该研究探讨了知识追踪(KT)模型的可解释性是否真正帮助教师做出教学决策。研究发现，虽然可解释模型在模拟中能更快达到掌握水平，但在实际教师决策中，不同模型所需的练习任务数量差异不大。


<details>
  <summary>Details</summary>
Motivation: 知识追踪模型对教学决策至关重要，但以往研究未验证其可解释性是否真的能帮助教师做出更好的教学决策。

Method: 首先进行模拟研究比较可解释与不可解释KT模型的决策效果；然后让12名教师基于不同KT模型提供的信息进行教学决策。

Result: 模拟显示可解释模型能更快达到掌握水平；但教师实验中，不同模型所需的练习任务数量差异不大，尽管教师对可解释模型的可用性和可信度评价更高。
...
### [235] [Calibration improves detection of mislabeled examples](https://arxiv.org/abs/2511.02738)
...
<details>
  <summary>Details</summary>
Motivation: 误标数据是影响机器学习系统性能的普遍问题，需要有效检测方法。基础模型的特性对误标检测至关重要，而校准可能改善其性能。

Method: 训练基础机器学习模型，然后使用校准方法对模型进行校准，通过探测每个实例获得标签可信度分数来检测误标实例。

Result: 实证结果表明，使用校准方法能提高误标实例检测的准确性和鲁棒性。

Conclusion: 模型校准为工业应用中的误标数据问题提供了实用有效的解决方案。
...
### [236] [ConMeZO: Adaptive Descent-Direction Sampling for Gradient-Free Finetuning of Large Language Models](https://arxiv.org/abs/2511.02757)
...
<details>
  <summary>Details</summary>
Motivation: 零阶优化（MeZO）在微调大语言模型时具有内存优势，但由于在高维参数空间中搜索下降方向，收敛速度缓慢。

Method: ConMeZO通过将采样限制在以动量估计为中心的锥形区域内，而不是均匀随机采样，从而将搜索集中在真实梯度更可能存在的方向上。
...
### [237] [From Solo to Symphony: Orchestrating Multi-Agent Collaboration with Single-Agent Demos](https://arxiv.org/abs/2511.02762)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习从头训练团队效率低下，而现有方法仍依赖昂贵的多智能体数据。单智能体经验在许多重要场景中更容易获取，但尚未被充分利用。

Method: SoCo框架：1) 从单智能体演示预训练共享策略；2) 通过包含MoE门控选择器和动作编辑器的策略融合机制，在多智能体训练期间适应协作。
...
### [238] [VecComp: Vector Computing via MIMO Digital Over-the-Air Computation](https://arxiv.org/abs/2511.02765)
*Saeed Razavikia,José Mairton Barros Da Silva Junior,Carlo Fischione*

Main category: cs.LG

TL;DR: VecComp扩展了ChannelComp框架，通过集成多天线技术实现向量函数计算，解决了原框架仅支持标量计算和对信道衰落的敏感性问题，同时保持计算复杂度随向量维度线性增长的高效性。


<details>
  <summary>Details</summary>
Motivation: 传统ChannelComp框架仅支持标量函数计算，而许多数据应用需要向量计算，且对信道衰落敏感。需要扩展框架以支持向量计算并提高对信道损伤的鲁棒性。

Method: 将ChannelComp与多天线技术集成，提出VecComp框架。通过多天线系统实现向量函数计算，确保计算复杂度仅随向量维度线性增长。

Result: 建立了VecComp在衰落信道条件下的均方误差非渐近上界，数值实验证明VecComp在噪声和衰落多址信道上能有效改进向量函数计算和衰落补偿。

Conclusion: VecComp框架成功扩展了ChannelComp，实现了高效的向量函数计算，对信道损伤具有鲁棒性，适用于高维数据应用。
...
### [239] [STAR-VAE: Latent Variable Transformers for Scalable and Controllable Molecular Generation](https://arxiv.org/abs/2511.02769)
...
TL;DR: STAR-VAE是一个基于Transformer的变分自编码器，使用SELFIES表示法生成药物分子，支持属性条件生成，并通过LoRA实现高效微调。
...
Method: 使用Transformer编码器和自回归Transformer解码器构建变分自编码器，在PubChem的7900万个类药物分子上训练，采用SELFIES保证语法有效性，通过属性预测器实现条件生成。

Result: 在GuacaMol和MOSES基准测试中达到或超过基线水平，潜在空间分析显示平滑的语义结构表示，在Tartarus基准测试中条件模型将对接分数分布向更强预测结合方向移动。

Conclusion: 现代化、规模适当的VAE与原则性条件生成和参数高效微调结合，在分子生成方面仍具有竞争力。
...
### [240] [Adam Reduces a Unique Form of Sharpness: Theoretical Insights Near the Minimizer Manifold](https://arxiv.org/abs/2511.02773)
*Xinghan Li,Haodong Wen,Kaifeng Lyu*

Main category: cs.LG

TL;DR: Adam优化器通过自适应更新隐式地最小化一种独特的锐度度量，产生与SGD不同的解。在训练过参数化模型时，Adam最小化Hessian矩阵对角线元素的平方根迹，而非SGD的Hessian迹，从而在稀疏线性回归中实现更好的稀疏性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管Adam优化器在实践中很流行，但大多数理论分析都研究SGD作为Adam的代理，对Adam找到的解与SGD的差异知之甚少。

Method: 通过随机微分方程的连续时间近似，严格刻画Adam在损失较小时在极小值流形上徘徊并自适应地最小化锐度度量的行为。

Result: 在带标签噪声的过参数化模型训练中，SGD最小化Hessian矩阵的迹tr(H)，而Adam最小化tr(diag(H)^(1/2))。在稀疏线性回归中，Adam比SGD实现了更好的稀疏性和泛化能力。

Conclusion: 该分析框架可扩展到包括RMSProp、Adam-mini、Adalayer和Shampoo在内的广泛自适应梯度方法，为这些自适应优化器如何减少锐度提供了统一视角，有助于未来优化器设计。
...
TL;DR: 提出一种基于量子计算启发的QUBO优化方法，通过选择最相关的客户端更新子集来显著降低联邦学习中的隐私泄露风险，在保持模型准确性的同时大幅减少客户端暴露。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护原始数据隐私，但随着迭代次数增加，客户端更新被包含在聚合模型中的风险累积增加，攻击者可能发起成员推理攻击、属性推理攻击和模型反转攻击。

Method: 使用量子计算启发的二次无约束二进制优化(QUBO)公式，在每轮训练中选择一小部分最相关的客户端更新，假设可信中央服务器和全局数据分布的验证集。

Result: 在MNIST数据集300个客户端20轮实验中，实现了95.2%的每轮隐私暴露减少和49%的累积隐私暴露减少，147个客户端的更新从未被使用，同时保持或优于全聚合的准确性。在CINIC-10数据集30个客户端实验中，实现了82%的每轮隐私改进和33%的累积隐私。

Conclusion: 该方法能有效降低联邦学习中的隐私风险，在减少客户端暴露的同时保持模型性能，且在不同规模和复杂度的模型中都表现出良好效果。
...
### [242] [Fast, Private, and Protected: Safeguarding Data Privacy and Defending Against Model Poisoning Attacks in Federated Learning](https://arxiv.org/abs/2511.02797)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在保护数据隐私的同时，难以防御恶意参与者对训练结果的攻击，需要一种既能保护隐私又能防御攻击的解决方案。

Method: FPP采用安全聚合保护数据隐私，通过参与者评估轮次、训练恢复机制和基于信誉的参与者选择来防御模型投毒攻击。

Result: 实验表明FPP在恶意参与者存在的情况下仍能快速收敛，收敛速度优于FedAvg、Power-of-Choice和修剪均值/中位数聚合等方法。
...
### [243] [TabTune: A Unified Library for Inference and Fine-Tuning Tabular Foundation Models](https://arxiv.org/abs/2511.02802)
...
Method: 提供对7个最先进模型的一致访问，支持多种适应策略（零样本推理、元学习、监督微调、参数高效微调），自动化模型感知预处理，管理架构异质性，并集成性能、校准和公平性评估模块。

Result: TabTune实现了表格基础模型适应策略的一致基准测试，支持可扩展性和可重复性。

Conclusion: TabTune是一个开源库，标准化了表格基础模型的完整工作流程，解决了当前采用障碍，促进了该领域的发展。
...
### [244] [Assessing win strength in MLB win prediction models](https://arxiv.org/abs/2511.02815)
*Morgan Allen,Paul Savala*

Main category: cs.LG

TL;DR: 该研究通过训练多种机器学习模型预测棒球比赛胜负概率，并分析这些概率与得分差的关系，最后评估了在跑分线投注中使用预测概率的决策效果。


<details>
  <summary>Details</summary>
Motivation: 扩展先前通过机器学习预测棒球比赛胜负的研究，使用统一数据集训练多种模型，并探索预测胜率与实际获胜强度之间的关系。

Method: 使用统一数据集训练多种常见的机器学习模型来预测比赛胜负概率，然后将预测概率与得分差（获胜强度指标）关联分析，最后评估在跑分线投注中使用这些概率的策略效果。

Result: 研究表明常见机器学习模型的预测胜率与实际获胜强度确实存在相关性，在跑分线投注中使用适当的投注策略可以获得正收益，但简单直接使用模型预测会导致显著亏损。

Conclusion: 机器学习模型能够有效预测棒球比赛胜负并反映获胜强度，但在实际投注应用中需要谨慎的策略设计，不能简单依赖模型预测结果。
...
### [245] [GeoCrossBench: Cross-Band Generalization for Remote Sensing](https://arxiv.org/abs/2511.02831)
...
<details>
  <summary>Details</summary>
Motivation: 随着遥感卫星数量和多样性的增加，大多数标注数据来自旧卫星，重新训练模型成本高昂，因此模型对新卫星的泛化能力变得至关重要。

Method: 扩展GeoBench基准测试，引入新的评估协议测试模型性能；开发自监督的ChannelViT扩展ChiViT；比较多种基础模型在跨卫星场景下的表现。

Result: 现有最佳遥感基础模型在分布内性能上不如通用模型DINOv3；当泛化到无波段重叠的新卫星时，所有模型性能下降2-4倍，ChiViT显著优于其他模型；当测试时提供额外波段时，所有模型性能平均下降5-25%。

Conclusion: 基准测试远未饱和，仅微调最后一层线性层就能获得相对一致的跨卫星性能，表明需要开发更具未来适应性的遥感模型。
