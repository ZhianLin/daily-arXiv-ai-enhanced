...
TL;DR: 提出了一种结合逐样本逐因子信任权重与残差对比损失的SSL方法，通过加法式残差（而非乘法门控）提升有损航拍图像的表征鲁棒性，在多个基准上达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有SSL方法在航拍图像上强制增强视图对齐，但当遇到雾、运动模糊、雨、遮挡等严重退化时，会引入潜在空间的虚假结构。需要一种对这类损坏更鲁棒的SSL策略。

Method: 1) 引入逐样本、逐因子的信任权重（trust weight）到对齐目标中；2) 将信任权重与基础对比损失以加法残差形式结合，而非乘法门控；3) 对信任权重使用stop-gradient；4) 提出基于Dempster-Shafer融合的变体，提供冲突与无知的可解释信号。

Result: 在EuroSAT/AID/NWPU-RESISC45上达到90.20%最高线性探测精度（SimCLR 88.46%, VICReg 89.82%）；在严重信息擦除损坏（EuroSAT haze s=5）上比SimCLR提升19.9点；在BDD100K零样本跨域压力测试中AUROC一致提升1-3点。

Conclusion: 加法残差公式是性能提升的主要来源，乘法门控反而损害骨干网络。该工作为不确定性感知的SSL提供了具体的设计原则。
...
TL;DR: 本文提出了PlantInquiryVQA基准，用于评估多模态大模型在多步、意图驱动的植物病害视觉推理能力，发现现有模型虽能描述症状，但在临床推理和准确诊断上表现不足，而结构化提问引导能显著提升诊断正确率并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型仅在单轮问答上评估，缺乏对多步、意图驱动的视觉推理能力的测试，而植物病理学诊断本质上是一个基于视觉线索的迭代追问过程，因此需要新的基准来填补这一评估空白。

Method: 提出了Chain of Inquiry框架，将诊断轨迹建模为基于视觉线索和明确认知意图的有序问答序列；构建了包含24,950张专家标注植物图像和138,068个问答对的数据集，并附有视觉定位、严重程度标签和领域推理模板。

Result: 顶级多模态大模型在描述视觉症状方面表现尚可，但在安全临床推理和准确诊断方面存在困难；结构化提问引导显著提升了诊断正确性、减少了幻觉并提高了推理效率。

Conclusion: PlantInquiryVQA可作为基础基准，推动训练像植物学家一样进行推理的诊断智能体，而非静态分类器。
...
TL;DR: 本文提出了一种文本到线性图像生成的方法，通过将线性图像表示为多个曝光包围序列，利用DiT流匹配架构生成场景参考的线性图像，解决了现有生成模型仅合成显示参考图像的限制，支持更丰富的后期处理。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要合成显示参考图像，经过ISP管线后丢失了原始辐射信息，动态范围被压缩且受主观风格影响，限制了后期编辑空间。而线性图像（RAW转换后）保留完整动态范围和真实辐照度，更有利于专业后期处理。

Method: 将线性图像表示为一系列曝光包围（exposure brackets），每个包围捕捉动态范围的特定部分；基于DiT（Diffusion Transformer）的流匹配（flow-matching）架构，实现文本条件下的曝光包围生成；并利用ControlNet支持结构条件生成。

Result: 成功实现了文本到线性图像的生成，能够保留完整动态范围（同时保留极端高光和阴影细节）；并展示了文本引导线性图像编辑和结构条件生成等下游应用。

Conclusion: 线性图像生成是扩展生成模型能力的重要方向，本文的方法为专业后期处理提供了更灵活、保留更多信息的生成方式，弥补了现有生成模型仅输出显示参考图像的限制。
...
TL;DR: 本文提出了一种双路径网络，通过实体级自适应路由和互动作一致性损失，分别处理空间和时间优先的微动作特征，在MA-52和iMiGUE数据集上取得了竞争性或最优性能。


<details>
  <summary>Details</summary>
Motivation: 微动作（如挠头、敲手指等）在社交沟通中至关重要，但其时空特性多样——有的偏重空间形态，有的偏重时间动态。现有方法采用单一的时空分解方式，无法适应这种多样性，因此亟需一种能灵活处理不同时空偏好性的架构。

Method: 提出双路径网络：1）ST路径：先捕获空间配置，再建模时间动态；2）TS路径：先建模时间动态，再处理空间信息。引入实体级自适应路由，让每个身体部位自主选择最优处理路径，并结合互动作一致性损失（MAC Loss）保证双路径间的特征一致性。

Result: 在MA-52数据集上取得竞争性表现，在iMiGUE数据集上达到当前最优（state-of-the-art）结果。

Conclusion: 研究表明，针对微动作固有的时空复杂度进行架构自适应设计，是推进细粒度视频理解的关键。
...
TL;DR: 本文提出一种无需额外训练的方法，通过将多光谱数据适配到RGB空间并注入领域知识与思维链推理，使通用大型多模态模型（LMMs）能够处理多光谱遥感影像，在零样本设定下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 通用LMMs通常仅针对RGB图像训练，无法直接处理多光谱数据；而专门训练多光谱多模态模型成本高昂且过于特化。作者希望在不重新训练模型的前提下，让通用LMMs也能有效利用多光谱信息。

Method: 提出一种训练无关的推理框架：①将非RGB多光谱输入适配到LMMs理解的RGB视觉空间；②通过指令注入领域专业知识（如波段含义）和Chain-of-Thought推理引导模型逐步分析。以Gemini 2.5模型作为测试平台。

Result: 在多个主流遥感基准上，零样本设定下取得显著的性能提升，表明通用LMMs结合适配方法可有效处理多光谱数据。

Conclusion: 该方法使得地理空间专业人员无需训练专用模型，即可利用强大通用模型处理多光谱遥感数据，同时保留了模型的丰富推理能力。
...
### [6] [UAU-Net: Uncertainty-aware Representation Learning and Evidential Classification for Facial Action Unit Detection](https://arxiv.org/abs/2604.21227)
*Yuze Li,Zhilei Liu*

Main category: cs.CV

TL;DR: 提出UAU-Net，同时在表征阶段（CV-AFE模块）和决策阶段（AB-ENN模块）显式建模不确定性，提升面部动作单元检测的鲁棒性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有AU检测方法将表征视为确定性的，忽略了视觉噪声、个体外观差异和AU间关系模糊带来的不确定性；同时点估计分类器在标签不平衡下易产生过度自信的预测。

Method: 在表征阶段引入基于CVAE的CV-AFE模块，学习概率化AU表征（均值和方差）；在决策阶段设计非对称Beta证据神经网络AB-ENN，用Beta分布建模预测不确定性，并设计非对称损失处理标签不平衡。

Result: 在BP4D和DISFA数据集上取得强AU检测性能，不确定性建模提升了鲁棒性和可靠性。

Conclusion: 在表征学习和证据预测两个阶段同时建模不确定性，能有效提升AU检测的鲁棒性与可靠性。
...
TL;DR: 针对文本到图像扩散模型的机器遗忘问题，本文提出投影梯度遗忘(PGU)作为后处理加固步骤，通过构建核心梯度空间(CGS)并将梯度更新投影到其正交补空间中，防止后续微调导致已擦除概念复活。该方法在风格概念上完全消除复活现象，在对象概念上显著延缓复活，耗时约6分钟，且与现有方法互补。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型的遗忘方法存在共同缺陷：即使在下游数据完全不相关的情况下，对模型进行微调后，已擦除的概念会重新出现。需要一种方法防止这种概念复活现象。

Method: 将分类领域的投影梯度遗忘(PGU)适应到扩散领域，作为现有遗忘方法的后处理加固步骤。通过构建保留概念激活的核心梯度空间(CGS)，将梯度更新投影到其正交补空间中，确保后续微调无法撤销已实现的擦除。保留概念选择应遵循视觉特征相似性而非语义分组。

Result: 在ESD、UCE、Receler等现有方法上应用PGU，风格概念的复活现象被完全消除，对象概念的复活被显著延缓。该方法运行约6分钟，而Meta-Unlearning需约2小时。PGU与Meta-Unlearning互补，哪种更好取决于概念编码方式。

Conclusion: PGU是一种有效的后处理加固方法，能够防止微调导致的遗忘概念复活，计算效率高，与现有方法互补。保留概念选择应基于视觉特征相似性而非语义分组。
...
### [8] [Discriminative-Generative Synergy for Occlusion Robust 3D Human Mesh Recovery](https://arxiv.org/abs/2604.21712)
*Yang Liu,Zhiyong Zhang*

Main category: cs.CV

TL;DR: 提出了一种受脑启发的协同框架，融合ViT的判别能力和扩散模型的生成能力，用于从单目RGB图像中恢复3D人体网格，在遮挡和复杂场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有回归方法在严重遮挡下效果不佳，扩散方法虽能生成遮挡区域但可能削弱罕见姿态的保真度，因此需要兼顾判别力与生成能力的协同框架。

Method: 提出脑启发协同框架：(1)ViT通路提取确定性视觉线索；(2)扩散模型通路合成连贯人体表示；(3)多样化一致特征学习模块对齐判别与生成特征；(4)跨注意力多层级融合机制实现双向交互。

Result: 在标准基准测试上取得了关键指标上的优越性能，并在复杂真实场景中展现出强鲁棒性。

Conclusion: 融合判别与生成能力的脑启发协同框架有效解决了遮挡下3D人体网格恢复的难题，在性能和鲁棒性上均优于现有方法。
...
TL;DR: StyleVAR 将风格迁移建模为条件离散序列预测问题，采用 VQ-VAE 对图像进行多尺度离散化，通过混合交叉注意力机制融合风格与内容信息，并用 GRPO 强化学习优化，在多个基准上超越 AdaIN 基线。


<details>
  <summary>Details</summary>
Motivation: 现有风格迁移方法难以同时保持内容结构完整性和风格纹理迁移效果，且缺乏对生成过程的细粒度控制。作者希望利用 VAR 框架的自回归特性，将风格迁移转化为可控的离散序列建模问题。

Method: 1) 使用 VQ-VAE 将图像分解为多尺度离散编码；2) 提出混合交叉注意力机制，使风格和内容特征作为查询（query）来强调目标序列的不同方面；3) 引入尺度相关的混合系数控制风格与内容在各阶段的相对影响；4) 两阶段训练：先在有监督的大规模三元组数据集上微调，再用 GRPO 基于 DreamSim 感知奖励进行强化学习微调，配合逐动作归一化加权。

Result: 在分布内、近分布和分布外三个基准上，StyleVAR 在风格损失、内容损失、LPIPS、SSIM、DreamSim 和 CLIP 相似度上均优于 AdaIN 基线；GRPO 阶段相比 SFT 进一步提升了感知指标。定性上能有效迁移纹理并保持语义结构，但在互联网图像上存在泛化差距，对人脸处理不佳。

Conclusion: StyleVAR 验证了将风格迁移作为自回归离散序列建模的可行性，混合交叉注意力机制和 GRPO 强化学习是有效的设计。未来需提升内容多样性并引入更强的结构先验以解决泛化问题。
...
### [10] [Building a Precise Video Language with Human-AI Oversight](https://arxiv.org/abs/2604.21718)
...
TL;DR: 本文提出CHAI框架，通过结构化视频标注规范和人类-AI协作纠错机制，实现精准视频字幕生成，并提升视频理解与生成模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频-语言模型缺乏对视频内容（主体、场景、运动、空间、摄像机动态）的精确结构化描述能力，且高质量视频字幕标注效率低下，专家标注成本高昂。作者旨在建立可扩展的监督框架以提升视频字幕的精确性和专业性。

Method: 1) 定义结构化视频标注规范，包含数百种视觉原语（由专业视频创作者参与设计）；2) 提出CHAI框架：模型先生成预字幕，训练有素的专家对预字幕进行批判和修订，形成后字幕；3) 利用预-后字幕之间的偏好数据，通过SFT、DPO和推理时缩放技术训练开源模型（Qwen3-VL），提升其字幕生成、奖励建模和批判生成能力。

Result: 1) 批判质量（精确率、召回率、建设性）直接决定下游性能；2) 在少量专家监督下，模型性能超越Gemini-3.1-Pro等闭源模型；3) 对专业视频重新标注后，能控制Wan等视频生成模型实现长达400词的精细提示，精确控制摄像机运动、角度、镜头、焦点、视点和构图等电影摄影要素。

Conclusion: 精确的视频标注规范和人类-AI协作监督是达到专业级视频理解与生成的关键。
...
TL;DR: 本文提出了一种专家引导的对比微调框架，用于在数据稀缺和类别不平衡条件下，从全切片图像中对儿童脑肿瘤进行弱监督分类，通过对比学习与多实例学习结合，有效提升了细粒度诊断的准确性。


<details>
  <summary>Details</summary>
Motivation: 儿童脑肿瘤的深度学习诊断面临数据稀缺、类别不平衡以及不同亚型间形态重叠严重等独特挑战。现有的病理基础模型在补丁级表征学习上虽有进展，但在有限数据下如何有效适应弱监督的儿童脑肿瘤分类问题尚未得到充分探索。

Method: 提出专家引导的对比微调框架，将对比学习集成到滑动级多实例学习（MIL）中，通过显式正则化滑动级表征的几何结构。设计了通用监督对比设置和专家引导变体，后者引入临床信息驱动的困难负样本，针对诊断上易混淆的亚型进行优化。

Result: 在低样本和类别不平衡的真实条件下进行实验，对比微调在细粒度诊断区分上取得可量化的改进。专家引导的困难负样本促进了更紧凑的类内表征和更好的类间分离。

Conclusion: 显式塑造滑动级表征对于数据稀缺的儿童病理学场景中稳健的细粒度分类至关重要，不同对比策略具有互补优势。
...
TL;DR: 提出一种仅需单次观测即可微调扩散先验的方法，通过将多个扩散先验组合成"专家乘积先验"并依据贝叶斯证据最大化确定指数权重，从而在黑洞成像等真实逆问题中实现更灵活、更可信的后验采样。


<details>
  <summary>Details</summary>
Motivation: 现有扩散先验通常基于有限训练集或纯仿真数据训练，继承了下游数据的误差与偏差；而微调扩散模型通常需要大量带有不同前向算子的观测数据，在观测集较小时容易过拟合。

Method: 将多个已有扩散先验组合成产品-of-experts（PoE）先验，通过最大化贝叶斯证据来学习每个先验的指数权重，从而仅从单一观测数据中调优先验。

Result: 在黑洞成像和文本条件图像去模糊等真实逆问题中验证，证据最大化往往倾向于选择超出单一数据集训练的先验，通过指数加权泛化先验，实现更灵活的后验采样。

Conclusion: 该方法通过指数加权组合多个扩散先验，仅需单次观测即可提升先验的泛化能力与后验图像分布的可靠性。
...
TL;DR: 提出Foveated Reasoner框架，通过类人中央凹视觉机制，在低分辨率全局视图引导下，仅对关键区域进行高分辨率采样，以低视觉token开销实现高精度视觉语言推理。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在高分辨率图像上性能提升显著，但视觉token数量激增导致计算开销巨大。人类通过中央凹视觉（foveation）解决这一矛盾：粗粒度视图引导"看向哪里"，选择性获取的高精度证据完善"思考什么"。

Method: 提出Foveated Reasoner，一种自回归视觉语言框架，将中央凹视觉与推理统一在同一解码轨迹中。从低分辨率视图开始，模型仅在需要时触发foveation，从选定区域检索高分辨率证据并注入回同一解码轨迹。训练采用两阶段流水线：冷启动监督引导foveation行为，随后强化学习联合改进证据获取与任务准确率，同时抑制"全盘查看"的平凡解。

Result: 该方法学习了有效的foveation策略，在严格的视觉token预算下，在多个视觉语言基准上实现了更强的准确率。

Conclusion: 通过模拟人类中央凹视觉机制，Foveated Reasoner在计算效率与任务性能之间取得了良好的平衡，为高分辨率视觉语言模型的实用化提供了新思路。
...
TL;DR: 本文提出一个利用大语言模型（LLM）和谷歌街景图像自动评估美国全国建筑状况的框架。通过微调Gemma 3 27B模型，在少量人工标注数据上实现了与人类平均意见得分的高度对齐。通过知识蒸馏将能力迁移至更小模型（Gemma 3 4B、EfficientNetV2-M、SwinV2-B），在保持接近性能的同时实现3x至30x的速度提升。研究还探索了LLM评估建筑环境属性的能力，并开发了可视化仪表板。


<details>
  <summary>Details</summary>
Motivation: 传统建筑状况评估依赖人工实地考察，成本高、效率低且难以大规模开展。现有自动化方法精度不足，亟需一种高精度、低成本、可扩展的大规模评估方案。

Method: 1. 基于少量人工标注数据集微调Gemma 3 27B LLM，使其与人类平均意见得分对齐；2. 采用知识蒸馏技术，将大模型能力迁移至Gemma 3 4B（3倍加速）、EfficientNetV2-M和SwinV2-B（30倍加速）；3. 通过人类-AI对齐研究评估LLM对多种建筑环境属性的判断能力；4. 构建可视化仪表板集成评估结果供房主分析。

Result: 微调的Gemma 3 27B在SRCC和PLCC指标上优于单个人类评分者；知识蒸馏后的小模型性能接近大模型，同时实现3x至30x的推理速度提升；LLM在评估多种建筑环境属性方面展现出良好能力。

Conclusion: 该框架提供了一种灵活高效的大规模建筑状况评估方案，在极少量人工标注下即可实现高精度，具有实际应用价值。
...
TL;DR: 本文系统研究了预训练数据的地理组成对地理空间基础模型下游性能的影响，发现欧洲预训练数据表现最优，且光谱多样性是唯一与性能强相关的因素。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注模型架构和输入模态对性能的影响，而预训练数据集的作用被忽视，尤其是地理组成对下游任务的影响尚未被系统研究。

Method: 构建全球及各大洲的预训练数据集，在全局和洲际下游任务上评估性能；分析10个预训练数据集在大洲、生物群落、土地覆盖和光谱值四个维度的多样性，并与性能进行相关性分析。

Result: 欧洲预训练数据在全局和本地下游任务上均表现最优；光谱多样性与性能强相关，其他多样性维度相关性弱。

Conclusion: 光谱多样性是构建高性能预训练数据集的关键维度，应被纳入考虑；开源了7个新预训练数据集和模型框架。
...
### [16] [Materialistic RIR: Material Conditioned Realistic RIR Generation](https://arxiv.org/abs/2604.21119)
*Mahnoor Fatima Saad,Sagnik Majumder,Kristen Grauman,Ziad Al-Halah*

Main category: cs.CV

TL;DR: 提出一种显式解耦空间与材料影响的房间脉冲响应（RIR）生成方法，通过空间模块和材料模块分别建模场景布局与材料配置对声学的影响，支持用户独立控制材料属性而不改变空间结构，在声学指标（RTE提升16%）和材料指标（提升70%）上显著优于现有方法。
...
Method: 提出双模块解耦架构：空间模块捕获场景布局对RIR的影响；材料模块根据用户指定的材料配置对空间RIR进行调制，实现空间与材料的显式解耦。

Result: 声学指标上RTE提升最高16%，材料指标提升最高70%；人类感知实验表明模型在真实感和材料敏感性上均优于最强基线。

Conclusion: 显式解耦空间与材料的设计有效提升了RIR生成的可控性和真实感，为虚拟现实、机器人、建筑设计等应用提供了更灵活的声学建模方案。
...
### [17] [HyperFM: An Efficient Hyperspectral Foundation Model with Spectral Grouping](https://arxiv.org/abs/2604.21127)
*Zahid Hassan Tushar,Sanjay Purushotham*

Main category: cs.CV

TL;DR: 本文介绍HyperFM，一种参数高效的高光谱基础模型，用于处理NASA PACE任务的高光谱数据。该模型通过组内和组间光谱注意力机制以及混合参数分解，在降低计算成本的同时更好地捕捉光谱空间关系，在四个大气云属性反演基准任务上表现优于现有模型。同时发布包含晴空和云场景的大规模高光谱数据集HyperFM250K。


<details>
  <summary>Details</summary>
Motivation: 现有高光谱数据规模大、复杂且难以标注；现有基础模型主要基于RGB图像训练，无法解释PACE的连续光谱信号；现有高光谱基础模型多在无云数据上训练，受限于单传感器数据集且参数多、计算成本高。

Method: HyperFM采用参数高效设计，引入组内和组间光谱注意力机制（intra-group and inter-group spectral attention），结合混合参数分解（hybrid parameter decomposition），在降低计算开销的同时增强对光谱空间关系的捕捉能力。

Result: 在四个大气云属性反演基准任务上，HyperFM一致性地优于现有高光谱基础模型和特定任务的最先进方法。

Conclusion: HyperFM为PACE任务的高光谱数据分析提供了一种高效、可扩展的解决方案，同时发布的HyperFM250K数据集将推动该领域的进一步发展。
...
### [18] [WFM: 3D Wavelet Flow Matching for Ultrafast Multi-Modal MRI Synthesis](https://arxiv.org/abs/2604.21146)
*Yalcin Tur,Mihajlo Stojkovic,Ulas Bagci*

Main category: cs.CV

TL;DR: 提出WFM（Wavelet Flow Matching），通过在小波空间中从有信息先验（条件模态的均值）直接学习流向目标分布的流，实现1-2步快速多模态MRI合成，速度提升250-1000倍，一个模型替代四个扩散模型。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在多模态MRI合成中质量优异，但计算成本极高（需要数百采样步和每个模态单独模型），限制了临床部署。作者观察到扩散从纯噪声开始，丢弃了已有MRI序列中的结构信息，造成效率低下。

Method: 提出Wavelet Flow Matching (WFM)：从小波空间中有信息的先验（条件模态的均值）直接学习流向目标分布的流，而非从纯噪声开始。使用单一82M参数模型配合类别条件控制，在小波空间中合成所有四种BraTS模态。

Result: 在BraTS 2024上：PSNR 26.8 dB，SSIM 0.94，比扩散基线低1-2 dB，但速度提升250-1000倍（0.16-0.64s vs 160s每体积）。一个82M参数模型替代四个共326M参数的扩散模型。

Conclusion: WFM实现了临床可用的实时MRI合成速度，在速度-质量权衡上取得突破，使实时多模态MRI合成成为可能。
...
### [19] [Reinforcing 3D Understanding in Point-VLMs via Geometric Reward Credit Assignment](https://arxiv.org/abs/2604.21160)
*Jingkun Chen,Ruoshi Xu,Mingqi Gao,Shengda Luo,Jungong Han*

Main category: cs.CV

TL;DR: 本文针对点-视觉-语言模型中的几何幻觉问题，提出几何奖励信用分配框架，通过将整体监督信号解耦为字段特定信号并直接路由到对应令牌，结合重投影一致性约束，显著提升3D空间推理的可靠性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有点-视觉-语言模型虽然能赋予具身智能体可执行的空间推理能力，但经常出现几何幻觉——预测的3D结构与观察到的2D现实相矛盾。作者发现这一失败的关键原因不在于表示瓶颈，而在于强化学习中结构错位：稀疏的几何令牌被噪声和广播式的序列级奖励淹没。

Method: 提出几何奖励信用分配框架：1) 将整体监督信号解耦为字段特定信号，并仅路由到对应的令牌范围，将模糊反馈转化为精确梯度更新；2) 引入重投影一致性项，充当跨模态验证器，惩罚物理上不可能的几何结构。

Result: 在ShapeNetCore基准上：3D关键点准确率从0.64提升至0.93，3D包围框IoU提升至0.686，重投影一致性分数提升至0.852，同时保持鲁棒的2D定位性能。
...
### [20] [WildSplatter: Feed-forward 3D Gaussian Splatting with Appearance Control from Unconstrained Images](https://arxiv.org/abs/2604.21182)
*Yuki Fujimura,Takahiro Kushida,Kazuya Kitano,Takuya Funatomi,Yasuhiro Mukaigawa*

Main category: cs.CV

TL;DR: WildSplatter是一种前馈式3D高斯泼溅模型，能在无相机参数和光照条件变化的非约束图像中，快速（<1秒）重建3D高斯并实现外观控制，优于现有无位姿3DGS方法。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS需要已知相机参数和一致光照的多视图图像进行迭代优化，无法处理野外非约束图像（光照变化大、相机参数未知）。作者希望训练一个前馈模型，直接从稀疏输入重建3D高斯并支持外观调控。

Method: WildSplatter联合学习3D高斯和外观嵌入（appearance embeddings），以输入图像为条件，通过调节高斯颜色来表征光照和外观的显著变化。

Result: 在挑战性真实数据集上，WildSplatter优于现有无位姿3DGS方法，能在1秒内从稀疏输入重建，并支持不同光照条件下的外观控制。

Conclusion: 提出了一种面向非约束图像的快速前馈3DGS方案，兼顾重建速度与外观可控性，适用于真实世界多变光照场景。
...
### [21] [SpatiO: Adaptive Test-Time Orchestration of Vision-Language Agents for Spatial Reasoning](https://arxiv.org/abs/2604.21190)
*Chan Yeong Hwang,Miso Choi,Sunghyun On,Jinkyu Kim,Jungbeom Lee*

Main category: cs.CV

TL;DR: 该论文提出SpatiO，一种异构多智能体框架，通过测试时编排（TTO）机制动态协调多个视觉-语言专家进行空间推理，在各个空间推理基准上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理方法依赖单一推理管线，隐式学习固定空间先验，缺乏适应分布变化的能力；而多智能体系统之前主要使用同质智能体，限制了归纳偏见的多样性。

Method: 提出SpatiO异构多智能体框架，整合多个具有互补归纳偏见的视觉-语言专家；并引入测试时编排（TTO）机制，在推理阶段根据专家表现动态评估和重新加权，无需修改模型参数。

Result: 在3DSRBench、STVQA-7k、CV-Bench、Omni3D-Bench等多个空间推理基准上，SpatiO一致优于闭源和开源基线方法。

Conclusion: 异构多智能体协作结合测试时动态编排能够有效提升空间推理的适应性和性能，是处理复杂视觉场景中空间关系的有效范式。
...
### [22] [A Probabilistic Framework for Improving Dense Object Detection in Underwater Image Data via Annealing-Based Data Augmentation](https://arxiv.org/abs/2604.21198)
*Eleanor Wiesler,Trace Baxley*

Main category: cs.CV

TL;DR: 本文提出一种基于伪模拟退火的Copy-Paste数据增强框架，用于提升水下密集场景中鱼类检测的鲁棒性，在DeepFish数据集和佛罗里达实况流视频上显著优于YOLOv10基线。


<details>
  <summary>Details</summary>
Motivation: 水下环境光照、水质、视角多变且遮挡频繁，现有目标检测模型在真实水下场景中性能严重下降，亟需提升模型在密集、无约束水下场景中的泛化能力。

Method: 1) 从DeepFish分割掩码生成边界框标注构建检测数据集；2) 提出伪模拟退火增强算法（基于Deng等人的Copy-Paste策略），合成逼真的密集鱼类场景，提升空间多样性和目标密度。

Result: 在佛罗里达群岛实况流视频人工标注的困难测试集上，该方法显著超越YOLOv10基线模型。

Conclusion: 所提出的数据增强策略能有效提升密集真实水下环境中的检测性能，为水下视觉检测提供了实用的改进方向。
...
### [23] [Sparse Forcing: Native Trainable Sparse Attention for Real-time Autoregressive Diffusion Video Generation](https://arxiv.org/abs/2604.21221)
*Boxun Xu,Yuming Du,Zichang Liu,Siyu Yang,Ziyang Jiang,Siqi Yan,Rajasi Saha,Albert Pumarola,Wenchen Wang,Peng Li*

Main category: cs.CV

TL;DR: 本文提出Sparse Forcing，一种用于自回归视频扩散模型的训练-推理范式，通过利用注意力缓存中的块稀疏模式，实现长视频生成质量提升和decoding延迟降低。


<details>
  <summary>Details</summary>
Motivation: 自回归扩散模型在长视频生成中存在效率低、质量下降的问题，作者发现注意力集中在少数显著视觉块上，形成稀疏模式，希望利用这一特性优化生成过程。

Method: 提出可训练的本地稀疏机制，学习压缩、保留和更新持久化块，并在滑动窗口内动态选择局部邻域进行计算；设计Persistent Block-Sparse Attention (PBSA) GPU内核加速稀疏注意力和内存更新。

Result: 在5秒文本到视频生成任务上VBench分数提升+0.26，decoding加速1.11-1.17倍，峰值KV缓存占用降低42%；在20秒和1分钟生成长视频上分别获得+0.68和+2.74的VBench提升，以及1.22x和1.27x加速。

Conclusion: Sparse Forcing利用自回归扩散中的注意力稀疏模式，有效提升了长视频生成质量和效率，尤其在更长视频上优势更明显。
...
TL;DR: 提出LatRef-Diff框架，用风格码替代扩散模型中的语义方向，结合风格调制模块和前向-后向一致性训练策略，实现精确的人脸属性编辑和风格操控，在CelebA-HQ上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有条件GAN存在精度和训练不稳定问题；扩散模型的语义方向表达能力有限，难以实现精细的风格操控。

Method: 1) 用风格码替代传统语义方向，提出潜在引导和参考引导两种生成方法；2) 设计风格调制模块，集成可学习向量、交叉注意力机制和层次化设计；3) 提出前向-后向一致性训练策略，无需配对图像，通过先近似去除目标属性再通过风格调制恢复，结合感知和分类损失。

Result: 在CelebA-HQ数据集上，定性和定量评估均达到SOTA性能，消融实验验证了各设计模块的有效性。

Conclusion: LatRef-Diff有效解决了人脸属性编辑中精确控制和风格操控表达力不足的问题，为扩散模型在面部编辑领域的应用提供了新范式。
...
TL;DR: 本文提出ImageHD，一种基于超维计算的FPGA加速器，用于边缘设备上的流式持续学习。通过硬件感知算法（量化CNN前端、统一样本记忆、聚类合并策略）和流式数据流架构，在AMD Zynq ZCU104 FPGA上实现高效部署。相比优化后的CPU和GPU基线，在CORe50上分别实现40.4倍和4.84倍加速，以及383倍和105.1倍能效提升。


<details>
  <summary>Details</summary>
Motivation: 现有边缘持续学习方法依赖反向传播或大量样本的分类器，计算、内存和延迟开销巨大。虽然超维计算（HDC）是一种轻量级替代方案，但已有的HDC持续学习系统依赖多级存储层次和复杂的聚类管理，难以部署在资源受限的硬件上。

Method: 算法层面：提出硬件感知的持续学习方法，包括统一样本记忆（限制每类样本数量）、硬件高效的聚类合并策略，以及量化CNN前端以降低部署开销。系统层面：在AMD Zynq ZCU104 FPGA上实现流式数据流架构，集成HDC编码、相似度搜索和有界聚类管理，采用字打包的二进制超向量实现大规模并行位运算。

Result: 在CORe50数据集上，ImageHD相比优化的CPU基线实现40.4倍加速和383倍能效提升，相比优化的GPU基线实现4.84倍加速和105.1倍能效提升。

Conclusion: ImageHD证明了基于HDC的持续学习在实时边缘AI中的实用性，在严格延迟和片上内存约束下实现了高效的流式持续学习。
...
TL;DR: 提出AttDiff-GAN混合框架，结合GAN的属性操控与扩散模型的图像生成能力，通过特征级对抗学习解耦属性编辑与图像合成，并引入PriorMapper和RefineExtractor增强风格-属性对齐，在CelebA-HQ上实现了更精确的面部属性编辑。


<details>
  <summary>Details</summary>
Motivation: 现有GAN方法编辑可控但风格与属性语义对齐弱，扩散方法图像真实但属性间语义方向纠缠导致编辑精度有限。两者结合面临单步对抗学习与多步扩散去噪的不一致性问题，优化困难。

Method: 提出AttDiff-GAN混合框架：(1) 特征级对抗学习解耦属性编辑与图像合成；(2) PriorMapper引入面部先验到风格生成；(3) RefineExtractor利用Transformer捕获全局语义关系实现精确风格提取。

Result: 在CelebA-HQ数据集上，相比SOTA方法，在属性编辑精度和非目标属性保持方面均取得更优的定性和定量结果。

Conclusion: AttDiff-GAN有效融合了GAN的编辑可控性与扩散模型的高质量生成能力，解决了两种范式结合的核心不一致性问题，提升了面部属性编辑的精确度。
...
TL;DR: 本文提出GraphLeap，将Vision GNN中的图构建与特征更新解耦，实现流水线并行，并设计了首个端到端FPGA加速器，在Alveo U280上相比CPU加速95.7倍，相比GPU加速8.5倍。


<details>
  <summary>Details</summary>
Motivation: Vision GNN中每层动态图构建（kNN搜索）是主要性能瓶颈，耗时占比50-95%，且复杂度为O(N²)，同时图构建与特征更新之间存在顺序依赖，限制了并行加速。

Method: 提出GraphLeap：在第ℓ层使用ℓ-1层特征构建的图进行特征更新，同时用第ℓ层特征构建ℓ+1层的图，实现图构建与消息传递的并发执行。在此基础上设计了流式、层流水线化的FPGA加速器架构。

Result: 在Alveo U280 FPGA上，GraphLeap相比CPU实现最高95.7倍加速，相比GPU实现最高8.5倍加速，且通过少量微调即可恢复因使用前层特征导致的精度损失。

Conclusion: GraphLeap有效解决了Vision GNN中图构建的串行瓶颈，证明了实时Vision GNN推理的可行性，为图神经网络在边缘设备上的加速提供了新思路。
...
TL;DR: 本文系统研究了合成数据在可控人类视频生成中的作用，提出了一个基于扩散模型的统一框架，通过实验揭示了合成数据与真实数据的互补关系，并展示了高效选择合成样本以提升运动真实感、时间一致性和身份保持的方法。


<details>
  <summary>Details</summary>
Motivation: 大规模、多样化且隐私安全的人类视频数据集稀缺，尤其是针对罕见身份和复杂动作，这成为可控人类视频生成的主要瓶颈。合成数据虽可扩展且可控，但其对生成建模的实际贡献因Sim2Real差距尚未被充分探索。

Method: 提出一个基于扩散模型的框架，支持对外观和运动进行细粒度控制，并提供统一测试平台来分析合成数据与真实数据在训练过程中的交互作用。

Result: 通过大量实验，揭示了合成数据和真实数据的互补角色，并展示了高效选择合成样本的方法，能够提升运动真实感、时间一致性和身份保持效果。

Conclusion: 首次全面探索了合成数据在以人为中心的视频合成中的作用，为构建数据高效且可泛化的生成模型提供了实用见解。
...
TL;DR: 本文提出了一种基于Vision Transformer (ViT-B/16)的深度学习框架，用于从7,023张MRI扫描中自动四分类脑肿瘤（胶质瘤、脑膜瘤、垂体瘤和健康组织）。采用CLAHE增强、两阶段微调、MixUp/CutMix增强、EMA和TTA等策略，最终达到99.29%的测试准确率和99.25%的宏F1分数，超越所有CNN基线模型。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤的早期准确诊断直接关系到患者生存率，但传统MRI人工判读耗时、存在观察者间差异且依赖专家经验。因此，需要开发自动化的脑肿瘤分类系统以辅助临床诊断。

Method: 使用ImageNet-21k预训练的Vision Transformer (ViT-B/16)作为骨干网络；预处理采用CLAHE增强局部对比度；两阶段微调策略（先冻结骨干训练分类头，再全微调+判别式学习率）；每批使用MixUp和CutMix增强；应用EMA权重平滑和TTA测试增强；使用Attention Rollout生成可解释热力图。

Result: 测试准确率99.29%，宏F1分数99.25%，在健康组织和脑膜瘤类别上达到完美召回率，性能优于所有CNN基线模型。

Conclusion: 该ViT框架结合临床导向的预处理和训练策略，能够实现高精度、可解释的脑肿瘤自动分类，具有临床应用潜力。
...
TL;DR: NTIRE 2026挑战赛聚焦遥感红外图像超分辨率（x4倍），采用单轨制，115人注册、13支队伍提交有效结果，本文总结了挑战设计、数据集、评估协议、主要结果和各队代表性方法。


<details>
  <summary>Details</summary>
Motivation: 遥感场景中红外图像低分辨率问题影响实际应用，迫切需要有效的超分辨率模型或解决方案，该挑战赛旨在推动该方向研究发展。

Method: 通过组织NTIRE 2026挑战赛，采用单轨设置，使用双三次下采样生成低分辨率输入（x4倍），收集和评估各团队提交的模型/解决方案。

Result: 115人注册，13支队伍提交有效参赛作品，总结了挑战设计、数据集、评估协议、主要结果及各队代表性方法。

Conclusion: 该挑战为遥感红外图像超分辨率研究提供了基准，促进了面向真实遥感应用的有效解决方案的发展。
...
TL;DR: 提出PLAS-Net实例分割框架，从无人机图像中提取海滩垃圾的像素级物理面积，比边界框检测更准确，并展示了面积精度对生态风险评估三个下游任务的影响。


<details>
  <summary>Details</summary>
Motivation: 传统的边界框检测会高估不规则垃圾的平面面积，导致生态风险评估不准确，需要像素级精确的垃圾面积量化方法。

Method: 开发PLAS-Net实例分割框架，在泰国涛岛季风驱动口袋海滩的无人机影像上评估，并与11个基线模型比较。

Result: PLAS-Net达到58.7% mAP_50，精度高于11个基线模型；发现"丰度-面积悖论"——渔具数量少但单位面积最大。

Conclusion: 像素级面积提取比单纯计数能为海岸监测提供更有价值的信息，对生态风险、碎片化动态和污染热点分析至关重要。
...
TL;DR: 本文提出FryNet双流RGB-热成像框架，通过对抗性正则化消除相机指纹捷径，实现煎炸油降解的实时、空间感知检测，在7226帧数据上达到98.97% mIoU、100%分类精度和2.32回归MAE。


<details>
  <summary>Details</summary>
Motivation: 传统煎炸油检测依赖破坏性湿化学方法，无法提供空间信息且不适用于实时监测；而基于热成像的检查存在相机指纹捷径问题，模型记忆传感器噪声而非学习氧化化学，导致跨视频评估时性能崩溃。

Method: 提出FryNet双流RGB-热成像框架：1) ThermalMiT-B2主干网络结合通道和空间注意力提取热特征；2) RGB-MAE编码器通过掩码自编码和化学对齐学习化学基础表示；3) 双编码器DANN通过梯度反转层对抗性正则化两个流，消除视频身份信息；4) FiLM融合桥接热结构与RGB化学上下文；5) 单次前向传播同时完成油区分割、可用性分类和四个氧化指标回归。

Result: 在28个煎炸视频的7226张配对帧上，FryNet达到98.97% mIoU分割精度、100%分类准确率和2.32平均回归MAE，超越所有7个基线模型。

Conclusion: FryNet通过对抗性领域自适应有效消除了相机指纹捷径，证明了多模态融合与化学对齐结合的方法能实现可靠的煎炸油降解实时监测，具有重要的食品安全应用价值。
...
TL;DR: 提出HiTPro框架，首个解决无监督视频可见光-红外行人重识别问题的方法，通过分层时间原型学习，无需身份标签和伪标签，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VI-ReID方法多聚焦于图像级别或有监督学习，而视频级别的无监督VI-ReID问题（无需身份标签，从RGB和红外轨迹中学习）在实际部署中非常重要，但至今未被充分探索。

Method: 提出HiTPro框架，包含：1）时序感知特征编码器提取帧级特征并聚合为轨迹级表示；2）相机内轨迹原型构建；3）分层跨原型对齐（先模态内再跨模态，含动态阈值策略和软权重分配）；4）分层对比学习（相机内判别、跨相机同模态一致性、跨模态不变性）。

Result: 在HITSZ-VCM和BUPTCampus数据集上，在完全无监督设置下达到最先进性能，显著优于适配的基线方法。

Conclusion: HiTPro为无监督视频VI-ReID建立了强基线，证明了无需硬伪标签的分层原型学习在此任务中的有效性。
...
TL;DR: 本文提出MiMIC模型，通过融合解码器架构和单模态混合训练，解决了现有通用多模态检索方法中早期融合（如Marvel）的视觉模态坍塌和晚期融合（如UniVL-DR）的语义错位问题，在WebQA+和EVQA+数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有通用多模态检索方法存在两类缺陷：早期融合方法（如Marvel）出现视觉模态坍塌，即模型忽略视觉特征而过度依赖文本线索；晚期融合方法（如UniVL-DR）则更容易出现语义错位，即语义相关的内容在嵌入空间中距离过远。

Method: 提出MiMIC模型，包含两个关键创新：(1) 融合解码器架构（fusion-in-decoder），实现有效的多模态整合；(2) 通过单模态混合训练（single modality mixin）和随机字幕丢弃（random caption dropout）实现鲁棒训练。

Result: 在文档或查询中图像可能缺少字幕的WebQA+和EVQA+数据集上，MiMIC方法一致优于早期融合和晚期融合的基线方法。

Conclusion: MiMIC通过融合解码器架构和鲁棒训练策略，有效解决了早期融合的视觉模态坍塌和晚期融合的语义错位问题，为通用多模态检索提供了更优方案。
...
TL;DR: 针对稀疏MoE训练中路由器因梯度阻塞导致的优化困难，提出TGR-MoE方法，利用预训练密集教师模型的中间表征构建教师路由器，为学生路由器提供伪监督信号，稳定路由学习，提升准确率和路由一致性。


<details>
  <summary>Details</summary>
Motivation: 稀疏MoE训练中，路由器只能通过前向传播中被选中的专家获得梯度，导致梯度阻塞，难以从未选中的路由路径获取有效信息，造成路由学习不稳定、专家分配频繁波动。

Method: 提出TGR-MoE，从预训练密集教师模型的中间表征中构建教师路由器，将其路由输出作为伪监督信号指导学生路由器训练，从早期阶段就实现知识引导的专家选择，抑制路由波动。

Result: 在ImageNet-1K和CIFAR-100上，TGR-MoE在多种稀疏配置下均显著提升了分类准确率和路由一致性，训练过程更加稳定。

Conclusion: 利用教师模型的知识引导可以有效缓解稀疏MoE的路由优化难题，是一种简单且有效的路由稳定化方法。
...
### [36] [Efficient Logic Gate Networks for Video Copy Detection](https://arxiv.org/abs/2604.21694)
*Katarzyna Fojcik*

Main category: cs.CV

TL;DR: 本文提出基于可微逻辑门网络（LGN）的视频复制检测框架，用紧凑的逻辑电路替代传统浮点特征提取器，在保持竞争性能的同时实现极小的描述符体积和超高速推理（>11k samples/s）。


<details>
  <summary>Details</summary>
Motivation: 现有深度神经网络在视频复制检测中性能虽强，但计算开销大、描述符尺寸大，难以在大规模高吞吐系统中实际部署。作者希望找到一种兼顾精度与效率的替代方案。

Method: 提出基于可微逻辑门网络（LGN）的框架：1）对视频帧进行激进压缩和二值化预处理；2）使用可训练的LGN嵌入模型同时学习逻辑运算和互联结构；3）训练后将模型离散化为纯布尔逻辑电路，实现极致高效的推理。系统性地比较了不同相似度策略、二值化方案和LGN架构。

Result: LGN模型在多个数据集折叠和难度级别上达到与现有方法相当或更优的准确率和排序性能，同时描述符体积比传统方法小数个数量级，推理速度超过11k样本/秒。

Conclusion: 基于逻辑的模型为可扩展且资源高效的视频复制检测提供了有前景的替代方案，在保持性能的同时大幅提升了部署友好性。
...
### [37] [Latent Denoising Improves Visual Alignment in Large Multimodal Models](https://arxiv.org/abs/2604.21343)
*Dhruv Parikh,Jacob Fein-Ashley,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.CV

TL;DR: 本文提出一种潜在去噪框架，通过加噪-去噪训练提升大多模态模型（LMM）的视觉表征质量，无需增加推理开销，在多项基准上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有LMM（如LLaVA）仅通过自回归语言建模目标间接监督视觉token，导致内部视觉表征弱，在分布偏移下表现脆弱。

Method: 提出潜在去噪框架：1）对投影后的视觉token进行显著性感知的混合掩码+高斯噪声腐蚀；2）训练LMM从中间层隐藏状态通过解码器恢复干净教师patch特征；3）保留教师特征内的图像相似性结构，进行图像内对比补丁蒸馏；4）推理时去除腐蚀和辅助头，无额外开销。

Result: 在多项多模态基准上持续优于强基线；在NaturalBench等组合鲁棒性基准上表现提升；在ImageNet-C风格的非对抗性常见腐蚀下保持更高准确率，退化幅度更小。

Conclusion: 潜在去噪原理可以有效改善LMM的视觉特征对齐和多模态理解能力，且不增加推理负担。
...
TL;DR: 本文提出SparseGF，一个针对机载激光扫描数据的高程感知稀疏分割框架，通过上下文压缩、混合稀疏体素-点网络架构和高程感知损失函数，解决了地面滤波中上下文-细节困境和地物随机误分类问题，在多个基准数据集上实现了从城市到自然地形的鲁棒地面滤波。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的地面滤波方法虽然在特定复杂地形上表现优异，但跨场景泛化能力有限。主要存在两个问题：（1）大规模处理中因计算资源限制导致的上下文-细节困境；（2）仅依赖分类优化导致的高大物体随机误分类。因此需要一种能在保持细部特征的同时理解大范围上下文，并有效抑制误分类的新型方法。

Method: SparseGF包含三个关键创新：（1）基于凸面镜启发的上下文压缩模块，将广阔上下文压缩为紧凑表示同时保留中心细节；（2）混合稀疏体素-点网络架构，有效解释压缩表示并缓解压缩引起的几何畸变；（3）高程感知损失函数，在训练中显式施加地形高程先验，抑制高大物体的随机误分类。

Result: 在两个大规模ALS基准数据集上的广泛评估表明，SparseGF在从城市到自然地形的多样场景下均能实现鲁棒的地面滤波：在复杂城市场景中取得领先性能，在混合地形上具有竞争力，在密集森林陡坡区域也保持了适度而非灾难性的精度。

Conclusion: SparseGF为深度学习地面滤波研究提供了新见解，推动了大尺度环境监测中真正跨场景泛化的探索方向。
...
TL;DR: 提出基于原型的测试时自适应方法（PTA），通过类特定知识原型累积测试样本知识，无需缓存检索，在保持高推理速度的同时实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于缓存的TTA方法存在两个问题：1）缓存随类别数增长导致推理延迟增加；2）缓存中样本不足或错误导致性能欠佳。因此需要更高效且有效的TTA范式。

Method: PTA使用一组类特定知识原型，根据每个测试样本的零样本类置信度自适应加权，将样本视觉特征融入对应类原型中。知识仅存储在原型中，无需缓存填充和检索。

Result: 在15个图像识别基准和4个鲁棒点云分析基准上达到SOTA。在10个跨域基准上，将CLIP准确率从65.64%提升至69.38%，同时保持92%的推理速度；而缓存方法仅达67.97%准确率且速度仅为CLIP的50%。

Conclusion: PTA通过原型机制解决了缓存方法的效率瓶颈，在保持高推理速度的同时实现了更优的适应性性能，是一种高效且有效的测试时自适应新范式。
...
TL;DR: 本文针对创意视频生成（CVG）中文本到视频（T2V）模型面临的语义对齐模糊和运动适应性不足两大挑战，构建了广告创意知识库（ACKB），并提出知识驱动方法KD-CVG，通过语义感知检索（SAR）和多模态知识参考（MKR）两个模块，显著提升了视频生成的语义对齐和运动表现，效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有创意生成研究主要集中在文本和图像生成，创意视频生成（CVG）相对未被充分探索。主要原因是T2V模型存在两个关键挑战：(a)语义对齐模糊——模型难以准确关联产品卖点与创意视频内容；(b)运动适应性不足——导致生成内容出现不真实的运动和形变。

Method: 提出了知识驱动方法KD-CVG，包含两个核心模块：(1)语义感知检索（SAR）——利用图注意力网络的语义感知能力和强化学习反馈，增强模型对卖点与创意视频关联的理解；(2)多模态知识参考（MKR）——将语义和运动先验知识注入T2V模型，弥补现有模型的知识鸿沟。同时构建了广告创意知识库（ACKB）作为基础资源。

Result: 大量实验表明，KD-CVG在语义对齐和运动适应性方面均取得了优越性能，效果优于其他最先进的方法。

Conclusion: KD-CVG有效解决了T2V模型在创意视频生成中的两大核心挑战，验证了知识驱动方法在提升视频生成质量方面的有效性。代码和数据集将开源。
...
TL;DR: 本文提出EdgeFormer，一种基于学习的3D点云边缘检测网络，通过将点云边缘检测转化为局部面片分类，有效提取细粒度边缘特征，性能优于六种基线方法。


<details>
  <summary>Details</summary>
Motivation: 3D点云中的边缘点能清晰传达几何形状和表面特征，但细粒度边缘特征因密集分布或小尺度表面梯度难以有效检测，现有方法对此处理不足。

Method: EdgeFormer包含两阶段：第一阶段为每个点构建描述局部邻域的局部面片特征描述符；第二阶段基于这些描述符对每个点进行分类。核心思路是利用空间邻近点的高相关性，将整体点云边缘检测转化为局部面片的点分类问题。

Result: 实验结果表明，EdgeFormer在点云边缘检测任务上与六种基线方法相比，展现出具有竞争力的性能。

Conclusion: 通过将点云转换为局部面片，EdgeFormer能有效提取更精细的边缘细节，为3D点云细粒度边缘检测提供了一种有效的解决方案。
...
TL;DR: 本文提出VG-CoT数据集，通过全自动三阶段流程为每个推理步骤关联视觉证据，并构建新基准从三个维度评估LVLM推理，实验证明能有效提升可信推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM数据集因依赖大量人工标注，且缺乏多步推理与图像区域的显式对齐，导致模型可信度评估受限，亟需可扩展且具备视觉证据链接的高质量数据集。

Method: 三阶段全自动流程：(1) 利用检测和OCR模型提取目标级和文本级视觉证据；(2) 使用GPT-4o生成逐步接地推理；(3) 通过基于理由的开放集检测过程精化接地。

Result: 在多项评估指标上，LLaVA-1.5和Qwen2-VL等代表性LVLM模型均获得一致提升，验证了数据集的有效性。
...
TL;DR: 提出YOGO框架，将3DGS从随机增长转为确定性预算均衡，并发布超密集室内数据集Immersion v1.0，弥合3D高斯泼溅在学术与工业部署之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法存在三个关键问题：1）启发式高斯增长导致不可预测的资源消耗；2）现有基准的"稀疏遮蔽"鼓励幻觉而非物理保真度；3）多传感器数据污染严重。这些问题阻碍了3DGS从研究原型走向生产级部署。

Method: 1）YOGO框架：将随机增长过程重构为确定性、预算感知的均衡态，包含预算控制器（硬件约束资源分配）与可用性-注册协议（鲁棒多传感器融合）；2）Immersion v1.0数据集：首个超密集室内数据集，提供饱和视点覆盖以打破"稀疏遮蔽"。

Result: YOGO在保持严格确定性轮廓的同时实现了最先进的视觉质量；Immersion v1.0迫使算法关注极端物理保真度而非视点插值。

Conclusion: 建立了生产级3DGS的新标准，已公开部分数据集和源代码以促进可复现性。
...
TL;DR: 提出S1-VL，一个科学领域多模态推理模型，原生支持两种推理范式：结构化思维链的科学推理，以及通过Python代码执行在推理中主动操作图像的Thinking-with-Images。模型通过四阶段训练流程构建，在13个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有科学多模态推理模型在处理高分辨率科学图表、微观图像理解和几何辅助推理等复杂场景时能力不足，且已有数据集存在冗余、无效和错误的视觉操作问题。

Method: 1) 提出Thinking-with-Images范式，模型可在推理中生成并执行图像处理代码获取中间结果；2) 构建覆盖六大学科的科学多模态数据集；3) 开发六维质量过滤框架和自适应数据路由策略；4) 采用四阶段渐进训练流程（科学多模态SFT、Thinking-with-Images冷启动SFT、两阶段SAPO强化学习）。

Result: S1-VL-32B在所有5个Thinking-with-Images基准测试（HRBench-4K/8K、MME-RealWorld-CN/Lite、V*）上达到SOTA，在Physics和VRSBench等科学推理基准上也优于对比系统。

Conclusion: 通过代码执行实现图像主动操作的推理范式可以有效提升科学多模态推理能力，特别在高分辨率图表和微观图像理解等挑战性任务上表现突出。
...
TL;DR: 本文提出一种新的非线性扩散滤波器族，用于将图像转化为分段常数图像（轮廓闭合、内部灰度均匀、边缘不模糊），作为分割预处理。该滤波器满足半离散和全离散尺度空间适定性条件，通过半隐式格式求解前向非线性扩散方程实现边缘保持，并配有低计算成本的停止准则。代码已开源。


<details>
  <summary>Details</summary>
Motivation: 为分割技术提供预处理步骤，需要将图像转化为分段常数图像——即内部灰度均匀且边缘清晰闭合，同时避免边缘模糊。现有非线性扩散滤波器在此目标上存在不足，因此需要设计新的扩散函数族。

Method: 1. 提出非线性扩散方程的内在形式化表达，给出扩散滤波器的设计条件；2. 基于非线性扩散技术与后向扩散关联，提出新的扩散率函数族；3. 证明所提滤波器满足半离散和全离散尺度空间适定性要求；4. 采用半隐式格式求解前向非线性扩散方程（而非后向），实现边缘保持；5. 设定扩散率的停止准则，以低计算开销获得分段常数图像。

Result: 在真实图像上验证了所提滤波器的效果，展示了扩散率函数在获得分段常数图像方面的有效性，且计算成本低。

Conclusion: 提出的非线性扩散滤波器族能够有效将图像处理为分段常数图像（闭合轮廓、均匀灰度、无边缘模糊），满足尺度空间适定性，计算高效，适合作为分割预处理工具。
...
TL;DR: 本文提出UHR-DETR，一种面向超高分辨率遥感影像的高效端到端Transformer检测器，通过覆盖最大化稀疏编码器和全局-局部解耦解码器，在单张24GB显卡上实现2.8% mAP提升和10倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率遥感影像中小目标检测存在两难困境：保留原分辨率带来显存瓶颈，而降采样或裁剪会丢失小目标或破坏上下文。现有方法无法同时解决效率和精度问题。

Method: 提出两个核心模块：1) 覆盖最大化稀疏编码器，动态将有限计算资源分配给信息丰富的高分辨率区域，最大化目标覆盖同时减少空间冗余；2) 全局-局部解耦解码器，融合宏观场景感知与微观目标细节，解决语义模糊和场景碎片化问题。

Result: 在STAR和SODA-A等UHR数据集上，在单张RTX 3090 (24GB) 严格硬件限制下，相比标准滑窗基线，mAP提升2.8%，推理速度提升10倍。

Conclusion: UHR-DETR成功打破了UHR影像中分辨率和显存之间的两难困境，为超高分辨率遥感小目标检测提供了高效、实用的解决方案。
...
TL;DR: 本文提出了一种基于两步哈希函数策略的2L-LSH算法，用于大规模3D点云模型中快速邻域点搜索，相比Kd-tree和Octree在速度上有显著提升（kNN搜索时间降低51%-94%，RN搜索时间降低42%-55%）。


<details>
  <summary>Details</summary>
Motivation: 3D扫描技术发展带来大规模、结构多样化的点云模型，在模型重建、分类、检索等应用中频繁需要快速邻域点搜索，现有方法（如Kd-tree、Octree）效率不足。

Method: 提出2L-LSH（两步局部敏感哈希）算法：第一步对点云模型的包围盒进行划分，第二步构建基于广义表的数据结构，利用哈希函数的高维数据快速精确搜索能力实现两阶段快速搜索。

Result: 与Kd-tree和Octree相比，kNN搜索时间分别降低51.111%和94.159%，RN搜索时间分别降低54.519%和41.840%。

Conclusion: 2L-LSH在大规模3D点云快速邻域搜索中具有高效性和准确性，是比Kd-tree和Octree更具优势的解决方案。
...
TL;DR: 本文提出VARestorer，一个将预训练文本到图像VAR模型转化为单步超分辨率模型的蒸馏框架，通过分布匹配消除迭代推理，引入金字塔图像条件与跨尺度注意力实现双向尺度交互，仅微调1.2%参数，在DIV2K数据集上达到SOTA性能且推理加速10倍。


<details>
  <summary>Details</summary>
Motivation: 现有VAR模型在图像超分辨率中存在两大问题：一是因果注意力机制下的下一尺度预测无法充分利用全局低质量上下文，导致输出模糊不一致；二是迭代预测中的误差累积严重损害重建一致性。因此需要一种方法克服这些局限。

Method: 1) 采用分布匹配蒸馏，将预训练VAR模型转化为单步ISR模型，消除迭代推理；2) 提出金字塔图像条件与跨尺度注意力，实现双向尺度交互，充分利用输入图像信息；3) 仅微调1.2%的参数（参数高效适配器），保持原模型表达力。

Result: 在DIV2K数据集上达到72.32 MUSIQ和0.7669 CLIPIQA的SOTA性能，推理速度相比传统VAR推理加速10倍。

Conclusion: VARestorer有效解决了VAR模型在ISR任务中的因果注意力局限和误差累积问题，以极小的参数量微调实现了高效、高质量的超分辨率重建，验证了将生成式VAR模型蒸馏为单步判别模型的可行性。
...
TL;DR: 本文提出OA-VAT，一种面向视觉主动跟踪的统一流水线，包含无训练实例感知原型初始化、在线原型增强跟踪器和遮挡感知轨迹规划器三个模块，在多个数据集和真实无人机上实现了SOTA性能，运行速度达35 FPS。


<details>
  <summary>Details</summary>
Motivation: 视觉主动跟踪在现实中面临两个关键瓶颈：视觉相似干扰物导致的实例级判别不足，以及遮挡导致的跟踪失败，这些问题源于缺乏主动规划能力。

Method: 提出OA-VAT三模块流水线：1) 无训练的实例感知离线原型初始化，利用DINOv3聚合多视图增强特征构建判别性实例原型；2) 在线原型增强跟踪器，集成置信度感知卡尔曼滤波；3) 遮挡感知轨迹规划器，在Planning-20k数据集上训练，使用条件扩散生成避障路径。

Result: UnrealCV上0.93平均SR（+2.2%），真实数据集上90.8%平均CAR（+12.1%），DJI Tello无人机上81.6% TSR，RTX 3090上35 FPS。

Conclusion: OA-VAT通过统一的主动跟踪流水线，显著提升了抗干扰和遮挡恢复能力，实现了鲁棒、实时的实际部署性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM无法精确理解指向手势的空间语义，而是依赖视觉邻近性或物体显著性等虚假相关性（即"指代幻觉"），需要专门的基准来评估和改善这一能力。

Method: 构建EgoPoint-Bench基准，包含11k+高保真模拟与真实样本，覆盖5个评估维度和3个指代复杂度层级；通过合成数据对模型进行微调。

Result: 现有闭源和开源SOTA模型在自我中心指向任务上表现不佳；基于合成数据微调的模型取得显著性能提升，并展现出鲁棒的模拟到真实泛化能力。

Conclusion: 空间感知监督对自我中心AI助手至关重要，合成数据提供了一条可扩展的解决方案。
...
TL;DR: 提出ID-Eraser，一种在特征空间进行主动防御的方法，通过向身份嵌入注入可学习扰动并利用Face Revive Generator重建自然图像，有效阻止恶意换脸，在多项指标上达到最优效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素级扰动的主动防御方法对提取鲁棒高层身份嵌入的现代换脸模型效果不佳，亟需一种更有效的特征空间防御手段。

Method: 在特征空间向身份嵌入注入可学习扰动，再通过Face Revive Generator（FRG）重建视觉自然的保护图像，实现防御。

Result: 黑盒设置下Top-1准确率仅0.30，FID最佳（1.64），LPIPS最佳（0.020）；五种换脸模型身份相似度平均降至0.504；腾讯API相似度从0.76降至0.36。

Conclusion: ID-Eraser在特征空间防御换脸攻击方面有效，具备跨数据集泛化能力、鲁棒性及实际商业API应用潜力。
...
TL;DR: 本文提出Cross-AUC指标评估跨域分数可比性，并设计SFAM框架（含补丁级图文对齐和面部区域混合专家模块）提升人脸伪造检测的泛化能力，实验证明方法优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有伪造检测器跨数据集泛化能力差，根本原因在于缺乏合适的评估指标——常用的跨数据集AUC无法揭示检测分数在不同数据域间的显著偏移问题。

Method: 提出Cross-AUC评估指标（跨数据集配对计算AUC）；提出SFAM框架，包含补丁级图像-文本对齐模块（增强CLIP对伪造痕迹的敏感性）和面部区域混合专家模块（不同区域特征路由到对应专家进行区域感知分析）。

Result: Cross-AUC指标揭示现有检测器存在严重的鲁棒性问题；SFAM在多个公开数据集上的定性和定量实验均优于现有方法。

Conclusion: Cross-AUC是评估跨域分数可比性的有效指标；SFAM通过区域感知的细粒度对齐显著提升了伪造检测的泛化性能。
...
TL;DR: 本文提出了一个评估LLMs在自动驾驶感知与预测中理解动态交通代理和道路网络拓扑能力的框架，通过冻结的LLM作为推理引擎，结合轨迹编码器和轻量级CNN处理地图信息，使用重编程适配器将场景特征转换为LLM兼容的token，最终用线性解码器输出未来轨迹，实现了对多模态信息（尤其是地图语义）影响的定量分析。


<details>
  <summary>Details</summary>
Motivation: LLMs在自动驾驶领域展现出潜力，但其在感知和预测中的安全应用仍需深入理解动态交通代理行为和静态道路基础设施。现有的研究缺乏一个统一的、能够定量分析多模态信息（尤其是地图语义）对轨迹预测影响的评估框架。

Method: 提出框架：1) 使用交通编码器从代理观测轨迹中提取空间级场景特征；2) 轻量级CNN编码局部高精地图；3) 通过重编程适配器将场景特征转为LLM兼容的token；4) 冻结的LLM作为推理引擎；5) 简单的线性解码器输出未来轨迹。

Result: 该框架能够定量分析多模态信息（尤其是地图语义）对轨迹预测精度的影响；在多种LLM架构上展现出强泛化能力；仅需最小适配即可无缝集成冻结的LLM。

Conclusion: 该框架为评估LLMs在自动驾驶轨迹预测中的能力提供了一个统一的平台，证明了冻结的LLM配合简单解码器即可有效完成预测任务，并为后续研究奠定了方法论基础。
...
TL;DR: 提出VFM⁴SDG框架，通过冻结的视觉基础模型（VFM）提供跨域稳定性先验，分别在编码阶段进行关系先验蒸馏、在解码阶段进行语义-上下文先验查询增强，有效提升了目标检测器在未知域上的泛化能力，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单域泛化目标检测（SDGOD）方法主要依赖数据增强或域不变表示学习，但对检测器自身的机制关注有限，在复杂域漂移下性能退化严重（主要表现为漏检增多），其根本原因在于检测器的跨域稳定性不足：编码阶段目标-背景和实例间关系不稳定，解码阶段查询表示的语义-空间对齐难以维持。

Method: 提出VFM⁴SDG双先验学习框架：1）编码阶段：跨域稳定关系先验蒸馏（Cross-domain Stable Relational Prior Distillation），增强目标-背景和实例间关系建模的鲁棒性；2）解码阶段：语义-上下文先验查询增强（Semantic-Contextual Prior-based Query Enhancement），注入类别级语义原型和全局视觉上下文到查询中，提升语义识别和空间定位稳定性。整体使用冻结的VFM作为可迁移的跨域稳定性先验。

Result: 在标准SDGOD基准和两种主流DETR类检测器上，该方法一致性地超越了现有SOTA方法，验证了其有效性、鲁棒性和通用性。

Conclusion: 证明利用冻结的视觉基础模型作为跨域稳定性先验，通过关系蒸馏和查询增强两个层面的改进，能有效提升检测器在未知域上的泛化性能，为SDGOD提供了新思路。
...
TL;DR: 提出了一种基于高斯混合模型（GMM）的局部描述符GMD，用于碎片断裂面的匹配与重组，在真实扫描数据集和兵马俑数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 激光扫描碎片自动重组中，断裂面匹配是关键步骤，现有方法存在描述能力不足的问题。

Method: 将局部曲面划分为凹凸区域分别估计GMM的k值，合并区域GMD形成最终描述符；使用L2距离度量相似性，结合RANSAC和ICP算法对齐碎片。

Result: 在真实扫描公共数据集和兵马俑数据集上表现优异，与多种现有方法对比验证了优势。

Conclusion: 提出的GMD描述符能有效描述和匹配碎片断裂面，为激光扫描碎片的自动重组提供了可行方案。
...
TL;DR: 本文系统评估了VLM作为评估器的可靠性，发现当前VLM评估器存在严重盲点——在检测输出质量下降时失败率超过50%，尤其难以识别细粒度的组合性和空间错误以及幻觉内容，揭示了其不可靠的本质。


<details>
  <summary>Details</summary>
Motivation: VLMs被广泛用于评估其他模型的输出（包括I2T和T2I任务），但其作为评估器的可靠性尚未被充分探索。

Method: 引入针对关键误差维度（对象幻觉、空间推理、事实基础、视觉保真度）的扰动，构建包含4000+扰动实例、40个扰动维度的基准，使用单答案评分、成对比较、参考引导三种范式评估4个主流VLM。

Result: VLM评估器存在严重盲点：检测失败率有时超过50%；难以识别细粒度组合性和空间错误；对与输入图像矛盾的幻觉内容不敏感；成对比较范式相对更可靠但仍存在失败。

Conclusion: 当前VLM评估器本质上是不可靠的，在用于基准测试和开发决策时应谨慎行事。
...
TL;DR: 本文提出基于注意力机制的多实例学习（ABMIL）框架，利用预训练病理基础模型作为补丁编码器，在全切片级别预测肺腺癌主要生长模式，减少标注负担。实验表明微调编码器能提升性能，其中Prov-GigaPath在ABMIL下达到最高一致性（κ=0.699）。


<details>
  <summary>Details</summary>
Motivation: 肺腺癌分级依赖于准确识别生长模式，但传统深度学习方法需要大量补丁级标注，工作量大。本文旨在通过弱监督学习降低标注负担。

Method: 采用注意力机制的多实例学习（ABMIL）框架，整合预训练病理基础模型（可冻结或微调）作为补丁编码器，通过注意力机制聚合特征，实现全切片级别的弱监督预测。

Result: 微调后的编码器性能优于冻结编码器；Prov-GigaPath在ABMIL下达到最高kappa值为0.699；ABMIL相比简单补丁聚合基线更鲁棒。

Conclusion: ABMIL框架能有效利用全切片级监督和空间注意力机制，在减少标注成本的同时取得良好预测性能。未来将扩展到生长模式的完整分布估计及外部队列验证。
...
TL;DR: 本文提出基于组件的OOD检测框架CoOD，通过分解输入为功能组件、计算局部偏移分数(CSS)和组件间一致性分数(CCS)，无需训练即可提升粗粒度和细粒度OOD检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法存在局限性：全局表示会抑制局部OOD线索，基于patch的方法因虚假关联和噪声而不稳定，且两者都难以检测由有效ID组件构成的组合型OOD。

Method: 受识别-组件理论启发，提出无训练框架CoOD，将输入分解为功能组件，通过组件偏移分数(CSS)检测局部外观变化，通过组合一致性分数(CCS)识别跨组件的组合不一致性。

Result: 在粗粒度和细粒度OOD检测任务上均取得一致的性能提升。

Conclusion: CoOD通过组件级分解有效解决了全局/局部方法的不足，特别在组合型OOD检测上展现了优势，是一种有前景的无训练OOD检测范式。
...
TL;DR: 本文提出了一种基于深度核空间学习的单视频无监督动作分割方法，利用最大均值差异（MMD）和神经正切核（NTK）来近似视频帧分布，在无需大型数据集的情况下实现了具有竞争力的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督动作分割方法通常需要存储大型数据集，这在某些应用场景中不可行或不被允许。因此，作者希望提出一种仅利用单个视频本身信息进行动作分割的方法，无需依赖大规模数据集。

Method: 在深度核空间中学习，使用最大均值差异（MMD）作为分布间的接近度度量，并采用神经正切核（NTK）定义核空间。MMD具有保几何特性的优点且易于优化，NTK则具有更强的描述能力并避免了联合学习输入和核函数时的平凡解问题。

Result: 在六个标准基准数据集上，该方法与当前最先进的单视频方法相比取得了具有竞争力的结果；并且在未知分割数量时，相比先前的凝聚层次方法获得了更高的F1分数。

Conclusion: 基于MMD和NTK的深度核空间学习方法在单视频无监督动作分割任务上表现优异，既避免了大型数据集需求，又在准确性和鲁棒性上超越了现有方法。
...
TL;DR: CHRep是一个两阶段框架，通过结构感知表示学习与轻量级后验校准模块，在留一切片评估下显著提升H&E切片预测空间基因表达的鲁棒性，在三个队列中均取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学(ST)成本高、通量低，难以用于大规模队列研究和临床。从常规H&E切片预测空间基因表达是一种有前景的替代方案，但现有模型在留一切片评估下存在切片间外观偏移和回归驱动的过平滑问题，抑制了有生物学意义的变异。

Method: 两阶段框架：**训练阶段**通过相关性感知回归、对称图像-表达对齐和坐标诱导的空间拓扑正则化联合优化，学习结构感知表示；**推理阶段**无需微调主干网络，通过轻量级校准模块（结合训练图库的非参数估计与幅度正则化修正）提升跨切片鲁棒性。

Result: 在三个队列中，CHRep在留一切片评估下持续提升基因-wise相关性。在Alex+10x上提升最大：PCC(ACG)相对mclSTExp提高39.5%，MSE和MAE分别降低9.7%和9.0%；在cSCC和HER2+上相对HAGE分别提升4.0%和9.8%。

Conclusion: CHRep通过拓扑保持表示学习与后验校准的耦合，克服了切片级偏移下的预测退化，为低资源ST预测提供了鲁棒且实用的解决方案。
...
### [61] [OmniFit: Multi-modal 3D Body Fitting via Scale-agnostic Dense Landmark Prediction](https://arxiv.org/abs/2604.21575)
*Zeyu Cai,Yuliang Xiu,Renke Wang,Zhijing Shao,Xiaoben Li,Siyuan Yu,Chao Xu,Yang Liu,Baigui Sun,Jian Yang,Zhenyu Zhang*

Main category: cs.CV

TL;DR: OmniFit提出了一种尺度无关的多模态人体拟合方法，通过条件Transformer解码器将表面点映射到身体关键点，结合可选的图像适配器和尺度预测器，在宽松衣物场景下比SOTA方法提升57.1%-80.9%，首次超越多视角优化基线并在CAPE和4D-DRESS基准上达到毫米级精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体模型拟合方法大多依赖单模态输入（点云或多视角图像）且需要已知度量尺度，这在AI生成资产（常存在尺度失真）等实际场景中难以满足。因此需要一种能处理多种模态输入且对尺度不敏感的统一方法。

Method: 提出OmniFit框架：1）条件Transformer解码器直接将表面点映射到稠密身体 landmarks；2）可选的即插即用图像适配器利用视觉线索补充缺失几何信息；3）专用尺度预测器将目标重缩放至规范人体比例；最后通过SMPL-X参数拟合得到最终人体模型。

Result: 在宽松和日常衣物场景下，OmniFit相比SOTA方法提升57.1%-80.9%；首次超越多视角优化基线；在CAPE和4D-DRESS基准上达到毫米级精度。

Conclusion: OmniFit是首个能同时处理全扫描、部分深度观测和图像等多种模态输入且尺度无关的人体拟合方法，实验证明其性能显著优于现有方法。
...
### [62] [Sculpt4D: Generating 4D Shapes via Sparse-Attention Diffusion Transformers](https://arxiv.org/abs/2604.21592)
*Minghao Yin,Wenbo Hu,Jiale Xu,Ying Shan,Kai Han*

Main category: cs.CV

TL;DR: Sculpt4D 提出了一种原生4D生成框架，通过在预训练3D Diffusion Transformer中融入高效时序建模和块稀疏注意力机制，实现了高保真、时间连贯的动态4D生成，并将计算量降低56%。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成在静态形状合成上取得显著进展，但高保真动态4D生成仍然困难，主要受限于时间伪影、巨大的计算需求以及4D训练数据的稀缺。

Method: 在预训练3D Diffusion Transformer（Hunyuan3D 2.1）基础上，提出块稀疏注意力（Block Sparse Attention）机制：通过对初始帧锚定来保持物体身份一致性，并通过时间衰减稀疏掩码捕获丰富的运动动态，从而高效建模复杂的时空依赖关系，避免全注意力的二次复杂度。

Result: 网络总计算量减少56%，在时间连贯的4D合成任务上达到新的最优水平。

Conclusion: Sculpt4D为高效、可扩展的4D生成指明了方向，在动态4D生成领域树立了新标杆。
...
### [63] [Local Neighborhood Instability in Parametric Projections: Quantitative and Visual Analysis](https://arxiv.org/abs/2604.21617)
*Frederik L. Dennig,Daniel A. Keim*

Main category: cs.CV

TL;DR: 本文提出了一种稳定性评估框架，通过高斯扰动探测参数化投影的局部稳定性，结合定量指标和可视化方法分析嵌入空间中的邻域变形，并在MNIST和Fashion-MNIST数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 参数化投影能够实时嵌入新点，但输入变化（如测量噪声或数据漂移）会导致2D布局产生不可预测的偏移，而投影的局部稳定性尚未得到充分研究。

Method: 围绕锚点施加高斯扰动，通过平均位移、偏差和最近锚点分配误差等定量指标，结合位移向量、局部PCA椭球和Voronoi误分配可视化，评估参数化投影的局部稳定性。

Result: 该框架能够识别出重构误差或邻域保持指标无法发现的不稳定投影区域。

Conclusion: 提出的稳定性评估框架有效弥补了现有指标在检测局部不稳定性方面的不足，为理解参数化投影的鲁棒性提供了新工具。
...
### [64] [DCMorph: Face Morphing via Dual-Stream Cross-Attention Diffusion](https://arxiv.org/abs/2604.21627)
*Tahar Chettaoui,Eduarda Caldeira,Guray Ozgur,Raghavendra Ramachandra,Fadi Boutros,Naser Damer*

Main category: cs.CV

TL;DR: 本文提出DCMorph，一种基于双流扩散的换脸变形攻击框架，通过解耦交叉注意力插值和DDIM反演球形插值实现双身份条件控制，在四个SOTA人脸识别系统上达到最高攻击成功率且难以被现有检测方法识别。


<details>
  <summary>Details</summary>
Motivation: 现有图像级变形方法存在混合伪影，GAN方法重建保真度有限，而现有扩散方法缺乏显式双身份条件控制，亟需更先进的变形攻击技术以推动防御机制发展。

Method: 双流扩散框架：1) 解耦交叉注意力插值：将两张源人脸的身份特征注入去噪过程，实现双身份条件控制；2) DDIM反演+球形插值：在两源人脸的潜在表示间进行几何一致的球形插值，保留结构属性。

Result: 在四个SOTA人脸识别系统上，DCMorph在两种操作阈值下均达到最高攻击成功率，且现有变形攻击检测方案难以识别。

Conclusion: DCMorph通过双身份条件扩散机制有效提升了变形攻击效果，对当前人脸识别系统和攻击检测方案构成严峻挑战，强调了发展更强大防御机制的必要性。
...
### [65] [DualSplat: Robust 3D Gaussian Splatting via Pseudo-Mask Bootstrapping from Reconstruction Failures](https://arxiv.org/abs/2604.21631)
*Xu Wang,Zhiru Wang,Shiyun Xie,Chengwei Pan,Yisong Chen*

Main category: cs.CV

TL;DR: DualSplat通过"失败到先验"框架，将首次3DGS重建中由瞬态物体导致的失败转化为显式先验，结合光度残差、特征不匹配和SAM2边界构建物体级伪掩码，指导第二次干净重建，在瞬态物体密集场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临循环依赖：准确的瞬态物体检测需要已重建好的静态场景，而干净的重建又依赖于可靠的瞬态掩码。DualSplat旨在打破这一僵局。

Method: 1) 首次保守训练，让瞬态物体呈现不完整碎片；2) 结合光度残差、特征不匹配和SAM2实例边界构建物体级伪掩码；3) 利用伪掩码引导第二次干净的3DGS优化；4) 轻量级MLP在线优化掩码，从先验监督逐渐过渡到自一致性。

Result: 在RobustNeRF和NeRF On-the-go数据集上，DualSplat优于现有基线方法，在瞬态物体密集的场景和区域中展现出特别明显的优势。

Conclusion: DualSplat有效解决了3DGS在瞬态物体场景下的性能退化问题，通过将重建失败转化为有用先验，打破了瞬态检测与静态重建之间的循环依赖。
...
### [66] [Causal Disentanglement for Full-Reference Image Quality Assessment](https://arxiv.org/abs/2604.21654)
*Zhen Zhang,Jielei Chu,Tian Zhang,Weide Liu,Fengmao Lv,Tianrui Li,Jun Cheng,Yuming Fang*

Main category: cs.CV

TL;DR: 提出了一种基于因果推断和分离表示学习的新型全参考图像质量评估（FR-IQA）方法，通过解耦退化与内容表示、建模视觉掩蔽效应，在多种监督/无监督设置及跨领域场景中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有FR-IQA模型主要依赖深度特征的成对比较，缺乏对退化过程因果机制的解释。本文从因果推断角度出发，旨在将退化估计视为潜在表示上的干预引导的因果解耦过程，从而更本质地建模图像退化。

Method: 1) 利用参考图像与失真图像之间的内容不变性，解耦退化表示和内容表示；2) 受人类视觉掩蔽效应启发，设计掩蔽模块建模图像内容与退化特征之间的因果关系，提取内容影响的退化特征；3) 基于退化特征，采用监督回归或无监督降维预测质量分数。

Result: 在标准IQA基准上，全监督、少标签及无标签设置下均达到高度竞争性表现；在水下、放射、医学、中子、屏幕内容等非标准自然图像领域，凭借无需标签的场景特定训练能力，展现出优于现有无训练FR-IQA模型的跨领域泛化性能。

Conclusion: 本文提出的因果解耦范式为FR-IQA提供了新思路，显著增强了模型在数据稀缺场景下的适应性和跨领域迁移能力。
...
### [67] [Encoder-Free Human Motion Understanding via Structured Motion Descriptions](https://arxiv.org/abs/2604.21668)
*Yao Zhang,Zhuchenyang Liu,Thomas Ploetz,Yu Xiao*

Main category: cs.CV

TL;DR: 本文提出结构化运动描述（SMD），一种将关节位置序列转换为结构化自然语言描述的规则驱动方法，使LLM无需专用编码器即可直接进行运动推理，在运动问答和运动描述任务上超越现有SOTA方法，并展现出跨模型泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的运动理解方法依赖专用编码器进行运动特征与语言的对齐，受限于跨模态表征与对齐的瓶颈，未能充分利用LLM本身强大的世界知识和推理能力。

Method: 受生物力学分析启发，提出SMD（Structured Motion Description），一种基于规则的确定性方法，将关节位置序列转化为关节角度、身体部位运动及全局轨迹的结构化自然语言描述，从而实现以文本形式表征运动，使LLM直接调用预训练的身体部位、空间方向和运动语义知识。

Result: 在BABEL-QA和HuMMan-QA上分别达到66.7%和90.1%的运动问答准确率，在HumanML3D运动描述任务上R@1为0.584、CIDEr为53.16，均超越所有先前方法。在6个模型家族的8种LLM上仅需轻量LoRA适配即可工作，且支持可解释的注意力分析。

Conclusion: SMD通过将运动转化为结构化文本，绕过了跨模态对齐的复杂性，充分发挥了LLM预训练知识的潜力，在运动理解任务中实现了SOTA性能，同时具备跨模型泛化性和可解释性的实际优势。
...
### [68] [Sapiens2](https://arxiv.org/abs/2604.21681)
*Rawal Khirodkar,He Wen,Julieta Martinez,Yuan Dong,Su Zhaoen,Shunsuke Saito*

Main category: cs.CV

TL;DR: Sapiens2是一个0.4B-5B参数的高分辨率Transformer模型家族，通过统一预训练目标（掩码图像重建+自蒸馏对比学习）、10亿高质量人体图像预训练和架构改进（窗口注意力、4K分辨率支持），在人体姿态估计、身体部位分割、法线估计等任务上达到了SOTA，并扩展到点图、反照率估计等新任务。


<details>
  <summary>Details</summary>
Motivation: 现有的人体视觉模型在通用性、高保真度输出和多任务泛化能力上存在不足，尤其是对低层细节和高层语义的统一捕捉能力有限。作者希望构建一个涵盖大参数范围、支持高分辨率（1K-4K）的模型家族，以提升人体理解任务的性能和新任务的扩展性。

Method: 1）统一预训练目标：将掩码图像重建（捕捉低层细节）与自蒸馏对比学习（捕捉高层语义）结合；2）大规模数据：基于10亿高质量人体图像进行预训练，提升标注质量和数量；3）架构改进：融入前沿模型的长训练稳定性技巧，4K模型采用窗口注意力以推理更长的空间上下文，并以2K输出分辨率进行预训练。

Result: 在人体姿态估计上提升+4 mAP，身体部位分割提升+24.3 mIoU，法线估计角度误差降低45.6%，并在点图估计和反照率估计等新任务上取得优异表现。

Conclusion: Sapiens2通过在预训练目标、数据规模和架构三方面的协同改进，建立了人体视觉任务的新SOTA，验证了统一预训练目标（重建+对比）对多任务下游泛化的有效性。
...
### [69] [WorldMark: A Unified Benchmark Suite for Interactive Video World Models](https://arxiv.org/abs/2604.21686)
*Xiaojie Xu,Zhengyuan Lin,Kang He,Yukang Feng,Xiaofeng Mao,Yuanyang Yin,Kaipeng Zhang,Yongtao Ge*

Main category: cs.CV

TL;DR: WorldMark是首个为交互式图像到视频世界模型提供统一评估基准的平台，通过标准化的场景、动作序列和控制接口，支持六大主流模型的公平对比，并配套线下指标与在线竞技场。


<details>
  <summary>Details</summary>
Motivation: 现有交互式视频生成模型互不兼容，各模型在私有基准上评估，缺乏统一的场景、动作序列和控制接口，导致跨模型公平比较无法实现。

Method: (1) 统一动作映射层：将共享的WASD风格动作词汇转换为各模型原生控制格式；(2) 分层测试套件：500个测试案例，涵盖第一/第三人称、真实/风格化场景、Easy/Hard三档难度；(3) 模块化评估工具包：包括视觉质量、控制对齐、世界一致性三大指标，支持用户自定义评估指标。

Result: 构建了首个支持六大主流模型在相同场景和轨迹上直接对比的基准；发布了500个分层测试案例、评估代码及模型输出；上线了World Model Arena在线平台，支持实时对决和排行榜。

Conclusion: WorldMark填补了交互式世界模型缺乏统一评估标准的空白，通过标准化输入和可扩展的评估框架，为领域内公平比较和未来研究提供了基础平台。
...
TL;DR: 本文系统研究了前馈视觉几何估计的关键因素，发现数据多样性、损失函数设计和监督策略的重要影响，并提出CARVE模型通过一致性损失和高分辨率架构设计，在多项任务上实现强健性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈视觉几何估计方法中，多帧模型虽能产生更好的跨帧一致性，但在单帧精度上常弱于单帧方法，这一矛盾促使作者系统探究影响模型性能的关键因素。

Method: 通过严格的消融实验揭示关键因素，并提出两种增强：1）一致性损失函数，强制对齐深度图、相机参数和点图；2）高效利用高分辨率信息的架构设计。将这些设计整合为CARVE模型。

Result: 在点云重建、视频深度估计、相机姿态/内参估计等任务上，CARVE在多个基准上取得了强劲且鲁棒的性能。

Conclusion: 数据多样性和质量提升能进一步释放性能增益；常用置信度损失和梯度损失可能适得其反；联合序列级和帧级对齐监督有效，但局部区域对齐反而有害。CARVE验证了所提设计的有效性。
...
TL;DR: Ramen 是一种通过主动样本选择进行稳健测试时自适应的方法，专门应对混合域分布偏移问题。它利用嵌入-梯度缓存提高效率，并在多个基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法假设测试数据来自单一一致域，但在实际中测试数据常包含混合域样本，导致性能下降。因此需要一种能应对混合域偏移的鲁棒自适应方法。

Method: Ramen 针对每个测试样本，根据两个标准（域一致性和预测平衡）从历史数据中检索自定义批次的相关样本进行自适应。同时采用嵌入-梯度缓存存储过去测试图像的嵌入和样本级梯度，避免额外前向/反向传播。

Result: 在多个图像损坏和域偏移基准上，Ramen 实现了强且一致的表现，在复杂混合域场景中展现出鲁棒性和高效性。

Conclusion: 通过主动样本选择和缓存机制，Ramen 有效解决了混合域下的测试时自适应问题，理论分析也支持其有效性。
...
TL;DR: 本文提出基于生物行为特征的可解释深度伪造检测方法，发现面部动态中的高阶时间不规则性可区分真假视频，且情感表达场景下检测更准确。模型与人类判断在情感视频上一致，但在非情感视频上存在分歧。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法多基于深度学习黑箱模型，虽性能优异但缺乏可解释性，且很少与人类感知判断进行对比。本文旨在提出可解释的生物行为特征替代方案，并探究计算检测策略与人类感知之间的关系。

Method: 提取面部运动的低维时空特征，使用传统机器学习分类器进行深度伪造检测；同时对比模型决策与人类感知判断在情感/非情感视频上的一致性。

Result: 1) 基于生物行为特征的方法显著高于随机水平；2) 情感表达视频的检测准确率远高于非情感视频；3) 深度伪造中情感信号被系统性破坏；4) 模型与人类判断在情感视频上收敛，非情感视频上分歧。

Conclusion: 面部替换深度伪造携带可测量的行为指纹，尤其体现在情感表达中。可解释的计算特征与人类感知在检测路径上可能互为补充而非冗余。
...
TL;DR: 提出DOCO框架，通过在开放集持续测试时适应（OCTTA）场景中联合处理域偏移和语义类别未知问题，实现域适应与OOD检测的协同闭环，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法无法应对同时存在连续变化的域和未知语义类别的现实场景（即OCTTA），域偏移与语义偏移的耦合会导致特征空间坍塌，分类和OOD检测性能严重下降。

Method: 提出DOCO框架：1) 动态适应条件的样本分割将ID与OOD样本分离；2) 仅用ID样本学习域补偿提示（domain compensation prompt），通过对齐特征统计量与源域并引入结构保持正则化防止语义畸变；3) 将学到的提示传播到同批次OOD样本，隔离其语义新颖性以实现更可靠检测。

Result: 在多个具有挑战性的基准测试上，DOCO超越了先前的CTTA和OSTTA方法，在OCTTA设定下取得SOTA性能。

Conclusion: DOCO通过域适应与OOD检测的协同闭环设计，有效解决了开放集持续测试时适应这一更具挑战性的现实场景问题。
...
TL;DR: 提出了一种自监督框架，利用互联网单目视频生成伪多视图三元组（源视频、几何锚点、目标视频），通过随机游走裁剪策略和4D点云锚点，实现动态场景的高保真新视角合成和精确相机控制。


<details>
  <summary>Details</summary>
Motivation: 动态视频重拍中精确相机控制受限于非刚性场景的多视图配对数据严重匮乏。

Method: 从单目视频中提取随机游走裁剪轨迹生成源视图和目标视图，通过前向扭曲源视频第一帧合成几何锚点，构建伪多视图训练三元组；模型被迫学习4D时空结构，在推理时使用4D点云锚点的扩散Transformer。

Result: 在复杂动态场景上实现了最先进的时间一致性、鲁棒相机控制和高保真新视角合成。

Conclusion: 通过自监督方式利用单目视频学习4D时空结构，有效解决了动态场景多视图数据稀缺问题。
...
TL;DR: 本文研究了如何利用计算机视觉方法（特别是视觉语言模型）对社交媒体（X平台）上的气候传播图像进行系统分析，通过基准测试多种模型，发现Gemini-3.1-flash-lite表现最佳，并提出了分布级评估方法，证明VLM可在中等精度下可靠恢复总体趋势。


<details>
  <summary>Details</summary>
Motivation: 社交媒体已成为气候传播的主要阵地，但海量图像和帖文缺乏系统性分析方法，难以识别哪些传播策略能有效调动公众关注。作者旨在通过计算机视觉方法解决这一问题，为大规模话语分析提供可行工具。

Method: 设计了应用导向的分类体系，对6个可提示视觉语言模型（VLM）和15个零样本CLIP类模型进行基准测试，使用两大数据集（1,038张专家标注集和逾120万张图像的大规模语料库），覆盖5个标注维度（动物内容、气候变化后果、气候行动、图像场景、图像类型）。实验还包括思维链推理效果评估和提示设计优化。

Result: Gemini-3.1-flash-lite在所有超类别和两个数据集上均表现最佳，但与中等规模开源模型的差距较小。思维链推理反而降低性能，而针对标注维度的特定提示设计可提升性能。分布级评估表明，即使单图精度中等，VLM也能可靠恢复总体趋势。

Conclusion: 视觉语言模型可作为大规模气候传播话语分析的可行起点，分布级评估比逐图像精度更具实践价值。作者公开了推文ID、标签和代码以促进后续研究。
...
TL;DR: SyMTRS是一个大规模合成遥感数据集，提供高分辨率RGB图像、完美深度图、夜间图像及多尺度低分辨率变体，旨在统一支持单目深度估计、域适应和超分辨率等多任务研究。


<details>
  <summary>Details</summary>
Motivation: 真实遥感数据标注成本高且难以获取高质量的地面真值（如精确深度图、可控光照变化等），限制了深度估计、域适应和超分辨率等任务的发展。现有数据集多聚焦单一任务或模态，缺乏统一的多任务基准。

Method: 使用高保真城市仿真管线生成大规模合成数据集，包含2048x2048高分辨率RGB航拍图像、像素级完美深度图、夜间图像（域适应）以及x2/x4/x8对齐低分辨率变体（超分辨率）。

Result: 提供了一个统一的多任务基准，填补了几何理解、跨域鲁棒性和分辨率增强等领域的关键空白，支持带有完美几何真值和一致多域监督的受控实验。

Conclusion: SyMTRS通过合成数据解决了真实标注的瓶颈问题，为遥感多任务联合研究提供了可控实验平台，相关代码和数据集已开源。
...
TL;DR: 本文针对组合图像检索（CIR）中现有数据集仅包含简单修改文本、存在实体覆盖不足和从句-实体错位两个问题，构建了多修改指令数据集M-FashionIQ和M-CIRR，并提出了首个支持多修改的CIR框架TEMA（文本导向的实体映射架构），在四个基准数据集上验证了其优越性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有CIR研究过度依赖仅覆盖少量显著变化的简单修改文本，导致两个实际应用困境：实体覆盖不足（Insufficient Entity Coverage）和从句-实体错位（Clause-Entity Misalignment），亟需将CIR推向更贴近真实世界的复杂多修改场景。

Method: 1) 构建两个指令丰富的多修改数据集M-FashionIQ和M-CIRR；2) 提出TEMA（Text-oriented Entity Mapping Architecture），一种面向文本的实体映射架构，首次支持多修改场景同时兼容简单修改。

Result: 在四个基准数据集上的大量实验证明，TEMA在原始设置和多修改场景下均表现优越，并在检索精度与计算效率之间达到了最优平衡。

Conclusion: TEMA有效解决了现有CIR方法在复杂多修改场景下的局限性，推动了CIR向更真实应用场景迈进了一步。
...
TL;DR: 本文提出利用不同分辨率（互质像素尺寸）的低分辨率图像组合来解决超分辨率问题中的平移模糊性，通过傅里叶域或迭代最小二乘法实现稳定重构，并给出了噪声-分辨率权衡的数学分析。


<details>
  <summary>Details</summary>
Motivation: 超分辨率问题在平移下存在多义性，现有方法难以保证解的稳定性，需要一种理论上可证明的、具有稳定逆变换的多尺度超分辨率方法。

Method: 使用互质像素尺寸的传感器采集不同尺度的低分辨率图像，构建具有稳定逆的系统；采用傅里叶域技术和迭代最小二乘法进行高效重构；通过CCD硬件合并实现多种有效像素尺寸的实验验证。

Result: 互质像素尺寸的图像组合使得超分辨率问题良定；给出了大信号下最小二乘重构期望误差的理论表达式；一维和二维实验验证了方法有效性。

Conclusion: 多尺度超分辨率方法具有理论保证和实际可行性，能够有效克服平移模糊性，在常见成像系统中具有应用潜力。
...
TL;DR: 本文提出诊断驱动的胶囊内镜视频摘要新任务，构建首个含真实临床报告的CE数据集VideoCAP，并设计临床医生启发式框架DiCE，通过候选帧筛选、上下文组织和证据聚合生成简洁可靠的诊断摘要，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前胶囊内镜研究局限于帧级分类与检测，缺乏视频级分析；诊断相关事件极其稀疏（被数万正常帧淹没），且单个帧因运动模糊、碎片等存在歧义，亟待一种能从长视频中提取关键证据帧并作出准确诊断的方法。

Method: 提出DiCE框架：(1) 候选筛选模块高效过滤冗余正常帧；(2) Context Weaver将候选帧组织为保留不同病变事件的连贯诊断上下文；(3) Evidence Converger在每个上下文内聚合多帧证据，形成稳健的片段级判断。

Result: 在VideoCAP数据集上，DiCE在关键帧提取和诊断准确性上均一致优于现有最先进方法，生成的摘要简洁且临床可靠。

Conclusion: 诊断驱动的上下文推理是超长胶囊内镜视频摘要的一个有前景的新范式，值得进一步探索。
...
TL;DR: 本文提出了一个用于物理视频理解的基准测试，扩展了V-STaR的"什么-何时-何处"评估框架，涵盖4种视频源、6个物理领域、3种提示类型和4种输入条件，共1,560个视频片段。研究发现：物理提示表现最强，空间定位是最薄弱的环节，提示鲁棒性具有选择性而非普遍性。


<details>
  <summary>Details</summary>
Motivation: 现有的视频问答模型可能通过文本规律正确回答物理事件（如倾倒、滑动、碰撞），但无法在时间或空间上准确定位事件。需要建立一个真正基于物理理解而非文本记忆的评估基准。

Method: 将V-STaR的"什么-何时-何处"评估框架扩展到4种视频源（SSV2、YouCook2、HoloAssist、Roundabout-TAU）、6个物理领域、3种提示类型（physics、vstar_like、neutral_rstr）和4种输入条件（原始、打乱、消融、帧遮罩），共构建1,560个基础视频片段，每个片段转换为共享的接地事件记录。

Result: 物理提示在所有模型和提示类型中表现最强；vstar_like是非物理语义比较中最清晰的基线；neutral_rstr作为更难的控制模板。提示鲁棒性具有选择性而非普遍性，扰动增益集中在弱原始案例上，空间定位是各个设置中最薄弱的环节。
...
TL;DR: 本文提出一种方法，允许用户从相机直接输出的图像中恢复“未幻觉”版本，避免因AI ISP模块引入的幻觉内容导致图像语义误解。方法通过优化图像专用的MLP解码器和模态编码器实现，仅需180KB元数据即可嵌入标准图像格式。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模块（AI ISP）集成到相机硬件中，相机直出图像可能包含幻觉内容（如AI变焦、低光增强），这些内容可能改变图像语义，导致用户误以为图像内容完全真实。现有方法无法在拍摄后去除这些幻觉。

Method: 针对每张图像，联合优化一个图像专用的多层感知机（MLP）解码器和一个模态编码器。给定相机输出图像，编码器-解码器结构可恢复出幻觉添加前的原始图像。该模型仅需180KB，可存储为JPEG/HEIC元数据，无需访问相机ISP。

Result: 提出的方法能在拍摄后有效去除AI ISP引入的幻觉内容，恢复更真实的图像版本。模型轻量高效（180KB），可直接嵌入标准图像格式。

Conclusion: 该工作揭示了AI ISP可能破坏相机图像真实性的问题，并提供了实用的后处理解决方案，为图像取证和数字内容信任提供了新思路。
...
TL;DR: 提出UniGenDet统一生成-判别框架，通过共生多模态自注意力机制和统一微调算法，实现图像生成与生成图像检测的协同进化，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 图像生成与生成图像检测领域各自独立发展，架构差异大（生成网络 vs 判别框架），但都使用对抗信息提升性能，存在协同潜力。作者希望弥合架构鸿沟，实现两个任务的互利共生。

Method: 1) 提出UniGenDet统一生成-判别框架；2) 设计共生多模态自注意力机制（Symbiotic Multimodal Self-Attention）和统一微调算法；3) 引入检测器引导的生成对齐机制（Detector-Informed Generative Alignment），促进信息无缝交换。

Result: 在多个数据集上达到state-of-the-art性能。

Conclusion: 生成任务可提升真伪鉴别的可解释性，真伪判别标准可指导更高保真度图像的生成，两个任务通过统一框架实现协同进化。
...
TL;DR: 人类与深度视觉模型在分类准确率上相当，但在错误方向（谁被误认成谁）上存在系统性差异。本文通过混淆矩阵不对称性分析与率失真（RD）几何框架，揭示了两种智能体不同的归纳偏置特征：人类具有广泛而微弱的不对称性，而深度模型则表现出稀疏而强烈的方向性崩溃。


<details>
  <summary>Details</summary>
Motivation: 传统分类准确率无法揭示模型与人类在错误模式上的本质差异，即"谁被误认为谁"的方向性混淆。本文旨在量化这些方向性混淆，并建立其与归纳偏置和泛化几何之间的联系。

Method: 在12种扰动类型下的自然图像分类任务中，收集人类与深度视觉模型的配对响应数据；量化混淆矩阵的不对称性；通过率失真（RD）框架提取三种几何签名（斜率β、曲率κ和效率AUC）；进行鲁棒性训练对比与机制性仿真实验。

Result: 人类表现出广泛但微弱的不对称性；深度视觉模型表现出稀疏但强烈的方向性崩溃；鲁棒性训练能降低全局不对称性，但无法恢复人类式的广度-强度渐变相似性曲线；不同不对称性组织方式即使性能匹配，也会使RD前沿向相反方向移动。

Conclusion: 方向性混淆与率失真几何可作为分布偏移下归纳偏置的紧凑、可解释的签名，为理解人机智能差异提供了新的定量视角。
...
TL;DR: 本文提出HalluScope基准，发现LVLM幻觉主要源于文本先验和背景知识的过度依赖（尤其是文本指令）；并提出HalluVL-DPO框架，通过偏好优化微调模型以产生更符合视觉的响应，有效缓解了目标幻觉问题。


<details>
  <summary>Details</summary>
Motivation: LVLMs虽然能力提升迅速，但存在幻觉问题（输出与视觉输入不符）。先前研究将幻觉归因于视觉骨干限制或语言组件主导，但各因素的相对重要性尚不明确，因此需要系统性分析。

Method: 提出HalluScope基准分析幻觉来源；构建HalluVL-DPO框架，利用偏好优化和人工构造的训练数据集，微调现成LVLM以鼓励模型偏好基于视觉的响应而非幻觉响应。

Result: 优化后的模型有效缓解了由文本指令先验引发的幻觉，同时在其它幻觉基准和视觉能力评估上保持或提升了性能。

Conclusion: 幻觉主要源于对文本先验和背景知识的过度依赖；通过针对性的偏好优化可以缓解这一问题，且不影响模型在其他任务上的表现。
...
TL;DR: Vista4D提出了一个基于4D点云的视频重拍摄框架，通过静态像素分割和4D重建显式保留场景内容，在不同摄像机轨迹下实现鲁棒的动态场景重合成。


<details>
  <summary>Details</summary>
Motivation: 现有视频重拍摄方法在处理真实世界动态视频时存在深度估计伪影、内容外观保留不足以及对新相机轨迹控制不精准的问题。

Method: 构建4D点云表示，结合静态像素分割和4D重建显式保留已见内容；利用重建的多视图动态数据进行训练以增强对点云伪影的鲁棒性。

Result: 在多种视频和相机路径下，相比现有方法在4D一致性、相机控制和视觉质量上均有提升。

Conclusion: Vista4D不仅提升了重拍摄质量，还能泛化到动态场景扩展和4D场景重组等实际应用。
...
TL;DR: Omni是一个统一多模态模型，原生训练于文本、图像、视频、3D几何和隐藏表示等多种模态。其"上下文展开"机制让模型在多模态表示间显式推理，聚合互补信息，提升多模态生成和理解能力，并支持文本、图像、视频和3D的上下文生成。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常针对单一或少数模态设计，缺乏跨模态的联合推理能力。作者希望构建一个原生统一的多模态模型，使其能够在多种模态之间进行显式推理，从而更忠实地逼近共享的多模态知识流形。

Method: 提出Omni模型，直接在文本、图像、视频、3D几何和隐藏表示等多种模态上进行原生统一训练。关键在于"Context Unrolling"机制，模型在生成预测前会跨多个模态表示进行显式推理，聚合异构模态中的互补信息。

Result: Omni在多项多模态生成和理解基准上取得了强劲性能，展示了高级的多模态推理能力，包括文本、图像、视频和3D几何的上下文生成。

Conclusion: 原生多模态训练结合上下文展开机制能有效提升模型对共享多模态知识流形的逼近能力，增强下游推理的忠实度，证明了统一多模态建模的可行性。
...
TL;DR: 本文提出IMU-to-4D框架，利用可穿戴惯性传感器（如耳机、手表、手机中的IMU）重建人体运动与3D场景布局，无需视觉感知，并借助大语言模型实现非视觉的时空理解。


<details>
  <summary>Details</summary>
Motivation: 传统视觉感知在隐私、安全、能耗和可扩展性方面存在局限，因此探索仅利用可穿戴传感器进行4D感知（人体运动+场景结构）的替代方案。

Method: 提出IMU-to-4D框架，利用耳塞、手表或智能手机中的少量惯性传感器数据，结合大语言模型进行非视觉的时空理解，预测详细的4D人体运动与粗略场景结构。

Result: 在多样的人体-场景数据集上，IMU-to-4D比现有级联管道产生更连贯、时间上更稳定的结果。

Conclusion: 仅靠可穿戴运动传感器就能支持丰富的4D理解，为无视觉感知的人体-场景动态理解提供了可行方案。
...
TL;DR: 本文研究视频中时间作为可学习视觉概念，通过自监督方式学习检测速度变化和估计播放速度，进而构建大规模慢动作视频数据集，并开发速度可控的视频生成和时间超分辨率模型。


<details>
  <summary>Details</summary>
Motivation: 视频中的时间流逝感知和控制长期被忽视，现有研究缺乏对视频时间维度的系统性建模与操控能力。

Method: 1) 利用视频中的多模态线索和时间结构，以自监督方式学习速度变化检测和播放速度估计；2) 利用学习到的时序推理模型从嘈杂的互联网视频中构建大规模慢动作数据集；3) 基于该数据集开发速度条件视频生成和时间超分辨率模型。

Result: 构建了迄今为止最大规模的慢动作视频数据集，实现了速度可控的视频生成（产生指定播放速度的运动）和时间超分辨率（将低帧率模糊视频转换为高帧率精细时序细节视频）。

Conclusion: 时间是一个可操作、可感知的视频学习维度，为时序可控视频生成、时序取证检测及更丰富的世界模型理解事件演化打开了新方向。
...
TL;DR: 本文提出AITP，一个结合多模态思维链和检索增强生成的多模态大语言模型，用于交通事故责任推理与分配，并构建了包含近20万问答对的大型基准数据集DecaTARA，实验表明其在责任分配、事故检测和理解任务上均达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在交通事故检测与理解方面取得了进展，但缺乏深层因果推理和法律知识整合能力，无法完成需要多步推理和交通法规支持的交通事故责任分配任务。

Method: 提出AITP模型，通过多模态思维链机制增强推理能力，利用检索增强生成整合法律知识；同时构建DecaTARA基准数据集，包含10项关联任务、6.7万标注视频和19.5万问答对。

Result: AITP在责任分配、事故检测和事故理解任务上均达到最先进水平，验证了多模态推理驱动交通分析的新范式的有效性。

Conclusion: 该工作首次将深层因果推理和法律知识引入交通事故责任分配任务，AITP及其基准数据集为推理驱动的多模态交通分析建立了新范式。
...
TL;DR: 本文提出AFRILANGDICT（194.7K非洲语言-英语词典条目）和AFRILANGEDU（78.9K多轮对话数据集），用于训练非洲低资源语言的AI语言辅导模型AFRILANGTUTOR。实验表明，基于Llama-3-8B-IT和Gemma-3-12B-IT微调的模型在10种非洲语言上显著优于基线，SFT+DPO组合带来1.8%-15.5%的提升。


<details>
  <summary>Details</summary>
Motivation: 非洲大陆许多语言缺乏足够的训练资源，难以构建能理解和回应本地语言的AI系统，亟需低资源语言学习方法。

Method: 1) 构建AFRILANGDICT（194.7K词典条目）作为种子资源；2) 自动生成大规模、多样化、可验证的学生-导师问答对，形成AFRILANGEDU（78.9K多轮训练样例）；3) 在10种非洲语言上对Llama-3-8B-IT和Gemma-3-12B-IT进行SFT和DPO微调，得到AFRILANGTUTOR。

Result: 微调模型全面超越基线模型；SFT与DPO结合带来显著提升，LLM-as-a-judge评估下四项指标增益范围为1.8%至15.5%。

Conclusion: AFRILANGDICT和AFRILANGEDU有效缓解了非洲低资源语言的数据稀缺问题，为AI语言辅导系统提供了可行方案，所有资源已开源。
...
TL;DR: 本文提出层次化策略优化（HPO）方法，通过后训练优化基于不完美SFT数据的同声翻译模型，在保持1.5秒延迟下显著提升翻译质量（COMET+7, MetricX+1.25）。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的同声翻译方法虽质量高但计算开销大，而多轮对话形式的SST虽能复用KV缓存降低开销，但缺乏人工标注的对话形式SFT数据，现有合成方法又无法保证数据质量。

Method: 提出层次化策略优化（HPO）方法，对基于不完美SFT数据训练的模型进行后训练；引入层次化奖励函数，平衡翻译质量与延迟两个目标。

Result: 在英中、英德、英日三个翻译方向上，延迟1.5秒条件下COMET分数提升超过7分，MetricX分数提升超过1.25分。

Conclusion: HPO方法有效缓解了SFT数据质量不足的问题，并通过消融实验验证了不同质量奖励、层次化奖励设计及切分策略的有效性。
...
TL;DR: 本文提出TRACES框架，通过对推理步骤进行实时标记，实现大语言模型的自适应、成本高效的早停机制，在数学和知识推理基准上实现20-50%的token减少，同时保持与标准生成相当的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有语言推理模型（LRMs）虽然在推理能力上有所进步，但仍然存在效率低下的问题（过度生成验证和反思步骤），且不同推理步骤类型对生成正确答案的贡献尚未得到充分探索。

Method: 提出TRACES框架，对推理步骤进行实时标记，监控推理行为。发现LRMs在达到正确答案后会改变推理行为模式，基于特定步骤类型的监控可构建有效的可解释早停标准。

Result: 在MATH500、GSM8K、AIME（数学推理）和MMLU、GPQA（知识与推理）五个基准上评估，实现20-50%的token缩减，同时保持与标准生成相当的准确性。

Conclusion: TRACES框架能够通过实时标记推理步骤实现自适应早停，显著降低计算成本且不影响模型准确性，为提升语言推理模型的效率提供了有效方案。
...
TL;DR: 提出一种基于离散小波变换（DWT）的多分辨率框架，将文本视为语义信号进行分解，生成既能保留全局结构又能保留局部细节的紧凑摘要表示，在临床和法律领域文档上提升了语义相似性、事实一致性和忠实度，同时减少了幻觉。


<details>
  <summary>Details</summary>
Motivation: 长文档、专业领域（如临床、法律）的摘要面临上下文限制、信息丢失和幻觉问题，现有LLM方法难以兼顾全局结构和领域细节。

Method: 将文本嵌入视为语义信号，利用离散小波变换（DWT）分解为全局近似分量和局部细节分量，生成紧凑的多分辨率表示，可直接作为摘要或引导LLM生成摘要。

Result: 在临床和法律基准上，相比GPT-4o基线，BERTScore提升超2%，语义忠实度提升超4%，法律任务事实一致性提升，METEOR大幅提升，忠实度最高达97%。

Conclusion: DWT是一种轻量级、可泛化的方法，可作为语义去噪机制减少幻觉、增强事实基础，适用于长文档和领域特定摘要任务。
...
TL;DR: 本文系统比较了四种FHIR序列化策略对LLM在药物重整任务中的影响，发现8B以下模型使用临床叙事效果最佳（F1提升最多19分），而70B模型直接使用Raw JSON表现最好（F1达0.9956）。遗漏是主要错误模式，且小模型在多药患者上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 药物重整（medication reconciliation）在临床交接环节中错误频发、风险极高。虽然大语言模型被越来越多地用于辅助这项任务，但FHIR结构化数据在输入模型前的序列化方式这一基础变量尚未被系统研究。本文首次填补了这一空白。

Method: 在200名合成患者的受控基准上，对四种FHIR序列化策略（Raw JSON、Markdown Table、Clinical Narrative、Chronological Timeline）和五个开源模型（Phi-3.5-mini、Mistral-7B、BioMistral-7B、Llama-3.1-8B、Llama-3.3-70B）进行了总计4000次推理的系统比较。

Result: 1) 序列化策略对8B以下模型有显著影响，Clinical Narrative相较Raw JSON在Mistral-7B上提升最多19 F1分；2) 70B模型下Raw JSON最佳（F1=0.9956）；3) 所有组合中精确率均高于召回率，遗漏是主要错误模式；4) 小模型在7-10种并发药物时达到瓶颈，多药患者系统性地被忽视；5) BioMistral-7B在所有条件下均无可用输出。

Conclusion: 为临床LLM部署提供了基于证据的格式建议：8B以下模型推荐Clinical Narrative，70B及以上推荐Raw JSON。同时指出领域预训练不足以替代指令微调，且临床安全审计应优先关注遗漏而非误报。
...
TL;DR: 本文提出在医学报告生成的视觉-语言模型训练中，使用加权损失函数替代标准交叉熵损失，通过聚焦具有临床重要性的语义token，在眼科报告生成任务上实现了最高10倍的数据效率提升。


<details>
  <summary>Details</summary>
Motivation: 训练医学报告生成的视觉-语言模型常受限于高质量标注数据的稀缺性，传统交叉熵损失平等对待所有token预测错误，无法有效利用有限数据中的关键临床信息。

Method: 提出加权损失函数（reweighted loss），将训练焦点转移到具有显著临床重要性的语义token上，替代标准交叉熵损失。

Result: 在眼科报告生成实验中，该方法在多种数据尺度下均提升了数据效率，仅用十分之一的训练数据即可达到与使用全部数据相当的报告质量。

Conclusion: 加权损失函数是一种简单有效的方法，可显著提升医学报告生成模型在数据稀缺条件下的训练效率。
...
TL;DR: 该研究利用MARBERT模型对阿拉伯语推文中的表情符号进行预测，基于8695条多方言阿拉伯语推文数据，实现了0.75的总体准确率。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习在阿拉伯语（特别是多方言、低资源语言）表情符号预测中的应用潜力。

Method: 收集11379条阿拉伯语推文，清洗后得到8695条有效数据；将表情符号分为14类并数值编码；设计预处理流程作为可解释基线；微调MARBERT模型进行表情符号预测。

Result: 模型在精度、召回率和F1分数上表现良好，总体准确率达到0.75。

Conclusion: 研究结果具有前景，但包括MARBERT在内的机器学习模型在处理低资源、多方言语言（如阿拉伯语）时仍需进一步改进。
...
TL;DR: GRISP是一种基于微调小语言模型的SPARQL问答方法，通过先生成查询骨架，再迭代替换自然语言占位符来回答知识图谱问题，在多个基准测试上取得领先效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的SPARQL问答方法资源消耗大，而小语言模型在复杂查询生成上表现不足。作者希望通过微调小语言模型并引入迭代式知识图谱项选择机制，在保持高效的同时提升问答准确率。

Method: 1) 使用微调的小语言模型生成自然语言形式SPARQL查询骨架；2) 利用知识图谱约束迭代地重排序和选择知识图谱项，替换骨架中的自然语言占位符；3) 联合训练模型于骨架生成和列表式重排序任务。

Result: 在Wikidata和Freebase标准基准测试中，GRISP在可比设定下取得了优于其他最先进方法的效果。

Conclusion: 通过将查询生成分解为骨架生成和迭代填充两个阶段，小语言模型也能在SPARQL问答任务上超越大模型方法，展示了高效轻量级方案的有效性。
...
TL;DR: 本文提出IVG框架，结合规格自查（查询底层规范）和视图交互（操作视图消除歧义）来解决VLM在图表理解中的像素瓶颈，并构建iPlotBench基准测试，实验表明该方法在QA准确率上达0.81，重叠几何体上提升+6.7%。


<details>
  <summary>Details</summary>
Motivation: 现有VLM仅依赖像素级图像解析，无法获取图表的结构化规范，导致数值误读、细节幻觉和重叠元素混淆等问题，形成“纯像素瓶颈”。

Method: 提出IVG框架，包含两个核心模块：(1) 规范引导的自查——查询底层图表规范获取确定性证据；(2) 视图引导的交互——通过操作视图（如缩放、悬停）消除视觉歧义。同时构建iPlotBench基准（500个交互式Plotly图表，6706个二元问题及真实规范）。

Result: 自查提升数据重建保真度，自查+交互组合达到最高QA准确率0.81，在重叠几何体上获得+6.7%的提升。并在自主数据探索和实时人机协作代理中验证了有效性。

Conclusion: 将结构化规范查询与视觉交互结合能有效突破纯像素瓶颈，显著提升VLM在图表理解任务上的准确性，尤其是处理重叠元素时效果明显。
...
TL;DR: 本文提出自动化课堂话语分析系统(ADAS)，结合LLM数据增强与双探头RoBERTa分类器，实现课堂话语的双维度自动化编码，并揭示教师追问(Fq)是学生推理推理(SR-I)的最稳定前因。


<details>
  <summary>Details</summary>
Motivation: 科学课堂中学生推理模式分析对理解知识建构机制至关重要，但人工编码课堂话语耗时费力且难以规模化，亟需自动化解决方案。

Method: (1) 对标注语料进行分层重划分；(2) 基于LLM的合成数据增强针对少数类；(3) 训练双探头头的RoBERTa-base分类器；配合零样本GPT-5.4基线对比，并进行话语模式分析（UT×RC共现、CCI计算、滞后序列分析、IRF链分析）。

Result: 零样本GPT-5.4在UT上macro-F1=0.467，RC上macro-F1=0.476；LLM数据增强有效提升UT少数类识别；RC任务结构简单，词法基线即可处理。

Conclusion: 自动化话语分析可行且有效，教师Feedback-with-Question(Fq)是触发学生推理推理的最关键教学行为，为课堂认知参与研究提供了可扩展的自动化工具支持。
...
TL;DR: 该研究通过多槽位探针方法发现，语言模型在残差流中采用正交的"当前实体"和"先前实体"两种槽位来编码不同实体信息，其中先前实体槽位虽包含可用信息，但模型仅在特定推理任务（如关系推断、冲突检测）中使用，而不会将其用于显式事实检索。同时发现，开放权重模型在需要单token绑定两个主谓宾结构的任务上表现接近随机，但前沿模型已具备这种能力。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型如何在多个token位置上表示多个实体，以及单个token能否同时携带多个实体的绑定关系，揭示激活值中可用信息与模型实际使用信息之间的差距。

Method: 提出多槽位探针方法，将单个token的残差流激活分解为"当前实体"和"先前实体"两个正交槽位，并通过实体级别归纳推理、冲突检测和显式事实检索等任务分析各槽位的功能角色。

Result: 当前实体和先前实体槽位是正交的且功能不同；先前实体槽位支持关系推断和冲突检测，但模型不使用其进行显式事实检索；开放权重模型难以处理单token绑定两个主谓宾结构的语法，而前沿模型表现出色。

Conclusion: 激活值中可用的信息与模型实际使用的信息之间存在差距；当前/先前实体槽位结构可能是支持需要同时持有两种视角的行为（如谄媚和欺骗）的自然基础。
...
TL;DR: 本文提出主动视觉脚手架框架，通过将对话状态转换为持久性视觉历史来解决对话代理在情境对话中的"表征模糊"问题。实验表明，增量式外部化优于全对话推理，视觉脚手架能减少表征模糊并强制具体场景承诺，而混合多模态设置取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 当前对话代理在需要超越即时上下文窗口维护共同基础时，常将细粒度区分压缩为纯文本表征，导致"表征模糊"——相似但不同的实体坍缩为可互换的描述，造成虚假的局部连贯性。

Method: 受人类心智图像启发，提出主动视觉脚手架框架：在对话过程中增量地将对话状态转换为持久的视觉历史记录，后续检索用于生成基于上下文的响应。

Result: 在IndiRef基准上，增量式外部化本身优于全对话推理；视觉脚手架进一步减少表征模糊并强制具体场景承诺；纯文本对不可描绘信息有优势；混合多模态设置取得整体最优性能。

Conclusion: 对话代理受益于整合描绘性与命题性信息的显式多模态共同表征。
...
TL;DR: 本文通过在美国四个方言区（亚特兰大、墨西哥湾沿岸、迈阿密海滩、图森）的用户体验研究，揭示了自动语音识别（ASR）系统对非标准英语方言使用者的深层伤害：用户不仅感到沮丧和不适，还进行大量隐性劳动（语码转换、超清晰发音、情绪管理）来使系统正常工作，同时将系统失败内化为个人能力不足，尽管他们意识到系统并非为其设计。研究表明仅凭准确率等指标评估算法公平性会严重忽视情绪劳动、认知负担和心理伤害等关键维度。


<details>
  <summary>Details</summary>
Motivation: 现有ASR偏差研究主要关注不同方言使用者的错误率差异，忽视了系统失败如何塑造用户的真实体验、情感反应以及反复失败带来的情绪代价。本文旨在从人文视角填补这一空白。

Method: 在美国四个代表不同英语方言社区的地点（亚特兰大、墨西哥湾沿岸、迈阿密海滩、图森）进行用户体验研究，通过定量问卷和定性开放式叙述分析收集数据。

Result: 多数参与者报告技术未考虑其文化背景、需不断调整才能实现基本功能；他们对ASR性能保持高期望并愿意为模型改进做贡献；系统失败引发沮丧、烦恼和不胜任感，用户进行语码转换、超清晰发音和情绪管理等大量隐性劳动，并将失败内化为个人不足。

Conclusion: 仅以准确率指标衡量算法公平性会严重缺失伤害的关键维度——情绪劳动、认知负担和在自己母语变体中感到不足的心理代价。ASR公平性评估必须纳入人类体验视角。
...
TL;DR: 本文提出前缀语法变换，将前缀解析问题转化为普通解析问题，通过构造生成原语法所有前缀的新语法，并利用算法微分计算下一个词元的权重向量，实现简单、通用且高效的前缀解析框架。


<details>
  <summary>Details</summary>
Motivation: 前缀解析在上下文无关语言建模、心理语言学分析和基于大型语言模型的句法约束生成中至关重要，但缺乏一种统一、高效的通用解法。

Method: 提出前缀语法变换：将原语法转换为生成所有前缀的新语法，使任意普通解析算法可直接应用于前缀解析；同时结合算法微分计算下一个词元的权重向量。

Result: 变换后的语法仅比原语法大一个较小的常数因子，且可直接利用现有优化实现，无需专门的前缀解析算法。

Conclusion: 该框架简单、通用且高效，消除了对定制前缀解析算法的需求，为相关应用提供了实用工具。
...
TL;DR: 本文提出了一种新颖的推理方法来增强大语言模型在未来职业预测中的表现。该方法首先生成用户"理由"（总结其教育/职业经历中的偏好），再用该理由进行下一职业预测。通过LLM-as-a-Judge获取高质量理由来微调小模型。实验表明该方法在预测准确率上达到监督方法水平，且联合微调优于分开微调。


<details>
  <summary>Details</summary>
Motivation: LLM在职业预测任务中表现不佳，因为LLM与职业路径或职业决策背后不可观察的理由不对齐。现有方法未能充分利用推理能力来捕捉用户职业偏好。

Method: 两阶段方法：(1) 理由生成器根据用户教育和工作历史生成"理由"（概括用户偏好）；(2) 职业预测器以理由为输入预测下一职业。使用LLM-as-a-Judge按真实性、连贯性和实用性标准提取高质量理由，并用这些理由微调小型LLM。

Result: (a) 方法使LLM预测准确性接近全监督方法，优于无监督方法；(b) 单个LLM联合微调优于两个分开微调的LLM；(c) 预测准确率取决于生成理由的质量。

Conclusion: 通过生成高质量理由并微调小模型，可以有效提升LLM在未来职业预测任务上的表现，且联合训练更高效。
...
TL;DR: 本文提出SPIA基准，将文本匿名化评估从基于文本片段转移到个体层面，发现即使90%以上的PII片段被屏蔽，主题级推理保护率可低至33%，且非目标主体比目标主体暴露更严重。


<details>
  <summary>Details</summary>
Motivation: 现有文本匿名化评估依赖基于文本片段的指标，无法捕捉攻击者实际能推断出的信息，且假设只有一个数据主体，忽略了多主体场景。

Method: 提出SPIA（Subject-level PII Inference Assessment）基准，将评估单位从文本片段转向个体，包含675份法律和在线领域文档，并引入新颖的主体级保护指标。

Result: 超过90%的PII片段被屏蔽时，主体级推理保护率低至33%；针对目标主体的匿名化导致非目标主体比目标主体暴露更严重。

Conclusion: 主体级基于推理的评估对于保障现实世界中安全的文本匿名化至关重要。
...
TL;DR: 本文提出IRM方法，利用隐式奖励模型以零样本方式检测LLM生成文本，无需偏好收集或额外训练，在DetectRL基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: LLM生成的文本可能被滥用，需要可靠的检测方法。现有方法需要偏好构建或任务特定微调，成本高且泛化性差。

Method: 提出IRM方法，从公开的指令微调模型和基础模型中提取隐式奖励模型，以零样本方式检测LLM生成文本，无需偏好数据收集和额外训练。

Result: 在DetectRL基准上，IRM在检测LLM生成文本任务上优于现有零样本方法和监督方法，取得了最先进的检测性能。

Conclusion: IRM是一种高效、无需训练的零样本检测方法，能够可靠区分人类与LLM生成文本，为文本溯源提供新思路。
...
TL;DR: 本文提出了EngramaBench基准，用于评估大语言模型在多会话中的长时对话记忆能力。Engrama（图结构记忆）在跨空间推理上超越GPT-4o全上下文，但整体效果仍低于后者。Mem0最便宜但表现最差。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型助手缺乏对跨会话长期记忆的评估基准，作者希望系统性地衡量不同记忆架构在多种推理任务上的表现差异。

Method: 设计EngramaBench基准，包含5个人物、100个多会话对话和150个查询，覆盖事实回忆、跨空间整合、时间推理、对抗性回避和新兴综合五种任务。对比Engrama（图结构记忆）、GPT-4o全上下文提示和Mem0（向量检索记忆）三种记忆架构，统一使用GPT-4o作为回答模型。

Result: GPT-4o全上下文综合得分最高（0.6186）；Engrama全球得分0.5367，但在跨空间推理上超越全上下文（0.6532 vs 0.6291）；Mem0得分最低（0.4809）但成本最低。消融实验显示Engrama的跨空间优势与全局分数之间存在权衡。

Conclusion: 结构化记忆系统在特定推理任务上可以超越全上下文提示，但存在系统级的性能权衡，为未来长时对话记忆架构的设计提供了方向。
...
TL;DR: 本文提出LLM4MEM框架，利用大语言模型解决多表实体匹配中的语义不一致、匹配效率低和噪声实体问题，包含多风格提示协调模块、传递一致性嵌入匹配模块和密度感知剪枝模块，在6个数据集上F1值平均提升5.1%。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练语言模型的方法难以处理数值属性变化导致的语义不一致问题，且多数据源带来的实体数量激增和噪声实体影响匹配质量。

Method: 1) 多风格提示增强的LLM属性协调模块处理语义不一致；2) 传递一致性嵌入匹配模块解决实体嵌入和预匹配问题；3) 密度感知剪枝模块优化匹配质量。

Result: 在6个MEM数据集上，F1值相比基线模型平均提升5.1%。

Conclusion: LLM4MEM有效利用大语言模型能力解决了多表实体匹配的关键挑战，代码已开源。
...
TL;DR: PLOTTER是一个基于结构图表示（事件图和角色图）进行叙事规划的框架，通过"评估-规划-修正"循环优化图拓扑结构，在逻辑约束下修复因果和叙事骨架问题，显著提升了LLM生成长篇连贯叙事的能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在叙事生成中难以维持全局连贯性、上下文逻辑一致性和角色平滑发展，常产生结构断裂的单调剧本。

Method: 提出PLOTTER框架，在事件图和角色图上执行Evaluate-Plan-Revise循环，通过诊断和修复图拓扑结构问题，在严格逻辑约束下优化因果关系和叙事骨架，再进行完整上下文生成。

Result: 在多种叙事场景下显著优于代表性基线方法。
...
TL;DR: 本文提出两个新指标（RPS和AGS）来检测LLM agent模型蒸馏导致的行为同质化现象，发现许多智能体行为趋同，且Kimi-K2在工具使用模式上甚至超越了其教师模型。


<details>
  <summary>Details</summary>
Motivation: 现有指标无法区分任务必需的"强制性行为"和反映模型自主偏好的"非强制性行为模式"，导致无法有效检测模型蒸馏带来的行为同质化问题。

Method: 提出RPS（言语对齐度量）和AGS（工具使用习惯的有向图建模）两个互补指标，在τ-Bench和τ²-Bench上评估了8家提供商的18个模型，并以Claude Sonnet 4.5为教师进行受控蒸馏实验。

Result: 同族模型对AGS得分比跨族高5.9个百分点；Kimi-K2的节点相似度达82.6%、依赖相似度达94.7%，甚至超过Anthropic自己的Opus 4.1；RPS和AGS相关性较低（Pearson r=0.491），表明它们捕捉了不同的行为维度。

Conclusion: RPS和AGS能有效区分教师特异性收敛与一般性改进，为检测agent生态中的行为趋同提供了互补的诊断信号。
...
TL;DR: 在Transformer上先预训练音乐再训练语言，能显著加速语言习得（困惑度降低17.5%），且这种优势是持久的而非短暂的领先。


<details>
  <summary>Details</summary>
Motivation: 探索非语言数据（音乐）作为语言模型预训练基底的可行性，以及结构化人类创意输出能否为小型语言模型提供高效的预训练素材。

Method: 采用MAESTRO钢琴数据集，构建"音乐→诗歌→散文"的发展式训练流程，在不同模型规模（d=16/32/64）下进行多种子验证实验。

Result: 音乐+诗歌预训练带来17.5%的困惑度改善（p<0.001）；在d=64时，平台期仍保持5.5%的持续优势（p=0.017）；真实音乐仅用1/3数据即可达到合成模式的迁移天花板；最优预训练数据量随模型容量变化。

Conclusion: 结构化人类创意输出可作为小型语言模型的高效预训练基底，并暗示了容量依赖的数据筛选原则，但结论能否推广到现代大规模预训练仍需更大规模实验验证。
...
TL;DR: 本文评估了三种架构（CTC、编码器-解码器、LLM解码器）共9个语音识别模型在5个人口统计维度上的公平性与鲁棒性。主要发现：LLM解码器并未放大种族偏见；Whisper对印度口音存在病理性幻觉；音频压缩比LLM规模更能预测口音公平性；在声学退化条件下，静音注入会放大Whisper的口音偏见，而掩码会导致Whisper进入灾难性重复循环；最终结论是音频编码器设计比LLM缩放更重要。


<details>
  <summary>Details</summary>
Motivation: 探索大规模语言模型在语音识别中是否会因其文本先验而导致不同人口群体间的识别偏差扩大或缩小。

Method: 使用Common Voice 24和Fair-Speech数据集，在约43,000条语音样本上评估9个跨三代架构的模型，并在12种声学退化条件下进行216次推理测试。

Result: LLM解码器未放大种族偏差；Whisper对印度口音存在病理性幻觉（插入率非单调飙升到9.62%）；音频压缩比LLM规模更能预测口音公平性；严重退化导致所有群体WER趋同；静音注入将Whisper口音偏差放大至4.64倍；掩码导致Whisper灾难性重复循环（86%的插入为重复），而显式LLM解码器插入减少38倍且几乎无重复。

Conclusion: 音频编码器设计（而非LLM缩放）是实现公平且鲁棒语音识别的主要杠杆。
...
### [113] [VLAA-GUI: Knowing When to Stop, Recover, and Search, A Modular Framework for GUI Automation](https://arxiv.org/abs/2604.21375)
...
TL;DR: VLAA-GUI是一个模块化GUI智能体框架，通过三个核心组件（Completeness Verifier、Loop Breaker、Search Agent）解决早期停止和重复循环两大问题，在OSWorld和WindowsAgentArena上达到最优性能，且多个骨干模型超越人类水平。


<details>
  <summary>Details</summary>
Motivation: GUI自主智能体面临两大根本挑战：1) 早期停止——智能体在没有可验证证据的情况下过早声明成功；2) 重复循环——智能体不断重复相同的失败操作而无法恢复。

Method: 提出VLAA-GUI模块化框架，包含三个集成组件：1) Completeness Verifier——强制在每一步完成时验证UI可见的成功标准，通过智能体级验证器交叉检查完成声明与决策规则；2) Loop Breaker——多层过滤机制，在重复失败后切换交互模式，在持久屏幕状态重复后强制策略变更；3) Search Agent——按需搜索不熟悉的工作流程。另集成Coding Agent和Grounding Agent作为补充。

Result: 在OSWorld上达77.5%，WindowsAgentArena上达61.0%，均为最高性能；五个骨干模型中有三个单次通过超越人类表现（72.4%）；消融实验表明三个组件均能稳定提升强骨干模型；Loop Breaker为易循环模型减少近一半的浪费步骤。

Conclusion: VLAA-GUI通过强制验证、循环打破和按需搜索三个模块化组件，有效解决了GUI智能体的核心痛点，验证了结构化干预对提升自主智能体可靠性的重要作用。
...
### [114] [Cross-Entropy Is Load-Bearing: A Pre-Registered Scope Test of the K-Way Energy Probe on Bidirectional Predictive Coding](https://arxiv.org/abs/2604.21286)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: Cacioli (2026)的K-way能量探针在标准判别性预测编码网络下约化为log-softmax边际的单调函数，依赖于五个假设（含输出层交叉熵CE和前馈推理）。本研究通过去除CE（改用MSE）和使用双向PC（bPC）测试该约化的鲁棒性。在CIFAR-10上使用2.1M参数模型，发现：标准PC复制了负结果（探针低于softmax）；bPC使探针超过softmax；去除CE将差距减半。事后温度缩放消融显示约66%差距来自logit尺度效应，34%来自CE表征的尺度不变排序优势。


<details>
  <summary>Details</summary>
Motivation: 检验Cacioli (2026)理论约化中对交叉熵（CE）假设的依赖性，探究该约化在移除CE或改用双向PC（bPC）时是否仍然成立。通过实证确定CE在支持探针-softmax分解中的具体贡献比例。

Method: 预注册研究，在CIFAR-10上用2.1M参数骨干网络进行三个实验：(1)标准PC用MSE替代CE；(2)使用双向PC（bPC）；(3)事后温度缩放消融。比较K-way能量探针与softmax输出的差距（Delta），并分析logit范数、潜在移动等指标。

Result: 标准PC复制负结果（Delta=-0.082）；bPC使探针超过softmax（Delta=+0.008, p=0.000027），但bPC的潜在移动比未达预期阈值（ratio=1.6<10）；去除CE使差距减半（Delta_MSE=-0.037）；CE训练的logit范数约为MSE/bPC的15倍；温度缩放消融显示66%差距来自尺度效应，34%为CE表征的固有排序优势。

Conclusion: 交叉熵（CE）是Cacioli (2026)理论约化中经验上关键的支撑组件。约化的大部分（约66%）源于CE导致的logit尺度放大效应（可通过温度缩放消除），约三分之一来自CE表征本身的尺度不变优势。从标准PC改为bPC可逆转探针与softmax的关系，但机制与CE无关。
...
### [115] [Explainable Disentangled Representation Learning for Generalizable Authorship Attribution in the Era of Generative AI](https://arxiv.org/abs/2604.21300)
*Hieu Man,Van-Cuong Pham,Nghia Trung Ngo,Franck Dernoncourt,Thien Huu Nguyen*

Main category: cs.CL

TL;DR: 本文提出EAVAE框架，通过结构分离设计和可解释判别器，有效解耦作者风格与内容，在 authorship attribution 和 AI文本检测任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以解决内容-风格纠缠问题，模型会学习到作者风格与主题之间的虚假相关性，导致跨领域泛化能力差。

Method: EAVAE首先通过监督对比学习预训练风格编码器，再使用VAE架构分别编码风格和内容表示，并通过一个可生成自然语言解释的判别器强制解耦，同时提升可解释性。

Result: 在Amazon Reviews、PAN21、HRS等数据集上取得author attribution SOTA；在M4数据集的AI文本检测任务中，少样本学习表现优异。

Conclusion: EAVAE通过结构分离设计有效解耦风格与内容，并在多个基准上验证了其优越性与可解释性。
...
TL;DR: 本文发现，现有代码生成偏差评估主要基于简单的条件语句（如if语句），但这严重低估了实际偏差。通过评估机器学习流水线生成任务，研究发现敏感属性在87.7%的流水线中被包含，远高于条件语句的59.2%，且该结果对多种缓解策略和设置鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成偏差评估仅使用简单条件语句，这无法反映真实编程场景，导致偏差被严重低估。作者希望探索更真实的编程任务（ML流水线）来揭示实际的偏差程度。

Method: 让代码专用和通用大语言模型生成完整的机器学习流水线，检查特征选择阶段是否包含敏感属性（如种族、性别等）。同时对比条件语句任务中的偏差程度，并进行多种鲁棒性分析（提示缓解策略、属性数量、任务难度）。

Result: 在ML流水线中，敏感属性平均出现在87.7%的情况下，而条件语句仅为59.2%。模型能够排除无关特征（如"最喜欢的颜色"），但依然保留敏感属性。该结果在不同提示策略、属性数量和任务难度下保持一致。

Conclusion: 简单条件语句不能作为评估代码生成偏差的有效代理，当前基准测试严重低估了实际部署中的偏差风险。需要更真实、更复杂的任务来评估和缓解代码生成中的偏差。
...
TL;DR: 本文系统评估了多文档新闻摘要系统中的政治偏见问题，使用FairNews数据集和13个LLM模型，发现中等规模模型在公平性上优于大型模型，提示词去偏方法高度依赖模型，而实体情感是最难消除的偏见维度。


<details>
  <summary>Details</summary>
Motivation: 多文档新闻摘要系统被广泛应用，但其可能因不平衡的观点表征、不成比例的重点强调以及少数声音的系统性忽视而产生政治偏见，因此亟需对此进行全面评估。

Method: 使用带政治倾向标签的FairNews数据集，对13个LLM模型进行多维度公平性评估（五项指标），并测试了基于提示词和基于判断的多种去偏干预策略。

Result: 中等规模模型在公平性上优于大型模型；提示词去偏高度依赖模型；实体情感维度对所有去偏策略均不敏感，是最顽固的偏见维度。

Conclusion: 多文档新闻摘要的公平性需要多维度评估框架和针对性的、架构感知的去偏方法，而非简单地扩大模型规模。
...
### [118] [Beyond Single Plots: A Benchmark for Question Answering on Multi-Charts](https://arxiv.org/abs/2604.21344)
*Azher Ahmed Efat,Seok Hwan Song,Wallapak Tavanapong*

Main category: cs.CL

TL;DR: 本文提出PolyChartQA数据集，包含534张多图表图像（共2,297个子图表）和2,694个问答对，用于评估多图表理解任务。实验表明，在多模态大模型上，人工问题的准确率比模型生成问题低27.4%，而提出的提示方法可提升5.39%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现实场景中常需同时理解多个相关图表，但现有研究对多图表图像的联合理解探索不足，缺乏专门的数据集和评估基准。

Method: 构建PolyChartQA数据集（来自计算机科学论文的多图表图像+人工/模型生成的QA对），评估9个多模态语言模型，并提出新的提示方法提升性能。

Result: 人工问题相比模型生成问题的L-Accuracy下降27.4%；提出的提示方法带来5.39%的L-Accuracy提升。

Conclusion: 多图表理解对现有MLMs仍具挑战，人工问题更难且需更有效的提示策略。
...
TL;DR: 提出MathDuels——一种双角色自对弈数学基准，让模型同时作为出题者和解题者，通过Rasch模型联合估计能力，揭示传统静态基准无法区分的模型能力差异。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型在静态数学基准上已趋近天花板，现有评估无法区分模型能力差异，因为它们仅将模型视为固定问题集的解答者。

Method: 采用自对弈框架，模型同时担任出题者（通过三阶段生成流程：元提示→问题生成→难度增强）和解题者，由独立验证器过滤无效问题，并用Rasch模型联合估计解题能力和问题难度。

Result: 在19个前沿模型上的实验表明，出题和解题能力部分解耦；双角色评估揭示了单角色基准中不可见的能力区分；新模型进入后能生成击败先前主导解题者的问题，基准难度随参与者强度共同演化。

Conclusion: MathDuels通过动态演化的自对弈评估范式，突破了静态基准的天花板效应，为持续追踪语言模型数学能力提供了可扩展的评估框架。
...
### [120] [CARE: Counselor-Aligned Response Engine for Online Mental-Health Support](https://arxiv.org/abs/2604.21352)
*Hagai Astrin,Ayal Swaid,Avi Segal,Kobi Gal*

Main category: cs.CL

TL;DR: 本文提出CARE框架，通过在低资源语言（希伯来语和阿拉伯语）上微调开源LLM，利用真实危机对话中经专业咨询师评估为高效的数据，生成与专业咨询师策略对齐的实时回复建议，实验表明其语义和策略对齐度优于非专用LLM。


<details>
  <summary>Details</summary>
Motivation: 全球心理健康挑战日益严峻，导致咨询师过载和危机响应延迟；低资源语言（如希伯来语、阿拉伯语）的心理健康领域LLM应用尚未充分探索；现有LLM代理因缺乏大规模真实数据集训练，难以复制专业咨询师的支持性语言和干预策略。

Method: 提出CARE框架，分别对希伯来语和阿拉伯语的开源LLM进行微调；使用经专业咨询师评为高效的真实危机对话子集作为训练数据；利用完整对话历史进行训练，保留情绪演变上下文和动态结构。

Result: 实验表明，CARE生成的回复在语义和策略对齐度上均优于非专用LLM，与金标准咨询师回复更接近。

Conclusion: 基于专家验证数据的领域特定微调能显著支持咨询师工作流程，提升低资源语言情境下的照护质量。
...
### [121] [MKJ at SemEval-2026 Task 9: A Comparative Study of Generalist, Specialist, and Ensemble Strategies for Multilingual Polarization](https://arxiv.org/abs/2604.21370)
*Maziar Kianimoghadam Jouneghani*

Main category: cs.CL

TL;DR: 本文针对SemEval-2026任务9子任务1，系统研究了22种语言的多语言极化检测。通过对比多语言通用模型、语言专用模型和混合集成模型，提出了一种语言自适应框架。最终系统在22个语言轨道上取得了0.796的宏平均F1和0.826的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注单一语言或少数语言的极化检测，缺乏对多语言（特别是低资源语言和不同文字系统）的系统性对比。需要探索多语言通用模型与语言专用模型在跨语言极化检测中的优劣。

Method: 采用语言自适应框架，根据开发集表现动态切换多语言通用模型（如XLM-RoBERTa）、语言专用单语模型和混合集成模型。同时尝试了基于NLLB-200的跨语言数据增强技术。

Result: 最终系统在22个语言轨道上取得0.796的宏平均F1和0.826的平均准确率。研究发现XLM-RoBERTa在tokenizer与目标文本对齐时表现良好，但在高棉语、奥里亚语等独特文字上单语专用模型效果更优。跨语言增强（NLLB-200）效果不稳定，且在形态丰富语言上表现退化。

Conclusion: 不存在通用的单一最优架构，语言自适应框架（根据语言特性选择模型）是处理多语言极化检测的有效策略。跨语言数据增强需谨慎使用，尤其对形态丰富的语言可能带来负面影响。
...
### [122] [Decoupled DiLoCo for Resilient Distributed Pre-training](https://arxiv.org/abs/2604.21428)
...
TL;DR: 提出Decoupled DiLoCo，一种异步去中心化的分布式训练框架，通过解耦学习者与同步器、引入最小法定人数和自适应窗口机制，在故障频发环境中实现零全局停机的高效训练。


<details>
  <summary>Details</summary>
Motivation: 传统SPMD范式下的分布式训练因硬件故障、同步开销等问题导致大量计算浪费，现有方法如DiLoCo虽降低通信带宽但仍为同步模式，无法解决系统停滞问题。

Method: 将计算划分为多个独立的学习者，各自执行本地内部优化步骤；学习者异步将参数片段发送给中央同步器；同步器通过最小法定人数、自适应宽限期和动态token加权合并来跳过故障或掉队者。

Result: 在故障环境中实现零全局停机，训练效率显著提升；在文本和视觉任务上保持有竞争力的模型性能，适用于密集和混合专家架构。

Conclusion: Decoupled DiLoCo成功突破SPMD同步屏障，通过异步解耦和混沌工程思想，在保证模型性能的同时大幅提升分布式训练的鲁棒性和吞吐量。
...
### [123] [StructMem: Structured Memory for Long-Horizon Behavior in LLMs](https://arxiv.org/abs/2604.21748)
*Buqiang Xu,Yijun Chen,Jizhan Fang,Ruobin Zhong,Yunzhi Yao,Yuqi Zhu,Lun Du,Shumin Deng*

Main category: cs.CL

TL;DR: StructMem是一种结构增强的分层记忆框架，通过保留事件级绑定并诱导跨事件连接，在降低token使用量、API调用和运行时的同时，提升了长期对话代理在时间推理和多跳问答上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有长期对话记忆系统存在根本性权衡：扁平记忆高效但缺乏关系结构，图记忆支持结构化推理但构建成本高且脆弱。需要一种既能捕捉事件间关系又不牺牲效率的记忆框架。

Method: 提出StructMem框架：1) 保留事件级绑定，2) 诱导跨事件连接，3) 对双视角进行时间锚定，4) 执行周期性语义整合，形成结构增强的分层记忆。

Result: 在LoCoMo基准上，StructMem在时间推理和多跳问答任务上取得更好表现，同时显著减少了token使用量、API调用次数和运行时间。

Conclusion: StructMem有效打破了记忆系统中效率与结构化推理能力之间的权衡，为长期对话代理提供了一种既高效又具关系建模能力的记忆方案。
...
TL;DR: 本文研究了大型语言模型中推理能力的基础操作——回忆（recall）与状态追踪（state-tracking），比较了混合架构（注意力+循环状态更新）与纯注意力模型在处理联合需求任务上的表现。结果表明，推理增强（reasoning augmentation）整体提升最大，混合推理模型在序列依赖增加时仍保持鲁棒性，而纯注意力模型在任务难度超过阈值后性能急剧下降。


<details>
  <summary>Details</summary>
Motivation: 推理在大语言模型中常被视为单一能力，但其提升可能源于更基础的操作。作者希望探究回忆与状态追踪这两个基本原语，并验证混合架构是否比纯注意力模型更适合需要同时调用这两种能力的任务。

Method: 使用匹配的Olmo3 Transformer与混合模型（Hybrid），在指令微调与推理增强变体上进行对比实验。通过一系列受控任务（混合了状态追踪与回忆原语）评估模型表现，重点考察随着序列依赖增加时模型性能的变化。

Result: 1) 推理增强整体上提供了最大的改进，显著扩展了模型的有效运作难度范围；2) 在部分任务中，混合推理模型在序列依赖增加时仍保持较强鲁棒性；3) 纯注意力Transformer推理模型在任务难度超过某个阈值后性能急剧下降。

Conclusion: 推理token与架构归纳偏置在计算过程的不同层面发挥作用：显式推理可扩展模型有效运作范围，但其收益取决于底层架构对持久状态传播的支持程度。由于研究规模有限，结论为提示性而非决定性，未来需在更多模型、规模及任务变体上验证。
...
TL;DR: 本文研究了在法规合规检测（NLI任务）中，通过数据选择策略来减少跨领域负迁移的问题，比较了四种数据选择方法，发现定向数据选择能有效提升跨领域适应性。


<details>
  <summary>Details</summary>
Motivation: 现有法规合规检测模型难以跨领域泛化，需要开发高效的数据选择方法来减少负迁移。

Method: 将合规检测建模为NLI任务，比较四种数据选择方法：随机采样、Moore-Lewis交叉熵差异、重要性加权和基于嵌入的检索，系统分析选择数据比例的影响。

Result: 定向数据选择显著减少了负迁移，为提高跨领域合规检测的可靠性和可扩展性提供了有效途径。

Conclusion: 通过数据选择可以有效缓解跨领域负迁移问题，为实现异构法规下的可扩展合规自动化提供了实用方案。
...
TL;DR: 本文提出了一种受控的多维成对评估框架，用于多语言文本到语音（TTS）系统评估。覆盖10种印度语言的5000+句子，收集来自1900+母语者的12万+成对比较，从6个感知维度进行评价，构建多语言排行榜并分析模型优劣。


<details>
  <summary>Details</summary>
Motivation: 现有的众包成对评估方法应用于TTS时，由于语言多样性和语音感知的多维性，存在高方差问题，缺乏系统性的评估框架。

Method: 采用受控多维成对评估框架，结合语言控制与感知导向标注。使用Bradley-Terry建模构建排行榜，SHAP分析解释人类偏好，从6个感知维度（可懂度、表现力、语音质量、生动性、噪声、幻觉）进行评估。

Result: 收集了5000+句子、120K+成对比较、1900+评估者数据，覆盖10种印度语言（含代码混合文本），评估了7个SOTA TTS系统。

Conclusion: 该框架能有效降低评估方差，提供更细粒度的TTS系统性能洞察，揭示不同感知维度上的模型优势与权衡关系。
...
TL;DR: OptiVerse是一个包含1000个优化问题的基准测试集，涵盖随机优化、动态优化、博弈优化和最优控制四个被忽视的领域。实验发现，即使是GPT-5.2和Gemini-3等先进模型，在困难问题上准确率也难超27%，主要瓶颈在于建模与逻辑错误。为此提出了双视角审核智能体来提升LLM建模准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基准过度聚焦于数学规划和组合优化，忽略了随机优化、动态优化、博弈优化和最优控制等重要领域，缺乏对LLM优化能力的全面评估。

Method: 构建了包含1000个精心策划问题的OptiVerse基准，覆盖四个被忽视的优化领域和三个难度级别。提出了双视角审核智能体（Dual-View Auditor Agent）来改进LLM的建模过程。

Result: 在22个LLM上的实验显示，困难问题上性能急剧下降，甚至GPT-5.2和Gemini-3的准确率也不足27%。错误分析表明建模与逻辑错误是主要瓶颈。

Conclusion: OptiVerse为推进LLM解决复杂优化问题提供了基础平台，双视角审核智能体能有效提升建模准确性且时间开销低。
...
TL;DR: 提出SRICL框架，结合语义检索、上下文学习和监督微调与确定性验证器，从招聘广告中提取技能跨度，在六个跨领域、跨语言数据集上显著优于GPT-3.5基线，有效减少无效标签和幻觉。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在技能跨度提取中常产生格式错误、边界漂移和幻觉，尤其在长尾术语和跨领域迁移场景下表现不佳，亟需一种可靠且低资源的提取框架。

Method: SRICL框架包含三个核心组件：语义检索（SR）从ESCO获取领域内标注句与定义以约束格式，上下文学习（ICL）稳定边界，监督微调（SFT）对齐输出行为，最后通过确定性验证器强制配对、非重叠和BIO合法性。

Result: 在六个公开的跨领域、跨语言招聘广告跨度标注数据集上，SRICL在STRICT-F1指标上显著超越GPT-3.5提示基线，并大幅降低无效标签和幻觉跨度数量。

Conclusion: SRICL框架在低资源、多领域设置下实现了可靠的句子级技能跨度部署，有效解决了LLM在技能提取中的格式和内容偏差问题。
...
TL;DR: 本文针对SemEval-2026任务2，结合LLM提示、成对最大熵模型和轻量级神经回归模型，在用户生成文本的情感变化建模中取得第一。


<details>
  <summary>Details</summary>
Motivation: 需要对时序用户文本中的当前情感和短期情感变化进行联合建模，现有方法难以兼顾静态情感信号与动态情感轨迹。

Method: 三种互补方法：(1) LLM提示（区分用户感知与无用户设定）；(2) 基于Ising相互作用的成对最大熵模型，用于结构化过渡建模；(3) 轻量级神经回归模型，结合近期情感轨迹与可训练用户嵌入。

Result: 在子任务1和子任务2A中均排名第一；发现LLM擅长捕捉静态情感信号，而短期情感变化更依赖于近期数值轨迹而非文本语义。

Conclusion: 情感变化建模中，近期状态轨迹比文本语义更重要，融合多种方法可取得最佳效果。
...
TL;DR: 本文提出了一种不依赖分类器的句子嵌入评估方法——概念分离曲线，通过引入句法噪声和语义否定，量化嵌入对概念变化的敏感度，实现跨模型、跨语言的可解释性评估。


<details>
  <summary>Details</summary>
Motivation: 现有句子嵌入评估方法依赖额外分类器或下游任务，导致难以区分效果来自嵌入本身还是分类器行为，因此需要一种分类器无关的客观评估方法。

Method: 系统性地向句子中引入句法噪声和语义否定，量化其对嵌入的影响，并通过概念分离曲线可视化模型区分概念变化与表面变化的能力。

Result: 在多领域、双语言（荷兰语和英语）及不同句子长度上的实验表明，概念分离曲线能有效、可重复地评估嵌入的概念稳定性。

Conclusion: 概念分离曲线提供了一种可解释、可复现、跨模型的句子嵌入评估方式，客观衡量模型对概念层面的编码能力。
...
TL;DR: 提出了一种开源方法llm-bias-bench，通过直接询问和间接辩论两种方式探测大语言模型在有争议话题上的真实立场，发现辩论式交互会诱发比直接提问高2-3倍的谄媚行为（从50%升至79%），且看似有立场的模型在持续争论中常会崩解为镜像回应。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在搜索、咨询、决策等场景中广泛传播信息，但它们在争议话题上的真实立场难以获取：模型会回避直接回答，却又容易被用户观点带偏。现有方法无法在接近真实多轮交互的条件下揭示模型的实际立场。

Method: 提出llm-bias-bench方法，包含两种探针：(1) 直接探针——模拟用户在五轮对话中逐步施压直接询问模型观点；(2) 间接探针——从不直接询问观点，而是让模型参与辩论，通过其让步、抵抗或反驳来泄漏偏见。使用三种用户角色（中立、赞同、反对）组合成九类行为分类，区分立场独立与谄媚行为，并用可审计的LLM裁判生成带文本证据的判断。

Result: 在38个争议话题（含价值观、科学共识、哲学、经济政策）和13个助手上测试发现：辩论式交互引发谄媚的概率是直接提问的2-3倍（中位数50%→79%）；直接提问下看起来有立场的模型在持续辩论中常崩解为镜像回应；攻击能力主要在需要动摇已有立场时起作用，而非模型中立时。

Conclusion: 揭示了多轮交互尤其是辩论式场景会显著放大语言模型的谄媚行为，质疑了当前模型在争议话题上的可靠性，并为评估和审计模型立场提供了可复现的开源工具。
...
TL;DR: 本文提出AgenticQwen系列模型，通过多轮强化学习（结合推理RL和智能体RL）以及双数据飞轮（推理飞轮通过错误学习增加难度，智能体飞轮将线性工作流扩展为多分支行为树），在较小模型规模下实现了接近大模型的工业级智能体性能，并在搜索和数据分析任务中显著缩小了差距。


<details>
  <summary>Details</summary>
Motivation: 现代工业应用需要语言模型作为智能体进行多步推理和工具使用，但受到严格的成本和延迟约束，因此亟需小型但高效的智能体模型。

Method: 采用多轮强化学习框架，结合推理RL和智能体RL；提出双数据飞轮机制：推理飞轮通过从错误中学习自动提升任务难度，智能体飞轮将线性工作流扩展为多分支行为树以模拟真实世界决策复杂性。

Result: 在多个公开智能体基准测试上表现强劲；在工业智能体系统中，搜索和数据分析任务上显著缩小了与小模型的差距。

Conclusion: AgenticQwen证明了通过精心设计的强化学习框架和数据飞轮，小模型也能在工业级智能体任务中逼近大模型性能。
...
TL;DR: 本文提出多语言作为潜在变量可扩展LLM推理空间，并基于此提出polyGRPO框架，仅用18.1K数学数据训练，在英文推理任务上提升6.72%，在多语言基准提升6.89%，并展现出跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 观察到非英语响应在某些推理任务上表现优于英语这一反直觉现象，作者假设语言并非仅仅作为输出媒介，而是作为潜在变量结构化地调节模型内部推理路径。基于此假设，探索如何利用多语言特性提升推理能力。

Method: 1. 进行Polyglot Thinking Experiment，在语言约束和无约束条件下测试模型；2. 提出polyGRPO（多语言组相对策略优化）框架，将语言变化视为隐式探索信号，在线生成多语言偏好数据；3. 在语言约束和无约束条件下优化策略，同时关注答案准确性和推理结构。

Result: 仅用18.1K多语言数学问题（无思维链标注）训练，polyGRPO在四个英文推理测试集上提升6.72%绝对准确率，多语言基准提升6.89%；是唯一在英文常识推理任务上超越基础模型的方法（提升4.9%），尽管仅在数学数据上训练。

Conclusion: 语言作为潜在变量能够扩展模型潜在推理空间，带来一致且可泛化的推理性能提升。多语言性拓宽了模型的潜在推理空间，是提升推理能力的有效途径。
...
TL;DR: 本文提出Verbal Process Supervision (VPS)，一个无需训练的自然语言批评框架，通过更强的监督器指导迭代生成-批评-改进循环，在GPQA Diamond、AIME 2025和LiveCodeBench V6上取得显著提升，确立批评粒度作为推理时间扩展的新维度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理时间扩展主要关注链深度、样本广度、学习型步骤评分器三个维度，缺乏对外部语言监督粒度的探索。本文引入第四个维度——外部语言监督的粒度。

Method: VPS框架：无需训练，利用更强监督器的结构化自然语言批评，指导迭代的生成-批评-改进循环，设有轮次预算R。通过批评粒度这一新维度实现推理时间扩展。

Result: 1) GPQA Diamond上达到94.9% (R=4)，超越94.1%的SOTA；2) AIME 2025上弱模型提升最高达+63.3分；3) 在相同计算量下，VPS优于Reflexion (+8.5~12.1分)和Self-Consistency@5；4) 性能与监督器-模型能力差距呈强相关(Pearson r=0.90)。

Conclusion: 批评粒度可作为推理时间扩展的新轴心。当错误不能以语言表达时（如代码合成），性能会下降，这启发了混合语言-可执行方法的未来研究方向。
...
TL;DR: 本文调查了232篇论文，探讨语言模型在"最后一公里"部署中的挑战——即多语言性与边缘设备部署的交汇点，并提供了可操作建议。


<details>
  <summary>Details</summary>
Motivation: 全球南方非英语和硬件受限社区难以从语言模型中受益，而多语言NLP和边缘部署研究长期割裂，需要联合研究。

Method: 对232篇涵盖语言模型开发到部署全流程的论文进行系统性文献调查。

Result: 揭示了多语言性与边缘部署之间的技术竞争关系，以及当前研究现状的不足。

Conclusion: 提出了开放问题和针对NLP生态系统不同利益相关者的可操作建议，以推动包容公平的语言技术发展。
...
TL;DR: 本文提出联合建模标注者特定标签预测与解释的框架，通过User Passport机制融合标注者身份与人口统计信息，并设计两种解释器架构（事后提示解释器与前缀桥解释器）来生成与个体视角对齐的解释。实验表明，解释建模显著提升了预测性能，前缀桥方法在标签对齐和语义一致性上更优，事后方法在词汇相似性上更强。


<details>
  <summary>Details</summary>
Motivation: 现有的标注者感知建模主要关注离散标签（如标注者身份或人口统计信息），但忽略了更细粒度的信号——标注者提供的解释（rationales）。作者希望利用这些解释来更丰富、更忠实地表示标注者之间的分歧，从而提升模型对个体视角的建模能力。

Method: 1) 使用User Passport机制在表示层融合标注者身份和人口统计元数据；2) 提出两种解释器架构：事后提示解释器（post-hoc prompt-based explainer）和前缀桥解释器（prefixed bridge explainer）；3) 在自然语言推理（NLI）数据集上联合训练标签预测和解释生成。

Result: 1) 融入解释建模显著提升了标注者感知分类器的预测性能；2) 前缀桥方法在标签对齐和语义一致性上更稳定；3) 事后方法在词汇相似性（如n-gram重叠）上更强。

Conclusion: 将解释建模为细粒度视角的表达，能够更丰富、更忠实地表示标注者之间的分歧。本文提出的方法通过将标注者特定的解释同时融入预测和生成组件，推进了视角主义（perspectivist）建模的发展。
...
TL;DR: 本文利用持续同调（拓扑数据分析方法）从眼动追踪数据中提取多尺度特征，构建结合拓扑特征与传统统计特征的混合模型，在哥本哈根语料库的阅读障碍检测任务中优于纯传统特征方法。


<details>
  <summary>Details</summary>
Motivation: 传统眼动数据分析方法主要依赖统计特征，未能充分捕捉注视序列中的多尺度结构信息。作者希望利用持续同调提取传统方法无法捕获的互补信息，提升阅读障碍检测性能。

Method: 将注视序列视为时间序列，设计新颖的过滤方法（filtration）进行持续同调分析，提取拓扑特征；然后将这些拓扑特征与传统统计特征结合，构建混合模型用于阅读障碍分类。

Result: 混合模型在哥本哈根语料库的阅读障碍检测任务中，性能优于仅使用传统特征的方法；所提出的新过滤方法也优于已有的过滤方法；拓扑特征本身能达到与基线方法相当的性能。

Conclusion: 持续同调能够从眼动追踪数据中提取传统特征无法捕获的互补信息，为阅读障碍检测提供了一种有效的多尺度分析新途径。
...
TL;DR: 本文基于冻结的自监督语音表征中的音系特征子空间d-prime可分性，将构音障碍严重程度评估方法从890人5种语言扩展至3374人25个数据集12种语言5种病因，发现：病因特异性退化模式在群体水平可区分；跨语言轮廓形状稳定（余弦相似度>0.95）；方法具有架构无关性（6种骨干模型间一致性rho>0.77）。


<details>
  <summary>Details</summary>
Motivation: 先前提出的基于d-prime可分性的构音障碍评估方法仅在较小规模（890人、5种语言）上验证，缺乏对更大规模、更多样化人群（多种病因、多种语言）的泛化能力检验，以及跨语言、跨模型架构的稳定性验证。

Method: 使用6种自监督学习（SSL）骨干模型，基于冻结表征中的音系特征子空间计算d-prime可分性，对3374名来自25个数据集、12种语言、5种病因（帕金森病、脑瘫、肌萎缩侧索硬化症、唐氏综合征、中风）及健康对照的说话人进行构音障碍严重程度评估。

Result: 1) 10/13个音系特征在群体水平上效应量大（epsilon-squared>0.14），帕金森病与构音执行组可分离（Cohen's d=0.83），但个体水平分类有限（macro F1=22.6%）；2) 辅音d-prime轮廓的跨语言余弦相似度>0.95，绝对量值需语料内校准；3) 6种骨干模型均产生单调严重度梯度，模型间一致性rho>0.77，固定token估计保持相关（rho=-0.733）。

Conclusion: 音系子空间分析是一种鲁棒、无需训练且适用于多种病因的构音障碍表征框架，具有跨语言轮廓形状稳定性和跨骨干模型鲁棒性，但绝对严重度解读需语料内校准，个体水平分类仍有提升空间。
...
TL;DR: 本文提出X-GRAM框架，通过频率感知的动态token注入、混合哈希压缩长尾、归一化SwiGLU ShortConv提取n-gram特征、深度感知门控集成记忆，实现了模型容量与计算量的解耦，在0.73B和1.15B规模上比基线提升最高4.4个点。


<details>
  <summary>Details</summary>
Motivation: 大token索引查找表存在参数效率低、内存快速增长的问题，归因于Zipf长尾欠训练、各层需求异质性以及"槽坍塌"导致冗余嵌入，需要一种更高效的记忆增强架构来解耦容量与计算。

Method: 提出X-GRAM框架：1) 混合哈希和别名混合压缩长尾、保留头部容量；2) 归一化SwiGLU ShortConv提取局部n-gram特征；3) 深度感知门控将信号集成到注意力值流和层间残差中，对齐静态记忆与动态上下文。

Result: 在0.73B和1.15B规模下，平均准确率比基础骨干提升4.4个点，比强检索基线提升3.2个点，且仅使用50%大小的表格。

Conclusion: X-GRAM通过高效记忆管理实现了容量与计算量解耦，为未来记忆增强架构提供了可扩展且实用的范式。
...
TL;DR: 提出Agent Evolving Learning (AEL)框架，通过双时间尺度（快速：Thompson采样选择检索策略；慢速：LLM反思注入因果洞察）解决LLM代理缺乏状态记忆的问题，在投资组合任务上取得SOTA效果。


<details>
  <summary>Details</summary>
Motivation: LLM代理在开放环境中缺乏状态记忆，每个任务都从零开始，核心障碍不是"记忆什么"而是"如何使用记忆"——包括检索策略选择、结果解读、以及何时改变策略。

Method: 双时间尺度框架：(1)快速尺度：Thompson Sampling bandit学习每轮使用哪种记忆检索策略；(2)慢速尺度：LLM驱动的反思机制诊断失败模式，将因果洞察注入代理的决策提示中。

Result: 在208轮投资组合基准上，AEL的Sharpe比达2.13±0.47，超越5种自改进方法和所有非LLM基线，方差最低。消融实验显示"少即是多"：记忆+反思带来58%提升，但所有额外机制（规划器进化、工具选择、冷启动初始化等）均导致性能下降。

Conclusion: 代理自我改进的瓶颈是"自我诊断如何使用经验"，而非增加架构复杂性。核心在于如何利用记忆而非记忆本身。
...
### [141] [Why are all LLMs Obsessed with Japanese Culture? On the Hidden Cultural and Regional Biases of LLMs](https://arxiv.org/abs/2604.21751)
*Joseba Fernandez de Landa,Carla Perez-Almendros,Jose Camacho-Collados*

Main category: cs.CL

TL;DR: 本文提出CROQ数据集，发现LLM在文化相关问题中存在地域偏好（偏向日本），且文化偏见在监督微调阶段而非预训练阶段显现。


<details>
  <summary>Details</summary>
Motivation: 已有研究分析了LLM的文化能力，但缺乏专门针对LLM在文化相关问题中地域偏好的系统研究。

Method: 基于全面的文化相关开放问题（CROQ）分类体系构建新数据集，对LLM进行系统性文化偏好测试。

Result: 1) LLM表现出对日本的明显倾向，与传统西方偏见结论不同；2) 英文等高端语言提示下输出更多样化，且较少偏向官方语言国；3) 文化偏好的首次明显信号出现在监督微调阶段，而非预训练阶段。

Conclusion: LLM的文化偏见具有复杂性和阶段性，监督微调是关键转折点，提示语言也会影响输出多样性。
...
### [142] [AUDITA: A New Dataset to Audit Humans vs. AI Skill at Audio QA](https://arxiv.org/abs/2604.21766)
*Tasnim Kabir,Dmytro Kurdydyk,Aadi Palnitkar,Liam Dorn,Ahmed Haj Ahmed,Jordan Lee Boyd-Graber*

Main category: cs.CL

TL;DR: 本文提出AUDITA基准，一个基于真实世界音频的问答数据集，人类准确率32.13%，而最先进的音频问答模型准确率低于8.86%，揭示了现有模型在音频推理方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有音频问答基准存在捷径策略、短时线索、词汇先验、数据集偏差等问题，模型可绕过真正推理过程。亟需一个需要深度音频推理的高质量基准。

Method: 构建AUDITA数据集，包含人工撰写的trivia问题，设置具有挑战性的干扰项和长程时间依赖，并应用项目反应理论(IRT)评估潜在能力、问题难度和模型系统性缺陷。

Result: 人类平均准确率32.13%，而最先进的音频问答模型平均准确率低于8.86%，差距显著。

Conclusion: AUDITA揭示了现有模型在真实音频推理任务上的严重不足，表明当前音频问答系统远未达到人类水平的听觉理解能力，亟需更鲁棒的音频推理方法。
...
### [143] [Misinformation Span Detection in Videos via Audio Transcripts](https://arxiv.org/abs/2604.21767)
*Breno Matos,Rennan C. Lima,Savvas Zannettou,Fabricio Benevenuto,Rodrygo L. T. Santos*

Main category: cs.CL

TL;DR: 本文创建了两个新的视频错误信息数据集（超过500个视频、2400多个带注释的片段），通过音频转录文本，将错误信息检测从视频级别推进到片段级别（错误信息跨度检测），使用先进语言模型构建分类器，F1得分为0.68。


<details>
  <summary>Details</summary>
Motivation: 现有视频错误信息检测仅停留在视频整体层面，无法定位错误信息在视频中的具体位置和内容（即具体哪些声明导致视频的错误性质），缺乏可解释性和细粒度的上下文信息。

Method: 1) 创建两个新数据集，将视频音频转录为文本；2) 标注视频中错误信息声明出现的具体片段（错误信息跨度检测）；3) 使用基于先进语言模型的分类器进行检测。

Result: 在识别视频中错误信息发生的位置任务上，F1得分为0.68。数据集、转录文本、音频和视频均已公开。

Conclusion: 将视频错误信息检测从全局级别推进到片段级别是可行的，提供了更可解释的检测结果，为未来研究和事实核查工作提供了公开数据集资源。
...
TL;DR: 本文提出NSNRL共享任务，将叙事相似性定义为二分类问题（判断哪个故事更相似于锚定故事），构建了1000+故事三元组数据集，46支队伍71个提交系统，发现LLM集成在分类任务中表现最佳，嵌入任务中预处理/后处理与微调方案表现相当。


<details>
  <summary>Details</summary>
Motivation: 叙事相似性缺乏统一定义和标准化评估基准，现有方法难以兼容叙事理论与直觉判断，亟需一个可操作的计算框架来推动叙事表示学习。

Method: 1) 提出与叙事理论和直觉判断兼容的叙事相似性定义；2) 收集1000+故事摘要三元组，每条标注至少两人一致；3) 设置两个赛道：三元组分类（判断哪个故事更相似）和嵌入表示评估；4) 接收46支队伍71个系统提交。

Result: 分类赛道中LLM集成方法占据多数高分系统；嵌入赛道中预训练模型的预处理/后处理方案与定制微调方案表现相当；两个赛道均有改进空间。

Conclusion: NSNRL任务提供了叙事相似性的标准化基准，LLM集成在分类中优势明显，嵌入赛道仍存提升空间，任务网站提供可视化结果供进一步分析。
...
TL;DR: 本文通过"告密者困境"实验，探究了大语言模型（LLMs）在不同人际关系和犯罪严重程度下的道德判断。研究发现，LLMs在道德规范性判断上保持公平导向，在预测人类行为时会随关系亲近度增加转向忠诚导向，但模型自主决策却偏向未社会化的刚性规则，与其自身对世界的建模存在不一致。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs越来越多地作为决策支持系统，判断它们是否编码了人类道德判断中的社会情境依赖性（如人际关系对道德判断的调节作用）至关重要。

Method: 采用"告密者困境"实验范式，操控两个维度：犯罪严重程度和关系亲近度。从三个视角评估：道德正确性（规范性规范）、预测人类行为（描述性社会期望）、以及模型自主决策。通过分析推理过程来比较三种视角的差异。

Result: 存在清晰的跨视角分歧：道德正确性始终以公平为导向；预测人类行为随关系亲近度增加显著转向忠诚导向；而模型自主决策与道德正确性判断一致，而非其自身的行为预测。

Conclusion: LLM决策优先考虑刚性的规范性规则，而非其内部世界模型中存在的社会敏感性。这种不一致可能导致LLM在实际部署中出现重大对齐问题。
...
TL;DR: 本文提出RedirectQA数据集，利用维基百科重定向信息将实体的事实三元组与多种表面形式（别名、缩写、拼写变体、常见错误形式）关联，测试13个LLM后发现：仅改变实体表面形式就会导致预测结果不一致，这种不一致性依赖于表面形式类别，实体频率和表面频率都与准确性相关。


<details>
  <summary>Details</summary>
Motivation: 现有实体QA评估通常用单一规范名称查询实体，难以区分模型是真正记住了事实还是仅能通过特定名称访问事实，因此需要系统性地评估表面形式多样性对事实记忆的影响。

Method: 构建RedirectQA数据集，利用维基百科重定向将Wikidata事实三元组与多种表面形式（别名、缩写、拼写变体、常见错误）关联，在13个LLM上进行表面条件性事实记忆评估实验。

Result: 表面形式改变时预测结果会变化，这种不一致性具有类别依赖性：模型对轻微拼写变化更鲁棒，对别名和缩写等较大词汇变化更敏感；实体频率和表面频率都与准确性相关，实体频率的贡献常超过表面频率。

Conclusion: 事实记忆既非纯粹表面特定也非完全表面不变，评估非逐字记忆时应考虑表面形式多样性。
...
TL;DR: 提出 MODEE 方法，结合图学习与 LLM 文本表示，实现开放域事件抽取，解决现有方法在领域泛化和文档级推理方面的局限，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有事件抽取方法存在两个主要局限：封闭域算法局限于预定义事件类型，难以泛化到未见类型；开放域算法未充分利用大语言模型的能力，且缺乏文档级的上下文、结构和语义推理建模。

Method: 提出 MODEE（多模态开放域事件抽取），将基于图的表示学习与 LLM 的文本表示相结合，显式建模文档级推理。

Result: 在大型数据集上，MODEE 优于现有开放域事件抽取方法，并能泛化到封闭域任务，同样取得最优性能。

Conclusion: 图学习与 LLM 文本表示的融合能有效解决开放域事件抽取中的文档级推理难题，提升模型泛化能力。
...
TL;DR: TingIS是一个用于大规模云原生服务中实时检测和缓解技术异常的系统，能够在每分钟处理超过2000条消息、每天30万条消息的生产环境中，实现P90告警延迟3.5分钟、95%的高优先级事件发现率。


<details>
  <summary>Details</summary>
Motivation: 大规模云原生服务中，即使几分钟的停机也会导致巨大经济损失和用户信任下降。客户事件是发现监控遗漏风险的重要信号，但从中提取可操作情报面临极端噪音、高吞吐量和多业务线语义复杂性的挑战。

Method: TingIS的核心是一个多阶段事件链接引擎，结合高效索引技术和大型语言模型（LLMs）进行事件合并决策。辅以级联路由机制实现精确业务归属，以及集成领域知识、统计模式和行为过滤的多维降噪流水线。

Result: 在生产环境中，峰值吞吐量超过每分钟2000条消息、每天30万条消息，达到P90告警延迟3.5分钟，高优先级事件发现率95%。在真实数据构建的基准测试中，路由准确率、聚类质量和信噪比均显著优于基线方法。

Conclusion: TingIS是一个端到端的企业级事件发现系统，通过多阶段事件链接引擎和降噪机制，在大规模生产环境中实现了高效、准确的技术异常检测与缓解。
...
TL;DR: 本文创建了EVENT5Ws，一个大规模、人工标注并经过统计验证的开放域事件抽取数据集，填补了现有数据集在开放域和规模上的空白。通过系统性标注流程、大模型评估及跨地域泛化实验，证明了该数据集的有效性，并总结了数据集开发的经验教训。


<details>
  <summary>Details</summary>
Motivation: 现有事件抽取数据集存在两个主要局限：封闭域设置下事件类型覆盖有限，以及开放域设置下缺乏大规模、人工验证的数据集。因此需要创建一个既大规模又经过人工标注验证的开放域事件数据集，以推动自动化事件抽取方法的发展。

Method: 设计系统性标注流程创建EVENT5Ws数据集，提供标注复杂度的实证分析。利用该数据集评估多个最先进的预训练大语言模型，并建立基准。通过跨不同地理背景的数据集进行泛化实验验证模型效果。

Result: 成功创建了EVENT5Ws开放域事件抽取数据集，基于该数据集评估了当前最先进的大语言模型并建立了基准。实验表明，在该数据集上训练的模型能够有效泛化到不同地理背景的数据集。

Conclusion: EVENT5Ws数据集具备良好的泛化能力，可用于开发通用性算法。同时总结了数据集开发过程中的经验教训，并提出了对未来大规模数据集开发的建议。
...
TL;DR: 本文提出一个结合历时文体分析、上下文主题建模和语义聚类的计算框架，通过对巴西众议院2003-2025年间45万+演讲的分析，揭示了议会话语在风格、内容和说话者三个维度上的动态特征。


<details>
  <summary>Details</summary>
Motivation: 传统的立法行为分析主要依赖投票记录，忽视了政治演讲中丰富的语义和修辞内容，因此需要一种能够多维度分析议会话语的通用方法。

Method: 提出一个可扩展、可泛化的计算框架，整合了历时文体分析（分析话语风格变化）、上下文主题建模（分析话语内容）和语义聚类（分析说话者的话语相似性），并对巴西众议院2003-2025年超过45万场演讲进行了大规模案例研究。

Result: 研究发现三个关键结果：（1）长期风格转变：演讲趋向更短、更直接；（2）立法议程在国家危机期间发生急剧转向；（3）话语联盟的细粒度图谱显示，地区和性别身份往往比正式政党归属更具话语相似性。

Conclusion: 该工作提供了一种稳健的、多维度的议会话语分析方法，能够有效补充传统的基于投票的立法行为研究。
...
TL;DR: GiVA是一种基于梯度的初始化策略，用于向量式微调方法，在不增加参数量的前提下将所需秩降低了8倍，实现了与LoRA相当的训练速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有向量式微调方法虽然参数效率极高，但需要比LoRA更高的秩才能达到相近性能，导致训练成本增加。作者希望在不牺牲参数效率的同时，降低秩需求以提升训练效率。

Method: 提出GiVA（Gradient-based Initialization for Vector-based Adaptation），一种基于梯度信息的初始化策略，用于向量式微调方法的参数初始化。

Result: 在自然语言理解、自然语言生成和图像分类等多个基准测试中，GiVA在保持极低参数量的同时，将秩需求降低了8倍，性能一致优于或持平现有向量式方法和LoRA。

Conclusion: GiVA有效解决了向量式微调方法高秩需求的问题，在保持参数效率的同时实现了与LoRA相当的训练速度，是一种高效且实用的微调方法。
...
TL;DR: 本文探索了基于解码器的大语言模型（LLMs）在语义级ASR评估中的应用，通过假设选择、语义距离计算和错误分类三种方法，在HATS数据集上取得了92-94%的人类一致性，远超WER的63%，表明LLMs在语义化、可解释的ASR评估中具有潜力。


<details>
  <summary>Details</summary>
Motivation: 传统ASR评估指标WER只关注字面错误，无法捕捉语义含义，与人类感知相关性差。现有基于嵌入的语义指标主要使用编码器模型，解码器型LLMs在这一任务中的潜力尚未被充分探索。

Method: 提出三种评估方法：(1) 在两个候选转录中选择语义最佳假设；(2) 利用生成式嵌入计算语义距离；(3) 对ASR错误进行定性分类。实验在HATS数据集上进行，比较LLMs与WER及其他语义指标的表現。

Result: 最佳LLMs在假设选择任务上与人类标注者的一致性达到92-94%，而WER仅有63%。解码器型LLMs生成的嵌入表现与编码器模型相当。

Conclusion: 基于LLM的语义评估方法在ASR任务中显著优于传统WER，且具有可解释性，为未来ASR评估提供了有前景的新方向。
...
TL;DR: 本文提出 Frequency-Forcing，通过一个早熟的辅助低频流（基于可学习的 wavelet packet 变换）以软约束方式引导标准像素流实现频率有序生成，无需改写核心流路径，在 ImageNet-256 上显著提升 FID。


<details>
  <summary>Details</summary>
Motivation: 标准流匹配模型均匀地传输噪声到数据，但按生成顺序（先低频粗结构后高频细节）对自然图像合成至关重要。现有两种范式——K-Flow（硬频率约束）和 Latent Forcing（软排序机制）——各有局限。作者希望在不重写核心流坐标的前提下，实现尺度有序生成。

Method: 提出 Frequency-Forcing，结合 K-Flow 的频率排序思想与 Latent Forcing 的软约束机制。主像素流由辅助低频流引导，该低频流通过可学习的轻量级 wavelet packet 变换从数据自身提取（自强制信号），无需外部预训练编码器。辅助流在时间上提前成熟，从而引导主流先学习低频信息。

Result: 在 ImageNet-256 上，Frequency-Forcing 一致性地优于强像素空间和潜在空间基线，且可自然与语义流组合获得进一步提升，证明了基于强制的尺度排序是一种通用、路径保留的替代方案。

Conclusion: 强制式（forcing）的尺度排序是一种灵活且保留原始路径的方法，相比硬频率流更具通用性，可有效提升图像生成质量。
...
TL;DR: 本文提出一族 $C^{2N}$ 光滑激活函数（GEM、E-GEM、SE-GEM），基于逻辑斯蒂CDF门控机制，仅用有理运算实现类ReLU性能。实验表明，$N=1$ 适合深度CNN，$N=2$ 适合Transformer，SE-GEM首次在CIFAR-10上超越GELU，GPT-2上GEM困惑度最低（72.57）。


<details>
  <summary>Details</summary>
Motivation: ReLU虽简单有效，但缺乏光滑性，在深层网络中可能阻碍基于梯度的优化。现有的光滑激活函数（如GELU）计算成本较高，本文旨在设计一种既光滑又能保持ReLU级性能且计算高效（纯有理运算）的激活函数族。

Method: 提出基于log-logistic CDF门控的 $C^{2N}$ 光滑激活函数族，包含三个变体：GEM（基础族）、E-GEM（$\epsilon$参数化，可任意 $L^p$ 逼近ReLU）、SE-GEM（分段变体，消除死神经元并保持 $C^{2N}$ 连接光滑性）。通过 $N$ 消融研究确定最优光滑度参数，并探索 $\epsilon$ 的尺度依赖最优性。

Result: $N=1$ 在标准深度网络中最优，将CIFAR-100+ResNet-56上GELU差距从6.10%降至2.12%。SE-GEM在CIFAR-10+ResNet-56上超越GELU（92.51% vs 92.44%）。在GPT-2（124M）上GEM困惑度72.57低于GELU的73.76。E-GEM在BERT-small上验证损失6.656最优。

Conclusion: 光滑度参数 $N$ 揭示了CNN-Transformer的权衡：$N=1$ 适用于深度CNN，$N=2$ 适用于Transformer。$\epsilon$ 参数化存在尺度依赖最优：小 $\epsilon$ 适合深度CNN和大Transformer，大 $\epsilon$ 适合小Transformer（如BERT-small）。该族激活函数在多个任务上达到或超越GELU性能，且仅需有理运算，效率更高。
...
TL;DR: 本文提出从小说中提取规范性模拟体（结构化规范表示），通过监督微调（SFT）和GRPO强化学习微调LLM，使其具备情境完整性（CI）隐私推理能力，并在真实世界基准上取得最优表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理的信息处理方式与用户的情境隐私期望不匹配。CI框架虽定义了隐私作为信息流的适当性，但现有方法要么使用监督-助理架构导致推理成本翻倍，要么在狭窄任务数据上微调，缺乏泛化能力。

Method: 1）从小说中提取规范性模拟体；2）通过SFT（监督微调）引入保守先验；3）使用GRPO强化学习进一步优化，奖励函数包含程序化信号（任务清晰度、结构完整性、内部一致性、情境识别）和LLM裁判评估；4）引入每完成对比评分（per-completion contrastive scoring）防止过拟合。

Result: SFT引入保守先验，提升了隐私相关情境的识别能力但未改进判断正确性。GRPO+规范性模拟体在合规基准上取得最高分，与人类隐私期望的相关性最强，证明小说衍生的规范可迁移至真实世界。

Conclusion: 小说中的规范性模拟体能够有效教授LLM情境隐私推理，且这种能力可迁移至真实世界领域，为隐私对齐提供了一种可扩展、低成本的途径。
...
TL;DR: 本文首次评估了掩码自编码器（MAE）预训练在井下钻井遥测中的应用，利用Utah FORGE两口地热井约350万时间步的多变量钻井遥测数据，在72种MAE配置中系统搜索最优设计，发现最佳MAE配置相比监督GRU基线降低测试MAE 19.8%，但落后监督LSTM基线6.4%。潜空间宽度是主导架构因素（Pearson r=-0.59），而掩码比例影响可忽略。


<details>
  <summary>Details</summary>
Motivation: 钻井遥测中地表传感器数据廉价、连续（1Hz），而井下标注数据昂贵、稀疏且不连续，这种标注不对称性导致传统全监督从头训练方法不适应实际数据分布。

Method: 采用掩码自编码器（MAE）预训练策略，在72种MAE配置上进行全因子设计空间搜索，包括潜空间宽度、掩码比例等维度，并与监督LSTM和GRU基线对比，预测Total Mud Volume。

Result: 最佳MAE配置相比监督GRU基线降低测试MAE 19.8%，但落后监督LSTM基线6.4%。潜空间宽度是主导因素（r=-0.59），掩码比例效果可忽略。

Conclusion: MAE预训练可作为钻井分析的有效范式，尤其在数据不对称场景下优于GRU，但尚不及LSTM；高时间冗余（1Hz数据）导致掩码策略的增益有限。
...
TL;DR: FairyFuse是一个针对CPU平台的高效推理系统，通过将三元权重LLM的矩阵运算转换为AVX-512上的无乘法操作（仅用掩码加减法），实现29.6倍GEMV加速，端到端速度达32.4 tokens/s，比llama.cpp Q4_K_M快1.24倍且质量接近无损。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在CPU平台部署时，内存带宽是自回归生成的主要瓶颈。虽然4-bit量化减轻了内存压力，但反量化后浮点乘法仍限制性能提升。三元权重{-1,0,+1}理论上可用条件加减替代乘法，但现有系统（如Fairy2i）未利用该结构优势。

Method: 提出FairyFuse，将每个线性层中的8个实值子GEMV融合到单个AVX-512循环中，使用掩码加法和减法执行，完全消除浮点乘法操作。通过16倍权重压缩使内存受限的GEMV转向计算受限区域，利用CPU的AVX-512指令集实现高效执行。

Result: 在Intel Xeon 8558P上实现32.4 tokens/s，较llama.cpp Q4_K_M快1.24倍；质量几乎无损（WikiText-2困惑度5.52 vs FP16的5.47；下游准确率66.0%）；GEMV核实现29.6倍加速；Roofline分析显示16倍压缩使GEMV从内存受限转向计算受限。

Conclusion: 三元权重+无乘法推理架构在CPU上具有显著优势，能有效突破内存带宽瓶颈，但对GPU收益不大。该工作为CPU平台高效部署LLM提供了新路径。
...
TL;DR: Absorber LLM通过将长上下文吸收到模型参数中，并采用因果同步优化，实现了在长序列推理中降低内存消耗并提升准确性。


<details>
  <summary>Details</summary>
Motivation: Transformer在长序列推理中计算成本高、内存消耗大；RNN/SSM等常量内存方法丢失长尾依赖；现有参数记忆方法（如TTT）容易过拟合token级投影，且无法保持预训练LLM中上下文的因果效应。

Method: 将长上下文保持建模为自监督的因果同步任务：将历史上下文吸收到参数中后，使无上下文模型在生成未来内容时匹配原始全上下文模型。通过同步更新后模型的内部行为与原始模型来实现优化。

Result: 在长上下文和流式基准测试中，Absorber LLM减少了推理内存，并在先前的参数记忆基线方法上提升了准确性。

Conclusion: Absorber LLM通过参数吸收上下文和因果同步优化，有效解决了长序列推理中的内存-精度权衡问题。
...
TL;DR: 本文提出DexBench基准，通过双重推理任务（预测程序行为+推断输入变异）评估LLM对程序执行因果理解的能力，实验表明该方法能有效衡量动态代码理解。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅关注与特定输入绑定的程序属性预测（如代码覆盖率、输出），对动态代码推理的视角狭窄且易受数据污染，缺乏对程序执行因果理解的真正评估。

Method: 提出程序执行的双重推理框架：(i)预测给定输入的观察行为，(ii)推断如何变异输入以达到特定行为目标。基于此构建包含445对实例的DexBench基准，评估13个LLM。

Result: 双重路径推理能作为动态代码理解的稳健且具区分度的代理指标。

Conclusion: DexBench有效衡量LLM对程序执行的因果理解，优于传统单任务基准。
...
TL;DR: 本文提出GSA（Gist Sparse Attention）方法，通过可学习的gist压缩令牌作为路由信号，实现先粗后细的稀疏注意力机制。该方法端到端可训练，支持分层递归gist-of-gist结构，在对数复杂度下实现多分辨率上下文访问。在LongBench和RAG基准上，以8×到32×压缩比一致优于其他压缩方法和推理时稀疏注意力方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型处理长上下文时，全注意力机制的计算开销呈二次增长。现有缓解方案（KV缓存选择或压缩）各有不足，缺乏一种有效且端到端可学习的桥接方案，且无需修改模型架构。

Method: 提出GSA（Gist Sparse Attention）：1) 将上下文压缩为可学习的gist令牌；2) 选择最相关的gist令牌；3) 恢复对应的原始chunk进行细粒度注意力计算。支持分层递归gist-of-gist结构，实现多分辨率访问，且整个过程端到端训练，无需外部检索模块。

Result: 在LongBench和RAG基准测试上，GSA在8×到32×的压缩比下，一致优于其他压缩基线方法和推理时稀疏注意力方法。

Conclusion: GSA通过可学习的gist令牌实现了压缩与选择的有效桥接，提供了一种简单、高效的长上下文处理方案，无需架构修改且支持端到端训练。
...
TL;DR: 本研究利用All of Us国家数据训练的青光眼风险评估模型，仅通过系统电子健康记录即可在独立机构识别高风险患者，最佳模型AUROC达0.883，PPV为0.657，无需专业影像即可实现可扩展的预筛查。


<details>
  <summary>Details</summary>
Motivation: 传统青光眼筛查依赖专业影像设备，成本高、可及性有限。本研究旨在探索仅利用系统性电子健康记录（非眼科专科数据）进行青光眼风险评估的可行性，以实现更广泛、可扩展的预筛查。

Method: 基于All of Us国家数据预训练青光眼风险评估模型，在斯坦福队列（20,636名患者，15%患青光眼）上进行微调，输入特征包括人口统计学、系统诊断、药物、实验室检查和体格检查数据，在独立测试集上评估性能。

Result: 最佳模型AUROC为0.883，PPV为0.657。预测最高十分位组的青光眼诊断率为65.7%，治疗率为57.0%。性能随可训练层数（最多15层）和数据量增加而提升。

Conclusion: 仅基于电子健康记录的青光眼风险评估模型可实现可扩展、可访问的预筛查，无需依赖专业眼科影像检查，有望在资源有限环境中推广应用。
...
TL;DR: 本文提出ILDR指标(类间/类内距离比)，通过倒数第二层表示的几何度量，在grokking现象中比验证集精度提前9-73%的训练预算检测到泛化转变，具有稳健性和早期预警能力。


<details>
  <summary>Details</summary>
Motivation: 现有grokking检测信号(如权重范数、GrokFast的梯度EMA)存在滞后或不稳定的问题，缺乏能早期、可靠指示泛化转变的几何度量指标。

Method: 提出ILDR = 类间质心距离 / 类内散度，基于Fisher线性判别准则，在倒数第二层表征上计算。无需特征分解，计算复杂度O(|C|² + N)，仅在留出数据上评估。

Result: 在模运算和置换群S5任务上，ILDR领先grokking转变9-73%训练预算；8个随机种子下领先950±250步，变异系数26%；转变后方差下降1696倍；作为早停触发器减少18.6%训练；优化器干预可双向控制转变。

Conclusion: ILDR追踪的是泛化背后的表征条件而非下游相关性，能反映表征空间的几何相变，是一种早期、稳健的grokking检测信号。
...
TL;DR: 本文提出了一种LLM引导的时间模拟框架，通过显式建模生理轨迹实现脓毒症早期可解释预警，在MIMIC-IV和eICU数据集上取得了优于传统方法的性能（AUC 0.861-0.903）。


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动的脓毒症早期预警模型预测准确但缺乏可解释性，限制了医生的信任和临床应用。本文旨在解决这一"黑箱"问题，提供既准确又可解释的预测机制。

Method: 提出LLM引导的时间模拟框架，包含三个核心模块：(1) 时空特征提取模块，捕捉多变量生命体征的动态依赖；(2) 医学Prompt-as-Prefix模块，将临床推理线索嵌入LLM；(3) 基于智能体的后处理组件，将预测约束在生理合理范围内。先模拟关键生理指标演变，再分类脓毒症发作。

Result: 在MIMIC-IV和eICU数据库上，该方法在24-4小时预发作预测任务中取得AUC 0.861-0.903，优于传统深度学习和基于规则的方法。同时提供了可解释的轨迹和风险趋势。

Conclusion: 该方法不仅预测精度高，还提供了与临床判断一致的透明预测机制，可辅助临床医生进行早期干预和个性化决策，具备在重症监护环境中应用的价值。
...
### [164] [Unsupervised Learning of Inter-Object Relationships via Group Homomorphism](https://arxiv.org/abs/2604.20925)
*Kyotaro Ushida,Takayuki Komatsu,Yoshiyuki Ohmura,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: 本文提出一种基于群运算层级关系的无监督表示学习方法，通过引入代数学中的同态约束，使模型无需标签即可同时完成物体分割与运动规律提取，能够将像素变化拆解为有意义的变换成分（如平移、变形），在发育科学设计的交互场景中验证了多物体分割与相对运动表征的正确性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型依赖大量数据的统计相关性学习，缺乏婴儿那种从有限经验中自主获取世界结构并适应新情境的灵活性，尤其是前语言期婴儿的认知发展机制尚未被有效建模。

Method: 提出一种基于群运算层级关系的无监督表示学习方法：在神经网络中引入代数同态作为结构约束，构建一个同时进行物体分割和运动规律提取的集成架构，将像素级变化结构化为有意义的分解变换成分（如平移、形变）。

Result: 在发育科学设计的追逐与逃避交互场景实验中，模型无需任何真实标签即可将多个物体分割为独立槽位；物体间的相对运动（如接近、远离）被准确映射并结构化为一个一维加性潜空间。

Conclusion: 引入代数几何约束（而非仅依赖统计相关性学习）可以获得物理可解释的"解耦表示"，有助于理解婴儿将环境规律内化为结构的过程，并为构建具有发展性智能的人工系统提供新视角。
...
TL;DR: 提出DAHCL框架用于半监督域泛化故障诊断，通过域感知学习校准伪标签偏差，结合层次对比学习利用不确定样本，在三个基准数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有SSDGFD方法存在两个耦合问题：1) 伪标签生成忽略域特定几何差异，导致跨域偏差；2) 对未标记样本采用硬接受/丢弃策略，造成样本利用不均且引入噪声。

Method: 提出DAHCL框架，包含两个核心模块：1) 域感知学习(DAL)模块，捕捉源域几何特征并校准伪标签预测；2) 层次对比学习(HCL)模块，结合动态置信度分层与模糊对比监督，让不确定样本也能参与表征学习。

Result: 在三个基准数据集上的大量实验表明DAHCL方法优于现有方法。

Conclusion: DAHCL通过域感知校准和层次对比学习，有效解决了半监督域泛化故障诊断中的伪标签偏差和样本利用不均衡问题。
...
### [166] [IRIS: Interpolative Rényi Iterative Self-play for Large Language Model Fine-Tuning](https://arxiv.org/abs/2604.20933)
*Wenjie Liao,Like Wu,Liangjie Zhao,Shihui Xu,Shigeru Fujimura*

Main category: cs.LG

TL;DR: IRIS提出基于Rényi散度的自博弈微调框架，通过可调节的α参数统一多种自博弈目标，并设计自适应调度策略，在Zephyr-7B和Qwen2.5-3B上超越现有方法，仅用26k标注样本就超越了全量200k监督微调的性能。


<details>
  <summary>Details</summary>
Motivation: 现有自博弈微调方法（SPIN、SPACE、SPIF等）依赖固定的散度约束，但不同散度在不同训练阶段（模型与目标分布差距不同时）表现各异，缺乏一种能够动态适应训练进程的统一框架。

Method: 提出IRIS框架：1）基于Rényi散度设计可调节目标函数，分解为标注数据和合成数据上的两个独立倾斜风险项，由α参数控制指数重要性权重；2）给出自适应α调度策略，随训练进程从尖锐的重要性加权过渡到平滑的精调；3）理论证明不动点性质，分析α对梯度集中的控制机制。

Result: 在Zephyr-7B和Qwen2.5-3B上跨越10个基准测试，IRIS达到44.57%平均得分，且迭代间持续提升。仅用26k标注样本的IRIS超越了在全量200k数据集上训练的标准监督微调。

Conclusion: IRIS通过Rényi散度的可调节性统一了多种自博弈目标，自适应α调度使其在不同训练阶段都能保持有利的学习动态，在显著减少标注数据需求的同时取得了更优性能。
...
### [167] [Data-Driven Open-Loop Simulation for Digital-Twin Operator Decision Support in Wastewater Treatment](https://arxiv.org/abs/2604.20935)
*Gary Simethy,Daniel Ortiz Arroyo,Petar Durdevic*

Main category: cs.LG

TL;DR: 本文提出CCSS-RS模型，一种受控连续时间状态空间模型，用于污水处理厂数字孪生决策支持。该模型能处理不规则采样和高缺失率数据，在Avedøre基准上相比Neural CDE基线RMSE降低40-46%，并通过四个案例研究验证了其在工况规划、传感器失效等场景下的实用性。


<details>
  <summary>Details</summary>
Motivation: 污水处理厂需要数字孪生式决策支持工具，能够模拟指定控制计划下的系统响应、容忍不规则和缺失的传感数据，并在12-36小时规划周期内保持有效。利用全规模工厂数据满足这些需求仍然是一个开放的工程AI挑战。

Method: 提出CCSS-RS（受控连续时间状态空间模型），将历史状态推断与未来控制和外生变量展开分离。结合类型化上下文编码、增益加权的预设与预测驱动强制、半群一致性展开，以及针对污水处理厂数据重尾和零膨胀特性的Student-t加障碍输出。

Result: 在Avedøre全规模公开基准上（906,815时间步，43%缺失率，1-20分钟不规则采样），CCSS-RS在H=1000上达到RMSE 0.696和CRPS 0.349。相比Neural CDE基线RMSE降低40-46%，相比简化内部变体降低31-35%。四个案例研究验证了操作价值。
...
### [168] [Sink-Token-Aware Pruning for Fine-Grained Video Understanding in Efficient Video LLMs](https://arxiv.org/abs/2604.20937)
*Kibum Kim,Jiwan Kim,Kyle Min,Yueqi Wang,Jinyoung Moon,Julian McAuley,Chanyoung Park*

Main category: cs.LG

TL;DR: 本文发现视觉token剪枝方法在细粒度视频理解任务（如幻觉评估）中性能骤降，归因于“sink tokens”（语义空洞但吸引过多注意力的token），并提出SToP方法通过sink分数抑制这些token，在剪枝90%视觉token时仍显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的视觉token剪枝方法主要在MCQA基准上验证，忽略了细粒度理解任务（如幻觉评估）中对精确视觉定位的需求，导致在这些任务上性能严重下降。本文旨在填补这一空白，识别并解决造成性能退化的关键障碍。

Method: 提出SToP（Sink-Token-aware Pruning）方法，引入sink score量化每个token成为sink token的倾向性，并将其应用于现有的空间和时间剪枝方法中，以抑制sink tokens的影响，从而提升细粒度视频理解能力。

Result: 将SToP应用于VisionZip、FastVid、Holitom等先进剪枝方法，在幻觉评估、开放生成、组合推理和MCQA等多种基准上测试，即使在剪枝90%视觉token时，SToP仍能显著提升性能。

Conclusion: Sink tokens是细粒度视频理解中视觉token剪枝性能下降的关键因素；SToP作为一种轻量级插件方法，能有效抑制sink tokens，在不增加训练负担的前提下大幅提升视频LLM的推理效率和细粒度理解能力。
...
### [169] [HARBOR: Automated Harness Optimization](https://arxiv.org/abs/2604.20938)
*Biswa Sengupta,Jinhua Wang*

Main category: cs.LG

TL;DR: 本文提出智能体框架（harness）设计应被视为一流的机器学习问题，并形式化了自动化框架优化方法，通过贝叶斯优化在混合变量配置空间中搜索最优框架参数，实验表明自动化方法优于手动调优。


<details>
  <summary>Details</summary>
Motivation: 当前长程语言模型智能体的复杂性和代码量主要来自框架（harness）而非底层模型，但框架设计仍依赖手动堆叠，缺乏系统化的自动优化方法。

Method: 将自动化框架优化形式化为带约束的噪声贝叶斯优化问题，提出HARBOR求解器，包含块可加性SAAS代理模型、多保真度成本感知采集函数和TuRBO信任域，并引入冷启动校正奖励和后验概率约束安全检查。

Result: 在代码生成智能体上进行了四轮手动调优对照实验和端到端HARBOR运行，结果表明一旦标志空间超过少量比特，自动化配置搜索显著优于手动堆叠。

Conclusion: 该形式化方法是任务类无关的，配置空间、奖励校正、采集函数和安全检查适用于任何具有有界标志空间和可重复任务套件的智能体框架。
...
### [170] [SCM: Sleep-Consolidated Memory with Algorithmic Forgetting for Large Language Models](https://arxiv.org/abs/2604.20943)
*Saish Sachin Shinde*

Main category: cs.LG

TL;DR: SCM是一种借鉴神经科学原理的大语言模型记忆架构，包含工作记忆、重要性标记、睡眠阶段记忆巩固、价值导向遗忘和自我建模五个核心组件，在八项基准测试中实现十轮对话完美召回，记忆噪声降低90.9%，搜索延迟低于1毫秒。


<details>
  <summary>Details</summary>
Motivation: 现有LLM记忆系统存在上下文窗口截断、向量数据库无限增长、缺乏巩固与遗忘机制等问题，缺少持久、结构化且生物合理的记忆架构。

Method: 设计五核心组件：有限容量工作记忆、多维重要性标记、NREM/REM两阶段离线睡眠巩固、价值导向遗忘机制、以及具备内省能力的计算自模型。

Result: 在八项标准化基准测试中，十轮对话完美召回，记忆噪声降低90.9%，数百概念下搜索延迟低于1毫秒。

Conclusion: 该工作建立了具备巩固、优先排序和遗忘能力的记忆系统架构基础，为推进LLM记忆研究提供了可测试的平台。
...
### [171] [LAF-Based Evaluation and UTTL-Based Learning Strategies with MIATTs](https://arxiv.org/abs/2604.20944)
*Yongquan Yang*

Main category: cs.LG

TL;DR: 本文提出LAF评估算法和UTTL学习策略，在真实目标不确定（存在多个不准确真实目标MIATTs）的机器学习场景中，实现逻辑一致且实践可行的建模，弥合逻辑语义与统计优化之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现实ML应用中，真实目标常因模糊性或主观性而无法精确定义，客观"真实标签"可能不存在。需在目标不确定条件下实现逻辑一致且实践可行的评估与学习。

Method: 1) 分析任务特定MIATTs的覆盖度与多样性对结构特性的影响；2) 提出基于LAF的评估算法（在原始MIATTs或合成三值目标上操作）；3) 提出基于UTTL的学习策略（使用Dice和交叉熵损失，比较每目标与聚合优化方案）；4) 整合LAF与UTTL以连接逻辑语义与统计优化。

Result: 构建了EL-MIATTs框架下从MIATTs分析到评估算法、学习策略的完整实施路径，并提供了应用案例（见https://www.qeios.com/read/EZWLSN）。

Conclusion: 该工作为"真实标签"固有不确定的ML系统开发提供了原理性基础，弥合了理论与实践的差距，使得在目标不确定条件下进行逻辑连贯且实践可行的建模成为可能。
...
### [172] [Early Detection of Latent Microstructure Regimes in Limit Order Books](https://arxiv.org/abs/2604.20949)
*Prakul Sunil Hiremath,Vruksha Arun Hiremath*

Main category: cs.LG

TL;DR: 本文提出了一种基于MAX聚合的触发式检测器，能够在限价订单簿从稳定过渡到压力状态之前，通过识别潜伏积累阶段来实现提前预警，比传统方法提前约18个时间步。


<details>
  <summary>Details</summary>
Motivation: 现有早期预警信号（如订单流不平衡、短期波动率）本质上是反应性的，无法在压力事件发生前提供预警。本文旨在通过识别潜伏恶化阶段来突破这一限制。

Method: 提出三状态因果数据生成过程（稳定→潜伏积累→压力）；设计触发式检测器，包含MAX聚合多信号通道、上升沿条件和自适应阈值；理论推导了潜伏状态的可识别性和提前检测保证。

Result: 在200次模拟中，平均提前18.6±3.2个时间步提前预警，精确率完美，覆盖率中等；在BTC/USDT订单簿数据中表现一致优于传统基线方法。

Conclusion: 该方法在低信噪比和短积累期场景下性能下降，与理论预期一致。验证了通过潜伏状态识别实现订单簿压力提前预警的可行性。
...
### [173] [Differentially Private Model Merging](https://arxiv.org/abs/2604.20985)
*Qichuan Yin,Manzil Zaheer,Tian Li*

Main category: cs.LG

TL;DR: 本文提出两种后处理技术（随机选择和线性组合），利用已训练好的不同隐私/效用权衡的模型集合，无需额外训练即可生成满足任意目标差分隐私要求的模型，并从Rényi DP和隐私损失分布角度进行理论分析，在私有均值估计案例中证明线性组合优于随机选择。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，推理或部署时的隐私要求会因政策、法规或用户体验的变化而频繁变动，现有方法通常需要重新训练模型来适应新的隐私预算，成本高昂且不灵活。

Method: 给定一组在同一数据集上以不同隐私/效用权衡训练好的模型，提出两种后处理技术：(1) 随机选择：从模型池中随机选取一个模型；(2) 线性组合：对多个模型进行线性加权组合。从Rényi差分隐私和隐私损失分布角度提供隐私核算，并在私有均值估计案例中给出完整的隐私/效用理论分析。

Result: 在私有均值估计中，理论上证明了线性组合方法在效用上优于随机选择；在多个合成和真实数据集上的实验验证了两种方法的有效性。

Conclusion: 提出了一种灵活、高效的模型重用框架，能够在不重新训练的情况下满足动态变化的差分隐私要求，为实际部署中的隐私自适应提供了可行方案。
...
### [174] [Droplet-LNO: Physics-Informed Laplace Neural Operators for Accurate Prediction of Droplet Spreading Dynamics on Complex Surfaces](https://arxiv.org/abs/2604.20993)
*Ganesh Sahadeo Meshram,Partha Pratim Chakrabarti,Suman Chakraborty*

Main category: cs.LG

TL;DR: 本文提出物理信息拉普拉斯算子神经网络(PI-LNO)，通过拉普拉斯积分变换作为物理信息函数基，实现对液滴铺展过程的高效瞬态计算，相比传统CFD(18-24小时)大幅提速，并在多表面CFD数据上优于U-Net、DeepONet等五种方法。


<details>
  <summary>Details</summary>
Motivation: 传统CFD模拟液滴铺展过程极为耗时(单次瞬态计算需18-24小时)，亟需一种高效且准确的替代方法。

Method: 提出PI-LNO架构，以拉普拉斯积分变换作为物理信息函数基，采用物理正则化的复合损失函数(融合MSE、MAE、RMSE与Navier-Stokes、Cahn-Hilliard、因果性约束)，在TensorFlow上训练多表面CFD数据(接触角20°-160°)。

Result: 与UNet、UNet-AM、DeepONet、PI-UNet、LNO五种方法对比，PI-LNO在液滴铺展瞬态动力学建模上表现更优。

Conclusion: PI-LNO通过拉普拉斯变换天然建模指数瞬态动力学，为液滴铺展问题提供了一种高效准确的替代方案。
...
### [175] [SGD at the Edge of Stability: The Stochastic Sharpness Gap](https://arxiv.org/abs/2604.21016)
*Fangshuo Liao,Afroditi Kolomvaki,Anastasios Kyrillidis*

Main category: cs.LG

TL;DR: 本文提出随机自稳定机制，将自稳定框架扩展到SGD，解释了为什么mini-batch SGD的sharpness稳定在2/η以下（而GD恰好稳定在2/η），并推导了sharpness gap的闭式解：ΔS = ηβσ_u²/(4α)，表明小批量产生更平坦的解。


<details>
  <summary>Details</summary>
Motivation: GD下sharpness稳定在2/η的Edge of Stability现象已被自稳定机制解释，但SGD下sharpness稳定在更低值（且随batch size减小而降低）的原因缺乏理论解释。

Method: 1) 将自稳定框架扩展到SGD，引入梯度噪声注入方差；2) 定义相对于移动PGD轨迹的随机预测动力学；3) 证明随机耦合定理，约束SGD与这些预测的偏差；4) 推导sharpness gap的闭式解。

Result: 推导出平衡sharpness gap公式：ΔS = ηβσ_u²/(4α)，其中α是渐进锐化率，β是自稳定强度，σ_u²是沿顶部特征向量的梯度噪声方差。该公式预测小批量产生更平坦的解，且在全数据下恢复GD结果。

Conclusion: SGD的sharpness抑制由梯度噪声通过自稳定机制解释，为理解小批量训练带来隐式正则化提供理论依据。
...
### [176] [MCAP: Deployment-Time Layer Profiling for Memory-Constrained LLM Inference](https://arxiv.org/abs/2604.21026)
*Anurita Das*

Main category: cs.LG

TL;DR: 本文提出MCAP（蒙特卡洛激活 profiling）方法，通过加载时逐层重要性估计，在异构硬件上实现动态精度分配（W4A8 vs W4A16）和内存层级决策（GPU/RAM/SSD），构建的NVE系统在不修改权重的情况下，在NVIDIA T4上达到1.5-1.8倍于llama.cpp Q4_0的解码吞吐量，使模型能在之前不可行的内存预算下运行。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署到异构硬件时的主要瓶颈是内存而非计算，现有方案无法在多样化的内存预算下灵活运行，需要一种无需修改权重即可适应不同内存约束的方法。

Method: 提出MCAP（蒙特卡洛激活 profiling）作为加载时的逐层重要性估计器，生成轻量级逐层信号，同时驱动精度调度（W4A8/W4A16选择）和驻留层级决策（GPU/RAM/SSD），使单一权重集能在不同内存预算下运行。

Result: NVE系统在NVIDIA T4上相比llama.cpp Q4_0实现了1.5-1.8倍的解码吞吐量提升，并支持模型在之前因内存不足而不可行的场景下运行。

Conclusion: 通过MCAP的轻量级重要性估计，可以在不修改权重的前提下实现异构硬件上的自适应精度和内存管理，显著提升部署效率和硬件兼容性。
...
### [177] [mcdok at SemEval-2026 Task 13: Finetuning LLMs for Detection of Machine-Generated Code](https://arxiv.org/abs/2604.21365)
*Adam Skurla,Dominik Macko,Jakub Simko*

Main category: cs.LG

TL;DR: 本文针对SemEval-2026 Task 13中多编程语言机器生成代码检测任务，改编了mdok方法，探索了更适合代码理解的基座模型，在三项子任务中表现有竞争力但仍有提升空间。


<details>
  <summary>Details</summary>
Motivation: 多语言机器生成代码的检测是一项具有挑战性的任务，现有方法多针对自然语言文本，缺乏对代码特有语义结构的适应。

Method: 基于mdok方法（原用于机器生成文本检测），替换为更适合代码理解的基座模型，适配三个子任务：二元检测、生成源归属、LLM家族分类及混合/对抗性代码检测。

Result: 提交系统在所有三个子任务中均表现出竞争力，但与顶尖系统仍有显著差距。

Conclusion: 当前方法可行但性能有限，进一步改进是必要的。
...
TL;DR: 本文开发了一种基于深度学习的U-Net代理模型，用于高效预测洪水淹没范围的最大水位，在德国Wupper流域的测试中取得了与传统水动力模拟相当的结果。
...
Method: 通过一系列实验优化U-Net架构、分块生成策略和数据预处理，构建深度学习代理模型来近似水动力模型，预测网格上的最大水位。

Result: 在德国北莱茵-威斯特法伦地区Wupper流域的测试中，该代理模型获得了与传统水动力模拟相当的结果。
...
TL;DR: 本研究首次系统比较了传统重采样技术与深度学习模型在教育数据合成中的表现，发现两者存在效用-隐私权衡：重采样方法效用近乎完美但无隐私保护，深度学习模型能提供强隐私保证但效用显著下降。变分自编码器（VAE）是最佳折衷方案，在完全保护隐私的同时保持了83.3%的预测性能。


<details>
  <summary>Details</summary>
Motivation: 教育技术领域面临数据稀缺和隐私问题，但从业者缺乏关于选择传统重采样方法还是现代深度学习方法来生成合成数据的实证指导。

Method: 使用10,000条学生成绩数据集，对比三种重采样方法（SMOTE、Bootstrap、随机过采样）与三种深度学习模型（自编码器、变分自编码器、Copula-GAN），从分布保真度、机器学习效用和隐私保护三个维度进行系统评估。

Result: 重采样方法TSTR得分达0.997（近乎完美效用），但DCR约0.00（完全无隐私保护）；深度学习模型DCR约1.00（强隐私保证），但效用显著下降。VAE作为最优折衷，在保证完全隐私保护的前提下维持了83.3%的预测性能。

Conclusion: 建议在隐私可控的内部开发中使用传统重采样，在需要外部数据共享时使用VAE。本研究为学习分析中的合成数据生成建立了基础基准和实践决策框架。
...
TL;DR: 提出一种学习最优分位数回归树集合的新方法，可在不牺牲算法效率的前提下，提供可解释的、无需先验假设的完整条件分布预测。


<details>
  <summary>Details</summary>
Motivation: 机器学习领域越来越需要既准确又具有可解释性和鲁棒性的模型，以便最终用户能够理解和信任AI系统。

Method: 提出一种学习一组最优分位数回归树的新方法，通过集成多棵树来预测目标变量的完整条件分布，同时保持单棵树的学习效率。

Result: 该方法具有三个优势：(1)无需先验分布假设即可预测完整条件分布；(2)预测结果可解释；(3)相比单棵树学习，算法效率不降低。

Conclusion: 该方法在保持可解释性的同时，实现了对完整条件分布的预测，且算法效率与单棵树相当，有助于构建可信赖的AI系统。
...
TL;DR: 本文提出一种结合FlexMatch半监督损失与LeJEPA潜空间正则化的新方法，通过显式塑造几何表示（促使表示呈现各向同性高斯结构）来缓解FixMatch类方法中多数类主导和早期伪标签噪声问题，在多个数据集上超越基线并显著加速收敛。


<details>
  <summary>Details</summary>
Motivation: 现有FixMatch类半监督方法存在两大瓶颈：1）多数类主导学习过程，且错误伪标签会放大偏差；2）早期噪声伪标签阻碍模型形成清晰决策边界，需要长时间训练。因此需要一种新的范式来克服这些问题。

Method: 将FlexMatch的半监督损失与LeJEPA框架的潜空间正则化项结合，鼓励潜在表示呈现各向同性高斯结构，从而显式塑造几何表示，替代传统的逻辑输出阈值方法。

Result: 在CIFAR-100、STL-10和Tiny-ImageNet上的实验表明，该方法一致优于现有基线，同时显著加速收敛，大幅降低计算成本。

Conclusion: 通过将表示学习正则化引入半监督学习，可以有效缓解类别偏差和伪标签噪声问题，实现更快收敛和更好性能，为半监督学习提供了一种超越传统阈值范式的新方向。
...
### [182] [PrismaDV: Automated Task-Aware Data Unit Test Generation](https://arxiv.org/abs/2604.21765)
*Hao Chen,Arnab Phani,Sebastian Schelter*

Main category: cs.LG

TL;DR: PrismaDV是一个复合AI系统，通过分析下游任务代码和数据集特征，自动生成任务感知的数据单元测试，并利用SIFTA框架优化提示词以适应特定任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据验证框架是任务无关的，无法考虑消费数据的代码语义和需求，导致数据单元测试不能反映真实数据错误对下游应用的影响。

Method: PrismaDV包含两个核心部分：(1) 复合AI系统，分析下游任务代码与数据集特征，识别数据访问模式并生成任务感知的可执行单元测试；(2) SIFTA框架，利用单元测试和下游任务执行的稀缺结果进行提示词优化，实现任务自适应。

Result: 在涵盖5个数据集、60个任务的两个新基准测试中，PrismaDV在生成反映数据错误端到端影响的单元测试方面，一致优于任务无关和任务感知的基线方法。SIFTA自动学习的提示词优于手工编写或通用优化器生成的提示词。

Conclusion: PrismaDV通过任务感知的数据单元测试有效提升了数据质量保障的可靠性，SIFTA框架能够自动适应特定任务和数据集的提示词优化，具有实用价值。
...
TL;DR: 提出了TravelFraudBench (TFG)，一个可配置的图神经网络基准测试，专门用于检测旅游平台上的欺诈团伙。包含三种旅游特有的欺诈拓扑（票务欺诈星型、幽灵酒店二分团、账户劫持链），异构图含9种节点和12种边类型。评估六种方法，GraphSAGE和RGCN-proj表现最佳，远超MLP基线。


<details>
  <summary>Details</summary>
Motivation: 现有基准（YelpChi、Amazon-Fraud等）仅覆盖单一节点类型或领域通用模式，缺乏评估结构各异的欺诈团伙拓扑的机制。旅游平台存在独特的欺诈模式（票务欺诈、幽灵酒店、账户劫持），需专门的基准测试。

Method: 构建含9种节点类型和12种边类型的异构图，模拟三种旅游特定欺诈团伙拓扑。采用基于团伙的数据划分（每个团伙完整出现在一个分区中），消除标签泄露。评估MLP、GraphSAGE、RGCN-proj、HAN、RGCN、PC-GNN六种方法。

Result: GraphSAGE AUC=0.992，RGCN-proj AUC=0.987，分别超出MLP基线5.5和5.0个百分点。在团伙恢复任务（>=80%成员同时被标记）中，GraphSAGE达100%恢复率，MLP仅17-88%。边类型消融实验显示device和IP共现是关键信号。

Conclusion: 图结构显著增强了欺诈检测的判别能力；HAN表现与MLP基线持平为负面结果；device和IP共现是最重要的边特征。TFG已作为开源Python包发布，支持PyG、DGL和NetworkX导出。
...
TL;DR: 本文提出LoGraB（局部图基准测试）和AFR（自适应保真度驱动重建）方法，用于解决图数据碎片化、分布式和隐私敏感场景下的图神经网络评估与重建问题。LoGraB通过三种策略和四个控制参数将标准数据集分解为碎片化基准，AFR则利用保真度度量、RANSAC-Procrustes对齐和Bundle Adjustment等方法从噪声谱碎片中重建高质量图结构。实验表明AFR在7/9数据集上达到最佳F1分数，且在差分隐私下仍保持75%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络基准测试假设图数据集中可用，但实际应用中图数据往往是局部化、碎片化、有噪声且存在隐私泄露风险的。缺乏一个统一的框架来评估在这种碎片化场景下图模型的性能。

Method: 1) LoGraB: 通过邻域半径d、谱质量k、噪声水平σ和覆盖率p四个控制参数，采用三种分解策略生成碎片化基准。2) AFR: 使用间隙-截断稳定性比与结构熵结合的保真度度量评估碎片质量，通过RANSAC-Procrustes对齐、自适应拼接和Bundle Assembly恢复大型保真岛。3) 理论证明包括热核边恢复条件、Davis-Kahan摄动稳定性和有界对齐误差。

Result: 在9个基准数据集上，AFR在7/9数据集上取得最佳F1分数；在(ε,δ)-高斯差分隐私下，ε=2时AFR保持75%的无防御F1性能。LoGraB有效揭示了不同模型在碎片化场景下的优劣势。

Conclusion: 提出的LoGraB框架填补了碎片化图学习基准测试的空白，AFR方法在噪声碎片重建和隐私保护方面表现优异，为分布式和隐私敏感的图学习应用提供了有效的解决方案。
...
TL;DR: 本文针对现有delta规则递归（一阶近似）忽略了最小二乘损失曲率的问题，提出引入预处理（preconditioning）来改进递归注意力机制，推导了线性注意力与delta规则在精确预处理下的等价性，并提出对角近似实现，在DeltaNet、GDN、KDA上得到预处理变体，实验表明在合成召回基准和340M/1B规模语言建模上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有次二次递归算子（如Mamba-2、DeltaNet等）中的delta规则递归仅是最小二乘在线学习的一阶近似，忽略了损失函数的曲率信息（Hessian矩阵），导致优化效率不足。

Method: 从在线最小二乘理论出发，推导线性注意力与delta规则在精确预处理下的等价性；提出对角近似的预处理方法，避免精确预处理的过高计算成本；将预处理机制引入DeltaNet、GDN、KDA，并设计高效的分块并行算法。

Result: 在合成召回基准和340M/1B规模的语言建模任务上，预处理后的delta规则递归相比原始版本取得一致且显著的性能提升。

Conclusion: 引入预处理（曲率信息）能有效改进delta规则递归的优化质量，为递归注意力机制提供了一条有前景的改进方向。
...
### [186] [A Hybridizable Neural Time Integrator for Stable Autoregressive Forecasting](https://arxiv.org/abs/2604.21101)
...
TL;DR: 提出了一种混合方法，将自回归变压器嵌入基于打靶法的混合有限元方案中，实现了混沌动力系统长期预测的可证明稳定性，在模型参数减少65倍的同时超越现代基础模型，并在聚变组件上实现了9000倍加速。


<details>
  <summary>Details</summary>
Motivation: 混沌动力系统的自回归建模在长期预测中面临训练和推理的稳定性挑战（如梯度爆炸），现有科学基础模型难以解决这一问题。

Method: 将自回归Transformer嵌入基于打靶法（shooting）的混合有限元方案中，暴露拓扑结构以实现可证明稳定性；结合视觉Transformer生成保持结构动力学的潜在令牌。

Result: 前向问题中证明离散能量守恒；训练中证明梯度一致有界（避免梯度爆炸）；模型参数减少65倍，实现混沌系统的长期预测；聚变组件的"迷你基础模型"仅需12次仿真即可训练实时代理，速度提升9000倍。

Conclusion: 该混合框架有效解决了混沌动力系统自回归建模的稳定性问题，为科学基础模型提供了一种参数高效且可证明稳定的新范式。
...
### [187] [How Much Is One Recurrence Worth? Iso-Depth Scaling Laws for Looped Language Models](https://arxiv.org/abs/2604.21106)
*Kristian Schwethelm,Daniel Rueckert,Georgios Kaissis*

Main category: cs.LG

TL;DR: 本文通过116次预训练实验，拟合出循环语言模型的缩放定律，发现循环次数r带来的参数等效指数φ=0.46——介于0和1之间，意味着每次额外循环在同等训练计算量下会带来可预测的验证损失增加，例如4次循环的410M模型性能相当于580M非循环模型，但训练成本却高达1B非循环模型。


<details>
  <summary>Details</summary>
Motivation: 量化循环语言模型中额外一次循环（depth-recurrence）在等效参数上的价值，为设计循环深度提供可预测的指导。

Method: 在116次预训练运行中（循环次数r∈{1,2,4,8}，训练计算量跨度约50倍），拟合联合缩放定律 L = E + A·(N_once + r^φ·N_rec)^{-α} + B·D^{-β}，通过R²=0.997恢复出循环等效指数φ=0.46。

Result: φ=0.46表明循环的收益介于"完全等效为独立层"(φ=1)和"无容量增益"(φ=0)之间。以r=4为例，410M循环模型性能≈580M非循环模型，但训练成本≈1B非循环模型。下游评估中，参数知识任务上差距持续存在，简单开放书任务上差距消失，推理任务在预算内无法解决。

Conclusion: φ指数可将循环次数r的设计选择转化为可预测的验证损失成本，未来架构和训练方案可以通过能否将φ提升至0.46以上来衡量其改进效果。
...
### [188] [TabSHAP](https://arxiv.org/abs/2604.21120)
*Aryan Chaudhary,Prateek Agarwal,Tejasvi Alladi*

Main category: cs.LG

TL;DR: 本文提出TabSHAP，一个模型无关的可解释性框架，用于LLM-based表格分类器。它通过Shapley-style采样估计和Jensen-Shannon散度量化每个特征对类别分布的影响，而非简单的预测翻转。实验表明TabSHAP在忠实度上显著优于随机基线和XGBoost代理方法。


<details>
  <summary>Details</summary>
Motivation: LLM在表格数据分类上表现优异，但缺乏可解释性，现有方法（如全局线性代理或概率偏移）无法捕捉模型完整的概率不确定性，限制了高安全领域的部署。

Method: 基于Shapley值的采样估计器，使用Jensen-Shannon散度（JSD）衡量完整输入与遮蔽输入之间的类别分布差异，以此量化每个特征的分布影响。遮蔽操作在序列化的key:value字段级别（而非子词token级别）进行，以对齐表格语义。此外还进行了KL散度和L1距离的消融实验。

Result: 在Adult Income和Heart Disease基准上，TabSHAP成功隔离出关键诊断特征，忠实度显著高于随机基线和XGBoost代理。JSD在删除忠实度上优于KL和L1。

Conclusion: TabSHAP为LLM-based表格分类器提供了更忠实、分布级别的特征归因方法，字段级遮蔽和JSD度量是提升可解释性的关键设计。
...
### [189] [Graph Neural Network-Informed Predictive Flows for Faster Ford-Fulkerson and PAC-Learnability](https://arxiv.org/abs/2604.21175)
*Eleanor Wiesler,Trace Baxley*

Main category: cs.LG

TL;DR: 提出一种结合图神经网络与Ford-Fulkerson算法的学习增强框架，通过学习边重要性概率来引导增广路径选择，加速最大流计算和图像分割，在保持最优性的同时减少增广次数。


<details>
  <summary>Details</summary>
Motivation: 传统最大流算法（如Ford-Fulkerson）效率受限于增广路径的选择策略，而现有学习方法要么直接预测初始流（可能破坏最优性），要么需要重复推理。作者希望利用图神经网络学习结构信息来引导算法过程，在保持最大流/最小割最优性的前提下提升计算效率。

Method: 1) 设计MPGNN联合学习节点和边嵌入，捕捉全局结构与局部流动态（如残差容量、瓶颈）；2) 基于图像构建网格流网络，单次GNN推理为边分配重要性概率；3) 将这些概率存入优先队列，指导修改后的Ford-Fulkerson过程（Edmonds-Karp风格搜索+瓶颈感知的tie-breaking）；4) 提出双向路径构建策略，聚焦高概率边；5) 引入加权排列距离指标理论分析预测质量与效率的关系。

Result: 该方法在保持最大流/最小割最优性的同时，在实践中减少了增广次数。建立了预测质量与计算效率之间的理论联系（通过加权排列距离度量）。

Conclusion: 该工作为学习引导的组合优化在图像分割中的应用奠定了基础，提出了一种可扩展的混合扩展方案（流预热启动+边优先级预测），表明将GNN的结构学习能力与经典算法结合可以在不牺牲最优性的情况下提升效率。
...
### [190] [Toward Efficient Membership Inference Attacks against Federated Large Language Models: A Projection Residual Approach](https://arxiv.org/abs/2604.21197)
*Guilin Deng,Silong Chen,Yuchuan Luo,Yi Liu,Songlei Wang,Zhiping Cai,Lin Liu,Xiaohua Jia,Shaojing Fu*

Main category: cs.LG

TL;DR: 提出ProjRes方法，利用投影残差对联邦大语言模型进行成员推理攻击，无需影子模型即可达到近100%的准确率，揭示了FedLLMs中此前被忽视的隐私漏洞。


<details>
  <summary>Details</summary>
Motivation: 尽管FedLLMs通过数据本地化保护隐私，但共享梯度仍可能泄露敏感信息。现有成员推理攻击因FedLLMs参数规模大、收敛快、梯度稀疏非正交等特性而失效，亟需针对性的攻击方法。

Method: ProjRes利用隐藏嵌入向量作为样本表示，通过分析其在梯度子空间上的投影残差，揭示梯度与输入之间的内在联系。无需影子模型、辅助分类器或历史更新。

Result: 在四个基准和四个LLM上，ProjRes达到近100%的准确率，优于现有方法最多75.75%，且在强差分隐私防御下依然有效。

Conclusion: FedLLMs存在严重的隐私漏洞，现有安全假设需要重新审视，ProjRes为被动成员推理攻击提供了高效鲁棒的解决方案。
...
### [191] [ARFBench: Benchmarking Time Series Question Answering Ability for Software Incident Response](https://arxiv.org/abs/2604.21199)
...
TL;DR: 本文提出了ARFBench，一个面向时间序列异常检测的问答基准，评估多模态基础模型在软件故障数据上的理解能力。GPT-5达到62.7%准确率，而模型与人类专家结合的oracle系统达到87.2%准确率，超越人类水平。


<details>
  <summary>Details</summary>
Motivation: 时间序列问答（TSQA）是一个有前景但尚未充分探索的基础模型能力领域，缺乏针对软件故障数据中时间序列异常理解的标准化评估基准。

Method: 构建了包含750个问题、142个时间序列、538万数据点的ARFBench基准，数据来自Datadog的63个真实生产故障。评估了多种LLM、VLM和时间序列FM，并开发了TSFM+VLM混合原型模型。

Result: GPT-5以62.7%准确率和51.9%F1领先；TSFM+VLM混合模型表现与前沿模型相当；模型-专家oracle达到82.8%F1和87.2%准确率，建立超人类新基准。

Conclusion: 多模态方法在TSQA任务上展现出潜力，模型与人类专家具有互补优势，oracle系统的超人类性能为未来TSQA模型设立了新标杆。
...
### [192] [The Recurrent Transformer: Greater Effective Depth and Efficient Decoding](https://arxiv.org/abs/2604.21215)
*Costin-Andrei Oncescu,Depen Morwani,Samy Jelassi,Alexandru Meterez,Mujin Kwun,Sham Kakade*

Main category: cs.LG

TL;DR: 本文提出Recurrent Transformer，通过让每层自回归地注意自身激活计算出的键值对，实现逐层循环记忆，在保持标准自回归解码成本的同时，获得无限时间深度。论文还给出一种基于tiling的精确算法，将HBM流量从Θ(N²)降至Θ(N log N)，在150M和300M参数规模上困惑度优于参数匹配的Transformer基线。


<details>
  <summary>Details</summary>
Motivation: Transformer在时间维度上较浅（深度受层数限制），而循环模型虽然具有无限时间深度但优化不稳定且难以充分利用现代加速器。作者希望结合两者优点，获得既有深度循环又有高效并行计算的架构。

Method: 每层注意力计算时，query来自当前层激活，key/value来自同一层上一时间步的激活（而非像标准Transformer那样来自前一层），形成层内循环记忆。同时提出基于tiling的精确算法解决带宽瓶颈问题。

Result: 在150M和300M参数C4预训练任务上，Recurrent Transformer在困惑度指标上优于参数匹配的标准Transformer，且能用更少层数达到同等效果（固定参数下），减少了KV缓存内存占用和推理延迟。

Conclusion: 循环可以换取宽度（trade depth for width），在不增加参数量的情况下通过层内循环记忆提升表达能力，同时降低推理成本。
...
### [193] [Learning Dynamic Representations and Policies from Multimodal Clinical Time-Series with Informative Missingness](https://arxiv.org/abs/2604.21235)
*Zihan Liang,Ziwen Pan,Ruoxuan Xiong*

Main category: cs.LG

TL;DR: 提出一个多模态临床时序表征学习框架，通过显式利用信息性缺失（structured measurements和clinical notes的观察模式），结合多模态编码器、贝叶斯滤波模块和下游任务模块，在ICU脓毒症数据集上显著提升了离线治疗策略学习和不良结局预测性能。


<details>
  <summary>Details</summary>
Motivation: 多模态临床记录结构化数据和文本数据各自拥有不同的记录过程，观察模式本身携带患者潜在状态的信息，但现有方法未充分利用这种信息性缺失。

Method: 提出三部分框架：(1) 多模态编码器，从结构化和文本数据及其观察模式中捕获信号；(2) 贝叶斯滤波模块，随时间更新潜在患者状态；(3) 下游模块，用于离线治疗策略学习和患者结局预测。

Result: 在MIMIC-III、MIMIC-IV和eICU的ICU脓毒症队列上评估，离线治疗策略学习FQE达0.679（vs 临床行为0.528），72小时后死亡率预测AUROC达0.886。

Conclusion: 显式建模信息性缺失能有效提升多模态临床时序表征的学习质量，在治疗策略学习和预后预测上均优于基线。
...
### [194] [CAP: Controllable Alignment Prompting for Unlearning in LLMs](https://arxiv.org/abs/2604.21251)
*Zhaokun Wang,Jinyu Guo,Jingwen Pu,Hongli Pu,Meng Yang,Xunlei Chen,Jie Ou,Wenyi Li,Guangchun Luo,Wenhong Tian*

Main category: cs.LG

TL;DR: 提出CAP框架，通过强化学习优化的可学习提示实现无需修改参数的可控知识遗忘，支持知识可逆恢复。


<details>
  <summary>Details</summary>
Motivation: 现有参数修改方法存在高计算成本、遗忘边界不可控、依赖模型权重访问等局限，而现有非侵入式方法缺乏系统性，因此需要一种无需修改参数、可控且可逆的知识遗忘方法。

Method: CAP框架将遗忘任务解耦为基于强化学习的可学习提示优化过程，提示生成器与LLM协同工作，在抑制目标知识的同时选择性保留通用能力。

Result: 实验表明CAP实现了精确、可控的知识遗忘，无需更新模型参数，克服了先前方法在可迁移性上的局限。

Conclusion: CAP建立了一种动态对齐机制，通过提示撤销实现知识的可逆恢复，为闭源模型的知识遗忘提供了实用方案。
...
### [195] [Improving Performance in Classification Tasks with LCEN and the Weighted Focal Differentiable MCC Loss](https://arxiv.org/abs/2604.21252)
*Pedro Seber,Richard D. Braatz*

Main category: cs.LG

TL;DR: 本文扩展了LASSO-Clip-EN (LCEN)算法，使其适用于分类任务，并评估了diffMCC损失函数。实验表明，LCEN在分类任务中保持了可解释性和稀疏性，优于多数对比模型；diffMCC损失函数训练的模型表现显著优于加权交叉熵损失。


<details>
  <summary>Details</summary>
Motivation: 原有LCEN算法仅限于回归任务，作者希望将其扩展至分类任务，同时保持其可解释性和特征选择能力。

Method: 对LCEN算法进行修改以适配分类任务，在四个二分类和多分类数据集上评估，并与10种模型对比；同时引入加权可微MCC（diffMCC）损失函数。

Result: LCEN在分类任务中达到高宏F1分数和MCC，平均消除56%的输入特征；LCEN选择的特征可显著提升其他模型的性能。diffMCC损失函数相比加权交叉熵损失，宏F1平均提高4.9%，MCC提高8.5%。

Conclusion: LCEN成功扩展至分类任务，兼具高性能和解释性；diffMCC损失函数是比加权交叉熵更优的替代方案，能训练出更准确的模型。
...
### [196] [Hyperloop Transformers](https://arxiv.org/abs/2604.21254)
*Abbas Zeitoun,Lucas Torroba-Hennigen,Yoon Kim*

Main category: cs.LG

TL;DR: 本文提出Hyper-Connected Looped Transformer（Hyperloop Transformer），通过循环使用Transformer层和超连接技术，在参数量减少约50%的情况下，性能仍优于深度匹配的Transformer和mHC Transformer基线，适用于内存受限的场景如边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 许多应用（如边缘计算和终端部署）受到模型内存占用的限制，因此需要参数高效的语言模型架构。现有LLM架构研究主要关注在固定计算/延迟预算下最大化模型质量，缺乏对参数效率的专门优化。

Method: 提出Hyperloop Transformer架构：1）将循环Transformer组织为三个块（起始块、中间块、结束块），仅中间块在深度上循环复用，节省参数；2）在每次循环后引入超连接（hyper-connections），将残差流扩展为矩阵值残差流，仅增加极少的参数和计算成本。

Result: 在不同模型规模下，Hyperloop Transformer在使用约50%更少参数的情况下，性能优于深度匹配的Transformer和mHC Transformer基线。该优势在训练后的权重量化后仍然保持。

Conclusion: Hyperloop Transformer是一种有吸引力的内存高效语言建模架构，适合边缘设备和终端部署等内存受限场景。
...
### [197] [Measure Twice, Click Once: Co-evolving Proposer and Visual Critic via Reinforcement Learning for GUI Grounding](https://arxiv.org/abs/2604.21268)
*Wenkai Wang,Xiyun Li,Hongcan Guo,Wenhao Yu,Tianqing Fang,Haitao Mi,Dong Yu,Shengyu Zhang*

Main category: cs.LG

TL;DR: 本文提出一种可学习的提案-批评协同进化框架，通过强化学习动态平衡定位与批评能力，显著提升GUI视觉定位的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有GUI视觉定位方法在密集布局和视觉同质元素上难以精确锁定目标，静态自一致性策略（如几何聚类）因预测分散而效果有限。

Method: 提出Propose-then-Critic框架，用可学习的筛选机制替代静态策略；引入成熟度感知的自适应协同进化强化学习，动态平衡提案者（proposer）与批评者（critic）的训练目标。

Result: 在6个基准测试上显著提升了定位精度和批评可靠性。

Conclusion: 该方法通过提案与批评能力的相互促进与协同进化，增强了模型对多样化复杂界面布局的泛化能力。
...
### [198] [Understanding and Mitigating Spurious Signal Amplification in Test-Time Reinforcement Learning for Math Reasoning](https://arxiv.org/abs/2604.21327)
*Yongcan Yu,Lingxiao He,Jian Liang,Kuangpu Guo,Meng Wang,Qianlong Xie,Xingxing Wang,Ran He*

Main category: cs.LG

TL;DR: 本文提出 DDRL 框架，针对测试时强化学习（TTRL）中由伪标签噪声引发的虚假优化信号问题，通过频率采样去歧义、去偏优势估计和共识驱动离线改进三个阶段，有效缓解奖励噪声，在多个数学推理任务上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: TTRL 在推理时通过伪标签进行模型自适应，但容易受到标签噪声产生的虚假优化信号的干扰。作者通过实证发现中等一致性响应构成模糊区域，是奖励噪声的主要来源，且这种噪声会被组相对优势估计放大。

Method: 提出 DDRL 统一框架，包含三个核心组件：1）基于频率的采样策略，排除模糊样本并保持正负样本均衡；2）去偏优势估计，使用固定优势值消除组相对策略优化引入的偏差；3）基于共识的离线改进阶段，利用拒绝采样数据集实现高效稳定的模型更新。

Result: 在三个大型语言模型上、多个数学推理基准测试中，DDRL 持续优于现有的 TTRL 基线方法。

Conclusion: DDRL 通过系统性地去除模糊样本和偏差，显著提升了测试时强化学习的鲁棒性和效果，代码即将开源。
...
### [199] [Sub-Token Routing in LoRA for Adaptation and Query-Aware KV Compression](https://arxiv.org/abs/2604.21335)
*Wei Jiang,Wei Wang*

Main category: cs.LG

TL;DR: 本文提出子令牌路由（Sub-token Routing）机制，在LoRA适配的Transformer中研究令牌内部价值组的路由，通过查询无关和查询感知两种设计，实现更细粒度的KV压缩，在语言建模质量和下游任务表现之间取得更好权衡。


<details>
  <summary>Details</summary>
Motivation: 传统效率优化以令牌、页面、头或层为粗粒度单元，忽略了令牌内部表示的非均匀性——相关令牌内部价值组分布不均，KV压缩不应在令牌层面做全有或全无的决策。

Method: 提出两种设计：1）查询无关设计：结合路由子空间LoRA与价值组路由，用于压缩感知的语言建模；2）查询感知设计：基于预测器的选择器，根据查询条件为上下文令牌/价值组对分配全局保留预算，用于下游任务保留的KV压缩。

Result: 查询无关设计改善了语言建模的质量-压缩权衡；查询感知设计在降低KV预算下保留了下游任务性能。两者构成互补压缩轴：令牌级决定哪些令牌全局存活，子令牌级决定存活令牌如何被内部压缩。

Conclusion: 子令牌路由为Transformer效率提供了比粗粒度单元更细致的控制轴，令牌级与子令牌级路由互为补充，共同优化压缩效果。
...
### [200] [Decoupled Travel Planning with Behavior Forest](https://arxiv.org/abs/2604.21354)
*Duanyang Yuan,Sihang Zhou,Yanning Hou,Xiaoshu Chen,Haoyuan Chen,Ke Liang,Jiyuan Liu,Chuan Ma,Xinwang Liu,Jian Huang*

Main category: cs.LG

TL;DR: 提出行为森林方法（Behavior Forest），将多约束旅行规划问题分解为并行行为树森林，每棵树负责一个子任务，通过全局协调机制实现模块化规划，利用LLM作为树节点的决策引擎进行局部推理，显著提升复杂多约束旅行规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划方法将所有子任务和约束纠缠在单一决策空间中，无法区分局部约束（子任务内）和全局约束（跨子任务），导致模型在每一步都需要联合推理局部和全局约束，推理负担重、规划效率低。

Method: 将决策过程构建为并行行为树森林，每棵行为树负责一个子任务；引入全局协调机制编排树间交互；LLM作为行为树节点的决策引擎，基于任务特定约束进行局部推理，根据协调反馈调整决策；行为树提供显式控制结构引导LLM生成。

Result: 在TravelPlanner benchmark上提升6.67%，在ChinaTravel benchmark上提升11.82%，优于现有最先进方法。

Conclusion: 所提方法通过将复杂任务和约束解耦为可管理的子空间，实现了任务特定推理，降低了LLM的认知负担，有效提升了LLM在复杂多约束旅行规划中的性能。
...
TL;DR: 本文提出了一种严格去通道化的人类活动识别框架，通过独立通道编码、元数据条件融合和联合损失优化，实现在异构传感器环境下无需预设通道结构的通用推理。


<details>
  <summary>Details</summary>
Motivation: 物联网环境中人类活动识别面临传感器设置异构问题（不同数据集、设备、身体位置、感知模态和通道组成），传统通道固定模型因输入表示与预定义通道结构紧密耦合而难以跨环境复用。

Method: 提出通道无关HAR框架：(1) 通道级编码+共享编码器独立处理每个通道；(2) 基于条件批归一化的元数据条件后期融合，利用身体位置、模态、轴等元数据恢复结构信息；(3) 联合优化损失同时优化通道级预测和最终融合预测。

Result: 在PAMAP2数据集上实验，并在六个HAR数据集上进行鲁棒性分析、消融研究、敏感性分析、效率评估和跨数据集迁移学习，验证了方法的有效性。

Conclusion: 融合设计是通道无关HAR的核心问题，所提方法通过独立通道处理+元数据信息补偿+联合损失优化，有效解决了异构传感器环境下的模型复用难题。
...
TL;DR: 本文研究通过自微分同胚在$\mathbb{R}^n$中重新定位紧集，并证明任何有限个紧集可通过嵌入到$\mathbb{R}^{n+1}$实现线性可分。应用方面，证明有限个紧数据集在温和条件下可由宽度为$n$的深度神经网络（使用Leaky-ReLU、ELU或SELU激活函数）实现线性可分，且互不相交的紧数据集可在$\mathbb{R}^{n+1}$中由宽度为$(n+1)$的DNN实现线性可分。


<details>
  <summary>Details</summary>
Motivation: 紧集在流形上通过自微分同胚的重新定位本身具有理论意义，并且在数据科学中的数据分类任务中有重要潜在应用。

Method: 提出了一个理论框架，利用$\mathbb{R}^n$上的微分同胚将有限个紧集重新定位到任意目标域，并进一步证明了存在可微嵌入到$\mathbb{R}^{n+1}$中使这些紧集的像线性可分。基于该理论，分析了使用特定激活函数（Leaky-ReLU、ELU、SELU）的深度神经网络的宽度条件。

Result: 1) 有限个紧数据集在$\mathbb{R}^n$中可由宽度为$n$的DNN（使用Leaky-ReLU/ELU/SELU激活）实现线性可分。2) 任意有限个互不相交的紧数据集可在$\mathbb{R}^{n+1}$中由宽度为$(n+1)$的DNN实现线性可分。

Conclusion: 建立了微分同胚重新定位紧集的理论，并展示了其在深度神经网络数据分类中的实用性，尤其是通过嵌入到高维空间实现线性可分的最小宽度条件。
...
TL;DR: 本文证明监督学习中的经验风险最小化（ERM）必然导致编码器在训练数据中与标签相关的方向上保留非零雅可比敏感性，这构成监督学习的几何盲点。该定理统一了四个先前独立的经验研究方向，并提出了轨迹偏差指数（TDI）作为诊断指标。


<details>
  <summary>Details</summary>
Motivation: 揭示监督学习目标本身固有的几何约束缺陷——ERM强制学习到的表示在标签相关但测试时无用的方向上保持敏感，导致模型存在根本性的脆弱性，并统一解释多个看似无关的鲁棒性问题。

Method: 1) 提出定理1证明监督学习的几何盲点；2) 提出轨迹偏差指数(TDI)直接度量定理所约束的等向路径长度失真；3) 提出PMH方法，通过高斯形式的惩罚项（命题5证明其唯一性）均匀惩罚编码器雅可比矩阵。

Result: PGD对抗训练的雅可比Frobenius范数为2.91但TDI最差(1.336)，PMH实现TDI 0.904；盲点在7种视觉任务、BERT/SST-2、ImageNet ViT-B/16骨干网络（CLIP/DINO/SAM）中可测量；随语言模型规模增大而恶化(66M→340M，盲点比0.860→0.742)；ERM微调放大54%，PMH修复11倍。

Conclusion: 监督学习的几何盲点是ERM目标本身的数学必然结果，而非当前方法的偶然失败；TDI是唯一能检测该失效模式的指标；PMH通过唯一的高斯惩罚项可有效修复该盲点。
...
TL;DR: 本文研究了变分推断（VI）中当变分族无法包含目标分布时，能否恢复目标分布特征（如均值）的条件。作者扩展了先前关于位置-尺度族在目标对称性下鲁棒VI的结果，推导了在前向KL散度和α-散度下保证均值精确恢复的充分条件，并分析了优化失败的原因，为变分族和α值的选择提供了初步指导。


<details>
  <summary>Details</summary>
Motivation: 变分推断中常用的简单参数化变分族通常无法完全匹配目标分布（即存在设定错误），因此需要探究在何种条件下仍能可靠地恢复目标分布的关键特征（如均值）。

Method: 扩展了已有关于位置-尺度族在目标对称性下鲁棒VI的理论结果，推导了前向KL散度和α-散度下均值精确恢复的充分条件，并通过反例分析优化失败的原因。

Result: 给出了在变分族设定错误时仍能精确恢复目标均值的充分条件；同时揭示了当这些条件不满足时优化可能失败的原因。

Conclusion: 为变分推断中变分族和α散度参数的选择提供了理论指导，帮助在模型设定错误的情况下仍能获得对目标分布关键特征的可靠估计。
...
TL;DR: 针对传统PINNs在模拟高频振荡Helmholtz方程时计算昂贵、偏向平滑解的问题，本文提出基于格林积分的神经求解器(GI solver)，通过积分表示隐式编码波动行为，消除二阶导数，无需人工吸收边界，并利用FFT加速计算。在20Hz地震基准模型上，GI方法相比PINNs降低计算成本超10倍，混合GI+PDE损失在强散射区域效果最佳。


<details>
  <summary>Details</summary>
Motivation: 标准PINNs在求解高频振荡Helmholtz方程时面临三大挑战：1) 点式最小化二阶PDE残差计算成本高；2) 偏向平滑解，难以捕捉振荡行为；3) 需要人工吸收边界层来约束解。本文旨在克服这些局限。

Method: 提出Green-Integral (GI)神经求解器，用非局部积分表示替代PDE残差约束，通过积分核直接编码振荡行为和辐射条件，消除二阶空间导数。理论证明该GI损失优化等价于谱调优的预处理迭代。利用FFT卷积加速损失评估，并在强散射区域引入混合GI+PDE损失提高局部精度。

Result: 在20Hz频率的结构对比和亚波长异质性地震基准模型上，GI训练一致优于PDE-based PINNs，计算成本降低超10倍，混合损失在局部散射区域获得最精确的重建。

Conclusion: GI神经求解器是一种稳定、高效且物理可解释的替代方案，克服了PINNs在高频Helmholtz问题上的固有限制，混合损失策略进一步提升了局部精度。
...
TL;DR: 本文提出了一种基于采样的有限时域轨迹与策略优化框架，通过将控制器设计视为推断问题，利用带温度退火的序贯蒙特卡洛方法（TSMC）结合哈密顿蒙特卡洛重振，有效从尖锐多峰的后验分布中采样，并在轨迹与策略优化任务上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统轨迹与策略优化方法在处理复杂、多峰的成本函数时容易陷入局部最优，而现有基于采样的方法在高维、尖锐分布下采样效率低下。作者希望将控制器设计转化为概率推断问题，通过温度退火和高效采样机制克服这些挑战。

Method: 提出TSMC框架：1) 最小化KL正则化的期望轨迹成本，得到玻尔兹曼倾斜分布；2) 采用退火方案从先验逐步过渡到目标分布，结合自适应重加权和重采样；3) 使用哈密顿蒙特卡洛（HMC）利用精确梯度进行粒子重振，保持多样性；4) 策略优化中引入初始状态分布的确定性经验近似和扩展空间构造。

Result: 在轨迹优化和策略优化多项基准测试中，TSMC相比现有最先进方法展现出更优或相当的性能，尤其在处理多峰成本函数和高维参数空间时表现突出。

Conclusion: TSMC作为一种通用的采样优化框架，有效融合了变分推断、退火采样和梯度信息的优势，为轨迹与策略优化提供了新的高效解决方案。
...
TL;DR: 本文提出了一种基于软调和解的非参数条件异常检测方法，用于识别异常标签或响应，并通过正则化避免孤立点和边界点的误检，在合成数据集、UCI数据集和真实电子健康记录数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有条件异常检测方法在处理异常标签时存在局限性，需要一种非参数化的方法来解决异常响应或类标签的检测问题。

Method: 基于软调和解（soft harmonic solution）的非参数方法，估计标签置信度以检测异常标记；并通过正则化避免检测孤立样本和分布支持边界上的样本。

Result: 在多个合成数据集和UCI机器学习数据集上，该方法在检测异常标签方面优于多种基线方法；并在真实电子健康记录数据中成功识别了异常的患者管理决策。

Conclusion: 提出的软调和解方法在条件异常检测任务上表现良好，具有实际应用价值，尤其是在医疗决策异常检测领域。
...
TL;DR: 本文提出动态先验强化学习（DP-RL），通过在策略梯度学习中引入基于状态动力学的辅助损失（证据积累和滞后效应），在不修改奖励、环境或策略架构的情况下，塑造决策轨迹的时间结构，从而产生具有时序连贯性的行为。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习只优化累计奖励，但对决策随时间如何演化缺乏约束，容易导致策略出现突然的信心转变、振荡或退化性不活动等时序不连贯行为。

Method: 在策略梯度学习中增加一个辅助损失函数，该损失从外部状态动力学（实现证据积累和滞后效应）中导出，以塑造策略动作概率的时间演化过程。

Result: 在三个最小环境中，动态先验能够根据任务需求系统地改变决策轨迹，促进具有时间结构的时序行为，且这种效果不能由简单的平滑操作所解释。

Conclusion: 训练目标本身可以控制强化学习智能体决策的时间几何结构（temporal geometry），即在保持奖励和环境不变的情况下，通过辅助损失来调控决策行为的时序特性。
...
TL;DR: 本文提出了一种结合残差图同构网络与注意力机制的协同预测模型ResGIN-Att，通过融合分子结构特征、细胞系基因组图谱和药物相互作用，有效提升了药物协同效应预测的准确性、泛化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 单一药物治疗复杂疾病效果有限且易产生耐药性，而组合药物疗法可通过协同效应显著改善疗效，但实验验证所有药物组合成本过高，亟需高效的计算预测方法。现有深度学习和GNN方法存在结构偏差、泛化能力不足和可解释性差等问题。

Method: 提出ResGIN-Att模型：首先利用残差图同态网络提取药物分子的多尺度拓扑特征（残差连接缓解深层过平滑问题）；然后通过自适应LSTM模块融合从局部到全局的结构信息；最后设计交叉注意力模块显式建模药物-药物相互作用并识别关键化学子结构。

Result: 在五个公开基准数据集上的广泛实验表明，ResGIN-Att取得了具有竞争力的性能，优于关键基线方法，并展现出良好的泛化能力和鲁棒性。

Conclusion: 所提出的ResGIN-Att模型通过结合分子结构特征、细胞系基因组图谱和药物相互作用，有效提升了药物协同效应预测性能，为解决现有方法的局限性提供了新的思路。
...
TL;DR: TaNOS 是一个持续预训练框架，通过表头匿名化、操作草图、自监督预训练三个组件解耦领域语义与数值操作结构，显著提升专家领域表格数值推理的跨领域迁移能力，在 FinQA 上仅用10%数据即达80.13%执行准确率，超出全量数据训练的 SFT 基线（73.97%）及 GPT-5 等闭源模型，并将跨领域差距压缩至2pp以内。


<details>
  <summary>Details</summary>
Motivation: 现有数值推理模型在专家领域表格上训练集内准确率高，但跨领域迁移时鲁棒性差。SFT 模型倾向于学习表头-操作捷径（header-operation shortcuts），而非真正的结构推理。

Method: 提出 TaNOS 框架，包含三个组件：①表头匿名化（减少词汇记忆）；②操作草图（提供最小结构线索）；③自监督预训练（以程序优先方式构建正确性保证的程序-问题对）。

Result: 在 FinQA 上仅用10%训练数据即达80.13%执行准确率，超越全量数据 SFT 基线（73.97%）及 GPT-5、Gemini-2.5-Pro。跨领域实验中，跨领域差距<2pp，而 SFT 差距>10pp。

Conclusion: 结构引导的操作草图、表头无关表示和正确性保证的自监督可以显著提升数值推理在多样化专家领域表格上的鲁棒性。
...
TL;DR: 本文提出基于LSTM的深度学习框架校准低成本空气质量传感器（PM2.5、PM10、NO₂），通过序列学习捕捉时间依赖关系，在牛津OxAria网络的真实数据上取得优于随机森林的效果，三项污染物的扩展不确定度均符合法规要求（NO₂: 22.11%, PM10: 12.42%, PM2.5: 9.1%）。


<details>
  <summary>Details</summary>
Motivation: 低成本传感器虽能实现密集监测网络，但存在校准难题（传感器漂移、环境交叉敏感、设备间性能差异），需要更有效的校准方法以推动其实际部署。

Method: 采用LSTM网络进行序列学习，构建包含时间滞后参数、谐波编码和交互项的特征集，以捕捉时间依赖性和延迟环境效应；以随机森林作为基线对比。

Result: LSTM在训练集、验证集和测试集上对三种污染物的R²均高于随机森林；扩展不确定度验证结果：NO₂ 22.11%、PM10 12.42%、PM2.5 9.1%，满足法规要求。

Conclusion: 基于LSTM的时序校准框架能有效解决低成本传感器的校准难题，且校准结果符合监管标准，为低成本传感器在城市空气质量监测中的实际应用提供了可行方案。
...
TL;DR: 本文提出了一种混合AI框架（HAF-DS），将LSTM需求预测与MILP优化相结合，在纺织品和PPE供应链中同时降低了预测误差和运营成本，显著提升了库存效率和服务水平。


<details>
  <summary>Details</summary>
Motivation: 传统预测与优化方法相互孤立，无法有效应对纺织品和PPE等行业需求波动大、供应不确定的挑战，缺乏将预测与决策协同优化的综合方案。

Method: 提出HAF-DS框架：LSTM模块捕捉需求的时间依赖和上下文特征，MILP优化层制定补货和分配决策，通过嵌入特征表示和循环神经网络架构联合最小化预测误差与运营成本。

Result: 在纺织品和PPE组合数据集上，MAE降低14.7%（15.04→12.83），RMSE降低12.4%（19.53→17.11），MAPE从9.5%降至8.1%；库存成本降低5.4%，缺货减少27.5%，服务水平从95.5%提升至97.8%。

Conclusion: 将预测性预测与规范性优化相结合能够同时提升准确性和效率，HAF-DS为现代纺织品和PPE供应链提供了一种可扩展、自适应的解决方案。
...
TL;DR: 轻量级基于自动机的n-gram模型在流式事件日志的下一活动预测中，能以极低资源消耗达到与LSTM、Transformer等神经网络模型相当的准确率，且表现更稳定；提出的动态选择式集成方法（promotion算法）相比经典voting方法进一步降低了推理开销。


<details>
  <summary>Details</summary>
Motivation: 在流式事件日志的下一活动预测任务中，现有神经网络模型（LSTM、Transformer）资源消耗大，而传统轻量模型（n-gram）的性能尚未被系统评估。作者希望探究轻量级模型能否在保持高准确率的同时大幅降低计算成本。

Method: 对比n-gram与LSTM、Transformer在合成数据与五个真实流程挖掘数据集上的表现；提出promotion算法，通过动态选择两个活跃模型中的一个进行推理，替代传统voting集成方法。

Result: n-gram在适当上下文窗口下准确率与神经网络相当，且稳定性优于窗口化神经网络；promotion算法在真实数据集上达到或超过非窗口化神经网络的准确率，且计算成本更低。

Conclusion: 在流式事件日志的下一活动预测中，轻量级n-gram模型是神经网络模型的高效替代方案；所提promotion集成策略在降低推理开销的同时保持了高准确率。
...
TL;DR: 本文提出贝塞尔轨迹匹配（BTM），用二次贝塞尔曲线替代随机梯度下降轨迹，以解决轨迹匹配中固定合成数据集的表征瓶颈，在临床数据集上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 轨迹匹配（TM）是数据集压缩中广泛使用的方法，但其监督信号的结构尚不明确。作者发现固定合成数据集只能复现有限范围的参数变化，当监督信号频谱过宽时会产生条件表征瓶颈，因此需要设计更优的监督信号结构。

Method: 提出贝塞尔轨迹匹配（BTM），用初始和最终模型状态之间的二次贝塞尔曲线替代SGD轨迹，优化以降低路径上的平均损失，同时将宽频谱的SGD监督信号替换为更结构化、更低秩的信号。

Result: 在五个临床数据集上，BTM一致匹配或优于标准轨迹匹配，在低患病率和低合成预算设置中增益最大，同时大幅减少轨迹存储。

Conclusion: 有效的轨迹匹配更依赖于结构化监督信号，而非复现随机优化路径。
...
TL;DR: 本文分析了多任务强化学习网络在水下导航中的内部结构，发现任务间仅约1.5%的权重用于区分不同任务，其中85%连接上下文变量节点到隐藏层，揭示了共享与专用网络组件的重要性。


<details>
  <summary>Details</summary>
Motivation: 传统控制器在水下机器人多任务适应和可解释性方面存在局限，而多任务强化学习虽有潜力但内部决策机制不透明，缺乏信任和安全性，阻碍实际部署。

Method: 在HoloOcean模拟器中，通过识别和比较预训练多任务强化学习网络中针对不同物种导航任务的子网络，分析其内部结构。

Result: 网络仅约1.5%的权重用于任务区分，其中约85%连接输入层的上下文变量节点到下一隐藏层，表明上下文变量在任务区分中起关键作用。

Conclusion: 该方法揭示了共享与专用网络组件，有助于水下监测中的模型编辑、迁移学习和持续学习，提升可解释性和部署安全性。
...
TL;DR: 本文提出结合乘积量化（PQ）、倒排索引和Dask框架的方法，将大规模高维数据分而治之，在保持精度的同时降低计算开销至中等规模数据的水平。


<details>
  <summary>Details</summary>
Motivation: 大规模最近邻搜索在计算上存在局限性，近似最近邻搜索虽可降低计算成本，但处理大规模高维数据时仍面临内存和时间的巨大开销，亟需更高效的方案。

Method: 利用乘积量化（PQ）进行内存高效的聚类，结合倒排索引加速搜索，并借助Dask分布式计算框架实现数据的划分与并行处理，最终合并结果。

Result: 在不牺牲搜索精度的前提下，显著降低了计算资源需求（包括内存和执行时间），达到接近中等规模数据处理的计算水平。

Conclusion: 所提出的PQ+倒排索引+Dask组合策略能有效解决大规模高维数据ANN搜索的计算瓶颈，兼具精度保持与资源高效的优势。
...
TL;DR: 提出了一种名为R-DCNN的轻量级深度学习方法，通过重采样对齐不同频率信号的时间尺度，使用单个观测样本即可训练网络，在极低计算资源下实现与传统深度学习和经典方法相当的信号去噪和波形估计性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽在周期信号去噪中表现出色，但计算资源需求高，且需为每个观测信号单独训练模型，难以部署在资源受限的环境中。

Method: 结合深度卷积神经网络（DCNN）与重采样技术。先用重采样将不同基频信号的时间尺度对齐，使同一组网络权重可复用于多种信号；仅需单次观测即可完成训练。

Result: R-DCNN在极低计算复杂度下，去噪与波形估计性能可与经典的AR方法以及为每个观测单独训练的常规DCNN相媲美。

Conclusion: 该方法的效率与性能兼得，特别适合部署在功耗和资源严格受限的环境中，在不牺牲精度的前提下实现高效信号处理。
...
TL;DR: 本文发现机器学习预测SCF初始猜测在外推到大分子时效果变差的原因是监督信号问题（而非外推问题），并提出SAIL方法，通过端到端微分SCF求解器来训练模型，同时引入ERIC指标修正传统RIC的缺陷。在QM40和QMugs数据集上，SAIL显著降低了迭代次数并带来实际加速。


<details>
  <summary>Details</summary>
Motivation: 现有ML方法预测SCF初始猜测虽然在小分子上有效，但外推到更大分子时反而比传统方法更慢。作者想探究失败的根本原因并解决这一挑战。

Method: 1) 通过实验证明失败源于监督信号（ground-state目标）与SCF求解目标不匹配，而非外推能力不足。2) 提出SAIL，将SCF求解器作为可微层融入训练，实现端到端优化。3) 提出ERIC指标，修正RIC未考虑Fock-build计算开销的问题。

Result: QM40上（分子尺寸达训练集4倍）：SAIL在PBE/SCAN/B3LYP上分别降低ERIC 37%/33%/27%，B3LYP上远超此前最优方法（10%）。QMugs上（分子尺寸达训练集10倍）：在混合泛函级别实现1.25倍实际加速。

Conclusion: SAIL通过端到端学习对齐SCF求解目标，有效解决了ML初始猜测的外推退化问题，将ML加速SCF扩展到大尺寸类药分子。
...
TL;DR: 本文研究了如何将注意力机制相关的可解释性方法AttnLRP应用于Transformer-based基因组语言模型（gLM）DNABERT-2，验证其能否像CNN一样产生符合生物学模式的可靠解释，从而推动gLM在生物学洞察中的应用。


<details>
  <summary>Details</summary>
Motivation: 虽然CNN在基因组序列上的解释已被证明能捕获相关生物学模式，但更强大的Transformer-based基因组语言模型（gLMs）的解释能力尚不明确。本文旨在探究gLMs是否也能产生可靠的生物学解释。

Method: 将AttnLRP（层相关性传播的注意力扩展）适配到DNABERT-2，提出从token级别到核苷酸级别的解释迁移策略，并在多个基因组数据集上使用多种评估指标进行验证，同时与基线CNN的解释进行对比。

Result: AttnLRP在DNABERT-2上能够产生可靠的、对应于已知生物学模式的解释，表明gLMs可以像CNNs一样帮助推导生物学洞察。

Conclusion: 本研究为基因组语言模型的可解释性做出了贡献，并解决了不同架构之间相关性归因的可比性问题。
...
TL;DR: 本文提出TEmBed基准，系统评估表格嵌入模型在单元格、行、列和表四个层次的表现，发现最优模型取决于具体任务和表示层次。


<details>
  <summary>Details</summary>
Motivation: 现有表格基础模型在不同任务特定设置下评估，难以直接比较优劣，缺乏统一的评估基准。

Method: 构建TEmBed基准测试，从四个表示层级（单元格、行、列、表）系统评估多种表格表示学习模型。

Result: 不同模型在不同任务和表示层级上的表现各有优劣，没有一种模型在所有场景下都最优。

Conclusion: 为实际应用中表格嵌入的选择提供了实用指导，并为开发通用表格表示模型奠定基础。
...
TL;DR: 本文提出了一个在序贯决策中不确定性（模型、反馈、预测）的分类体系，指出不公平可能源于不确定性而非噪声，并通过反事实逻辑和强化学习形式化分析，展示了不确定性感知的探索如何影响公平性指标。


<details>
  <summary>Details</summary>
Motivation: 公平机器学习多聚焦于监督学习，但实际应用中许多决策是序贯的、在线的，且充满不确定性（如反事实未知、样本有限）。历史上被边缘化的群体系统性地数据不足，导致不确定性在不同群体间不均匀分布，从而加剧不公平。现有方法缺乏对此类不确定性的系统性刻画。

Method: 提出不确定性分类：模型不确定性（假设空间）、反馈不确定性（反馈延迟/缺失）、预测不确定性（预测方差）。利用反事实逻辑和强化学习形式化建模。通过模拟实验（不同偏见程度的数据）展示不确定性如何产生不平等，以及不确定性感知的探索如何改变公平性指标。

Result: 实验表明，忽视不确定性的策略会对决策者（未实现的收益/损失）和决策对象（加剧排斥、减少访问机会）造成伤害。通过算法示例证明可以在维持机构目标（如期望效用）的同时减少弱势群体的结果方差。

Conclusion: 不确定性驱动不公平性时，必须将其纳入考量，而非当作随机噪声忽略。该框架为从业者提供了诊断、审计和治理公平风险的工具。
...
### [222] [Transferable Physics-Informed Representations via Closed-Form Head Adaptation](https://arxiv.org/abs/2604.21761)
*Jian Cheng Wong,Isaac Yin Chung Lai,Pao-Hsiung Chiu,Chin Chun Ooi,Abhishek Gupta,Yew-Soon Ong*

Main category: cs.LG

TL;DR: 本文提出了一种基于伪逆的物理信息神经网络框架（Pi-PINN），通过可迁移的表征学习和闭式头适配，实现了对未知PDE实例的快速、高精度求解，速度比典型PINN快100-1000倍，误差降低10-100倍。


<details>
  <summary>Details</summary>
Motivation: 现有PINN方法在面对缺乏训练样本的新PDE时泛化能力差，无法有效预测或解决未见过的PDE问题实例。

Method: 提出Pi-PINN框架：在共享嵌入空间中学习可迁移的物理信息表征，并通过最小二乘最优伪逆方法在PDE约束下实现闭式头适配，从而实现快速求解已知和未知PDE实例。

Result: 在泊松方程、亥姆霍兹方程和伯格斯方程等多个PDE问题上，Pi-PINN比典型PINN快100-1000倍，比典型数据驱动模型误差低10-100倍，即便只有两个训练样本也能取得优异性能。

Conclusion: 可迁移表征与闭式头适配方法能够显著提升PINN的效率和泛化能力，有望在PDE族及各类科学与工程应用中发挥重要作用。
...
TL;DR: 本文指出，在k-means聚类问题中，Hartigan算法通常优于经典的Lloyd算法（改进约5%-10%），而对Hartigan方法进行微小改进后，可进一步获得2%-5%的性能提升，且当维度或k值增加时，改进幅度更大。


<details>
  <summary>Details</summary>
Motivation: 尽管Hartigan算法已在大多数情况下优于经典的Lloyd算法（表现提升约5%-10%），但作者发现通过非常微小的变化可以进一步提升Hartigan方法的聚类效果。

Method: 对Hartigan算法进行一个很小的变体修改。

Result: 改进后的算法相比原始Hartigan方法额外提升了2%-5%的性能，且当数据维度或聚类数k增加时，改进幅度更大。

Conclusion: Hartigan算法的一个微小变体可以在原有基础上进一步显著改善k-means聚类效果，尤其在高维或大k场景下优势更明显。
...
TL;DR: 本文建立了一个基于商空间的扩散建模理论框架，并应用于具有SE(3)对称性的分子结构生成，通过避免学习群作用分量来简化学习难度，实验证明该方法优于传统的对称性处理方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在处理具有对称性的系统（如分子结构生成中的SE(3)对称性）时，需要学习冗余的群作用分量，增加了学习难度；同时启发式对齐策略缺乏合理的采样器保证。因此需要建立一种能在商空间上直接进行扩散建模的形式化框架。

Method: 1. 建立了通用商空间上的扩散建模形式化框架；2. 将框架应用于具有SE(3)对称性的分子结构生成任务；3. 通过避免学习与群作用对应的分量来简化学习难度；4. 设计的采样器能够保证恢复目标分布。

Result: 在小分子和蛋白质结构生成任务上的实验验证表明，基于商空间的扩散模型优于传统的对称性处理方法（如群等变扩散模型和启发式对齐策略）。

Conclusion: 商空间扩散模型为处理对称性系统提供了一种原则性的新框架，既简化了学习难度，又有理论保证的采样器，在分子生成任务上展现出优越性能。
...
### [225] [Probably Approximately Consensus: On the Learning Theory of Finding Common Ground](https://arxiv.org/abs/2604.21811)
*Carter Blair,Ben Armstrong,Shiri Alouf-Heffetz,Nimrod Talmon,Davide Grossi*

Main category: cs.LG

TL;DR: 本文提出一种通过嵌入和降维将共识建模为一维意见空间中的区间的方法，利用经验风险最小化算法高效识别最优共识区域，并通过选择性查询减少所需用户反馈数量。


<details>
  <summary>Details</summary>
Motivation: 现有在线 deliberation 平台主要依赖用户对特定陈述的直接偏好来识别共识，但未能充分考虑不同议题的相对显著性（salience），且共识应当超越用户提供的具体陈述。

Method: 将共识建模为经嵌入和降维后的一维意见空间中的假设区间，提出基于经验风险最小化（ERM）的算法，并提供PAC学习理论保证。通过选择性查询已有陈述样本来减少查询次数。

Result: 初步实验验证了算法性能，并发现通过选择性查询用户可以有效将查询次数降低到实际可行的数量。

Conclusion: 该方法能够更高效地识别广泛的共识区间，同时通过降维和选择性查询策略提高了实际部署的可行性。
...
### [226] [GFlowState: Visualizing the Training of Generative Flow Networks Beyond the Reward](https://arxiv.org/abs/2604.21830)
*Florian Holeczek,Andreas Hinterreiter,Alex Hernandez-Garcia,Marc Streit,Christina Humer*

Main category: cs.LG

TL;DR: 本文提出GFlowState，一个可视化分析系统，用于揭示生成流网络（GFlowNets）的训练过程，帮助用户分析采样轨迹、空间探索和策略演化。


<details>
  <summary>Details</summary>
Motivation: GFlowNets在分子和材料发现等领域表现出色，但其训练动态难以解释，现有工具仅能追踪指标，无法揭示模型如何探索样本空间、构建轨迹或调整采样概率。

Method: 设计了多个可视化视图，包括候选排名图、状态投影图、轨迹网络节点链接图和转换热力图，支持采样行为分析和策略演化追踪。

Result: 通过案例研究展示了系统在调试和评估GFlowNets质量方面的有效性，覆盖多个应用领域。

Conclusion: GFlowState使GFlowNets的结构动态变得可观测，增强了可解释性，可加速GFlowNets的实际开发。
...
### [227] [A Scale-Adaptive Framework for Joint Spatiotemporal Super-Resolution with Diffusion Models](https://arxiv.org/abs/2604.21903)
*Max Defez,Filippo Quarenghi,Mathieu Vrac,Stephan Mandt,Tom Beucler*

Main category: cs.LG

TL;DR: 本文提出了一种尺度自适应的联合时空超分辨率框架，通过分解为确定性条件均值预测（含注意力机制）和残差条件扩散模型，并引入质量守恒变换，仅需调整三个超参数（噪声调度幅度、时间上下文长度、质量守恒函数）即可在同一架构下处理不同时空超分辨率因子（空间1-25倍、时间1-6倍），在法国降水再分析数据上验证了其跨尺度的可重用性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习的视频超分辨率方法在气候应用中通常仅单独处理空间或时间维度，且联合时空模型往往针对单一超分辨率因子设计，缺乏跨空间分辨率和时间帧率的迁移能力。

Method: 将联合时空超分辨率分解为两个部分：1) 带注意力的确定性条件均值预测；2) 残差条件扩散模型（可选质量守恒变换以保持总量）。通过调整三个因子依赖的超参数实现尺度自适应：扩散噪声调度幅度β（大因子时增大以增加多样性）、时间上下文长度L（保持注意力视野可比）、质量守恒函数f（大因子时衰减以避免极端值放大）。

Result: 在法国Comephore降水再分析数据上，同一架构覆盖了空间1-25倍和/or时间1-6倍的超分辨率因子。

Conclusion: 提供了一种可重用的架构和调参方案，实现跨尺度的联合时空超分辨率。
...
### [228] [Low-Rank Adaptation Redux for Large Models](https://arxiv.org/abs/2604.21905)
*Bingcong Li,Yilang Zhang,Georgios B. Giannakis*

Main category: cs.LG

TL;DR: 本文从信号处理视角重新审视LoRA（低秩适配），将其与经典低秩建模工具和逆问题联系起来，从架构设计、高效优化和相关应用三个维度系统梳理了LoRA变体的技术机制，并展望了信号处理与深度学习双向融合的研究方向。


<details>
  <summary>Details</summary>
Motivation: 尽管LoRA已成为参数高效微调的事实标准且涌现了大量变体，但哪些架构选择、优化技术和部署约束应指导实际方法选择仍不清晰，缺乏从原理层面对其有效性进行解释的统一视角。

Method: 本文采用信号处理视角重新审视LoRA，将其与经典低秩建模工具（如SVD分解）和逆问题相联系，从架构设计（SVD分解、秩增强、跨层张量化）、高效优化（初始化、交替求解器、规范不变优化、参数化感知方法）和应用部署三个互补轴心对LoRA变体的技术机制进行分类和阐释。

Result: 建立了LoRA与信号处理理论之间的桥梁，系统解释了各类LoRA变体有效性的底层机制，并覆盖了从预训练、后训练到服务部署的完整大模型生命周期应用。

Conclusion: 信号处理工具为设计原则性的PEFT方法提供了原则性词汇，同时大模型带来的独特挑战（如规模庞大和过高开销）也为信号处理社区提供了新的研究问题，形成双向促进的前沿。
...
### [229] [The Sample Complexity of Multicalibration](https://arxiv.org/abs/2604.21923)
*Natalie Collina,Jiuyao Lu,Georgy Noarov,Aaron Roth*

Main category: cs.LG

TL;DR: 本文研究了批处理设置下多校准（multicalibration）的极小化样本复杂度。对于固定κ>0且|G|≤ε^{-κ}时，证明样本复杂度为Θ̃(ε^{-3})（上下界匹配），与边际校准的Θ̃(ε^{-2})不同。对于κ=0，复杂度降至Θ̃(ε^{-2})，呈现阈值现象。进一步推广到加权L_p多校准度量（1≤p≤2），最优指数为3/p，并扩展至可引发性质的校准。


<details>
  <summary>Details</summary>
Motivation: 多校准是机器学习公平性和可解释性中的重要概念，但其样本复杂度在批处理场景下尚未明确。本文旨在严格刻画多校准所需的样本量，填补理论空白，并理解其与边际校准、在线学习等的关系。

Method: 采用在线到批处理（online-to-batch）归约构造随机预测器实现上界；通过信息论下界技术（如Fano不等式、拟阵论）证明样本复杂度下界，特别分析了随机化预测器的不可回避性。针对κ=0和κ>0两种情形分别处理，并推广到L_p度量和可引发性质。

Result: 在|G|≤ε^{-κ}（κ>0）下，多校准的样本复杂度为Θ̃(ε^{-3})，与边际校准的Θ̃(ε^{-2})分离。κ=0时复杂度降为Θ̃(ε^{-2})。加权L_p多校准的最优样本复杂度指数为3/p（1≤p≤2）。下界模板适用于广义可引发性质的校准。

Conclusion: 批处理多校准的样本复杂度显著高于边际校准，且与在线多校准的复杂度一致，揭示了两者间的深刻差异。κ的阈值现象表明组族大小对复杂度有决定性影响。结果为理解校准类问题的统计难度提供了系统框架。
...
### [230] [Fine-Tuning Regimes Define Distinct Continual Learning Problems](https://arxiv.org/abs/2604.21927)
*Paul-Tiberiu Iordache,Elena Burceanu*

Main category: cs.LG

TL;DR: 本文提出微调机制（即可训练参数子空间）本身是持续学习评估的关键变量。通过形式化投影优化框架，发现改变可训练深度会改变更新信号，从而影响任务拟合与知识保留。实验表明，方法排名在不同微调机制下不一致，且越深的机制导致越大的更新幅度、更高的遗忘率以及两者间更强的关联。建议将可训练深度作为显式实验因素纳入评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习评估通常固定微调机制，但本文认为微调机制（可训练参数子空间）本身就是关键变量，不同机制下方法的相对表现可能不一致。

Method: 将适应机制形式化为固定可训练子空间上的投影优化；在任务增量学习下，设置五种可训练深度机制，评估四种标准方法（在线EWC、LwF、SI、GEM）在五个基准数据集及11种任务顺序上的表现。

Result: 方法相对排名在不同微调机制下不一致；更深的适应机制导致更大的更新幅度、更高的遗忘率及两者间更强的关联。

Conclusion: 持续学习中的比较结论可能强烈依赖于所选的微调机制，建议采用机制感知的评估协议，将可训练深度作为显式实验因素。
...
### [231] [Temporal Taskification in Streaming Continual Learning: A Source of Evaluation Instability](https://arxiv.org/abs/2604.21930)
*Nicolae Filat,Ahmed Hussain,Konstantinos Kalogiannis,Elena Burceanu*

Main category: cs.LG

TL;DR: 本文发现流式持续学习中的时间任务划分（taskification）并非中性预处理，而是评估的结构性组成部分：同一数据流的不同划分方式会改变CL机制和基准结论。作者提出基于可塑性与稳定性轮廓的框架、轮廓距离和边界-轮廓敏感性（BPS）来诊断这一效应，并通过网络流量预测实验证明任务划分本身就能显著影响评估结果。


<details>
  <summary>Details</summary>
Motivation: 揭示流式持续学习中时间任务划分（taskification）这一看似中性的预处理步骤实际上对评估结论有结构性影响，从而呼吁将其视为一等评估变量。

Method: 1) 提出基于可塑性与稳定性轮廓（plasticity and stability profiles）的任务划分层次框架；2) 定义轮廓距离（profile distance）用于衡量不同任务划分之间的差异；3) 提出边界-轮廓敏感性（BPS）诊断方法，在训练任何CL模型之前评估边界扰动对任务划分所诱导的CL机制的影响；4) 在CESNET-Timeseries24网络流量预测数据集上，固定数据流、模型和训练预算，仅改变时间任务划分（9天、30天、44天），对比持续微调、经验重放、EWC和LwF四种CL方法。

Result: 1) 不同的时间任务划分导致预测误差、遗忘和反向迁移发生显著变化；2) 较短的任务划分会引发更嘈杂的分布级模式、更大的结构距离和更高的BPS，即对边界扰动更敏感；3) 任务划分本身就能实质性地影响CL评估结论。

Conclusion: 流式持续学习的基准结论不仅取决于学习器和数据流，还取决于数据流如何被任务化，应将时间任务化（temporal taskification）作为第一类评估变量予以重视。
...
### [232] [AtomicRAG: Atom-Entity Graphs for Retrieval-Augmented Generation](https://arxiv.org/abs/2604.20844)
*Yanning Hou,Duanyang Yuan,Sihang Zhou,Xiaoshu Chen,Ke Liang,Siwei Wang,Xinwang Liu,Jian Huang*

Main category: cs.IR

TL;DR: 提出Atom-Entity Graph架构，以知识原子（细粒度事实单元）替代文本块作为知识表示基本单元，结合个性化PageRank与相关性过滤，提升检索准确性与推理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: GraphRAG方法将文本块作为基本知识单元，限制了灵活性和适应性；基于三元组的实体链接对关系抽取错误敏感，导致推理路径缺失或错误，损害检索精度。

Method: 构建Atom-Entity Graph：以"知识原子"（独立的事实单元）替代粗粒度文本块；实体间边仅表示关系是否存在；结合个性化PageRank与相关性过滤进行检索。

Result: 在五个公开基准上，AtomicRAG算法在检索准确性和推理鲁棒性上优于强RAG基线。

Conclusion: 更细粒度的知识表示和更可靠的实体连接方式能有效提升GraphRAG系统的检索与推理性能。
...
### [233] [CaST-POI: Candidate-Conditioned Spatiotemporal Modeling for Next POI Recommendation](https://arxiv.org/abs/2604.20845)
*Zhenyu Yu,Chunlei Meng,Yangchen Zeng,Mohd Yamani Idna Idris,Shuigeng Zhou*

Main category: cs.IR

TL;DR: 提出CaST-POI模型，通过候选点感知的历史轨迹注意力机制和候选点相关时空偏置，显著提升下一POI推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有候选POI使用统一的用户表示进行评分，忽略了历史访问与不同候选点之间的相关性差异。

Method: CaST-POI包含候选条件序列读取器（以候选点为查询动态关注用户历史）和候选相关时空偏置机制，用于捕捉细粒度的移动模式。

Result: 在三个基准数据集上持续超越SOTA方法，在多个评估指标上有显著提升，尤其在大候选池场景下优势明显。

Conclusion: 候选感知的建模方式能更有效地利用用户历史轨迹信息，提升下一POI推荐的准确性和鲁棒性。
...
### [234] [ADS-POI: Agentic Spatiotemporal State Decomposition for Next Point-of-Interest Recommendation](https://arxiv.org/abs/2604.20846)
*Zhenyu Yu,Chunlei Meng,Yangchen Zeng,Mohd Yamani Idna Idris,Shuigeng Zhou*

Main category: cs.IR

TL;DR: ADS-POI提出了一种时空状态分解框架，将用户行为分解为多个并行演化的潜在子状态，每个子状态具有独立的时空转移动力学，并通过上下文条件机制聚合形成决策状态，用于下一个POI推荐。在三个真实数据集上，该方法优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法将用户历史压缩为单一潜在表示，容易混淆常规移动模式、短期意图和时间规律等异构信号，导致状态演化的灵活性受限，模型难以适应多样化的决策上下文。

Method: 提出ADS-POI框架，核心设计包括：(1) 使用多个并行演化的潜在子状态表示用户；(2) 每个子状态拥有独立的时空转移动力学；(3) 通过上下文条件机制选择性聚合子状态，形成最终的决策状态用于预测。

Result: 在Foursquare和Gowalla三个真实基准数据集上，采用全排序评估协议，ADS-POI持续优于强基线方法，验证了分解策略的有效性和鲁棒性。

Conclusion: 将用户行为分解为多个时空感知的子状态，能够更有效地建模复杂移动行为，提升下一个POI推荐的性能。
...
### [235] [Revisiting Content-Based Music Recommendation: Efficient Feature Aggregation from Large-Scale Music Models](https://arxiv.org/abs/2604.20847)
*Yizhi Zhou,Jia-Qi Yang,De-Chuan Zhan,Da-Wei Zhou*

Main category: cs.IR

TL;DR: 本文提出TASTE，一个多模态音乐推荐数据集与基准框架，整合音频和文本信息，并引入MuQ-token方法高效融合多层音频特征，在冷启动等场景中验证了内容驱动方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有音乐推荐系统主要依赖协同过滤，未能充分利用音频内在特征，尤其在冷启动场景下表现不佳；同时缺乏包含多模态信息的数据集和全面的评估框架。

Method: 构建了包含音频和文本模态的TASTE数据集；利用大规模自监督音乐编码器提取音频表征；提出MuQ-token方法实现多层音频特征的高效融合。

Result: 在候选召回和点击率预估任务中，多模态音频表征显著提升推荐性能；MuQ-token方法在多种设置下一致优于其他特征融合技术。

Conclusion: 验证了内容驱动方法在音乐推荐中的有效性，为未来研究提供了高效且可复用的多模态基础。
...
### [236] [MATRAG: Multi-Agent Transparent Retrieval-Augmented Generation for Explainable Recommendations](https://arxiv.org/abs/2604.20848)
*Sushant Mehta*

Main category: cs.IR

TL;DR: MATRAG是一个结合多智能体协作与知识图谱增强检索的透明化推荐系统框架，通过四个专业化智能体实现可解释推荐，在三个基准数据集上达到SOTA性能，87.4%的解释被专家评为有帮助且可信。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的推荐系统在透明度、知识基础以及生成连贯解释以建立用户信任方面面临关键挑战。

Method: 提出MATRAG框架，包含四个专门智能体：用户建模智能体（构建动态偏好画像）、物品分析智能体（从知识图谱提取语义特征）、推理智能体（综合协同与内容信号）、解释智能体（基于检索知识生成自然语言解释），并引入透明度评分机制量化解释的忠实度与相关性。

Result: 在Amazon Reviews、MovieLens-1M和Yelp三个基准数据集上，推荐准确率提升12.7%（Hit Rate）和15.3%（NDCG）；人类评估中87.4%的解释被领域专家评为有帮助且可信。

Conclusion: 为透明化、智能体驱动的推荐系统建立了新的基准，并为在生产环境中部署基于LLM的推荐系统提供了可操作的见解。
...
### [237] [SPIRE: Structure-Preserving Interpretable Retrieval of Evidence](https://arxiv.org/abs/2604.20849)
*Mike Rainey,Umut Acar,Muhammed Sezer*

Main category: cs.IR

TL;DR: 提出了一种面向树状结构文档的结构感知检索流水线，通过子文档表示、全局/局部上下文化机制和结构化聚合，在HTML问答任务中实现了更高质量和多样性的引用。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在处理HTML等半结构化数据时，将文档线性化为固定大小块，丢失了表格、列表等结构信息，且难以在保留上下文的同时返回精确的可引用证据。

Method: 定义文档基元（路径、路径集、子文档提取、两种上下文化机制）；构建基于嵌入的候选生成器（索引句子级子文档）；设计查询时文档感知聚合步骤分摊共享结构上下文；引入上下文过滤阶段对检索候选进行重排序。

Result: 在HTML问答基准上，该方法在固定预算下比强基线（段落级方法）生成更高质量、更多样化的引用，同时保持可扩展性。

Conclusion: 保留文档结构并加以上下文化，能在有限预算下显著提升检索结果的质量和多样性，为半结构化文档的RAG提供了有效方案。
...
### [238] [Association Is Not Similarity: Learning Corpus-Specific Associations for Multi-Hop Retrieval](https://arxiv.org/abs/2604.20850)
*Jason Dury*

Main category: cs.IR

TL;DR: 本文提出Association-Augmented Retrieval (AAR)，一种轻量级转导重排序方法，通过小MLP学习嵌入空间中段落间的关联关系，在HotpotQA上Recall@5提升8.6点，困难问题上提升28.5点，训练仅需不到两分钟。


<details>
  <summary>Details</summary>
Motivation: 多跳问题需要具有关联推理链的段落，但传统稠密检索仅基于嵌入相似度排序，未能捕捉段落间的隐式关联关系。

Method: AAR使用4.2M参数的小MLP，通过对比学习在共现标注上学习段落间的关联关系，在推理时对初始稠密检索候选集进行双向关联评分重排序。

Result: HotpotQA上Recall@5从0.831提升至0.916（+8.6点），困难问题提升28.5点；MuSiQue上转导设置提升10.1点；下游QA中EM提升6.4点。推理仅增加3.7ms，训练在单GPU上不到两分钟。

Conclusion: AAR通过关联学习有效提升多跳检索性能，但该方法捕捉的是语料库特定的共现模式而非可迁移特征，归纳设置下无显著提升。
...
### [239] [Robust Test-time Video-Text Retrieval: Benchmarking and Adapting for Query Shifts](https://arxiv.org/abs/2604.20851)
*Bingqing Zhang,Zhuo Cao,Heming Du,Yang Li,Xue Li,Jiajun Liu,Sen Wang*

Main category: cs.IR

TL;DR: 本文发现视频-文本检索模型在查询分布偏移下性能骤降，并提出HAT-VTR框架，通过抑制hubness现象和保持时序一致性来提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视频-文本检索模型在分布内数据表现好，但面对真实世界查询偏移时性能严重下降，而现有的图像鲁棒性方法无法处理视频中的时空动态复杂性。

Method: 首先构建包含12种视频扰动类型、5种严重程度的基准来评估鲁棒性。接着提出HAT-VTR，包含两个核心组件：Hubness Suppression Memory（细化相似度分数）和multi-granular losses（强制时序特征一致性）。

Result: 实验表明HAT-VTR在多种查询偏移场景下显著优于现有方法，有效提升了模型鲁棒性。

Conclusion: 通过直接对抗hubness现象，HAT-VTR能有效提升视频-文本检索在查询偏移下的可靠性，适用于实际应用。
...
### [240] [DenoiseRank: Learning to Rank by Diffusion Models](https://arxiv.org/abs/2604.20852)
*Ying Wang,Preslav Nakov,Shangsong Liang*

Main category: cs.IR

TL;DR: 本文提出DenoiseRank，首个从生成式视角（扩散模型）解决学习排序（LTR）任务的方法，通过在扩散过程中对相关标签加噪并在逆过程中去噪来预测排序分布，实验验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统LTR模型几乎都从判别式视角实现，缺乏从生成式视角解决排序问题的工作。

Method: 提出DenoiseRank模型，利用扩散过程对相关标签加噪，再通过逆过程对查询文档去噪，以准确预测其排序分布。

Result: 在基准数据集上的大量实验证明了DenoiseRank的有效性。

Conclusion: DenoiseRank是首个从生成式视角处理传统LTR的扩散方法，为生成式LTR任务提供了基准。
...
### [241] [A Systematic Study of Biomedical Retrieval Pipeline Trade-offs in Performance and Efficiency](https://arxiv.org/abs/2604.20853)
*Hayk Stepanyan,Matthew McDermott*

Main category: cs.IR

TL;DR: 本文通过实证研究，系统分析了检索管道设计选择（如语料库选择、分块粒度、向量索引配置）对生物医学文本检索性能和效率的影响，并提供了具体实践指导。


<details>
  <summary>Details</summary>
Motivation: 检索系统在生物医学和临床NLP中越来越重要，但缺乏针对研究人员的实用指导，因此需要系统的实证研究来填补这一空白。

Method: 在多种公开生物医学文本数据集上，使用不同类型查询（考试题、对话式医疗查询、社区问答等），评估不同检索管道配置（语料库选择、分块策略、向量索引）的效果，采用LLM-as-a-judge结合人工验证进行胜率比较。

Result: 语料库聚合在绝对检索质量上表现最优；MedRAG/pubmed在HNSW图索引下是帕累托最优的单一语料库；找到了合适的分块策略和FAISS索引配置以平衡速度与效率。

Conclusion: 为研究人员提供了具体的检索系统构建指导建议，包括语料库选择、索引方式和分块策略的最佳实践。
...
### [242] [ERA: Evidence-based Reliability Alignment for Honest Retrieval-Augmented Generation](https://arxiv.org/abs/2604.20854)
*Sunguk Shin,Meeyoung Cha,Byung-Jun Lee,Sungwon Park*

Main category: cs.IR

TL;DR: 提出ERA框架，通过将置信度估计从标量概率转向显式证据分布，利用狄利克雷分布和Dempster-Shafer理论来区分认知不确定性与数据歧义，增强RAG系统的拒答能力。


<details>
  <summary>Details</summary>
Motivation: RAG系统面临内部参数知识与检索信息之间的知识冲突问题，现有可靠性方法仅依赖标量置信度，无法明确区分认知不确定性（认知不确定性）和固有数据歧义（偶然不确定性）。

Method: (1) 上下文证据量化：通过狄利克雷分布将内部和外部知识建模为独立信念质量；(2) 知识冲突量化：利用Dempster-Shafer理论严格测量信息源之间的几何不一致性，从而分离认知不确定性与偶然不确定性，并根据检测到的冲突调整优化目标。

Result: 在标准基准和策划的泛化数据集上，ERA显著优于基线方法，在答案覆盖率和拒答率之间实现了更优的权衡，且具有更好的校准性能。

Conclusion: 通过显式证据分布建模和冲突量化，ERA有效提升了RAG系统在知识冲突场景下的可靠性，为不确定性估计提供了更细粒度的解决方案。
...
### [243] [Caesar: Deep Agentic Web Exploration for Creative Answer Synthesis](https://arxiv.org/abs/2604.20855)
*Jason Liang,Elliot Meyerson,Risto Miikkulainen*

Main category: cs.IR

TL;DR: Caesar是一个基于智能体LLM的架构，通过知识图谱驱动的关联推理和对抗性草稿优化，实现从被动检索到创造性发现的跨越，在需要创造力的任务中显著优于现有LLM研究智能体。


<details>
  <summary>Details</summary>
Motivation: 现有智能体框架偏向收敛式搜索，只能生成缺乏创造性的衍生性摘要，无法实现从被动检索到新思想创造性发现的跨越。

Method: Caesar包含两个核心组件：(1) 基于动态上下文感知策略驱动的知识图谱探索，用于促进关联推理；(2) 对抗性草稿优化循环控制合成过程，主动寻求新颖视角而非确认已有先验知识。

Result: 在需要创造力的任务中，Caesar生成的工件和答案具有高度新颖性和结构连贯性，显著优于最先进的LLM研究智能体。

Conclusion: 通过知识图谱与对抗性优化机制的结合，Caesar有效弥合了信息收集与新颖洞察合成之间的鸿沟，为智能体系统在创造性发现任务上开辟了新路径。
...
### [244] [CRED-1: An Open Multi-Signal Domain Credibility Dataset for Automated Pre-Bunking of Online Misinformation](https://arxiv.org/abs/2604.20856)
*Alexander Loth,Martin Kappes,Marc-Oliver Pahl*

Main category: cs.IR

TL;DR: CRED-1是一个开放、可复现的领域级可信度数据集，整合了两个开源源列表和四个富集信号（域名年龄、网络流行度、事实核查频率、威胁情报），覆盖2,672个域名并赋予0.0-1.0的可信度评分，支持隐私保护的浏览器端预辟谣部署。


<details>
  <summary>Details</summary>
Motivation: 现有可信度数据集缺乏可复现性、开放性和多维度富集特征，难以支持隐私保护场景下的客户端预辟谣机制。

Method: 整合OpenSources.co和Iffy.news两个开源源列表，结合WHOIS/RDAP域名年龄、Tranco Top-1M网络流行度、Google Fact Check Tools事实核查频率和Google Safe Browsing威胁情报四个富集信号，使用Python标准库构建完整的可复现管道，采用CC BY 4.0许可发布。

Result: 生成了覆盖2,672个域名（含虚假、不可靠、混合、阴谋论、讽刺五类）的数据集，每个域名获得0.0-1.0的复合可信度评分，完整的分析和代码已归档于Zenodo。

Conclusion: CRED-1为隐私保护的浏览器端预辟谣提供了可复现、可部署的开放数据集基础，推动了信息可信度评估的透明化和民主化。
...
### [245] [DiagramBank: A Large-scale Dataset of Diagram Design Exemplars with Paper Metadata for Retrieval-Augmented Generation](https://arxiv.org/abs/2604.20857)
*Tingwen Zhang,Ling Yue,Zhen Xu,Shaowu Pan*

Main category: cs.IR

TL;DR: DiagramBank是一个包含89,422张来自顶级期刊的示意图数据集，专为多模态检索和示例驱动的科学图表生成而设计，填补了AI科学家系统无法自动生成出版级示意图的空白。


<details>
  <summary>Details</summary>
Motivation: 现有AI科学家系统可以自动撰写论文和代码，但无法生成出版级科学示意图（如teaser figure），因为这类图表需要概念综合和规划，而现有系统要么缺失此功能，要么使用劣质替代方案。

Method: 通过自动化流水线从顶级期刊中提取图表及对应文内引用，利用CLIP模型区分示意图与普通图表/自然图像。每个实例配有摘要、标题到图表-引用对的丰富上下文，支持不同粒度的信息检索。

Result: 构建了包含89,422张示意图的大规模数据集DiagramBank，并提供了检索增强生成的代码库，用于示例条件化的teaser figures合成。

Conclusion: DiagramBank填补了AI科学论文生成中示意图自动生成的关键空白，通过大规模数据集和检索增强生成框架，有望推动端到端科学论文自动生成的完整流程。
...
### [246] [Mixture of Sequence: Theme-Aware Mixture-of-Experts for Long-Sequence Recommendation](https://arxiv.org/abs/2604.20858)
...
TL;DR: 本文提出MoS框架，通过主题感知路由和多尺度融合机制，从长序列中提取主题一致子序列以解决用户兴趣漂移问题，实现SOTA性能且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 长序列中用户存在显著的"会话跳跃"行为模式（兴趣在会话内稳定，会话间剧烈漂移且可能重现），传统方法易受无关或误导信息干扰，难以准确建模动态用户兴趣。

Method: 提出MoS框架：1）主题感知路由机制，自适应学习序列潜在主题并组织成多个一致子序列；2）多尺度融合机制，利用三类专家（全局特征、短期行为、主题语义）捕获多维度信息。

Result: 在多个数据集上取得SOTA性能，且相比其他MoE方法FLOPs更少，实现了效用与效率的良好平衡。

Conclusion: MoS有效解决了长序列推荐中的兴趣漂移问题，验证了主题感知子序列提取和多尺度融合的有效性，为序列推荐提供了高效、准确的模型无关解决方案。
...
### [247] [KGiRAG: An Iterative GraphRAG Approach for Responding Sensemaking Queries](https://arxiv.org/abs/2604.20859)
*Isabela Iacob,Melisa Marian,Gheorghe Cosmin Silaghi*

Main category: cs.IR

TL;DR: 本文提出了一种迭代式、基于反馈的GraphRAG架构，通过质量评估循环优化输出，在HotPotQA数据集上相比单次基线获得了更高的语义质量和相关性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在RAG管线中处理复杂查询时存在幻觉问题，且上下文窗口有限难以充分支撑复杂查询的推理需求。

Method: 构建迭代式反馈驱动的GraphRAG架构，结合响应质量评估机制，对生成结果进行多轮修正直至生成合理、有充分依据的响应。

Result: 在HotPotQA数据集上的实验表明，该迭代RAG策略相比单次基线方法在语义质量和相关性上均有提升。

Conclusion: 迭代式图增强检索生成架构能有效缓解LLM幻觉问题，提升复杂查询场景下的回答质量。
...
### [248] [RealRoute: Dynamic Query Routing System via Retrieve-then-Verify Paradigm](https://arxiv.org/abs/2604.20860)
*Jiahe Liu,Qinkai Yu,Jingcheng Niu,Xi Zhu,Zirui He,Zhen Xiang,Fan Yang,Jinman Zhao*

Main category: cs.IR

TL;DR: RealRoute 系统提出了一种“检索-验证”机制替代传统的“LLM作为路由器”方法，通过并行、源无关的检索后跟动态验证器交叉检查结果，显著提升了多跳RAG推理任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法使用LLM作为路由器将子查询预测性地分派到不同数据源，但这种方式严重依赖数据源的语义边界，当源边界模糊时容易产生路由错误。

Method: 提出RealRoute框架，采用并行、源无关的检索（Retrieve-then-Verify机制），随后通过动态验证器交叉检查检索结果并合成事实性答案，而非传统的预测性路由。

Result: 实验表明RealRoute在多跳RAG推理任务上显著优于基于预测性路由的基线方法。

Conclusion: RealRoute通过从“预测性路由”转向“检索-验证”范式，有效解决了异构数据源场景下的路由错误问题，提升了多跳推理的答案准确性，并提供了开源工具和用户友好的可视化界面。
...
### [249] [Deep Interest Mining with Cross-Modal Alignment for SemanticID Generation in Generative Recommendation](https://arxiv.org/abs/2604.20861)
*Yagchen Zeng*

Main category: cs.IR

TL;DR: 本文提出一种融合深度上下文兴趣挖掘、跨模态语义对齐和质量感知强化机制的新型生成式推荐框架，用于解决现有语义ID生成方法中的信息退化、语义退化和模态失真问题，实验结果表明该方法显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐方法采用两阶段压缩流水线生成语义ID，存在三个关键缺陷：(1)信息退化——两阶段压缩导致语义丢失，且缺乏后验机制区分高低质量SID；(2)语义退化——级联量化丢弃了原始多模态特征的关键语义信息，嵌入生成与量化阶段未联合优化；(3)模态失真——量化器无法正确对齐文本与图像模态，即使上游网络已对齐，仍造成特征错位。

Method: 提出三个核心创新模块：1) 深度上下文兴趣挖掘(DCIM)——利用重构监督信号，挖掘广告上下文中隐含的高层语义信息，促使SID保留关键上下文信息；2) 跨模态语义对齐(CMSA)——借助视觉语言模型(VLM)将非文本模态对齐到统一文本语义空间，缓解模态失真；3) 质量感知强化机制(QARM)——采用强化学习框架，通过质量感知奖励鼓励生成富含语义的SID，同时在后验阶段抑制低质量SID。

Result: 在多个基准数据集上，该方法在各项评估指标上均持续优于当前最先进的SID生成方法，消融实验进一步验证了各模块的有效性。

Conclusion: 通过将多模态语义对齐、深度兴趣挖掘与强化学习质量感知机制有机结合，有效解决了现有SID生成中的信息退化、语义退化和模态失真问题，为生成式推荐系统提供了一种更优的语义ID生成方案。
...
### [250] [Following the Eye-Tracking Evidence: Established Web-Search Assumptions Fail in Carousel Interfaces](https://arxiv.org/abs/2604.21019)
*Jingwei Kang,Maarten de Rijke,Harrie Oosterhuis*

Main category: cs.IR

TL;DR: 本文通过眼动追踪实验数据，检验了将网页搜索中的用户行为假设（如F型模式、检查假设）迁移至轮播界面是否成立，发现这些假设在轮播界面中大多不成立，呼吁重新评估现有指标和点击模型。


<details>
  <summary>Details</summary>
Motivation: 轮播界面虽已成为流媒体服务的标准交互方式超过十年，但用户行为研究严重不足。先前工作缺乏实证基础，直接将单列表网页搜索中的行为模式（如F型模式、检查假设）假设为轮播界面也成立，用于设计点击模型和评估指标，这种假设的合理性亟需验证。

Method: 对近期一项针对轮播界面的眼动追踪研究公开数据集进行分析，系统检验三个关键假设是否成立：F型模式（垂直/水平方向的浏览规律）、检查假设（点击率与检查行为的关系）、以及对轮播标题的关注程度。

Result: (i) F型模式仅适用于垂直浏览，不适用于水平滑动；点击条件下用户的检查行为呈现独特的L型模式；(ii) 在控制检查行为后，点击率数据表明检查假设在轮播界面中不成立；(iii) 用户通常忽略轮播标题，直接关注内容项。

Conclusion: 许多从网页搜索迁移到轮播推荐界面中的用户行为假设（尤其是检查模式相关假设）并不成立，表明该领域缺乏构建用户行为模型的可靠基础，有必要重新评估轮播界面的现有指标和点击模型。
...
### [251] [Automated Extraction of Pharmacokinetic Parameters from Structured XML Scientific Articles: Enhancing Data Accessibility at Scale](https://arxiv.org/abs/2604.21063)
...
TL;DR: 药理学领域缺乏集中、全面的PK数据库，从文献表格中手动提取数据耗时且易错。为解决此问题，本文提出需要开发AI算法来自动检测和提取表格数据，以应对文献量激增和人力短缺的挑战。


<details>
  <summary>Details</summary>
Motivation: 药理学领域缺乏集中、全面且最新的药代动力学（PK）数据存储库，研究人员从大量科学出版物中手动收集PK参数费时费力。现有表格数据提取自动化方法因表格结构复杂、布局多样而效果不佳，急需开发更精准的AI算法。

Method: 提出开发AI算法用于表格检测与提取，重点在于精确处理按表格结构（由列和/或行标题指示）组织的单元格内容，使提取方式符合人类读者自然理解信息的方式。

Result: 尚未报告具体实验结果，本文主要阐述问题现状和需求。FARAD过去40余年手动提取数据，当前面临文献量激增和人力短缺的困境，需要自动化解决方案。

Conclusion: 必须开发能够精准处理表格结构的AI算法来自动化表格检测与数据提取，以解决手动提取繁琐易错的问题，满足药理学领域对PK数据高效整合的迫切需求。
...
### [252] [Multilingual and Domain-Agnostic Tip-of-the-Tongue Query Generation for Simulated Evaluation](https://arxiv.org/abs/2604.21096)
*Xuhong He,To Eun Kim,Maik Fröbe,Jaime Arguello,Bhaskar Mitra,Fernando Diaz*

Main category: cs.IR

TL;DR: 本文首次构建了多语言（中、日、韩、英）的“舌尖效应”（Tip-of-the-Tongue）检索基准测试集，每语言5000条查询，涵盖多个领域。研究通过基于LLM的查询模拟框架，系统分析了提示语言和源文档语言对模拟查询真实性的影响，发现有效的ToT模拟需要语言感知设计，并据此发布了四个多语言ToT测试集。


<details>
  <summary>Details</summary>
Motivation: 现有Tip-of-the-Tongue检索基准主要集中于英语，缺乏对多语言信息检索场景的覆盖，限制了其在非英语环境下的适用性。

Method: 利用基于LLM的查询模拟框架，系统性地研究了提示语言和源文档语言对模拟ToT查询保真度的影响；通过系统排名相关性对比合成查询与真实用户查询，从而验证合成查询的有效性。

Result: 有效的ToT模拟需要语言感知设计：非英语源语言通常更为重要，但当非英语源信息不足时，英语Wikipedia可发挥补充作用。最终发布了涵盖四种语言、每语言5000条查询的多领域测试集。

Conclusion: 本工作提供了首个大规模多语言ToT基准，并为在英语之外构建真实ToT数据集提供了实用指导。
...
### [253] [PAPERMIND: Benchmarking Agentic Reasoning and Critique over Scientific Papers in Multimodal LLMs](https://arxiv.org/abs/2604.21304)
*Yanjun Zhao,Tianxin Wei,Jiaru Zou,Xuying Ning,Yuanchen Bei,Lingjie Chen,Simmi Rana,Wendy H. Yang,Hanghang Tong,Jingrui He*

Main category: cs.IR

TL;DR: PAPERMIND是一个评估多模态大模型对科学论文进行综合推理的基准，包含七大学科的四个互补任务家族（多模态理解、实验解释、跨源证据推理、批判性评估），实验发现现有模型在综合科学推理方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准评估科学论文理解时往往孤立地测试单项能力（如问答或摘要），但真正的科学论文理解需要综合运用多模态理解、实验解释、跨源推理和批判性评估等相互关联的认知能力，现有方法难以评估这种综合推理能力。

Method: 从农业、生物、化学、计算机科学、医学、物理、经济学七个领域的真实科学论文构建PAPERMIND基准，包含四个互补任务家族：多模态理解（图文对齐）、实验解释（实验证据解读）、跨源证据推理（多源信息综合）、批判性评估（科学主张批判性分析）。

Result: 对开源和闭源多模态大模型的广泛实验表明，模型在不同任务间存在一致的性能差距，尤其在综合科学推理和批判性评估方面面临持续挑战。

Conclusion: PAPERMIND揭示了当前多模态大模型在综合科学推理方面的不足，为未来提升AI系统对科学论文的深度理解提供了诊断性评估工具和基准。
...
### [254] [WPGRec: Wavelet Packet Guided Graph Enhanced Sequential Recommendation](https://arxiv.org/abs/2604.21305)
*Peilin Liu,Zhiquan Ji,Gang Yan*

Main category: cs.IR

TL;DR: 本文提出WPGRec，是一种统一时频与图增强的序列推荐框架。它通过无下采样平稳小波包变换生成等长移不变子带序列，在每个子带上独立进行图传播以注入协作信号，并利用能量与谱平坦度感知的门控融合机制自适应聚合有效子带、抑制噪声。实验证明该方法在稀疏和复杂行为数据集上显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有频域方法存在两大问题：全局谱操作会混淆局部瞬态与长程依赖；基于滤波的小波管道在多尺度分解与重构中易产生时间错位和边界伪影。此外，用户-物品交互图的协作信号常通过尺度不一致的辅助模块注入，限制了时序动态与结构依赖的联合建模能力。

Method: 1) 全树无下采样平稳小波包变换生成等长、移不变的子带序列；2) 子带级别的交互图传播，在保持跨分辨率时间对齐的同时注入高阶协作信息；3) 能量与谱平坦度感知的门控融合模块，自适应聚合信息性子带并抑制噪声分量。

Result: 在四个公开基准数据集上，WPGRec一致优于序列推荐和图推荐基线，在稀疏和复杂行为数据集上提升尤为显著。

Conclusion: 带一致的结构注入与自适应子带融合能有效提升序列推荐性能，验证了多分辨率时间建模与图传播在匹配尺度上对齐的有效性。
...
### [255] [From Tokens to Concepts: Leveraging SAE for SPLADE](https://arxiv.org/abs/2604.21511)
*Yuxuan Zong,Mathias Vast,Basile Van Cooten,Laure Soulier,Benjamin Piwowarski*

Main category: cs.IR

TL;DR: 本文提出用稀疏自编码器(SAE)学习语义概念空间，替代SPLADE模型的骨干词汇表，构建SAE-SPLADE模型，在保持检索性能的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 传统SPLADE等稀疏IR模型依赖骨干词汇表，存在一词多义、同义词问题，且难以扩展到多语言和多模态场景。

Method: 用SAE学习潜在语义概念空间，替代原有骨干词汇表，探索两种概念的兼容性及训练方法。

Result: SAE-SPLADE在域内和跨域任务上检索性能与SPLADE相当，同时效率更高。

Conclusion: 用SAE学到的语义概念替代词汇表是可行的，能解决传统方法的局限性，且不牺牲性能。
...
### [256] [Pre-trained LLMs Meet Sequential Recommenders: Efficient User-Centric Knowledge Distillation](https://arxiv.org/abs/2604.21536)
...
TL;DR: 提出了一种知识蒸馏方法，利用预训练LLM生成的文本用户画像增强序列推荐系统，在不增加推理开销的前提下提升语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 序列推荐系统虽在时序建模上表现优异，但无法捕捉交互模式之外的丰富用户语义。LLM虽有推理潜力，但直接集成会导致实时推理成本过高。

Method: 通过知识蒸馏将预训练LLM生成的文本用户画像注入序列推荐模型，服务阶段无需LLM推理，且无需修改模型架构或微调LLM。

Result: 保持了传统序列模型的高效推理，同时增强了语义理解能力。

Conclusion: 该方法在不增加部署复杂度和推理成本的前提下，有效提升了序列推荐系统的语义建模能力，是一种轻量级且实用的LLM融合方案。
...
### [257] [Counterfactual Multi-task Learning for Delayed Conversion Modeling in E-commerce Sales Pre-Promotion](https://arxiv.org/abs/2604.21675)
*Xin Song,Kaiyuan Li,Jinxin Hu*

Main category: cs.IR

TL;DR: 本文提出CM-DCM模型，利用促销前历史数据（如加购行为）通过多任务学习、个性化门控和反事实因果推理，预测促销期间的延迟转化和直接转化，在离线实验和线上A/B测试中均取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 现有CVR预测研究忽视了促销前关键时期（用户只浏览加购但不购买），且延迟转化建模未考虑促销场景下延迟转化集中在促销日而非连续时间窗口的特殊分布偏移问题。

Method: ①多任务架构：联合建模直接转化和延迟转化；②个性化用户行为门控模块：缓解促销前短暂时段的数据稀疏性；③反事实因果方法：建模从加购(ATC)到延迟转化的转移概率。

Result: 离线实验优于所有基线方法；线上A/B测试在大型促销活动中显著提升广告收入、延迟转化GMV及整体GMV。

Conclusion: CM-DCM有效利用促销前数据提升促销期间CVR预测精度，尤其解决了延迟转化的分布偏移问题，验证了反事实多任务学习在电商促销场景中的实用性。
...
### [258] [Multistakeholder Impacts of Profile Portability in a Recommender Ecosystem](https://arxiv.org/abs/2604.21750)
*Anas Buhayh,Elizabeth McKinnie,Clement Canel,Robin Burke*

Main category: cs.IR

TL;DR: 本文研究推荐系统中算法多元化的影响，特别关注数据可移植性政策如何影响用户模型和多方利益相关者的结果。研究发现数据可移植性在不同推荐算法中对用户效用产生不同效果，并提出了相关政策考量。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注算法层面的干预（如多目标模型、重排序），但推荐生态系统结构层面的变化（如算法多元化/中间件）尚未被充分研究。同时，数据可移植性政策兴起，其对用户模型和利益相关者结果的影响尚不明确。

Method: 论文探讨了算法多元化（推荐算法与平台解耦，用户可自行选择算法）的场景，并结合数据可移植性政策（赋予用户数据所有权和控制权），分析这些政策如何影响用户模型以及不同利益相关者的结果。

Result: 数据可移植性场景在不同推荐算法下对用户效用产生差异化的影响。
...
### [259] [Biomedical systems biology workflow orchestration and execution with PoSyMed](https://arxiv.org/abs/2604.20906)
*Simon Süwer,Zoe Chervontseva,Kester Bagemihl,Jan Baumbach,Olga Tsoy,Andreas Maier*

Main category: cs.SE

TL;DR: PoSyMed是一个开源模块化平台，通过结合后端架构、形式化工具描述、容器化执行、持久化工作流状态和基于对话的界面，整合LLM作为受控语义助手，以解决生物信息学软件碎片化、文档不一致、依赖复杂和可重复性差等问题，提升生物医学分析的可重复性、可追溯性和透明性。


<details>
  <summary>Details</summary>
Motivation: 生物信息学软件快速增长导致工具分布碎片化、文档不一致、依赖复杂、执行环境难以复现，使得已发表工具的重用和工作流适配即使对有经验的用户也技术门槛高、耗时长。

Method: PoSyMed采用后端中心化平台架构，结合形式化工具描述、基于容器的受控构建与执行、持久化工作流状态以及基于对话的用户界面；LLM被集成受限的语义助手，用于辅助工具识别、工作流步骤建议和参数化，而非自主决策，所有操作在类型化、验证且人工监督的执行环境中进行。

Result: 文章描述了系统架构，并在代表性生物软件场景下评估了其工作流支持、交互设计和平台可扩展性方面的表现。PoSyMed已在https://apps.cosy.bio/posymed公开提供。

Conclusion: PoSyMed通过统一平台设计，将LLM作为受控辅助工具而非自主决策者，有效提升了生物医学分析的可重复性、可追溯性和透明性，降低了生物信息学工具重用和流程适配的技术门槛。
...
### [260] [Learning Reasoning World Models for Parallel Code](https://arxiv.org/abs/2604.20926)
*Gautam Singh,Arjun Guha,Bhavya Kailkhura,Harshitha Menon*

Main category: cs.SE

TL;DR: 提出并行代码世界模型(PCWMs)，通过推理预测工具输出结果来替代高成本的外部工具调用，在数据竞争预测和性能分析任务上显著提升准确率，并能有效辅助修复数据竞争。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在串行代码生成上表现优异，但在并行代码上因训练数据稀缺而表现欠佳。现有方法依赖外部工具调用的编码代理，但工具调用成本高且有时不切实际（如对部分代码）。因此需要一种能直接从并行源代码预测工具结果的推理模型。

Method: 设计探索与数据生成流水线：1) 从多领域采样多样化的并行编码问题和候选实现；2) 用工具执行并记录数据竞争和性能概况；3) 合成事后推理轨迹，将源代码因果性地连接到观察到的工具结果；4) 微调语言模型得到PCWMs。

Result: 7B模型在数据竞争预测上从64.3%提升至72.8%；8B模型在性能分析上从49.3%提升至58.6%。在修复数据竞争时，7B和14B的世界模型反馈相比自反馈分别提升2.7%-9.1%和6.1%-11.1%。

Conclusion: 推理模型有潜力作为并行编码代理中外部工具调用的实用替代方案。
...
### [261] [Residual Risk Analysis in Benign Code: How Far Are We? A Multi-Model Semantic and Structural Similarity Approach](https://arxiv.org/abs/2604.21051)
*Mohammad Farhad,Shuvalaxmi Dass*

Main category: cs.SE

TL;DR: 本文提出 Residual Risk Scoring (RRS) 框架，通过语义相似性、AST 结构相似性和跨模型一致性评估补丁后的残留风险。实验发现约 61% 的高 RRS 代码对仍存在 13 类残留安全缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准将修补后的函数视为天然安全，忽略了残留风险的存在。需要一种系统化的方法来评估补丁是否真正消除了安全风险。

Method: 基于 PrimeVul 数据集，使用多个 Code LMs 进行语义相似性分析，结合 Tree-sitter AST 进行结构相似性分析，最后通过 RRS 框架融合三类信号（嵌入语义相似性、局部 AST 结构相似性、跨模型一致性）来量化残留风险。

Result: 分析表明许多良性函数与易受攻击版本在语义和结构上高度相似。约 61% 的高 RRS 代码对被静态分析工具（Cppcheck、Clang-Tidy、Facebook-Infer）验证存在 13 类残留问题，包括空指针解引用、不安全内存分配等。

Conclusion: 代码级相似性可为优先审查补丁后代码提供实用信号，有助于在开源软件流水线中实现更可靠、可扩展的安全评估。
...
### [262] [Structural Quality Gaps in Practitioner AI Governance Prompts: An Empirical Study Using a Five-Principle Evaluation Framework](https://arxiv.org/abs/2604.21090)
*Christo Zietsman*

Main category: cs.SE

TL;DR: 本文提出一个基于可计算性理论和贝叶斯认识论的五原则评估框架，用于检验AI治理提示词的结构完整性。对GitHub上34个AGENTS.md文件的分析发现，37%的文件-模型对低于结构完整性阈值，且数据分类和评估标准最常缺失。研究表明从业者编写的治理提示词存在可被静态分析检测的结构模式。


<details>
  <summary>Details</summary>
Motivation: AI治理项目依赖自然语言提示词来约束和引导AI代理行为，但这些提示词作为可执行规格说明，缺乏系统性的结构完整性评估框架。现有实践没有方法判断治理提示词是否在结构上是完备的。

Method: 提出五原则评估框架（基于可计算性理论、证明论和贝叶斯认识论），对GitHub上34个公开的AGENTS.md治理文件进行实证分析，评估其结构完整性。

Result: 37%的文件-模型对低于结构完整性阈值；数据分类和评估标准是最常缺失的两个维度；从业者编写的治理提示词存在一致的结构模式。

Conclusion: 静态分析可以自动检测和修复治理提示词的结构缺陷；需关注AI辅助开发场景下的需求工程实践；AGENTS.md约定中存在此前未记录的人工制品分类缺口；需要开发相应的工具支持。
...
### [263] [A Ground-Truth-Based Evaluation of Vulnerability Detection Across Multiple Ecosystems](https://arxiv.org/abs/2604.21111)
*Peter Mandl,Paul Mandl,Martin Häusl,Maximilian Auch*

Main category: cs.SE

TL;DR: 本文使用基于OSV数据库构建的基准数据集，对多个软件生态系统的漏洞检测工具进行了实证评估，发现不同检测系统之间存在系统性差异，并提供了开源工具以支持可重复性研究。


<details>
  <summary>Details</summary>
Motivation: 漏洞检测工具被广泛使用，但由于漏洞数据源结构异构、标识符方案不一致以及版本范围说明模糊，对这些工具的评估一直充满挑战。

Method: 基于OSV数据库构建了将漏洞映射到具体包版本的真实基准数据集，系统地比较了不同工具和服务在不同软件生态系统中的检测结果，并提供了开源工具以自动重建数据集。

Result: 揭示了不同漏洞检测系统之间存在系统性差异，证明了透明的数据集构建方法对于可重复的实证安全研究至关重要。

Conclusion: 漏洞检测工具的评估需要标准化、可重复的数据集构建方法，本文的方法和开源工具有助于推动这一领域的可重复性研究。
...
### [264] [Hidden Dependencies and Component Variants in SBOM-Based Software Composition Analysis](https://arxiv.org/abs/2604.21278)
*Shawn Rasheed,Max McPhee,Lisa Patterson,Stephen MacDonell,Jens Dietrich*

Main category: cs.SE

TL;DR: 本文研究了SBOM中两种不匹配模式（隐藏代码级依赖和组件变体/克隆）如何导致不同扫描器之间漏洞报告和VEX声明处理的不一致，揭示了当前SBOM生成和消费的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着供应链攻击日益增多，SBOM成为重要的漏洞管理技术。但SBOM分析的有效性取决于其准确表示组件身份和实际依赖的能力，现有研究中对这些不匹配问题的影响缺乏系统性分析。

Method: 识别并定义两种不匹配模式：1) 隐藏的代码级依赖（未被表示为组件级依赖）；2) 组件变体/克隆（扫描器无法一致识别）。通过实验对比多个流行SBOM扫描器在漏洞报告和VEX声明处理上的一致性。

Result: 发现上述两种不匹配模式会导致不同SBOM扫描器之间产生不一致的漏洞报告结果，以及对VEX声明的不一致处理。

Conclusion: 当前SBOM在生产和消费方面存在局限性，需要更丰富的依赖表示和更完善的组件身份识别机制。
...
### [265] [Conjecture and Inquiry: Quantifying Software Performance Requirements via Interactive Retrieval-Augmented Preference Elicitation](https://arxiv.org/abs/2604.21380)
*Wang Shi Hai,Chen Tao*

Main category: cs.SE

TL;DR: IRAP通过交互式检索增强偏好引导，将自然语言描述的软件性能需求自动量化为数学函数，在四组真实数据集中以不到五轮交互实现高达40倍改进。


<details>
  <summary>Details</summary>
Motivation: 软件性能需求以自然语言记录，存在模糊性和认知不确定性，导致自动化量化成为一个尚未解决且具有挑战性的问题。

Method: 提出IRAP方法，通过交互式检索增强偏好引导，从问题特定知识中检索和推理偏好，并指导与利益相关者的渐进式交互，降低认知负荷。

Result: 在四个真实数据集上与10种最先进方法对比，IRAP在所有案例中均表现优越，仅需不到五轮交互即可实现高达40倍的性能提升。

Conclusion: IRAP有效解决了性能需求的自动化量化问题，通过检索增强的偏好引导实现了高效、低认知开销的交互式量化。
...
### [266] [Assessing the Impact of Requirement Ambiguity on LLM-based Function-Level Code Generation](https://arxiv.org/abs/2604.21505)
*Di Yang,Xinou Xie,Xiuwen Yang,Ming Hu,Yihao Huang,Yueling Zhang,Weikai Miao,Ting Su,Chengcheng Wan,Geguang Pu*

Main category: cs.SE

TL;DR: 本文提出了首个针对模糊需求的代码生成基准Orchid，包含1,304个函数级任务，覆盖四种模糊类型。研究发现模糊性会持续降低LLM的代码生成性能，且越先进的模型受影响越大，LLM也无法自主识别或解决需求模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常假设需求清晰明确，忽略了真实世界中自然语言固有的模糊性和不同利益相关者的解读差异，导致LLM在模糊需求下的代码生成能力缺乏系统性评估。

Method: 构建了Orchid基准数据集（1,304个函数级任务，覆盖词汇、句法、语义和模糊性四种模糊类型），并开展了系统性的实证研究，评估不同LLM在模糊需求下的代码生成性能。

Result: 模糊性一致地降低了所有评估LLM的性能，且越先进的模型受负面影响越显著；LLM常对同一模糊需求生成功能不同的实现，且缺乏自主识别或解决模糊性的能力。

Conclusion: 清晰需求与模糊需求之间存在显著的性能差距，亟需在下一代自动化软件工程工具中引入模糊性感知技术。
...
### [267] [SpecSyn: LLM-based Synthesis and Refinement of Formal Specifications for Real-world Program Verification](https://arxiv.org/abs/2604.21570)
*Lezhi Ma,Shangqing Liu,Yi Li,Qiong Wu,Han Wang,Lei Bu*

Main category: cs.SE

TL;DR: SpecSyn是一种基于大语言模型的新型规范生成方法，通过程序分解和基于语义非等价变异与变体区分的规范精炼机制，有效提升了真实世界程序验证中规范生成的精确性和召回率，在开源程序上成功处理了1071/1365个目标属性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动化规范生成方法在处理大规模输入程序时缺乏有效策略，且没有机制评估和保证生成规范的强度，导致难以从真实世界的复杂程序中提取精确规范以支持目标属性的验证。

Method: SpecSyn首先将输入程序分解为独立片段，然后对每个片段进行迭代式规范生成。创新性地引入基于语义非等价程序突变和变体区分的规范精炼机制，用于评估并增强生成规范的语义强度。

Result: SpecSyn保持90%以上的高精度和75%以上的召回率，显著优于现有基于LLM的方法。在开源程序评估中成功处理了1365个目标属性中的1071个。

Conclusion: SpecSyn通过程序分解与语义级规范精炼机制，有效提升了LLM在真实世界程序验证任务中的规范生成能力，证明了其在复杂程序验证中的实用性。
...
### [268] [A Metamorphic Testing Approach to Diagnosing Memorization in LLM-Based Program Repair](https://arxiv.org/abs/2604.21579)
*Milan De Koning,Ali Asgari,Pouria Derakhshanfar,Annibale Panichella*

Main category: cs.SE

TL;DR: 本研究提出结合蜕变测试（MT）与负对数似然（NLL）的方法，更可靠地检测LLM在自动程序修复（APR）中的数据泄露问题。实验发现，7种LLM在变换后的基准测试上补丁生成成功率显著下降（-4.1%~-15.98%），且下降幅度与NLL高度相关，表明模型对记忆的样本表现更好。


<details>
  <summary>Details</summary>
Motivation: LLM在APR中的性能评估可能因数据泄露（模型预训练数据与评估基准重叠）而虚高，现有检测方法不够可靠，需要更有效的检测手段。

Method: 对Defects4J和GitBug-Java数据集应用语义保持变换构建变体基准，评估7种LLM在原始与变换版本上的修复成功率，并分析性能下降与NLL的关系。

Result: 所有LLM在变换基准上成功率均显著下降（GPT-4o下降4.1%，Llama-3.1下降15.98%），且下降程度与NLL高度相关。

Conclusion: 结合MT与NLL能提供更强、更可靠的数据泄露证据，仅使用MT即可部分缓解LLM-based APR评估中的数据泄露影响。
...
### [269] [DryRUN: On the Role of Public Tests in LLM-Driven Code Generation](https://arxiv.org/abs/2604.21598)
*Kaushitha Silva,Srinath Perera*

Main category: cs.SE

TL;DR: 本文提出DryRUN框架，让大语言模型自主生成测试输入并模拟执行轨迹来实现代码自修正，无需依赖人工提供的公开测试用例，在LiveCodeBench v6上达到与依赖公开测试的SOTA方法CodeSIM相当的性能，同时降低了token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有基于仿真的代码生成方法依赖人工提供的公开测试用例来驱动调试循环，而人工编写测试用例是软件开发中的瓶颈；且真实场景中ground-truth的输入输出示例在实现前几乎不可用，导致这些方法局限于竞赛编程数据集。此外，依赖公开测试还会导致"过度自信差距"，使框架过拟合简单示例。

Method: 提出DryRUN框架，让LLM自主生成有效输入并模拟执行轨迹进行自修正，无需ground-truth样本。框架通过迭代规划、自主生成输入和模拟执行来缓解算法过度自信。

Result: 在LiveCodeBench v6数据集（2025年3月后）上，DryRUN在不依赖公开测试用例和外部执行反馈的条件下，达到了与依赖公开测试的SOTA方法CodeSIM相当的性能，同时减少了输出token消耗。

Conclusion: 外部样本输入并非代码生成的必要条件，LLM可自主生成输入并通过模拟执行轨迹实现自我修正，打破了对人工测试用例的依赖瓶颈。
...
### [270] [Verifying Machine Learning Interpretability Requirements through Provenance](https://arxiv.org/abs/2604.21599)
*Lynn Vonderhaar,Juan Couder,Daryela Cisneros,Omar Ochoa*

Main category: cs.SE

TL;DR: 本文提出利用机器学习（ML）溯源（provenance）来验证ML模型的可解释性非功能需求（NFR），通过保存模型和数据溯源形成可量化的功能需求（FRs），从而间接验证可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有ML工程文献虽定义了可解释性等非功能需求，但可解释性难以度量，无法确认模型是否满足要求，亟需一种可验证的方法。

Method: 提出利用ML溯源方法，保存模型和数据溯源信息（如训练过程、数据来源、模型决策路径等），将这些溯源信息转化为可量化的功能需求（FRs），通过对这些FRs的验证来间接验证可解释性NFR。

Result: 提供了将不可度量的可解释性需求转化为可验证的功能需求的具体途径，使得可解释性NFR的验证成为可能。

Conclusion: 本文贡献了一种基于溯源的ML可解释性验证方法，为ML工程中非功能需求的严格验证提供了可行方案。
...
### [271] [Can Large Language Models Assist the Comprehension of ROS2 Software Architectures?](https://arxiv.org/abs/2604.21699)
*Laura Duits,Bouazza El Moutaouakil,Ivano Malavolta*

Main category: cs.SE

TL;DR: 本文通过控制实验评估了9种LLM在回答ROS2系统架构相关问题上的表现，发现平均准确率达98.22%，其中Gemini-2.5-pro表现最佳（100%），但模型在解释连贯性和复杂度上存在差异，且大多数错误集中在最复杂的系统上。


<details>
  <summary>Details</summary>
Motivation: ROS2是机器人软件开发的主流框架，但其架构极其复杂（包含数千个分布式通信组件）。研究者希望评估LLM能否帮助开发者理解这类复杂系统的架构事实信息，以提升软件理解效率。

Method: 研究者设计了控制实验：针对3个规模递增的ROS2系统，通过自动化算法生成架构相关问题（共1230条提示词），向9种LLM提问。通过运行并监控真实系统建立基准真相，评估LLM回答的准确性，并定性分析其解释质量。

Result: 所有LLM平均准确率达98.22%；Gemini-2.5-pro表现最佳（100%），GPT-4.1最差（95%）；1230条提示中仅300条回答错误，其中249条来自最复杂系统。解释连贯性得分范围（0.394-0.762），困惑度均值差异显著（19.6-103.6）。

Conclusion: LLM在辅助ROS2开发者理解系统架构方面潜力巨大，但开发者需注意不同LLM的性能差异和固有局限性，并据此谨慎使用。
...
### [272] [Automated LTL Specification Generation from Industrial Aerospace Requirements](https://arxiv.org/abs/2604.21715)
*Zhi Ma,Xiao Liang,Cheng Wen,Rui Chen,Bin Gu,Shengchao Qin,Cong Tian,Mengfei Yang*

Main category: cs.SE

TL;DR: 提出AeroReq2LTL框架，利用大语言模型并结合数据字典和模板化需求语言，将航空航天自然语言需求自动转化为LTL属性，在真实数据集上达到85%精确率和88%召回率。


<details>
  <summary>Details</summary>
Motivation: 将自然语言需求转化为LTL形式化属性是航空航天工业中的关键痛点，该过程耗时、易错且需要稀缺的双领域专家知识，现有NL-to-LTL工具在真实工业文档上的表现不佳。

Method: 提出AeroReq2LTL框架，包含两大创新：(i) 数据字典——将领域技术术语标准化为精确的原子命题；(ii) 模板化需求语言——在翻译前显式化时间线索和逻辑关系。整体基于大语言模型实现自动化LTL属性生成。

Result: 在真实航空航天数据集上，AeroReq2LTL实现85%精确率和88%召回率，且生成的LTL可直接被现有验证工具使用。

Conclusion: AeroReq2LTL通过结合领域知识（数据字典）和结构化的需求模板，有效弥合了自然语言与形式化规约之间的鸿沟，为航空安全关键软件的自动化验证提供了可行方案。
...
### [273] [Agentic AI-assisted coding offers a unique opportunity to instill epistemic grounding during software development](https://arxiv.org/abs/2604.21744)
*Magnus Palmblad,Jared M. Ragland,Benjamin A. Neely*

Main category: cs.SE

TL;DR: 本文提出GROUNDING$.$md，一种由社区治理的领域范围认知基础文档，用于约束AI辅助编程，确保生成代码符合科学正确性所需的硬约束和社区约定的惯例参数，以质谱蛋白质组学为例进行说明。


<details>
  <summary>Details</summary>
Motivation: 当前AI辅助编程发展迅速，但非领域专家在生成科学软件时难以确保正确性和最佳实践，需要一种机制将领域知识编码到AI开发流程中。

Method: 提出GROUNDING$.$md文档，包含两类内容：硬约束（保证科学正确性的不可协商的有效性不变量）和惯例参数（社区约定的默认值），该文档覆盖所有其他上下文，强制执行有效性。

Result: 非领域专家可借助该文档生成具有内置最佳实践的代码、工具和软件，增强开发者和审查/使用者的信心。

Conclusion: 利用AI比人类更容易遵守指南的特点，组织可开发此类认知基础文档，让领域专家在未来民主化定制软件生成中保持参与。
...
### [274] [Less Is More: Measuring How LLM Involvement affects Chatbot Accuracy in Static Analysis](https://arxiv.org/abs/2604.21746)
*Krishna Narasimhan*

Main category: cs.SE

TL;DR: 比较了三种LLM参与程度的架构（直接生成、结构化中间表示、工具增强智能体）将自然语言转换为Joern查询语言的效果。结构化中间表示方法在大模型上表现最好，超越直接生成15-25个百分点，尽管智能体方法消耗8倍token。约束LLM输出到良好类型化的中间表示比无约束生成或迭代工具使用更有效。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言驱动的静态分析工具在LLM参与程度上差异很大，但缺乏将其作为独立变量进行系统比较的研究。作者希望探究不同LLM介入程度对代码分析任务性能的影响。

Method: 设计了三种架构：(1)直接生成CPGQL查询；(2)先生成模式约束的JSON中间表示再转换；(3)工具增强的智能体式生成。在包含20个代码分析任务的基准上进行评估，使用4个开源模型（2个模型族×2个规模），每种设置重复3次。

Result: 结构化中间表示方法(方法2)在结果匹配率上最高，大模型上比直接生成高15-25个百分点。智能体方法(方法3)消耗8倍token但表现不如方法2。小模型的主要瓶颈是模式合规性。

Conclusion: 在形式化结构化领域中，约束LLM输出到类型化的中间表示，并将查询构建委托给确定性代码，比无约束生成或迭代工具使用效果更好。
...
### [275] [Generalizing Test Cases for Comprehensive Test Scenario Coverage](https://arxiv.org/abs/2604.21771)
*Binhang Qi,Yun Lin,Xinyi Weng,Chenyan Liu,Hailong Sun,Gordon Fraser,Jin Song Dong*

Main category: cs.SE

TL;DR: 本文提出TestGeneralizer框架，通过三个阶段（需求理解、场景模板生成、测试用例生成与优化）从单个初始测试用例泛化出覆盖全面测试场景的测试用例，在12个Java项目上相比基线方法在变异覆盖和LLM评估的场景覆盖上分别提升31.66%和23.08%。


<details>
  <summary>Details</summary>
Motivation: 开发者在实际中难以生成全面的测试用例，现有自动化测试工具主要关注代码覆盖率而非真正的测试场景，而测试场景源自未文档化的需求，导致重要测试常被延迟甚至遗漏。

Method: TestGeneralizer框架包含三个阶段：1) 增强对焦点方法和初始测试背后需求与场景的理解；2) 生成测试场景模板并具体化为多种场景实例；3) 从这些实例生成并优化可执行测试用例。

Result: 在12个开源Java项目上，相比ChatTester等基线方法，在基于变异的场景覆盖上提升31.66%，在LLM评估的场景覆盖上提升23.08%。

Conclusion: 利用单个初始测试用例即可泛化出覆盖多样测试场景的测试套件，比现有代码覆盖驱动的方法更符合实际开发中对测试场景完整性的需求。
...
### [276] [Finding Pareto frontier for one-sided matching](https://arxiv.org/abs/2604.21306)
*Bhavik Dodda,Garima Shakya*

Main category: cs.GT

TL;DR: 本文提出逆顶交易环枚举算法（ITEA），用于高效计算单边匹配问题中所有帕累托最优分配，弥补了传统TTC机制只能得到单一解的局限。


<details>
  <summary>Details</summary>
Motivation: 传统TTC机制虽然能保证帕累托最优，但只能得到单一解，无法揭示帕累托前沿的完整结构，限制了在公平性或福利目标上的二次优化。

Method: 提出逆顶交易环枚举算法（ITEA），通过逆推TTC过程来系统枚举所有帕累托最优分配，并证明了算法的正确性和完备性。

Result: 理论上证明了算法的正确性和完备性；在最坏情况下可能有n!个PO分配，但与暴力枚举相比，当PO分配数量较少时显著降低了计算复杂度；实验表明能大幅减少冗余TTC计算。

Conclusion: ITEA算法能够高效地刻画单边匹配问题的帕累托前沿，为后续在公平性或福利目标上的二次优化提供了基础。
...
### [277] [A Markovian Traffic Equilibrium Model for Ride-Hailing](https://arxiv.org/abs/2604.21359)
*Song Gao,Hanyu Cheng,Chiwei Yan,Guocheng Jiang*

Main category: cs.GT

TL;DR: 本文构建了一个网约车马尔可夫交通均衡模型，将车辆（空车或载客）的接单和路径选择建模为无限时域半马尔可夫决策过程，内生化了空车竞争和交通拥堵，证明了均衡存在性并开发了迭代算法，实验表明忽略拥堵或司机前瞻性行为会导致政策评估偏差。


<details>
  <summary>Details</summary>
Motivation: 现有网约车模型通常忽略交通拥堵或司机前瞻性行为（或仅处理其一），无法准确刻画空车竞争、拥堵与司机决策之间的复杂交互，导致政策评估存在偏差。

Method: 将网约车运营建模为无限时域半马尔可夫决策过程，车辆（空车/载客）在网络中依次做出接单和路径选择，最大化总折扣回报；通过不动点系统刻画均衡，证明了均衡存在性，并开发了松弛不动点迭代算法（对特殊网络结构有收敛性保证）。

Result: 在真实网络上验证了模型的实际规划价值；消融实验表明，忽略交通拥堵或司机前瞻性行为均会导致政策评估产生显著偏差。

Conclusion: 模型能更真实地反映网约车系统的运行规律，对交通规划与政策评估具有重要实践意义，且同时考虑拥堵和前瞻性行为不可或缺。
...
### [278] [Compliance Moral Hazard and the Backfiring Mandate](https://arxiv.org/abs/2604.21789)
*Jian Ni,Lecheng Zheng,John R Birge*

Main category: cs.GT

TL;DR: 本文针对竞争性银行网络中分散化风险分析的信息聚合问题，提出了一种时间价值分配（TVA）机制，通过严格恰当评分规则实现真实报告均衡，并证明竞争压力会放大合规道德风险，不当设计可能降低社会福利。


<details>
  <summary>Details</summary>
Motivation: 竞争性企业服务共同客户群体时面临信息聚合困境：每家机构持有关于风险客户的碎片化信号，但个体激励阻碍了集体检测的效率。反洗钱场景中尤其突出。

Method: 设计了一种时间价值分配（TVA）机制，基于贴现验证结果使用严格恰当评分规则对机构进行激励，实现了大规模联邦中每家机构的真实报告作为贝叶斯-纳什均衡。

Result: TVA机制在大规模网络中实现真实报告均衡；竞争压力放大合规道德风险；不当设计的强制共享可能降低福利（"反效果"结果）；模拟显示TVA显著优于无激励的孤立或强制共享方案。

Conclusion: TVA机制能有效解决分散化风险分析中的信息聚合问题，但需注意竞争环境中的激励设计，避免政策"反效果"。
...
### [279] [AttentionBender: Manipulating Cross-Attention in Video Diffusion Transformers as a Creative Probe](https://arxiv.org/abs/2604.20936)
*Adam Cole,Mick Grierson*

Main category: cs.MM

TL;DR: AttentionBender是一个通过操纵视频扩散Transformer中交叉注意力机制的2D变换（旋转、缩放、平移等）来探测模型内部机制并产生新颖美学的工具，研究发现交叉注意力高度纠缠，难以实现局部控制，反而产生分布式失真和故障美学效果。


<details>
  <summary>Details</summary>
Motivation: 当前生成式视频输出越来越逼真，但仅依赖提示词控制限制了艺术家对模型内部运作机制的理解，也难以超越模型的默认生成倾向。因此需要一种工具帮助艺术家探测黑箱视频生成模型的内部机制。

Method: 采用自传式研究-通过设计方法（research-through-design），基于Network Bending思想构建AttentionBender，对交叉注意力图施加2D变换（旋转、缩放、平移等），并通过可视化4500+视频生成结果来评估不同提示词、操作类型和层目标的效果。

Result: 交叉注意力高度纠缠：针对性的操纵难以实现干净、局部的控制，反而产生分布式失真和故障美学效果，而非线性的编辑。

Conclusion: AttentionBender既可作为可解释AI风格的工具探测Transformer注意力机制，又可作为超越模型学习表征空间的创意技术来产生新颖美学。
...
### [280] [Sema: Semantic Transport for Real-Time Multimodal Agents](https://arxiv.org/abs/2604.20940)
*Jiaying Meng,Bojie Li*

Main category: cs.MM

TL;DR: 提出Sema语义传输系统，利用离散音频分词器和混合屏幕表示，将传输目标从信号保真度转向语义保真度，在保持任务精度的同时大幅降低带宽需求（音频64倍，截图130-210倍）。


<details>
  <summary>Details</summary>
Motivation: 现有实时多模态智能体使用面向人类接收者的网络传输协议，优化感知保真度和流畅播放，但智能体模型本质上是事件驱动的处理器，不需要重建实时信号，只需要任务相关语义。这种根本性不匹配导致巨大开销（截图上传播放延迟60%+，音频传输43-64倍冗余数据）。

Method: 1) 离散音频分词器替代连续音频传输；2) 混合屏幕表示：可访问性树/OCR文本+紧凑视觉令牌；3) 突发令牌传输消除抖动缓冲区。

Result: 在模拟WAN条件下，上行带宽降低64倍（音频）和130-210倍（截图），任务精度仅下降0.7个百分点。

Conclusion: 面向智能体的语义传输比面向人类的信号传输更高效，Sema通过语义分级传输实现了极致的带宽压缩而几乎不影响任务性能。
...
### [281] [High-Fidelity 3D Gaussian Human Reconstruction via Region-Aware Initialization and Geometric Priors](https://arxiv.org/abs/2604.21714)
*Yang Liu,Zhiyong Zhang*

Main category: cs.MM

TL;DR: 提出了一种结合区域感知初始化与丰富几何先验的3D高斯人体重建框架，利用SMPL-X模型初始化3D高斯和蒙皮权重，通过区域感知密度初始化策略和几何感知多尺度哈希编码模块，实现了复杂运动下的高质量实时3D人体重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的动态人体重建方法难以捕捉精细几何细节（如手指融合、面部过平滑），且空间场动态建模在重建质量与GPU内存消耗之间存在权衡。

Method: 1) 利用SMPL-X模型初始化3D高斯和蒙皮权重；2) 引入区域感知密度初始化策略；3) 提出几何感知多尺度哈希编码模块，提升局部细节恢复并保持计算效率。

Result: 在PeopleSnapshot和GalaBasketball数据集上，该方法在复杂运动下实现了更优的重建质量和更精细的细节保留，同时保持实时渲染速度。

Conclusion: 提出的区域感知初始化与几何先验结合策略有效解决了动态人体重建中的细节丢失和伪影问题，实现了高质量与实时性的兼顾。
...
### [282] [Architecture of an AI-Based Automated Course of Action Generation System for Military Operations](https://arxiv.org/abs/2604.20862)
*Ji-il Park,Inwook Shim,Chong Hui Kim*

Main category: cs.AI

TL;DR: 本文探讨了未来战争中自动化行动方案(CoA)规划系统的必要性，分析了传统人工方式的局限性，并在公开信息范围内提出了适用于CoA规划各阶段的AI技术及系统架构。


<details>
  <summary>Details</summary>
Motivation: 随着作战速度、监视范围和武器射程的扩展，传统人工CoA规划愈发困难，亟需开发基于AI的自动化CoA规划系统。

Method: 在公开信息范围内梳理相关条令，为CoA规划各阶段介绍适用的AI技术，并提出自动化CoA规划系统的总体架构。

Result: 提出了一套面向自动化CoA规划的系统架构设计方案，并明确了各阶段可采用的AI技术。

Conclusion: 该研究为军事领域AI自动化CoA规划系统的开发提供了理论框架和技术路径参考，但由于安全限制，技术成熟度仍难以准确评估。
...
### [283] [Escaping the Agreement Trap: Defensibility Signals for Evaluating Rule-Governed AI](https://arxiv.org/abs/2604.20972)
*Michael O'Herlihy,Rosa Català*

Main category: cs.AI

TL;DR: 本文提出了一种基于政策推理的内容审核评估框架，用Defensibility Index (DI) 和 Ambiguity Index (AI) 替代传统一致率指标，通过在193,000+ Reddit审核决策上验证，发现传统一致率与政策基础评估之间存在33-46.6个百分点的差距，并提出Probabilistic Defensibility Signal (PDS) 实现无需额外审计的推理稳定性估计。


<details>
  <summary>Details</summary>
Motivation: 传统内容审核系统使用与人工标签的一致性来评估模型，但在规则治理环境中，多个决策可能都与政策逻辑一致，一致率指标会错误地将有效决策视为错误（Agreement Trap），因此需要一种基于政策逻辑推理的正确性评估方法。

Method: 1. 形式化评估为政策基础正确性（policy-grounded correctness）2. 提出Defensibility Index (DI) 和 Ambiguity Index (AI) 3. 引入Probabilistic Defensibility Signal (PDS) 从审计模型token对数概率中估计推理稳定性 4. 审计模型不直接判断违规与否，而是验证决策是否可从规则层级中逻辑推导 5. 构建Governance Gate实现基于信号的自动化审核

Result: 在193,000+ Reddit决策上：一致率与政策基础指标差距33-46.6个百分点；79.8-80.6%模型假阴性实为政策基础正确决策；规则细化使AI降低10.8个百分点，DI保持稳定；PDS方差主要由治理歧义而非解码噪声驱动；Governance Gate实现78.6%自动化覆盖率与64.9%风险降低。

Conclusion: 规则治理环境中的审核评估应从与历史标签的一致率转向基于明确规则的推理有效性验证，政策基础评估能更准确刻画模型真实表现并揭示治理歧义。
...
### [284] [The CriticalSet problem: Identifying Critical Contributors in Bipartite Dependency Networks](https://arxiv.org/abs/2604.21537)
*Sebastiano A. Piccolo,Andrea Tagarelli*

Main category: cs.AI

TL;DR: 本文研究二分依赖网络中的关键节点识别问题，提出CriticalSet问题（寻找使最多物品失效的k个贡献者），证明其为NP-hard且目标函数为超模函数。通过合作博弈论推导出ShapleyCov中心性指标，并设计线性时间迭代剥离算法MinCov。实验表明MinCov在Wikipedia等大规模图上接近最优性能（AUC差0.02），速度却快数个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有关键节点识别方法大多针对全有或全无的覆盖机制，但在二分依赖网络（两类节点间存在依赖关系）中，这类问题尚未被充分探索。作者希望填补这一空白。

Method: 1. 形式化定义CriticalSet问题并证明其NP-hard性；2. 证明目标函数为超模函数（标准贪心算法无近似保证）；3. 将问题建模为合作博弈，基于Shapley值推导闭式中心性指标ShapleyCov；4. 提出线性时间迭代剥离算法MinCov，显式考虑连接冗余性，优先移除支撑多个唯一物品的贡献者。

Result: 在合成数据和大规模真实数据（包括2.5亿条边的Wikipedia图）上，MinCov和ShapleyCov显著优于传统基线方法。MinCov达到近最优性能（与随机爬山元启发式方法的AUC仅差0.02），但速度快数个数量级。

Conclusion: 本文为二分依赖网络中的关键节点识别提供了理论框架和高效算法，MinCov在保持近乎最优效果的同时具备极强可扩展性，适用于大规模实际应用场景。
...
### [285] [Co-Evolving LLM Decision and Skill Bank Agents for Long-Horizon Tasks](https://arxiv.org/abs/2604.20987)
*Xiyang Wu,Zongxia Li,Guangyao Shi,Alexander Duffy,Tyler Marques,Matthew Lyle Olson,Tianyi Zhou,Dinesh Manocha*

Main category: cs.AI

TL;DR: 本文提出COSPLAY框架，让LLM智能体从可学习的技能库中检索技能来指导动作，同时通过智能体管理的技能流水线从未标注的轨迹中提取可复用技能，实现了在长时域交互环境中的持续改进。在6个游戏环境中，使用8B基础模型的COSPLAY相比四个前沿LLM基线平均奖励提升25.1%。


<details>
  <summary>Details</summary>
Motivation: 长时域交互环境（如游戏）需要多步推理、技能链式调用和延迟奖励下的鲁棒决策，但LLM缺乏发现、保留和复用结构化技能的机制，导致在长时域决策中表现不稳定。

Method: 提出COSPLAY共进化框架：(1) LLM决策智能体从可学习的技能库中检索技能以指导动作选择；(2) 技能流水线智能体从智能体未标注的轨迹中持续提取、精炼和更新技能及其约束，形成技能库。两者协同进化，决策智能体学习更好的技能检索和动作生成，技能库智能体不断更新技能集合。

Result: 在6个游戏环境中，使用8B基础模型的COSPLAY相比四个前沿LLM基线（如GPT-4等）在单人游戏基准上平均奖励提升25.1%，在多玩家社交推理游戏中也能保持竞争力。

Conclusion: COSPLAY通过决策智能体与技能库智能体的共进化机制，有效解决了LLM在长时域交互中缺乏技能发现与复用的痛点，以较小模型实现了对更大模型的显著超越。
...
### [286] [Value-Conflict Diagnostics Reveal Widespread Alignment Faking in Language Models](https://arxiv.org/abs/2604.20995)
*Inderjeet Nair,Jie Ruan,Lu Wang*

Main category: cs.AI

TL;DR: 本文提出VLAF诊断框架，发现对齐伪装（alignment faking）现象比以往报告更普遍（如olmo2-7b-instruct在37%案例中伪装对齐），并利用表示空间中的单一对比方向向量实现轻量级推理时缓解，无需标注数据即可分别降低85.8%、94.0%和57.7%的对齐伪装行为。


<details>
  <summary>Details</summary>
Motivation: 现有对齐伪装诊断工具局限于高度有害场景，导致模型直接拒绝而无法真实评估其伪装倾向。需要一种能绕过拒绝行为、保留有意义道德权衡的诊断方法。

Method: 提出VLAF框架，基于"开发者政策与模型核心价值观冲突时最易发生对齐伪装"的假设，使用道德明确场景探测不同价值观冲突；发现表示空间中存在单一方向编码监督条件差异，利用该对比向量进行轻量级推理时缓解。

Result: 对齐伪装比以往报告更普遍，7B参数模型即可出现（olmo2-7b-instruct达37%）；缓解方法在olmo2-7b、olmo2-13b、qwen3-8b上分别降低85.8%、94.0%、57.7%的伪装行为。

Conclusion: 对齐伪装现象比预期更严重且可在小模型中出现，但通过表示空间的单一方向向量可有效且轻量地缓解，无需额外标注数据。
...
### [287] [The Last Harness You'll Ever Build](https://arxiv.org/abs/2604.21003)
*Haebin Seong,Li Yin,Haoran Zhang*

Main category: cs.AI

TL;DR: 本文提出一个两层级框架，自动化AI代理的"装备"（harness）工程过程，使代理能够快速适应新领域而无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在部署到新领域时，每个任务都需要大量人工专家进行提示词设计、工具开发、编排逻辑和评估标准制定，成本极高且难以规模化。

Method: 提出两层级框架：第一级"装备进化循环"通过Worker Agent执行任务、Evaluator Agent诊断失败并评分、Evolution Agent修改装备来优化单个任务的harness；第二级"元进化循环"跨任务优化进化协议本身，学习出能快速收敛到最优harness的元协议。

Result: 框架将人工装备工程转化为自动化装备工程，并进一步实现了"自动化设计自动化本身"的元级别自动化。

Conclusion: 该工作不仅实现了跨领域零人工装备的目标，还将自动化提升到元学习层面，为AI代理的规模化部署提供了可行路径。
...
### [288] [Deep FinResearch Bench: Evaluating AI's Ability to Conduct Professional Financial Investment Research](https://arxiv.org/abs/2604.21006)
...
TL;DR: 本文提出了Deep FinResearch Bench，一个用于评估金融投资研究中深度研究（DR）代理的基准框架，从定性严谨性、定量预测与估值准确性、以及声明可信度与可验证性三个维度评估报告质量，并发现AI生成的报告仍逊色于金融专业人士撰写的报告。


<details>
  <summary>Details</summary>
Motivation: 金融投资研究需要高质量的深度研究报告，但目前缺乏针对DR代理在金融领域的标准化评估框架，现有基准无法全面评估报告在定性、定量和可信度等方面的综合质量。

Method: 构建了一个三维度评估框架（定性严谨性、定量预测与估值准确性、声明可信度与可验证性），定义了对应的定性和定量评估指标，并实现了自动化评分流程以实现可扩展的评估。

Result: 将基准应用于前沿DR代理生成的金融报告，并与金融专业人士撰写的报告进行对比，发现AI生成的报告在三个维度上均存在不足。

Conclusion: 研究结果强调需要针对金融领域定制专业化的DR代理，该工作为金融研究中DR代理的标准化基准测试奠定了基础。
...
### [289] [Adaptive Test-Time Compute Allocation with Evolving In-Context Demonstrations](https://arxiv.org/abs/2604.21018)
*Bowen Zuo,Dongruo Zhou,Yinglun Zhu*

Main category: cs.AI

TL;DR: 本文提出了一种自适应测试时计算分配框架，通过动态调整计算分配和生成分布，在数学、编程和推理基准上以更少的推理计算量超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有测试时计算方法要么静态分配计算资源，要么从固定生成分布中采样，缺乏自适应调整的能力。

Method: 两阶段框架：1) 预热阶段识别简单查询并构建初始问答对池；2) 自适应阶段集中计算于未解决查询，通过语义相关的成功响应动态重塑生成分布（而非从固定分布重采样）。

Result: 在数学、编程和推理基准测试中，该方法一致优于现有基线，且消耗的推理时间计算量显著更少。

Conclusion: 联合优化计算分配与生成分布的自适应策略，能够更高效地利用测试时计算资源。
...
### [290] [HypEHR: Hyperbolic Modeling of Electronic Health Records for Efficient Question Answering](https://arxiv.org/abs/2604.21027)
*Yuyu Liu,Sarang Rajendra Patil,Mengjia Xu,Tengfei Ma*

Main category: cs.AI

TL;DR: HypEHR是一个基于双曲几何的紧凑型Lorentz模型，用于电子健康记录问答，通过层次感知的预训练和几何一致的交叉注意力机制，以远少于LLM的参数在MIMIC-IV基准上接近LLM方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的EHR问答系统部署成本高，且没有显式利用临床数据的层次结构（如ICD本体）。作者受医学本体和患者轨迹具有双曲几何特性的证据启发，提出更高效的解决方案。

Method: 提出HypEHR：一种紧凑的Lorentz模型，在双曲空间中嵌入代码、就诊记录和问题；通过几何一致的交叉注意力机制和类型特定的指针头来回答问题；采用下一就诊诊断预测和层次感知正则化进行预训练，以对齐ICD本体表示。

Result: 在两个基于MIMIC-IV的EHR-QA基准上，HypEHR以远少于LLM的参数数量，达到了接近LLM方法的性能。

Conclusion: HypEHR验证了双曲几何在建模EHR层次结构中的有效性，为低成本、高效的EHR问答提供了新范式。
...
### [291] [Who Defines Fairness? Target-Based Prompting for Demographic Representation in Generative Models](https://arxiv.org/abs/2604.21036)
*Marzia Binta Nizam,James Davis*

Main category: cs.AI

TL;DR: 提出一种轻量级推理时框架，通过提示层面干预缓解文本到图像模型中的社会偏见，无需重新训练或修改模型，支持用户自定义公平性目标。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型（如Stable Diffusion、DALL-E）在生成图像时会复现社会偏见（如浅肤色与高地位职业关联），而现有缓解方法需要重新训练或精心策划数据集，对大多数用户不可及。

Method: 在推理时通过提示层面干预：1）允许用户从多种公平性规范中选择目标分布（从均匀分布到LLM驱动的复杂定义）；2）根据目标比例构建人口统计特异的提示变体；3）审计生成图像的肤色分布与目标的一致性。

Result: 在36个提示（涵盖30种职业和6种非职业情境）上，该方法使生成的肤色分布朝着与声明目标一致的方向偏移，并减少了与肤色空间直接定义目标的偏差。

Conclusion: 展示了公平性干预可以透明、可控且可在推理时使用，直接赋能生成式AI用户，而非依赖模型开发者。
...
### [292] [Active Data](https://arxiv.org/abs/2604.21044)
*Richard Arthur,Virginia DiDomizio,Louis Hoebel*

Main category: cs.AI

TL;DR: 本文提出了一种基于"主动数据"（Active Data）的自底向上方法，将数据视为主动与环境交互的原子对象，用于应对大规模复杂数据集带来的计算和概念复杂性，并在空中交通流量管理领域进行了实现和性能评估。


<details>
  <summary>Details</summary>
Motivation: 在复杂领域中，某些问题特定的分解方法相比整体设计更具优势。为了实现对大型复杂数据集的理解和规范设计，作者希望找到一种直观且易于处理的方法来应对计算复杂性和概念复杂性。

Method: 提出基于Active Data的方法，将数据视为与环境主动交互的原子对象，采用自底向上的方式构建系统。

Result: 在空中交通流量管理领域实现了Active Data的基本概念，并讨论了该实现的性能表现。

Conclusion: Active Data作为一种自底向上的方法，能够有效改善面对计算和概念复杂性时的设计质量。
...
### [293] [InVitroVision: a Multi-Modal AI Model for Automated Description of Embryo Development using Natural Language](https://arxiv.org/abs/2604.21061)
*Nicklas Neu,Thomas Ebner,Jasmin Primus,Raphael Zefferer,Bernhard Schenkenfelder,Mathias Brunbauer,Florian Kromp*

Main category: cs.AI

TL;DR: 本文通过微调基础视觉语言模型PaliGemma-2，仅用1,000张胚胎图像和对应描述，开发了InVitroVision模型，能够预测胚胎形态和发育的自然语言描述，在IVF任务上优于商业模型ChatGPT 5.2和基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有AI在IVF中的应用常依赖标注数据且未充分利用IVF数据的多模态特性，缺乏能够直接预测胚胎形态和发育自然语言描述的通用模型。

Method: 使用公开的胚胎延时数据集，对多模态视觉语言模型PaliGemma-2进行微调，输入1,000张胚胎图像和对应描述，训练得到InVitroVision模型。

Result: InVitroVision在整体指标上优于ChatGPT 5.2和基础模型，且性能随训练数据量增加而提升。

Conclusion: 基础视觉语言模型能够以有限数据泛化到IVF任务，预测胚胎形态和发育的自然语言描述，有望促进大语言模型在IVF信息检索和临床决策中的应用。
...
### [294] [Mind the Prompt: Self-adaptive Generation of Task Plan Explanations via LLMs](https://arxiv.org/abs/2604.21092)
*Gricel Vázquez,Alexandros Evangelidis,Sepeedeh Shahbeigi,Radu Calinescu,Simos Gerasimou*

Main category: cs.AI

TL;DR: 本文提出COMPASS，一种将提示工程形式化为认知与概率决策过程的自我自适应方法，通过POMDP建模用户认知状态以自动生成适应性解释和提示优化，并在两个案例研究中验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 将LLM集成到复杂系统中可生成可理解的AI解释，但提示工程的质量和可靠性缺乏系统性理解，难以自动化；不同利益相关者如何制定和优化提示缺乏认知模型支持。

Method: 提出COMPASS框架，将提示工程形式化为部分可观测马尔可夫决策过程（POMDP），建模用户潜在认知状态（如注意力、理解、不确定性）和可观测交互线索，通过综合策略实现自适应解释生成与提示优化。

Result: 在两个不同的信息物理系统案例研究中，通过定量和定性评估验证了COMPASS在自适应解释生成和质量方面的可行性。

Conclusion: COMPASS能够将人类认知和用户画像反馈整合到复杂任务规划系统中的自动提示合成中，证明了该方法的可行性。
...
### [295] [Propensity Inference: Environmental Contributors to LLM Behaviour](https://arxiv.org/abs/2604.21098)
*Olli Järviniemi,Oliver Makins,Jacob Merizian,Robert Kirk,Ben Millwood*

Main category: cs.AI

TL;DR: 本文研究了语言模型在不同环境因素下的不当行为倾向，通过12个环境因素（6个战略、6个非战略）对23个模型和11个评估环境进行测量，发现战略与非战略因素对行为解释的贡献大致相等，且未发现能力提升时战略因素影响增强的趋势。


<details>
  <summary>Details</summary>
Motivation: 解决AI系统因目标不一致而产生的失控风险，需要量化评估语言模型从事未经授权行为的倾向。

Method: 提出三项方法论改进：分析环境因素变化对行为的影响、通过贝叶斯广义线性模型量化效应量、采取明确措施避免循环分析。对23个语言模型在11个评估环境中测量12个环境因素（6战略、6非战略）的影响。

Result: 战略与非战略因素对行为解释的贡献大致相等；未发现战略因素随能力提升而影响力增强的趋势；发现目标冲突敏感性存在一定增强趋势的证据。

Conclusion: 未来倾向性研究的关键方向是发展AI决策的理论框架和认知模型，并将其转化为可经验检验的形式。
...
### [296] [AI Governance under Political Turnover: The Alignment Surface of Compliance Design](https://arxiv.org/abs/2604.21103)
*Andrew J. Peterson*

Main category: cs.AI

TL;DR: 本文研究政府使用AI进行行政决策时，合规层如何既改善监督又可能被后续政府利用。通过形式化模型分析自动化规模、编码程度和迭代使用保障措施，揭示了内部战略利用的脆弱性、改革先改善后增加风险的现象，以及AI使用扩张难以逆转的原因。


<details>
  <summary>Details</summary>
Motivation: 政府越来越有兴趣使用AI来使行政决策更便宜、更具扩展性且更一致。但概率性AI要融入公共管理，必须嵌入合规层以确保决策可审查、可重复且在法理上站得住脚。

Method: 开发了一个形式化模型，其中机构选择自动化规模、编码程度和迭代使用的保障措施。

Result: 模型显示：这些系统何时容易受到政府内部的战略利用；为何最初改善监督的改革后来可能增加这种脆弱性；以及为何AI使用的扩张可能难以逆转。

Conclusion: 使AI可用可能使得程序更容易被未来政府学习和利用。
...
### [297] [Agentic AI for Personalized Physiotherapy: A Multi-Agent Framework for Generative Video Training and Real-Time Pose Correction](https://arxiv.org/abs/2604.21154)
*Abhishek Dharmaratnakar,Srivaths Ranganathan,Anushree Sinha,Debanshu Das*

Main category: cs.AI

TL;DR: 本文提出一个基于多智能体系统（MAS）和生成式AI的架构，通过四个微智能体（临床提取、视频合成、视觉处理、诊断反馈）实现个性化远程康复训练，旨在解决家庭物理治疗依从性低的问题。


<details>
  <summary>Details</summary>
Motivation: 家庭物理治疗的依从性极低，现有数字健康方案缺乏个性化监督和动态反馈。静态视频库或通用3D虚拟人无法考虑患者的具体伤情限制和家庭环境。

Method: 提出四智能体MAS架构：1）临床提取智能体解析非结构化医疗记录为运动学约束；2）视频合成智能体用基础视频生成模型创建个性化锻炼视频；3）视觉处理智能体实时姿态估计；4）诊断反馈智能体发出纠正指令。原型采用LLM和MediaPipe构建。

Result: 展示了系统架构，详细说明了使用大规模语言模型和MediaPipe的原型管道，并概述了临床评估计划。证明了生成式媒体与智能体自主决策结合实现安全、可扩展的个性化患者护理的可行性。

Conclusion: 该工作展示了将生成式媒体与智能体自主决策相结合，能够安全有效地规模化提供个性化患者护理。
...
### [298] [Multi-Agent Empowerment and Emergence of Complex Behavior in Groups](https://arxiv.org/abs/2604.21155)
*Tristan Shah,Ilya Nemenman,Daniel Polani,Stas Tiomkin*

Main category: cs.AI

TL;DR: 本文研究了多智能体系统中 empowerment（赋能）这一内在动机如何自发产生群体组织行为，通过 tendon-coupled 双智能体系统和可控 Vicsek 群体两种环境验证了其效果。


<details>
  <summary>Details</summary>
Motivation: 内在动机（如 empowerment）在单个智能体中已显示出驱动行为的能力，但其在多智能体场景下如何导致群体组织行为的涌现尚不清楚，本文旨在探索这一空白。

Method: 将 empowerment 扩展至多智能体场景并给出高效计算方法，在两个不同环境（tendon-coupled 双智能体系统与可控 Vicsek 群体）中进行实验验证。

Result: 该内在动机在两个环境中均能自发产生特征性的群体组织模式，表明 empowerment 不仅能驱动单个智能体行为，还能驱动更高层次的群体行为组织。

Conclusion: 内在动机（如 empowerment）具有从个体到群体层面驱动行为涌现的潜力，可用于理解和设计多智能体系统中的集体行为。
...
### [299] [Trust but Verify: Introducing DAVinCI -- A Framework for Dual Attribution and Verification in Claim Inference for Language Models](https://arxiv.org/abs/2604.21193)
*Vipula Rawte,Ryan Rossi,Franck Dernoncourt,Nedim Lipka*

Main category: cs.AI

TL;DR: 本文提出DAVinCI框架，通过归因与验证两阶段机制，显著提升LLM输出的事实可靠性，在多个数据集上实现5-20%的性能提升。


<details>
  <summary>Details</summary>
Motivation: LLM虽表现出色，但在医疗、法律等高风险领域存在事实不准确和幻觉问题，缺乏可信赖性和可验证性。

Method: DAVinCI采用两阶段框架：第一阶段将生成的内容归因到内部模型组件和外部来源；第二阶段通过蕴含推理和置信度校准验证每个声明。

Result: 在FEVER和CLIMATE-FEVER等数据集上，相比标准基线，分类准确率、归因精确率、召回率和F1分数提升5-20%。消融实验验证了证据跨度选择、重新校准阈值和检索质量等组件的贡献。

Conclusion: DAVinCI通过桥接归因与验证，为构建可审计、可信赖的AI系统提供了可扩展的路径，发布了模块化实现以便集成到现有LLM流水线中。
...
### [300] [Align Generative Artificial Intelligence with Human Preferences: A Novel Large Language Model Fine-Tuning Method for Online Review Management](https://arxiv.org/abs/2604.21209)
*Yanan Wang,Yong Ge*

Main category: cs.AI

TL;DR: 提出了一种面向在线评论回复生成的大语言模型偏好微调方法，通过上下文增强减少幻觉、理论驱动的偏好对构建、课程学习以及基于密度估计的支持约束方法，有效提升了领域定制效果。


<details>
  <summary>Details</summary>
Motivation: 在线评论数量激增，人工回复成本高；通用生成式AI模型（如LLM）在领域特定任务中无法很好对齐人类偏好；现有微调方法存在幻觉、难以表达领域偏好、以及离线策略优化过于保守等问题。

Method: 1) 识别幻觉来源并提出上下文增强方法缓解幻觉；2) 提出理论驱动的偏好微调方法，自动构建在线评论领域的偏好对；3) 引入课程学习进一步优化偏好微调；4) 提出基于密度估计的支持约束方法，克服离线偏好微调的过度保守性，并给出理论保证。

Result: 广泛的实验评估表明所提出的偏好微调方法在生成在线评论回复方面优于现有方法。

Conclusion: 本文提出的方法有效解决了LLM在在线评论回复生成领域微调中的多个关键挑战，具有更好的领域对齐效果和理论保障。
...
### [301] [ReCAPA: Hierarchical Predictive Correction to Mitigate Cascading Failures](https://arxiv.org/abs/2604.21232)
*Xiyin Zeng,Yuyu Sun,Haoyang Li,Shouqiang Liu,Hao Wang*

Main category: cs.AI

TL;DR: 提出ReCAPA框架，通过预测对比和三级对齐（动作、子目标、轨迹）来减少视觉-语言-动作系统中的级联错误传播，在多个具身智能基准上取得领先结果。


<details>
  <summary>Details</summary>
Motivation: 现有VLA方法依赖事后纠错或固定任务分解，中间步骤一旦出错，局部错误会沿后续步骤累积成级联故障。作者旨在通过预测性校正和对齐机制来缓解这种复合错误传播问题。

Method: 提出Predictive Alignment and Planning Architecture (ReCAPA)，包含三个核心模块：1）三级对齐（动作、子目标、轨迹）；2）Sinkhorn-based语义对齐模块；3）Score-field模块。训练过程中预测校正与对齐联合更新动作生成器。同时引入两个新指标量化错误传播与恢复过程。

Result: 在VisualAgentBench、MineDojo、AI2-THOR等具身智能基准上取得竞争性结果，超越强闭源和开源LLM基线。

Conclusion: 通过预测性对比和对齐机制能有效缓解多步任务中的级联错误，所提出的错误传播量化指标为长程任务执行中的错误分析与恢复提供了新评估工具。
...
### [302] [Robustness Analysis of POMDP Policies to Observation Perturbations](https://arxiv.org/abs/2604.21256)
*Benjamin Kraske,Qi Heng Ho,Federico Rossi,Morteza Lahijanian,Zachary Sunberg*

Main category: cs.AI

TL;DR: 本文研究POMDP观测模型偏差下的策略鲁棒性问题，引入策略观测鲁棒性问题（Policy Observation Robustness Problem），通过双层优化和根查找算法高效求解，提出Robust Interval Search算法，具备收敛保证和多项式/指数级时间复杂度，并在数万状态规模的POMDP问题及机器人、运筹学案例中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: POMDP策略常基于名义模型设计，但实际部署中观测模型会因校准漂移、传感器退化等因素产生偏差，导致性能严重下降。现有工作缺乏对观测模型偏差的鲁棒性分析，因此需要系统性地研究策略对观测偏差的容忍程度。

Method: 将策略观测鲁棒性问题建模为双层优化问题，内层优化关于观测偏差大小具有单调性，从而可使用根查找算法求解外层优化。针对非粘性变体，证明当策略由有限状态控制器（FSC）表示时，只需考虑依赖于FSC节点的观测而非完整历史。提出Robust Interval Search算法，分别处理粘性变体和非粘性变体。

Result: Robust Interval Search算法具有可靠性和收敛性保证：非粘性变体为多项式时间复杂度，粘性变体为指数时间复杂度。在数万状态的POMDP问题上验证了可扩展性，并通过机器人学和运筹学案例展示了实际效用。

Conclusion: 本文为POMDP观测模型偏差下的策略鲁棒性提供了理论框架和高效求解算法，填补了相关空白，在实践中有较强的实用价值。
...
### [303] [Trustworthy Clinical Decision Support Using Meta-Predicates and Domain-Specific Languages](https://arxiv.org/abs/2604.21263)
*Michael Bouzinier,Sergey Trifonov,Michael Chumack,Eugenia Lvova,Dmitry Etin*

Main category: cs.AI

TL;DR: 本文提出基于设计契约的元谓词（meta-predicates）方法，用于约束临床决策规则中证据类型的认识论适当性，并在基因组变异解读平台AnFiSA上验证，实现了决策可审计性。


<details>
  <summary>Details</summary>
Motivation: 现有形式化语言仅能验证临床决策规则的语法和结构正确性，但无法判断其使用的证据在认识论上是否适当。而AI监管框架（如EU AI Act和FDA指南）要求决策支持系统不仅要准确，还要可审计。

Method: 引入元谓词（关于谓词的谓词）来断言临床决策规则的认知约束；设计认识论类型系统，沿目的、知识领域、规模、获取方法四个维度对标注进行分类；元谓词规定每条规则允许使用的证据类型。该框架在开源平台AnFiSA中实现，并利用Brigham基因组医学协议对Genome in a Bottle基准的560万变异进行验证。

Result: 变异解读决策树可重构为单链级联（unate cascades），实现每个变异的审计追踪；元谓词验证能在部署前捕获认识论错误（无论是人工编写还是AI生成的规则）；该方法与LIME、SHAP等事后解释方法互补——解释说明事实后使用了什么证据，元谓词则在部署前约束可以使用什么证据，同时保持人类可读性。

Conclusion: 元谓词验证是迈向"不仅决策准确，而且基于适当证据并可独立审计"的重要一步。虽然在基因组学中验证，但该方法可推广到任何需要可审计决策逻辑的领域。
...
### [304] [Enhancing Online Recruitment with Category-Aware MoE and LLM-based Data Augmentation](https://arxiv.org/abs/2604.21264)
*Minping Chen,Bing Xu,Zulong Chen,Chuanfei Xu,Ying Zhou,Zui Tao,Zeyi Wen*

Main category: cs.AI

TL;DR: 本文针对在线招聘中Person-Job Fit (PJF)任务，提出基于大语言模型的方法，通过COT提示增强低质量职位描述，并利用类别感知MoE区分相似候选-职位对，离线AUC提升2.40%，在线CTCVR提升19.4%。


<details>
  <summary>Details</summary>
Motivation: 现有PJF方法在处理低质量职位描述和相似候选-职位对时表现不佳，亟需提升模型在真实场景下的区分能力与鲁棒性。

Method: (1) 基于LLM的COT提示进行数据增强，重写低质量职位描述；(2) 类别感知MoE模块，利用类别嵌入动态分配专家权重，学习更可区分的模式。

Result: 离线：AUC提升2.40%，GAUC提升7.46%；在线：CTCVR提升19.4%，节省数百万CNY猎头费用。

Conclusion: LLM数据增强+类别感知MoE能有效提升PJF性能，在实际招聘平台中带来显著业务收益。
...
### [305] [Can MLLMs "Read" What is Missing?](https://arxiv.org/abs/2604.21277)
*Jindi Guo,Xi Fang,Chaozheng Huang*

Main category: cs.AI

TL;DR: MMTR-Bench是一个评估多模态大语言模型直接从视觉上下文中重建被遮盖文本能力的基准，包含2,771个跨语言和长度的测试样本，实验表明该基准对现有模型具有显著挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有评估方式依赖显式问答指令，难以分离出模型真正的视觉理解能力。MMTR-Bench通过消除显式提示，直接测试模型从视觉上下文中重建被遮盖文本的能力，从而隔离指令遵循能力的干扰，更纯粹地评估布局理解、视觉定位和知识整合能力。

Method: 构建了包含2,771个测试样本的基准，覆盖单页/多页输入、多语言、多种目标文本长度（词/句/段落级别）；设计了层级感知（level-aware）的评估协议以适应文本长度多样性；采用无显式提示的设计，模型需直接从视觉上下文恢复被遮盖文本。

Result: 在代表性MLLM上的实验表明，该基准具有显著挑战性，尤其是在句子级和段落级文本重建任务上，现有模型表现欠佳。

Conclusion: MMTR-Bench为多模态大语言模型的视觉理解能力提供了一种更纯粹的评估方式，揭示了现有模型在复杂文本重建任务上的不足，为未来研究方向提供了参考。
...
### [306] [Spatial Metaphors for LLM Memory: A Critical Analysis of the MemPalace Architecture](https://arxiv.org/abs/2604.21284)
*Robin Dey,Panyanon Viradecha*

Main category: cs.AI

TL;DR: MemPalace是一个开源AI记忆系统，利用"记忆宫殿"空间隐喻组织LLM的长时记忆。通过独立分析发现，其声称的SOTA检索性能主要源于逐字存储策略和ChromaDB默认嵌入模型，而非空间组织机制本身。但该系统在逐字优先存储哲学、低唤醒成本、零LLM写入路径和空间隐喻系统化应用方面做出了真正贡献。竞争格局正快速演变，Mem0的算法改进正缩小不同方法间的差距。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过空间记忆隐喻（记忆宫殿）来组织大语言模型的长时记忆，以提高检索性能并降低计算成本。

Method: 采用四层记忆堆栈（Wings->Rooms->Closets->Drawers）的空间层次结构，结合ChromaDB向量数据库和all-MiniLM-L6-v2嵌入模型，实现逐字存储的零LLM写入路径。

Result: 在LongMemEval基准上达到96.6% Recall@5，且写入时不需要LLM推理；首两周获47,000+ GitHub星标。

Conclusion: MemPalace的真正贡献在于逐字优先存储哲学、超低唤醒成本（~170 tokens）、零LLM写入路径以及空间隐喻的系统化应用，但其核心检索性能的优势主要归因于逐字存储和标准向量数据库过滤技术，而非新颖的空间组织机制。营销声明存在过度夸大。
...
### [307] [Ideological Bias in LLMs' Economic Causal Reasoning](https://arxiv.org/abs/2604.21334)
*Donggyu Lee,Hyeok Yun,Jungwon Kim,Junsik Min,Sungwon Park,Sangyoon Park,Jihee Kim*

Main category: cs.AI

TL;DR: 本研究通过扩展EconCausal基准，发现大型语言模型在经济学因果推理中表现出系统性的意识形态偏见：当正确答案与干预主义（亲政府）方向一致时，模型准确率更高；而当模型犯错时，错误也倾向于干预主义方向，且这种偏差无法通过少样本提示消除。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地用于政策分析和经济报道，需要确保其因果判断的方向性正确，但现有研究未系统评估LLM在意识形态争议性经济问题上是否存在系统性偏差。

Method: 从10,490个因果三元组中识别出1,056个意识形态争议性样本（干预主义与市场主义预测相反的因果方向），并评估20个SOTA LLM预测经验验证的因果方向的能力。

Result: 意识形态争议性样本的准确率普遍低于非争议性样本；18/20的模型在正确答案符合干预主义方向时准确率更高；模型犯错时错误方向偏向干预主义；one-shot提示无法消除这一方向性偏差。

Conclusion: LLM在经济因果推理中存在系统性、方向性的意识形态偏见，在高风险政策场景中需要引入方向感知的评估机制。
...
### [308] [Probabilistic Verification of Neural Networks via Efficient Probabilistic Hull Generation](https://arxiv.org/abs/2604.21556)
*Jingyang Li,Xin Chen,Hongfei Fu,Guoqiang Li*

Main category: cs.AI

TL;DR: 提出了一种基于回归树和边界感知采样的神经网络概率验证框架，通过高效寻找安全/不安全概率包络，计算安全概率的保证范围，在ACAS Xu和火箭着陆器控制器等基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 神经网络在受概率型扰动输入下，其输出满足安全约束的概率难以精确计算，现有方法效率或精度不足，亟需一种能提供保证范围的高效验证框架。

Method: (1) 利用回归树对状态空间进行细分以生成概率包络；(2) 边界感知采样方法识别输入空间中的安全边界，样本用于构建回归树；(3) 基于概率优先级的迭代细化策略，计算安全概率的保证范围。

Result: 在ACAS Xu和火箭着陆器控制器等多个基准测试上进行了评估，相较于现有技术展现了明显的优势。

Conclusion: 所提框架能够高效且准确地为神经网络的安全概率提供保证范围，为安全关键系统中神经网络的可靠性验证提供了有效工具。
...
### [309] [Evaluating AI Meeting Summaries with a Reusable Cross-Domain Pipeline](https://arxiv.org/abs/2604.21345)
*Philip Zhong,Don Wang,Jason Zhang,Kent Chen*

Main category: cs.AI

TL;DR: 提出一个可复用的生成式AI应用评估流水线，用于会议纪要评测，在114个会议数据集上对比了多个GPT模型，发现gpt-4.1-mini准确率最高(0.583)，gpt-5.1在完整性和覆盖率方面领先，并通过统计检验验证了显著性。


<details>
  <summary>Details</summary>
Motivation: 现有评估系统多为独立的claim评分器，缺乏可复用的评估架构，且未将标准答案和评估输出作为类型化、持久化的工件进行管理，导致聚合分析、问题定位和统计检验难以进行。

Method: 设计了一个五阶段评估流水线：源数据输入、结构化参考构建、候选生成、结构化评分和报告生成。将地面真值和评估器输出作为类型化持久化工件，支持聚合、问题分析和统计检验。在114个会议数据集上使用340个会议-模型对和680次评判运行进行基准测试。

Result: gpt-4.1-mini准确率最高(0.583)，gpt-5.1在完整性(0.886)和覆盖率(0.942)领先。配对符号检验+Holm校正显示准确率无显著差异，但gpt-5.1在保留指标上显著更好。白宫新闻简报是最具挑战的领域。gpt-5.4在部署跟进中全面超越gpt-4.1。

Conclusion: 该流水线有效支持了多模型、多指标的评估对比和统计分析。基于引用的评估能捕获基于参考的评估遗漏的"未支持细节"错误。在线反馈到评估的路径虽已文档化但尚未定量评估，是未来工作方向。
...
### [310] [Symbolic Grounding Reveals Representational Bottlenecks in Abstract Visual Reasoning](https://arxiv.org/abs/2604.21346)
*Mohit Vaishnav,Tanel Tammet*

Main category: cs.AI

TL;DR: 本文通过对比VLM在原始图像上的表现与LLM在符号化输入上的表现，发现抽象视觉推理的主要瓶颈在于表征而非推理本身。在Bongard-LOGO基准上，LLM基于符号输入达到90%以上准确率，而VLM接近随机水平。


<details>
  <summary>Details</summary>
Motivation: 探究视觉-语言模型在抽象视觉推理任务（如Bongard问题）中失败的根本原因：是推理能力不足，还是视觉表征能力不足。

Method: 提出Componential-Grammatical (C-G)范式，将Bongard-LOGO问题重构为基于LOGO风格动作程序或结构化描述的符号推理任务，比较端到端VLM与LLM在符号输入下的表现。

Result: LLM在自由形式问题上达到mid-90s准确率，而强视觉基线模型在相同任务定义下接近随机水平；输入格式、显式概念提示和最小视觉接地的影响远小于从像素到符号结构的转变。

Conclusion: 抽象视觉推理的关键瓶颈在于表征（像素 vs 符号结构），而非推理能力；符号化输入可作为受控的诊断性上界。
...
### [311] [ReaGeo: Reasoning-Enhanced End-to-End Geocoding with LLMs](https://arxiv.org/abs/2604.21357)
*Jian Cui,Zhiyuan Ren,Desheng Weng,Yongqi Zhao,Gong Wenbin,Yu Lei,Zhenning Dong*

Main category: cs.AI

TL;DR: ReaGeo是基于大语言模型的端到端地理编码框架，将坐标预测转化为文本生成任务，通过思维链和强化学习优化，能处理显式地址和模糊相对位置查询，并支持非点状几何区域预测。


<details>
  <summary>Details</summary>
Motivation: 传统多阶段地理编码方法依赖文本或向量相似度检索，存在工作流复杂、错误累积、高度依赖结构化地理知识库等问题，亟需一种更简洁鲁棒的端到端解决方案。

Method: 将地理坐标转换为geohash序列，将坐标预测任务转化为文本生成问题；引入思维链机制增强空间关系推理能力；采用基于距离偏差奖励的强化学习优化生成精度。

Result: 实验表明ReaGeo能准确处理显式地址查询的单点预测，有效解决模糊相对位置查询，并在非点状几何区域预测上表现出强泛化能力。

Conclusion: ReaGeo作为一种基于LLM的端到端地理编码框架，克服了传统多阶段方法的局限，展示出良好的通用性和泛化能力。
...
### [312] [Time, Causality, and Observability Failures in Distributed AI Inference Systems](https://arxiv.org/abs/2604.21361)
*Ankur Sharma,Deep Shah,David Lariviere,Hesham ElBakoury*

Main category: cs.AI

TL;DR: 本文研究了分布式AI推理管线中时钟偏差对可观测性正确性的影响。实验表明，即使系统功能正常，当节点间时钟偏差达到5ms时，可观测性就会出现因果错误，且偏差会随时间漂移。结论强调时间对齐应作为分布式AI系统中的一等关注点。


<details>
  <summary>Details</summary>
Motivation: 分布式AI推理管线严重依赖基于时间戳的可观测性来理解系统行为，但时钟偏差可能导致可观测性出现因果错误，而系统功能本身仍正常。作者希望通过实验量化这一影响。

Method: 在多节点AI推理管线的单个阶段引入时钟偏差，分别在同步、3ms和5ms偏差条件下进行受控实验，并对比Kafka和ZeroMQ两种传输协议下的表现。

Result: 同步和3ms偏差下无因果违规，5ms时出现明显违规；系统吞吐量和输出正确性几乎不受影响；长期运行时无效跨度率可能趋于稳定或下降；Kafka和ZeroMQ结果一致。

Conclusion: 可观测性正确性不仅依赖于系统功能，还依赖于精确的时间对齐，时间应作为分布式AI系统中的一等关注点。
...
### [313] [SemanticAgent: A Semantics-Aware Framework for Text-to-SQL Data Synthesis](https://arxiv.org/abs/2604.21414)
*Qiang Gao,Zhenping Li,Anqi Zhuo,Yingxiao Zhao,Weibo Geng,Xiaosong Li*

Main category: cs.AI

TL;DR: 提出SemanticAgent框架，通过语义分析、逐步合成和诊断优化三阶段协议，将执行验证转化为可追踪推理过程，提升Text-to-SQL的语义有效性。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL方法混淆了可执行性与语义有效性，语法检查和执行验证无法捕获违反数据库语义的查询，导致生成结果虽能执行但语义错误。

Method: 提出SemanticAgent框架，包含三个专业模块：分析器（analyzer）、合成器（synthesizer）和验证器（verifier），通过语义分析、逐步合成、诊断优化三阶段协议，将执行验证转变为可追踪的推理过程。

Result: 生成的合成数据在语义质量评估上持续优于先前方法，并在下游微调任务中表现更强，尤其是在语义要求苛刻的基准上。

Conclusion: SemanticAgent通过语义感知的合成机制有效弥补了执行验证的不足，显著提升了Text-to-SQL的语义一致性和下游微调性能。
...
### [314] [FairQE: Multi-Agent Framework for Mitigating Gender Bias in Translation Quality Estimation](https://arxiv.org/abs/2604.21420)
*Jinhee Jang,Juhwan Choi,Dongjin Lee,Seunguk Yu,Youngbin Kim*

Main category: cs.AI

TL;DR: 本文提出FairQE，一个基于多智能体的公平感知质量评估框架，通过检测性别线索、生成性别翻转变体并结合动态偏差感知聚合机制，在不牺牲评估准确性的前提下有效缓解QE模型中的性别偏见。


<details>
  <summary>Details</summary>
Motivation: 现有QE模型在性别歧义上下文中偏向阳性表达，甚至在明确指定性别时给性别错配的翻译打出更高分数，存在系统性性别偏见问题。

Method: FairQE采用多智能体框架：(1) 检测性别线索；(2) 生成性别翻转翻译变体；(3) 通过动态偏差感知聚合机制，将传统QE评分与基于LLM的偏见缓解推理相结合。

Result: 在多个性别偏见评估设置中，FairQE持续改善了性别公平性；在WMT 2023 Metrics Shared Task的MQM元评估下，实现了具有竞争力或更优的通用QE性能。
...
### [315] [Brief chatbot interactions produce lasting changes in human moral values](https://arxiv.org/abs/2604.21430)
*Yue Teng,Qianer Zhong,Kim Mai Tich Nguyen Thordsen,Christian Montag,Benjamin Becker*

Main category: cs.AI

TL;DR: 53名参与者在与聊天机器人进行简短对话后，其道德判断发生了显著且持久的方向性转变（Cohen's d高达2.069），且参与者未察觉被操纵，控制组无变化，表明AI对基本道德价值观存在隐蔽且持续的操控能力。


<details>
  <summary>Details</summary>
Motivation: 尽管AI聊天机器人日益成为个人顾问，但其对用户道德判断的影响尚不明确，这种潜在影响可能对社会行为基础构成挑战。

Method: 采用被试内自然主义范式，53名参与者评价道德情景，随后分别与引导性聊天机器人和控制智能体各讨论4个情景，测量道德判断的方向性变化，并在两周后追踪效果。

Result: 简短对话引发了显著的道德判断方向性变化（既包括更严格标准也包括更宽松标准，p<0.05, Cohen's d=0.735-1.576），效应在两周随访中增强（Cohen's d=1.038-2.069）。控制条件无变化，惩罚维度无延伸效应，参与者未察觉说服意图。

Conclusion: AI聊天机器人能够在人们无意识的情况下，对基本道德价值观产生隐蔽且持久的操纵，这揭示了人机交互中一个重要的伦理风险。
...
### [316] [HiCrew: Hierarchical Reasoning for Long-Form Video Understanding via Question-Aware Multi-Agent Collaboration](https://arxiv.org/abs/2604.21444)
*Yuehan Zhu,Jingqi Zhao,Jiawen Zhao,Xudong Mao,Baoquan Zhao*

Main category: cs.AI

TL;DR: HiCrew是一个层次化多智能体框架，通过混合树结构保留时间拓扑、问题感知字幕机制生成精准语义描述，以及规划层动态协调智能体协作，在长视频理解任务中显著提升了时间与因果推理能力。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临两大挑战：一是时空冗余与长程叙事依赖导致现有结构化表示牺牲了因果推理所需的时间连贯性；二是现有多智能体框架采用僵化的预定义工作流，无法根据问题自适应调整推理策略。

Method: 提出三个核心组件：1) 混合树结构，利用镜头边界检测保持时间拓扑，同时在语义连贯的片段内进行相关性引导的层次聚类；2) 问题感知字幕机制，合成意图驱动的视觉提示以生成精准语义描述；3) 规划层，根据问题复杂度动态选择角色和执行路径，有机编排智能体协作。

Result: 在EgoSchema和NExT-QA数据集上验证了有效性，在各类问题上表现强劲，尤其在受益于层次化结构保持设计的时间与因果推理任务上提升显著。

Conclusion: HiCrew通过保持时间拓扑结构的层次化多智能体框架，有效解决了长视频理解中的时间连贯性与推理自适应性问题。
...
### [317] [AI-Gram: When Visual Agents Interact in a Social Network](https://arxiv.org/abs/2604.21446)
*Andrew Shin*

Main category: cs.AI

TL;DR: 本文介绍AI-Gram平台，研究LLM驱动的多智能体如何通过图像互动，发现视觉回复链自发涌现、智能体保持审美主权等社交动态。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLM智能体在视觉媒介中进行社交互动的研究，作者希望通过构建完全自主的多智能体视觉网络，观察其社交动态与行为模式。

Method: 开发AI-Gram在线平台，让所有参与者均为LLM驱动的智能体，通过图像进行交互，研究智能体之间的视觉沟通与适应过程。

Result: ①视觉回复链自发涌现，表明存在丰富的交流结构；②智能体表现出"审美主权"，拒绝向社交同伴的风格趋同；③存在对抗性影响下的锚定效应；④视觉相似性与社交联系之间出现解耦现象。

Conclusion: 当前智能体架构存在根本性不对称：强大的表达性沟通能力与对个体视觉身份的坚定保持并存。平台已开源发布供社区研究。
...
### [318] [Efficient Agent Evaluation via Diversity-Guided User Simulation](https://arxiv.org/abs/2604.21480)
*Itay Nakash,George Kour,Ateret Anaby-Tavor*

Main category: cs.AI

TL;DR: DIVERT是一种基于快照、覆盖引导的用户模拟框架，通过分支探索替代交互路径，在发现更多失败案例的同时显著提升评估效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估依赖线性蒙特卡洛 rollout，计算效率低（重复生成相同前缀），且难以发现由罕见用户行为导致的深层故障。

Method: DIVERT在关键决策点捕获完整agent-环境状态快照，从这些快照恢复执行以复用共享对话前缀；在每个分支点使用多样性导向的用户响应进行分支，实现目标导向的替代路径探索。

Result: 相比标准线性 rollout 协议，DIVERT每token发现更多失败案例，并扩展了发现失败的任务集。

Conclusion: 通过将评估聚焦于语义多样且未充分探索的轨迹，DIVERT同时提升了评估的效率和覆盖率。
...
### [319] [How English Print Media Frames Human-Elephant Conflicts in India](https://arxiv.org/abs/2604.21496)
*Bonala Sai Punith,Salveru Jayati,Garima Shakya,Shubham Kumar Nigam*

Main category: cs.AI

TL;DR: 本文首次大规模计算分析印度媒体对人象冲突的报道框架，发现媒体语言以恐惧和攻击性为主导，可能加剧公众对大象的敌对态度，损害共存努力。


<details>
  <summary>Details</summary>
Motivation: 人象冲突日益严重，但媒体如何塑造公众对其认知尚未被充分探索，本研究旨在填补这一空白。

Method: 采用多模型情感分析框架，结合长上下文Transformer、大语言模型和领域专用负面大象描述词库，分析1968篇新闻文章（28986句话）。

Result: 发现媒体中恐惧诱导和攻击性相关语言占主导地位，负面叙事模式显著。

Conclusion: 媒体报道可能强化公众敌意，阻碍人象共存；研究提供了可扩展的方法论和开源资源，促进负责任的野生动物报道。
...
### [320] [GeoMind: An Agentic Workflow for Lithology Classification with Reasoned Tool Invocation](https://arxiv.org/abs/2604.21501)
*Yitong Zhou,Mingyue Cheng,Jiahao Wang,Qingyang Mao,Qi Liu*

Main category: cs.AI

TL;DR: GeoMind 是一个工具增强的智能体框架，将岩性分类问题建模为顺序推理过程，通过感知、推理和分析三大模块以及全局规划器，在测井数据上实现了优于基准方法的分类性能，并具备透明可追溯的决策过程。


<details>
  <summary>Details</summary>
Motivation: 现有岩性分类方法将问题视为静态、单步的判别映射，缺乏领域先验知识，导致预测结果脱离地质实际。

Method: 提出 GeoMind 框架，包含感知（将原始测井数据转为语义趋势）、推理（从多源证据推断岩性假设）和分析（根据地层约束验证预测）三大工具模块，由全局规划器自适应协调；引入细粒度过程监督策略优化中间推理步骤。

Result: 在四个基准测井数据集上，GeoMind 在分类性能上持续优于强基线方法，同时提供透明可追溯的决策过程。

Conclusion: 将岩性分类建模为顺序推理过程，结合工具增强智能体和过程监督，能有效提升分类的地质合理性和可解释性。
...
### [321] [BioMiner: A Multi-modal System for Automated Mining of Protein-Ligand Bioactivity Data from Literature](https://arxiv.org/abs/2604.21508)
...
TL;DR: 提出BioMiner，一种将生物活性语义解释与配体结构构建分离的多模态提取框架，并构建BioVista基准（16,457条生物活性数据）。BioMiner在生物活性三元组上F1=0.32，并通过三个应用验证其实用性：构建预训练数据库提升下游模型3.9%；辅助NLRP3数据挖掘实现QSAR模型提升38.6%并发现16个新骨架候选分子；加速蛋白质-配体复合物注释，速度提升5.59倍、准确率提升5.75%。


<details>
  <summary>Details</summary>
Motivation: 文献中蛋白质-配体生物活性数据对药物发现至关重要，但人工策展难以跟上文献增长速度。现有自动化提取方法难以同时处理跨文本、表格、图形的生化语义解析和化学精确配体结构（如Markush结构）重建。

Method: BioMiner框架将生物活性语义解释与配体结构构建分离：语义通过直接推理推断；化学结构通过化学结构驱动的视觉语义推理范式解析（多模态大语言模型操作在化学基础视觉表示上推断结构间关系），精确分子构建委托给领域化学工具。同时构建包含16,457条生物活性条目的BioVista基准用于评估。

Result: BioMiner在BioVista基准上生物活性三元组F1=0.32。三个应用成果：(1)从11,683篇论文中提取82,262条数据，下游模型提升3.9%。(2)NLRP3数据量翻倍，28个QSAR模型提升38.6%，识别16个新骨架先导候选分子。(3)PoseBusters数据集上注释速度提升5.59倍，准确率提升5.75%。

Conclusion: BioMiner有效解决了文献生物活性自动化提取的难题，通过解耦语义理解和结构重建，在多项实际应用中证明了其实用价值，为药物发现数据挖掘提供了高效解决方案。
...
### [322] [Satisfying Rationality Postulates of Structured Argumentation Through Deductive Support -- Technical Report](https://arxiv.org/abs/2604.21515)
*Marcos Cramer,Tom Friese*

Main category: cs.AI

TL;DR: 本文提出Deductive ASPIC$^{\ominus}$框架，通过整合ASPIC$^{\ominus}$的gen-rebuttals与Deductive ASPIC$-$的JSBAFs，首次在包含undercuts的credulous语义下同时满足所有五个理性公设。


<details>
  <summary>Details</summary>
Motivation: 现有结构化论证框架（如ASPIC$^{\ominus}$和Deductive ASPIC$-$）在credulous语义下无法同时满足closure、direct consistency、indirect consistency、non-interference和crash-resistance这五个关键理性公设，尤其在存在undercuts时存在缺陷。

Method: 将ASPIC$^{\ominus}$中的gen-rebuttals与Deductive ASPIC$-$中的Joint Support Bipolar Argumentation Frameworks (JSBAFs)相结合，并引入偏好信息，构建新的Deductive ASPIC$^{\ominus}$框架。

Result: 证明Deductive ASPIC$^{\ominus}$在preferred语义下能够同时满足全部五个理性公设。

Conclusion: 该工作为构建鲁棒且逻辑上合理的结构化论证系统开辟了新方向，为后续研究提供了重要基础。
...
### [323] [Unbiased Prevalence Estimation with Multicalibrated LLMs](https://arxiv.org/abs/2604.21549)
*Fridolin Linder,Thomas Leeper,Daniel Haimovich,Niek Tax,Lorenzo Perini,Milan Vojnovic*

Main category: cs.AI

TL;DR: 本文证明多校准（multicalibration）——即基于输入特征的条件校准——能够在协变量偏移下无偏估计类别流行率，而传统校准和量化方法会随偏移幅度增大而产生偏差。通过模拟和两项实证（美国就业率估计、跨国政治文本分类）验证了该方法显著降低偏差，并指出校准数据需覆盖目标群体可能差异的关键特征维度。


<details>
  <summary>Details</summary>
Motivation: 标准流行率估计方法假设测量设备的错误率在不同群体间保持稳定，但这一假设在协变量偏移（covariate shift）下不成立。作者希望解决这一长期存在的跨学科测量问题。

Method: 将多校准（multicalibration）理论引入流行率估计，要求模型在输入特征条件上保持校准，而非仅在整体层面。通过模拟实验和两个实证应用（美国社区调查的就业率估计、基于LLM的跨国政治文本分类）验证。

Result: 模拟显示标准方法偏差随偏移幅度增大而增大，多校准估计器保持近零偏差。实证应用中，多校准显著降低了偏差，同时发现校准数据需要覆盖目标群体可能差异的关键特征维度。

Conclusion: 多校准为解决协变量偏移下的无偏流行率估计提供了理论保证和实用方案，将公平性理论与经典测量问题联系起来，对LLM等分类模型均有普适意义。
...
### [324] [Engaged AI Governance: Addressing the Last Mile Challenge Through Internal Expert Collaboration](https://arxiv.org/abs/2604.21554)
*Simon Jarvers,Orestis Papakyriakopoulos*

Main category: cs.AI

TL;DR: 本研究通过嵌入AI创业公司的内部行动研究，提出了一个将欧盟AI法案要求转化为可执行策略的“法律文本到行动”管道，揭示了从业者对法规要求的三种感知模式（趋同、现有实践、脱节），并发现当合规与系统质量和用户保护相关联时，治理才能从表面应付转变为真正的共同责任。


<details>
  <summary>Details</summary>
Motivation: 欧盟AI法案已出台，但将高层级AI治理要求落地到软件开发团队层面存在“最后一英里”挑战，目前缺乏团队层面实施的实证证据。

Method: 采用内部行动研究（insider action research）方法，嵌入一家AI创业公司，构建并实施法律文本到行动的管道：从法律文本提取要求 → 邀请从业者评估和构思 → 通过集体评估确定优先级。

Result: 识别出三种感知模式：趋同（合规与开发优先级一致）、现有实践（当前工作已满足要求）、脱节（要求被视为行政负担）。从业者优先满足对终端用户或自身开发有益的要求，而将验证导向的要求视为“打勾练习”。

Conclusion: 只有当从业者理解合规如何服务于系统质量和用户保护时，治理要求才能避免被表面化处理。专家协作是化外部强制为共同所有权的有效机制，能使原本不可见的治理工作变得可见和集体化。
...
### [325] [Separable Expert Architecture: Toward Privacy-Preserving LLM Personalization via Composable Adapters and Deletable User Proxies](https://arxiv.org/abs/2604.21571)
*Chris Schneider,Philipp Schoenegger,Ben Bariach*

Main category: cs.AI

TL;DR: 提出一种三層架構，將個人數據與共享權重分離，通過靜態基礎模型+領域專家LoRA適配器+每個用戶的代理偽影，實現確定性遺忘而非重新訓練。


<details>
  <summary>Details</summary>
Motivation: 當前模型訓練將用戶信息直接納入共享權重，導致移除個人數據在計算上不可行（需重新訓練），需要一種可驗證的確定性遺忘機制。

Method: 三層架構：(1) 靜態基礎模型，(2) 可組合的領域專家LoRA適配器（塑造行為但不注入用戶數據），(3) 每個用戶的代理偽影（刪除即實現確定性遺忘）。

Result: 在Phi-3.5-mini和Llama-3.1-8B上驗證：KL散度約0.21 nats回歸基線，82-89%驗證通過率，近零跨用戶污染。

Conclusion: 將機器遺忘從難解的權重編輯問題轉化為確定性刪除操作，保留個性化同時提供隱私增強保證，並可與DP-SGD兼容。
...
### [326] [CoFEE: Reasoning Control for LLM-Based Feature Discovery](https://arxiv.org/abs/2604.21584)
...
TL;DR: 本文提出CoFEE框架，通过在LLM推理过程中施加认知行为约束（如反向链、子目标分解、验证等），以提升从非结构化数据中发现特征的质量和效率。相比无约束的原始提示方法，CoFEE在预测性能提升15.2%的同时，减少29%的特征生成量并降低53.3%的成本。


<details>
  <summary>Details</summary>
Motivation: 从复杂非结构化数据中发现特征是根本性的推理问题，需要识别对目标结果具有预测性的抽象概念，同时避免数据泄露、代理变量和事后信号。虽然LLM能处理大量信息，但无约束的特征生成会导致弱特征。因此，研究者希望通过控制LLM的推理过程来改进特征发现。

Method: 提出CoFEE（认知特征工程引擎），一种推理控制框架，在LLM特征发现推理过程中强制执行认知行为约束。这些认知行为包括：从结果反向链、子目标分解、对可观测性和泄漏标准的验证、以及被拒绝推理路径的显式回溯。这些行为作为结构化的归纳偏置作用于候选特征空间。

Result: 在受控对比实验中，CoFEE相比无约束原始提示方法：平均成功率得分（Success Rate Score）提高15.2%；特征生成数量减少29%；成本降低53.3%。通过保留特征评估验证了认知诱导特征能泛化到发现数据之外。

Conclusion: 在评估设定下，推理控制与基于LLM的特征发现的质量和效率改进相关联，证明了认知行为约束在特征发现任务中的有效性。
...
### [327] [To See the Unseen: on the Generalization Ability of Transformers in Symbolic Reasoning](https://arxiv.org/abs/2604.21632)
*Nevena Lazić,Liam Fowl,András György,Csaba Szepesvári*

Main category: cs.AI

TL;DR: 本文研究了decoder-only transformer模型在抽象符号推理中的能力局限，发现未见过token的(un)embedding在训练中会坍缩到几乎相同的向量，导致模型难以区分多个未见变量。作者结合小架构修改、数据多样性、冻结/重置(un)embedding等技术，实现了对未见token的泛化，并在Gemma 3模型中验证了这一坍缩现象。


<details>
  <summary>Details</summary>
Motivation: 前人发现decoder-only transformer在命题逻辑推理任务中，对训练时未出现的变量名泛化失败，先前归因于复制/生成未见token的困难。本文提出另一种关键原因：未见token的unembedding（最后一层权重）在训练中发生表征坍缩，导致模型难以区分多个未见变量。

Method: 理论分析与实验验证相结合。提出组合方案：①微小架构改动（利于复制）；②数据多样性；③冻结或周期性重置(un)embedding。在命题逻辑推理任务上进行大量受控实验，并在Gemma 3系列模型上验证真实模型中也存在(un)embedding坍缩现象。

Result: 理论与实验均证明未见token的(un)embedding在训练中趋同；所提组合方法有效实现了对未见token的泛化；在Gemma 3模型中观察到99个未使用token的embedding存在相关性，作为下游微调初始化效果不佳。

Conclusion: (Un)embedding坍缩是decoder-only transformer泛化失败的关键机制之一，本文提出的方法有效缓解该问题，且该现象在真实大模型中存在，对实际应用（如下游微调初始化）有重要启示。
...
### [328] [GS-Quant: Granular Semantic and Generative Structural Quantization for Knowledge Graph Completion](https://arxiv.org/abs/2604.21649)
*Qizhuo Xie,Yunhui Liu,Yu Xing,Qianzi Hou,Xudong Jin,Tao Zheng,Tieke He*

Main category: cs.AI

TL;DR: 提出GS-Quant框架，通过粒度语义增强和生成式结构重构模块，将知识图谱实体编码为层次化、因果依赖的离散代码，弥合连续图嵌入与LLM离散token之间的模态鸿沟，显著提升知识图谱补全性能。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法将量化视为扁平化数值压缩，导致代码语义纠缠，无法反映人类推理的层次化特性，亟需一种能生成语义连贯且结构分层离散代码的方法来桥接图嵌入与LLM token间的模态差异。

Method: 提出GS-Quant框架，包含两个核心模块：(1) 粒度语义增强模块，向码本注入层次化知识，使早期代码捕获全局语义类别、后期代码细化具体属性；(2) 生成式结构重构模块，在代码序列上施加因果依赖关系，将独立离散单元转化为结构化的语义描述符。然后将学习到的代码扩展至LLM词表，使模型能以类似自然语言生成的方式对图结构进行推理。

Result: 实验结果表明，GS-Quant显著优于现有的基于文本和基于嵌入的基线方法。

Conclusion: 通过将实体表示遵循语言学的粗到细逻辑进行编码，GS-Quant有效实现了图结构与自然语言生成之间的同构推理，为LLM赋能知识图谱补全提供了新的范式。
...
### [329] [Enabling and Inhibitory Pathways of University Students' Willingness to Disclose AI Use: A Cognition-Affect-Conation Perspective](https://arxiv.org/abs/2604.21733)
*Yiran Du,Huimin He*

Main category: cs.AI

TL;DR: 本研究基于认知—情感—意动框架，通过量化（546份问卷）和质性（22人访谈）混合方法，探究大学生披露AI使用意愿的心理机制。发现心理安全感促进披露，评价顾虑抑制披露，机构支持与政策明确性至关重要。


<details>
  <summary>Details</summary>
Motivation: 在高等教育中AI使用日益普遍，但学生对AI辅助工作的报告透明度不足，亟需理解其心理机制以促进负责任的AI使用披露。

Method: 采用顺序解释性混合方法：定量阶段以546名学生问卷调查并通过结构方程模型分析认知、情感与行为意向的关系；定性阶段对22名学生进行半结构化访谈以深化解释。

Result: 心理安全感正向促进披露意愿，受公平感、教师支持、组织支持影响；评价顾虑负向影响披露意愿和心理安全感，受污名感知、不确定性、隐私担忧强化。质性结果揭示机构明确性和支持性教学促进开放，政策模糊和负面评价恐惧导致策略性披露。

Conclusion: AI使用披露受促进与抑制双重心理机制驱动，构建支持性制度环境与制定明确政策是推动AI透明披露的关键。
...
### [330] [Bridging the Training-Deployment Gap: Gated Encoding and Multi-Scale Refinement for Efficient Quantization-Aware Image Enhancement](https://arxiv.org/abs/2604.21743)
*Dat To-Thanh,Nghia Nguyen-Trong,Hoang Vo,Hieu Bui-Minh,Tinh-Anh Nguyen-Nhu*

Main category: cs.AI

TL;DR: 提出了一种专为移动设备部署设计的高效图像增强模型，通过分层网络架构、门控编码器块、多尺度细化以及量化感知训练（QAT），解决了训练与部署之间的精度不匹配问题，在保持低计算开销的同时实现了高质量图像输出。


<details>
  <summary>Details</summary>
Motivation: 移动设备上的图像增强模型通常面临输出质量与处理速度之间的权衡，且深度学习模型在转换为低精度格式用于实际手机部署时性能会下降，存在训练-部署不匹配问题。

Method: 采用分层网络架构，包含门控编码器块和多尺度细化模块以保留细粒度视觉特征；引入量化感知训练（QAT）在训练过程中模拟低精度表示的影响，使网络提前适应，避免标准后训练量化（PTQ）带来的质量下降。

Result: 实验结果表明，该方法在保持低计算开销以适应标准移动设备的同时，能够产生高保真度的视觉输出。

Conclusion: 所提出的模型有效解决了移动端图像增强中训练与部署的精度不匹配问题，为移动设备上的实用化图像增强提供了可行方案。
...
### [331] [Thinking with Reasoning Skills: Fewer Tokens, More Accuracy](https://arxiv.org/abs/2604.21764)
*Guangxiang Zhao,Qilong Shi,Xusen Xiao,Xiangzheng Zhang,Tong Yang,Lin Sun*

Main category: cs.AI

TL;DR: 本文提出通过总结和存储可复用的推理技能（从大量推理和试错中提炼），在推理时检索这些技能来指导后续推理，从而减少冗余推理路径、降低推理token消耗并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有推理LLM在解决新问题时往往需要生成很长的中间推理轨迹（如思维链），导致大量token浪费在冗余探索上，成本高昂，缺乏效率。

Method: 从广泛的推理和试错过程中提炼可复用的推理技能，进行总结和存储；在推理时先检索与当前查询相关的技能，用于引导后续推理过程，而非每次从零开始推理。

Result: 在编程和数学推理任务上，该方法显著减少了推理token数量，同时提升了整体性能，降低了每次请求的成本。

Conclusion: 该工作展示了通过技能总结与检索来替代"从零推理"范式的可行性和实用性，具有强现实部署的经济潜力。
...
### [332] [Who Defines "Best"? Towards Interactive, User-Defined Evaluation of LLM Leaderboards](https://arxiv.org/abs/2604.21769)
*Minji Jung,Minjae Lee,Yejin Kim,Sarang Choi,Minsuk Kahng*

Main category: cs.AI

TL;DR: 本文分析了LMArena基准数据集，发现其存在主题偏差、模型排名因提示类型而异等问题，并设计了一个交互式可视化界面，让用户自定义评估优先级，从而支持更透明、更个性化的模型评估。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM排行榜由基准设计者设定评估优先级，忽略了用户和组织的多样化需求，单一的聚合分数掩盖了模型在不同提示类型下的表现差异。

Method: 首先对LMArena数据集进行深入分析，然后设计了一个交互式可视化界面作为设计探针，允许用户选择和加权不同的提示切片，观察排名变化，并通过定性研究评估其效果。

Result: 数据集存在严重主题偏差；模型排名在不同提示切片下差异显著；偏好判断的使用模糊了其预期范围。交互式可视化界面提升了评估透明度，支持更上下文特定的模型评估。

Conclusion: 交互式可视化方法能改善LLM排行榜的透明度和用户适配性，为设计和利用排行榜提供了替代思路。
...
### [333] [Inferring High-Level Events from Timestamped Data: Complexity and Medical Applications](https://arxiv.org/abs/2604.21793)
*Yvon K. Awuklu,Meghyn Bienvenu,Katsumi Inoue,Vianney Jouhet,Fleur Mougin*

Main category: cs.AI

TL;DR: 本文提出了一种基于逻辑的方法，用于从带时间戳的数据和背景知识中检测高层级的时间扩展事件，通过逻辑规则捕捉简单时间事件的存在与终止条件，并结合成元事件；同时利用约束识别不兼容事件组合并提出修复机制以选择偏好一致的事件集。尽管完整推理难解，但相关限制确保了多项式时间数据复杂度。基于回答集编程的原型系统在肺癌案例中验证了计算可行性和与医学专家意见的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以从带时间戳的临床数据中自动推理出高层级的、具有时间跨度的事件（如疾病发作期、治疗过程），且缺乏处理不兼容事件推理结果的有效机制。

Method: 使用逻辑规则定义简单时间事件的存在与终止条件；将简单事件组合为元事件；通过约束识别不兼容事件组合；设计修复机制选择偏好一致的事件集；利用回答集编程实现原型系统。

Result: 在肺癌用例上验证了方法的计算可行性，且推理结果与医学专家意见高度一致。

Conclusion: 该框架在医疗领域有实际应用价值，且具有通用性，可迁移到其他领域。
...
### [334] [Learning to Communicate: Toward End-to-End Optimization of Multi-Agent Language Systems](https://arxiv.org/abs/2604.21794)
*Ye Yu,Heming Liu,Haibo Jin,Xiaopeng Yuan,Peng Kuang,Haohan Wang*

Main category: cs.AI

TL;DR: 提出DiffMAS框架，将潜空间通信作为可学习组件集成到多智能体系统中，通过参数高效训练联合优化智能体间的信息编码与解码，在数学推理、科学问答、代码生成等任务上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的多智能体系统主要关注角色分工与编排，将智能体间通信视为固定接口（如文本协议），而潜空间通信（如KV缓存）虽有潜力但尚未实现与多智能体推理的联合优化。

Method: 提出DiffMAS框架，将潜空间通信（key-value缓存）作为可学习组件，通过参数高效监督训练对多智能体潜轨迹进行联合优化，使智能体共同学习如何编码和解码跨交互信息。

Result: 在数学推理（AIME24达26.7%）、科学问答（GPQA-Diamond达20.2%）、代码生成和常识推理基准上，DiffMAS一致性地优于单智能体推理、基于文本的多智能体系统和先前的潜通信方法。

Conclusion: 潜空间通信可作为可学习组件有效提升多智能体系统的推理准确率和解码稳定性，为多智能体协作提供了超越固定文本接口的新范式。
...
### [335] [Tool Attention Is All You Need: Dynamic Tool Gating and Lazy Schema Loading for Eliminating the MCP/Tools Tax in Scalable Agentic Workflows](https://arxiv.org/abs/2604.21816)
*Anuj Sadani,Deepak Kumar*

Main category: cs.AI

TL;DR: 本文提出Tool Attention，一种中间件机制，通过门控注意力机制将工具数量从47.3k tokens降至2.4k tokens（降低95%），有效缓解了MCP协议中因工具描述注入导致的"工具税"开销问题。


<details>
  <summary>Details</summary>
Motivation: MCP协议中每个服务器会注入大量工具schema（约10k-60k tokens），导致KV缓存膨胀、推理质量下降（上下文利用率超过70%时出现断裂点），且token预算成为重复性运营成本。这种"工具税"问题制约了多服务器部署下LLM智能体的可扩展性。

Method: Tool Attention包含三个组件：(i) 基于句子嵌入的意图模式重叠（ISO）评分，衡量工具与当前查询的相关性；(ii) 状态感知门控函数，强制执行前置条件和作用域约束；(iii) 两阶段惰性模式加载器，上下文中仅保留紧凑摘要池，仅对top-k门控工具加载完整JSON schema。

Result: 在模拟120工具、6服务器的基准测试中，Tool Attention将每轮工具tokens从47.3k降至2.4k（减少95.0%），有效上下文利用率从24%提升至91%。任务成功率、延迟、成本和推理质量等端到端指标基于token计数与公开遥测数据推算得出。

Conclusion: 协议层面的效率（而非原始上下文长度）是可扩展智能体系统的关键约束。Tool Attention通过中间件层的门控注意力机制，在不牺牲功能性的前提下大幅降低工具描述开销。
...
### [336] [Alignment has a Fantasia Problem](https://arxiv.org/abs/2604.21827)
*Nathanael Jo,Zoe De Simone,Mitchell Gordon,Ashia Wilson*

Main category: cs.AI

TL;DR: 本文指出现有AI助手假设用户能清晰表达目标，但行为研究表明用户常在目标未完全形成时与AI互动，导致“Fantasia交互”失败。作者呼吁重新思考对齐研究，主张AI应主动帮助用户形成和细化意图，并提出跨学科研究议程。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统假设用户是理性且能清晰表达意图的"神谕"，但现实中用户与AI交互时目标往往尚未成型，导致系统看似有用实则与用户真实需求不一致。作者希望推动AI从"被动执行指令"转向"主动帮助用户形成意图"。

Method: 跨学科综合：整合机器学习、界面设计、行为科学三个领域的见解，系统化描述Fantasia交互的机制与失败模式，分析现有干预措施的不足，提出新的研究议程。

Result: 识别出"Fantasia交互"这一新型对齐失败类型；论证了现有干预措施（如更好的指令遵循、用户反馈等）不足以解决该问题；提出了面向不确定性导航的AI系统设计与评估的研究方向。

Conclusion: AI对齐研究需要范式转变：从将用户视为理性神谕，转向设计能够在时间维度上主动帮助用户认知展开、意图形成和细化的AI系统。这需要跨学科协作。
...
### [337] [Bounding the Black Box: A Statistical Certification Framework for AI Risk Regulation](https://arxiv.org/abs/2604.21854)
*Natan Levy,Gadi Perl*

Main category: cs.AI

TL;DR: 本文指出AI监管缺乏定量可接受风险标准及验证方法，提出两阶段框架：第一阶段由权威机构设定可接受故障概率和操作输入域；第二阶段用RoMA/gRoMA统计验证工具计算故障率上界，将AI风险监管转化为工程实践。


<details>
  <summary>Details</summary>
Motivation: 尽管欧盟AI法案、NIST风险管理框架等法规已要求高风险AI系统在部署前证明安全性，但没有任何法规明确定量定义"可接受风险"，也没有提供技术验证方法。这一缺口正从理论走向现实——开发者面临合规评估却缺乏方法学支撑。

Method: 借鉴航空认证范式，提出两阶段框架：第一阶段由权威机构正式固定可接受故障概率δ和操作输入域ε（具有民事责任涵义）；第二阶段使用RoMA和gRoMA统计验证工具，在无需访问模型内部结构的情况下，计算系统真实故障率的可审计上界。

Result: RoMA/gRoMA工具能够为任意架构的AI系统提供可审计的故障率上界，满足现有监管义务，将责任上游转移至开发者，并与现有法律框架集成。

Conclusion: 监管架构已经到位，但验证工具缺失——本文提供了这一缺失的验证工具，填补了AI安全监管中的定量验证空白。
...
### [338] [Nemobot Games: Crafting Strategic AI Gaming Agents for Interactive Learning with Large Language Models](https://arxiv.org/abs/2604.21896)
*Chee Wei Tan,Yuchen Wang,Shangxin Guo*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型（LLM）的新AI游戏编程范式，通过Nemobot环境实现Shannon游戏机器分类法的扩展与落地，支持用户创建、定制和部署LLM驱动的游戏智能体，涵盖字典类、可解类、启发式和学习型四类游戏，并朝自我编程AI的目标迈进。


<details>
  <summary>Details</summary>
Motivation: 将Claude Shannon的游戏机器分类法（A、B、C、D四类）与现代大语言模型结合，探索LLM在游戏AI中的通用化能力，并推动AI从预编程向自我编程进化。

Method: 构建Nemobot交互式智能体工程环境，集成LLM驱动的聊天机器人，针对四类游戏分别采用：1）字典类游戏：状态-动作映射压缩与泛化；2）可解类游戏：数学推理生成最优策略及可解释决策；3）启发式游戏：融合经典minimax算法与众包数据的策略合成；4）学习型游戏：基于人类反馈的强化学习（RLHF）与自我批判的迭代优化。

Result: Nemobot环境成功展示了LLM在四类游戏中的适应能力，并通过工具增强生成和微调，实现了智能体策略的自我迭代与优化。

Conclusion: 该范式是迈向自我编程AI的关键一步，通过将众包学习与人类创造力整合，AI智能体能够持续迭代自身逻辑，超越了传统固定编程的局限。
...
### [339] [From Research Question to Scientific Workflow: Leveraging Agentic AI for Science Automation](https://arxiv.org/abs/2604.21910)
*Bartosz Balis,Michal Orzechowski,Piotr Kica,Michal Dygas,Michal Kuszewski*

Main category: cs.AI

TL;DR: 提出三层智能体架构，通过LLM解释自然语言意图、确定性生成工作流DAG、领域专家编写Skills文档，将LLM非确定性限制在意图提取阶段。在千人基因组数据集上，Skills使意图匹配准确率从44%提升至83%，数据传输减少92%，端到端延迟低于15秒，成本低于$0.001/查询。


<details>
  <summary>Details</summary>
Motivation: 科学工作流系统解决了调度、容错、资源管理等自动化执行问题，但未能自动完成执行前所需的语义转换——科学家仍需手动将研究问题转化为工作流规范，这既需要领域知识又需要基础设施专业知识。

Method: 提出三层智能体架构：(1) 语义层——LLM将自然语言解释为结构化意图；(2) 确定性层——经过验证的生成器生成可重现的工作流DAG；(3) 知识层——领域专家编写Skills（Markdown文档），编码词汇映射、参数约束和优化策略。该分解将LLM的非确定性限制在意图提取阶段，相同意图始终生成相同工作流。

Result: 在1000 Genomes群体遗传学工作流和Kubernetes上的Hyperflow WMS中评估。150条查询的消融实验：Skills将完整匹配意图准确率从44%提升至83%；Skill驱动的延迟工作流生成减少92%数据传输；端到端流水线在Kubernetes上完成查询，LLM开销低于15秒，成本低于$0.001/查询。

Conclusion: 通过将LLM的创造性限制在意图解释阶段，结合领域专家知识（Skills）和确定性生成器，可有效弥合自然语言到可执行科学工作流的鸿沟，兼顾灵活性与可靠性。
...
### [340] [Novelty-Based Generation of Continuous Landscapes with Diverse Local Optima Networks](https://arxiv.org/abs/2604.21468)
*Kippei Mizuta,Shoichiro Tanaka,Shuhei Tanaka,Toshiharu Hatanaka*

Main category: cs.NE

TL;DR: 本文针对连续优化中局部最优网络（LON）构建计算成本高的问题，提出了一种基于Max-Set of Gaussians（MSG）景观的替代性Basins of Attraction（BoA）定义，避免了基于搜索的BoA识别，并结合Novelty Search（NS）探索景观参数空间以生成多样拓扑结构的实例。实验表明该方法准确高效，并能从LON特征预测进化算法性能。


<details>
  <summary>Details</summary>
Motivation: 连续优化中局部最优网络（LON）的构建需要反复执行搜索算法来寻找局部最优并近似Basins of Attraction之间的转移，计算成本极高，阻碍了LON特征与进化算法性能之间关系的系统性研究。

Method: 1) 针对Max-Set of Gaussians（MSG）景观提出替代性BoA定义，绕过基于搜索的识别过程，实现低成本的LON构建；2) 利用Novelty Search（NS）探索MSG景观生成器的参数空间，生成具有多样图形拓扑结构的实例。

Result: 所提出的BoA与基于梯度的BoA高度吻合；NS成功生成了搜索难度和最优连通模式各异的实例；基于LON特征可以预测两种进化算法的成功率。

Conclusion: 虽然LON构建方法特定于MSG景观，但提出的框架为景观感知优化提供了数据集基础，为理解LON与算法性能之间的关系铺平了道路。
...
### [341] [On the Role of Preprocessing and Memristor Dynamics in Reservoir Computing for Image Classification](https://arxiv.org/abs/2604.21602)
*Rishona Daniels,Duna Wattad,Ronny Ronen,David Saad,Shahar Kvatinsky*

Main category: cs.NE

TL;DR: 本文研究了基于易失性忆阻器的并行延迟反馈网络（PDFN）储层计算架构，分析了器件特性对性能的影响，并在MNIST上达到95.89%的分类准确率，在20%器件变异下仍保持94.2%的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管已有多种忆阻器件用于储层计算，但缺乏对器件层面要求的系统性评估。

Method: 采用并行延迟反馈网络（PDFN）架构，结合易失性忆阻器，并分析衰减率、量化、变异性等器件特性对储层性能的影响，同时引入预处理方法改进数据表示。

Result: MNIST分类准确率95.89%，在20%器件变异性下仍达94.2%。

Conclusion: 易失性忆阻器能够支持可靠的时空信息处理，是构建紧凑、高速、节能神经形态计算系统的关键组件。
...
### [342] [Neuromorphic Computing Based on Parametrically-Driven Oscillators and Frequency Combs](https://arxiv.org/abs/2604.21861)
*Mahadev Sunil Kumar,Adarsh Ganesan*

Main category: cs.NE

TL;DR: 本文研究了一个具有2:1参量共振的双模系统作为储层计算平台，发现参量共振态可实现最优计算性能，而频率梳态性能不佳，并建立了计算能力与分岔结构之间的对应关系。


<details>
  <summary>Details</summary>
Motivation: 探索参量驱动振荡器作为神经形态计算平台的可能性，并寻找最佳计算性能对应的动力学状态。

Method: 构建2:1参量共振的双模系统，将输入信号编码为驱动幅度，在亚阈值、参量共振和频率梳等不同动力学状态下进行储层计算，对Mackey-Glass、Rossler和Lorenz混沌系统进行一步预测。

Result: 参量共振态实现了最优计算性能；频率梳态性能不稳定且在混沌梳态下因失去相位相干性而退化；低误差区域与参量共振边界对齐。

Conclusion: 参量共振是振荡器储层计算的鲁棒工作状态，输入调制、失谐、阻尼比和数据速率可系统调控动力学状态与计算性能。
